...
import { setActiveWorkspace, setIsSidebarOpen } from "@/store/systemSlice";
...
export function PersistentLayout({ children }: { children: React.ReactNode }) {
  ...
  const currentMode = pathname?.startsWith("/chat")
    ? "chat"
  ...
  const activeTabId = activeTabIds[currentMode];
  const activeTab = tabs.find((t) => t.id === activeTabId);
  const workspaceName = activeTab?.title || "local-workspace";

  const [isSidebarOpenLocal, setIsSidebarOpenLocal] = useState(true);
  ...
  useEffect(() => {
    const handleSidebarState = (e: Event) => {
      ...
      dispatch(setIsSidebarOpen(customEvent.detail));
    };
          ...
          <div className="flex w-full h-full overflow-hidden relative">
              ...
              <div className="z-[110] relative h-full max-h-full overflow-hidden shrink-0">
                <React.Suspense
                  fallback={
                    <div
                      className={cn(
                        "h-full bg-default-100 dark:bg-sidebar transition-all duration-300",
            ...
            {isGhostEditorOpen ? (
              ...
              >
                ...
                <Panel
                  className="h-full bg-[#DFDFDF] dark:bg-[#161616] border-l border-default-100 flex flex-col"
                  defaultSize={60}
                  minSize={30}
                >
              ...
              <div className="flex-1 h-full flex flex-col overflow-hidden bg-[#DFDFDF] dark:bg-[#161616] z-10 relative">
                {children}
              </div>
            )}
          </div>
