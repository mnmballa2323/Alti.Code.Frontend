...
import CoworkSubToggleBar, { CoworkSubTab } from "./cowork/CoworkSubToggleBar";
import ChatSubToggleBar, { ChatSubTab } from "./chat/ChatSubToggleBar";
import { MOCK_COMPLETED_TASKS } from "./cowork/CompletedInboxView";
...
import { startNewChat, setChatContext } from "@/store/messagesSlice";
...
const codeProjects = [
  {
    id: "p1",
    name: "Inso Tech / Premier Website Code",
    runs: [
      "Running Web App",
      "Launch Landing Page",
      "do you know how to build a landing page?",
      "load this page",
      "run this project",
      "Push From Local",
    ],
  ...
  {
    id: "p3",
    name: "Stoneroc Project",
    runs: ["Evaluating Stack", "Build Verification"],
  },
];

const createCategoryHistory: Record<
  "image" | "audio" | "video",
...
> = {
  image: {
    ...
    items: [
      ...
      "App Design Mockups",
      "Product Shot Studio Render",
      "Liquid Glass UI Icons",
  ...
  audio: {
    ...
    items: [
      ...
      "AI Vocal Synthesis Sample",
      "Spatial Audio FX Loop",
    ],
  },
  video: {
    ...
    items: [
      ...
      "Product Demo 3D Animation",
      "Motion Graphic Reel",
      "4K Scene Render Loop",
      "UI Interaction Screen Recording",
    ],
  },
};
...
const coworkSwarms = [
  ...
  },
  {
    ...
    tasks: ["Sprint Execution Loop", "Continuous Integration Monitor"],
  },
];
...
export default function Sidebar() {
  ...
  const [coworkSubTab, setCoworkSubTab] = useState<CoworkSubTab>("tasks");
  const [chatSubTab, setChatSubTab] = useState<ChatSubTab>("chat");
  const [selectedInboxTaskId, setSelectedInboxTaskId] =
    useState<string>("task-098");
  ...
  // Evaluate current route mode cleanly
  const currentMode = (() => {
  ...
  useEffect(() => {
    if (selectedModeOverride && currentMode === selectedModeOverride) {
      setSelectedModeOverride(null);
    }
  }, [pathname, currentMode, selectedModeOverride]);

  const activeMode = selectedModeOverride || currentMode;

  const handleModeClick = (mode: "chat" | "code" | "create" | "cowork") => {
    ...
    dispatch(
      setChatContext({
        sessionId: null,
        model: mode,
        domain: mode.charAt(0).toUpperCase() + mode.slice(1),
      }),
    );
    dispatch(setActiveTab({ id: null, mode }));
    if (mode === "create" || mode === "chat" || mode === "cowork") {
      dispatch(setActiveWorkspace(""));
    }
    window.dispatchEvent(new CustomEvent("inso-mode-change", { detail: mode }));
  };
  ...
  useEffect(() => {
    if (router) {
      ...
      router.prefetch("/cowork");
    }
  }, [router]);
  const { tabs, activeTabIds } = useSelector((state: RootState) => state.tabs);
  const selectedRepo =
  ...
  useEffect(() => {
    ...
    const handleToggleSidebar = (e: Event) => {
      ...
      dispatch(setIsSidebarOpen(targetState));
  ...
  const content = (
    <div
      ...
      data-theme="dark"
    >
      <div className="flex h-full max-h-full overflow-hidden z-20 bg-[#121214] text-foreground">
        {/* Primary Column (Left Side Menu) */}
        <div
          className={cn(
            "flex h-full max-h-full overflow-hidden flex-col transition-all duration-300 bg-[#121214] border-r border-zinc-800 relative",
        ...
        >
          ...
          {isSidebarOpen ? (
            <div className="w-full h-[52px] flex-none flex items-center justify-between px-2.5 gap-2 border-b border-zinc-800 select-none relative z-50">
              ...
              >
                <PanelLeftClose className="size-4 text-default-400 hover:text-white transition-colors" />
              </button>
            </div>
          ) : (
            <div className="w-full h-[52px] flex-none flex items-center justify-center border-b border-zinc-800 select-none">
              <button
                ...
                onClick={toggleLeftSidebar}
              >
                <PanelLeftOpen className="size-4 text-default-400 hover:text-white transition-colors" />
              </button>
            </div>
          )}

          {/* Chat Sub-Toggle Bar when in Chat mode */}
          {isSidebarOpen && activeMode === "chat" && (
            <div className="flex items-center justify-between border-b border-white/5 w-full flex-none py-1.5 px-2.5">
              <ChatSubToggleBar
                ...
                onTabChange={(tab) => {
                  setChatSubTab(tab);
                  window.dispatchEvent(
                    new CustomEvent("inso-chat-subtab-change", { detail: tab }),
                  );
                }}
              />
            </div>
          ...
          {/* Create Category Sub-Toggle when in Create mode */}
          {isSidebarOpen && activeMode === "create" && (
            <div className="flex items-center justify-between border-b border-white/5 w-full flex-none py-1.5 px-2.5">
              <div className="flex items-center justify-between bg-[#08080a] border border-[#1c1c22] shadow-[inset_0_0_8px_rgba(0,0,255,0.15)] rounded-xl p-1 w-full gap-1">
                <button
                  ...
                  onClick={() => {
                    ...
                    window.dispatchEvent(
                      new CustomEvent("inso-create-category-change", {
                        detail: "image",
                      }),
                    );
                  }}
            ...
            <div className="flex items-center justify-between border-b border-white/5 w-full flex-none py-1.5 px-2.5">
              <CoworkSubToggleBar
                ...
                onTabChange={(tab) => {
                  setCoworkSubTab(tab);
                  window.dispatchEvent(
                    new CustomEvent("inso-cowork-subtab-change", {
                      detail: tab,
                    }),
                  );
                }}
        ...
        */}
          ...
          >
            <div
              className={cn(
                "bg-transparent border border-[#0000ff] shadow-[0_0_12px_rgba(0,0,255,0.8)] rounded-xl p-1 flex flex-row items-center justify-between gap-1 w-full h-[40px] flex-none transition-all duration-200",
              )}
            >
              {/* Search Input */}
            ...
            >
              {pathname === "/engines" ? (
                ...
                <div className="flex flex-col gap-0.5 px-2 mt-2 pb-6 w-full">
                  <div className="mb-3">
                    {teamMembers
                      .filter((m) => m.category === "Language")
                      .sort((a, b) => a.name.localeCompare(b.name))
                      .map((member) => (
                        <button
                          key={member.id}
                          className="w-full text-left px-3 py-2 rounded-xl text-[13px] flex items-center gap-3 transition-colors mb-0.5 text-gray-600 dark:text-gray-300 hover:bg-indigo-500/10 hover:text-indigo-600 dark:hover:text-indigo-400"
                          onClick={() =>
                            router.push(
                ...
                <div className="flex flex-col gap-2 px-3 mt-3 w-full animate-pulse">
                ...
                <div className="flex flex-col gap-1 px-2 mt-2 w-full">
                  {(() => {
                    const q = leftSidebarSearch.toLowerCase();
                    ...
                    if (activeMode === "chat") {
                      const allChats = [
                        ...
                        ...sortedChats.map((c) => ({
                          ...
                          topic: c.responses[0]?.prompt || "Untitled Chat",
                      ...
                      return (
                        <>
                          <div className="px-2 py-1 text-[11px] font-bold text-zinc-500 uppercase tracking-wider">
                            Conversations
                          </div>
                          {filtered.map((item) => (
                            <button
                              key={item.id}
                              className={cn(
                                "w-full text-left px-3 py-2 rounded-xl text-[13px] transition-all truncate cursor-pointer",
                                pathname === `/chat/${item.sessionId}`
                                  ? "bg-[#0000ff]/20 text-white border border-[#0000ff] shadow-[0_0_15px_rgba(0,0,255,0.6)] font-medium"
                    ...
                    // 2. CODE MODE
                    if (activeMode === "code") {
                      const activeTabId = activeTabIds?.code;
                      const activeTab = (tabs || []).find(
                        (t) => t?.id === activeTabId,
                      );
                      const currentProjName = activeTab?.title || "Inso Tech";
                      ...
                      const matchingProj =
                        codeProjects.find(
                          (p) =>
                            p.name
                              .toLowerCase()
                              .includes(currentProjName.toLowerCase()) ||
                            currentProjName
                              .toLowerCase()
                              .includes(p.name.toLowerCase()) ||
                            p.id === activeTab?.projectPath,
                        ) || codeProjects[0];

                      const filteredRuns = (matchingProj?.runs || []).filter(
                        ...
                        <>
                          <div className="px-2 py-1 text-[11px] font-bold text-zinc-500 uppercase tracking-wider flex items-center justify-between">
                            <span className="truncate">
                              {matchingProj?.name || "Inso Tech"}
                            </span>
                          </div>
                          {filteredRuns.map((run, rIdx) => (
                            <button
                              key={rIdx}
                              className="w-full text-left px-3 py-2 rounded-xl text-[13px] text-gray-400 hover:text-white hover:bg-white/5 transition-all truncate cursor-pointer"
                              type="button"
                            >
                    ...
                    if (activeMode === "create") {
                      const categoryData =
                        createCategoryHistory[createCategory] ||
                        createCategoryHistory.image;
                    ...
                    if (activeMode === "cowork") {
                      if (coworkSubTab === "inbox") {
                        const filteredInboxTasks = MOCK_COMPLETED_TASKS.filter(
                          (t) =>
                        ...
                        return (
                          <>
                            <div className="px-2 py-1 text-[11px] font-bold text-zinc-500 uppercase tracking-wider flex items-center justify-between">
                              <span className="truncate">Completed Inbox</span>
                              <span className="text-[10px] text-zinc-600 font-mono">
                                {filteredInboxTasks.length} items
                              </span>
                            </div>
                            {filteredInboxTasks.map((task) => {
                              const isSelected =
                                selectedInboxTaskId === task.id;

                              return (
                                <button
                                  key={task.id}
                                  className={cn(
                                    "w-full text-left px-3 py-2.5 rounded-xl transition-all flex flex-col gap-0.5 cursor-pointer border relative group",
                                    isSelected
                                      ? "bg-[#0000ff]/20 text-white border-[#0000ff]/60 shadow-[0_0_10px_rgba(0,0,255,0.3)]"
                                      : "bg-transparent text-gray-400 border-transparent hover:text-white hover:bg-white/5",
                                  )}
                                  type="button"
                                  onClick={() => {
                                    setSelectedInboxTaskId(task.id);
                                    window.dispatchEvent(
                                      new CustomEvent(
                                ...
                                >
                                  <div className="flex items-center justify-between gap-1.5 w-full">
                                    <span className="flex items-center gap-1.5 text-[12px] font-semibold truncate text-white">
                                      {task.status === "success" ? (
                                  ...
                                  <div className="flex items-center justify-between text-[10px] text-zinc-500 font-mono pl-5">
                                    <span className="truncate">
                                      {task.agent}
                                    </span>
                      ...
                      const activeTabId = activeTabIds?.cowork;
                      const activeTab = (tabs || []).find(
                        (t) => t?.id === activeTabId,
                      );
                      const currentProjName =
                        activeTab?.title || "Autonomous Engineering Swarm";
                      ...
                      const filteredTasks = (matchingSwarm?.tasks || []).filter(
          ...
          >
            {status === "unauthenticated" ? (
              <div className={cn("flex gap-2", !isSidebarOpen && "hidden")}>
                <Button
                  className="flex-1 h-9 bg-white text-zinc-950 hover:bg-zinc-200 text-[13px] font-semibold rounded-lg transition-all shadow-sm"
                  radius="lg"
                  onClick={() => {
                    if (
                      typeof window !== "undefined" &&
  ...
  if (!mounted) {
    return (
      <div className="flex h-screen max-h-screen overflow-hidden border-r border-white/5 bg-[#121214] w-[64px] transition-all" />
    );
  }
