...
- **Alerting policy**: `alti-code-studio-gke-node-drain` fires on node NotReady or pod eviction events
- **Dashboard**: Check **Alti Code Studio — Kubernetes Cluster** dashboard, panel "Node Status"
...
1. **Check node status**:
   ```bash
   kubectl get nodes -o wide --context=gke_alti-code-studio_us-central1_alti-byoc-cluster
   ```
2. **Check for evicted pods in the alti-system namespace**:
   ```bash
   kubectl get pods -n alti-system --field-selector=status.phase=Failed \
     --context=gke_alti-code-studio_us-central1_alti-byoc-cluster
...
1. **If node upgrade is draining pods** — verify it's expected and monitor:
   ...
   gcloud container operations list \
     ...
     --region=us-central1 \
     --project=alti-code-studio --filter="status=RUNNING"

   # Check node pool upgrade status
   gcloud container node-pools describe alti-system-pool \
     --cluster=alti-byoc-cluster \
     --region=us-central1 \
     --project=alti-code-studio --format="value(management.autoUpgrade, version)"
   ```
...
- All nodes are `Ready`:
  ```bash
  kubectl get nodes --context=gke_alti-code-studio_us-central1_alti-byoc-cluster
  ```
...
- Resource utilization is within bounds:
  ```bash
  kubectl top nodes --context=gke_alti-code-studio_us-central1_alti-byoc-cluster
  kubectl top pods -n alti-system --context=gke_alti-code-studio_us-central1_alti-byoc-cluster
  ```
...
## Escalation
- **If multiple nodes are NotReady simultaneously**: Page on-call SRE — `alti-code-studio-p1`
