...
test.describe("Authentication and RBAC", () => {
  ...
  test("standard users cannot access admin routes", async ({
    context,
    page,
  }) => {
    ...
    await page.route("**/api/auth/session", (route) => {
      route.fulfill({
        ...
        body: JSON.stringify({
          user: { role: "user", mfaRequired: false },
  ...
  test("admin users can access admin routes", async ({ context, page }) => {
    ...
    await page.route("**/api/auth/session", (route) => {
      route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          user: { role: "admin", mfaRequired: false },
          expires: "2099-01-01T00:00:00.000Z",
        }),
      });
    });
  ...
  test("users with mfaRequired are redirected to MFA challenge", async ({
    context,
    page,
  }) => {
    ...
    await page.route("**/api/auth/session", (route) => {
      route.fulfill({
        ...
        contentType: "application/json",
        body: JSON.stringify({
          user: { role: "user", mfaRequired: true },
          expires: "2099-01-01T00:00:00.000Z",
        }),
      });
    });
