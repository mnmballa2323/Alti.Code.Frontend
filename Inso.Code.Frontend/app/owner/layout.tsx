...
const accountItems: SidebarItem[] = [
  { label: "Add New", href: "/owner/add-new", icon: Plus },
  { label: "API Keys", href: "/owner/api-keys", icon: Key },
  { label: "Email Accounts", href: "/owner/email-accounts", icon: Mail },
  { label: "Account Logins", href: "/owner/platform-logins", icon: Shield },
];
...
export default function OwnerLayout({
  ...
  const showSearch =
    pathname === "/owner/transactions" || pathname === "/owner/customers";
  ...
  const getPageTitle = () => {
    ...
    if (pathname.startsWith("/owner/add-new")) return "Add New";
    if (pathname.startsWith("/owner/platform-logins")) return "Account Logins";
    ...
    <div className="flex flex-col h-screen w-screen bg-[#F3F4F6] dark:bg-[#0d1117] overflow-hidden text-neutral-900 dark:text-neutral-100">
      ...
      <div className="h-14 w-full bg-white dark:bg-[#161b22] border-b border-neutral-100 dark:border-neutral-800 flex items-center shrink-0 z-20">
        ...
        <div className="flex-1 h-full flex items-center justify-between">
          <div className="flex items-center h-full">
            <div
              className={`h-full flex items-center gap-3 ${pathname.startsWith("/owner/support") ? "w-72 border-r border-neutral-200 dark:border-neutral-800 shrink-0 px-6" : "pl-10"}`}
            >
              {isMemberDetail ? (
                <Link
      ...
      <div className="flex-1 flex w-full overflow-hidden">
        ...
        <div className="flex-1 flex flex-col h-full bg-[#F3F4F6] dark:bg-[#0d1117] relative overflow-hidden">
          {/* Content Children */}
          <div
            className={`flex-1 overflow-y-auto relative flex flex-col h-full ${pathname.startsWith("/owner/support") ? "" : "pt-4 px-10"}`}
          >
            {children}
