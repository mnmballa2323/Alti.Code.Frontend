...
router.post('/webhook/github', verifyGitHubSignature, async (req, res) => {
  ...
  res
    .status(202)
    .send({ status: 'Accepted', message: 'Tri-Brain Swarm Activated.' });
  ...
  try {
    if (eventType === 'push') {
      ...
      logger.info(
        `🚀 [Webhook Interceptor] Push detected on ${repoUrl}. Commits: ${commits.length}`,
      );
      ...
      for (const commit of commits) {
        ...
        logger.info(
          `✅ [Webhook Interceptor] Tri-Brain resolved commit ${commit.id}: ${consensusResult.status}`,
        );
      }
    ...
    logger.error(
      `❌ [Webhook Interceptor] Autonomous execution failed: ${err.message}`,
    );
  }
});
