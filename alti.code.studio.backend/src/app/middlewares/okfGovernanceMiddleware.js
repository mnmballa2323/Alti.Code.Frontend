...
export const okfGovernanceMiddleware = async (req, res, next) => {
    try {
        ...
        const routeName = pathParts.find(p => p && p !== 'api' && p !== 'v1') || 'root';
        ...
        logger.info(`🛡️ [OKF Governance] Request to API route [${routeName}] intercepted. Enforcing catalog guidelines.`);

        const specificationText = concept.body || '';

        // 1. Zero-Trust Identity-Aware Proxy (IAP) check
        const requiresIap = /Authentication.*Zero-Trust IAP/i.test(specificationText);
        console.log('=== MIDDLEWARE REQUIRES IAP ===', requiresIap);
        if (requiresIap) {
            const hasIapHeader = req.headers['x-goog-iap-jwt-assertion'] || req.headers['authorization'];
            if (!hasIapHeader) {
                logger.warn(`❌ [OKF Governance] Security Violation on [${routeName}]: Missing Zero-Trust Identity assertion.`);
                ...
                return res.status(httpStatus.UNAUTHORIZED).json({
                    success: false,
                    error: 'Access Denied: Missing Zero-Trust Identity assertion (IAP token required).'
                });
            }
        }
        ...
        if (isFedrampEnforced) {
            ...
            const isLocalOrPrivate = clientIp.includes('127.0.0.1') || clientIp.includes('::1') || clientIp.includes('10.') || clientIp.includes('192.168.');
            
            if (process.env.NODE_ENV === 'production' && !isLocalOrPrivate) {
                logger.warn(`❌ [OKF Governance] Compliance Violation on [${routeName}]: Request from outside FedRAMP boundary.`);
                ...
                return res.status(httpStatus.FORBIDDEN).json({
                    success: false,
                    error: 'Access Denied: Request originates outside the compliant FedRAMP boundary.'
                });
            }
        }
        ...
        logger.error(`❌ [OKF Governance] Middleware encountered error: ${err.message}`);
        next(err);
    }
};
