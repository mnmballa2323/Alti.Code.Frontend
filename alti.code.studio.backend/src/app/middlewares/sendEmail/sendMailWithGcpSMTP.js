...
/**
 * Sends an email using GCP SMTP / Nodemailer
 ...
 */
export const sendMailWithGcpSMTP = async mailData => {
  ...
  try {
    ...
    logger.info(
      `[SMTP] Transactional email sent successfully to ${userEmail} (MessageId: ${info.messageId})`,
    );
    return info;
  } catch (error) {
    logger.error(
      `[SMTP] Failed to send transactional email to ${userEmail}: ${error.message}`,
    );
    throw error;
  }
};
