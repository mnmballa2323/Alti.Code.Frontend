  ...
  return async (req, res, next) => {
    try {
      ...
      if (user.clearanceLevel < requiredClearanceLevel) {
        logger.warn(
          `[ABAC] Clearance check failed for ${user.email}. Has: ${user.clearanceLevel}, Required: ${requiredClearanceLevel}`,
        );
        ...
        throw new ApiError(
          ...
          `Access Denied: Insufficient Clearance Level. Requires Level ${requiredClearanceLevel}.`,
        );
      }
      ...
      if (user.allowedIps && user.allowedIps.length > 0) {
        ...
        if (!isAllowed) {
          logger.warn(
            `[ABAC] IP Whitelist violation for ${user.email}. Attempted from ${clientIp}`,
          );
          ...
          throw new ApiError(
            httpStatus.FORBIDDEN,
            `Access Denied: Your IP address (${clientIp}) is not whitelisted for this account.`,
          );
        }
      }
