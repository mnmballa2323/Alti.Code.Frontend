...
export const dlpMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        if (hasPromptInjection(req.body)) {
            logger.warn(`🔒 [WAF] Blocked request containing prompt injection patterns.`);
            ...
            try {
                ...
                const { siemService } = await import('../modules/security/siem.service.js');
                await siemService.dispatchEvent(tenantId, 'WAF_PAYLOAD_INJECTION', {
                    endpoint: req.originalUrl,
                    userId: req.user?.id || 'anonymous'
                });
            } catch (siemErr) {
                logger.warn('Failed to dispatch SIEM event for WAF violation:', siemErr.message);
            }
...
// GCP SCC removed
        ...
        if (findings && findings.length > 0) {
            ...
            try {
                ...
                logger.warn('Failed to dispatch SIEM event for DLP violation:', siemErr.message);
            }
