...
class AutoProfilerService {
    ...
    _recordMetric(route, durationMs) {
        let metrics = this.routeAverages.get(route) || { count: 0, totalMs: 0, average: 0 };
    ...
    async _triggerAutonomousRefactor(route, latency) {
        logger.warn(`⚠️ [AutoProfiler] CRITICAL BOTTLENECK DETECTED: ${route} is averaging ${latency.toFixed(2)}ms.`);
        logger.warn(`🤖 [AutoProfiler] Dispatching Autonomous Performance Refactor to the Swarm...`);
        ...
        try {
            ...
            logger.error(`❌ [AutoProfiler] Failed to dispatch Swarm: ${err.message}`);
        }
    }
}
