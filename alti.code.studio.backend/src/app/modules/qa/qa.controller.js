...
qaRoutes.post('/spec', async (req, res) => {
  ...
  if (!sourceFile || typeof sourceFile !== 'string') {
    return res.status(400).json({
      success: false,
      message: 'sourceFile is required (absolute path).',
    });
  }
  try {
...
qaRoutes.post('/suite', async (req, res) => {
  ...
  if (!moduleDir || typeof moduleDir !== 'string') {
    return res.status(400).json({
      success: false,
      message: 'moduleDir is required (absolute path).',
    });
  }
  try {
    logger.info(`🧪 QA Suite API: scanning ${moduleDir}`);
    const result = await qaService.generateSuite(moduleDir, {
      overwrite,
      exclude,
    });
...
qaRoutes.post('/api-tests', async (req, res) => {
  ...
  if (!Array.isArray(routes) || routes.length === 0) {
    return res.status(400).json({
      success: false,
      message: 'routes[] is required and must be non-empty.',
    });
  }
  try {
...
qaRoutes.post('/sprint', async (req, res) => {
  ...
  try {
    ...
    const { autonomousSprintService } =
      await import('../autonomousSprint/autonomousSprint.service.js');

    logger.info(
      `🧪 QA Sprint API: Launching self-healing sprint for: "${prompt}"`,
    );
    ...
    const sprintId = await autonomousSprintService.launchSprint(
      prompt || 'Run tests and autonomously heal any failures.',
    );
