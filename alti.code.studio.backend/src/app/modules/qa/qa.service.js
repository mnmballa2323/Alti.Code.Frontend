...
`.trim();
  ...
  async generateSuite(moduleDir, opts = {}) {
    ...
    try {
      ...
      logger.error(
        `🧪 QA Suite: Cannot read directory ${moduleDir}:`,
        e.message,
      );
      return results;
    }
    ...
    logger.info(
      `🧪 QA Suite: Found ${sourceFiles.length} source files to cover`,
    );
    ...
    logger.info(
      `🧪 QA Suite: ✅ ${results.generated.length} generated, ⏭️ ${results.skipped.length} skipped, ❌ ${results.failed.length} failed`,
    );
    return results;
  }
  ...
  async generateApiTests(
    ...
    logger.info(
      `🧪 QA API: Generating integration tests for ${routes.length} routes`,
    );
...
`.trim();
      ...
      proc.on('close', exitCode => {
        ...
        try {
          ...
          if (jsonMatch) {
            ...
            const duration = report.startTime
              ? Date.now() - report.startTime
              : null;
            logger.info(
              `🧪 QA Run: ${passed}/${total} passed, ${failed} failed. Exit: ${exitCode}`,
            );
            return resolve({
              passed,
              failed,
        ...
        logger.info(
          `🧪 QA Run (parsed): ${passed} passed, ${failed} failed. Exit: ${exitCode}`,
        );
  ...
  async coverageReport(
    ...
    logger.info(
      `🧪 QA Coverage: Reading ${summaryPath} (threshold: ${threshold}%)`,
    );
    ...
    try {
      ...
      logger.info(
        `🧪 Coverage: ${passed ? '✅ PASS' : '❌ FAIL'} — Lines: ${metrics.lines}%, Branches: ${metrics.branches}%, Fns: ${metrics.functions}%, Stmts: ${metrics.statements}%`,
      );

      return { passed, threshold, ...metrics, failures };
    } catch (error) {
      logger.error(
        '🧪 Coverage: Failed to read coverage-summary.json',
        error.message,
      );
