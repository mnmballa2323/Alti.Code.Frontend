...
export class SymbolicExecutionService {
  ...
  async generateBoundsProof(sourceCode, targetFunction) {
    logger.info(
      `🔢 Symbolic Execution: Generating Z3 bounds proof for [${targetFunction}]`,
    );
    ...
    try {
      ...
      if (matches.length === 0) {
        logger.warn(
          `Symbolic Execution: Function ${targetFunction} not found for proof generation.`,
        );
        return null;
      }
...
(check-sat)
      ...
      logger.info(
        `✨ Symbolic Execution: Generated Formal Constraints:\n${smtLibProof}`,
      );
