...
export const qaWorkerProcessor = async job => {
  ...
  logger.info(
    `🧪 QAWorker [${jobId}]: Generating tests for ${path.basename(filePath)}...`,
  );
  ...
  try {
    ...
    try {
      ...
      await vectorStoreService.add(
        `Test Suite for ${path.basename(filePath)}`,
        { type: 'qa-test', passed: true },
      );
    } catch (testError) {
      logger.warn(
        `⚠️ QAWorker [${jobId}]: Tests Failed! Initiating Self-Repair...`,
      );
      ...
      const repairResponse =
        await GeminiAiService.generateContent(repairPrompt);
      testCode = repairResponse
        .replace(/^```javascript\n/, '')
        .replace(/^```\n/, '')
        .replace(/\n```$/, '');
