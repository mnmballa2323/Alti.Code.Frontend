...
Example: ["original query", "specific technical term query", "architectural pattern query"]`;
            ...
            if (Array.isArray(expanded) && expanded.length > 0) {
                ...
                logger.info(`[Ultimate RAG] Query expanded: "${query}" → ${queries.length} sub-queries`);
                return queries;
            }
        } catch (e) {
            logger.warn(`[Ultimate RAG] Query expansion failed (non-blocking): ${e.message}`);
        }
    ...
    async executeRetrieval(query, expandedQueries) {
        ...
        logger.info(`🌐 [Ultimate RAG] Initiating retrieval for ${allQueries.length} query variant(s)`);
        ...
        const timedSource = async (id, label, fn) => {
            ...
            try {
                ...
                return { id, label, status: 'done', durationMs: Date.now() - start, result };
            } catch (e) {
                logger.warn(`[Ultimate RAG] ${label} failed: ${e.message}`);
                return { id, label, status: 'error', durationMs: Date.now() - start, result: null, error: e.message };
            }
        };
        ...
        const sources = await Promise.all([
            ...
            timedSource('vertex', 'Vertex AI Discovery Engine', async () => {
                const allResults = await Promise.all(
                    allQueries.map(q => discoveryEngineService.searchCodebase(q).catch(() => []))
                );
            ...
            timedSource('cli', 'Gemini CLI Expert', () =>
                GeminiCliService.runGeminiCLI('ask', [`"Provide world-class software engineering best practices for: ${primaryQuery}"`])
            ),
        ...
        if (vertexResult && Array.isArray(vertexResult) && vertexResult.length > 0) {
            ...
            vertexResult.slice(0, 8).forEach((res, index) => {
                ...
                const fileSource = res.document?.id || res.document?.name || `Source_${index + 1}`;
                if (snippet) {
                    combinedContext += `\n--- SOURCE: ${fileSource} ---\n${snippet}\n`;
                }
            });
        ...
        if (spannerResult && Array.isArray(spannerResult) && spannerResult.length > 0) {
        ...
        logger.info(`✅ [Ultimate RAG] Pipeline complete: ${activeSources}/${sources.length} sources in ${totalDurationMs}ms`);
    ...
    async _executeFileSearchRetrieval(query) {
        ...
        if (stores.length > MAX_STORES_PER_QUERY) {
            logger.warn(`[Ultimate RAG] ${stores.length} stores found, capped to ${MAX_STORES_PER_QUERY}.`);
        }

        logger.info(`[Ultimate RAG] Querying ${storeNames.length} File Search store(s)...`);

        const queryPromise = fileSearchService.query(query, storeNames);
        const timeoutPromise = new Promise((_, reject) =>
            setTimeout(() => reject(new Error('File Search query timed out')), FILE_SEARCH_TIMEOUT_MS)
        );
    ...
    async synthesize(query, mode, domain, language) {
        ...
        try {
            ...
            if (cached) {
                logger.info(`⚡ [Ultimate RAG] CACHE HIT — returning synthesis in <1ms`);
                return {
                    ...cached,
                    fromCache: true,
            ...
            logger.warn(`[Ultimate RAG] Cache read failed (non-blocking): ${e.message}`);
        }
        ...
        // ── Core Retrieval ───────────────────────────────────────────
        const { combinedContext, pipeline, totalDurationMs: retrievalMs } = await this.executeRetrieval(query, expandedQueries);
...
==============================================================\n\n` + systemPrompt;
        ...
        try {
            ...
            logger.info(`⚡ [Ultimate RAG] Result cached in Memorystore (TTL: ${CACHE_TTL_SECONDS}s)`);
        } catch (e) {
            logger.warn(`[Ultimate RAG] Cache write failed (non-blocking): ${e.message}`);
        }
    ...
    async _persistFeedback(query, result) {
        try {
            ...
            const synthesisSnippet = (result.synthesis || '').substring(0, MAX_FEEDBACK_LENGTH);
            ...
            logger.info(`🔄 [Ultimate RAG] Feedback loop: synthesis persisted to vector store for future recall.`);
        } catch (e) {
