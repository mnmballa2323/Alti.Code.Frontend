...
export class AgentEvaluator {
  ...
  static evaluateCode(code, rules = {}) {
    ...
    if (lines.length > maxLines) {
      ...
      score -= deduction;
      deductions.push(
        `Code length exceeded ${maxLines} lines (Actual: ${lines.length} lines). Deducted ${deduction} points.`,
      );
    }
    ...
    forbiddenPatterns.forEach(pattern => {
      if (pattern.test(code)) {
        score -= 30;
        deductions.push(
          `Security Hazard: Detected restricted execution pattern matching: ${pattern.toString()}. Deducted 30 points.`,
        );
      }
    });
    ...
    try {
      ...
      score -= 40;
      deductions.push(
        `Syntax Failure: Code has syntax errors: "${e.message}". Deducted 40 points.`,
      );
    }
    ...
    if (code.includes('require(') && !code.includes('import ')) {
      score -= 10;
      deductions.push(
        'Styling Warning: Code uses CommonJS require instead of modern ES6 import. Deducted 10 points.',
      );
    }
  ...
  static logScorecard(taskId, scorecard, logDirectory = './logs/evaluation') {
    try {
      ...
      console.log(
        `📊 Scorecard [${scorecard.status} - ${scorecard.score}/100] compiled successfully: ${filePath}`,
      );
    } catch (e) {
