      ...
      if (userId) {
        ...
        if (githubConn && githubConn.credentials) {
          const creds = JSON.parse(
            await encryptionService.decrypt(githubConn.credentials),
          );
          const githubToken =
          ...
          if (githubToken) {
            ...
            logger.info(
              `🔑 OpenCode: Dynamically injected user-scoped GITHUB_TOKEN environment variable.`,
            );
          }
        }
      }
...
const runTask = async (taskDescription, workspacePath) => {
  ...
  try {
    ...
    logger.info(
      `🔌 OpenCode: Sending task to serve daemon at ${OPENCODE_API_URL}...`,
    );
    const response = await axios.post(
      `${OPENCODE_API_URL}/api/v1/sessions`,
      {
        task: taskVal,
    ...
    logger.warn(
      `⚠️ OpenCode serve daemon offline: ${error.message}. Spawning local opencode CLI child process...`,
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      exec(
        cmd,
        { cwd: workspaceVal || process.cwd(), env: customEnv },
        (execErr, stdout, stderr) => {
          if (execErr) {
            logger.error(
              `❌ OpenCode CLI Execution failed: ${execErr.message}`,
            );
            return reject(
              new ApiError(
...
const getTaskStatus = async sessionId => {
  ...
  try {
    const response = await axios.get(
      `${OPENCODE_API_URL}/api/v1/sessions/${idVal}`,
    );
    return response.data;
  } catch (error) {
    throw new ApiError(
