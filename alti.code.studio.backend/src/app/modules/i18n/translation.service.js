...
class TranslationService {
  ...
  async translate(text, targetLang) {
    ...
    if (cached !== undefined) {
      logger.debug(
        `🌐 Translation cache HIT for [${targetLang}] "${text.substring(0, 30)}…"`,
      );
      return cached;
    }
    ...
    try {
      logger.info(
        `🌐 Translating to [${targetLang}]: "${text.substring(0, 50)}…"`,
      );
      ...
      logger.warn(
        `❌ Translation failed for [${targetLang}]: ${error.message}. Returning original.`,
      );
      return text;
    }
  }
  ...
  async translateBatch(texts, targetLang) {
    ...
    for (let i = 0; i < texts.length; i += CONCURRENCY) {
      ...
      const translated = await Promise.all(
        batch.map(t => this.translate(t, targetLang)),
      );
