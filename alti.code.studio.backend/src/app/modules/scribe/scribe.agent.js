...
class ScribeAgent {
    ...
    async documentCode(code, filename) {
        ...
        const documentedCode = response.replace(/^```(javascript|js)?|```$/g, '').trim();

        return { originalLength: code.length, documentedLength: documentedCode.length, code: documentedCode };
    }
