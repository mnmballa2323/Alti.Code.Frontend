...
const auditSmartContract = async (userId, sessionId, contractCode) => {
    logger.info(`🔗 [Smart Contract Auditor] Analyzing Web3 contract for vulnerabilities...`);
...
Return a detailed security report and the exact patched code to resolve any vulnerabilities.`;
        ...
        const result = await LlmGatewayService.routeCompletion(
            userId, sessionId, auditPrompt, 'gpt-5.5', 'Smart-Contract-Auditor'
        );
        ...
        logger.error(`❌ [Smart Contract Auditor] Contract audit critically failed:`, error);
        throw error;
    }
};
