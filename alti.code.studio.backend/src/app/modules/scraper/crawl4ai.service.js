...
export class Crawl4AiService {
  ...
  static async crawl(url, options = {}) {
    ...
    try {
      logger.info(
        '🌐 [Scraper Wrapper] Attempting scrape via Crawl4AI microservice...',
      );
      const crawlResult = await Crawl4AiAgentService.crawlAndExtract(
        url,
        'markdown',
      );
      ...
      logger.warn(
        `⚠️ [Scraper Wrapper] Crawl4AI request failed: ${error.message}. Triggering Puppeteer fallback...`,
      );
    }

    // 2. Fallback to PageBrowserAutomation (Puppeteer)
    try {
      const fallbackResult = await PageBrowserAutomation.scrapePage(
        url,
        options,
      );
      if (fallbackResult.success) {
        ...
        throw new Error(
          fallbackResult.error || 'Puppeteer extraction returned unsuccessful.',
        );
      }
    } catch (fallbackError) {
      logger.error(
        `❌ [Scraper Wrapper] Both Crawl4AI and Puppeteer fallback failed: ${fallbackError.message}`,
      );
      return {
        success: false,
        title: 'Error',
