      ...
      if (userId) {
        ...
        if (githubConn && githubConn.credentials) {
          ...
          if (githubToken) {
            ...
            logger.info(
              `🔑 OpenSpec: Dynamically injected user-scoped GITHUB_TOKEN environment variable.`,
            );
          }
        }
      }
...
const executeCommand = async (cmd, workspacePath) => {
  ...
  return new Promise((resolve, reject) => {
    logger.info(
      `📋 OpenSpec CLI: Executing [${cmd}] inside ${workspacePath || process.cwd()}...`,
    );
    exec(
      cmd,
      { cwd: workspacePath || process.cwd(), env: customEnv },
      (err, stdout, stderr) => {
        if (err) {
          ...
          if (
            ...
            logger.warn(
              `⚠️ CLI binary 'openspec' not found. Re-running via npx...`,
            );
            return resolve(
              executeCommand(
                `npx -y @fission-ai/openspec@latest ${cmd.substring(9)}`,
                workspacePath,
              ),
            );
          }
...
const propose = async (name, workspacePath) => {
  ...
  if (!nameVal)
    throw new ApiError(httpStatus.BAD_REQUEST, 'Proposal name is required.');
