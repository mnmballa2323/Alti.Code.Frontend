...
const applyPattern = async (filePath, pattern, context) => {
  if (!filePath || typeof filePath !== 'string')
    throw new Error('SurgeonService: filePath must be a non-empty string.');
  if (!pattern || typeof pattern !== 'string')
    throw new Error('SurgeonService: pattern must be a non-empty string.');
  logger.info(
    `🔮 SurgeonService: Applying pattern "${pattern}" to ${filePath}`,
  );
