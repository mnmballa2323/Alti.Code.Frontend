...
export class AutonomicService {
  constructor() {
     ...
     */
    this.MIN_CHANGE_RATIO = 0.2;
    logger.info(
      '🔬 AutonomicService: Proactive Surgeon initialized. Ready to hunt technical debt.',
    );
  }
  ...
  async commenceProactiveSweep(targetFiles) {
    if (!Array.isArray(targetFiles) || targetFiles.length === 0) {
      throw new Error(
        'AutonomicService: targetFiles must be a non-empty array of file paths.',
      );
    }
    logger.info(
      `🔬 AutonomicService: Commencing deep AST structural sweep of ${targetFiles.length} files...`,
    );
    ...
    for (const filePath of targetFiles) {
      ...
      if (!acquired) {
        logger.warn(
          `🔬 AutonomicService: File ${path.basename(filePath)} is already locked by another AST sweep. Skipping.`,
        );
      ...
      try {
        ...
        if (complexity > this.COMPLEXITY_THRESHOLD) {
          logger.warn(
            `🔬 AutonomicService: [TECH DEBT] ${path.basename(filePath)} scored Cyclomatic Complexity=${complexity}. Initiating Surgical Rewrite.`,
          );
          ...
          const changeRatio = this._computeChangeRatio(
            sourceCode,
            engineeredPatch,
          );
          ...
          if (changeRatio < this.MIN_CHANGE_RATIO) {
            logger.info(
              `🔬 AutonomicService: Patch for ${path.basename(filePath)} rejected — only ${(changeRatio * 100).toFixed(1)}% changed (threshold: ${this.MIN_CHANGE_RATIO * 100}%). Skipping destructive write.`,
            );
            patches.push({
              filePath,
              skipped: true,
          ...
          const patchResult = await this._applyAndStageRepair(
            filePath,
            engineeredPatch,
            complexity,
          );
        ...
        logger.error(
          `❌ AutonomicService: Failed to analyze ${filePath}`,
          error,
        );
      } finally {
    ...
    logger.info(
      `🔬 AutonomicService: Sweep Complete. Eliminated ${debtEliminated} instances of technical debt.`,
    );
  ...
  _generateDiff(original, patched) {
    ...
    return (
      [...removed.slice(0, 30), ...added.slice(0, 30)].join('\n') ||
      '(no textual diff)'
    );
  }
  ...
  async _applyAndStageRepair(filePath, newCode, complexityScore) {
    ...
    try {
      ...
      logger.info(
        `💾 AutonomicService: Committed on branch "${branchName}" — "${commitMsg}"`,
      );
      ...
      logger.error(
        `❌ AutonomicService: Git operation failed for ${filePath}`,
        gitError,
      );
