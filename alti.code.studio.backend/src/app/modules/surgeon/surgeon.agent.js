...
class SurgeonAgent {
    ...
    async applyPattern(filePath, pattern, context = "") {
        ...
        const refactoredCode = response.replace(/^```(javascript|js|ts)?|```$/g, '').trim();
        ...
        await fs.writeFile(path.resolve(this.projectRoot, filePath), refactoredCode);
    ...
    async analyzeComplexity(filePath) {
        ...
        return jsonMatch ? JSON.parse(jsonMatch[0]) : { complexity_score: 0, verdict: "Unknown" };
    }
}
