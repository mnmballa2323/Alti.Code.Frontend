...
const executeResearch = catchAsync(async (req, res) => {
    ...
    if (!objective) {
        return res.status(httpStatus.BAD_REQUEST).json({ success: false, message: 'Objective is required for Deep Research.' });
    }

    // Since this is a deep research that might take up to 30-45s for 'exhaustive', 
    // the client must set a high timeout.
    const report = await researchService.executeDeepResearch(objective, depth || 'deep');
