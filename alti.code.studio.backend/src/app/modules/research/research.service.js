...
class ResearchService {
  ...
  async executeDeepResearch(objective, depth = 'deep') {
    logger.info(
      `🔬 [Deep Research] Commencing ${depth.toUpperCase()} research on: "${objective.substring(0, 50)}..."`,
    );
    ...
    try {
      if (numQueries > 1) {
        ...
        const cleaned = queryResponse.content
          .replace(/^```json?\n?/m, '')
          .replace(/\n?```$/m, '')
          .trim();
      ...
      logger.warn(
        `⚠️ [Deep Research] Search formulation failed, falling back to original objective. ${e.message}`,
      );
    ...
    logger.info(
      `🔬 [Deep Research] Phase 3: Synthesizing ${aggregatedContext.length} chars of gathered intelligence...`,
    );
