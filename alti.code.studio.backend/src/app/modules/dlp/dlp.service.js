...
class DlpService {
  ...
  redact(text) {
    ...
    const genericSecretRegex =
      /(?:password|secret|api_key|apikey|token)["'\s:=]+(["'][a-zA-Z0-9\-_]{8,}["'])/gi;
    ...
    if (redactionCount > 0) {
      logger.info(
        `[${SERVICE_NAME}] Intercepted and redacted ${redactionCount} sensitive patterns.`,
      );
    }
  ...
  checkComplianceTags(text, isAirGapped = false) {
    ...
    const cuiRegex =
      /\b(?:CUI|CONTROLLED UNCLASSIFIED INFORMATION|ITAR|EAR)\b/i;
    if (cuiRegex.test(text) && !isAirGapped) {
      throw new Error(
        'DLP BLOCKED: Controlled Unclassified Information (CUI/ITAR) detected. This payload cannot be sent to public cloud LLMs. Enable AIR_GAPPED_MODE.',
      );
    }
  }
}
