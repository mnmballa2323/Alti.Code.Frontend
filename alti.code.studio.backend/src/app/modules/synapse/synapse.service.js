...
const init = async () => {
  ...
  if (process.env.DISABLE_REDIS === 'true') {
    logger.warn(
      '⚠️ Synapse: Redis disabled via env. Switching to local in-memory Mock Mode.',
    );
    isConnected = true;
    isMock = true;
  ...
  try {
    ...
    await subscriber.subscribe(BROADCAST_CHANNEL, message => {
      try {
        ...
        logger.info(
          `📡 [Synapse] Broadcast received from ${from}: ${type}`,
          payload,
        );
    ...
    logger.warn(
      '⚠️ Synapse: Redis unreachable. Switching to local in-memory Mock Mode.',
    );
    isConnected = true;
    isMock = true;
...
const directMessage = async (fromAgent, toAgentName, payload) => {
  ...
  return {
    status: delivered > 0 ? 'delivered' : 'no_subscribers',
    to: toAgentName,
  };
};
...
const swarm = async (taskDescription, requiredCapabilities = []) => {
  ...
  if (
    ...
    !squad.find(a => a.agentName === planningAgent.agentName)
  ) {
    squad.push(planningAgent);
  }

  logger.info(
    `🐝 Synapse: Swarm Squad Assembled: [${squad.map(a => a.agentName).join(', ')}]`,
  );

  const results = await Promise.all(
    squad.map(async agent => {
      try {
