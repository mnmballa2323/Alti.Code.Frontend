...
class CodeGraphService {
  constructor() {
    ...
    logger.info(
      `🔷 [CodeGraphService] Initialized. Workspace: ${this.workspaceRoot}, Bundled Node: ${this.bundledNodePath}`,
    );
  }
  ...
  findBundledNode() {
    ...
    const platformDir = path.join(
      this.workspaceRoot,
      `alti.code.studio.backend/node_modules/@colbymchenry/codegraph-${target}`,
    );
    ...
    const topPlatformDir = path.join(
      this.workspaceRoot,
      `node_modules/@colbymchenry/codegraph-${target}`,
    );
    ...
    logger.warn(
      `⚠️ [CodeGraphService] Bundled Node 24 not found for target ${target}. Falling back to system node.`,
    );
    return 'node';
  }
  ...
  async getGraphForPath(targetPath, limit = 150) {
    try {
      ...
      logger.info(
        `🔷 [CodeGraphService] Querying graph for relative target: "${relativeTarget}" (limit: ${limit})`,
      );
      ...
      logger.info(
        `✅ [CodeGraphService] Retrieved ${parsedGraph.nodes.length} nodes and ${parsedGraph.links.length} edges successfully.`,
      );

      return parsedGraph;
    } catch (error) {
      logger.error(
        `❌ [CodeGraphService] Graph extraction failed: ${error.message}`,
      );
