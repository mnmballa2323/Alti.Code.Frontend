  ...
  extractGraph: async (req, res) => {
    try {
      ...
      if (!targetPath) {
        return res
          .status(400)
          .json({ success: false, message: 'targetPath is required' });
      }
      ...
      if (!fs.existsSync(absoluteTarget)) {
        return res
          .status(404)
          .json({ success: false, message: 'Target directory not found' });
      }

      console.log(
        `[Graphify] Starting CodeGraph symbol extraction on ${absoluteTarget}`,
      );
      ...
      try {
        ...
        console.warn(
          `[Graphify] DLP Scrubbing bypassed or failed: ${e.message}`,
        );
      }
      ...
      try {
        const { neo4jService } =
          await import('../../services/neo4j.service.js');
        await neo4jService.ingestAstContext(cleanGraph);
      } catch (e) {
        console.warn(
          `[Graphify] Failed to ingest graph into Neo4j: ${e.message}`,
        );
      }
  ...
  getPageRank: async (req, res) => {
    try {
      ...
      try {
        ...
        await auditService.logAction(
          'Engineer',
          'EXECUTE_GDS_PAGERANK',
          { target: 'Neo4j_AST_Graph' },
          'SUCCESS',
        );
      } catch (e) {}
