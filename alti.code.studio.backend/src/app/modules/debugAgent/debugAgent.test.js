...
vi.mock('../gemini/gemini.service.js', () => ({
    GeminiAiService: { generateContent: vi.fn().mockResolvedValue('{"rootCause":"mocked","bugSeverity":"HIGH","patchedCode":"..."}') }
}));
...
describe('DebugAgentService — analyzeError (with memory integration)', () => {
    ...
    it('returns recalledMemories count of 0 when no memories exist', async () => {
        ...
        const { DebugAgentService: RealService } = await vi.importActual('./debugAgent.service.js');
        ...
        if (result) {
            ...
            expect(['LOW', 'MEDIUM', 'HIGH', 'CRITICAL']).toContain(result.bugSeverity);
    ...
    it('throws on non-string errorLog', async () => {
        const { DebugAgentService: RealService } = await vi.importActual('./debugAgent.service.js');
        await expect(RealService.analyzeError(null, 'some stack')).rejects.toThrow();
    });

    it('returns recalledMemories count matching memory search results', async () => {
        VectorMemory.search.mockResolvedValue([
            { content: 'Past fix: add null guard', metadata: { bugSeverity: 'HIGH' } },
            { content: 'Past fix: check array length', metadata: { bugSeverity: 'MEDIUM' } },
        ]);
