...
const POWERS_REGISTRY = [
  ...
  },
  {
    ...
    description:
      'Create PRs, review code, manage issues and branches directly from the AI.',
    provider: 'Inso Code',
    category: 'DevOps',
    ...
    hooks: [
      {
        name: 'Auto-commit docs on save',
        trigger: { type: 'file-save', filePattern: '*.md' },
        action: {
          type: 'agent-prompt',
          value:
            'Review the saved markdown file for broken links and formatting issues. Suggest corrections.',
        },
      },
    ],
  },
  {
    ...
    name: 'Terraform + IaC',
    description:
      'Plan, apply, and review Terraform infrastructure with AI assistance.',
    provider: 'Inso Code',
    ...
    hooks: [
      {
        ...
        trigger: { type: 'file-save', filePattern: '*.tf' },
        action: { type: 'shell-command', value: 'terraform validate' },
      },
    ],
  },
  {
    ...
    description:
      'Build, query, and manage your Supabase database and edge functions.',
    provider: 'Supabase',
    category: 'Database',
  ...
  },
  {
    ...
    description:
      'Integrate Stripe payment flows, webhooks, and customer management.',
    provider: 'Stripe',
    category: 'Payments',
  ...
  },
  {
    ...
    description:
      'Automatically generate unit tests whenever you save a source file.',
    provider: 'Inso Code',
    category: 'Testing',
    ...
    hooks: [
      {
        name: 'Generate tests on file save',
        trigger: { type: 'file-save', filePattern: '*.js' },
        action: {
          type: 'agent-prompt',
          value:
            "Analyze the saved file and generate comprehensive Jest unit tests for any exported functions that don't have tests yet. Place tests in the same directory.",
        },
      },
    ],
  },
  {
    ...
    hooks: [
      {
        name: 'Update JSDoc on save',
        trigger: { type: 'file-save', filePattern: '*.js' },
        action: {
          type: 'agent-prompt',
          value:
            'Review the saved file and add or update JSDoc comments for any exported functions missing documentation. Preserve existing comments.',
        },
      },
    ],
  },
    ...
    hooks: [
      {
        ...
        action: {
          type: 'agent-prompt',
          value:
            'Security-scan this file for: SQL injection, XSS, SSRF, hardcoded secrets, missing input validation, insecure deserialization. Report any issues found.',
        },
      },
    ],
  },
...
class PowersService {
  ...
  async installPower(powerId) {
    ...
    if (power.mcpConfig) {
      results.mcp = {
        ...power.mcpConfig,
        note: 'Fill in env vars in .alti/mcp.json',
      };
  ...
  async uninstallPower(powerId) {
    ...
    for (const sName of entry.steeringFiles || []) {
      try {
        ...
        logger.warn(
          `Powers: could not delete steering "${sName}": ${e.message}`,
        );
      }
    }
