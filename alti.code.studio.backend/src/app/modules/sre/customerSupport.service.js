...
Return the exact debugging hypothesis and the specific file/function to fix.`;
    ...
    logger.info(
      `🔍 [Customer Support] Hypothesis formulated: ${hypothesis.substring(0, 100)}...`,
    );

    // Step 2: Delegate the fix to the Hive Swarm
    logger.warn(
      `🐝 [Customer Support] Delegating bug fix to Alti Hive Swarm...`,
    );
    const fixCommand = `A customer reported a bug. Here is the hypothesis: ${hypothesis}. Fix the code flawlessly.`;

    const fixedCode = await HiveOrchestratorService.executeSwarm(
      userId,
      sessionId,
      fixCommand,
    );
    ...
    return {
      ...
      customerMessage:
        "Hi there! I'm an autonomous agent on the support team. I've located the issue, written a patch, and deployed the fix to production. Let us know if you need anything else!",
      fixedCode,
    };
  } catch (error) {
    logger.error(
      `❌ [Customer Support] Issue resolution critically failed:`,
      error,
    );
    throw error;
  }
