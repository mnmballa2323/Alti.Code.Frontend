...
class TaskDecomposerService {
     ...
     */
    decompose(tasks) {
        logger.info(`🕸️ TaskDecomposer: Sorting execution graph for ${tasks.length} tasks`);
        ...
        for (const task of tasks) {
            if (!task.id) {
                throw new Error('TaskDecomposer Error: Task is missing required field: id');
            }
            adjList.set(task.id, []);
        ...
        for (const task of tasks) {
            ...
            for (const depId of deps) {
                if (!inDegree.has(depId)) {
                    throw new Error(`TaskDecomposer Error: Task [${task.id}] depends on non-existent task [${depId}]`);
                }
        ...
        if (executionOrder.length !== tasks.length) {
            throw new Error('TaskDecomposer Error: Circular dependency detected in task graph');
        }

        logger.info('🕸️ TaskDecomposer: Graph topological sort completed successfully');
