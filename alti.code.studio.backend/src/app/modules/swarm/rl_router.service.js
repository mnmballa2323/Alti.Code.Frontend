...
class RlRouterService {
  ...
  recordReward(agentId, reward) {
    ...
    const record = this.rewardsMap.get(agentId) || {
      sumRewards: 0,
      count: 0,
      avgReward: 0,
    };
    ...
    logger.info(
      `📈 RL Router: Recorded reward of ${reward} for agent [${agentId}]. New Avg: ${record.avgReward.toFixed(4)}`,
    );
  }
  ...
  selectBestAgent(agentIds, epsilon = 0.1) {
    ...
    if (Math.random() < epsilon) {
      ...
      logger.info(
        `🎲 RL Router: Exploration mode selected agent [${selected}]`,
      );
      return selected;
    }
    ...
    logger.info(
      `🎯 RL Router: Exploitation mode selected best agent [${bestAgentId}] (Avg Reward: ${maxAvgReward.toFixed(4)})`,
    );
    return bestAgentId;
  }
}
