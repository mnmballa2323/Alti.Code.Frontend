...
const executeSwarm = async (userId, sessionId, epicPrompt) => {
  logger.info(
    `🐝 [Alti Hive] Starting Swarm Orchestrator for Epic: ${epicPrompt.substring(0, 50)}...`,
  );
...
Format your output as a strict JSON array of objects with "taskName", "description", and "assignedModel" (choose strictly from: 'claude-3-5-sonnet', 'gemini-3.1-pro-002', 'gpt-5.4').
Assign architecture to claude, logic to gemini, and security/edge-cases to gpt-5.4.
Epic: ${epicPrompt}
...
Output ONLY valid JSON.`;
    ...
    if (!jsonMatch)
      throw new Error('Failed to parse Hive Queen JSON breakdown.');
    const tasks = JSON.parse(jsonMatch[0]);

    logger.info(
      `🐝 [Alti Hive] Hive Queen (GPT-5.5) identified ${tasks.length} micro-tasks.`,
    );

    // Step 2: Spawn sub-agents in parallel across the Tri-Cloud (Upgraded for Infinite Swarm Concurrency)
    logger.info(
      `🐝 [Alti Hive] Upgrading concurrency to handle ${tasks.length} sub-agents...`,
    );
    const workerResults = [];
    ...
    for (let i = 0; i < tasks.length; i += CHUNK_SIZE) {
      ...
      logger.info(
        `🚀 [Alti Hive] Launching Swarm Cluster ${i / CHUNK_SIZE + 1} (${chunk.length} agents)...`,
      );

      const chunkPromises = chunk.map(async task => {
        logger.info(
          `🤖 [Hive Worker] Executing task: ${task.taskName} on ${task.assignedModel}`,
        );
...
Execute your task flawlessly and return the exact output/code/architecture.`;
    ...
    logger.info(
      `🐝 [Alti Hive] All swarm clusters completed their tasks. Commencing final Hive consolidation.`,
    );
...
Provide the flawless, finalized epic solution based on their work.`;
    ...
    logger.error(
      `❌ [Alti Hive] Swarm orchestration critically failed:`,
      error,
    );
    throw error;
  }
