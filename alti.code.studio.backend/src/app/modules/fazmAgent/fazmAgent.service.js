...
const discoverTunnel = async userId => {
  ...
  const lastActive = data.lastHeartbeatAt
    ? new Date(data.lastHeartbeatAt).getTime()
    : new Date(data.registeredAt).getTime();
...
const generateSubjectToken = async deviceId => {
  ...
  const issuer =
    process.env.VERTEX_ISSUER || 'http://localhost:5000/api/v1/oss-swarm/fazm';
  const kid = crypto
    .createHash('sha256')
...
const requestMagicLink = async email => {
  ...
  try {
    const { sendMailForRegisterWithGcp } =
      await import('../../middlewares/sendEmail/verifyRegisterEmail.js').catch(
        () => ({}),
      );
    if (sendMailForRegisterWithGcp) {
      await sendMailForRegisterWithGcp({
        to: normalizedEmail,
      ...
      console.log(
        `[Fazm Auth] Mock Email sent to ${normalizedEmail} with code: ${code}`,
      );
    }
  } catch (emailErr) {
