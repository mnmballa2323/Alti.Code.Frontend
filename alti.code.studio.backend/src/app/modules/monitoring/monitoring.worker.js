...
export const monitoringWorkerProcessor = async job => {
  ...
  logger.info(
    `🔭 MonitoringWorker [${jobId}]: Performing check '${checkType}'...`,
  );
  ...
  try {
    ...
    if (checkType === 'anomaly') {
      ...
      if (evaluation.status === 'CRITICAL' || evaluation.status === 'WARNING') {
        logger.warn(
          `🚨 MonitoringWorker [${jobId}] Sentinel Alert: ${evaluation.summary}`,
        );
        return { status: evaluation.status, report: evaluation };
      }

      logger.info(
        `✅ MonitoringWorker [${jobId}]: System Health OK. No AI anomalies.`,
      );
      return { status: 'OK' };
    }
  } catch (error) {
