...
class HooksService {
    ...
    async createHook(def) {
        ...
        if (!trigger?.type || !VALID_TRIGGERS.includes(trigger.type)) {
            throw new Error(`"trigger.type" must be one of: ${VALID_TRIGGERS.join(', ')}`);
        }
        if (!action?.type || !VALID_ACTIONS.includes(action.type)) {
            throw new Error(`"action.type" must be one of: ${VALID_ACTIONS.join(', ')}`);
        }
    ...
    async deleteHook(id) {
        ...
        if (filtered.length === hooks.length) throw new Error(`Hook "${id}" not found.`);
    ...
    async triggerEvent(eventType, context = {}) {
        ...
        const matching = hooks.filter(h => {
            ...
            if (h.trigger.filePattern && context.file) {
                ...
                if (!filename.endsWith(ext) && !filename.includes(pattern.replace(/\*/g, ''))) {
                    return false;
                }
            }
            return true;
        ...
        logger.info(`🪝 Hooks: firing ${matching.length} hook(s) for event "${eventType}"`);
