...
describe('Crabbox Integration Tests', () => {
  ...
  it('should run a remote command successfully via CLI', async () => {
    ...
    expect(exec).toHaveBeenCalledWith(
      'crabbox run --provider cloudflare --class standard -- "node app.js"',
      expect.any(Object),
      expect.any(Function),
    );
  });
  ...
  it('should warmup a box and return lease ID', async () => {
    ...
    const result = await crabboxService.warmup({
      provider: 'hetzner',
      class: 'standard',
      ttl: 300,
    });
    ...
    expect(exec).toHaveBeenCalledWith(
      'crabbox warmup --provider hetzner --class standard --ttl 300',
      expect.any(Object),
      expect.any(Function),
    );
  });
  ...
  it('CodeExecutionSandbox: should execute code in Crabbox if provider is crabbox', async () => {
    ...
    const result = await CodeExecutionSandbox.execute('console.log("hello")', {
      provider: 'crabbox',
    });
    expect(result.success).toBe(true);
    expect(result.exitCode).toBe(0);
  ...
  it('AgentContainerOrchestrator: should execute agent tool in Crabbox if provider is crabbox', async () => {
    ...
    const runSpy = vi.spyOn(crabboxService, 'run').mockResolvedValue({
      ...
      exitCode: 0,
      stdout:
        'RESULT_PAYLOAD:{"success":true,"result":"tool output","isHandoff":false}',
      stderr: '',
    });
  ...
  it('DockerWorkspaceManager: should execute user code in Crabbox if provider is crabbox', async () => {
    const { DockerWorkspaceManager } =
      await import('../sandbox/docker_workspace_manager.js');
    ...
    const result = await manager.executeCode('user-1', 'console.log("hello")', {
      provider: 'crabbox',
    });
    expect(result.success).toBe(true);
  ...
  it('DockerWorkspaceManager: should execute OSS code in Crabbox if provider is crabbox', async () => {
    ...
    const result = await manager.executeOssCode(
      'module-1',
      'console.log("hello")',
      './logs/workspaces/module-1',
      { provider: 'crabbox' },
    );
    expect(result.success).toBe(true);
    expect(result.exitCode).toBe(0);
