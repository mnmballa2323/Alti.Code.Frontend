...
class CrabboxSandboxService {
    ...
    async executeInSandbox(command, options = {}) {
        ...
        logger.info(`🛡️ Sandbox: Initializing secure execution for command: "${command}"`);
        ...
        try {
            ...
            if (!lease || !lease.success || !lease.id) {
                throw new Error(`Failed to provision sandbox lease: ${lease?.stderr || 'Unknown error'}`);
            }

            logger.info(`🛡️ Sandbox: Lease [${lease.id}] active. Running execution...`);
    ...
    async _releaseLease(leaseId) {
        try {
            ...
            logger.warn(`🛡️ Sandbox: Cleanup failed for lease [${leaseId}]: ${err.message}`);
        }
    }
}
