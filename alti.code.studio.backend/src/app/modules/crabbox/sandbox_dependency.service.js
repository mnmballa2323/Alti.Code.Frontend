...
class SandboxDependencyService {
  ...
  resolveMissingPackage(errorLog) {
    ...
    const pythonMatch = errorLog.match(
  ...
  async installDependency(
    ...
    if (this.installedPackages.has(packageName)) {
      logger.info(
        `📦 SandboxDependency: Package [${packageName}] is already marked installed.`,
      );
      return true;
    }

    logger.warn(
      `📦 SandboxDependency: Attempting dynamic installation of missing package [${packageName}] for runtime [${runtimeEnv}]`,
    );
    ...
    try {
      ...
      logger.info(
        `📦 SandboxDependency: Dynamic installation of [${packageName}] completed successfully.`,
      );
      return true;
    } catch (e) {
      logger.error(
        `📦 SandboxDependency: Failed to install package [${packageName}] inside sandbox.`,
        e,
      );
      return false;
    }
