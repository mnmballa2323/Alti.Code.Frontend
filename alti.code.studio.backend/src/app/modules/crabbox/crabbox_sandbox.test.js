...
describe('CrabboxSandboxService Tests', () => {
  ...
  it('should lease a sandbox, execute command successfully, and release lease', async () => {
    ...
    const warmupSpy = vi
      .spyOn(crabboxService, 'warmup')
      .mockResolvedValue(mockLease);
    const runSpy = vi.spyOn(crabboxService, 'run').mockResolvedValue(mockRun);
    const stopSpy = vi
      .spyOn(crabboxService, 'stop')
      .mockResolvedValue({ success: true });

    const result = await crabboxSandboxService.executeInSandbox('npm test', {
      timeoutMs: 5000,
    });
    ...
    expect(runSpy).toHaveBeenCalledWith(
      'npm test',
      expect.objectContaining({ id: 'lease-xyz-123' }),
    );
  ...
  it('should fail if the sandbox warmup lease fails to provision', async () => {
    const mockLease = {
      success: false,
      stderr: 'No cloud resources available',
    };
    ...
    const result =
      await crabboxSandboxService.executeInSandbox('npm run build');
  ...
  it('should trigger a timeout error if sandbox command execution hangs', async () => {
    ...
    vi.spyOn(crabboxService, 'run').mockImplementation(
      () => new Promise(() => {}),
    );
    const stopSpy = vi
      .spyOn(crabboxService, 'stop')
    ...
    const result = await crabboxSandboxService.executeInSandbox(
      'node infinite_loop.js',
      { timeoutMs: 50 },
    );
