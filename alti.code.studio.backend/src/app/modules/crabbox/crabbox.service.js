...
export class CrabboxService {
  ...
  async run(command, options = {}) {
    ...
    try {
      ...
      if (error.code === 'ENOENT' || error.message.includes('not found')) {
        logger.warn(
          `⚠️ Crabbox CLI not found on host. Falling back to REST API...`,
        );
        return await this._runRest('run', {
          command,
          id,
          provider,
          class: boxClass,
  ...
  async warmup(options = {}) {
    ...
    try {
      ...
      const match =
        result.stdout.match(/Lease ID:\s*([a-zA-Z0-9_-]+)/i) ||
      ...
      if (error.code === 'ENOENT' || error.message.includes('not found')) {
        logger.warn(
          `⚠️ Crabbox CLI not found. Falling back to REST API warmup...`,
        );
        return await this._runRest('warmup', {
          provider,
          class: boxClass,
          ttl,
        });
  ...
  async stop(leaseId) {
    ...
    try {
      ...
      if (error.code === 'ENOENT' || error.message.includes('not found')) {
        logger.warn(
          `⚠️ Crabbox CLI not found. Falling back to REST API stop...`,
        );
  ...
  async status(leaseId) {
    ...
    try {
      ...
      if (error.code === 'ENOENT' || error.message.includes('not found')) {
        logger.warn(
          `⚠️ Crabbox CLI not found. Falling back to REST API status...`,
        );
  ...
  async syncPlan(options = {}) {
    ...
    try {
      ...
      if (error.code === 'ENOENT' || error.message.includes('not found')) {
        logger.warn(
          `⚠️ Crabbox CLI not found. Falling back to REST API sync-plan...`,
        );
  ...
  _runRest(endpoint, payload) {
    return new Promise((resolve, reject) => {
      if (!this.coordinatorUrl) {
        logger.warn(
          '⚠️ Crabbox Coordinator URL not configured. Simulating remote execution...',
        );
      ...
      req.on('error', err => {
        logger.error(
          `❌ Crabbox REST Coordinator connection failed: ${err.message}`,
        );
  ...
  _simulateExecution(endpoint, payload) {
    ...
    const leaseId =
      payload.leaseId ||
      `mock_lease_${Math.random().toString(36).substring(2, 9)}`;
