...
describe('SpecKit Regional Compliance Matrix Tests', () => {
    ...
    describe('Service Compliance Layer', () => {
        it('should generate requirements without regional injection if regions list is empty', async () => {
            GeminiAiService.generateContent.mockResolvedValue('Mock requirements output');
        ...
        it('should inject regional requirements when EU and US regions are supplied', async () => {
            GeminiAiService.generateContent.mockResolvedValue('Mock regional requirements');

            const result = await specKitService.createSpec('Create a user signup form', ['EU', 'US']);
    ...
    describe('Controller / Endpoint Integration', () => {
        it('should extract regions from body and pass to createSpec service call', async () => {
            ...
            const serviceSpy = vi.spyOn(specKitService, 'createSpec').mockResolvedValue({
                specId: 'spec-med-999',
            ...
            expect(serviceSpy).toHaveBeenCalledWith('Build medical upload portal', ['US']);
        ...
        it('should pass explicit regions from body to createDesign service call', async () => {
            ...
            const serviceSpy = vi.spyOn(specKitService, 'createDesign').mockResolvedValue({
                specId,
