...
class CaptainAgent {
    ...
    async monitorHealth() {
        ...
        logger.info(`⚓ Captain: System Health Check - MEM: ${memUsage.toFixed(1)}% | LOAD: ${loadAvg.toFixed(2)}`);

        // Simple threshold logic
        if (memUsage > 90 || loadAvg > 5) {
            return this.declareEmergency('critical', `High resource usage: MEM ${memUsage.toFixed(1)}%, LOAD ${loadAvg.toFixed(2)}`);
    ...
    declareEmergency(level, reason) {
        logger.error(`⚓ Captain: 🚨 DECLARING ${level.toUpperCase()} EMERGENCY: ${reason}`);
        this.status = level;
