...
import { dlpService } from '../ai/gcpDlp.service.js';
...
class LspGateway {
  ...
  initialize(httpServer) {
    ...
    this.wss.on('connection', (ws, req) => {
      ...
      logger.info(
        `🔮 LSP Telepathy: IDE Session Connected [${sessionId}] for Tenant [${tenantId}]`,
      );
      this.activeSessions.set(sessionId, ws);

      // Asynchronously provision the secure Docker workspace for this session
      this.tenantOrchestrator
        .provisionWorkspace(tenantId)
        .then(() => {
          logger.info(
            `✅ [LSP Gateway] Docker workspace provisioned for Tenant [${tenantId}]`,
          );
        })
        .catch(err => {
          logger.error(
            `❌ [LSP Gateway] Failed to provision Docker workspace: ${err.message}`,
          );
        });
      ...
      ws.on('close', () => {
        logger.info(
          `🔮 LSP Telepathy: IDE Session Disconnected [${sessionId}]`,
        );
    ...
    logger.info(
      '🔮 LSP Telepathy Gateway Initialized and listening on /api/v1/lsp/telepathy',
    );
  }
      ...
      if (msg.method === 'textDocument/didChange') {
        ...
        try {
          ...
          if (dlpResult && dlpResult.length > 0) {
            logger.warn(
              `🚨 LSP Telepathy: Hardcoded secret detected in ${uri} via Google Cloud DLP. Pushing auto-remediation...`,
            );
            ...
            this.pushSuggestion(uri, {
              ...
              message:
                'Google Cloud DLP detected a hardcoded secret. Press Tab to securely migrate to Secret Manager.',
              replacement:
                'await SecretManagerService.getSecret("MIGRATED_SECRET_1")',
        ...
        this.typingDebouncers.set(
          uri,
          setTimeout(async () => {
            try {
              logger.info(
                `🧠 LSP Telepathy: Triggering Tri-Cloud fast inference prediction for ${uri}...`,
              );
              ...
              logger.error(
                `⚠️ LSP Telepathy: Autocomplete failure: ${e.message}`,
              );
            }
          }, 800),
        ); // 800ms debounce
        ...
        logger.info(
          `🧠 Telepathy Intent Triggered at ${uri}:${cursorLine}. Intent: ${intent}`,
        );
        ...
        const taskResult = await swarmBrain.executeTask(
          `User is at line ${cursorLine} in file ${uri}. The current file content is:\n\n${contextBuffer}\n\nThe user's intent is: ${intent}. Please write the required code.`,
          [],
        );
