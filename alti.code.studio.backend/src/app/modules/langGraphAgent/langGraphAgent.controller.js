...
const executeGraph = catchAsync(async (req, res) => {
    ...
    if (!graphDefinition || !initialState) {
        return res.status(httpStatus.BAD_REQUEST).json({ success: false, message: 'graphDefinition and initialState are required.' });
    }

    const result = await LangGraphAgentService.runStatefulGraph(graphDefinition, initialState);
