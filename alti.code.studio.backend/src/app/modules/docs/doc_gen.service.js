...
class DocGenService {
  ...
  async enrichFile(filePath) {
    try {
      ...
      logger.info(
        `📜 DocGen: Generating documentation for ${path.basename(filePath)}...`,
      );
  ...
  async enrichDirectory(dirPath) {
    ...
    try {
      ...
      logger.info(
        `📜 DocGen: Found ${allFiles.length} eligible files. Processing in batches of ${CONCURRENT_LIMIT}...`,
      );
      ...
      logger.info(
        `📜 DocGen: Complete — ${this.stats.documented} documented, ${this.stats.skipped} skipped, ${this.stats.failed} failed.`,
      );
      return this.stats;
    } catch (error) {
  ...
  async generateCoverageReport(outputDir = './') {
    const { total, documented, skipped, failed } = this.stats ?? {
      total: 0,
      documented: 0,
      skipped: 0,
      failed: 0,
    ...
    const coverage =
      total > 0 ? ((documented / total) * 100).toFixed(1) : '0.0';
    const status =
      total > 0 && failed === 0 ? '✅ **Healthy**' : '⚠️ **Needs Attention**';
