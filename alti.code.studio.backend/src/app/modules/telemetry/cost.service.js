...
class CostService {
  ...
  async track(model, inputTokens, outputTokens, userId, agentId = 'system') {
    ...
    if (typeof outputTokens !== 'number' || isNaN(outputTokens))
      outputTokens = 0;

    const price = this.prices[model];
    if (!price) {
      logger.warn(
        `💰 CostService: Unknown model "${model}" — cost tracking skipped.`,
      );
    }
    ...
    logger.info(
      `💰 Cost: $${totalCost.toFixed(6)} | Model: ${model} | In: ${inputTokens} Out: ${outputTokens} | User: ${userId}`,
    );

    // Accumulate in-memory totals for dashboard queries
    this._userTotals.set(
      userId,
      (this._userTotals.get(userId) ?? 0) + totalCost,
    );
    this._modelTotals.set(
      model,
      (this._modelTotals.get(model) ?? 0) + totalCost,
    );
  ...
  getSummary() {
    ...
    const grandTotal = [...this._userTotals.values()].reduce(
      (a, b) => a + b,
      0,
    );
