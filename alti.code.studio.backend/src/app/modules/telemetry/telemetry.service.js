...
const LLM_ERROR_THRESHOLD = 0.2; // 20% LLM failures → anomaly
...
class TelemetryCollector {
  ...
  recordLlmCall({
    ...
    if (tokens > 0) {
      cloudMonitoringService.emitCustomMetric(
        'custom.googleapis.com/swarm/tokens_consumed',
        tokens,
        labels,
      );
    }
    cloudMonitoringService.emitCustomMetric(
      'custom.googleapis.com/swarm/latency_ms',
      latencyMs,
      labels,
    );
  }
  ...
  getMetrics() {
    ...
    const httpErrorRate = httpEvents.length
      ? httpErrors / httpEvents.length
      : 0;
...
class AnomalyDetector {
  ...
  detect(metrics, rawWindow) {
    ...
    // 1. HTTP error rate
    if (
      metrics.http.total >= 10 &&
      metrics.http.errorRate / 100 >= ERROR_RATE_THRESHOLD
    ) {
    ...
    if (
      metrics.llm.total >= 5 &&
      metrics.llm.errorRate / 100 >= LLM_ERROR_THRESHOLD
    ) {
      this._push(anomalies, {
    ...
    // 4. Novel exceptions
    const novelExceptions = rawWindow.filter(
      e => e.type === 'exception' && e.isNovel,
    );
...
class IncidentResponder {
  ...
  async respond(anomaly) {
    logger.warn(
      `🚨 IncidentResponder: Handling anomaly [${anomaly.type}] — ${anomaly.message}`,
    );
    ...
    if (
      ['novel_exception', 'llm_error_spike'].some(t =>
        anomaly.type.startsWith(t),
      )
    ) {
      try {
        ...
        const report =
          'Mock Surfer Agent Output: Surfer agent is currently offline.';
        incident.webInsight = report.synthesizedSolution || '';
        logger.info(
          `🚨 Incident: Web insight gathered — ${incident.webInsight.substring(0, 80)}`,
        );
      } catch (e) {
    ...
    try {
      ...
      if (sreResult?.success) {
        ...
        logger.warn(
          `🚨 SREAgent Triage Vetoed/Failed: ${sreResult?.reason || 'Unknown'}`,
        );
      }
    } catch (e) {
...
export class TelemetryService {
  constructor() {
    ...
    logger.info(
      '📡 TelemetryService v6.4.0: Runtime Observability initialized.',
    );
  }
  ...
  recordLlmCall(opts) {
    this.collector.recordLlmCall(opts);
  }

  /** Record an exception */
  recordException(error, context) {
    return this.collector.recordException(error, context);
  }
  ...
  startDetection(intervalMs = 10_000) {
    ...
    logger.info(
      `📡 Telemetry: Anomaly detection started (every ${intervalMs / 1000}s)`,
    );
  ...
  async _runDetection() {
    try {
      ...
      logger.warn(
        `📡 Telemetry: ${anomalies.length} anomaly(ies) detected. Responding…`,
      );
      telemetryBus.emit('anomaly:detected', {
  ...
  async _ingestRemoteTelemetry() {
    ...
    if (process.env.AXIOM_TOKEN && process.env.AXIOM_DATASET) {
      try {
        ...
        const res = await fetch(
          `https://api.axiom.co/v1/datasets/${process.env.AXIOM_DATASET}/query`,
          {
            method: 'POST',
            headers: {
        ...
        if (res.ok) {
          ...
          for (const ev of events) {
            this.collector.recordException(
              new Error(
                `[Axiom Cloud] ${ev.message || ev.error || 'Unknown Production Error'}`,
              ),
          ...
          if (events.length > 0) {
            logger.info(
              `🚨 SREAgent: Ingested ${events.length} fresh cloud errors from Axiom.`,
            );
          }
        }
      } catch (e) {
    ...
    if (process.env.DATADOG_API_KEY && process.env.DATADOG_APP_KEY) {
      ...
      logger.debug(
        'SREAgent: Datadog pipeline ready for remote log ingestion.',
      );
    }
  }
  ...
  async analyzeVideoGlitch(videoBuffer, contextPrompt, domSnapshot = '') {
    logger.info(
      `👁️ VIDEO_EYE: Received UI Glitch Recording (${videoBuffer.length} bytes). Analyzing...`,
    );
    ...
    try {
      ...
      logger.info(
        '🛡️ Google Cloud DLP: Scrubbing UI context for PII using advanced Infotype redaction...',
      );

      const scrubbedContext = await GoogleDlpService.redactText(contextPrompt);
      const scrubbedDom = domSnapshot
        ? await GoogleDlpService.redactText(domSnapshot)
        : '';

      // Format the DOM Snapshot string block if provided
      const domContextBlock =
        scrubbedDom && scrubbedDom.trim().length > 0
          ? `\n\n--- LIVE DOM SNAPSHOT AT TIME OF RECORDING ---\n\`\`\`html\n${scrubbedDom}\n\`\`\`\n----------------------------------------------`
          : '';

      // 👁️ 1. Google Cloud Video Intelligence: Frame-by-frame autonomous extraction
      logger.info(
        '👁️ Google Cloud Video Intelligence: Initiating structural video extraction...',
      );
      const videoIntellRes =
        await videoIntelligenceService.analyzeUiGlitch(videoBuffer);
