...
class SteeringService {
    ...
    _safeFilename(name) {
        return name.toLowerCase().replace(/[^a-z0-9-_]/g, '-').replace(/-+/g, '-') + '.md';
    }
    ...
    async createSteering(name, content, options = {}) {
        ...
        if (!VALID_SCOPES.includes(scope)) throw new Error(`scope must be: ${VALID_SCOPES.join(', ')}`);
        if (!VALID_INCLUSION.includes(inclusion)) throw new Error(`inclusion must be: ${VALID_INCLUSION.join(', ')}`);
        ...
        try {
            ...
            throw new Error(`Steering file "${filename}" already exists. Use update instead.`);
        } catch (e) {
    ...
    async listSteering() {
        await this._ensureDir();
        try {
            const entries = await fs.readdir(this.steeringDir, { withFileTypes: true });
            const files = await Promise.all(
                entries
                    .filter(e => e.isFile() && e.name.endsWith('.md'))
                    .map(async e => {
                        const raw = await fs.readFile(path.join(this.steeringDir, e.name), 'utf-8');
    ...
    async getActiveContext(opts = {}) {
        ...
        for (const f of files) {
            ...
            if (f.inclusion === 'always') {
                ...
                const patterns = Array.isArray(f.filePatterns) ? f.filePatterns : [f.filePatterns];
                ...
                include = manualIncludes.includes(f.filename) || manualIncludes.includes(f.name);
    ...
    async initDefaults() {
        const defaults = [
            ...
            },
            {
                ...
                options: { scope: 'workspace', inclusion: 'conditional', filePatterns: ['*.test.js', '*.spec.ts'] },
            },
        ];
