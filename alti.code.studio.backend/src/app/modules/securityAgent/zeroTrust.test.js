    ...
    it('should ALLOW access and execute the Mandiant audit if Identity-Aware validation succeeds', async () => {
        ...
        const mockReport = {
            ...
            codeAnalysis: {
                ...
                mandiantIntelligence: 'Known zero-day exploit detected in heavily outdated generic package tied to APT29.',
        ...
        expect(SecurityAgentService.auditCodebase).toHaveBeenCalledWith('./src', 'test-user-123', 'test-session');
        expect(mockRes.status).toHaveBeenCalledWith(httpStatus.OK);

        const jsonResponse = mockRes.json.mock.calls[0][0];
        expect(jsonResponse.success).toBe(true);
        expect(jsonResponse.data.codeAnalysis.mandiantIntelligence).toContain('APT29');
    });
});
