...
const startAudit = catchAsync(async (req, res) => {
    ...
    const isValidPosture = TRUSTED_POSTURE_VALUES.has(String(devicePostureHeader ?? '').toLowerCase());
    ...
    if (!isValidPosture) {
        ...
        return sendResponse(res, {
            ...
            success: false,
            message: 'Access Denied: BeyondCorp Zero-Trust Identity-Aware Proxy validation failed.',
    ...
    if (isPathTraversal(repoPath)) {
        logger.warn(`[SecurityAgent] Rejected audit request with invalid repoPath: "${repoPath}" from User: ${userId}`);
        return sendResponse(res, {
            statusCode: httpStatus.BAD_REQUEST,
            success: false,
            message: 'Invalid repoPath: path must be provided and must not contain path-traversal sequences.',
            data: null,
        });
    }

    const result = await SecurityAgentService.auditCodebase(repoPath, userId, sessionId);
    sendResponse(res, {
