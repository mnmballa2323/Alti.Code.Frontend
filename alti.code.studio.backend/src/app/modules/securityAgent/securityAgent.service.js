...
const auditCodebase = async (
  ...
  if (
    ...
    throw new ApiError(
      ...
      'SecurityAgent: repoPath must be a non-empty string.',
    );
  }
  ...
  try {
    ...
    if (
      report.dependencyAudit &&
      report.dependencyAudit.metadata &&
      report.dependencyAudit.metadata.vulnerabilities
    ) {
    ...
    try {
      ...
      report.codeAnalysis = {
        ...
        mandiantIntelligence:
          'Parse failure. Assume critical compromise. Review raw logs.',
    ...
    logger.info(
      `🛡️ [MILITARY AUDIT] Immutable record saved: ${AUDIT_LOG_PATH}`,
    );
  } catch (error) {
