...
export const securityWorkerProcessor = async job => {
  ...
  logger.info(
    `🛡️ SecurityWorker [${jobId}]: Starting audit scan for ${repoPath}...`,
  );

  try {
    if (job.data.task === 'pentest') {
      logger.info(
        `🛡️ SecurityWorker [${jobId}]: Escalating to RedTeam Agent for dynamic code assault.`,
      );
      return await redTeamAgent.attack(job.data.code, job.data.context);
    }

    // 1. Perform Audit
    const report = await SecurityAgentService.auditCodebase(
      repoPath,
      userId,
      sessionId,
    );
    ...
    if (report.dependencyAudit?.metadata?.vulnerabilities) {
      ...
      if (vulns.critical > 0 || vulns.high > 0) {
        logger.info(
          `🛡️ SecurityWorker [${jobId}]: High risk detected. Memorizing signature...`,
        );
    ...
    logger.info(
      `✅ SecurityWorker [${jobId}]: Audit Complete. Score: ${report.score}`,
    );
    return report;
  } catch (error) {
