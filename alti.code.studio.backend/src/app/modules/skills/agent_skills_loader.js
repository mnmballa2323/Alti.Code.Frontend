...
export class AgentSkillsLoader {
    ...
    registerSkill(name, description, parameterSchema, handler) {
        ...
        console.log(`🔌 Skill registered successfully: [${name}] - "${description}"`);
        return this;
    }
    ...
    async executeSkill(name, params = {}) {
        ...
        console.log(`🚀 Executing skill [${name}] with args:`, JSON.stringify(params));
        ...
        const checkValue = (val) => {
            if (typeof val === 'string') {
                for (const pattern of restrictedPatterns) {
                    if (pattern.test(val)) {
                        throw new Error(`Zero-Trust Violation: Parameter contains restricted or malicious shell pattern: "${val}"`);
                    }
                }
