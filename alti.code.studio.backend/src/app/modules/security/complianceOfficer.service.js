 ...
 */
const auditCompliance = async (userId, sessionId, codebaseSnapshot, logSample) => {
    logger.info(`🛡️ [Compliance Officer] Commencing SOC2/GDPR compliance audit...`);
...
Generate a strict compliance audit report, and output the exact code patches required to fix the violations.`;
        ...
        const result = await LlmGatewayService.routeCompletion(
            userId, sessionId, compliancePrompt, 'gpt-5.5', 'Compliance-Officer'
        );
        ...
        logger.error(`❌ [Compliance Officer] Compliance audit critically failed:`, error);
        throw error;
    }
};
