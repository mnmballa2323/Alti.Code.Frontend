...
const huntAndFixVulnerabilities = async (userId, sessionId, targetDir) => {
  logger.info(
    `🛡️ [AgentShield] Commencing Autonomous Red-Team Hunt on ${targetDir}`,
  );
...
${criticalCode}`;
    ...
    if (
      huntResult.reply.includes('SECURE') ||
      !huntResult.reply.includes('---')
    ) {
      logger.info(
        '🛡️ [AgentShield] Scan complete. Target directory is SECURE.',
      );
    ...
    logger.warn(
      `🚨 [AgentShield] Vulnerability Detected!\n${explanation.trim()}`,
    );
    logger.warn(
      `🧪 [AgentShield] Exploit payload generated. Executing in safe sandbox...`,
    );
...
Generate the flawlessly patched raw implementation code. Return ONLY the raw javascript code, no markdown.`;
    ...
    const patchedCode = fixResult.reply
