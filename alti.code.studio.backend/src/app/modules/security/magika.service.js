...
class GoogleMagikaService {
    ...
    async init() {
        if (!this.isLoaded) {
            logger.info('🧠 [Magika] Loading Google Magika AI deep learning model for file detection...');
    ...
    async identifyBuffer(buffer, originalFilename = 'unknown') {
        ...
        try {
            ...
            logger.info(`🔍 [Magika] Scanned [${originalFilename}]. AI predicts: ${result.label} (Score: ${result.score})`);
            ...
            sentinelService.reportEvent('MAGIKA_SCAN_FAILURE', 'MEDIUM', { file: originalFilename, error: error.message });
            return null;
        }
    }
