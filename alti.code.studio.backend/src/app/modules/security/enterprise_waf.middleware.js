  ...
  constructor() {
    ...
    this.rateLimiter = rateLimit({
      ...
      handler: (req, res, next, options) => {
        ...
        logger.warn(
          `🛑 [WAF] Rate Limit Exceeded for IP: ${ip}. Blocking request to ${req.originalUrl}`,
        );

        // Dispatch to SIEM
        const tenantId = req.user?.tenantId || req.tenant?.id;
        if (tenantId) {
          siemService.dispatchEvent(tenantId, 'WAF_RATE_LIMIT', {
            ip,
    ...
    if (req.method === 'POST' || req.method === 'PUT') {
      ...
      for (const pattern of maliciousPatterns) {
        if (payloadStr.includes(pattern)) {
          logger.error(
            `🚨 [WAF] Malicious payload detected containing: "${pattern}". Connection terminated.`,
          );
          ...
          return res.status(403).json({
            error: 'Forbidden',
            message:
              'Payload violates Enterprise Zero-Trust policies (Code: WAF_001).',
          });
        }
      }
    }
