...
class AzureZanzibarService {
  ...
  async checkAstClearance(developerEmail, astNodeId) {
    logger.info(
      `🔐 [Zanzibar] Swarm is validating cryptographic clearance for [${developerEmail}] on node [${astNodeId}]...`,
    );
    ...
    try {
      ...
      if (hasClearance) {
        logger.info(
          `✅ [Zanzibar] Clearance verified. AST Node authorized for RAG context window.`,
        );
        return true;
      } else {
        logger.warn(
          `⚠️ [Zanzibar] CLEARANCE DENIED. Developer attempted to access unauthorized RAG context.`,
        );
        return false;
      }
    } catch (error) {
      logger.error(
        `❌ [Zanzibar] Authorization execution failed:`,
        error.message,
      );
