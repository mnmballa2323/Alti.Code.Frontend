...
class AzureCaService {
    constructor() {
        ...
        logger.info('🔐 [CA Service] Azure Key Vault Certificate Authority Service initialized.');
    }
    ...
    async mintAgentCertificate(agentId, publicKeyPem) {
        logger.info(`🔐 [CA Service] Swarm is minting a cryptographic identity for Agent [${agentId}] via Azure Key Vault Certificate issuer...`);
        ...
        try {
            ...
            logger.info(`✅ [CA Service] mTLS Certificate minted successfully via Key Vault. Identity verified.`);
            return mockCertPem;
        } catch (error) {
