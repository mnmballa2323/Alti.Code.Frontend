...
class DeploymentSentinelService {
  ...
  async auditDeployment(plan) {
    logger.info(
      `🛡️ [Sentinel] Auditing deployment mission to ${plan.platform}...`,
    );
    ...
    if (isSafe) {
      logger.info(
        '✅ [Sentinel] Deployment Mission APPROVED. No vulnerabilities detected.',
      );
    } else {
      logger.error(
        '❌ [Sentinel] Deployment Mission REJECTED. Security violations found.',
      );
    }
