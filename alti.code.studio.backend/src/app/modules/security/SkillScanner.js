...
class SkillScanner {
  ...
  async scanSkill(skillDirectory) {
    if (!this.hasScanner) {
      console.warn(
        '[Auditor] Cisco Skill Scanner not found, bypassing for now.',
      );
      return {
        isSafe: true,
        report: { message: 'Scanner not installed locally' },
      };
    }

    console.log(
      `[Auditor] Initiating zero-trust scan on skill directory: ${skillDirectory}`,
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      exec(
        `python3 -m cisco_skill_scanner --target ${skillDirectory} --format json`,
        (error, stdout, stderr) => {
          if (error) {
            console.error(
              `[Auditor] Skill scan failed or found vulnerabilities: ${stderr}`,
            );
            return resolve({
              isSafe: false,
          ...
          try {
            ...
            resolve({
              isSafe: true,
              report: { message: 'Heuristic scan passed', raw: stdout },
            });
          }
        },
      );
