...
class RBACService {
  ...
  requireRole(requiredRole) {
    return (req, res, next) => {
      ...
      if (!userEmail) {
        logger.warn(
          `🛑 [RBAC] Access Denied: Unauthenticated request attempting to breach ${req.originalUrl}`,
        );
        return res
          .status(401)
          .json({ error: 'Unauthorized', message: 'Missing IAP Identity.' });
      }
      ...
      if (!userRoles.includes(requiredRole) && !userRoles.includes('ADMIN')) {
        logger.error(
          `🚨 [RBAC] Access Denied: ${userEmail} attempted to execute a [${requiredRole}] operation without privileges.`,
        );
      ...
      logger.info(
        `🔐 [RBAC] Access Granted: ${userEmail} authorized as [${requiredRole}].`,
      );
      next();
    };
  }
  ...
  enforceModelTierPolicy() {
    return (req, res, next) => {
      const requestedModel =
        req.body?.modelTier || req.body?.model || 'gemini-3.1-flash';
      ...
      if (
        ...
        logger.warn(
          `💸 [RBAC/FinOps] Access Denied: ${userEmail} attempted to use ${requestedModel} without budget/clearance.`,
        );
        return res.status(403).json({
          error: 'Forbidden',
