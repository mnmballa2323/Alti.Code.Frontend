...
class VoiceBiometricsService {
    constructor() {
        ...
        this.authorizedFingerprints = new Set(
            ...
            process.env.AUTHORIZED_VOICE_HASHES ? process.env.AUTHORIZED_VOICE_HASHES.split(',') : ['admin_voice_hash_v1']
        );
    ...
    async verifyIntent(audioBuffer) {
        try {
            ...
            if (!fingerprint) {
                logger.warn('🎙️ Biometric Verification Failed: Empty audio payload provided.');
                return false;
            }
            ...
            if (isAuthorized) {
                logger.info(`✅ Biometric Verification Passed: Identified authorized operator fingerprint [${fingerprint}].`);
                return true;
            } else {
                logger.warn(`🛑 Biometric Verification Failed: Unauthorized voice print detected [${fingerprint}].`);
                return false;
            }
        } catch (error) {
            logger.error(`[Voice Biometrics] Error during verification flow: ${error.message}`);
            return false;
        }
    }
}
