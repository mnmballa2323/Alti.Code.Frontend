...
class PolicyEngine {
    ...
    this.defaultPolicies = [
      {
        ...
        description:
          'Prevents installation of libraries with generic copyleft licenses.',
        type: 'dependency',
        condition: (action, metadata) => {
          if (
            action === 'install_dependency' &&
            metadata?.license?.toUpperCase().includes('GPL')
          ) {
            return {
              allowed: false,
              reason:
                'GPL licenses are prohibited in this enterprise environment.',
            };
          }
          return { allowed: true };
        },
      },
      {
        ...
        description:
          'Requires all pull requests or code merges to have at least 80% coverage.',
        type: 'code_quality',
        condition: (action, metadata) => {
          if (action === 'merge_code' && metadata?.coverage < 80) {
            return {
              allowed: false,
              reason: `Test coverage is ${metadata.coverage}%. Minimum required is 80%.`,
            };
          }
          return { allowed: true };
        },
      },
      {
        ...
        description:
          'Infrastructure provisioning must only target approved US regions.',
        type: 'infrastructure',
        condition: (action, metadata) => {
          if (
            action === 'provision_infrastructure' &&
            metadata?.region &&
            !metadata.region.startsWith('us-')
          ) {
            return {
              allowed: false,
              reason: `Region ${metadata.region} is not approved. Only US regions are allowed.`,
            };
          }
          return { allowed: true };
        },
  ...
  async evaluateAction(agentName, action, metadata) {
    logger.info(
      `🛡️ PolicyEngine: Evaluating ${agentName} attempting ${action}`,
    );
    const violations = [];
    ...
    for (const policy of this.defaultPolicies) {
      try {
        ...
        if (!result.allowed) {
          violations.push(
            `Violation of ${policy.id} (${policy.name}): ${result.reason}`,
          );
        }
      } catch (e) {
    ...
    if (violations.length > 0) {
      logger.warn(
        `🚫 PolicyEngine: Blocked action by ${agentName}. Violations: ${violations.join(', ')}`,
      );
