...
class CodeBrowserService {
        ...
        try {
            ...
            return stdout.trim().split('\n')
                ...
                .map(line => {
                    ...
                    const status = statusChar === 'A' ? 'CREATED' : statusChar === 'M' ? 'MODIFIED' : statusChar === 'D' ? 'DELETED' : 'CHANGED';
    ...
    async readFile(filePath) {
        ...
        const languageMap = { js: 'javascript', ts: 'typescript', tsx: 'typescript', jsx: 'javascript', py: 'python', md: 'markdown', json: 'json', sh: 'shell', css: 'css', html: 'html' };
    ...
    async applyPatch(filePath, newContent) {
        ...
        const audit = await GuardianService.auditCode(newContent, `human_patch:${path.basename(abs)}`);
        if (!audit.safe) {
