...
codeBrowserRoutes.get('/file', async (req, res) => {
    ...
    if (!filePath) return res.status(400).json({ success: false, message: 'path query param required.' });
    try {
        const result = await codeBrowserService.readFile(filePath);
        return res.json({ success: true, data: result });
    } catch (e) {
        logger.warn(`CodeBrowser /file: ${e.message}`);
        return res.status(e.message.includes('traversal') ? 403 : 404).json({ success: false, message: e.message });
    }
});
...
codeBrowserRoutes.post('/patch', async (req, res) => {
    ...
    try {
        ...
        const status = e.message.includes('Guardian') ? 403 : e.message.includes('traversal') ? 403 : 500;
...
codeBrowserRoutes.post('/suggest', async (req, res) => {
    ...
    if (!code || !instruction) {
        return res.status(400).json({ success: false, message: 'code and instruction required.' });
    }

    const prompt = `You are an expert ${language} engineer providing a concise inline code suggestion.
...
codeBrowserRoutes.post('/exec', async (req, res) => {
    ...
    try {
        ...
        return res.json({ success: true, stdout: stdout.substring(0, 10000), stderr: stderr.substring(0, 2000) });
    } catch (e) {
        return res.json({ success: true, stdout: '', stderr: e.message.substring(0, 2000) });
    }
});
