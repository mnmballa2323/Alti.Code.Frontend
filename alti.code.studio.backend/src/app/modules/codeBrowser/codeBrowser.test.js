...
vi.mock('../../../shared/evolution.service.js', () => {
    return {
        evolutionService: {
            ...
            getStylisticParameters: vi.fn().mockResolvedValue('\nEnforced styles: Use async/await')
        }
    };
});
...
describe('CodeBrowserService Operations', () => {
    ...
    it('should safely apply a human patch, execute Guardian check, backup file, write changes, sweep AST, and commit', async () => {
        ...
        // 1. Audit check
        expect(GuardianService.auditCode).toHaveBeenCalledWith(newContent, 'human_patch:swarm_brain.js');

        // 2. Backup check
        expect(fs.readFile).toHaveBeenCalled();
        expect(fs.writeFile).toHaveBeenCalledWith(expect.stringContaining('bak.'), 'original raw content');

        // 3. Write patch check
        expect(fs.writeFile).toHaveBeenCalledWith(expect.stringContaining('swarm_brain.js'), newContent);
