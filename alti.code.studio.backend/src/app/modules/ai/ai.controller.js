...
const executeTask = catchAsync(async (req, res) => {
    ...
    const sessionData = await AgentMemoryHooks.startSession(userId, 'alti-code-studio', {
        source: 'api',
    ...
    try {
        ...
        AgentMemoryHooks.endSession(userId, `Task completed: ${prompt.substring(0, 200)}`).catch(() => {});
