...
// ALL SUPPORTED PROVIDERS - STRICT SOVEREIGN AZURE INFERENCE STRATEGY ENFORCEMENT
const PROVIDERS = {
  azure_openai: {
    name: 'Azure OpenAI Foundry (GPT)',
    initialize: async () => {
      const { OpenAIClient, AzureKeyCredential } =
...
class AIProvider {
  ...
  async init() {
    ...
    try {
      ...
      logger.warn(
        `⚠️ AIProvider: Failed to init [${provider.name}]: ${err.message}. Trying fallback...`,
      );
  ...
  _selectLocalModel(prompt, options = {}) {
    ...
    const codeKeywords = [
      ...
      'struct ',
      'impl ',
      'refactor the current proposed solution',
      'provide the updated, refactored solution',
  ...
  async reason(prompt, options = {}) {
    ...
    try {
      ...
      for (const conceptId of conceptIds) {
        ...
        if (concept) {
          ...
          if (
            prompt.toLowerCase().includes(conceptName.toLowerCase()) ||
            prompt.toLowerCase().includes(conceptId.toLowerCase())
          ) {
    ...
    const provider = this.providers[this.activeProvider];
    return provider.reason(this.client, scrubbedPrompt, {
      ...options,
      temperature: 0.2,
    });
  }
  ...
  async generate(prompt, options = {}) {
    ...
    const provider = this.providers[this.activeProvider];
    return provider.generate(this.client, scrubbedPrompt, {
      ...options,
      temperature: 0.7,
    });
  }
  ...
  async switchTo(providerName) {
    ...
    logger.info(
      `🧠 AIProvider: Switched to [${this.providers[providerName].name}]`,
    );
  }
  ...
  _scrubCredentials(text) {
    ...
    scrubbed = scrubbed.replace(
      /-----BEGIN[A-Z0-9\s_]+PRIVATE\s+KEY-----[a-zA-Z0-9/+=:\r\n\s]+-----END[A-Z0-9\s_]+PRIVATE\s+KEY-----/g,
    ...
    scrubbed = scrubbed.replace(
      /"private_key"\s*:\s*"[^"]+"/g,
      '"private_key": "[REDACTED_SECURE_CREDENTIAL]"',
    );

    // 3. GCP API Keys
    scrubbed = scrubbed.replace(
      /AIzaSy[A-Za-z0-9_-]{33,40}/g,
      '[REDACTED_SECURE_CREDENTIAL]',
    );

    // 4. General secrets/tokens assignments
    scrubbed = scrubbed.replace(
      /(secret|password|passwd|api_key|apikey|private_key|auth_token|token|credential|oauth_token)\s*([:=]+)\s*["']([^"'\r\n\s]{16,})["']/gi,
      (match, p1, p2, p3) => {
    ...
    scrubbed = scrubbed.replace(
      /(mongodb(?:\+srv)?|postgres|postgresql|mysql|sqlite|redis):\/\/([^:]+):([^@]+)@/g,
      '$1://$2:[REDACTED_SECURE_CREDENTIAL]@',
    );
