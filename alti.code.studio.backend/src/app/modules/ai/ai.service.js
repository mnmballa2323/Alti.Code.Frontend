...
class AiService {
    ...
    async createWorkflow(userId, payload) {
        ...
        if (triggerType === 'schedule') {
            logger.info(`[MOCK GCP SCHEDULER] Creating job for workflow ${workflow._id} with cron: ${rawCron}`);
    ...
    async executeWorkflow(workflowId) {
        ...
        try {
            ...
            try {
                ...
                steps = [
                    { action: 'Analyzed objective', tool: 'reasoning', payload: { intent: workflow.prompt } },
                    { action: 'Executed tools', tool: 'system', payload: { result: 'success', raw: response.substring(0, 100) } }
                ];
            }
            ...
            logger.error(`AiService: executeWorkflow failed for ${workflowId}`, error);
            run.status = 'failed';
    ...
    async getWorkflowRuns(workflowId) {
        return await WorkflowRun.find({ workflowId }).sort({ createdAt: -1 }).limit(50);
    }
}
