...
try {
  OfflineEmbeddingModel =
    mongoose.models.OfflineEmbedding ||
    mongoose.model('OfflineEmbedding', OfflineEmbeddingSchema);
} catch (e) {
  logger.warn(
    `⚠️ [OfflineEmbeddingCache] Mongoose model initialization failed: ${e.message}`,
  );
}
  ...
  static async getEmbedding(text, options = {}) {
    ...
    if (
      ...
      try {
        ...
        if (cached) {
          logger.info(
            `✨ [OfflineEmbeddingCache] Cache HIT (DB) for hash: ${textHash}`,
          );
          return cached.embedding;
        }
      } catch (err) {
        logger.warn(
          `⚠️ [OfflineEmbeddingCache] DB cache read error: ${err.message}`,
        );
      }
    }
    ...
    try {
      if (fs.existsSync(FILE_CACHE_PATH)) {
        ...
        if (fileCache[textHash] && fileCache[textHash].model === model) {
          logger.info(
            `✨ [OfflineEmbeddingCache] Cache HIT (JSON File) for hash: ${textHash}`,
          );
      ...
      logger.warn(
        `⚠️ [OfflineEmbeddingCache] File cache read error: ${err.message}`,
      );
    }
    ...
    try {
      ...
      logger.error(
        `❌ [OfflineEmbeddingCache] Failed to fetch embedding: ${err.message}`,
      );
      throw err;
    }
    ...
    if (
      ...
      try {
        ...
        logger.info(
          `💾 [OfflineEmbeddingCache] Saved embedding to DB for hash: ${textHash}`,
        );
      } catch (err) {
        logger.warn(
          `⚠️ [OfflineEmbeddingCache] Failed to save to DB: ${err.message}`,
        );
      }
    }
    ...
    try {
      ...
      logger.info(
        `💾 [OfflineEmbeddingCache] Saved embedding to JSON File for hash: ${textHash}`,
      );
    } catch (err) {
      logger.warn(
        `⚠️ [OfflineEmbeddingCache] Failed to save to JSON File: ${err.message}`,
      );
    }
