...
export class VercelAiService {
    ...
    static async generate(prompt, options = {}) {
        ...
        try {
            ...
            const usage = {
                promptTokens: response.usage?.promptTokens || Math.max(1, Math.ceil(prompt.length / 4)),
                completionTokens: response.usage?.completionTokens || Math.max(1, Math.ceil(text.length / 4))
            };
