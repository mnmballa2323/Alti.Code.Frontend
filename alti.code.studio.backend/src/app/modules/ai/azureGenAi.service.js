...
const generateContent = async (prompt, modelName = PRIMARY_MODEL, temperature = 0.5) => {
    ...
    const result = await multiCloudInferenceService.executeMultiCloudInference(prompt, 'azure_genai', { modelId: modelName });
    return {
        model: modelName,
    ...
    return {
        generateContent: async (contents) => {
            const prompt = Array.isArray(contents) 
                ? contents.map(c => typeof c === 'string' ? c : JSON.stringify(c)).join('\n') 
                : contents;
            const result = await multiCloudInferenceService.executeMultiCloudInference(prompt, 'azure_genai_model', { modelId: modelName });
            return {
                response: {
                    text: () => result.content
                }
        ...
        startChat: () => {
            return {
                sendMessage: async (msg) => {
                    ...
                    const result = await multiCloudInferenceService.executeMultiCloudInference(prompt, 'azure_genai_chat', { modelId: modelName });
                    return {
                        response: {
                            text: () => result.content,
