...
class AstGraphService {
  ...
  buildGraph(directoryPath) {
    ...
    try {
      ...
      logger.warn(
        'AST Graph: Error adding path files, project might be preloaded.',
        e,
      );
    }
          ...
          if (name) {
            ...
            if (baseClassNode) {
              ...
              if (extendsName) {
                ...
                const currentClassRecord = this.inheritanceMap.get(name) || {
                  extendsClass: null,
                  subclasses: [],
                };
                ...
                const parentClassRecord = this.inheritanceMap.get(
                  extendsName,
                ) || { extendsClass: null, subclasses: [] };
  ...
  findCallers(symbolName) {
    if (!this.isGraphBuilt) {
      logger.warn(
        'AST Graph: findCallers called before graph build. Returning empty.',
      );
      return [];
    }
