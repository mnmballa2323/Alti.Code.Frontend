...
class QualityGateService {
    ...
    calculateComplexity(codeContent) {
        ...
        try {
            ...
            const sourceFile = this.project.createSourceFile('temp_gate_eval.js', codeContent, { overwrite: true });
            ...
            logger.error('QualityGate: Failed to analyze code complexity. Falling back to default baseline.', e);
            return 1;
        }
    }
    ...
    validateQuality(codeContent, maxAllowedComplexity = 15) {
        ...
        logger.info(`🛡️ QualityGate: Code complexity is ${complexity} (Max allowed: ${maxAllowedComplexity}). Gate outcome: ${passed ? 'PASSED' : 'FAILED'}`);
