...
export class AstPatcherService {
  ...
  async applyPatch(sourceCode, patch, langName = 'javascript', intent = null) {
    ...
    if (!nodeQuery || typeof replacementCode !== 'string') {
      throw new Error(
        'AST Patcher: Invalid patch format. Must include nodeQuery and replacementCode.',
      );
    }

    const matches = await treeSitterService.query(
      sourceCode,
      nodeQuery,
      langName,
    );
    if (!matches || matches.length === 0) {
      throw new Error(
        'AST Patcher: No nodes matched the provided tree-sitter query.',
      );
    }
    ...
    if (!targetNode) {
      throw new Error(
        'AST Patcher: Query matched but no @target capture was found.',
      );
    }
    ...
    try {
      ...
      if (tempTree.rootNode.hasError()) {
        throw new Error(
          'Tree-sitter detected syntax errors in the replacement code block.',
        );
      }
    } catch (syntaxErr) {
      logger.error(`AST Patcher: Rejected patch due to syntax error.`);
      throw new Error(
        `AST Patcher: Rejected patch due to syntax error: ${syntaxErr.message}`,
      );
    }
    ...
    logger.info(
      `✨ AST Patcher: Successfully replaced AST node at [${startIndex}:${endIndex}]`,
    );

    // Phase 2 Hive Mind Integration: Memorize this successful patch
    if (intent) {
      knowledgeGraphService
        .embedAstPatch(intent, patch)
        .catch(e => logger.error(`Hive Mind Embed Error: ${e.message}`));
    }
