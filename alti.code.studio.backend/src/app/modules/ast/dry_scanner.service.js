...
class DryScannerService {
  ...
  scanDuplicates(filesData, minLines = 3) {
    logger.info(
      `🔍 DRY Scanner: Auditing ${filesData.length} files for code duplication.`,
    );
    const functions = [];
    ...
    try {
      for (const file of filesData) {
        ...
        for (const node of declarations) {
          ...
          const bodyText =
            typeof node.getBody === 'function'
              ? node.getBody()?.getText() || rawText
              : rawText;
    ...
    for (let i = 0; i < functions.length; i++) {
      for (let j = i + 1; j < functions.length; j++) {
        ...
        const similarity = this._computeSimilarity(
          fnA.normalizedTokens,
          fnB.normalizedTokens,
        );
    ...
    logger.info(
      `🔍 DRY Scanner: Audit complete. Found ${duplicates.length} duplicate patterns.`,
    );
    return duplicates;
  }
  ...
  _tokenizeAndNormalize(text) {
    ...
    const tokens = noComments.toLowerCase().match(/[a-z0-9_]+/g) || [];
