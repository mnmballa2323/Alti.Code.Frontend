...
class OpenClawProxyService {
  constructor() {
    ...
    this.localSurrogateUrl =
      process.env.LOCAL_SURROGATE_URL || 'http://localhost:8081';
  ...
  configureMoltworker(url, token) {
    if (!url) {
      ...
      logger.info(
        `🔌 OpenClaw Tunnel: Reverted target to Localhost Surrogate.`,
      );
    } else {
      this.localSurrogateUrl = url;
      this.moltworkerToken = token;
      logger.info(
        `☁️ OpenClaw Tunnel: Rerouting Swarm Intelligence target to Cloudflare Edge: ${url}`,
      );
    }
  }
  ...
  async delegateToLocalHost(intent, expectedResultDescription) {
    if (process.env.LOCAL_SURROGATE_ENABLED !== 'true') {
      logger.warn(
        `⚠️ OpenClaw Tunnel: Surrogate is disabled. Cannot execute local intent: "${intent}"`,
      );
      return `[SIMULATED LOCAL EXECUTION]: Executed intent on host: ${intent}`;
    }

    logger.info(
      `🚇 OpenClaw Tunnel: Delegating intent to physical host machine...`,
    );
    ...
    try {
      ...
      if (response && response.success) {
        logger.info(
          `✅ OpenClaw Tunnel: Local Surrogate completed the intent successfully.`,
        );
        return response.output;
      } else if (response && response.error) {
        logger.warn(
          `⚠️ OpenClaw Tunnel: Surrogate executed but returned semantic failure: ${response.error}`,
        );
      ...
      logger.error(
        `❌ OpenClaw Tunnel: Communication or Execution with physical host failed: ${error.message}`,
      );
