...
class OpenClawCoreIntegration {
  constructor() {
    ...
    this.clawdinatorPath = path.resolve(
      'submodules',
      'openclaw',
      'clawdinators',
    );
  ...
  async init() {
    if (!fs.existsSync(this.basePath)) {
      logger.warn(
        '[openclaw-core] OpenClaw submodule not found. Deferring initialization.',
      );
      return;
    }
    ...
    agentRegistry.register({
      name: 'openclawCore',
      description:
        'OpenClaw Autonomous Coordination Engine — ACP, Lobster, Skills, ClawhHub',
      prompt:
        'You are the OpenClaw orchestration engine. You dispatch requests via ACP to clawdinators and manage lobster workflows.',
      capabilities: [
        'openclaw',
        'routing',
        'delegation',
    ...
    logger.info(
      `[openclaw-core] Initialized. Skills loaded: ${openClawSkills.skillCount}`,
    );
  }
  ...
  async executeClawdinator(
    ...
    if (!this.isInitialized)
      throw new Error('[openclaw-core] OpenClaw Core not initialized');
    ...
    logger.info(
      `[openclaw-core] Dispatching to Clawdinator via ACP: "${validated.taskPattern}" agent="${validated.agent}"`,
    );
    ...
    if (!(await this.breaker.isAvailable())) {
      throw new Error(
        `[openclaw-core] Circuit Breaker OPEN for ${this.breaker.name}`,
      );
    }
    ...
    try {
      ...
      const contextHint =
        Object.keys(validated.payload).length > 0
      ...
      if (validated.oneShot) {
        ...
        const output = await acpxService.exec(validated.agent, fullPrompt, {
          timeout: 60000,
        });
        result = {
          status: 'success',
        ...
        await acpxService.ensureSession(
          validated.agent,
  ...
  getSkillsSummary() {
    return {
      ...
      bySource: {
        'openclaw/skills':
          openClawSkills.getSkillsBySource('openclaw/skills').length,
        'agent-skills-context': openClawSkills.getSkillsBySource(
