...
class AcpxService {
  ...
  async ensureSession(agent = 'gemini', sessionName = null) {
    ...
    logger.info(
      `[acpx] Session ensured for agent="${agent}" name="${sessionName || 'default'}"`,
    );
    return result;
  }
  ...
  streamPrompt(agent = 'gemini', prompt, onData, opts = {}) {
    ...
    child.stderr.on('data', data =>
      logger.debug(`[acpx stream] ${data.toString().trim()}`),
    );
  ...
  _validateAgent(agent) {
    if (!SUPPORTED_AGENTS.includes(agent)) {
      throw new Error(
        `[acpx] Unsupported agent: "${agent}". Supported: ${SUPPORTED_AGENTS.join(', ')}`,
      );
    }
  }
