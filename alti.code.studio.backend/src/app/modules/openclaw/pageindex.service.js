...
class PageIndexService {
  ...
  async indexDocument(docPath, indexId = null) {
    if (!this.isAvailable()) {
      return {
        status: 'unavailable',
        message: 'PageIndex submodule not initialized.',
      };
    }
  ...
  async query(query, indexId, opts = {}) {
    ...
    logger.info(
      `[pageindex] Querying index "${indexId}": "${query.slice(0, 80)}"`,
    );
    const { stdout } = await execFileAsync('python', args, {
      cwd: this.basePath,
      timeout: 60000,
  ...
  async indexAndQuery(docPath, query, opts = {}) {
    ...
    const indexId =
      indexed.indexId || path.basename(docPath, path.extname(docPath));
  ...
  getCapabilities() {
    return {
      name: 'PageIndex',
      description:
        'Vectorless, reasoning-based RAG — page-level document indexing',
      features: [
