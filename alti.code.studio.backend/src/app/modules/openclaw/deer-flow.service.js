...
class DeerFlowService {
  ...
  isAvailable() {
    if (this._available === null) {
      ...
      const hasPy =
        fs.existsSync(path.join(this.basePath, 'pyproject.toml')) ||
      ...
      if (!this._available) {
        logger.warn(
          '[deer-flow] Submodule not initialized or entry point not found.',
        );
      }
    }
  ...
  async runTask(task, opts = {}) {
    if (!this.isAvailable()) {
      return {
        status: 'unavailable',
        message:
          'deer-flow submodule not initialized. Run: git submodule update --init submodules/deer-flow',
        task,
      };
    }
    ...
    logger.info(
      `[deer-flow] Starting deep research task: "${task.slice(0, 100)}..."`,
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      const args = useNode
        ? [
            path.join(this.basePath, 'src', 'server', 'run_agent_task.js'),
            '--task',
            task,
          ]
      ...
      if (opts.telemetryLevel)
        args.push('--log-level', String(opts.telemetryLevel));
      ...
      const timer = setTimeout(() => {
        ...
        resolve({
          status: 'timeout',
          output,
          message: 'Task exceeded maximum allowed time',
        });
      }, timeout);
      ...
      child.on('close', code => {
        ...
        if (code === 0) {
          ...
          logger.error(
            `[deer-flow] Task failed with code ${code}: ${errOutput.slice(0, 500)}`,
          );
          resolve({
            status: 'error',
            output: output.trim(),
  ...
  streamTask(task, onData, opts = {}) {
    ...
    child.stderr.on('data', d =>
      logger.debug(`[deer-flow stream] ${d.toString().trim()}`),
    );
  ...
  getCapabilities() {
    return {
      name: 'deer-flow',
      description:
        'ByteDance open-source SuperAgent harness — researches, codes, and creates',
      features: [
