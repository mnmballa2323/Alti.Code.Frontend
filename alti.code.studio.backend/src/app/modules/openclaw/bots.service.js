...
class OpenClawBotsService {
    constructor() {
        this.registeredBots = ['flawd-bot', 'butter.bot', 'barnacle', 'lobster', 'trust'];
    ...
    async dispatchBotTask(botName, payload = {}, tenantId = 'system') {
        ...
        logger.info(`Dispatching task to ${validated.botName}: ${JSON.stringify(validated.payload)}`);
        ...
        try {
            const result = await openClawCore.executeClawdinator('community-bot-task', {
