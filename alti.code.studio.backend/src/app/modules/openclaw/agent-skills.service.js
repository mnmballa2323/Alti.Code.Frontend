...
class AgentSkillsService {
  ...
  loadSkills() {
    ...
    if (!fs.existsSync(SKILLS_BASE)) {
      logger.warn(
        '[agent-skills] Submodule not initialized. Run: git submodule update --init',
      );
      return [];
    }
    ...
    this._skills = skills;
    logger.info(
      `[agent-skills] Loaded ${skills.length} context engineering skills`,
    );
    return skills;
  }
  ...
  applyTokenBudget(context, maxTokens = 8000) {
    ...
    logger.debug(
      `[agent-skills] Token budget applied: truncated from ${context.length} to ${maxChars} chars`,
    );
  ...
  get skillCount() {
    return (this._skills || this.loadSkills()).length;
  }
}
