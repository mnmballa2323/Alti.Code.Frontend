...
async function toolBash(command, cwd = process.cwd()) {
    if (isBanned(command)) {
        return { ok: false, error: `Command blocked by safety policy: "${command}"` };
    }
    try {
...
class NanoAgentService {
    ...
    async run(task, opts = {}) {
        ...
        const systemPrompt = opts.systemPrompt || `You are a nano code agent. You have access to these tools:
...
Task: ${task}`;
        ...
        logger.info(`[nano-agent] Starting task: "${task.slice(0, 100)}" (max ${maxIter} iterations)`);
        ...
        while (iterations < maxIter) {
            ...
            history.push({ iteration: iterations, response: response.slice(0, 2000) });
            logger.debug(`[nano-agent] Iteration ${iterations}: ${response.slice(0, 200)}`);

            // Check if done
            const doneMatch = response.match(/^DONE:\s*(.+)/m);
            if (doneMatch) {
                logger.info(`[nano-agent] Completed in ${iterations} iterations: ${doneMatch[1]}`);
            ...
            try {
                ...
                messages.push(`Error: could not parse ARGS JSON. Got:\n${toolMatch[2]}`);
                continue;
            }

            const tool = TOOL_REGISTRY[toolName];
            if (!tool) {
                messages.push(`Error: Unknown tool "${toolName}". Available: ${Object.keys(TOOL_REGISTRY).join(', ')}`);
                continue;
            }
        ...
        logger.warn(`[nano-agent] Reached max iterations (${maxIter}) without completing.`);
