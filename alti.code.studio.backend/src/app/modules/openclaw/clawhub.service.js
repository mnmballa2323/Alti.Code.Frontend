...
function httpsGet(url, timeoutMs = 10000) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, { timeout: timeoutMs }, res => {
      ...
      res.on('end', () => {
        try {
          ...
          reject(new Error(`ClawhHub parse error: ${body.slice(0, 200)}`));
        }
      });
    });
    req.on('error', reject);
    req.on('timeout', () => {
      req.destroy();
      reject(new Error('ClawhHub request timed out'));
    });
  });
}
...
class ClawhubService {
  ...
  async listSkills() {
    ...
    try {
      ...
      this._cacheTs = now;
      logger.info(
        `[clawhub] Fetched ${this._cache.length} skills from ClawhHub`,
      );
      return this._cache;
    } catch (err) {
      logger.warn(
        `[clawhub] Failed to fetch skills: ${err.message}. Using local fallback.`,
      );
  ...
  async getSkillDetails(skillId) {
    try {
      ...
      logger.warn(
        `[clawhub] Could not fetch skill details for "${skillId}": ${err.message}`,
      );
      return null;
    }
  }
  ...
  _localSkillsFallback() {
    ...
    logger.info(
      `[clawhub] Local fallback: found ${skills.length} skills from submodule`,
    );
    return skills;
  }
