    ...
    try {
      ...
      if (!payload || !payload.originalIntent) {
        return res.status(400).json({
          error: 'Malformed OpenClaw payload. "originalIntent" is required.',
        });
      }

      logger.info(
        `📱 OpenClaw Ambassador: Received requirement via ${payload.source || 'Unknown Channel'}`,
      );
...
Synthesize a precise Sprint Goal:`;
      ...
      logger.info(
        `✅ OpenClaw Ambassador: Converted ${payload.source} message into Sprint Goal: ${cleanGoal}`,
      );
      ...
      logger.error(
        `❌ OpenClaw Ambassador: Webhook processing failed: ${error.message}`,
      );
