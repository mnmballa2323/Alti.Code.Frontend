...
class OpenClawSensoryIntegration {
  ...
  async pollSensoryTelemetry() {
    ...
    try {
      ...
      if (randomMetric > 95) {
        logger.warn(
          `OpenClaw Sensory: Abnormal IoT telemetry detected - ${randomMetric.toFixed(2)}`,
        );
  ...
  async processVoiceCommand(
    audioBuffer,
    sourceId = 'voice-community-edge',
    tenantId = 'system',
  ) {
    const validated = audioCommandSchema.parse({
      audioBuffer,
      sourceId,
      tenantId,
    });
    ...
    logger.info(
      `Processing voice command via OpenClaw voice-community integration for ${validated.sourceId}`,
    );
