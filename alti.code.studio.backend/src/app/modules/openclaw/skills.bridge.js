...
class OpenClawSkillsBridge {
                ...
                for (const mdPath of skillMds) {
                    try {
                        ...
                        const description = extractFrontmatter(content, 'description') ||
                            `OpenClaw skill: ${name}`;
                        skills.push({ name, description, source: 'openclaw/skills', path: mdPath });
                    } catch (_) { /* skip bad files */ }
                }
                logger.info(`[skills-bridge] Scanned ${skillMds.length} SKILL.md files from openclaw/skills`);
            } else {
                logger.warn('[skills-bridge] openclaw/skills submodule not initialized.');
            }
            ...
            if (fs.existsSync(this.agentSkillsPath)) {
                ...
                for (const mdPath of agentMds) {
                    try {
                        ...
                        skills.push({ name, description, source: 'agent-skills-context', path: mdPath });
                    } catch (_) { /* skip */ }
                }
                logger.info(`[skills-bridge] Scanned ${agentMds.length} SKILL.md files from agent-skills-context`);
            }
    ...
    get skillCount() { return this.availableSkills.length; }
