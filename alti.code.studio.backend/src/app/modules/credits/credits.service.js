...
const INPUT_COST_PER_M = parseFloat(
  process.env.GEMINI_INPUT_COST_PER_M || '3.50',
);
const OUTPUT_COST_PER_M = parseFloat(
  process.env.GEMINI_OUTPUT_COST_PER_M || '10.50',
);
...
class CreditTrackingService {
  ...
  record(sessionId, usageMetadata = {}) {
    ...
    logger.debug(
      `💰 Credits: session=${sessionId} +${total} tokens (+$${costUsd.toFixed(6)})`,
    );
