...
class GooseRouterService {
  ...
  async executeTask(prompt, context = [], onProgress = null) {
    ...
    logger.info(
      `🔍 GooseRouter: Generating dynamic workspace structural map...`,
    );
    ...
    logger.info(
      `🔍 GooseRouter: Scanning baseline diagnostics before execution...`,
    );
    const baseline = await this.verifyChanges();
    logger.info(
      `🔍 GooseRouter: Baseline diagnostics scanned. Found ${baseline.errors.length} pre-existing warning(s).`,
    );
    ...
    while (attempt <= maxAttempts) {
      try {
        logger.info(
          `🤖 GooseRouter: Executing Goose Task - Attempt ${attempt}/${maxAttempts}...`,
        );
        if (onProgress)
          onProgress({
            status: 'executing',
            message: `Goose executing task (Attempt ${attempt}/${maxAttempts})`,
          });

        const output = await this._runGooseCli(
          currentPrompt,
          enrichedContext,
          onProgress,
        );
        ...
        logger.info(
          `🔍 GooseRouter: Running Self-Healing Quality Verification Gates...`,
        );
        if (onProgress)
          onProgress({
            status: 'verifying',
        ...
        if (newErrors.length === 0) {
          logger.info(
            `✅ GooseRouter: All Self-Healing checks passed successfully.`,
          );
        ...
        logger.warn(
          `🛑 GooseRouter: Verification gates failed on Attempt ${attempt}/${maxAttempts}.`,
        );
...
=======================================`;
        ...
        logger.error(
          `❌ GooseRouter: Task execution failed on Attempt ${attempt}/${maxAttempts}: ${error.message}`,
        );
        if (attempt === maxAttempts) {
          throw error;
        }
  ...
  async _runGooseCli(prompt, context = [], onProgress = null) {
    return new Promise((resolve, reject) => {
      ...
      const gooseProcess = spawn(
        this.goosePath,
        ['run', '--no-profile', '-q', '--no-session', '-t', fullPrompt],
        {
      ...
      gooseProcess.on('close', code => {
        if (code === 0) {
          ...
          reject(
            new Error(
              `Goose execution failed (code ${code}):\n${stderr || stdout}`,
            ),
          );
        }
      });
