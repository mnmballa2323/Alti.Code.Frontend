...
class GoogleGenkitService {
  ...
  initializeFlows() {
    logger.info(
      '🌊 [Genkit] Initializing strongly-typed AI Orchestration Flows...',
    );
    ...
    this.codeAuditFlow = this.ai.defineFlow(
      ...
      async input => {
        logger.info(
          `🌊 [Genkit Flow] Executing Autonomous Code Audit (Strictness: ${input.strictnessLevel})`,
        );
        ...
        logger.info(
          `✅ [Genkit Flow] Audit complete. Score: ${output?.securityScore}`,
        );
        return output;
      },
    );
