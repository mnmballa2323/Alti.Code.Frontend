...
collaborationRoutes.post('/session/create', (req, res) => {
    ...
    if (!sessionId || !ownerId) {
        return res.status(400).json({ success: false, message: 'sessionId and ownerId are required.' });
    }
    try {
...
collaborationRoutes.post('/session/join', (req, res) => {
    ...
    if (!sessionId || !userId || !collabToken) {
        return res.status(400).json({ success: false, message: 'sessionId, userId, and collabToken are required.' });
    }
    const result = collaborationService.joinSession(sessionId, userId, collabToken);
    if (!result.success) return res.status(403).json({ success: false, message: result.reason });
...
collaborationRoutes.post('/session/:id/context', (req, res) => {
    ...
    const item = collaborationService.appendContext(req.params.id, userId, { role, content, agentName });
    if (!item) return res.status(404).json({ success: false, message: 'Session not found.' });
