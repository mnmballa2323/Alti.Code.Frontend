...
export class CollaborationService {
  constructor() {
    ...
    logger.info(
      '🤝 CollaborationService v6.1.0: Real-Time Collaboration initialized.',
    );
  }
  ...
  createCollabSession(sessionId, ownerId) {
    if (sessions.has(sessionId)) {
      ...
      return {
        sessionId,
        collabToken: session.collabToken,
    ...
    logger.info(
      `🤝 Collab: Session created — ${sessionId} (owner: ${ownerId})`,
    );
  ...
  joinSession(sessionId, userId, collabToken) {
    ...
    if (session.collabToken !== collabToken)
      return { success: false, reason: 'invalid_token' };

    session.members.add(userId);
    this._updatePresence(sessionId, userId, {
      status: 'active',
      joinedAt: new Date().toISOString(),
    });

    this._broadcast(sessionId, 'collab:user_joined', {
      userId,
      members: [...session.members],
    });
    logger.info(
      `🤝 Collab: ${userId} joined session ${sessionId} (${session.members.size} members)`,
    );
  ...
  leaveSession(sessionId, userId) {
    ...
    this._broadcast(sessionId, 'collab:user_left', {
      userId,
      members: [...session.members],
    });
      ...
      setTimeout(
        () => {
          if (sessions.get(sessionId)?.members.size === 0) {
            ...
            logger.info(
              `🤝 Collab: Session ${sessionId} cleaned up (no members).`,
            );
          }
        },
        5 * 60 * 1000,
  ...
  updatePresence(sessionId, userId, presenceData) {
    ...
    this._broadcast(sessionId, 'collab:presence_update', {
      userId,
  ...
  getPresence(sessionId) {
    ...
    return [...presenceMap.entries()].map(([userId, data]) => ({
      userId,
      ...data,
    }));
  }
  ...
  broadcastAgentToken(sessionId, agentName, token, done = false) {
    this._broadcast(
      sessionId,
      done ? 'collab:agent:done' : 'collab:agent:token',
      {
        agentName,
        token,
        done,
  ...
  _registerSocketHandlers(io) {
    io.on('connection', socket => {
      ...
      socket.on('collab:message', ({ sessionId, userId, message }) => {
        const item = this.appendContext(sessionId, userId, {
          role: 'user',
          content: message,
        });
