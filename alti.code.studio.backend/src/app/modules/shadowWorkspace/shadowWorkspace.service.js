...
const initShadow = async sessionId => {
  ...
  logger.info(
    `🌑 ShadowWorkspace: Initializing session "${sessionId}" at ${shadowPath}`,
  );

  await fs.mkdir(shadowPath, { recursive: true });

  try {
    await fs.cp(path.join(sourcePath, 'src'), path.join(shadowPath, 'src'), {
      recursive: true,
    });
    await fs.copyFile(
      path.join(sourcePath, 'package.json'),
      path.join(shadowPath, 'package.json'),
    );
...
const applyDiff = async (sessionId, filePath, content) => {
  ...
  if (!targetFile.startsWith(shadowPath + path.sep)) {
    throw new Error(
      `ShadowWorkspace: path traversal attempt blocked — "${filePath}"`,
    );
  }

  logger.info(
    `🌑 ShadowWorkspace: Applying change to ${path.relative(shadowPath, targetFile)}`,
  );
...
const runCommand = async (sessionId, command) => {
  ...
  if (!isAllowed) {
    logger.warn(
      `🌑 ShadowWorkspace: Rejected disallowed command: "${command}"`,
    );
    return {
      stdout: '',
  ...
  logger.info(
    `🌑 ShadowWorkspace: Running "${command}" in shadow session "${sessionId}"`,
  );
...
const commitShadow = async (sessionId, confirm = false) => {
  if (!confirm) {
    throw new Error(
      'ShadowWorkspace: commitShadow requires confirm=true to prevent accidental overwrites.',
    );
  }
  ...
  logger.info(
    `🌑 ShadowWorkspace: Committing session "${sessionId}" → main workspace`,
  );

  await fs.cp(path.join(shadowPath, 'src'), path.join(sourcePath, 'src'), {
    recursive: true,
    force: true,
  });
