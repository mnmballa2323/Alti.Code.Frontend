...
const igniteProductGenesis = async (userId, sessionId, rawIdea) => {
    logger.info(`🚀 [Genesis Manager] Igniting product genesis for idea: "${rawIdea}"`);
...
Return ONLY the raw markdown of the PRD.`;
        ...
        const prdResult = await LlmGatewayService.routeCompletion(
            userId, sessionId, prdPrompt, 'claude-3-5-sonnet', 'Genesis-PRD'
        );

        const generatedPRD = prdResult.reply.replace(/```markdown/g, '').replace(/```/g, '');
        logger.info(`📝 [Genesis Manager] PRD generation complete. Storing in project workspace...`);
        ...
        logger.warn(`🐝 [Genesis Manager] Handoff initiated. The Hive is waking up to build the product...`);
        const buildCommand = `Execute this PRD flawlessly: \n\n${generatedPRD}`;
        
        // This is where business meets engineering natively
        const finalAppCode = await HiveOrchestratorService.executeSwarm(userId, sessionId, buildCommand);
        ...
        logger.error(`❌ [Genesis Manager] Product genesis critically failed:`, error);
        throw error;
    }
};
