...
class WatchmanController {
  ...
  async receiveSignal(req, res) {
    ...
    if (!source || typeof source !== 'string') {
      return res
        .status(400)
        .json({ status: 'ERR', message: 'source param is required' });
    }
    ...
    try {
      ...
      switch (source.toLowerCase()) {
        ...
        default:
          logger.warn(
            `👀 Watchman: Unknown source [${source}]. Processing as generic signal.`,
          );
          mission = {
            agent: 'surfer',
            action: 'research',
      ...
      if (mission) {
        ...
        logger.info(
          `👀 Watchman: Dispatching mission [${traceId}] for agent [${mission.agent}]...`,
        );
      ...
      return res.status(200).json({
        status: 'IGNORED',
        message: 'Signal ignored — no matching handler.',
      });
    } catch (error) {
