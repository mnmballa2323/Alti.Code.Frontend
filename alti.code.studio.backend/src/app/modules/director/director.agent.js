...
class DirectorAgent {
  ...
  async startMission(goal, context = {}) {
    ...
    logger.info(
      `🎬 Director: Action! Starting Mission ${missionId}: "${goal}"`,
    );
  ...
  async decomposeGoal(mission) {
    ...
    try {
      ...
      logger.info(
        `🎬 Director: Decomposed into ${mission.tasks.length} tasks.`,
      );
    } catch (error) {
  ...
  async executeMission(mission) {
    ...
    for (const task of mission.tasks) {
      ...
      logger.info(
        `🎬 Director: Assigning task to ${task.agent}: "${task.task}"`,
      );
      ...
      try {
        ...
        const mappedName =
          agentMap[task.agent] ||
          task.agent.toLowerCase().replace(/[^a-z0-9]/g, '');
        const agentDef = agentRegistry.get(mappedName);

        if (
          agentDef &&
          agentDef.instance &&
          typeof agentDef.instance.consult === 'function'
        ) {
          logger.info(
            `🎬 Director: Activating real specialist ${agentDef.name} for task: "${task.task}"`,
          );
          ...
          const consultResult = await agentDef.instance.consult(
            task.task,
            contextData,
          );
          ...
          task.result =
            typeof consultResult === 'object' && consultResult.content
              ? consultResult.content
              : consultResult;
          mission.results[task.agent] = task.result;
        } else {
          logger.warn(
            `🎬 Director: Specialist agent ${task.agent} (mapped: ${mappedName}) not found or has no active instance. Falling back to high-fidelity AI Gateway emulation...`,
          );
