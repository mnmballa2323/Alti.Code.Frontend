...
class CiceroLawEnforcementService {
  ...
  async enforceSlaCompliance(userId, tenantId, payload) {
    logger.info(
      `⚖️ Cicero Compliance: Queueing SLA compliance enforcement job for contract [${payload?.contractId || 'unknown'}]`,
    );

    if (!payload || !payload.slaConditions || !payload.telemetry) {
      throw new Error(
        'SLA conditions and telemetry data are required for enforcement.',
      );
    }
    ...
    return {
      ...
      jobId: job.id,
      message:
        'SLA compliance enforcement task has been queued asynchronously.',
    };
  }
  ...
  async processSlaEnforcementJob(jobData) {
    ...
    logger.info(
      `⚖️ Cicero Compliance: Executing background SLA compliance check for contract [${payload?.contractId || 'unknown'}]`,
    );
    ...
    if (
      ...
      slaConditions.minUptime !== undefined
    ) {
      if (telemetry.uptime < slaConditions.minUptime) {
        violations.push(
          `Uptime breach: actual ${telemetry.uptime} < target SLA minUptime ${slaConditions.minUptime}`,
        );
        severity = 'HIGH';
      }
    }
    ...
    if (
      telemetry.avgLatencyMs !== undefined &&
      slaConditions.maxLatencyMs !== undefined
    ) {
      if (telemetry.avgLatencyMs > slaConditions.maxLatencyMs) {
        violations.push(
          `Latency breach: actual ${telemetry.avgLatencyMs}ms > target SLA maxLatencyMs ${slaConditions.maxLatencyMs}ms`,
        );
        if (severity !== 'HIGH') {
          severity = 'MEDIUM';
        }
    ...
    if (
      telemetry.requestCount !== undefined &&
      slaConditions.rateLimit !== undefined
    ) {
      if (telemetry.requestCount > slaConditions.rateLimit) {
        violations.push(
          `Rate limit breach: actual requestCount ${telemetry.requestCount} > SLA limit ${slaConditions.rateLimit}`,
        );
        if (severity === 'LOW') {
          severity = 'MEDIUM';
        }
    ...
    if (violations.length === 0) {
      logger.info(
        `✅ Cicero Compliance: Contract [${contractId}] is fully SLA-compliant.`,
      );
      return {
    ...
    logger.warn(
      `⚠️ Cicero Compliance: SLA breach detected for contract [${contractId}]! Violations: ${violations.join('; ')}`,
    );
...
You MUST return a JSON response matching the required envelope schema:
{
  ...
  "gcp_routing_metadata": {}
    ...
    const agentResponse = await ciceroLawEnforcementAgent.consult(
      prompt,
      context,
      tenantId,
    );
    ...
    try {
      ...
      logger.warn(
        'Failed to parse Cicero Agent response as JSON. Wrapping response in structured envelope.',
        error,
      );
      result = {
        breach_detected: true,
    ...
    result.breach_detected =
      result.breach_detected !== undefined ? result.breach_detected : true;
    ...
    const dispatchResult = await gcpLegalNoticeService.dispatchNotice(
      result.legal_notice_draft,
      dispatchMetadata,
    );

    // Merge routing metadata into envelope payload
    result.gcp_routing_metadata = dispatchResult;
