...
class ClawCodeRouterService {
  constructor() {
    this.clawPath = path.join(
      process.cwd(),
      'src/app/modules/clawCode/bin/claw',
    );
  }
  ...
  async verifyChanges() {
    ...
    for (const file of modifiedFiles) {
      ...
      if (file.endsWith('.prisma')) {
        logger.info(
          `🔍 QualityGate: Running Prisma schema validation check on ${relativePath}...`,
        );
        const prismaResult = await runCommand('npx prisma validate', cwd);
        if (!prismaResult.success) {
          errors.push(
            `[Prisma Validation Error in ${relativePath}]:\n${prismaResult.stdout || prismaResult.stderr}`,
          );
        }
      }
      ...
      if (
        ...
        if (fs.existsSync(eslintPath)) {
          logger.info(
            `🔍 QualityGate: Running ESLint check on ${relativePath}...`,
          );
          const lintResult = await runCommand(
            `npx eslint ${relativePath}`,
            cwd,
          );
          if (!lintResult.success) {
            errors.push(
              `[ESLint Error in ${relativePath}]:\n${lintResult.stdout || lintResult.stderr}`,
            );
          }
        }
      }
      ...
      if (isFrontend && (file.endsWith('.ts') || file.endsWith('.tsx'))) {
        ...
        if (fs.existsSync(tscPath)) {
          logger.info(
            `🔍 QualityGate: Running TypeScript compiler checks on frontend...`,
          );
          ...
          if (!tscResult.success) {
            ...
            if (tscOutput.includes(baseName)) {
              const fileErrors = tscOutput
                .split('\n')
                .filter(line => line.includes(baseName))
                .join('\n');
              errors.push(
                `[TypeScript Compiler Error in ${baseName}]:\n${fileErrors}`,
              );
            }
          }
        }
      ...
      if (file.endsWith('.js') || file.endsWith('.ts')) {
        ...
        const candidates = [
          ...
          path.join(
            path.dirname(path.join(cwd, relativePath)),
        ...
        for (const candidate of candidates) {
          if (fs.existsSync(candidate)) {
            logger.info(
              `🔍 QualityGate: Running matching Vitest regression test suite: ${path.basename(candidate)}...`,
            );
            ...
            if (!testResult.success) {
              errors.push(
                `[Vitest Regression Test Failure in ${path.basename(candidate)}]:\n${testResult.stdout || testResult.stderr}`,
              );
            }
  ...
  async executeTask(prompt, context = [], onProgress = null) {
    ...
    logger.info(
      `🔍 ClawCodeRouter: Generating dynamic workspace structural map...`,
    );
    ...
    logger.info(
      `🔍 ClawCodeRouter: Scanning baseline diagnostics before execution...`,
    );
    const baseline = await this.verifyChanges();
    logger.info(
      `🔍 ClawCodeRouter: Baseline diagnostics scanned. Found ${baseline.errors.length} pre-existing warning(s).`,
    );
    ...
    while (attempt <= maxAttempts) {
      try {
        logger.info(
          `🤖 ClawCodeRouter: Executing Claw-Code Task - Attempt ${attempt}/${maxAttempts}...`,
        );
        if (onProgress)
          onProgress({
            status: 'executing',
            message: `Claw-Code executing task (Attempt ${attempt}/${maxAttempts})`,
          });

        const output = await this._runClawCli(
          currentPrompt,
          enrichedContext,
          onProgress,
        );
        ...
        logger.info(
          `🔍 ClawCodeRouter: Running Self-Healing Quality Verification Gates...`,
        );
        if (onProgress)
          onProgress({
            status: 'verifying',
            message: 'Verifying code changes...',
          });

        const verification = await this.verifyChanges();

        // Filter errors to find only NEW errors introduced by this run
        const newErrors = verification.errors.filter(
          err => !baseline.errors.includes(err),
        );

        if (newErrors.length === 0) {
          logger.info(
            `✅ ClawCodeRouter: All Self-Healing checks passed successfully.`,
          );
        ...
        logger.warn(
          `🛑 ClawCodeRouter: Verification gates failed on Attempt ${attempt}/${maxAttempts}.`,
        );
        logger.warn(`New errors detected:\n${newErrors.join('\n')}`);

        if (attempt === maxAttempts) {
          throw new Error(
            `Self-Healing checks failed after ${maxAttempts} attempts:\n${newErrors.join('\n')}`,
          );
        }
...
=======================================`;
        ...
        logger.error(
          `❌ ClawCodeRouter: Task execution failed on Attempt ${attempt}/${maxAttempts}: ${error.message}`,
        );
        if (attempt === maxAttempts) {
          throw error;
        }
  ...
  async _runClawCli(prompt, context = [], onProgress = null) {
    return new Promise((resolve, reject) => {
      ...
      const clawProcess = spawn(
        this.clawPath,
        ['--skip-permissions', '--cwd', workspaceRoot, 'prompt', fullPrompt],
        {
          cwd: workspaceRoot,
          env,
        },
      ...
      clawProcess.on('close', code => {
        if (code === 0) {
          ...
          reject(
            new Error(
              `Claw-Code execution failed (code ${code}):\n${stderr || stdout}`,
            ),
          );
        }
      });
