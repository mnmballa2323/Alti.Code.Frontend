...
class AutoscalerService {
  ...
  async evaluateScaleEvent(serviceName, currentLatencyMs, errorRate) {
    ...
    logger.info(
      `⚖️ Zero-Touch DevOps: Evaluating scale event for ${serviceName}...`,
    );
    ...
    try {
      ...
      if (this.currentMonthlyRunRate > this.maxMonthlyBudgetUsd * 0.9) {
        logger.warn(
          `💸 Zero-Touch DevOps: Critical budget threshold reached ($${this.currentMonthlyRunRate} / $${this.maxMonthlyBudgetUsd}). Forcing scale down to preserve capital.`,
        );
      ...
      if (errorRate > 0.05 || currentLatencyMs > 2000) {
        ...
        const targetReplicas = Math.min(
          currentReplicas + 2,
          Math.floor(this.maxMonthlyBudgetUsd / 500),
        ); // Arbitrary max based on budget
        if (currentReplicas < targetReplicas) {
          logger.info(
            `📈 Zero-Touch DevOps: High load detected latency=${currentLatencyMs}ms, errorRate=${(errorRate * 100).toFixed(1)}%. Scaling UP to ${targetReplicas} replicas.`,
          );
        ...
        logger.info(
          `📉 Zero-Touch DevOps: Low load detected latency=${currentLatencyMs}ms. Scaling DOWN to save compute cost.`,
        );
        await this.scaleDeployment(serviceName, currentReplicas - 1);
      }
    } catch (error) {
      logger.error(
        `❌ Zero-Touch DevOps: Autoscaler evaluation failed: ${error.message}`,
      );
    } finally {
      this.isScaling = false;
    }
  ...
  async getCurrentReplicas(serviceName) {
    ...
    try {
      const { stdout } = await execAsync(
        `kubectl get deployment ${serviceName} -n ${this.namespace} -o=jsonpath='{.spec.replicas}'`,
      );
  ...
  async scaleDeployment(serviceName, replicas) {
    if (process.env.KUBECTL_AUTORUN !== 'true') {
      logger.warn(
        `⚠️ Zero-Touch DevOps: KUBECTL_AUTORUN disabled. Simulated scaling ${serviceName} to ${replicas} replicas.`,
      );
      return;
    }
    ...
    try {
      await execAsync(
        `kubectl scale deployment ${serviceName} --replicas=${replicas} -n ${this.namespace}`,
      );
      logger.info(
        `✅ Zero-Touch DevOps: Successfully scaled ${serviceName} to ${replicas} replicas.`,
      );
    } catch (error) {
      logger.error(
        `❌ Zero-Touch DevOps: Failed to scale deployment ${serviceName}: ${error.message}`,
      );
    }
  }
}
