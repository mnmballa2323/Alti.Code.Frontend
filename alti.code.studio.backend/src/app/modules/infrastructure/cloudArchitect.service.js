 ...
 */
const provisionInfrastructure = async (userId, sessionId, microserviceCode, executeDeployment = false) => {
    logger.info(`🏗️ [Cloud Architect] Starting autonomous infrastructure provisioning...`);
...
${microserviceCode.substring(0, 50000)}`;
        ...
        const tfResult = await LlmGatewayService.routeCompletion(
            userId, sessionId, tfPrompt, 'claude-3-5-sonnet', 'CloudArchitect-Generate'
        );

        let terraformCode = tfResult.reply.replace(/```hcl/g, '').replace(/```terraform/g, '').replace(/```/g, '');
        logger.info(`✅ [Cloud Architect] Terraform infrastructure code generated successfully.`);

        if (!executeDeployment) {
            logger.info(`⚠️ [Cloud Architect] Dry-Run mode. Returning Terraform code without executing.`);
            return { status: 'plan_ready', terraform: terraformCode };
        }

        // Step 2: Sandbox Deployment Execution
        logger.warn(`🔥 [Cloud Architect] Execution Mode! Initializing Terraform...`);
        const tfDir = path.join(process.cwd(), '.terraform-sandbox', Date.now().toString());
        ...
        logger.info(`✅ [Cloud Architect] Infrastructure provisioned successfully!`);
