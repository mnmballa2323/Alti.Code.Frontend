...
class BroadcasterAgent {
  ...
  async sendSlack(message, channel) {
    ...
    if (!this.config.slackWebhook) {
      logger.warn(
        '📢 Broadcaster: No Slack Webhook configured. Mocking success.',
      );
  ...
  async sendDiscord(message) {
    ...
    if (!this.config.discordWebhook) {
      logger.warn(
        '📢 Broadcaster: No Discord Webhook configured. Mocking success.',
      );
  ...
  async sendEmail(to, subject, body) {
    logger.info(
      `📢 Broadcaster: Sending Email to ${to}... Subject: "${subject}"`,
    );
