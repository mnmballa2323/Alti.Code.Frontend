...
const sendSlack = async (message) => {
    ...
    logger.info(`📢 BroadcasterService: Sending Slack message (${message.length} chars)`);
    return await broadcasterAgent.sendSlack(message);
};

const sendDiscord = async (message) => {
    validateMessage(message);
    logger.info(`📢 BroadcasterService: Sending Discord message (${message.length} chars)`);
    return await broadcasterAgent.sendDiscord(message);
};

const sendEmail = async (to, subject, body) => {
    if (!to || typeof to !== 'string') throw new Error('BroadcasterService: to must be a string.');
    if (!subject || typeof subject !== 'string') throw new Error('BroadcasterService: subject must be a string.');
...
const broadcastAlert = async (alert) => {
    ...
    logger.info(`🚨 BroadcasterService: Broadcasting alert — ${alert.title ?? alert.message ?? JSON.stringify(alert).slice(0, 80)}`);
