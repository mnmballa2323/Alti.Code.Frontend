 ...
 * Plain object grows forever; Map allows O(1) deletion of oldest key.
 */
...
const claudeResponseService = async (
  ...
  try {
    ...
    const reply = completion.reply || 'No response generated by Claude.';

    // Payment and usage limit tracking (Active in Production)
...
const deleteAllAiSessionsService = async userId => {
  ...
  try {
    ...
    session.endSession();
    logger.error('DyadService: deleteAllAiSessions failed', error);
