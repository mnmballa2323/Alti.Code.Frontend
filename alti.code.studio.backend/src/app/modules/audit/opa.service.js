...
class OPAEnforcementService {
  constructor() {
    ...
    this.opaUrl =
      config.opa?.url || 'http://localhost:8181/v1/data/alti/audit/allow';
    ...
    try {
      ...
      if (response.data && response.data.result === true) {
        ...
        return {
          allowed: false,
          reason:
            response.data?.explain ||
            'Action implicitly denied by enterprise OPA firewall.',
        };
      }
    } catch (error) {
      ...
      logger.error(
        `❌ OPAService: Policy evaluation failed for ${actor}:${action}`,
        error.message,
      );
      return {
        allowed: false,
        reason: 'OPA evaluation timeout or unreachable. Failing closed.',
      };
    }
  }
}
