...
class QuotaMeter {
  ...
  getUsageDashboard(tenantId) {
    ...
    for (const f of fields) {
      ...
      dashboard.resources[f.key] = {
        ...
        percentUsed:
          limit !== Infinity ? Math.round((current / limit) * 100) : 0,
        status:
          limit === Infinity
            ? 'UNLIMITED'
            : current >= limit
  ...
  generateInvoice(tenantId) {
    ...
    const tokenOverage = Math.max(
      0,
      usage.current.tokensUsed - usage.quotas.tokensPerMonth,
    );
    const storageOverage = Math.max(
      0,
      usage.current.storageUsedGb - usage.quotas.storageGb,
    );
    const apiOverage = Math.max(
      0,
      usage.current.apiCalls -
        (usage.quotas.apiCallsPerMonth === Infinity
          ? Infinity
          : usage.quotas.apiCallsPerMonth),
    );

    const lineItems = [
      {
        description: `${usage.plan.charAt(0).toUpperCase() + usage.plan.slice(1)} Plan - Base`,
        amount: rate.base,
      },
    ];
    ...
    if (tokenOverage > 0) {
      lineItems.push({
        ...
        amount: Math.round(tokenOverage * rate.tokenRate * 100) / 100,
      });
    }
    if (storageOverage > 0) {
      lineItems.push({
        description: `Storage Overage (${storageOverage} GB)`,
        amount: Math.round(storageOverage * rate.storageRate * 100) / 100,
      });
    }
    if (apiOverage > 0 && isFinite(apiOverage)) {
      lineItems.push({
        description: `API Call Overage (${apiOverage.toLocaleString()} calls)`,
        amount: Math.round(apiOverage * rate.apiRate * 100) / 100,
      });
    }
  ...
  forecast(tenantId) {
    ...
    // Prevent division by zero or extremely small floats creating Infinity
    const progress = Math.max(0.0001, elapsed / total);

    if (progress <= 0) return null;

    const projections = {};
    const fields = {
      tokensUsed: 'tokensPerMonth',
    ...
    for (const [field, quota] of Object.entries(fields)) {
      ...
      projections[field] = {
        ...
        projectedOverage:
          limit !== Infinity ? Math.max(0, projected - limit) : 0,
      };
    }

    return {
      tenantId,
      periodProgress: `${Math.round(progress * 100)}%`,
      projections,
    };
  }
  ...
  _triggerOverage(tenantId, resource, current, limit) {
    ...
    logger.warn(
      `🚨 Quota exceeded: ${tenantId} ${resource} ${current}/${limit}`,
    );
  }
