...
class AdminDashboardController {
    ...
    getAgentFleet() {
        const sdkStats = agentSDK.getStats();
        const customAgents = agentSDK.listCustomAgents ? agentSDK.listCustomAgents() : [];
    ...
    _getAgentCategories() {
        return {
            code: { label: 'Code Quality', count: 25, examples: ['codeReview', 'debugAgent', 'refactorAgent'] },
            security: { label: 'Security', count: 15, examples: ['securityAgent', 'guardianAgent', 'dlpAgent'] },
            devops: { label: 'DevOps', count: 18, examples: ['devOpsAgent', 'ciCdAgent', 'iacAgent'] },
            ai: { label: 'AI/ML', count: 12, examples: ['geminiAgent', 'claudeAgent', 'ollamaAgent'] },
            general: { label: 'General', count: 20, examples: ['chatAgent', 'docsAgent', 'searchAgent'] },
            enterprise: { label: 'Enterprise', count: 25, examples: ['complianceAgent', 'auditAgent', 'billingAgent'] },
        };
    }
    ...
    getTenantAnalytics(tenantId) {
        ...
        const compliance = complianceEngine.generateReport(tenantId, ['sox', 'hipaa', 'pci-dss', 'gdpr']);
    ...
    _generateRecommendations(overview, sla) {
        ...
        if (overview.infrastructure.clusters.totalClusters < 2) {
            recs.push({
                ...
                message: 'Running on single cluster. Deploy to at least 2 regions for HA.',
            });
        }
        ...
        if (recs.length === 0) {
            recs.push({
                severity: 'INFO',
                category: 'GENERAL',
                message: 'System is operating within all target parameters. No action required.',
            });
        }
    ...
    getCapacityProjections() {
        ...
        const dispatchesPerHour = uptime > 0 ? Math.round(bridgeStats.totalDispatches / (uptime / 3600)) : 0;
        const tokensPerHour = uptime > 0 ? Math.round(bridgeStats.totalTokens / (uptime / 3600)) : 0;
    ...
    _getCapacityRecommendations(dph) {
        if (dph > 10000) return [{ action: 'SCALE_UP', message: 'Consider adding worker nodes. >10K dispatches/hour.' }];
        if (dph > 5000) return [{ action: 'MONITOR', message: 'Approaching capacity threshold. Monitor closely.' }];
    ...
    _countMetrics() {
        ...
        return Object.keys(json.counters).length +
