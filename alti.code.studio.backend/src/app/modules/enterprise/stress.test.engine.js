...
class StressTestEngine {
  ...
  async runSuite(suiteName) {
    ...
    const memGrowthMB =
      Math.round(((endMem - startMem) / 1024 / 1024) * 100) / 100;
    ...
    if (passed) this.stats.totalPassed++;
    else this.stats.totalFailed++;

    logger.info(
      `🏋️ Stress: ${suiteName} — ${passed ? '✅ PASS' : '❌ FAIL'} | ${throughput} ops/s | p99=${p99}ms`,
    );
    return result;
  }
      ...
      this.registerSuite(
        ...
        async i => {
          const def = workflowEngine.defineWorkflow({
            name: `stress-wf-${i}`,
            steps: [
              { id: `s${i}`, type: 'agent', config: { agentName: 'stress' } },
            ],
          });
        ...
        i => {
          const pipe = dataPipeline.definePipeline({
            ...
            stages: [
              {
                type: 'filter',
                config: { field: 'score', operator: 'gte', value: 50 },
              },
            ],
          });
