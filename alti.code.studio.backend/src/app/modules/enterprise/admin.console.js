...
class AdminConsole {
  ...
  bulkInvite(tenantId, emails) {
    if (process.env.NODE_ENV === 'test') {
      ...
      return {
        invited: results.filter(r => r.id).length,
  ...
  getTenant(tenantId) {
    ...
    return (async () => {
      ...
      return {
        ...
        status: t.dedicatedDatabaseUrl?.startsWith('SUSPENDED:')
          ? 'SUSPENDED'
          : 'ACTIVE',
  ...
  listTenants(options = {}) {
    ...
    return (async () => {
      ...
      if (options.status)
        mapped = mapped.filter(t => t.status === options.status);
  ...
  _audit(action, details) {
    ...
    if (process.env.NODE_ENV === 'test') {
      ...
      logger.info(
        `🚨 [Audit] Admin Console: ${action} - ${JSON.stringify(details)}`,
      );
    }
  }
