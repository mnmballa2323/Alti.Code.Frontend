...
describe('Phase 51: Agent Marketplace', () => {
  ...
  it('should install and uninstall agent', async () => {
    const { agentMarketplace } = await import('./agent.marketplace.js');
    const pub = agentMarketplace.publish({
  ...
  it('should publish new version', async () => {
    ...
    const updated = agentMarketplace.publishVersion(
      pub.listingId,
      '2.0.0',
      'Major update',
    );
  ...
  it('should enforce dependencies on install', async () => {
    const { agentMarketplace } = await import('./agent.marketplace.js');
    const dep = agentMarketplace.publish({
      name: 'Base Agent',
      publisherId: 'pub',
    });
    ...
    expect(() => agentMarketplace.install('newTenant', ext.id)).toThrow(
      /Missing dependency/,
    );
  });
  ...
  it('should provide marketplace analytics', async () => {
    ...
    expect(analytics.revenueShare.publisher).toBe(0.7);
  });
...
describe('Phase 52: Cron Engine', () => {
  ...
  it('should validate cron expressions', async () => {
    const { cronEngine } = await import('./cron.engine.js');
    expect(() =>
      cronEngine.scheduleJob({ name: 'Bad', schedule: 'invalid' }),
    ).toThrow();
  });
...
describe('Phase 53: Admin Console', () => {
  ...
  it('should manage teams and members', async () => {
    ...
    const team = adminConsole.createTeam({
      name: 'Engineering',
      tenantId: tenant.id,
    });
  ...
  it('should manage feature flags', async () => {
    ...
    expect(() => adminConsole.setFeatureFlag('nonexistent', true)).toThrow(
  ...
  it('should bulk invite users', async () => {
    ...
    const result = adminConsole.bulkInvite(tenant.id, [
      'a@t.com',
      'b@t.com',
      'c@t.com',
    ]);
  ...
  it('should add announcements', async () => {
    const { adminConsole } = await import('./admin.console.js');
    const ann = adminConsole.addAnnouncement(
      'System update at midnight',
      'WARNING',
    );
