...
const COMPLIANCE_PROFILES = {
  ...
  FEDRAMP: {
    label: 'FedRAMP',
    regulations: ['fedramp', 'nist-800-53'],
    auditRetention: 1095,
    fips: true,
  },
  GDPR: {
...
class TenantService {
  ...
  register(config) {
    ...
    logger.info(
      `🏢 TenantService: Registered "${tenant.name}" [${tenant.plan}] in ${tenant.region}`,
    );
    return tenant;
  }
  ...
  trackUsage(tenantId, { tokens = 0, requests = 1 } = {}) {
    ...
    // Check quota limits
    if (
      tenant.quotas.tokensPerMonth > 0 &&
      tenant.usage.currentMonthTokens > tenant.quotas.tokensPerMonth
    ) {
