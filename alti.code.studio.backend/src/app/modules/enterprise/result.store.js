...
class ResultStore {
  ...
  async query(tenantId, filters = {}) {
    ...
    if (filters.agentName)
      filtered = filtered.filter(r => r.agentName === filters.agentName);
    if (filters.squad)
      filtered = filtered.filter(r => r.squad === filters.squad);
    if (filters.userId)
      filtered = filtered.filter(r => r.userId === filters.userId);
    if (filters.since)
      filtered = filtered.filter(r => r.createdAt >= filters.since);
    if (filters.until)
      filtered = filtered.filter(r => r.createdAt <= filters.until);
    if (filters.status)
  ...
  async getCostAttribution(tenantId, options = {}) {
    ...
    if (PROJECT_ID) {
      try {
        ...
        if (options.since)
          queryRef = queryRef.where('createdAt', '>=', options.since);
        if (options.until)
          queryRef = queryRef.where('createdAt', '<=', options.until);

        const snapshot = await queryRef.get();
        filtered = snapshot.docs.map(doc => doc.data());
        logger.debug(
          `🔥 Firestore: Fetched ${snapshot.size} records for cost attribution`,
        );
      } catch (err) {
        logger.warn(
          `⚠️ Firestore cost query failed (${err.message}). Falling back to local store...`,
        );
        const results = this.store.get(tenantId) || [];
        filtered = results;
        if (options.since)
          filtered = filtered.filter(r => r.createdAt >= options.since);
        if (options.until)
          filtered = filtered.filter(r => r.createdAt <= options.until);
      }
    } else {
      const results = this.store.get(tenantId) || [];
      filtered = results;
    ...
    for (const r of filtered) {
      ...
      if (!byAgent[r.agentName])
        byAgent[r.agentName] = { tokens: 0, requests: 0, durationMs: 0 };
