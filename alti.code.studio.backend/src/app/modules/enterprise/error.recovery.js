...
class DeadLetterQueue {
  ...
  enqueue(failedJob) {
    ...
    logger.warn(
      `📬 DLQ: Enqueued ${entry.id} (agent: ${entry.agentName}, attempts: ${entry.attempts})`,
    );
    return entry;
  }
  ...
  dequeue(limit = 10, filter = {}) {
    ...
    if (filter.agentName)
      items = items.filter(e => e.agentName === filter.agentName);
    if (filter.errorType)
      items = items.filter(e => e.error.type === filter.errorType);
  ...
  async execute(fn, policyName = 'default', context = {}) {
    ...
    while (attempt <= policy.maxRetries) {
      try {
        ...
        logger.warn(
          `🔄 Retry ${attempt}/${policy.maxRetries} for ${policyName}: ${error.message} (delay: ${Math.round(delay)}ms)`,
        );
...
class SchemaValidator {
  ...
  _registerDefaults() {
    ...
    this.register('tenant', {
      ...
      properties: {
        ...
        plan: {
          type: 'string',
          enum: ['starter', 'professional', 'enterprise', 'sp500'],
        },
        region: {
          type: 'string',
          enum: ['us-central1', 'europe-west1', 'asia-east1', 'me-west1'],
        },
  ...
  validate(schemaName, data) {
    ...
    if (!schema)
      return {
        valid: true,
        warnings: [`Schema '${schemaName}' not found, skipping`],
      };
...
class ErrorBudgetTracker {
  ...
  getBudget() {
    ...
    const errorBudget = Math.max(
      1,
      Math.ceil(window.total * (1 - this.sloTarget)),
    );
    const errorsConsumed = window.errors;
    const remaining = Math.max(0, errorBudget - errorsConsumed);
    const percentUsed =
      errorBudget > 0 ? Math.round((errorsConsumed / errorBudget) * 100) : 0;
