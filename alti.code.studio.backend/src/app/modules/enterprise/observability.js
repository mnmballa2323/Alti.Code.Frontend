...
class MetricCollector {
  constructor() {
    ...
    this.cloudBuffer = [];
    this.cloudFlushInterval = setInterval(
      () => this._flushCloudMetrics(),
      10000,
    );
  ...
  _defineMetrics() {
    ...
    this.defineHistogram(
      'agent.latency.ms',
      'Agent execution latency in ms',
      [50, 100, 250, 500, 1000, 2500, 5000, 10000],
    );
    this.defineHistogram(
      'provider.latency.ms',
      'AI provider latency in ms',
      ...
      points: [
        {
          ...
          value:
            m.type === 'INT64'
              ? { int64Value: Math.floor(m.value) }
              : { doubleValue: Number(m.value) },
        },
      ],
    }));
  ...
  getStats() {
    const avgDuration =
      this.completedTraces.length > 0
        ? Math.round(
            this.completedTraces.reduce((s, t) => s + (t.durationMs || 0), 0) /
    ...
    return {
      ...
      errorRate:
        this.completedTraces.length > 0
          ? Math.round(
              (this.completedTraces.filter(t => t.status === 'ERROR').length /
                this.completedTraces.length) *
                100,
            )
          : 0,
...
class AgentDashboard {
  ...
  getAgentPerformance(agentName) {
    const traces = this.tracer.completedTraces.filter(
      t => t.metadata?.agentName === agentName,
    );
    ...
    const avgLatency = Math.round(
      traces.reduce((s, t) => s + (t.durationMs || 0), 0) / total,
    );
    const p95 =
      traces.map(t => t.durationMs || 0).sort((a, b) => a - b)[
  ...
  getSLAReport() {
    ...
    const availability =
      total > 0 ? ((successful / total) * 100).toFixed(4) : '100.0000';
