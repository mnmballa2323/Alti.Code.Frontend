...
router.get('/health', async (req, res) => {
  try {
    ...
    res
      ...
      .json({ ...health, aiProviders: aiHealth });
  } catch (err) {
...
router.get('/scim/v2/Users', rbac('users:manage'), async (req, res) => {
  try {
    ...
    const paginatedUsers = users.slice(
      ...
      parseInt(startIndex) - 1 + parseInt(count),
    );
  ...
  try {
    ...
    if (Array.isArray(Operations)) {
      const activeOp = Operations.find(
        op =>
          op.path === 'active' ||
          (op.value && typeof op.value.active !== 'undefined'),
      );
      if (activeOp) {
        activeVal =
          typeof activeOp.value === 'boolean'
            ? activeOp.value
...
router.get('/costs/export', rbac('costs:export'), async (req, res) => {
  ...
  res.setHeader(
    'Content-Disposition',
    'attachment; filename="cost_report.csv"',
  );
...
router.post('/marketplace/install', rbac('tenants:configure'), (req, res) => {
  try {
    const result = agentMarketplace.install(
      req.body.tenantId,
      req.body.listingId,
    );
...
router.get(
  '/marketplace/installed/:tenantId',
  rbac('audit:read'),
  (req, res) => {
...
router.get('/regions', rbac('audit:read'), (req, res) => {
  res.json({
    clusters: multiRegion.getClustersStatus(),
    stats: multiRegion.getStats(),
  });
});

router.post('/regions', rbac('tenants:configure'), (req, res) => {
  try {
    const cluster = multiRegion.registerCluster(
      req.body.region,
      req.body.options || {},
    );
...
router.post(
  '/regions/failover',
  rbac('tenants:configure'),
  async (req, res) => {
    try {
      const result = await multiRegion.triggerFailover(
        req.body.fromRegion,
        req.body.reason,
      );
      res.json(result);
...
router.post('/keys/rotate', rbac('tenants:configure'), (req, res) => {
  try {
    const result = apiKeyManager.rotateKey(
      req.body.tenantId,
      req.body.keyId,
      req.body,
    );
...
router.delete(
  ...
  (req, res) => {
    try {
      const result = apiKeyManager.revokeKey(
        req.params.tenantId,
        req.params.keyId,
      );
      res.json(result);
...
router.get('/audit/export/:format', rbac('audit:read'), (req, res) => {
  ...
  try {
    switch (format) {
      case 'json':
        res.type('application/json').send(auditExporter.exportJSON(filters));
        break;
      case 'ndjson':
        res
          .type('application/x-ndjson')
          .send(auditExporter.exportNDJSON(filters));
        break;
      case 'csv':
        res
          .type('text/csv')
          .set('Content-Disposition', 'attachment; filename=audit.csv')
          .send(auditExporter.exportCSV(filters));
        break;
      case 'cef':
        res.type('text/plain').send(auditExporter.exportCEF(filters));
        break;
      case 'splunk':
        res
          .type('application/json')
          .send(auditExporter.exportSplunkHEC(filters));
        break;
      case 'datadog':
        res.type('application/json').send(auditExporter.exportDatadog(filters));
        break;
      case 'elastic':
        res
          .type('application/x-ndjson')
          .send(auditExporter.exportElastic(filters));
        break;
      default:
        res.status(400).json({
...
router.post(
  '/ws/:connectionId/subscribe',
  rbac('agents:dispatch'),
  (req, res) => {
    try {
      const result = wsFeed.subscribe(
...
router.get('/ws/messages', rbac('audit:read'), (req, res) => {
  res.json({
    messages: wsFeed.getRecentMessages(
      req.query.channel,
...
router.post(
  ...
  (req, res) => {
    try {
      const result = secretManager.rollbackSecret(
        req.params.secretId,
        req.body.version,
      );
      res.json(result);
...
router.post('/quotas/record', rbac('agents:dispatch'), (req, res) => {
  try {
    const result = quotaMeter.recordUsage(
      req.body.tenantId,
      req.body.resource,
      req.body.amount,
    );
...
router.get(
  '/quotas/check/:tenantId/:resource',
  rbac('audit:read'),
  (req, res) => {
...
router.post(
  '/notifications/read-all/:tenantId',
  rbac('agents:dispatch'),
  (req, res) => {
...
router.put(
  '/notifications/preferences/:tenantId',
  rbac('tenants:configure'),
  (req, res) => {
  ...
  (req, res) => {
    try {
      res
        .status(201)
        .json(
          workflowEngine.createFromTemplate(req.params.templateId, req.body),
        );
    } catch (err) {
      res.status(400).json({ error: err.message });
    }
...
router.post(
  '/pipelines/execute/:pipelineId',
  rbac('agents:dispatch'),
  (req, res) => {
    try {
      const result = dataPipeline.executePipeline(
...
router.get('/search', rbac('audit:read'), (req, res) => {
  const { q, type, tenantId, limit, offset, fuzzy, severity, from, to } =
    req.query;
  if (!q)
    return res.status(400).json({ error: 'Query parameter q is required' });
  res.json(
    enterpriseSearch.search(q, {
      type,
      tenantId,
...
router.get('/search/suggest', rbac('audit:read'), (req, res) => {
  res.json({
    suggestions: enterpriseSearch.suggest(
      req.query.q || '',
...
router.post('/cron/pause/:jobId', rbac('tenants:configure'), (req, res) => {
  try {
    res.json(cronEngine.pauseJob(req.params.jobId));
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
router.post('/cron/resume/:jobId', rbac('tenants:configure'), (req, res) => {
  try {
    res.json(cronEngine.resumeJob(req.params.jobId));
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
router.post('/cron/cancel/:jobId', rbac('tenants:configure'), (req, res) => {
  try {
    res.json(cronEngine.cancelJob(req.params.jobId));
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
router.put(
  ...
  async (req, res) => {
    try {
      const result = await adminConsole.updateTenant(
        req.params.tenantId,
        req.body,
      );
      res.json(result);
...
router.post(
  ...
  async (req, res) => {
    try {
      const result = await adminConsole.suspendTenant(
        req.params.tenantId,
        req.body.reason,
      );
      res.json(result);
...
router.post(
  ...
  async (req, res, next) => {
    try {
      const result = await adminConsole.bulkInvite(
        req.body.tenantId,
        req.body.emails,
      );
      res.json(result);
...
router.post(
  ...
  async (req, res) => {
    try {
      const result = await adminConsole.addTeamMember(
        req.params.teamId,
        req.body.userId,
      );
      res.json(result);
...
router.delete(
  ...
  async (req, res) => {
    try {
      const result = await adminConsole.removeTeamMember(
        req.params.teamId,
        req.params.userId,
      );
      res.json(result);
...
router.post('/admin/feature-flags', rbac('platform:manage'), (req, res) => {
  try {
    res.json(adminConsole.setFeatureFlag(req.body.flag, req.body.enabled));
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
router.post('/admin/announcements', rbac('platform:manage'), (req, res) => {
  res
    .status(201)
    .json(adminConsole.addAnnouncement(req.body.message, req.body.severity));
});
...
router.get('/team/members', rbac(), async (req, res, next) => {
  try {
    ...
    logger.warn(
      '⚠️ [Postgres Offline] Falling back to mock users database for /team/members',
    );
    try {
      const mockFilePath = path.join(process.cwd(), 'users_mock.json');
      if (fs.existsSync(mockFilePath)) {
...
router.post('/team/members', rbac(), async (req, res, next) => {
  try {
    ...
    logger.warn(
      '⚠️ [Postgres Offline] Falling back to mock users database for POST /team/members',
    );
    try {
      const mockFilePath = path.join(process.cwd(), 'users_mock.json');
      if (fs.existsSync(mockFilePath)) {
...
router.delete('/team/members/:userId', rbac(), async (req, res, next) => {
  try {
    ...
    if (user.tenantRole === 'owner' || user.tenantRole === 'admin') {
      return res
        .status(400)
        .json({ error: 'Cannot remove the owner or admin of the workspace' });
    }
    ...
    logger.warn(
      '⚠️ [Postgres Offline] Falling back to mock users database for DELETE /team/members/:userId',
    );
    try {
      const mockFilePath = path.join(process.cwd(), 'users_mock.json');
      if (fs.existsSync(mockFilePath)) {
        ...
        const userIndex = users.findIndex(
          u => u.id === req.params.userId && u.tenantId === req.tenantId,
        );
        if (userIndex === -1)
          return res
            .status(404)
            .json({ error: 'Member not found in this team' });

        const user = users[userIndex];
        if (user.tenantRole === 'owner' || user.tenantRole === 'admin') {
          return res.status(400).json({
            error: 'Cannot remove the owner or admin of the workspace',
          });
        }
...
router.put('/team/members/:userId', rbac(), async (req, res, next) => {
  try {
      ...
      if (activeSubscription) {
        ...
        try {
          ...
          if (stripeKey && !stripeKey.includes('dummy')) {
            ...
            if (
              ...
              stripeSubscriptionId.startsWith('cs_')
            ) {
              const stripeSession =
            ...
            if (stripeSubscriptionId) {
              ...
              if (
                ...
                logger.info(
                  `Successfully updated Stripe subscription ${stripeSubscriptionId} to price ${numericPrice}`,
                );
              }
            }
          }
          ...
          logger.error(
            `Failed to update Stripe subscription for user ${userId}:`,
            stripeErr,
          );
        }
      } else {
        ...
        await prisma.subscription.create({
          data: {
            userId: userId,
            transactionId:
              'manual_billing_' + Math.random().toString(36).substring(2, 11),
            price: numericPrice,
            planName: 'individual',
            duration: 'month',
    ...
    logger.warn(
      '⚠️ [Postgres Offline] Falling back to mock users database for PUT /team/members/:userId',
    );
    try {
      const mockFilePath = path.join(process.cwd(), 'users_mock.json');
      if (fs.existsSync(mockFilePath)) {
...
router.put('/team/name', rbac(), async (req, res, next) => {
  try {
    ...
    logger.warn(
      '⚠️ [Postgres Offline] Falling back to mock response for PUT /team/name',
    );
    res.json({
      success: true,
      tenant: { id: req.tenantId, name: req.body.name },
    });
  }
});
...
router.get('/costs/roi/:agentName', rbac('audit:read'), (req, res) => {
  res.json(
    costAnalytics.calculateROI(
      req.params.agentName,
      parseFloat(req.query.revenue) || 0,
    ),
  );
});
...
router.get(
  '/benchmarks/efficiency/:agentName',
  rbac('audit:read'),
  (req, res) => {
...
router.post('/billing/accounts', rbac('tenants:configure'), (req, res) => {
  try {
    res
      .status(201)
      .json(tokenBilling.createAccount(req.body.tenantId, req.body.plan));
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});
...
router.post('/webhooks/dispatch', rbac('admin:write'), (req, res) => {
  webhookManager
    .dispatch(req.body.event, req.body.payload, req.body.tenantId)
...
router.post('/dashboards/:id/widgets', rbac('admin:write'), (req, res) => {
  res
    .status(201)
    .json(dashboardBuilder.addWidget(req.params.id, req.body.type, req.body));
});

router.delete(
  '/dashboards/:dashId/widgets/:widgetId',
  rbac('admin:write'),
  (req, res) => {
    res.json(
      dashboardBuilder.removeWidget(req.params.dashId, req.params.widgetId),
    );
  },
);

router.put(
  '/dashboards/widgets/:id/config',
  rbac('admin:write'),
  (req, res) => {
...
router.post('/dashboards/:id/clone', rbac('admin:write'), (req, res) => {
  res.json(
    dashboardBuilder.cloneDashboard(
      req.params.id,
      req.body.name,
      req.body.userId,
    ),
...
router.get('/dashboards/list/:tenantId', rbac('audit:read'), (req, res) => {
  res.json(
    dashboardBuilder.listDashboards(req.params.tenantId, req.query.userId),
  );
});
...
router.post('/collaboration/:id/message', rbac('admin:write'), (req, res) => {
  res.json(
    agentCollaboration.sendMessage(
      req.params.id,
      req.body.from,
      req.body.to,
      req.body.payload,
...
router.post('/collaboration/:id/state', rbac('admin:write'), (req, res) => {
  res.json(
    agentCollaboration.setSharedState(
      req.params.id,
      req.body.key,
      req.body.value,
      req.body.agent,
...
router.post('/collaboration/:id/execute', rbac('admin:write'), (req, res) => {
  agentCollaboration
    .executePattern(req.params.id, req.body.input)
    .then(result => res.json(result));
});
...
router.post('/collaboration/channels', rbac('admin:write'), (req, res) => {
  res
    .status(201)
    .json(agentCollaboration.createChannel(req.body.name, req.body.type));
});

router.post(
  '/collaboration/channels/:id/subscribe',
  rbac('admin:write'),
  (req, res) => {
...
router.post('/migration/:id/execute', rbac('admin:write'), (req, res) => {
  tenantMigration
    .executeMigration(req.params.id)
    .then(result => res.json(result))
...
router.post('/certifications/assess', rbac('admin:write'), (req, res) => {
  res
    .status(201)
    .json(
      complianceCertification.startAssessment(
        req.body.tenantId,
        req.body.framework,
      ),
    );
...
router.post(
  '/certifications/:id/auto-collect',
  rbac('admin:write'),
  (req, res) => {
...
router.get(
  '/certifications/:id/remediations',
  rbac('audit:read'),
  (req, res) => {
...
router.put(
  '/certifications/remediations/:id/assign',
  rbac('admin:write'),
  (req, res) => {
    res.json(
      complianceCertification.assignRemediation(
        req.params.id,
        req.body.owner,
        req.body.dueDate,
      ),
...
router.put(
  '/certifications/remediations/:id/complete',
  rbac('admin:write'),
  (req, res) => {
