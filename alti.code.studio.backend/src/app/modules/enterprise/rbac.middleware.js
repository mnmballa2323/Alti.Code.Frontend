...
export function rbac(...requiredPermissions) {
  return async (req, res, next) => {
    try {
      ...
      if (requiredPermissions.length > 0) {
        ...
        if (!hasWildcard) {
          const missing = requiredPermissions.filter(
            p => !identity.permissions.includes(p),
          );
          if (missing.length > 0) {
            logger.warn(
              `🚫 RBAC: ${identity.email} (${identity.role}) denied [${missing.join(', ')}]`,
            );
            return res.status(403).json({
              error: 'FORBIDDEN',
      ...
      // 6.5. Prevent Cross-Tenant IDOR (Critical Security Fix)
      const hasWildcard = identity.permissions.includes('*');
      if (!hasWildcard) {
        if (req.params.tenantId && req.params.tenantId !== tenant.id) {
          logger.error(
            `🚨 IDOR ATTEMPT: ${identity.email} tried to access ${req.params.tenantId}`,
          );
          return res.status(403).json({
            error: 'FORBIDDEN',
            message: 'Cross-tenant access blocked by Zero-Trust Shield',
          });
        }
        if (req.body.tenantId && req.body.tenantId !== tenant.id) {
          logger.error(
            `🚨 IDOR ATTEMPT: ${identity.email} tried to act on ${req.body.tenantId}`,
          );
          return res.status(403).json({
            error: 'FORBIDDEN',
            message: 'Cross-tenant action blocked by Zero-Trust Shield',
          });
        }
        if (req.query.tenantId && req.query.tenantId !== tenant.id) {
          logger.error(
            `🚨 IDOR ATTEMPT: ${identity.email} queried ${req.query.tenantId}`,
          );
          return res.status(403).json({
            error: 'FORBIDDEN',
            message: 'Cross-tenant query blocked by Zero-Trust Shield',
          });
        }
      }
      ...
      logger.debug(
        `✅ RBAC: ${identity.email} (${identity.role}) → ${req.method} ${req.path}`,
      );

      // Propagate the active user's authorization token using AsyncLocalStorage context
      const { mcpTokenContext } =
        await import('../mcp/mcp_gateway.service.js').catch(() => ({}));
      if (mcpTokenContext) {
      ...
      if (err.message.startsWith('AUTH_')) {
        return res
          .status(401)
          .json({ error: 'AUTH_FAILED', message: err.message });
      }
...
export function enforceResidency() {
  return (req, res, next) => {
    ...
    if (tenantRegion && !tenantRegion.startsWith(serverRegion.split('-')[0])) {
      logger.warn(
        `🌍 Data Residency: Tenant ${req.tenant.name} requires ${tenantRegion}, server is ${serverRegion}`,
      );
...
export function rateLimiter() {
  ...
  return (req, res, next) => {
    ...
    if (!tenantCounters.has(key)) {
      ...
      // Memory Leak Prevention: Sweep expired counters occasionally
      if (Math.random() < 0.05) {
        // 5% chance on request to sweep
        for (const [k, v] of tenantCounters.entries()) {
          if (now > v.resetAt + 60000) tenantCounters.delete(k);
        }
      }
    }
    ...
    res.set(
      ...
      String(Math.max(0, maxRequests - counter.count)),
    );
    res.set('X-RateLimit-Reset', String(Math.ceil(counter.resetAt / 1000)));

    if (counter.count > maxRequests) {
      logger.warn(
        `⚠️ Rate Limit: Tenant ${req.tenant.name} exceeded ${maxRequests} req/min`,
      );
