        ...
        try {
            ...
            if (!hasWildcard) {
                if (req.params.tenantId && req.params.tenantId !== tenant.id) {
                    logger.error(`🚨 IDOR ATTEMPT: ${identity.email} tried to access ${req.params.tenantId}`);
                    return res.status(403).json({ error: 'FORBIDDEN', message: 'Cross-tenant access blocked by Zero-Trust Shield' });
                }
                if (req.body.tenantId && req.body.tenantId !== tenant.id) {
                    logger.error(`🚨 IDOR ATTEMPT: ${identity.email} tried to act on ${req.body.tenantId}`);
                    return res.status(403).json({ error: 'FORBIDDEN', message: 'Cross-tenant action blocked by Zero-Trust Shield' });
                }
                if (req.query.tenantId && req.query.tenantId !== tenant.id) {
                    logger.error(`🚨 IDOR ATTEMPT: ${identity.email} queried ${req.query.tenantId}`);
                    return res.status(403).json({ error: 'FORBIDDEN', message: 'Cross-tenant query blocked by Zero-Trust Shield' });
                }
            }
...
export function rateLimiter() {
    ...
    return (req, res, next) => {
        ...
        if (!tenantCounters.has(key)) {
            ...
            if (Math.random() < 0.05) { // 5% chance on request to sweep
