...
class CircuitBreaker {
  ...
  async isAvailable() {
    ...
    if (this.state === STATES.OPEN) {
      ...
      if (Date.now() - this.lastFailure > this.resetTimeoutMs) {
        ...
        this._pushToRedis();
        logger.info(
          `🔄 CircuitBreaker [${this.name}]: OPEN → HALF_OPEN (global test)`,
        );
        return true;
      }
      return false;
  ...
  async recordSuccess() {
    ...
    if (this.state === STATES.HALF_OPEN) {
      ...
      if (this.successCount >= this.successThreshold) {
        ...
        this._pushToRedis();
        logger.info(
          `✅ CircuitBreaker [${this.name}]: HALF_OPEN → CLOSED (globally recovered)`,
        );
      } else {
        this._pushToRedis();
      }
  ...
  async recordFailure(error) {
    ...
    if (this.state === STATES.HALF_OPEN) {
      ...
      this._pushToRedis();
      logger.warn(
        `🔴 CircuitBreaker [${this.name}]: HALF_OPEN → OPEN (still failing globally)`,
      );
    } else if (this.failureCount >= this.failureThreshold) {
      this.state = STATES.OPEN;
      this._pushToRedis();
      logger.error(
        `🔴 CircuitBreaker [${this.name}]: CLOSED → OPEN (tripped globally at ${this.failureCount} failures)`,
      );
    } else {
      this._pushToRedis();
    }
...
class ResilientAIProvider {
  ...
  async execute(aiCallFactory, options = {}) {
    ...
    for (const provider of this.providers) {
      ...
      if (!(await circuit.isAvailable())) {
        logger.debug(
          `⏭️ Failover: Skipping ${provider.name} (circuit OPEN globally)`,
        );
        continue;
      }
      ...
      for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
          ...
          logger.warn(
            `⚠️ ${provider.name} attempt ${attempt + 1}/${maxRetries + 1}: ${err.message}`,
          );
      ...
      logger.warn(
        `🔄 Failover: ${provider.name} exhausted → trying next provider`,
      );
    }
  ...
  getHealth() {
    ...
    return {
      providers: statuses,
      healthyProviders: Object.values(statuses).filter(
        s => s.state === 'CLOSED',
      ).length,
