...
const COLLAB_PATTERNS = {
    ...
    pipeline: {
        description: 'Stream-based processing — each agent transforms and passes downstream',
        minAgents: 2,
        topology: 'stream',
    },
};
...
class AgentCollaboration {
    ...
    createSession(config) {
        ...
        if (!COLLAB_PATTERNS[pattern]) throw new Error(`Unknown pattern: ${pattern}`);
        if (participants.length < COLLAB_PATTERNS[pattern].minAgents) {
            throw new Error(`Pattern '${pattern}' requires at least ${COLLAB_PATTERNS[pattern].minAgents} agents`);
        }
        ...
        logger.info(`🤝 Collaboration session: ${session.id} [${pattern}] with ${participants.length} agents`);
        return { sessionId: session.id, pattern, participants: participants.length, topology: session.topology };
    }
    ...
    sendMessage(sessionId, from, to, payload, priority = 'NORMAL') {
        ...
        if (to) {
            ...
            // Broadcast
            session.participants.forEach(p => { if (p.agentName !== from) p.messagesReceived++; });
        }
    ...
    getMessages(sessionId, agentName, limit = 50) {
        ...
        if (agentName) {
            msgs = msgs.filter(m => m.to === agentName || m.to === 'broadcast' || m.from === agentName);
        }
    ...
    _executeChain(session, input) {
        ...
        for (const participant of session.participants) {
            const step = {
                ...
                input: current,
                output: { processed: true, agent: participant.agentName, data: current },
            };
            current = step.output;
            steps.push(step);
            this.sendMessage(session.id, participant.agentName, session.participants[session.participants.indexOf(participant) + 1]?.agentName || 'result', current);
        }
        return { pattern: 'chain', steps: steps.length, result: current, trace: steps };
    }
    ...
    _executeDebate(session, input) {
        ...
        return {
            pattern: 'debate',
            topic: input,
            proponent: { agent: proponent.agentName, argument: `Supporting: ${JSON.stringify(input)}` },
            opponent: { agent: opponent.agentName, argument: `Opposing: ${JSON.stringify(input)}` },
    ...
    _executePipeline(session, input) {
        ...
        return { pattern: 'pipeline', stages: stages.length, output: stream, trace: stages };
    }
    ...
    subscribe(channelId, agentName) {
        ...
        if (!channel.subscribers.includes(agentName)) channel.subscribers.push(agentName);
    ...
    getPatterns() { return COLLAB_PATTERNS; }
    getChannelTypes() { return CHANNEL_TYPES; }
