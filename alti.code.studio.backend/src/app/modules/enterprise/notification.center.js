...
class NotificationCenter {
    ...
    send(options) {
        ...
        // Determine channels
        let channels = explicitChannels || (template ? template.channels : null) || SEVERITY_ROUTES[severity] || ['in_app'];

        // Apply tenant preferences
        const prefs = this.preferences.get(tenantId);
        if (prefs) {
            channels = channels.filter(ch => !prefs.muted?.includes(ch));
            if (prefs.digest && prefs.digest !== 'realtime' && severity !== 'CRITICAL' && severity !== 'EMERGENCY') {
                return this._queueForDigest(tenantId, { subject, body, severity, channels, templateId, variables });
            }
        }
        ...
        if (this.notifications.length > this.maxHistory) {
            this.notifications = this.notifications.slice(-Math.floor(this.maxHistory * 0.8));
        }

        logger.info(`📨 Notification sent: ${subject} → ${channels.join(', ')} (${tenantId})`);
    ...
    getInbox(tenantId, options = {}) {
        let notifs = this.notifications.filter(n => n.tenantId === tenantId && n.channel === 'in_app');

        if (options.unreadOnly) notifs = notifs.filter(n => !n.read);
        if (options.severity) notifs = notifs.filter(n => n.severity === options.severity);
    ...
    _queueForDigest(tenantId, notification) {
        ...
        return { queued: true, digestSchedule: this.preferences.get(tenantId)?.digest };
    }
    ...
    getAnalytics(tenantId) {
        ...
        for (const n of notifs) {
            ...
            if (n.templateId) byTemplate[n.templateId] = (byTemplate[n.templateId] || 0) + 1;
            if (n.channel === 'in_app') {
                totalInApp++;
                if (n.read) readCount++;
            }
        ...
        return {
            ...
            bySeverity,
            byTemplate,
            inAppReadRate: totalInApp > 0 ? `${Math.round(readCount / totalInApp * 100)}%` : '0%',
        };
    }
...
export { CHANNELS as NOTIFICATION_CHANNELS, SEVERITY_ROUTES, DIGEST_SCHEDULES, DEFAULT_TEMPLATES };
