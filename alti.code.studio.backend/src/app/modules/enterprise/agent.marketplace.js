...
const BADGES = {
  verified: {
    label: 'Verified',
    icon: '✅',
    description: 'Identity and code verified',
  },
  enterprise: {
    label: 'Enterprise',
    icon: '🏢',
  ...
  certified: {
    label: 'Certified',
    icon: '🏆',
    description: 'Alti.Code certified',
  },
  community: {
    label: 'Community',
    icon: '🌍',
...
const REVENUE_SPLIT = { publisher: 0.7, platform: 0.3 };
...
class AgentMarketplace {
  ...
  install(tenantId, listingId) {
    ...
    if (!this.installations.has(tenantId))
      this.installations.set(tenantId, new Set());
  ...
  review(listingId, userId, rating, comment = '') {
    ...
    const total = Object.entries(listing.rating.distribution).reduce(
      (s, [r, c]) => s + parseInt(r) * c,
      0,
    );
    listing.rating.average =
      Math.round((total / listing.rating.count) * 10) / 10;
  ...
  browse(options = {}) {
    ...
    if (options.badge)
      listings = listings.filter(l => l.badges.includes(options.badge));
    if (options.pricing)
      listings = listings.filter(l => l.pricing.model === options.pricing);
  ...
  getInstalled(tenantId) {
    ...
    return [...installed]
      .map(id => {
        ...
        return l
          ? { id: l.id, name: l.name, version: l.version, category: l.category }
          : null;
      })
      .filter(Boolean);
  }
