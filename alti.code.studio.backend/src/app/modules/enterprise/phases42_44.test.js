...
describe('Phase 43: WebSocket Real-Time Feed', () => {
  ...
  it('should deny access to insufficient roles', async () => {
    ...
    expect(() => wsFeed.subscribe(conn.connectionId, 'security')).toThrow(
...
describe('Phase 44: Secret Rotation Automation', () => {
  ...
  it('should set rotation schedules', async () => {
    const { secretManager, ROTATION_SCHEDULES } =
      await import('./secret.rotation.js');

    const secret = await secretManager.createSecret({
      name: 'scheduled',
      type: 'oauth_token',
    });
    const schedule = secretManager.setRotationSchedule(
      secret.secretId,
      'monthly',
    );
