...
const CHANNELS = {
    'system': { description: 'System-wide events (health, metrics, alerts)', requiredRole: 'viewer' },
    'dispatches': { description: 'Agent dispatch lifecycle events', requiredRole: 'viewer' },
    'compliance': { description: 'Compliance alerts and PII detections', requiredRole: 'compliance_officer' },
    'security': { description: 'Security events (auth, key rotation, threats)', requiredRole: 'security_admin' },
    'costs': { description: 'Cost attribution and billing events', requiredRole: 'billing_admin' },
    'admin': { description: 'Admin operations (tenant CRUD, config changes)', requiredRole: 'tenant_admin' },
    'dlq': { description: 'Dead letter queue events', requiredRole: 'operator' },
    'regions': { description: 'Multi-region health and failover events', requiredRole: 'operator' },
};
...
class WebSocketFeed {
    ...
    addConnection(options = {}) {
        ...
        logger.info(`🔌 WebSocket connected: ${connectionId} (tenant: ${connection.tenantId})`);
    ...
    subscribe(connectionId, channel) {
        ...
        return { subscribed: true, channel, description: channelConfig.description };
    }
    ...
    _passesFilters(message, filters) {
        ...
        if (filters.severity) {
            ...
            if ((minSev[message.severity] || 0) < (minSev[filters.severity] || 0)) return false;
        }
