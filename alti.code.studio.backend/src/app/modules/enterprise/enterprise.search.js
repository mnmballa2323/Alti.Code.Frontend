...
const ENTITY_TYPES = {
  agent: {
    label: 'Agent',
    fields: ['name', 'description', 'capabilities', 'model'],
  },
  tenant: { label: 'Tenant', fields: ['name', 'plan', 'domain', 'industry'] },
  dispatch: {
    label: 'Dispatch',
    fields: ['agentName', 'task', 'result', 'status'],
  },
  log: {
    label: 'Audit Log',
    fields: ['action', 'actor', 'details', 'severity'],
  },
...
class EnterpriseSearch {
  ...
  index(entity) {
    const docId =
      entity.id || `doc_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`;
    ...
    if (this.documents.size > this.maxDocuments) {
      const oldest = [...this.documents.keys()].slice(
        0,
        Math.floor(this.maxDocuments * 0.2),
      );
  ...
  search(query, options = {}) {
    ...
    for (const term of queryTerms) {
      ...
      if (fuzzy) {
        for (const [indexedTerm] of this.invertedIndex) {
          if (
            this._editDistance(term, indexedTerm) <= 2 &&
            indexedTerm !== term
          ) {
      ...
      for (const matchTerm of matchingTerms) {
        ...
        for (const docId of docs) {
          ...
          // TF-IDF scoring
          const tf =
            doc._terms.filter(t => t === matchTerm).length / doc._termCount;
          const score = tf * idf;

          // Title boost
          const titleBoost = doc.title.toLowerCase().includes(matchTerm)
            ? 2.0
            : 1.0;
    ...
    if (from)
      results = results.filter(d => new Date(d.indexedAt) >= new Date(from));
    if (to)
      results = results.filter(d => new Date(d.indexedAt) <= new Date(to));
  ...
  _trackQuery(query, results, duration) {
    ...
    for (const term of terms) {
      this.analytics.popularTerms.set(
        term,
        (this.analytics.popularTerms.get(term) || 0) + 1,
      );
    }
  ...
  getAnalytics() {
    ...
    return {
      ...
      avgDuration:
        this.analytics.queries.length > 0
          ? Math.round(
              this.analytics.queries.reduce((s, q) => s + q.duration, 0) /
                this.analytics.queries.length,
            )
          : 0,
    };
  }
