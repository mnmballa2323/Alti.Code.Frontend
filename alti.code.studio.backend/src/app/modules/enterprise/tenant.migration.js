...
class TenantMigration {
  ...
  planMigration(config) {
    ...
    if (!REGIONS.includes(sourceRegion))
      throw new Error(`Unknown source region: ${sourceRegion}`);
    ...
    this._audit(
      plan.id,
      'PLAN_CREATED',
      `Migration planned: ${sourceRegion} → ${targetRegion}`,
    );
  ...
  async executeMigration(migrationId) {
    ...
    try {
      ...
      await this._executeStage(migration, 0, () => {
        return {
          valid: true,
          checks: migration.categories.length,
          passed: migration.categories.length,
        };
      });
      ...
      this._audit(
        migrationId,
        'COMPLETED',
        `Migration completed successfully${migration.dryRun ? ' (dry run)' : ''}`,
      );
    } catch (err) {
  ...
  rollback(migrationId) {
    ...
    this._audit(
      migrationId,
      'ROLLED_BACK',
      'Migration rolled back to source state',
    );
  ...
  async _executeStage(migration, stageIndex, executor) {
    ...
    migration.progress.percent = Math.round(
      ((stageIndex + 1) / MIGRATION_STAGES.length) * 100,
    );
  ...
  getStats() {
    return {
      ...
      activeMigrations: [...this.migrations.values()].filter(
