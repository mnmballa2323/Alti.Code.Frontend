...
class WebhookManager {
  ...
  register(tenantIdOrConfig, config) {
    ...
    if (typeof tenantIdOrConfig === 'string') {
      ...
      if (tenantIdOrConfig.active !== undefined)
        active = tenantIdOrConfig.active;
      if (tenantIdOrConfig.headers) headers = tenantIdOrConfig.headers;
      if (tenantIdOrConfig.payloadTemplate)
        payloadTemplate = tenantIdOrConfig.payloadTemplate;
      if (tenantIdOrConfig.description)
        description = tenantIdOrConfig.description;
    }
  ...
  async _deliver(webhook, event, payload) {
    const body = webhook.payloadTemplate
      ? this._applyTemplate(webhook.payloadTemplate, {
          event,
          payload,
    ...
    return {
      ...
      successRate:
        this.stats.totalDeliveries > 0
          ...
          : '0%',
      circuitOpen: [...this.webhooks.values()].filter(
        w => w.circuit.state === 'OPEN',
      ).length,
