...
class BatchProcessor {
  ...
  async execute(params) {
    ...
    logger.info(
      `📦 Batch [${batchId}]: ${items.length} items → ${agentName} (concurrency: ${concurrency})`,
    );
    ...
    for (let i = 0; i < items.length; i += concurrency) {
      ...
      const progress = Math.round(
        ((batch.completed + batch.failed) / batch.totalItems) * 100,
      );
      logger.info(
        `📦 Batch [${batchId}]: ${progress}% (${batch.completed} ok, ${batch.failed} failed)`,
      );
    }
