...
class AuditLogExporter {
  ...
  exportCSV(filters = {}) {
    ...
    for (const record of data.results) {
      const row = headers.map(h => {
        const val =
          h === 'details' ? JSON.stringify(record[h] || {}) : record[h] || '';
  ...
  exportCEF(filters = {}) {
    ...
    return data.results
      .map(r => {
        ...
        return (
          `CEF:0|InsoCode|EnterprisePlatform|2.0|${r.action}|${r.action}|${sev}|` +
  ...
  exportElastic(filters = {}) {
    ...
    for (const r of data.results) {
      lines.push(
        JSON.stringify({
          index: { _index: `audit-${r.timestamp.substring(0, 7)}`, _id: r.id },
        }),
      );
  ...
  applyRetention(framework = 'default') {
    ...
    return {
      framework,
      policy: policy.label,
      purged: before - this.logs.length,
