...
if (redis) {
  ...
  redis.on('connect', () =>
    logger.info(`🔗 Connected to Cloud Memorystore (Redis)`),
  );
}
  ...
  async get(key) {
    if (redis) {
      try {
        ...
        logger.warn(
          `⚠️ Redis L2 get failed (${err.message}). Falling back to local...`,
        );
      }
    }
    ...
    if (redis) {
      try {
        ...
        for (const tag of tags) {
          ...
          if (ttlMs > 0)
            await redis.expire(`tag:${tag}`, Math.ceil(ttlMs / 1000) * 2);
        }
        this.stats.writes++;
        return;
      } catch (err) {
        logger.warn(
          `⚠️ Redis L2 set failed (${err.message}). Falling back to local...`,
        );
      }
    }
...
class CacheController {
  ...
  async invalidateByTag(tag) {
    ...
    logger.info(
      `🗑️ Cache invalidated tag '${tag}': L1=${l1Count}, L2=${l2Count}`,
    );
