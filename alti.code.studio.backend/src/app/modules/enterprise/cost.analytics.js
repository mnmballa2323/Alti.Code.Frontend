...
class CostAnalytics {
    ...
    getBudgetStatus(tenantId) {
        ...
        const spent = this.records
            .filter(r => r.tenantId === tenantId && new Date(r.timestamp) >= monthStart)
        ...
        return {
            ...
            status: usage >= 1 ? 'EXCEEDED' : usage >= budget.alertThreshold ? 'WARNING' : 'OK',
        };
    }
    ...
    getByTenant(tenantId, options = {}) {
        ...
        if (options.from) records = records.filter(r => new Date(r.timestamp) >= new Date(options.from));
        if (options.to) records = records.filter(r => new Date(r.timestamp) <= new Date(options.to));
        ...
        return {
            ...
            byCategory,
            byAgent,
            topAgents: Object.entries(byAgent).sort((a, b) => b[1] - a[1]).slice(0, 10).map(([name, cost]) => ({ name, cost: Math.round(cost * 100) / 100 })),
        };
    }
    ...
    getTrends(tenantId, period = 'daily') {
        const records = tenantId ? this.records.filter(r => r.tenantId === tenantId) : this.records;
        const buckets = {};
        ...
        for (const r of records) {
            ...
            else if (period === 'weekly') key = `W${Math.ceil(date.getDate() / 7)}-${date.getFullYear()}-${date.getMonth() + 1}`;
    ...
    calculateROI(agentName, revenueGenerated) {
        ...
        const roi = agentCost.totalCost > 0
    ...
    _groupBy(records, field) {
        const groups = {};
        for (const r of records) groups[r[field]] = (groups[r[field]] || 0) + r.cost;
        return groups;
    }
