...
const RATE_TIERS = {
  ...
  sp500: {
    ...
    requestsPerHour: 250_000,
    requestsPerDay: 2_500_000,
    tokensPerDay: 500_000_000,
    concurrentJobs: 500,
  },
};
...
class ApiKeyManager {
  ...
  generateKey(tenantId, options = {}) {
    ...
    this._audit('KEY_CREATED', tenantId, {
      keyId: keyData.id,
      name,
      scopes,
    ...
    logger.info(
      `🔑 API key created: ${keyData.id} for tenant ${tenantId} [${prefix}]`,
    );
  ...
  rotateKey(tenantId, oldKeyId, options = {}) {
    // Find the old key
    const oldEntry = [...this.keys.values()].find(
      k => k.id === oldKeyId && k.tenantId === tenantId,
    );
    ...
    logger.info(
      `🔄 Key rotated: ${oldKeyId} → ${newKeyResult.keyId} (grace: ${gracePeriod}ms)`,
    );
  ...
  revokeKey(tenantId, keyId) {
    const entry = [...this.keys.values()].find(
  ...
  async check(tenantId, plan = 'starter') {
    ...
    if (memorystoreService.isInitialized) {
      try {
        ...
        logger.warn(
          `⚠️ Redis rate limit check failed, falling back to memory: ${err.message}`,
        );
