...
class DataPipeline {
  ...
  definePipeline(options) {
    ...
    logger.info(
      `🔄 Pipeline defined: ${name} (${validatedStages.length} stages) [${pipelineId}]`,
    );
  ...
  executePipeline(pipelineId, inputData) {
    ...
    logger.info(
      `${errors.length === 0 ? '✅' : '❌'} Pipeline ${execution.status}: ${pipeline.name} (${data.length} records, ${execution.duration}ms)`,
    );
    return execution;
  }
    ...
    switch (type) {
      ...
      case 'aggregate':
        ...
        return Object.entries(groups).map(([key, records]) => {
          ...
          if (config.sumField)
            agg.sum = records.reduce(
              (s, r) => s + (r[config.sumField] || 0),
              0,
            );
          if (config.avgField)
            agg.avg =
              records.reduce((s, r) => s + (r[config.avgField] || 0), 0) /
              records.length;
          return agg;
        });
      ...
      case 'enrich':
        return data.map(record => {
          ...
          if (config.computations) {
            for (const [key, computation] of Object.entries(
              config.computations,
            )) {
              if (computation.type === 'concat') {
                enriched[key] = computation.fields
                  .map(f => record[f])
                  .join(computation.separator || ' ');
              } else if (computation.type === 'uppercase') {
                enriched[key] = String(
                  record[computation.field] || '',
                ).toUpperCase();
              } else if (computation.type === 'hash') {
                enriched[key] = crypto
                  .createHash('sha256')
                  .update(String(record[computation.field] || ''))
                  .digest('hex')
                  .substring(0, 8);
              }
            }
  ...
  listSchemas() {
    return [...this.schemas.values()].map(s => ({
      id: s.id,
      name: s.name,
