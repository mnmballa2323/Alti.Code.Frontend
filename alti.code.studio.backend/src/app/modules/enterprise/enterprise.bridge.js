...
class EnterpriseBridge {
  ...
  async onBeforeDispatch(job) {
    ...
    if (!tokenCheck.allowed || !apiCheck.allowed) {
      ...
      logger.warn(
        `🚫 Quota Exceeded: Dispatch to ${agentName} blocked for tenant ${tenantId} due to ${exceededType} limits`,
      );
      throw new Error(
        `QUOTA_EXCEEDED: Tenant ${tenantId} exceeded ${exceededType} quota (Limit: ${limitVal})`,
      );
    }
    ...
    if (classification.findings.length > 0) {
      ...
      if (classification.requiresMasking) {
        ...
        logger.info(
          `🛡️ DLP: Active redaction applied to task input for ${agentName}`,
        );
      }
      ...
      logger.warn(
        `🔐 PII detected in dispatch to ${agentName}: ${classification.findings.length} findings`,
      );
    }

    // 2. Publish event
    await eventBus.publish(
      EVENT_TYPES.AGENT_DISPATCHED,
      {
        agentName,
        tenantId,
        mode,
  ...
  async onDispatchError(job, error) {
    ...
    logger.error(
      `❌ Agent ${agentName} failed for tenant ${tenantId}: ${error.message}`,
    );
  }
