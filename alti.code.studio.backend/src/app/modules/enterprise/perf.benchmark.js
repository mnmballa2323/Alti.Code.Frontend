...
class PerfBenchmark {
  ...
  getTokenEfficiency(agentName) {
    ...
    const efficiency =
      totalOptimal > 0 ? (totalOptimal / totalUsed) * 100 : 100;
  ...
  getSLACompliance(agentName) {
    ...
    return {
      ...
      latency: {
        maxP99: sla.maxLatencyP99,
        actualP99: p99,
        met: latencyCompliant,
      },
  ...
  compareAgents(agentNames) {
    ...
    // Rank by p99 latency
    results.sort(
      (a, b) => (a.percentiles?.p99 || 0) - (b.percentiles?.p99 || 0),
    );
