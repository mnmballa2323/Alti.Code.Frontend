...
class MultiRegionController {
    ...
    registerCluster(region, options = {}) {
        ...
        if (redis) {
            redis.hset('multi_region:weights', region, this.trafficWeights.get(region)).catch(() => { });
        }
    ...
    async triggerFailover(fromRegion, reason = 'manual') {
        ...
        if (redis) {
            try {
                ...
                await redis.hset('multi_region:weights', fromRegion, 0, target.region, oldWeight || 100);
            } catch (err) {
    ...
    async setTrafficWeights(weights) {
        ...
        if (redisMulti) {
            try {
                ...
                logger.warn(`⚠️ Failed to sync traffic weights to Redis: ${err.message}`);
            }
        }
