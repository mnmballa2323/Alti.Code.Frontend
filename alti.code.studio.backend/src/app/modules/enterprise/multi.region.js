...
const REDIS_HOST =
  process.env.DISABLE_REDIS === 'true'
    ? null
    : process.env.REDIS_URL || process.env.GCP_REDIS_HOST;
...
class MultiRegionController {
  constructor() {
    ...
    // Periodically sync state from Redis if distributed
    if (redis) {
      this._syncInterval = setInterval(() => this._syncFromRedis(), 10000);
      this._syncFromRedis();
    }
  }
  ...
  async _syncFromRedis() {
    if (!redis) return;
    try {
      const weights = await redis.hgetall('multi_region:weights');
      if (weights && Object.keys(weights).length > 0) {
        for (const [region, weight] of Object.entries(weights)) {
          this.trafficWeights.set(region, parseInt(weight, 10));
        }
      }

      const primaryRegion = await redis.get('multi_region:primary');
      if (primaryRegion) {
        for (const [region, cluster] of this.clusters) {
          cluster.isPrimary = region === primaryRegion;
        }
      }
    } catch (err) {
      logger.debug(`⚠️ MultiRegion Redis sync failed: ${err.message}`);
    }
  }
  ...
  registerCluster(region, options = {}) {
    ...
    if (cluster.isPrimary) {
      ...
      if (redis) redis.set('multi_region:primary', region).catch(() => {});
    } else {
      if (!this.trafficWeights.has(region)) this.trafficWeights.set(region, 0);
    }

    if (redis) {
      redis
        .hset('multi_region:weights', region, this.trafficWeights.get(region))
        .catch(() => {});
    }

    logger.info(
      `🌐 Registered cluster: ${region} (${regionInfo.displayName}) [${cluster.isPrimary ? 'PRIMARY' : 'STANDBY'}]`,
    );
    return cluster;
  }
  ...
  async triggerFailover(fromRegion, reason = 'manual') {
    ...
    if (!sourceCluster)
      throw new Error(`Source cluster not found: ${fromRegion}`);
    ...
    logger.warn(
      `🔄 FAILOVER: ${fromRegion} → ${target.region} (reason: ${reason})`,
    );
    ...
    if (redis) {
      try {
        await redis.set('multi_region:primary', target.region);
        await redis.hset(
          'multi_region:weights',
          fromRegion,
          0,
          target.region,
          oldWeight || 100,
        ...
        logger.warn(`⚠️ Failed to sync failover to Redis: ${err.message}`);
      }
    }
    ...
    failoverEvent.rtoMs =
      new Date(failoverEvent.completedAt) - new Date(failoverEvent.triggeredAt);
    failoverEvent.status =
      failoverEvent.rtoMs <= this.drConfig.rtoTargetSeconds * 1000
        ? 'SUCCESS'
        : 'SUCCESS_SLOW';
    ...
    logger.info(
      `✅ Failover complete: ${target.region} is now PRIMARY (RTO: ${failoverEvent.rtoMs}ms)`,
    );
  ...
  _findBestFailoverTarget(excludeRegion) {
    ...
    for (const [region, cluster] of this.clusters) {
      ...
      const zoneBonus =
        cluster.zone === this.clusters.get(excludeRegion)?.zone ? 20 : 0;
  ...
  async setTrafficWeights(weights) {
    const total = Object.values(weights).reduce((s, v) => s + v, 0);
    if (total !== 100)
      throw new Error(`Traffic weights must sum to 100, got ${total}`);

    const redisMulti = redis ? redis.pipeline() : null;

    for (const [region, weight] of Object.entries(weights)) {
      if (!this.clusters.has(region))
        throw new Error(`Unknown region: ${region}`);
      this.trafficWeights.set(region, weight);
      if (redisMulti) redisMulti.hset('multi_region:weights', region, weight);
    }
    ...
    if (redisMulti) {
      try {
        ...
        logger.warn(
          `⚠️ Failed to sync traffic weights to Redis: ${err.message}`,
        );
      }
    }
  ...
  getDisasterRecoveryStatus() {
    ...
    const avgRto =
      this.failoverEvents.length > 0
        ? Math.round(
            this.failoverEvents.reduce((s, e) => s + (e.rtoMs || 0), 0) /
    ...
    return {
      ...
      rpoMet:
        lastBackupAge !== null
          ? lastBackupAge <= this.drConfig.rpoTargetSeconds
          : false,
  ...
  getStats() {
    return {
      ...
      healthyClusters: [...this.clusters.values()].filter(
        c => c.status === 'HEALTHY',
      ).length,
