...
const DEFAULT_DEPENDENCIES = {
    ...
    external_api: { required: false, timeout: 10000, status: 'HEALTHY', latency: 0 },
};
...
class HealthCheckEngine {
    ...
    readyz() {
        ...
        const allRequiredHealthy = deps.every(d => !d.required || d.status === 'HEALTHY');
    ...
    livez() {
        ...
        const result = {
            ...
            degradedServices: deps.filter(d => d.status !== 'HEALTHY').map(d => d.name),
        };
    ...
    setDependencyStatus(name, status, latency = 0) {
        if (!this.dependencies[name]) {
            this.dependencies[name] = { required: false, timeout: 5000, status, latency };
        } else {
    ...
    _computeOverallStatus(deps, customResults) {
        ...
        const anyDegraded = deps.some(d => d.status !== 'HEALTHY') ||
    ...
    getStats() {
        return {
            ...
            healthyDeps: Object.values(this.dependencies).filter(d => d.status === 'HEALTHY').length,
        };
    }
}
...
export async function registerSystemHealthChecks() {
    try {
        ...
        logger.warn('🟡 HealthCheck: embeddingCache check not registered (VectorMemory unavailable):', err.message);
    }
    ...
    try {
        ...
        logger.warn('🟡 HealthCheck: swarmHealth check not registered (swarmHealthMonitor unavailable):', err.message);
    }
}
