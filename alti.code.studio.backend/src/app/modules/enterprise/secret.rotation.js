...
const KeyManagementServiceClient = class {
    encrypt() { throw new Error('Azure Key Vault KMS not initialized'); }
    decrypt() { throw new Error('Azure Key Vault KMS not initialized'); }
};
...
const SECRET_TYPES = {
    ...
    encryption: { label: 'Encryption Key', defaultRotationDays: 365, maxAge: 1095 },
...
class SecretRotationManager {
    ...
    async createSecret(options) {
        ...
        const encryptedValue = await this._encrypt(value || crypto.randomBytes(32).toString('hex'));
        ...
        const secret = {
            ...
            nextRotationAt: new Date(Date.now() + (typeConfig.defaultRotationDays * 86400000)).toISOString(),
            expiresAt: new Date(Date.now() + (typeConfig.maxAge * 86400000)).toISOString(),
            status: 'ACTIVE',
            metadata,
        };
        ...
        logger.info(`🔐 Secret created: ${name} (${typeConfig.label}) [${secretId}]`);
    ...
    async rotateSecret(secretId, options = {}) {
        ...
        secret.nextRotationAt = new Date(Date.now() + (typeConfig.defaultRotationDays * 86400000)).toISOString();

        this.stats.totalRotations++;
        logger.info(`🔄 Secret rotated: ${secret.name} → v${newVersion} (overlap: ${overlapMs}ms)`);
    ...
    checkExpiring(daysAhead = 30) {
        ...
        for (const [id, secret] of this.secrets) {
            ...
            if (new Date(secret.expiresAt) <= cutoff) {
                const daysRemaining = Math.ceil((new Date(secret.expiresAt) - Date.now()) / 86400000);
                const alert = {
                    secretId: id,
                    name: secret.name,
                    ...
                    expiresAt: secret.expiresAt,
                    daysRemaining,
                    severity: daysRemaining <= 7 ? 'CRITICAL' : daysRemaining <= 14 ? 'WARNING' : 'NOTICE',
                };
                expiring.push(alert);
            }
        }
        ...
        if (expiring.length > 0) {
            this.alerts.push(...expiring.map(a => ({
                ...a,
    ...
    async _encrypt(plaintext) {
        if (!KMS_KEY_NAME) {
            ...
            return { iv: iv.toString('hex'), data: encrypted, tag, algorithm: 'aes-256-gcm' };
        }
        ...
        try {
            ...
            logger.warn(`⚠️ KMS Encryption failed (${error.message}). Falling back to local AES-256-GCM...`);
            const iv = crypto.randomBytes(16);
            const cipher = crypto.createCipheriv('aes-256-gcm', this.masterKey, iv);
            let encrypted = cipher.update(plaintext, 'utf8', 'hex');
            encrypted += cipher.final('hex');
