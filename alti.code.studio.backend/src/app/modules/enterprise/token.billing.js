...
const TOKEN_PRICING_PLANS = {
    ...
    starter: {
        ...
        overage: { allowed: true, ratePerToken: 0.00010 },
    ...
    professional: {
        ...
        features: ['all_agents', 'priority_support', 'api_access', 'custom_agents', 'workflows'],
    },
    enterprise: {
        name: 'Enterprise',
        ...
        features: ['all_agents', 'dedicated_support', 'sla', 'sso', 'compliance', 'multi_region'],
    },
    unlimited: {
        name: 'Unlimited',
...
class TokenBillingEngine {
    ...
    consumeTokens(tenantId, event) {
        ...
        // Check allowance
        const isOverage = account.tokensUsed + totalTokens > account.monthlyAllowance;
        ...
        let overageTokens = 0;
        if (isOverage) {
            overageTokens = Math.max(0, (account.tokensUsed + totalTokens) - account.monthlyAllowance);
        ...
        account.tokensRemaining = Math.max(0, account.monthlyAllowance - account.tokensUsed);
        ...
        return {
            ...
            usagePercent: Math.round(account.tokensUsed / account.monthlyAllowance * 100),
        };
    }
    ...
    estimateCost(tenantId, inputTokens, outputTokens, model = 'default') {
        ...
        const wouldExceed = account
            ? account.tokensUsed + inputTokens + outputTokens > account.monthlyAllowance
            : false;
    ...
    getUsageReport(tenantId, period = 'monthly') {
        ...
        return {
            ...
            usagePercent: account ? Math.round(account.tokensUsed / account.monthlyAllowance * 100) : 0,
            byAgent: Object.entries(byAgent).sort((a, b) => b[1] - a[1]).map(([name, tokens]) => ({ name, tokens })),
            byModel: Object.entries(byModel).sort((a, b) => b[1] - a[1]).map(([name, tokens]) => ({ name, tokens })),
            overageTokens: records.filter(r => r.isOverage).reduce((s, r) => s + r.overageTokens, 0),
    ...
    generateInvoice(tenantId) {
        ...
        const invoice = {
            ...
            lineItems: [
                ...
                { description: `Token Usage (${account.tokensUsed.toLocaleString()} tokens)`, amount: Math.round(tokenCost * 100) / 100 },
                ...(overageRecords.length > 0 ? [{ description: `Overage Tokens`, amount: Math.round(overageCost * 100) / 100 }] : []),
            ],
    ...
    getAccount(tenantId) { return this.accounts.get(tenantId) || null; }
    listAccounts() { return [...this.accounts.values()]; }
    listPlans() { return TOKEN_PRICING_PLANS; }
    listModels() { return MODEL_RATES; }
