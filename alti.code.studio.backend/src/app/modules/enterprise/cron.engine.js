...
class CronEngine {
  ...
  async _performAction(action) {
    ...
    switch (type) {
      case 'agent':
        return {
          dispatched: action.agentName || 'default',
          result: 'completed',
        };
      case 'workflow':
        return { workflowId: action.workflowId, triggered: true };
      case 'webhook':
        return { url: action.url, status: 200 };
      case 'pipeline':
        return { pipelineId: action.pipelineId, executed: true };
      default:
  ...
  getDueJobs() {
    ...
    return [...this.jobs.values()].filter(
      j => j.status === 'ACTIVE' && j.nextRun && new Date(j.nextRun) <= now,
    );
  }
  ...
  getStats() {
    return {
      ...
      activeJobs: [...this.jobs.values()].filter(j => j.status === 'ACTIVE')
        .length,
