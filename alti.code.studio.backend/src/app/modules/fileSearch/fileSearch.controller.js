...
function assertSafeParam(value, name) {
    if (!value || typeof value !== 'string') {
        throw Object.assign(new Error(`${name} is required.`), { statusCode: httpStatus.BAD_REQUEST });
    }
    if (value.includes('..') || value.includes('\x00') || value.includes('/') || value.includes('\\')) {
        throw Object.assign(new Error(`${name} contains forbidden characters.`), { statusCode: httpStatus.BAD_REQUEST });
    }
    return value.trim();
}
...
function assertString(value, name, maxLength = 10000) {
    if (!value || typeof value !== 'string' || value.trim().length === 0) {
        throw Object.assign(new Error(`${name} is required and must be a non-empty string.`), { statusCode: httpStatus.BAD_REQUEST });
    }
    if (value.length > maxLength) {
        throw Object.assign(new Error(`${name} exceeds maximum length of ${maxLength}.`), { statusCode: httpStatus.BAD_REQUEST });
    }
    return value.trim();
}
...
function assertStoreNames(storeNames) {
    if (!Array.isArray(storeNames) || storeNames.length === 0) {
        throw Object.assign(new Error('storeNames must be a non-empty array of strings.'), { statusCode: httpStatus.BAD_REQUEST });
    }
    if (storeNames.length > 20) {
        throw Object.assign(new Error('storeNames cannot contain more than 20 entries.'), { statusCode: httpStatus.BAD_REQUEST });
    }
    for (const name of storeNames) {
        if (!name || typeof name !== 'string') {
            throw Object.assign(new Error('Each storeName must be a non-empty string.'), { statusCode: httpStatus.BAD_REQUEST });
        }
    }
    return storeNames;
}
...
const createStore = catchAsync(async (req, res) => {
    ...
    const store = await fileSearchService.createStore(displayName, embeddingModel);
...
const getStore = catchAsync(async (req, res) => {
    ...
    const store = await fileSearchService.getStore(`fileSearchStores/${storeName}`);
...
const listDocuments = catchAsync(async (req, res) => {
    const storeName = assertSafeParam(req.params.storeName, 'storeName');
    const documents = await fileSearchService.listDocuments(`fileSearchStores/${storeName}`);
...
const queryStructured = catchAsync(async (req, res) => {
    ...
    const result = await fileSearchService.queryStructured(
        query, storeNames, req.body.jsonSchema, { model: req.body.model }
    );
