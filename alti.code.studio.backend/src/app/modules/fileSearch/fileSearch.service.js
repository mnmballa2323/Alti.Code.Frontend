...
class CircuitBreaker {
  ...
  recordFailure() {
    ...
    if (this.failures >= this.failureThreshold) {
      this.state = 'OPEN';
      logger.warn(
        `[FileSearch] Circuit breaker OPEN after ${this.failures} failures. Cooldown: ${this.resetTimeMs / 1000}s`,
      );
    }
  }
}
...
function validateResourceName(name, label = 'resourceName') {
  ...
  for (const seg of segments) {
    if (seg.length > 0 && !RESOURCE_NAME_PATTERN.test(seg)) {
      throw new Error(
        `${label} contains invalid characters. Only alphanumeric, hyphens, and underscores are allowed.`,
      );
    }
  }
  return trimmed;
...
function sanitizeDisplayName(name) {
  ...
  if (
    sanitized.length === 0 ||
    sanitized.length > LIMITS.maxDisplayNameLength
  ) {
    throw new Error(
      `displayName must be between 1-${LIMITS.maxDisplayNameLength} characters after sanitization.`,
    );
  }
  return sanitized;
}
...
function validateFilePath(filePath) {
  ...
  try {
    ...
    if (stat.size > LIMITS.maxFileSizeBytes) {
      throw new Error(
        `File exceeds maximum size of ${LIMITS.maxFileSizeBytes / (1024 * 1024)} MB.`,
      );
    }
    if (stat.size === 0) {
...
function validateChunkingConfig(cfg) {
  ...
  if (maxTokens && (maxTokens < 10 || maxTokens > LIMITS.maxChunkTokens)) {
    throw new Error(
      `maxTokensPerChunk must be between 10-${LIMITS.maxChunkTokens}.`,
    );
  }
  if (overlap && (overlap < 0 || overlap > LIMITS.maxOverlapTokens)) {
    throw new Error(
      `maxOverlapTokens must be between 0-${LIMITS.maxOverlapTokens}.`,
    );
  }
...
function validateMetadata(metadata) {
  ...
  if (metadata.length > LIMITS.maxMetadataKeys) {
    throw new Error(
      `customMetadata cannot exceed ${LIMITS.maxMetadataKeys} entries.`,
    );
  }
  for (const entry of metadata) {
    ...
    if (
      entry.stringValue === undefined &&
      entry.numericValue === undefined &&
      entry.string_value === undefined &&
      entry.numeric_value === undefined
    ) {
      throw new Error(
        `Metadata key "${entry.key}" must have a stringValue or numericValue.`,
      );
    }
  }
  return metadata;
...
async function scrubQuery(text) {
  try {
    const { GoogleDlpService } = await import('../ai/gcpDlp.service.js');
...
class FileSearchService {
  ...
  _getClient() {
    ...
    if (!apiKey) {
      throw new Error(
        'FileSearchService: GEMINI_API_KEY is required but not set.',
      );
    }
  ...
  async _execute(operationName, fn) {
    if (!this._circuitBreaker.canExecute()) {
      throw new Error(
        `[FileSearch] Circuit breaker is OPEN — ${operationName} rejected. Retry later.`,
      );
    }
    ...
    try {
      ...
      const durationMs = Date.now() - startTime;
      logger.error(
        `[FileSearch] ${operationName} failed after ${durationMs}ms: ${err.message}`,
      );
      throw err;
    }
  }
  ...
  async _recordTelemetry(operation, durationMs) {
    try {
      const { metricsService } =
        await import('../monitoring/metrics.service.js');
  ...
  async _auditLog(action, details) {
    try {
      const { CloudLoggingService } =
        await import('../googleCloud/logging.service.js');
  ...
  async createStore(displayName, embeddingModel = DEFAULT_EMBEDDING_MODEL) {
    ...
    if (!ALLOWED_EMBEDDING_MODELS.has(embeddingModel)) {
      throw new Error(
        `Unsupported embedding model: "${embeddingModel}". Allowed: ${[...ALLOWED_EMBEDDING_MODELS].join(', ')}`,
      );
    }
  ...
  async uploadFile(
    ...
    return this._execute('uploadFile', async () => {
      ...
      this._auditLog('FILE_UPLOADED', {
        storeName: safeName,
        displayName: safeDisplayName,
      });
      logger.info(
        `[FileSearch] File "${safeDisplayName}" uploaded and indexed.`,
      );
      return operation;
    });
  }
  ...
  async listDocuments(storeName) {
    ...
    return this._execute('listDocuments', async () => {
      const ai = this._getClient();
      const documents = [];
      const docIterator = await ai.fileSearchStores.documents.list({
        parent: safeName,
      });
      ...
      logger.info(
        `[FileSearch] Found ${documents.length} document(s) in ${safeName}`,
      );
      return documents;
    });
  }
  ...
  async query(query, storeNames, options = {}) {
    ...
    if (storeNames.length > LIMITS.maxStoreNamesPerQuery) {
      throw new Error(
        `Cannot query more than ${LIMITS.maxStoreNamesPerQuery} stores at once.`,
      );
    }

    // Validate each store name
    const safeStoreNames = storeNames.map(n =>
      validateResourceName(n, 'storeName'),
    );
    ...
    return this._execute('query', async () => {
      ...
      const fileSearchTool = {
        fileSearch: { fileSearchStoreNames: safeStoreNames },
      };
      if (
        options.metadataFilter &&
        typeof options.metadataFilter === 'string'
      ) {
      ...
      logger.info(
        `[FileSearch] Query complete. ${citations.length} citation(s), model=${model}`,
      );
      return {
        text,
        citations,
  ...
  async queryStructured(query, storeNames, jsonSchema, options = {}) {
    ...
    if (!Array.isArray(storeNames) || storeNames.length === 0)
      throw new Error('storeNames is required.');
    if (!jsonSchema || typeof jsonSchema !== 'object')
      throw new Error('jsonSchema must be a valid object.');
  ...
  async _waitForOperation(operation) {
    ...
    while (!operation.done && attempts < POLL_CONFIG.maxAttempts) {
      ...
      intervalMs = Math.min(
        intervalMs * POLL_CONFIG.backoffMultiplier,
    ...
    if (!operation.done) {
      const totalWaitSec = Math.round(
        (POLL_CONFIG.maxAttempts *
          (POLL_CONFIG.baseIntervalMs + POLL_CONFIG.maxIntervalMs)) /
          2 /
          1000,
      );
      throw new Error(
        `[FileSearch] Operation timed out after ~${totalWaitSec}s (${attempts} polls).`,
      );
    }
