...
class UndoService {
  ...
  async snapshot(sessionId, filePath, after = null) {
    ...
    logger.debug(
      `↩ UndoService: Snapshot of "${path.basename(filePath)}" for session ${sessionId}`,
    );
    return entry.id;
  }
  ...
  async undoLast(sessionId) {
    ...
    if (!entry)
      throw new Error(`No applied changes to undo in session "${sessionId}".`);
  ...
  async undoFile(sessionId, filePath) {
    ...
    const entry = [...stack]
      .reverse()
      .find(
        e => path.resolve(e.filePath) === absPath && e.status === 'applied',
      );
    if (!entry)
      throw new Error(
        `No applied changes found for file "${filePath}" in session "${sessionId}".`,
      );
    return this._restoreEntry(entry);
  }
  ...
  async undoAll(sessionId) {
    ...
    for (const entry of applied) {
      try {
        ...
        logger.warn(
          `↩ UndoService: Could not restore "${entry.filePath}": ${e.message}`,
        );
      }
    }

    logger.info(
      `↩ UndoService: Undid all changes in session ${sessionId} — ${results.restored} restored, ${results.deleted} deleted.`,
    );
    return results;
  }
  ...
  async _restoreEntry(entry) {
    if (entry.before === null) {
      ...
      entry.status = 'undone';
      logger.info(
        `↩ UndoService: Restored "${entry.filePath}" to previous state.`,
      );
