...
class AutopilotService extends EventEmitter {
  ...
  setMode(mode) {
    if (!['autopilot', 'supervised'].includes(mode)) {
      throw new Error(
        `Invalid mode "${mode}". Must be "autopilot" or "supervised".`,
      );
    }
    this.mode = mode;
  ...
  _createSession(goal) {
    ...
    logger.info(
      `[Autopilot] Session ${sessionId} created (${this.mode} mode).`,
    );
    return session;
  }
  ...
  async launchSession(goal) {
    ...
    if (this.mode === 'autopilot') {
      ...
      this._runAutopilot(session).catch(e => {
        logger.error(
          `[Autopilot] Session ${session.sessionId} crashed: ${e.message}`,
        );
      ...
      this._runSupervised(session).catch(e => {
        logger.error(
          `[Autopilot] Supervised session ${session.sessionId} crashed: ${e.message}`,
        );
        session.status = 'error';
        session.error = e.message;
      });
  ...
  async _runSupervised(session) {
    ...
    this.emit('pending-changes', {
      sessionId: session.sessionId,
      pendingChanges: diffPreview,
    });
    logger.info(
      `[Autopilot] Supervised session ${session.sessionId}: ${diffPreview.length} change(s) pending approval.`,
    );
  }
...
[
  ...
  async approveHunk(sessionId, hunkId) {
    ...
    if (session.mode !== 'supervised')
      throw new Error('Session is not in supervised mode.');

    let approved = [];
    if (hunkId === 'all') {
      session.pendingChanges.forEach(h => {
        h.status = 'approved';
      });
    ...
    const allSettled = session.pendingChanges.every(
      h => h.status !== 'pending',
    );
    if (allSettled) {
  ...
  async revertSession(sessionId) {
    ...
    try {
      ...
      logger.warn(
        `[Autopilot] Git revert failed: ${e.message}. Trying git checkout...`,
      );
      try {
  ...
  listSessions() {
    return Array.from(this.activeSessions.values()).map(s => ({
      ...
      status: s.status,
      pendingCount:
        s.pendingChanges?.filter(h => h.status === 'pending').length || 0,
  ...
  async processHeartbeat(correlationId = Date.now().toString()) {
    try {
      ...
      logger.info(
        `[Autopilot] 🎯 Synthesized Objective: "${objectiveResult.goal}"`,
      );
      this._launchBackgroundMission(objectiveResult.goal, correlationId);
      return {
        status: 'dispatched',
        goal: objectiveResult.goal,
