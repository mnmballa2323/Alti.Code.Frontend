...
const processHeartbeat = async (req, res) => {
    try {
        const correlationId = req.headers['x-cloud-trace-context'] || Date.now().toString();
        const result = await autopilotService.processHeartbeat(correlationId);
        res.status(202).json({ success: true, message: 'Heartbeat acknowledged', data: result });
    } catch (err) {
...
const approveHunk = async (req, res) => {
    try {
        ...
        if (!hunkId) return res.status(400).json({ error: '"hunkId" is required (or "all").' });
...
const rejectHunk = async (req, res) => {
    try {
        const { hunkId } = req.body;
        const result = await autopilotService.rejectHunk(req.params.id, hunkId || 'all');
        res.json({ success: true, ...result });
    } catch (err) {
