...
describe('AdversarialReviewerService Tests', () => {
    ...
    it('should reach unanimous consensus to APPROVE when both agents approve', async () => {
        ...
        aiProvider.reason
            .mockResolvedValueOnce(JSON.stringify({ vote: 'APPROVE', reasoning: 'Looks very secure.' }))
            .mockResolvedValueOnce(JSON.stringify({ vote: 'APPROVE', reasoning: 'Code is very clean and meets styles.' }));
    ...
    it('should reach unanimous consensus to REJECT when both agents reject', async () => {
        aiProvider.reason
            .mockResolvedValueOnce(JSON.stringify({ vote: 'REJECT', reasoning: 'Vulnerable to SQLi.' }))
            .mockResolvedValueOnce(JSON.stringify({ vote: 'REJECT', reasoning: 'Highly unreadable formatting.' }));
    ...
    it('should trigger debate/arbitration and resolve split decisions', async () => {
        ...
        aiProvider.reason
            .mockResolvedValueOnce(JSON.stringify({ vote: 'REJECT', reasoning: 'Slight risk of memory leaks.' }))
            .mockResolvedValueOnce(JSON.stringify({ vote: 'APPROVE', reasoning: 'Abstractions look fantastic.' }))
            .mockResolvedValueOnce(JSON.stringify({
                consensusReached: true,
                finalOutcome: 'APPROVE',
                arbitrationReasoning: 'Risk of memory leak is minimal, the design improvements outweigh it.'
            }));
