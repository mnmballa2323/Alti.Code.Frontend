...
export class AutonomousSprintService {
  constructor() {
    ...
    logger.info(
      '🏃 AutonomousSprintService v6.2.0: Closed-Loop Autonomous Validation initialized.',
    );
  }
  ...
  async launchSprint(goal, opts = {}) {
    if (!goal?.trim())
      throw new Error(
        'AutonomousSprintService: goal must be a non-empty string.',
      );
    ...
    logger.info(
      `🏃 Sprint [${sprintId}]: Started — "${goal.substring(0, 80)}"`,
    );
    ...
    try {
      ...
      logger.info(
        `🧠 Sprint [${sprintId}]: Consulting Enterprise Memory for context...`,
      );
      const ragContext = await ragService.query(
        `Architectural guidelines and past lessons for: ${goal}`,
        3,
      );
      ...
      try {
        ...
        if (stores && stores.length > 0) {
          ...
          if (fsResult && fsResult.text) {
            ...
            logger.info(
              `📚 Sprint [${sprintId}]: File Search injected ${fileSearchContext.length} chars into sprint planning.`,
            );
          }
        }
      } catch (e) {
        logger.warn(
          `⚠️ Sprint [${sprintId}]: File Search context failed (non-blocking): ${e.message}`,
        );
      }
      ...
      if (
        goal
          .toLowerCase()
          .match(/cost|arbitrage|carbon|green software|cheapest cloud/)
      ) {
        logger.info(
          `💸 Sprint [${sprintId}]: Omniscient FinOps engaged. Evaluating arbitrage and carbon footprint...`,
        );
        this._emit('sprint:phase', { sprintId, phase: 'FINOPS_ARBITRAGE' });
        const finopsData = await finopsAgent.execute('arbitrage', {
          query: goal,
        });
      ...
      if (
        goal
          .toLowerCase()
          .match(/ux research|accessibility|synthetic user|persona|usability/)
      ) {
        logger.info(
          `🤖 Sprint [${sprintId}]: Synthetic Demographics engaged. Spawning phantom users to discover UX defects...`,
        );
        this._emit('sprint:phase', { sprintId, phase: 'UX_RESEARCH' });
        const uxData = await syntheticUserAgent.execute('simulate', {
          query: goal,
        });
      ...
      logger.info(
        `🔍 Sprint [${sprintId}]: The Auditor checking plan AST complexity...`,
      );
      ...
      if (!auditResult.isOptimized) {
        ...
        this._emit('sprint:audit_failed', {
          sprintId,
          reason: auditResult.reason,
        });
        logger.warn(
          `🛑 Sprint [${sprintId}]: AUDIT FAILED — ${auditResult.reason}`,
        );
      ...
      if (autoDeploy) {
        try {
          ...
          logger.warn(
            `🏃 Sprint [${sprintId}]: Branch creation failed (${e.message}) — continuing on current branch.`,
          );
        }
      }
      ...
      for (let i = 0; i < Math.min(plan.steps.length, this.MAX_STEPS); i++) {
        ...
        if (totalCostUsd >= maxCostUsd) {
          logger.warn(
            `💸 Sprint [${sprintId}]: Cost $${totalCostUsd.toFixed(3)} ≥ budget $${maxCostUsd}. Halting.`,
          );
          sprintState.status = 'budget_exceeded';
          this._emit('sprint:halted', {
            sprintId,
        ...
        if (stepResult.codeSnippet && !skipTests) {
          ...
          this._emit('sprint:phase', { sprintId, phase: 'PERFORMANCE_AUDIT' });
          logger.info(
            `🔍 Sprint [${sprintId}] Step ${i}: The Auditor checking AST complexity...`,
          );
          const astAuditResult = await performanceAuditorService.auditAST(
            stepResult.codeSnippet,
          );
          if (!astAuditResult.isOptimized) {
            logger.warn(
              `🛑 Sprint [${sprintId}] Step ${i}: AUDIT FAILED — ${astAuditResult.reason}`,
            );
          ...
          const sandboxResult = await this._runSandbox(
            stepResult.codeSnippet,
            sprintId,
            i,
          );
          stepResult.sandboxResult = sandboxResult;
          this._emit('sprint:sandbox', {
            sprintId,
            stepIndex: i,
            sandboxResult,
          });
      ...
      if (!skipTests && testGatePassed) {
        ...
        logger.info(
          `⚖️ Sprint [${sprintId}]: Legal Oracle auditing dependencies for Copyleft contamination...`,
        );
        const legalResult = await legalAgent.execute('audit_license', {
          repoPath,
        });
        if (!legalResult.isSafeToDeploy) {
          logger.warn(
            `🛑 Sprint [${sprintId}]: LEGAL GATE FAILED — blocking deploy due to Open-Source License Risk.`,
          );
      ...
      if (!skipTests && !testGatePassed) {
        logger.warn(
          `🚦 Sprint [${sprintId}]: TEST/LEGAL GATE FAILED — blocking deploy.`,
        );
        sprintState.status =
      ...
      if (
        ...
        try {
          ...
          logger.warn(
            `🏃 Sprint [${sprintId}]: Deploy (git sync) failed — ${e.message}`,
          );
      ...
      if (autoDoc && (testGatePassed || skipTests)) {
        ...
        try {
          ...
          logger.warn(
            `🏃 Sprint [${sprintId}]: Auto-doc failed — ${e.message}`,
          );
        }
      }
      ...
      await this._persistSprintMemory(
        sprintId,
        goal,
        sprintState,
        steps,
      ...
      if (sprintState.status === 'completed') {
        ...
        for (const step of steps) {
          if (step.neuralPathwayId) {
            try {
              await neuralRouterService.backpropagate(
                step.neuralPathwayId,
                reviewScore,
              );
            } catch (e) {
              logger.warn(
                `Neural backprop failed for ${step.neuralPathwayId}: ${e.message}`,
              );
            }
          }
        }
      ...
      logger.info(
        `✅ Sprint [${sprintId}]: ${sprintState.status} in ${sprintState.durationMs}ms — $${totalCostUsd.toFixed(4)} | tests: ${testGatePassed ? '✅' : '❌'}`,
      );
  ...
  async _generatePlan(goal, ragContext = '') {
    const prompt =
      `You are an autonomous software engineering planner. Break this goal into an ordered sequence of executable steps.
...
Shape:
{
  ...
  async _executeStep(step, sprintId, stepIndex, repoPath) {
    ...
    try {
      ...
      if (step.agentHint === 'git') {
        ...
        const runResult = await qaService.runTests(repoPath, {
          coverage: false,
        });
        ...
        const routeCtx = {};
        const routeResult = await neuralRouterService.route(
      ...
      logger.error(
        `❌ Sprint [${sprintId}] Step ${stepIndex} failed:`,
        error.message,
      );
      return {
        stepIndex,
  ...
  async _runSandbox(codeSnippet, sprintId, stepIndex) {
    try {
      logger.info(
        `🧪 Sprint [${sprintId}] Step ${stepIndex}: Running sandbox smoke-test…`,
      );
      const result = await sandboxExecute(codeSnippet, { exports: {} });
      return { passed: true, result: String(result).substring(0, 200) };
    } catch (err) {
      logger.warn(
        `🧪 Sprint [${sprintId}] Step ${stepIndex}: Sandbox error — ${err.message}`,
      );
  ...
  async _autoTestAndHeal(filePaths, sprintId, repoPath, goal) {
    ...
    for (const filePath of filePaths) {
      ...
      logger.info(
        `🧪 Sprint [${sprintId}]: Auto-testing ${path.basename(filePath)}`,
      );
      this._emit('sprint:auto_test', {
        sprintId,
        file: filePath,
        stage: 'generate_spec',
      });
      ...
      // 1. Generate spec
      const specResult = await qaService
        .generateSpec(filePath)
        .catch(e => ({ success: false, error: e.message }));
      if (!specResult.success) {
        logger.warn(
          `🧪 Sprint: spec gen failed for ${filePath}: ${specResult.error}`,
        );
      ...
      while (!passed && retries < MAX_HEAL_RETRIES) {
        retries++;
        logger.info(
          `🔧 Sprint [${sprintId}]: Self-heal attempt ${retries}/${MAX_HEAL_RETRIES} for ${path.basename(filePath)}`,
        );
        this._emit('sprint:heal', {
          sprintId,
          file: filePath,
          attempt: retries,
          error: runResult.output?.substring(0, 300),
        });
        ...
        try {
          ...
          /* surfer failure is non-blocking */
        }
...
Return ONLY the complete fixed source code. No markdown fences.`.trim();
        ...
        try {
          ...
          logger.error(`Sprint self-heal write failed: ${e.message}`);
          break;
        }
  ...
  async _reviewSprint(sprintId, goal, steps) {
    const summary = steps
      ...
      .join('\n');
    const prompt =
      `You are a senior engineering reviewer. Evaluate whether this sprint achieved its goal.
...
Only valid JSON, no markdown.`.trim();
    ...
    try {
      ...
      return {
        passed: steps.every(s => s.status === 'success'),
        score: 70,
  ...
  async _autoDoc(sprintId, goal, writtenFiles, repoPath) {
    logger.info(
      `📝 Sprint [${sprintId}]: Auto-doc — regenerating docs for ${writtenFiles.length} changed files`,
    );
    try {
      if (writtenFiles.length > 0) {
        await docGenService.generateForFiles(writtenFiles, {
          reason: `Autonomous Sprint ${sprintId}: ${goal}`,
        });
      } else {
      ...
      logger.warn(
        `Sprint [${sprintId}]: docGenService.generateForFiles not available — ${e.message}`,
      );
    }
  }
...
`.trim();
    ...
    logger.info(
      `🧠 Sprint [${sprintId}]: Outcome persisted to episodic vector Memory Palace.`,
    );
  }
  ...
  getActiveSprints() {
    return [...this.activeSprints.values()];
  }
  ...
  _emit(event, data) {
    ...
    logger.info(
      `🏃 SprintBus [${event}]: ${JSON.stringify(data).substring(0, 120)}`,
    );
  }
}
