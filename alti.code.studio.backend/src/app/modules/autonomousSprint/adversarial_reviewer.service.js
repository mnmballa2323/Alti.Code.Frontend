...
class AdversarialReviewerService {
    ...
    async conductReview(patchData) {
        logger.info(`⚔️ Adversarial Review: Starting debate for patch on file: ${patchData.file}`);
...
Return a JSON structure:
{
  ...
  "reasoning": "Detailed technical analysis of security & robustness issues or lack thereof."
        ...
        try {
            ...
            logger.error('Adversarial Review: Red-Team analysis failed. Defaulting to REJECT.', e);
            redTeamVote = 'REJECT';
            debateLogs.push({
                agent: 'Red-Team (Security)',
                vote: 'REJECT',
...
Return a JSON structure:
{
  ...
  "reasoning": "Detailed technical analysis of design patterns, code style, and readability."
        ...
        try {
            ...
            logger.error('Adversarial Review: Blue-Team analysis failed. Defaulting to REJECT.', e);
            blueTeamVote = 'REJECT';
            debateLogs.push({
                agent: 'Blue-Team (Quality)',
                vote: 'REJECT',
                argument: 'Failed to complete architectural review due to runtime error.'
            });
        }

        logger.info(`⚔️ Adversarial Review: Initial Votes - Red-Team: ${redTeamVote}, Blue-Team: ${blueTeamVote}`);
        ...
        if (redTeamVote === 'APPROVE' && blueTeamVote === 'APPROVE') {
            ...
            consensusReached = true;
            finalOutcome = 'REJECT';
            logger.warn('⚔️ Adversarial Review: Swarm unanimously rejected the patch.');
        } else {
            // One approved, one rejected. Triggering Debate Round!
            logger.warn('⚔️ Adversarial Review: Split decision. Triggering active debate round...');
  ...
  "arbitrationReasoning": "Provide the final architectural decision summary."
        ...
        try {
            ...
            rounds.push({
                agent: 'Arbitrator',
                vote: finalOutcome,
                argument: parsed.arbitrationReasoning || 'Arbitration final decision compiled.'
            });
        } catch (e) {
    ...
    _parseAgentResponse(response) {
        try {
            ...
            logger.error('Adversarial Review: Failed to parse JSON response. Using fallback mock.', e);
            return {
                vote: 'REJECT',
