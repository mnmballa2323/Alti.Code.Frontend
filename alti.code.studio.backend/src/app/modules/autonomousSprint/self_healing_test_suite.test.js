...
describe('SelfHealingTestSuiteService Tests', () => {
    ...
    it('should return early if the test suite is already passing', async () => {
        const mockExec = vi.fn().mockResolvedValue({ success: true, stdout: '', stderr: '' });
    ...
    it('should apply patch and report success if the healed test run passes', async () => {
        ...
        const mockExec = vi.fn()
            .mockResolvedValueOnce({ success: false, stdout: 'Test failed: expected 2 to be 3', stderr: '' })
            .mockResolvedValueOnce({ success: true, stdout: 'All tests passed', stderr: '' });

        aiProvider.reason.mockResolvedValue(JSON.stringify({
    ...
    it('should roll back changes if the test suite still fails after applying the patch', async () => {
        // Both exec runs fail
        const mockExec = vi.fn().mockResolvedValue({ success: false, stdout: 'Test failed', stderr: '' });
