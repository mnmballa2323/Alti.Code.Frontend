...
class SelfHealingTestSuiteService {
  ...
  async healTestSuite(
    ...
    logger.info(
      `🩹 TestSuiteHealer: Initiating self-healing loop for test suite [${testFilePath}]`,
    );
    ...
    if (success) {
      logger.info(
        `🩹 TestSuiteHealer: Test suite [${testFilePath}] is already passing cleanly.`,
      );
    ...
    logger.warn(
      `🩹 TestSuiteHealer: Initial test run failed. Capturing error context...`,
    );
    ...
    try {
      ...
      logger.error(
        `🩹 TestSuiteHealer: Failed to read backup files: ${e.message}`,
      );
...
Return a JSON block containing the new content for both files:
{
  ...
  "fixedImplementationContent": "The entire modified implementation file content"
    ...
    try {
      ...
      logger.error(
        `🩹 TestSuiteHealer: AI reasoning/parse failed. Rolling back changes. Error: ${e.message}`,
      );
    ...
    try {
      ...
      if (patchResult.fixedImplementationContent) {
        await writeFn(
          implementationFilePath,
          patchResult.fixedImplementationContent,
        );
      }
    } catch (e) {
    ...
    logger.info(
      `🩹 TestSuiteHealer: Patch applied. Re-running verification test command...`,
    );
    ...
    if (verifyResult.success) {
      ...
      logger.warn(
        `🩹 TestSuiteHealer: Verification failed. Rolling back changes to original state.`,
      );
      await this._rollback(
        testFilePath,
        implementationFilePath,
