...
class ReleaseService {
  ...
  async calculateAndApplySemVer() {
    logger.info(
      '🚀 The Publisher: Analyzing project history for autonomous release...',
    );
    ...
    try {
      ...
      try {
        const { stdout } = await execAsync(
          `git -C ${this.rootDir} describe --tags --abbrev=0`,
        );
      ...
      const { stdout: commits } = await execAsync(
        `git -C ${this.rootDir} log ${latestTag}..HEAD --oneline`,
      );

      if (!commits.trim()) {
        logger.info(
          '💿 The Publisher: No new commits since last release. Skipping bump.',
        );
        return null;
      }
...
${commits}`;
      ...
      logger.info(
        `📈 The Publisher: Swarm determined a '${bumpType}' version bump is required.`,
      );
      ...
      if (process.env.AUTO_PUBLISH === 'true') {
        ...
        const { stdout: newVersionStdout } = await execAsync(
        ...
        await fs.writeFile(
          path.join(this.rootDir, 'VERSION'),
          newVersion,
          'utf8',
        );
        ...
        await execAsync(
          `git -C ${this.rootDir} commit -m "chore(release): v${newVersion} [skip ci]"`,
        );
        await execAsync(
          `git -C ${this.rootDir} tag -a v${newVersion} -m "Autonomous Release v${newVersion}"`,
        );

        logger.info(
          `✅ The Publisher: Version bumped to v${newVersion} and tagged successfully.`,
        );

        return newVersion;
      } else {
        logger.warn(
          `⚠️ The Publisher: AUTO_PUBLISH is disabled. Simulated a '${bumpType}' bump.`,
        );
        return `simulated-${bumpType}`;
      }
    } catch (error) {
      logger.error(
        `❌ The Publisher: Failed to calculate and apply release: ${error.message}`,
      );
      throw error;
    }
  }
