...
class DistributionService {
    ...
    async distributeRelease(version) {
        if (process.env.AUTO_PUBLISH !== 'true') {
            logger.warn(`⚠️ The Publisher: AUTO_PUBLISH is disabled. Skipping distribution for v${version}.`);
            return;
        }

        logger.info(`📦 The Publisher: Commencing global distribution for v${version}...`);

        try {
            // 1. Publish to npm (Backend or SDK)
            logger.info('📦 The Publisher: Running `npm publish` for the backend package...');
            ...
            const dockerRepo = process.env.DOCKER_REPO || 'mnmballa2323/alti.code.studio';
            logger.info(`🐳 The Publisher: Building Docker image ${dockerRepo}:${version}...`);
            await execAsync(`docker build -t ${dockerRepo}:${version} -t ${dockerRepo}:latest ${this.rootDir}`);
            ...
            logger.info('✅ The Publisher: Successfully pushed images to Docker Hub.');

            // 3. Notify Stakeholders
            if (slackAmbassadorService.isInitialized) {
                // Fetch the release notes (commits since previous version)
                const { stdout: commits } = await execAsync(`git -C ${this.rootDir} log HEAD~10..HEAD --oneline`);
...
_Powered by The Publisher Service_`;
            ...
            logger.info(`🎉 The Publisher: Global distribution for v${version} fully completed.`);
        } catch (error) {
