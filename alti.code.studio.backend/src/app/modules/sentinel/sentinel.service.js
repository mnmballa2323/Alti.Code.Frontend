...
const startWatch = async () => {
  if (_watching) {
    logger.warn(
      '⚔️  SentinelService: Watch already active — ignoring duplicate startWatch().',
    );
...
const stopWatch = async () => {
  if (!_watching) {
    logger.warn(
      '⚔️  SentinelService: stopWatch() called but watch is not active.',
    );
...
const analyzeError = async errorData => {
  if (!errorData) {
    logger.warn(
      '⚔️  SentinelService: analyzeError() received null/undefined errorData — skipping.',
    );
    return { status: 'skipped', reason: 'No error data provided' };
  }
  logger.info(
    `⚔️  SentinelService: Analysing error — ${errorData.incidentId ?? '(no incident ID)'}`,
  );
