...
class SentinelAgent {
  constructor() {
    ...
    this.breaker.on('open', () =>
      logger.warn('🔥 Sentinel Circuit Breaker OPEN: Pausing AI Analysis'),
    );
    this.breaker.on('close', () =>
      logger.info('✅ Sentinel Circuit Breaker CLOSED: Resuming AI Analysis'),
    );
  }
  ...
  async startWatch() {
    ...
    EventBus.subscribe(
      'guardian.audit.rejected',
      this.handleSecurityRefusal.bind(this),
    );

    return {
      status: 'watching',
      message: 'Sentinel is now monitoring system events.',
    };
  }
  ...
  async handleSystemError(data) {
    this.errorCount++;
    logger.warn(
      `👁️ Sentinel detected ERROR #${this.errorCount}: ${data.message}`,
    );
    ...
    if (lastSeen && now - lastSeen < 5 * 60 * 1000) {
      logger.info(
        `👁️ Sentinel: Skipping duplicate analysis for error: ${data.message.substring(0, 50)}...`,
      );
      return {
        analysis: { rootCause: 'Duplicate Error - Previously Analyzed' },
        remediation: 'skipped',
      };
    }
    ...
    try {
      ...
      if (analysis.isFixable && analysis.filePath) {
        logger.info(
          `👁️ Fixable error in ${analysis.filePath}. dispatching Surgeon...`,
        );
        // Auto-remediation!
  ...
  async scanSystem() {
    ...
    const status =
      vulnReport.critical > 0 || complianceReport.violations.length > 0
        ? 'THREATS_DETECTED'
        : 'CLEAN';
  ...
  async scanVulnerabilities() {
    ...
    try {
      ...
      if (!stdout)
        return {
          status: 'error',
          message: 'npm audit failed to produce output',
        };
      ...
      logger.info(
        `👁️ Sentinel: Vulnerability Scan Complete. Critical: ${stats.critical || 0}, High: ${stats.high || 0}`,
      );
  ...
  async enforceCompliance() {
    ...
    try {
      ...
      for (const file of controllers) {
        ...
        // This is a heuristic.
        if (
          content.match(/import .* from .*model\.js/i) ||
          content.match(/require\(.*model.*\)/i)
        ) {
          violations.push({
            file: path.relative(process.cwd(), file),
            rule: 'NO_DIRECT_MODEL_IMPORT',
            message:
              'Controller imports a Model directly. Must use Service layer.',
          });
        }
      ...
      logger.info(
        `👁️ Sentinel: Compliance Scan Complete. Violations: ${violations.length}`,
      );
      return {
  ...
  async evaluateAgentAction(agentName, action, metadata) {
    logger.info(
      `👁️ Sentinel: Verifying ${agentName} action [${action}] against Enterprise Policy Engine...`,
    );
