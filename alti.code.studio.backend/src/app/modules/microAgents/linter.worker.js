...
export const linterWorkerProcessor = async (job) => {
    ...
    const result = await aiProvider.reason(`Lint this ${language || ''} code for issues. Rules: ${rules || 'standard'}.\nCode:\n\`\`\`\n${code}\n\`\`\`\nRespond in JSON: { "errors": [], "warnings": [], "fixedCode": string }`);
...
export const formatterWorkerProcessor = async (job) => {
    ...
    const result = await aiProvider.generate(`Format this ${language || ''} code according to ${style || 'standard'} style.\nCode:\n\`\`\`\n${code}\n\`\`\`\nRespond in JSON: { "formatted": string, "changes": number }`);
...
export const secretScannerWorkerProcessor = async (job) => {
    ...
    logger.info(`🕵️ SecretScanner [${job.id}]: Scanning ${filePath || 'code'}...`);
    const result = await aiProvider.reason(`Scan this code for exposed secrets, API keys, tokens, passwords, or credentials.\nFile: ${filePath || 'unknown'}\nCode:\n\`\`\`\n${code}\n\`\`\`\n\nRespond in JSON: { "secrets": [{ "type": string, "line": number, "severity": string }], "clean": boolean }`);
...
export const deadCodeWorkerProcessor = async (job) => {
    ...
    const result = await aiProvider.reason(`Find dead code — unused functions, variables, imports, and unreachable code.\nFile: ${filePath}\nCode:\n\`\`\`\n${code}\n\`\`\`\nImports: ${JSON.stringify(imports || [])}\n\nRespond in JSON: { "deadCode": [{ "name": string, "type": string, "line": number }], "unusedImports": [] }`);
