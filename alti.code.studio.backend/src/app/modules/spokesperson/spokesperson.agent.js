...
class SpokespersonAgent {
  ...
  async generateChangelog(sinceTag = '') {
    logger.info(
      `📢 Spokesperson: Gathering commits since ${sinceTag || 'initial'}...`,
    );
    ...
    try {
      ...
      return {
        status: 'error',
        message: 'Could not fetch git log. Is this a git repo?',
      };
    }
  ...
  async polishReadme() {
    ...
    try {
      content = await fs.readFile(
        path.resolve(this.projectRoot, 'README.md'),
        'utf-8',
      );
    } catch (e) {
    ...
    return {
      ...
      polished_length: polished.length,
