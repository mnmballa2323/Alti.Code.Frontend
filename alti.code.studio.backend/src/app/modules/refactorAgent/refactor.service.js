...
class RefactorService {
  ...
  async processJob(job) {
    ...
    logger.info(
      `🤖 RefactorService: Processing job ${job.id} - "${instruction}"`,
    );
    ...
    try {
      ...
      for (const filePath of filePaths) {
        try {
          ...
          logger.info(
            `   - Parsed ${path.basename(filePath)}: root node='${tree.rootNode.type}'`,
          );
          ...
          logger.error(
            `   - Failed to process ${filePath}: ${fileErr.message}`,
          );
          results.push({
            file: filePath,
            success: false,
      ...
      logger.info(
        `🤖 RefactorService: Job ${job.id} completed — ${succeeded}/${filePaths.length} files processed.`,
      );
      return {
        result: 'success',
        details: `Refactored ${succeeded} of ${filePaths.length} files`,
        files: results,
      };
    } catch (error) {
  ...
  async scanComplexity(dirPath) {
    ...
    try {
      ...
      logger.info(
        `🤖 RefactorService: Found ${smellyFiles.length} complex files.`,
      );
      return smellyFiles;
    } catch (error) {
