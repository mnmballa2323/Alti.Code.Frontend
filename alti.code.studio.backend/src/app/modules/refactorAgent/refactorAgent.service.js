...
const refactorCode = async (codeSnippet, instruction, userId = 'system', sessionId = 'refactor-session') => {
    ...
    if (!codeSnippet || typeof codeSnippet !== 'string') {
        throw new Error('RefactorAgent: codeSnippet is required and must be a string.');
    }
    if (codeSnippet.length < MIN_CODE_LENGTH) {
        throw new Error(`RefactorAgent: codeSnippet is too short (min ${MIN_CODE_LENGTH} chars).`);
    }
    if (codeSnippet.length > MAX_CODE_LENGTH) {
        throw new Error(`RefactorAgent: codeSnippet exceeds max length (${MAX_CODE_LENGTH} chars). Split into smaller chunks.`);
    }
    if (!instruction || typeof instruction !== 'string' || instruction.trim().length === 0) {
        throw new Error('RefactorAgent: instruction is required and must be a non-empty string.');
    }

    logger.info(`🚀 RefactorAgent: Queueing job — "${instruction.substring(0, 60)}" (${codeSnippet.length} chars)`);
    ...
    try {
        ...
        if (!job) {
            throw new Error('Failed to enqueue refactor job — queueService returned null.');
        }
