...
class CodeMorphService {
    ...
    try {
      ...
      if (existingImport) {
        ...
        if (namedImports.length > 0) {
          const currentNamed = existingImport
            .getNamedImports()
            .map(n => n.getName());
  ...
  addClassMethod(
    ...
    if (!className || typeof className !== 'string')
      throw new Error(
        'CodeMorphService: className must be a non-empty string.',
      );
    if (!methodName || typeof methodName !== 'string')
      throw new Error(
        'CodeMorphService: methodName must be a non-empty string.',
      );
    ...
    try {
      ...
      if (existingMethod) {
        logger.warn(
          `⚠️ CodeMorph: Method "${methodName}" already exists in "${className}" — overwriting body.`,
        );
        ...
        logger.debug(
          `✨ CodeMorph: Added method "${methodName}" to "${className}"`,
        );
      }
    } catch (error) {
      logger.error(
        `❌ CodeMorph: Failed to add method "${methodName}" to "${className}"`,
        error,
      );
      throw error;
    }
  ...
  organizeImports(sourceFile) {
    try {
      ...
      logger.info(
        `✨ CodeMorph: Imports organized for ${sourceFile.getFilePath()}`,
      );
    } catch (error) {
