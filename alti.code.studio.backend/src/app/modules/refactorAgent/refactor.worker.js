...
export const refactorWorkerProcessor = async job => {
  ...
  try {
    ...
    if (memoryResults && memoryResults.ids && memoryResults.ids.length > 0) {
      ...
      logger.info(
        `🧠 RefactorWorker [${jobId}]: Found ${memoryResults.ids[0].length} relevant memories.`,
      );
    } else {
      logger.info(
        `🧠 RefactorWorker [${jobId}]: No relevant memories found. Starting fresh.`,
      );
    }
    ...
    for (let attempt = 0; attempt <= MAX_CORRECTION_ATTEMPTS; attempt++) {
      try {
        ...
        syntaxVerified = true;
        logger.info(
          `✅ RefactorWorker [${jobId}]: Syntax verified (attempt ${attempt + 1}).`,
        );
        break;
      } catch (parseError) {
        correctionAttempts++;
        if (attempt < MAX_CORRECTION_ATTEMPTS) {
          logger.warn(
            `⚠️  RefactorWorker [${jobId}]: Syntax error (attempt ${attempt + 1}/${MAX_CORRECTION_ATTEMPTS}): ${parseError.message}. Re-prompting LLM...`,
          );
          refactoredCode = await _selfCorrect(
            refactoredCode,
            parseError.message,
            instruction,
            jobId,
          ...
          logger.error(
            `❌ RefactorWorker [${jobId}]: Syntax still broken after ${MAX_CORRECTION_ATTEMPTS} correction attempts.`,
          );
          return {
            status: 'failed',
