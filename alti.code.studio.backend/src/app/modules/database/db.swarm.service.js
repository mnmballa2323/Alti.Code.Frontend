...
class DBSwarmService {
    ...
    async spinUpTestContainer(testSuiteName) {
        ...
        logger.info(`🐳 [DB Swarm] Pillar 30: Spinning up Ephemeral PostgreSQL Container [ID: ${containerId}] for Test Suite: ${testSuiteName}...`);
        ...
        logger.info(`   [Docker API] Executing Prisma Migrations in isolated container [${containerId}]...`);
        // Simulating prisma db push
        await new Promise(resolve => setTimeout(resolve, 500));
        
        this.activeContainers.set(testSuiteName, { containerId, port, connectionString });
        
        logger.info(`✅ [DB Swarm] Ephemeral DB Ready! Connection: ${connectionString}`);
    ...
    async tearDownTestContainer(testSuiteName) {
        ...
        if (!containerInfo) {
            logger.warn(`⚠️ [DB Swarm] Container for suite ${testSuiteName} not found.`);
            return { success: false, error: "Container not found" };
        }
        
        logger.info(`💥 [DB Swarm] Tearing down Ephemeral PostgreSQL Container [ID: ${containerInfo.containerId}]...`);
        // Simulating docker stop and rm
        await new Promise(resolve => setTimeout(resolve, 400));
        
        this.activeContainers.delete(testSuiteName);
        logger.info(`   [Docker API] Container ${containerInfo.containerId} completely destroyed.`);
