...
export class CodeExecutionSandbox {
  ...
  static async execute(code, options = {}) {
    ...
    const workspacePath = options.workspacePath
      ...
      : resolve(`./logs/workspaces/agent_${cleanAgentId}`);
    ...
    if (provider === 'flue') {
      ...
      try {
        ...
        const runnerPath = resolve(
          new URL('./isolated_vm_runner.js', import.meta.url).pathname,
        );
        let result;
        let usedFallback = false;

        try {
          result = await sessionEnv.exec(
            `node "${runnerPath}" "${tempFileName}"`,
          );
        } catch (e) {
          result = null;
        }
        ...
        if (
          ...
          const fallbackRunnerPath = resolve(
            new URL('./vm_fallback_runner.js', import.meta.url).pathname,
          );
          usedFallback = true;
          try {
            result = await sessionEnv.exec(
              `node "${fallbackRunnerPath}" "${tempFileName}"`,
            );
            if (result && result.stdout) {
              parsedResult = JSON.parse(result.stdout.trim());
            }
    ...
    return await manager.executeOssCode(
      `agent_${cleanAgentId}`,
      code,
      workspacePath,
      options,
