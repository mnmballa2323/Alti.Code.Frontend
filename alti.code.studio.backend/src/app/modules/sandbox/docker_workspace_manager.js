...
export class DockerWorkspaceManager {
  ...
  async startUserContainer(userId, options = {}) {
    ...
    if (!hasDocker) {
      ...
      console.log(
        `⚠️ Docker daemon not responding. Running [${containerName}] in high-fidelity Mock Fallback Mode.`,
      );
    ...
    if (!runResult.success) {
      ...
      console.log(
        `⚠️ Docker container creation failed: ${runResult.error}. Cascading [${containerName}] to Mock Sandbox.`,
      );
      this.activeContainers.add(containerName);
      return {
        containerName,
    ...
    console.log(
      `🚀 Provisioned isolated user container: [${containerName}] -> Mounted: ${hostPath}`,
    );
    this.activeContainers.add(containerName);
    return { containerName, hostPath, isMock: false };
  }
  ...
  async stopUserContainer(userId) {
    const cleanUserId = userId.replace(/[^a-zA-Z0-9_]/g, '');
    const containerName = `user-sandbox-${cleanUserId}`
      .toLowerCase()
      .replace(/_/g, '-');
    ...
    if (process.env.ARM_SUBSCRIPTION_ID) {
      ...
      console.log(
        `🧹 Cleaned up mock state for user container: [${containerName}]`,
      );
    }
  ...
  safeWriteFile(userId, relativeFilePath, content) {
    ...
    if (!targetPath.startsWith(hostPath)) {
      throw new Error(
        `Zero-Trust Trap: Attempted path traversal escape detected. Scoped boundary is: ${hostPath}`,
      );
    }
    ...
    try {
      const resolveSafePath = filePath => {
        ...
        if (!targetPath.startsWith(hostWorkspacePath)) {
          throw new Error(
            `Zero-Trust Violation: Path traversal attempt outside workspace: ${targetPath}`,
          );
        }
        return targetPath;
      };
  ...
  async startOssContainer(moduleName, hostPath, options = {}) {
    ...
    const containerName =
      `oss-container-${cleanModuleName}${isPython ? '-python' : ''}`
        .toLowerCase()
        .replace(/_/g, '-');
    ...
    if (!runResult.success) {
      console.log(
        `⚠️ Docker OSS container launch failed: ${runResult.error}. Cascading to Mock Sandbox.`,
      );
      this.activeContainers.add(containerName);
      return { containerName, hostPath: targetHostPath, isMock: true };
    }
    ...
    console.log(
      `🚀 Provisioned isolated OSS Container: [${containerName}] -> Mounted: ${targetHostPath}`,
    );
    this.activeContainers.add(containerName);
    return { containerName, hostPath: targetHostPath, isMock: false };
  }
  ...
  async executeOssCode(moduleName, code, hostPath, options = {}) {
    ...
    if (provider === 'crabbox') {
      ...
      try {
        const runtimeCmd = isPython ? 'python' : 'node';
        const result = await crabboxService.run(
          `${runtimeCmd} ${tempFileName}`,
          {
            id: options.leaseId,
            provider: options.crabboxProvider,
            class: options.crabboxClass,
    ...
    const containerResult = await this.startOssContainer(
      moduleName,
      targetHostPath,
      options,
    );
    ...
    if (execResult.exitCode === 137 || execResult.killed) {
      errors.push(
        'Error: Execution timeout exceeded. The sandbox process was forcefully terminated.',
      );
    }
  ...
  async pruneOrphanedContainers() {
    if (process.env.ARM_SUBSCRIPTION_ID) {
      ...
      for (const c of containers) {
        if (
          c.Names[0] &&
          (c.Names[0].startsWith('user-sandbox-') ||
    ...
    if (listResult.success && listResult.stdout) {
      ...
      for (const name of names) {
        console.log(
          `🧹 Self-Healing: Pruning orphaned container detected: [${name}]`,
        );
