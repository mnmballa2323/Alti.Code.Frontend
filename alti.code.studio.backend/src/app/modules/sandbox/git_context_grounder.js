...
export class GitContextGrounder {
    ...
    static async getLineBlameContext(filePath, startLine, endLine) {
        ...
        try {
            ...
            const { stdout: blameOutput } = await execAsync(blameCmd, { cwd: process.cwd() });
            ...
            for (const line of lines) {
                ...
                if (firstWord.length === 40 && /^[0-9a-fA-F]+$/.test(firstWord)) {
                    ...
                    if (line.startsWith('author ')) {
                        ...
                        commits[currentHash].date = new Date(timestamp * 1000).toISOString().split('T')[0];
                    }
                }
            }

            // Sort commits by how many lines they affect in this range
            const sortedCommits = Object.values(commits).sort((a, b) => b.count - a.count);
            ...
            for (const commit of sortedCommits.slice(0, 3)) {
                ...
                try {
                    ...
                    const { stdout: showOutput } = await execAsync(showCmd, { cwd: process.cwd() });
                    commitMsg = showOutput.trim();
                } catch (e) {
                    logger.debug(`[Git Blame Grounder] Failed to run git show for ${commit.hash}: ${e.message}`);
                }
            ...
            logger.debug(`[Git Blame Grounder] Failed to fetch blame: ${error.message}`);
