...
export class TenantContainerOrchestrator {
  ...
  async provisionTenantDatabase(
    tenantId,
    dbPassword = 'enterprise_secure_pw_123',
  ) {
    const cleanTenantId = tenantId.replace(/[^a-zA-Z0-9_]/g, '');
    const containerName = `tenant-db-${cleanTenantId}`
      .toLowerCase()
      .replace(/_/g, '-');
    ...
    if (process.env.ARM_SUBSCRIPTION_ID) {
      ...
      if (
        existingGroup &&
        existingGroup.State === 'Running' &&
        existingGroup.ipAddress?.ip
      ) {
      ...
      if (!ipAddress) {
        throw new Error(
          `Failed to obtain ACI IP address for tenant database ${containerName}`,
        );
      }
    ...
    if (inspect.success && inspect.stdout === 'true') {
      const portRes = await this._execCmd(
    ...
    console.log(
      `[Enterprise] Provisioning isolated database for Tenant ${tenantId}...`,
    );
    const runResult = await this._execCmd(dockerRunCmd);

    if (!runResult.success) {
      throw new Error(
        `Failed to provision tenant database: ${runResult.error}`,
      );
    }
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const check = await this._execCmd(
    ...
    if (!isReady) {
      throw new Error(
        `Database for tenant ${tenantId} failed to start in time.`,
      );
    }
    ...
    console.log(
      `[Enterprise] Successfully provisioned isolated database on port ${hostPort}`,
    );
    return databaseUrl;
  }
