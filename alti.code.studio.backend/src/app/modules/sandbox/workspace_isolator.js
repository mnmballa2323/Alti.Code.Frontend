...
export class WorkspaceIsolator {
  ...
  provision() {
    ...
    console.log(
      `📂 Provisioned isolated sandbox workspace: [${id}] -> ${path}`,
    );
  ...
  safeWrite(workspace, relativeFilePath, content) {
    ...
    if (!targetPath.startsWith(workspace.path)) {
      throw new Error(
        `Zero-Trust Trap: Attempted path traversal escape detected. Scoped boundary is: ${workspace.path}`,
      );
    }
  ...
  destroy(workspace) {
    if (existsSync(workspace.path)) {
      ...
      console.log(
        `🧹 Cleanly destroyed isolated sandbox workspace: [${workspace.id}]`,
      );
    }
  }
