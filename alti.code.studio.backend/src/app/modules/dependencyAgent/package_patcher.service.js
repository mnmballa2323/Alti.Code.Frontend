...
class PackagePatcherService {
    ...
    applyPatch(packageName, relativeFilePath, patchedContent, rootDir = './') {
        ...
        if (!fs.existsSync(packageDir)) {
            logger.error(`🩹 PackagePatcher: Package directory not found: ${packageDir}`);
            return false;
        }

        if (!fs.existsSync(targetFile)) {
            logger.error(`🩹 PackagePatcher: Target file to patch not found: ${targetFile}`);
            return false;
        }
        ...
        try {
            ...
            logger.info(`🩹 PackagePatcher: Successfully applied dynamic hotfix patch to [${patchKey}]`);
            return true;
        } catch (e) {
            logger.error(`🩹 PackagePatcher: Failed to apply patch to ${targetFile}`, e);
            return false;
        }
    }
    ...
    rollbackPatch(packageName, relativeFilePath) {
        ...
        if (!record) {
            logger.warn(`🩹 PackagePatcher: No active patch registered for [${patchKey}]. Skipping rollback.`);
            return false;
        }
        ...
        try {
            ...
            logger.info(`🩹 PackagePatcher: Successfully rolled back patch for [${patchKey}]`);
            return true;
        } catch (e) {
            logger.error(`🩹 PackagePatcher: Failed to roll back patch for ${record.filePath}`, e);
            return false;
        }
    }
     ...
     */
    rollbackAll() {
        logger.info('🩹 PackagePatcher: Reverting all applied dependency hotfixes.');
