...
class SlackAmbassadorService {
  ...
  async init() {
    if (!process.env.SLACK_BOT_TOKEN || !process.env.SLACK_SIGNING_SECRET) {
      logger.warn(
        '👔 The Ambassador: Slack tokens missing. Slack Bot disabled.',
      );
      return;
    }
    ...
    try {
      ...
      logger.info(
        '👔 The Ambassador: Slack Bot connected and listening for requirements.',
      );
    } catch (error) {
      logger.error(
        `❌ The Ambassador: Slack Bot failed to start: ${error.message}`,
      );
    }
  }
  ...
  registerListeners() {
    ...
    this.app.event('app_mention', async ({ event, context, client, say }) => {
      try {
        ...
        await say(
          `_Good day <@${event.user}>. The Ambassador is processing your request into technical specifications..._`,
        );
...
Synthesize a precise Sprint Goal:`;
        ...
        await say(
          `✅ *New Sprint Goal Added to Backlog*\n> ${cleanGoal}\n\nThe Swarm will autonomously plan and execute this in the upcoming cycle.`,
        );
        logger.info(
          `👔 The Ambassador: Converted Slack request from ${event.user} into backlog goal.`,
        );
      } catch (error) {
        logger.error(
          `❌ The Ambassador: Failed to process mention: ${error.message}`,
        );
        await say(
          `⚠️ I apologize, <@${event.user}>, but I encountered an error translating your request: ${error.message}`,
        );
      }
    });
  }
