...
class GithubPrService {
    ...
    init() {
        if (!process.env.GITHUB_TOKEN) {
            logger.warn('👔 The Ambassador: GitHub Token missing. GitHub PR automation disabled.');
            return;
        }
        ...
        try {
            ...
            logger.error(`❌ The Ambassador: GitHub Service failed to start: ${error.message}`);
        }
    }
    ...
    async createAutonomousPR({ branch, title, body, reviewers = [] }) {
        ...
        try {
            logger.info(`🐙 The Ambassador: Opening Pull Request for branch ${branch}...`);
    ...
    async handleWebhook(req, res) {
        if (!this.isInitialized) return res.status(503).json({ error: 'GitHub service disabled' });
        ...
        if (event === 'pull_request_review_comment' && req.body.action === 'created') {
            ...
            logger.info(`💬 The Ambassador: Human PR comment detected from @${author}`);
...
Write a highly focused, 1-2 sentence declarative Sprint Goal to fix this specific issue on this specific branch.`;
                ...
                await sprintSchedulerService.addGoalToBacklog(
                    ...
                    1,
                    { source: 'github', branch: pr.head.ref, prNumber: pr.number, author }
                );
                ...
                logger.info(`✅ The Ambassador: PR feedback synthesized into Sprint Goal.`);
                return res.status(200).send('Review handled successfully');

            } catch (err) {
                logger.error(`❌ The Ambassador: Failed to handle PR review: ${err.message}`);
