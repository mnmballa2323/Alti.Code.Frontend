...
const SCAN_RULES = [
  // --- CRITICAL ---
  {
    source: 'AWS_SECRET_KEY|AKIA[0-9A-Z]{16}',
    flags: 'i',
    severity: 'CRITICAL',
  ...
  },
  {
    ...
    flags: '',
    severity: 'CRITICAL',
    message: 'Private key material embedded in code',
  },
  {
  ...
  {
    source: 'new Function\\s*\\(',
    flags: '',
    severity: 'HIGH',
    message: 'Dynamic code execution via new Function()',
  },
  {
    source: 'child_process\\.exec\\s*\\(\\s*`.*\\$\\{',
    flags: 's',
    severity: 'HIGH',
    message: 'Potential command injection via template literal in exec()',
  },
  {
  ...
  // --- MEDIUM ---
  {
    source: 'process\\.env\\b.*console\\.log',
    flags: 's',
    severity: 'MEDIUM',
    message: 'Environment variable may be logged to console',
  },
  {
    source: 'Math\\.random\\(\\)',
    flags: '',
  ...
  },
  {
    ...
    flags: '',
    severity: 'MEDIUM',
    message: 'Non-HTTPS URL in source code',
  },
  {
    source: 'TODO|FIXME|HACK',
    flags: 'i',
    severity: 'MEDIUM',
    message: 'Technical debt marker detected (TODO/FIXME/HACK)',
  },
  {
    source: 'require\\s*\\(\\s*["\'](\\.\\./){4,}',
    flags: '',
    severity: 'MEDIUM',
    message: 'Deep relative path traversal detected',
  },
];
...
const runSemgrepScan = code => {
  ...
  if (findings.length > 0) {
    logger.warn(
      `[${SERVICE_NAME}] Static scan: ${criticals} CRITICAL, ${highs} HIGH, ${mediums} MEDIUM`,
    );
  }
...
const auditCode = async (code, context = 'general') => {
  ...
  if (code.length > 500000) {
    logger.warn(
      `[${SERVICE_NAME}] Code payload exceeds 500KB safe bound limit. Rejecting.`,
    );
    return {
      safe: false,
      score: 0,
      reasoning:
        'Payload size exceeds 500KB limit. Potential denial of service vector.',
  ...
  const blockingFindings = staticFindings.filter(
  ...
  if (blockingFindings.length > 0) {
    ...
    logger.warn(
      `[${SERVICE_NAME}] Static Analysis Blocked: ${messages.join(', ')}`,
    );
    ...
    await EventBus.publish('guardian.audit.rejected', {
      context,
  ...
  try {
    ...
    try {
      ...
      if (auditResult.safe === true) {
        const dualReviewPassed = await auditorService.enforceDualReview(
          redactedCode,
          context,
        );
        if (!dualReviewPassed) {
          ...
          auditResult.reasoning =
            'REJECTED BY ADVERSARIAL AUDITOR (Maker-Checker Policy Violation)';
          auditResult.vulnerabilities.push(
            '[CRITICAL] Failed independent adversarial review',
          );
          await EventBus.publish('guardian.audit.rejected', {
            context,
      ...
      logger.warn(
        `[${SERVICE_NAME}] AI returned unparseable JSON — using static-only result`,
        cleaned.slice(0, 200),
      );
      auditResult = {
        ...
        reasoning:
          'AI response could not be parsed. Score derived from static analysis only.',
        vulnerabilities: staticFindings.map(
    ...
    if (!auditResult.safe) {
      logger.warn(
        `[${SERVICE_NAME}] Code rejected by AI: ${auditResult.reasoning}`,
      );
      await EventBus.publish('guardian.audit.rejected', {
        context,
        reasoning: auditResult.reasoning,
      ...
      logger.info(
        `[${SERVICE_NAME}] Code approved (Score: ${auditResult.score})`,
      );
      await EventBus.publish('guardian.audit.approved', {
        context,
