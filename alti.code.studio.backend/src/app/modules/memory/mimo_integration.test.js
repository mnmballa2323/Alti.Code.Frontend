...
describe('MiMo-Code Active Memory Integration', () => {
  ...
  afterEach(async () => {
    ...
    try {
      const testAgentPath = path.join(
        mimoDistillerService.definitionsDir,
  ...
  describe('MimoCheckpointService', () => {
    ...
    it('should compact context and write milestone checkpoint if size exceeds threshold', async () => {
      ...
      const fileContent = await fs.readFile(
        mimoCheckpointService.checkpointPath,
        'utf8',
      );
  ...
  describe('MimoDreamService', () => {
    it('should process dream cycle and synthesize MEMORY.md', async () => {
      ...
      const memoryFileContent = await fs.readFile(
        mimoDreamService.memoryMdPath,
        'utf8',
      );
...
`;
  ...
  describe('Tri-Brain Consensus Distillation Trigger', () => {
    it('should trigger mimoDistillerService.distill on APPROVED consensus status', async () => {
      const { triBrainService } =
        await import('../agents/tri_brain.service.js');
      const mockDistill = vi
        .spyOn(mimoDistillerService, 'distill')
        .mockResolvedValue({ success: true });
      ...
      expect(mockDistill.mock.calls[0][2]).toContain(
        'agent.distilled.consensus',
      );
    });
  });
  ...
  describe('AutonomousRepairDaemon Dream Trigger', () => {
    it('should trigger mimoDreamService.dream during scanForArchitecturalDecay', async () => {
      const { autonomousRepairDaemon } =
        await import('../agents/autonomous_repair_daemon.js');
      const mockDream = vi
        .spyOn(mimoDreamService, 'dream')
        .mockResolvedValue({ success: true });
      ...
      vi.spyOn(
        autonomousRepairDaemon,
        'calculateLocalDependencyCentrality',
      ).mockResolvedValue([
  ...
  describe('AgentMemoryHooks Local MD Entrenchment', () => {
    it('should inject local memory rules matched by keywords', async () => {
      ...
      const results = await AgentMemoryHooks.recallContext(
        'We need to check the Redis port configuration',
        5,
      );

      expect(results).toContain(
        '[MimoMemory] Redis running on port 6379 in server.js',
      );
      expect(results).not.toContain(
        '[MimoMemory] Postgres database uses pgvector extension',
      );
    ...
    it('should prepend local memory rules to project context', async () => {
      ...
      const { AgentMemoryHooks } = await import('./agentmemory.hooks.js');
      agentMemoryService.getContext.mockResolvedValue({
        context: 'AgentMemory context',
      });
