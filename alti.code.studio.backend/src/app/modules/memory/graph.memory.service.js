...
class GraphMemoryService {
    ...
    async init() {
        ...
        this._seedMockData();
        this.isReady = true;
        logger.info(`✅ [Graph Memory] Mapped ${this.nodes.size} AST nodes and ${this.edges.length} neural edges.`);
    }

    _seedMockData() {
        this.nodes.set('tri_brain.service.js', { type: 'File', owner: 'Project Nova' });
        this.nodes.set('PR-492', { type: 'PullRequest', author: 'michaelmeram', description: 'Liquid Routing matrix' });
        this.edges.push({ source: 'PR-492', target: 'tri_brain.service.js', relation: 'MODIFIED' });
    }
    ...
    async queryIntentGraph(intent) {
        ...
        logger.info(`🧠 [Graph Memory] Traversing relational ontology for intent...`);
