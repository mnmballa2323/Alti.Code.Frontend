...
class PgVectorService {
    ...
    async ingestCodeFile(userId, agentId, codeContent) {
        ...
        logger.info(`🧠 [GodMode] Chunked code file into ${chunks.length} segments.`);
        ...
        for (const chunk of chunks) {
            try {
                ...
                const embedding = await azureSovereignCompatService.getEmbeddings(chunk);
        ...
        logger.info(`✅ [GodMode] Successfully ingested ${successCount}/${chunks.length} chunks into AgentMemory.`);
        return successCount;
    }
