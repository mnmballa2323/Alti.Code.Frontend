...
describe('Global Enterprise Memory (AlloyDB/pgvector)', () => {
  ...
  it('ragService should chunk documents and store them in the vector store', async () => {
    const mockTexts = [
      ...
      'Fragment 2 detailing architecture',
    ];
    ...
    expect(vectorStoreService.add).toHaveBeenCalledWith(
      'Fragment 1 representing code context',
      { source: 'rag_service', chunkIndex: 0 },
      'default_enterprise_tenant',
    );
    expect(vectorStoreService.add).toHaveBeenCalledWith(
      'Fragment 2 detailing architecture',
      { source: 'rag_service', chunkIndex: 1 },
      'default_enterprise_tenant',
    );
  });
  ...
  it('ragService should query the vector store and synthesize a response via Gemini', async () => {
    ...
    expect(vectorStoreService.getByIds).toHaveBeenCalledWith(
      ['test_id'],
      'default_enterprise_tenant',
    );
