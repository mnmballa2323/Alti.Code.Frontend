...
class SemanticCacheService {
    ...
    async init() {
        ...
        try {
            ...
            logger.info('🧠 [SemanticCache] pgvector Semantic Cache Layer initialized successfully.');
        } catch (error) {
            logger.warn(`⚠️ [SemanticCache] Failed to initialize table. Semantic Caching disabled: ${error.message}`);
        }
    }
    ...
    async getCachedResponse(prompt, threshold = 0.98) {
        ...
        try {
            ...
            const embedding = await vertexService.getEmbeddings(prompt);
            ...
            if (results && results.length > 0) {
                ...
                logger.info(`⚡ [SemanticCache] CACHE HIT! Detected ${simScore}% semantic similarity. Bypassing massive LLM execution latency.`);
    ...
    async setCachedResponse(prompt, response) {
        ...
        try {
            ...
            logger.info('🧠 [SemanticCache] Intelligence permanently vectorized and locked into pgvector memory.');
        } catch (err) {
