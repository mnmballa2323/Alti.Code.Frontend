...
class V8MemoryService {
  constructor() {
    logger.info(
      '⚙️ [V8 Memory] Google V8 Native Physical Control initialized.',
    );
  }
  ...
  enforceMemoryLimits(thresholdMB = 1000) {
    ...
    if (usedMB > thresholdMB) {
      logger.warn(
        `⚠️ [V8 Memory] Critical memory pressure detected (${usedMB}MB). Triggering physical V8 Garbage Collection...`,
      );
      ...
      if (global.gc) {
        ...
        logger.info(
          `✅ [V8 Memory] GC Successful. Memory recovered: ${usedMB - postUsedMB} MB. Crisis averted.`,
        );
      } else {
        logger.warn(
          `⚠️ [V8 Memory] global.gc() is not exposed. Please restart the backend with node --expose-gc.`,
        );
      }
    }
  }
