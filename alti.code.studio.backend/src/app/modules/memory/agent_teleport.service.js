...
class AzureAgentTeleportationService {
    constructor() {
        ...
        this.containerName = config.azure?.teleportation_container || 'alti-swarm-v8-snapshots';
        logger.info('🌌 [Agent Teleport] Azure Blob Storage Client initialized for V8 snapshot teleportation.');
    }
    ...
    async teleportConsciousness(agentId) {
        ...
        try {
            ...
            logger.info(`🌌 [Agent Teleport] Beaming consciousness to Azure Blob Storage container [${this.containerName}]...`);
            
            // Read snapshot file content
            const fileContent = fs.readFileSync(localPath);
            await this.storage.uploadFile(this.containerName, snapshotFilename, fileContent);
