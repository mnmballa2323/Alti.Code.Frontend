...
export class ZepMemoryService {
  constructor() {
    ...
    if (this.apiUrl) {
      try {
        ...
        logger.info(
          `🧠 [Zep Memory] Client initialized (Server: ${this.apiUrl}).`,
        );
      } catch (error) {
      ...
      logger.info(
        '🧠 [Zep Memory] ZEP_API_URL not set. Running in local Redis fallback mode.',
      );
    }
  }
  ...
  async addMemory(sessionId, role, content) {
    ...
    try {
      ...
      logger.info(
        `🧠 [Zep Memory] Successfully appended ${zepRole} message to session ${sessionId}.`,
      );
    } catch (error) {
  ...
  async getMemory(sessionId) {
    ...
    try {
      ...
      logger.warn(
        `⚠️ [Zep Memory] Failed to fetch session memory: ${error.message}`,
      );
    }
