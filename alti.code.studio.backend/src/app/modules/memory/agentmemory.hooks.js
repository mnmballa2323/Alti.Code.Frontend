...
class AgentMemoryHooks {
  ...
  static async startSession(
    ...
    try {
      ...
      logger.info(
        `[AgentMemory:Hook] 🟢 SessionStart: User ${userId} → Session ${sessionId}`,
      );
  ...
  static async endSession(userId, summary = '') {
    ...
    try {
      ...
      logger.info(
        `[AgentMemory:Hook] 🔴 SessionEnd: Session ${sessionId} → Memories consolidated`,
      );
      return result;
    } catch (err) {
  ...
  static async captureToolUse(agentName, task, result, metadata = {}) {
    ...
    if (isDuplicate(sanitized)) {
      logger.debug(
        `[AgentMemory:Hook] Dedup: Skipping duplicate observation for ${agentName}`,
      );
      return;
    }
    ...
    try {
      ...
      logger.debug(
        `[AgentMemory:Hook] PostToolUse capture failed: ${err.message}`,
      );
    }
  }
  ...
  static async captureToolFailure(agentName, task, error, metadata = {}) {
    ...
    try {
      ...
      logger.debug(
        `[AgentMemory:Hook] PostToolUseFailure capture failed: ${err.message}`,
      );
    }
  }
  ...
  static async captureUserPrompt(userId, prompt, metadata = {}) {
    ...
    try {
      ...
      logger.debug(
        `[AgentMemory:Hook] UserPromptSubmit capture failed: ${err.message}`,
      );
    }
  }
   ...
   */
  static async captureWorkflowExecution(
    workflowId,
    nodes,
    edges,
    executionTimeMs,
    ...
    try {
      ...
      logger.debug(
        `[AgentMemory:Hook] Workflow capture failed: ${err.message}`,
      );
    }
  }
  ...
  static async captureDecision(decision, reasoning, metadata = {}) {
    ...
    try {
      ...
      logger.debug(
        `[AgentMemory:Hook] Decision capture failed: ${err.message}`,
      );
    }
  }
  ...
  static async recallContext(query, limit = 5) {
    ...
    if (agentMemoryService.isReady) {
      try {
        ...
        if (result?.results && Array.isArray(result.results)) {
          memories = result.results.map(
            r => `[Memory] ${r.content || r.text || JSON.stringify(r)}`,
          );
        }
      } catch (err) {
        logger.debug(
          `[AgentMemory:Hook] Context recall failed: ${err.message}`,
        );
      }
    }
    ...
    try {
      const localRules =
        await AgentMemoryHooks._getMatchedLocalMemoryRules(query);
      const formattedRules = localRules.map(r => `[MimoMemory] ${r}`);
      const merged = [...formattedRules, ...memories];
      return merged.slice(0, limit);
    } catch (err) {
      logger.debug(
        `[AgentMemory:Hook] Failed to merge local memory rules: ${err.message}`,
      );
  ...
  static async getProjectContext(tokenBudget = 2000) {
    ...
    if (agentMemoryService.isReady) {
      try {
        ...
        logger.debug(
          `[AgentMemory:Hook] Project context failed: ${err.message}`,
        );
      }
    }
