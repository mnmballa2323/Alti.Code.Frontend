...
export const gitWorkerProcessor = async (job) => {
    ...
    try {
        ...
        if (changelog) {
            ...
            let existing = '';
            try { existing = await fs.readFile(changelogPath, 'utf-8'); } catch { /* file doesn't exist yet */ }
        ...
        logger.info(`✅ GitWorker [${jobId}]: Committed: "${commitMsg}" (${commitResult.commit || 'no changes'})`);
        ...
        if (tag) {
            try {
                ...
                logger.warn(`⚠️ GitWorker [${jobId}]: Tag failed (may already exist): ${tagErr.message}`);
            }
        }
        ...
        try {
            ...
            logger.info(`🚀 GitWorker [${jobId}]: Pushed to origin/${currentBranch}.`);
        }
