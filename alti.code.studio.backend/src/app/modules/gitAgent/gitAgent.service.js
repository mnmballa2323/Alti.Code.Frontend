...
const commitChanges = async (message, { skipStage = false } = {}) => {
  if (!message || typeof message !== 'string' || message.trim().length === 0) {
    throw new Error(
      'GitAgentService: commit message must be a non-empty string.',
    );
  }
  try {
    ...
    logger.info(
      `💾 Git Agent: Committed via Isomorphic-Git [${sha.substring(0, 7)}] - "${message}"`,
    );
...
const startWatcher = async (dir = process.cwd()) => {
  ...
  watcher
    ...
    .on('change', filePath => {
      logger.info(`✏️  File changed: ${filePath}`);
      debouncedSync();
    })
    .on('unlink', filePath => {
...
const createBranch = async name => {
  ...
  if (!SAFE_BRANCH_RE.test(name)) {
    throw new Error(
      `GitAgentService: invalid branch name "${name}". Use only alphanumeric, hyphen, or slash characters.`,
    );
  }
  try {
...
const initializeRepository = async (
  ...
  if (!process.env.GITHUB_PAT)
    throw new Error(
      'GITHUB_PAT environment variable is utterly required for Genesis autonomous repo creation.',
    );
  ...
  try {
    ...
    if (e.status === 422) {
      // Already exists
      logger.warn(
        `⚠️ GitHub Repository "${repoName}" already exists. Proceeding with scaffolding phase...`,
      );
  ...
  const authUrl = githubUrl.replace(
    'https://',
    `https://${process.env.GITHUB_PAT}@`,
  );

  // 2. Provision Local Shadow Workspace
  const shadowSpace = path.join(
    process.cwd(),
    '.shadow',
    'workspaces',
    repoName,
  ...
  if (!isRepo) {
    logger.info(
      `⬇️ Genesis Engine: Cloning repository into shadow workspace...`,
    );
  ...
  logger.info(
    `🏗️ Genesis Engine: Scaffolding baseline files in ${shadowSpace}`,
  );
  ...
  if (!status.isClean()) {
    await localGit.commit(
      'chore: Genesis Engine autonomous workspace scaffolding',
    );
    await localGit.push('origin', 'main');
    logger.info(
      `✅ Genesis Engine: Scaffolding committed and pushed to remote.`,
    );
  }
...
const processPullRequestWebhook = async payload => {
  ...
  logger.info(
    `🐙 GitOps: PR #${prNumber} updated on ${branchName}. Initiating QA Swarm...`,
  );
  ...
  try {
    ...
    if (testResults.exitCode !== 0) {
      logger.warn(
        `❌ GitOps: Tests failed on PR #${prNumber}. Triggering Self-Healing Sprint...`,
      );
...
`;
      ...
      logger.info(
        `✅ GitOps: Tests passed seamlessly on PR #${prNumber}. No action required.`,
      );
...
const submitProactivePullRequest = async (
  targetRepoOwner,
  targetRepoName,
  branchName,
  title,
  ...
  logger.info(
    `🐙 GitOps: Submitting proactive Pull Request to ${targetRepoOwner}/${targetRepoName}...`,
  );

  if (!process.env.GITHUB_PAT)
    throw new Error('Missing GITHUB_PAT for PR submission');
...
const getUserGithubToken = async (userId) => {
  ...
  try {
    ...
    if (githubConn && githubConn.credentials) {
      const decrypted = await encryptionService.decrypt(githubConn.credentials);
...
const searchRepositories = async (query = '', userId = null) => {
