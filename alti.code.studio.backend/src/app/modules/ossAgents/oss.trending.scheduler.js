...
export class OssTrendingScheduler {
  ...
  async start() {
    ...
    logger.info(
      `📅 OssTrendingScheduler: First scan in ${Math.round(msUntilNextRun / 60000)} min`,
    );
    ...
    if (stats.total === 0) {
      logger.info(
        '📅 OssTrendingScheduler: No existing OSS agents — running seed scan now…',
      );
      if (process.env.NODE_ENV === 'production') {
        setTimeout(
          () =>
            this._runScan('seed').catch(e =>
              logger.error('OssTrendingScheduler seed error:', e),
            ),
          5000,
        );
      } else {
        logger.info(
          '📅 OssTrendingScheduler: Skipped seed scan (Local Development mode)',
        );
      }
    }
  ...
  async forceCreate(repoUrl) {
    ...
    const readme = await gitHubTrendingService
      .fetchReadme(slug)
      .catch(() => '');
    const repo = {
      slug,
  ...
  _scheduleDailyCron() {
    this._runScan('scheduled').catch(e =>
      logger.error('OssTrendingScheduler cron error:', e),
    );
  ...
  async _runScan(trigger = 'scheduled', language = '') {
    logger.info(
      `📅 OssTrendingScheduler: Scan started (trigger=${trigger}, language=${language || 'all'})`,
    );
    ...
    try {
      ...
      logger.info(
        `📅 OssTrendingScheduler: ${trending.length} trending, ${newRepos.length} new → creating agents…`,
      );
    ...
    logger.info(
      `📅 OssTrendingScheduler: Scan done — created=${created}, errors=${errors}, total=${summary.totalAgents}`,
    );
  ...
  async _rehydrateAgents() {
    ...
    logger.info(
      `📅 OssTrendingScheduler: Re-hydrating ${records.length} OSS agents into router…`,
    );
    let loaded = 0;
    let failed = 0;
    ...
    logger.info(
      `📅 OssTrendingScheduler: Rehydrated ${loaded} agents (${failed} missing files skipped)`,
    );
