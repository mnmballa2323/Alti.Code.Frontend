...
export class OssAgentRegistry {
  ...
  async load() {
    ...
    try {
      ...
      logger.info(
        `📚 OssRegistry: Loaded ${this._records.size} agents from disk`,
      );
    } catch {
      logger.info(
        '📚 OssRegistry: No existing registry found — starting fresh',
      );
    }
    this._loaded = true;
  }
  ...
  async add(record) {
    ...
    await this._persist();
    logger.info(
      `📚 OssRegistry: Saved agent for ${record.slug} (total: ${this._records.size})`,
    );
  }
  ...
  list(filter = {}) {
    ...
    if (filter.language) {
      results = results.filter(
        r => r.language?.toLowerCase() === filter.language.toLowerCase(),
      );
    }
