...
export class OssAgentFactory {
    ...
    async createAgentForRepo(repo) {
        ...
        if (ossAgentRegistry.has(repo.slug)) {
            logger.info(`🏭 OssFactory [${agentId}]: Agent for ${repo.slug} already exists — skipping`);
            return { agentId, status: 'exists', slug: repo.slug };
        }

        logger.info(`🏭 OssFactory [${agentId}]: Synthesizing agent for ${repo.slug} (${repo.stars?.toLocaleString()} ⭐)…`);
        swarmBus.emit('oss:agent_synthesis_started', { agentId, slug: repo.slug, stars: repo.stars });
        ...
        try {
            ...
            const agentCode = await this._synthesizeOssAgentCode(repo, readme, agentName, exportName, keywords);
            ...
            if (!guardianResult.isSafeToExecute) {
                logger.warn(`🛡️ OssFactory: Guardian BLOCKED ${repo.slug}: ${guardianResult.vetoReason}`);
                swarmBus.emit('oss:agent_blocked', { agentId, slug: repo.slug, reason: guardianResult.vetoReason });
            ...
            swarmBus.emit('oss:agent_written', { agentId, slug: repo.slug, filePath });
            ...
            swarmBus.emit('oss:agent_created', { agentId, slug: repo.slug, agentName, registered, keywords });
            logger.info(`✅ OssFactory [${agentId}]: ${agentName} created for ${repo.slug} (registered=${registered})`);

            return { agentId, status: 'created', slug: repo.slug, agentName, filePath, registered, keywords };

        } catch (error) {
            logger.error(`❌ OssFactory [${agentId}]: Failed for ${repo.slug} —`, error.message);
            swarmBus.emit('oss:agent_failed', { agentId, slug: repo.slug, error: error.message });
            return { agentId, status: 'failed', slug: repo.slug, error: error.message };
        }
    }
    ...
    async createBatch(repos, cap = this.batchCap) {
        ...
        const results = [];
        logger.info(`🏭 OssFactory: Starting batch of ${toProcess.length} repos (cap=${cap})`);
        ...
        logger.info(`🏭 OssFactory: Batch done — created=${created}, skipped=${skipped}, failed=${results.length - created - skipped}`);
        swarmBus.emit('oss:batch_done', { created, skipped, total: results.length });
    ...
    async _synthesizeOssAgentCode(repo, readme, agentName, exportName, keywords) {
        ...
        const prompt = `You are synthesizing a specialist AI agent for the open-source repository "${repo.fullName}".
...
- No TODO comments — everything must be implemented`.trim();
    ...
    _generateKeywords(repo) {
        ...
        if (repo.description) {
            const descWords = repo.description.toLowerCase()
                .replace(/[^a-z0-9 ]/g, ' ')
                .split(/\s+/)
                .filter(w => w.length > 4 && !['their', 'about', 'using', 'with', 'from', 'that', 'this', 'have', 'will'].includes(w));
    ...
    async _hotLoad(filePath, agentName, keywords) {
        try {
            ...
            logger.info(`🔌 OssFactory: Hot-loaded ${agentName} with ${keywords.length} keywords`);
            return true;
        } catch (err) {
            logger.error(`🏭 OssFactory: Hot-load failed for ${agentName}: ${err.message}`);
            return false;
        }
    }
    ...
    _toSlugSafe(slug) {
        return slug.replace('/', '__').replace(/[^a-zA-Z0-9_.-]/g, '_').toLowerCase();
    }
}
