...
const DEFAULT_REPO_REGISTRY = [
    { id: 'backend', name: 'Alti Backend', path: process.env.BACKEND_REPO_PATH || path.resolve(process.cwd()) },
    { id: 'frontend', name: 'Alti Frontend', path: process.env.FRONTEND_REPO_PATH || path.resolve(process.cwd(), '../alti.code.studio.frontend') },
    { id: 'infra', name: 'Alti Infrastructure', path: process.env.INFRA_REPO_PATH || path.resolve(process.cwd(), '../alti.code.studio.infra') },
    { id: 'monorepo', name: 'Alti Monorepo', path: process.env.MONOREPO_PATH || path.resolve(process.cwd(), '../..') },
];
...
export class MultiRepoOrchestrationService {
    constructor(repoRegistry = DEFAULT_REPO_REGISTRY) {
        ...
        logger.info(`🔗 MultiRepoOrchestration: Initialized with ${this.repos.length} repos.`);
    }
    ...
    async scanAllRepos(goal) {
        logger.info(`🔎 MultiRepo: Scanning ${this.repos.length} repos for goal: "${goal.substring(0, 60)}"`);
...
Only JSON, no markdown.`.trim();
                    ...
                    return { repoId: repo.id, repoName: repo.name, path: repo.path, exists: false, error: error.message, relevant: false };
                }
            })
        );

        return scans.map(r => r.status === 'fulfilled' ? r.value : { error: r.reason?.message, relevant: false });
    }
    ...
    async planCrossRepoChanges(goal, scanResults) {
        ...
        const repoSummary = relevantRepos.map(r => `- ${r.repoId}: ${r.repoName} (branch: ${r.currentBranch}, ${r.uncommittedChanges} uncommitted, affected: ${r.affectedFilePatterns.join(', ')})`).join('\n');
...
Return JSON (no markdown):
{
  ...
  "steps": [
    ...
    { "repoId": "frontend", "action": "modify_file", "description": "...", "files": ["app/..."], "details": "..." }
        ...
        logger.info(`📋 MultiRepo: Plan generated [${taskId}] — ${plan.steps?.length ?? 0} cross-repo steps.`);
        this._emit('multirepo:plan', { taskId, plan });

        return { taskId, plan: plan.steps || [], relevantRepos, strategy: plan.strategy };
    }
    ...
    async executeAtomically(taskId, branchName, plan, relevantRepos) {
        ...
        logger.info(`⚡ MultiRepo [${taskId}]: Executing ${plan.length} steps atomically on branch "${branchName}"`);
        ...
        try {
            ...
            for (const repo of relevantRepos) {
                ...
                try {
                    ...
                    this._emit('multirepo:branch_created', { taskId, repoId: repo.repoId, branchName });
                } catch (e) {
            ...
            for (let i = 0; i < plan.length; i++) {
                ...
                if (!repoEntry) {
                    results.push({ step: i, repoId: step.repoId, status: 'skipped', reason: 'repo_not_found' });
                    continue;
                }

                this._emit('multirepo:step:start', { taskId, stepIndex: i, repoId: step.repoId, action: step.action });
                logger.info(`🔧 MultiRepo [${taskId}] Step ${i}: [${step.repoId}] ${step.description}`);

                const stepResult = await this._executeRepoStep(step, repoEntry, taskId);
                results.push({ step: i, ...stepResult });

                if (stepResult.status === 'failed') {
                    throw new Error(`Step ${i} failed in ${step.repoId}: ${stepResult.error}`);
                }

                this._emit('multirepo:step:done', { taskId, stepIndex: i, result: stepResult });
            }
            ...
            logger.info(`✅ MultiRepo [${taskId}]: All ${plan.length} steps completed successfully.`);
            this._emit('multirepo:completed', { taskId, branchName, stepsCompleted: plan.length });
            return { taskId, status: 'completed', results, branchName };

        } catch (error) {
            logger.error(`❌ MultiRepo [${taskId}]: FAILED — rolling back all repos`, error.message);
            ...
            for (const { repoId, git } of createdBranches) {
                try {
                    ...
                    logger.error(`MultiRepo rollback failed for ${repoId}:`, rollbackErr.message);
                }
            }

            return { taskId, status: 'failed', error: error.message, results, rolledBack: true };
        }
    }
    ...
    async runCrossRepoTask(goal, opts = {}) {
        ...
        const { plan, relevantRepos, strategy } = await this.planCrossRepoChanges(goal, scanResults);

        if (plan.length === 0) {
            return { taskId, status: 'no_plan', reason: 'No relevant repos or no steps generated', scanResults };
        }

        const result = await this.executeAtomically(taskId, branchName, plan, relevantRepos);
    ...
    async _executeRepoStep(step, repoEntry, taskId) {
        ...
        try {
            if (step.action === 'run_command') {
                ...
                const output = await GeminiAiService.generateContent(`Generate implementation for: ${step.details || step.description} in repo ${repoId}`);
                return { repoId, action: step.action, status: 'success', output: output.substring(0, 500) };
                ...
                return { repoId, action: step.action, file: step.files[0], status: 'success' };
            } else {
                // Default: describe planned change
                return { repoId, action: step.action, status: 'planned', description: step.description };
            }
        } catch (error) {
            return { repoId, action: step.action, status: 'failed', error: error.message };
        }
    }
    ...
    async detectSubmoduleBreaks(astDiff) {
        logger.info(`🕸️ The Weaver: Auditing AST diff for submodule boundary violations...`);
        const prompt = `
...
Only JSON, no markdown.`.trim();
        ...
        try {
            ...
            if (result.breaksSubmodule) {
                logger.warn(`🕸️ The Weaver: Breaking change detected! Reason: ${result.reason}`);
            }
    ...
    async forkAndPropagate(submodulePath, astChanges) {
        logger.info(`🕸️ The Weaver: Propagating AST changes to submodule at ${submodulePath}`);
...
Only JSON, no markdown.`.trim();
            ...
            await git.add('.');
            await git.commit('feat(weaver): automated submodule sync via AST propagation');

            this._emit('multirepo:submodule:propagated', { submodulePath, branchName, changes: updates.length });
            return { status: 'success', branch: branchName };
        } catch (e) {
            logger.error(`🕸️ The Weaver: Submodule propagation failed: ${e.message}`);
            // Rollback
            try { await git.checkout('main'); } catch (e2) { }
    ...
    registerRepo(repo) {
        if (!repo.id || !repo.path) throw new Error('MultiRepoOrchestration: repo must have id and path.');
        this.repos.push(repo);
        logger.info(`🔗 MultiRepo: Registered new repo "${repo.id}" at ${repo.path}`);
    }
