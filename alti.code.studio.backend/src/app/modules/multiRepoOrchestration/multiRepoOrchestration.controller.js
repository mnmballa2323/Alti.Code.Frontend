...
import {
  ...
  multiRepoBus,
} from '../multiRepoOrchestration/multiRepoOrchestration.service.js';
import {
  selfExpandingSwarmService,
  swarmBus,
} from '../agents/self_expanding_swarm.service.js';
...
multiRepoRouter.post('/plan', async (req, res) => {
  ...
  if (!goal || !Array.isArray(scanResults)) {
    return res.status(400).json({
      success: false,
      message: 'goal and scanResults[] are required.',
    });
  }
  try {
    const result = await multiRepoOrchestrationService.planCrossRepoChanges(
      goal,
      scanResults,
    );
...
multiRepoRouter.post('/run', async (req, res) => {
  ...
  if (!goal)
    return res
      ...
      .json({ success: false, message: 'goal is required.' });
  try {
    const result = await multiRepoOrchestrationService.runCrossRepoTask(goal, {
      branchPrefix,
    });
    return res.json({ success: true, data: result });
  } catch (e) {
...
multiRepoRouter.post('/register', (req, res) => {
  ...
  if (!id || !repoPath)
    return res
      .status(400)
      .json({ success: false, message: 'id and path are required.' });
  try {
    multiRepoOrchestrationService.registerRepo({
      id,
      name: name || id,
      path: repoPath,
    });
...
multiRepoRouter.get('/events', (req, res) => {
  ...
  const multiEvents = [
    'multirepo:task_started',
    'multirepo:plan',
    'multirepo:branch_created',
    'multirepo:step:start',
    'multirepo:step:done',
    'multirepo:committed',
    'multirepo:completed',
    'multirepo:rollback',
  ...
  const swarmEvents = [
    'swarm:gap_detected',
    'swarm:synthesis_started',
    'swarm:agent_written',
    'swarm:agent_registered',
    'swarm:synthesis_blocked',
    'swarm:synthesis_failed',
  ];
