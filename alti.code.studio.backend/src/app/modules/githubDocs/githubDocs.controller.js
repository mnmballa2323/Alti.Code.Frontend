...
export class GithubDocsController {
  ...
  static triggerSync = catchAsync(async (req, res) => {
    ...
    sendResponse(res, {
      ...
      success: true,
      message:
        'GitHub developer documentation sync initiated in the background.',
  ...
  static consultSwarm = catchAsync(async (req, res) => {
    ...
    const consultResult = await githubDocsService.dispatchQueryToSwarm(
      query,
      agentId,
    );
  ...
  static orchestrate = catchAsync(async (req, res) => {
    ...
    const orchestrationResult =
      await githubDocsService.orchestrateSwarmWorkflow(query);
  ...
  static processSelfHealingWebhook = catchAsync(async (req, res) => {
    ...
    const healingResult =
      await githubDocsService.processSelfHealingWebhook(payload);
  ...
  static auditPrVisualLayout = catchAsync(async (req, res) => {
    ...
    const auditResult = await githubDocsService.auditPrVisualLayout(
      base64Image,
      layoutParams,
    );
  ...
  static analyzeBlastRadius = catchAsync(async (req, res) => {
    ...
    const blastRadiusResult =
      await githubDocsService.analyzePullRequestBlastRadius(filesChanged);
  ...
  static triggerAutopilotPr = catchAsync(async (req, res) => {
    ...
    const { githubAutopilotService } =
      await import('./githubAutopilot.service.js');
    const autopilotResult =
      await githubAutopilotService.createPullRequestAutopilot(userIntent);
  ...
  static consultStatefulSession = catchAsync(async (req, res) => {
    ...
    const { githubAutopilotService } =
      await import('./githubAutopilot.service.js');
    const sessionResult = await githubAutopilotService.consultStatefulSession(
      sessionId,
      message,
    );
