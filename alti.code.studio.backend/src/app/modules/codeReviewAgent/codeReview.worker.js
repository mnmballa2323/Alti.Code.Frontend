...
export const codeReviewWorkerProcessor = async (job) => {
    const { code, filePath, context, commitHash, from, to, useOcr = true } = job.data;
    logger.info(`🔍 CodeReview [${job.id}]: Reviewing ${filePath || 'diff'}...`);

    let ocrFeedbackContext = '';
    if (useOcr || commitHash || (from && to)) {
        try {
            logger.info(`🔍 CodeReview [${job.id}]: Running Alibaba open-code-review engine...`);
            let ocrResult = '';
            if (commitHash) {
...
`;
            ...
            logger.warn(`⚠️ [CodeReviewWorker] Open-code-review execution failed: ${err.message}`);
        }
    }
