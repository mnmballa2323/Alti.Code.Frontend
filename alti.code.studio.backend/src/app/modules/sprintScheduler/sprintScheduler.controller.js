...
import { sprintSchedulerService, schedulerBus } from './sprintScheduler.service.js';
...
schedulerRouter.post('/start', (req, res) => {
    ...
    try {
        const result = sprintSchedulerService.start({ intervalMs, scanDir, runNow });
...
schedulerRouter.post('/trigger', async (req, res) => {
    ...
    try {
        ...
        res.status(202).json({ success: true, message: 'Scheduler cycle triggered.', reason });
...
schedulerRouter.post('/backlog/add', (req, res) => {
    ...
    if (!goal || typeof goal !== 'string') {
        return res.status(400).json({ success: false, message: 'goal (string) is required.' });
    }
...
schedulerRouter.get('/history', (req, res) => {
    ...
    return res.json({ success: true, data: sprintSchedulerService.getHistory(limit) });
});
...
schedulerRouter.get('/events', (req, res) => {
    ...
    const events = [
        'scheduler:started', 'scheduler:stopped', 'scheduler:triggered',
        'scheduler:cycle:start', 'scheduler:cycle:done', 'scheduler:cycle:idle', 'scheduler:cycle:skipped',
        'scheduler:phase', 'scheduler:signals', 'scheduler:goals:synthesized',
        'scheduler:backlog:updated', 'scheduler:backlog:cleared',
        'scheduler:sprint:done', 'scheduler:sprint:failed',
    ];
