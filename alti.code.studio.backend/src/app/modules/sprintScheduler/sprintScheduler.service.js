...
class CodebaseObserver {
                    ...
                    for (let i = 0; i < lines.length; i++) {
                        ...
                        if (match) {
                            signals.push({
                                ...
                                severity: type === 'UNSAFE' ? 'critical' : type === 'TODO' ? 'medium' : 'low',
    ...
    async _queryMemorySignals() {
        const results = [];
        try {
            const failSearch = await vectorStoreService.search('sprint_outcome tests failed self-heal', 5).catch(() => []);
            const docs = Array.isArray(failSearch?.documents?.[0]) ? failSearch.documents[0]
                : Array.isArray(failSearch) ? failSearch.map(r => typeof r === 'string' ? r : r?.text ?? '') : [];
...
class GoalSynthesizer {
    ...
    async synthesize(signals) {
        ...
        let pastGoals = '';
        try {
            pastGoals = await ragService.query('recent sprint goals and outcomes completed', 5);
        } catch { /* memory unavailable */ }

        const signalList = signals
            .sort((a, b) => ({ critical: 0, high: 1, medium: 2, low: 3 }[a.severity] - { critical: 0, high: 1, medium: 2, low: 3 }[b.severity]))
            .slice(0, 20)
            .map(s => `[${s.severity.toUpperCase()}] ${s.type} @ ${path.basename(s.file)}:${s.line} — "${s.text}"`)
            .join('\n');

        const prompt = `You are the Goal Synthesizer for an autonomous AI engineering system.
  ...
  "goals": [
    {
      ...
      "sourceSignals": ["signal id or description"]
        ...
        try {
            ...
            return (parsed.goals || []).map(g => ({ ...g, synthesizedAt: new Date().toISOString() }));
        } catch (e) {
            logger.warn(`GoalSynthesizer: LLM parse failed — ${e.message}. Using heuristic fallback.`);
...
export class SprintSchedulerService {
    constructor() {
        ...
        logger.info('🗓️ SprintSchedulerService v6.3.0: Autonomous Goal Synthesis initialized.');
    }
    ...
    start(opts = {}) {
        ...
        const { intervalMs = DEFAULT_INTERVAL_MS, scanDir = process.cwd(), runNow = true } = opts;
        this.scanDir = scanDir;
        this.running = true;

        logger.info(`🗓️ Scheduler: Starting — interval ${intervalMs / 60000}min, dir: ${scanDir}`);
        this._emit('scheduler:started', { intervalMs, scanDir });

        if (runNow) {
            setImmediate(() => this.runCycle().catch(e => logger.error('Scheduler cycle error:', e.message)));
        }

        this._cronTimer = setInterval(
            () => this.runCycle().catch(e => logger.error('Scheduler cycle error:', e.message)),
            intervalMs
        );
    ...
    async runCycle() {
        ...
        this._emit('scheduler:cycle:start', { cycleId, backlogSize: this.backlog.length });

        // Guard: don't start a new sprint if one is already executing
        if (this.activeScheduledSprint) {
            logger.warn(`🗓️ Scheduler [${cycleId}]: Sprint still active — skipping this cycle.`);
            this._emit('scheduler:cycle:skipped', { cycleId, reason: 'sprint_in_progress' });
            return;
        }
        ...
        this._emit('scheduler:signals', { cycleId, count: signals.length, signals: signals.slice(0, 5) });

        // 2. SYNTHESIZE
        this._emit('scheduler:phase', { cycleId, phase: 'SYNTHESIZE' });
        const newGoals = await this.synthesizer.synthesize(signals);
        logger.info(`🗓️ Scheduler [${cycleId}]: Synthesized ${newGoals.length} goals.`);
        this._emit('scheduler:goals:synthesized', { cycleId, count: newGoals.length, goals: newGoals });
        ...
        if (Math.random() < 0.05) {
            this._emit('scheduler:phase', { cycleId, phase: 'METAMORPHIC_REFLECTION' });
        ...
        const existing = new Set(this.backlog.map(g => g.goal.toLowerCase().substring(0, 40)));
        ...
        if (this.backlog.length > BACKLOG_LIMIT) this.backlog = this.backlog.slice(0, BACKLOG_LIMIT);
        this._emit('scheduler:backlog:updated', { cycleId, backlogSize: this.backlog.length, backlog: this.backlog });
        ...
        if (this.backlog.length > 0) {
            ...
            logger.info(`🗓️ Scheduler [${cycleId}]: Launching sprint for goal (p${topGoal.priority}): "${topGoal.goal.substring(0, 80)}"`);

            try {
                this.activeScheduledSprint = topGoal;
                sprintResult = await autonomousSprintService.launchSprint(topGoal.goal, {
                    repoPath: this.scanDir,
                    autoDeploy: true,
                    autoDoc: true,
                    branchPrefix: 'auto-sprint',
                });
                this._emit('scheduler:sprint:done', { cycleId, goal: topGoal.goal, status: sprintResult.status });
            } catch (e) {
                logger.error(`🗓️ Scheduler [${cycleId}]: Sprint failed — ${e.message}`);
                this._emit('scheduler:sprint:failed', { cycleId, goal: topGoal.goal, error: e.message });
            } finally {
                this.activeScheduledSprint = null;
            }
        } else {
        ...
        await this._writeGovernanceDigest(cycleRecord, signals, newGoals, sprintResult);

        logger.info(`🗓️ Scheduler [${cycleId}]: Cycle done in ${cycleRecord.durationMs}ms.`);
    ...
    getHistory(limit = 10) { return this.runHistory.slice(0, limit); }
    ...
    addGoalToBacklog(goal, priority = 3) {
        ...
        this.backlog.sort((a, b) => a.priority - b.priority);
        if (this.backlog.length > BACKLOG_LIMIT) this.backlog = this.backlog.slice(0, BACKLOG_LIMIT);
        this._emit('scheduler:backlog:updated', { backlogSize: this.backlog.length, backlog: this.backlog });
        return entry;
    }
    ...
    async _writeGovernanceDigest(cycle, signals, goals, sprintResult) {
        try {
            const digestPath = path.join(process.cwd(), 'reports', `scheduler_digest_${new Date().toISOString().split('T')[0]}.md`);
            await fs.mkdir(path.dirname(digestPath), { recursive: true });

            const top5Signals = signals.slice(0, 5).map(s =>
                `- \`[${s.severity}] ${s.type}\` @ \`${path.basename(s.file)}:${s.line}\` — ${s.text.substring(0, 80)}`
            ).join('\n');

            const goalList = goals.slice(0, 5).map(g =>
                `- **p${g.priority}** ${g.goal.substring(0, 100)}`
            ).join('\n');
...
`;
            ...
            let existing = '';
            try { existing = await fs.readFile(digestPath, 'utf-8'); } catch { }
    ...
    _emit(event, data) {
        ...
        logger.info(`🗓️ SchedulerBus [${event}]: ${JSON.stringify(data).substring(0, 120)}`);
    }
}
