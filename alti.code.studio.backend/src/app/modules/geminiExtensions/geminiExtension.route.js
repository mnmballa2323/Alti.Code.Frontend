  ...
  catchAsync(async (req, res) => {
    ...
    if (!extensionName)
      return sendResponse(res, {
        statusCode: httpStatus.BAD_REQUEST,
        success: false,
        message: 'extensionName is required.',
        data: null,
      });
    const data = await GeminiExtensionService.invokeExtension(
      extensionName,
      inputArgs || [],
    );
    sendResponse(res, {
...
router.delete(
  '/uninstall/:name',
  catchAsync(async (req, res) => {
    const data = await GeminiExtensionService.uninstallExtension(
      req.params.name,
    );
    sendResponse(res, {
      statusCode: httpStatus.OK,
