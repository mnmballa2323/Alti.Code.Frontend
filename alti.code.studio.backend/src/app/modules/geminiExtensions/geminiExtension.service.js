...
const invokeExtension = async (extensionName, inputArgs) => {
    try {
        ...
        const argStr = (inputArgs || []).map(a => `"${String(a).replace(/"/g, '\\"')}"`).join(' ');
...
const installExtension = async (githubUrl) => {
    try {
        const { stdout } = await execPromise(`gemini extensions install ${githubUrl}`);
        return stdout;
    } catch (error) {
        throw new ApiError(
            httpStatus.INTERNAL_SERVER_ERROR,
...
const uninstallExtension = async (extensionName) => {
    try {
        const sanitized = extensionName.replace(/[^a-zA-Z0-9_\-]/g, '');
        const { stdout } = await execPromise(`gemini extensions uninstall ${sanitized}`);
        return stdout;
    } catch (error) {
        throw new ApiError(
            httpStatus.INTERNAL_SERVER_ERROR,
