...
const preloadExtensions = async () => {
  try {
    ...
    const prebundled = fs.readdirSync(submodulesDir).filter(item => {
      ...
      return fs.statSync(fullPath).isDirectory() && /^[a-zA-Z0-9_\-]+$/.test(item);
    });
    ...
    const isCacheValid = prebundled.length > 0 &&
      prebundled.every(name => cachedNames.includes(name)) &&
      cachedNames.every(name => prebundled.includes(name));

    if (isCacheValid) {
      console.log('⚡ [Gemini Extensions] Cache check passed. All 65+ extensions are fully preloaded (0ms startup overhead).');
      return;
    }

    console.log('🔄 [Gemini Extensions] Synchrozining preloaded registry with local submodules...');
    const installed = await listExtensions();
    const installedNames = (installed || []).map(ext => ext.name?.toLowerCase() || '');

    const toInstall = prebundled.filter(name => !installedNames.includes(name.toLowerCase()));

    if (toInstall.length === 0) {
      fs.writeFileSync(cachePath, JSON.stringify(prebundled), 'utf8');
      console.log('✅ [Gemini Extensions] Registry is fully up to date. Cache generated.');
      return;
    }

    console.log(`🚀 Preloading ${toInstall.length} Gemini extensions from local paths in parallel batches...`);
    ...
    fs.writeFileSync(cachePath, JSON.stringify(prebundled), 'utf8');
    console.log('✅ [Gemini Extensions] Auto-preloading complete. Cache updated.');
  } catch (error) {
