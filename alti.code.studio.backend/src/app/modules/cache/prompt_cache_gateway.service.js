...
class PromptCacheGatewayService {
    ...
    optimizePrompt(systemInstruction, fileContexts = [], userQuery) {
        ...
        const sortedFiles = [...fileContexts].sort((a, b) => a.path.localeCompare(b.path));
        ...
        const staticHash = crypto.createHash('sha256').update(staticPayload).digest('hex');
        ...
        if (this.cacheRegistry.has(staticHash)) {
            ...
            logger.info('🧠 PromptCacheGateway: Prompt cache MISS registered. Static payload cached.');
        }
