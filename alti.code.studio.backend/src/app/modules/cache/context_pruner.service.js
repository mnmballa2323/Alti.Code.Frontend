...
class ContextPrunerService {
  ...
  pruneCode(codeContent) {
    ...
    const savingsPercent =
      originalSize > 0
        ? parseFloat(
            (((originalSize - prunedSize) / originalSize) * 100).toFixed(2),
          )
        : 0;

    logger.info(
      `✂️ ContextPruner: Pruned code. Size reduced from ${originalSize} to ${prunedSize} chars (${savingsPercent}% savings).`,
    );
