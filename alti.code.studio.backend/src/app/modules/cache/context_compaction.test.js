...
describe('ContextCompactionService Tests', () => {
    ...
    it('should return raw text unmodified if it is under the threshold', async () => {
        ...
        const result = await contextCompactionService.compactContext('ctx-1', text, 100);
        expect(result).toBe(text);
    });

    it('should compact text via AI provider and store it in cache if length exceeds threshold', async () => {
        const text = 'This is a very long text that exceeds the threshold of fifty characters for sure.';
        ...
        const consultSpy = vi.spyOn(aiProvider, 'reason').mockResolvedValue(mockSummary);

        const result = await contextCompactionService.compactContext('ctx-2', text, 30);
    ...
    it('should return from cache on cache hit without invoking AI provider', async () => {
        const text = 'This is another long text that would otherwise trigger compaction.';
        const cachedSummary = '- Pre-existing cached summary';

        const getSpy = vi.spyOn(cacheService, 'get').mockResolvedValue(cachedSummary);
        const consultSpy = vi.spyOn(aiProvider, 'reason');

        const result = await contextCompactionService.compactContext('ctx-3', text, 20);
    ...
    it('should fall back to truncation if the AI provider fails', async () => {
        const text = 'This is a long text that triggers compaction but the AI call will fail.';
        
        vi.spyOn(cacheService, 'get').mockResolvedValue(null);
        vi.spyOn(aiProvider, 'reason').mockRejectedValue(new Error('API limit reached'));

        const result = await contextCompactionService.compactContext('ctx-4', text, 20);
