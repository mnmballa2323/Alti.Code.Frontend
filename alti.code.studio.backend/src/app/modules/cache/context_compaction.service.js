...
class ContextCompactionService {
  ...
  async compactContext(contextId, rawText, threshold = 8000) {
    ...
    if (rawText.length < threshold) {
      logger.debug(
        `ContextCompaction: Text length (${rawText.length}) below threshold (${threshold}). Skipping compaction.`,
      );
      return rawText;
    }

    logger.info(
      `🔄 ContextCompaction: Compacting token-heavy context for [${contextId}] (${rawText.length} chars)`,
    );
...
${rawText}`;
    ...
    try {
      ...
      logger.info(
        `🔄 ContextCompaction: Successfully compacted context. Size reduced from ${rawText.length} to ${compacted.length} chars.`,
      );
      return compacted;
    } catch (e) {
      logger.warn(
        `ContextCompaction: LLM compaction failed. Falling back to truncation. (${e.message})`,
      );
      const fallback =
        rawText.slice(0, threshold) + '\n\n[TRUNCATED DUE TO CONTEXT SIZE]';
      return fallback;
    }
  }
