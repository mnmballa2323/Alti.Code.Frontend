...
class LedgerService {
  ...
  async record(entry) {
    ...
    if (!entry.actor || !entry.action) {
      throw new Error(
        'LedgerService: entry must contain at least actor and action.',
      );
    }
    try {
      ...
      logger.info(
        `🔐 Ledger: Action recorded. Hash: ${hash.substring(0, 8)}...`,
      );
      return newLog;
    } catch (error) {
  ...
  async verifyChain() {
    ...
    for (let i = 0; i < logs.length; i++) {
      ...
      if (previous && current.previousHash !== previous.hash) {
        logger.error(
          `🚨 Ledger Breach! Log ${current._id} links to wrong hash.`,
        );
        return false;
      }
