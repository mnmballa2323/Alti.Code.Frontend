...
describe('FeatureFlagMockerService Tests', () => {
  ...
  it('should return default values when no overrides are registered', () => {
    const val = featureFlagMockerService.getFlagValue(
      'new-billing-flow',
      false,
    );
    expect(val).toBe(false);

    const valString = featureFlagMockerService.getFlagValue(
      'variant-name',
      'control',
    );
  ...
  it('should set, retrieve, and remove feature flag overrides successfully', () => {
    ...
    // Check values
    expect(featureFlagMockerService.getFlagValue('new-billing-flow')).toBe(
      true,
    );
    expect(featureFlagMockerService.getFlagValue('variant-name')).toBe(
      'experiment-a',
    );
    ...
    expect(
      featureFlagMockerService.getFlagValue('new-billing-flow', false),
    ).toBe(false);
    expect(featureFlagMockerService.getFlagValue('variant-name')).toBe(
      'experiment-a',
    );
