...
const getLeadsFromKeywords = async keywords => {
  if (!Array.isArray(keywords) || keywords.length === 0) {
    logger.warn(
      'AutomationService: getLeadsFromKeywords called with empty keywords array.',
    );
    return { message: 'No keywords provided', data: [] };
  }

  logger.info(
    `🔍 AutomationService: Scraping ${keywords.length} keyword(s) in parallel...`,
  );
  ...
  const results = await Promise.allSettled(
    keywords.map(async kw => {
      ...
      const leadsWithoutWebsite = ads.filter(
        ad => !NO_WEBSITE_RE.test(ad.link ?? ''),
      );
  ...
  for (const result of results) {
    if (result.status === 'fulfilled') {
      ...
      logger.warn(
        `AutomationService: A keyword scrape failed — ${result.reason?.message}`,
      );
    }
  }
  ...
  if (allLeads.length > 0) {
    ...
    logger.info(
      `✅ AutomationService: ${allLeads.length} leads saved to sheet.`,
    );
    return {
