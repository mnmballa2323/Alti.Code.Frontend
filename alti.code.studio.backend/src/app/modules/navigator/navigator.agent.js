...
class NavigatorAgent {
    ...
    async scaleServices(load) {
        logger.info(`🧭 Navigator: Analyzing load (${load}%) for scaling decision...`);
        ...
        if (load > 80) {
            ...
            logger.info(`🧭 Navigator: High load detected. Scaling UP to ${this.resources.nodes} nodes.`);
        } else if (load < 30 && this.resources.nodes > 1) {
            decision = 'scale-down';
            adjustment = -1;
            this.resources.nodes += adjustment;
            logger.info(`🧭 Navigator: Low load detected. Scaling DOWN to ${this.resources.nodes} nodes.`);
        } else {
    ...
    async optimizeCost() {
        ...
        // Mock analysis
        const suggestions = [
            { resource: 'Database', action: 'Downgrade IOPS during off-peak', savings: '$50/mo' },
            { resource: 'Storage', action: 'Archive logs > 30 days', savings: '$12/mo' }
        ];
