...
const HANDSHAKE_SECRET =
  process.env.A2A_HANDSHAKE_SECRET || 'universe-protocol-secret';

const negotiateTask = async (remoteAgentUrl, taskData) => {
  try {
    logger.info(
      `🤝 [A2A] Initiating negotiation with external agent at ${remoteAgentUrl}`,
    );

    const response = await axios.post(
      `${remoteAgentUrl}/.well-known/ai-agent/negotiate`,
      {
    ...
    if (response.status === 200) {
      logger.info(
        '✅ [A2A] Negotiation successful. Task accepted by remote agent.',
      );
      return response.data;
    } else {
      throw new Error(
        `Remote agent rejected negotiation with status ${response.status}`,
      );
    }
  } catch (error) {
    ...
    throw new ApiError(
      ...
      `A2A Negotiation failed: ${error.message}`,
    );
  }
};
...
const handleIncomingNegotiation = async reqPayload => {
  logger.info(
    `🛰️ [A2A Ingress] Received negotiation from ${reqPayload.source_agent}`,
  );
  ...
  return {
    ...
    tracking_id: `alti_${Date.now()}`,
    message:
      'Alti.Code.Studio failed to evaluate task safety or capacity. Task rejected.',
  };
};
