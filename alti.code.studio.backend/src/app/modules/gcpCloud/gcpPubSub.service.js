...
class GcpPubSubService {
  ...
  init() {
    try {
      ...
      logger.warn(`⚠️ Google Cloud Pub/Sub initialization failed, falling back to local in-memory event bus: ${e.message}`);
      this.isInitialized = false;
    }
  }
  ...
  async publishEvent(topicName, message) {
    if (this.isInitialized && this.pubsub) {
      try {
        logger.info(`📡 Google Cloud Pub/Sub: Publishing event to topic ${topicName}: ${JSON.stringify(message)}`);
        const dataBuffer = Buffer.from(JSON.stringify(message));
        const messageId = await this.pubsub.topic(topicName).publishMessage({ data: dataBuffer });
        return messageId;
      } catch (e) {
        logger.error(`❌ Google Cloud Pub/Sub publish failed: ${e.message}. Falling back to mock.`);
      }
    }

    // Mock In-Memory Fallback
    logger.info(`📡 Google Cloud Pub/Sub Mock: Publishing event to topic ${topicName}: ${JSON.stringify(message)}`);
  ...
  async subscribe(subName, topicName, callback) {
    if (this.isInitialized && this.pubsub) {
      try {
        logger.info(`📡 Google Cloud Pub/Sub: Subscribing subscription ${subName} to topic ${topicName}`);
        const subscription = this.pubsub.subscription(subName);
        subscription.on('message', message => {
          logger.info(`📡 Google Cloud Pub/Sub: Received message on subscription ${subName}`);
          try {
        ...
        logger.error(`❌ Google Cloud Pub/Sub subscription failed: ${e.message}. Falling back to mock.`);
      }
    }

    // Mock In-Memory Fallback
    logger.info(`📡 Google Cloud Pub/Sub Mock: Subscribing ${subName} to topic ${topicName}`);
  ...
  async subscribeToEvent(topicName, callback) {
    logger.info(`📡 Google Cloud Pub/Sub Mock: Subscribing to event topic ${topicName}`);
    if (typeof callback === 'function') {
