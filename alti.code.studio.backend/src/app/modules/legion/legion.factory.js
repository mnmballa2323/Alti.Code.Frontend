...
function createProcessor(name, definition) {
  return async job => {
    ...
    logger.info(
      `${definition.emoji} ${name} [${job.id}]: ${task || 'Processing'}...`,
    );
...
class LegionFactory {
  ...
  boot() {
    ...
    logger.info(
      `⚔️ LegionFactory: Booting ${entries.length} single-purpose agents...`,
    );
    ...
    logger.info(
      `⚔️ LegionFactory: ✅ ${entries.length} agents armed and ready`,
    );
