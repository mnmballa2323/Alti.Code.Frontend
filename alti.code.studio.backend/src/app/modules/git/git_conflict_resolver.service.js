...
class GitConflictResolverService {
    ...
    async resolveConflicts(fileContent, filename) {
        ...
        if (!fileContent.includes('<<<<<<<') || !fileContent.includes('=======') || !fileContent.includes('>>>>>>>')) {
            logger.debug(`GitConflictResolver: No conflict markers found in [${filename}]. Skipping.`);
            return fileContent;
        }

        logger.info(`⚔️ GitConflictResolver: Resolving merge conflicts for [${filename}]`);
        ...
        while ((match = conflictRegex.exec(fileContent)) !== null) {
            ...
            logger.info(`GitConflictResolver: Found conflict block in [${filename}]. Running AI merge...`);
...
Return ONLY the resolved code block. Do NOT include markdown fences, explaining text, or git markers.`;
            ...
            try {
                ...
                const cleanMerge = response.trim().replace(/^```[a-zA-Z]*\n/, '').replace(/\n```$/, '');
                resolved = resolved.replace(fullMarkerBlock, cleanMerge);
                logger.info(`GitConflictResolver: Successfully merged block in [${filename}]`);
            } catch (e) {
                logger.error(`GitConflictResolver: Failed to merge block in [${filename}]. Falling back to ours.`, e);
