...
agentProfilerRoutes.post('/record', (req, res) => {
  ...
  if (!agentName || typeof success !== 'boolean') {
    return res.status(400).json({
      success: false,
      message: 'agentName and success (boolean) required.',
    });
  }
  agentProfiler.record(agentName, success, latencyMs || 0, {
    testPassed,
    costUsd,
  });
  return res.json({
    success: true,
    profile: agentProfiler.getProfile(agentName),
  });
});
...
agentProfilerRoutes.post('/flush', async (req, res) => {
  try {
    ...
    return res.json({
      success: true,
      message: 'Profiles flushed to vector store.',
    });
  } catch (e) {
...
agentProfilerRoutes.delete('/surge/:name', (req, res) => {
  ...
  return res.json({
    success: true,
    message: `Surge cleared for "${req.params.name}".`,
  });
});
...
agentProfilerRoutes.get('/events', (req, res) => {
  ...
  const events = [
    'profiler:recorded',
    'profiler:surge',
    'profiler:self_tune:started',
    'profiler:self_tune:done',
    'profiler:self_tune:failed',
  ];
  ...
  sendEvent('profiler:connected', {
    leaderboard: agentProfiler.getLeaderboard(10),
  });
