...
const EXPLORE_RATIO = 0.10;     // 10% of calls explore untested agents
...
export class AgentProfiler {
    ...
    record(agentName, success, latencyMs, opts = {}) {
        ...
        profilerBus.emit('profiler:recorded', { agentName, success, latencyMs, score: p.score });

        // Self-tuner: fire when score drops below threshold (only after MIN_SAMPLES)
        if (p.calls >= MIN_SAMPLES_FOR_SCORE && p.score < LOW_SCORE_THRESHOLD && !p._selfTuneQueued) {
    ...
    isSurged(agentName) {
        ...
        p.surgedUntil = null;
        p.consecutiveFails = 0;
        logger.info(`📊 Profiler: Agent "${agentName}" surge expired — back in rotation.`);
        return false;
    }
    ...
    getSurgedAgents() {
        ...
        return [...this.profiles.entries()]
            ...
            .map(([name, p]) => ({ name, surgedUntil: new Date(p.surgedUntil).toISOString(), score: p.score }));
    }

    _tripCircuit(agentName, profile) {
        profile.surgedUntil = Date.now() + SURGE_SUPPRESS_MS;
        logger.warn(`⚡ SurgeProtection: Agent "${agentName}" demoted for ${SURGE_SUPPRESS_MS / 60000} min (${SURGE_CONSECUTIVE_FAILS} consecutive failures).`);
        profilerBus.emit('profiler:surge', { agentName, surgedUntil: new Date(profile.surgedUntil).toISOString() });
    }
    ...
    selectBest(candidates) {
        ...
        if (!available.length) {
            logger.warn('📊 Profiler: All candidates surged — using first regardless.');
            return candidates[0];
        }

        // Explore: 10% chance to pick lowest-samples agent (discovery)
        const untested = available.filter(c => (this.profiles.get(c.name)?.calls ?? 0) < MIN_SAMPLES_FOR_SCORE);
        if (untested.length && Math.random() < EXPLORE_RATIO) {
            const pick = untested[Math.floor(Math.random() * untested.length)];
            logger.info(`📊 Profiler: EXPLORE — trying under-sampled agent "${pick.name}"`);
            return pick;
        }
        ...
        const scored = available.map(c => {
            ...
            const profileScore = profile && profile.calls >= MIN_SAMPLES_FOR_SCORE ? profile.score : 50;
    ...
    _computeScore(p) {
        ...
        const testRate = p.testPassCount + p.testFailCount > 0
        ...
        return Math.round(
            (successRate * 40) + (testRate * 30) + (latencyScore * 30)
        );
    }
    ...
    async _triggerSelfTune(agentName, profile) {
        logger.warn(`🔧 Self-Tuner: Agent "${agentName}" score=${profile.score} < ${LOW_SCORE_THRESHOLD}. Escalating…`);
        profilerBus.emit('profiler:self_tune:started', { agentName, score: profile.score });
        ...
        try {
            ...
            const synthesis = await selfExpandingSwarmService.synthesizeAgent(synthesisPrompt).catch(e => {
            ...
            if (synthesis?.filePath) {
                ...
                await autonomicService.commenceProactiveSweep([synthesis.filePath]).catch(() => { });
                logger.info(`🔧 Self-Tuner: Replacement agent written to ${synthesis.filePath}`);
                profilerBus.emit('profiler:self_tune:done', { agentName, replacementFile: synthesis.filePath });
            ...
            profilerBus.emit('profiler:self_tune:failed', { agentName, error: e.message });
        }
    }
    ...
    startPeriodicFlush() {
        ...
        this._flushTimer = setInterval(() => this._flushToStore(), FLUSH_INTERVAL_MS);
        logger.info(`📊 Profiler: Vector flush every ${FLUSH_INTERVAL_MS / 60000} min.`);
    }
    ...
    async _flushToStore() {
        ...
        await Promise.allSettled(entries.map(([name, p]) =>
            vectorStoreService.add(
                ...
                { type: 'agent_profile', agentName: name, score: p.score, calls: p.calls }
            )
        ));
        logger.info(`📊 Profiler: Flushed ${entries.length} agent profiles to vector store.`);
    }
    ...
    getProfile(agentName) { return this.profiles.get(agentName) ?? null; }
    ...
    getAllProfiles() {
        return [...this.profiles.entries()]
            .map(([name, p]) => ({
                ...
                testPassRate: (p.testPassCount + p.testFailCount) > 0
                    ? Math.round((p.testPassCount / (p.testPassCount + p.testFailCount)) * 100)
                    : null,
                score: p.score,
    ...
    _getOrCreate(name) {
        if (!this.profiles.has(name)) {
            this.profiles.set(name, {
                name, calls: 0, successes: 0, failures: 0, consecutiveFails: 0,
                totalLatencyMs: 0, testPassCount: 0, testFailCount: 0, totalCostUsd: 0,
                surgedUntil: null, score: 50, lastCalledAt: null, _selfTuneQueued: false,
            });
        }
