...
import { A2aService } from '../gcpCloud/gcpA2a.service.js';
...
class SwarmBrain {
  constructor() {
    ...
    import('./yaml_agent_loader.js')
      .then(async ({ loadYamlAgents, watchDefinitions }) => {
        try {
          ...
          const proxyMap = new Proxy(reactiveMap, {
            set(target, prop, value) {
              if (value && value.manifest) {
                logger.info(
                  `♻️  [SwarmBrain] Hot-reloading YAML Agent: ${value.name}`,
                );
                capabilityRouter.registerAgent(
                  value,
    ...
    logger.info(
      '🧠 SwarmBrain: Initialized. Thousands-Agent Code Swarm is Online.',
    );
  }
  ...
  async getSpecialistWorkflow(prompt) {
    ...
    if (slashMatch) {
      ...
      let targetAgent = agents.find(
        a =>
          ...
          a.name.toLowerCase().endsWith(`_${command}`),
      );
      if (!targetAgent) {
      ...
      if (targetAgent) {
        logger.info(
          `⚡ SwarmBrain: Direct Slash Command detected [/${command}]. Bypassing CapabilityRouter and binding directly to [${targetAgent.name}].`,
        );
        workflowData = {
          nodes: [
            {
              ...
              task:
                prompt.replace(slashMatch[0], '').trim() ||
                'Execute your primary engineering protocol.',
    ...
    const mcpTools = await import('./mcp.service.js')
      .then(m => m.mcpBridgeService.getGeminiTools(prompt))
      .catch(() => []);
    const mcpContextString =
      mcpTools.length > 0
    ...
    const workflowNodes = workflowData.nodes.map(node => {
      ...
      if (!agentDefinition) {
        logger.warn(
          `⚠️ SwarmBrain: capabilityRouter selected invalid ID [${node.agentId}]. Falling back to [jules].`,
        );
        agentDefinition = agentRegistry.get('jules');
      }

      logger.info(
        `🧠 SwarmBrain: Assembling workflow node [${node.id}]: [${agentDefinition.name}] -> Task: ${node.task}`,
      );
      ...
      const isComplex =
        promptLower.length > 500 ||
        /(architect|refactor|design|complex|optimize|system|secure|dag|mesh|build|deploy)/.test(
          promptLower,
        ) ||
        /(architect|refactor|design|generate|create)/.test(taskLower);

      const dynamicModelTier =
        node.modelTier || (isComplex ? 'gemini-3.1-pro' : 'gemini-3.1-flash');

      if (dynamicModelTier === 'gemini-3.1-flash' && !node.modelTier) {
        logger.info(
          `⚡ [Model Router] Task complexity low. Downgrading Node [${node.id}] to ${dynamicModelTier} to optimize cost and latency.`,
        );
      }
      ...
      return {
        ...
        consult: async (p, ctx) => {
          ...
          try {
            ...
            const customSkill = await Skill.findOne({
              name: agentDefinition.name,
            });
            if (customSkill) {
              if (customSkill.systemInstruction) {
                ...
                logger.info(
                  `🔌 [SkillOpt] Dynamic Prompt Binding: Loaded evolved optimized instructions for [${agentDefinition.name}]`,
                );
              }
              if (customSkill.successes && customSkill.successes.length > 0) {
                ...
                logger.info(
                  `🔌 [SkillOpt] Reinforced Learning: Loaded ${customSkill.successes.length} success cases as in-context demonstrations for [${agentDefinition.name}]`,
                );
              }
            }
          } catch (err) {
            logger.debug(
              `[SkillOpt] Failed to fetch custom skill override: ${err.message}`,
            );
          }
          ...
          try {
            ...
            stylisticPreferences =
              await evolutionService.getStylisticParameters(process.cwd());
            if (stylisticPreferences) {
              logger.info(
                `🔌 [EvolutionService] Style Integration: Loaded stylistic constraints for [${agentDefinition.name}]`,
              );
            }
          } catch (err) {
            logger.debug(
              `[EvolutionService] Failed to fetch stylistic preferences: ${err.message}`,
            );
          }
...
\`\`\``;
          const { hybridRouterService } =
            await import('../ai/hybrid_router.service.js');
    ...
    socketService.broadcast('swarm', 'dag_topology', {
      nodes: workflowNodes,
  ...
  async executeTask(prompt, context = [], onProgress = null) {
    ...
    if (onProgress)
      onProgress({
        status: 'started',
        message: 'SwarmBrain pipeline commenced',
      });
    ...
    try {
      ...
      if (recalledMemories.length > 0) {
        logger.info(
          `🧠 [AgentMemory] Recalled ${recalledMemories.length} relevant memories from past sessions`,
        );
        context.push(
          `[PERSISTENT_AGENT_MEMORY] The following insights were recalled from previous coding sessions:\n${recalledMemories.join('\n')}`,
        );
        if (onProgress)
          onProgress({
      ...
      logger.debug(
        `[AgentMemory] Pre-execution recall skipped: ${memErr.message}`,
      );
    }
    ...
    try {
      ...
      if (stores && stores.length > 0) {
        ...
        if (fsResult && fsResult.text) {
          ...
          logger.info(
            `📚 [FileSearch] Injected document RAG context with ${fsResult.citations?.length || 0} citation(s) into SwarmBrain pipeline.`,
          );
          if (onProgress)
            onProgress({
              status: 'file_search_context',
              citations: fsResult.citations?.length || 0,
            });
        }
      }
    } catch (fsErr) {
      logger.debug(
        `[FileSearch] SwarmBrain context injection skipped: ${fsErr.message}`,
      );
    }
    ...
    const governanceCheck = await dataCatalogService.governFileIngestion(
      'runtime_context_memory',
      combinedContext,
    );
    if (!governanceCheck.isSafe) {
      logger.warn(
        `🛑 SwarmBrain: Context blocked by Data Catalog. Reporting to SCC...`,
      );
    ...
    for (const match of secretMatches) {
      ...
      logger.info(
        `🔐 SwarmBrain: Requested Secret detected [${secretId}]. Authenticating with Google Cloud Secret Manager...`,
      );
      try {
        const secretValue = await SecretManagerService.getSecret(secretId);
        injectedPrompt = injectedPrompt.replace(
          match[0],
          `[INJECTED_SECRET_VALUE:${secretValue}]`,
        );
        logger.info(
          `✅ SwarmBrain: Secret [${secretId}] dynamically injected into secure runtime context.`,
        );
      } catch (e) {
        logger.warn(
          `⚠️ SwarmBrain: Secret [${secretId}] could not be retrieved from Secret Manager. Executing without it.`,
        );
      }
    }
    ...
    if (videoMatches.length > 0) {
      logger.info(
        `👁️ SwarmBrain: Screen recording detected in context. Triggering Google Cloud Video Intelligence...`,
      );
      if (onProgress)
        onProgress({
          status: 'analyzing_video',
          message: 'Swarm is watching the bug screen recording...',
        });

      for (const match of videoMatches) {
        const videoUri = match[0];
        const extractedVisualText =
          await videoEyeService.analyzeBugRecording(videoUri);
        if (extractedVisualText && extractedVisualText.length > 0) {
          context.push(
            `[SYSTEM_VIDEO_EYE] I have watched the screen recording (${videoUri}). Here is the raw text and errors flashing on the screen during the bug: ${extractedVisualText.join(' | ')}`,
          );
        }
      }
    }
    ...
    if (docsMatches.length > 0) {
      logger.info(
        `📄 SwarmBrain: Google Doc links detected in prompt. Interfacing with Google Workspace API...`,
      );
      if (onProgress)
        onProgress({
          status: 'analyzing_doc',
          message:
            'Swarm is extracting enterprise technical specifications from Google Docs...',
        });
      ...
      for (const match of docsMatches) {
        ...
        try {
          ...
          if (docText) {
            context.push(
              `[SYSTEM_WORKSPACE_DOC] I have extracted the strict engineering requirements from the attached Google Doc: \n\n${docText}`,
            );
            logger.info(
              `✅ SwarmBrain: Successfully ingested technical spec from Google Doc [${docId}].`,
            );
          }
        } catch (e) {
          logger.warn(
            `⚠️ SwarmBrain: Failed to read Google Doc [${docId}]. Permission denied or file not found.`,
          );
        }
      }
    }
    ...
    for (const match of matches) {
      const lib = match[1];
      const precomputed = await featureStoreService.getPrecomputedEmbedding(
        `std_${lib}`,
      );
      if (precomputed) {
        context.push(
          `[SYSTEM] Injected sub-millisecond Vertex AI context for ${lib}`,
        );
      }
    }
    ...
    try {
      const { gooseRouterService } =
        await import('../goose/gooseRouter.service.js');
      const { clawCodeRouterService } =
        await import('../clawCode/clawCodeRouter.service.js');
      const fs = await import('fs');

      const isCodeTask =
        gooseRouterService.shouldRouteToGoose(injectedPrompt) ||
      ...
      if (isCodeTask) {
        ...
        logger.info(
          `🧠 SwarmBrain: Code task detected. Preferred execution agent: ${preferredAgent.toUpperCase()}`,
        );

        if (preferredAgent === 'claw') {
          try {
            logger.info(`🤖 SwarmBrain: Routing code task to Claw-Code.`);
            const clawResult = await clawCodeRouterService.executeTask(
              injectedPrompt,
              context,
              onProgress,
            );
            if (onProgress)
              onProgress({ status: 'completed', finalResult: clawResult });
            return clawResult;
          } catch (clawError) {
            logger.warn(
              `⚠️ SwarmBrain: Claw-Code execution failed. Attempting failover to Goose... Error: ${clawError.message}`,
            );
            const gooseResult = await gooseRouterService.executeTask(
              injectedPrompt,
              context,
              onProgress,
            );
            if (onProgress)
              onProgress({ status: 'completed', finalResult: gooseResult });
            return gooseResult;
          }
        } else {
          try {
            ...
            if (clawBinaryExists) {
              logger.warn(
                `⚠️ SwarmBrain: Goose execution failed. Attempting failover to Claw-Code... Error: ${gooseError.message}`,
              );
              const clawResult = await clawCodeRouterService.executeTask(
                injectedPrompt,
                context,
      ...
      logger.warn(
        `⚠️ SwarmBrain: Smart Agent Router failed or bypassed. Falling back to standard specialists: ${routerError.message}`,
      );
    }
    ...
    if (onProgress)
      onProgress({
        ...
        message: `DAG Generated with ${nodes.length} nodes and ${edges.length} edges`,
      });

    // 3. Hyperscale Forking (Google Cloud Run Jobs)
    if (combinedContext.length > 500000) {
      logger.warn(
        `🚀 SwarmBrain: Massive context detected (>500k chars). Forking to parallel Cloud Run Jobs...`,
      );
      await cloudRunJobsService.createExecutionJob(`swarm-fork-${Date.now()}`, {
    ...
    const executeNode = async (nodeId, attempt = 1) => {
      ...
      logger.info(
        `🤖 SwarmBrain: [PARALLEL] Spawning Node [${nodeId}] -> ${agent.name} (${agent.assignedTask}) [Attempt ${attempt}/3]`,
      );
      if (onProgress)
        onProgress({
          status: 'node_started',
      ...
      socketService.broadcast('swarm', 'node_status', {
        nodeId,
        agentName: agent.name,
        status: 'running',
        attempt,
      ...
      for (const pId of parentIds) {
        ...
        if (parentOutput && parentOutput.length > 50000) {
          logger.warn(
            `🗜️ SwarmBrain: Parent output from [${pId}] exceeds 50k chars. Offloading to Google Cloud Storage...`,
          );
          const bucketName = 'alti_swarm_artifacts'; // Default bucket
          const fileName = `swarm_artifact_${pId}_${Date.now()}.txt`;

          try {
            const gcsUri = await GcsService.uploadFile(
              bucketName,
              fileName,
              parentOutput,
            );
            specificContext.push(
              `[Context from Node ${pId} (${nodeMap.get(pId).name})]:\nFULL ARTIFACT OFFLOADED TO GCS: ${gcsUri}\n(Retrieve this URI using your GCS reading capability). Snippet:\n${parentOutput.substring(0, 5000)}...`,
            );
          } catch (e) {
            logger.error(
              `⚠️ SwarmBrain: GCS upload failed for [${pId}]. Falling back to raw text injection.`,
            );
            specificContext.push(
              `[Context from Node ${pId} (${nodeMap.get(pId).name})]: ${parentOutput}`,
            );
          }
        } else {
          specificContext.push(
      ...
      try {
        const { mimoCheckpointService } =
          await import('../memory/mimo_checkpoint.service.js');
        specificContext =
          await mimoCheckpointService.checkAndCompact(specificContext);
      } catch (checkpointErr) {
        logger.debug(
          `[MimoCheckpoint] Skipping out-of-band checkpoint check: ${checkpointErr.message}`,
        );
      }
      ...
      if (agent.hardwareProfile === 'CLOUD_RUN') {
        logger.warn(
          `🚀 SwarmBrain: Node [${nodeId}] requires hyperscale hardware. Dispatching agent [${agent.name}] to Google Cloud Run with IAM Role: [${agent.iamRole}]...`,
        );
      ...
      for (const block of jsonBlocks) {
        try {
          ...
          if (parsed.__SWARM_INJECT__) {
            logger.warn(
              `🧬 SwarmBrain: Runtime Node Spawning detected from Node [${nodeId}]! Mutating execution graph...`,
            );
          ...
          logger.error(
            `⚠️ SwarmBrain: Failed to parse dynamic graph injection from node [${nodeId}]: ${e.message}`,
          );
        }
      }
      ...
      if (mcpCallsToExecute.length > 0) {
        logger.info(
          `🔌 SwarmBrain: Intercepted ${mcpCallsToExecute.length} parallel MCP tool calls from Node [${nodeId}]. Fanning out execution...`,
        );

        const mcpPromises = mcpCallsToExecute.map(async mcpCall => {
          try {
            const [formattedServerName, ...toolNameParts] =
              mcpCall.tool.split('__');
            ...
            this.hiveMindMesh.emit(
              'cognitive_alert',
              `Agent [${agent.name}] failed to use MCP tool ${mcpCall.tool}. Error: ${mcpErr.message}`,
            );
        ...
        logger.info(
          `✅ SwarmBrain: ${mcpCallsToExecute.length} parallel MCP tools executed. Injecting results and re-running node...`,
        );
      ...
      const evalScore = await vertexEval.evaluateOutput(
        agent.name,
        injectedPrompt,
        result,
      );
      ...
      if (evalScore.isApproved && !result.includes('[SYSTEM_DISPATCH]')) {
        logger.info(
          `🤝 SwarmBrain: Node [${nodeId}] passed Eval. Initiating Mandatory A2A Peer Review with Staff Engineer...`,
        );
        ...
        try {
          ...
          if (!review.content.includes('APPROVED')) {
            ...
            logger.warn(
              `🛑 SwarmBrain: Node [${nodeId}] REJECTED by Staff Engineer via A2A Protocol.`,
            );
          }
        } catch (e) {
          logger.warn(
            `⚠️ SwarmBrain: Peer Review failed to execute. Bypassing A2A check.`,
          );
        }
      }
      ...
      if (!result.includes('[SYSTEM_DISPATCH]')) {
        if (
          ...
          result.match(/password\s*=/i) ||
          result.match(/SELECT.*FROM.*WHERE.*\$\{/i)
        ) {
          secOpsPassed = false;
          logger.error(
            `🚨 SwarmBrain: Node [${nodeId}] REJECTED by Google Cloud Security Command Center (SAST Violation).`,
          );
          await sccService.reportFinding(
            'SAST_VULNERABILITY',
            `node_${nodeId}`,
      ...
      if (!result.includes('[SYSTEM_DISPATCH]')) {
        ...
        if (codeMatch && codeMatch[1]) {
          logger.info(
            `⚡ SwarmBrain: Node [${nodeId}] generated code. Initiating Compile-Time Verification via Cloud Run Dynamic Sessions...`,
          );
          const codeToRun = codeMatch[1];
          // Naively assuming python for the sandbox if not specified
          const execResult = await dynamicSessionsService.executeCode(
            codeToRun,
            'python',
          );
          ...
          if (!execResult.success || execResult.stderr) {
            ...
            compilerFeedback =
              execResult.stderr || 'Syntax Error or Execution Timeout.';
            logger.error(
              `🚨 SwarmBrain: Node [${nodeId}] failed Compile-Time Verification. Compiler threw an exception.`,
            );
          } else {
            logger.info(
              `✅ SwarmBrain: Node [${nodeId}] code compiled and executed flawlessly in gVisor sandbox.`,
            );
          }
        }
      }
      ...
      let aguiFeedback = '';
      if (
        !result.includes('[SYSTEM_DISPATCH]') &&
        (result.includes('<html') || result.includes('import React'))
      ) {
        logger.info(
          `👁️ SwarmBrain: Node [${nodeId}] generated frontend code. Initiating A2UI Visual QA Automation...`,
        );
        ...
        if (renderResult && renderResult.screenshot) {
          ...
          try {
            ...
            if (!qaResponse.includes('APPROVED')) {
              ...
              logger.warn(
                `🛑 SwarmBrain: AGUI QA Engineer REJECTED the visual render: ${qaResponse}`,
              );
            } else {
              logger.info(
                `✅ SwarmBrain: Node [${nodeId}] passed AGUI Visual QA.`,
              );
            }
          } catch (e) {
            logger.warn(
              `⚠️ SwarmBrain: AGUI QA execution failed: ${e.message}`,
            );
          }
          ...
          if (aguiPassed) {
            logger.info(
              `🎭 SwarmBrain: AGUI visual check passed. Synthesizing deterministic Playwright E2E tests...`,
            );
            ...
            try {
              ...
              const testMatch = e2eResult.content.match(
                /```(?:typescript|ts)?\n([\s\S]*?)```/,
              );
              if (testMatch && testMatch[1]) {
                const testCode = testMatch[1];
                specificContext.push(
                  `[SYSTEM_PLAYWRIGHT_SYNTHESIS]: I have generated a Playwright E2E Test Suite for your frontend component:\n\`\`\`typescript\n${testCode}\n\`\`\``,
                );
                const { socketService } =
                  await import('../../services/socket.service.js');
                socketService.broadcast('swarm', 'hive_mind_alert', {
                  alert: `Synthesized ${testCode.split('test(').length - 1} Playwright E2E tests for Node [${nodeId}].`,
                });
              }
            } catch (e) {
              logger.warn(
                `⚠️ SwarmBrain: Playwright Test Synthesis failed: ${e.message}`,
              );
            }
          }
        }
      ...
      if (
        ...
        logger.warn(
          `🔁 SwarmBrain: Node [${nodeId}] failed AGI Quality Gate. Triggering Auto-Remediation Loop (Attempt ${attempt + 1}/3)...`,
        );
        ...
        if (!evalScore.isApproved) {
          this.hiveMindMesh.emit(
            'cognitive_alert',
            `Node [${nodeId}] failed Quality Gate: ${evalScore.reasoning}. Ensure other agents adjust strategy.`,
          );
          specificContext.push(
            `[SYSTEM_EVALUATOR_FEEDBACK]: Your previous output was rejected. Reason: ${evalScore.reasoning}. Please fix these issues.`,
          );
        }
        if (!peerReviewPassed)
          specificContext.push(
            `[STAFF_ENGINEER_PEER_REVIEW]: Your code was rejected by the Staff Engineer. Fix these architectural flaws: ${peerReviewFeedback}`,
          );
        if (!secOpsPassed) {
          this.hiveMindMesh.emit(
            'cognitive_alert',
            `Node [${nodeId}] triggered a SECOPS violation (SAST). Aborting unsafe operations.`,
          );
          specificContext.push(
            `[SECOPS_SAST_FEEDBACK]: Your code contains severe security vulnerabilities. Remove all eval(), hardcoded secrets, and SQL injections.`,
          );
        }
        if (!compilerPassed)
          specificContext.push(
            `[COMPILER_TRACEBACK]: Your code failed to compile/run in the sandbox. Fix this exact runtime error:\n${compilerFeedback}`,
          );
        if (!aguiPassed)
          specificContext.push(
            `[AGUI_VISUAL_FEEDBACK]: Your frontend code was visually rejected by the AGUI QA Engineer. Fix these visual defects:\n${aguiFeedback}`,
          );
      ...
      if (
        ...
        logger.error(
          `❌ SwarmBrain: Node [${nodeId}] exhausted all auto-remediation attempts. Triggering Agent-to-Human (A2H) Escalation Protocol...`,
        );
        socketService.broadcast('swarm', 'node_status', {
          nodeId,
          agentName: agent.name,
        ...
        try {
          const { SkillOptService } =
            await import('../skillopt/skillopt.service.js');
          ...
          SkillOptService.registerFailure(
            ...
            logger.warn(`[SkillOpt] Failed to record failure: ${err.message}`),
          );
        } catch (err) {
        ...
        const a2hPayload = {
          ...
          failedGates: {
            evalScore: evalScore.isApproved,
            peerReviewPassed,
            secOpsPassed,
            compilerPassed,
            aguiPassed,
        ...
        logger.warn(
          `🚨 [A2H Protocol] Paging human on-call engineer via Google Chat Webhook: ${JSON.stringify(a2hPayload)}`,
        );

        // Proceed with degraded output since human hasn't instantly responded
      } else {
        logger.info(
          `✅ SwarmBrain: Node [${nodeId}] passed all AGI Quality Gates (Attempt ${attempt}).`,
        );
        if (onProgress)
          onProgress({
        ...
        if (!result.includes('[SYSTEM_DISPATCH]')) {
          try {
            import('../skillopt/skillopt.service.js')
              .then(({ SkillOptService }) => {
        ...
        if (attempt > 1 && !result.includes('[SYSTEM_DISPATCH]')) {
          logger.info(
            `🧠 [MLOps] Recording successful remediation trajectory into Vertex AI Fine-Tuning Dataset...`,
          );
          BigQueryService.streamMetrics(
            'alti_metrics',
            'vertex_finetuning_dataset',
            [
              {
      ...
      AgentMemoryHooks.captureToolUse(agent.name, agent.assignedTask, result, {
        nodeId,
        domain: agent.domain,
        attempt,
      ...
      if (executableNodes.length > 0 && inProgress.size < MAX_CONCURRENCY) {
        ...
        batch.forEach(nodeId => {
          executeNode(nodeId).catch(err => {
            logger.error(
              `⚠️ SwarmBrain: Node [${nodeId}] catastrophic failure: ${err.message}`,
            );
            inProgress.delete(nodeId);
          });
        });
    ...
    if (
      !finalResult.includes('I cannot resolve this') &&
      !finalResult.includes('Error')
    ) {
      ...
      import('../memory/mimo_distiller.service.js')
        .then(({ mimoDistillerService }) => {
          mimoDistillerService
            .distill(prompt, finalResult, agentId)
            .catch(err => {
              logger.debug(
                `[MimoDistiller] Trajectory distillation failed: ${err.message}`,
              );
            });
        })
        .catch(() => {});
    ...
    await FirestoreSyncService.persistCrdtUpdate(
      `doc_${Date.now()}`,
    ...
    if (
      ...
      logger.warn(
        `🤝 SwarmBrain: Local agent failed. Invoking A2A Negotiation Protocol to hire external Google Cloud agent...`,
      );
      result = await A2aService.negotiateTask(
        'https://external-google-agent.com',
        { task: prompt },
      );
    }
    ...
    if (!evalScore.isApproved) {
      logger.error(
        `❌ SwarmBrain: Workflow output failed Vertex Eval Quality Gate (Score: ${evalScore.score}). Reporting to SCC...`,
      );
      await sccService.reportFinding(
        'LOW_FIDELITY_AI_OUTPUT',
        `workflow_${nodes[0]?.name || 'unknown'}`,
        'MEDIUM',
      );
    }
    ...
    try {
      ...
      await neo4jService.logAgentReasoningNode(
        Date.now().toString(),
        orchestratorName,
        executionTimeMs,
      );
      ...
      logger.error(
        `❌ SwarmBrain: Failed to log cognitive node to Neo4j: ${e.message}`,
      );
    }

    // 8. Workspace Backup (Google Workspace Admin)
    if (evalScore.isApproved) {
      await workspaceAdminService.createWorkspaceFolder(
        'Alti Swarm Artifacts',
        'user@domain.com',
      );
    }
    ...
    if (evalScore.isApproved) {
      try {
        ...
        if (status.trim().length > 0) {
          logger.info(
            `🐙 SwarmBrain: Uncommitted file changes detected after successful execution. Triggering Autonomous Shadow Git Sync...`,
          );
          ...
          try {
            execSync(
              `git commit -m "🤖 Swarm Autonomous Patch\n\nGenerated for prompt: ${prompt.substring(0, 100)}"`,
              { cwd: process.cwd() },
            );
          } catch (commitErr) {
            logger.warn(
              `⚠️ SwarmBrain: Commit failed. Checking for Merge Conflicts...`,
            );
            ...
            if (diff.includes('<<<<<<< HEAD')) {
              ...
              logger.info(
                `✅ SwarmBrain: Merge conflict resolved mathematically. Awaiting human verification.`,
              );
              // Auto-commit resolution
              execSync(`git commit -am "🤖 Swarm Merge Resolution"`, {
                cwd: process.cwd(),
              });
            }
          }
          ...
          exec(
            ...
            err => {
              if (!err) {
                logger.info(
                  `✅ SwarmBrain: Shadow branch ${branchName} pushed successfully.`,
                );
              } else {
                logger.warn(
                  `⚠️ SwarmBrain: Failed to push shadow branch. (Is origin configured?)`,
                );
              }
            },
          );
          ...
          const { socketService } =
            await import('../../services/socket.service.js');
          socketService.broadcast('swarm', 'hive_mind_alert', {
            alert: `Shadow Git Sync completed. Branch: ${branchName}`,
          });
        }
      } catch (gitErr) {
        logger.warn(
          `⚠️ SwarmBrain: Shadow Git sync failed or not in a git repository: ${gitErr.message}`,
        );
      }
    }
    ...
    if (
      prompt.toLowerCase().includes('optimize infrastructure') ||
      prompt.toLowerCase().includes('reduce costs')
    ) {
      logger.info(
        `🔍 SwarmBrain: User requested optimization. Querying Google Cloud Recommender API...`,
      );
    ...
    if (
      ...
      logger.info(
        `⚡ SwarmBrain: User requested autonomous testing. Executing code in Cloud Run Dynamic Session sandbox...`,
      );

      // Extract the first code block (assuming python for now, could be improved)
      const codeMatch = finalResult.match(
        /```(?:python|javascript|nodejs)?\n([\s\S]*?)```/,
      );
      if (codeMatch && codeMatch[1]) {
        const codeToRun = codeMatch[1];
    ...
    if (
      ...
      const hostUrl =
        await cloudWorkstationsService.provisionWorkstation(workstationId);
    ...
    if (
      prompt
        .toLowerCase()
        .match(
          /(train|tensorflow|pytorch|machine learning|deep learning|fine-tune)/,
        )
    ) {
      logger.info(
    ...
    if (evalScore.isApproved && prompt.toLowerCase().includes('email')) {
      logger.info(
        `📧 SwarmBrain: User requested email delivery. Dispatching payload via Google Workspace Gmail API...`,
      );
      try {
        const htmlBody = `
                ...
                `;
        await workspaceService.emailAdministrator(
          'Autonomous Swarm Delivery: Task Completed',
          htmlBody,
        );
        ...
        logger.warn(
          `⚠️ SwarmBrain: Failed to dispatch Gmail report: ${e.message}`,
        );
      }
    }

    // 🛡️ Immutable Enterprise Audit Trail via Google Cloud Logging
    CloudLoggingService.writeAuditLog(
      'agentic-audit-trail',
      {
