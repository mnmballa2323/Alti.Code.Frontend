...
class McpBridgeService extends EventEmitter {
    ...
    async init() {
        ...
        try {
            ...
            if (parsedConfig.mcpServers) {
                for (const [name, config] of Object.entries(parsedConfig.mcpServers)) {
                    ...
                    if (name === 'peekaboo') {
                        if (process.platform !== 'darwin' || process.env.ENABLE_DESKTOP_AUTOMATION !== 'true') {
                            logger.warn('⚠️ [MCP] Peekaboo disabled. Requires macOS (darwin) and ENABLE_DESKTOP_AUTOMATION=true.');
                            continue;
                        }
                    }
        ...
        logger.info(`✅ [MCP] Successfully registered ${this.servers.size} standardized MCP servers.`);
    ...
    async shutdown() {
        ...
        logger.info(`🛑 [MCP] Shutting down ${this.clients.size} active MCP clients...`);
    ...
    registerServer(name, config) {
        ...
        this.emit('telemetry', { level: 'info', message: `Registered server: ${name}` });
    ...
    async provisionServer(serverName, packageName) {
        logger.warn(`🤖 [MCP] Autonomous self-provisioning triggered for ${serverName} (${packageName})...`);
        this.emit('telemetry', { level: 'warn', message: `Autonomously provisioning new MCP capability: ${serverName}` });
        ...
        try {
            ...
            return { success: true, message: `Server ${serverName} provisioned successfully.` };
        } catch(e) {
            logger.error(`[MCP] Failed to self-provision ${serverName}: ${e.message}`);
    ...
    async discoverTools(serverName) {
        ...
        this.toolsCache.set(serverName, { timestamp: Date.now(), tools: discoveredTools });
        ...
        AgentMemoryHooks.captureToolUse(
            ...
            logger.warn(`⚠️ [MCP] Failed to capture memory for ${serverName}: ${e.message}`);
        });
    ...
    async executeTool(serverName, toolName, args = {}) {
        ...
        this.emit('telemetry', { level: 'info', message: telemetryMsg, data: args });

        
        // Payload Validation/Sanitization (simple existence check)
        if (!serverName || !toolName) {
            throw new Error(`Invalid MCP execution request. Missing serverName or toolName.`);
        }

        // Circuit Breaker Check
        const circuitKey = `${serverName}_${toolName}`;
        const breaker = this.circuitBreaker.get(circuitKey) || { failures: 0, nextRetryTime: 0 };
        if (Date.now() < breaker.nextRetryTime) {
            logger.warn(`🛑 [MCP] Circuit Breaker prevented execution of ${circuitKey}`);
            return { isError: true, error: `[SYSTEM: CIRCUIT BREAKER TRIPPED] Tool ${serverName}/${toolName} is temporarily blacklisted due to repeated failures. DO NOT attempt to call this tool again. Find another approach.` };
        }
            ...
            if (toolSchema) {
                ...
                for (const req of required) {
                    if (args[req] === undefined) {
                        throw new Error(`Validation failed: Missing required argument '${req}' for tool ${serverName}/${toolName}.`);
                    }
                }
            }
        }
        ...
        if (config && config.approvalRequired) {
            logger.warn(`🛑 [MCP HITL] Suspending execution. Tool ${serverName}/${toolName} requires human approval.`);
            // Simulate WebSockets/SSE approval loop
            const isApproved = await new Promise(resolve => {
                logger.info(`[MCP HITL] Waiting for UI approval via WebSocket... (Auto-approving for headless backend)`);
                setTimeout(() => resolve(true), 1500); 
            });
            if (!isApproved) {
                return { isError: true, error: `[HITL] Execution rejected by human administrator.` };
            }
        }
        ...
        try {
            ...
            if (resultStr.length > 50000) {
                logger.warn(`🗜️ [MCP] Tool payload is massive (${resultStr.length} bytes). Offloading to Google Cloud Storage...`);
                try {
                    const { GcsService } = await import('../googleCloud/gcs.service.js');
                    const uri = await GcsService.uploadFile('alti_swarm_artifacts', `mcp_payload_${Date.now()}.json`, resultStr);
                    return { _offloaded: true, uri: uri, summary: "Result was too large to fit in your context window and was offloaded to GCS. Use your tools to read the URI if you absolutely need it." };
                } catch(e) {
                     logger.error(`[MCP] Failed to offload MCP payload to GCS: ${e.message}. Returning raw data.`);
                }
            }
            ...
            if (breaker.failures >= 3) {
                ...
                logger.error(`🚨 [MCP] CIRCUIT BREAKER TRIPPED for ${circuitKey}. Tool blacklisted for 1 hour.`);
            }
            this.circuitBreaker.set(circuitKey, breaker);

            logger.error(`❌ [MCP] Execution failed for ${serverName}/${toolName}: ${error.message}`);
    ...
    async getGeminiTools(prompt = '') {
        ...
        for (const [serverName] of this.servers) {
            try {
                    ...
                    return {
                        ...
                        parameters: {
                            ...
                            properties: Object.fromEntries(
                                Object.entries(schema.properties || {}).map(([k, v]) => [k, { type: (v.type || 'string').toUpperCase(), description: v.description }])
                            ),
                ...
                logger.error(`❌ [MCP] Failed to format tools for ${serverName}: ${error.message}`);
            }
        }

        // Semantic Routing Filter (Top 10)
        if (prompt && geminiTools.length > 10) {
            geminiTools = geminiTools.map(t => ({ 
                tool: t, 
            ...
            .slice(0, 10)
            .map(t => t.tool);
            logger.info(`[MCP] Semantically filtered ${geminiTools.length} tools for the current prompt.`);
        }
