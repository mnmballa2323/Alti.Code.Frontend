...
export class SwarmEngine {
  constructor() {
    logger.info(
      '🐝 Custom SwarmEngine initialized. Ready for autonomous multi-agent handoffs.',
    );
  }
  ...
  async run(agent, messages, contextVariables = {}, maxTurns = 6) {
    ...
    logger.info(
      `🐝 SwarmEngine: Running session starting with Agent [${currentAgent.name}]`,
    );

    while (turn < maxTurns) {
      turn++;
      logger.info(
        `🐝 SwarmEngine Turn ${turn}/${maxTurns} - Current Active Agent: [${currentAgent.name}]`,
      );
      ...
      const onToolCall = async (name, args) => {
        ...
        if (!targetFn) {
          throw new Error(
            `Tool ${name} not found on agent ${currentAgent.name}`,
          );
        }
        ...
        if (result && result.isHandoff) {
          ...
          let targetAgent =
            swarmModule[result.handoffAgentName + 'Agent'] ||
            swarmModule[result.handoffAgentName];
          ...
          if (!targetAgent) {
            ...
            try {
              ...
              if (agentDef && agentDef.importPath) {
                ...
                const specialistInstance = Object.values(shardModule).find(
                  val => val && val.consult,
                );
                    ...
                    functions: [
                      {
                        ...
                        parameters: {
                          ...
                          properties: {
                            prompt: {
                              type: 'STRING',
                              description:
                                'The domain prompt to audit or generate code',
                            },
                          },
                          required: ['prompt'],
                        },
                        execute: async (args, context) => {
                          const contextBlock =
                            ...
                            '';
                          const consultResult =
                            await specialistInstance.consult(args.prompt, [
                              {
                                content:
                                  typeof contextBlock === 'string'
                                    ? contextBlock
              ...
              logger.error(
                `Swarm Engine: Failed to dynamically load agent shard for [${result.handoffAgentName}]: ${e.message}`,
              );
            }
          }
          ...
          if (targetAgent) {
            logger.info(
              `🔄 Swarm Handoff: [${currentAgent.name}] -> Delegated to [${targetAgent.name}]`,
            );
      ...
      logger.info(
        `🐝 SwarmEngine: Execution completed successfully in ${turn} turns.`,
      );
      break;
    }
    ...
    if (turn >= maxTurns) {
      logger.warn(
        `🐝 SwarmEngine: Terminated because turn limit (${maxTurns}) was reached.`,
      );
    }
