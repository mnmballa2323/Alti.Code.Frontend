...
class SmartFileWriterService {
  ...
  async writeFile(filePath, content, documentId = 'workspace-123') {
    logger.info(
      `🏗️ [SmartWriter] Initiating atomic write for ${path.basename(filePath)}`,
    );
    ...
    try {
      ...
      const auditResult = await auditorAgent.verifyFileIntegrity(
        filePath,
        content,
      );
      ...
      if (!auditResult.isPassed) {
        logger.warn(
          `⚠️ [SmartWriter] Auditor rejected write for ${filePath}. Triggering self-healing rollback...`,
        );
      ...
      logger.error(
        `❌ [SmartWriter] Write failure. Attempting emergency recovery...`,
        error,
      );
      if (previousContent) {
        await this.rollback(filePath, previousContent);
  ...
  async rollback(filePath, content) {
    logger.warn(
      `🔄 [SmartWriter] Rolling back ${filePath} to last known stable state...`,
    );
    await fs.writeFile(filePath, content, 'utf8');
  }
}
