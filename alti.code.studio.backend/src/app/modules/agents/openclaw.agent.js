...
class OpenClawAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.description =
      "The Local Automator — Specialist Swarm Agent dedicated natively to delegating complex local host tasks directly to the user's embedded OpenClaw instance via the Proxy Tunnel.";
  }
  ...
  async _invoke(prompt, contextBlock) {
    ...
    logger.info(
      `🤖 Local Automator: Planning delegation to physical host for goal: "${goal}"`,
    );
...
Return ONLY the raw intent string, no markdown.`;
    ...
    while (attempt <= maxAttempts) {
      try {
        logger.info(
          `🤖 Local Automator: Routing Intent (Attempt ${attempt}/${maxAttempts}): "${currentIntent}"`,
        );
        ...
        if (
          surrogateOutput.toLowerCase().includes('critical failure') ||
          surrogateOutput.toLowerCase().includes('error:') ||
          surrogateOutput.toLowerCase().includes('command not found')
        ) {
          throw new Error(
            `Execution error captured in stdout: ${surrogateOutput.substring(0, 300)}`,
          );
        }
        ...
        logger.warn(
          `⚠️ Local Automator: Attempt ${attempt} failed: ${error.message}`,
        );
        if (attempt === maxAttempts) {
          logger.error(
            `❌ Local Automator: Moltbot execution permanently failed on physical host.`,
          );
          throw error;
        }
        ...
        logger.info(
          `🧠 [Self-Healing] Triggering Neuromorphic Hermes Socratic Debate to analyze failed execution trajectory...`,
        );
        let socraticFeedback = '';
        try {
          const { neuromorphicHermesAgent } =
            await import('./neuromorphic_hermes.agent.js');
...
${error.message}`;
          ...
          logger.info(
            `🧠 [Self-Healing] Socratic Debate Verdict: ${socraticFeedback.substring(0, 300)}...`,
          );
        } catch (debateErr) {
          logger.warn(
            `Socratic debate failed: ${debateErr.message}. Falling back to standard healing prompt.`,
          );
        }
...
Return ONLY the revised raw intent string, no markdown or explanation.`;
        ...
        try {
          const healedIntent =
            await GeminiAiService.generateContent(healingPrompt);
          currentIntent = healedIntent.replace(/^["'\`]+|["'\`]+$/g, '').trim();
          logger.info(
            `🧠 [Self-Healing] Evolved corrected intent: "${currentIntent}"`,
          );
        } catch (healErr) {
