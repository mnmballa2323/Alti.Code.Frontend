...
describe('Modular Graphics, 3D Rendering & Game Engines Swarm Router Integration', () => {
  describe('Recursive Declarative YAML Parsing & Ingestion', () => {
    ...
    it('should successfully parse and load Ray Tracing & Global Illumination Specialist', async () => {
      const agentPath = path.join(
        GRAPHICS_DIR,
        'graphics.raytracing.agent.yaml',
      );
      const def = parseYaml(await fs.readFile(agentPath, 'utf8'));
    ...
    it('should successfully parse and load Game Physics Engine & Rigid Body Dynamics Specialist', async () => {
      ...
      expect(def.name).toBe(
        'Game Physics Engine & Rigid Body Dynamics Specialist',
      );
    ...
    it('should successfully parse and load Scene Graph & Entity Component System (ECS) Architect', async () => {
      const agentPath = path.join(
        GRAPHICS_DIR,
        'graphics.scene_graph.agent.yaml',
      );
      const def = parseYaml(await fs.readFile(agentPath, 'utf8'));

      expect(def.id).toBe('agent.graphics.scene_graph');
      expect(def.name).toBe(
        'Scene Graph & Entity Component System (ECS) Architect',
      );
      expect(def.capabilities).toContain(
        'quadtree_octree_spatial_partitioning',
      );
    ...
    it('should successfully parse and load Video & Image Compression Codec Specialist', async () => {
      const agentPath = path.join(
        GRAPHICS_DIR,
        'graphics.video_codec.agent.yaml',
      );
      const def = parseYaml(await fs.readFile(agentPath, 'utf8'));
    ...
    it('should successfully parse and load Spatial Audio & Interactive Acoustics Engineer', async () => {
      const agentPath = path.join(
        GRAPHICS_DIR,
        'graphics.spatial_audio.agent.yaml',
      );
      const def = parseYaml(await fs.readFile(agentPath, 'utf8'));
    ...
    it('should successfully parse and load CAD Engine & Vector Graphics Specialist', async () => {
      const agentPath = path.join(
        GRAPHICS_DIR,
        'graphics.cad_vector.agent.yaml',
      );
      const def = parseYaml(await fs.readFile(agentPath, 'utf8'));
  ...
  describe('Precision Swarm Routing for All 10 Graphics & Rendering Agents', () => {
    it('should route WebGPU keywords to WebGPU & WebGL Rendering Pipeline Engineer agent', async () => {
      const output =
        'Perform webgpu_gpu_render_passes commands and trigger wgsl_glsl_shader_compile tasks';
    ...
    it('should route Ray Tracing keywords to Ray Tracing & Global Illumination Specialist agent', async () => {
      const output =
        'Examine bounding_volume_hierarchies_bvh traversals and execute ray_box_intersection_algorithms calculations';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);

      expect(strategy).toBe(
        'Graphics Swarm: Ray Tracing & Global Illumination',
      );
      const agentIds = sequence.map(s => s.agentId);
      expect(agentIds).toContain(
        'Ray Tracing & Global Illumination Specialist',
      );
    });

    it('should route Shader Artist keywords to Creative Shader & Visual Effects Artist agent', async () => {
      const output =
        'Apply fragment_vertex_shader_effects rendering and run raymarching_signed_distance_fields simulations';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);
    ...
    it('should route Physics Engine keywords to Game Physics Engine & Rigid Body Dynamics Specialist agent', async () => {
      const output =
        'Calculate collision_detection_gjk_aabb intersections and resolve rigid_body_impulse_solvers vectors';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);
    ...
    it('should route Scene Graph keywords to Scene Graph & Entity Component System (ECS) Architect agent', async () => {
      const output =
        'Structure quadtree_octree_spatial_partitioning grids and run entity_component_system_ecs_layouts contiguous sweeps';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);
    ...
    it('should route Video Codec keywords to Video & Image Compression Codec Specialist agent', async () => {
      const output =
        'Assert h264_hevc_av1_quantization bounds and track macroblock_entropy_coding bitstreams';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);
    ...
    it('should route Spatial Audio keywords to Spatial Audio & Interactive Acoustics Engineer agent', async () => {
      const output =
        'Audit web_audio_api_node_connections bindings and analyze acoustic_room_impulse_responses results';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);
      ...
      const agentIds = sequence.map(s => s.agentId);
      expect(agentIds).toContain(
        'Spatial Audio & Interactive Acoustics Engineer',
      );
    });

    it('should route AR/VR keywords to AR/VR XR Runtime & Spatial Computing Specialist agent', async () => {
      const output =
        'Fetch webxr_device_pose_estimation tracks and compensate lens_distortion_warp_compensation ratios';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);
      ...
      const agentIds = sequence.map(s => s.agentId);
      expect(agentIds).toContain(
        'AR/VR XR Runtime & Spatial Computing Specialist',
      );
    });

    it('should route CAD Engine keywords to CAD Engine & Vector Graphics Specialist agent', async () => {
      const output =
        'Examine bezier_bspline_subdivision_curves segments and execute boolean_polygon_clipping_operations cuts';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);
    ...
    it('should route Texture Material keywords to Texture Mapping & Materials Shader Specialist agent', async () => {
      const output =
        'Build pbr_material_reflectance_rendering profiles and verify mipmap_filtering_anisotropic_levels interpolation';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);
      ...
      const agentIds = sequence.map(s => s.agentId);
      expect(agentIds).toContain(
        'Texture Mapping & Materials Shader Specialist',
      );
    });
  });
});
