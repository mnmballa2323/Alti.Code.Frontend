...
class AgentForgeGeneratorAgent extends GeminiCliBaseAgent {
  ...
  async generateAgent(missingCapabilityRequirement) {
    logger.info(
      `🔥 [AgentForge] Ignition sequence started. Forging a new specialized agent to handle missing capability...`,
    );
...
4. Since this file is written to the 'custom' subdirectory, you MUST import GeminiCliBaseAgent from '../gemini_cli_base.agent.js'.
    ...
    try {
      ...
      logger.info(
        `✅ [AgentForge] New Agent forged successfully in memory. Pending disk write and registration.`,
      );
      return cleanCode;
    } catch (err) {
...
export const agentForgeGeneratorAgent = Object.freeze(
  new AgentForgeGeneratorAgent(),
);
