...
class DesktopFileOrganizerAgent extends BaseSpecialistAgent {
    ...
    async _invoke(context, contextBlock) {
        ...
        if (!goal) {
            throw new Error('Desktop file organizer requires a goal or organization strategy.');
        }

        logger.info(`🤖 Desktop File Organizer: Translating file organization intent: "${goal}"`);
        ...
        try {
            ...
            logger.error(`❌ Desktop File Organizer: Automation failed: ${error.message}`);
            throw error;
        }
    }
}
