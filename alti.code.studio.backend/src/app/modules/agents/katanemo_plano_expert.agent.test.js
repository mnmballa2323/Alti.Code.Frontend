...
describe('KatanemoPlanoExpertAgent Unit Tests', () => {
  ...
  it('should initialize with correct name, description, and preamble', () => {
    ...
    expect(katanemoPlanoExpertAgent.description).toContain(
      'katanemo/plano, the AI-native data plane',
    );
    expect(katanemoPlanoExpertAgent.preamble).toContain(
      'AI-Native Proxy & Data Plane Specialist',
    );
  });

  it('should successfully consult the LLM with context and prompt', async () => {
    const mockResponse =
      'Plano: Routing nested agent reasoning steps to sidecar.';
  ...
  it('should handle API errors and trigger retries in BaseSpecialistAgent', async () => {
    ...
    const result = await katanemoPlanoExpertAgent.consult(
      'Trace A2A handover',
      [],
    );
