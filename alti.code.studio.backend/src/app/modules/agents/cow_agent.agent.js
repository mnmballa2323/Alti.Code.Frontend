...
class CowAgentAgent extends BaseSpecialistAgent {
    constructor() {
        super();
        this.name = 'cow_agent';
        this.description = 'The CowAgent Multi-Channel Specialist. Configures autonomous assistants for WeChat, Lark, DingTalk, equipped with zero-trust CVE-2026-6129 secure authentication overrides.';
...
`;
    ...
    async _invoke(prompt, contextBlock) {
        ...
        const hasAuthToken = contextBlock && contextBlock.includes('Authorization: Bearer');
        
        logger.info(`[CowAgent Security] Validating incoming dispatch auth token to prevent CVE-2026-6129 bypass...`);
        
        if (!hasAuthToken && process.env.NODE_ENV === 'production') {
            logger.error(`[CowAgent Security] CRITICAL SECURITY ALERT: Unauthorized request blocked! Prevented CVE-2026-6129 authentication bypass.`);
            throw new Error('SecurityException: Unauthorized Agent Mode Access Blocked (CVE-2026-6129 Mitigation Active)');
        }
