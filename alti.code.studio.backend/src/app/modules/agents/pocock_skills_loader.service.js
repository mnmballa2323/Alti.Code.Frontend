...
class PocockSkillAgent extends BaseSpecialistAgent {
  constructor(config) {
    super(
      ...
      'Tier 17: Engineering Fundamentals',
    );
...
class PocockSkillsLoaderService {
  constructor() {
    ...
    this.skillsRootDir = path.resolve(
      process.cwd(),
      '../submodules/mattpocock-skills/skills',
    );
  ...
  init() {
    logger.info(
      '🧠 PocockSkillsLoaderService: Initializing God-Tier Engineering Fundamentals...',
    );
    this.loadAll();
  }
  ...
  loadAll() {
    try {
      if (!fs.existsSync(this.skillsRootDir)) {
        logger.warn(
          `Pocock Skills directory not found at ${this.skillsRootDir}. Skipping.`,
        );
        return;
      }
  ...
  loadSkill(filePath, skillName, category) {
    try {
      ...
      logger.info(
        `✨ Ingested Pocock Protocol [${config.title}] into Swarm Capability Router.`,
      );
    } catch (e) {
      logger.error(
        `[PocockSkills Loader] Failed to parse skill ${skillName}: ${e.message}`,
      );
    }
  }
}
