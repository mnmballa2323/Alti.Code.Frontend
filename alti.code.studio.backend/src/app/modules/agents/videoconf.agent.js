 ...
 * track manipulation, recording, and low-latency WebRTC flows.
 */
...
class VideoConfAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.description =
      'Video communication specialist for WebRTC-based platforms (Twilio Video, Daily.co, LiveKit, 100ms): room management, access tokens, track manipulation, recording, and scalable video architectures.';
...
Output production TypeScript. Store all credentials in environment variables: \`LIVEKIT_API_KEY\`, \`LIVEKIT_API_SECRET\`, \`LIVEKIT_WS_URL\`, \`DAILY_API_KEY\`, etc.`;
  ...
  async consult(prompt, contextData = []) {
    logger.info(
      `🎥 VideoConf Expert: Synthesizing video communication logic...`,
    );
...
export const videoConfAgent = new VideoConfAgent();
