...
export default function Button() {
    ...
    visionService.analyze
      ...
      .mockResolvedValueOnce(
        `The button has white text on a white background, making it invisible. It also lacks padding. Change the background to blue and add px-4 py-2.`,
      )
    ...
    // Assertions
    expect(DesignerAgent_IO_mockCalled(designerAgent._mockWriteToDisk)).toBe(
      true,
    );
    ...
    expect(visionService.capturePage).toHaveBeenCalledWith(
      'http://localhost:3000/preview',
    );
