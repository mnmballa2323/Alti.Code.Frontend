...
class OrchestratorAgent {
  ...
  _deserializeMission(missionFields) {
    ...
    if (mission.completed_steps) {
      try {
        mission.completed_steps = JSON.parse(mission.completed_steps);
      } catch (e) {}
    }
    return mission;
  }
  ...
  async execute(goal, userId, sessionId = `mission-${crypto.randomUUID()}`) {
    ...
    logger.info(
      `🐝 Orchestrator: Receiving goal: "${goal}" [TraceID: ${traceId}]`,
    );
    ...
    const timeoutPromise = new Promise((_, reject) =>
      setTimeout(
        () =>
          reject(
            new Error(`Mission timed out after ${MISSION_TIMEOUT_MS / 1000}s`),
          ),
        MISSION_TIMEOUT_MS,
      ),
    );
    ...
    try {
      ...
      await redisClient.hsetobj(
        missionKey,
        this._serializeMission(updatedMission),
      );
      ...
      logger.error(
        `❌ Orchestrator ${isTimeout ? 'Timeout' : 'Failed'} [TraceID: ${traceId}]`,
        error,
      );
      throw error;
    }
