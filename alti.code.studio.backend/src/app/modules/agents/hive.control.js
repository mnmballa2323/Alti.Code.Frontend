...
class HiveControl {
  ...
  async startSession(goal, requiredRoles) {
    ...
    if (requiredRoles.includes('collaboration')) {
      logger.info(
        '🧠 Hive: Engaging Collaborative Intelligence (The Hive Mind)...',
      );
      ...
      try {
        ...
        if (sentinel) {
          securityReview = await sentinel.execute('reviewDesign', {
            goal,
            design,
          });
          await synapseService.setMemory(
            sessionId,
            'security_review',
            securityReview,
          );
          this.logAction(
            sessionId,
            'Guardian',
            `Security Review: ${securityReview.approved ? 'PASS' : 'FAIL'} (${securityReview.riskLevel || 'N/A'})`,
          );

          if (!securityReview.approved) {
            logger.error(
              `🛑 Hive: Security Violation Detected! Aborting Session.`,
            );
        ...
        if (strategist) {
          ...
          if (critic) {
            ...
            this.logAction(
              sessionId,
              'Critic',
              `Reviewed Plan: ${review.status}`,
            );
          }
        }
      } catch (err) {
        logger.warn(
          'Failed to engage Strategist/Critic in standard swarm:',
          err,
        );
      }
    }
  ...
  logAction(sessionId, agentName, action) {
    ...
    synapseService
      .findAgent('recordEvent')
      .then(historian => {
        if (historian) {
          historian.execute('recordEvent', {
            sessionId,
            agent: agentName,
            action,
          });
