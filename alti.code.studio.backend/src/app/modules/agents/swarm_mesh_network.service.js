...
class SwarmMeshNetworkService {
  ...
  async start() {
    ...
    logger.info(
      `🌐 [Swarm Mesh] Booting P2P LibP2P node: ${this.nodeId.substring(0, 8)}...`,
    );
    ...
    setTimeout(() => {
      ...
      logger.info(
        `🤝 [Swarm Mesh] Discovered peer node: ${peerId.substring(0, 8)} on local subnet.`,
      );
    }, 1500);
    ...
    this.subscribe('TASK_DISCOVERY', async payload => {
      ...
      logger.info(
        `🕸️ [Swarm Mesh] Node ${this.nodeId.substring(0, 8)} intercepted TASK_DISCOVERY broadcast.`,
      );
...
Request: ${prompt.substring(0, 500)}`;
        ...
        if (fs.existsSync(fullPath)) {
          logger.info(
            `🔥 [Swarm Mesh] Dynamic Mesh Resolution hit! Lazy-loading agent: ${agentName}.agent.js into memory...`,
          );

          const module = await import(agentFilePath);
          const AgentInstance = Object.values(module).find(
            val => val && typeof val.consult === 'function',
          );

          if (AgentInstance) {
            logger.info(
              `🚀 [Swarm Mesh] Activating autonomous agent execution via dynamic P2P dispatch...`,
            );
            // The agent's generateContent call is automatically intercepted globally!
            const solution = await AgentInstance.consult(
              prompt,
              contextPayload,
            );
            logger.info(
              `✅ [Swarm Mesh] Agent ${agentName} executed successfully via P2P Mesh.`,
            );
          ...
          logger.warn(
            `⚠️ [Swarm Mesh] Agent ${agentName}.agent.js not found in local replica. Ignoring payload...`,
          );
        }
      } catch (err) {
  ...
  broadcast(topic, payload) {
    logger.info(
      `📢 [Swarm Mesh] Gossiping payload to ${this.knownPeers.size} peers on topic: ${topic}`,
    );
