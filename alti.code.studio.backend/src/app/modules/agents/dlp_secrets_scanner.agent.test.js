...
describe('DlpSecretsScannerAgent Unit Tests', () => {
    ...
    it('should initialize with correct name, description, and preamble', () => {
        ...
        expect(dlpSecretsScannerAgent.description).toContain('DLP & Hardened Secrets Scanner Specialist');
        expect(dlpSecretsScannerAgent.preamble).toContain('DLP & HARDENED SECRETS SCANNER SPECIALIST');
    });

    it('should successfully consult the LLM with context and prompt', async () => {
        const mockResponse = '| Severity | Secret Type | File Path | Recommended Var |\n| High | AWS Secret | config.json | process.env.AWS_SECRET_KEY |';
        ...
        expect(generatedPrompt).toContain('DLP & HARDENED SECRETS SCANNER SPECIALIST');
    ...
    it('should handle API errors and trigger retries in BaseSpecialistAgent', async () => {
        ...
        const result = await dlpSecretsScannerAgent.consult('Scan git diff for credentials', []);
