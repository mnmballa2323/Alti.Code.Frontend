...
class DevOpsAutomationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsAutomationSystem(objective) {
    logger.info(
      `💻 [DevOpsAutomationOrchestratorAgent] Analyzing DevOps Automation Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Automation Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsAutomationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsAutomationOrchestratorAgent = Object.freeze(
  new DevOpsAutomationOrchestratorAgent(),
);
