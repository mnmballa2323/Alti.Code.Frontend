...
class DevSecOpsOrchestrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsOrchestrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsOrchestrationSpecialistAgent] Analyzing DevSecOps Orchestration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Orchestration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsOrchestrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsOrchestrationSpecialistAgent = Object.freeze(
  new DevSecOpsOrchestrationSpecialistAgent(),
);
