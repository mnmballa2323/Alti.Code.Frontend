...
class DatabaseLoadBalancingTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseLoadBalancingSystem(objective) {
    logger.info(
      `💻 [DatabaseLoadBalancingTesterAgent] Analyzing Database LoadBalancing Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database LoadBalancing Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseLoadBalancingTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseLoadBalancingTesterAgent = Object.freeze(
  new DatabaseLoadBalancingTesterAgent(),
);
