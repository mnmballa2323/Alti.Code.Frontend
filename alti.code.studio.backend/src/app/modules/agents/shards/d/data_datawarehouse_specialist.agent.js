...
class DataDataWarehouseSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDataDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DataDataWarehouseSpecialistAgent] Analyzing Data DataWarehouse Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data DataWarehouse Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataDataWarehouseSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataDataWarehouseSpecialistAgent = Object.freeze(
  new DataDataWarehouseSpecialistAgent(),
);
