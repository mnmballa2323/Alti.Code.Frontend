...
class DevOpsIntegrationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsIntegrationSystem(objective) {
    logger.info(
      `💻 [DevOpsIntegrationDirectorAgent] Analyzing DevOps Integration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Integration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsIntegrationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsIntegrationDirectorAgent = Object.freeze(
  new DevOpsIntegrationDirectorAgent(),
);
