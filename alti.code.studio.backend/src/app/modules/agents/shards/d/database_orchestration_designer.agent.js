...
class DatabaseOrchestrationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseOrchestrationSystem(objective) {
        logger.info(`💻 [DatabaseOrchestrationDesignerAgent] Analyzing Database Orchestration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Orchestration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseOrchestrationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseOrchestrationDesignerAgent = Object.freeze(new DatabaseOrchestrationDesignerAgent());
