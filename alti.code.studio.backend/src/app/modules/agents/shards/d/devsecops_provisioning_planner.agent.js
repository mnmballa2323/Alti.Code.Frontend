...
class DevSecOpsProvisioningPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsProvisioningSystem(objective) {
        logger.info(`💻 [DevSecOpsProvisioningPlannerAgent] Analyzing DevSecOps Provisioning Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Provisioning Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsProvisioningPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsProvisioningPlannerAgent = Object.freeze(new DevSecOpsProvisioningPlannerAgent());
