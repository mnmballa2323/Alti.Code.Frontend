...
class DevSecOpsProvisioningArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsProvisioningSystem(objective) {
    logger.info(
      `💻 [DevSecOpsProvisioningArchitectAgent] Analyzing DevSecOps Provisioning Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Provisioning Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsProvisioningArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsProvisioningArchitectAgent = Object.freeze(
  new DevSecOpsProvisioningArchitectAgent(),
);
