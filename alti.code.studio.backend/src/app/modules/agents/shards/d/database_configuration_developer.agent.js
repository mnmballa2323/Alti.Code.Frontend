...
class DatabaseConfigurationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseConfigurationSystem(objective) {
    logger.info(
      `💻 [DatabaseConfigurationDeveloperAgent] Analyzing Database Configuration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Configuration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseConfigurationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseConfigurationDeveloperAgent = Object.freeze(
  new DatabaseConfigurationDeveloperAgent(),
);
