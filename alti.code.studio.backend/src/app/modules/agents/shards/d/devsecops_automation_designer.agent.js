...
class DevSecOpsAutomationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsAutomationSystem(objective) {
        logger.info(`💻 [DevSecOpsAutomationDesignerAgent] Analyzing DevSecOps Automation Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Automation Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsAutomationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsAutomationDesignerAgent = Object.freeze(new DevSecOpsAutomationDesignerAgent());
