...
class DatabaseGovernanceDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseGovernanceSystem(objective) {
        logger.info(`💻 [DatabaseGovernanceDeveloperAgent] Analyzing Database Governance Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Governance Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseGovernanceDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseGovernanceDeveloperAgent = Object.freeze(new DatabaseGovernanceDeveloperAgent());
