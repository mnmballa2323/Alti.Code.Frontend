...
class DevOpsStreamingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsStreamingSystem(objective) {
    logger.info(
      `💻 [DevOpsStreamingSpecialistAgent] Analyzing DevOps Streaming Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Streaming Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsStreamingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsStreamingSpecialistAgent = Object.freeze(
  new DevOpsStreamingSpecialistAgent(),
);
