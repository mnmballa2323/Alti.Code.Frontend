...
class DatabaseComplianceOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseComplianceSystem(objective) {
    logger.info(
      `💻 [DatabaseComplianceOrchestratorAgent] Analyzing Database Compliance Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Compliance Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseComplianceOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseComplianceOrchestratorAgent = Object.freeze(
  new DatabaseComplianceOrchestratorAgent(),
);
