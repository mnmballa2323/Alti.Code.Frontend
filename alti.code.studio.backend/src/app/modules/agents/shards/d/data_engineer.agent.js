...
class DataEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateEtlPipeline(sourceSchema, destination) {
        logger.info(`📊 [DataEngineer] Designing ETL pipeline from raw schema to ${destination}...`);
...
${sourceSchema}
        ...
        try {
            ...
            logger.info(`✅ [DataEngineer] Airflow ETL Pipeline generated successfully.`);
            return cleanDag;
        } catch (err) {
            logger.error(`❌ [DataEngineer] Failed to generate ETL pipeline: ${err.message}`);
            throw err;
        }
    }
}
