...
class DataComplianceOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataComplianceSystem(objective) {
    logger.info(
      `💻 [DataComplianceOrchestratorAgent] Analyzing Data Compliance Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Compliance Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataComplianceOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataComplianceOrchestratorAgent = Object.freeze(
  new DataComplianceOrchestratorAgent(),
);
