...
class DevSecOpsCachingStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsCachingSystem(objective) {
    logger.info(
      `💻 [DevSecOpsCachingStrategistAgent] Analyzing DevSecOps Caching Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Caching Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsCachingStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsCachingStrategistAgent = Object.freeze(
  new DevSecOpsCachingStrategistAgent(),
);
