...
class DatabaseMicroservicesConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseMicroservicesSystem(objective) {
    logger.info(
      `💻 [DatabaseMicroservicesConsultantAgent] Analyzing Database Microservices Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Microservices Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseMicroservicesConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseMicroservicesConsultantAgent = Object.freeze(
  new DatabaseMicroservicesConsultantAgent(),
);
