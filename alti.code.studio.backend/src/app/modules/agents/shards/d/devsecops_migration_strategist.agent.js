...
class DevSecOpsMigrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsMigrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsMigrationStrategistAgent] Analyzing DevSecOps Migration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Migration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsMigrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsMigrationStrategistAgent = Object.freeze(
  new DevSecOpsMigrationStrategistAgent(),
);
