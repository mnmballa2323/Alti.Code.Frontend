...
class DevSecOpsVirtualizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsVirtualizationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsVirtualizationLeadAgent] Analyzing DevSecOps Virtualization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Virtualization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsVirtualizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsVirtualizationLeadAgent = Object.freeze(
  new DevSecOpsVirtualizationLeadAgent(),
);
