...
class DatabaseVirtualizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseVirtualizationSystem(objective) {
    logger.info(
      `💻 [DatabaseVirtualizationDirectorAgent] Analyzing Database Virtualization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Virtualization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseVirtualizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseVirtualizationDirectorAgent = Object.freeze(
  new DatabaseVirtualizationDirectorAgent(),
);
