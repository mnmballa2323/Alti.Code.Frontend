...
class DevSecOpsAutomationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsAutomationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsAutomationConsultantAgent] Analyzing DevSecOps Automation Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Automation Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsAutomationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsAutomationConsultantAgent = Object.freeze(
  new DevSecOpsAutomationConsultantAgent(),
);
