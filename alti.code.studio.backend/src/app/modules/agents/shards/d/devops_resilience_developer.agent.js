...
class DevOpsResilienceDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsResilienceSystem(objective) {
    logger.info(
      `💻 [DevOpsResilienceDeveloperAgent] Analyzing DevOps Resilience Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Resilience Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsResilienceDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsResilienceDeveloperAgent = Object.freeze(
  new DevOpsResilienceDeveloperAgent(),
);
