...
class DatabaseResilienceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseResilienceSystem(objective) {
    logger.info(
      `💻 [DatabaseResilienceDirectorAgent] Analyzing Database Resilience Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Resilience Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseResilienceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseResilienceDirectorAgent = Object.freeze(
  new DatabaseResilienceDirectorAgent(),
);
