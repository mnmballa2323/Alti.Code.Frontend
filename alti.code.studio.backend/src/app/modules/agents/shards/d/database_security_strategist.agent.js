...
class DatabaseSecurityStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseSecuritySystem(objective) {
    logger.info(
      `💻 [DatabaseSecurityStrategistAgent] Analyzing Database Security Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Security Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseSecurityStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseSecurityStrategistAgent = Object.freeze(
  new DatabaseSecurityStrategistAgent(),
);
