...
class DatabaseContainerizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseContainerizationSystem(objective) {
    logger.info(
      `💻 [DatabaseContainerizationLeadAgent] Analyzing Database Containerization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Containerization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseContainerizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseContainerizationLeadAgent = Object.freeze(
  new DatabaseContainerizationLeadAgent(),
);
