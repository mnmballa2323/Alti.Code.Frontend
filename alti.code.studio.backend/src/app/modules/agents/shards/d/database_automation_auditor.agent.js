...
class DatabaseAutomationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseAutomationSystem(objective) {
        logger.info(`💻 [DatabaseAutomationAuditorAgent] Analyzing Database Automation Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Automation Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseAutomationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseAutomationAuditorAgent = Object.freeze(new DatabaseAutomationAuditorAgent());
