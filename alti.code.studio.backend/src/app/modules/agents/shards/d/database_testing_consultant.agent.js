...
class DatabaseTestingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseTestingSystem(objective) {
        logger.info(`💻 [DatabaseTestingConsultantAgent] Analyzing Database Testing Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Testing Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseTestingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseTestingConsultantAgent = Object.freeze(new DatabaseTestingConsultantAgent());
