...
class DevSecOpsIntegrationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsIntegrationSystem(objective) {
        logger.info(`💻 [DevSecOpsIntegrationTesterAgent] Analyzing DevSecOps Integration Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Integration Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsIntegrationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsIntegrationTesterAgent = Object.freeze(new DevSecOpsIntegrationTesterAgent());
