...
class DatabaseAutomationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseAutomationSystem(objective) {
        logger.info(`💻 [DatabaseAutomationDirectorAgent] Analyzing Database Automation Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Automation Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseAutomationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseAutomationDirectorAgent = Object.freeze(new DatabaseAutomationDirectorAgent());
