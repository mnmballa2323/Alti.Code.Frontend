...
class DatabaseMigrationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseMigrationSystem(objective) {
        logger.info(`💻 [DatabaseMigrationArchitectAgent] Analyzing Database Migration Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Migration Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseMigrationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseMigrationArchitectAgent = Object.freeze(new DatabaseMigrationArchitectAgent());
