...
class DevSecOpsAnalyticsOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsAnalyticsSystem(objective) {
    logger.info(
      `💻 [DevSecOpsAnalyticsOrchestratorAgent] Analyzing DevSecOps Analytics Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Analytics Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsAnalyticsOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsAnalyticsOrchestratorAgent = Object.freeze(
  new DevSecOpsAnalyticsOrchestratorAgent(),
);
