...
class DataVirtualizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataVirtualizationSystem(objective) {
    logger.info(
      `💻 [DataVirtualizationDirectorAgent] Analyzing Data Virtualization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Virtualization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataVirtualizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataVirtualizationDirectorAgent = Object.freeze(
  new DataVirtualizationDirectorAgent(),
);
