...
class DatabaseOptimizationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseOptimizationSystem(objective) {
        logger.info(`💻 [DatabaseOptimizationAnalystAgent] Analyzing Database Optimization Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Optimization Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseOptimizationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseOptimizationAnalystAgent = Object.freeze(new DatabaseOptimizationAnalystAgent());
