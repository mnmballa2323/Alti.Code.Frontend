...
class DevOpsArchitectureArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsArchitectureSystem(objective) {
    logger.info(
      `💻 [DevOpsArchitectureArchitectAgent] Analyzing DevOps Architecture Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Architecture Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsArchitectureArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsArchitectureArchitectAgent = Object.freeze(
  new DevOpsArchitectureArchitectAgent(),
);
