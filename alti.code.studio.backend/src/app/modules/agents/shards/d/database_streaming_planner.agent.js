...
class DatabaseStreamingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseStreamingSystem(objective) {
    logger.info(
      `💻 [DatabaseStreamingPlannerAgent] Analyzing Database Streaming Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Streaming Planner Target',
      );
...
export const databaseStreamingPlannerAgent = Object.freeze(
  new DatabaseStreamingPlannerAgent(),
);
