...
class DatabaseResilienceAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseResilienceSystem(objective) {
        logger.info(`💻 [DatabaseResilienceAuditorAgent] Analyzing Database Resilience Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Resilience Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseResilienceAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseResilienceAuditorAgent = Object.freeze(new DatabaseResilienceAuditorAgent());
