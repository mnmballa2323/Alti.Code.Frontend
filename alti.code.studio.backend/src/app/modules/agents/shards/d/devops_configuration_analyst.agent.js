...
class DevOpsConfigurationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsConfigurationSystem(objective) {
    logger.info(
      `💻 [DevOpsConfigurationAnalystAgent] Analyzing DevOps Configuration Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Configuration Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsConfigurationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsConfigurationAnalystAgent = Object.freeze(
  new DevOpsConfigurationAnalystAgent(),
);
