...
class DatabaseResilienceManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseResilienceSystem(objective) {
    logger.info(
      `💻 [DatabaseResilienceManagerAgent] Analyzing Database Resilience Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Resilience Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseResilienceManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseResilienceManagerAgent = Object.freeze(
  new DatabaseResilienceManagerAgent(),
);
