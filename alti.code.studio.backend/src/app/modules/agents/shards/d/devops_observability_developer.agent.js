...
class DevOpsObservabilityDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsObservabilitySystem(objective) {
    logger.info(
      `💻 [DevOpsObservabilityDeveloperAgent] Analyzing DevOps Observability Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Observability Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsObservabilityDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsObservabilityDeveloperAgent = Object.freeze(
  new DevOpsObservabilityDeveloperAgent(),
);
