...
class DataStreamingOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateDataStreamingSystem(objective) {
        logger.info(`💻 [DataStreamingOrchestratorAgent] Analyzing Data Streaming Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data Streaming Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataStreamingOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataStreamingOrchestratorAgent = Object.freeze(new DataStreamingOrchestratorAgent());
