...
class DevSecOpsGovernancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsGovernanceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsGovernancePlannerAgent] Analyzing DevSecOps Governance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Governance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsGovernancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsGovernancePlannerAgent = Object.freeze(
  new DevSecOpsGovernancePlannerAgent(),
);
