...
class DataOptimizationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDataOptimizationSystem(objective) {
    logger.info(
      `💻 [DataOptimizationArchitectAgent] Analyzing Data Optimization Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Optimization Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataOptimizationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataOptimizationArchitectAgent = Object.freeze(
  new DataOptimizationArchitectAgent(),
);
