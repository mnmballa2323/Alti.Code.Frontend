...
class DevOpsProvisioningDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsProvisioningSystem(objective) {
        logger.info(`💻 [DevOpsProvisioningDeveloperAgent] Analyzing DevOps Provisioning Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Provisioning Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsProvisioningDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsProvisioningDeveloperAgent = Object.freeze(new DevOpsProvisioningDeveloperAgent());
