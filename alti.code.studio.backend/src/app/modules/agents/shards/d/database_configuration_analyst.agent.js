...
class DatabaseConfigurationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseConfigurationSystem(objective) {
        logger.info(`💻 [DatabaseConfigurationAnalystAgent] Analyzing Database Configuration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Configuration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseConfigurationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseConfigurationAnalystAgent = Object.freeze(new DatabaseConfigurationAnalystAgent());
