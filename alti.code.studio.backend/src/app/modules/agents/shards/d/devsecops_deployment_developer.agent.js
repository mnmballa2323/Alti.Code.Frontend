...
class DevSecOpsDeploymentDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsDeploymentSystem(objective) {
    logger.info(
      `💻 [DevSecOpsDeploymentDeveloperAgent] Analyzing DevSecOps Deployment Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Deployment Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsDeploymentDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsDeploymentDeveloperAgent = Object.freeze(
  new DevSecOpsDeploymentDeveloperAgent(),
);
