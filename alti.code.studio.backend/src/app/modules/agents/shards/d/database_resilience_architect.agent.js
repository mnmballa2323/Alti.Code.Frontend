...
class DatabaseResilienceArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseResilienceSystem(objective) {
    logger.info(
      `💻 [DatabaseResilienceArchitectAgent] Analyzing Database Resilience Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Resilience Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseResilienceArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseResilienceArchitectAgent = Object.freeze(
  new DatabaseResilienceArchitectAgent(),
);
