...
class DevOpsAutomationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsAutomationSystem(objective) {
        logger.info(`💻 [DevOpsAutomationConsultantAgent] Analyzing DevOps Automation Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Automation Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsAutomationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsAutomationConsultantAgent = Object.freeze(new DevOpsAutomationConsultantAgent());
