...
class DevOpsCachingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsCachingSystem(objective) {
    logger.info(
      `💻 [DevOpsCachingOrchestratorAgent] Analyzing DevOps Caching Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Caching Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsCachingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsCachingOrchestratorAgent = Object.freeze(
  new DevOpsCachingOrchestratorAgent(),
);
