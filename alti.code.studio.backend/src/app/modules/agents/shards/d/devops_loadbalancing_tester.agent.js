...
class DevOpsLoadBalancingTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsLoadBalancingSystem(objective) {
    logger.info(
      `💻 [DevOpsLoadBalancingTesterAgent] Analyzing DevOps LoadBalancing Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps LoadBalancing Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsLoadBalancingTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsLoadBalancingTesterAgent = Object.freeze(
  new DevOpsLoadBalancingTesterAgent(),
);
