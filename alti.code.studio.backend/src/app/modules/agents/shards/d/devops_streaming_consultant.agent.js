...
class DevOpsStreamingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsStreamingSystem(objective) {
        logger.info(`💻 [DevOpsStreamingConsultantAgent] Analyzing DevOps Streaming Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Streaming Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsStreamingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsStreamingConsultantAgent = Object.freeze(new DevOpsStreamingConsultantAgent());
