...
class DevOpsFaultTolerancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsFaultToleranceSystem(objective) {
    logger.info(
      `💻 [DevOpsFaultTolerancePlannerAgent] Analyzing DevOps FaultTolerance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps FaultTolerance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsFaultTolerancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsFaultTolerancePlannerAgent = Object.freeze(
  new DevOpsFaultTolerancePlannerAgent(),
);
