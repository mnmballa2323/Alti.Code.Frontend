...
class DevSecOpsDataWarehouseDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsDataWarehouseSystem(objective) {
        logger.info(`💻 [DevSecOpsDataWarehouseDirectorAgent] Analyzing DevSecOps DataWarehouse Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps DataWarehouse Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsDataWarehouseDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsDataWarehouseDirectorAgent = Object.freeze(new DevSecOpsDataWarehouseDirectorAgent());
