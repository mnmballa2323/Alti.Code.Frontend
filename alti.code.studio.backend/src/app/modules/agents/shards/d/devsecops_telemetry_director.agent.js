...
class DevSecOpsTelemetryDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsTelemetrySystem(objective) {
    logger.info(
      `💻 [DevSecOpsTelemetryDirectorAgent] Analyzing DevSecOps Telemetry Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Telemetry Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsTelemetryDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsTelemetryDirectorAgent = Object.freeze(
  new DevSecOpsTelemetryDirectorAgent(),
);
