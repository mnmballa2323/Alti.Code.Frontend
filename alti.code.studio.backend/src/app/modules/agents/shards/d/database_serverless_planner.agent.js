...
class DatabaseServerlessPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseServerlessSystem(objective) {
    logger.info(
      `💻 [DatabaseServerlessPlannerAgent] Analyzing Database Serverless Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Serverless Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseServerlessPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseServerlessPlannerAgent = Object.freeze(
  new DatabaseServerlessPlannerAgent(),
);
