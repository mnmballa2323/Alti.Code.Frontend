...
class DatabaseOptimizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseOptimizationSystem(objective) {
        logger.info(`💻 [DatabaseOptimizationSpecialistAgent] Analyzing Database Optimization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Optimization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseOptimizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseOptimizationSpecialistAgent = Object.freeze(new DatabaseOptimizationSpecialistAgent());
