...
class DeploymentAgent {
  ...
  async deploy(provider = 'vercel') {
    ...
    try {
      ...
      return {
        ...
        url:
          this._extractUrl(stdout) ||
          this._extractUrl(stderr) ||
  ...
  async deployToVercelZeroTouch(githubRepoName, framework = 'nextjs') {
    logger.info(
      `🚀 DevOps Agent: Initiating Vercel Zero-Touch Deployment for repo ${githubRepoName}...`,
    );

    if (!process.env.VERCEL_TOKEN) {
      logger.warn(
        '⚠️ No VERCEL_TOKEN found. Simulating Vercel API deployment for Genesis Engine.',
      );
      return {
    ...
    try {
      ...
      if (
        !createRes.ok &&
        projectData.error?.code !== 'project_already_exists'
      ) {
        throw new Error(
          projectData.error?.message || 'Failed to create Vercel project',
        );
      }
  ...
  async rollbackVercelDeployment(projectId) {
    logger.warn(
      `🚨 DevOps Agent: Initiating emergency Vercel Rollback for project ${projectId}...`,
    );
    ...
    try {
      ...
      const getDeploymentsRes = await fetch(
        `https://api.vercel.com/v6/deployments?projectId=${projectId}&limit=2`,
        {
      ...
      if (!data.deployments || data.deployments.length < 2) {
        logger.warn(
          `DevOps Agent: Cannot rollback. Not enough deployment history.`,
        );
      ...
      const rollbackRes = await fetch(
        `https://api.vercel.com/v2/deployments/${stableDeploymentId}/aliases`,
        {
          method: 'POST',
          headers: { Authorization: `Bearer ${process.env.VERCEL_TOKEN}` },
        },
      ...
      if (!rollbackRes.ok)
        throw new Error('Failed to assign Vercel aliases on rollback');

      logger.info(
        `✅ DevOps Agent: Successfully rolled back project ${projectId} to deployment ${stableDeploymentId}.`,
      );
