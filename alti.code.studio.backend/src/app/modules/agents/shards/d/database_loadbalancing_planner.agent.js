...
class DatabaseLoadBalancingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseLoadBalancingSystem(objective) {
    logger.info(
      `💻 [DatabaseLoadBalancingPlannerAgent] Analyzing Database LoadBalancing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database LoadBalancing Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseLoadBalancingPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseLoadBalancingPlannerAgent = Object.freeze(
  new DatabaseLoadBalancingPlannerAgent(),
);
