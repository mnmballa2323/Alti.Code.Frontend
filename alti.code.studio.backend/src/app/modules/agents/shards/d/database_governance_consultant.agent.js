...
class DatabaseGovernanceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseGovernanceSystem(objective) {
        logger.info(`💻 [DatabaseGovernanceConsultantAgent] Analyzing Database Governance Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Governance Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseGovernanceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseGovernanceConsultantAgent = Object.freeze(new DatabaseGovernanceConsultantAgent());
