...
class DevSecOpsDataLakeDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsDataLakeSystem(objective) {
    logger.info(
      `💻 [DevSecOpsDataLakeDesignerAgent] Analyzing DevSecOps DataLake Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps DataLake Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsDataLakeDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsDataLakeDesignerAgent = Object.freeze(
  new DevSecOpsDataLakeDesignerAgent(),
);
