...
class DevOpsOptimizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsOptimizationSystem(objective) {
        logger.info(`💻 [DevOpsOptimizationSpecialistAgent] Analyzing DevOps Optimization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Optimization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsOptimizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsOptimizationSpecialistAgent = Object.freeze(new DevOpsOptimizationSpecialistAgent());
