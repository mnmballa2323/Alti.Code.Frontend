...
class DevOpsContainerizationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsContainerizationSystem(objective) {
        logger.info(`💻 [DevOpsContainerizationPlannerAgent] Analyzing DevOps Containerization Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Containerization Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsContainerizationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsContainerizationPlannerAgent = Object.freeze(new DevOpsContainerizationPlannerAgent());
