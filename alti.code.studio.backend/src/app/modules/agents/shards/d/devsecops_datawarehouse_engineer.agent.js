...
class DevSecOpsDataWarehouseEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsDataWarehouseSystem(objective) {
        logger.info(`💻 [DevSecOpsDataWarehouseEngineerAgent] Analyzing DevSecOps DataWarehouse Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps DataWarehouse Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsDataWarehouseEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsDataWarehouseEngineerAgent = Object.freeze(new DevSecOpsDataWarehouseEngineerAgent());
