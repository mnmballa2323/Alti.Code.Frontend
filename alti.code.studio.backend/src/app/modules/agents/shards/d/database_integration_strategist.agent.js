...
class DatabaseIntegrationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseIntegrationSystem(objective) {
        logger.info(`💻 [DatabaseIntegrationStrategistAgent] Analyzing Database Integration Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Integration Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseIntegrationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseIntegrationStrategistAgent = Object.freeze(new DatabaseIntegrationStrategistAgent());
