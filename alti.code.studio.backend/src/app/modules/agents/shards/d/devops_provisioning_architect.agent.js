...
class DevOpsProvisioningArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsProvisioningSystem(objective) {
        logger.info(`💻 [DevOpsProvisioningArchitectAgent] Analyzing DevOps Provisioning Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Provisioning Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsProvisioningArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsProvisioningArchitectAgent = Object.freeze(new DevOpsProvisioningArchitectAgent());
