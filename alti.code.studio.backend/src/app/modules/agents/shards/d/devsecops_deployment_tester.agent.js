...
class DevSecOpsDeploymentTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsDeploymentSystem(objective) {
        logger.info(`💻 [DevSecOpsDeploymentTesterAgent] Analyzing DevSecOps Deployment Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Deployment Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsDeploymentTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsDeploymentTesterAgent = Object.freeze(new DevSecOpsDeploymentTesterAgent());
