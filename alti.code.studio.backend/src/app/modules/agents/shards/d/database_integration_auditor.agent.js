...
class DatabaseIntegrationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseIntegrationSystem(objective) {
    logger.info(
      `💻 [DatabaseIntegrationAuditorAgent] Analyzing Database Integration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Integration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseIntegrationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseIntegrationAuditorAgent = Object.freeze(
  new DatabaseIntegrationAuditorAgent(),
);
