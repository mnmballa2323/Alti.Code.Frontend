...
class DevSecOpsStreamingManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsStreamingSystem(objective) {
        logger.info(`💻 [DevSecOpsStreamingManagerAgent] Analyzing DevSecOps Streaming Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Streaming Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsStreamingManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsStreamingManagerAgent = Object.freeze(new DevSecOpsStreamingManagerAgent());
