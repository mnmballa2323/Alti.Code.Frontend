...
class DatabaseGovernanceAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseGovernanceSystem(objective) {
    logger.info(
      `💻 [DatabaseGovernanceAnalystAgent] Analyzing Database Governance Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Governance Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseGovernanceAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseGovernanceAnalystAgent = Object.freeze(
  new DatabaseGovernanceAnalystAgent(),
);
