...
class DatabaseMigrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseMigrationSystem(objective) {
    logger.info(
      `💻 [DatabaseMigrationStrategistAgent] Analyzing Database Migration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Migration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseMigrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseMigrationStrategistAgent = Object.freeze(
  new DatabaseMigrationStrategistAgent(),
);
