...
class DevSecOpsTestingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsTestingSystem(objective) {
    logger.info(
      `💻 [DevSecOpsTestingConsultantAgent] Analyzing DevSecOps Testing Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Testing Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsTestingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsTestingConsultantAgent = Object.freeze(
  new DevSecOpsTestingConsultantAgent(),
);
