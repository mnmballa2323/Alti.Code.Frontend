...
class DatabaseConfigurationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseConfigurationSystem(objective) {
        logger.info(`💻 [DatabaseConfigurationArchitectAgent] Analyzing Database Configuration Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Configuration Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseConfigurationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseConfigurationArchitectAgent = Object.freeze(new DatabaseConfigurationArchitectAgent());
