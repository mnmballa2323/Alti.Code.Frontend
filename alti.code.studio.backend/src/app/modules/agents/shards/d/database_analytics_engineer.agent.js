...
class DatabaseAnalyticsEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseAnalyticsSystem(objective) {
    logger.info(
      `💻 [DatabaseAnalyticsEngineerAgent] Analyzing Database Analytics Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Analytics Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseAnalyticsEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseAnalyticsEngineerAgent = Object.freeze(
  new DatabaseAnalyticsEngineerAgent(),
);
