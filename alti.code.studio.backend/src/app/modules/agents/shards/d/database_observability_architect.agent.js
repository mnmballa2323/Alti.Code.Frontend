...
class DatabaseObservabilityArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseObservabilitySystem(objective) {
    logger.info(
      `💻 [DatabaseObservabilityArchitectAgent] Analyzing Database Observability Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Observability Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseObservabilityArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseObservabilityArchitectAgent = Object.freeze(
  new DatabaseObservabilityArchitectAgent(),
);
