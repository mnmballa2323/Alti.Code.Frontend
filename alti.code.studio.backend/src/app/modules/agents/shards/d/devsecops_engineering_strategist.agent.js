...
class DevSecOpsEngineeringStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsEngineeringSystem(objective) {
    logger.info(
      `💻 [DevSecOpsEngineeringStrategistAgent] Analyzing DevSecOps Engineering Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Engineering Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsEngineeringStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsEngineeringStrategistAgent = Object.freeze(
  new DevSecOpsEngineeringStrategistAgent(),
);
