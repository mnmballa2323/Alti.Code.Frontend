...
class DatabaseContainerizationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseContainerizationSystem(objective) {
        logger.info(`💻 [DatabaseContainerizationStrategistAgent] Analyzing Database Containerization Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Containerization Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseContainerizationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseContainerizationStrategistAgent = Object.freeze(new DatabaseContainerizationStrategistAgent());
