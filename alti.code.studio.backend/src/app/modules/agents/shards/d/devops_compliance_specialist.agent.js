...
class DevOpsComplianceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsComplianceSystem(objective) {
    logger.info(
      `💻 [DevOpsComplianceSpecialistAgent] Analyzing DevOps Compliance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Compliance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsComplianceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsComplianceSpecialistAgent = Object.freeze(
  new DevOpsComplianceSpecialistAgent(),
);
