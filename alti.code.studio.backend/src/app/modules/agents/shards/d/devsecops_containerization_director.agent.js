...
class DevSecOpsContainerizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsContainerizationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsContainerizationDirectorAgent] Analyzing DevSecOps Containerization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Containerization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsContainerizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsContainerizationDirectorAgent = Object.freeze(
  new DevSecOpsContainerizationDirectorAgent(),
);
