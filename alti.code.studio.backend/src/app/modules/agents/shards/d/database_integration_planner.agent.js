...
class DatabaseIntegrationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseIntegrationSystem(objective) {
        logger.info(`💻 [DatabaseIntegrationPlannerAgent] Analyzing Database Integration Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Integration Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseIntegrationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseIntegrationPlannerAgent = Object.freeze(new DatabaseIntegrationPlannerAgent());
