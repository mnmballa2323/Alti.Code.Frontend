...
class DevOpsContainerizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsContainerizationSystem(objective) {
    logger.info(
      `💻 [DevOpsContainerizationLeadAgent] Analyzing DevOps Containerization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Containerization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsContainerizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsContainerizationLeadAgent = Object.freeze(
  new DevOpsContainerizationLeadAgent(),
);
