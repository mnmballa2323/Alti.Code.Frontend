...
class DataIntegrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataIntegrationSystem(objective) {
    logger.info(
      `💻 [DataIntegrationOrchestratorAgent] Analyzing Data Integration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Integration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataIntegrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataIntegrationOrchestratorAgent = Object.freeze(
  new DataIntegrationOrchestratorAgent(),
);
