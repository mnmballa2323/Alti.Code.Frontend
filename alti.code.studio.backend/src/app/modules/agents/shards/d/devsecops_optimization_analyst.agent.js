...
class DevSecOpsOptimizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsOptimizationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsOptimizationAnalystAgent] Analyzing DevSecOps Optimization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Optimization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsOptimizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsOptimizationAnalystAgent = Object.freeze(
  new DevSecOpsOptimizationAnalystAgent(),
);
