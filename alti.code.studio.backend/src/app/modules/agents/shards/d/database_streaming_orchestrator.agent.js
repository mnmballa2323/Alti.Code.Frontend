...
class DatabaseStreamingOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseStreamingSystem(objective) {
        logger.info(`💻 [DatabaseStreamingOrchestratorAgent] Analyzing Database Streaming Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Streaming Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseStreamingOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseStreamingOrchestratorAgent = Object.freeze(new DatabaseStreamingOrchestratorAgent());
