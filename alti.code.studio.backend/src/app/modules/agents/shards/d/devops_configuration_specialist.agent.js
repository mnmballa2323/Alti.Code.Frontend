...
class DevOpsConfigurationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsConfigurationSystem(objective) {
        logger.info(`💻 [DevOpsConfigurationSpecialistAgent] Analyzing DevOps Configuration Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Configuration Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsConfigurationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsConfigurationSpecialistAgent = Object.freeze(new DevOpsConfigurationSpecialistAgent());
