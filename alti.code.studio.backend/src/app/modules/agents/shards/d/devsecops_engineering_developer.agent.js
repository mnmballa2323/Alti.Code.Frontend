...
class DevSecOpsEngineeringDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsEngineeringSystem(objective) {
    logger.info(
      `💻 [DevSecOpsEngineeringDeveloperAgent] Analyzing DevSecOps Engineering Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Engineering Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsEngineeringDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsEngineeringDeveloperAgent = Object.freeze(
  new DevSecOpsEngineeringDeveloperAgent(),
);
