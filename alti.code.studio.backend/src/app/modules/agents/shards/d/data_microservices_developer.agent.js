...
class DataMicroservicesDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDataMicroservicesSystem(objective) {
    logger.info(
      `💻 [DataMicroservicesDeveloperAgent] Analyzing Data Microservices Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Microservices Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataMicroservicesDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataMicroservicesDeveloperAgent = Object.freeze(
  new DataMicroservicesDeveloperAgent(),
);
