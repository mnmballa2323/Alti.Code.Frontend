...
class DevOpsLoadBalancingOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsLoadBalancingSystem(objective) {
        logger.info(`💻 [DevOpsLoadBalancingOrchestratorAgent] Analyzing DevOps LoadBalancing Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps LoadBalancing Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsLoadBalancingOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsLoadBalancingOrchestratorAgent = Object.freeze(new DevOpsLoadBalancingOrchestratorAgent());
