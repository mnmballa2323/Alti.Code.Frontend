...
class DevOpsEngineeringDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsEngineeringSystem(objective) {
    logger.info(
      `💻 [DevOpsEngineeringDirectorAgent] Analyzing DevOps Engineering Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Engineering Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsEngineeringDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsEngineeringDirectorAgent = Object.freeze(
  new DevOpsEngineeringDirectorAgent(),
);
