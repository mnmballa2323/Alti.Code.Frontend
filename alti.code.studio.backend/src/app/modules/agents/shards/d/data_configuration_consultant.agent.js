...
class DataConfigurationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDataConfigurationSystem(objective) {
    logger.info(
      `💻 [DataConfigurationConsultantAgent] Analyzing Data Configuration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Configuration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataConfigurationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataConfigurationConsultantAgent = Object.freeze(
  new DataConfigurationConsultantAgent(),
);
