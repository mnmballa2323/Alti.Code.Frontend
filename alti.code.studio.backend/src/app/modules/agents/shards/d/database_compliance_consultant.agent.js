...
class DatabaseComplianceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseComplianceSystem(objective) {
        logger.info(`💻 [DatabaseComplianceConsultantAgent] Analyzing Database Compliance Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Compliance Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseComplianceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseComplianceConsultantAgent = Object.freeze(new DatabaseComplianceConsultantAgent());
