...
class DatabaseDeploymentOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseDeploymentSystem(objective) {
    logger.info(
      `💻 [DatabaseDeploymentOrchestratorAgent] Analyzing Database Deployment Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Deployment Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseDeploymentOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseDeploymentOrchestratorAgent = Object.freeze(
  new DatabaseDeploymentOrchestratorAgent(),
);
