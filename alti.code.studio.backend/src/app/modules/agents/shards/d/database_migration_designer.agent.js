...
class DatabaseMigrationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseMigrationSystem(objective) {
    logger.info(
      `💻 [DatabaseMigrationDesignerAgent] Analyzing Database Migration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Migration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseMigrationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseMigrationDesignerAgent = Object.freeze(
  new DatabaseMigrationDesignerAgent(),
);
