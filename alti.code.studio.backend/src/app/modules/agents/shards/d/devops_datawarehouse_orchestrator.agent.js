...
class DevOpsDataWarehouseOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DevOpsDataWarehouseOrchestratorAgent] Analyzing DevOps DataWarehouse Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps DataWarehouse Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsDataWarehouseOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsDataWarehouseOrchestratorAgent = Object.freeze(
  new DevOpsDataWarehouseOrchestratorAgent(),
);
