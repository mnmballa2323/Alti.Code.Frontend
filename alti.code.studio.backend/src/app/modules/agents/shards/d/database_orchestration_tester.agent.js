...
class DatabaseOrchestrationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseOrchestrationSystem(objective) {
        logger.info(`💻 [DatabaseOrchestrationTesterAgent] Analyzing Database Orchestration Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Orchestration Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseOrchestrationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseOrchestrationTesterAgent = Object.freeze(new DatabaseOrchestrationTesterAgent());
