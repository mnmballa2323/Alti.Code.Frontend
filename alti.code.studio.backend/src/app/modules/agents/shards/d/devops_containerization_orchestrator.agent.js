...
class DevOpsContainerizationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsContainerizationSystem(objective) {
    logger.info(
      `💻 [DevOpsContainerizationOrchestratorAgent] Analyzing DevOps Containerization Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Containerization Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsContainerizationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsContainerizationOrchestratorAgent = Object.freeze(
  new DevOpsContainerizationOrchestratorAgent(),
);
