...
class DatabaseFaultToleranceDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseFaultToleranceSystem(objective) {
        logger.info(`💻 [DatabaseFaultToleranceDirectorAgent] Analyzing Database FaultTolerance Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database FaultTolerance Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseFaultToleranceDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseFaultToleranceDirectorAgent = Object.freeze(new DatabaseFaultToleranceDirectorAgent());
