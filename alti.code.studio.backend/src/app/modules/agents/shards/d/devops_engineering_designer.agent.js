...
class DevOpsEngineeringDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsEngineeringSystem(objective) {
    logger.info(
      `💻 [DevOpsEngineeringDesignerAgent] Analyzing DevOps Engineering Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Engineering Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsEngineeringDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsEngineeringDesignerAgent = Object.freeze(
  new DevOpsEngineeringDesignerAgent(),
);
