...
class DevOpsObservabilityAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsObservabilitySystem(objective) {
        logger.info(`💻 [DevOpsObservabilityAuditorAgent] Analyzing DevOps Observability Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Observability Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsObservabilityAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsObservabilityAuditorAgent = Object.freeze(new DevOpsObservabilityAuditorAgent());
