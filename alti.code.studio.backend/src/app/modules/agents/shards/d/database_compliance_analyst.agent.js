...
class DatabaseComplianceAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseComplianceSystem(objective) {
    logger.info(
      `💻 [DatabaseComplianceAnalystAgent] Analyzing Database Compliance Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Compliance Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseComplianceAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseComplianceAnalystAgent = Object.freeze(
  new DatabaseComplianceAnalystAgent(),
);
