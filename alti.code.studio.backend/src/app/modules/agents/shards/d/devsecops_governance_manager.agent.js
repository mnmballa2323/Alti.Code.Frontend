...
class DevSecOpsGovernanceManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsGovernanceSystem(objective) {
        logger.info(`💻 [DevSecOpsGovernanceManagerAgent] Analyzing DevSecOps Governance Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Governance Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsGovernanceManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsGovernanceManagerAgent = Object.freeze(new DevSecOpsGovernanceManagerAgent());
