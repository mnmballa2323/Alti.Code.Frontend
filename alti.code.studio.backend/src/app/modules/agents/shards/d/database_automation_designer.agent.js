...
class DatabaseAutomationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseAutomationSystem(objective) {
    logger.info(
      `💻 [DatabaseAutomationDesignerAgent] Analyzing Database Automation Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Automation Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseAutomationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseAutomationDesignerAgent = Object.freeze(
  new DatabaseAutomationDesignerAgent(),
);
