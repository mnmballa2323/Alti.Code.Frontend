...
class DevOpsTelemetrySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsTelemetrySystem(objective) {
    logger.info(
      `💻 [DevOpsTelemetrySpecialistAgent] Analyzing DevOps Telemetry Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Telemetry Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsTelemetrySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsTelemetrySpecialistAgent = Object.freeze(
  new DevOpsTelemetrySpecialistAgent(),
);
