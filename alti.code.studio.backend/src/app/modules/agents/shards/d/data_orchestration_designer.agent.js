...
class DataOrchestrationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDataOrchestrationSystem(objective) {
    logger.info(
      `💻 [DataOrchestrationDesignerAgent] Analyzing Data Orchestration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Orchestration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataOrchestrationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataOrchestrationDesignerAgent = Object.freeze(
  new DataOrchestrationDesignerAgent(),
);
