...
class DevSecOpsTelemetrySpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsTelemetrySystem(objective) {
        logger.info(`💻 [DevSecOpsTelemetrySpecialistAgent] Analyzing DevSecOps Telemetry Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Telemetry Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsTelemetrySpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsTelemetrySpecialistAgent = Object.freeze(new DevSecOpsTelemetrySpecialistAgent());
