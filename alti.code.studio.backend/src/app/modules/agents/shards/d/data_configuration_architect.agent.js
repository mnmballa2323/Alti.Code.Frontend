...
class DataConfigurationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDataConfigurationSystem(objective) {
    logger.info(
      `💻 [DataConfigurationArchitectAgent] Analyzing Data Configuration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Configuration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataConfigurationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataConfigurationArchitectAgent = Object.freeze(
  new DataConfigurationArchitectAgent(),
);
