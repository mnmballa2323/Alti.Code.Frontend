...
class DevOpsTelemetryConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsTelemetrySystem(objective) {
        logger.info(`💻 [DevOpsTelemetryConsultantAgent] Analyzing DevOps Telemetry Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Telemetry Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsTelemetryConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsTelemetryConsultantAgent = Object.freeze(new DevOpsTelemetryConsultantAgent());
