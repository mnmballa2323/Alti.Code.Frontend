...
class DevSecOpsObservabilityAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsObservabilitySystem(objective) {
    logger.info(
      `💻 [DevSecOpsObservabilityAnalystAgent] Analyzing DevSecOps Observability Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Observability Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsObservabilityAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsObservabilityAnalystAgent = Object.freeze(
  new DevSecOpsObservabilityAnalystAgent(),
);
