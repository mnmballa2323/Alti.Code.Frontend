...
class DataContainerizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDataContainerizationSystem(objective) {
    logger.info(
      `💻 [DataContainerizationTesterAgent] Analyzing Data Containerization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Containerization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataContainerizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataContainerizationTesterAgent = Object.freeze(
  new DataContainerizationTesterAgent(),
);
