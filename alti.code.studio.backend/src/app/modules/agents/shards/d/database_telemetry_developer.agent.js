...
class DatabaseTelemetryDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseTelemetrySystem(objective) {
    logger.info(
      `💻 [DatabaseTelemetryDeveloperAgent] Analyzing Database Telemetry Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Telemetry Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseTelemetryDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseTelemetryDeveloperAgent = Object.freeze(
  new DatabaseTelemetryDeveloperAgent(),
);
