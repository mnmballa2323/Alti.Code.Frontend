...
class DevSecOpsServerlessPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsServerlessSystem(objective) {
        logger.info(`💻 [DevSecOpsServerlessPlannerAgent] Analyzing DevSecOps Serverless Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Serverless Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsServerlessPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsServerlessPlannerAgent = Object.freeze(new DevSecOpsServerlessPlannerAgent());
