...
class DevOpsConfigurationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsConfigurationSystem(objective) {
    logger.info(
      `💻 [DevOpsConfigurationArchitectAgent] Analyzing DevOps Configuration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Configuration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsConfigurationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsConfigurationArchitectAgent = Object.freeze(
  new DevOpsConfigurationArchitectAgent(),
);
