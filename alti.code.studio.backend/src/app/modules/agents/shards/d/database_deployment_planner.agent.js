...
class DatabaseDeploymentPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseDeploymentSystem(objective) {
        logger.info(`💻 [DatabaseDeploymentPlannerAgent] Analyzing Database Deployment Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Deployment Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseDeploymentPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseDeploymentPlannerAgent = Object.freeze(new DatabaseDeploymentPlannerAgent());
