...
class DatabaseObservabilityLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseObservabilitySystem(objective) {
    logger.info(
      `💻 [DatabaseObservabilityLeadAgent] Analyzing Database Observability Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Observability Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseObservabilityLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseObservabilityLeadAgent = Object.freeze(
  new DatabaseObservabilityLeadAgent(),
);
