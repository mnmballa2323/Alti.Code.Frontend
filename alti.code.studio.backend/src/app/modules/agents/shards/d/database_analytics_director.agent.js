...
class DatabaseAnalyticsDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseAnalyticsSystem(objective) {
    logger.info(
      `💻 [DatabaseAnalyticsDirectorAgent] Analyzing Database Analytics Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Analytics Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseAnalyticsDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseAnalyticsDirectorAgent = Object.freeze(
  new DatabaseAnalyticsDirectorAgent(),
);
