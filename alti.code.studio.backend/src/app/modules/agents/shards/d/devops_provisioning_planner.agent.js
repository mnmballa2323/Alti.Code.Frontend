...
class DevOpsProvisioningPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsProvisioningSystem(objective) {
    logger.info(
      `💻 [DevOpsProvisioningPlannerAgent] Analyzing DevOps Provisioning Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Provisioning Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsProvisioningPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsProvisioningPlannerAgent = Object.freeze(
  new DevOpsProvisioningPlannerAgent(),
);
