...
class DeadCodeAgent {
    ...
    async analyzeProject(directory) {
        logger.info(`💀 DeadCodeReaper: Engaging total project AST analysis in [${directory}]...`);
        ...
        try {
            ...
            if (err.stdout) {
                ...
                if (lines.length > 0) {
                    logger.info(`💀 DeadCodeReaper: Detected ${lines.length} unused export blocks.`);
                    return {
                        success: true,
                        unusedExports: lines
                    };
            ...
            logger.error(`❌ DeadCodeReaper: Critical failure mapping AST exports.`, err.message);
            return {
                success: false,
                unusedExports: []
            };
...
export const deadCodeAgent = Object.freeze(new DeadCodeAgent());
