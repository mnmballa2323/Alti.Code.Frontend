...
class DatabaseGovernanceManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseGovernanceSystem(objective) {
    logger.info(
      `💻 [DatabaseGovernanceManagerAgent] Analyzing Database Governance Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Governance Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseGovernanceManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseGovernanceManagerAgent = Object.freeze(
  new DatabaseGovernanceManagerAgent(),
);
