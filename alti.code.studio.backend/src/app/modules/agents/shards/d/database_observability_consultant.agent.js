...
class DatabaseObservabilityConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseObservabilitySystem(objective) {
    logger.info(
      `💻 [DatabaseObservabilityConsultantAgent] Analyzing Database Observability Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Observability Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseObservabilityConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseObservabilityConsultantAgent = Object.freeze(
  new DatabaseObservabilityConsultantAgent(),
);
