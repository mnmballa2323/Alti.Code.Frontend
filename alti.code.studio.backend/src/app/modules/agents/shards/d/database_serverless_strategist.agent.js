...
class DatabaseServerlessStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseServerlessSystem(objective) {
    logger.info(
      `💻 [DatabaseServerlessStrategistAgent] Analyzing Database Serverless Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Serverless Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseServerlessStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseServerlessStrategistAgent = Object.freeze(
  new DatabaseServerlessStrategistAgent(),
);
