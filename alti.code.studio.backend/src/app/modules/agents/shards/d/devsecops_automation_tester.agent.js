...
class DevSecOpsAutomationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsAutomationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsAutomationTesterAgent] Analyzing DevSecOps Automation Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Automation Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsAutomationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsAutomationTesterAgent = Object.freeze(
  new DevSecOpsAutomationTesterAgent(),
);
