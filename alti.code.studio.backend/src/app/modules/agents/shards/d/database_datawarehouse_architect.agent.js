...
class DatabaseDataWarehouseArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseDataWarehouseSystem(objective) {
        logger.info(`💻 [DatabaseDataWarehouseArchitectAgent] Analyzing Database DataWarehouse Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database DataWarehouse Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseDataWarehouseArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseDataWarehouseArchitectAgent = Object.freeze(new DatabaseDataWarehouseArchitectAgent());
