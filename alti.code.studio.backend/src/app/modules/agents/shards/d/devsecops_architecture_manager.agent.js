...
class DevSecOpsArchitectureManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsArchitectureSystem(objective) {
    logger.info(
      `💻 [DevSecOpsArchitectureManagerAgent] Analyzing DevSecOps Architecture Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Architecture Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsArchitectureManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsArchitectureManagerAgent = Object.freeze(
  new DevSecOpsArchitectureManagerAgent(),
);
