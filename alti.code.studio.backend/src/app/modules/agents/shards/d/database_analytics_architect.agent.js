...
class DatabaseAnalyticsArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseAnalyticsSystem(objective) {
        logger.info(`💻 [DatabaseAnalyticsArchitectAgent] Analyzing Database Analytics Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Analytics Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseAnalyticsArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseAnalyticsArchitectAgent = Object.freeze(new DatabaseAnalyticsArchitectAgent());
