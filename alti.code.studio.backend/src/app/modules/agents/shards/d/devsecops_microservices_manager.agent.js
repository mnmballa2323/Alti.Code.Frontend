...
class DevSecOpsMicroservicesManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsMicroservicesSystem(objective) {
        logger.info(`💻 [DevSecOpsMicroservicesManagerAgent] Analyzing DevSecOps Microservices Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Microservices Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsMicroservicesManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsMicroservicesManagerAgent = Object.freeze(new DevSecOpsMicroservicesManagerAgent());
