...
class DevOpsObservabilityStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsObservabilitySystem(objective) {
        logger.info(`💻 [DevOpsObservabilityStrategistAgent] Analyzing DevOps Observability Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Observability Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsObservabilityStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsObservabilityStrategistAgent = Object.freeze(new DevOpsObservabilityStrategistAgent());
