...
class DataOrchestrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDataOrchestrationSystem(objective) {
    logger.info(
      `💻 [DataOrchestrationStrategistAgent] Analyzing Data Orchestration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Orchestration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataOrchestrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataOrchestrationStrategistAgent = Object.freeze(
  new DataOrchestrationStrategistAgent(),
);
