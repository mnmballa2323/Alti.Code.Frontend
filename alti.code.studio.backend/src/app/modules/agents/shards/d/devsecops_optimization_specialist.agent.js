...
class DevSecOpsOptimizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsOptimizationSystem(objective) {
        logger.info(`💻 [DevSecOpsOptimizationSpecialistAgent] Analyzing DevSecOps Optimization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Optimization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsOptimizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsOptimizationSpecialistAgent = Object.freeze(new DevSecOpsOptimizationSpecialistAgent());
