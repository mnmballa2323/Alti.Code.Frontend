...
class DevOpsContainerizationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsContainerizationSystem(objective) {
        logger.info(`💻 [DevOpsContainerizationDesignerAgent] Analyzing DevOps Containerization Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Containerization Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsContainerizationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsContainerizationDesignerAgent = Object.freeze(new DevOpsContainerizationDesignerAgent());
