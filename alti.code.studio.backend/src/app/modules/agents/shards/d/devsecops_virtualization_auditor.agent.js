...
class DevSecOpsVirtualizationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsVirtualizationSystem(objective) {
        logger.info(`💻 [DevSecOpsVirtualizationAuditorAgent] Analyzing DevSecOps Virtualization Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Virtualization Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsVirtualizationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsVirtualizationAuditorAgent = Object.freeze(new DevSecOpsVirtualizationAuditorAgent());
