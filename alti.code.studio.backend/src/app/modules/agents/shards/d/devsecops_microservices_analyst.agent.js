...
class DevSecOpsMicroservicesAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsMicroservicesSystem(objective) {
        logger.info(`💻 [DevSecOpsMicroservicesAnalystAgent] Analyzing DevSecOps Microservices Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Microservices Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsMicroservicesAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsMicroservicesAnalystAgent = Object.freeze(new DevSecOpsMicroservicesAnalystAgent());
