...
class DatabaseAutomationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseAutomationSystem(objective) {
        logger.info(`💻 [DatabaseAutomationPlannerAgent] Analyzing Database Automation Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Automation Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseAutomationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseAutomationPlannerAgent = Object.freeze(new DatabaseAutomationPlannerAgent());
