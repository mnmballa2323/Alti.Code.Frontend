...
class DatabaseOptimizationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseOptimizationSystem(objective) {
        logger.info(`💻 [DatabaseOptimizationArchitectAgent] Analyzing Database Optimization Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Optimization Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseOptimizationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseOptimizationArchitectAgent = Object.freeze(new DatabaseOptimizationArchitectAgent());
