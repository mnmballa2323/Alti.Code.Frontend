...
class DatabaseAutomationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseAutomationSystem(objective) {
    logger.info(
      `💻 [DatabaseAutomationAnalystAgent] Analyzing Database Automation Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Automation Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseAutomationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseAutomationAnalystAgent = Object.freeze(
  new DatabaseAutomationAnalystAgent(),
);
