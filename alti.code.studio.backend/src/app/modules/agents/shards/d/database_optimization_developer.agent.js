...
class DatabaseOptimizationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseOptimizationSystem(objective) {
    logger.info(
      `💻 [DatabaseOptimizationDeveloperAgent] Analyzing Database Optimization Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Optimization Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseOptimizationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseOptimizationDeveloperAgent = Object.freeze(
  new DatabaseOptimizationDeveloperAgent(),
);
