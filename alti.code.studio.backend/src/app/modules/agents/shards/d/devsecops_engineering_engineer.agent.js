...
class DevSecOpsEngineeringEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsEngineeringSystem(objective) {
    logger.info(
      `💻 [DevSecOpsEngineeringEngineerAgent] Analyzing DevSecOps Engineering Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Engineering Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsEngineeringEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsEngineeringEngineerAgent = Object.freeze(
  new DevSecOpsEngineeringEngineerAgent(),
);
