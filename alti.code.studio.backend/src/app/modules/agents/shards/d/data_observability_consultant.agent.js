...
class DataObservabilityConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDataObservabilitySystem(objective) {
    logger.info(
      `💻 [DataObservabilityConsultantAgent] Analyzing Data Observability Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Observability Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataObservabilityConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataObservabilityConsultantAgent = Object.freeze(
  new DataObservabilityConsultantAgent(),
);
