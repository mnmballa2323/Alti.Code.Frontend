...
class DataAutomationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataAutomationSystem(objective) {
    logger.info(
      `💻 [DataAutomationOrchestratorAgent] Analyzing Data Automation Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Automation Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataAutomationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataAutomationOrchestratorAgent = Object.freeze(
  new DataAutomationOrchestratorAgent(),
);
