...
class DatabaseCachingStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseCachingSystem(objective) {
    logger.info(
      `💻 [DatabaseCachingStrategistAgent] Analyzing Database Caching Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Caching Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseCachingStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseCachingStrategistAgent = Object.freeze(
  new DatabaseCachingStrategistAgent(),
);
