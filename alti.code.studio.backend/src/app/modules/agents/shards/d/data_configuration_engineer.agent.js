...
class DataConfigurationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDataConfigurationSystem(objective) {
    logger.info(
      `💻 [DataConfigurationEngineerAgent] Analyzing Data Configuration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Configuration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataConfigurationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataConfigurationEngineerAgent = Object.freeze(
  new DataConfigurationEngineerAgent(),
);
