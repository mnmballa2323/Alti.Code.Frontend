...
class DevOpsResilienceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsResilienceSystem(objective) {
    logger.info(
      `💻 [DevOpsResilienceConsultantAgent] Analyzing DevOps Resilience Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Resilience Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsResilienceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsResilienceConsultantAgent = Object.freeze(
  new DevOpsResilienceConsultantAgent(),
);
