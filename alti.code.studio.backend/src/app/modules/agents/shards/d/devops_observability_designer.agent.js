...
class DevOpsObservabilityDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsObservabilitySystem(objective) {
    logger.info(
      `💻 [DevOpsObservabilityDesignerAgent] Analyzing DevOps Observability Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Observability Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsObservabilityDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsObservabilityDesignerAgent = Object.freeze(
  new DevOpsObservabilityDesignerAgent(),
);
