...
class DatabaseStreamingStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseStreamingSystem(objective) {
    logger.info(
      `💻 [DatabaseStreamingStrategistAgent] Analyzing Database Streaming Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Streaming Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseStreamingStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseStreamingStrategistAgent = Object.freeze(
  new DatabaseStreamingStrategistAgent(),
);
