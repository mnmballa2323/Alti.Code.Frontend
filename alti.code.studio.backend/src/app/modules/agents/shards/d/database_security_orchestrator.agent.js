...
class DatabaseSecurityOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseSecuritySystem(objective) {
        logger.info(`💻 [DatabaseSecurityOrchestratorAgent] Analyzing Database Security Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Security Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseSecurityOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseSecurityOrchestratorAgent = Object.freeze(new DatabaseSecurityOrchestratorAgent());
