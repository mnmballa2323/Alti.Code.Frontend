...
class DatabaseGovernanceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseGovernanceSystem(objective) {
    logger.info(
      `💻 [DatabaseGovernanceEngineerAgent] Analyzing Database Governance Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Governance Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseGovernanceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseGovernanceEngineerAgent = Object.freeze(
  new DatabaseGovernanceEngineerAgent(),
);
