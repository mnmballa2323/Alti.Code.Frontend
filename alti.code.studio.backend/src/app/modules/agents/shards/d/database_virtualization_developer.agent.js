...
class DatabaseVirtualizationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseVirtualizationSystem(objective) {
    logger.info(
      `💻 [DatabaseVirtualizationDeveloperAgent] Analyzing Database Virtualization Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Virtualization Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseVirtualizationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseVirtualizationDeveloperAgent = Object.freeze(
  new DatabaseVirtualizationDeveloperAgent(),
);
