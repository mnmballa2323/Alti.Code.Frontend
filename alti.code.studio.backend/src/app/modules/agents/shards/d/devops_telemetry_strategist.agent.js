...
class DevOpsTelemetryStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsTelemetrySystem(objective) {
    logger.info(
      `💻 [DevOpsTelemetryStrategistAgent] Analyzing DevOps Telemetry Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Telemetry Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsTelemetryStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsTelemetryStrategistAgent = Object.freeze(
  new DevOpsTelemetryStrategistAgent(),
);
