...
class DevOpsContainerizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsContainerizationSystem(objective) {
    logger.info(
      `💻 [DevOpsContainerizationAuditorAgent] Analyzing DevOps Containerization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Containerization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsContainerizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsContainerizationAuditorAgent = Object.freeze(
  new DevOpsContainerizationAuditorAgent(),
);
