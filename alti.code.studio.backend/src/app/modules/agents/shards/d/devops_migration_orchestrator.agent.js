...
class DevOpsMigrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsMigrationSystem(objective) {
    logger.info(
      `💻 [DevOpsMigrationOrchestratorAgent] Analyzing DevOps Migration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Migration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsMigrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsMigrationOrchestratorAgent = Object.freeze(
  new DevOpsMigrationOrchestratorAgent(),
);
