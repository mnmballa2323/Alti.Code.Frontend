...
class DevSecOpsEngineeringArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsEngineeringSystem(objective) {
    logger.info(
      `💻 [DevSecOpsEngineeringArchitectAgent] Analyzing DevSecOps Engineering Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Engineering Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsEngineeringArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsEngineeringArchitectAgent = Object.freeze(
  new DevSecOpsEngineeringArchitectAgent(),
);
