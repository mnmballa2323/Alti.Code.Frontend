...
class DevSecOpsMicroservicesDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsMicroservicesSystem(objective) {
        logger.info(`💻 [DevSecOpsMicroservicesDeveloperAgent] Analyzing DevSecOps Microservices Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Microservices Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsMicroservicesDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsMicroservicesDeveloperAgent = Object.freeze(new DevSecOpsMicroservicesDeveloperAgent());
