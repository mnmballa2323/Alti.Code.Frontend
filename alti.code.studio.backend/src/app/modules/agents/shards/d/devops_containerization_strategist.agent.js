...
class DevOpsContainerizationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsContainerizationSystem(objective) {
        logger.info(`💻 [DevOpsContainerizationStrategistAgent] Analyzing DevOps Containerization Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Containerization Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsContainerizationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsContainerizationStrategistAgent = Object.freeze(new DevOpsContainerizationStrategistAgent());
