...
class DevOpsVirtualizationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsVirtualizationSystem(objective) {
        logger.info(`💻 [DevOpsVirtualizationDeveloperAgent] Analyzing DevOps Virtualization Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Virtualization Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsVirtualizationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsVirtualizationDeveloperAgent = Object.freeze(new DevOpsVirtualizationDeveloperAgent());
