...
class DevSecOpsTelemetryEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsTelemetrySystem(objective) {
    logger.info(
      `💻 [DevSecOpsTelemetryEngineerAgent] Analyzing DevSecOps Telemetry Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Telemetry Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsTelemetryEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsTelemetryEngineerAgent = Object.freeze(
  new DevSecOpsTelemetryEngineerAgent(),
);
