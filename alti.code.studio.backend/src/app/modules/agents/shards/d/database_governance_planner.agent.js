...
class DatabaseGovernancePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseGovernanceSystem(objective) {
        logger.info(`💻 [DatabaseGovernancePlannerAgent] Analyzing Database Governance Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Governance Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseGovernancePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseGovernancePlannerAgent = Object.freeze(new DatabaseGovernancePlannerAgent());
