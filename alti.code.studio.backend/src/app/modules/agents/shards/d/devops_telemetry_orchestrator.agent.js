...
class DevOpsTelemetryOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsTelemetrySystem(objective) {
    logger.info(
      `💻 [DevOpsTelemetryOrchestratorAgent] Analyzing DevOps Telemetry Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Telemetry Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsTelemetryOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsTelemetryOrchestratorAgent = Object.freeze(
  new DevOpsTelemetryOrchestratorAgent(),
);
