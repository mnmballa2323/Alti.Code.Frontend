...
class DatabaseContainerizationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseContainerizationSystem(objective) {
    logger.info(
      `💻 [DatabaseContainerizationOrchestratorAgent] Analyzing Database Containerization Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Containerization Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseContainerizationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseContainerizationOrchestratorAgent = Object.freeze(
  new DatabaseContainerizationOrchestratorAgent(),
);
