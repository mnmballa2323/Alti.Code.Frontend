...
class DevOpsDataWarehouseAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsDataWarehouseSystem(objective) {
        logger.info(`💻 [DevOpsDataWarehouseAuditorAgent] Analyzing DevOps DataWarehouse Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps DataWarehouse Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsDataWarehouseAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsDataWarehouseAuditorAgent = Object.freeze(new DevOpsDataWarehouseAuditorAgent());
