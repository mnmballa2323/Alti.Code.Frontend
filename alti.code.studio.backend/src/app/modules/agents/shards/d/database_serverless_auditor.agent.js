...
class DatabaseServerlessAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseServerlessSystem(objective) {
    logger.info(
      `💻 [DatabaseServerlessAuditorAgent] Analyzing Database Serverless Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Serverless Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseServerlessAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseServerlessAuditorAgent = Object.freeze(
  new DatabaseServerlessAuditorAgent(),
);
