...
class DevSecOpsVirtualizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsVirtualizationSystem(objective) {
        logger.info(`💻 [DevSecOpsVirtualizationDirectorAgent] Analyzing DevSecOps Virtualization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Virtualization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsVirtualizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsVirtualizationDirectorAgent = Object.freeze(new DevSecOpsVirtualizationDirectorAgent());
