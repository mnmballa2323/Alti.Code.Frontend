...
class DevSecOpsTelemetryDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsTelemetrySystem(objective) {
    logger.info(
      `💻 [DevSecOpsTelemetryDeveloperAgent] Analyzing DevSecOps Telemetry Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Telemetry Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsTelemetryDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsTelemetryDeveloperAgent = Object.freeze(
  new DevSecOpsTelemetryDeveloperAgent(),
);
