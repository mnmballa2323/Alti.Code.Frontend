...
class DevSecOpsMigrationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsMigrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsMigrationAnalystAgent] Analyzing DevSecOps Migration Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Migration Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsMigrationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsMigrationAnalystAgent = Object.freeze(
  new DevSecOpsMigrationAnalystAgent(),
);
