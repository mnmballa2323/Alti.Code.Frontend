...
class DevSecOpsIntegrationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsIntegrationSystem(objective) {
        logger.info(`💻 [DevSecOpsIntegrationConsultantAgent] Analyzing DevSecOps Integration Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Integration Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsIntegrationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsIntegrationConsultantAgent = Object.freeze(new DevSecOpsIntegrationConsultantAgent());
