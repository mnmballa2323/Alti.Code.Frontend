...
class DevSecOpsOptimizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsOptimizationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsOptimizationPlannerAgent] Analyzing DevSecOps Optimization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Optimization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsOptimizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsOptimizationPlannerAgent = Object.freeze(
  new DevSecOpsOptimizationPlannerAgent(),
);
