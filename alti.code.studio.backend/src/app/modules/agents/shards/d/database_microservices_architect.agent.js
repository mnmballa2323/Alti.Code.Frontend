...
class DatabaseMicroservicesArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseMicroservicesSystem(objective) {
    logger.info(
      `💻 [DatabaseMicroservicesArchitectAgent] Analyzing Database Microservices Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Microservices Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseMicroservicesArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseMicroservicesArchitectAgent = Object.freeze(
  new DatabaseMicroservicesArchitectAgent(),
);
