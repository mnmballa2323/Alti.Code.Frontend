...
class DatabaseGovernanceOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseGovernanceSystem(objective) {
    logger.info(
      `💻 [DatabaseGovernanceOrchestratorAgent] Analyzing Database Governance Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Governance Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseGovernanceOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseGovernanceOrchestratorAgent = Object.freeze(
  new DatabaseGovernanceOrchestratorAgent(),
);
