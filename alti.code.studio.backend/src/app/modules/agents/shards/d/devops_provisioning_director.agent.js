...
class DevOpsProvisioningDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsProvisioningSystem(objective) {
    logger.info(
      `💻 [DevOpsProvisioningDirectorAgent] Analyzing DevOps Provisioning Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Provisioning Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsProvisioningDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsProvisioningDirectorAgent = Object.freeze(
  new DevOpsProvisioningDirectorAgent(),
);
