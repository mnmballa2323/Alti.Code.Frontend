...
class DatabaseContainerizationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseContainerizationSystem(objective) {
        logger.info(`💻 [DatabaseContainerizationDeveloperAgent] Analyzing Database Containerization Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Containerization Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseContainerizationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseContainerizationDeveloperAgent = Object.freeze(new DatabaseContainerizationDeveloperAgent());
