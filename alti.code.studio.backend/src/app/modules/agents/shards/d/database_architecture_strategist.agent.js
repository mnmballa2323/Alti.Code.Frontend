...
class DatabaseArchitectureStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseArchitectureSystem(objective) {
        logger.info(`💻 [DatabaseArchitectureStrategistAgent] Analyzing Database Architecture Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Architecture Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseArchitectureStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseArchitectureStrategistAgent = Object.freeze(new DatabaseArchitectureStrategistAgent());
