...
class DatabaseVirtualizationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseVirtualizationSystem(objective) {
        logger.info(`💻 [DatabaseVirtualizationTesterAgent] Analyzing Database Virtualization Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Virtualization Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseVirtualizationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseVirtualizationTesterAgent = Object.freeze(new DatabaseVirtualizationTesterAgent());
