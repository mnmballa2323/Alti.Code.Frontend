...
class DevOpsOrchestrationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsOrchestrationSystem(objective) {
    logger.info(
      `💻 [DevOpsOrchestrationAuditorAgent] Analyzing DevOps Orchestration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Orchestration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsOrchestrationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsOrchestrationAuditorAgent = Object.freeze(
  new DevOpsOrchestrationAuditorAgent(),
);
