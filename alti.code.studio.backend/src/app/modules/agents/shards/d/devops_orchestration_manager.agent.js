...
class DevOpsOrchestrationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsOrchestrationSystem(objective) {
    logger.info(
      `💻 [DevOpsOrchestrationManagerAgent] Analyzing DevOps Orchestration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Orchestration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsOrchestrationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsOrchestrationManagerAgent = Object.freeze(
  new DevOpsOrchestrationManagerAgent(),
);
