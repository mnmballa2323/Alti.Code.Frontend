...
class DatabaseIntegrationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseIntegrationSystem(objective) {
        logger.info(`💻 [DatabaseIntegrationDesignerAgent] Analyzing Database Integration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Integration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseIntegrationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseIntegrationDesignerAgent = Object.freeze(new DatabaseIntegrationDesignerAgent());
