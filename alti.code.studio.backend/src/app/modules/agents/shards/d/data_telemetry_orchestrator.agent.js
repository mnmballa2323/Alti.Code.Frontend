...
class DataTelemetryOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataTelemetrySystem(objective) {
    logger.info(
      `💻 [DataTelemetryOrchestratorAgent] Analyzing Data Telemetry Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Telemetry Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataTelemetryOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataTelemetryOrchestratorAgent = Object.freeze(
  new DataTelemetryOrchestratorAgent(),
);
