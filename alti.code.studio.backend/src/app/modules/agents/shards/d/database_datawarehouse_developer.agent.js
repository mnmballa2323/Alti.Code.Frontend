...
class DatabaseDataWarehouseDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseDataWarehouseSystem(objective) {
        logger.info(`💻 [DatabaseDataWarehouseDeveloperAgent] Analyzing Database DataWarehouse Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database DataWarehouse Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseDataWarehouseDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseDataWarehouseDeveloperAgent = Object.freeze(new DatabaseDataWarehouseDeveloperAgent());
