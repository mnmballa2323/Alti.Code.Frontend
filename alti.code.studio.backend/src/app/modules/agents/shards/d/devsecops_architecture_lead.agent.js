...
class DevSecOpsArchitectureLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsArchitectureSystem(objective) {
    logger.info(
      `💻 [DevSecOpsArchitectureLeadAgent] Analyzing DevSecOps Architecture Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Architecture Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsArchitectureLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsArchitectureLeadAgent = Object.freeze(
  new DevSecOpsArchitectureLeadAgent(),
);
