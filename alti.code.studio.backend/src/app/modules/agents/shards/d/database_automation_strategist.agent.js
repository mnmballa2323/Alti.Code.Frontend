...
class DatabaseAutomationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseAutomationSystem(objective) {
    logger.info(
      `💻 [DatabaseAutomationStrategistAgent] Analyzing Database Automation Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Automation Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseAutomationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseAutomationStrategistAgent = Object.freeze(
  new DatabaseAutomationStrategistAgent(),
);
