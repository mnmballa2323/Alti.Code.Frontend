...
class DatabaseDataWarehouseAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DatabaseDataWarehouseAnalystAgent] Analyzing Database DataWarehouse Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database DataWarehouse Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseDataWarehouseAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseDataWarehouseAnalystAgent = Object.freeze(
  new DatabaseDataWarehouseAnalystAgent(),
);
