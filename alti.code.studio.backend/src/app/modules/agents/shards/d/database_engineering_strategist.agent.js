...
class DatabaseEngineeringStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseEngineeringSystem(objective) {
        logger.info(`💻 [DatabaseEngineeringStrategistAgent] Analyzing Database Engineering Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Engineering Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseEngineeringStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseEngineeringStrategistAgent = Object.freeze(new DatabaseEngineeringStrategistAgent());
