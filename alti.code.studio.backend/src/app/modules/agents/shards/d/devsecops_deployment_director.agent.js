...
class DevSecOpsDeploymentDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsDeploymentSystem(objective) {
    logger.info(
      `💻 [DevSecOpsDeploymentDirectorAgent] Analyzing DevSecOps Deployment Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Deployment Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsDeploymentDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsDeploymentDirectorAgent = Object.freeze(
  new DevSecOpsDeploymentDirectorAgent(),
);
