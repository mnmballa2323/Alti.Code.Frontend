...
class DevSecOpsMicroservicesDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsMicroservicesSystem(objective) {
        logger.info(`💻 [DevSecOpsMicroservicesDirectorAgent] Analyzing DevSecOps Microservices Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Microservices Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsMicroservicesDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsMicroservicesDirectorAgent = Object.freeze(new DevSecOpsMicroservicesDirectorAgent());
