...
class DatabaseConfigurationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseConfigurationSystem(objective) {
        logger.info(`💻 [DatabaseConfigurationDesignerAgent] Analyzing Database Configuration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Configuration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseConfigurationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseConfigurationDesignerAgent = Object.freeze(new DatabaseConfigurationDesignerAgent());
