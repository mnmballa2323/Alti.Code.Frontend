...
class DatabaseMicroservicesSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseMicroservicesSystem(objective) {
        logger.info(`💻 [DatabaseMicroservicesSpecialistAgent] Analyzing Database Microservices Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Microservices Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseMicroservicesSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseMicroservicesSpecialistAgent = Object.freeze(new DatabaseMicroservicesSpecialistAgent());
