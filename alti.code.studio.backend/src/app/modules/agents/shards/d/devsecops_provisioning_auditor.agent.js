...
class DevSecOpsProvisioningAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsProvisioningSystem(objective) {
    logger.info(
      `💻 [DevSecOpsProvisioningAuditorAgent] Analyzing DevSecOps Provisioning Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Provisioning Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsProvisioningAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsProvisioningAuditorAgent = Object.freeze(
  new DevSecOpsProvisioningAuditorAgent(),
);
