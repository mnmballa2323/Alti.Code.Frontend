...
class DatabaseDataWarehouseEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DatabaseDataWarehouseEngineerAgent] Analyzing Database DataWarehouse Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database DataWarehouse Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseDataWarehouseEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseDataWarehouseEngineerAgent = Object.freeze(
  new DatabaseDataWarehouseEngineerAgent(),
);
