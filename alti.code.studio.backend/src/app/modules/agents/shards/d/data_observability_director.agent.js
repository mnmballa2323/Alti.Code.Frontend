...
class DataObservabilityDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataObservabilitySystem(objective) {
    logger.info(
      `💻 [DataObservabilityDirectorAgent] Analyzing Data Observability Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Observability Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataObservabilityDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataObservabilityDirectorAgent = Object.freeze(
  new DataObservabilityDirectorAgent(),
);
