...
class DatabaseCompliancePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseComplianceSystem(objective) {
        logger.info(`💻 [DatabaseCompliancePlannerAgent] Analyzing Database Compliance Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Compliance Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseCompliancePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseCompliancePlannerAgent = Object.freeze(new DatabaseCompliancePlannerAgent());
