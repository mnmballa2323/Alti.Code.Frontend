...
class DevOpsStreamingStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsStreamingSystem(objective) {
        logger.info(`💻 [DevOpsStreamingStrategistAgent] Analyzing DevOps Streaming Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Streaming Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsStreamingStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsStreamingStrategistAgent = Object.freeze(new DevOpsStreamingStrategistAgent());
