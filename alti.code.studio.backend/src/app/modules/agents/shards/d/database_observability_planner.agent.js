...
class DatabaseObservabilityPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseObservabilitySystem(objective) {
    logger.info(
      `💻 [DatabaseObservabilityPlannerAgent] Analyzing Database Observability Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Observability Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseObservabilityPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseObservabilityPlannerAgent = Object.freeze(
  new DatabaseObservabilityPlannerAgent(),
);
