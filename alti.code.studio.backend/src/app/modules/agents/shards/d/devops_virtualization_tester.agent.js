...
class DevOpsVirtualizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsVirtualizationSystem(objective) {
    logger.info(
      `💻 [DevOpsVirtualizationTesterAgent] Analyzing DevOps Virtualization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Virtualization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsVirtualizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsVirtualizationTesterAgent = Object.freeze(
  new DevOpsVirtualizationTesterAgent(),
);
