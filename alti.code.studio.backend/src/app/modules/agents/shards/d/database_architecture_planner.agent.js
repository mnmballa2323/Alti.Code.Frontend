...
class DatabaseArchitecturePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseArchitectureSystem(objective) {
    logger.info(
      `💻 [DatabaseArchitecturePlannerAgent] Analyzing Database Architecture Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Architecture Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseArchitecturePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseArchitecturePlannerAgent = Object.freeze(
  new DatabaseArchitecturePlannerAgent(),
);
