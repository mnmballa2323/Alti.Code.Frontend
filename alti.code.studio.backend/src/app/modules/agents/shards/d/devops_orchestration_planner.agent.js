...
class DevOpsOrchestrationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsOrchestrationSystem(objective) {
    logger.info(
      `💻 [DevOpsOrchestrationPlannerAgent] Analyzing DevOps Orchestration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Orchestration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsOrchestrationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsOrchestrationPlannerAgent = Object.freeze(
  new DevOpsOrchestrationPlannerAgent(),
);
