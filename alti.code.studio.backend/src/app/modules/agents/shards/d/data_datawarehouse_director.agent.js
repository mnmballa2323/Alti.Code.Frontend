...
class DataDataWarehouseDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DataDataWarehouseDirectorAgent] Analyzing Data DataWarehouse Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data DataWarehouse Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataDataWarehouseDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataDataWarehouseDirectorAgent = Object.freeze(
  new DataDataWarehouseDirectorAgent(),
);
