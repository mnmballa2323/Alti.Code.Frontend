...
class DataDeploymentOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataDeploymentSystem(objective) {
    logger.info(
      `💻 [DataDeploymentOrchestratorAgent] Analyzing Data Deployment Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Deployment Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataDeploymentOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataDeploymentOrchestratorAgent = Object.freeze(
  new DataDeploymentOrchestratorAgent(),
);
