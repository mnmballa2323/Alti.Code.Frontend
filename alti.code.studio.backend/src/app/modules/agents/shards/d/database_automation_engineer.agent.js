...
class DatabaseAutomationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseAutomationSystem(objective) {
    logger.info(
      `💻 [DatabaseAutomationEngineerAgent] Analyzing Database Automation Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Automation Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseAutomationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseAutomationEngineerAgent = Object.freeze(
  new DatabaseAutomationEngineerAgent(),
);
