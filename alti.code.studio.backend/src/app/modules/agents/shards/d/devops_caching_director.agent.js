...
class DevOpsCachingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsCachingSystem(objective) {
    logger.info(
      `💻 [DevOpsCachingDirectorAgent] Analyzing DevOps Caching Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Caching Director Target',
      );
...
export const devOpsCachingDirectorAgent = Object.freeze(
  new DevOpsCachingDirectorAgent(),
);
