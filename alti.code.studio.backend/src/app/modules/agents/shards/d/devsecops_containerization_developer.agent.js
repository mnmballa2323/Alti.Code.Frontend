...
class DevSecOpsContainerizationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsContainerizationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsContainerizationDeveloperAgent] Analyzing DevSecOps Containerization Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Containerization Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsContainerizationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsContainerizationDeveloperAgent = Object.freeze(
  new DevSecOpsContainerizationDeveloperAgent(),
);
