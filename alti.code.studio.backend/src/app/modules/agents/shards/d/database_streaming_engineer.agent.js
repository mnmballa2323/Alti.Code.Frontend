...
class DatabaseStreamingEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseStreamingSystem(objective) {
        logger.info(`💻 [DatabaseStreamingEngineerAgent] Analyzing Database Streaming Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Streaming Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseStreamingEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseStreamingEngineerAgent = Object.freeze(new DatabaseStreamingEngineerAgent());
