...
class DevSecOpsMigrationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsMigrationSystem(objective) {
        logger.info(`💻 [DevSecOpsMigrationConsultantAgent] Analyzing DevSecOps Migration Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Migration Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsMigrationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsMigrationConsultantAgent = Object.freeze(new DevSecOpsMigrationConsultantAgent());
