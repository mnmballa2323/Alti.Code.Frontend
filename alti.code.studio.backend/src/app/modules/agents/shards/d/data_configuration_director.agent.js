...
class DataConfigurationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDataConfigurationSystem(objective) {
        logger.info(`💻 [DataConfigurationDirectorAgent] Analyzing Data Configuration Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data Configuration Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataConfigurationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataConfigurationDirectorAgent = Object.freeze(new DataConfigurationDirectorAgent());
