...
class DevOpsLoadBalancingEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsLoadBalancingSystem(objective) {
        logger.info(`💻 [DevOpsLoadBalancingEngineerAgent] Analyzing DevOps LoadBalancing Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps LoadBalancing Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsLoadBalancingEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsLoadBalancingEngineerAgent = Object.freeze(new DevOpsLoadBalancingEngineerAgent());
