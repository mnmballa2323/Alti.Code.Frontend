...
class DatabaseConfigurationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseConfigurationSystem(objective) {
        logger.info(`💻 [DatabaseConfigurationAuditorAgent] Analyzing Database Configuration Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Configuration Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseConfigurationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseConfigurationAuditorAgent = Object.freeze(new DatabaseConfigurationAuditorAgent());
