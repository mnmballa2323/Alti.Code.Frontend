...
class DatabaseAutomationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseAutomationSystem(objective) {
    logger.info(
      `💻 [DatabaseAutomationOrchestratorAgent] Analyzing Database Automation Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Automation Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseAutomationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseAutomationOrchestratorAgent = Object.freeze(
  new DatabaseAutomationOrchestratorAgent(),
);
