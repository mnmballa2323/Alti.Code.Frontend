...
class DevSecOpsGovernanceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsGovernanceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsGovernanceDesignerAgent] Analyzing DevSecOps Governance Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Governance Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsGovernanceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsGovernanceDesignerAgent = Object.freeze(
  new DevSecOpsGovernanceDesignerAgent(),
);
