...
class DevSecOpsProvisioningTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsProvisioningSystem(objective) {
        logger.info(`💻 [DevSecOpsProvisioningTesterAgent] Analyzing DevSecOps Provisioning Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Provisioning Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsProvisioningTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsProvisioningTesterAgent = Object.freeze(new DevSecOpsProvisioningTesterAgent());
