...
class DevOpsComplianceOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsComplianceSystem(objective) {
        logger.info(`💻 [DevOpsComplianceOrchestratorAgent] Analyzing DevOps Compliance Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Compliance Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsComplianceOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsComplianceOrchestratorAgent = Object.freeze(new DevOpsComplianceOrchestratorAgent());
