...
class DevSecOpsConfigurationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsConfigurationSystem(objective) {
        logger.info(`💻 [DevSecOpsConfigurationManagerAgent] Analyzing DevSecOps Configuration Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Configuration Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsConfigurationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsConfigurationManagerAgent = Object.freeze(new DevSecOpsConfigurationManagerAgent());
