...
class DevOpsResilienceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsResilienceSystem(objective) {
    logger.info(
      `💻 [DevOpsResilienceSpecialistAgent] Analyzing DevOps Resilience Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Resilience Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsResilienceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsResilienceSpecialistAgent = Object.freeze(
  new DevOpsResilienceSpecialistAgent(),
);
