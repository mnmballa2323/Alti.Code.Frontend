...
class DatabaseDeploymentDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseDeploymentSystem(objective) {
    logger.info(
      `💻 [DatabaseDeploymentDirectorAgent] Analyzing Database Deployment Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Deployment Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseDeploymentDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseDeploymentDirectorAgent = Object.freeze(
  new DatabaseDeploymentDirectorAgent(),
);
