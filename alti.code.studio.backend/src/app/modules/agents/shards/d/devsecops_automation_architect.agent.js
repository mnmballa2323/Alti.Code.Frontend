...
class DevSecOpsAutomationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsAutomationSystem(objective) {
        logger.info(`💻 [DevSecOpsAutomationArchitectAgent] Analyzing DevSecOps Automation Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Automation Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsAutomationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsAutomationArchitectAgent = Object.freeze(new DevSecOpsAutomationArchitectAgent());
