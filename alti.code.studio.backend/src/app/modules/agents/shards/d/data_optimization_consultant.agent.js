...
class DataOptimizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDataOptimizationSystem(objective) {
    logger.info(
      `💻 [DataOptimizationConsultantAgent] Analyzing Data Optimization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Optimization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataOptimizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataOptimizationConsultantAgent = Object.freeze(
  new DataOptimizationConsultantAgent(),
);
