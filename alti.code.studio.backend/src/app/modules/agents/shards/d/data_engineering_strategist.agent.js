...
class DataEngineeringStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDataEngineeringSystem(objective) {
    logger.info(
      `💻 [DataEngineeringStrategistAgent] Analyzing Data Engineering Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Engineering Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataEngineeringStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataEngineeringStrategistAgent = Object.freeze(
  new DataEngineeringStrategistAgent(),
);
