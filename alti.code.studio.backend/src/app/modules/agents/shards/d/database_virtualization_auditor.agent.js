...
class DatabaseVirtualizationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseVirtualizationSystem(objective) {
        logger.info(`💻 [DatabaseVirtualizationAuditorAgent] Analyzing Database Virtualization Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Virtualization Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseVirtualizationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseVirtualizationAuditorAgent = Object.freeze(new DatabaseVirtualizationAuditorAgent());
