...
class DependencyAgent {
  ...
  async audit() {
    ...
    return {
      status:
        outdatedResult.status === 'OK' && cveResult.status === 'OK'
          ? 'OK'
          : 'ACTION_REQUIRED',
  ...
  async auditOutdated() {
    ...
    try {
      ...
      return {
        status: 'ERROR',
        message: 'Could not run npm outdated.',
        error: error.message,
      };
    }
  ...
  async auditSecurity() {
    logger.info(
      '🔐 Dependency Radar: Scanning for CVE vulnerabilities (npm audit)...',
    );
    ...
    for (const [pkg, data] of Object.entries(vulns)) {
      ...
      if (data.fixAvailable && (sev === 'critical' || sev === 'high')) {
        fixPlan.push(
          `npm audit fix${data.fixAvailable?.isSemVerMajor ? ' --force' : ''} # ${pkg}`,
        );
      }
    }
    ...
    logger.warn(
      `🔐 CVE Scan: ${totalVulns} vulnerabilities found (${buckets.critical.length} critical, ${buckets.high.length} high)`,
    );
  ...
  async process(state) {
    try {
      ...
      return {
        ...
        results: [
          ...(state.results ?? []),
          `❌ FAILED DependencyAgent: ${err.message}`,
        ],
      };
    }
  }
...
export const dependencyAgent = Object.freeze(new DependencyAgent());
