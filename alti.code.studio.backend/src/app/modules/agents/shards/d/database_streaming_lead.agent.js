...
class DatabaseStreamingLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseStreamingSystem(objective) {
    logger.info(
      `💻 [DatabaseStreamingLeadAgent] Analyzing Database Streaming Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Streaming Lead Target',
      );
...
export const databaseStreamingLeadAgent = Object.freeze(
  new DatabaseStreamingLeadAgent(),
);
