...
class DatabaseIntegrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseIntegrationSystem(objective) {
    logger.info(
      `💻 [DatabaseIntegrationConsultantAgent] Analyzing Database Integration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Integration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseIntegrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseIntegrationConsultantAgent = Object.freeze(
  new DatabaseIntegrationConsultantAgent(),
);
