...
class DatabaseDeploymentEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseDeploymentSystem(objective) {
        logger.info(`💻 [DatabaseDeploymentEngineerAgent] Analyzing Database Deployment Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Deployment Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseDeploymentEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseDeploymentEngineerAgent = Object.freeze(new DatabaseDeploymentEngineerAgent());
