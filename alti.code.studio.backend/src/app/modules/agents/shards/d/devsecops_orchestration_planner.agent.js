...
class DevSecOpsOrchestrationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsOrchestrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsOrchestrationPlannerAgent] Analyzing DevSecOps Orchestration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Orchestration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsOrchestrationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsOrchestrationPlannerAgent = Object.freeze(
  new DevSecOpsOrchestrationPlannerAgent(),
);
