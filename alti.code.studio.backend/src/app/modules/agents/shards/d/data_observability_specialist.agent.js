...
class DataObservabilitySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDataObservabilitySystem(objective) {
    logger.info(
      `💻 [DataObservabilitySpecialistAgent] Analyzing Data Observability Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Observability Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataObservabilitySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataObservabilitySpecialistAgent = Object.freeze(
  new DataObservabilitySpecialistAgent(),
);
