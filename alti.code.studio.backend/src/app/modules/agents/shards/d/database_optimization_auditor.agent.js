...
class DatabaseOptimizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseOptimizationSystem(objective) {
    logger.info(
      `💻 [DatabaseOptimizationAuditorAgent] Analyzing Database Optimization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Optimization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseOptimizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseOptimizationAuditorAgent = Object.freeze(
  new DatabaseOptimizationAuditorAgent(),
);
