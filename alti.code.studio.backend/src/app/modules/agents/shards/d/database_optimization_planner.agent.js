...
class DatabaseOptimizationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseOptimizationSystem(objective) {
        logger.info(`💻 [DatabaseOptimizationPlannerAgent] Analyzing Database Optimization Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Optimization Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseOptimizationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseOptimizationPlannerAgent = Object.freeze(new DatabaseOptimizationPlannerAgent());
