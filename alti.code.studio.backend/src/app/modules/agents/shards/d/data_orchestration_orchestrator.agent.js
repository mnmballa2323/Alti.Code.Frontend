...
class DataOrchestrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataOrchestrationSystem(objective) {
    logger.info(
      `💻 [DataOrchestrationOrchestratorAgent] Analyzing Data Orchestration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Orchestration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataOrchestrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataOrchestrationOrchestratorAgent = Object.freeze(
  new DataOrchestrationOrchestratorAgent(),
);
