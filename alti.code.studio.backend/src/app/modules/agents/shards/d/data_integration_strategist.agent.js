...
class DataIntegrationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateDataIntegrationSystem(objective) {
        logger.info(`💻 [DataIntegrationStrategistAgent] Analyzing Data Integration Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data Integration Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataIntegrationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataIntegrationStrategistAgent = Object.freeze(new DataIntegrationStrategistAgent());
