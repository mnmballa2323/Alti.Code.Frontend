...
class DevOpsMicroservicesStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsMicroservicesSystem(objective) {
        logger.info(`💻 [DevOpsMicroservicesStrategistAgent] Analyzing DevOps Microservices Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Microservices Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsMicroservicesStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsMicroservicesStrategistAgent = Object.freeze(new DevOpsMicroservicesStrategistAgent());
