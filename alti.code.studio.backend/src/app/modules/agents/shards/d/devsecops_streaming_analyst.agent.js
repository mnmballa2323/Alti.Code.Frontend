...
class DevSecOpsStreamingAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsStreamingSystem(objective) {
        logger.info(`💻 [DevSecOpsStreamingAnalystAgent] Analyzing DevSecOps Streaming Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Streaming Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsStreamingAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsStreamingAnalystAgent = Object.freeze(new DevSecOpsStreamingAnalystAgent());
