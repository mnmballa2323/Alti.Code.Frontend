...
class DevOpsArchitectureManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsArchitectureSystem(objective) {
    logger.info(
      `💻 [DevOpsArchitectureManagerAgent] Analyzing DevOps Architecture Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Architecture Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsArchitectureManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsArchitectureManagerAgent = Object.freeze(
  new DevOpsArchitectureManagerAgent(),
);
