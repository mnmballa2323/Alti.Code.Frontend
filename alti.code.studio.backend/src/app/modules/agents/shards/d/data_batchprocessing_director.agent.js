...
class DataBatchProcessingDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDataBatchProcessingSystem(objective) {
        logger.info(`💻 [DataBatchProcessingDirectorAgent] Analyzing Data BatchProcessing Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data BatchProcessing Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataBatchProcessingDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataBatchProcessingDirectorAgent = Object.freeze(new DataBatchProcessingDirectorAgent());
