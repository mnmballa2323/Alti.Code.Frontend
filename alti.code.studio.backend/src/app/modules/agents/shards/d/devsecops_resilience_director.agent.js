...
class DevSecOpsResilienceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsResilienceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsResilienceDirectorAgent] Analyzing DevSecOps Resilience Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Resilience Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsResilienceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsResilienceDirectorAgent = Object.freeze(
  new DevSecOpsResilienceDirectorAgent(),
);
