...
class DevSecOpsContainerizationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsContainerizationSystem(objective) {
        logger.info(`💻 [DevSecOpsContainerizationAuditorAgent] Analyzing DevSecOps Containerization Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Containerization Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsContainerizationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsContainerizationAuditorAgent = Object.freeze(new DevSecOpsContainerizationAuditorAgent());
