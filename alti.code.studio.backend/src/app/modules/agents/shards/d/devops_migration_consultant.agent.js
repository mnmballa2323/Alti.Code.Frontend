...
class DevOpsMigrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsMigrationSystem(objective) {
    logger.info(
      `💻 [DevOpsMigrationConsultantAgent] Analyzing DevOps Migration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Migration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsMigrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsMigrationConsultantAgent = Object.freeze(
  new DevOpsMigrationConsultantAgent(),
);
