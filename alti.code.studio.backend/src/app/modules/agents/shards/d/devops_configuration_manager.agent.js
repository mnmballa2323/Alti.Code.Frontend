...
class DevOpsConfigurationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsConfigurationSystem(objective) {
        logger.info(`💻 [DevOpsConfigurationManagerAgent] Analyzing DevOps Configuration Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Configuration Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsConfigurationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsConfigurationManagerAgent = Object.freeze(new DevOpsConfigurationManagerAgent());
