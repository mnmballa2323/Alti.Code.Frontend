...
class DatabaseEngineeringTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseEngineeringSystem(objective) {
        logger.info(`💻 [DatabaseEngineeringTesterAgent] Analyzing Database Engineering Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Engineering Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseEngineeringTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseEngineeringTesterAgent = Object.freeze(new DatabaseEngineeringTesterAgent());
