...
class DevOpsAnalyticsOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsAnalyticsSystem(objective) {
        logger.info(`💻 [DevOpsAnalyticsOrchestratorAgent] Analyzing DevOps Analytics Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Analytics Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsAnalyticsOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsAnalyticsOrchestratorAgent = Object.freeze(new DevOpsAnalyticsOrchestratorAgent());
