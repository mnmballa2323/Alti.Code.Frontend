...
class DatabaseContainerizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseContainerizationSystem(objective) {
    logger.info(
      `💻 [DatabaseContainerizationTesterAgent] Analyzing Database Containerization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Containerization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseContainerizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseContainerizationTesterAgent = Object.freeze(
  new DatabaseContainerizationTesterAgent(),
);
