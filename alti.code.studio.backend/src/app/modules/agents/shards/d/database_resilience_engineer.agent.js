...
class DatabaseResilienceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseResilienceSystem(objective) {
    logger.info(
      `💻 [DatabaseResilienceEngineerAgent] Analyzing Database Resilience Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Resilience Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseResilienceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseResilienceEngineerAgent = Object.freeze(
  new DatabaseResilienceEngineerAgent(),
);
