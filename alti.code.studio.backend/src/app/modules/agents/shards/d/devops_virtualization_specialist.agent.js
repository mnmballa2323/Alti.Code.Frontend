...
class DevOpsVirtualizationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsVirtualizationSystem(objective) {
    logger.info(
      `💻 [DevOpsVirtualizationSpecialistAgent] Analyzing DevOps Virtualization Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Virtualization Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsVirtualizationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsVirtualizationSpecialistAgent = Object.freeze(
  new DevOpsVirtualizationSpecialistAgent(),
);
