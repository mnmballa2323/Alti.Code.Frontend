...
class DatabaseContainerizationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseContainerizationSystem(objective) {
        logger.info(`💻 [DatabaseContainerizationAnalystAgent] Analyzing Database Containerization Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Containerization Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseContainerizationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseContainerizationAnalystAgent = Object.freeze(new DatabaseContainerizationAnalystAgent());
