...
class DatabaseVirtualizationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseVirtualizationSystem(objective) {
        logger.info(`💻 [DatabaseVirtualizationDesignerAgent] Analyzing Database Virtualization Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Virtualization Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseVirtualizationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseVirtualizationDesignerAgent = Object.freeze(new DatabaseVirtualizationDesignerAgent());
