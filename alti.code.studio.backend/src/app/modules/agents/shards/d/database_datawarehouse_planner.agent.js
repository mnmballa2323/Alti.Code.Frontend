...
class DatabaseDataWarehousePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseDataWarehouseSystem(objective) {
        logger.info(`💻 [DatabaseDataWarehousePlannerAgent] Analyzing Database DataWarehouse Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database DataWarehouse Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseDataWarehousePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseDataWarehousePlannerAgent = Object.freeze(new DatabaseDataWarehousePlannerAgent());
