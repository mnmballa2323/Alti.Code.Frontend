...
class DatabaseLoadBalancingAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseLoadBalancingSystem(objective) {
    logger.info(
      `💻 [DatabaseLoadBalancingAnalystAgent] Analyzing Database LoadBalancing Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database LoadBalancing Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseLoadBalancingAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseLoadBalancingAnalystAgent = Object.freeze(
  new DatabaseLoadBalancingAnalystAgent(),
);
