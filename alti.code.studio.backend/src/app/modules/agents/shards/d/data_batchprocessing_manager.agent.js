...
class DataBatchProcessingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDataBatchProcessingSystem(objective) {
    logger.info(
      `💻 [DataBatchProcessingManagerAgent] Analyzing Data BatchProcessing Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data BatchProcessing Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataBatchProcessingManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataBatchProcessingManagerAgent = Object.freeze(
  new DataBatchProcessingManagerAgent(),
);
