...
class DatabaseDeploymentSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseDeploymentSystem(objective) {
    logger.info(
      `💻 [DatabaseDeploymentSpecialistAgent] Analyzing Database Deployment Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Deployment Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseDeploymentSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseDeploymentSpecialistAgent = Object.freeze(
  new DatabaseDeploymentSpecialistAgent(),
);
