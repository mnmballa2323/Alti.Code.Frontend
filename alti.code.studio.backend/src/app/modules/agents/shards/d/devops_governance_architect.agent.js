...
class DevOpsGovernanceArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsGovernanceSystem(objective) {
        logger.info(`💻 [DevOpsGovernanceArchitectAgent] Analyzing DevOps Governance Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Governance Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsGovernanceArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsGovernanceArchitectAgent = Object.freeze(new DevOpsGovernanceArchitectAgent());
