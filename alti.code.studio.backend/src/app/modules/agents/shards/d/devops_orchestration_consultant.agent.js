...
class DevOpsOrchestrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsOrchestrationSystem(objective) {
    logger.info(
      `💻 [DevOpsOrchestrationConsultantAgent] Analyzing DevOps Orchestration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Orchestration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsOrchestrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsOrchestrationConsultantAgent = Object.freeze(
  new DevOpsOrchestrationConsultantAgent(),
);
