...
class DataOrchestrationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDataOrchestrationSystem(objective) {
        logger.info(`💻 [DataOrchestrationArchitectAgent] Analyzing Data Orchestration Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data Orchestration Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataOrchestrationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataOrchestrationArchitectAgent = Object.freeze(new DataOrchestrationArchitectAgent());
