...
class DevSecOpsIntegrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsIntegrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsIntegrationArchitectAgent] Analyzing DevSecOps Integration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Integration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsIntegrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsIntegrationArchitectAgent = Object.freeze(
  new DevSecOpsIntegrationArchitectAgent(),
);
