...
class DevOpsDataWarehousePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsDataWarehouseSystem(objective) {
        logger.info(`💻 [DevOpsDataWarehousePlannerAgent] Analyzing DevOps DataWarehouse Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps DataWarehouse Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsDataWarehousePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsDataWarehousePlannerAgent = Object.freeze(new DevOpsDataWarehousePlannerAgent());
