...
class DatabaseScalingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseScalingSystem(objective) {
        logger.info(`💻 [DatabaseScalingConsultantAgent] Analyzing Database Scaling Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Scaling Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseScalingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseScalingConsultantAgent = Object.freeze(new DatabaseScalingConsultantAgent());
