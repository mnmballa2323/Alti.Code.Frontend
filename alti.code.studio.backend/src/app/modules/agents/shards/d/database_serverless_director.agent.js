...
class DatabaseServerlessDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseServerlessSystem(objective) {
    logger.info(
      `💻 [DatabaseServerlessDirectorAgent] Analyzing Database Serverless Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Serverless Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseServerlessDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseServerlessDirectorAgent = Object.freeze(
  new DatabaseServerlessDirectorAgent(),
);
