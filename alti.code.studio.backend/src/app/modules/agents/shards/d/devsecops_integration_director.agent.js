...
class DevSecOpsIntegrationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsIntegrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsIntegrationDirectorAgent] Analyzing DevSecOps Integration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Integration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsIntegrationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsIntegrationDirectorAgent = Object.freeze(
  new DevSecOpsIntegrationDirectorAgent(),
);
