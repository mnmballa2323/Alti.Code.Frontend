...
class DevSecOpsConfigurationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsConfigurationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsConfigurationSpecialistAgent] Analyzing DevSecOps Configuration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Configuration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsConfigurationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsConfigurationSpecialistAgent = Object.freeze(
  new DevSecOpsConfigurationSpecialistAgent(),
);
