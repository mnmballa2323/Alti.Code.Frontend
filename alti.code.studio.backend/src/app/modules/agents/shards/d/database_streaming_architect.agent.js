...
class DatabaseStreamingArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseStreamingSystem(objective) {
    logger.info(
      `💻 [DatabaseStreamingArchitectAgent] Analyzing Database Streaming Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Streaming Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseStreamingArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseStreamingArchitectAgent = Object.freeze(
  new DatabaseStreamingArchitectAgent(),
);
