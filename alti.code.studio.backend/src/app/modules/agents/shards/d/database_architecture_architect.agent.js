...
class DatabaseArchitectureArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseArchitectureSystem(objective) {
    logger.info(
      `💻 [DatabaseArchitectureArchitectAgent] Analyzing Database Architecture Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Architecture Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseArchitectureArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseArchitectureArchitectAgent = Object.freeze(
  new DatabaseArchitectureArchitectAgent(),
);
