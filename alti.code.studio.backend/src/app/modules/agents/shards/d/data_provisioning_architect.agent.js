...
class DataProvisioningArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDataProvisioningSystem(objective) {
    logger.info(
      `💻 [DataProvisioningArchitectAgent] Analyzing Data Provisioning Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Provisioning Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataProvisioningArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataProvisioningArchitectAgent = Object.freeze(
  new DataProvisioningArchitectAgent(),
);
