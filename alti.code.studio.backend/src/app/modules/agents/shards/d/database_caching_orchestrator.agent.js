...
class DatabaseCachingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseCachingSystem(objective) {
    logger.info(
      `💻 [DatabaseCachingOrchestratorAgent] Analyzing Database Caching Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Caching Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseCachingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseCachingOrchestratorAgent = Object.freeze(
  new DatabaseCachingOrchestratorAgent(),
);
