...
class DataContainerizationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateDataContainerizationSystem(objective) {
        logger.info(`💻 [DataContainerizationAnalystAgent] Analyzing Data Containerization Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data Containerization Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataContainerizationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataContainerizationAnalystAgent = Object.freeze(new DataContainerizationAnalystAgent());
