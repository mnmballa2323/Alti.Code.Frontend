...
class DevOpsIntegrationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsIntegrationSystem(objective) {
    logger.info(
      `💻 [DevOpsIntegrationDeveloperAgent] Analyzing DevOps Integration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Integration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsIntegrationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsIntegrationDeveloperAgent = Object.freeze(
  new DevOpsIntegrationDeveloperAgent(),
);
