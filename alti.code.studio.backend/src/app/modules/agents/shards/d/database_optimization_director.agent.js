...
class DatabaseOptimizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseOptimizationSystem(objective) {
        logger.info(`💻 [DatabaseOptimizationDirectorAgent] Analyzing Database Optimization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Optimization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseOptimizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseOptimizationDirectorAgent = Object.freeze(new DatabaseOptimizationDirectorAgent());
