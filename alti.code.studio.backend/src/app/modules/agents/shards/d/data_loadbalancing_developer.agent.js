...
class DataLoadBalancingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDataLoadBalancingSystem(objective) {
    logger.info(
      `💻 [DataLoadBalancingDeveloperAgent] Analyzing Data LoadBalancing Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data LoadBalancing Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataLoadBalancingDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataLoadBalancingDeveloperAgent = Object.freeze(
  new DataLoadBalancingDeveloperAgent(),
);
