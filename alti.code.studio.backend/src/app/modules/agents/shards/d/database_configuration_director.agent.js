...
class DatabaseConfigurationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseConfigurationSystem(objective) {
    logger.info(
      `💻 [DatabaseConfigurationDirectorAgent] Analyzing Database Configuration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Configuration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseConfigurationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseConfigurationDirectorAgent = Object.freeze(
  new DatabaseConfigurationDirectorAgent(),
);
