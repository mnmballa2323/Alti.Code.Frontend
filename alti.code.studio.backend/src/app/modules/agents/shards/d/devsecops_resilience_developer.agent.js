...
class DevSecOpsResilienceDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsResilienceSystem(objective) {
        logger.info(`💻 [DevSecOpsResilienceDeveloperAgent] Analyzing DevSecOps Resilience Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Resilience Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsResilienceDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsResilienceDeveloperAgent = Object.freeze(new DevSecOpsResilienceDeveloperAgent());
