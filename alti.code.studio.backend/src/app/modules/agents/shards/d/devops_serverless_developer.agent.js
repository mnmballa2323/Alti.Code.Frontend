...
class DevOpsServerlessDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsServerlessSystem(objective) {
    logger.info(
      `💻 [DevOpsServerlessDeveloperAgent] Analyzing DevOps Serverless Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Serverless Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsServerlessDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsServerlessDeveloperAgent = Object.freeze(
  new DevOpsServerlessDeveloperAgent(),
);
