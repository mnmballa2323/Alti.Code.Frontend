...
class DevSecOpsComplianceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsComplianceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsComplianceDirectorAgent] Analyzing DevSecOps Compliance Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Compliance Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsComplianceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsComplianceDirectorAgent = Object.freeze(
  new DevSecOpsComplianceDirectorAgent(),
);
