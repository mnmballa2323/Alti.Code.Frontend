...
class DevOpsComplianceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsComplianceSystem(objective) {
        logger.info(`💻 [DevOpsComplianceConsultantAgent] Analyzing DevOps Compliance Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Compliance Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsComplianceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsComplianceConsultantAgent = Object.freeze(new DevOpsComplianceConsultantAgent());
