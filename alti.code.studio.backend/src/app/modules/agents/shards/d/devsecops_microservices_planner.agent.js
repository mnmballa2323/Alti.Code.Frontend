...
class DevSecOpsMicroservicesPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsMicroservicesSystem(objective) {
    logger.info(
      `💻 [DevSecOpsMicroservicesPlannerAgent] Analyzing DevSecOps Microservices Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Microservices Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsMicroservicesPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsMicroservicesPlannerAgent = Object.freeze(
  new DevSecOpsMicroservicesPlannerAgent(),
);
