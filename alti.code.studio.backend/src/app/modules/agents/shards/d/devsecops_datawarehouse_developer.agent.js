...
class DevSecOpsDataWarehouseDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DevSecOpsDataWarehouseDeveloperAgent] Analyzing DevSecOps DataWarehouse Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps DataWarehouse Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsDataWarehouseDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsDataWarehouseDeveloperAgent = Object.freeze(
  new DevSecOpsDataWarehouseDeveloperAgent(),
);
