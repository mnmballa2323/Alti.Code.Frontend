...
class DatabaseLoadBalancingLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseLoadBalancingSystem(objective) {
    logger.info(
      `💻 [DatabaseLoadBalancingLeadAgent] Analyzing Database LoadBalancing Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database LoadBalancing Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseLoadBalancingLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseLoadBalancingLeadAgent = Object.freeze(
  new DatabaseLoadBalancingLeadAgent(),
);
