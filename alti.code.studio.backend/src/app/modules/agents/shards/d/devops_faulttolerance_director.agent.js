...
class DevOpsFaultToleranceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsFaultToleranceSystem(objective) {
    logger.info(
      `💻 [DevOpsFaultToleranceDirectorAgent] Analyzing DevOps FaultTolerance Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps FaultTolerance Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsFaultToleranceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsFaultToleranceDirectorAgent = Object.freeze(
  new DevOpsFaultToleranceDirectorAgent(),
);
