...
class DatabaseVirtualizationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseVirtualizationSystem(objective) {
    logger.info(
      `💻 [DatabaseVirtualizationManagerAgent] Analyzing Database Virtualization Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Virtualization Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseVirtualizationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseVirtualizationManagerAgent = Object.freeze(
  new DatabaseVirtualizationManagerAgent(),
);
