...
class DataFaultTolerancePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDataFaultToleranceSystem(objective) {
        logger.info(`💻 [DataFaultTolerancePlannerAgent] Analyzing Data FaultTolerance Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data FaultTolerance Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataFaultTolerancePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataFaultTolerancePlannerAgent = Object.freeze(new DataFaultTolerancePlannerAgent());
