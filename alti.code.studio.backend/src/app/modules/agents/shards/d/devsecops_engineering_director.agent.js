...
class DevSecOpsEngineeringDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsEngineeringSystem(objective) {
    logger.info(
      `💻 [DevSecOpsEngineeringDirectorAgent] Analyzing DevSecOps Engineering Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Engineering Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsEngineeringDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsEngineeringDirectorAgent = Object.freeze(
  new DevSecOpsEngineeringDirectorAgent(),
);
