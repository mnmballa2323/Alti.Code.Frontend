...
class DevSecOpsVirtualizationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsVirtualizationSystem(objective) {
        logger.info(`💻 [DevSecOpsVirtualizationAnalystAgent] Analyzing DevSecOps Virtualization Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Virtualization Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsVirtualizationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsVirtualizationAnalystAgent = Object.freeze(new DevSecOpsVirtualizationAnalystAgent());
