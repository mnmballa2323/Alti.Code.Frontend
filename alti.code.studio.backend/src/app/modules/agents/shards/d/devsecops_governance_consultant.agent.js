...
class DevSecOpsGovernanceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsGovernanceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsGovernanceConsultantAgent] Analyzing DevSecOps Governance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Governance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsGovernanceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsGovernanceConsultantAgent = Object.freeze(
  new DevSecOpsGovernanceConsultantAgent(),
);
