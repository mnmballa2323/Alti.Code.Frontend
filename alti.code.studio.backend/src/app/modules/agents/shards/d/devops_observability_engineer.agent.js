...
class DevOpsObservabilityEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsObservabilitySystem(objective) {
    logger.info(
      `💻 [DevOpsObservabilityEngineerAgent] Analyzing DevOps Observability Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Observability Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsObservabilityEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsObservabilityEngineerAgent = Object.freeze(
  new DevOpsObservabilityEngineerAgent(),
);
