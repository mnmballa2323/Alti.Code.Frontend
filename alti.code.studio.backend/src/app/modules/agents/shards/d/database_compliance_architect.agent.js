...
class DatabaseComplianceArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseComplianceSystem(objective) {
    logger.info(
      `💻 [DatabaseComplianceArchitectAgent] Analyzing Database Compliance Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Compliance Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseComplianceArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseComplianceArchitectAgent = Object.freeze(
  new DatabaseComplianceArchitectAgent(),
);
