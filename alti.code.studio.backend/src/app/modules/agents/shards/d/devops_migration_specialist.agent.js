...
class DevOpsMigrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsMigrationSystem(objective) {
    logger.info(
      `💻 [DevOpsMigrationSpecialistAgent] Analyzing DevOps Migration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Migration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsMigrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsMigrationSpecialistAgent = Object.freeze(
  new DevOpsMigrationSpecialistAgent(),
);
