...
class DatabaseServerlessEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseServerlessSystem(objective) {
    logger.info(
      `💻 [DatabaseServerlessEngineerAgent] Analyzing Database Serverless Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Serverless Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseServerlessEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseServerlessEngineerAgent = Object.freeze(
  new DatabaseServerlessEngineerAgent(),
);
