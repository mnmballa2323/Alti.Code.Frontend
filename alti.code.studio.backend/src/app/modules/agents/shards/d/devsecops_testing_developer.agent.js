...
class DevSecOpsTestingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsTestingSystem(objective) {
    logger.info(
      `💻 [DevSecOpsTestingDeveloperAgent] Analyzing DevSecOps Testing Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Testing Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsTestingDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsTestingDeveloperAgent = Object.freeze(
  new DevSecOpsTestingDeveloperAgent(),
);
