...
class DevOpsMicroservicesSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsMicroservicesSystem(objective) {
    logger.info(
      `💻 [DevOpsMicroservicesSpecialistAgent] Analyzing DevOps Microservices Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Microservices Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsMicroservicesSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsMicroservicesSpecialistAgent = Object.freeze(
  new DevOpsMicroservicesSpecialistAgent(),
);
