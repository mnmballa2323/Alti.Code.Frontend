...
class DevSecOpsDataWarehouseTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DevSecOpsDataWarehouseTesterAgent] Analyzing DevSecOps DataWarehouse Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps DataWarehouse Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsDataWarehouseTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsDataWarehouseTesterAgent = Object.freeze(
  new DevSecOpsDataWarehouseTesterAgent(),
);
