...
class DevSecOpsProvisioningDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsProvisioningSystem(objective) {
    logger.info(
      `💻 [DevSecOpsProvisioningDeveloperAgent] Analyzing DevSecOps Provisioning Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Provisioning Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsProvisioningDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsProvisioningDeveloperAgent = Object.freeze(
  new DevSecOpsProvisioningDeveloperAgent(),
);
