...
class DevSecOpsAnalyticsSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsAnalyticsSystem(objective) {
    logger.info(
      `💻 [DevSecOpsAnalyticsSpecialistAgent] Analyzing DevSecOps Analytics Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Analytics Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsAnalyticsSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsAnalyticsSpecialistAgent = Object.freeze(
  new DevSecOpsAnalyticsSpecialistAgent(),
);
