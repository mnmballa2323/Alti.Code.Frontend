...
class DevOpsContainerizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsContainerizationSystem(objective) {
    logger.info(
      `💻 [DevOpsContainerizationConsultantAgent] Analyzing DevOps Containerization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Containerization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsContainerizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsContainerizationConsultantAgent = Object.freeze(
  new DevOpsContainerizationConsultantAgent(),
);
