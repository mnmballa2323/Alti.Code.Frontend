...
class DevSecOpsTestingArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsTestingSystem(objective) {
    logger.info(
      `💻 [DevSecOpsTestingArchitectAgent] Analyzing DevSecOps Testing Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Testing Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsTestingArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsTestingArchitectAgent = Object.freeze(
  new DevSecOpsTestingArchitectAgent(),
);
