...
class DevOpsMicroservicesTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsMicroservicesSystem(objective) {
        logger.info(`💻 [DevOpsMicroservicesTesterAgent] Analyzing DevOps Microservices Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Microservices Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsMicroservicesTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsMicroservicesTesterAgent = Object.freeze(new DevOpsMicroservicesTesterAgent());
