...
class DevSecOpsOrchestrationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsOrchestrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsOrchestrationAuditorAgent] Analyzing DevSecOps Orchestration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Orchestration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsOrchestrationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsOrchestrationAuditorAgent = Object.freeze(
  new DevSecOpsOrchestrationAuditorAgent(),
);
