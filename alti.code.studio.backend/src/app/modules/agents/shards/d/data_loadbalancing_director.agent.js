...
class DataLoadBalancingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataLoadBalancingSystem(objective) {
    logger.info(
      `💻 [DataLoadBalancingDirectorAgent] Analyzing Data LoadBalancing Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data LoadBalancing Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataLoadBalancingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataLoadBalancingDirectorAgent = Object.freeze(
  new DataLoadBalancingDirectorAgent(),
);
