...
class DatabaseContainerizationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseContainerizationSystem(objective) {
    logger.info(
      `💻 [DatabaseContainerizationSpecialistAgent] Analyzing Database Containerization Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Containerization Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseContainerizationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseContainerizationSpecialistAgent = Object.freeze(
  new DatabaseContainerizationSpecialistAgent(),
);
