...
class DevSecOpsIntegrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsIntegrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsIntegrationSpecialistAgent] Analyzing DevSecOps Integration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Integration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsIntegrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsIntegrationSpecialistAgent = Object.freeze(
  new DevSecOpsIntegrationSpecialistAgent(),
);
