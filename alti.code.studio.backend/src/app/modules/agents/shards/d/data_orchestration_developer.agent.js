...
class DataOrchestrationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDataOrchestrationSystem(objective) {
    logger.info(
      `💻 [DataOrchestrationDeveloperAgent] Analyzing Data Orchestration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Orchestration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataOrchestrationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataOrchestrationDeveloperAgent = Object.freeze(
  new DataOrchestrationDeveloperAgent(),
);
