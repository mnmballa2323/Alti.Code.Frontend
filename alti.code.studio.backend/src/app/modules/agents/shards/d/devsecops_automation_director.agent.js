...
class DevSecOpsAutomationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsAutomationSystem(objective) {
        logger.info(`💻 [DevSecOpsAutomationDirectorAgent] Analyzing DevSecOps Automation Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Automation Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsAutomationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsAutomationDirectorAgent = Object.freeze(new DevSecOpsAutomationDirectorAgent());
