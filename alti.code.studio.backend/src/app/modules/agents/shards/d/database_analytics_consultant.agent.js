...
class DatabaseAnalyticsConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseAnalyticsSystem(objective) {
    logger.info(
      `💻 [DatabaseAnalyticsConsultantAgent] Analyzing Database Analytics Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Analytics Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseAnalyticsConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseAnalyticsConsultantAgent = Object.freeze(
  new DatabaseAnalyticsConsultantAgent(),
);
