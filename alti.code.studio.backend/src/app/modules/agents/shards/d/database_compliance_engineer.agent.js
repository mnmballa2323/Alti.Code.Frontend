...
class DatabaseComplianceEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseComplianceSystem(objective) {
        logger.info(`💻 [DatabaseComplianceEngineerAgent] Analyzing Database Compliance Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Compliance Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseComplianceEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseComplianceEngineerAgent = Object.freeze(new DatabaseComplianceEngineerAgent());
