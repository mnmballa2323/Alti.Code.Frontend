...
class DevSecOpsResilienceArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsResilienceSystem(objective) {
        logger.info(`💻 [DevSecOpsResilienceArchitectAgent] Analyzing DevSecOps Resilience Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Resilience Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsResilienceArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsResilienceArchitectAgent = Object.freeze(new DevSecOpsResilienceArchitectAgent());
