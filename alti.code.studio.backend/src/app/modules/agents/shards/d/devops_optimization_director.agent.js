...
class DevOpsOptimizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsOptimizationSystem(objective) {
        logger.info(`💻 [DevOpsOptimizationDirectorAgent] Analyzing DevOps Optimization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Optimization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsOptimizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsOptimizationDirectorAgent = Object.freeze(new DevOpsOptimizationDirectorAgent());
