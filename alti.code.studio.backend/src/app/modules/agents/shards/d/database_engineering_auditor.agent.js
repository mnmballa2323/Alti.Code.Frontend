...
class DatabaseEngineeringAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseEngineeringSystem(objective) {
    logger.info(
      `💻 [DatabaseEngineeringAuditorAgent] Analyzing Database Engineering Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Engineering Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseEngineeringAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseEngineeringAuditorAgent = Object.freeze(
  new DatabaseEngineeringAuditorAgent(),
);
