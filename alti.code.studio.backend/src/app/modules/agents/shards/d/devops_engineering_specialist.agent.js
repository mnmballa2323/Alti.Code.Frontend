...
class DevOpsEngineeringSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsEngineeringSystem(objective) {
        logger.info(`💻 [DevOpsEngineeringSpecialistAgent] Analyzing DevOps Engineering Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Engineering Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsEngineeringSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsEngineeringSpecialistAgent = Object.freeze(new DevOpsEngineeringSpecialistAgent());
