...
class DevOpsDataWarehouseManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DevOpsDataWarehouseManagerAgent] Analyzing DevOps DataWarehouse Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps DataWarehouse Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsDataWarehouseManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsDataWarehouseManagerAgent = Object.freeze(
  new DevOpsDataWarehouseManagerAgent(),
);
