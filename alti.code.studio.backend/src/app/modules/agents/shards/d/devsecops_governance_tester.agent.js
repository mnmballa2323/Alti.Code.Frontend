...
class DevSecOpsGovernanceTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsGovernanceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsGovernanceTesterAgent] Analyzing DevSecOps Governance Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Governance Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsGovernanceTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsGovernanceTesterAgent = Object.freeze(
  new DevSecOpsGovernanceTesterAgent(),
);
