...
class DevSecOpsTelemetryArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsTelemetrySystem(objective) {
        logger.info(`💻 [DevSecOpsTelemetryArchitectAgent] Analyzing DevSecOps Telemetry Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Telemetry Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsTelemetryArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsTelemetryArchitectAgent = Object.freeze(new DevSecOpsTelemetryArchitectAgent());
