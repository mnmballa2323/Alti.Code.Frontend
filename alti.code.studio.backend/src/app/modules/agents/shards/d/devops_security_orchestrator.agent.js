...
class DevOpsSecurityOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsSecuritySystem(objective) {
    logger.info(
      `💻 [DevOpsSecurityOrchestratorAgent] Analyzing DevOps Security Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Security Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsSecurityOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsSecurityOrchestratorAgent = Object.freeze(
  new DevOpsSecurityOrchestratorAgent(),
);
