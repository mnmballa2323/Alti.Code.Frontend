...
class DatabaseMicroservicesDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseMicroservicesSystem(objective) {
    logger.info(
      `💻 [DatabaseMicroservicesDesignerAgent] Analyzing Database Microservices Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Microservices Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseMicroservicesDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseMicroservicesDesignerAgent = Object.freeze(
  new DatabaseMicroservicesDesignerAgent(),
);
