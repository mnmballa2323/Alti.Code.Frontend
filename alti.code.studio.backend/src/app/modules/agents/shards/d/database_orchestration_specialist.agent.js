...
class DatabaseOrchestrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseOrchestrationSystem(objective) {
    logger.info(
      `💻 [DatabaseOrchestrationSpecialistAgent] Analyzing Database Orchestration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Orchestration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseOrchestrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseOrchestrationSpecialistAgent = Object.freeze(
  new DatabaseOrchestrationSpecialistAgent(),
);
