...
class DevSecOpsMigrationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsMigrationSystem(objective) {
        logger.info(`💻 [DevSecOpsMigrationDirectorAgent] Analyzing DevSecOps Migration Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Migration Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsMigrationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsMigrationDirectorAgent = Object.freeze(new DevSecOpsMigrationDirectorAgent());
