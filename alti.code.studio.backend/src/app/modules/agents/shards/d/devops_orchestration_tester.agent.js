...
class DevOpsOrchestrationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsOrchestrationSystem(objective) {
    logger.info(
      `💻 [DevOpsOrchestrationTesterAgent] Analyzing DevOps Orchestration Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Orchestration Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsOrchestrationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsOrchestrationTesterAgent = Object.freeze(
  new DevOpsOrchestrationTesterAgent(),
);
