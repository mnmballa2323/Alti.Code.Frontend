...
class DatabaseFaultTolerancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseFaultToleranceSystem(objective) {
    logger.info(
      `💻 [DatabaseFaultTolerancePlannerAgent] Analyzing Database FaultTolerance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database FaultTolerance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseFaultTolerancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseFaultTolerancePlannerAgent = Object.freeze(
  new DatabaseFaultTolerancePlannerAgent(),
);
