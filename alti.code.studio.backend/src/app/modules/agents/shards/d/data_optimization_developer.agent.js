...
class DataOptimizationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDataOptimizationSystem(objective) {
    logger.info(
      `💻 [DataOptimizationDeveloperAgent] Analyzing Data Optimization Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Optimization Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataOptimizationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataOptimizationDeveloperAgent = Object.freeze(
  new DataOptimizationDeveloperAgent(),
);
