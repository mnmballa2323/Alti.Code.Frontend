...
class DatabaseArchitectureAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseArchitectureSystem(objective) {
    logger.info(
      `💻 [DatabaseArchitectureAuditorAgent] Analyzing Database Architecture Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Architecture Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseArchitectureAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseArchitectureAuditorAgent = Object.freeze(
  new DatabaseArchitectureAuditorAgent(),
);
