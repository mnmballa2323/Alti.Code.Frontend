...
class DatabaseEngineeringArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseEngineeringSystem(objective) {
    logger.info(
      `💻 [DatabaseEngineeringArchitectAgent] Analyzing Database Engineering Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Engineering Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseEngineeringArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseEngineeringArchitectAgent = Object.freeze(
  new DatabaseEngineeringArchitectAgent(),
);
