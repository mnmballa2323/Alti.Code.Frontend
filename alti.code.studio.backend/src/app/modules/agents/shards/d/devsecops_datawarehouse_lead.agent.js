...
class DevSecOpsDataWarehouseLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DevSecOpsDataWarehouseLeadAgent] Analyzing DevSecOps DataWarehouse Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps DataWarehouse Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsDataWarehouseLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsDataWarehouseLeadAgent = Object.freeze(
  new DevSecOpsDataWarehouseLeadAgent(),
);
