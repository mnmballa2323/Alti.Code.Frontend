...
class DataResilienceOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataResilienceSystem(objective) {
    logger.info(
      `💻 [DataResilienceOrchestratorAgent] Analyzing Data Resilience Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Resilience Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataResilienceOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataResilienceOrchestratorAgent = Object.freeze(
  new DataResilienceOrchestratorAgent(),
);
