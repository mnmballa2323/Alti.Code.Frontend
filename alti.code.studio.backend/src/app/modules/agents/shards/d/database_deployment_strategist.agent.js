...
class DatabaseDeploymentStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseDeploymentSystem(objective) {
    logger.info(
      `💻 [DatabaseDeploymentStrategistAgent] Analyzing Database Deployment Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Deployment Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseDeploymentStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseDeploymentStrategistAgent = Object.freeze(
  new DatabaseDeploymentStrategistAgent(),
);
