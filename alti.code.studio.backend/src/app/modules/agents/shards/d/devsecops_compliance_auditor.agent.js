...
class DevSecOpsComplianceAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsComplianceSystem(objective) {
        logger.info(`💻 [DevSecOpsComplianceAuditorAgent] Analyzing DevSecOps Compliance Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Compliance Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsComplianceAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsComplianceAuditorAgent = Object.freeze(new DevSecOpsComplianceAuditorAgent());
