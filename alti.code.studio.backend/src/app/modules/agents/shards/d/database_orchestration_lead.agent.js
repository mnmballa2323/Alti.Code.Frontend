...
class DatabaseOrchestrationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseOrchestrationSystem(objective) {
    logger.info(
      `💻 [DatabaseOrchestrationLeadAgent] Analyzing Database Orchestration Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Orchestration Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseOrchestrationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseOrchestrationLeadAgent = Object.freeze(
  new DatabaseOrchestrationLeadAgent(),
);
