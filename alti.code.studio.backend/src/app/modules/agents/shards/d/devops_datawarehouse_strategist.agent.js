...
class DevOpsDataWarehouseStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DevOpsDataWarehouseStrategistAgent] Analyzing DevOps DataWarehouse Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps DataWarehouse Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsDataWarehouseStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsDataWarehouseStrategistAgent = Object.freeze(
  new DevOpsDataWarehouseStrategistAgent(),
);
