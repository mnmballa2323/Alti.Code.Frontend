...
class DevSecOpsResilienceAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsResilienceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsResilienceAnalystAgent] Analyzing DevSecOps Resilience Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Resilience Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsResilienceAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsResilienceAnalystAgent = Object.freeze(
  new DevSecOpsResilienceAnalystAgent(),
);
