...
class DevSecOpsOptimizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsOptimizationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsOptimizationDirectorAgent] Analyzing DevSecOps Optimization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Optimization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsOptimizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsOptimizationDirectorAgent = Object.freeze(
  new DevSecOpsOptimizationDirectorAgent(),
);
