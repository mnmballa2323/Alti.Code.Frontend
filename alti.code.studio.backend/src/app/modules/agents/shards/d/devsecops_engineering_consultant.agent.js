...
class DevSecOpsEngineeringConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsEngineeringSystem(objective) {
        logger.info(`💻 [DevSecOpsEngineeringConsultantAgent] Analyzing DevSecOps Engineering Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Engineering Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsEngineeringConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsEngineeringConsultantAgent = Object.freeze(new DevSecOpsEngineeringConsultantAgent());
