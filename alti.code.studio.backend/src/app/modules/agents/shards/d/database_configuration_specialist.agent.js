...
class DatabaseConfigurationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseConfigurationSystem(objective) {
    logger.info(
      `💻 [DatabaseConfigurationSpecialistAgent] Analyzing Database Configuration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Configuration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseConfigurationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseConfigurationSpecialistAgent = Object.freeze(
  new DatabaseConfigurationSpecialistAgent(),
);
