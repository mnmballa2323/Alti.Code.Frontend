...
class DatabaseLoadBalancingAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseLoadBalancingSystem(objective) {
    logger.info(
      `💻 [DatabaseLoadBalancingAuditorAgent] Analyzing Database LoadBalancing Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database LoadBalancing Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseLoadBalancingAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseLoadBalancingAuditorAgent = Object.freeze(
  new DatabaseLoadBalancingAuditorAgent(),
);
