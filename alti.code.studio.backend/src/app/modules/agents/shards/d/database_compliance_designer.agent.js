...
class DatabaseComplianceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseComplianceSystem(objective) {
    logger.info(
      `💻 [DatabaseComplianceDesignerAgent] Analyzing Database Compliance Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Compliance Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseComplianceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseComplianceDesignerAgent = Object.freeze(
  new DatabaseComplianceDesignerAgent(),
);
