...
class DevSecOpsArchitecturePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsArchitectureSystem(objective) {
        logger.info(`💻 [DevSecOpsArchitecturePlannerAgent] Analyzing DevSecOps Architecture Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Architecture Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsArchitecturePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsArchitecturePlannerAgent = Object.freeze(new DevSecOpsArchitecturePlannerAgent());
