...
class DevSecOpsCompliancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsComplianceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsCompliancePlannerAgent] Analyzing DevSecOps Compliance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Compliance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsCompliancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsCompliancePlannerAgent = Object.freeze(
  new DevSecOpsCompliancePlannerAgent(),
);
