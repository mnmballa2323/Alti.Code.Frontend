...
class DevSecOpsSecurityEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsSecuritySystem(objective) {
    logger.info(
      `💻 [DevSecOpsSecurityEngineerAgent] Analyzing DevSecOps Security Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Security Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsSecurityEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsSecurityEngineerAgent = Object.freeze(
  new DevSecOpsSecurityEngineerAgent(),
);
