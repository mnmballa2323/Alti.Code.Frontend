...
class DevSecOpsDataWarehousePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DevSecOpsDataWarehousePlannerAgent] Analyzing DevSecOps DataWarehouse Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps DataWarehouse Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsDataWarehousePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsDataWarehousePlannerAgent = Object.freeze(
  new DevSecOpsDataWarehousePlannerAgent(),
);
