...
class DevSecOpsContainerizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsContainerizationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsContainerizationAnalystAgent] Analyzing DevSecOps Containerization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Containerization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsContainerizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsContainerizationAnalystAgent = Object.freeze(
  new DevSecOpsContainerizationAnalystAgent(),
);
