...
class DatabaseTestingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseTestingSystem(objective) {
    logger.info(
      `💻 [DatabaseTestingSpecialistAgent] Analyzing Database Testing Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Testing Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseTestingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseTestingSpecialistAgent = Object.freeze(
  new DatabaseTestingSpecialistAgent(),
);
