...
class DataVirtualizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDataVirtualizationSystem(objective) {
    logger.info(
      `💻 [DataVirtualizationPlannerAgent] Analyzing Data Virtualization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Virtualization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataVirtualizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataVirtualizationPlannerAgent = Object.freeze(
  new DataVirtualizationPlannerAgent(),
);
