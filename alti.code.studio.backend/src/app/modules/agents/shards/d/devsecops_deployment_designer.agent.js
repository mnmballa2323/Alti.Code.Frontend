...
class DevSecOpsDeploymentDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsDeploymentSystem(objective) {
        logger.info(`💻 [DevSecOpsDeploymentDesignerAgent] Analyzing DevSecOps Deployment Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Deployment Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsDeploymentDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsDeploymentDesignerAgent = Object.freeze(new DevSecOpsDeploymentDesignerAgent());
