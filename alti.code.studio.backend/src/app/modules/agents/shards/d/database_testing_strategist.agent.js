...
class DatabaseTestingStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseTestingSystem(objective) {
        logger.info(`💻 [DatabaseTestingStrategistAgent] Analyzing Database Testing Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Testing Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseTestingStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseTestingStrategistAgent = Object.freeze(new DatabaseTestingStrategistAgent());
