...
class DevSecOpsMicroservicesOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsMicroservicesSystem(objective) {
        logger.info(`💻 [DevSecOpsMicroservicesOrchestratorAgent] Analyzing DevSecOps Microservices Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Microservices Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsMicroservicesOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsMicroservicesOrchestratorAgent = Object.freeze(new DevSecOpsMicroservicesOrchestratorAgent());
