...
class DataContainerizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDataContainerizationSystem(objective) {
    logger.info(
      `💻 [DataContainerizationConsultantAgent] Analyzing Data Containerization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Containerization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataContainerizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataContainerizationConsultantAgent = Object.freeze(
  new DataContainerizationConsultantAgent(),
);
