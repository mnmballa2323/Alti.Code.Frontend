...
class DatabaseProvisioningArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseProvisioningSystem(objective) {
    logger.info(
      `💻 [DatabaseProvisioningArchitectAgent] Analyzing Database Provisioning Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Provisioning Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseProvisioningArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseProvisioningArchitectAgent = Object.freeze(
  new DatabaseProvisioningArchitectAgent(),
);
