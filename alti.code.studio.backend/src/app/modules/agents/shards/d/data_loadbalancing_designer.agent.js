...
class DataLoadBalancingDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDataLoadBalancingSystem(objective) {
        logger.info(`💻 [DataLoadBalancingDesignerAgent] Analyzing Data LoadBalancing Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data LoadBalancing Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataLoadBalancingDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataLoadBalancingDesignerAgent = Object.freeze(new DataLoadBalancingDesignerAgent());
