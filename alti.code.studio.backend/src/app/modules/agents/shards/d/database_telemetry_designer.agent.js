...
class DatabaseTelemetryDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseTelemetrySystem(objective) {
        logger.info(`💻 [DatabaseTelemetryDesignerAgent] Analyzing Database Telemetry Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Telemetry Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseTelemetryDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseTelemetryDesignerAgent = Object.freeze(new DatabaseTelemetryDesignerAgent());
