...
class DevSecOpsResilienceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsResilienceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsResilienceDesignerAgent] Analyzing DevSecOps Resilience Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Resilience Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsResilienceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsResilienceDesignerAgent = Object.freeze(
  new DevSecOpsResilienceDesignerAgent(),
);
