...
class DatabaseEngineeringPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseEngineeringSystem(objective) {
        logger.info(`💻 [DatabaseEngineeringPlannerAgent] Analyzing Database Engineering Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Engineering Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseEngineeringPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseEngineeringPlannerAgent = Object.freeze(new DatabaseEngineeringPlannerAgent());
