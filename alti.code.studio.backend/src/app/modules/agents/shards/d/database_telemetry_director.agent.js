...
class DatabaseTelemetryDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseTelemetrySystem(objective) {
    logger.info(
      `💻 [DatabaseTelemetryDirectorAgent] Analyzing Database Telemetry Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Telemetry Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseTelemetryDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseTelemetryDirectorAgent = Object.freeze(
  new DatabaseTelemetryDirectorAgent(),
);
