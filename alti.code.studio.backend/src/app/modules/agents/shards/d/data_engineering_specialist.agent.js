...
class DataEngineeringSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDataEngineeringSystem(objective) {
    logger.info(
      `💻 [DataEngineeringSpecialistAgent] Analyzing Data Engineering Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Engineering Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataEngineeringSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataEngineeringSpecialistAgent = Object.freeze(
  new DataEngineeringSpecialistAgent(),
);
