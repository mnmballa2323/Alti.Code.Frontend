...
class DatabaseStreamingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseStreamingSystem(objective) {
    logger.info(
      `💻 [DatabaseStreamingManagerAgent] Analyzing Database Streaming Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Streaming Manager Target',
      );
...
export const databaseStreamingManagerAgent = Object.freeze(
  new DatabaseStreamingManagerAgent(),
);
