...
class DevOpsStreamingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsStreamingSystem(objective) {
    logger.info(
      `💻 [DevOpsStreamingOrchestratorAgent] Analyzing DevOps Streaming Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Streaming Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsStreamingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsStreamingOrchestratorAgent = Object.freeze(
  new DevOpsStreamingOrchestratorAgent(),
);
