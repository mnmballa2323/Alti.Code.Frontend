...
class DevOpsDataWarehouseDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsDataWarehouseSystem(objective) {
        logger.info(`💻 [DevOpsDataWarehouseDirectorAgent] Analyzing DevOps DataWarehouse Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps DataWarehouse Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsDataWarehouseDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsDataWarehouseDirectorAgent = Object.freeze(new DevOpsDataWarehouseDirectorAgent());
