...
class DevSecOpsObservabilityDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsObservabilitySystem(objective) {
    logger.info(
      `💻 [DevSecOpsObservabilityDeveloperAgent] Analyzing DevSecOps Observability Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Observability Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsObservabilityDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsObservabilityDeveloperAgent = Object.freeze(
  new DevSecOpsObservabilityDeveloperAgent(),
);
