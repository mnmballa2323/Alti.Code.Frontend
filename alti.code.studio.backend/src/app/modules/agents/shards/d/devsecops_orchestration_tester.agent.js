...
class DevSecOpsOrchestrationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsOrchestrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsOrchestrationTesterAgent] Analyzing DevSecOps Orchestration Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Orchestration Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsOrchestrationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsOrchestrationTesterAgent = Object.freeze(
  new DevSecOpsOrchestrationTesterAgent(),
);
