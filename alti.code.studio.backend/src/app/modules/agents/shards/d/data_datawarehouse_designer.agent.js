...
class DataDataWarehouseDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDataDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DataDataWarehouseDesignerAgent] Analyzing Data DataWarehouse Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data DataWarehouse Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataDataWarehouseDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataDataWarehouseDesignerAgent = Object.freeze(
  new DataDataWarehouseDesignerAgent(),
);
