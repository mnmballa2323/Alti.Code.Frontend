...
class DatabaseEngineeringAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseEngineeringSystem(objective) {
    logger.info(
      `💻 [DatabaseEngineeringAnalystAgent] Analyzing Database Engineering Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Engineering Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseEngineeringAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseEngineeringAnalystAgent = Object.freeze(
  new DatabaseEngineeringAnalystAgent(),
);
