...
class DataLoadBalancingSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDataLoadBalancingSystem(objective) {
        logger.info(`💻 [DataLoadBalancingSpecialistAgent] Analyzing Data LoadBalancing Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data LoadBalancing Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataLoadBalancingSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataLoadBalancingSpecialistAgent = Object.freeze(new DataLoadBalancingSpecialistAgent());
