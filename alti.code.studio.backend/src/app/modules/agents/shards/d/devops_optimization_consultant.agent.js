...
class DevOpsOptimizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsOptimizationSystem(objective) {
    logger.info(
      `💻 [DevOpsOptimizationConsultantAgent] Analyzing DevOps Optimization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Optimization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsOptimizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsOptimizationConsultantAgent = Object.freeze(
  new DevOpsOptimizationConsultantAgent(),
);
