...
class DevSecOpsMicroservicesSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsMicroservicesSystem(objective) {
    logger.info(
      `💻 [DevSecOpsMicroservicesSpecialistAgent] Analyzing DevSecOps Microservices Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Microservices Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsMicroservicesSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsMicroservicesSpecialistAgent = Object.freeze(
  new DevSecOpsMicroservicesSpecialistAgent(),
);
