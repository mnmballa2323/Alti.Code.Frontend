...
class DevOpsDataWarehouseSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsDataWarehouseSystem(objective) {
        logger.info(`💻 [DevOpsDataWarehouseSpecialistAgent] Analyzing DevOps DataWarehouse Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps DataWarehouse Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsDataWarehouseSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsDataWarehouseSpecialistAgent = Object.freeze(new DevOpsDataWarehouseSpecialistAgent());
