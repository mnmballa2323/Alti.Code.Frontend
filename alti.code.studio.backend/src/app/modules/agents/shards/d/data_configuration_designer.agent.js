...
class DataConfigurationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDataConfigurationSystem(objective) {
    logger.info(
      `💻 [DataConfigurationDesignerAgent] Analyzing Data Configuration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Configuration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataConfigurationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataConfigurationDesignerAgent = Object.freeze(
  new DataConfigurationDesignerAgent(),
);
