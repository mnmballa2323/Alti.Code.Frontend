...
class DevSecOpsArchitectureAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsArchitectureSystem(objective) {
    logger.info(
      `💻 [DevSecOpsArchitectureAnalystAgent] Analyzing DevSecOps Architecture Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Architecture Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsArchitectureAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsArchitectureAnalystAgent = Object.freeze(
  new DevSecOpsArchitectureAnalystAgent(),
);
