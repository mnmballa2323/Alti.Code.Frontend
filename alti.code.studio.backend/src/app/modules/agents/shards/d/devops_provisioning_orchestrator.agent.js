...
class DevOpsProvisioningOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsProvisioningSystem(objective) {
        logger.info(`💻 [DevOpsProvisioningOrchestratorAgent] Analyzing DevOps Provisioning Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Provisioning Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsProvisioningOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsProvisioningOrchestratorAgent = Object.freeze(new DevOpsProvisioningOrchestratorAgent());
