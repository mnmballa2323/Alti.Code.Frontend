...
class DatabaseComplianceAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseComplianceSystem(objective) {
    logger.info(
      `💻 [DatabaseComplianceAuditorAgent] Analyzing Database Compliance Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Compliance Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseComplianceAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseComplianceAuditorAgent = Object.freeze(
  new DatabaseComplianceAuditorAgent(),
);
