...
class DatabaseAutomationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseAutomationSystem(objective) {
    logger.info(
      `💻 [DatabaseAutomationDeveloperAgent] Analyzing Database Automation Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Automation Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseAutomationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseAutomationDeveloperAgent = Object.freeze(
  new DatabaseAutomationDeveloperAgent(),
);
