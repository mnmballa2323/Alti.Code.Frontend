...
class DevOpsDataWarehouseTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsDataWarehouseSystem(objective) {
        logger.info(`💻 [DevOpsDataWarehouseTesterAgent] Analyzing DevOps DataWarehouse Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps DataWarehouse Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsDataWarehouseTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsDataWarehouseTesterAgent = Object.freeze(new DevOpsDataWarehouseTesterAgent());
