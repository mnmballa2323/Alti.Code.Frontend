...
class DatabaseIntegrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseIntegrationSystem(objective) {
    logger.info(
      `💻 [DatabaseIntegrationOrchestratorAgent] Analyzing Database Integration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Integration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseIntegrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseIntegrationOrchestratorAgent = Object.freeze(
  new DatabaseIntegrationOrchestratorAgent(),
);
