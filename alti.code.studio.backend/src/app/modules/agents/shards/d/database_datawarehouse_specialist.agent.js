...
class DatabaseDataWarehouseSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DatabaseDataWarehouseSpecialistAgent] Analyzing Database DataWarehouse Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database DataWarehouse Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseDataWarehouseSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseDataWarehouseSpecialistAgent = Object.freeze(
  new DatabaseDataWarehouseSpecialistAgent(),
);
