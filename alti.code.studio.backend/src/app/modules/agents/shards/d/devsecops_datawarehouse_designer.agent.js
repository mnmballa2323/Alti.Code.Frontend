...
class DevSecOpsDataWarehouseDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsDataWarehouseSystem(objective) {
        logger.info(`💻 [DevSecOpsDataWarehouseDesignerAgent] Analyzing DevSecOps DataWarehouse Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps DataWarehouse Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsDataWarehouseDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsDataWarehouseDesignerAgent = Object.freeze(new DevSecOpsDataWarehouseDesignerAgent());
