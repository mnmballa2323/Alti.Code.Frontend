...
class DevSecOpsMicroservicesArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsMicroservicesSystem(objective) {
    logger.info(
      `💻 [DevSecOpsMicroservicesArchitectAgent] Analyzing DevSecOps Microservices Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Microservices Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsMicroservicesArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsMicroservicesArchitectAgent = Object.freeze(
  new DevSecOpsMicroservicesArchitectAgent(),
);
