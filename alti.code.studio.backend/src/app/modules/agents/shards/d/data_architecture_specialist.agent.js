...
class DataArchitectureSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDataArchitectureSystem(objective) {
    logger.info(
      `💻 [DataArchitectureSpecialistAgent] Analyzing Data Architecture Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Architecture Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataArchitectureSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataArchitectureSpecialistAgent = Object.freeze(
  new DataArchitectureSpecialistAgent(),
);
