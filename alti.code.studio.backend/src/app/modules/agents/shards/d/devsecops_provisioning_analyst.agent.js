...
class DevSecOpsProvisioningAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsProvisioningSystem(objective) {
        logger.info(`💻 [DevSecOpsProvisioningAnalystAgent] Analyzing DevSecOps Provisioning Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Provisioning Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsProvisioningAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsProvisioningAnalystAgent = Object.freeze(new DevSecOpsProvisioningAnalystAgent());
