...
class DevSecOpsProvisioningSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsProvisioningSystem(objective) {
    logger.info(
      `💻 [DevSecOpsProvisioningSpecialistAgent] Analyzing DevSecOps Provisioning Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Provisioning Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsProvisioningSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsProvisioningSpecialistAgent = Object.freeze(
  new DevSecOpsProvisioningSpecialistAgent(),
);
