...
class DatabaseStreamingDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseStreamingSystem(objective) {
        logger.info(`💻 [DatabaseStreamingDirectorAgent] Analyzing Database Streaming Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Streaming Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseStreamingDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseStreamingDirectorAgent = Object.freeze(new DatabaseStreamingDirectorAgent());
