...
class DatabaseMicroservicesPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseMicroservicesSystem(objective) {
    logger.info(
      `💻 [DatabaseMicroservicesPlannerAgent] Analyzing Database Microservices Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Microservices Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseMicroservicesPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseMicroservicesPlannerAgent = Object.freeze(
  new DatabaseMicroservicesPlannerAgent(),
);
