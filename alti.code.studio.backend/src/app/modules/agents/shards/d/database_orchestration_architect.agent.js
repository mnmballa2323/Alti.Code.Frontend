...
class DatabaseOrchestrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseOrchestrationSystem(objective) {
    logger.info(
      `💻 [DatabaseOrchestrationArchitectAgent] Analyzing Database Orchestration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Orchestration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseOrchestrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseOrchestrationArchitectAgent = Object.freeze(
  new DatabaseOrchestrationArchitectAgent(),
);
