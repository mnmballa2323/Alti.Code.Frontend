...
class DevSecOpsMicroservicesTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsMicroservicesSystem(objective) {
    logger.info(
      `💻 [DevSecOpsMicroservicesTesterAgent] Analyzing DevSecOps Microservices Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Microservices Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsMicroservicesTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsMicroservicesTesterAgent = Object.freeze(
  new DevSecOpsMicroservicesTesterAgent(),
);
