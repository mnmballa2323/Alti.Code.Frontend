...
class DatabaseMigrationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseMigrationSystem(objective) {
        logger.info(`💻 [DatabaseMigrationDirectorAgent] Analyzing Database Migration Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Migration Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseMigrationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseMigrationDirectorAgent = Object.freeze(new DatabaseMigrationDirectorAgent());
