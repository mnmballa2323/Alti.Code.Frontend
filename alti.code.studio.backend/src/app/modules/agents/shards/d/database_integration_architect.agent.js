...
class DatabaseIntegrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseIntegrationSystem(objective) {
    logger.info(
      `💻 [DatabaseIntegrationArchitectAgent] Analyzing Database Integration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Integration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseIntegrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseIntegrationArchitectAgent = Object.freeze(
  new DatabaseIntegrationArchitectAgent(),
);
