...
class DataAnalyticsOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateDataAnalyticsSystem(objective) {
        logger.info(`💻 [DataAnalyticsOrchestratorAgent] Analyzing Data Analytics Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data Analytics Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataAnalyticsOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataAnalyticsOrchestratorAgent = Object.freeze(new DataAnalyticsOrchestratorAgent());
