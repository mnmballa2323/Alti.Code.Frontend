...
class DatabaseMicroservicesAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseMicroservicesSystem(objective) {
    logger.info(
      `💻 [DatabaseMicroservicesAuditorAgent] Analyzing Database Microservices Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Microservices Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseMicroservicesAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseMicroservicesAuditorAgent = Object.freeze(
  new DatabaseMicroservicesAuditorAgent(),
);
