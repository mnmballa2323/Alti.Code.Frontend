...
class DataContainerizationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDataContainerizationSystem(objective) {
    logger.info(
      `💻 [DataContainerizationArchitectAgent] Analyzing Data Containerization Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Containerization Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataContainerizationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataContainerizationArchitectAgent = Object.freeze(
  new DataContainerizationArchitectAgent(),
);
