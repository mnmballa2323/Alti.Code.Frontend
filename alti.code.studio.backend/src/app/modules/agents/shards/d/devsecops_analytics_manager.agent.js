...
class DevSecOpsAnalyticsManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsAnalyticsSystem(objective) {
    logger.info(
      `💻 [DevSecOpsAnalyticsManagerAgent] Analyzing DevSecOps Analytics Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Analytics Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsAnalyticsManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsAnalyticsManagerAgent = Object.freeze(
  new DevSecOpsAnalyticsManagerAgent(),
);
