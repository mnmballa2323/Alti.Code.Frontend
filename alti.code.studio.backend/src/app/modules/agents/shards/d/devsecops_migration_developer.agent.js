...
class DevSecOpsMigrationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsMigrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsMigrationDeveloperAgent] Analyzing DevSecOps Migration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Migration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsMigrationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsMigrationDeveloperAgent = Object.freeze(
  new DevSecOpsMigrationDeveloperAgent(),
);
