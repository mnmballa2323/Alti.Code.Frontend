...
class DevSecOpsDataWarehouseAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsDataWarehouseSystem(objective) {
        logger.info(`💻 [DevSecOpsDataWarehouseAnalystAgent] Analyzing DevSecOps DataWarehouse Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps DataWarehouse Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsDataWarehouseAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsDataWarehouseAnalystAgent = Object.freeze(new DevSecOpsDataWarehouseAnalystAgent());
