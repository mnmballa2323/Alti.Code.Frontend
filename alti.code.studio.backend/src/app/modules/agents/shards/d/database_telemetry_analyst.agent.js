...
class DatabaseTelemetryAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseTelemetrySystem(objective) {
    logger.info(
      `💻 [DatabaseTelemetryAnalystAgent] Analyzing Database Telemetry Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Telemetry Analyst Target',
      );
...
export const databaseTelemetryAnalystAgent = Object.freeze(
  new DatabaseTelemetryAnalystAgent(),
);
