...
class DataMicroservicesEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateDataMicroservicesSystem(objective) {
        logger.info(`💻 [DataMicroservicesEngineerAgent] Analyzing Data Microservices Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data Microservices Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataMicroservicesEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataMicroservicesEngineerAgent = Object.freeze(new DataMicroservicesEngineerAgent());
