...
class DatabaseTelemetryEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseTelemetrySystem(objective) {
        logger.info(`💻 [DatabaseTelemetryEngineerAgent] Analyzing Database Telemetry Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Telemetry Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseTelemetryEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseTelemetryEngineerAgent = Object.freeze(new DatabaseTelemetryEngineerAgent());
