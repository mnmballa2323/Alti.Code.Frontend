...
class DevOpsProvisioningDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsProvisioningSystem(objective) {
        logger.info(`💻 [DevOpsProvisioningDesignerAgent] Analyzing DevOps Provisioning Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Provisioning Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsProvisioningDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsProvisioningDesignerAgent = Object.freeze(new DevOpsProvisioningDesignerAgent());
