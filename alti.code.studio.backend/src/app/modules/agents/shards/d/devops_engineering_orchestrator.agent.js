...
class DevOpsEngineeringOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsEngineeringSystem(objective) {
    logger.info(
      `💻 [DevOpsEngineeringOrchestratorAgent] Analyzing DevOps Engineering Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Engineering Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsEngineeringOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsEngineeringOrchestratorAgent = Object.freeze(
  new DevOpsEngineeringOrchestratorAgent(),
);
