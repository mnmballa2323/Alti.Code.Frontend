...
class DevSecOpsStreamingAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsStreamingSystem(objective) {
    logger.info(
      `💻 [DevSecOpsStreamingAuditorAgent] Analyzing DevSecOps Streaming Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Streaming Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsStreamingAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsStreamingAuditorAgent = Object.freeze(
  new DevSecOpsStreamingAuditorAgent(),
);
