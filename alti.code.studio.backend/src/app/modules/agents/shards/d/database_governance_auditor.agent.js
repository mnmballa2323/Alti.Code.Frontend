...
class DatabaseGovernanceAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseGovernanceSystem(objective) {
    logger.info(
      `💻 [DatabaseGovernanceAuditorAgent] Analyzing Database Governance Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Governance Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseGovernanceAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseGovernanceAuditorAgent = Object.freeze(
  new DatabaseGovernanceAuditorAgent(),
);
