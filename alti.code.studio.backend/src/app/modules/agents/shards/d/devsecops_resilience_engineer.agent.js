...
class DevSecOpsResilienceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsResilienceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsResilienceEngineerAgent] Analyzing DevSecOps Resilience Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Resilience Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsResilienceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsResilienceEngineerAgent = Object.freeze(
  new DevSecOpsResilienceEngineerAgent(),
);
