...
class DevOpsComplianceDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsComplianceSystem(objective) {
    logger.info(
      `💻 [DevOpsComplianceDeveloperAgent] Analyzing DevOps Compliance Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Compliance Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsComplianceDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsComplianceDeveloperAgent = Object.freeze(
  new DevOpsComplianceDeveloperAgent(),
);
