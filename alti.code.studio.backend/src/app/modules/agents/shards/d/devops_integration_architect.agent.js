...
class DevOpsIntegrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsIntegrationSystem(objective) {
    logger.info(
      `💻 [DevOpsIntegrationArchitectAgent] Analyzing DevOps Integration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Integration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsIntegrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsIntegrationArchitectAgent = Object.freeze(
  new DevOpsIntegrationArchitectAgent(),
);
