...
class DatabaseAutomationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseAutomationSystem(objective) {
        logger.info(`💻 [DatabaseAutomationSpecialistAgent] Analyzing Database Automation Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Automation Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseAutomationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseAutomationSpecialistAgent = Object.freeze(new DatabaseAutomationSpecialistAgent());
