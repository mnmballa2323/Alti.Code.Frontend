...
class DevOpsMicroservicesAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsMicroservicesSystem(objective) {
    logger.info(
      `💻 [DevOpsMicroservicesAuditorAgent] Analyzing DevOps Microservices Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Microservices Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsMicroservicesAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsMicroservicesAuditorAgent = Object.freeze(
  new DevOpsMicroservicesAuditorAgent(),
);
