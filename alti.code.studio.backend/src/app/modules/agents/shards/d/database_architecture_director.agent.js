...
class DatabaseArchitectureDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseArchitectureSystem(objective) {
        logger.info(`💻 [DatabaseArchitectureDirectorAgent] Analyzing Database Architecture Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Architecture Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseArchitectureDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseArchitectureDirectorAgent = Object.freeze(new DatabaseArchitectureDirectorAgent());
