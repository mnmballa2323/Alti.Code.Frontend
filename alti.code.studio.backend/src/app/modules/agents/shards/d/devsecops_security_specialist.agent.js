...
class DevSecOpsSecuritySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsSecuritySystem(objective) {
    logger.info(
      `💻 [DevSecOpsSecuritySpecialistAgent] Analyzing DevSecOps Security Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Security Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsSecuritySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsSecuritySpecialistAgent = Object.freeze(
  new DevSecOpsSecuritySpecialistAgent(),
);
