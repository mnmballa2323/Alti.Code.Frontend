...
class DevSecOpsArchitectureStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsArchitectureSystem(objective) {
    logger.info(
      `💻 [DevSecOpsArchitectureStrategistAgent] Analyzing DevSecOps Architecture Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Architecture Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsArchitectureStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsArchitectureStrategistAgent = Object.freeze(
  new DevSecOpsArchitectureStrategistAgent(),
);
