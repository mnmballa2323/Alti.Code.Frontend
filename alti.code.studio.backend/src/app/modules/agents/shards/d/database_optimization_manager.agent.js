...
class DatabaseOptimizationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseOptimizationSystem(objective) {
    logger.info(
      `💻 [DatabaseOptimizationManagerAgent] Analyzing Database Optimization Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Optimization Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseOptimizationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseOptimizationManagerAgent = Object.freeze(
  new DatabaseOptimizationManagerAgent(),
);
