...
class DevSecOpsStreamingEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsStreamingSystem(objective) {
    logger.info(
      `💻 [DevSecOpsStreamingEngineerAgent] Analyzing DevSecOps Streaming Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Streaming Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsStreamingEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsStreamingEngineerAgent = Object.freeze(
  new DevSecOpsStreamingEngineerAgent(),
);
