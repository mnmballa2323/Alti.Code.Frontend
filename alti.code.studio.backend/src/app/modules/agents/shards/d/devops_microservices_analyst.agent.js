...
class DevOpsMicroservicesAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsMicroservicesSystem(objective) {
        logger.info(`💻 [DevOpsMicroservicesAnalystAgent] Analyzing DevOps Microservices Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Microservices Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsMicroservicesAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsMicroservicesAnalystAgent = Object.freeze(new DevOpsMicroservicesAnalystAgent());
