...
class DevSecOpsTelemetryDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsTelemetrySystem(objective) {
    logger.info(
      `💻 [DevSecOpsTelemetryDesignerAgent] Analyzing DevSecOps Telemetry Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Telemetry Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsTelemetryDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsTelemetryDesignerAgent = Object.freeze(
  new DevSecOpsTelemetryDesignerAgent(),
);
