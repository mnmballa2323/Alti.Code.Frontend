...
class DatabaseConfigurationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseConfigurationSystem(objective) {
        logger.info(`💻 [DatabaseConfigurationManagerAgent] Analyzing Database Configuration Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Configuration Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseConfigurationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseConfigurationManagerAgent = Object.freeze(new DatabaseConfigurationManagerAgent());
