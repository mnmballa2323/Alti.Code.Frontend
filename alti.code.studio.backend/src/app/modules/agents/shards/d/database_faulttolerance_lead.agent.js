...
class DatabaseFaultToleranceLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseFaultToleranceSystem(objective) {
        logger.info(`💻 [DatabaseFaultToleranceLeadAgent] Analyzing Database FaultTolerance Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database FaultTolerance Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseFaultToleranceLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseFaultToleranceLeadAgent = Object.freeze(new DatabaseFaultToleranceLeadAgent());
