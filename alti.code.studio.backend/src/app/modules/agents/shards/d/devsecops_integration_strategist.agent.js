...
class DevSecOpsIntegrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsIntegrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsIntegrationStrategistAgent] Analyzing DevSecOps Integration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Integration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsIntegrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsIntegrationStrategistAgent = Object.freeze(
  new DevSecOpsIntegrationStrategistAgent(),
);
