...
class DevOpsDataWarehouseEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DevOpsDataWarehouseEngineerAgent] Analyzing DevOps DataWarehouse Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps DataWarehouse Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsDataWarehouseEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsDataWarehouseEngineerAgent = Object.freeze(
  new DevOpsDataWarehouseEngineerAgent(),
);
