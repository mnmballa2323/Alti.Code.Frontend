...
class DevOpsMicroservicesPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsMicroservicesSystem(objective) {
    logger.info(
      `💻 [DevOpsMicroservicesPlannerAgent] Analyzing DevOps Microservices Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Microservices Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsMicroservicesPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsMicroservicesPlannerAgent = Object.freeze(
  new DevOpsMicroservicesPlannerAgent(),
);
