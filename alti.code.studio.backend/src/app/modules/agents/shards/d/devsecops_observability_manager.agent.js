...
class DevSecOpsObservabilityManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsObservabilitySystem(objective) {
    logger.info(
      `💻 [DevSecOpsObservabilityManagerAgent] Analyzing DevSecOps Observability Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Observability Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsObservabilityManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsObservabilityManagerAgent = Object.freeze(
  new DevSecOpsObservabilityManagerAgent(),
);
