...
class DatabaseVirtualizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseVirtualizationSystem(objective) {
    logger.info(
      `💻 [DatabaseVirtualizationAnalystAgent] Analyzing Database Virtualization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Virtualization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseVirtualizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseVirtualizationAnalystAgent = Object.freeze(
  new DatabaseVirtualizationAnalystAgent(),
);
