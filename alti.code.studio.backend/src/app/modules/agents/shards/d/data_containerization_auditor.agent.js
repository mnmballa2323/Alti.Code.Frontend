...
class DataContainerizationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateDataContainerizationSystem(objective) {
        logger.info(`💻 [DataContainerizationAuditorAgent] Analyzing Data Containerization Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data Containerization Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataContainerizationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataContainerizationAuditorAgent = Object.freeze(new DataContainerizationAuditorAgent());
