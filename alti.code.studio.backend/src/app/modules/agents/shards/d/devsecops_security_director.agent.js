...
class DevSecOpsSecurityDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsSecuritySystem(objective) {
    logger.info(
      `💻 [DevSecOpsSecurityDirectorAgent] Analyzing DevSecOps Security Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Security Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsSecurityDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsSecurityDirectorAgent = Object.freeze(
  new DevSecOpsSecurityDirectorAgent(),
);
