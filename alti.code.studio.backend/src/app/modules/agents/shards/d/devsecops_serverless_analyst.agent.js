...
class DevSecOpsServerlessAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsServerlessSystem(objective) {
    logger.info(
      `💻 [DevSecOpsServerlessAnalystAgent] Analyzing DevSecOps Serverless Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Serverless Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsServerlessAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsServerlessAnalystAgent = Object.freeze(
  new DevSecOpsServerlessAnalystAgent(),
);
