...
class DatabaseArchitectureTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseArchitectureSystem(objective) {
    logger.info(
      `💻 [DatabaseArchitectureTesterAgent] Analyzing Database Architecture Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Architecture Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseArchitectureTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseArchitectureTesterAgent = Object.freeze(
  new DatabaseArchitectureTesterAgent(),
);
