...
class DevOpsAnalyticsStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsAnalyticsSystem(objective) {
    logger.info(
      `💻 [DevOpsAnalyticsStrategistAgent] Analyzing DevOps Analytics Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Analytics Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsAnalyticsStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsAnalyticsStrategistAgent = Object.freeze(
  new DevOpsAnalyticsStrategistAgent(),
);
