...
class DevOpsArchitectureAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsArchitectureSystem(objective) {
    logger.info(
      `💻 [DevOpsArchitectureAnalystAgent] Analyzing DevOps Architecture Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Architecture Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsArchitectureAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsArchitectureAnalystAgent = Object.freeze(
  new DevOpsArchitectureAnalystAgent(),
);
