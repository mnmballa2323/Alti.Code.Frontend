...
class DatabaseAnalyticsOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseAnalyticsSystem(objective) {
        logger.info(`💻 [DatabaseAnalyticsOrchestratorAgent] Analyzing Database Analytics Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Analytics Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseAnalyticsOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseAnalyticsOrchestratorAgent = Object.freeze(new DatabaseAnalyticsOrchestratorAgent());
