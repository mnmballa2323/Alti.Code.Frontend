...
class DatabaseEngineeringEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseEngineeringSystem(objective) {
    logger.info(
      `💻 [DatabaseEngineeringEngineerAgent] Analyzing Database Engineering Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Engineering Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseEngineeringEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseEngineeringEngineerAgent = Object.freeze(
  new DatabaseEngineeringEngineerAgent(),
);
