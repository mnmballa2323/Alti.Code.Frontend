...
class DevOpsAutomationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsAutomationSystem(objective) {
    logger.info(
      `💻 [DevOpsAutomationDeveloperAgent] Analyzing DevOps Automation Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Automation Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsAutomationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsAutomationDeveloperAgent = Object.freeze(
  new DevOpsAutomationDeveloperAgent(),
);
