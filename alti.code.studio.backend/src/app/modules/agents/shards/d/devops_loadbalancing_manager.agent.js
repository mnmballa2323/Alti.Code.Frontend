...
class DevOpsLoadBalancingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsLoadBalancingSystem(objective) {
    logger.info(
      `💻 [DevOpsLoadBalancingManagerAgent] Analyzing DevOps LoadBalancing Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps LoadBalancing Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsLoadBalancingManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsLoadBalancingManagerAgent = Object.freeze(
  new DevOpsLoadBalancingManagerAgent(),
);
