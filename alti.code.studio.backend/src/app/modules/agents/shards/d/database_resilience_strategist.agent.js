...
class DatabaseResilienceStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseResilienceSystem(objective) {
    logger.info(
      `💻 [DatabaseResilienceStrategistAgent] Analyzing Database Resilience Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Resilience Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseResilienceStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseResilienceStrategistAgent = Object.freeze(
  new DatabaseResilienceStrategistAgent(),
);
