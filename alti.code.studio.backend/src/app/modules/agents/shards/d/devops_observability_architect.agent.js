...
class DevOpsObservabilityArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsObservabilitySystem(objective) {
        logger.info(`💻 [DevOpsObservabilityArchitectAgent] Analyzing DevOps Observability Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Observability Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsObservabilityArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsObservabilityArchitectAgent = Object.freeze(new DevOpsObservabilityArchitectAgent());
