...
class DevSecOpsContainerizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsContainerizationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsContainerizationLeadAgent] Analyzing DevSecOps Containerization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Containerization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsContainerizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsContainerizationLeadAgent = Object.freeze(
  new DevSecOpsContainerizationLeadAgent(),
);
