...
class DevSecOpsObservabilityLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsObservabilitySystem(objective) {
        logger.info(`💻 [DevSecOpsObservabilityLeadAgent] Analyzing DevSecOps Observability Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Observability Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsObservabilityLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsObservabilityLeadAgent = Object.freeze(new DevSecOpsObservabilityLeadAgent());
