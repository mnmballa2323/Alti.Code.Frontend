...
class DevSecOpsVirtualizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsVirtualizationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsVirtualizationDesignerAgent] Analyzing DevSecOps Virtualization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Virtualization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsVirtualizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsVirtualizationDesignerAgent = Object.freeze(
  new DevSecOpsVirtualizationDesignerAgent(),
);
