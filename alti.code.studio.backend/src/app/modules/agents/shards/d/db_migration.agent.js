...
class DbMigrationAgent extends GeminiCliBaseAgent {
  ...
  async generateZeroDowntimeMigration(currentSchema, newSchema) {
    logger.info(
      `🗄️ [DbMigrationAgent] Analyzing schema diffs for zero-downtime migration...`,
    );
...
${newSchema}
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Architect Scope',
      );
      const cleanSql = output.replace(/```sql|```/g, '').trim();
      logger.info(
        `✅ [DbMigrationAgent] Zero-Downtime SQL Migration generated successfully.`,
      );
      return cleanSql;
    } catch (err) {
      logger.error(
        `❌ [DbMigrationAgent] Failed to generate migration: ${err.message}`,
      );
      throw err;
    }
  }
