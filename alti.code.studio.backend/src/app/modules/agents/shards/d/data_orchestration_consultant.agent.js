...
class DataOrchestrationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDataOrchestrationSystem(objective) {
        logger.info(`💻 [DataOrchestrationConsultantAgent] Analyzing Data Orchestration Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data Orchestration Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataOrchestrationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataOrchestrationConsultantAgent = Object.freeze(new DataOrchestrationConsultantAgent());
