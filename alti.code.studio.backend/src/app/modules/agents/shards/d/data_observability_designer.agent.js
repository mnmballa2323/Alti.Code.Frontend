...
class DataObservabilityDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDataObservabilitySystem(objective) {
    logger.info(
      `💻 [DataObservabilityDesignerAgent] Analyzing Data Observability Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Observability Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataObservabilityDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataObservabilityDesignerAgent = Object.freeze(
  new DataObservabilityDesignerAgent(),
);
