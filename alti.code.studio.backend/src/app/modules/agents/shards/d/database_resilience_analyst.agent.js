...
class DatabaseResilienceAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseResilienceSystem(objective) {
    logger.info(
      `💻 [DatabaseResilienceAnalystAgent] Analyzing Database Resilience Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Resilience Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseResilienceAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseResilienceAnalystAgent = Object.freeze(
  new DatabaseResilienceAnalystAgent(),
);
