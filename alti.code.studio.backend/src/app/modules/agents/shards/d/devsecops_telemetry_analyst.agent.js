...
class DevSecOpsTelemetryAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsTelemetrySystem(objective) {
        logger.info(`💻 [DevSecOpsTelemetryAnalystAgent] Analyzing DevSecOps Telemetry Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Telemetry Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsTelemetryAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsTelemetryAnalystAgent = Object.freeze(new DevSecOpsTelemetryAnalystAgent());
