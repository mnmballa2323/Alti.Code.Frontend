...
class DevSecOpsServerlessSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsServerlessSystem(objective) {
        logger.info(`💻 [DevSecOpsServerlessSpecialistAgent] Analyzing DevSecOps Serverless Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Serverless Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsServerlessSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsServerlessSpecialistAgent = Object.freeze(new DevSecOpsServerlessSpecialistAgent());
