...
class DevOpsMicroservicesManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsMicroservicesSystem(objective) {
        logger.info(`💻 [DevOpsMicroservicesManagerAgent] Analyzing DevOps Microservices Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Microservices Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsMicroservicesManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsMicroservicesManagerAgent = Object.freeze(new DevOpsMicroservicesManagerAgent());
