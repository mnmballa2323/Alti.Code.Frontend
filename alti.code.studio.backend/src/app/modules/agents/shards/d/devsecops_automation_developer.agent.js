...
class DevSecOpsAutomationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsAutomationSystem(objective) {
        logger.info(`💻 [DevSecOpsAutomationDeveloperAgent] Analyzing DevSecOps Automation Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Automation Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsAutomationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsAutomationDeveloperAgent = Object.freeze(new DevSecOpsAutomationDeveloperAgent());
