...
class DatabaseIntegrationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseIntegrationSystem(objective) {
    logger.info(
      `💻 [DatabaseIntegrationManagerAgent] Analyzing Database Integration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Integration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseIntegrationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseIntegrationManagerAgent = Object.freeze(
  new DatabaseIntegrationManagerAgent(),
);
