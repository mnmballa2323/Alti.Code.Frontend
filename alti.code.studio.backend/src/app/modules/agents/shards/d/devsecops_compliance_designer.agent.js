...
class DevSecOpsComplianceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsComplianceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsComplianceDesignerAgent] Analyzing DevSecOps Compliance Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Compliance Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsComplianceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsComplianceDesignerAgent = Object.freeze(
  new DevSecOpsComplianceDesignerAgent(),
);
