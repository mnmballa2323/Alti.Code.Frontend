...
class DatabaseDataLakeStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseDataLakeSystem(objective) {
    logger.info(
      `💻 [DatabaseDataLakeStrategistAgent] Analyzing Database DataLake Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database DataLake Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseDataLakeStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseDataLakeStrategistAgent = Object.freeze(
  new DatabaseDataLakeStrategistAgent(),
);
