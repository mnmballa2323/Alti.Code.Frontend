...
class DataDataWarehouseArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDataDataWarehouseSystem(objective) {
        logger.info(`💻 [DataDataWarehouseArchitectAgent] Analyzing Data DataWarehouse Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data DataWarehouse Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataDataWarehouseArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataDataWarehouseArchitectAgent = Object.freeze(new DataDataWarehouseArchitectAgent());
