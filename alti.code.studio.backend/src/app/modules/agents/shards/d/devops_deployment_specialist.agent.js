...
class DevOpsDeploymentSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsDeploymentSystem(objective) {
    logger.info(
      `💻 [DevOpsDeploymentSpecialistAgent] Analyzing DevOps Deployment Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Deployment Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsDeploymentSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsDeploymentSpecialistAgent = Object.freeze(
  new DevOpsDeploymentSpecialistAgent(),
);
