...
class DatabaseDeploymentAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseDeploymentSystem(objective) {
    logger.info(
      `💻 [DatabaseDeploymentAuditorAgent] Analyzing Database Deployment Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Deployment Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseDeploymentAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseDeploymentAuditorAgent = Object.freeze(
  new DatabaseDeploymentAuditorAgent(),
);
