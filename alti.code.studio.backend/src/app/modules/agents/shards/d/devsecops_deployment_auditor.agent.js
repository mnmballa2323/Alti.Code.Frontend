...
class DevSecOpsDeploymentAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsDeploymentSystem(objective) {
    logger.info(
      `💻 [DevSecOpsDeploymentAuditorAgent] Analyzing DevSecOps Deployment Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Deployment Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsDeploymentAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsDeploymentAuditorAgent = Object.freeze(
  new DevSecOpsDeploymentAuditorAgent(),
);
