...
class DevSecOpsCachingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsCachingSystem(objective) {
    logger.info(
      `💻 [DevSecOpsCachingOrchestratorAgent] Analyzing DevSecOps Caching Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Caching Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsCachingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsCachingOrchestratorAgent = Object.freeze(
  new DevSecOpsCachingOrchestratorAgent(),
);
