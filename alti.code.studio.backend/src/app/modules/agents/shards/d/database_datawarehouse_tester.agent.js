...
class DatabaseDataWarehouseTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DatabaseDataWarehouseTesterAgent] Analyzing Database DataWarehouse Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database DataWarehouse Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseDataWarehouseTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseDataWarehouseTesterAgent = Object.freeze(
  new DatabaseDataWarehouseTesterAgent(),
);
