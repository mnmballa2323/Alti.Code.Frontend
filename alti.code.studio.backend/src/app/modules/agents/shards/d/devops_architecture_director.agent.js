...
class DevOpsArchitectureDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsArchitectureSystem(objective) {
    logger.info(
      `💻 [DevOpsArchitectureDirectorAgent] Analyzing DevOps Architecture Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Architecture Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsArchitectureDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsArchitectureDirectorAgent = Object.freeze(
  new DevOpsArchitectureDirectorAgent(),
);
