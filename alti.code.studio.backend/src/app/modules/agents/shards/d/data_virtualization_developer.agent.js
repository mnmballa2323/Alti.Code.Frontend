...
class DataVirtualizationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDataVirtualizationSystem(objective) {
    logger.info(
      `💻 [DataVirtualizationDeveloperAgent] Analyzing Data Virtualization Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Virtualization Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataVirtualizationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataVirtualizationDeveloperAgent = Object.freeze(
  new DataVirtualizationDeveloperAgent(),
);
