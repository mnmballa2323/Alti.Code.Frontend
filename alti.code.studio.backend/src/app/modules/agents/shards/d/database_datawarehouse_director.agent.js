...
class DatabaseDataWarehouseDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DatabaseDataWarehouseDirectorAgent] Analyzing Database DataWarehouse Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database DataWarehouse Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseDataWarehouseDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseDataWarehouseDirectorAgent = Object.freeze(
  new DatabaseDataWarehouseDirectorAgent(),
);
