...
class DevSecOpsServerlessAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsServerlessSystem(objective) {
    logger.info(
      `💻 [DevSecOpsServerlessAuditorAgent] Analyzing DevSecOps Serverless Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Serverless Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsServerlessAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsServerlessAuditorAgent = Object.freeze(
  new DevSecOpsServerlessAuditorAgent(),
);
