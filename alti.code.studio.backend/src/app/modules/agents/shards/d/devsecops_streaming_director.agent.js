...
class DevSecOpsStreamingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsStreamingSystem(objective) {
    logger.info(
      `💻 [DevSecOpsStreamingDirectorAgent] Analyzing DevSecOps Streaming Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Streaming Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsStreamingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsStreamingDirectorAgent = Object.freeze(
  new DevSecOpsStreamingDirectorAgent(),
);
