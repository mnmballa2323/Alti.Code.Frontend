...
class DatabaseFaultToleranceTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseFaultToleranceSystem(objective) {
        logger.info(`💻 [DatabaseFaultToleranceTesterAgent] Analyzing Database FaultTolerance Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database FaultTolerance Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseFaultToleranceTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseFaultToleranceTesterAgent = Object.freeze(new DatabaseFaultToleranceTesterAgent());
