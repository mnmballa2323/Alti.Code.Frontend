...
class DevSecOpsGovernanceOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsGovernanceSystem(objective) {
        logger.info(`💻 [DevSecOpsGovernanceOrchestratorAgent] Analyzing DevSecOps Governance Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Governance Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsGovernanceOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsGovernanceOrchestratorAgent = Object.freeze(new DevSecOpsGovernanceOrchestratorAgent());
