...
class DatabaseStreamingAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseStreamingSystem(objective) {
    logger.info(
      `💻 [DatabaseStreamingAuditorAgent] Analyzing Database Streaming Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Streaming Auditor Target',
      );
...
export const databaseStreamingAuditorAgent = Object.freeze(
  new DatabaseStreamingAuditorAgent(),
);
