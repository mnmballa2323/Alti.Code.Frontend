...
class DevSecOpsOrchestrationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsOrchestrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsOrchestrationDesignerAgent] Analyzing DevSecOps Orchestration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Orchestration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsOrchestrationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsOrchestrationDesignerAgent = Object.freeze(
  new DevSecOpsOrchestrationDesignerAgent(),
);
