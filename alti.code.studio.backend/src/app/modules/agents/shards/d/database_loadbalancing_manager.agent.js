...
class DatabaseLoadBalancingManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseLoadBalancingSystem(objective) {
        logger.info(`💻 [DatabaseLoadBalancingManagerAgent] Analyzing Database LoadBalancing Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database LoadBalancing Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseLoadBalancingManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseLoadBalancingManagerAgent = Object.freeze(new DatabaseLoadBalancingManagerAgent());
