...
class DesignerAgent extends BaseSpecialistAgent {
  ...
  async analyzeFigmaNode(fileId, nodeId = null) {
    logger.info(
      `🎨 DesignerAgent: Analyzing Figma File ${fileId}${nodeId ? ` (Node: ${nodeId})` : ''}...`,
    );

    if (!process.env.FIGMA_ACCESS_TOKEN) {
      logger.warn(
        '⚠️ No FIGMA_ACCESS_TOKEN found. Simulating DesignerAgent response.',
      );
    ...
    try {
      ...
      const componentCode = await this._generateComponentFromTree(
        rawTree,
        designTokens,
      );
...
3. Output ONLY the React JSX/TSX code without markdown blocks.
  ...
  _simulateFigmaResponse() {
    return {
      ...
      message:
        'No FIGMA_ACCESS_TOKEN present. Returned simulated generic React Component.',
...
export default function SimulatedFigmaComponent() {
  ...
  async _mockWriteToDisk(filePath, code) {
    logger.info(
      `💾 DesignerAgent: Writing generated component code to ${filePath}`,
    );
    return true;
  }
