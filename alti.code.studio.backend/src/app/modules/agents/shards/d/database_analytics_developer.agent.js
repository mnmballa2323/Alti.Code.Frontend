...
class DatabaseAnalyticsDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseAnalyticsSystem(objective) {
    logger.info(
      `💻 [DatabaseAnalyticsDeveloperAgent] Analyzing Database Analytics Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Analytics Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseAnalyticsDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseAnalyticsDeveloperAgent = Object.freeze(
  new DatabaseAnalyticsDeveloperAgent(),
);
