...
class DatabaseTelemetryConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseTelemetrySystem(objective) {
        logger.info(`💻 [DatabaseTelemetryConsultantAgent] Analyzing Database Telemetry Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Telemetry Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseTelemetryConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseTelemetryConsultantAgent = Object.freeze(new DatabaseTelemetryConsultantAgent());
