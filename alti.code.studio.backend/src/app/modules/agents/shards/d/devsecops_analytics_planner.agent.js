...
class DevSecOpsAnalyticsPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsAnalyticsSystem(objective) {
    logger.info(
      `💻 [DevSecOpsAnalyticsPlannerAgent] Analyzing DevSecOps Analytics Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Analytics Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsAnalyticsPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsAnalyticsPlannerAgent = Object.freeze(
  new DevSecOpsAnalyticsPlannerAgent(),
);
