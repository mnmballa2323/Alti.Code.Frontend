...
class DevSecOpsComplianceAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsComplianceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsComplianceAnalystAgent] Analyzing DevSecOps Compliance Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Compliance Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsComplianceAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsComplianceAnalystAgent = Object.freeze(
  new DevSecOpsComplianceAnalystAgent(),
);
