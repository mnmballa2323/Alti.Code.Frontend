...
class DevOpsMicroservicesDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsMicroservicesSystem(objective) {
    logger.info(
      `💻 [DevOpsMicroservicesDirectorAgent] Analyzing DevOps Microservices Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Microservices Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsMicroservicesDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsMicroservicesDirectorAgent = Object.freeze(
  new DevOpsMicroservicesDirectorAgent(),
);
