...
class DataContainerizationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDataContainerizationSystem(objective) {
    logger.info(
      `💻 [DataContainerizationDeveloperAgent] Analyzing Data Containerization Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Containerization Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataContainerizationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataContainerizationDeveloperAgent = Object.freeze(
  new DataContainerizationDeveloperAgent(),
);
