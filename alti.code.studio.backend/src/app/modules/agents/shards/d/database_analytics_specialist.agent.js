...
class DatabaseAnalyticsSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseAnalyticsSystem(objective) {
        logger.info(`💻 [DatabaseAnalyticsSpecialistAgent] Analyzing Database Analytics Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Analytics Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseAnalyticsSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseAnalyticsSpecialistAgent = Object.freeze(new DatabaseAnalyticsSpecialistAgent());
