...
class DevSecOpsArchitectureDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsArchitectureSystem(objective) {
        logger.info(`💻 [DevSecOpsArchitectureDesignerAgent] Analyzing DevSecOps Architecture Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Architecture Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsArchitectureDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsArchitectureDesignerAgent = Object.freeze(new DevSecOpsArchitectureDesignerAgent());
