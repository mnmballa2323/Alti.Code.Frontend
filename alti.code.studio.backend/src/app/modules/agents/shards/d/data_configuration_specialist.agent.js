...
class DataConfigurationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDataConfigurationSystem(objective) {
    logger.info(
      `💻 [DataConfigurationSpecialistAgent] Analyzing Data Configuration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Configuration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataConfigurationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataConfigurationSpecialistAgent = Object.freeze(
  new DataConfigurationSpecialistAgent(),
);
