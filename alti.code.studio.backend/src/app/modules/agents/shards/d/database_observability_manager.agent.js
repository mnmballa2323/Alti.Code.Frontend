...
class DatabaseObservabilityManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseObservabilitySystem(objective) {
        logger.info(`💻 [DatabaseObservabilityManagerAgent] Analyzing Database Observability Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Observability Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseObservabilityManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseObservabilityManagerAgent = Object.freeze(new DatabaseObservabilityManagerAgent());
