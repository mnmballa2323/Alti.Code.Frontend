...
class DatabaseTelemetryStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseTelemetrySystem(objective) {
    logger.info(
      `💻 [DatabaseTelemetryStrategistAgent] Analyzing Database Telemetry Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Telemetry Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseTelemetryStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseTelemetryStrategistAgent = Object.freeze(
  new DatabaseTelemetryStrategistAgent(),
);
