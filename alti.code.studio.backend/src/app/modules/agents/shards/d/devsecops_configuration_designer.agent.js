...
class DevSecOpsConfigurationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsConfigurationSystem(objective) {
        logger.info(`💻 [DevSecOpsConfigurationDesignerAgent] Analyzing DevSecOps Configuration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Configuration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsConfigurationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsConfigurationDesignerAgent = Object.freeze(new DevSecOpsConfigurationDesignerAgent());
