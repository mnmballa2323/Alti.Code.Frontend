...
class DataBatchProcessingTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDataBatchProcessingSystem(objective) {
        logger.info(`💻 [DataBatchProcessingTesterAgent] Analyzing Data BatchProcessing Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data BatchProcessing Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataBatchProcessingTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataBatchProcessingTesterAgent = Object.freeze(new DataBatchProcessingTesterAgent());
