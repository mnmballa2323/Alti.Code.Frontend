...
class DataArchitectureOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataArchitectureSystem(objective) {
    logger.info(
      `💻 [DataArchitectureOrchestratorAgent] Analyzing Data Architecture Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Architecture Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataArchitectureOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataArchitectureOrchestratorAgent = Object.freeze(
  new DataArchitectureOrchestratorAgent(),
);
