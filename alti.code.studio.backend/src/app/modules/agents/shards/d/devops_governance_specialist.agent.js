...
class DevOpsGovernanceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsGovernanceSystem(objective) {
    logger.info(
      `💻 [DevOpsGovernanceSpecialistAgent] Analyzing DevOps Governance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Governance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsGovernanceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsGovernanceSpecialistAgent = Object.freeze(
  new DevOpsGovernanceSpecialistAgent(),
);
