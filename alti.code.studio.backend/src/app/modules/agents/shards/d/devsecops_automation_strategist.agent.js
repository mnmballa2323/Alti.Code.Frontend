...
class DevSecOpsAutomationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsAutomationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsAutomationStrategistAgent] Analyzing DevSecOps Automation Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Automation Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsAutomationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsAutomationStrategistAgent = Object.freeze(
  new DevSecOpsAutomationStrategistAgent(),
);
