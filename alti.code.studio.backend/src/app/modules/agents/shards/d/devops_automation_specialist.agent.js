...
class DevOpsAutomationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsAutomationSystem(objective) {
    logger.info(
      `💻 [DevOpsAutomationSpecialistAgent] Analyzing DevOps Automation Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Automation Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsAutomationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsAutomationSpecialistAgent = Object.freeze(
  new DevOpsAutomationSpecialistAgent(),
);
