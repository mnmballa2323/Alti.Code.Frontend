...
class DevSecOpsStreamingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsStreamingSystem(objective) {
    logger.info(
      `💻 [DevSecOpsStreamingSpecialistAgent] Analyzing DevSecOps Streaming Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Streaming Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsStreamingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsStreamingSpecialistAgent = Object.freeze(
  new DevSecOpsStreamingSpecialistAgent(),
);
