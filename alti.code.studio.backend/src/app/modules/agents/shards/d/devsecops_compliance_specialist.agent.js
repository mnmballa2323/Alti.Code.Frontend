...
class DevSecOpsComplianceSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsComplianceSystem(objective) {
        logger.info(`💻 [DevSecOpsComplianceSpecialistAgent] Analyzing DevSecOps Compliance Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Compliance Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsComplianceSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsComplianceSpecialistAgent = Object.freeze(new DevSecOpsComplianceSpecialistAgent());
