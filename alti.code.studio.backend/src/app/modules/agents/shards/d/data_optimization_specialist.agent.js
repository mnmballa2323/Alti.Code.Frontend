...
class DataOptimizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDataOptimizationSystem(objective) {
        logger.info(`💻 [DataOptimizationSpecialistAgent] Analyzing Data Optimization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data Optimization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataOptimizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataOptimizationSpecialistAgent = Object.freeze(new DataOptimizationSpecialistAgent());
