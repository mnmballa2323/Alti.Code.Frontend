...
class DevOpsComplianceArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsComplianceSystem(objective) {
    logger.info(
      `💻 [DevOpsComplianceArchitectAgent] Analyzing DevOps Compliance Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Compliance Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsComplianceArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsComplianceArchitectAgent = Object.freeze(
  new DevOpsComplianceArchitectAgent(),
);
