...
class DevOpsProvisioningSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsProvisioningSystem(objective) {
    logger.info(
      `💻 [DevOpsProvisioningSpecialistAgent] Analyzing DevOps Provisioning Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Provisioning Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsProvisioningSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsProvisioningSpecialistAgent = Object.freeze(
  new DevOpsProvisioningSpecialistAgent(),
);
