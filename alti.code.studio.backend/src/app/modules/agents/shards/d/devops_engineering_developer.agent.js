...
class DevOpsEngineeringDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsEngineeringSystem(objective) {
    logger.info(
      `💻 [DevOpsEngineeringDeveloperAgent] Analyzing DevOps Engineering Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Engineering Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsEngineeringDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsEngineeringDeveloperAgent = Object.freeze(
  new DevOpsEngineeringDeveloperAgent(),
);
