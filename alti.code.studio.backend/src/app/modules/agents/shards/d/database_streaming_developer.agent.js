...
class DatabaseStreamingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseStreamingSystem(objective) {
    logger.info(
      `💻 [DatabaseStreamingDeveloperAgent] Analyzing Database Streaming Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Streaming Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseStreamingDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseStreamingDeveloperAgent = Object.freeze(
  new DatabaseStreamingDeveloperAgent(),
);
