...
class DevSecOpsCachingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsCachingSystem(objective) {
    logger.info(
      `💻 [DevSecOpsCachingDeveloperAgent] Analyzing DevSecOps Caching Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Caching Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsCachingDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsCachingDeveloperAgent = Object.freeze(
  new DevSecOpsCachingDeveloperAgent(),
);
