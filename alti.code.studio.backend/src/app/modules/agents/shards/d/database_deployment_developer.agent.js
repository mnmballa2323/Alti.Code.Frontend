...
class DatabaseDeploymentDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseDeploymentSystem(objective) {
        logger.info(`💻 [DatabaseDeploymentDeveloperAgent] Analyzing Database Deployment Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Deployment Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseDeploymentDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseDeploymentDeveloperAgent = Object.freeze(new DatabaseDeploymentDeveloperAgent());
