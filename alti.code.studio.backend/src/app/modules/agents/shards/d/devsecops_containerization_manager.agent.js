...
class DevSecOpsContainerizationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsContainerizationSystem(objective) {
        logger.info(`💻 [DevSecOpsContainerizationManagerAgent] Analyzing DevSecOps Containerization Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Containerization Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsContainerizationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsContainerizationManagerAgent = Object.freeze(new DevSecOpsContainerizationManagerAgent());
