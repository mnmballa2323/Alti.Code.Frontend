...
class DataGovernanceOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataGovernanceSystem(objective) {
    logger.info(
      `💻 [DataGovernanceOrchestratorAgent] Analyzing Data Governance Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Governance Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataGovernanceOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataGovernanceOrchestratorAgent = Object.freeze(
  new DataGovernanceOrchestratorAgent(),
);
