...
class DatabaseStreamingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseStreamingSystem(objective) {
    logger.info(
      `💻 [DatabaseStreamingDesignerAgent] Analyzing Database Streaming Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Streaming Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseStreamingDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseStreamingDesignerAgent = Object.freeze(
  new DatabaseStreamingDesignerAgent(),
);
