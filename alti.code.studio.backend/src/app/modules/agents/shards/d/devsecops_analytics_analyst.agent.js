...
class DevSecOpsAnalyticsAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsAnalyticsSystem(objective) {
    logger.info(
      `💻 [DevSecOpsAnalyticsAnalystAgent] Analyzing DevSecOps Analytics Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Analytics Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsAnalyticsAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsAnalyticsAnalystAgent = Object.freeze(
  new DevSecOpsAnalyticsAnalystAgent(),
);
