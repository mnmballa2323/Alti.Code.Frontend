...
class DatabaseProvisioningStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseProvisioningSystem(objective) {
    logger.info(
      `💻 [DatabaseProvisioningStrategistAgent] Analyzing Database Provisioning Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Provisioning Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseProvisioningStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseProvisioningStrategistAgent = Object.freeze(
  new DatabaseProvisioningStrategistAgent(),
);
