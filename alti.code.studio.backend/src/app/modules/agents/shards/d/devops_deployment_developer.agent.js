...
class DevOpsDeploymentDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsDeploymentSystem(objective) {
        logger.info(`💻 [DevOpsDeploymentDeveloperAgent] Analyzing DevOps Deployment Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Deployment Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsDeploymentDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsDeploymentDeveloperAgent = Object.freeze(new DevOpsDeploymentDeveloperAgent());
