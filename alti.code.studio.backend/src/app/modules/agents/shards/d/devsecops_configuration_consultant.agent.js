...
class DevSecOpsConfigurationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsConfigurationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsConfigurationConsultantAgent] Analyzing DevSecOps Configuration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Configuration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsConfigurationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsConfigurationConsultantAgent = Object.freeze(
  new DevSecOpsConfigurationConsultantAgent(),
);
