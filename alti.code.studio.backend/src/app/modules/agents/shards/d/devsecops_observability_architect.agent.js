...
class DevSecOpsObservabilityArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsObservabilitySystem(objective) {
    logger.info(
      `💻 [DevSecOpsObservabilityArchitectAgent] Analyzing DevSecOps Observability Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Observability Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsObservabilityArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsObservabilityArchitectAgent = Object.freeze(
  new DevSecOpsObservabilityArchitectAgent(),
);
