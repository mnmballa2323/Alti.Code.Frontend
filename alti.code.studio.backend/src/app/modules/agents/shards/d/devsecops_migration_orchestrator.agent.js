...
class DevSecOpsMigrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsMigrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsMigrationOrchestratorAgent] Analyzing DevSecOps Migration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Migration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsMigrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsMigrationOrchestratorAgent = Object.freeze(
  new DevSecOpsMigrationOrchestratorAgent(),
);
