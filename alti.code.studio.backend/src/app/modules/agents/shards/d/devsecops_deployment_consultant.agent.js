...
class DevSecOpsDeploymentConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsDeploymentSystem(objective) {
    logger.info(
      `💻 [DevSecOpsDeploymentConsultantAgent] Analyzing DevSecOps Deployment Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Deployment Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsDeploymentConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsDeploymentConsultantAgent = Object.freeze(
  new DevSecOpsDeploymentConsultantAgent(),
);
