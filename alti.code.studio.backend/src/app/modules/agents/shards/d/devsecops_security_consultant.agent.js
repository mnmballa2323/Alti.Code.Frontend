...
class DevSecOpsSecurityConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsSecuritySystem(objective) {
        logger.info(`💻 [DevSecOpsSecurityConsultantAgent] Analyzing DevSecOps Security Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Security Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsSecurityConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsSecurityConsultantAgent = Object.freeze(new DevSecOpsSecurityConsultantAgent());
