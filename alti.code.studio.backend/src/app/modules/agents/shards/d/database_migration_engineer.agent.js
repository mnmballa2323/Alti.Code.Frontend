...
class DatabaseMigrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseMigrationSystem(objective) {
    logger.info(
      `💻 [DatabaseMigrationEngineerAgent] Analyzing Database Migration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Migration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseMigrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseMigrationEngineerAgent = Object.freeze(
  new DatabaseMigrationEngineerAgent(),
);
