...
class DevOpsLoadBalancingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsLoadBalancingSystem(objective) {
    logger.info(
      `💻 [DevOpsLoadBalancingPlannerAgent] Analyzing DevOps LoadBalancing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps LoadBalancing Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsLoadBalancingPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsLoadBalancingPlannerAgent = Object.freeze(
  new DevOpsLoadBalancingPlannerAgent(),
);
