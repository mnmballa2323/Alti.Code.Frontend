...
class DatabaseArchitectureEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseArchitectureSystem(objective) {
    logger.info(
      `💻 [DatabaseArchitectureEngineerAgent] Analyzing Database Architecture Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Architecture Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseArchitectureEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseArchitectureEngineerAgent = Object.freeze(
  new DatabaseArchitectureEngineerAgent(),
);
