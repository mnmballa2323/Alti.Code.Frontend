...
class DatabaseProvisioningConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseProvisioningSystem(objective) {
    logger.info(
      `💻 [DatabaseProvisioningConsultantAgent] Analyzing Database Provisioning Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Provisioning Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseProvisioningConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseProvisioningConsultantAgent = Object.freeze(
  new DatabaseProvisioningConsultantAgent(),
);
