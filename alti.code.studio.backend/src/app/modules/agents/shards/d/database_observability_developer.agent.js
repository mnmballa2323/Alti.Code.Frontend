...
class DatabaseObservabilityDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseObservabilitySystem(objective) {
    logger.info(
      `💻 [DatabaseObservabilityDeveloperAgent] Analyzing Database Observability Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Observability Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseObservabilityDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseObservabilityDeveloperAgent = Object.freeze(
  new DatabaseObservabilityDeveloperAgent(),
);
