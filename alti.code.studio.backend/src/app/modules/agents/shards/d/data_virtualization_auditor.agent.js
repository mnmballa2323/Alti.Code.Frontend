...
class DataVirtualizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataVirtualizationSystem(objective) {
    logger.info(
      `💻 [DataVirtualizationAuditorAgent] Analyzing Data Virtualization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Virtualization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataVirtualizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataVirtualizationAuditorAgent = Object.freeze(
  new DataVirtualizationAuditorAgent(),
);
