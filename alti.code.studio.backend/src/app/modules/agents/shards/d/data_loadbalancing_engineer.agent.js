...
class DataLoadBalancingEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDataLoadBalancingSystem(objective) {
    logger.info(
      `💻 [DataLoadBalancingEngineerAgent] Analyzing Data LoadBalancing Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data LoadBalancing Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataLoadBalancingEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataLoadBalancingEngineerAgent = Object.freeze(
  new DataLoadBalancingEngineerAgent(),
);
