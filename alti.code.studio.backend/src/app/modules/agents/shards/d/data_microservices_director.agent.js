...
class DataMicroservicesDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataMicroservicesSystem(objective) {
    logger.info(
      `💻 [DataMicroservicesDirectorAgent] Analyzing Data Microservices Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Microservices Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataMicroservicesDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataMicroservicesDirectorAgent = Object.freeze(
  new DataMicroservicesDirectorAgent(),
);
