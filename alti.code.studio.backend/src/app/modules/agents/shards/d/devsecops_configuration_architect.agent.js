...
class DevSecOpsConfigurationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsConfigurationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsConfigurationArchitectAgent] Analyzing DevSecOps Configuration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Configuration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsConfigurationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsConfigurationArchitectAgent = Object.freeze(
  new DevSecOpsConfigurationArchitectAgent(),
);
