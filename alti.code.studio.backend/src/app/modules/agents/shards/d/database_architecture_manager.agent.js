...
class DatabaseArchitectureManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseArchitectureSystem(objective) {
        logger.info(`💻 [DatabaseArchitectureManagerAgent] Analyzing Database Architecture Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Architecture Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseArchitectureManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseArchitectureManagerAgent = Object.freeze(new DatabaseArchitectureManagerAgent());
