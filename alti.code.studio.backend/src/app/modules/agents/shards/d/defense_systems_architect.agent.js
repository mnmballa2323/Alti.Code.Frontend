...
class DefenseSystemsArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDefenseSystem(defenseObjective) {
        logger.info(`🛡️ [DefenseSystemsArchitect] Analyzing objective for tactical logic and secure C4ISR...`);
...
${defenseObjective}
        ...
        try {
            ...
            logger.info(`✅ [DefenseSystemsArchitect] Defense architecture generated successfully.`);
            return cleanCode;
        } catch (err) {
            logger.error(`❌ [DefenseSystemsArchitect] Failed to generate defense system: ${err.message}`);
            throw err;
        }
    }
}

export const defenseSystemsArchitectAgent = Object.freeze(new DefenseSystemsArchitectAgent());
