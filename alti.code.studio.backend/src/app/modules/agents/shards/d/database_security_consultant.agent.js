...
class DatabaseSecurityConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseSecuritySystem(objective) {
    logger.info(
      `💻 [DatabaseSecurityConsultantAgent] Analyzing Database Security Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Security Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseSecurityConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseSecurityConsultantAgent = Object.freeze(
  new DatabaseSecurityConsultantAgent(),
);
