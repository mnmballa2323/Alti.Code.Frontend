...
class DevSecOpsContainerizationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsContainerizationSystem(objective) {
        logger.info(`💻 [DevSecOpsContainerizationPlannerAgent] Analyzing DevSecOps Containerization Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Containerization Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsContainerizationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsContainerizationPlannerAgent = Object.freeze(new DevSecOpsContainerizationPlannerAgent());
