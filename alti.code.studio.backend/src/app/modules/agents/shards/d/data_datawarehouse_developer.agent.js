...
class DataDataWarehouseDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateDataDataWarehouseSystem(objective) {
        logger.info(`💻 [DataDataWarehouseDeveloperAgent] Analyzing Data DataWarehouse Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data DataWarehouse Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataDataWarehouseDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataDataWarehouseDeveloperAgent = Object.freeze(new DataDataWarehouseDeveloperAgent());
