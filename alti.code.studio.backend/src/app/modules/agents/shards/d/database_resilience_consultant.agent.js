...
class DatabaseResilienceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseResilienceSystem(objective) {
    logger.info(
      `💻 [DatabaseResilienceConsultantAgent] Analyzing Database Resilience Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Resilience Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseResilienceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseResilienceConsultantAgent = Object.freeze(
  new DatabaseResilienceConsultantAgent(),
);
