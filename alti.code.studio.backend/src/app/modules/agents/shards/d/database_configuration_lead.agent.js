...
class DatabaseConfigurationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseConfigurationSystem(objective) {
    logger.info(
      `💻 [DatabaseConfigurationLeadAgent] Analyzing Database Configuration Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Configuration Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseConfigurationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseConfigurationLeadAgent = Object.freeze(
  new DatabaseConfigurationLeadAgent(),
);
