...
class DatabaseContainerizationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseContainerizationSystem(objective) {
    logger.info(
      `💻 [DatabaseContainerizationArchitectAgent] Analyzing Database Containerization Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Containerization Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseContainerizationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseContainerizationArchitectAgent = Object.freeze(
  new DatabaseContainerizationArchitectAgent(),
);
