...
class DevOpsObservabilityTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsObservabilitySystem(objective) {
    logger.info(
      `💻 [DevOpsObservabilityTesterAgent] Analyzing DevOps Observability Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Observability Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsObservabilityTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsObservabilityTesterAgent = Object.freeze(
  new DevOpsObservabilityTesterAgent(),
);
