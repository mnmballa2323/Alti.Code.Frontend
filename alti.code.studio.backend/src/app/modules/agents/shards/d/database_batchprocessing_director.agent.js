...
class DatabaseBatchProcessingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseBatchProcessingSystem(objective) {
    logger.info(
      `💻 [DatabaseBatchProcessingDirectorAgent] Analyzing Database BatchProcessing Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database BatchProcessing Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseBatchProcessingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseBatchProcessingDirectorAgent = Object.freeze(
  new DatabaseBatchProcessingDirectorAgent(),
);
