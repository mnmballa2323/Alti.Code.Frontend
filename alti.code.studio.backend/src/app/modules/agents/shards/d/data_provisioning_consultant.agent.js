...
class DataProvisioningConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDataProvisioningSystem(objective) {
    logger.info(
      `💻 [DataProvisioningConsultantAgent] Analyzing Data Provisioning Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Provisioning Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataProvisioningConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataProvisioningConsultantAgent = Object.freeze(
  new DataProvisioningConsultantAgent(),
);
