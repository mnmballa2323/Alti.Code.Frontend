...
class DevSecOpsMigrationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsMigrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsMigrationDesignerAgent] Analyzing DevSecOps Migration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Migration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsMigrationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsMigrationDesignerAgent = Object.freeze(
  new DevSecOpsMigrationDesignerAgent(),
);
