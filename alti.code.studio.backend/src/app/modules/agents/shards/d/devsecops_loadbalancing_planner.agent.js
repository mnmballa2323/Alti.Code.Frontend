...
class DevSecOpsLoadBalancingPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsLoadBalancingSystem(objective) {
        logger.info(`💻 [DevSecOpsLoadBalancingPlannerAgent] Analyzing DevSecOps LoadBalancing Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps LoadBalancing Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsLoadBalancingPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsLoadBalancingPlannerAgent = Object.freeze(new DevSecOpsLoadBalancingPlannerAgent());
