...
class DevSecOpsArchitectureConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsArchitectureSystem(objective) {
    logger.info(
      `💻 [DevSecOpsArchitectureConsultantAgent] Analyzing DevSecOps Architecture Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Architecture Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsArchitectureConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsArchitectureConsultantAgent = Object.freeze(
  new DevSecOpsArchitectureConsultantAgent(),
);
