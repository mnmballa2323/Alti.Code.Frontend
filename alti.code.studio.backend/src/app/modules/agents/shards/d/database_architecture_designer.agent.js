...
class DatabaseArchitectureDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseArchitectureSystem(objective) {
    logger.info(
      `💻 [DatabaseArchitectureDesignerAgent] Analyzing Database Architecture Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Architecture Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseArchitectureDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseArchitectureDesignerAgent = Object.freeze(
  new DatabaseArchitectureDesignerAgent(),
);
