...
class DevSecOpsResilienceManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsResilienceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsResilienceManagerAgent] Analyzing DevSecOps Resilience Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Resilience Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsResilienceManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsResilienceManagerAgent = Object.freeze(
  new DevSecOpsResilienceManagerAgent(),
);
