...
class DataVirtualizationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDataVirtualizationSystem(objective) {
        logger.info(`💻 [DataVirtualizationManagerAgent] Analyzing Data Virtualization Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data Virtualization Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataVirtualizationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataVirtualizationManagerAgent = Object.freeze(new DataVirtualizationManagerAgent());
