...
class DevSecOpsStreamingArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsStreamingSystem(objective) {
    logger.info(
      `💻 [DevSecOpsStreamingArchitectAgent] Analyzing DevSecOps Streaming Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Streaming Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsStreamingArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsStreamingArchitectAgent = Object.freeze(
  new DevSecOpsStreamingArchitectAgent(),
);
