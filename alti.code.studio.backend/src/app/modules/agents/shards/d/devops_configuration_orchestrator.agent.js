...
class DevOpsConfigurationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsConfigurationSystem(objective) {
        logger.info(`💻 [DevOpsConfigurationOrchestratorAgent] Analyzing DevOps Configuration Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Configuration Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsConfigurationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsConfigurationOrchestratorAgent = Object.freeze(new DevOpsConfigurationOrchestratorAgent());
