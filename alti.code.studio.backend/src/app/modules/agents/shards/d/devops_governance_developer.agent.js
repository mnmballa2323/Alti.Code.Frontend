...
class DevOpsGovernanceDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsGovernanceSystem(objective) {
    logger.info(
      `💻 [DevOpsGovernanceDeveloperAgent] Analyzing DevOps Governance Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Governance Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsGovernanceDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsGovernanceDeveloperAgent = Object.freeze(
  new DevOpsGovernanceDeveloperAgent(),
);
