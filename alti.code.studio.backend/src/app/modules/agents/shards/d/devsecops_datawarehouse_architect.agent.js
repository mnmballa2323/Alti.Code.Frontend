...
class DevSecOpsDataWarehouseArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DevSecOpsDataWarehouseArchitectAgent] Analyzing DevSecOps DataWarehouse Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps DataWarehouse Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsDataWarehouseArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsDataWarehouseArchitectAgent = Object.freeze(
  new DevSecOpsDataWarehouseArchitectAgent(),
);
