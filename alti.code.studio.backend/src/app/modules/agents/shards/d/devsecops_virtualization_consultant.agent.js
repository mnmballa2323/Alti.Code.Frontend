...
class DevSecOpsVirtualizationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsVirtualizationSystem(objective) {
        logger.info(`💻 [DevSecOpsVirtualizationConsultantAgent] Analyzing DevSecOps Virtualization Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Virtualization Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsVirtualizationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsVirtualizationConsultantAgent = Object.freeze(new DevSecOpsVirtualizationConsultantAgent());
