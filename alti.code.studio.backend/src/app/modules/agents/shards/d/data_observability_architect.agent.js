...
class DataObservabilityArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDataObservabilitySystem(objective) {
    logger.info(
      `💻 [DataObservabilityArchitectAgent] Analyzing Data Observability Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Observability Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataObservabilityArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataObservabilityArchitectAgent = Object.freeze(
  new DataObservabilityArchitectAgent(),
);
