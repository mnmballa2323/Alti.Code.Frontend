...
class DatabaseResiliencePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseResilienceSystem(objective) {
    logger.info(
      `💻 [DatabaseResiliencePlannerAgent] Analyzing Database Resilience Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Resilience Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseResiliencePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseResiliencePlannerAgent = Object.freeze(
  new DatabaseResiliencePlannerAgent(),
);
