...
class DevSecOpsCachingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsCachingSystem(objective) {
    logger.info(
      `💻 [DevSecOpsCachingConsultantAgent] Analyzing DevSecOps Caching Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Caching Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsCachingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsCachingConsultantAgent = Object.freeze(
  new DevSecOpsCachingConsultantAgent(),
);
