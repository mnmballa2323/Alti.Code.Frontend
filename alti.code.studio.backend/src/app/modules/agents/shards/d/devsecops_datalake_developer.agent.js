...
class DevSecOpsDataLakeDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsDataLakeSystem(objective) {
    logger.info(
      `💻 [DevSecOpsDataLakeDeveloperAgent] Analyzing DevSecOps DataLake Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps DataLake Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsDataLakeDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsDataLakeDeveloperAgent = Object.freeze(
  new DevSecOpsDataLakeDeveloperAgent(),
);
