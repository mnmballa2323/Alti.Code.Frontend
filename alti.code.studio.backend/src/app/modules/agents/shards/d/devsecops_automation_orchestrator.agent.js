...
class DevSecOpsAutomationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsAutomationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsAutomationOrchestratorAgent] Analyzing DevSecOps Automation Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Automation Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsAutomationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsAutomationOrchestratorAgent = Object.freeze(
  new DevSecOpsAutomationOrchestratorAgent(),
);
