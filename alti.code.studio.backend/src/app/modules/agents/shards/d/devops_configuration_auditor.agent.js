...
class DevOpsConfigurationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsConfigurationSystem(objective) {
    logger.info(
      `💻 [DevOpsConfigurationAuditorAgent] Analyzing DevOps Configuration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Configuration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsConfigurationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsConfigurationAuditorAgent = Object.freeze(
  new DevOpsConfigurationAuditorAgent(),
);
