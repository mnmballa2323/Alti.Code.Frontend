...
class DatabaseBatchProcessingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseBatchProcessingSystem(objective) {
    logger.info(
      `💻 [DatabaseBatchProcessingDesignerAgent] Analyzing Database BatchProcessing Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database BatchProcessing Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseBatchProcessingDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseBatchProcessingDesignerAgent = Object.freeze(
  new DatabaseBatchProcessingDesignerAgent(),
);
