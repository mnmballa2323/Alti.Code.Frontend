...
class DevOpsLoadBalancingSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsLoadBalancingSystem(objective) {
        logger.info(`💻 [DevOpsLoadBalancingSpecialistAgent] Analyzing DevOps LoadBalancing Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps LoadBalancing Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsLoadBalancingSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsLoadBalancingSpecialistAgent = Object.freeze(new DevOpsLoadBalancingSpecialistAgent());
