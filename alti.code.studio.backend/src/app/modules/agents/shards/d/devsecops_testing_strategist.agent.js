...
class DevSecOpsTestingStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsTestingSystem(objective) {
        logger.info(`💻 [DevSecOpsTestingStrategistAgent] Analyzing DevSecOps Testing Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Testing Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsTestingStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsTestingStrategistAgent = Object.freeze(new DevSecOpsTestingStrategistAgent());
