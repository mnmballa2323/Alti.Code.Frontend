...
class DatabaseDataWarehouseManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseDataWarehouseSystem(objective) {
        logger.info(`💻 [DatabaseDataWarehouseManagerAgent] Analyzing Database DataWarehouse Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database DataWarehouse Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseDataWarehouseManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseDataWarehouseManagerAgent = Object.freeze(new DatabaseDataWarehouseManagerAgent());
