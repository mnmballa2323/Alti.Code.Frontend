...
class DevOpsObservabilityConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsObservabilitySystem(objective) {
    logger.info(
      `💻 [DevOpsObservabilityConsultantAgent] Analyzing DevOps Observability Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Observability Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsObservabilityConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsObservabilityConsultantAgent = Object.freeze(
  new DevOpsObservabilityConsultantAgent(),
);
