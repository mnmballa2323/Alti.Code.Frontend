...
class DatabaseObservabilityTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseObservabilitySystem(objective) {
        logger.info(`💻 [DatabaseObservabilityTesterAgent] Analyzing Database Observability Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Observability Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseObservabilityTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseObservabilityTesterAgent = Object.freeze(new DatabaseObservabilityTesterAgent());
