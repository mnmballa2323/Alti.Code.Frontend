...
class DatabaseConfigurationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseConfigurationSystem(objective) {
    logger.info(
      `💻 [DatabaseConfigurationConsultantAgent] Analyzing Database Configuration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Configuration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseConfigurationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseConfigurationConsultantAgent = Object.freeze(
  new DatabaseConfigurationConsultantAgent(),
);
