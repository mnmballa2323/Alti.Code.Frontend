...
class DataContainerizationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDataContainerizationSystem(objective) {
        logger.info(`💻 [DataContainerizationManagerAgent] Analyzing Data Containerization Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data Containerization Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataContainerizationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataContainerizationManagerAgent = Object.freeze(new DataContainerizationManagerAgent());
