...
class DevOpsAnalyticsSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsAnalyticsSystem(objective) {
        logger.info(`💻 [DevOpsAnalyticsSpecialistAgent] Analyzing DevOps Analytics Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Analytics Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsAnalyticsSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsAnalyticsSpecialistAgent = Object.freeze(new DevOpsAnalyticsSpecialistAgent());
