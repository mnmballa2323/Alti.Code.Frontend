...
class DevSecOpsGovernanceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsGovernanceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsGovernanceDirectorAgent] Analyzing DevSecOps Governance Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Governance Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsGovernanceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsGovernanceDirectorAgent = Object.freeze(
  new DevSecOpsGovernanceDirectorAgent(),
);
