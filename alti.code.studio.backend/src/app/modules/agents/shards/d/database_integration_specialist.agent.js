...
class DatabaseIntegrationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseIntegrationSystem(objective) {
        logger.info(`💻 [DatabaseIntegrationSpecialistAgent] Analyzing Database Integration Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Integration Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseIntegrationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseIntegrationSpecialistAgent = Object.freeze(new DatabaseIntegrationSpecialistAgent());
