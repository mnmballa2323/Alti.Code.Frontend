...
class DataMicroservicesSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDataMicroservicesSystem(objective) {
    logger.info(
      `💻 [DataMicroservicesSpecialistAgent] Analyzing Data Microservices Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Microservices Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataMicroservicesSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataMicroservicesSpecialistAgent = Object.freeze(
  new DataMicroservicesSpecialistAgent(),
);
