...
class DatabaseCachingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseCachingSystem(objective) {
    logger.info(
      `💻 [DatabaseCachingConsultantAgent] Analyzing Database Caching Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Caching Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseCachingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseCachingConsultantAgent = Object.freeze(
  new DatabaseCachingConsultantAgent(),
);
