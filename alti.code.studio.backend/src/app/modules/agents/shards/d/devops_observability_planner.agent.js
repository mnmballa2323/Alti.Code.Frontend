...
class DevOpsObservabilityPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsObservabilitySystem(objective) {
        logger.info(`💻 [DevOpsObservabilityPlannerAgent] Analyzing DevOps Observability Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Observability Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsObservabilityPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsObservabilityPlannerAgent = Object.freeze(new DevOpsObservabilityPlannerAgent());
