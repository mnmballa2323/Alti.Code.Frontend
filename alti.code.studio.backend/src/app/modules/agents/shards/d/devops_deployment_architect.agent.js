...
class DevOpsDeploymentArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsDeploymentSystem(objective) {
    logger.info(
      `💻 [DevOpsDeploymentArchitectAgent] Analyzing DevOps Deployment Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Deployment Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsDeploymentArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsDeploymentArchitectAgent = Object.freeze(
  new DevOpsDeploymentArchitectAgent(),
);
