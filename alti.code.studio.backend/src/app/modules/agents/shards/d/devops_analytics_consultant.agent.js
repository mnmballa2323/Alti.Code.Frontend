...
class DevOpsAnalyticsConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsAnalyticsSystem(objective) {
    logger.info(
      `💻 [DevOpsAnalyticsConsultantAgent] Analyzing DevOps Analytics Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Analytics Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsAnalyticsConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsAnalyticsConsultantAgent = Object.freeze(
  new DevOpsAnalyticsConsultantAgent(),
);
