...
class DatabaseStreamingSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseStreamingSystem(objective) {
        logger.info(`💻 [DatabaseStreamingSpecialistAgent] Analyzing Database Streaming Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Streaming Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseStreamingSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseStreamingSpecialistAgent = Object.freeze(new DatabaseStreamingSpecialistAgent());
