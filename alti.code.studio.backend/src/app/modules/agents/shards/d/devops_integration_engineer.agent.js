...
class DevOpsIntegrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsIntegrationSystem(objective) {
    logger.info(
      `💻 [DevOpsIntegrationEngineerAgent] Analyzing DevOps Integration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Integration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsIntegrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsIntegrationEngineerAgent = Object.freeze(
  new DevOpsIntegrationEngineerAgent(),
);
