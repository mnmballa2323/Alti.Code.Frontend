...
class DevSecOpsSecurityStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsSecuritySystem(objective) {
    logger.info(
      `💻 [DevSecOpsSecurityStrategistAgent] Analyzing DevSecOps Security Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Security Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsSecurityStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsSecurityStrategistAgent = Object.freeze(
  new DevSecOpsSecurityStrategistAgent(),
);
