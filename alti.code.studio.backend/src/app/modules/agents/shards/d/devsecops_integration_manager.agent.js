...
class DevSecOpsIntegrationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsIntegrationSystem(objective) {
        logger.info(`💻 [DevSecOpsIntegrationManagerAgent] Analyzing DevSecOps Integration Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Integration Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsIntegrationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsIntegrationManagerAgent = Object.freeze(new DevSecOpsIntegrationManagerAgent());
