...
class DevSecOpsTestingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsTestingSystem(objective) {
    logger.info(
      `💻 [DevSecOpsTestingOrchestratorAgent] Analyzing DevSecOps Testing Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Testing Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsTestingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsTestingOrchestratorAgent = Object.freeze(
  new DevSecOpsTestingOrchestratorAgent(),
);
