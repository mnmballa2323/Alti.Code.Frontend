...
class DataVirtualizationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateDataVirtualizationSystem(objective) {
        logger.info(`💻 [DataVirtualizationAnalystAgent] Analyzing Data Virtualization Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data Virtualization Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataVirtualizationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataVirtualizationAnalystAgent = Object.freeze(new DataVirtualizationAnalystAgent());
