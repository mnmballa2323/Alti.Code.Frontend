...
class DatabaseOptimizationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseOptimizationSystem(objective) {
        logger.info(`💻 [DatabaseOptimizationTesterAgent] Analyzing Database Optimization Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Optimization Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseOptimizationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseOptimizationTesterAgent = Object.freeze(new DatabaseOptimizationTesterAgent());
