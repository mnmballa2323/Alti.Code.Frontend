...
class DevSecOpsAutomationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsAutomationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsAutomationSpecialistAgent] Analyzing DevSecOps Automation Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Automation Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsAutomationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsAutomationSpecialistAgent = Object.freeze(
  new DevSecOpsAutomationSpecialistAgent(),
);
