...
class DevOpsGovernanceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsGovernanceSystem(objective) {
        logger.info(`💻 [DevOpsGovernanceConsultantAgent] Analyzing DevOps Governance Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Governance Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsGovernanceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsGovernanceConsultantAgent = Object.freeze(new DevOpsGovernanceConsultantAgent());
