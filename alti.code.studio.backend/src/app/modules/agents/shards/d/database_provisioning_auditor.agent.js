...
class DatabaseProvisioningAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseProvisioningSystem(objective) {
    logger.info(
      `💻 [DatabaseProvisioningAuditorAgent] Analyzing Database Provisioning Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Provisioning Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseProvisioningAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseProvisioningAuditorAgent = Object.freeze(
  new DatabaseProvisioningAuditorAgent(),
);
