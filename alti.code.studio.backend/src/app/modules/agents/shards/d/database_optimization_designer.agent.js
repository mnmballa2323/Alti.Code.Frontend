...
class DatabaseOptimizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseOptimizationSystem(objective) {
    logger.info(
      `💻 [DatabaseOptimizationDesignerAgent] Analyzing Database Optimization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Optimization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseOptimizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseOptimizationDesignerAgent = Object.freeze(
  new DatabaseOptimizationDesignerAgent(),
);
