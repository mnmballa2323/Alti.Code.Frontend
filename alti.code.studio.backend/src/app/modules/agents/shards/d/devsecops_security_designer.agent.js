...
class DevSecOpsSecurityDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsSecuritySystem(objective) {
    logger.info(
      `💻 [DevSecOpsSecurityDesignerAgent] Analyzing DevSecOps Security Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Security Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsSecurityDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsSecurityDesignerAgent = Object.freeze(
  new DevSecOpsSecurityDesignerAgent(),
);
