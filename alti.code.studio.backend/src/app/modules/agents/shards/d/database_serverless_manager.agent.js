...
class DatabaseServerlessManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseServerlessSystem(objective) {
    logger.info(
      `💻 [DatabaseServerlessManagerAgent] Analyzing Database Serverless Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Serverless Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseServerlessManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseServerlessManagerAgent = Object.freeze(
  new DatabaseServerlessManagerAgent(),
);
