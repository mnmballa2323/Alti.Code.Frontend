...
class DatabaseDataLakeArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseDataLakeSystem(objective) {
    logger.info(
      `💻 [DatabaseDataLakeArchitectAgent] Analyzing Database DataLake Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database DataLake Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseDataLakeArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseDataLakeArchitectAgent = Object.freeze(
  new DatabaseDataLakeArchitectAgent(),
);
