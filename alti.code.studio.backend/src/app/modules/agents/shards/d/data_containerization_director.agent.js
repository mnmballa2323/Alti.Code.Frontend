...
class DataContainerizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataContainerizationSystem(objective) {
    logger.info(
      `💻 [DataContainerizationDirectorAgent] Analyzing Data Containerization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Containerization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataContainerizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataContainerizationDirectorAgent = Object.freeze(
  new DataContainerizationDirectorAgent(),
);
