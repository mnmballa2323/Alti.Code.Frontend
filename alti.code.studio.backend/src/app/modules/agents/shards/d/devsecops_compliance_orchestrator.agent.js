...
class DevSecOpsComplianceOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsComplianceSystem(objective) {
    logger.info(
      `💻 [DevSecOpsComplianceOrchestratorAgent] Analyzing DevSecOps Compliance Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Compliance Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsComplianceOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsComplianceOrchestratorAgent = Object.freeze(
  new DevSecOpsComplianceOrchestratorAgent(),
);
