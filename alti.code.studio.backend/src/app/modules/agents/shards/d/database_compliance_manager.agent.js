...
class DatabaseComplianceManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseComplianceSystem(objective) {
    logger.info(
      `💻 [DatabaseComplianceManagerAgent] Analyzing Database Compliance Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Compliance Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseComplianceManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseComplianceManagerAgent = Object.freeze(
  new DatabaseComplianceManagerAgent(),
);
