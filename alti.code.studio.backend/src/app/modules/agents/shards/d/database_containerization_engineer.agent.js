...
class DatabaseContainerizationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseContainerizationSystem(objective) {
        logger.info(`💻 [DatabaseContainerizationEngineerAgent] Analyzing Database Containerization Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Containerization Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseContainerizationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseContainerizationEngineerAgent = Object.freeze(new DatabaseContainerizationEngineerAgent());
