...
class DevSecOpsContainerizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsContainerizationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsContainerizationConsultantAgent] Analyzing DevSecOps Containerization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Containerization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsContainerizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsContainerizationConsultantAgent = Object.freeze(
  new DevSecOpsContainerizationConsultantAgent(),
);
