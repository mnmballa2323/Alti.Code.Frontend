...
class DevOpsContainerizationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsContainerizationSystem(objective) {
        logger.info(`💻 [DevOpsContainerizationEngineerAgent] Analyzing DevOps Containerization Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Containerization Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsContainerizationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsContainerizationEngineerAgent = Object.freeze(new DevOpsContainerizationEngineerAgent());
