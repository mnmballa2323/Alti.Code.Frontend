...
class DevSecOpsLoadBalancingTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsLoadBalancingSystem(objective) {
        logger.info(`💻 [DevSecOpsLoadBalancingTesterAgent] Analyzing DevSecOps LoadBalancing Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps LoadBalancing Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsLoadBalancingTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsLoadBalancingTesterAgent = Object.freeze(new DevSecOpsLoadBalancingTesterAgent());
