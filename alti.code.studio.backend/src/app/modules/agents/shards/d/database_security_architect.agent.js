...
class DatabaseSecurityArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseSecuritySystem(objective) {
    logger.info(
      `💻 [DatabaseSecurityArchitectAgent] Analyzing Database Security Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Security Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseSecurityArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseSecurityArchitectAgent = Object.freeze(
  new DatabaseSecurityArchitectAgent(),
);
