...
class DevOpsDataWarehouseConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [DevOpsDataWarehouseConsultantAgent] Analyzing DevOps DataWarehouse Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps DataWarehouse Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsDataWarehouseConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsDataWarehouseConsultantAgent = Object.freeze(
  new DevOpsDataWarehouseConsultantAgent(),
);
