...
class DatabaseServerlessAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseServerlessSystem(objective) {
        logger.info(`💻 [DatabaseServerlessAnalystAgent] Analyzing Database Serverless Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Serverless Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseServerlessAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseServerlessAnalystAgent = Object.freeze(new DatabaseServerlessAnalystAgent());
