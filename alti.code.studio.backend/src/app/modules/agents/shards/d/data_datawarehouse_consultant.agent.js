...
class DataDataWarehouseConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDataDataWarehouseSystem(objective) {
        logger.info(`💻 [DataDataWarehouseConsultantAgent] Analyzing Data DataWarehouse Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data DataWarehouse Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataDataWarehouseConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataDataWarehouseConsultantAgent = Object.freeze(new DataDataWarehouseConsultantAgent());
