...
class DevSecOpsConfigurationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsConfigurationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsConfigurationAuditorAgent] Analyzing DevSecOps Configuration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Configuration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsConfigurationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsConfigurationAuditorAgent = Object.freeze(
  new DevSecOpsConfigurationAuditorAgent(),
);
