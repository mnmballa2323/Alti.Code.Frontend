...
class DatabaseResilienceDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseResilienceSystem(objective) {
        logger.info(`💻 [DatabaseResilienceDeveloperAgent] Analyzing Database Resilience Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Resilience Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseResilienceDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseResilienceDeveloperAgent = Object.freeze(new DatabaseResilienceDeveloperAgent());
