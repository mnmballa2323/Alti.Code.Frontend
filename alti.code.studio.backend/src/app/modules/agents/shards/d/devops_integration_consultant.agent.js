...
class DevOpsIntegrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsIntegrationSystem(objective) {
    logger.info(
      `💻 [DevOpsIntegrationConsultantAgent] Analyzing DevOps Integration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Integration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsIntegrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsIntegrationConsultantAgent = Object.freeze(
  new DevOpsIntegrationConsultantAgent(),
);
