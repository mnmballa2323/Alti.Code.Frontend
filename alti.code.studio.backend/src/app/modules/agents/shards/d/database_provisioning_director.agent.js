...
class DatabaseProvisioningDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseProvisioningSystem(objective) {
    logger.info(
      `💻 [DatabaseProvisioningDirectorAgent] Analyzing Database Provisioning Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Provisioning Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseProvisioningDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseProvisioningDirectorAgent = Object.freeze(
  new DatabaseProvisioningDirectorAgent(),
);
