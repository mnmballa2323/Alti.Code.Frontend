...
class DataFaultToleranceDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDataFaultToleranceSystem(objective) {
        logger.info(`💻 [DataFaultToleranceDesignerAgent] Analyzing Data FaultTolerance Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data FaultTolerance Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataFaultToleranceDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataFaultToleranceDesignerAgent = Object.freeze(new DataFaultToleranceDesignerAgent());
