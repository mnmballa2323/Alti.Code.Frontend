...
class DataOrchestrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDataOrchestrationSystem(objective) {
    logger.info(
      `💻 [DataOrchestrationSpecialistAgent] Analyzing Data Orchestration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Orchestration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataOrchestrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataOrchestrationSpecialistAgent = Object.freeze(
  new DataOrchestrationSpecialistAgent(),
);
