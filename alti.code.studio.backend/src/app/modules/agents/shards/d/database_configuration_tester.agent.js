...
class DatabaseConfigurationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseConfigurationSystem(objective) {
    logger.info(
      `💻 [DatabaseConfigurationTesterAgent] Analyzing Database Configuration Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Configuration Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseConfigurationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseConfigurationTesterAgent = Object.freeze(
  new DatabaseConfigurationTesterAgent(),
);
