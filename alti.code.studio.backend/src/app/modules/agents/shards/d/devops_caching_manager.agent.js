...
class DevOpsCachingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsCachingSystem(objective) {
    logger.info(
      `💻 [DevOpsCachingManagerAgent] Analyzing DevOps Caching Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Caching Manager Target',
      );
...
export const devOpsCachingManagerAgent = Object.freeze(
  new DevOpsCachingManagerAgent(),
);
