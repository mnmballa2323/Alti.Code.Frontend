...
class DevSecOpsVirtualizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsVirtualizationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsVirtualizationPlannerAgent] Analyzing DevSecOps Virtualization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Virtualization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsVirtualizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsVirtualizationPlannerAgent = Object.freeze(
  new DevSecOpsVirtualizationPlannerAgent(),
);
