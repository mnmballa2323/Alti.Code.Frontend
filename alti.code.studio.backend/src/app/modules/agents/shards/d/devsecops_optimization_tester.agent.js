...
class DevSecOpsOptimizationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsOptimizationSystem(objective) {
        logger.info(`💻 [DevSecOpsOptimizationTesterAgent] Analyzing DevSecOps Optimization Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Optimization Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsOptimizationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsOptimizationTesterAgent = Object.freeze(new DevSecOpsOptimizationTesterAgent());
