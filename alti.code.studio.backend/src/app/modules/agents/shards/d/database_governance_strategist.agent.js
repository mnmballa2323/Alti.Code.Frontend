...
class DatabaseGovernanceStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseGovernanceSystem(objective) {
        logger.info(`💻 [DatabaseGovernanceStrategistAgent] Analyzing Database Governance Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Governance Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseGovernanceStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseGovernanceStrategistAgent = Object.freeze(new DatabaseGovernanceStrategistAgent());
