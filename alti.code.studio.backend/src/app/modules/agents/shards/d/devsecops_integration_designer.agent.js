...
class DevSecOpsIntegrationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsIntegrationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsIntegrationDesignerAgent] Analyzing DevSecOps Integration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Integration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsIntegrationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsIntegrationDesignerAgent = Object.freeze(
  new DevSecOpsIntegrationDesignerAgent(),
);
