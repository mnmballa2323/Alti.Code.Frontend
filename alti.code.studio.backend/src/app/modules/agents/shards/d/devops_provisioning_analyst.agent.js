...
class DevOpsProvisioningAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsProvisioningSystem(objective) {
    logger.info(
      `💻 [DevOpsProvisioningAnalystAgent] Analyzing DevOps Provisioning Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Provisioning Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsProvisioningAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsProvisioningAnalystAgent = Object.freeze(
  new DevOpsProvisioningAnalystAgent(),
);
