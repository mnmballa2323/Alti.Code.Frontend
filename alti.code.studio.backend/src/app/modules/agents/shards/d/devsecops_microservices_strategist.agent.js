...
class DevSecOpsMicroservicesStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsMicroservicesSystem(objective) {
    logger.info(
      `💻 [DevSecOpsMicroservicesStrategistAgent] Analyzing DevSecOps Microservices Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Microservices Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsMicroservicesStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsMicroservicesStrategistAgent = Object.freeze(
  new DevSecOpsMicroservicesStrategistAgent(),
);
