...
class DevSecOpsAnalyticsAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsAnalyticsSystem(objective) {
        logger.info(`💻 [DevSecOpsAnalyticsAuditorAgent] Analyzing DevSecOps Analytics Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Analytics Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsAnalyticsAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsAnalyticsAuditorAgent = Object.freeze(new DevSecOpsAnalyticsAuditorAgent());
