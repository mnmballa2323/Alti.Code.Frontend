...
class DataDataWarehouseEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateDataDataWarehouseSystem(objective) {
        logger.info(`💻 [DataDataWarehouseEngineerAgent] Analyzing Data DataWarehouse Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Data DataWarehouse Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DataDataWarehouseEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const dataDataWarehouseEngineerAgent = Object.freeze(new DataDataWarehouseEngineerAgent());
