...
class DatabaseOrchestrationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseOrchestrationSystem(objective) {
    logger.info(
      `💻 [DatabaseOrchestrationAuditorAgent] Analyzing Database Orchestration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Orchestration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseOrchestrationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseOrchestrationAuditorAgent = Object.freeze(
  new DatabaseOrchestrationAuditorAgent(),
);
