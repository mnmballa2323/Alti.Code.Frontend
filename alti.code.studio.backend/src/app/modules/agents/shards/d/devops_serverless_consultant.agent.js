...
class DevOpsServerlessConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsServerlessSystem(objective) {
        logger.info(`💻 [DevOpsServerlessConsultantAgent] Analyzing DevOps Serverless Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Serverless Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsServerlessConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsServerlessConsultantAgent = Object.freeze(new DevOpsServerlessConsultantAgent());
