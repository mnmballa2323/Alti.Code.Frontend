...
class DatabaseContainerizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateDatabaseContainerizationSystem(objective) {
        logger.info(`💻 [DatabaseContainerizationDirectorAgent] Analyzing Database Containerization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Database Containerization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DatabaseContainerizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const databaseContainerizationDirectorAgent = Object.freeze(new DatabaseContainerizationDirectorAgent());
