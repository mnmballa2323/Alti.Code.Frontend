...
class DevOpsArchitectureStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevOpsArchitectureSystem(objective) {
    logger.info(
      `💻 [DevOpsArchitectureStrategistAgent] Analyzing DevOps Architecture Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevOps Architecture Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevOpsArchitectureStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devOpsArchitectureStrategistAgent = Object.freeze(
  new DevOpsArchitectureStrategistAgent(),
);
