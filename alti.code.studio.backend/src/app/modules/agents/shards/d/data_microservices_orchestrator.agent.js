...
class DataMicroservicesOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateDataMicroservicesSystem(objective) {
    logger.info(
      `💻 [DataMicroservicesOrchestratorAgent] Analyzing Data Microservices Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Data Microservices Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DataMicroservicesOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const dataMicroservicesOrchestratorAgent = Object.freeze(
  new DataMicroservicesOrchestratorAgent(),
);
