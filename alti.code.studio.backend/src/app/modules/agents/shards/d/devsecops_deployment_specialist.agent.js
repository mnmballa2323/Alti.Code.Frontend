...
class DevSecOpsDeploymentSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsDeploymentSystem(objective) {
    logger.info(
      `💻 [DevSecOpsDeploymentSpecialistAgent] Analyzing DevSecOps Deployment Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Deployment Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsDeploymentSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsDeploymentSpecialistAgent = Object.freeze(
  new DevSecOpsDeploymentSpecialistAgent(),
);
