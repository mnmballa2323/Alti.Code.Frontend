...
class DatabaseEngineeringConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateDatabaseEngineeringSystem(objective) {
    logger.info(
      `💻 [DatabaseEngineeringConsultantAgent] Analyzing Database Engineering Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Database Engineering Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DatabaseEngineeringConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const databaseEngineeringConsultantAgent = Object.freeze(
  new DatabaseEngineeringConsultantAgent(),
);
