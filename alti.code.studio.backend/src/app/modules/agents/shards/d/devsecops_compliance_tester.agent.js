...
class DevSecOpsComplianceTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsComplianceSystem(objective) {
        logger.info(`💻 [DevSecOpsComplianceTesterAgent] Analyzing DevSecOps Compliance Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Compliance Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsComplianceTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsComplianceTesterAgent = Object.freeze(new DevSecOpsComplianceTesterAgent());
