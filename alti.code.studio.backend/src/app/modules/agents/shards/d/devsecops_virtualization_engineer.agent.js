...
class DevSecOpsVirtualizationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsVirtualizationSystem(objective) {
    logger.info(
      `💻 [DevSecOpsVirtualizationEngineerAgent] Analyzing DevSecOps Virtualization Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps Virtualization Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsVirtualizationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsVirtualizationEngineerAgent = Object.freeze(
  new DevSecOpsVirtualizationEngineerAgent(),
);
