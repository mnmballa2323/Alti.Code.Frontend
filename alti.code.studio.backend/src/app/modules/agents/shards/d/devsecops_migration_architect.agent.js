...
class DevSecOpsMigrationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateDevSecOpsMigrationSystem(objective) {
        logger.info(`💻 [DevSecOpsMigrationArchitectAgent] Analyzing DevSecOps Migration Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevSecOps Migration Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevSecOpsMigrationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devSecOpsMigrationArchitectAgent = Object.freeze(new DevSecOpsMigrationArchitectAgent());
