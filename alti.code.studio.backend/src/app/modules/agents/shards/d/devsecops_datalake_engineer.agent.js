...
class DevSecOpsDataLakeEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateDevSecOpsDataLakeSystem(objective) {
    logger.info(
      `💻 [DevSecOpsDataLakeEngineerAgent] Analyzing DevSecOps DataLake Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - DevSecOps DataLake Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [DevSecOpsDataLakeEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const devSecOpsDataLakeEngineerAgent = Object.freeze(
  new DevSecOpsDataLakeEngineerAgent(),
);
