...
class DevOpsConfigurationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateDevOpsConfigurationSystem(objective) {
        logger.info(`💻 [DevOpsConfigurationDesignerAgent] Analyzing DevOps Configuration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - DevOps Configuration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [DevOpsConfigurationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const devOpsConfigurationDesignerAgent = Object.freeze(new DevOpsConfigurationDesignerAgent());
