...
class EnterpriseArchitectureLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseArchitectureSystem(objective) {
    logger.info(
      `💻 [EnterpriseArchitectureLeadAgent] Analyzing Enterprise Architecture Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Architecture Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseArchitectureLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseArchitectureLeadAgent = Object.freeze(
  new EnterpriseArchitectureLeadAgent(),
);
