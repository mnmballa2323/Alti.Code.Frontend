...
class EnterpriseConfigurationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseConfigurationSystem(objective) {
        logger.info(`💻 [EnterpriseConfigurationDeveloperAgent] Analyzing Enterprise Configuration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Configuration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseConfigurationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseConfigurationDeveloperAgent = Object.freeze(new EnterpriseConfigurationDeveloperAgent());
