...
class EnterpriseTelemetryConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseTelemetrySystem(objective) {
        logger.info(`💻 [EnterpriseTelemetryConsultantAgent] Analyzing Enterprise Telemetry Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Telemetry Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseTelemetryConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseTelemetryConsultantAgent = Object.freeze(new EnterpriseTelemetryConsultantAgent());
