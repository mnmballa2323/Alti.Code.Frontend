...
class EnterpriseAutomationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseAutomationSystem(objective) {
        logger.info(`💻 [EnterpriseAutomationArchitectAgent] Analyzing Enterprise Automation Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Automation Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseAutomationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseAutomationArchitectAgent = Object.freeze(new EnterpriseAutomationArchitectAgent());
