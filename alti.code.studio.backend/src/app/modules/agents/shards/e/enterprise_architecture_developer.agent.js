...
class EnterpriseArchitectureDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseArchitectureSystem(objective) {
        logger.info(`💻 [EnterpriseArchitectureDeveloperAgent] Analyzing Enterprise Architecture Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Architecture Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseArchitectureDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseArchitectureDeveloperAgent = Object.freeze(new EnterpriseArchitectureDeveloperAgent());
