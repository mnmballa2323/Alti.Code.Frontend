...
class EnterpriseOptimizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseOptimizationSystem(objective) {
    logger.info(
      `💻 [EnterpriseOptimizationConsultantAgent] Analyzing Enterprise Optimization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Optimization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseOptimizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseOptimizationConsultantAgent = Object.freeze(
  new EnterpriseOptimizationConsultantAgent(),
);
