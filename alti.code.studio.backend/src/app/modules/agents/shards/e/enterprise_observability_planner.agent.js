...
class EnterpriseObservabilityPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseObservabilitySystem(objective) {
    logger.info(
      `💻 [EnterpriseObservabilityPlannerAgent] Analyzing Enterprise Observability Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Observability Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseObservabilityPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseObservabilityPlannerAgent = Object.freeze(
  new EnterpriseObservabilityPlannerAgent(),
);
