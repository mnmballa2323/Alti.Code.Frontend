...
class ExoplanetClimatologistAgent extends GeminiCliBaseAgent {
    ...
    async generateExoplanetSystem(exoObjective) {
        logger.info(`🔭 [ExoplanetClimatologist] Analyzing objective for atmospheric biosignatures and spectroscopy...`);
...
${exoObjective}
        ...
        try {
            ...
            const cleanCode = output.replace(/```javascript|```typescript|```json|```python|```fortran|```/gi, '').trim();
            logger.info(`✅ [ExoplanetClimatologist] Exoplanet architecture generated successfully.`);
            return cleanCode;
        } catch (err) {
            logger.error(`❌ [ExoplanetClimatologist] Failed to generate exoplanet system: ${err.message}`);
            throw err;
        }
    }
}

export const exoplanetClimatologistAgent = Object.freeze(new ExoplanetClimatologistAgent());
