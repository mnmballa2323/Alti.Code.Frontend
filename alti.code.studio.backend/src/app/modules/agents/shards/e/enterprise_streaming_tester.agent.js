...
class EnterpriseStreamingTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseStreamingSystem(objective) {
        logger.info(`💻 [EnterpriseStreamingTesterAgent] Analyzing Enterprise Streaming Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Streaming Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseStreamingTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseStreamingTesterAgent = Object.freeze(new EnterpriseStreamingTesterAgent());
