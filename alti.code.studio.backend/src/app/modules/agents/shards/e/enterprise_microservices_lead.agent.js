...
class EnterpriseMicroservicesLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseMicroservicesSystem(objective) {
    logger.info(
      `💻 [EnterpriseMicroservicesLeadAgent] Analyzing Enterprise Microservices Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Microservices Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseMicroservicesLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseMicroservicesLeadAgent = Object.freeze(
  new EnterpriseMicroservicesLeadAgent(),
);
