...
class EnterpriseOrchestrationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseOrchestrationSystem(objective) {
    logger.info(
      `💻 [EnterpriseOrchestrationAnalystAgent] Analyzing Enterprise Orchestration Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Orchestration Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseOrchestrationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseOrchestrationAnalystAgent = Object.freeze(
  new EnterpriseOrchestrationAnalystAgent(),
);
