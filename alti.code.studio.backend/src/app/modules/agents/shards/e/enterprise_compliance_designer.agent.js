...
class EnterpriseComplianceDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseComplianceSystem(objective) {
        logger.info(`💻 [EnterpriseComplianceDesignerAgent] Analyzing Enterprise Compliance Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Compliance Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseComplianceDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseComplianceDesignerAgent = Object.freeze(new EnterpriseComplianceDesignerAgent());
