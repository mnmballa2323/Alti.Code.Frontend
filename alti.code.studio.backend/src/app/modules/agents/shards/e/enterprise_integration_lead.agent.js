...
class EnterpriseIntegrationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseIntegrationSystem(objective) {
    logger.info(
      `💻 [EnterpriseIntegrationLeadAgent] Analyzing Enterprise Integration Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Integration Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseIntegrationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseIntegrationLeadAgent = Object.freeze(
  new EnterpriseIntegrationLeadAgent(),
);
