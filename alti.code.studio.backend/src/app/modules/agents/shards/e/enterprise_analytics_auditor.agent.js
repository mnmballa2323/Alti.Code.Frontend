...
class EnterpriseAnalyticsAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseAnalyticsSystem(objective) {
    logger.info(
      `💻 [EnterpriseAnalyticsAuditorAgent] Analyzing Enterprise Analytics Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Analytics Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseAnalyticsAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseAnalyticsAuditorAgent = Object.freeze(
  new EnterpriseAnalyticsAuditorAgent(),
);
