...
class EnterpriseDataWarehouseConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [EnterpriseDataWarehouseConsultantAgent] Analyzing Enterprise DataWarehouse Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise DataWarehouse Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseDataWarehouseConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseDataWarehouseConsultantAgent = Object.freeze(
  new EnterpriseDataWarehouseConsultantAgent(),
);
