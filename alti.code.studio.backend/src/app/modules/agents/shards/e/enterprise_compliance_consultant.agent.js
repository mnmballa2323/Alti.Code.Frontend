...
class EnterpriseComplianceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseComplianceSystem(objective) {
        logger.info(`💻 [EnterpriseComplianceConsultantAgent] Analyzing Enterprise Compliance Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Compliance Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseComplianceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseComplianceConsultantAgent = Object.freeze(new EnterpriseComplianceConsultantAgent());
