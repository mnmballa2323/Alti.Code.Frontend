...
class EnterpriseStreamingDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseStreamingSystem(objective) {
        logger.info(`💻 [EnterpriseStreamingDirectorAgent] Analyzing Enterprise Streaming Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Streaming Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseStreamingDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseStreamingDirectorAgent = Object.freeze(new EnterpriseStreamingDirectorAgent());
