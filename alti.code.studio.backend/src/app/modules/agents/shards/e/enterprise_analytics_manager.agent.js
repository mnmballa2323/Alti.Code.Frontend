...
class EnterpriseAnalyticsManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseAnalyticsSystem(objective) {
    logger.info(
      `💻 [EnterpriseAnalyticsManagerAgent] Analyzing Enterprise Analytics Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Analytics Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseAnalyticsManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseAnalyticsManagerAgent = Object.freeze(
  new EnterpriseAnalyticsManagerAgent(),
);
