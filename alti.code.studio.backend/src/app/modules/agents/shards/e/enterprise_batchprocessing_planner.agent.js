...
class EnterpriseBatchProcessingPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseBatchProcessingSystem(objective) {
        logger.info(`💻 [EnterpriseBatchProcessingPlannerAgent] Analyzing Enterprise BatchProcessing Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise BatchProcessing Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseBatchProcessingPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseBatchProcessingPlannerAgent = Object.freeze(new EnterpriseBatchProcessingPlannerAgent());
