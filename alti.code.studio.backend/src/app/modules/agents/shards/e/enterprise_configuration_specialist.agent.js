...
class EnterpriseConfigurationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseConfigurationSystem(objective) {
    logger.info(
      `💻 [EnterpriseConfigurationSpecialistAgent] Analyzing Enterprise Configuration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Configuration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseConfigurationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseConfigurationSpecialistAgent = Object.freeze(
  new EnterpriseConfigurationSpecialistAgent(),
);
