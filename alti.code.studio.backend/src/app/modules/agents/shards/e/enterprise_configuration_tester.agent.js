...
class EnterpriseConfigurationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseConfigurationSystem(objective) {
    logger.info(
      `💻 [EnterpriseConfigurationTesterAgent] Analyzing Enterprise Configuration Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Configuration Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseConfigurationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseConfigurationTesterAgent = Object.freeze(
  new EnterpriseConfigurationTesterAgent(),
);
