...
class EnterpriseGovernanceManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseGovernanceSystem(objective) {
        logger.info(`💻 [EnterpriseGovernanceManagerAgent] Analyzing Enterprise Governance Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Governance Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseGovernanceManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseGovernanceManagerAgent = Object.freeze(new EnterpriseGovernanceManagerAgent());
