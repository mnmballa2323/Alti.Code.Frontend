...
class EnterpriseOptimizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseOptimizationSystem(objective) {
    logger.info(
      `💻 [EnterpriseOptimizationPlannerAgent] Analyzing Enterprise Optimization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Optimization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseOptimizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseOptimizationPlannerAgent = Object.freeze(
  new EnterpriseOptimizationPlannerAgent(),
);
