...
class EnterpriseArchitectureDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseArchitectureSystem(objective) {
        logger.info(`💻 [EnterpriseArchitectureDirectorAgent] Analyzing Enterprise Architecture Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Architecture Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseArchitectureDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseArchitectureDirectorAgent = Object.freeze(new EnterpriseArchitectureDirectorAgent());
