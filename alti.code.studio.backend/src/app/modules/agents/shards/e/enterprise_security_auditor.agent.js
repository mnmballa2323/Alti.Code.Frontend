...
class EnterpriseSecurityAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseSecuritySystem(objective) {
    logger.info(
      `💻 [EnterpriseSecurityAuditorAgent] Analyzing Enterprise Security Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Security Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseSecurityAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseSecurityAuditorAgent = Object.freeze(
  new EnterpriseSecurityAuditorAgent(),
);
