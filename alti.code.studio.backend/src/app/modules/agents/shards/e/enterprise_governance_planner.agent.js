...
class EnterpriseGovernancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseGovernanceSystem(objective) {
    logger.info(
      `💻 [EnterpriseGovernancePlannerAgent] Analyzing Enterprise Governance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Governance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseGovernancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseGovernancePlannerAgent = Object.freeze(
  new EnterpriseGovernancePlannerAgent(),
);
