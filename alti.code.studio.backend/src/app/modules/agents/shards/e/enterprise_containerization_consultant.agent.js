...
class EnterpriseContainerizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseContainerizationSystem(objective) {
    logger.info(
      `💻 [EnterpriseContainerizationConsultantAgent] Analyzing Enterprise Containerization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Containerization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseContainerizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseContainerizationConsultantAgent = Object.freeze(
  new EnterpriseContainerizationConsultantAgent(),
);
