...
class EnterpriseIntegrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseIntegrationSystem(objective) {
    logger.info(
      `💻 [EnterpriseIntegrationEngineerAgent] Analyzing Enterprise Integration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Integration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseIntegrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseIntegrationEngineerAgent = Object.freeze(
  new EnterpriseIntegrationEngineerAgent(),
);
