...
class EnterpriseIntegrationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseIntegrationSystem(objective) {
        logger.info(`💻 [EnterpriseIntegrationTesterAgent] Analyzing Enterprise Integration Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Integration Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseIntegrationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseIntegrationTesterAgent = Object.freeze(new EnterpriseIntegrationTesterAgent());
