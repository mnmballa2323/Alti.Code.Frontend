...
class EnterpriseETLOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseETLSystem(objective) {
        logger.info(`💻 [EnterpriseETLOrchestratorAgent] Analyzing Enterprise ETL Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise ETL Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseETLOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseETLOrchestratorAgent = Object.freeze(new EnterpriseETLOrchestratorAgent());
