...
class EnterpriseProvisioningLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseProvisioningSystem(objective) {
    logger.info(
      `💻 [EnterpriseProvisioningLeadAgent] Analyzing Enterprise Provisioning Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Provisioning Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseProvisioningLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseProvisioningLeadAgent = Object.freeze(
  new EnterpriseProvisioningLeadAgent(),
);
