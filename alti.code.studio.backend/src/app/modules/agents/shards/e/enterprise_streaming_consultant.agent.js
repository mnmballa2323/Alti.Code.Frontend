...
class EnterpriseStreamingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseStreamingSystem(objective) {
    logger.info(
      `💻 [EnterpriseStreamingConsultantAgent] Analyzing Enterprise Streaming Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Streaming Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseStreamingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseStreamingConsultantAgent = Object.freeze(
  new EnterpriseStreamingConsultantAgent(),
);
