...
class EnterpriseVirtualizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseVirtualizationSystem(objective) {
    logger.info(
      `💻 [EnterpriseVirtualizationConsultantAgent] Analyzing Enterprise Virtualization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Virtualization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseVirtualizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseVirtualizationConsultantAgent = Object.freeze(
  new EnterpriseVirtualizationConsultantAgent(),
);
