...
class EnterpriseTestingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseTestingSystem(objective) {
    logger.info(
      `💻 [EnterpriseTestingSpecialistAgent] Analyzing Enterprise Testing Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Testing Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseTestingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseTestingSpecialistAgent = Object.freeze(
  new EnterpriseTestingSpecialistAgent(),
);
