...
class EnterpriseContainerizationLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseContainerizationSystem(objective) {
        logger.info(`💻 [EnterpriseContainerizationLeadAgent] Analyzing Enterprise Containerization Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Containerization Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseContainerizationLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseContainerizationLeadAgent = Object.freeze(new EnterpriseContainerizationLeadAgent());
