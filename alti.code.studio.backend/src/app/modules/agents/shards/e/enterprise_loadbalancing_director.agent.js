...
class EnterpriseLoadBalancingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseLoadBalancingSystem(objective) {
    logger.info(
      `💻 [EnterpriseLoadBalancingDirectorAgent] Analyzing Enterprise LoadBalancing Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise LoadBalancing Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseLoadBalancingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseLoadBalancingDirectorAgent = Object.freeze(
  new EnterpriseLoadBalancingDirectorAgent(),
);
