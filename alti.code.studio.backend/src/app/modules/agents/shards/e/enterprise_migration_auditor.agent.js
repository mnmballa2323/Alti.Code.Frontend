...
class EnterpriseMigrationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseMigrationSystem(objective) {
    logger.info(
      `💻 [EnterpriseMigrationAuditorAgent] Analyzing Enterprise Migration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Migration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseMigrationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseMigrationAuditorAgent = Object.freeze(
  new EnterpriseMigrationAuditorAgent(),
);
