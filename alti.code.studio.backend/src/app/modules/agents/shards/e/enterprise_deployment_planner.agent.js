...
class EnterpriseDeploymentPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseDeploymentSystem(objective) {
    logger.info(
      `💻 [EnterpriseDeploymentPlannerAgent] Analyzing Enterprise Deployment Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Deployment Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseDeploymentPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseDeploymentPlannerAgent = Object.freeze(
  new EnterpriseDeploymentPlannerAgent(),
);
