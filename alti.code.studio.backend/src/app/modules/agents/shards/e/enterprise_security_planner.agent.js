...
class EnterpriseSecurityPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseSecuritySystem(objective) {
    logger.info(
      `💻 [EnterpriseSecurityPlannerAgent] Analyzing Enterprise Security Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Security Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseSecurityPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseSecurityPlannerAgent = Object.freeze(
  new EnterpriseSecurityPlannerAgent(),
);
