...
class EnterpriseLoadBalancingTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseLoadBalancingSystem(objective) {
    logger.info(
      `💻 [EnterpriseLoadBalancingTesterAgent] Analyzing Enterprise LoadBalancing Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise LoadBalancing Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseLoadBalancingTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseLoadBalancingTesterAgent = Object.freeze(
  new EnterpriseLoadBalancingTesterAgent(),
);
