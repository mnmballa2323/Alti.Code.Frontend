...
class EnterpriseTestingDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseTestingSystem(objective) {
        logger.info(`💻 [EnterpriseTestingDirectorAgent] Analyzing Enterprise Testing Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Testing Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseTestingDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseTestingDirectorAgent = Object.freeze(new EnterpriseTestingDirectorAgent());
