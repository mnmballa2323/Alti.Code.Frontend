...
class EnterpriseEngineeringLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseEngineeringSystem(objective) {
    logger.info(
      `💻 [EnterpriseEngineeringLeadAgent] Analyzing Enterprise Engineering Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Engineering Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseEngineeringLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseEngineeringLeadAgent = Object.freeze(
  new EnterpriseEngineeringLeadAgent(),
);
