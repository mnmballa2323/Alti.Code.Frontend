...
class EnterpriseDataWarehouseTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [EnterpriseDataWarehouseTesterAgent] Analyzing Enterprise DataWarehouse Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise DataWarehouse Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseDataWarehouseTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseDataWarehouseTesterAgent = Object.freeze(
  new EnterpriseDataWarehouseTesterAgent(),
);
