...
class EnterpriseResiliencePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseResilienceSystem(objective) {
        logger.info(`💻 [EnterpriseResiliencePlannerAgent] Analyzing Enterprise Resilience Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Resilience Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseResiliencePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseResiliencePlannerAgent = Object.freeze(new EnterpriseResiliencePlannerAgent());
