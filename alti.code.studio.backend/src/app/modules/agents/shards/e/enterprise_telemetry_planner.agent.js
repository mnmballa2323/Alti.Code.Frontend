...
class EnterpriseTelemetryPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseTelemetrySystem(objective) {
        logger.info(`💻 [EnterpriseTelemetryPlannerAgent] Analyzing Enterprise Telemetry Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Telemetry Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseTelemetryPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseTelemetryPlannerAgent = Object.freeze(new EnterpriseTelemetryPlannerAgent());
