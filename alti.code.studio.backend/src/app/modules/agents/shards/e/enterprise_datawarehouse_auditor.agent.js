...
class EnterpriseDataWarehouseAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [EnterpriseDataWarehouseAuditorAgent] Analyzing Enterprise DataWarehouse Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise DataWarehouse Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseDataWarehouseAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseDataWarehouseAuditorAgent = Object.freeze(
  new EnterpriseDataWarehouseAuditorAgent(),
);
