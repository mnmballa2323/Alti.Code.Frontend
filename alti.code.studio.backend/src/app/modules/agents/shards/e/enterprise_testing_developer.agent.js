...
class EnterpriseTestingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseTestingSystem(objective) {
    logger.info(
      `💻 [EnterpriseTestingDeveloperAgent] Analyzing Enterprise Testing Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Testing Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseTestingDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseTestingDeveloperAgent = Object.freeze(
  new EnterpriseTestingDeveloperAgent(),
);
