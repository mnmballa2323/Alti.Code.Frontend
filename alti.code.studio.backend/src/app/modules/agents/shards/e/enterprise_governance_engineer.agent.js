...
class EnterpriseGovernanceEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseGovernanceSystem(objective) {
        logger.info(`💻 [EnterpriseGovernanceEngineerAgent] Analyzing Enterprise Governance Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Governance Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseGovernanceEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseGovernanceEngineerAgent = Object.freeze(new EnterpriseGovernanceEngineerAgent());
