...
class EnterpriseIntegrationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseIntegrationSystem(objective) {
        logger.info(`💻 [EnterpriseIntegrationAnalystAgent] Analyzing Enterprise Integration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Integration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseIntegrationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseIntegrationAnalystAgent = Object.freeze(new EnterpriseIntegrationAnalystAgent());
