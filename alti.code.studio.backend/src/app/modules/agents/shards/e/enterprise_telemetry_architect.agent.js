...
class EnterpriseTelemetryArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseTelemetrySystem(objective) {
        logger.info(`💻 [EnterpriseTelemetryArchitectAgent] Analyzing Enterprise Telemetry Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Telemetry Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseTelemetryArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseTelemetryArchitectAgent = Object.freeze(new EnterpriseTelemetryArchitectAgent());
