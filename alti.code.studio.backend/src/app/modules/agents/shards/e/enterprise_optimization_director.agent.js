...
class EnterpriseOptimizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseOptimizationSystem(objective) {
        logger.info(`💻 [EnterpriseOptimizationDirectorAgent] Analyzing Enterprise Optimization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Optimization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseOptimizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseOptimizationDirectorAgent = Object.freeze(new EnterpriseOptimizationDirectorAgent());
