...
class EnterpriseSecuritySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseSecuritySystem(objective) {
    logger.info(
      `💻 [EnterpriseSecuritySpecialistAgent] Analyzing Enterprise Security Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Security Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseSecuritySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseSecuritySpecialistAgent = Object.freeze(
  new EnterpriseSecuritySpecialistAgent(),
);
