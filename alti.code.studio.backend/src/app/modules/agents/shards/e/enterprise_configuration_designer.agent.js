...
class EnterpriseConfigurationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseConfigurationSystem(objective) {
        logger.info(`💻 [EnterpriseConfigurationDesignerAgent] Analyzing Enterprise Configuration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Configuration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseConfigurationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseConfigurationDesignerAgent = Object.freeze(new EnterpriseConfigurationDesignerAgent());
