...
class EnterpriseProvisioningPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseProvisioningSystem(objective) {
    logger.info(
      `💻 [EnterpriseProvisioningPlannerAgent] Analyzing Enterprise Provisioning Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Provisioning Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseProvisioningPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseProvisioningPlannerAgent = Object.freeze(
  new EnterpriseProvisioningPlannerAgent(),
);
