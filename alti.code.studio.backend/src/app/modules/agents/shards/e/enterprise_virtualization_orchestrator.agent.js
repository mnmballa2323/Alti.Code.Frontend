...
class EnterpriseVirtualizationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseVirtualizationSystem(objective) {
    logger.info(
      `💻 [EnterpriseVirtualizationOrchestratorAgent] Analyzing Enterprise Virtualization Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Virtualization Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseVirtualizationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseVirtualizationOrchestratorAgent = Object.freeze(
  new EnterpriseVirtualizationOrchestratorAgent(),
);
