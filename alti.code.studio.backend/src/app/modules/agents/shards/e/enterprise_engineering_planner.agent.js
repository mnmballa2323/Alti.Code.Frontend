...
class EnterpriseEngineeringPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseEngineeringSystem(objective) {
    logger.info(
      `💻 [EnterpriseEngineeringPlannerAgent] Analyzing Enterprise Engineering Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Engineering Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseEngineeringPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseEngineeringPlannerAgent = Object.freeze(
  new EnterpriseEngineeringPlannerAgent(),
);
