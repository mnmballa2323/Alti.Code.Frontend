...
class EnterpriseConfigurationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseConfigurationSystem(objective) {
    logger.info(
      `💻 [EnterpriseConfigurationManagerAgent] Analyzing Enterprise Configuration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Configuration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseConfigurationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseConfigurationManagerAgent = Object.freeze(
  new EnterpriseConfigurationManagerAgent(),
);
