...
class ElixirOtpArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateElixirSystem(objective) {
    logger.info(
      `💧 [ElixirOtpArchitect] Analyzing GenServer supervisors and BEAM VM state...`,
    );
    ...
    try {
      ...
      return output
        .replace(/```javascript|```typescript|```json|```elixir|```/gi, '')
        .trim();
    } catch (err) {
...
export const elixirOtpArchitectAgent = Object.freeze(
  new ElixirOtpArchitectAgent(),
);
