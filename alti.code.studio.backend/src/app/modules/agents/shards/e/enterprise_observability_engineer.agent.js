...
class EnterpriseObservabilityEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseObservabilitySystem(objective) {
        logger.info(`💻 [EnterpriseObservabilityEngineerAgent] Analyzing Enterprise Observability Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Observability Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseObservabilityEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseObservabilityEngineerAgent = Object.freeze(new EnterpriseObservabilityEngineerAgent());
