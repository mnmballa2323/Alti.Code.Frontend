...
class EnterpriseResilienceDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseResilienceSystem(objective) {
    logger.info(
      `💻 [EnterpriseResilienceDeveloperAgent] Analyzing Enterprise Resilience Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Resilience Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseResilienceDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseResilienceDeveloperAgent = Object.freeze(
  new EnterpriseResilienceDeveloperAgent(),
);
