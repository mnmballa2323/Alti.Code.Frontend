...
class EnterpriseAutomationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseAutomationSystem(objective) {
    logger.info(
      `💻 [EnterpriseAutomationTesterAgent] Analyzing Enterprise Automation Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Automation Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseAutomationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseAutomationTesterAgent = Object.freeze(
  new EnterpriseAutomationTesterAgent(),
);
