...
class EnterpriseOptimizationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseOptimizationSystem(objective) {
        logger.info(`💻 [EnterpriseOptimizationStrategistAgent] Analyzing Enterprise Optimization Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Optimization Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseOptimizationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseOptimizationStrategistAgent = Object.freeze(new EnterpriseOptimizationStrategistAgent());
