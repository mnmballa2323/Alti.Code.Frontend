...
class EnterpriseGovernanceAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseGovernanceSystem(objective) {
        logger.info(`💻 [EnterpriseGovernanceAnalystAgent] Analyzing Enterprise Governance Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Governance Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseGovernanceAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseGovernanceAnalystAgent = Object.freeze(new EnterpriseGovernanceAnalystAgent());
