...
class EnterpriseArchitectureSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseArchitectureSystem(objective) {
    logger.info(
      `💻 [EnterpriseArchitectureSpecialistAgent] Analyzing Enterprise Architecture Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Architecture Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseArchitectureSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseArchitectureSpecialistAgent = Object.freeze(
  new EnterpriseArchitectureSpecialistAgent(),
);
