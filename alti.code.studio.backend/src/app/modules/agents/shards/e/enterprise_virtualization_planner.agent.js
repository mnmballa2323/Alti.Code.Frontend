...
class EnterpriseVirtualizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseVirtualizationSystem(objective) {
    logger.info(
      `💻 [EnterpriseVirtualizationPlannerAgent] Analyzing Enterprise Virtualization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Virtualization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseVirtualizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseVirtualizationPlannerAgent = Object.freeze(
  new EnterpriseVirtualizationPlannerAgent(),
);
