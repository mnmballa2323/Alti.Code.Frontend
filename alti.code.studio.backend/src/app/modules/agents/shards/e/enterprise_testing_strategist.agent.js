...
class EnterpriseTestingStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseTestingSystem(objective) {
    logger.info(
      `💻 [EnterpriseTestingStrategistAgent] Analyzing Enterprise Testing Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Testing Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseTestingStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseTestingStrategistAgent = Object.freeze(
  new EnterpriseTestingStrategistAgent(),
);
