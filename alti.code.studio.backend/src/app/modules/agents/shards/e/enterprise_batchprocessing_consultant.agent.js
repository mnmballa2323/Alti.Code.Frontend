...
class EnterpriseBatchProcessingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseBatchProcessingSystem(objective) {
    logger.info(
      `💻 [EnterpriseBatchProcessingConsultantAgent] Analyzing Enterprise BatchProcessing Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise BatchProcessing Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseBatchProcessingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseBatchProcessingConsultantAgent = Object.freeze(
  new EnterpriseBatchProcessingConsultantAgent(),
);
