...
class EnterpriseContainerizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseContainerizationSystem(objective) {
        logger.info(`💻 [EnterpriseContainerizationSpecialistAgent] Analyzing Enterprise Containerization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Containerization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseContainerizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseContainerizationSpecialistAgent = Object.freeze(new EnterpriseContainerizationSpecialistAgent());
