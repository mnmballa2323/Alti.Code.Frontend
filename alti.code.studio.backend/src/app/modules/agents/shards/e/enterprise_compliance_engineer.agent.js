...
class EnterpriseComplianceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseComplianceSystem(objective) {
    logger.info(
      `💻 [EnterpriseComplianceEngineerAgent] Analyzing Enterprise Compliance Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Compliance Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseComplianceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseComplianceEngineerAgent = Object.freeze(
  new EnterpriseComplianceEngineerAgent(),
);
