...
class EnterpriseScalingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseScalingSystem(objective) {
    logger.info(
      `💻 [EnterpriseScalingDirectorAgent] Analyzing Enterprise Scaling Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Scaling Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseScalingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseScalingDirectorAgent = Object.freeze(
  new EnterpriseScalingDirectorAgent(),
);
