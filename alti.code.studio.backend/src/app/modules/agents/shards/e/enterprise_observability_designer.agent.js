...
class EnterpriseObservabilityDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseObservabilitySystem(objective) {
    logger.info(
      `💻 [EnterpriseObservabilityDesignerAgent] Analyzing Enterprise Observability Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Observability Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseObservabilityDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseObservabilityDesignerAgent = Object.freeze(
  new EnterpriseObservabilityDesignerAgent(),
);
