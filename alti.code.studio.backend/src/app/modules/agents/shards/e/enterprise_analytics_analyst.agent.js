...
class EnterpriseAnalyticsAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseAnalyticsSystem(objective) {
    logger.info(
      `💻 [EnterpriseAnalyticsAnalystAgent] Analyzing Enterprise Analytics Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Analytics Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseAnalyticsAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseAnalyticsAnalystAgent = Object.freeze(
  new EnterpriseAnalyticsAnalystAgent(),
);
