...
class EnterpriseVirtualizationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseVirtualizationSystem(objective) {
        logger.info(`💻 [EnterpriseVirtualizationTesterAgent] Analyzing Enterprise Virtualization Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Virtualization Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseVirtualizationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseVirtualizationTesterAgent = Object.freeze(new EnterpriseVirtualizationTesterAgent());
