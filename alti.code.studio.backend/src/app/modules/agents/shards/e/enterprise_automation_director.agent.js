...
class EnterpriseAutomationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseAutomationSystem(objective) {
        logger.info(`💻 [EnterpriseAutomationDirectorAgent] Analyzing Enterprise Automation Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Automation Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseAutomationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseAutomationDirectorAgent = Object.freeze(new EnterpriseAutomationDirectorAgent());
