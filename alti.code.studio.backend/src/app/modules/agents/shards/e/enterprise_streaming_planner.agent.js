...
class EnterpriseStreamingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseStreamingSystem(objective) {
    logger.info(
      `💻 [EnterpriseStreamingPlannerAgent] Analyzing Enterprise Streaming Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Streaming Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseStreamingPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseStreamingPlannerAgent = Object.freeze(
  new EnterpriseStreamingPlannerAgent(),
);
