...
class EnterpriseEngineeringConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseEngineeringSystem(objective) {
    logger.info(
      `💻 [EnterpriseEngineeringConsultantAgent] Analyzing Enterprise Engineering Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Engineering Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseEngineeringConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseEngineeringConsultantAgent = Object.freeze(
  new EnterpriseEngineeringConsultantAgent(),
);
