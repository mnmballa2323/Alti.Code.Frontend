...
class EnterpriseEngineeringTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseEngineeringSystem(objective) {
    logger.info(
      `💻 [EnterpriseEngineeringTesterAgent] Analyzing Enterprise Engineering Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Engineering Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseEngineeringTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseEngineeringTesterAgent = Object.freeze(
  new EnterpriseEngineeringTesterAgent(),
);
