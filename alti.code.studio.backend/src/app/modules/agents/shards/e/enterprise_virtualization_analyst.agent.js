...
class EnterpriseVirtualizationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseVirtualizationSystem(objective) {
        logger.info(`💻 [EnterpriseVirtualizationAnalystAgent] Analyzing Enterprise Virtualization Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Virtualization Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseVirtualizationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseVirtualizationAnalystAgent = Object.freeze(new EnterpriseVirtualizationAnalystAgent());
