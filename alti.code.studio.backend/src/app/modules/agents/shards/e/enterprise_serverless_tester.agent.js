...
class EnterpriseServerlessTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseServerlessSystem(objective) {
    logger.info(
      `💻 [EnterpriseServerlessTesterAgent] Analyzing Enterprise Serverless Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Serverless Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseServerlessTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseServerlessTesterAgent = Object.freeze(
  new EnterpriseServerlessTesterAgent(),
);
