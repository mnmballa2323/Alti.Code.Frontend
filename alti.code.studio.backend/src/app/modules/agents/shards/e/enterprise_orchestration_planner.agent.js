...
class EnterpriseOrchestrationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseOrchestrationSystem(objective) {
        logger.info(`💻 [EnterpriseOrchestrationPlannerAgent] Analyzing Enterprise Orchestration Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Orchestration Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseOrchestrationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseOrchestrationPlannerAgent = Object.freeze(new EnterpriseOrchestrationPlannerAgent());
