...
class EnterpriseResilienceArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseResilienceSystem(objective) {
        logger.info(`💻 [EnterpriseResilienceArchitectAgent] Analyzing Enterprise Resilience Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Resilience Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseResilienceArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseResilienceArchitectAgent = Object.freeze(new EnterpriseResilienceArchitectAgent());
