...
class EnterpriseSecurityAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseSecuritySystem(objective) {
        logger.info(`💻 [EnterpriseSecurityAnalystAgent] Analyzing Enterprise Security Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Security Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseSecurityAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseSecurityAnalystAgent = Object.freeze(new EnterpriseSecurityAnalystAgent());
