...
class EnterpriseServerlessAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseServerlessSystem(objective) {
    logger.info(
      `💻 [EnterpriseServerlessAuditorAgent] Analyzing Enterprise Serverless Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Serverless Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseServerlessAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseServerlessAuditorAgent = Object.freeze(
  new EnterpriseServerlessAuditorAgent(),
);
