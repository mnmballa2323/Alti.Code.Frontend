...
class EnterpriseTestingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseTestingSystem(objective) {
    logger.info(
      `💻 [EnterpriseTestingConsultantAgent] Analyzing Enterprise Testing Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Testing Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseTestingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseTestingConsultantAgent = Object.freeze(
  new EnterpriseTestingConsultantAgent(),
);
