...
class EnterpriseCompliancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseComplianceSystem(objective) {
    logger.info(
      `💻 [EnterpriseCompliancePlannerAgent] Analyzing Enterprise Compliance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Compliance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseCompliancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseCompliancePlannerAgent = Object.freeze(
  new EnterpriseCompliancePlannerAgent(),
);
