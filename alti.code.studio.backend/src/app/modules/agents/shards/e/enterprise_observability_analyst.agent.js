...
class EnterpriseObservabilityAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseObservabilitySystem(objective) {
    logger.info(
      `💻 [EnterpriseObservabilityAnalystAgent] Analyzing Enterprise Observability Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Observability Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseObservabilityAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseObservabilityAnalystAgent = Object.freeze(
  new EnterpriseObservabilityAnalystAgent(),
);
