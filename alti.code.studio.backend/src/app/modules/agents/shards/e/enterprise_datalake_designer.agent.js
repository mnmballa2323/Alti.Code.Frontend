...
class EnterpriseDataLakeDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseDataLakeSystem(objective) {
    logger.info(
      `💻 [EnterpriseDataLakeDesignerAgent] Analyzing Enterprise DataLake Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise DataLake Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseDataLakeDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseDataLakeDesignerAgent = Object.freeze(
  new EnterpriseDataLakeDesignerAgent(),
);
