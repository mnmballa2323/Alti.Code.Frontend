...
class EnterpriseCachingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseCachingSystem(objective) {
    logger.info(
      `💻 [EnterpriseCachingSpecialistAgent] Analyzing Enterprise Caching Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Caching Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseCachingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseCachingSpecialistAgent = Object.freeze(
  new EnterpriseCachingSpecialistAgent(),
);
