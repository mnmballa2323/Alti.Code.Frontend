...
class EnterpriseArchitectureAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseArchitectureSystem(objective) {
        logger.info(`💻 [EnterpriseArchitectureAnalystAgent] Analyzing Enterprise Architecture Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Architecture Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseArchitectureAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseArchitectureAnalystAgent = Object.freeze(new EnterpriseArchitectureAnalystAgent());
