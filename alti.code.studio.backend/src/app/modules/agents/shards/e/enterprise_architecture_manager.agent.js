...
class EnterpriseArchitectureManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseArchitectureSystem(objective) {
    logger.info(
      `💻 [EnterpriseArchitectureManagerAgent] Analyzing Enterprise Architecture Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Architecture Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseArchitectureManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseArchitectureManagerAgent = Object.freeze(
  new EnterpriseArchitectureManagerAgent(),
);
