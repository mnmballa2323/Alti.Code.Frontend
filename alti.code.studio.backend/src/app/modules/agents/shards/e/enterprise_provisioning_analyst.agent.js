...
class EnterpriseProvisioningAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseProvisioningSystem(objective) {
    logger.info(
      `💻 [EnterpriseProvisioningAnalystAgent] Analyzing Enterprise Provisioning Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Provisioning Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseProvisioningAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseProvisioningAnalystAgent = Object.freeze(
  new EnterpriseProvisioningAnalystAgent(),
);
