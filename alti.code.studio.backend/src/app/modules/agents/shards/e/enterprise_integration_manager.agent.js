...
class EnterpriseIntegrationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseIntegrationSystem(objective) {
        logger.info(`💻 [EnterpriseIntegrationManagerAgent] Analyzing Enterprise Integration Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Integration Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseIntegrationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseIntegrationManagerAgent = Object.freeze(new EnterpriseIntegrationManagerAgent());
