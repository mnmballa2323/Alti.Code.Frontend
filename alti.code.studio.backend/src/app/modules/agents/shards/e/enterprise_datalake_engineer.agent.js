...
class EnterpriseDataLakeEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseDataLakeSystem(objective) {
    logger.info(
      `💻 [EnterpriseDataLakeEngineerAgent] Analyzing Enterprise DataLake Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise DataLake Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseDataLakeEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseDataLakeEngineerAgent = Object.freeze(
  new EnterpriseDataLakeEngineerAgent(),
);
