...
class EnterpriseProvisioningManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseProvisioningSystem(objective) {
        logger.info(`💻 [EnterpriseProvisioningManagerAgent] Analyzing Enterprise Provisioning Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Provisioning Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseProvisioningManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseProvisioningManagerAgent = Object.freeze(new EnterpriseProvisioningManagerAgent());
