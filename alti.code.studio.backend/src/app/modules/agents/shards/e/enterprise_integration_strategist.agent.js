...
class EnterpriseIntegrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseIntegrationSystem(objective) {
    logger.info(
      `💻 [EnterpriseIntegrationStrategistAgent] Analyzing Enterprise Integration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Integration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseIntegrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseIntegrationStrategistAgent = Object.freeze(
  new EnterpriseIntegrationStrategistAgent(),
);
