...
class EnterpriseComplianceArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseComplianceSystem(objective) {
        logger.info(`💻 [EnterpriseComplianceArchitectAgent] Analyzing Enterprise Compliance Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Compliance Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseComplianceArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseComplianceArchitectAgent = Object.freeze(new EnterpriseComplianceArchitectAgent());
