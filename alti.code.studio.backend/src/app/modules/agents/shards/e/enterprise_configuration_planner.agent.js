...
class EnterpriseConfigurationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseConfigurationSystem(objective) {
        logger.info(`💻 [EnterpriseConfigurationPlannerAgent] Analyzing Enterprise Configuration Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Configuration Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseConfigurationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseConfigurationPlannerAgent = Object.freeze(new EnterpriseConfigurationPlannerAgent());
