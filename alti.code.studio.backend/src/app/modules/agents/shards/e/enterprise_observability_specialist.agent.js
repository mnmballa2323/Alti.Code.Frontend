...
class EnterpriseObservabilitySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseObservabilitySystem(objective) {
    logger.info(
      `💻 [EnterpriseObservabilitySpecialistAgent] Analyzing Enterprise Observability Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Observability Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseObservabilitySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseObservabilitySpecialistAgent = Object.freeze(
  new EnterpriseObservabilitySpecialistAgent(),
);
