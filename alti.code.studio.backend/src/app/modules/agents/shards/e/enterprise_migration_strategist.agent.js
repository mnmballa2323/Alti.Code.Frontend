...
class EnterpriseMigrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseMigrationSystem(objective) {
    logger.info(
      `💻 [EnterpriseMigrationStrategistAgent] Analyzing Enterprise Migration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Migration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseMigrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseMigrationStrategistAgent = Object.freeze(
  new EnterpriseMigrationStrategistAgent(),
);
