...
class EnterpriseComplianceStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseComplianceSystem(objective) {
    logger.info(
      `💻 [EnterpriseComplianceStrategistAgent] Analyzing Enterprise Compliance Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Compliance Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseComplianceStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseComplianceStrategistAgent = Object.freeze(
  new EnterpriseComplianceStrategistAgent(),
);
