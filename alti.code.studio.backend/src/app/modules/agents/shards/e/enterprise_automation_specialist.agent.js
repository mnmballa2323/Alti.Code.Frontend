...
class EnterpriseAutomationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseAutomationSystem(objective) {
    logger.info(
      `💻 [EnterpriseAutomationSpecialistAgent] Analyzing Enterprise Automation Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Automation Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseAutomationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseAutomationSpecialistAgent = Object.freeze(
  new EnterpriseAutomationSpecialistAgent(),
);
