...
class EnterpriseProvisioningStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseProvisioningSystem(objective) {
        logger.info(`💻 [EnterpriseProvisioningStrategistAgent] Analyzing Enterprise Provisioning Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Provisioning Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseProvisioningStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseProvisioningStrategistAgent = Object.freeze(new EnterpriseProvisioningStrategistAgent());
