...
class EnterpriseContainerizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseContainerizationSystem(objective) {
    logger.info(
      `💻 [EnterpriseContainerizationDesignerAgent] Analyzing Enterprise Containerization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Containerization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseContainerizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseContainerizationDesignerAgent = Object.freeze(
  new EnterpriseContainerizationDesignerAgent(),
);
