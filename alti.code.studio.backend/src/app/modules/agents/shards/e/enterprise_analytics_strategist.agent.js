...
class EnterpriseAnalyticsStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseAnalyticsSystem(objective) {
    logger.info(
      `💻 [EnterpriseAnalyticsStrategistAgent] Analyzing Enterprise Analytics Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Analytics Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseAnalyticsStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseAnalyticsStrategistAgent = Object.freeze(
  new EnterpriseAnalyticsStrategistAgent(),
);
