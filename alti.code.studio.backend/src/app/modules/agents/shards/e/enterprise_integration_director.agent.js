...
class EnterpriseIntegrationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseIntegrationSystem(objective) {
    logger.info(
      `💻 [EnterpriseIntegrationDirectorAgent] Analyzing Enterprise Integration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Integration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseIntegrationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseIntegrationDirectorAgent = Object.freeze(
  new EnterpriseIntegrationDirectorAgent(),
);
