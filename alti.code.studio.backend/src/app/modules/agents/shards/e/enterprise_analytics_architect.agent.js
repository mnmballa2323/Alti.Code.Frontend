...
class EnterpriseAnalyticsArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseAnalyticsSystem(objective) {
        logger.info(`💻 [EnterpriseAnalyticsArchitectAgent] Analyzing Enterprise Analytics Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Analytics Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseAnalyticsArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseAnalyticsArchitectAgent = Object.freeze(new EnterpriseAnalyticsArchitectAgent());
