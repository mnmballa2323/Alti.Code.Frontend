...
class EnterpriseConfigurationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseConfigurationSystem(objective) {
        logger.info(`💻 [EnterpriseConfigurationAnalystAgent] Analyzing Enterprise Configuration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Configuration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseConfigurationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseConfigurationAnalystAgent = Object.freeze(new EnterpriseConfigurationAnalystAgent());
