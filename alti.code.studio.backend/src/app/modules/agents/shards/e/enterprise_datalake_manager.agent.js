...
class EnterpriseDataLakeManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseDataLakeSystem(objective) {
    logger.info(
      `💻 [EnterpriseDataLakeManagerAgent] Analyzing Enterprise DataLake Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise DataLake Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseDataLakeManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseDataLakeManagerAgent = Object.freeze(
  new EnterpriseDataLakeManagerAgent(),
);
