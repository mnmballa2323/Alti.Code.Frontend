...
class EnterpriseTelemetrySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseTelemetrySystem(objective) {
    logger.info(
      `💻 [EnterpriseTelemetrySpecialistAgent] Analyzing Enterprise Telemetry Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Telemetry Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseTelemetrySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseTelemetrySpecialistAgent = Object.freeze(
  new EnterpriseTelemetrySpecialistAgent(),
);
