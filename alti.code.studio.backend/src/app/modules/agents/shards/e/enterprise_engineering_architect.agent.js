...
class EnterpriseEngineeringArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseEngineeringSystem(objective) {
    logger.info(
      `💻 [EnterpriseEngineeringArchitectAgent] Analyzing Enterprise Engineering Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Engineering Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseEngineeringArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseEngineeringArchitectAgent = Object.freeze(
  new EnterpriseEngineeringArchitectAgent(),
);
