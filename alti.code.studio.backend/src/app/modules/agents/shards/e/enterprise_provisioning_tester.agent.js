...
class EnterpriseProvisioningTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseProvisioningSystem(objective) {
    logger.info(
      `💻 [EnterpriseProvisioningTesterAgent] Analyzing Enterprise Provisioning Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Provisioning Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseProvisioningTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseProvisioningTesterAgent = Object.freeze(
  new EnterpriseProvisioningTesterAgent(),
);
