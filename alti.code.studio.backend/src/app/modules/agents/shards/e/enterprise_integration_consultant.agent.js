...
class EnterpriseIntegrationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseIntegrationSystem(objective) {
        logger.info(`💻 [EnterpriseIntegrationConsultantAgent] Analyzing Enterprise Integration Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Integration Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseIntegrationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseIntegrationConsultantAgent = Object.freeze(new EnterpriseIntegrationConsultantAgent());
