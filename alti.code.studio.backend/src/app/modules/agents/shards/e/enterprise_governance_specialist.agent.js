...
class EnterpriseGovernanceSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseGovernanceSystem(objective) {
        logger.info(`💻 [EnterpriseGovernanceSpecialistAgent] Analyzing Enterprise Governance Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Governance Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseGovernanceSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseGovernanceSpecialistAgent = Object.freeze(new EnterpriseGovernanceSpecialistAgent());
