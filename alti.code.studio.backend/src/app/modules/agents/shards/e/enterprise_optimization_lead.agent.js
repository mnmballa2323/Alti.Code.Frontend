...
class EnterpriseOptimizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseOptimizationSystem(objective) {
    logger.info(
      `💻 [EnterpriseOptimizationLeadAgent] Analyzing Enterprise Optimization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Optimization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseOptimizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseOptimizationLeadAgent = Object.freeze(
  new EnterpriseOptimizationLeadAgent(),
);
