...
class EnterpriseObservabilityLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseObservabilitySystem(objective) {
        logger.info(`💻 [EnterpriseObservabilityLeadAgent] Analyzing Enterprise Observability Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Observability Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseObservabilityLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseObservabilityLeadAgent = Object.freeze(new EnterpriseObservabilityLeadAgent());
