...
class EnterpriseOrchestrationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseOrchestrationSystem(objective) {
        logger.info(`💻 [EnterpriseOrchestrationTesterAgent] Analyzing Enterprise Orchestration Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Orchestration Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseOrchestrationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseOrchestrationTesterAgent = Object.freeze(new EnterpriseOrchestrationTesterAgent());
