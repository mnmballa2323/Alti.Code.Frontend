...
class EnterpriseSecurityDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseSecuritySystem(objective) {
    logger.info(
      `💻 [EnterpriseSecurityDesignerAgent] Analyzing Enterprise Security Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Security Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseSecurityDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseSecurityDesignerAgent = Object.freeze(
  new EnterpriseSecurityDesignerAgent(),
);
