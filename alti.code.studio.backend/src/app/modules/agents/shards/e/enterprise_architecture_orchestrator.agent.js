...
class EnterpriseArchitectureOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseArchitectureSystem(objective) {
    logger.info(
      `💻 [EnterpriseArchitectureOrchestratorAgent] Analyzing Enterprise Architecture Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Architecture Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseArchitectureOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseArchitectureOrchestratorAgent = Object.freeze(
  new EnterpriseArchitectureOrchestratorAgent(),
);
