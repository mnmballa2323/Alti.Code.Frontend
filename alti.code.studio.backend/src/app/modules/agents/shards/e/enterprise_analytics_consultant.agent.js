...
class EnterpriseAnalyticsConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseAnalyticsSystem(objective) {
        logger.info(`💻 [EnterpriseAnalyticsConsultantAgent] Analyzing Enterprise Analytics Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Analytics Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseAnalyticsConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseAnalyticsConsultantAgent = Object.freeze(new EnterpriseAnalyticsConsultantAgent());
