...
class EnterpriseStreamingDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseStreamingSystem(objective) {
        logger.info(`💻 [EnterpriseStreamingDeveloperAgent] Analyzing Enterprise Streaming Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Streaming Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseStreamingDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseStreamingDeveloperAgent = Object.freeze(new EnterpriseStreamingDeveloperAgent());
