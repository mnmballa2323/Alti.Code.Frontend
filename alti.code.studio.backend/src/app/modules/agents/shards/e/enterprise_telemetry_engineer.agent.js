...
class EnterpriseTelemetryEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseTelemetrySystem(objective) {
        logger.info(`💻 [EnterpriseTelemetryEngineerAgent] Analyzing Enterprise Telemetry Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Telemetry Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseTelemetryEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseTelemetryEngineerAgent = Object.freeze(new EnterpriseTelemetryEngineerAgent());
