...
class EnterpriseContainerizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseContainerizationSystem(objective) {
    logger.info(
      `💻 [EnterpriseContainerizationAnalystAgent] Analyzing Enterprise Containerization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Containerization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseContainerizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseContainerizationAnalystAgent = Object.freeze(
  new EnterpriseContainerizationAnalystAgent(),
);
