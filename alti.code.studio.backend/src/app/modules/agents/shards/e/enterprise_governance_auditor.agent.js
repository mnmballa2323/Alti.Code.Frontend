...
class EnterpriseGovernanceAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseGovernanceSystem(objective) {
    logger.info(
      `💻 [EnterpriseGovernanceAuditorAgent] Analyzing Enterprise Governance Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Governance Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseGovernanceAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseGovernanceAuditorAgent = Object.freeze(
  new EnterpriseGovernanceAuditorAgent(),
);
