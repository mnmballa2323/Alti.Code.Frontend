...
class EnterpriseAnalyticsDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseAnalyticsSystem(objective) {
        logger.info(`💻 [EnterpriseAnalyticsDirectorAgent] Analyzing Enterprise Analytics Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Analytics Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseAnalyticsDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseAnalyticsDirectorAgent = Object.freeze(new EnterpriseAnalyticsDirectorAgent());
