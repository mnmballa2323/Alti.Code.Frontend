...
class EnterpriseStreamingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseStreamingSystem(objective) {
    logger.info(
      `💻 [EnterpriseStreamingDesignerAgent] Analyzing Enterprise Streaming Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Streaming Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseStreamingDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseStreamingDesignerAgent = Object.freeze(
  new EnterpriseStreamingDesignerAgent(),
);
