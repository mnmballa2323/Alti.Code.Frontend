...
class EnterpriseIntegrationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseIntegrationSystem(objective) {
        logger.info(`💻 [EnterpriseIntegrationDesignerAgent] Analyzing Enterprise Integration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Integration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseIntegrationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseIntegrationDesignerAgent = Object.freeze(new EnterpriseIntegrationDesignerAgent());
