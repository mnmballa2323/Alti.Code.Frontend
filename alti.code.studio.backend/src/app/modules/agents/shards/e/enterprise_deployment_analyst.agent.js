...
class EnterpriseDeploymentAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseDeploymentSystem(objective) {
    logger.info(
      `💻 [EnterpriseDeploymentAnalystAgent] Analyzing Enterprise Deployment Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Deployment Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseDeploymentAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseDeploymentAnalystAgent = Object.freeze(
  new EnterpriseDeploymentAnalystAgent(),
);
