...
class EnterpriseStreamingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseStreamingSystem(objective) {
    logger.info(
      `💻 [EnterpriseStreamingOrchestratorAgent] Analyzing Enterprise Streaming Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Streaming Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseStreamingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseStreamingOrchestratorAgent = Object.freeze(
  new EnterpriseStreamingOrchestratorAgent(),
);
