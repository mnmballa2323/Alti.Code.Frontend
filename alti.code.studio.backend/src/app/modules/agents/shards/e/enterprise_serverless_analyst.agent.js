...
class EnterpriseServerlessAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseServerlessSystem(objective) {
    logger.info(
      `💻 [EnterpriseServerlessAnalystAgent] Analyzing Enterprise Serverless Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Serverless Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseServerlessAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseServerlessAnalystAgent = Object.freeze(
  new EnterpriseServerlessAnalystAgent(),
);
