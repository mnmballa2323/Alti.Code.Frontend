...
class EnterpriseIntegrationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseIntegrationSystem(objective) {
        logger.info(`💻 [EnterpriseIntegrationAuditorAgent] Analyzing Enterprise Integration Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Integration Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseIntegrationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseIntegrationAuditorAgent = Object.freeze(new EnterpriseIntegrationAuditorAgent());
