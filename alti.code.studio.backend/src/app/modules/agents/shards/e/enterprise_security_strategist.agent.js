...
class EnterpriseSecurityStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseSecuritySystem(objective) {
        logger.info(`💻 [EnterpriseSecurityStrategistAgent] Analyzing Enterprise Security Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Security Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseSecurityStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseSecurityStrategistAgent = Object.freeze(new EnterpriseSecurityStrategistAgent());
