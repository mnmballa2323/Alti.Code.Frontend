...
class EnterpriseEngineeringDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseEngineeringSystem(objective) {
        logger.info(`💻 [EnterpriseEngineeringDirectorAgent] Analyzing Enterprise Engineering Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Engineering Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseEngineeringDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseEngineeringDirectorAgent = Object.freeze(new EnterpriseEngineeringDirectorAgent());
