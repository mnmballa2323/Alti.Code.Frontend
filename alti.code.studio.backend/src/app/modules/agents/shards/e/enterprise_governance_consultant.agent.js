...
class EnterpriseGovernanceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseGovernanceSystem(objective) {
    logger.info(
      `💻 [EnterpriseGovernanceConsultantAgent] Analyzing Enterprise Governance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Governance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseGovernanceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseGovernanceConsultantAgent = Object.freeze(
  new EnterpriseGovernanceConsultantAgent(),
);
