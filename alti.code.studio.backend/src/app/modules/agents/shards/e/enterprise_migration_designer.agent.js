...
class EnterpriseMigrationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseMigrationSystem(objective) {
    logger.info(
      `💻 [EnterpriseMigrationDesignerAgent] Analyzing Enterprise Migration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Migration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseMigrationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseMigrationDesignerAgent = Object.freeze(
  new EnterpriseMigrationDesignerAgent(),
);
