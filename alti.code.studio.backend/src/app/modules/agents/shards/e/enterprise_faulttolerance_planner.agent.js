...
class EnterpriseFaultTolerancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseFaultToleranceSystem(objective) {
    logger.info(
      `💻 [EnterpriseFaultTolerancePlannerAgent] Analyzing Enterprise FaultTolerance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise FaultTolerance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseFaultTolerancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseFaultTolerancePlannerAgent = Object.freeze(
  new EnterpriseFaultTolerancePlannerAgent(),
);
