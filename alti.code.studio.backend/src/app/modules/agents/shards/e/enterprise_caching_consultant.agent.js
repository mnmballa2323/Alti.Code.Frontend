...
class EnterpriseCachingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseCachingSystem(objective) {
    logger.info(
      `💻 [EnterpriseCachingConsultantAgent] Analyzing Enterprise Caching Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Caching Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseCachingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseCachingConsultantAgent = Object.freeze(
  new EnterpriseCachingConsultantAgent(),
);
