...
class EnterpriseResilienceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseResilienceSystem(objective) {
    logger.info(
      `💻 [EnterpriseResilienceConsultantAgent] Analyzing Enterprise Resilience Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Resilience Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseResilienceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseResilienceConsultantAgent = Object.freeze(
  new EnterpriseResilienceConsultantAgent(),
);
