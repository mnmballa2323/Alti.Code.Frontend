...
class EnterpriseOrchestrationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseOrchestrationSystem(objective) {
        logger.info(`💻 [EnterpriseOrchestrationSpecialistAgent] Analyzing Enterprise Orchestration Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Orchestration Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseOrchestrationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseOrchestrationSpecialistAgent = Object.freeze(new EnterpriseOrchestrationSpecialistAgent());
