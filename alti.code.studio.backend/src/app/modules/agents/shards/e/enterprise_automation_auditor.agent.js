...
class EnterpriseAutomationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseAutomationSystem(objective) {
    logger.info(
      `💻 [EnterpriseAutomationAuditorAgent] Analyzing Enterprise Automation Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Automation Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseAutomationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseAutomationAuditorAgent = Object.freeze(
  new EnterpriseAutomationAuditorAgent(),
);
