...
class EnterpriseTelemetryDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseTelemetrySystem(objective) {
    logger.info(
      `💻 [EnterpriseTelemetryDeveloperAgent] Analyzing Enterprise Telemetry Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Telemetry Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseTelemetryDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseTelemetryDeveloperAgent = Object.freeze(
  new EnterpriseTelemetryDeveloperAgent(),
);
