...
class EnterpriseTestingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseTestingSystem(objective) {
    logger.info(
      `💻 [EnterpriseTestingOrchestratorAgent] Analyzing Enterprise Testing Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Testing Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseTestingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseTestingOrchestratorAgent = Object.freeze(
  new EnterpriseTestingOrchestratorAgent(),
);
