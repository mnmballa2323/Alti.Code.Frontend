...
class EnterpriseMigrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseMigrationSystem(objective) {
    logger.info(
      `💻 [EnterpriseMigrationArchitectAgent] Analyzing Enterprise Migration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Migration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseMigrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseMigrationArchitectAgent = Object.freeze(
  new EnterpriseMigrationArchitectAgent(),
);
