...
class EnterpriseIntegrationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseIntegrationSystem(objective) {
        logger.info(`💻 [EnterpriseIntegrationOrchestratorAgent] Analyzing Enterprise Integration Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Integration Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseIntegrationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseIntegrationOrchestratorAgent = Object.freeze(new EnterpriseIntegrationOrchestratorAgent());
