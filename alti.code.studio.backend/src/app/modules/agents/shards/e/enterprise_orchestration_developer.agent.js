...
class EnterpriseOrchestrationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseOrchestrationSystem(objective) {
        logger.info(`💻 [EnterpriseOrchestrationDeveloperAgent] Analyzing Enterprise Orchestration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Orchestration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseOrchestrationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseOrchestrationDeveloperAgent = Object.freeze(new EnterpriseOrchestrationDeveloperAgent());
