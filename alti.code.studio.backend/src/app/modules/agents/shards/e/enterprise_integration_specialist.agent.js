...
class EnterpriseIntegrationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseIntegrationSystem(objective) {
        logger.info(`💻 [EnterpriseIntegrationSpecialistAgent] Analyzing Enterprise Integration Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Integration Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseIntegrationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseIntegrationSpecialistAgent = Object.freeze(new EnterpriseIntegrationSpecialistAgent());
