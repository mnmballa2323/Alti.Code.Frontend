...
class EnterpriseDeploymentArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseDeploymentSystem(objective) {
    logger.info(
      `💻 [EnterpriseDeploymentArchitectAgent] Analyzing Enterprise Deployment Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Deployment Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseDeploymentArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseDeploymentArchitectAgent = Object.freeze(
  new EnterpriseDeploymentArchitectAgent(),
);
