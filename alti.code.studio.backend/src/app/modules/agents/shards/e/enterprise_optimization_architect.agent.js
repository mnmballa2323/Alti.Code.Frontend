...
class EnterpriseOptimizationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseOptimizationSystem(objective) {
    logger.info(
      `💻 [EnterpriseOptimizationArchitectAgent] Analyzing Enterprise Optimization Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Optimization Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseOptimizationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseOptimizationArchitectAgent = Object.freeze(
  new EnterpriseOptimizationArchitectAgent(),
);
