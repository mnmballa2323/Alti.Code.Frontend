...
class EnterpriseDataWarehouseStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [EnterpriseDataWarehouseStrategistAgent] Analyzing Enterprise DataWarehouse Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise DataWarehouse Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseDataWarehouseStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseDataWarehouseStrategistAgent = Object.freeze(
  new EnterpriseDataWarehouseStrategistAgent(),
);
