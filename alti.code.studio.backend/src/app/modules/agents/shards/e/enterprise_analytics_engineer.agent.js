...
class EnterpriseAnalyticsEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseAnalyticsSystem(objective) {
    logger.info(
      `💻 [EnterpriseAnalyticsEngineerAgent] Analyzing Enterprise Analytics Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Analytics Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseAnalyticsEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseAnalyticsEngineerAgent = Object.freeze(
  new EnterpriseAnalyticsEngineerAgent(),
);
