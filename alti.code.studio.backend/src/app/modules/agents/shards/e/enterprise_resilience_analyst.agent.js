...
class EnterpriseResilienceAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseResilienceSystem(objective) {
    logger.info(
      `💻 [EnterpriseResilienceAnalystAgent] Analyzing Enterprise Resilience Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Resilience Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseResilienceAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseResilienceAnalystAgent = Object.freeze(
  new EnterpriseResilienceAnalystAgent(),
);
