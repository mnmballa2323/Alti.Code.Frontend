...
class EnterpriseArchitectureDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseArchitectureSystem(objective) {
    logger.info(
      `💻 [EnterpriseArchitectureDesignerAgent] Analyzing Enterprise Architecture Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Architecture Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseArchitectureDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseArchitectureDesignerAgent = Object.freeze(
  new EnterpriseArchitectureDesignerAgent(),
);
