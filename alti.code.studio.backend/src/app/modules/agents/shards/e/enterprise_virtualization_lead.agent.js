...
class EnterpriseVirtualizationLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseVirtualizationSystem(objective) {
        logger.info(`💻 [EnterpriseVirtualizationLeadAgent] Analyzing Enterprise Virtualization Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Virtualization Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseVirtualizationLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseVirtualizationLeadAgent = Object.freeze(new EnterpriseVirtualizationLeadAgent());
