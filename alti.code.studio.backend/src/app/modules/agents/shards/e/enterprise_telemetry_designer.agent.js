...
class EnterpriseTelemetryDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseTelemetrySystem(objective) {
    logger.info(
      `💻 [EnterpriseTelemetryDesignerAgent] Analyzing Enterprise Telemetry Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Telemetry Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseTelemetryDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseTelemetryDesignerAgent = Object.freeze(
  new EnterpriseTelemetryDesignerAgent(),
);
