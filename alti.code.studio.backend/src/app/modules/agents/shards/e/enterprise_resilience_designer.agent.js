...
class EnterpriseResilienceDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseResilienceSystem(objective) {
        logger.info(`💻 [EnterpriseResilienceDesignerAgent] Analyzing Enterprise Resilience Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Resilience Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseResilienceDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseResilienceDesignerAgent = Object.freeze(new EnterpriseResilienceDesignerAgent());
