...
class EnterpriseTelemetryStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseTelemetrySystem(objective) {
    logger.info(
      `💻 [EnterpriseTelemetryStrategistAgent] Analyzing Enterprise Telemetry Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Telemetry Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseTelemetryStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseTelemetryStrategistAgent = Object.freeze(
  new EnterpriseTelemetryStrategistAgent(),
);
