...
class EnterpriseConfigurationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseConfigurationSystem(objective) {
        logger.info(`💻 [EnterpriseConfigurationAuditorAgent] Analyzing Enterprise Configuration Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Configuration Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseConfigurationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseConfigurationAuditorAgent = Object.freeze(new EnterpriseConfigurationAuditorAgent());
