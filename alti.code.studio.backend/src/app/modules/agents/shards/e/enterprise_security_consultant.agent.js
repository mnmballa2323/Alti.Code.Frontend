...
class EnterpriseSecurityConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseSecuritySystem(objective) {
    logger.info(
      `💻 [EnterpriseSecurityConsultantAgent] Analyzing Enterprise Security Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Security Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseSecurityConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseSecurityConsultantAgent = Object.freeze(
  new EnterpriseSecurityConsultantAgent(),
);
