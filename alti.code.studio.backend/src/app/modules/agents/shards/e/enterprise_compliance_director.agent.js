...
class EnterpriseComplianceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseComplianceSystem(objective) {
    logger.info(
      `💻 [EnterpriseComplianceDirectorAgent] Analyzing Enterprise Compliance Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Compliance Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseComplianceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseComplianceDirectorAgent = Object.freeze(
  new EnterpriseComplianceDirectorAgent(),
);
