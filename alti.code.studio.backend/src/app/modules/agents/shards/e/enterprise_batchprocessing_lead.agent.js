...
class EnterpriseBatchProcessingLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseBatchProcessingSystem(objective) {
    logger.info(
      `💻 [EnterpriseBatchProcessingLeadAgent] Analyzing Enterprise BatchProcessing Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise BatchProcessing Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseBatchProcessingLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseBatchProcessingLeadAgent = Object.freeze(
  new EnterpriseBatchProcessingLeadAgent(),
);
