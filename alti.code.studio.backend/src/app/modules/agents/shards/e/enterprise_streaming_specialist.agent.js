...
class EnterpriseStreamingSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseStreamingSystem(objective) {
        logger.info(`💻 [EnterpriseStreamingSpecialistAgent] Analyzing Enterprise Streaming Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Streaming Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseStreamingSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseStreamingSpecialistAgent = Object.freeze(new EnterpriseStreamingSpecialistAgent());
