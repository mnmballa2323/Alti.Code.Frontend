...
class EnterpriseProvisioningEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseProvisioningSystem(objective) {
    logger.info(
      `💻 [EnterpriseProvisioningEngineerAgent] Analyzing Enterprise Provisioning Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Provisioning Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseProvisioningEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseProvisioningEngineerAgent = Object.freeze(
  new EnterpriseProvisioningEngineerAgent(),
);
