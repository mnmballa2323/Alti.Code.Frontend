...
class EnterpriseStreamingEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseStreamingSystem(objective) {
        logger.info(`💻 [EnterpriseStreamingEngineerAgent] Analyzing Enterprise Streaming Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Streaming Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseStreamingEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseStreamingEngineerAgent = Object.freeze(new EnterpriseStreamingEngineerAgent());
