...
class EnterpriseIntegrationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseIntegrationSystem(objective) {
    logger.info(
      `💻 [EnterpriseIntegrationPlannerAgent] Analyzing Enterprise Integration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Integration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseIntegrationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseIntegrationPlannerAgent = Object.freeze(
  new EnterpriseIntegrationPlannerAgent(),
);
