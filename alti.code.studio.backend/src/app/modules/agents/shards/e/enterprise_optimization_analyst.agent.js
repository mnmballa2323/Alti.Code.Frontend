...
class EnterpriseOptimizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseOptimizationSystem(objective) {
    logger.info(
      `💻 [EnterpriseOptimizationAnalystAgent] Analyzing Enterprise Optimization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Optimization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseOptimizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseOptimizationAnalystAgent = Object.freeze(
  new EnterpriseOptimizationAnalystAgent(),
);
