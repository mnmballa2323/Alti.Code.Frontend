...
class EnterpriseGovernanceTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseGovernanceSystem(objective) {
    logger.info(
      `💻 [EnterpriseGovernanceTesterAgent] Analyzing Enterprise Governance Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Governance Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseGovernanceTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseGovernanceTesterAgent = Object.freeze(
  new EnterpriseGovernanceTesterAgent(),
);
