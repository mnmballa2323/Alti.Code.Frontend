...
class EnterpriseContainerizationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseContainerizationSystem(objective) {
    logger.info(
      `💻 [EnterpriseContainerizationManagerAgent] Analyzing Enterprise Containerization Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Containerization Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseContainerizationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseContainerizationManagerAgent = Object.freeze(
  new EnterpriseContainerizationManagerAgent(),
);
