...
class EnterpriseMicroservicesPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseMicroservicesSystem(objective) {
        logger.info(`💻 [EnterpriseMicroservicesPlannerAgent] Analyzing Enterprise Microservices Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Microservices Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseMicroservicesPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseMicroservicesPlannerAgent = Object.freeze(new EnterpriseMicroservicesPlannerAgent());
