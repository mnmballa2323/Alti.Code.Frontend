...
class EnterpriseContainerizationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseContainerizationSystem(objective) {
    logger.info(
      `💻 [EnterpriseContainerizationArchitectAgent] Analyzing Enterprise Containerization Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Containerization Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseContainerizationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseContainerizationArchitectAgent = Object.freeze(
  new EnterpriseContainerizationArchitectAgent(),
);
