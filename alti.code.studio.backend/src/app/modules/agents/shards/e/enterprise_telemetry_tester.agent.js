...
class EnterpriseTelemetryTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseTelemetrySystem(objective) {
    logger.info(
      `💻 [EnterpriseTelemetryTesterAgent] Analyzing Enterprise Telemetry Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Telemetry Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseTelemetryTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseTelemetryTesterAgent = Object.freeze(
  new EnterpriseTelemetryTesterAgent(),
);
