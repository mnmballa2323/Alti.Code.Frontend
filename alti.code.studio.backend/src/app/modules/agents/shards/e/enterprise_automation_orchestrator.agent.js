...
class EnterpriseAutomationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseAutomationSystem(objective) {
    logger.info(
      `💻 [EnterpriseAutomationOrchestratorAgent] Analyzing Enterprise Automation Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Automation Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseAutomationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseAutomationOrchestratorAgent = Object.freeze(
  new EnterpriseAutomationOrchestratorAgent(),
);
