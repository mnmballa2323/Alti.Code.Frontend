...
class EnterpriseConfigurationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseConfigurationSystem(objective) {
        logger.info(`💻 [EnterpriseConfigurationConsultantAgent] Analyzing Enterprise Configuration Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Configuration Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseConfigurationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseConfigurationConsultantAgent = Object.freeze(new EnterpriseConfigurationConsultantAgent());
