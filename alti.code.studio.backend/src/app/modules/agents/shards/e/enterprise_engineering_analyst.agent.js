...
class EnterpriseEngineeringAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseEngineeringSystem(objective) {
        logger.info(`💻 [EnterpriseEngineeringAnalystAgent] Analyzing Enterprise Engineering Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Engineering Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseEngineeringAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseEngineeringAnalystAgent = Object.freeze(new EnterpriseEngineeringAnalystAgent());
