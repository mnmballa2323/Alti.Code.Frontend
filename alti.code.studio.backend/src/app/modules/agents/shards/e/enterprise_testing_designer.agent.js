...
class EnterpriseTestingDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseTestingSystem(objective) {
        logger.info(`💻 [EnterpriseTestingDesignerAgent] Analyzing Enterprise Testing Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Testing Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseTestingDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseTestingDesignerAgent = Object.freeze(new EnterpriseTestingDesignerAgent());
