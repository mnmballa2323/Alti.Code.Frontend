...
class EnterpriseDataLakeAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseDataLakeSystem(objective) {
        logger.info(`💻 [EnterpriseDataLakeAnalystAgent] Analyzing Enterprise DataLake Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise DataLake Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseDataLakeAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseDataLakeAnalystAgent = Object.freeze(new EnterpriseDataLakeAnalystAgent());
