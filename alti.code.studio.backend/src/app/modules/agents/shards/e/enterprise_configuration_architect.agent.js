...
class EnterpriseConfigurationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseConfigurationSystem(objective) {
    logger.info(
      `💻 [EnterpriseConfigurationArchitectAgent] Analyzing Enterprise Configuration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Configuration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseConfigurationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseConfigurationArchitectAgent = Object.freeze(
  new EnterpriseConfigurationArchitectAgent(),
);
