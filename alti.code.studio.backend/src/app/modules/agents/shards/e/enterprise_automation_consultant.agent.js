...
class EnterpriseAutomationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseAutomationSystem(objective) {
        logger.info(`💻 [EnterpriseAutomationConsultantAgent] Analyzing Enterprise Automation Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Automation Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseAutomationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseAutomationConsultantAgent = Object.freeze(new EnterpriseAutomationConsultantAgent());
