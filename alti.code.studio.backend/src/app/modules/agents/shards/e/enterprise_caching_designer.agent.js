...
class EnterpriseCachingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseCachingSystem(objective) {
    logger.info(
      `💻 [EnterpriseCachingDesignerAgent] Analyzing Enterprise Caching Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Caching Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseCachingDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseCachingDesignerAgent = Object.freeze(
  new EnterpriseCachingDesignerAgent(),
);
