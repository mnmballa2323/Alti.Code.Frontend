...
class EnterpriseOrchestrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseOrchestrationSystem(objective) {
    logger.info(
      `💻 [EnterpriseOrchestrationConsultantAgent] Analyzing Enterprise Orchestration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Orchestration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseOrchestrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseOrchestrationConsultantAgent = Object.freeze(
  new EnterpriseOrchestrationConsultantAgent(),
);
