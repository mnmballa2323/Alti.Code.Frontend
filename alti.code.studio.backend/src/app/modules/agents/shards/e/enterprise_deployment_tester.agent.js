...
class EnterpriseDeploymentTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseDeploymentSystem(objective) {
        logger.info(`💻 [EnterpriseDeploymentTesterAgent] Analyzing Enterprise Deployment Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Deployment Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseDeploymentTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseDeploymentTesterAgent = Object.freeze(new EnterpriseDeploymentTesterAgent());
