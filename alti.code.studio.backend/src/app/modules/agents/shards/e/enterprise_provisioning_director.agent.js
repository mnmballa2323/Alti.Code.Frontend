...
class EnterpriseProvisioningDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseProvisioningSystem(objective) {
        logger.info(`💻 [EnterpriseProvisioningDirectorAgent] Analyzing Enterprise Provisioning Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Provisioning Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseProvisioningDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseProvisioningDirectorAgent = Object.freeze(new EnterpriseProvisioningDirectorAgent());
