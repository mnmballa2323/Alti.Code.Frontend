...
class EnterpriseProvisioningSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseProvisioningSystem(objective) {
        logger.info(`💻 [EnterpriseProvisioningSpecialistAgent] Analyzing Enterprise Provisioning Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Provisioning Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseProvisioningSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseProvisioningSpecialistAgent = Object.freeze(new EnterpriseProvisioningSpecialistAgent());
