...
class EnterpriseEngineeringManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseEngineeringSystem(objective) {
        logger.info(`💻 [EnterpriseEngineeringManagerAgent] Analyzing Enterprise Engineering Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Engineering Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseEngineeringManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseEngineeringManagerAgent = Object.freeze(new EnterpriseEngineeringManagerAgent());
