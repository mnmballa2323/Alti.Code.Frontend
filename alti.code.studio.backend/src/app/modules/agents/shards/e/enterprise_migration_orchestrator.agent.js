...
class EnterpriseMigrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseMigrationSystem(objective) {
    logger.info(
      `💻 [EnterpriseMigrationOrchestratorAgent] Analyzing Enterprise Migration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Migration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseMigrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseMigrationOrchestratorAgent = Object.freeze(
  new EnterpriseMigrationOrchestratorAgent(),
);
