...
class EnterpriseAutomationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseAutomationSystem(objective) {
        logger.info(`💻 [EnterpriseAutomationEngineerAgent] Analyzing Enterprise Automation Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Automation Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseAutomationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseAutomationEngineerAgent = Object.freeze(new EnterpriseAutomationEngineerAgent());
