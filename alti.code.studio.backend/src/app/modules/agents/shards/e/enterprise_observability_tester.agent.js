...
class EnterpriseObservabilityTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseObservabilitySystem(objective) {
    logger.info(
      `💻 [EnterpriseObservabilityTesterAgent] Analyzing Enterprise Observability Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Observability Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseObservabilityTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseObservabilityTesterAgent = Object.freeze(
  new EnterpriseObservabilityTesterAgent(),
);
