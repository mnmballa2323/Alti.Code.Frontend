...
class EnterpriseComplianceDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseComplianceSystem(objective) {
        logger.info(`💻 [EnterpriseComplianceDeveloperAgent] Analyzing Enterprise Compliance Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Compliance Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseComplianceDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseComplianceDeveloperAgent = Object.freeze(new EnterpriseComplianceDeveloperAgent());
