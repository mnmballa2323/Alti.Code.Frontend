...
class EnterpriseEngineeringDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseEngineeringSystem(objective) {
        logger.info(`💻 [EnterpriseEngineeringDesignerAgent] Analyzing Enterprise Engineering Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Engineering Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseEngineeringDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseEngineeringDesignerAgent = Object.freeze(new EnterpriseEngineeringDesignerAgent());
