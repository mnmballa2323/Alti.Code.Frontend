...
class EnterpriseServerlessPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseServerlessSystem(objective) {
    logger.info(
      `💻 [EnterpriseServerlessPlannerAgent] Analyzing Enterprise Serverless Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Serverless Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseServerlessPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseServerlessPlannerAgent = Object.freeze(
  new EnterpriseServerlessPlannerAgent(),
);
