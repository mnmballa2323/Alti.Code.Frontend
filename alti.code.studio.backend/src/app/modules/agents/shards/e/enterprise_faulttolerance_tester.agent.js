...
class EnterpriseFaultToleranceTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseFaultToleranceSystem(objective) {
        logger.info(`💻 [EnterpriseFaultToleranceTesterAgent] Analyzing Enterprise FaultTolerance Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise FaultTolerance Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseFaultToleranceTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseFaultToleranceTesterAgent = Object.freeze(new EnterpriseFaultToleranceTesterAgent());
