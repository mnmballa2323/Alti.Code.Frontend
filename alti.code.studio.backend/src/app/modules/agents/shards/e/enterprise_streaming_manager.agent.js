...
class EnterpriseStreamingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseStreamingSystem(objective) {
    logger.info(
      `💻 [EnterpriseStreamingManagerAgent] Analyzing Enterprise Streaming Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Streaming Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseStreamingManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseStreamingManagerAgent = Object.freeze(
  new EnterpriseStreamingManagerAgent(),
);
