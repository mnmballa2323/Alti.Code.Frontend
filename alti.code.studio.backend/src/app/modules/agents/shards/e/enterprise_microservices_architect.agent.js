...
class EnterpriseMicroservicesArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseMicroservicesSystem(objective) {
        logger.info(`💻 [EnterpriseMicroservicesArchitectAgent] Analyzing Enterprise Microservices Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Microservices Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseMicroservicesArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseMicroservicesArchitectAgent = Object.freeze(new EnterpriseMicroservicesArchitectAgent());
