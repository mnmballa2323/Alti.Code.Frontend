...
class EnterpriseAnalyticsSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseAnalyticsSystem(objective) {
        logger.info(`💻 [EnterpriseAnalyticsSpecialistAgent] Analyzing Enterprise Analytics Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Analytics Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseAnalyticsSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseAnalyticsSpecialistAgent = Object.freeze(new EnterpriseAnalyticsSpecialistAgent());
