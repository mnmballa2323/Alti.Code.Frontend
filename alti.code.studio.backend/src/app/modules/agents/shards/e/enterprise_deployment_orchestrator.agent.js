...
class EnterpriseDeploymentOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseDeploymentSystem(objective) {
    logger.info(
      `💻 [EnterpriseDeploymentOrchestratorAgent] Analyzing Enterprise Deployment Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Deployment Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseDeploymentOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseDeploymentOrchestratorAgent = Object.freeze(
  new EnterpriseDeploymentOrchestratorAgent(),
);
