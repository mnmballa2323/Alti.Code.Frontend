...
class EnterpriseVirtualizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseVirtualizationSystem(objective) {
        logger.info(`💻 [EnterpriseVirtualizationDirectorAgent] Analyzing Enterprise Virtualization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Virtualization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseVirtualizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseVirtualizationDirectorAgent = Object.freeze(new EnterpriseVirtualizationDirectorAgent());
