...
class EnterpriseMicroservicesConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseMicroservicesSystem(objective) {
    logger.info(
      `💻 [EnterpriseMicroservicesConsultantAgent] Analyzing Enterprise Microservices Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Microservices Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseMicroservicesConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseMicroservicesConsultantAgent = Object.freeze(
  new EnterpriseMicroservicesConsultantAgent(),
);
