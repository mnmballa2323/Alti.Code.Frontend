...
class EnterpriseDataLakePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseDataLakeSystem(objective) {
        logger.info(`💻 [EnterpriseDataLakePlannerAgent] Analyzing Enterprise DataLake Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise DataLake Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseDataLakePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseDataLakePlannerAgent = Object.freeze(new EnterpriseDataLakePlannerAgent());
