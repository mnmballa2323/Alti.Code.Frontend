...
class EnterpriseDataWarehouseArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [EnterpriseDataWarehouseArchitectAgent] Analyzing Enterprise DataWarehouse Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise DataWarehouse Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseDataWarehouseArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseDataWarehouseArchitectAgent = Object.freeze(
  new EnterpriseDataWarehouseArchitectAgent(),
);
