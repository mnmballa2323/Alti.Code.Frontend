...
class EnterpriseAutomationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateEnterpriseAutomationSystem(objective) {
    logger.info(
      `💻 [EnterpriseAutomationPlannerAgent] Analyzing Enterprise Automation Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Enterprise Automation Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [EnterpriseAutomationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const enterpriseAutomationPlannerAgent = Object.freeze(
  new EnterpriseAutomationPlannerAgent(),
);
