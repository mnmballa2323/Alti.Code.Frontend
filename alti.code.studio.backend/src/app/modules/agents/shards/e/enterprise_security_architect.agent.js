...
class EnterpriseSecurityArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateEnterpriseSecuritySystem(objective) {
        logger.info(`💻 [EnterpriseSecurityArchitectAgent] Analyzing Enterprise Security Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Enterprise Security Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [EnterpriseSecurityArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const enterpriseSecurityArchitectAgent = Object.freeze(new EnterpriseSecurityArchitectAgent());
