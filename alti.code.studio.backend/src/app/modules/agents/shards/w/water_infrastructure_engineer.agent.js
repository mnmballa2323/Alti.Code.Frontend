...
class WaterInfrastructureEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateWaterSystem(waterObjective) {
    logger.info(
      `💧 [WaterInfrastructureEngineer] Analyzing objective for desalination fluidics and municipal water routing...`,
    );
...
${waterObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [WaterInfrastructureEngineer] Water architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [WaterInfrastructureEngineer] Failed to generate water system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const waterInfrastructureEngineerAgent = Object.freeze(
  new WaterInfrastructureEngineerAgent(),
);
