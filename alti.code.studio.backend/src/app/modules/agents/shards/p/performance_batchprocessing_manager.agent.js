...
class PerformanceBatchProcessingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceBatchProcessingSystem(objective) {
    logger.info(
      `💻 [PerformanceBatchProcessingManagerAgent] Analyzing Performance BatchProcessing Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance BatchProcessing Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceBatchProcessingManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceBatchProcessingManagerAgent = Object.freeze(
  new PerformanceBatchProcessingManagerAgent(),
);
