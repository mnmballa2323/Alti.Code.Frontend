...
class PerformanceVirtualizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceVirtualizationSystem(objective) {
    logger.info(
      `💻 [PerformanceVirtualizationAnalystAgent] Analyzing Performance Virtualization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Virtualization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceVirtualizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceVirtualizationAnalystAgent = Object.freeze(
  new PerformanceVirtualizationAnalystAgent(),
);
