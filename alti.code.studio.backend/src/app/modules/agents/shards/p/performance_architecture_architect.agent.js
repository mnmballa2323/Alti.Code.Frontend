...
class PerformanceArchitectureArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceArchitectureSystem(objective) {
    logger.info(
      `💻 [PerformanceArchitectureArchitectAgent] Analyzing Performance Architecture Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Architecture Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceArchitectureArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceArchitectureArchitectAgent = Object.freeze(
  new PerformanceArchitectureArchitectAgent(),
);
