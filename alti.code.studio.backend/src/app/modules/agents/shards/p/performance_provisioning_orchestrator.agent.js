...
class PerformanceProvisioningOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceProvisioningSystem(objective) {
    logger.info(
      `💻 [PerformanceProvisioningOrchestratorAgent] Analyzing Performance Provisioning Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Provisioning Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceProvisioningOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceProvisioningOrchestratorAgent = Object.freeze(
  new PerformanceProvisioningOrchestratorAgent(),
);
