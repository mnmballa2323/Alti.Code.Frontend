...
class PerformanceTestingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceTestingSystem(objective) {
    logger.info(
      `💻 [PerformanceTestingPlannerAgent] Analyzing Performance Testing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Testing Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceTestingPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceTestingPlannerAgent = Object.freeze(
  new PerformanceTestingPlannerAgent(),
);
