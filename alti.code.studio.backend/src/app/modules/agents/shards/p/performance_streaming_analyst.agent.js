...
class PerformanceStreamingAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceStreamingSystem(objective) {
    logger.info(
      `💻 [PerformanceStreamingAnalystAgent] Analyzing Performance Streaming Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Streaming Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceStreamingAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceStreamingAnalystAgent = Object.freeze(
  new PerformanceStreamingAnalystAgent(),
);
