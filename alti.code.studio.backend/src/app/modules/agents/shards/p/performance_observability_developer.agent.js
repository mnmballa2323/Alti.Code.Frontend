...
class PerformanceObservabilityDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceObservabilitySystem(objective) {
    logger.info(
      `💻 [PerformanceObservabilityDeveloperAgent] Analyzing Performance Observability Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Observability Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceObservabilityDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceObservabilityDeveloperAgent = Object.freeze(
  new PerformanceObservabilityDeveloperAgent(),
);
