...
class PerformanceGovernancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceGovernanceSystem(objective) {
    logger.info(
      `💻 [PerformanceGovernancePlannerAgent] Analyzing Performance Governance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Governance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceGovernancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceGovernancePlannerAgent = Object.freeze(
  new PerformanceGovernancePlannerAgent(),
);
