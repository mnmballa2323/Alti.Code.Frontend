...
class PerformanceDataWarehouseTesterAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDataWarehouseSystem(objective) {
    logger.info(
      `💻 [PerformanceDataWarehouseTesterAgent] Analyzing Performance DataWarehouse Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance DataWarehouse Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDataWarehouseTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDataWarehouseTesterAgent = Object.freeze(
  new PerformanceDataWarehouseTesterAgent(),
);
