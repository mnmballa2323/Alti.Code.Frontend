...
class PerformanceServerlessDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceServerlessSystem(objective) {
    logger.info(
      `💻 [PerformanceServerlessDirectorAgent] Analyzing Performance Serverless Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Serverless Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceServerlessDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceServerlessDirectorAgent = Object.freeze(
  new PerformanceServerlessDirectorAgent(),
);
