...
class PerformanceConfigurationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceConfigurationSystem(objective) {
        logger.info(`💻 [PerformanceConfigurationAnalystAgent] Analyzing Performance Configuration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Configuration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceConfigurationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceConfigurationAnalystAgent = Object.freeze(new PerformanceConfigurationAnalystAgent());
