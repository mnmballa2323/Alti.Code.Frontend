...
class PerformanceDataLakeAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDataLakeSystem(objective) {
    logger.info(
      `💻 [PerformanceDataLakeAnalystAgent] Analyzing Performance DataLake Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance DataLake Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDataLakeAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDataLakeAnalystAgent = Object.freeze(
  new PerformanceDataLakeAnalystAgent(),
);
