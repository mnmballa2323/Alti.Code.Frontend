...
class PerformanceEngineeringDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceEngineeringSystem(objective) {
    logger.info(
      `💻 [PerformanceEngineeringDirectorAgent] Analyzing Performance Engineering Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Engineering Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceEngineeringDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceEngineeringDirectorAgent = Object.freeze(
  new PerformanceEngineeringDirectorAgent(),
);
