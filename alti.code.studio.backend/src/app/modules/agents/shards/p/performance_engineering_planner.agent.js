...
class PerformanceEngineeringPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceEngineeringSystem(objective) {
    logger.info(
      `💻 [PerformanceEngineeringPlannerAgent] Analyzing Performance Engineering Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Engineering Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceEngineeringPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceEngineeringPlannerAgent = Object.freeze(
  new PerformanceEngineeringPlannerAgent(),
);
