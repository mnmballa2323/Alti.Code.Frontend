...
class PerformanceStreamingOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceStreamingSystem(objective) {
        logger.info(`💻 [PerformanceStreamingOrchestratorAgent] Analyzing Performance Streaming Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Streaming Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceStreamingOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceStreamingOrchestratorAgent = Object.freeze(new PerformanceStreamingOrchestratorAgent());
