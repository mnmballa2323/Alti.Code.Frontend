...
class PerformanceConfigurationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceConfigurationSystem(objective) {
        logger.info(`💻 [PerformanceConfigurationArchitectAgent] Analyzing Performance Configuration Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Configuration Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceConfigurationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceConfigurationArchitectAgent = Object.freeze(new PerformanceConfigurationArchitectAgent());
