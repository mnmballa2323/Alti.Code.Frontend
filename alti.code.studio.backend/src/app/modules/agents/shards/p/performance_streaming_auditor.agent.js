...
class PerformanceStreamingAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceStreamingSystem(objective) {
    logger.info(
      `💻 [PerformanceStreamingAuditorAgent] Analyzing Performance Streaming Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Streaming Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceStreamingAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceStreamingAuditorAgent = Object.freeze(
  new PerformanceStreamingAuditorAgent(),
);
