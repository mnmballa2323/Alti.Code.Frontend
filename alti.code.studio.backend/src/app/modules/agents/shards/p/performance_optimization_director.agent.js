...
class PerformanceOptimizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceOptimizationSystem(objective) {
        logger.info(`💻 [PerformanceOptimizationDirectorAgent] Analyzing Performance Optimization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Optimization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceOptimizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceOptimizationDirectorAgent = Object.freeze(new PerformanceOptimizationDirectorAgent());
