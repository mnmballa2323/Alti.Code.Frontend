...
class PerformanceConfigurationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceConfigurationSystem(objective) {
    logger.info(
      `💻 [PerformanceConfigurationPlannerAgent] Analyzing Performance Configuration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Configuration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceConfigurationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceConfigurationPlannerAgent = Object.freeze(
  new PerformanceConfigurationPlannerAgent(),
);
