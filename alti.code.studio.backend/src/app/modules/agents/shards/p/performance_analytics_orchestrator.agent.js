...
class PerformanceAnalyticsOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceAnalyticsSystem(objective) {
        logger.info(`💻 [PerformanceAnalyticsOrchestratorAgent] Analyzing Performance Analytics Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Analytics Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceAnalyticsOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceAnalyticsOrchestratorAgent = Object.freeze(new PerformanceAnalyticsOrchestratorAgent());
