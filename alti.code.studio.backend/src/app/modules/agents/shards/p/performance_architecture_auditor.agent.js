...
class PerformanceArchitectureAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceArchitectureSystem(objective) {
        logger.info(`💻 [PerformanceArchitectureAuditorAgent] Analyzing Performance Architecture Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Architecture Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceArchitectureAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceArchitectureAuditorAgent = Object.freeze(new PerformanceArchitectureAuditorAgent());
