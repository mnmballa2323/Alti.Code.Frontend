...
class PerformanceTelemetryTesterAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceTelemetrySystem(objective) {
    logger.info(
      `💻 [PerformanceTelemetryTesterAgent] Analyzing Performance Telemetry Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Telemetry Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceTelemetryTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceTelemetryTesterAgent = Object.freeze(
  new PerformanceTelemetryTesterAgent(),
);
