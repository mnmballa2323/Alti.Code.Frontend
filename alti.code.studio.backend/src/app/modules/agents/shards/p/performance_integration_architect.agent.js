...
class PerformanceIntegrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceIntegrationSystem(objective) {
    logger.info(
      `💻 [PerformanceIntegrationArchitectAgent] Analyzing Performance Integration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Integration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceIntegrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceIntegrationArchitectAgent = Object.freeze(
  new PerformanceIntegrationArchitectAgent(),
);
