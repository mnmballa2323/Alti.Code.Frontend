...
class PerformanceComplianceSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceComplianceSystem(objective) {
        logger.info(`💻 [PerformanceComplianceSpecialistAgent] Analyzing Performance Compliance Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Compliance Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceComplianceSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceComplianceSpecialistAgent = Object.freeze(new PerformanceComplianceSpecialistAgent());
