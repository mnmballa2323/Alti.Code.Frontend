...
class PerformanceDeploymentTesterAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDeploymentSystem(objective) {
    logger.info(
      `💻 [PerformanceDeploymentTesterAgent] Analyzing Performance Deployment Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Deployment Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDeploymentTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDeploymentTesterAgent = Object.freeze(
  new PerformanceDeploymentTesterAgent(),
);
