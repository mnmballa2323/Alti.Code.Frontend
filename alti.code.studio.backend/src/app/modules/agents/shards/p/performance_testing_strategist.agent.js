...
class PerformanceTestingStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceTestingSystem(objective) {
    logger.info(
      `💻 [PerformanceTestingStrategistAgent] Analyzing Performance Testing Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Testing Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceTestingStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceTestingStrategistAgent = Object.freeze(
  new PerformanceTestingStrategistAgent(),
);
