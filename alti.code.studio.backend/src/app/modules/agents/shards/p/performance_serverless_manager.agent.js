...
class PerformanceServerlessManagerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceServerlessSystem(objective) {
    logger.info(
      `💻 [PerformanceServerlessManagerAgent] Analyzing Performance Serverless Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Serverless Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceServerlessManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceServerlessManagerAgent = Object.freeze(
  new PerformanceServerlessManagerAgent(),
);
