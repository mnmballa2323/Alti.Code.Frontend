...
class PerformanceAnalyticsDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceAnalyticsSystem(objective) {
    logger.info(
      `💻 [PerformanceAnalyticsDesignerAgent] Analyzing Performance Analytics Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Analytics Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceAnalyticsDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceAnalyticsDesignerAgent = Object.freeze(
  new PerformanceAnalyticsDesignerAgent(),
);
