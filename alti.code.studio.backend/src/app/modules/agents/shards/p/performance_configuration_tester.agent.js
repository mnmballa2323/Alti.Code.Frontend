...
class PerformanceConfigurationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceConfigurationSystem(objective) {
    logger.info(
      `💻 [PerformanceConfigurationTesterAgent] Analyzing Performance Configuration Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Configuration Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceConfigurationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceConfigurationTesterAgent = Object.freeze(
  new PerformanceConfigurationTesterAgent(),
);
