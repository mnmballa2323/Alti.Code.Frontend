...
class PerformanceProvisioningManagerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceProvisioningSystem(objective) {
        logger.info(`💻 [PerformanceProvisioningManagerAgent] Analyzing Performance Provisioning Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Provisioning Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceProvisioningManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceProvisioningManagerAgent = Object.freeze(new PerformanceProvisioningManagerAgent());
