...
class PerformanceGovernanceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceGovernanceSystem(objective) {
    logger.info(
      `💻 [PerformanceGovernanceDirectorAgent] Analyzing Performance Governance Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Governance Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceGovernanceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceGovernanceDirectorAgent = Object.freeze(
  new PerformanceGovernanceDirectorAgent(),
);
