...
class PerformanceLoadBalancingAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceLoadBalancingSystem(objective) {
    logger.info(
      `💻 [PerformanceLoadBalancingAnalystAgent] Analyzing Performance LoadBalancing Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance LoadBalancing Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceLoadBalancingAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceLoadBalancingAnalystAgent = Object.freeze(
  new PerformanceLoadBalancingAnalystAgent(),
);
