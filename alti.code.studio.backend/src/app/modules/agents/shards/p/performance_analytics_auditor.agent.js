...
class PerformanceAnalyticsAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceAnalyticsSystem(objective) {
        logger.info(`💻 [PerformanceAnalyticsAuditorAgent] Analyzing Performance Analytics Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Analytics Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceAnalyticsAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceAnalyticsAuditorAgent = Object.freeze(new PerformanceAnalyticsAuditorAgent());
