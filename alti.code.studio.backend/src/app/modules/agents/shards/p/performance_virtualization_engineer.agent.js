...
class PerformanceVirtualizationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceVirtualizationSystem(objective) {
        logger.info(`💻 [PerformanceVirtualizationEngineerAgent] Analyzing Performance Virtualization Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Virtualization Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceVirtualizationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceVirtualizationEngineerAgent = Object.freeze(new PerformanceVirtualizationEngineerAgent());
