...
class PerformanceDataLakeManagerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDataLakeSystem(objective) {
    logger.info(
      `💻 [PerformanceDataLakeManagerAgent] Analyzing Performance DataLake Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance DataLake Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDataLakeManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDataLakeManagerAgent = Object.freeze(
  new PerformanceDataLakeManagerAgent(),
);
