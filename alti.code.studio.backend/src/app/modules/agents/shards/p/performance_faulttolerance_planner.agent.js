...
class PerformanceFaultTolerancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceFaultToleranceSystem(objective) {
    logger.info(
      `💻 [PerformanceFaultTolerancePlannerAgent] Analyzing Performance FaultTolerance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance FaultTolerance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceFaultTolerancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceFaultTolerancePlannerAgent = Object.freeze(
  new PerformanceFaultTolerancePlannerAgent(),
);
