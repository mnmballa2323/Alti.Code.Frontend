...
class PerformanceObservabilitySpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceObservabilitySystem(objective) {
        logger.info(`💻 [PerformanceObservabilitySpecialistAgent] Analyzing Performance Observability Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Observability Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceObservabilitySpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceObservabilitySpecialistAgent = Object.freeze(new PerformanceObservabilitySpecialistAgent());
