...
class PerformanceEngineeringTesterAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceEngineeringSystem(objective) {
    logger.info(
      `💻 [PerformanceEngineeringTesterAgent] Analyzing Performance Engineering Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Engineering Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceEngineeringTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceEngineeringTesterAgent = Object.freeze(
  new PerformanceEngineeringTesterAgent(),
);
