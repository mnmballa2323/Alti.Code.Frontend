...
class PerformanceServerlessAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceServerlessSystem(objective) {
    logger.info(
      `💻 [PerformanceServerlessAnalystAgent] Analyzing Performance Serverless Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Serverless Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceServerlessAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceServerlessAnalystAgent = Object.freeze(
  new PerformanceServerlessAnalystAgent(),
);
