...
class PerformanceOrchestrationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceOrchestrationSystem(objective) {
    logger.info(
      `💻 [PerformanceOrchestrationManagerAgent] Analyzing Performance Orchestration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Orchestration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceOrchestrationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceOrchestrationManagerAgent = Object.freeze(
  new PerformanceOrchestrationManagerAgent(),
);
