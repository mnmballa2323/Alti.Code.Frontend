...
class PerformanceMicroservicesStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceMicroservicesSystem(objective) {
    logger.info(
      `💻 [PerformanceMicroservicesStrategistAgent] Analyzing Performance Microservices Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Microservices Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceMicroservicesStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceMicroservicesStrategistAgent = Object.freeze(
  new PerformanceMicroservicesStrategistAgent(),
);
