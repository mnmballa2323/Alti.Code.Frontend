...
class PerformanceDataWarehouseConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDataWarehouseSystem(objective) {
    logger.info(
      `💻 [PerformanceDataWarehouseConsultantAgent] Analyzing Performance DataWarehouse Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance DataWarehouse Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDataWarehouseConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDataWarehouseConsultantAgent = Object.freeze(
  new PerformanceDataWarehouseConsultantAgent(),
);
