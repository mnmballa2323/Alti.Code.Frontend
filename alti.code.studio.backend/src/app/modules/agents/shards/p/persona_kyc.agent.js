...
class PersonaAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.description =
      'Identity verification and KYC specialist for Persona: Inquiry API (create/open/complete), embedded + hosted flows, document + selfie verifications, webhook event handling, case management, watchlist AML screening, and government ID database checks.';
...
Output production TypeScript. Store \`PERSONA_API_KEY\` and \`PERSONA_WEBHOOK_SECRET\` server-side.`;
  ...
  async consult(prompt, contextData = []) {
    logger.info(
      `🔍 Persona KYC Expert: Synthesizing identity verification logic...`,
    );
...
export const personaAgent = Object.freeze(new PersonaAgent());
