...
class PerformanceOrchestrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceOrchestrationSystem(objective) {
    logger.info(
      `💻 [PerformanceOrchestrationSpecialistAgent] Analyzing Performance Orchestration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Orchestration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceOrchestrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceOrchestrationSpecialistAgent = Object.freeze(
  new PerformanceOrchestrationSpecialistAgent(),
);
