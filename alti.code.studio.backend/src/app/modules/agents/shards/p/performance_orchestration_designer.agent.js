...
class PerformanceOrchestrationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceOrchestrationSystem(objective) {
        logger.info(`💻 [PerformanceOrchestrationDesignerAgent] Analyzing Performance Orchestration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Orchestration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceOrchestrationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceOrchestrationDesignerAgent = Object.freeze(new PerformanceOrchestrationDesignerAgent());
