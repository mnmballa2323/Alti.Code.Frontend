...
class PerformanceOptimizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceOptimizationSystem(objective) {
    logger.info(
      `💻 [PerformanceOptimizationTesterAgent] Analyzing Performance Optimization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Optimization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceOptimizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceOptimizationTesterAgent = Object.freeze(
  new PerformanceOptimizationTesterAgent(),
);
