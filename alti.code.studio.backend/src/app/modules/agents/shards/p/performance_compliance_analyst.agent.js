...
class PerformanceComplianceAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceComplianceSystem(objective) {
        logger.info(`💻 [PerformanceComplianceAnalystAgent] Analyzing Performance Compliance Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Compliance Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceComplianceAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceComplianceAnalystAgent = Object.freeze(new PerformanceComplianceAnalystAgent());
