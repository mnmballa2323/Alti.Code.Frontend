...
class PerformanceTelemetryDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceTelemetrySystem(objective) {
    logger.info(
      `💻 [PerformanceTelemetryDirectorAgent] Analyzing Performance Telemetry Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Telemetry Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceTelemetryDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceTelemetryDirectorAgent = Object.freeze(
  new PerformanceTelemetryDirectorAgent(),
);
