...
class PerformanceContainerizationLeadAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceContainerizationSystem(objective) {
        logger.info(`💻 [PerformanceContainerizationLeadAgent] Analyzing Performance Containerization Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Containerization Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceContainerizationLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceContainerizationLeadAgent = Object.freeze(new PerformanceContainerizationLeadAgent());
