...
class PerformanceAutomationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceAutomationSystem(objective) {
        logger.info(`💻 [PerformanceAutomationDeveloperAgent] Analyzing Performance Automation Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Automation Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceAutomationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceAutomationDeveloperAgent = Object.freeze(new PerformanceAutomationDeveloperAgent());
