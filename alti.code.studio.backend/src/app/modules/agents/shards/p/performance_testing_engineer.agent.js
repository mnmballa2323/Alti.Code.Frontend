...
class PerformanceTestingEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceTestingSystem(objective) {
        logger.info(`💻 [PerformanceTestingEngineerAgent] Analyzing Performance Testing Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Testing Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceTestingEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceTestingEngineerAgent = Object.freeze(new PerformanceTestingEngineerAgent());
