...
class PerformanceServerlessTesterAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceServerlessSystem(objective) {
        logger.info(`💻 [PerformanceServerlessTesterAgent] Analyzing Performance Serverless Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Serverless Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceServerlessTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceServerlessTesterAgent = Object.freeze(new PerformanceServerlessTesterAgent());
