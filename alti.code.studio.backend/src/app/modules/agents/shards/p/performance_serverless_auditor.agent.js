...
class PerformanceServerlessAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceServerlessSystem(objective) {
        logger.info(`💻 [PerformanceServerlessAuditorAgent] Analyzing Performance Serverless Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Serverless Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceServerlessAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceServerlessAuditorAgent = Object.freeze(new PerformanceServerlessAuditorAgent());
