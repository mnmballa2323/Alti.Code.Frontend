...
class PerformanceStreamingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceStreamingSystem(objective) {
    logger.info(
      `💻 [PerformanceStreamingDeveloperAgent] Analyzing Performance Streaming Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Streaming Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceStreamingDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceStreamingDeveloperAgent = Object.freeze(
  new PerformanceStreamingDeveloperAgent(),
);
