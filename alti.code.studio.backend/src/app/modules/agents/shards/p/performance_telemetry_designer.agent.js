...
class PerformanceTelemetryDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceTelemetrySystem(objective) {
    logger.info(
      `💻 [PerformanceTelemetryDesignerAgent] Analyzing Performance Telemetry Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Telemetry Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceTelemetryDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceTelemetryDesignerAgent = Object.freeze(
  new PerformanceTelemetryDesignerAgent(),
);
