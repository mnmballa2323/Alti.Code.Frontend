...
class PerformanceIntegrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceIntegrationSystem(objective) {
    logger.info(
      `💻 [PerformanceIntegrationSpecialistAgent] Analyzing Performance Integration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Integration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceIntegrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceIntegrationSpecialistAgent = Object.freeze(
  new PerformanceIntegrationSpecialistAgent(),
);
