...
class PerformanceTelemetryPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceTelemetrySystem(objective) {
    logger.info(
      `💻 [PerformanceTelemetryPlannerAgent] Analyzing Performance Telemetry Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Telemetry Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceTelemetryPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceTelemetryPlannerAgent = Object.freeze(
  new PerformanceTelemetryPlannerAgent(),
);
