...
class PerformanceMigrationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceMigrationSystem(objective) {
    logger.info(
      `💻 [PerformanceMigrationManagerAgent] Analyzing Performance Migration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Migration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceMigrationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceMigrationManagerAgent = Object.freeze(
  new PerformanceMigrationManagerAgent(),
);
