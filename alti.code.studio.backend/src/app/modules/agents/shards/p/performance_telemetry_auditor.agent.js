...
class PerformanceTelemetryAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceTelemetrySystem(objective) {
        logger.info(`💻 [PerformanceTelemetryAuditorAgent] Analyzing Performance Telemetry Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Telemetry Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceTelemetryAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceTelemetryAuditorAgent = Object.freeze(new PerformanceTelemetryAuditorAgent());
