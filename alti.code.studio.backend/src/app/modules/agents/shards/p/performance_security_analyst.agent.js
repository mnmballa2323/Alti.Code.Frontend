...
class PerformanceSecurityAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceSecuritySystem(objective) {
    logger.info(
      `💻 [PerformanceSecurityAnalystAgent] Analyzing Performance Security Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Security Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceSecurityAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceSecurityAnalystAgent = Object.freeze(
  new PerformanceSecurityAnalystAgent(),
);
