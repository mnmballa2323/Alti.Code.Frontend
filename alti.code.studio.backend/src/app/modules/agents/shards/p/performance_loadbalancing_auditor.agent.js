...
class PerformanceLoadBalancingAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceLoadBalancingSystem(objective) {
        logger.info(`💻 [PerformanceLoadBalancingAuditorAgent] Analyzing Performance LoadBalancing Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance LoadBalancing Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceLoadBalancingAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceLoadBalancingAuditorAgent = Object.freeze(new PerformanceLoadBalancingAuditorAgent());
