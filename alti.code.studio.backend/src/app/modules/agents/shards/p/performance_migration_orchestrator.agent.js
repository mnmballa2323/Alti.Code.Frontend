...
class PerformanceMigrationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceMigrationSystem(objective) {
        logger.info(`💻 [PerformanceMigrationOrchestratorAgent] Analyzing Performance Migration Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Migration Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceMigrationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceMigrationOrchestratorAgent = Object.freeze(new PerformanceMigrationOrchestratorAgent());
