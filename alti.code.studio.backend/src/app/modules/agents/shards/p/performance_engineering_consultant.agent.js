...
class PerformanceEngineeringConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceEngineeringSystem(objective) {
    logger.info(
      `💻 [PerformanceEngineeringConsultantAgent] Analyzing Performance Engineering Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Engineering Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceEngineeringConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceEngineeringConsultantAgent = Object.freeze(
  new PerformanceEngineeringConsultantAgent(),
);
