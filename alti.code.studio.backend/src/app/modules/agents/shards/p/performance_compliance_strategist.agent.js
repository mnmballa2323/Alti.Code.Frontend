...
class PerformanceComplianceStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceComplianceSystem(objective) {
        logger.info(`💻 [PerformanceComplianceStrategistAgent] Analyzing Performance Compliance Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Compliance Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceComplianceStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceComplianceStrategistAgent = Object.freeze(new PerformanceComplianceStrategistAgent());
