...
class PerformanceObservabilityAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceObservabilitySystem(objective) {
    logger.info(
      `💻 [PerformanceObservabilityAuditorAgent] Analyzing Performance Observability Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Observability Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceObservabilityAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceObservabilityAuditorAgent = Object.freeze(
  new PerformanceObservabilityAuditorAgent(),
);
