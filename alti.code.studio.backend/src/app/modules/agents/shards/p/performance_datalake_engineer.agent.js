...
class PerformanceDataLakeEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDataLakeSystem(objective) {
    logger.info(
      `💻 [PerformanceDataLakeEngineerAgent] Analyzing Performance DataLake Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance DataLake Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDataLakeEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDataLakeEngineerAgent = Object.freeze(
  new PerformanceDataLakeEngineerAgent(),
);
