...
class PerformanceTelemetryAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceTelemetrySystem(objective) {
    logger.info(
      `💻 [PerformanceTelemetryAnalystAgent] Analyzing Performance Telemetry Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Telemetry Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceTelemetryAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceTelemetryAnalystAgent = Object.freeze(
  new PerformanceTelemetryAnalystAgent(),
);
