...
class PerformanceFaultToleranceAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceFaultToleranceSystem(objective) {
        logger.info(`💻 [PerformanceFaultToleranceAnalystAgent] Analyzing Performance FaultTolerance Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance FaultTolerance Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceFaultToleranceAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceFaultToleranceAnalystAgent = Object.freeze(new PerformanceFaultToleranceAnalystAgent());
