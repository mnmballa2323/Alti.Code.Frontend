...
class PerformanceTestingDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceTestingSystem(objective) {
        logger.info(`💻 [PerformanceTestingDeveloperAgent] Analyzing Performance Testing Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Testing Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceTestingDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceTestingDeveloperAgent = Object.freeze(new PerformanceTestingDeveloperAgent());
