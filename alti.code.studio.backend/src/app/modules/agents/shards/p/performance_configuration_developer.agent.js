...
class PerformanceConfigurationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceConfigurationSystem(objective) {
    logger.info(
      `💻 [PerformanceConfigurationDeveloperAgent] Analyzing Performance Configuration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Configuration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceConfigurationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceConfigurationDeveloperAgent = Object.freeze(
  new PerformanceConfigurationDeveloperAgent(),
);
