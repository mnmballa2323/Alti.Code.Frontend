...
class PerformanceGovernanceDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceGovernanceSystem(objective) {
        logger.info(`💻 [PerformanceGovernanceDeveloperAgent] Analyzing Performance Governance Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Governance Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceGovernanceDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceGovernanceDeveloperAgent = Object.freeze(new PerformanceGovernanceDeveloperAgent());
