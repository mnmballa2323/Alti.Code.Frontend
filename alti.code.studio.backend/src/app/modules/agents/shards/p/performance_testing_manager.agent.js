...
class PerformanceTestingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceTestingSystem(objective) {
    logger.info(
      `💻 [PerformanceTestingManagerAgent] Analyzing Performance Testing Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Testing Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceTestingManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceTestingManagerAgent = Object.freeze(
  new PerformanceTestingManagerAgent(),
);
