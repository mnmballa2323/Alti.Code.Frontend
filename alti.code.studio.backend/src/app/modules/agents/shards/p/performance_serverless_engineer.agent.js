...
class PerformanceServerlessEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceServerlessSystem(objective) {
    logger.info(
      `💻 [PerformanceServerlessEngineerAgent] Analyzing Performance Serverless Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Serverless Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceServerlessEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceServerlessEngineerAgent = Object.freeze(
  new PerformanceServerlessEngineerAgent(),
);
