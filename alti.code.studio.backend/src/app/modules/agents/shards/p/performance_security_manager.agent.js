...
class PerformanceSecurityManagerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceSecuritySystem(objective) {
        logger.info(`💻 [PerformanceSecurityManagerAgent] Analyzing Performance Security Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Security Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceSecurityManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceSecurityManagerAgent = Object.freeze(new PerformanceSecurityManagerAgent());
