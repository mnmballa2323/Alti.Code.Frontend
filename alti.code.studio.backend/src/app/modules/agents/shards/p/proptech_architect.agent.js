...
class ProptechArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateProptechSystem(proptechObjective) {
        logger.info(`🏢 [ProptechArchitect] Analyzing objective for geospatial and MLS/IDX integration...`);
...
${proptechObjective}
        ...
        try {
            ...
            logger.info(`✅ [ProptechArchitect] PropTech architecture generated successfully.`);
            return cleanCode;
        } catch (err) {
            logger.error(`❌ [ProptechArchitect] Failed to generate PropTech system: ${err.message}`);
            throw err;
        }
    }
}

export const proptechArchitectAgent = Object.freeze(new ProptechArchitectAgent());
