...
class PerformanceStreamingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceStreamingSystem(objective) {
    logger.info(
      `💻 [PerformanceStreamingManagerAgent] Analyzing Performance Streaming Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Streaming Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceStreamingManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceStreamingManagerAgent = Object.freeze(
  new PerformanceStreamingManagerAgent(),
);
