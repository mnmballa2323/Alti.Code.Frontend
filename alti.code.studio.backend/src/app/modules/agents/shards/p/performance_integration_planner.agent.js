...
class PerformanceIntegrationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceIntegrationSystem(objective) {
        logger.info(`💻 [PerformanceIntegrationPlannerAgent] Analyzing Performance Integration Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Integration Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceIntegrationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceIntegrationPlannerAgent = Object.freeze(new PerformanceIntegrationPlannerAgent());
