...
class PerformanceDataLakeConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDataLakeSystem(objective) {
    logger.info(
      `💻 [PerformanceDataLakeConsultantAgent] Analyzing Performance DataLake Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance DataLake Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDataLakeConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDataLakeConsultantAgent = Object.freeze(
  new PerformanceDataLakeConsultantAgent(),
);
