...
class PerformanceConfigurationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceConfigurationSystem(objective) {
        logger.info(`💻 [PerformanceConfigurationManagerAgent] Analyzing Performance Configuration Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Configuration Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceConfigurationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceConfigurationManagerAgent = Object.freeze(new PerformanceConfigurationManagerAgent());
