...
class PerformanceIntegrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceIntegrationSystem(objective) {
    logger.info(
      `💻 [PerformanceIntegrationEngineerAgent] Analyzing Performance Integration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Integration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceIntegrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceIntegrationEngineerAgent = Object.freeze(
  new PerformanceIntegrationEngineerAgent(),
);
