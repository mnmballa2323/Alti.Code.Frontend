...
class PerformanceArchitecturePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceArchitectureSystem(objective) {
    logger.info(
      `💻 [PerformanceArchitecturePlannerAgent] Analyzing Performance Architecture Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Architecture Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceArchitecturePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceArchitecturePlannerAgent = Object.freeze(
  new PerformanceArchitecturePlannerAgent(),
);
