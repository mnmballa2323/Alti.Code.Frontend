...
class PerformanceEngineeringSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceEngineeringSystem(objective) {
        logger.info(`💻 [PerformanceEngineeringSpecialistAgent] Analyzing Performance Engineering Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Engineering Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceEngineeringSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceEngineeringSpecialistAgent = Object.freeze(new PerformanceEngineeringSpecialistAgent());
