...
class PerformanceLoadBalancingEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceLoadBalancingSystem(objective) {
    logger.info(
      `💻 [PerformanceLoadBalancingEngineerAgent] Analyzing Performance LoadBalancing Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance LoadBalancing Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceLoadBalancingEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceLoadBalancingEngineerAgent = Object.freeze(
  new PerformanceLoadBalancingEngineerAgent(),
);
