...
class PerformanceLoadBalancingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceLoadBalancingSystem(objective) {
    logger.info(
      `💻 [PerformanceLoadBalancingDirectorAgent] Analyzing Performance LoadBalancing Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance LoadBalancing Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceLoadBalancingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceLoadBalancingDirectorAgent = Object.freeze(
  new PerformanceLoadBalancingDirectorAgent(),
);
