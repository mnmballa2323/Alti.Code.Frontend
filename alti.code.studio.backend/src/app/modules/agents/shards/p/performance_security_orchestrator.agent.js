...
class PerformanceSecurityOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceSecuritySystem(objective) {
    logger.info(
      `💻 [PerformanceSecurityOrchestratorAgent] Analyzing Performance Security Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Security Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceSecurityOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceSecurityOrchestratorAgent = Object.freeze(
  new PerformanceSecurityOrchestratorAgent(),
);
