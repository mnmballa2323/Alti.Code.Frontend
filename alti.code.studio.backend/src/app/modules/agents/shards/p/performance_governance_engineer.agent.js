...
class PerformanceGovernanceEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceGovernanceSystem(objective) {
        logger.info(`💻 [PerformanceGovernanceEngineerAgent] Analyzing Performance Governance Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Governance Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceGovernanceEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceGovernanceEngineerAgent = Object.freeze(new PerformanceGovernanceEngineerAgent());
