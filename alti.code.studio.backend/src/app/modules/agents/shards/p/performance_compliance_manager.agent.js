...
class PerformanceComplianceManagerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceComplianceSystem(objective) {
    logger.info(
      `💻 [PerformanceComplianceManagerAgent] Analyzing Performance Compliance Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Compliance Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceComplianceManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceComplianceManagerAgent = Object.freeze(
  new PerformanceComplianceManagerAgent(),
);
