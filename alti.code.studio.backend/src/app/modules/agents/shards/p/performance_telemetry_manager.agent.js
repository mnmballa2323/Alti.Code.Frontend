...
class PerformanceTelemetryManagerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceTelemetrySystem(objective) {
        logger.info(`💻 [PerformanceTelemetryManagerAgent] Analyzing Performance Telemetry Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Telemetry Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceTelemetryManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceTelemetryManagerAgent = Object.freeze(new PerformanceTelemetryManagerAgent());
