...
class PerformanceGovernanceStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceGovernanceSystem(objective) {
    logger.info(
      `💻 [PerformanceGovernanceStrategistAgent] Analyzing Performance Governance Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Governance Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceGovernanceStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceGovernanceStrategistAgent = Object.freeze(
  new PerformanceGovernanceStrategistAgent(),
);
