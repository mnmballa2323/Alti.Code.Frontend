...
class PureMathematicsTheoristAgent extends GeminiCliBaseAgent {
  ...
  async generateMathSystem(mathObjective) {
    logger.info(
      `📐 [PureMathematicsTheorist] Analyzing objective for automated theorem proving and algebraic geometry...`,
    );
...
${mathObjective}
    ...
    try {
      ...
      const cleanCode = output
        .replace(
          /```javascript|```typescript|```json|```lean|```coq|```haskell|```/gi,
          '',
        )
        .trim();
      logger.info(
        `✅ [PureMathematicsTheorist] Mathematical architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [PureMathematicsTheorist] Failed to generate math system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const pureMathematicsTheoristAgent = Object.freeze(
  new PureMathematicsTheoristAgent(),
);
