...
class PerformanceLoadBalancingPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceLoadBalancingSystem(objective) {
        logger.info(`💻 [PerformanceLoadBalancingPlannerAgent] Analyzing Performance LoadBalancing Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance LoadBalancing Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceLoadBalancingPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceLoadBalancingPlannerAgent = Object.freeze(new PerformanceLoadBalancingPlannerAgent());
