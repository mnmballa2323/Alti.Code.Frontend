...
class PerformanceConfigurationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceConfigurationSystem(objective) {
    logger.info(
      `💻 [PerformanceConfigurationEngineerAgent] Analyzing Performance Configuration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Configuration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceConfigurationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceConfigurationEngineerAgent = Object.freeze(
  new PerformanceConfigurationEngineerAgent(),
);
