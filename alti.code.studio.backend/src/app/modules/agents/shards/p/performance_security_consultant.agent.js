...
class PerformanceSecurityConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceSecuritySystem(objective) {
    logger.info(
      `💻 [PerformanceSecurityConsultantAgent] Analyzing Performance Security Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Security Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceSecurityConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceSecurityConsultantAgent = Object.freeze(
  new PerformanceSecurityConsultantAgent(),
);
