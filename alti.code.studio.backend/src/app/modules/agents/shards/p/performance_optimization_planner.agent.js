...
class PerformanceOptimizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceOptimizationSystem(objective) {
    logger.info(
      `💻 [PerformanceOptimizationPlannerAgent] Analyzing Performance Optimization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Optimization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceOptimizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceOptimizationPlannerAgent = Object.freeze(
  new PerformanceOptimizationPlannerAgent(),
);
