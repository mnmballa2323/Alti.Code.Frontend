...
class PerformanceScalingAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceScalingSystem(objective) {
    logger.info(
      `💻 [PerformanceScalingAuditorAgent] Analyzing Performance Scaling Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Scaling Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceScalingAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceScalingAuditorAgent = Object.freeze(
  new PerformanceScalingAuditorAgent(),
);
