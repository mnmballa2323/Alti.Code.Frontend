...
class PerformanceComplianceLeadAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceComplianceSystem(objective) {
    logger.info(
      `💻 [PerformanceComplianceLeadAgent] Analyzing Performance Compliance Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Compliance Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceComplianceLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceComplianceLeadAgent = Object.freeze(
  new PerformanceComplianceLeadAgent(),
);
