...
class ParticlePhysicsColliderAgent extends GeminiCliBaseAgent {
  ...
  async generateColliderSystem(physicsObjective) {
    logger.info(
      `⚛️ [ParticlePhysicsCollider] Analyzing objective for LHC collision data and Geant4 simulations...`,
    );
...
${physicsObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [ParticlePhysicsCollider] Particle physics architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [ParticlePhysicsCollider] Failed to generate physics system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const particlePhysicsColliderAgent = Object.freeze(
  new ParticlePhysicsColliderAgent(),
);
