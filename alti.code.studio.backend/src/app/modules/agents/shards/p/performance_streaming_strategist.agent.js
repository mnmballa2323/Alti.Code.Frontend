...
class PerformanceStreamingStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceStreamingSystem(objective) {
    logger.info(
      `💻 [PerformanceStreamingStrategistAgent] Analyzing Performance Streaming Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Streaming Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceStreamingStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceStreamingStrategistAgent = Object.freeze(
  new PerformanceStreamingStrategistAgent(),
);
