...
class PerformanceVirtualizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceVirtualizationSystem(objective) {
    logger.info(
      `💻 [PerformanceVirtualizationDesignerAgent] Analyzing Performance Virtualization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Virtualization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceVirtualizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceVirtualizationDesignerAgent = Object.freeze(
  new PerformanceVirtualizationDesignerAgent(),
);
