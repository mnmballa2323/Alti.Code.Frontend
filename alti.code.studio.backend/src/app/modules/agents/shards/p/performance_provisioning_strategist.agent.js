...
class PerformanceProvisioningStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceProvisioningSystem(objective) {
    logger.info(
      `💻 [PerformanceProvisioningStrategistAgent] Analyzing Performance Provisioning Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Provisioning Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceProvisioningStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceProvisioningStrategistAgent = Object.freeze(
  new PerformanceProvisioningStrategistAgent(),
);
