...
class PerformanceScalingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceScalingSystem(objective) {
    logger.info(
      `💻 [PerformanceScalingPlannerAgent] Analyzing Performance Scaling Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Scaling Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceScalingPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceScalingPlannerAgent = Object.freeze(
  new PerformanceScalingPlannerAgent(),
);
