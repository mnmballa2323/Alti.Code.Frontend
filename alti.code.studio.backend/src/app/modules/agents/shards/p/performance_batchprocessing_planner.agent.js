...
class PerformanceBatchProcessingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceBatchProcessingSystem(objective) {
    logger.info(
      `💻 [PerformanceBatchProcessingPlannerAgent] Analyzing Performance BatchProcessing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance BatchProcessing Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceBatchProcessingPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceBatchProcessingPlannerAgent = Object.freeze(
  new PerformanceBatchProcessingPlannerAgent(),
);
