...
class PerformanceOrchestrationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceOrchestrationSystem(objective) {
        logger.info(`💻 [PerformanceOrchestrationArchitectAgent] Analyzing Performance Orchestration Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Orchestration Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceOrchestrationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceOrchestrationArchitectAgent = Object.freeze(new PerformanceOrchestrationArchitectAgent());
