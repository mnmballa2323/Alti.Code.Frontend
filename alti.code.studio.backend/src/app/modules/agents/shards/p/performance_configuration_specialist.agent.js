...
class PerformanceConfigurationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceConfigurationSystem(objective) {
        logger.info(`💻 [PerformanceConfigurationSpecialistAgent] Analyzing Performance Configuration Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Configuration Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceConfigurationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceConfigurationSpecialistAgent = Object.freeze(new PerformanceConfigurationSpecialistAgent());
