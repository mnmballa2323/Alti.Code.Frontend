...
class PerformanceDeploymentManagerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDeploymentSystem(objective) {
    logger.info(
      `💻 [PerformanceDeploymentManagerAgent] Analyzing Performance Deployment Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Deployment Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDeploymentManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDeploymentManagerAgent = Object.freeze(
  new PerformanceDeploymentManagerAgent(),
);
