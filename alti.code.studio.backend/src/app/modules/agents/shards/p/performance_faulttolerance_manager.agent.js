...
class PerformanceFaultToleranceManagerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceFaultToleranceSystem(objective) {
    logger.info(
      `💻 [PerformanceFaultToleranceManagerAgent] Analyzing Performance FaultTolerance Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance FaultTolerance Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceFaultToleranceManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceFaultToleranceManagerAgent = Object.freeze(
  new PerformanceFaultToleranceManagerAgent(),
);
