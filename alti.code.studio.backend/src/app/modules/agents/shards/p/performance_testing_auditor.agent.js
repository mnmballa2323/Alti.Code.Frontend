...
class PerformanceTestingAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceTestingSystem(objective) {
    logger.info(
      `💻 [PerformanceTestingAuditorAgent] Analyzing Performance Testing Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Testing Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceTestingAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceTestingAuditorAgent = Object.freeze(
  new PerformanceTestingAuditorAgent(),
);
