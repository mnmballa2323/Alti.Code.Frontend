...
class PerformanceObservabilityManagerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceObservabilitySystem(objective) {
    logger.info(
      `💻 [PerformanceObservabilityManagerAgent] Analyzing Performance Observability Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Observability Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceObservabilityManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceObservabilityManagerAgent = Object.freeze(
  new PerformanceObservabilityManagerAgent(),
);
