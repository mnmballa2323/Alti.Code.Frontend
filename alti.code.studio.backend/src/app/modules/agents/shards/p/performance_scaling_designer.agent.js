...
class PerformanceScalingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceScalingSystem(objective) {
    logger.info(
      `💻 [PerformanceScalingDesignerAgent] Analyzing Performance Scaling Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Scaling Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceScalingDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceScalingDesignerAgent = Object.freeze(
  new PerformanceScalingDesignerAgent(),
);
