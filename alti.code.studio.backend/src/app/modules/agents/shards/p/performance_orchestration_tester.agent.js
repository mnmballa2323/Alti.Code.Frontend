...
class PerformanceOrchestrationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceOrchestrationSystem(objective) {
    logger.info(
      `💻 [PerformanceOrchestrationTesterAgent] Analyzing Performance Orchestration Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Orchestration Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceOrchestrationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceOrchestrationTesterAgent = Object.freeze(
  new PerformanceOrchestrationTesterAgent(),
);
