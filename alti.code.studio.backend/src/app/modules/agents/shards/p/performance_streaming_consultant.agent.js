...
class PerformanceStreamingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceStreamingSystem(objective) {
    logger.info(
      `💻 [PerformanceStreamingConsultantAgent] Analyzing Performance Streaming Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Streaming Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceStreamingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceStreamingConsultantAgent = Object.freeze(
  new PerformanceStreamingConsultantAgent(),
);
