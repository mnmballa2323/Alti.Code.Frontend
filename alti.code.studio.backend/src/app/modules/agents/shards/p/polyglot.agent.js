...
class PolyglotAgent {
  ...
  async localize(sourceFile) {
    ...
    try {
      ...
      logger.info(
        `🌐 Polyglot: Found ${extractedStrings.length} strings. Generating locales...`,
      );
      ...
      for (const lang of this.targetLanguages) {
        ...
        for (const str of extractedStrings) {
          translations[lang][str] = await translationService.translate(
            str,
            lang,
          );
        }
...
export const polyglotAgent = Object.freeze(new PolyglotAgent());
