...
class PerformanceSecurityAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceSecuritySystem(objective) {
        logger.info(`💻 [PerformanceSecurityAuditorAgent] Analyzing Performance Security Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Security Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceSecurityAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceSecurityAuditorAgent = Object.freeze(new PerformanceSecurityAuditorAgent());
