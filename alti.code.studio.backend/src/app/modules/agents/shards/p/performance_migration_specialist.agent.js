...
class PerformanceMigrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceMigrationSystem(objective) {
    logger.info(
      `💻 [PerformanceMigrationSpecialistAgent] Analyzing Performance Migration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Migration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceMigrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceMigrationSpecialistAgent = Object.freeze(
  new PerformanceMigrationSpecialistAgent(),
);
