...
class PerformanceMicroservicesLeadAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceMicroservicesSystem(objective) {
        logger.info(`💻 [PerformanceMicroservicesLeadAgent] Analyzing Performance Microservices Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Microservices Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceMicroservicesLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceMicroservicesLeadAgent = Object.freeze(new PerformanceMicroservicesLeadAgent());
