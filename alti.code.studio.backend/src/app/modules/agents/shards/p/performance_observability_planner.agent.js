...
class PerformanceObservabilityPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceObservabilitySystem(objective) {
        logger.info(`💻 [PerformanceObservabilityPlannerAgent] Analyzing Performance Observability Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Observability Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceObservabilityPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceObservabilityPlannerAgent = Object.freeze(new PerformanceObservabilityPlannerAgent());
