...
class PerformanceStreamingArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceStreamingSystem(objective) {
        logger.info(`💻 [PerformanceStreamingArchitectAgent] Analyzing Performance Streaming Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Streaming Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceStreamingArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceStreamingArchitectAgent = Object.freeze(new PerformanceStreamingArchitectAgent());
