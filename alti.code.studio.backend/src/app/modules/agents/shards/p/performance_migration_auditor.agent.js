...
class PerformanceMigrationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceMigrationSystem(objective) {
        logger.info(`💻 [PerformanceMigrationAuditorAgent] Analyzing Performance Migration Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Migration Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceMigrationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceMigrationAuditorAgent = Object.freeze(new PerformanceMigrationAuditorAgent());
