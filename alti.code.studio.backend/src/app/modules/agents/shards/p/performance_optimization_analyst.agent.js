...
class PerformanceOptimizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceOptimizationSystem(objective) {
    logger.info(
      `💻 [PerformanceOptimizationAnalystAgent] Analyzing Performance Optimization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Optimization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceOptimizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceOptimizationAnalystAgent = Object.freeze(
  new PerformanceOptimizationAnalystAgent(),
);
