...
class PerformanceResilienceManagerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceResilienceSystem(objective) {
        logger.info(`💻 [PerformanceResilienceManagerAgent] Analyzing Performance Resilience Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Resilience Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceResilienceManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceResilienceManagerAgent = Object.freeze(new PerformanceResilienceManagerAgent());
