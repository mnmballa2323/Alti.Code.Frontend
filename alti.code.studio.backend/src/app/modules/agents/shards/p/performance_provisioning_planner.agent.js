...
class PerformanceProvisioningPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceProvisioningSystem(objective) {
    logger.info(
      `💻 [PerformanceProvisioningPlannerAgent] Analyzing Performance Provisioning Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Provisioning Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceProvisioningPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceProvisioningPlannerAgent = Object.freeze(
  new PerformanceProvisioningPlannerAgent(),
);
