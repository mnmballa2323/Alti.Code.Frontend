...
class PerformanceTelemetryConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceTelemetrySystem(objective) {
    logger.info(
      `💻 [PerformanceTelemetryConsultantAgent] Analyzing Performance Telemetry Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Telemetry Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceTelemetryConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceTelemetryConsultantAgent = Object.freeze(
  new PerformanceTelemetryConsultantAgent(),
);
