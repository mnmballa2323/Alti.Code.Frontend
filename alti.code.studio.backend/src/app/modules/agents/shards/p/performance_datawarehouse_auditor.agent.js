...
class PerformanceDataWarehouseAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceDataWarehouseSystem(objective) {
        logger.info(`💻 [PerformanceDataWarehouseAuditorAgent] Analyzing Performance DataWarehouse Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance DataWarehouse Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceDataWarehouseAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceDataWarehouseAuditorAgent = Object.freeze(new PerformanceDataWarehouseAuditorAgent());
