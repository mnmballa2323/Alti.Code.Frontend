...
class PerformanceContainerizationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceContainerizationSystem(objective) {
        logger.info(`💻 [PerformanceContainerizationManagerAgent] Analyzing Performance Containerization Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Containerization Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceContainerizationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceContainerizationManagerAgent = Object.freeze(new PerformanceContainerizationManagerAgent());
