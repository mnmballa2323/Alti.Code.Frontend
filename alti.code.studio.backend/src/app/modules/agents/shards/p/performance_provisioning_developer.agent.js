...
class PerformanceProvisioningDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceProvisioningSystem(objective) {
    logger.info(
      `💻 [PerformanceProvisioningDeveloperAgent] Analyzing Performance Provisioning Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Provisioning Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceProvisioningDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceProvisioningDeveloperAgent = Object.freeze(
  new PerformanceProvisioningDeveloperAgent(),
);
