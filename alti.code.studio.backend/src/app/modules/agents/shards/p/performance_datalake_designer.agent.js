...
class PerformanceDataLakeDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDataLakeSystem(objective) {
    logger.info(
      `💻 [PerformanceDataLakeDesignerAgent] Analyzing Performance DataLake Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance DataLake Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDataLakeDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDataLakeDesignerAgent = Object.freeze(
  new PerformanceDataLakeDesignerAgent(),
);
