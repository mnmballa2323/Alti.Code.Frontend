...
class ProductManagerAgent extends GeminiCliBaseAgent {
  ...
  async defineScope(rawRequirements, codebaseContext) {
    logger.info(
      `📋 [ProductManager] Analyzing user requirements and scoping engineering tasks...`,
    );
...
${rawRequirements}
    ...
    try {
      ...
      logger.info(
        `📋 [ProductManager] Successfully scoped ${scope.epics?.length || 0} Epics.`,
      );
      return scope;
    } catch (err) {
      logger.error(
        `❌ [ProductManager] Failed to define scope: ${err.message}`,
      );
      throw err;
    }
  }
