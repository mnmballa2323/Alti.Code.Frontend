...
class PerformanceMigrationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceMigrationSystem(objective) {
    logger.info(
      `💻 [PerformanceMigrationDirectorAgent] Analyzing Performance Migration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Migration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceMigrationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceMigrationDirectorAgent = Object.freeze(
  new PerformanceMigrationDirectorAgent(),
);
