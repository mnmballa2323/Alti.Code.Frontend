...
class PerformanceIntegrationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceIntegrationSystem(objective) {
        logger.info(`💻 [PerformanceIntegrationDeveloperAgent] Analyzing Performance Integration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Integration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceIntegrationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceIntegrationDeveloperAgent = Object.freeze(new PerformanceIntegrationDeveloperAgent());
