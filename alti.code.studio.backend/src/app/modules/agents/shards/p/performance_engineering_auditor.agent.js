...
class PerformanceEngineeringAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceEngineeringSystem(objective) {
        logger.info(`💻 [PerformanceEngineeringAuditorAgent] Analyzing Performance Engineering Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Engineering Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceEngineeringAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceEngineeringAuditorAgent = Object.freeze(new PerformanceEngineeringAuditorAgent());
