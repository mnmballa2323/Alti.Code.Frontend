...
class PerformanceAnalyticsDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceAnalyticsSystem(objective) {
        logger.info(`💻 [PerformanceAnalyticsDirectorAgent] Analyzing Performance Analytics Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Analytics Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceAnalyticsDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceAnalyticsDirectorAgent = Object.freeze(new PerformanceAnalyticsDirectorAgent());
