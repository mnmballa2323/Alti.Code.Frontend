...
class PerformanceIntegrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceIntegrationSystem(objective) {
    logger.info(
      `💻 [PerformanceIntegrationStrategistAgent] Analyzing Performance Integration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Integration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceIntegrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceIntegrationStrategistAgent = Object.freeze(
  new PerformanceIntegrationStrategistAgent(),
);
