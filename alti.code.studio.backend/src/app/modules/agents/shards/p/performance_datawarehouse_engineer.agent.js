...
class PerformanceDataWarehouseEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceDataWarehouseSystem(objective) {
        logger.info(`💻 [PerformanceDataWarehouseEngineerAgent] Analyzing Performance DataWarehouse Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance DataWarehouse Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceDataWarehouseEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceDataWarehouseEngineerAgent = Object.freeze(new PerformanceDataWarehouseEngineerAgent());
