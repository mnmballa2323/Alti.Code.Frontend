...
class PerformanceOrchestrationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceOrchestrationSystem(objective) {
    logger.info(
      `💻 [PerformanceOrchestrationAnalystAgent] Analyzing Performance Orchestration Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Orchestration Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceOrchestrationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceOrchestrationAnalystAgent = Object.freeze(
  new PerformanceOrchestrationAnalystAgent(),
);
