...
class PerformanceProvisioningTesterAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceProvisioningSystem(objective) {
    logger.info(
      `💻 [PerformanceProvisioningTesterAgent] Analyzing Performance Provisioning Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Provisioning Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceProvisioningTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceProvisioningTesterAgent = Object.freeze(
  new PerformanceProvisioningTesterAgent(),
);
