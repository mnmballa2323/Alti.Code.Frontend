...
class PerformanceComplianceDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceComplianceSystem(objective) {
    logger.info(
      `💻 [PerformanceComplianceDeveloperAgent] Analyzing Performance Compliance Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Compliance Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceComplianceDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceComplianceDeveloperAgent = Object.freeze(
  new PerformanceComplianceDeveloperAgent(),
);
