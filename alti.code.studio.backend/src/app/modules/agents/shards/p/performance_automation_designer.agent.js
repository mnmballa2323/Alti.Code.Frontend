...
class PerformanceAutomationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceAutomationSystem(objective) {
        logger.info(`💻 [PerformanceAutomationDesignerAgent] Analyzing Performance Automation Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Automation Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceAutomationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceAutomationDesignerAgent = Object.freeze(new PerformanceAutomationDesignerAgent());
