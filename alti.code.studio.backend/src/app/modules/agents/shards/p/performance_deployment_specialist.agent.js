...
class PerformanceDeploymentSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceDeploymentSystem(objective) {
        logger.info(`💻 [PerformanceDeploymentSpecialistAgent] Analyzing Performance Deployment Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Deployment Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceDeploymentSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceDeploymentSpecialistAgent = Object.freeze(new PerformanceDeploymentSpecialistAgent());
