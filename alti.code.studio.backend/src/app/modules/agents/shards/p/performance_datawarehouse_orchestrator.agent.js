...
class PerformanceDataWarehouseOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDataWarehouseSystem(objective) {
    logger.info(
      `💻 [PerformanceDataWarehouseOrchestratorAgent] Analyzing Performance DataWarehouse Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance DataWarehouse Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDataWarehouseOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDataWarehouseOrchestratorAgent = Object.freeze(
  new PerformanceDataWarehouseOrchestratorAgent(),
);
