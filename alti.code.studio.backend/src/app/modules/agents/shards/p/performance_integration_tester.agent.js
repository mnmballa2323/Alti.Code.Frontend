...
class PerformanceIntegrationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceIntegrationSystem(objective) {
        logger.info(`💻 [PerformanceIntegrationTesterAgent] Analyzing Performance Integration Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Integration Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceIntegrationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceIntegrationTesterAgent = Object.freeze(new PerformanceIntegrationTesterAgent());
