...
class PerformanceAutomationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceAutomationSystem(objective) {
        logger.info(`💻 [PerformanceAutomationEngineerAgent] Analyzing Performance Automation Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Automation Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceAutomationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceAutomationEngineerAgent = Object.freeze(new PerformanceAutomationEngineerAgent());
