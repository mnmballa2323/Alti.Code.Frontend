...
class PerformanceArchitectureAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceArchitectureSystem(objective) {
    logger.info(
      `💻 [PerformanceArchitectureAnalystAgent] Analyzing Performance Architecture Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Architecture Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceArchitectureAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceArchitectureAnalystAgent = Object.freeze(
  new PerformanceArchitectureAnalystAgent(),
);
