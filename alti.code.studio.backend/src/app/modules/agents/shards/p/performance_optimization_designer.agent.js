...
class PerformanceOptimizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceOptimizationSystem(objective) {
    logger.info(
      `💻 [PerformanceOptimizationDesignerAgent] Analyzing Performance Optimization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Optimization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceOptimizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceOptimizationDesignerAgent = Object.freeze(
  new PerformanceOptimizationDesignerAgent(),
);
