...
class PerformanceComplianceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceComplianceSystem(objective) {
    logger.info(
      `💻 [PerformanceComplianceConsultantAgent] Analyzing Performance Compliance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Compliance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceComplianceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceComplianceConsultantAgent = Object.freeze(
  new PerformanceComplianceConsultantAgent(),
);
