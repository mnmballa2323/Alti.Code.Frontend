...
class PerformanceAnalyticsDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceAnalyticsSystem(objective) {
    logger.info(
      `💻 [PerformanceAnalyticsDeveloperAgent] Analyzing Performance Analytics Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Analytics Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceAnalyticsDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceAnalyticsDeveloperAgent = Object.freeze(
  new PerformanceAnalyticsDeveloperAgent(),
);
