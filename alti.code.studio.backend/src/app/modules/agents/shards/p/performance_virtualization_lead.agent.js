...
class PerformanceVirtualizationLeadAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceVirtualizationSystem(objective) {
        logger.info(`💻 [PerformanceVirtualizationLeadAgent] Analyzing Performance Virtualization Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Virtualization Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceVirtualizationLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceVirtualizationLeadAgent = Object.freeze(new PerformanceVirtualizationLeadAgent());
