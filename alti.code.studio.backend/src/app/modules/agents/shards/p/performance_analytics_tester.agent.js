...
class PerformanceAnalyticsTesterAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceAnalyticsSystem(objective) {
    logger.info(
      `💻 [PerformanceAnalyticsTesterAgent] Analyzing Performance Analytics Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Analytics Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceAnalyticsTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceAnalyticsTesterAgent = Object.freeze(
  new PerformanceAnalyticsTesterAgent(),
);
