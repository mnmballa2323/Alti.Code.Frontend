...
class PerformanceMigrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceMigrationSystem(objective) {
    logger.info(
      `💻 [PerformanceMigrationConsultantAgent] Analyzing Performance Migration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Migration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceMigrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceMigrationConsultantAgent = Object.freeze(
  new PerformanceMigrationConsultantAgent(),
);
