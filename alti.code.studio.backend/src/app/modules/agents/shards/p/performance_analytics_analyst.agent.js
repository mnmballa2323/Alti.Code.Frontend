...
class PerformanceAnalyticsAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceAnalyticsSystem(objective) {
        logger.info(`💻 [PerformanceAnalyticsAnalystAgent] Analyzing Performance Analytics Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Analytics Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceAnalyticsAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceAnalyticsAnalystAgent = Object.freeze(new PerformanceAnalyticsAnalystAgent());
