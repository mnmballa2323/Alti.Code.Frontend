...
class PerformanceCachingSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceCachingSystem(objective) {
        logger.info(`💻 [PerformanceCachingSpecialistAgent] Analyzing Performance Caching Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Caching Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceCachingSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceCachingSpecialistAgent = Object.freeze(new PerformanceCachingSpecialistAgent());
