...
class PerformanceArchitectureLeadAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceArchitectureSystem(objective) {
    logger.info(
      `💻 [PerformanceArchitectureLeadAgent] Analyzing Performance Architecture Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Architecture Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceArchitectureLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceArchitectureLeadAgent = Object.freeze(
  new PerformanceArchitectureLeadAgent(),
);
