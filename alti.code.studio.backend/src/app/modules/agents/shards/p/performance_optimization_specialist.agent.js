...
class PerformanceOptimizationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceOptimizationSystem(objective) {
    logger.info(
      `💻 [PerformanceOptimizationSpecialistAgent] Analyzing Performance Optimization Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Optimization Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceOptimizationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceOptimizationSpecialistAgent = Object.freeze(
  new PerformanceOptimizationSpecialistAgent(),
);
