...
class PerformanceLoadBalancingTesterAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceLoadBalancingSystem(objective) {
        logger.info(`💻 [PerformanceLoadBalancingTesterAgent] Analyzing Performance LoadBalancing Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance LoadBalancing Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceLoadBalancingTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceLoadBalancingTesterAgent = Object.freeze(new PerformanceLoadBalancingTesterAgent());
