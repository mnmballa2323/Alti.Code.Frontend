...
class PerformanceAutomationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceAutomationSystem(objective) {
    logger.info(
      `💻 [PerformanceAutomationSpecialistAgent] Analyzing Performance Automation Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Automation Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceAutomationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceAutomationSpecialistAgent = Object.freeze(
  new PerformanceAutomationSpecialistAgent(),
);
