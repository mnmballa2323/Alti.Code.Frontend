...
class PerformanceIntegrationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceIntegrationSystem(objective) {
        logger.info(`💻 [PerformanceIntegrationAuditorAgent] Analyzing Performance Integration Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Integration Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceIntegrationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceIntegrationAuditorAgent = Object.freeze(new PerformanceIntegrationAuditorAgent());
