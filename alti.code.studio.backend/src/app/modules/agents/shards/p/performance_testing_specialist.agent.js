...
class PerformanceTestingSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceTestingSystem(objective) {
        logger.info(`💻 [PerformanceTestingSpecialistAgent] Analyzing Performance Testing Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Testing Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceTestingSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceTestingSpecialistAgent = Object.freeze(new PerformanceTestingSpecialistAgent());
