...
class PerformanceTestingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceTestingSystem(objective) {
        logger.info(`💻 [PerformanceTestingConsultantAgent] Analyzing Performance Testing Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Testing Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceTestingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceTestingConsultantAgent = Object.freeze(new PerformanceTestingConsultantAgent());
