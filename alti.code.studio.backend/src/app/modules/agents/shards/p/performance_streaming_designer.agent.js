...
class PerformanceStreamingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceStreamingSystem(objective) {
    logger.info(
      `💻 [PerformanceStreamingDesignerAgent] Analyzing Performance Streaming Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Streaming Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceStreamingDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceStreamingDesignerAgent = Object.freeze(
  new PerformanceStreamingDesignerAgent(),
);
