...
class PerformanceFaultToleranceLeadAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceFaultToleranceSystem(objective) {
    logger.info(
      `💻 [PerformanceFaultToleranceLeadAgent] Analyzing Performance FaultTolerance Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance FaultTolerance Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceFaultToleranceLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceFaultToleranceLeadAgent = Object.freeze(
  new PerformanceFaultToleranceLeadAgent(),
);
