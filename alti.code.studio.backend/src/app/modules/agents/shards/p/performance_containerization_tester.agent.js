...
class PerformanceContainerizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceContainerizationSystem(objective) {
    logger.info(
      `💻 [PerformanceContainerizationTesterAgent] Analyzing Performance Containerization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Containerization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceContainerizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceContainerizationTesterAgent = Object.freeze(
  new PerformanceContainerizationTesterAgent(),
);
