...
class PerformanceMigrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceMigrationSystem(objective) {
    logger.info(
      `💻 [PerformanceMigrationEngineerAgent] Analyzing Performance Migration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Migration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceMigrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceMigrationEngineerAgent = Object.freeze(
  new PerformanceMigrationEngineerAgent(),
);
