...
class PerformanceCachingManagerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceCachingSystem(objective) {
        logger.info(`💻 [PerformanceCachingManagerAgent] Analyzing Performance Caching Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Caching Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceCachingManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceCachingManagerAgent = Object.freeze(new PerformanceCachingManagerAgent());
