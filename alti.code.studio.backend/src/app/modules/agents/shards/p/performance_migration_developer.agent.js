...
class PerformanceMigrationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceMigrationSystem(objective) {
        logger.info(`💻 [PerformanceMigrationDeveloperAgent] Analyzing Performance Migration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Migration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceMigrationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceMigrationDeveloperAgent = Object.freeze(new PerformanceMigrationDeveloperAgent());
