...
class PerformanceMigrationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceMigrationSystem(objective) {
    logger.info(
      `💻 [PerformanceMigrationPlannerAgent] Analyzing Performance Migration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Migration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceMigrationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceMigrationPlannerAgent = Object.freeze(
  new PerformanceMigrationPlannerAgent(),
);
