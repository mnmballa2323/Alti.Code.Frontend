...
class PerformanceDataLakeDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDataLakeSystem(objective) {
    logger.info(
      `💻 [PerformanceDataLakeDirectorAgent] Analyzing Performance DataLake Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance DataLake Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDataLakeDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDataLakeDirectorAgent = Object.freeze(
  new PerformanceDataLakeDirectorAgent(),
);
