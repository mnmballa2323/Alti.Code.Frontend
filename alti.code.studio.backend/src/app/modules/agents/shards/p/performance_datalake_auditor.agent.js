...
class PerformanceDataLakeAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDataLakeSystem(objective) {
    logger.info(
      `💻 [PerformanceDataLakeAuditorAgent] Analyzing Performance DataLake Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance DataLake Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDataLakeAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDataLakeAuditorAgent = Object.freeze(
  new PerformanceDataLakeAuditorAgent(),
);
