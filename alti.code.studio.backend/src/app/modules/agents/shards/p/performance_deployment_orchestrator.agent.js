...
class PerformanceDeploymentOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDeploymentSystem(objective) {
    logger.info(
      `💻 [PerformanceDeploymentOrchestratorAgent] Analyzing Performance Deployment Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Deployment Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDeploymentOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDeploymentOrchestratorAgent = Object.freeze(
  new PerformanceDeploymentOrchestratorAgent(),
);
