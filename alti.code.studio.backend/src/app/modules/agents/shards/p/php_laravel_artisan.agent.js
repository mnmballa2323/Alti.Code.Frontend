...
class PhpLaravelArtisanAgent extends GeminiCliBaseAgent {
  ...
  async generatePhpSystem(objective) {
    logger.info(
      `🐘 [PhpLaravelArtisan] Analyzing Eloquent models and PHP 8.2+ syntax...`,
    );
    ...
    try {
      ...
      return output
        .replace(/```javascript|```typescript|```json|```php|```/gi, '')
        .trim();
    } catch (err) {
...
export const phpLaravelArtisanAgent = Object.freeze(
  new PhpLaravelArtisanAgent(),
);
