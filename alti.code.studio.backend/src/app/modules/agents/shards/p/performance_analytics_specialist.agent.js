...
class PerformanceAnalyticsSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceAnalyticsSystem(objective) {
        logger.info(`💻 [PerformanceAnalyticsSpecialistAgent] Analyzing Performance Analytics Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Analytics Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceAnalyticsSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceAnalyticsSpecialistAgent = Object.freeze(new PerformanceAnalyticsSpecialistAgent());
