...
class PerformanceGovernanceAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceGovernanceSystem(objective) {
    logger.info(
      `💻 [PerformanceGovernanceAnalystAgent] Analyzing Performance Governance Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Governance Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceGovernanceAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceGovernanceAnalystAgent = Object.freeze(
  new PerformanceGovernanceAnalystAgent(),
);
