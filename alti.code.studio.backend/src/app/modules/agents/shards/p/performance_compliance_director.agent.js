...
class PerformanceComplianceDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceComplianceSystem(objective) {
        logger.info(`💻 [PerformanceComplianceDirectorAgent] Analyzing Performance Compliance Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Compliance Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceComplianceDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceComplianceDirectorAgent = Object.freeze(new PerformanceComplianceDirectorAgent());
