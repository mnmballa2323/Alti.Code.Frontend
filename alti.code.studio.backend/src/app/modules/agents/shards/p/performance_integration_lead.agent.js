...
class PerformanceIntegrationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceIntegrationSystem(objective) {
    logger.info(
      `💻 [PerformanceIntegrationLeadAgent] Analyzing Performance Integration Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Integration Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceIntegrationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceIntegrationLeadAgent = Object.freeze(
  new PerformanceIntegrationLeadAgent(),
);
