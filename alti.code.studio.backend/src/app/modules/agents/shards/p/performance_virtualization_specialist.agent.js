...
class PerformanceVirtualizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceVirtualizationSystem(objective) {
        logger.info(`💻 [PerformanceVirtualizationSpecialistAgent] Analyzing Performance Virtualization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Virtualization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceVirtualizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceVirtualizationSpecialistAgent = Object.freeze(new PerformanceVirtualizationSpecialistAgent());
