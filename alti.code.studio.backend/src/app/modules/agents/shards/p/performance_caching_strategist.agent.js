...
class PerformanceCachingStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceCachingSystem(objective) {
    logger.info(
      `💻 [PerformanceCachingStrategistAgent] Analyzing Performance Caching Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Caching Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceCachingStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceCachingStrategistAgent = Object.freeze(
  new PerformanceCachingStrategistAgent(),
);
