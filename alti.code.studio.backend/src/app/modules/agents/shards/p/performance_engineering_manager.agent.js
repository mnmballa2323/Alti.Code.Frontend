...
class PerformanceEngineeringManagerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceEngineeringSystem(objective) {
        logger.info(`💻 [PerformanceEngineeringManagerAgent] Analyzing Performance Engineering Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Engineering Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceEngineeringManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceEngineeringManagerAgent = Object.freeze(new PerformanceEngineeringManagerAgent());
