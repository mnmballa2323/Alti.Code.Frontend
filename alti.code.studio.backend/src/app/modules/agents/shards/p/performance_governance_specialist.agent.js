...
class PerformanceGovernanceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceGovernanceSystem(objective) {
    logger.info(
      `💻 [PerformanceGovernanceSpecialistAgent] Analyzing Performance Governance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Governance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceGovernanceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceGovernanceSpecialistAgent = Object.freeze(
  new PerformanceGovernanceSpecialistAgent(),
);
