...
class PerformanceCachingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceCachingSystem(objective) {
        logger.info(`💻 [PerformanceCachingConsultantAgent] Analyzing Performance Caching Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Caching Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceCachingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceCachingConsultantAgent = Object.freeze(new PerformanceCachingConsultantAgent());
