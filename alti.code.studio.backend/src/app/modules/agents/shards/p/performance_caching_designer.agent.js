...
class PerformanceCachingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceCachingSystem(objective) {
    logger.info(
      `💻 [PerformanceCachingDesignerAgent] Analyzing Performance Caching Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Caching Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceCachingDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceCachingDesignerAgent = Object.freeze(
  new PerformanceCachingDesignerAgent(),
);
