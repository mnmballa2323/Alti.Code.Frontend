...
class PerformanceDeploymentConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDeploymentSystem(objective) {
    logger.info(
      `💻 [PerformanceDeploymentConsultantAgent] Analyzing Performance Deployment Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Deployment Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDeploymentConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDeploymentConsultantAgent = Object.freeze(
  new PerformanceDeploymentConsultantAgent(),
);
