...
class PerformanceProvisioningConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceProvisioningSystem(objective) {
        logger.info(`💻 [PerformanceProvisioningConsultantAgent] Analyzing Performance Provisioning Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Provisioning Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceProvisioningConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceProvisioningConsultantAgent = Object.freeze(new PerformanceProvisioningConsultantAgent());
