...
class PerformanceDataWarehouseManagerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDataWarehouseSystem(objective) {
    logger.info(
      `💻 [PerformanceDataWarehouseManagerAgent] Analyzing Performance DataWarehouse Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance DataWarehouse Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDataWarehouseManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDataWarehouseManagerAgent = Object.freeze(
  new PerformanceDataWarehouseManagerAgent(),
);
