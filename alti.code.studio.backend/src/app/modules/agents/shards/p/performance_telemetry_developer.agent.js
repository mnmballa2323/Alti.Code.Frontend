...
class PerformanceTelemetryDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceTelemetrySystem(objective) {
        logger.info(`💻 [PerformanceTelemetryDeveloperAgent] Analyzing Performance Telemetry Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Telemetry Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceTelemetryDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceTelemetryDeveloperAgent = Object.freeze(new PerformanceTelemetryDeveloperAgent());
