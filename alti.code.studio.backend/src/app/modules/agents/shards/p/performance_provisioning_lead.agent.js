...
class PerformanceProvisioningLeadAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceProvisioningSystem(objective) {
    logger.info(
      `💻 [PerformanceProvisioningLeadAgent] Analyzing Performance Provisioning Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Provisioning Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceProvisioningLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceProvisioningLeadAgent = Object.freeze(
  new PerformanceProvisioningLeadAgent(),
);
