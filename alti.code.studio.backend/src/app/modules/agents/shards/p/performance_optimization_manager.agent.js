...
class PerformanceOptimizationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceOptimizationSystem(objective) {
        logger.info(`💻 [PerformanceOptimizationManagerAgent] Analyzing Performance Optimization Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Optimization Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceOptimizationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceOptimizationManagerAgent = Object.freeze(new PerformanceOptimizationManagerAgent());
