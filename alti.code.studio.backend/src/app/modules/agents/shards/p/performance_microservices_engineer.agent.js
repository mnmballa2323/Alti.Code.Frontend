...
class PerformanceMicroservicesEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceMicroservicesSystem(objective) {
    logger.info(
      `💻 [PerformanceMicroservicesEngineerAgent] Analyzing Performance Microservices Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Microservices Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceMicroservicesEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceMicroservicesEngineerAgent = Object.freeze(
  new PerformanceMicroservicesEngineerAgent(),
);
