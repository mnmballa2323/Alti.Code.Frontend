...
class PerformanceGovernanceLeadAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceGovernanceSystem(objective) {
    logger.info(
      `💻 [PerformanceGovernanceLeadAgent] Analyzing Performance Governance Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Governance Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceGovernanceLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceGovernanceLeadAgent = Object.freeze(
  new PerformanceGovernanceLeadAgent(),
);
