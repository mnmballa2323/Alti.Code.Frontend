...
class PerformanceObservabilityAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceObservabilitySystem(objective) {
        logger.info(`💻 [PerformanceObservabilityAnalystAgent] Analyzing Performance Observability Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Observability Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceObservabilityAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceObservabilityAnalystAgent = Object.freeze(new PerformanceObservabilityAnalystAgent());
