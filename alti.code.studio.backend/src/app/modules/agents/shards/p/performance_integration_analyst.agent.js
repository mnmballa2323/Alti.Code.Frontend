...
class PerformanceIntegrationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceIntegrationSystem(objective) {
        logger.info(`💻 [PerformanceIntegrationAnalystAgent] Analyzing Performance Integration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Integration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceIntegrationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceIntegrationAnalystAgent = Object.freeze(new PerformanceIntegrationAnalystAgent());
