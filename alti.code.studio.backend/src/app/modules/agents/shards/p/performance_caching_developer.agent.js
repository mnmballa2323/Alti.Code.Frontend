...
class PerformanceCachingDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceCachingSystem(objective) {
        logger.info(`💻 [PerformanceCachingDeveloperAgent] Analyzing Performance Caching Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Caching Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceCachingDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceCachingDeveloperAgent = Object.freeze(new PerformanceCachingDeveloperAgent());
