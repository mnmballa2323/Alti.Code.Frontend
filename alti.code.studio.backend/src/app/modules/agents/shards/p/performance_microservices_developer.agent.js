...
class PerformanceMicroservicesDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceMicroservicesSystem(objective) {
    logger.info(
      `💻 [PerformanceMicroservicesDeveloperAgent] Analyzing Performance Microservices Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Microservices Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceMicroservicesDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceMicroservicesDeveloperAgent = Object.freeze(
  new PerformanceMicroservicesDeveloperAgent(),
);
