...
class PerformanceCachingEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceCachingSystem(objective) {
    logger.info(
      `💻 [PerformanceCachingEngineerAgent] Analyzing Performance Caching Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Caching Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceCachingEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceCachingEngineerAgent = Object.freeze(
  new PerformanceCachingEngineerAgent(),
);
