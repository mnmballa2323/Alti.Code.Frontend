...
class PerformanceVirtualizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceVirtualizationSystem(objective) {
    logger.info(
      `💻 [PerformanceVirtualizationPlannerAgent] Analyzing Performance Virtualization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Virtualization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceVirtualizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceVirtualizationPlannerAgent = Object.freeze(
  new PerformanceVirtualizationPlannerAgent(),
);
