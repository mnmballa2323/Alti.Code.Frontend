...
class PerformanceCachingPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceCachingSystem(objective) {
        logger.info(`💻 [PerformanceCachingPlannerAgent] Analyzing Performance Caching Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Caching Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceCachingPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceCachingPlannerAgent = Object.freeze(new PerformanceCachingPlannerAgent());
