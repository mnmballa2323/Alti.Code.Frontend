...
class PerformanceMigrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceMigrationSystem(objective) {
    logger.info(
      `💻 [PerformanceMigrationArchitectAgent] Analyzing Performance Migration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Migration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceMigrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceMigrationArchitectAgent = Object.freeze(
  new PerformanceMigrationArchitectAgent(),
);
