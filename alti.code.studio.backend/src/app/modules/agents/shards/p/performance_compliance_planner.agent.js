...
class PerformanceCompliancePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceComplianceSystem(objective) {
        logger.info(`💻 [PerformanceCompliancePlannerAgent] Analyzing Performance Compliance Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Compliance Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceCompliancePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceCompliancePlannerAgent = Object.freeze(new PerformanceCompliancePlannerAgent());
