...
class PerformanceObservabilityEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceObservabilitySystem(objective) {
        logger.info(`💻 [PerformanceObservabilityEngineerAgent] Analyzing Performance Observability Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Observability Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceObservabilityEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceObservabilityEngineerAgent = Object.freeze(new PerformanceObservabilityEngineerAgent());
