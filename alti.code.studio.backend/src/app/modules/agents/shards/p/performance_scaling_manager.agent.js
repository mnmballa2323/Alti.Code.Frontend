...
class PerformanceScalingManagerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceScalingSystem(objective) {
        logger.info(`💻 [PerformanceScalingManagerAgent] Analyzing Performance Scaling Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Scaling Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceScalingManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceScalingManagerAgent = Object.freeze(new PerformanceScalingManagerAgent());
