...
class PerformanceDataLakeSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDataLakeSystem(objective) {
    logger.info(
      `💻 [PerformanceDataLakeSpecialistAgent] Analyzing Performance DataLake Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance DataLake Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDataLakeSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDataLakeSpecialistAgent = Object.freeze(
  new PerformanceDataLakeSpecialistAgent(),
);
