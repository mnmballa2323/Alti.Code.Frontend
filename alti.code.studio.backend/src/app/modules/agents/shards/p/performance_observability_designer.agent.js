...
class PerformanceObservabilityDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceObservabilitySystem(objective) {
    logger.info(
      `💻 [PerformanceObservabilityDesignerAgent] Analyzing Performance Observability Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Observability Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceObservabilityDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceObservabilityDesignerAgent = Object.freeze(
  new PerformanceObservabilityDesignerAgent(),
);
