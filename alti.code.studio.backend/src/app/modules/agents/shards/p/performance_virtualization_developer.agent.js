...
class PerformanceVirtualizationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceVirtualizationSystem(objective) {
    logger.info(
      `💻 [PerformanceVirtualizationDeveloperAgent] Analyzing Performance Virtualization Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Virtualization Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceVirtualizationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceVirtualizationDeveloperAgent = Object.freeze(
  new PerformanceVirtualizationDeveloperAgent(),
);
