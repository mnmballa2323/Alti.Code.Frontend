...
class PerformanceLoadBalancingManagerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceLoadBalancingSystem(objective) {
        logger.info(`💻 [PerformanceLoadBalancingManagerAgent] Analyzing Performance LoadBalancing Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance LoadBalancing Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceLoadBalancingManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceLoadBalancingManagerAgent = Object.freeze(new PerformanceLoadBalancingManagerAgent());
