...
class PerformanceResilienceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceResilienceSystem(objective) {
    logger.info(
      `💻 [PerformanceResilienceDirectorAgent] Analyzing Performance Resilience Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Resilience Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceResilienceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceResilienceDirectorAgent = Object.freeze(
  new PerformanceResilienceDirectorAgent(),
);
