...
class PerformanceDeploymentAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDeploymentSystem(objective) {
    logger.info(
      `💻 [PerformanceDeploymentAnalystAgent] Analyzing Performance Deployment Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Deployment Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDeploymentAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDeploymentAnalystAgent = Object.freeze(
  new PerformanceDeploymentAnalystAgent(),
);
