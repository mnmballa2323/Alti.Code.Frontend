...
class PerformanceMigrationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceMigrationSystem(objective) {
        logger.info(`💻 [PerformanceMigrationAnalystAgent] Analyzing Performance Migration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Migration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceMigrationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceMigrationAnalystAgent = Object.freeze(new PerformanceMigrationAnalystAgent());
