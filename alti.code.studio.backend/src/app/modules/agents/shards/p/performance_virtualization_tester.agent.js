...
class PerformanceVirtualizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceVirtualizationSystem(objective) {
    logger.info(
      `💻 [PerformanceVirtualizationTesterAgent] Analyzing Performance Virtualization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Virtualization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceVirtualizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceVirtualizationTesterAgent = Object.freeze(
  new PerformanceVirtualizationTesterAgent(),
);
