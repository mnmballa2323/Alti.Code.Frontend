...
class PerformanceConfigurationLeadAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceConfigurationSystem(objective) {
        logger.info(`💻 [PerformanceConfigurationLeadAgent] Analyzing Performance Configuration Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Configuration Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceConfigurationLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceConfigurationLeadAgent = Object.freeze(new PerformanceConfigurationLeadAgent());
