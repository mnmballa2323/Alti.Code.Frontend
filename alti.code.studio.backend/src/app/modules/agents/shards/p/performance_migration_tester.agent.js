...
class PerformanceMigrationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceMigrationSystem(objective) {
        logger.info(`💻 [PerformanceMigrationTesterAgent] Analyzing Performance Migration Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Migration Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceMigrationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceMigrationTesterAgent = Object.freeze(new PerformanceMigrationTesterAgent());
