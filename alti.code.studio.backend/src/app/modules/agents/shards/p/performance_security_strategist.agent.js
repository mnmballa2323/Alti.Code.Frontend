...
class PerformanceSecurityStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceSecuritySystem(objective) {
    logger.info(
      `💻 [PerformanceSecurityStrategistAgent] Analyzing Performance Security Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Security Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceSecurityStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceSecurityStrategistAgent = Object.freeze(
  new PerformanceSecurityStrategistAgent(),
);
