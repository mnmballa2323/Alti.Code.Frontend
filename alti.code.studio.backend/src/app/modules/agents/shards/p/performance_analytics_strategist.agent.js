...
class PerformanceAnalyticsStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceAnalyticsSystem(objective) {
        logger.info(`💻 [PerformanceAnalyticsStrategistAgent] Analyzing Performance Analytics Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Analytics Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceAnalyticsStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceAnalyticsStrategistAgent = Object.freeze(new PerformanceAnalyticsStrategistAgent());
