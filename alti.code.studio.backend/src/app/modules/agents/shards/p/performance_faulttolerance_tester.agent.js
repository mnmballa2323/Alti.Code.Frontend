...
class PerformanceFaultToleranceTesterAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceFaultToleranceSystem(objective) {
        logger.info(`💻 [PerformanceFaultToleranceTesterAgent] Analyzing Performance FaultTolerance Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance FaultTolerance Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceFaultToleranceTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceFaultToleranceTesterAgent = Object.freeze(new PerformanceFaultToleranceTesterAgent());
