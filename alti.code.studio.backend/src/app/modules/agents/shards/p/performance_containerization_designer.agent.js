...
class PerformanceContainerizationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceContainerizationSystem(objective) {
        logger.info(`💻 [PerformanceContainerizationDesignerAgent] Analyzing Performance Containerization Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Containerization Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceContainerizationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceContainerizationDesignerAgent = Object.freeze(new PerformanceContainerizationDesignerAgent());
