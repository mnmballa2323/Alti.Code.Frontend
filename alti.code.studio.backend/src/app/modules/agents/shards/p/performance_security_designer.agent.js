...
class PerformanceSecurityDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceSecuritySystem(objective) {
    logger.info(
      `💻 [PerformanceSecurityDesignerAgent] Analyzing Performance Security Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Security Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceSecurityDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceSecurityDesignerAgent = Object.freeze(
  new PerformanceSecurityDesignerAgent(),
);
