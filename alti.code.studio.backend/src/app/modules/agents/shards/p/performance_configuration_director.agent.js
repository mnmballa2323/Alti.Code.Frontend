...
class PerformanceConfigurationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceConfigurationSystem(objective) {
        logger.info(`💻 [PerformanceConfigurationDirectorAgent] Analyzing Performance Configuration Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Configuration Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceConfigurationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceConfigurationDirectorAgent = Object.freeze(new PerformanceConfigurationDirectorAgent());
