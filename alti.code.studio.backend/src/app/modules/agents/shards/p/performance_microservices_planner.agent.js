...
class PerformanceMicroservicesPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceMicroservicesSystem(objective) {
        logger.info(`💻 [PerformanceMicroservicesPlannerAgent] Analyzing Performance Microservices Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Microservices Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceMicroservicesPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceMicroservicesPlannerAgent = Object.freeze(new PerformanceMicroservicesPlannerAgent());
