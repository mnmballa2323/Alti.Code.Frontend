...
class PerformanceCachingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceCachingSystem(objective) {
    logger.info(
      `💻 [PerformanceCachingDirectorAgent] Analyzing Performance Caching Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Caching Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceCachingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceCachingDirectorAgent = Object.freeze(
  new PerformanceCachingDirectorAgent(),
);
