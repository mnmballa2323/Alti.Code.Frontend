...
class PerformanceStreamingEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceStreamingSystem(objective) {
    logger.info(
      `💻 [PerformanceStreamingEngineerAgent] Analyzing Performance Streaming Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Streaming Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceStreamingEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceStreamingEngineerAgent = Object.freeze(
  new PerformanceStreamingEngineerAgent(),
);
