...
class PerformanceBatchProcessingLeadAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceBatchProcessingSystem(objective) {
        logger.info(`💻 [PerformanceBatchProcessingLeadAgent] Analyzing Performance BatchProcessing Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance BatchProcessing Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceBatchProcessingLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceBatchProcessingLeadAgent = Object.freeze(new PerformanceBatchProcessingLeadAgent());
