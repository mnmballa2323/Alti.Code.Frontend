...
class PerformanceObservabilityArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceObservabilitySystem(objective) {
        logger.info(`💻 [PerformanceObservabilityArchitectAgent] Analyzing Performance Observability Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Observability Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceObservabilityArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceObservabilityArchitectAgent = Object.freeze(new PerformanceObservabilityArchitectAgent());
