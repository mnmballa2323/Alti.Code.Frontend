...
class PerformanceMigrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceMigrationSystem(objective) {
    logger.info(
      `💻 [PerformanceMigrationStrategistAgent] Analyzing Performance Migration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Migration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceMigrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceMigrationStrategistAgent = Object.freeze(
  new PerformanceMigrationStrategistAgent(),
);
