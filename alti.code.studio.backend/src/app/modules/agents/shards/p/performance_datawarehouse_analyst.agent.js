...
class PerformanceDataWarehouseAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceDataWarehouseSystem(objective) {
        logger.info(`💻 [PerformanceDataWarehouseAnalystAgent] Analyzing Performance DataWarehouse Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance DataWarehouse Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceDataWarehouseAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceDataWarehouseAnalystAgent = Object.freeze(new PerformanceDataWarehouseAnalystAgent());
