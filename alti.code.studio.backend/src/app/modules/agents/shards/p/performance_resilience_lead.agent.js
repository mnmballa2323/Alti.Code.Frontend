...
class PerformanceResilienceLeadAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceResilienceSystem(objective) {
    logger.info(
      `💻 [PerformanceResilienceLeadAgent] Analyzing Performance Resilience Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Resilience Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceResilienceLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceResilienceLeadAgent = Object.freeze(
  new PerformanceResilienceLeadAgent(),
);
