...
class PerformanceMigrationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceMigrationSystem(objective) {
        logger.info(`💻 [PerformanceMigrationDesignerAgent] Analyzing Performance Migration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Migration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceMigrationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceMigrationDesignerAgent = Object.freeze(new PerformanceMigrationDesignerAgent());
