...
class PerformanceTestingArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceTestingSystem(objective) {
    logger.info(
      `💻 [PerformanceTestingArchitectAgent] Analyzing Performance Testing Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Testing Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceTestingArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceTestingArchitectAgent = Object.freeze(
  new PerformanceTestingArchitectAgent(),
);
