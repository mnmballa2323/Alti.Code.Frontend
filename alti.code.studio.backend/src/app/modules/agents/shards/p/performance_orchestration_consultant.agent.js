...
class PerformanceOrchestrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceOrchestrationSystem(objective) {
    logger.info(
      `💻 [PerformanceOrchestrationConsultantAgent] Analyzing Performance Orchestration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Orchestration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceOrchestrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceOrchestrationConsultantAgent = Object.freeze(
  new PerformanceOrchestrationConsultantAgent(),
);
