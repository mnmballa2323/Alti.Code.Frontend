...
class PerformanceArchitectureTesterAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceArchitectureSystem(objective) {
    logger.info(
      `💻 [PerformanceArchitectureTesterAgent] Analyzing Performance Architecture Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Architecture Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceArchitectureTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceArchitectureTesterAgent = Object.freeze(
  new PerformanceArchitectureTesterAgent(),
);
