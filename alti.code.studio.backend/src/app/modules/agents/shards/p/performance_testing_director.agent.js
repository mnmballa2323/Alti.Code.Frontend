...
class PerformanceTestingDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceTestingSystem(objective) {
        logger.info(`💻 [PerformanceTestingDirectorAgent] Analyzing Performance Testing Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Testing Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceTestingDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceTestingDirectorAgent = Object.freeze(new PerformanceTestingDirectorAgent());
