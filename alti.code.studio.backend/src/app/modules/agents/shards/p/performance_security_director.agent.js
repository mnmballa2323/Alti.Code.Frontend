...
class PerformanceSecurityDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceSecuritySystem(objective) {
    logger.info(
      `💻 [PerformanceSecurityDirectorAgent] Analyzing Performance Security Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Security Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceSecurityDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceSecurityDirectorAgent = Object.freeze(
  new PerformanceSecurityDirectorAgent(),
);
