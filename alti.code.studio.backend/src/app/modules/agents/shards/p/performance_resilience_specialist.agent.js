...
class PerformanceResilienceSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceResilienceSystem(objective) {
        logger.info(`💻 [PerformanceResilienceSpecialistAgent] Analyzing Performance Resilience Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Resilience Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceResilienceSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceResilienceSpecialistAgent = Object.freeze(new PerformanceResilienceSpecialistAgent());
