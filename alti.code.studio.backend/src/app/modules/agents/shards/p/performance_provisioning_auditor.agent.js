...
class PerformanceProvisioningAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceProvisioningSystem(objective) {
        logger.info(`💻 [PerformanceProvisioningAuditorAgent] Analyzing Performance Provisioning Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Provisioning Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceProvisioningAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceProvisioningAuditorAgent = Object.freeze(new PerformanceProvisioningAuditorAgent());
