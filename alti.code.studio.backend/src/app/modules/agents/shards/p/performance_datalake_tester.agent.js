...
class PerformanceDataLakeTesterAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceDataLakeSystem(objective) {
        logger.info(`💻 [PerformanceDataLakeTesterAgent] Analyzing Performance DataLake Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance DataLake Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceDataLakeTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceDataLakeTesterAgent = Object.freeze(new PerformanceDataLakeTesterAgent());
