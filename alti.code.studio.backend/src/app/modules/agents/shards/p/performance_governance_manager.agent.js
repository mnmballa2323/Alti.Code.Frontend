...
class PerformanceGovernanceManagerAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceGovernanceSystem(objective) {
    logger.info(
      `💻 [PerformanceGovernanceManagerAgent] Analyzing Performance Governance Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Governance Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceGovernanceManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceGovernanceManagerAgent = Object.freeze(
  new PerformanceGovernanceManagerAgent(),
);
