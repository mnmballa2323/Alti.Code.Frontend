...
class PerformanceDataWarehouseStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceDataWarehouseSystem(objective) {
    logger.info(
      `💻 [PerformanceDataWarehouseStrategistAgent] Analyzing Performance DataWarehouse Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance DataWarehouse Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceDataWarehouseStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceDataWarehouseStrategistAgent = Object.freeze(
  new PerformanceDataWarehouseStrategistAgent(),
);
