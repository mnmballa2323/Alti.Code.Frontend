...
class PerformanceConfigurationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceConfigurationSystem(objective) {
    logger.info(
      `💻 [PerformanceConfigurationConsultantAgent] Analyzing Performance Configuration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Configuration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceConfigurationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceConfigurationConsultantAgent = Object.freeze(
  new PerformanceConfigurationConsultantAgent(),
);
