...
class PerformanceOptimizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceOptimizationSystem(objective) {
    logger.info(
      `💻 [PerformanceOptimizationAuditorAgent] Analyzing Performance Optimization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Optimization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceOptimizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceOptimizationAuditorAgent = Object.freeze(
  new PerformanceOptimizationAuditorAgent(),
);
