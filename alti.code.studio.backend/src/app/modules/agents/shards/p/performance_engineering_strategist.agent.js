...
class PerformanceEngineeringStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generatePerformanceEngineeringSystem(objective) {
        logger.info(`💻 [PerformanceEngineeringStrategistAgent] Analyzing Performance Engineering Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Performance Engineering Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [PerformanceEngineeringStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const performanceEngineeringStrategistAgent = Object.freeze(new PerformanceEngineeringStrategistAgent());
