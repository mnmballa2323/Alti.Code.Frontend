...
class PerformanceServerlessSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceServerlessSystem(objective) {
    logger.info(
      `💻 [PerformanceServerlessSpecialistAgent] Analyzing Performance Serverless Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Serverless Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceServerlessSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceServerlessSpecialistAgent = Object.freeze(
  new PerformanceServerlessSpecialistAgent(),
);
