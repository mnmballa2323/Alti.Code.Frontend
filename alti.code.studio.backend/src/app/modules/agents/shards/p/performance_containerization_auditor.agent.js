...
class PerformanceContainerizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generatePerformanceContainerizationSystem(objective) {
    logger.info(
      `💻 [PerformanceContainerizationAuditorAgent] Analyzing Performance Containerization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Performance Containerization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [PerformanceContainerizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const performanceContainerizationAuditorAgent = Object.freeze(
  new PerformanceContainerizationAuditorAgent(),
);
