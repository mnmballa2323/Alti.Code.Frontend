...
class HaskellFunctionalPuristAgent extends GeminiCliBaseAgent {
    ...
    async generateHaskellSystem(objective) {
        logger.info(`λ [HaskellFunctionalPurist] Analyzing monadic bindings and pure functions...`);
        ...
        try {
            ...
            return output.replace(/```javascript|```typescript|```json|```haskell|```/gi, '').trim();
        } catch (err) {
...
export const haskellFunctionalPuristAgent = Object.freeze(new HaskellFunctionalPuristAgent());
