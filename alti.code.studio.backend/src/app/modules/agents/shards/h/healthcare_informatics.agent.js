...
class HealthcareInformaticsAgent extends GeminiCliBaseAgent {
  ...
  async generateHealthcareSystem(medicalObjective) {
    logger.info(
      `⚕️ [HealthcareInformatics] Analyzing objective for FHIR interoperability and HIPAA compliance...`,
    );
...
${medicalObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [HealthcareInformatics] Healthcare architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [HealthcareInformatics] Failed to generate healthcare system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const healthcareInformaticsAgent = Object.freeze(
  new HealthcareInformaticsAgent(),
);
