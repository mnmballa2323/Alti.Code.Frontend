...
class HighFrequencyTradingQuantAgent extends GeminiCliBaseAgent {
  ...
  async generateHftSystem(hftObjective) {
    logger.info(
      `📈 [HighFrequencyTradingQuant] Analyzing objective for nanosecond latency and order book arbitrage...`,
    );
...
${hftObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [HighFrequencyTradingQuant] HFT architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [HighFrequencyTradingQuant] Failed to generate HFT system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const highFrequencyTradingQuantAgent = Object.freeze(
  new HighFrequencyTradingQuantAgent(),
);
