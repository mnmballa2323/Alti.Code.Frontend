...
class HardwareAgent extends BaseSpecialistAgent {
  constructor() {
    super({
      ...
      name: 'HardwareAgent',
      description:
        'Compiles C/C++ firmware (FreeRTOS, PlatformIO) and executes simulated Over-The-Air (OTA) hardware deployments.',
      tier: 28,
      version: '6.8.0',
      capabilities: [
  ...
  async execute(action, context) {
    if (!context)
      throw new Error('Context is required for HardwareAgent operations');
    ...
    if (action === 'compile_and_flash') {
      ...
      try {
        ...
        return {
          status: 'failed',
          error: 'Failed to generate firmware payload.',
        };
      }

      // Simulate the Cross-Compilation and OTA Push
      logger.info(
        `🏭 HardwareAgent: Cross-compiling ESP-IDF toolchain payload...`,
      );
      ...
      logger.info(
        `🏭 HardwareAgent: Initializing secure OTA pipeline over MQTT...`,
      );
      await new Promise(resolve => setTimeout(resolve, 500)); // Simulated transport
      logger.info(
        `🏭 HardwareAgent: Edge Swarm acknowledged image ${firmwareHash}. Rebooting to new firmware.`,
      );
...
export const hardwareAgent = Object.freeze(new HardwareAgent());
