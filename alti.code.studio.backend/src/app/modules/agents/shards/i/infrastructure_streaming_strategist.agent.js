...
class InfrastructureStreamingStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureStreamingSystem(objective) {
        logger.info(`💻 [InfrastructureStreamingStrategistAgent] Analyzing Infrastructure Streaming Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Streaming Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureStreamingStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureStreamingStrategistAgent = Object.freeze(new InfrastructureStreamingStrategistAgent());
