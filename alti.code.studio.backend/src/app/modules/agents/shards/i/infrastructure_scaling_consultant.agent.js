...
class InfrastructureScalingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureScalingSystem(objective) {
    logger.info(
      `💻 [InfrastructureScalingConsultantAgent] Analyzing Infrastructure Scaling Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Scaling Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureScalingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureScalingConsultantAgent = Object.freeze(
  new InfrastructureScalingConsultantAgent(),
);
