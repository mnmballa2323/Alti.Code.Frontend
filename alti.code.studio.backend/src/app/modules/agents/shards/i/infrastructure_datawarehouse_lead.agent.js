...
class InfrastructureDataWarehouseLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureDataWarehouseSystem(objective) {
        logger.info(`💻 [InfrastructureDataWarehouseLeadAgent] Analyzing Infrastructure DataWarehouse Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure DataWarehouse Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureDataWarehouseLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureDataWarehouseLeadAgent = Object.freeze(new InfrastructureDataWarehouseLeadAgent());
