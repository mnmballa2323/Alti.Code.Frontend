...
class InfrastructureAnalyticsDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureAnalyticsSystem(objective) {
    logger.info(
      `💻 [InfrastructureAnalyticsDesignerAgent] Analyzing Infrastructure Analytics Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Analytics Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureAnalyticsDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureAnalyticsDesignerAgent = Object.freeze(
  new InfrastructureAnalyticsDesignerAgent(),
);
