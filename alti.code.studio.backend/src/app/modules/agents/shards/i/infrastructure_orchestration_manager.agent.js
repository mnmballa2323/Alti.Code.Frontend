...
class InfrastructureOrchestrationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureOrchestrationSystem(objective) {
    logger.info(
      `💻 [InfrastructureOrchestrationManagerAgent] Analyzing Infrastructure Orchestration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Orchestration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureOrchestrationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureOrchestrationManagerAgent = Object.freeze(
  new InfrastructureOrchestrationManagerAgent(),
);
