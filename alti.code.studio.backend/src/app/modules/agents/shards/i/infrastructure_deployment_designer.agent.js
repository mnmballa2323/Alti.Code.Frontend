...
class InfrastructureDeploymentDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureDeploymentSystem(objective) {
        logger.info(`💻 [InfrastructureDeploymentDesignerAgent] Analyzing Infrastructure Deployment Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Deployment Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureDeploymentDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureDeploymentDesignerAgent = Object.freeze(new InfrastructureDeploymentDesignerAgent());
