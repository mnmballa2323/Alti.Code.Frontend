...
class InfrastructureEngineeringConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureEngineeringSystem(objective) {
        logger.info(`💻 [InfrastructureEngineeringConsultantAgent] Analyzing Infrastructure Engineering Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Engineering Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureEngineeringConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureEngineeringConsultantAgent = Object.freeze(new InfrastructureEngineeringConsultantAgent());
