...
class InfrastructureResilienceManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureResilienceSystem(objective) {
    logger.info(
      `💻 [InfrastructureResilienceManagerAgent] Analyzing Infrastructure Resilience Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Resilience Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureResilienceManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureResilienceManagerAgent = Object.freeze(
  new InfrastructureResilienceManagerAgent(),
);
