...
class InfrastructureConfigurationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureConfigurationSystem(objective) {
    logger.info(
      `💻 [InfrastructureConfigurationEngineerAgent] Analyzing Infrastructure Configuration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Configuration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureConfigurationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureConfigurationEngineerAgent = Object.freeze(
  new InfrastructureConfigurationEngineerAgent(),
);
