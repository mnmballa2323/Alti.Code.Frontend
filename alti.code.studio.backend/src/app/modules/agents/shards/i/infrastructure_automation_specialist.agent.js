...
class InfrastructureAutomationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureAutomationSystem(objective) {
    logger.info(
      `💻 [InfrastructureAutomationSpecialistAgent] Analyzing Infrastructure Automation Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Automation Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureAutomationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureAutomationSpecialistAgent = Object.freeze(
  new InfrastructureAutomationSpecialistAgent(),
);
