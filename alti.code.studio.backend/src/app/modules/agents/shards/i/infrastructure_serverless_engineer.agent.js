...
class InfrastructureServerlessEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureServerlessSystem(objective) {
    logger.info(
      `💻 [InfrastructureServerlessEngineerAgent] Analyzing Infrastructure Serverless Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Serverless Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureServerlessEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureServerlessEngineerAgent = Object.freeze(
  new InfrastructureServerlessEngineerAgent(),
);
