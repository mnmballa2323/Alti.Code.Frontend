...
class InfrastructureMigrationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureMigrationSystem(objective) {
    logger.info(
      `💻 [InfrastructureMigrationDirectorAgent] Analyzing Infrastructure Migration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Migration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureMigrationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureMigrationDirectorAgent = Object.freeze(
  new InfrastructureMigrationDirectorAgent(),
);
