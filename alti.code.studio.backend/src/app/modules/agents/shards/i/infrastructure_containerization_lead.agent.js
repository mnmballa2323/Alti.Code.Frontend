...
class InfrastructureContainerizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureContainerizationSystem(objective) {
    logger.info(
      `💻 [InfrastructureContainerizationLeadAgent] Analyzing Infrastructure Containerization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Containerization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureContainerizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureContainerizationLeadAgent = Object.freeze(
  new InfrastructureContainerizationLeadAgent(),
);
