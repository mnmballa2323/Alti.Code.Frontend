...
class InfrastructureOrchestrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureOrchestrationSystem(objective) {
    logger.info(
      `💻 [InfrastructureOrchestrationArchitectAgent] Analyzing Infrastructure Orchestration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Orchestration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureOrchestrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureOrchestrationArchitectAgent = Object.freeze(
  new InfrastructureOrchestrationArchitectAgent(),
);
