...
class InfrastructureConfigurationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureConfigurationSystem(objective) {
    logger.info(
      `💻 [InfrastructureConfigurationPlannerAgent] Analyzing Infrastructure Configuration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Configuration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureConfigurationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureConfigurationPlannerAgent = Object.freeze(
  new InfrastructureConfigurationPlannerAgent(),
);
