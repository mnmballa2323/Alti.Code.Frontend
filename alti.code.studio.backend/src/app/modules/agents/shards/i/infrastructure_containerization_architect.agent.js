...
class InfrastructureContainerizationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureContainerizationSystem(objective) {
        logger.info(`💻 [InfrastructureContainerizationArchitectAgent] Analyzing Infrastructure Containerization Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Containerization Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureContainerizationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureContainerizationArchitectAgent = Object.freeze(new InfrastructureContainerizationArchitectAgent());
