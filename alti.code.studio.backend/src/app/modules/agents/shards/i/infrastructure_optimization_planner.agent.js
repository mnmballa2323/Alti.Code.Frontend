...
class InfrastructureOptimizationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureOptimizationSystem(objective) {
        logger.info(`💻 [InfrastructureOptimizationPlannerAgent] Analyzing Infrastructure Optimization Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Optimization Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureOptimizationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureOptimizationPlannerAgent = Object.freeze(new InfrastructureOptimizationPlannerAgent());
