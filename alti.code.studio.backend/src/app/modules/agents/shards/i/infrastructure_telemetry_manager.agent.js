...
class InfrastructureTelemetryManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureTelemetrySystem(objective) {
    logger.info(
      `💻 [InfrastructureTelemetryManagerAgent] Analyzing Infrastructure Telemetry Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Telemetry Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureTelemetryManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureTelemetryManagerAgent = Object.freeze(
  new InfrastructureTelemetryManagerAgent(),
);
