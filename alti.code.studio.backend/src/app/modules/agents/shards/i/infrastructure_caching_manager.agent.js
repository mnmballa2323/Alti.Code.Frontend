...
class InfrastructureCachingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureCachingSystem(objective) {
    logger.info(
      `💻 [InfrastructureCachingManagerAgent] Analyzing Infrastructure Caching Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Caching Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureCachingManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureCachingManagerAgent = Object.freeze(
  new InfrastructureCachingManagerAgent(),
);
