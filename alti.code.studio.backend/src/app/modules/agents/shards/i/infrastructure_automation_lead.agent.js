...
class InfrastructureAutomationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureAutomationSystem(objective) {
    logger.info(
      `💻 [InfrastructureAutomationLeadAgent] Analyzing Infrastructure Automation Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Automation Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureAutomationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureAutomationLeadAgent = Object.freeze(
  new InfrastructureAutomationLeadAgent(),
);
