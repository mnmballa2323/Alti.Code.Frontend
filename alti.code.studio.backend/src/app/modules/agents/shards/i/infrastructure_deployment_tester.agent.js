...
class InfrastructureDeploymentTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureDeploymentSystem(objective) {
    logger.info(
      `💻 [InfrastructureDeploymentTesterAgent] Analyzing Infrastructure Deployment Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Deployment Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureDeploymentTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureDeploymentTesterAgent = Object.freeze(
  new InfrastructureDeploymentTesterAgent(),
);
