...
class InfrastructureTestingAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureTestingSystem(objective) {
    logger.info(
      `💻 [InfrastructureTestingAnalystAgent] Analyzing Infrastructure Testing Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Testing Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureTestingAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureTestingAnalystAgent = Object.freeze(
  new InfrastructureTestingAnalystAgent(),
);
