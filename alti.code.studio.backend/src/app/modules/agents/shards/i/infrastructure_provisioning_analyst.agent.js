...
class InfrastructureProvisioningAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureProvisioningSystem(objective) {
    logger.info(
      `💻 [InfrastructureProvisioningAnalystAgent] Analyzing Infrastructure Provisioning Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Provisioning Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureProvisioningAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureProvisioningAnalystAgent = Object.freeze(
  new InfrastructureProvisioningAnalystAgent(),
);
