...
class InfrastructureDataLakeDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureDataLakeSystem(objective) {
        logger.info(`💻 [InfrastructureDataLakeDirectorAgent] Analyzing Infrastructure DataLake Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure DataLake Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureDataLakeDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureDataLakeDirectorAgent = Object.freeze(new InfrastructureDataLakeDirectorAgent());
