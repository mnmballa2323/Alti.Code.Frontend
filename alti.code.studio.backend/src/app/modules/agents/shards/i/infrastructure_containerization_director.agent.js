...
class InfrastructureContainerizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureContainerizationSystem(objective) {
    logger.info(
      `💻 [InfrastructureContainerizationDirectorAgent] Analyzing Infrastructure Containerization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Containerization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureContainerizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureContainerizationDirectorAgent = Object.freeze(
  new InfrastructureContainerizationDirectorAgent(),
);
