...
class InfrastructureObservabilityDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureObservabilitySystem(objective) {
    logger.info(
      `💻 [InfrastructureObservabilityDirectorAgent] Analyzing Infrastructure Observability Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Observability Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureObservabilityDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureObservabilityDirectorAgent = Object.freeze(
  new InfrastructureObservabilityDirectorAgent(),
);
