...
class IbmCloudMainframeArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateIbmSystem(ibmObjective) {
    logger.info(
      `☁️ [IbmCloudMainframeArchitect] Analyzing objective for IBM Cloud and z/OS Mainframe integration...`,
    );
...
${ibmObjective}
    ...
    try {
      ...
      const cleanCode = output
        .replace(
          /```javascript|```typescript|```json|```yaml|```java|```/gi,
          '',
        )
        .trim();
      logger.info(
        `✅ [IbmCloudMainframeArchitect] IBM Cloud architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [IbmCloudMainframeArchitect] Failed to generate IBM system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const ibmCloudMainframeArchitectAgent = Object.freeze(
  new IbmCloudMainframeArchitectAgent(),
);
