...
class InfrastructureAutomationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureAutomationSystem(objective) {
        logger.info(`💻 [InfrastructureAutomationArchitectAgent] Analyzing Infrastructure Automation Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Automation Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureAutomationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureAutomationArchitectAgent = Object.freeze(new InfrastructureAutomationArchitectAgent());
