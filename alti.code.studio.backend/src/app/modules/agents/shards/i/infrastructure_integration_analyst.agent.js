...
class InfrastructureIntegrationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureIntegrationSystem(objective) {
    logger.info(
      `💻 [InfrastructureIntegrationAnalystAgent] Analyzing Infrastructure Integration Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Integration Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureIntegrationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureIntegrationAnalystAgent = Object.freeze(
  new InfrastructureIntegrationAnalystAgent(),
);
