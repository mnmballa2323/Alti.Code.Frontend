...
class InfrastructureETLConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureETLSystem(objective) {
    logger.info(
      `💻 [InfrastructureETLConsultantAgent] Analyzing Infrastructure ETL Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure ETL Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureETLConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureETLConsultantAgent = Object.freeze(
  new InfrastructureETLConsultantAgent(),
);
