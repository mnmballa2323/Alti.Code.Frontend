...
class InfrastructureBatchProcessingLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureBatchProcessingSystem(objective) {
    logger.info(
      `💻 [InfrastructureBatchProcessingLeadAgent] Analyzing Infrastructure BatchProcessing Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure BatchProcessing Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureBatchProcessingLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureBatchProcessingLeadAgent = Object.freeze(
  new InfrastructureBatchProcessingLeadAgent(),
);
