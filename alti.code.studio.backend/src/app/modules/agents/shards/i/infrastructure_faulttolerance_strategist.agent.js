...
class InfrastructureFaultToleranceStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureFaultToleranceSystem(objective) {
        logger.info(`💻 [InfrastructureFaultToleranceStrategistAgent] Analyzing Infrastructure FaultTolerance Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure FaultTolerance Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureFaultToleranceStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureFaultToleranceStrategistAgent = Object.freeze(new InfrastructureFaultToleranceStrategistAgent());
