...
class InfrastructureAutomationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureAutomationSystem(objective) {
        logger.info(`💻 [InfrastructureAutomationPlannerAgent] Analyzing Infrastructure Automation Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Automation Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureAutomationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureAutomationPlannerAgent = Object.freeze(new InfrastructureAutomationPlannerAgent());
