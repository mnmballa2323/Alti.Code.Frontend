...
class InfrastructureGovernanceLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureGovernanceSystem(objective) {
    logger.info(
      `💻 [InfrastructureGovernanceLeadAgent] Analyzing Infrastructure Governance Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Governance Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureGovernanceLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureGovernanceLeadAgent = Object.freeze(
  new InfrastructureGovernanceLeadAgent(),
);
