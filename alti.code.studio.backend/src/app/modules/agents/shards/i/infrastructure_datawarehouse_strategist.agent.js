...
class InfrastructureDataWarehouseStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureDataWarehouseSystem(objective) {
    logger.info(
      `💻 [InfrastructureDataWarehouseStrategistAgent] Analyzing Infrastructure DataWarehouse Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure DataWarehouse Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureDataWarehouseStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureDataWarehouseStrategistAgent = Object.freeze(
  new InfrastructureDataWarehouseStrategistAgent(),
);
