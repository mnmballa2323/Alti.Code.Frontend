...
class InfrastructureStreamingAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureStreamingSystem(objective) {
        logger.info(`💻 [InfrastructureStreamingAnalystAgent] Analyzing Infrastructure Streaming Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Streaming Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureStreamingAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureStreamingAnalystAgent = Object.freeze(new InfrastructureStreamingAnalystAgent());
