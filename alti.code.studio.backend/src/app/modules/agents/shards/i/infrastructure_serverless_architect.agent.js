...
class InfrastructureServerlessArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureServerlessSystem(objective) {
    logger.info(
      `💻 [InfrastructureServerlessArchitectAgent] Analyzing Infrastructure Serverless Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Serverless Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureServerlessArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureServerlessArchitectAgent = Object.freeze(
  new InfrastructureServerlessArchitectAgent(),
);
