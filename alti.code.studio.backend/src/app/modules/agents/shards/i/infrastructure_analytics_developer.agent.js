...
class InfrastructureAnalyticsDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureAnalyticsSystem(objective) {
        logger.info(`💻 [InfrastructureAnalyticsDeveloperAgent] Analyzing Infrastructure Analytics Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Analytics Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureAnalyticsDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureAnalyticsDeveloperAgent = Object.freeze(new InfrastructureAnalyticsDeveloperAgent());
