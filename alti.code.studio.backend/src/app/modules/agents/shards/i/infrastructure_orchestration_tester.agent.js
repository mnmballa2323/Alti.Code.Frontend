...
class InfrastructureOrchestrationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureOrchestrationSystem(objective) {
    logger.info(
      `💻 [InfrastructureOrchestrationTesterAgent] Analyzing Infrastructure Orchestration Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Orchestration Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureOrchestrationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureOrchestrationTesterAgent = Object.freeze(
  new InfrastructureOrchestrationTesterAgent(),
);
