...
class InfrastructureDeploymentLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureDeploymentSystem(objective) {
    logger.info(
      `💻 [InfrastructureDeploymentLeadAgent] Analyzing Infrastructure Deployment Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Deployment Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureDeploymentLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureDeploymentLeadAgent = Object.freeze(
  new InfrastructureDeploymentLeadAgent(),
);
