...
class InfrastructureStreamingLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureStreamingSystem(objective) {
    logger.info(
      `💻 [InfrastructureStreamingLeadAgent] Analyzing Infrastructure Streaming Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Streaming Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureStreamingLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureStreamingLeadAgent = Object.freeze(
  new InfrastructureStreamingLeadAgent(),
);
