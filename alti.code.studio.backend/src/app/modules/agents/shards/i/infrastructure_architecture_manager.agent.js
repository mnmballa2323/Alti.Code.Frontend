...
class InfrastructureArchitectureManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureArchitectureSystem(objective) {
        logger.info(`💻 [InfrastructureArchitectureManagerAgent] Analyzing Infrastructure Architecture Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Architecture Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureArchitectureManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureArchitectureManagerAgent = Object.freeze(new InfrastructureArchitectureManagerAgent());
