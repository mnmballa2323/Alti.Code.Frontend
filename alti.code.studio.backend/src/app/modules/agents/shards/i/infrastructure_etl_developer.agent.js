...
class InfrastructureETLDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureETLSystem(objective) {
        logger.info(`💻 [InfrastructureETLDeveloperAgent] Analyzing Infrastructure ETL Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure ETL Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureETLDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureETLDeveloperAgent = Object.freeze(new InfrastructureETLDeveloperAgent());
