...
class InfrastructureMigrationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureMigrationSystem(objective) {
        logger.info(`💻 [InfrastructureMigrationAnalystAgent] Analyzing Infrastructure Migration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Migration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureMigrationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureMigrationAnalystAgent = Object.freeze(new InfrastructureMigrationAnalystAgent());
