...
class InfrastructureGovernanceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureGovernanceSystem(objective) {
    logger.info(
      `💻 [InfrastructureGovernanceEngineerAgent] Analyzing Infrastructure Governance Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Governance Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureGovernanceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureGovernanceEngineerAgent = Object.freeze(
  new InfrastructureGovernanceEngineerAgent(),
);
