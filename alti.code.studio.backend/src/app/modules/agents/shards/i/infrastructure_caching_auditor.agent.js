...
class InfrastructureCachingAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureCachingSystem(objective) {
    logger.info(
      `💻 [InfrastructureCachingAuditorAgent] Analyzing Infrastructure Caching Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Caching Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureCachingAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureCachingAuditorAgent = Object.freeze(
  new InfrastructureCachingAuditorAgent(),
);
