...
class InfrastructureFaultToleranceManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureFaultToleranceSystem(objective) {
    logger.info(
      `💻 [InfrastructureFaultToleranceManagerAgent] Analyzing Infrastructure FaultTolerance Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure FaultTolerance Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureFaultToleranceManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureFaultToleranceManagerAgent = Object.freeze(
  new InfrastructureFaultToleranceManagerAgent(),
);
