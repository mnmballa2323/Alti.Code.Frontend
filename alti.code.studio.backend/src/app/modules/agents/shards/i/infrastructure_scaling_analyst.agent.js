...
class InfrastructureScalingAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureScalingSystem(objective) {
        logger.info(`💻 [InfrastructureScalingAnalystAgent] Analyzing Infrastructure Scaling Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Scaling Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureScalingAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureScalingAnalystAgent = Object.freeze(new InfrastructureScalingAnalystAgent());
