...
class InfrastructureResilienceOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureResilienceSystem(objective) {
    logger.info(
      `💻 [InfrastructureResilienceOrchestratorAgent] Analyzing Infrastructure Resilience Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Resilience Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureResilienceOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureResilienceOrchestratorAgent = Object.freeze(
  new InfrastructureResilienceOrchestratorAgent(),
);
