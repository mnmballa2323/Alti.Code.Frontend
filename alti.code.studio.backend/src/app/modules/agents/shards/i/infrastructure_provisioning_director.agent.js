...
class InfrastructureProvisioningDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureProvisioningSystem(objective) {
    logger.info(
      `💻 [InfrastructureProvisioningDirectorAgent] Analyzing Infrastructure Provisioning Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Provisioning Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureProvisioningDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureProvisioningDirectorAgent = Object.freeze(
  new InfrastructureProvisioningDirectorAgent(),
);
