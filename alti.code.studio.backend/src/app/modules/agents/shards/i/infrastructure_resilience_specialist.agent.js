...
class InfrastructureResilienceSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureResilienceSystem(objective) {
        logger.info(`💻 [InfrastructureResilienceSpecialistAgent] Analyzing Infrastructure Resilience Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Resilience Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureResilienceSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureResilienceSpecialistAgent = Object.freeze(new InfrastructureResilienceSpecialistAgent());
