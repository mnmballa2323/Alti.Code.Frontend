...
class InfrastructureMigrationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureMigrationSystem(objective) {
    logger.info(
      `💻 [InfrastructureMigrationLeadAgent] Analyzing Infrastructure Migration Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Migration Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureMigrationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureMigrationLeadAgent = Object.freeze(
  new InfrastructureMigrationLeadAgent(),
);
