...
class InfrastructureFaultToleranceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureFaultToleranceSystem(objective) {
    logger.info(
      `💻 [InfrastructureFaultToleranceDirectorAgent] Analyzing Infrastructure FaultTolerance Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure FaultTolerance Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureFaultToleranceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureFaultToleranceDirectorAgent = Object.freeze(
  new InfrastructureFaultToleranceDirectorAgent(),
);
