...
class InfrastructureOptimizationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureOptimizationSystem(objective) {
    logger.info(
      `💻 [InfrastructureOptimizationManagerAgent] Analyzing Infrastructure Optimization Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Optimization Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureOptimizationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureOptimizationManagerAgent = Object.freeze(
  new InfrastructureOptimizationManagerAgent(),
);
