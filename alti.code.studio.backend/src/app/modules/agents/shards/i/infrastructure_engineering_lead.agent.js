...
class InfrastructureEngineeringLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureEngineeringSystem(objective) {
    logger.info(
      `💻 [InfrastructureEngineeringLeadAgent] Analyzing Infrastructure Engineering Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Engineering Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureEngineeringLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureEngineeringLeadAgent = Object.freeze(
  new InfrastructureEngineeringLeadAgent(),
);
