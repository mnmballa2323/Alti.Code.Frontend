...
class InfrastructureIntegrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureIntegrationSystem(objective) {
    logger.info(
      `💻 [InfrastructureIntegrationStrategistAgent] Analyzing Infrastructure Integration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Integration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureIntegrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureIntegrationStrategistAgent = Object.freeze(
  new InfrastructureIntegrationStrategistAgent(),
);
