...
class InfrastructureScalingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureScalingSystem(objective) {
    logger.info(
      `💻 [InfrastructureScalingDesignerAgent] Analyzing Infrastructure Scaling Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Scaling Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureScalingDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureScalingDesignerAgent = Object.freeze(
  new InfrastructureScalingDesignerAgent(),
);
