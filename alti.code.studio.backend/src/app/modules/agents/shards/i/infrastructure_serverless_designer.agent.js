...
class InfrastructureServerlessDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureServerlessSystem(objective) {
    logger.info(
      `💻 [InfrastructureServerlessDesignerAgent] Analyzing Infrastructure Serverless Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Serverless Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureServerlessDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureServerlessDesignerAgent = Object.freeze(
  new InfrastructureServerlessDesignerAgent(),
);
