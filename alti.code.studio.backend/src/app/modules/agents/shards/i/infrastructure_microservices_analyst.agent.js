...
class InfrastructureMicroservicesAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureMicroservicesSystem(objective) {
    logger.info(
      `💻 [InfrastructureMicroservicesAnalystAgent] Analyzing Infrastructure Microservices Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Microservices Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureMicroservicesAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureMicroservicesAnalystAgent = Object.freeze(
  new InfrastructureMicroservicesAnalystAgent(),
);
