...
class InfrastructureEngineeringManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureEngineeringSystem(objective) {
    logger.info(
      `💻 [InfrastructureEngineeringManagerAgent] Analyzing Infrastructure Engineering Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Engineering Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureEngineeringManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureEngineeringManagerAgent = Object.freeze(
  new InfrastructureEngineeringManagerAgent(),
);
