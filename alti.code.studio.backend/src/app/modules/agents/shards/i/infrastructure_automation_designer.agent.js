...
class InfrastructureAutomationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureAutomationSystem(objective) {
    logger.info(
      `💻 [InfrastructureAutomationDesignerAgent] Analyzing Infrastructure Automation Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Automation Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureAutomationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureAutomationDesignerAgent = Object.freeze(
  new InfrastructureAutomationDesignerAgent(),
);
