...
class InfrastructureIntegrationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureIntegrationSystem(objective) {
        logger.info(`💻 [InfrastructureIntegrationDesignerAgent] Analyzing Infrastructure Integration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Integration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureIntegrationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureIntegrationDesignerAgent = Object.freeze(new InfrastructureIntegrationDesignerAgent());
