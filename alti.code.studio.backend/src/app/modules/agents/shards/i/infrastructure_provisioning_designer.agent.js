...
class InfrastructureProvisioningDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureProvisioningSystem(objective) {
    logger.info(
      `💻 [InfrastructureProvisioningDesignerAgent] Analyzing Infrastructure Provisioning Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Provisioning Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureProvisioningDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureProvisioningDesignerAgent = Object.freeze(
  new InfrastructureProvisioningDesignerAgent(),
);
