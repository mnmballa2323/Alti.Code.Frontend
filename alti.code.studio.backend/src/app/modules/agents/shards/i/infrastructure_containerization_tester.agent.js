...
class InfrastructureContainerizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureContainerizationSystem(objective) {
    logger.info(
      `💻 [InfrastructureContainerizationTesterAgent] Analyzing Infrastructure Containerization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Containerization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureContainerizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureContainerizationTesterAgent = Object.freeze(
  new InfrastructureContainerizationTesterAgent(),
);
