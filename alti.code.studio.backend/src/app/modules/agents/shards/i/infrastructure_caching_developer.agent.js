...
class InfrastructureCachingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureCachingSystem(objective) {
    logger.info(
      `💻 [InfrastructureCachingDeveloperAgent] Analyzing Infrastructure Caching Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Caching Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureCachingDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureCachingDeveloperAgent = Object.freeze(
  new InfrastructureCachingDeveloperAgent(),
);
