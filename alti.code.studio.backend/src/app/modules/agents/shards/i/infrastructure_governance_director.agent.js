...
class InfrastructureGovernanceDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureGovernanceSystem(objective) {
        logger.info(`💻 [InfrastructureGovernanceDirectorAgent] Analyzing Infrastructure Governance Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Governance Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureGovernanceDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureGovernanceDirectorAgent = Object.freeze(new InfrastructureGovernanceDirectorAgent());
