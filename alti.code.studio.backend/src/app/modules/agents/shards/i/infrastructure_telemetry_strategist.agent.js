...
class InfrastructureTelemetryStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureTelemetrySystem(objective) {
    logger.info(
      `💻 [InfrastructureTelemetryStrategistAgent] Analyzing Infrastructure Telemetry Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Telemetry Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureTelemetryStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureTelemetryStrategistAgent = Object.freeze(
  new InfrastructureTelemetryStrategistAgent(),
);
