...
class InfrastructureObservabilitySpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureObservabilitySystem(objective) {
        logger.info(`💻 [InfrastructureObservabilitySpecialistAgent] Analyzing Infrastructure Observability Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Observability Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureObservabilitySpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureObservabilitySpecialistAgent = Object.freeze(new InfrastructureObservabilitySpecialistAgent());
