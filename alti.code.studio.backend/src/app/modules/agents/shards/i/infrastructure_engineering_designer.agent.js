...
class InfrastructureEngineeringDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureEngineeringSystem(objective) {
    logger.info(
      `💻 [InfrastructureEngineeringDesignerAgent] Analyzing Infrastructure Engineering Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Engineering Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureEngineeringDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureEngineeringDesignerAgent = Object.freeze(
  new InfrastructureEngineeringDesignerAgent(),
);
