...
class InfrastructureArchitecturePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureArchitectureSystem(objective) {
    logger.info(
      `💻 [InfrastructureArchitecturePlannerAgent] Analyzing Infrastructure Architecture Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Architecture Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureArchitecturePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureArchitecturePlannerAgent = Object.freeze(
  new InfrastructureArchitecturePlannerAgent(),
);
