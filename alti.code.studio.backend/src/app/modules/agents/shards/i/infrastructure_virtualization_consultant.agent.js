...
class InfrastructureVirtualizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureVirtualizationSystem(objective) {
    logger.info(
      `💻 [InfrastructureVirtualizationConsultantAgent] Analyzing Infrastructure Virtualization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Virtualization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureVirtualizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureVirtualizationConsultantAgent = Object.freeze(
  new InfrastructureVirtualizationConsultantAgent(),
);
