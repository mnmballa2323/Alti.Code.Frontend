...
class InfrastructureETLEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureETLSystem(objective) {
    logger.info(
      `💻 [InfrastructureETLEngineerAgent] Analyzing Infrastructure ETL Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure ETL Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureETLEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureETLEngineerAgent = Object.freeze(
  new InfrastructureETLEngineerAgent(),
);
