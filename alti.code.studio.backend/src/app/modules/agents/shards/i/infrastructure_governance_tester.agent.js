...
class InfrastructureGovernanceTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureGovernanceSystem(objective) {
    logger.info(
      `💻 [InfrastructureGovernanceTesterAgent] Analyzing Infrastructure Governance Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Governance Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureGovernanceTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureGovernanceTesterAgent = Object.freeze(
  new InfrastructureGovernanceTesterAgent(),
);
