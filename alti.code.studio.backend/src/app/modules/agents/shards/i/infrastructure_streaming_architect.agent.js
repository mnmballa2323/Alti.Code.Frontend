...
class InfrastructureStreamingArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureStreamingSystem(objective) {
    logger.info(
      `💻 [InfrastructureStreamingArchitectAgent] Analyzing Infrastructure Streaming Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Streaming Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureStreamingArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureStreamingArchitectAgent = Object.freeze(
  new InfrastructureStreamingArchitectAgent(),
);
