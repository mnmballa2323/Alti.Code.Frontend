...
class InfrastructureComplianceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureComplianceSystem(objective) {
    logger.info(
      `💻 [InfrastructureComplianceEngineerAgent] Analyzing Infrastructure Compliance Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Compliance Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureComplianceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureComplianceEngineerAgent = Object.freeze(
  new InfrastructureComplianceEngineerAgent(),
);
