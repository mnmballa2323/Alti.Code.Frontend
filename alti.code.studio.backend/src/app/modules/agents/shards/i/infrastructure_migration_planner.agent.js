...
class InfrastructureMigrationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureMigrationSystem(objective) {
        logger.info(`💻 [InfrastructureMigrationPlannerAgent] Analyzing Infrastructure Migration Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Migration Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureMigrationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureMigrationPlannerAgent = Object.freeze(new InfrastructureMigrationPlannerAgent());
