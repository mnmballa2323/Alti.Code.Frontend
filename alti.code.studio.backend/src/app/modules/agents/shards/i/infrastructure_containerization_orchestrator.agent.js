...
class InfrastructureContainerizationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureContainerizationSystem(objective) {
    logger.info(
      `💻 [InfrastructureContainerizationOrchestratorAgent] Analyzing Infrastructure Containerization Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Containerization Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureContainerizationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureContainerizationOrchestratorAgent = Object.freeze(
  new InfrastructureContainerizationOrchestratorAgent(),
);
