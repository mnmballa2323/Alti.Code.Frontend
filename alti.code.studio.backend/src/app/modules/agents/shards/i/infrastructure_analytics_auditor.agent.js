...
class InfrastructureAnalyticsAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureAnalyticsSystem(objective) {
    logger.info(
      `💻 [InfrastructureAnalyticsAuditorAgent] Analyzing Infrastructure Analytics Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Analytics Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureAnalyticsAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureAnalyticsAuditorAgent = Object.freeze(
  new InfrastructureAnalyticsAuditorAgent(),
);
