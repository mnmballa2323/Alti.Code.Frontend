...
class InfrastructureDataLakeStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureDataLakeSystem(objective) {
        logger.info(`💻 [InfrastructureDataLakeStrategistAgent] Analyzing Infrastructure DataLake Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure DataLake Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureDataLakeStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureDataLakeStrategistAgent = Object.freeze(new InfrastructureDataLakeStrategistAgent());
