...
class InfrastructureArchitectureDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureArchitectureSystem(objective) {
        logger.info(`💻 [InfrastructureArchitectureDirectorAgent] Analyzing Infrastructure Architecture Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Architecture Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureArchitectureDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureArchitectureDirectorAgent = Object.freeze(new InfrastructureArchitectureDirectorAgent());
