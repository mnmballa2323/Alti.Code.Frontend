...
class InfrastructureStreamingEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureStreamingSystem(objective) {
        logger.info(`💻 [InfrastructureStreamingEngineerAgent] Analyzing Infrastructure Streaming Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Streaming Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureStreamingEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureStreamingEngineerAgent = Object.freeze(new InfrastructureStreamingEngineerAgent());
