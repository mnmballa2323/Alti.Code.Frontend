...
class InfrastructureGovernanceOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureGovernanceSystem(objective) {
        logger.info(`💻 [InfrastructureGovernanceOrchestratorAgent] Analyzing Infrastructure Governance Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Governance Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureGovernanceOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureGovernanceOrchestratorAgent = Object.freeze(new InfrastructureGovernanceOrchestratorAgent());
