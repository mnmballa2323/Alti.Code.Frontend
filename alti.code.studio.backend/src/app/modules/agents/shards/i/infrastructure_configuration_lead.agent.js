...
class InfrastructureConfigurationLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureConfigurationSystem(objective) {
        logger.info(`💻 [InfrastructureConfigurationLeadAgent] Analyzing Infrastructure Configuration Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Configuration Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureConfigurationLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureConfigurationLeadAgent = Object.freeze(new InfrastructureConfigurationLeadAgent());
