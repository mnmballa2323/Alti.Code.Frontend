...
class InfrastructureMigrationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureMigrationSystem(objective) {
        logger.info(`💻 [InfrastructureMigrationOrchestratorAgent] Analyzing Infrastructure Migration Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Migration Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureMigrationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureMigrationOrchestratorAgent = Object.freeze(new InfrastructureMigrationOrchestratorAgent());
