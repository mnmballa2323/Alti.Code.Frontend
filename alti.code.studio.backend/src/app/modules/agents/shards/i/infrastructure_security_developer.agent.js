...
class InfrastructureSecurityDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureSecuritySystem(objective) {
        logger.info(`💻 [InfrastructureSecurityDeveloperAgent] Analyzing Infrastructure Security Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Security Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureSecurityDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureSecurityDeveloperAgent = Object.freeze(new InfrastructureSecurityDeveloperAgent());
