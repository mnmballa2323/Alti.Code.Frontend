...
class InfrastructureDeploymentDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureDeploymentSystem(objective) {
    logger.info(
      `💻 [InfrastructureDeploymentDeveloperAgent] Analyzing Infrastructure Deployment Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Deployment Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureDeploymentDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureDeploymentDeveloperAgent = Object.freeze(
  new InfrastructureDeploymentDeveloperAgent(),
);
