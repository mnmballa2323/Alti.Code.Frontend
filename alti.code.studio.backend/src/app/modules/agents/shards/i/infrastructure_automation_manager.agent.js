...
class InfrastructureAutomationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureAutomationSystem(objective) {
        logger.info(`💻 [InfrastructureAutomationManagerAgent] Analyzing Infrastructure Automation Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Automation Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureAutomationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureAutomationManagerAgent = Object.freeze(new InfrastructureAutomationManagerAgent());
