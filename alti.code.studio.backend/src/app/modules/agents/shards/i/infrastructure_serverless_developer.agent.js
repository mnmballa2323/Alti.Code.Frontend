...
class InfrastructureServerlessDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureServerlessSystem(objective) {
    logger.info(
      `💻 [InfrastructureServerlessDeveloperAgent] Analyzing Infrastructure Serverless Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Serverless Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureServerlessDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureServerlessDeveloperAgent = Object.freeze(
  new InfrastructureServerlessDeveloperAgent(),
);
