...
class InfrastructureBatchProcessingAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureBatchProcessingSystem(objective) {
    logger.info(
      `💻 [InfrastructureBatchProcessingAnalystAgent] Analyzing Infrastructure BatchProcessing Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure BatchProcessing Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureBatchProcessingAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureBatchProcessingAnalystAgent = Object.freeze(
  new InfrastructureBatchProcessingAnalystAgent(),
);
