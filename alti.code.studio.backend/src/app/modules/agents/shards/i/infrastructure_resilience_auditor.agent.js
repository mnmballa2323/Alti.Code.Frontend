...
class InfrastructureResilienceAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureResilienceSystem(objective) {
    logger.info(
      `💻 [InfrastructureResilienceAuditorAgent] Analyzing Infrastructure Resilience Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Resilience Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureResilienceAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureResilienceAuditorAgent = Object.freeze(
  new InfrastructureResilienceAuditorAgent(),
);
