...
class InfrastructureOptimizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureOptimizationSystem(objective) {
    logger.info(
      `💻 [InfrastructureOptimizationAuditorAgent] Analyzing Infrastructure Optimization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Optimization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureOptimizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureOptimizationAuditorAgent = Object.freeze(
  new InfrastructureOptimizationAuditorAgent(),
);
