...
class InfrastructureGovernanceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureGovernanceSystem(objective) {
    logger.info(
      `💻 [InfrastructureGovernanceSpecialistAgent] Analyzing Infrastructure Governance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Governance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureGovernanceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureGovernanceSpecialistAgent = Object.freeze(
  new InfrastructureGovernanceSpecialistAgent(),
);
