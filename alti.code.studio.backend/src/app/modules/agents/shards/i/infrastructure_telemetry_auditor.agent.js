...
class InfrastructureTelemetryAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureTelemetrySystem(objective) {
    logger.info(
      `💻 [InfrastructureTelemetryAuditorAgent] Analyzing Infrastructure Telemetry Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Telemetry Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureTelemetryAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureTelemetryAuditorAgent = Object.freeze(
  new InfrastructureTelemetryAuditorAgent(),
);
