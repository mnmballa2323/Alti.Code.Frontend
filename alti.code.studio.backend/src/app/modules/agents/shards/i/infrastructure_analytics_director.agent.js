...
class InfrastructureAnalyticsDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureAnalyticsSystem(objective) {
    logger.info(
      `💻 [InfrastructureAnalyticsDirectorAgent] Analyzing Infrastructure Analytics Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Analytics Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureAnalyticsDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureAnalyticsDirectorAgent = Object.freeze(
  new InfrastructureAnalyticsDirectorAgent(),
);
