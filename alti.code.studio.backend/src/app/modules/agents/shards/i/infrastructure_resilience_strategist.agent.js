...
class InfrastructureResilienceStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureResilienceSystem(objective) {
        logger.info(`💻 [InfrastructureResilienceStrategistAgent] Analyzing Infrastructure Resilience Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Resilience Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureResilienceStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureResilienceStrategistAgent = Object.freeze(new InfrastructureResilienceStrategistAgent());
