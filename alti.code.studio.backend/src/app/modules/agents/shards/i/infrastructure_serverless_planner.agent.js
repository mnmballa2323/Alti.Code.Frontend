...
class InfrastructureServerlessPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureServerlessSystem(objective) {
        logger.info(`💻 [InfrastructureServerlessPlannerAgent] Analyzing Infrastructure Serverless Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Serverless Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureServerlessPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureServerlessPlannerAgent = Object.freeze(new InfrastructureServerlessPlannerAgent());
