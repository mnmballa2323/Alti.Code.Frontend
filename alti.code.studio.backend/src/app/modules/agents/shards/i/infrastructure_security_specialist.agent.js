...
class InfrastructureSecuritySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureSecuritySystem(objective) {
    logger.info(
      `💻 [InfrastructureSecuritySpecialistAgent] Analyzing Infrastructure Security Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Security Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureSecuritySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureSecuritySpecialistAgent = Object.freeze(
  new InfrastructureSecuritySpecialistAgent(),
);
