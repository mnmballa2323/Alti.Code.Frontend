...
class InfrastructureProvisioningEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureProvisioningSystem(objective) {
        logger.info(`💻 [InfrastructureProvisioningEngineerAgent] Analyzing Infrastructure Provisioning Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Provisioning Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureProvisioningEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureProvisioningEngineerAgent = Object.freeze(new InfrastructureProvisioningEngineerAgent());
