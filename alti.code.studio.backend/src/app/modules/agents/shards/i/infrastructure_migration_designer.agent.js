...
class InfrastructureMigrationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureMigrationSystem(objective) {
        logger.info(`💻 [InfrastructureMigrationDesignerAgent] Analyzing Infrastructure Migration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Migration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureMigrationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureMigrationDesignerAgent = Object.freeze(new InfrastructureMigrationDesignerAgent());
