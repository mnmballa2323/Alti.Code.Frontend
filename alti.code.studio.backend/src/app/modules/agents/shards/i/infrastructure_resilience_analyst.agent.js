...
class InfrastructureResilienceAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureResilienceSystem(objective) {
    logger.info(
      `💻 [InfrastructureResilienceAnalystAgent] Analyzing Infrastructure Resilience Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Resilience Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureResilienceAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureResilienceAnalystAgent = Object.freeze(
  new InfrastructureResilienceAnalystAgent(),
);
