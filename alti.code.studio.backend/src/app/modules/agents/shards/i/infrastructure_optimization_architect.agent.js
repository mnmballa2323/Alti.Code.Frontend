...
class InfrastructureOptimizationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureOptimizationSystem(objective) {
        logger.info(`💻 [InfrastructureOptimizationArchitectAgent] Analyzing Infrastructure Optimization Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Optimization Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureOptimizationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureOptimizationArchitectAgent = Object.freeze(new InfrastructureOptimizationArchitectAgent());
