...
class InfrastructureBatchProcessingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureBatchProcessingSystem(objective) {
    logger.info(
      `💻 [InfrastructureBatchProcessingSpecialistAgent] Analyzing Infrastructure BatchProcessing Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure BatchProcessing Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureBatchProcessingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureBatchProcessingSpecialistAgent = Object.freeze(
  new InfrastructureBatchProcessingSpecialistAgent(),
);
