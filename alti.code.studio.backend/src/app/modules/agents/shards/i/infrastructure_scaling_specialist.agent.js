...
class InfrastructureScalingSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureScalingSystem(objective) {
        logger.info(`💻 [InfrastructureScalingSpecialistAgent] Analyzing Infrastructure Scaling Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Scaling Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureScalingSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureScalingSpecialistAgent = Object.freeze(new InfrastructureScalingSpecialistAgent());
