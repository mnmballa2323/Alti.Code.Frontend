...
class InfrastructureScalingManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureScalingSystem(objective) {
        logger.info(`💻 [InfrastructureScalingManagerAgent] Analyzing Infrastructure Scaling Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Scaling Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureScalingManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureScalingManagerAgent = Object.freeze(new InfrastructureScalingManagerAgent());
