...
class InfrastructureGovernanceDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureGovernanceSystem(objective) {
        logger.info(`💻 [InfrastructureGovernanceDesignerAgent] Analyzing Infrastructure Governance Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Governance Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureGovernanceDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureGovernanceDesignerAgent = Object.freeze(new InfrastructureGovernanceDesignerAgent());
