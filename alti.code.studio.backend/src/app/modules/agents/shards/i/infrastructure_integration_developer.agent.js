...
class InfrastructureIntegrationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureIntegrationSystem(objective) {
    logger.info(
      `💻 [InfrastructureIntegrationDeveloperAgent] Analyzing Infrastructure Integration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Integration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureIntegrationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureIntegrationDeveloperAgent = Object.freeze(
  new InfrastructureIntegrationDeveloperAgent(),
);
