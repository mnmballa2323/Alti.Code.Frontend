...
class InfrastructureAnalyticsPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureAnalyticsSystem(objective) {
    logger.info(
      `💻 [InfrastructureAnalyticsPlannerAgent] Analyzing Infrastructure Analytics Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Analytics Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureAnalyticsPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureAnalyticsPlannerAgent = Object.freeze(
  new InfrastructureAnalyticsPlannerAgent(),
);
