...
class InfrastructureOptimizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureOptimizationSystem(objective) {
    logger.info(
      `💻 [InfrastructureOptimizationAnalystAgent] Analyzing Infrastructure Optimization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Optimization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureOptimizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureOptimizationAnalystAgent = Object.freeze(
  new InfrastructureOptimizationAnalystAgent(),
);
