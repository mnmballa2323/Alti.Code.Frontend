...
class InfrastructureContainerizationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureContainerizationSystem(objective) {
        logger.info(`💻 [InfrastructureContainerizationAuditorAgent] Analyzing Infrastructure Containerization Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Containerization Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureContainerizationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureContainerizationAuditorAgent = Object.freeze(new InfrastructureContainerizationAuditorAgent());
