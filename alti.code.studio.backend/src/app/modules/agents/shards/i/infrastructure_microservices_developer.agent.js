...
class InfrastructureMicroservicesDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureMicroservicesSystem(objective) {
    logger.info(
      `💻 [InfrastructureMicroservicesDeveloperAgent] Analyzing Infrastructure Microservices Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Microservices Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureMicroservicesDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureMicroservicesDeveloperAgent = Object.freeze(
  new InfrastructureMicroservicesDeveloperAgent(),
);
