...
class InfrastructureGovernanceAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureGovernanceSystem(objective) {
    logger.info(
      `💻 [InfrastructureGovernanceAnalystAgent] Analyzing Infrastructure Governance Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Governance Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureGovernanceAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureGovernanceAnalystAgent = Object.freeze(
  new InfrastructureGovernanceAnalystAgent(),
);
