...
class InfrastructureArchitectureAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureArchitectureSystem(objective) {
    logger.info(
      `💻 [InfrastructureArchitectureAnalystAgent] Analyzing Infrastructure Architecture Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Architecture Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureArchitectureAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureArchitectureAnalystAgent = Object.freeze(
  new InfrastructureArchitectureAnalystAgent(),
);
