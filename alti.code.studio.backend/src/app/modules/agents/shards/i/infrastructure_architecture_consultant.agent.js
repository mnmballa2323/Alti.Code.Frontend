...
class InfrastructureArchitectureConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureArchitectureSystem(objective) {
        logger.info(`💻 [InfrastructureArchitectureConsultantAgent] Analyzing Infrastructure Architecture Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Architecture Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureArchitectureConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureArchitectureConsultantAgent = Object.freeze(new InfrastructureArchitectureConsultantAgent());
