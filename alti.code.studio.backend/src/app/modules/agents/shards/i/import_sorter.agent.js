...
class ImportSorterAgent {
  ...
  async sortImports(filePath) {
    ...
    try {
      ...
      const outputBlocks = [builtin, external, absolute, relative].filter(
        b => b.length > 0,
      );

      let sortedImportsText = outputBlocks
      ...
      while (
        codeStartIndex < code.length &&
        code[codeStartIndex].trim() === ''
      ) {
        codeStartIndex++;
      }
...
export const importSorterAgent = Object.freeze(new ImportSorterAgent());
