...
class InfrastructureBatchProcessingTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureBatchProcessingSystem(objective) {
        logger.info(`💻 [InfrastructureBatchProcessingTesterAgent] Analyzing Infrastructure BatchProcessing Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure BatchProcessing Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureBatchProcessingTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureBatchProcessingTesterAgent = Object.freeze(new InfrastructureBatchProcessingTesterAgent());
