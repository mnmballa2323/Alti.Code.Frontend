...
class InfrastructureScalingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureScalingSystem(objective) {
    logger.info(
      `💻 [InfrastructureScalingDirectorAgent] Analyzing Infrastructure Scaling Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Scaling Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureScalingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureScalingDirectorAgent = Object.freeze(
  new InfrastructureScalingDirectorAgent(),
);
