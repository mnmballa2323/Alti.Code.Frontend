...
class InfrastructureConfigurationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureConfigurationSystem(objective) {
        logger.info(`💻 [InfrastructureConfigurationDeveloperAgent] Analyzing Infrastructure Configuration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Configuration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureConfigurationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureConfigurationDeveloperAgent = Object.freeze(new InfrastructureConfigurationDeveloperAgent());
