...
class InfrastructureTelemetryConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureTelemetrySystem(objective) {
        logger.info(`💻 [InfrastructureTelemetryConsultantAgent] Analyzing Infrastructure Telemetry Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Telemetry Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureTelemetryConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureTelemetryConsultantAgent = Object.freeze(new InfrastructureTelemetryConsultantAgent());
