...
class InfrastructureDeploymentArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureDeploymentSystem(objective) {
        logger.info(`💻 [InfrastructureDeploymentArchitectAgent] Analyzing Infrastructure Deployment Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Deployment Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureDeploymentArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureDeploymentArchitectAgent = Object.freeze(new InfrastructureDeploymentArchitectAgent());
