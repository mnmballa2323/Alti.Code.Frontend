...
class InfrastructureAnalyticsTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureAnalyticsSystem(objective) {
        logger.info(`💻 [InfrastructureAnalyticsTesterAgent] Analyzing Infrastructure Analytics Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Analytics Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureAnalyticsTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureAnalyticsTesterAgent = Object.freeze(new InfrastructureAnalyticsTesterAgent());
