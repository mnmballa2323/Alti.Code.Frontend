...
class InfrastructureTelemetrySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureTelemetrySystem(objective) {
    logger.info(
      `💻 [InfrastructureTelemetrySpecialistAgent] Analyzing Infrastructure Telemetry Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Telemetry Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureTelemetrySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureTelemetrySpecialistAgent = Object.freeze(
  new InfrastructureTelemetrySpecialistAgent(),
);
