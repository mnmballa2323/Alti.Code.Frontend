...
class InfrastructureServerlessAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureServerlessSystem(objective) {
        logger.info(`💻 [InfrastructureServerlessAnalystAgent] Analyzing Infrastructure Serverless Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Serverless Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureServerlessAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureServerlessAnalystAgent = Object.freeze(new InfrastructureServerlessAnalystAgent());
