...
class InfrastructureMigrationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureMigrationSystem(objective) {
    logger.info(
      `💻 [InfrastructureMigrationTesterAgent] Analyzing Infrastructure Migration Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Migration Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureMigrationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureMigrationTesterAgent = Object.freeze(
  new InfrastructureMigrationTesterAgent(),
);
