...
class InfrastructureObservabilityAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureObservabilitySystem(objective) {
    logger.info(
      `💻 [InfrastructureObservabilityAuditorAgent] Analyzing Infrastructure Observability Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Observability Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureObservabilityAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureObservabilityAuditorAgent = Object.freeze(
  new InfrastructureObservabilityAuditorAgent(),
);
