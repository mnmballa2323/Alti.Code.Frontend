...
class InfrastructureEngineeringTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureEngineeringSystem(objective) {
    logger.info(
      `💻 [InfrastructureEngineeringTesterAgent] Analyzing Infrastructure Engineering Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Engineering Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureEngineeringTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureEngineeringTesterAgent = Object.freeze(
  new InfrastructureEngineeringTesterAgent(),
);
