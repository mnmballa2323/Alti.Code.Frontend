...
class InfrastructureScalingArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureScalingSystem(objective) {
        logger.info(`💻 [InfrastructureScalingArchitectAgent] Analyzing Infrastructure Scaling Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Scaling Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureScalingArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureScalingArchitectAgent = Object.freeze(new InfrastructureScalingArchitectAgent());
