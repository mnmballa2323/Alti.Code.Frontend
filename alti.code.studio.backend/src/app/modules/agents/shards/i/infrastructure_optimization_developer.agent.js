...
class InfrastructureOptimizationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureOptimizationSystem(objective) {
        logger.info(`💻 [InfrastructureOptimizationDeveloperAgent] Analyzing Infrastructure Optimization Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Optimization Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureOptimizationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureOptimizationDeveloperAgent = Object.freeze(new InfrastructureOptimizationDeveloperAgent());
