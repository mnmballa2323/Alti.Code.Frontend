...
class InfrastructureArchitectureLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureArchitectureSystem(objective) {
    logger.info(
      `💻 [InfrastructureArchitectureLeadAgent] Analyzing Infrastructure Architecture Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Architecture Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureArchitectureLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureArchitectureLeadAgent = Object.freeze(
  new InfrastructureArchitectureLeadAgent(),
);
