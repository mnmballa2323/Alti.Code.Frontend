...
class InfrastructureConfigurationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureConfigurationSystem(objective) {
        logger.info(`💻 [InfrastructureConfigurationAuditorAgent] Analyzing Infrastructure Configuration Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Configuration Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureConfigurationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureConfigurationAuditorAgent = Object.freeze(new InfrastructureConfigurationAuditorAgent());
