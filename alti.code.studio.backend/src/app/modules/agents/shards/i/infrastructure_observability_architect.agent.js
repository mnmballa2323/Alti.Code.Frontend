...
class InfrastructureObservabilityArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureObservabilitySystem(objective) {
        logger.info(`💻 [InfrastructureObservabilityArchitectAgent] Analyzing Infrastructure Observability Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Observability Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureObservabilityArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureObservabilityArchitectAgent = Object.freeze(new InfrastructureObservabilityArchitectAgent());
