...
class InfrastructureDataLakeArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureDataLakeSystem(objective) {
        logger.info(`💻 [InfrastructureDataLakeArchitectAgent] Analyzing Infrastructure DataLake Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure DataLake Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureDataLakeArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureDataLakeArchitectAgent = Object.freeze(new InfrastructureDataLakeArchitectAgent());
