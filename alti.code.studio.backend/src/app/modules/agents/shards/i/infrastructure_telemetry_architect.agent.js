...
class InfrastructureTelemetryArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureTelemetrySystem(objective) {
    logger.info(
      `💻 [InfrastructureTelemetryArchitectAgent] Analyzing Infrastructure Telemetry Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Telemetry Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureTelemetryArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureTelemetryArchitectAgent = Object.freeze(
  new InfrastructureTelemetryArchitectAgent(),
);
