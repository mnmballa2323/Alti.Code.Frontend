...
class InfrastructureResilienceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureResilienceSystem(objective) {
    logger.info(
      `💻 [InfrastructureResilienceConsultantAgent] Analyzing Infrastructure Resilience Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Resilience Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureResilienceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureResilienceConsultantAgent = Object.freeze(
  new InfrastructureResilienceConsultantAgent(),
);
