...
class InfrastructureDataLakePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureDataLakeSystem(objective) {
    logger.info(
      `💻 [InfrastructureDataLakePlannerAgent] Analyzing Infrastructure DataLake Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure DataLake Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureDataLakePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureDataLakePlannerAgent = Object.freeze(
  new InfrastructureDataLakePlannerAgent(),
);
