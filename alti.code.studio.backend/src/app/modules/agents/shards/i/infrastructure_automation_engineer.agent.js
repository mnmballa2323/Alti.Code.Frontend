...
class InfrastructureAutomationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureAutomationSystem(objective) {
    logger.info(
      `💻 [InfrastructureAutomationEngineerAgent] Analyzing Infrastructure Automation Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Automation Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureAutomationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureAutomationEngineerAgent = Object.freeze(
  new InfrastructureAutomationEngineerAgent(),
);
