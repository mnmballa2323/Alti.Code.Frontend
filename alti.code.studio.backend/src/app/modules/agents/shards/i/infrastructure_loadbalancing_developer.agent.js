...
class InfrastructureLoadBalancingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureLoadBalancingSystem(objective) {
    logger.info(
      `💻 [InfrastructureLoadBalancingDeveloperAgent] Analyzing Infrastructure LoadBalancing Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure LoadBalancing Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureLoadBalancingDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureLoadBalancingDeveloperAgent = Object.freeze(
  new InfrastructureLoadBalancingDeveloperAgent(),
);
