...
class InfrastructureScalingLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureScalingSystem(objective) {
    logger.info(
      `💻 [InfrastructureScalingLeadAgent] Analyzing Infrastructure Scaling Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Scaling Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureScalingLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureScalingLeadAgent = Object.freeze(
  new InfrastructureScalingLeadAgent(),
);
