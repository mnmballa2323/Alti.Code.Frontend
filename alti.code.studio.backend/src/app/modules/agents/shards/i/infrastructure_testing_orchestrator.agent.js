...
class InfrastructureTestingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureTestingSystem(objective) {
    logger.info(
      `💻 [InfrastructureTestingOrchestratorAgent] Analyzing Infrastructure Testing Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Testing Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureTestingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureTestingOrchestratorAgent = Object.freeze(
  new InfrastructureTestingOrchestratorAgent(),
);
