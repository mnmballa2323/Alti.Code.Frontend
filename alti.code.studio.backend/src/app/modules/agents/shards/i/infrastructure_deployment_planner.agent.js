...
class InfrastructureDeploymentPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureDeploymentSystem(objective) {
    logger.info(
      `💻 [InfrastructureDeploymentPlannerAgent] Analyzing Infrastructure Deployment Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Deployment Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureDeploymentPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureDeploymentPlannerAgent = Object.freeze(
  new InfrastructureDeploymentPlannerAgent(),
);
