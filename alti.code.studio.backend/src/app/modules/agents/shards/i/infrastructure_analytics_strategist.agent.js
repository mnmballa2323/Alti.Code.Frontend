...
class InfrastructureAnalyticsStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureAnalyticsSystem(objective) {
    logger.info(
      `💻 [InfrastructureAnalyticsStrategistAgent] Analyzing Infrastructure Analytics Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Analytics Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureAnalyticsStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureAnalyticsStrategistAgent = Object.freeze(
  new InfrastructureAnalyticsStrategistAgent(),
);
