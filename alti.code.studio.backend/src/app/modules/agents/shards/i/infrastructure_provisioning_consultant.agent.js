...
class InfrastructureProvisioningConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureProvisioningSystem(objective) {
    logger.info(
      `💻 [InfrastructureProvisioningConsultantAgent] Analyzing Infrastructure Provisioning Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Provisioning Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureProvisioningConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureProvisioningConsultantAgent = Object.freeze(
  new InfrastructureProvisioningConsultantAgent(),
);
