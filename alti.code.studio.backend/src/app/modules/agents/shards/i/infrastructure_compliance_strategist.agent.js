...
class InfrastructureComplianceStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureComplianceSystem(objective) {
        logger.info(`💻 [InfrastructureComplianceStrategistAgent] Analyzing Infrastructure Compliance Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Compliance Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureComplianceStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureComplianceStrategistAgent = Object.freeze(new InfrastructureComplianceStrategistAgent());
