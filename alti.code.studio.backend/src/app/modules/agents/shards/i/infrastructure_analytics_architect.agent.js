...
class InfrastructureAnalyticsArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureAnalyticsSystem(objective) {
        logger.info(`💻 [InfrastructureAnalyticsArchitectAgent] Analyzing Infrastructure Analytics Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Analytics Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureAnalyticsArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureAnalyticsArchitectAgent = Object.freeze(new InfrastructureAnalyticsArchitectAgent());
