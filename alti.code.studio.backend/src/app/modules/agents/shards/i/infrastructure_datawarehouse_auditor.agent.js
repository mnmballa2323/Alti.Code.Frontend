...
class InfrastructureDataWarehouseAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureDataWarehouseSystem(objective) {
    logger.info(
      `💻 [InfrastructureDataWarehouseAuditorAgent] Analyzing Infrastructure DataWarehouse Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure DataWarehouse Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureDataWarehouseAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureDataWarehouseAuditorAgent = Object.freeze(
  new InfrastructureDataWarehouseAuditorAgent(),
);
