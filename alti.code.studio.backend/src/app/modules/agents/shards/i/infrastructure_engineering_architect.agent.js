...
class InfrastructureEngineeringArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureEngineeringSystem(objective) {
    logger.info(
      `💻 [InfrastructureEngineeringArchitectAgent] Analyzing Infrastructure Engineering Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Engineering Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureEngineeringArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureEngineeringArchitectAgent = Object.freeze(
  new InfrastructureEngineeringArchitectAgent(),
);
