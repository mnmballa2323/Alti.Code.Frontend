...
class InfrastructureComplianceTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureComplianceSystem(objective) {
    logger.info(
      `💻 [InfrastructureComplianceTesterAgent] Analyzing Infrastructure Compliance Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Compliance Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureComplianceTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureComplianceTesterAgent = Object.freeze(
  new InfrastructureComplianceTesterAgent(),
);
