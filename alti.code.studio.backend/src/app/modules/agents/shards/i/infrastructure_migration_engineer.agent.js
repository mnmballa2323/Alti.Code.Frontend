...
class InfrastructureMigrationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureMigrationSystem(objective) {
        logger.info(`💻 [InfrastructureMigrationEngineerAgent] Analyzing Infrastructure Migration Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Migration Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureMigrationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureMigrationEngineerAgent = Object.freeze(new InfrastructureMigrationEngineerAgent());
