...
class InfrastructureOrchestrationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureOrchestrationSystem(objective) {
        logger.info(`💻 [InfrastructureOrchestrationAnalystAgent] Analyzing Infrastructure Orchestration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Orchestration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureOrchestrationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureOrchestrationAnalystAgent = Object.freeze(new InfrastructureOrchestrationAnalystAgent());
