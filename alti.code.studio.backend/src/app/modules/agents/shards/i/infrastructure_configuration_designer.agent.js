...
class InfrastructureConfigurationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureConfigurationSystem(objective) {
    logger.info(
      `💻 [InfrastructureConfigurationDesignerAgent] Analyzing Infrastructure Configuration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Configuration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureConfigurationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureConfigurationDesignerAgent = Object.freeze(
  new InfrastructureConfigurationDesignerAgent(),
);
