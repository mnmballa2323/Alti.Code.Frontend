...
class InfrastructureETLStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureETLSystem(objective) {
    logger.info(
      `💻 [InfrastructureETLStrategistAgent] Analyzing Infrastructure ETL Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure ETL Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureETLStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureETLStrategistAgent = Object.freeze(
  new InfrastructureETLStrategistAgent(),
);
