...
class InfrastructureSecurityAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureSecuritySystem(objective) {
    logger.info(
      `💻 [InfrastructureSecurityAnalystAgent] Analyzing Infrastructure Security Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Security Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureSecurityAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureSecurityAnalystAgent = Object.freeze(
  new InfrastructureSecurityAnalystAgent(),
);
