...
class InfrastructureAutomationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureAutomationSystem(objective) {
    logger.info(
      `💻 [InfrastructureAutomationTesterAgent] Analyzing Infrastructure Automation Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Automation Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureAutomationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureAutomationTesterAgent = Object.freeze(
  new InfrastructureAutomationTesterAgent(),
);
