...
class InfrastructureScalingTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureScalingSystem(objective) {
        logger.info(`💻 [InfrastructureScalingTesterAgent] Analyzing Infrastructure Scaling Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Scaling Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureScalingTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureScalingTesterAgent = Object.freeze(new InfrastructureScalingTesterAgent());
