...
class InfrastructureMicroservicesTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureMicroservicesSystem(objective) {
        logger.info(`💻 [InfrastructureMicroservicesTesterAgent] Analyzing Infrastructure Microservices Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Microservices Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureMicroservicesTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureMicroservicesTesterAgent = Object.freeze(new InfrastructureMicroservicesTesterAgent());
