...
class InfrastructureScalingAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureScalingSystem(objective) {
        logger.info(`💻 [InfrastructureScalingAuditorAgent] Analyzing Infrastructure Scaling Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Scaling Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureScalingAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureScalingAuditorAgent = Object.freeze(new InfrastructureScalingAuditorAgent());
