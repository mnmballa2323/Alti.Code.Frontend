...
class InfrastructureSecurityTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureSecuritySystem(objective) {
        logger.info(`💻 [InfrastructureSecurityTesterAgent] Analyzing Infrastructure Security Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Security Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureSecurityTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureSecurityTesterAgent = Object.freeze(new InfrastructureSecurityTesterAgent());
