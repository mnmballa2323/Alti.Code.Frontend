...
class InfrastructureETLOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureETLSystem(objective) {
    logger.info(
      `💻 [InfrastructureETLOrchestratorAgent] Analyzing Infrastructure ETL Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure ETL Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureETLOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureETLOrchestratorAgent = Object.freeze(
  new InfrastructureETLOrchestratorAgent(),
);
