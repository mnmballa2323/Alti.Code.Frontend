...
class InfrastructureDataWarehousePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureDataWarehouseSystem(objective) {
    logger.info(
      `💻 [InfrastructureDataWarehousePlannerAgent] Analyzing Infrastructure DataWarehouse Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure DataWarehouse Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureDataWarehousePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureDataWarehousePlannerAgent = Object.freeze(
  new InfrastructureDataWarehousePlannerAgent(),
);
