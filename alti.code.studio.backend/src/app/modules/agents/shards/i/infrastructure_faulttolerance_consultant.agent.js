...
class InfrastructureFaultToleranceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureFaultToleranceSystem(objective) {
        logger.info(`💻 [InfrastructureFaultToleranceConsultantAgent] Analyzing Infrastructure FaultTolerance Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure FaultTolerance Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureFaultToleranceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureFaultToleranceConsultantAgent = Object.freeze(new InfrastructureFaultToleranceConsultantAgent());
