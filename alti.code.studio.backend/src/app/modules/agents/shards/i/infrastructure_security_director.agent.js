...
class InfrastructureSecurityDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureSecuritySystem(objective) {
    logger.info(
      `💻 [InfrastructureSecurityDirectorAgent] Analyzing Infrastructure Security Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Security Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureSecurityDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureSecurityDirectorAgent = Object.freeze(
  new InfrastructureSecurityDirectorAgent(),
);
