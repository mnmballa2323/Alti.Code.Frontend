...
class InfrastructureMicroservicesSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureMicroservicesSystem(objective) {
    logger.info(
      `💻 [InfrastructureMicroservicesSpecialistAgent] Analyzing Infrastructure Microservices Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Microservices Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureMicroservicesSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureMicroservicesSpecialistAgent = Object.freeze(
  new InfrastructureMicroservicesSpecialistAgent(),
);
