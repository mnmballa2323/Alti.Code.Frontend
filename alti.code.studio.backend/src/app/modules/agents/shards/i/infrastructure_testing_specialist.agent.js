...
class InfrastructureTestingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureTestingSystem(objective) {
    logger.info(
      `💻 [InfrastructureTestingSpecialistAgent] Analyzing Infrastructure Testing Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Testing Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureTestingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureTestingSpecialistAgent = Object.freeze(
  new InfrastructureTestingSpecialistAgent(),
);
