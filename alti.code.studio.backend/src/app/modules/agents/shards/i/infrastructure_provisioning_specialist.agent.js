...
class InfrastructureProvisioningSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureProvisioningSystem(objective) {
    logger.info(
      `💻 [InfrastructureProvisioningSpecialistAgent] Analyzing Infrastructure Provisioning Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Provisioning Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureProvisioningSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureProvisioningSpecialistAgent = Object.freeze(
  new InfrastructureProvisioningSpecialistAgent(),
);
