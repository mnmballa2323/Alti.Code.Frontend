...
class InfrastructureOrchestrationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureOrchestrationSystem(objective) {
        logger.info(`💻 [InfrastructureOrchestrationOrchestratorAgent] Analyzing Infrastructure Orchestration Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Orchestration Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureOrchestrationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureOrchestrationOrchestratorAgent = Object.freeze(new InfrastructureOrchestrationOrchestratorAgent());
