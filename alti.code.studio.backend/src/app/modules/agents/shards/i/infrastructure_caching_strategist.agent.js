...
class InfrastructureCachingStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureCachingSystem(objective) {
    logger.info(
      `💻 [InfrastructureCachingStrategistAgent] Analyzing Infrastructure Caching Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Caching Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureCachingStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureCachingStrategistAgent = Object.freeze(
  new InfrastructureCachingStrategistAgent(),
);
