...
class InfrastructureDeploymentSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureDeploymentSystem(objective) {
        logger.info(`💻 [InfrastructureDeploymentSpecialistAgent] Analyzing Infrastructure Deployment Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Deployment Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureDeploymentSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureDeploymentSpecialistAgent = Object.freeze(new InfrastructureDeploymentSpecialistAgent());
