...
class InfrastructureArchitectureOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureArchitectureSystem(objective) {
    logger.info(
      `💻 [InfrastructureArchitectureOrchestratorAgent] Analyzing Infrastructure Architecture Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Architecture Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureArchitectureOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureArchitectureOrchestratorAgent = Object.freeze(
  new InfrastructureArchitectureOrchestratorAgent(),
);
