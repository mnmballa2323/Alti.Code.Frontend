...
class InfrastructureComplianceOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureComplianceSystem(objective) {
        logger.info(`💻 [InfrastructureComplianceOrchestratorAgent] Analyzing Infrastructure Compliance Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Compliance Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureComplianceOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureComplianceOrchestratorAgent = Object.freeze(new InfrastructureComplianceOrchestratorAgent());
