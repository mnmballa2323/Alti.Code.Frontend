...
class InfrastructureTestingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureTestingSystem(objective) {
    logger.info(
      `💻 [InfrastructureTestingDirectorAgent] Analyzing Infrastructure Testing Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Testing Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureTestingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureTestingDirectorAgent = Object.freeze(
  new InfrastructureTestingDirectorAgent(),
);
