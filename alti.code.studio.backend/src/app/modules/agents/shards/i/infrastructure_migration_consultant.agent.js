...
class InfrastructureMigrationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureMigrationSystem(objective) {
        logger.info(`💻 [InfrastructureMigrationConsultantAgent] Analyzing Infrastructure Migration Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Migration Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureMigrationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureMigrationConsultantAgent = Object.freeze(new InfrastructureMigrationConsultantAgent());
