...
class InfrastructureConfigurationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureConfigurationSystem(objective) {
        logger.info(`💻 [InfrastructureConfigurationSpecialistAgent] Analyzing Infrastructure Configuration Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Configuration Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureConfigurationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureConfigurationSpecialistAgent = Object.freeze(new InfrastructureConfigurationSpecialistAgent());
