...
class InfrastructureOptimizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureOptimizationSystem(objective) {
    logger.info(
      `💻 [InfrastructureOptimizationConsultantAgent] Analyzing Infrastructure Optimization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Optimization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureOptimizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureOptimizationConsultantAgent = Object.freeze(
  new InfrastructureOptimizationConsultantAgent(),
);
