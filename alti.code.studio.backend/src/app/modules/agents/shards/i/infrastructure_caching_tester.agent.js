...
class InfrastructureCachingTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureCachingSystem(objective) {
    logger.info(
      `💻 [InfrastructureCachingTesterAgent] Analyzing Infrastructure Caching Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Caching Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureCachingTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureCachingTesterAgent = Object.freeze(
  new InfrastructureCachingTesterAgent(),
);
