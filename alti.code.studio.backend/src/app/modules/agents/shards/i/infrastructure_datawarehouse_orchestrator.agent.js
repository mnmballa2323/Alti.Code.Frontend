...
class InfrastructureDataWarehouseOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureDataWarehouseSystem(objective) {
        logger.info(`💻 [InfrastructureDataWarehouseOrchestratorAgent] Analyzing Infrastructure DataWarehouse Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure DataWarehouse Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureDataWarehouseOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureDataWarehouseOrchestratorAgent = Object.freeze(new InfrastructureDataWarehouseOrchestratorAgent());
