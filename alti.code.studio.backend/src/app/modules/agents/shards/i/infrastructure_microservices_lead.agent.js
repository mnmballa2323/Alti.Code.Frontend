...
class InfrastructureMicroservicesLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureMicroservicesSystem(objective) {
        logger.info(`💻 [InfrastructureMicroservicesLeadAgent] Analyzing Infrastructure Microservices Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Microservices Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureMicroservicesLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureMicroservicesLeadAgent = Object.freeze(new InfrastructureMicroservicesLeadAgent());
