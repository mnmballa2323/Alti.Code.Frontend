...
class InfrastructureDataWarehouseEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureDataWarehouseSystem(objective) {
    logger.info(
      `💻 [InfrastructureDataWarehouseEngineerAgent] Analyzing Infrastructure DataWarehouse Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure DataWarehouse Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureDataWarehouseEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureDataWarehouseEngineerAgent = Object.freeze(
  new InfrastructureDataWarehouseEngineerAgent(),
);
