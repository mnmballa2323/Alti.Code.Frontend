...
class InfrastructureComplianceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureComplianceSystem(objective) {
    logger.info(
      `💻 [InfrastructureComplianceSpecialistAgent] Analyzing Infrastructure Compliance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Compliance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureComplianceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureComplianceSpecialistAgent = Object.freeze(
  new InfrastructureComplianceSpecialistAgent(),
);
