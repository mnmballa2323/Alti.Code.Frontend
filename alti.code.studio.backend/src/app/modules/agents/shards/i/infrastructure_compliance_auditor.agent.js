...
class InfrastructureComplianceAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureComplianceSystem(objective) {
        logger.info(`💻 [InfrastructureComplianceAuditorAgent] Analyzing Infrastructure Compliance Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Compliance Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureComplianceAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureComplianceAuditorAgent = Object.freeze(new InfrastructureComplianceAuditorAgent());
