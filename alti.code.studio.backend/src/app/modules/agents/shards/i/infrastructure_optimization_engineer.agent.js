...
class InfrastructureOptimizationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureOptimizationSystem(objective) {
        logger.info(`💻 [InfrastructureOptimizationEngineerAgent] Analyzing Infrastructure Optimization Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Optimization Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureOptimizationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureOptimizationEngineerAgent = Object.freeze(new InfrastructureOptimizationEngineerAgent());
