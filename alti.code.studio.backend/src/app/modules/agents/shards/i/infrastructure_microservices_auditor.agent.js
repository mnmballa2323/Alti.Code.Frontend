...
class InfrastructureMicroservicesAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureMicroservicesSystem(objective) {
    logger.info(
      `💻 [InfrastructureMicroservicesAuditorAgent] Analyzing Infrastructure Microservices Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Microservices Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureMicroservicesAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureMicroservicesAuditorAgent = Object.freeze(
  new InfrastructureMicroservicesAuditorAgent(),
);
