...
class InfrastructureCompliancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureComplianceSystem(objective) {
    logger.info(
      `💻 [InfrastructureCompliancePlannerAgent] Analyzing Infrastructure Compliance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Compliance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureCompliancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureCompliancePlannerAgent = Object.freeze(
  new InfrastructureCompliancePlannerAgent(),
);
