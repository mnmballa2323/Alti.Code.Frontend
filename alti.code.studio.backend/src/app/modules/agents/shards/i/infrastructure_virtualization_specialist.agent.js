...
class InfrastructureVirtualizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureVirtualizationSystem(objective) {
        logger.info(`💻 [InfrastructureVirtualizationSpecialistAgent] Analyzing Infrastructure Virtualization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Virtualization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureVirtualizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureVirtualizationSpecialistAgent = Object.freeze(new InfrastructureVirtualizationSpecialistAgent());
