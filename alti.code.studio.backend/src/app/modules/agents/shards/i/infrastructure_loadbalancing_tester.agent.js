...
class InfrastructureLoadBalancingTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureLoadBalancingSystem(objective) {
    logger.info(
      `💻 [InfrastructureLoadBalancingTesterAgent] Analyzing Infrastructure LoadBalancing Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure LoadBalancing Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureLoadBalancingTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureLoadBalancingTesterAgent = Object.freeze(
  new InfrastructureLoadBalancingTesterAgent(),
);
