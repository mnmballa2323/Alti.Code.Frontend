...
class InfrastructureLoadBalancingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureLoadBalancingSystem(objective) {
    logger.info(
      `💻 [InfrastructureLoadBalancingSpecialistAgent] Analyzing Infrastructure LoadBalancing Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure LoadBalancing Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureLoadBalancingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureLoadBalancingSpecialistAgent = Object.freeze(
  new InfrastructureLoadBalancingSpecialistAgent(),
);
