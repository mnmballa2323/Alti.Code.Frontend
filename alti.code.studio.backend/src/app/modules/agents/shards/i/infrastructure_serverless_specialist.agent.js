...
class InfrastructureServerlessSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureServerlessSystem(objective) {
    logger.info(
      `💻 [InfrastructureServerlessSpecialistAgent] Analyzing Infrastructure Serverless Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Serverless Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureServerlessSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureServerlessSpecialistAgent = Object.freeze(
  new InfrastructureServerlessSpecialistAgent(),
);
