...
class InfrastructureCachingDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureCachingSystem(objective) {
        logger.info(`💻 [InfrastructureCachingDirectorAgent] Analyzing Infrastructure Caching Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Caching Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureCachingDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureCachingDirectorAgent = Object.freeze(new InfrastructureCachingDirectorAgent());
