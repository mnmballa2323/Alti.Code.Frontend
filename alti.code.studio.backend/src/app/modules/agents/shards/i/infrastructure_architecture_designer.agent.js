...
class InfrastructureArchitectureDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureArchitectureSystem(objective) {
    logger.info(
      `💻 [InfrastructureArchitectureDesignerAgent] Analyzing Infrastructure Architecture Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Architecture Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureArchitectureDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureArchitectureDesignerAgent = Object.freeze(
  new InfrastructureArchitectureDesignerAgent(),
);
