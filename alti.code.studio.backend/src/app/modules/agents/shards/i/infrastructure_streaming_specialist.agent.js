...
class InfrastructureStreamingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureStreamingSystem(objective) {
    logger.info(
      `💻 [InfrastructureStreamingSpecialistAgent] Analyzing Infrastructure Streaming Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Streaming Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureStreamingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureStreamingSpecialistAgent = Object.freeze(
  new InfrastructureStreamingSpecialistAgent(),
);
