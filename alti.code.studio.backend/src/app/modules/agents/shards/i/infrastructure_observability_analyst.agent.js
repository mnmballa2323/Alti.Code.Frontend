...
class InfrastructureObservabilityAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureObservabilitySystem(objective) {
    logger.info(
      `💻 [InfrastructureObservabilityAnalystAgent] Analyzing Infrastructure Observability Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Observability Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureObservabilityAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureObservabilityAnalystAgent = Object.freeze(
  new InfrastructureObservabilityAnalystAgent(),
);
