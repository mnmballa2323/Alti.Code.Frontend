...
class InfrastructureMigrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureMigrationSystem(objective) {
    logger.info(
      `💻 [InfrastructureMigrationSpecialistAgent] Analyzing Infrastructure Migration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Migration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureMigrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureMigrationSpecialistAgent = Object.freeze(
  new InfrastructureMigrationSpecialistAgent(),
);
