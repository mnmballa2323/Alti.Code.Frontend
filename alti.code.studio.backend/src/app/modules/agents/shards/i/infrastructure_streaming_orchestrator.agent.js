...
class InfrastructureStreamingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureStreamingSystem(objective) {
    logger.info(
      `💻 [InfrastructureStreamingOrchestratorAgent] Analyzing Infrastructure Streaming Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Streaming Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureStreamingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureStreamingOrchestratorAgent = Object.freeze(
  new InfrastructureStreamingOrchestratorAgent(),
);
