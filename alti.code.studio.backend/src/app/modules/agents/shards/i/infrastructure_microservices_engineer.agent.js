...
class InfrastructureMicroservicesEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureMicroservicesSystem(objective) {
        logger.info(`💻 [InfrastructureMicroservicesEngineerAgent] Analyzing Infrastructure Microservices Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Microservices Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureMicroservicesEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureMicroservicesEngineerAgent = Object.freeze(new InfrastructureMicroservicesEngineerAgent());
