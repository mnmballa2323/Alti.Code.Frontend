...
class InfrastructureBatchProcessingAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureBatchProcessingSystem(objective) {
    logger.info(
      `💻 [InfrastructureBatchProcessingAuditorAgent] Analyzing Infrastructure BatchProcessing Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure BatchProcessing Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureBatchProcessingAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureBatchProcessingAuditorAgent = Object.freeze(
  new InfrastructureBatchProcessingAuditorAgent(),
);
