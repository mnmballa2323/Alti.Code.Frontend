...
class InfrastructureOrchestrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureOrchestrationSystem(objective) {
    logger.info(
      `💻 [InfrastructureOrchestrationEngineerAgent] Analyzing Infrastructure Orchestration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Orchestration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureOrchestrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureOrchestrationEngineerAgent = Object.freeze(
  new InfrastructureOrchestrationEngineerAgent(),
);
