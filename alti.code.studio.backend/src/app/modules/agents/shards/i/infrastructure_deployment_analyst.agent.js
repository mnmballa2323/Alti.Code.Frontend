...
class InfrastructureDeploymentAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureDeploymentSystem(objective) {
    logger.info(
      `💻 [InfrastructureDeploymentAnalystAgent] Analyzing Infrastructure Deployment Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Deployment Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureDeploymentAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureDeploymentAnalystAgent = Object.freeze(
  new InfrastructureDeploymentAnalystAgent(),
);
