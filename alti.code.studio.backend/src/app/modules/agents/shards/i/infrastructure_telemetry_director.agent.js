...
class InfrastructureTelemetryDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureTelemetrySystem(objective) {
        logger.info(`💻 [InfrastructureTelemetryDirectorAgent] Analyzing Infrastructure Telemetry Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Telemetry Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureTelemetryDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureTelemetryDirectorAgent = Object.freeze(new InfrastructureTelemetryDirectorAgent());
