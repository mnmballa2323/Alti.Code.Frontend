...
class InfrastructureBatchProcessingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureBatchProcessingSystem(objective) {
    logger.info(
      `💻 [InfrastructureBatchProcessingDesignerAgent] Analyzing Infrastructure BatchProcessing Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure BatchProcessing Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureBatchProcessingDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureBatchProcessingDesignerAgent = Object.freeze(
  new InfrastructureBatchProcessingDesignerAgent(),
);
