...
class InfrastructureDataWarehouseTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureDataWarehouseSystem(objective) {
        logger.info(`💻 [InfrastructureDataWarehouseTesterAgent] Analyzing Infrastructure DataWarehouse Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure DataWarehouse Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureDataWarehouseTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureDataWarehouseTesterAgent = Object.freeze(new InfrastructureDataWarehouseTesterAgent());
