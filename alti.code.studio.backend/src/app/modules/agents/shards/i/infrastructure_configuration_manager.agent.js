...
class InfrastructureConfigurationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureConfigurationSystem(objective) {
    logger.info(
      `💻 [InfrastructureConfigurationManagerAgent] Analyzing Infrastructure Configuration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Configuration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureConfigurationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureConfigurationManagerAgent = Object.freeze(
  new InfrastructureConfigurationManagerAgent(),
);
