...
class InfrastructureFaultToleranceAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureFaultToleranceSystem(objective) {
        logger.info(`💻 [InfrastructureFaultToleranceAuditorAgent] Analyzing Infrastructure FaultTolerance Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure FaultTolerance Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureFaultToleranceAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureFaultToleranceAuditorAgent = Object.freeze(new InfrastructureFaultToleranceAuditorAgent());
