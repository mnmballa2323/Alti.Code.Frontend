...
class InfrastructureStreamingDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureStreamingSystem(objective) {
        logger.info(`💻 [InfrastructureStreamingDesignerAgent] Analyzing Infrastructure Streaming Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Streaming Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureStreamingDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureStreamingDesignerAgent = Object.freeze(new InfrastructureStreamingDesignerAgent());
