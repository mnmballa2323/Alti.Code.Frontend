...
class InfrastructureArchitectureTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureArchitectureSystem(objective) {
    logger.info(
      `💻 [InfrastructureArchitectureTesterAgent] Analyzing Infrastructure Architecture Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Architecture Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureArchitectureTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureArchitectureTesterAgent = Object.freeze(
  new InfrastructureArchitectureTesterAgent(),
);
