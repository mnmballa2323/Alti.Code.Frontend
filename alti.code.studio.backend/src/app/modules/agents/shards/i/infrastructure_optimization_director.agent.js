...
class InfrastructureOptimizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureOptimizationSystem(objective) {
    logger.info(
      `💻 [InfrastructureOptimizationDirectorAgent] Analyzing Infrastructure Optimization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Optimization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureOptimizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureOptimizationDirectorAgent = Object.freeze(
  new InfrastructureOptimizationDirectorAgent(),
);
