...
class InfrastructureContainerizationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureContainerizationSystem(objective) {
        logger.info(`💻 [InfrastructureContainerizationStrategistAgent] Analyzing Infrastructure Containerization Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Containerization Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureContainerizationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureContainerizationStrategistAgent = Object.freeze(new InfrastructureContainerizationStrategistAgent());
