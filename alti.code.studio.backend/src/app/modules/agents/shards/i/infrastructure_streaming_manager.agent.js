...
class InfrastructureStreamingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureStreamingSystem(objective) {
    logger.info(
      `💻 [InfrastructureStreamingManagerAgent] Analyzing Infrastructure Streaming Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Streaming Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureStreamingManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureStreamingManagerAgent = Object.freeze(
  new InfrastructureStreamingManagerAgent(),
);
