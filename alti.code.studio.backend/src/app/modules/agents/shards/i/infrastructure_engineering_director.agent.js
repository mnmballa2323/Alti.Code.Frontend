...
class InfrastructureEngineeringDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureEngineeringSystem(objective) {
        logger.info(`💻 [InfrastructureEngineeringDirectorAgent] Analyzing Infrastructure Engineering Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Engineering Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureEngineeringDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureEngineeringDirectorAgent = Object.freeze(new InfrastructureEngineeringDirectorAgent());
