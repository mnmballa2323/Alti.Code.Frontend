...
class InfrastructureProvisioningManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureProvisioningSystem(objective) {
        logger.info(`💻 [InfrastructureProvisioningManagerAgent] Analyzing Infrastructure Provisioning Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Provisioning Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureProvisioningManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureProvisioningManagerAgent = Object.freeze(new InfrastructureProvisioningManagerAgent());
