...
class InfrastructureComplianceDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureComplianceSystem(objective) {
    logger.info(
      `💻 [InfrastructureComplianceDeveloperAgent] Analyzing Infrastructure Compliance Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Compliance Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureComplianceDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureComplianceDeveloperAgent = Object.freeze(
  new InfrastructureComplianceDeveloperAgent(),
);
