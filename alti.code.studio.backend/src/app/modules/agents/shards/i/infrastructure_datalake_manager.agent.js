...
class InfrastructureDataLakeManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureDataLakeSystem(objective) {
    logger.info(
      `💻 [InfrastructureDataLakeManagerAgent] Analyzing Infrastructure DataLake Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure DataLake Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureDataLakeManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureDataLakeManagerAgent = Object.freeze(
  new InfrastructureDataLakeManagerAgent(),
);
