...
class InfrastructureDataLakeConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureDataLakeSystem(objective) {
    logger.info(
      `💻 [InfrastructureDataLakeConsultantAgent] Analyzing Infrastructure DataLake Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure DataLake Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureDataLakeConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureDataLakeConsultantAgent = Object.freeze(
  new InfrastructureDataLakeConsultantAgent(),
);
