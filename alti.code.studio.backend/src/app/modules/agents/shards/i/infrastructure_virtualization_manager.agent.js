...
class InfrastructureVirtualizationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureVirtualizationSystem(objective) {
    logger.info(
      `💻 [InfrastructureVirtualizationManagerAgent] Analyzing Infrastructure Virtualization Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Virtualization Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureVirtualizationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureVirtualizationManagerAgent = Object.freeze(
  new InfrastructureVirtualizationManagerAgent(),
);
