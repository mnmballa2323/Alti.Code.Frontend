...
class InfrastructureSecurityLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureSecuritySystem(objective) {
    logger.info(
      `💻 [InfrastructureSecurityLeadAgent] Analyzing Infrastructure Security Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Security Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureSecurityLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureSecurityLeadAgent = Object.freeze(
  new InfrastructureSecurityLeadAgent(),
);
