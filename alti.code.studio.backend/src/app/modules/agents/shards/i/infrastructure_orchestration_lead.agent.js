...
class InfrastructureOrchestrationLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureOrchestrationSystem(objective) {
        logger.info(`💻 [InfrastructureOrchestrationLeadAgent] Analyzing Infrastructure Orchestration Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Orchestration Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureOrchestrationLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureOrchestrationLeadAgent = Object.freeze(new InfrastructureOrchestrationLeadAgent());
