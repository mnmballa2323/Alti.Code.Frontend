...
class InfrastructureOrchestrationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureOrchestrationSystem(objective) {
        logger.info(`💻 [InfrastructureOrchestrationConsultantAgent] Analyzing Infrastructure Orchestration Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Orchestration Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureOrchestrationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureOrchestrationConsultantAgent = Object.freeze(new InfrastructureOrchestrationConsultantAgent());
