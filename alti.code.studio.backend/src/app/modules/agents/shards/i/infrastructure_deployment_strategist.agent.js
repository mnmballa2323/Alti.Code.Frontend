...
class InfrastructureDeploymentStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureDeploymentSystem(objective) {
        logger.info(`💻 [InfrastructureDeploymentStrategistAgent] Analyzing Infrastructure Deployment Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Deployment Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureDeploymentStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureDeploymentStrategistAgent = Object.freeze(new InfrastructureDeploymentStrategistAgent());
