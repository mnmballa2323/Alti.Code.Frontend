...
class InfrastructureLoadBalancingPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureLoadBalancingSystem(objective) {
        logger.info(`💻 [InfrastructureLoadBalancingPlannerAgent] Analyzing Infrastructure LoadBalancing Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure LoadBalancing Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureLoadBalancingPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureLoadBalancingPlannerAgent = Object.freeze(new InfrastructureLoadBalancingPlannerAgent());
