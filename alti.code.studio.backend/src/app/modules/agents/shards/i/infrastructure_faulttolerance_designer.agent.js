...
class InfrastructureFaultToleranceDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureFaultToleranceSystem(objective) {
        logger.info(`💻 [InfrastructureFaultToleranceDesignerAgent] Analyzing Infrastructure FaultTolerance Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure FaultTolerance Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureFaultToleranceDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureFaultToleranceDesignerAgent = Object.freeze(new InfrastructureFaultToleranceDesignerAgent());
