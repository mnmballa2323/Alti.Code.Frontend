...
class InfrastructureTestingLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureTestingSystem(objective) {
    logger.info(
      `💻 [InfrastructureTestingLeadAgent] Analyzing Infrastructure Testing Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Testing Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureTestingLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureTestingLeadAgent = Object.freeze(
  new InfrastructureTestingLeadAgent(),
);
