...
class InfrastructureVirtualizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureVirtualizationSystem(objective) {
    logger.info(
      `💻 [InfrastructureVirtualizationPlannerAgent] Analyzing Infrastructure Virtualization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Virtualization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureVirtualizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureVirtualizationPlannerAgent = Object.freeze(
  new InfrastructureVirtualizationPlannerAgent(),
);
