...
class InfrastructureObservabilityDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureObservabilitySystem(objective) {
    logger.info(
      `💻 [InfrastructureObservabilityDesignerAgent] Analyzing Infrastructure Observability Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Observability Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureObservabilityDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureObservabilityDesignerAgent = Object.freeze(
  new InfrastructureObservabilityDesignerAgent(),
);
