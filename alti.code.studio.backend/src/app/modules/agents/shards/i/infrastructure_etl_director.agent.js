...
class InfrastructureETLDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureETLSystem(objective) {
    logger.info(
      `💻 [InfrastructureETLDirectorAgent] Analyzing Infrastructure ETL Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure ETL Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureETLDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureETLDirectorAgent = Object.freeze(
  new InfrastructureETLDirectorAgent(),
);
