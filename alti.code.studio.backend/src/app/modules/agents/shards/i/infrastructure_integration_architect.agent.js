...
class InfrastructureIntegrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureIntegrationSystem(objective) {
    logger.info(
      `💻 [InfrastructureIntegrationArchitectAgent] Analyzing Infrastructure Integration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Integration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureIntegrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureIntegrationArchitectAgent = Object.freeze(
  new InfrastructureIntegrationArchitectAgent(),
);
