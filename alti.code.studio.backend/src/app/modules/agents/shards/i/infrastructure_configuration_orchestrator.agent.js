...
class InfrastructureConfigurationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureConfigurationSystem(objective) {
    logger.info(
      `💻 [InfrastructureConfigurationOrchestratorAgent] Analyzing Infrastructure Configuration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Configuration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureConfigurationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureConfigurationOrchestratorAgent = Object.freeze(
  new InfrastructureConfigurationOrchestratorAgent(),
);
