...
class InfrastructureCachingEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureCachingSystem(objective) {
        logger.info(`💻 [InfrastructureCachingEngineerAgent] Analyzing Infrastructure Caching Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Caching Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureCachingEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureCachingEngineerAgent = Object.freeze(new InfrastructureCachingEngineerAgent());
