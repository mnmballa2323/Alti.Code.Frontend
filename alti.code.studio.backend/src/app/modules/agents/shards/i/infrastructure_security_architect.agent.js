...
class InfrastructureSecurityArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureSecuritySystem(objective) {
    logger.info(
      `💻 [InfrastructureSecurityArchitectAgent] Analyzing Infrastructure Security Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Security Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureSecurityArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureSecurityArchitectAgent = Object.freeze(
  new InfrastructureSecurityArchitectAgent(),
);
