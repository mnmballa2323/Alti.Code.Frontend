...
class InfrastructureProvisioningLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureProvisioningSystem(objective) {
        logger.info(`💻 [InfrastructureProvisioningLeadAgent] Analyzing Infrastructure Provisioning Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Provisioning Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureProvisioningLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureProvisioningLeadAgent = Object.freeze(new InfrastructureProvisioningLeadAgent());
