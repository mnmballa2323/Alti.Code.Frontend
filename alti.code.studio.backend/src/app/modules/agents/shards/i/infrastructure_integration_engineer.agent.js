...
class InfrastructureIntegrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureIntegrationSystem(objective) {
    logger.info(
      `💻 [InfrastructureIntegrationEngineerAgent] Analyzing Infrastructure Integration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Integration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureIntegrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureIntegrationEngineerAgent = Object.freeze(
  new InfrastructureIntegrationEngineerAgent(),
);
