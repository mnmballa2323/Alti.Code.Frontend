...
class InfrastructureTestingDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureTestingSystem(objective) {
        logger.info(`💻 [InfrastructureTestingDesignerAgent] Analyzing Infrastructure Testing Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Testing Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureTestingDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureTestingDesignerAgent = Object.freeze(new InfrastructureTestingDesignerAgent());
