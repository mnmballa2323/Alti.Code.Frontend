...
class InfrastructureObservabilityDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateInfrastructureObservabilitySystem(objective) {
    logger.info(
      `💻 [InfrastructureObservabilityDeveloperAgent] Analyzing Infrastructure Observability Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Infrastructure Observability Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [InfrastructureObservabilityDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const infrastructureObservabilityDeveloperAgent = Object.freeze(
  new InfrastructureObservabilityDeveloperAgent(),
);
