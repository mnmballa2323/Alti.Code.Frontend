...
class InfrastructureStreamingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateInfrastructureStreamingSystem(objective) {
        logger.info(`💻 [InfrastructureStreamingConsultantAgent] Analyzing Infrastructure Streaming Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Infrastructure Streaming Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [InfrastructureStreamingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const infrastructureStreamingConsultantAgent = Object.freeze(new InfrastructureStreamingConsultantAgent());
