...
class CloudOrchestrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudOrchestrationSystem(objective) {
    logger.info(
      `💻 [CloudOrchestrationEngineerAgent] Analyzing Cloud Orchestration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Orchestration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudOrchestrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudOrchestrationEngineerAgent = Object.freeze(
  new CloudOrchestrationEngineerAgent(),
);
