...
class CloudProvisioningOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudProvisioningSystem(objective) {
    logger.info(
      `💻 [CloudProvisioningOrchestratorAgent] Analyzing Cloud Provisioning Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Provisioning Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudProvisioningOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudProvisioningOrchestratorAgent = Object.freeze(
  new CloudProvisioningOrchestratorAgent(),
);
