...
class CloudStreamingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudStreamingSystem(objective) {
    logger.info(
      `💻 [CloudStreamingOrchestratorAgent] Analyzing Cloud Streaming Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Streaming Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudStreamingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudStreamingOrchestratorAgent = Object.freeze(
  new CloudStreamingOrchestratorAgent(),
);
