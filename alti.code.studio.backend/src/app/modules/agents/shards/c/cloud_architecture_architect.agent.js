...
class CloudArchitectureArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudArchitectureSystem(objective) {
    logger.info(
      `💻 [CloudArchitectureArchitectAgent] Analyzing Cloud Architecture Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Architecture Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudArchitectureArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudArchitectureArchitectAgent = Object.freeze(
  new CloudArchitectureArchitectAgent(),
);
