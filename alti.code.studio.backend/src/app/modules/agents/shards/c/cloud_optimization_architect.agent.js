...
class CloudOptimizationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudOptimizationSystem(objective) {
        logger.info(`💻 [CloudOptimizationArchitectAgent] Analyzing Cloud Optimization Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Optimization Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudOptimizationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudOptimizationArchitectAgent = Object.freeze(new CloudOptimizationArchitectAgent());
