...
class CloudOrchestrationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudOrchestrationSystem(objective) {
    logger.info(
      `💻 [CloudOrchestrationAnalystAgent] Analyzing Cloud Orchestration Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Orchestration Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudOrchestrationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudOrchestrationAnalystAgent = Object.freeze(
  new CloudOrchestrationAnalystAgent(),
);
