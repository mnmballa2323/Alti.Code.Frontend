...
class CloudObservabilityAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudObservabilitySystem(objective) {
        logger.info(`💻 [CloudObservabilityAuditorAgent] Analyzing Cloud Observability Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Observability Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudObservabilityAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudObservabilityAuditorAgent = Object.freeze(new CloudObservabilityAuditorAgent());
