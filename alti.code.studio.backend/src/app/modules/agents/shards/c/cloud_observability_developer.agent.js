...
class CloudObservabilityDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudObservabilitySystem(objective) {
    logger.info(
      `💻 [CloudObservabilityDeveloperAgent] Analyzing Cloud Observability Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Observability Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudObservabilityDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudObservabilityDeveloperAgent = Object.freeze(
  new CloudObservabilityDeveloperAgent(),
);
