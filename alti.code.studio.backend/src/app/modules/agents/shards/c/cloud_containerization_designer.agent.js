...
class CloudContainerizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudContainerizationSystem(objective) {
    logger.info(
      `💻 [CloudContainerizationDesignerAgent] Analyzing Cloud Containerization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Containerization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudContainerizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudContainerizationDesignerAgent = Object.freeze(
  new CloudContainerizationDesignerAgent(),
);
