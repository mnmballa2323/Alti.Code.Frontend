...
class CloudEngineeringConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudEngineeringSystem(objective) {
    logger.info(
      `💻 [CloudEngineeringConsultantAgent] Analyzing Cloud Engineering Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Engineering Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudEngineeringConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudEngineeringConsultantAgent = Object.freeze(
  new CloudEngineeringConsultantAgent(),
);
