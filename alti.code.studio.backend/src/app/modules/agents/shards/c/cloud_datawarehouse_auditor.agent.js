...
class CloudDataWarehouseAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudDataWarehouseSystem(objective) {
    logger.info(
      `💻 [CloudDataWarehouseAuditorAgent] Analyzing Cloud DataWarehouse Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud DataWarehouse Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudDataWarehouseAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudDataWarehouseAuditorAgent = Object.freeze(
  new CloudDataWarehouseAuditorAgent(),
);
