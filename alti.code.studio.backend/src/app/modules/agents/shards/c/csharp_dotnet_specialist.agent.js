...
class CsharpDotnetSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateCsharpSystem(objective) {
    logger.info(
      `🔷 [CsharpDotnetSpecialist] Analyzing LINQ expressions and .NET Core runtime...`,
    );
    ...
    try {
      ...
      return output
        .replace(
          /```javascript|```typescript|```json|```csharp|```cs|```/gi,
          '',
        )
        .trim();
    } catch (err) {
...
export const csharpDotnetSpecialistAgent = Object.freeze(
