...
class CloudConfigurationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudConfigurationSystem(objective) {
        logger.info(`💻 [CloudConfigurationDesignerAgent] Analyzing Cloud Configuration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Configuration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudConfigurationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudConfigurationDesignerAgent = Object.freeze(new CloudConfigurationDesignerAgent());
