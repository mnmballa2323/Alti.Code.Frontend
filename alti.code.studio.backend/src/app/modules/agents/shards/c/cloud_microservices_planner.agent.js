...
class CloudMicroservicesPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudMicroservicesSystem(objective) {
    logger.info(
      `💻 [CloudMicroservicesPlannerAgent] Analyzing Cloud Microservices Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Microservices Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudMicroservicesPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudMicroservicesPlannerAgent = Object.freeze(
  new CloudMicroservicesPlannerAgent(),
);
