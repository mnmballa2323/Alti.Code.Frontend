...
class CloudMicroservicesAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudMicroservicesSystem(objective) {
    logger.info(
      `💻 [CloudMicroservicesAuditorAgent] Analyzing Cloud Microservices Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Microservices Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudMicroservicesAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudMicroservicesAuditorAgent = Object.freeze(
  new CloudMicroservicesAuditorAgent(),
);
