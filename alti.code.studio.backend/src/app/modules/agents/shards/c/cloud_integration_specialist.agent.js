...
class CloudIntegrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudIntegrationSystem(objective) {
    logger.info(
      `💻 [CloudIntegrationSpecialistAgent] Analyzing Cloud Integration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Integration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudIntegrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudIntegrationSpecialistAgent = Object.freeze(
  new CloudIntegrationSpecialistAgent(),
);
