...
class CloudOptimizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudOptimizationSystem(objective) {
    logger.info(
      `💻 [CloudOptimizationDirectorAgent] Analyzing Cloud Optimization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Optimization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudOptimizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudOptimizationDirectorAgent = Object.freeze(
  new CloudOptimizationDirectorAgent(),
);
