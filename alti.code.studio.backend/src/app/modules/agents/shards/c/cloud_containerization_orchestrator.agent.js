...
class CloudContainerizationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudContainerizationSystem(objective) {
        logger.info(`💻 [CloudContainerizationOrchestratorAgent] Analyzing Cloud Containerization Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Containerization Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudContainerizationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudContainerizationOrchestratorAgent = Object.freeze(new CloudContainerizationOrchestratorAgent());
