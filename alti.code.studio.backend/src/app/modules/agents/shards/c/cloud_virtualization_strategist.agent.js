...
class CloudVirtualizationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudVirtualizationSystem(objective) {
    logger.info(
      `💻 [CloudVirtualizationStrategistAgent] Analyzing Cloud Virtualization Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Virtualization Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudVirtualizationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudVirtualizationStrategistAgent = Object.freeze(
  new CloudVirtualizationStrategistAgent(),
);
