...
class CloudflareEdgeEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudflareSystem(cloudflareObjective) {
    logger.info(
      `☁️ [CloudflareEdgeEngineer] Analyzing objective for Edge Compute Workers and Zero Trust...`,
    );
...
${cloudflareObjective}
    ...
    try {
      ...
      const cleanCode = output
        .replace(/```javascript|```typescript|```json|```toml|```/gi, '')
        .trim();
      logger.info(
        `✅ [CloudflareEdgeEngineer] Cloudflare architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [CloudflareEdgeEngineer] Failed to generate Cloudflare system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const cloudflareEdgeEngineerAgent = Object.freeze(
  new CloudflareEdgeEngineerAgent(),
);
