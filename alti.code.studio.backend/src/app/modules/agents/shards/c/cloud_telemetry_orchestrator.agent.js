...
class CloudTelemetryOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudTelemetrySystem(objective) {
    logger.info(
      `💻 [CloudTelemetryOrchestratorAgent] Analyzing Cloud Telemetry Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Telemetry Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudTelemetryOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudTelemetryOrchestratorAgent = Object.freeze(
  new CloudTelemetryOrchestratorAgent(),
);
