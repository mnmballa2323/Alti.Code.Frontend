...
class CloudMicroservicesManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudMicroservicesSystem(objective) {
    logger.info(
      `💻 [CloudMicroservicesManagerAgent] Analyzing Cloud Microservices Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Microservices Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudMicroservicesManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudMicroservicesManagerAgent = Object.freeze(
  new CloudMicroservicesManagerAgent(),
);
