...
class CloudOptimizationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudOptimizationSystem(objective) {
    logger.info(
      `💻 [CloudOptimizationDeveloperAgent] Analyzing Cloud Optimization Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Optimization Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudOptimizationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudOptimizationDeveloperAgent = Object.freeze(
  new CloudOptimizationDeveloperAgent(),
);
