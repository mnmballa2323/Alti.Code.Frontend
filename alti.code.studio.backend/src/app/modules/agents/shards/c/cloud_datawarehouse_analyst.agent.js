...
class CloudDataWarehouseAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudDataWarehouseSystem(objective) {
    logger.info(
      `💻 [CloudDataWarehouseAnalystAgent] Analyzing Cloud DataWarehouse Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud DataWarehouse Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudDataWarehouseAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudDataWarehouseAnalystAgent = Object.freeze(
  new CloudDataWarehouseAnalystAgent(),
);
