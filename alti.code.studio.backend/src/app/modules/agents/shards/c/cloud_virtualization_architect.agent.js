...
class CloudVirtualizationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudVirtualizationSystem(objective) {
        logger.info(`💻 [CloudVirtualizationArchitectAgent] Analyzing Cloud Virtualization Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Virtualization Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudVirtualizationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudVirtualizationArchitectAgent = Object.freeze(new CloudVirtualizationArchitectAgent());
