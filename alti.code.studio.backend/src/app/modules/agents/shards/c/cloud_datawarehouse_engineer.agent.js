...
class CloudDataWarehouseEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudDataWarehouseSystem(objective) {
        logger.info(`💻 [CloudDataWarehouseEngineerAgent] Analyzing Cloud DataWarehouse Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud DataWarehouse Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudDataWarehouseEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudDataWarehouseEngineerAgent = Object.freeze(new CloudDataWarehouseEngineerAgent());
