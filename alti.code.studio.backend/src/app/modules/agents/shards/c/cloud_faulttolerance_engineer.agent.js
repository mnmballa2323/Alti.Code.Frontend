...
class CloudFaultToleranceEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudFaultToleranceSystem(objective) {
        logger.info(`💻 [CloudFaultToleranceEngineerAgent] Analyzing Cloud FaultTolerance Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud FaultTolerance Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudFaultToleranceEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudFaultToleranceEngineerAgent = Object.freeze(new CloudFaultToleranceEngineerAgent());
