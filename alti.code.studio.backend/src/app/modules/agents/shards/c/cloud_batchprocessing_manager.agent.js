...
class CloudBatchProcessingManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudBatchProcessingSystem(objective) {
        logger.info(`💻 [CloudBatchProcessingManagerAgent] Analyzing Cloud BatchProcessing Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud BatchProcessing Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudBatchProcessingManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudBatchProcessingManagerAgent = Object.freeze(new CloudBatchProcessingManagerAgent());
