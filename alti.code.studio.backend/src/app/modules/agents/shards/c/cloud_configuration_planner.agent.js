...
class CloudConfigurationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudConfigurationSystem(objective) {
    logger.info(
      `💻 [CloudConfigurationPlannerAgent] Analyzing Cloud Configuration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Configuration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudConfigurationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudConfigurationPlannerAgent = Object.freeze(
  new CloudConfigurationPlannerAgent(),
);
