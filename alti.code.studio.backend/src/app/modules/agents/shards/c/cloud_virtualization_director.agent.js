...
class CloudVirtualizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudVirtualizationSystem(objective) {
        logger.info(`💻 [CloudVirtualizationDirectorAgent] Analyzing Cloud Virtualization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Virtualization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudVirtualizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudVirtualizationDirectorAgent = Object.freeze(new CloudVirtualizationDirectorAgent());
