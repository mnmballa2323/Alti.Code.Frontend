...
class CloudAutomationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudAutomationSystem(objective) {
        logger.info(`💻 [CloudAutomationStrategistAgent] Analyzing Cloud Automation Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Automation Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudAutomationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudAutomationStrategistAgent = Object.freeze(new CloudAutomationStrategistAgent());
