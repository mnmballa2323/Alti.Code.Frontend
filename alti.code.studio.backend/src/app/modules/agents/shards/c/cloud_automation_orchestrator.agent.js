...
class CloudAutomationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudAutomationSystem(objective) {
    logger.info(
      `💻 [CloudAutomationOrchestratorAgent] Analyzing Cloud Automation Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Automation Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudAutomationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudAutomationOrchestratorAgent = Object.freeze(
  new CloudAutomationOrchestratorAgent(),
);
