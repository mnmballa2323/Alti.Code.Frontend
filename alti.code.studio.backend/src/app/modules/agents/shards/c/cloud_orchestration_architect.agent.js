...
class CloudOrchestrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudOrchestrationSystem(objective) {
    logger.info(
      `💻 [CloudOrchestrationArchitectAgent] Analyzing Cloud Orchestration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Orchestration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudOrchestrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudOrchestrationArchitectAgent = Object.freeze(
  new CloudOrchestrationArchitectAgent(),
);
