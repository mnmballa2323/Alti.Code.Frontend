...
class CloudConfigurationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudConfigurationSystem(objective) {
    logger.info(
      `💻 [CloudConfigurationManagerAgent] Analyzing Cloud Configuration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Configuration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudConfigurationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudConfigurationManagerAgent = Object.freeze(
  new CloudConfigurationManagerAgent(),
);
