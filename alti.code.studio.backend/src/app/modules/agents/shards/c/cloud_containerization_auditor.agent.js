...
class CloudContainerizationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudContainerizationSystem(objective) {
        logger.info(`💻 [CloudContainerizationAuditorAgent] Analyzing Cloud Containerization Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Containerization Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudContainerizationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudContainerizationAuditorAgent = Object.freeze(new CloudContainerizationAuditorAgent());
