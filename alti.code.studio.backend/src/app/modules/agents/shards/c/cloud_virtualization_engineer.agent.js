...
class CloudVirtualizationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudVirtualizationSystem(objective) {
    logger.info(
      `💻 [CloudVirtualizationEngineerAgent] Analyzing Cloud Virtualization Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Virtualization Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudVirtualizationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudVirtualizationEngineerAgent = Object.freeze(
  new CloudVirtualizationEngineerAgent(),
);
