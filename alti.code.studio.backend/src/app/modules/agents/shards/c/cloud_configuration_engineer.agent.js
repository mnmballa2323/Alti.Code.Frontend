...
class CloudConfigurationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudConfigurationSystem(objective) {
    logger.info(
      `💻 [CloudConfigurationEngineerAgent] Analyzing Cloud Configuration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Configuration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudConfigurationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudConfigurationEngineerAgent = Object.freeze(
  new CloudConfigurationEngineerAgent(),
);
