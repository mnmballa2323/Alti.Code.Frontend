...
class CloudFaultToleranceTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudFaultToleranceSystem(objective) {
    logger.info(
      `💻 [CloudFaultToleranceTesterAgent] Analyzing Cloud FaultTolerance Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud FaultTolerance Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudFaultToleranceTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudFaultToleranceTesterAgent = Object.freeze(
  new CloudFaultToleranceTesterAgent(),
);
