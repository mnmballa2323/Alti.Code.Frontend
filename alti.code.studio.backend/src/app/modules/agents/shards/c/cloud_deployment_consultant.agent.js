...
class CloudDeploymentConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudDeploymentSystem(objective) {
    logger.info(
      `💻 [CloudDeploymentConsultantAgent] Analyzing Cloud Deployment Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Deployment Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudDeploymentConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudDeploymentConsultantAgent = Object.freeze(
  new CloudDeploymentConsultantAgent(),
);
