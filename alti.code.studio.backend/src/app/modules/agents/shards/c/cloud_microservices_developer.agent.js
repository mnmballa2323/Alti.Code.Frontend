...
class CloudMicroservicesDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudMicroservicesSystem(objective) {
        logger.info(`💻 [CloudMicroservicesDeveloperAgent] Analyzing Cloud Microservices Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Microservices Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudMicroservicesDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudMicroservicesDeveloperAgent = Object.freeze(new CloudMicroservicesDeveloperAgent());
