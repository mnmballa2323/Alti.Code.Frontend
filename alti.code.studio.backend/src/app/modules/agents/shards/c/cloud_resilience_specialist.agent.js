...
class CloudResilienceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudResilienceSystem(objective) {
    logger.info(
      `💻 [CloudResilienceSpecialistAgent] Analyzing Cloud Resilience Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Resilience Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudResilienceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudResilienceSpecialistAgent = Object.freeze(
  new CloudResilienceSpecialistAgent(),
);
