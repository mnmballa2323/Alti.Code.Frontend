...
class CloudVirtualizationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudVirtualizationSystem(objective) {
    logger.info(
      `💻 [CloudVirtualizationDeveloperAgent] Analyzing Cloud Virtualization Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Virtualization Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudVirtualizationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudVirtualizationDeveloperAgent = Object.freeze(
  new CloudVirtualizationDeveloperAgent(),
);
