...
class CloudContainerizationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudContainerizationSystem(objective) {
    logger.info(
      `💻 [CloudContainerizationEngineerAgent] Analyzing Cloud Containerization Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Containerization Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudContainerizationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudContainerizationEngineerAgent = Object.freeze(
  new CloudContainerizationEngineerAgent(),
);
