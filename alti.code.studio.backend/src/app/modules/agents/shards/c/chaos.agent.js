...
export class ChaosAgent {
  ...
  async injectFault(scenario = 'db_timeout') {
    logger.warn(
      `🔥 Chaos Agent: Authorized for active fault injection. Commencing scenario: [${scenario.toUpperCase()}]`,
    );

    try {
      switch (scenario) {
        case 'db_timeout':
          this._simulateLogSpike(
            'ERROR',
            'Timeout acquiring connection from database pool (pg-cluster-main)',
            5,
          );
          break;
        case '500_storm':
          this._simulateLogSpike(
            'CRITICAL',
            '500 Internal Server Error: Gateway timeout from downstream auth service',
            10,
          );
          break;
        case 'memory_leak':
          this._simulateLogSpike(
            'WARN',
            'V8 Heap Memory exceeded 90% threshold. GC thrashing detected.',
            3,
          );
          break;
        default:
          logger.warn(
            `🔥 Chaos Agent: Unknown scenario '${scenario}'. Defaulting to slight latency fuzzing.`,
          );
          this._simulateLogSpike(
            'WARN',
            'API response time degraded > 2500ms',
            2,
          );
      }

      logger.info(
        `🔥 Chaos Agent: Fault successfully injected into global telemetry buffer.`,
      );
  ...
  _simulateLogSpike(level, message, count) {
    if (!global.observabilityService) {
      logger.warn(
        '🔥 Chaos Agent: Global observability service not mounted. Mocking locally.',
      );
    }
  ...
  async process(state) {
    ...
    return {
      ...
      results: [
        ...(state.results || []),
        `Chaos Scenario Injected: ${scenario}`,
      ],
    };
  }
}

export const chaosAgent = Object.freeze(new ChaosAgent());
