...
class CloudArchitectureSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudArchitectureSystem(objective) {
    logger.info(
      `💻 [CloudArchitectureSpecialistAgent] Analyzing Cloud Architecture Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Architecture Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudArchitectureSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudArchitectureSpecialistAgent = Object.freeze(
  new CloudArchitectureSpecialistAgent(),
);
