...
class ComputationalSociologistAgent extends GeminiCliBaseAgent {
  ...
  async generateSociologySystem(sociologyObjective) {
    logger.info(
      `📊 [ComputationalSociologist] Analyzing objective for behavioral modeling and population dynamics...`,
    );
...
${sociologyObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [ComputationalSociologist] Sociology architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [ComputationalSociologist] Failed to generate sociology system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const computationalSociologistAgent = Object.freeze(
  new ComputationalSociologistAgent(),
);
