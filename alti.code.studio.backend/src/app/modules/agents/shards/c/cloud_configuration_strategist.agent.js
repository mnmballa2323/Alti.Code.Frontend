...
class CloudConfigurationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudConfigurationSystem(objective) {
        logger.info(`💻 [CloudConfigurationStrategistAgent] Analyzing Cloud Configuration Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Configuration Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudConfigurationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudConfigurationStrategistAgent = Object.freeze(new CloudConfigurationStrategistAgent());
