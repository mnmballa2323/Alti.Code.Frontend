...
class CloudOptimizationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudOptimizationSystem(objective) {
    logger.info(
      `💻 [CloudOptimizationEngineerAgent] Analyzing Cloud Optimization Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Optimization Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudOptimizationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudOptimizationEngineerAgent = Object.freeze(
  new CloudOptimizationEngineerAgent(),
);
