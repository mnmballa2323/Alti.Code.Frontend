...
class CloudMicroservicesSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudMicroservicesSystem(objective) {
        logger.info(`💻 [CloudMicroservicesSpecialistAgent] Analyzing Cloud Microservices Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Microservices Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudMicroservicesSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudMicroservicesSpecialistAgent = Object.freeze(new CloudMicroservicesSpecialistAgent());
