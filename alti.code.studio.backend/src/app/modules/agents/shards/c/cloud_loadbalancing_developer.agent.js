...
class CloudLoadBalancingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudLoadBalancingSystem(objective) {
    logger.info(
      `💻 [CloudLoadBalancingDeveloperAgent] Analyzing Cloud LoadBalancing Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud LoadBalancing Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudLoadBalancingDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudLoadBalancingDeveloperAgent = Object.freeze(
  new CloudLoadBalancingDeveloperAgent(),
);
