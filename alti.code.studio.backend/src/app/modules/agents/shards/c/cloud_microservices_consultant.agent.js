...
class CloudMicroservicesConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudMicroservicesSystem(objective) {
        logger.info(`💻 [CloudMicroservicesConsultantAgent] Analyzing Cloud Microservices Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Microservices Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudMicroservicesConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudMicroservicesConsultantAgent = Object.freeze(new CloudMicroservicesConsultantAgent());
