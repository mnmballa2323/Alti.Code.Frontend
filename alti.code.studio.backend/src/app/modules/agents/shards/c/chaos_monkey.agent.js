...
class ChaosMonkeyAgent extends GeminiCliBaseAgent {
  ...
  async simulateChaos(componentSource) {
    logger.warn(
      `🔥 [ChaosMonkey] Initiating Chaos simulation against target architecture...`,
    );
...
${componentSource}
    ...
    try {
      ...
      logger.info(
        `✅ [ChaosMonkey] Chaos injection script synthesized successfully.`,
      );
      return cleanScript;
    } catch (err) {
      logger.error(
        `❌ [ChaosMonkey] Failed to generate chaos script: ${err.message}`,
      );
      throw err;
    }
  }
