...
class CloudArchitectureEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudArchitectureSystem(objective) {
    logger.info(
      `💻 [CloudArchitectureEngineerAgent] Analyzing Cloud Architecture Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Architecture Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudArchitectureEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudArchitectureEngineerAgent = Object.freeze(
  new CloudArchitectureEngineerAgent(),
);
