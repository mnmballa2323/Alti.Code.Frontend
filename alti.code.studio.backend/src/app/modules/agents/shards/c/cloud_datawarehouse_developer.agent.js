...
class CloudDataWarehouseDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudDataWarehouseSystem(objective) {
        logger.info(`💻 [CloudDataWarehouseDeveloperAgent] Analyzing Cloud DataWarehouse Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud DataWarehouse Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudDataWarehouseDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudDataWarehouseDeveloperAgent = Object.freeze(new CloudDataWarehouseDeveloperAgent());
