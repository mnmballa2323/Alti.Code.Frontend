...
class CloudFaultToleranceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudFaultToleranceSystem(objective) {
    logger.info(
      `💻 [CloudFaultToleranceSpecialistAgent] Analyzing Cloud FaultTolerance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud FaultTolerance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudFaultToleranceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudFaultToleranceSpecialistAgent = Object.freeze(
  new CloudFaultToleranceSpecialistAgent(),
);
