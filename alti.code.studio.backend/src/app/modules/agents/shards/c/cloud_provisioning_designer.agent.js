...
class CloudProvisioningDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudProvisioningSystem(objective) {
    logger.info(
      `💻 [CloudProvisioningDesignerAgent] Analyzing Cloud Provisioning Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Provisioning Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudProvisioningDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudProvisioningDesignerAgent = Object.freeze(
  new CloudProvisioningDesignerAgent(),
);
