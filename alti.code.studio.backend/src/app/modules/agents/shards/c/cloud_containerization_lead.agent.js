...
class CloudContainerizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudContainerizationSystem(objective) {
    logger.info(
      `💻 [CloudContainerizationLeadAgent] Analyzing Cloud Containerization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Containerization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudContainerizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudContainerizationLeadAgent = Object.freeze(
  new CloudContainerizationLeadAgent(),
);
