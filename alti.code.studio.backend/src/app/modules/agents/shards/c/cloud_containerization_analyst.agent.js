...
class CloudContainerizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudContainerizationSystem(objective) {
    logger.info(
      `💻 [CloudContainerizationAnalystAgent] Analyzing Cloud Containerization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Containerization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudContainerizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudContainerizationAnalystAgent = Object.freeze(
  new CloudContainerizationAnalystAgent(),
);
