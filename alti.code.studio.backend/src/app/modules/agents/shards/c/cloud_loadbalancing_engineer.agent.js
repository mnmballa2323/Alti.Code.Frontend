...
class CloudLoadBalancingEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudLoadBalancingSystem(objective) {
    logger.info(
      `💻 [CloudLoadBalancingEngineerAgent] Analyzing Cloud LoadBalancing Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud LoadBalancing Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudLoadBalancingEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudLoadBalancingEngineerAgent = Object.freeze(
  new CloudLoadBalancingEngineerAgent(),
);
