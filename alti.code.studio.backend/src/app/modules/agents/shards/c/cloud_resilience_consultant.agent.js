...
class CloudResilienceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudResilienceSystem(objective) {
    logger.info(
      `💻 [CloudResilienceConsultantAgent] Analyzing Cloud Resilience Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Resilience Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudResilienceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudResilienceConsultantAgent = Object.freeze(
  new CloudResilienceConsultantAgent(),
);
