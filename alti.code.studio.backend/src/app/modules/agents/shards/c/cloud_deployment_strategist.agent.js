...
class CloudDeploymentStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudDeploymentSystem(objective) {
    logger.info(
      `💻 [CloudDeploymentStrategistAgent] Analyzing Cloud Deployment Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Deployment Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudDeploymentStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudDeploymentStrategistAgent = Object.freeze(
  new CloudDeploymentStrategistAgent(),
);
