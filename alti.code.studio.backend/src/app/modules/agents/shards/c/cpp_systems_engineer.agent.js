...
class CppSystemsEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateCppSystem(objective) {
        logger.info(`⚡ [CppSystemsEngineer] Analyzing pointer arithmetic and zero-cost abstractions...`);
        ...
        try {
            ...
            return output.replace(/```javascript|```typescript|```json|```cpp|```c\+\+|```/gi, '').trim();
        } catch (err) {
...
export const cppSystemsEngineerAgent = Object.freeze(new CppSystemsEngineerAgent());
