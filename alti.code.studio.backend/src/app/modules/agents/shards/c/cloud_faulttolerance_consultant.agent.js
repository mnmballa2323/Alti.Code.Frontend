...
class CloudFaultToleranceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudFaultToleranceSystem(objective) {
        logger.info(`💻 [CloudFaultToleranceConsultantAgent] Analyzing Cloud FaultTolerance Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud FaultTolerance Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudFaultToleranceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudFaultToleranceConsultantAgent = Object.freeze(new CloudFaultToleranceConsultantAgent());
