...
class CloudObservabilityArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudObservabilitySystem(objective) {
        logger.info(`💻 [CloudObservabilityArchitectAgent] Analyzing Cloud Observability Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Observability Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudObservabilityArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudObservabilityArchitectAgent = Object.freeze(new CloudObservabilityArchitectAgent());
