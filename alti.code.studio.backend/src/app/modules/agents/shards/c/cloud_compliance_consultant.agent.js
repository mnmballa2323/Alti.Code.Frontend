...
class CloudComplianceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudComplianceSystem(objective) {
    logger.info(
      `💻 [CloudComplianceConsultantAgent] Analyzing Cloud Compliance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Compliance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudComplianceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudComplianceConsultantAgent = Object.freeze(
  new CloudComplianceConsultantAgent(),
);
