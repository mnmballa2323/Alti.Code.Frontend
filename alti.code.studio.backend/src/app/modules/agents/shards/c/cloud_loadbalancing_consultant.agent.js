...
class CloudLoadBalancingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudLoadBalancingSystem(objective) {
        logger.info(`💻 [CloudLoadBalancingConsultantAgent] Analyzing Cloud LoadBalancing Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud LoadBalancing Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudLoadBalancingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudLoadBalancingConsultantAgent = Object.freeze(new CloudLoadBalancingConsultantAgent());
