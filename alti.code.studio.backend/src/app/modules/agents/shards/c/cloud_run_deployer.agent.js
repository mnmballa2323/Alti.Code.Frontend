...
class CloudRunDeployerAgent extends GeminiCliBaseAgent {
  ...
  async deployToCloudRun(sourceCode) {
    logger.info(
      `☁️ [CloudRunDeployer] Synthesizing GCP deployment strategy...`,
    );
...
Output strictly as a JSON object: { "dockerfile": "...", "cloudbuild": "...", "command": "..." }
    ...
    try {
      ...
      logger.info(
        `☁️ [CloudRunDeployer] Deployment specs generated successfully.`,
      );
      return deploymentSpec;
    } catch (err) {
      logger.error(
        `❌ [CloudRunDeployer] Failed to generate deployment specs: ${err.message}`,
      );
      throw err;
    }
  }
