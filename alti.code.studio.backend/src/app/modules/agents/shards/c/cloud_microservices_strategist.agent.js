...
class CloudMicroservicesStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudMicroservicesSystem(objective) {
        logger.info(`💻 [CloudMicroservicesStrategistAgent] Analyzing Cloud Microservices Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Microservices Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudMicroservicesStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudMicroservicesStrategistAgent = Object.freeze(new CloudMicroservicesStrategistAgent());
