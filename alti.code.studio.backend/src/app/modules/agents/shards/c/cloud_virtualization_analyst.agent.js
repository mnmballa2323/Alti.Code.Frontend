...
class CloudVirtualizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudVirtualizationSystem(objective) {
    logger.info(
      `💻 [CloudVirtualizationAnalystAgent] Analyzing Cloud Virtualization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Virtualization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudVirtualizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudVirtualizationAnalystAgent = Object.freeze(
  new CloudVirtualizationAnalystAgent(),
);
