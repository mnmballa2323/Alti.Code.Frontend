...
class CloudObservabilityDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudObservabilitySystem(objective) {
    logger.info(
      `💻 [CloudObservabilityDirectorAgent] Analyzing Cloud Observability Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Observability Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudObservabilityDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudObservabilityDirectorAgent = Object.freeze(
  new CloudObservabilityDirectorAgent(),
);
