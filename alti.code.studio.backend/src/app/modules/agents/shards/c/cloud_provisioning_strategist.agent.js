...
class CloudProvisioningStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudProvisioningSystem(objective) {
    logger.info(
      `💻 [CloudProvisioningStrategistAgent] Analyzing Cloud Provisioning Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Provisioning Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudProvisioningStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudProvisioningStrategistAgent = Object.freeze(
  new CloudProvisioningStrategistAgent(),
);
