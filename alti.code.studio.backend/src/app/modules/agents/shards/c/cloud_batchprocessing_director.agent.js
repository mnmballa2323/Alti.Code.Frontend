...
class CloudBatchProcessingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudBatchProcessingSystem(objective) {
    logger.info(
      `💻 [CloudBatchProcessingDirectorAgent] Analyzing Cloud BatchProcessing Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud BatchProcessing Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudBatchProcessingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudBatchProcessingDirectorAgent = Object.freeze(
  new CloudBatchProcessingDirectorAgent(),
);
