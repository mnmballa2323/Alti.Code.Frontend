...
class CloudGovernanceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudGovernanceSystem(objective) {
    logger.info(
      `💻 [CloudGovernanceConsultantAgent] Analyzing Cloud Governance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Governance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudGovernanceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudGovernanceConsultantAgent = Object.freeze(
  new CloudGovernanceConsultantAgent(),
);
