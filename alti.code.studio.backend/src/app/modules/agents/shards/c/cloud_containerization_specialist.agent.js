...
class CloudContainerizationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudContainerizationSystem(objective) {
    logger.info(
      `💻 [CloudContainerizationSpecialistAgent] Analyzing Cloud Containerization Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Containerization Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudContainerizationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudContainerizationSpecialistAgent = Object.freeze(
  new CloudContainerizationSpecialistAgent(),
);
