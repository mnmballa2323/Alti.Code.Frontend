...
class CloudObservabilityDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudObservabilitySystem(objective) {
        logger.info(`💻 [CloudObservabilityDesignerAgent] Analyzing Cloud Observability Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Observability Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudObservabilityDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudObservabilityDesignerAgent = Object.freeze(new CloudObservabilityDesignerAgent());
