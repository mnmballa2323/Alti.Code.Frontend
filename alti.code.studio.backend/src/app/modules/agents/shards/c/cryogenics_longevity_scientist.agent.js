...
class CryogenicsLongevityScientistAgent extends GeminiCliBaseAgent {
    ...
    async generateCryoSystem(cryoObjective) {
        logger.info(`❄️ [CryogenicsLongevityScientist] Analyzing objective for vitrification thermodynamics...`);
...
${cryoObjective}
        ...
        try {
            ...
            logger.info(`✅ [CryogenicsLongevityScientist] Cryogenics architecture generated successfully.`);
            return cleanCode;
        } catch (err) {
            logger.error(`❌ [CryogenicsLongevityScientist] Failed to generate cryogenics system: ${err.message}`);
            throw err;
        }
    }
}

export const cryogenicsLongevityScientistAgent = Object.freeze(new CryogenicsLongevityScientistAgent());
