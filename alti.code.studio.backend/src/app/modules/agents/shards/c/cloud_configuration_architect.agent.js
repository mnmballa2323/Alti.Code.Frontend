...
class CloudConfigurationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudConfigurationSystem(objective) {
    logger.info(
      `💻 [CloudConfigurationArchitectAgent] Analyzing Cloud Configuration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Configuration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudConfigurationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudConfigurationArchitectAgent = Object.freeze(
  new CloudConfigurationArchitectAgent(),
);
