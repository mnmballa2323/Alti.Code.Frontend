...
class CloudDataWarehouseConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudDataWarehouseSystem(objective) {
    logger.info(
      `💻 [CloudDataWarehouseConsultantAgent] Analyzing Cloud DataWarehouse Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud DataWarehouse Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudDataWarehouseConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudDataWarehouseConsultantAgent = Object.freeze(
  new CloudDataWarehouseConsultantAgent(),
);
