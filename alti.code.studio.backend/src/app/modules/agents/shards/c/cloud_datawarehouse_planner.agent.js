...
class CloudDataWarehousePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudDataWarehouseSystem(objective) {
    logger.info(
      `💻 [CloudDataWarehousePlannerAgent] Analyzing Cloud DataWarehouse Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud DataWarehouse Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudDataWarehousePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudDataWarehousePlannerAgent = Object.freeze(
  new CloudDataWarehousePlannerAgent(),
);
