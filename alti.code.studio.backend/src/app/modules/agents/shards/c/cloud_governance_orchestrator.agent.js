...
class CloudGovernanceOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudGovernanceSystem(objective) {
    logger.info(
      `💻 [CloudGovernanceOrchestratorAgent] Analyzing Cloud Governance Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Governance Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudGovernanceOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudGovernanceOrchestratorAgent = Object.freeze(
  new CloudGovernanceOrchestratorAgent(),
);
