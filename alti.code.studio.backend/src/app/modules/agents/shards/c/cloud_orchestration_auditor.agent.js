...
class CloudOrchestrationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudOrchestrationSystem(objective) {
    logger.info(
      `💻 [CloudOrchestrationAuditorAgent] Analyzing Cloud Orchestration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Orchestration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudOrchestrationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudOrchestrationAuditorAgent = Object.freeze(
  new CloudOrchestrationAuditorAgent(),
);
