...
class CloudLoadBalancingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudLoadBalancingSystem(objective) {
    logger.info(
      `💻 [CloudLoadBalancingDirectorAgent] Analyzing Cloud LoadBalancing Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud LoadBalancing Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudLoadBalancingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudLoadBalancingDirectorAgent = Object.freeze(
  new CloudLoadBalancingDirectorAgent(),
);
