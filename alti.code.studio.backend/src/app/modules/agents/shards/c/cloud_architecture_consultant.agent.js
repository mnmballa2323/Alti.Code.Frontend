...
class CloudArchitectureConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudArchitectureSystem(objective) {
    logger.info(
      `💻 [CloudArchitectureConsultantAgent] Analyzing Cloud Architecture Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Architecture Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudArchitectureConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudArchitectureConsultantAgent = Object.freeze(
  new CloudArchitectureConsultantAgent(),
);
