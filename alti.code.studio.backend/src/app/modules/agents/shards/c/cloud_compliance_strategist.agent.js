...
class CloudComplianceStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudComplianceSystem(objective) {
        logger.info(`💻 [CloudComplianceStrategistAgent] Analyzing Cloud Compliance Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Compliance Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudComplianceStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudComplianceStrategistAgent = Object.freeze(new CloudComplianceStrategistAgent());
