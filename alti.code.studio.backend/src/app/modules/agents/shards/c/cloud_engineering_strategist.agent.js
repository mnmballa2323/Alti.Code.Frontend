...
class CloudEngineeringStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudEngineeringSystem(objective) {
    logger.info(
      `💻 [CloudEngineeringStrategistAgent] Analyzing Cloud Engineering Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Engineering Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudEngineeringStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudEngineeringStrategistAgent = Object.freeze(
  new CloudEngineeringStrategistAgent(),
);
