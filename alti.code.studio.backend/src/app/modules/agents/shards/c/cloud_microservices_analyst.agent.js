...
class CloudMicroservicesAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudMicroservicesSystem(objective) {
    logger.info(
      `💻 [CloudMicroservicesAnalystAgent] Analyzing Cloud Microservices Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Microservices Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudMicroservicesAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudMicroservicesAnalystAgent = Object.freeze(
  new CloudMicroservicesAnalystAgent(),
);
