    ...
    async reviewPlan(plan) {
        ...
        if (isDesign) {
            ...
            if (riskyKeywords.some(w => subject.toLowerCase().includes(w))) {
                feedback.push({ severity: 'high', message: 'Plan involves destructive actions. Require manual approval.' });
                approved = false;
            }

            if (plan.steps && plan.steps.length < 2) {
                feedback.push({ severity: 'medium', message: 'Plan seems too simple. Are you missing verification steps?' });
            }

            // Check if Security/QA is involved
            const hasSecurity = plan.steps && plan.steps.some(s => s.agent === 'Sentinel' || s.agent === 'Security');
            if (plan.steps && !hasSecurity) {
                feedback.push({ severity: 'low', message: 'Suggestion: Add a security scan step.' });
            }
        }
...
export const criticAgent = Object.freeze(new CriticAgent());
