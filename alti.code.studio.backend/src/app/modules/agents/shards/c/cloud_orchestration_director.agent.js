...
class CloudOrchestrationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudOrchestrationSystem(objective) {
        logger.info(`💻 [CloudOrchestrationDirectorAgent] Analyzing Cloud Orchestration Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Orchestration Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudOrchestrationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudOrchestrationDirectorAgent = Object.freeze(new CloudOrchestrationDirectorAgent());
