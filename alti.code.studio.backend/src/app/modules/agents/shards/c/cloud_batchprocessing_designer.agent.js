...
class CloudBatchProcessingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudBatchProcessingSystem(objective) {
    logger.info(
      `💻 [CloudBatchProcessingDesignerAgent] Analyzing Cloud BatchProcessing Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud BatchProcessing Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudBatchProcessingDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudBatchProcessingDesignerAgent = Object.freeze(
  new CloudBatchProcessingDesignerAgent(),
);
