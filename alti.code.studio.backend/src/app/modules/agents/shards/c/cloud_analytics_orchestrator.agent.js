...
class CloudAnalyticsOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudAnalyticsSystem(objective) {
    logger.info(
      `💻 [CloudAnalyticsOrchestratorAgent] Analyzing Cloud Analytics Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Analytics Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudAnalyticsOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudAnalyticsOrchestratorAgent = Object.freeze(
  new CloudAnalyticsOrchestratorAgent(),
);
