...
class CloudContainerizationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudContainerizationSystem(objective) {
        logger.info(`💻 [CloudContainerizationArchitectAgent] Analyzing Cloud Containerization Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Containerization Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudContainerizationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudContainerizationArchitectAgent = Object.freeze(new CloudContainerizationArchitectAgent());
