...
class ChemicalMaterialsScientistAgent extends GeminiCliBaseAgent {
  ...
  async generateChemistrySystem(chemistryObjective) {
    logger.info(
      `🧪 [ChemicalMaterialsScientist] Analyzing objective for molecular simulation and thermodynamics...`,
    );
...
${chemistryObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [ChemicalMaterialsScientist] Chemistry architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [ChemicalMaterialsScientist] Failed to generate chemistry system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const chemicalMaterialsScientistAgent = Object.freeze(
  new ChemicalMaterialsScientistAgent(),
);
