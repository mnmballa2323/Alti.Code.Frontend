...
class ComposerAgent {
  constructor() {
    ...
    this.description =
      'Agent for scaffolding features and composing multi-file code execution plans.';
    this.preamble =
      'You are the Composer, an expert software architect that plans and scaffolds full features.';
  ...
  async compose(request) {
    ...
    logger.info(
      `🎹 Composer: Generated plan with ${plan.files.length} operations.`,
    );
  ...
  async scaffoldFeature(design) {
    logger.info(
      `🎹 Composer: Scaffolding feature "${design.feature}" from Design Doc...`,
    );
    ...
    for (const file of design.files) {
      ...
      try {
        ...
        let content = await this._generateContent(
          file.path,
          file.instruction,
          design.feature,
        );
        ...
        if (!content || content.trim().length < 10) {
          logger.warn(
            `🎹 Composer: LLM returned empty response for ${file.path}, using fallback scaffold.`,
          );
        ...
        try {
          await this._writeFile(
            file.path,
            MINIMAL_SCAFFOLD_FALLBACK(design.feature, file.instruction),
          );
    ...
    for (const op of plan.files) {
      try {
        if (op.type === 'create' || op.type === 'edit') {
          ...
          await fs
            .unlink(path.resolve(this.projectRoot, op.path))
            .catch(() => {});
...
export const composerAgent = Object.freeze(new ComposerAgent());
