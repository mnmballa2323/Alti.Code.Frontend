...
class CloudArchitectureDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudArchitectureSystem(objective) {
    logger.info(
      `💻 [CloudArchitectureDeveloperAgent] Analyzing Cloud Architecture Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Architecture Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudArchitectureDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudArchitectureDeveloperAgent = Object.freeze(
  new CloudArchitectureDeveloperAgent(),
);
