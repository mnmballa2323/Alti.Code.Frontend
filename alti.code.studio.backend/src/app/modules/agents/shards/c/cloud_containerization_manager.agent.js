...
class CloudContainerizationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudContainerizationSystem(objective) {
        logger.info(`💻 [CloudContainerizationManagerAgent] Analyzing Cloud Containerization Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Containerization Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudContainerizationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudContainerizationManagerAgent = Object.freeze(new CloudContainerizationManagerAgent());
