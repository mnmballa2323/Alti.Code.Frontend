...
class CloudVirtualizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudVirtualizationSystem(objective) {
        logger.info(`💻 [CloudVirtualizationSpecialistAgent] Analyzing Cloud Virtualization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Virtualization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudVirtualizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudVirtualizationSpecialistAgent = Object.freeze(new CloudVirtualizationSpecialistAgent());
