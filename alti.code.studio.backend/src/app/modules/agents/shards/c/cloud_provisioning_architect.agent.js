...
class CloudProvisioningArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudProvisioningSystem(objective) {
        logger.info(`💻 [CloudProvisioningArchitectAgent] Analyzing Cloud Provisioning Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Provisioning Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudProvisioningArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudProvisioningArchitectAgent = Object.freeze(new CloudProvisioningArchitectAgent());
