...
class CloudOrchestrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudOrchestrationSystem(objective) {
    logger.info(
      `💻 [CloudOrchestrationStrategistAgent] Analyzing Cloud Orchestration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Orchestration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudOrchestrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudOrchestrationStrategistAgent = Object.freeze(
  new CloudOrchestrationStrategistAgent(),
);
