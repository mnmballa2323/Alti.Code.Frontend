...
class CloudResilienceStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudResilienceSystem(objective) {
        logger.info(`💻 [CloudResilienceStrategistAgent] Analyzing Cloud Resilience Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Resilience Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudResilienceStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudResilienceStrategistAgent = Object.freeze(new CloudResilienceStrategistAgent());
