...
class CloudDataLakeOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudDataLakeSystem(objective) {
    logger.info(
      `💻 [CloudDataLakeOrchestratorAgent] Analyzing Cloud DataLake Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud DataLake Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudDataLakeOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudDataLakeOrchestratorAgent = Object.freeze(
  new CloudDataLakeOrchestratorAgent(),
);
