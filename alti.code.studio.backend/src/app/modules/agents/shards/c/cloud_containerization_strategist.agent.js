...
class CloudContainerizationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudContainerizationSystem(objective) {
        logger.info(`💻 [CloudContainerizationStrategistAgent] Analyzing Cloud Containerization Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Containerization Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudContainerizationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudContainerizationStrategistAgent = Object.freeze(new CloudContainerizationStrategistAgent());
