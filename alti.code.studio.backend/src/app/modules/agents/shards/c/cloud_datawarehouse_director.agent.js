...
class CloudDataWarehouseDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudDataWarehouseSystem(objective) {
    logger.info(
      `💻 [CloudDataWarehouseDirectorAgent] Analyzing Cloud DataWarehouse Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud DataWarehouse Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudDataWarehouseDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudDataWarehouseDirectorAgent = Object.freeze(
  new CloudDataWarehouseDirectorAgent(),
);
