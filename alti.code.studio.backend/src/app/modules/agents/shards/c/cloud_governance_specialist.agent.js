...
class CloudGovernanceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudGovernanceSystem(objective) {
    logger.info(
      `💻 [CloudGovernanceSpecialistAgent] Analyzing Cloud Governance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Governance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudGovernanceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudGovernanceSpecialistAgent = Object.freeze(
  new CloudGovernanceSpecialistAgent(),
);
