...
class CloudOptimizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudOptimizationSystem(objective) {
    logger.info(
      `💻 [CloudOptimizationDesignerAgent] Analyzing Cloud Optimization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Optimization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudOptimizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudOptimizationDesignerAgent = Object.freeze(
  new CloudOptimizationDesignerAgent(),
);
