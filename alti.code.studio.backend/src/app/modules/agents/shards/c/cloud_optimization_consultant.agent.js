...
class CloudOptimizationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudOptimizationSystem(objective) {
        logger.info(`💻 [CloudOptimizationConsultantAgent] Analyzing Cloud Optimization Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Optimization Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudOptimizationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudOptimizationConsultantAgent = Object.freeze(new CloudOptimizationConsultantAgent());
