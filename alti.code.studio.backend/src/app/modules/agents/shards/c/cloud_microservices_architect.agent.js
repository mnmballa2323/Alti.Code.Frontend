...
class CloudMicroservicesArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudMicroservicesSystem(objective) {
    logger.info(
      `💻 [CloudMicroservicesArchitectAgent] Analyzing Cloud Microservices Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Microservices Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudMicroservicesArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudMicroservicesArchitectAgent = Object.freeze(
  new CloudMicroservicesArchitectAgent(),
);
