...
class CloudVirtualizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudVirtualizationSystem(objective) {
    logger.info(
      `💻 [CloudVirtualizationAuditorAgent] Analyzing Cloud Virtualization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Virtualization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudVirtualizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudVirtualizationAuditorAgent = Object.freeze(
  new CloudVirtualizationAuditorAgent(),
);
