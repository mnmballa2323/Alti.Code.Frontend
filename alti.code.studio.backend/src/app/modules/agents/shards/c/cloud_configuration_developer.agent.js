...
class CloudConfigurationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudConfigurationSystem(objective) {
    logger.info(
      `💻 [CloudConfigurationDeveloperAgent] Analyzing Cloud Configuration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Configuration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudConfigurationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudConfigurationDeveloperAgent = Object.freeze(
  new CloudConfigurationDeveloperAgent(),
);
