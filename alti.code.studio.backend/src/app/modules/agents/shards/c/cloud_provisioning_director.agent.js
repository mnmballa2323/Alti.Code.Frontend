...
class CloudProvisioningDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudProvisioningSystem(objective) {
    logger.info(
      `💻 [CloudProvisioningDirectorAgent] Analyzing Cloud Provisioning Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Provisioning Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudProvisioningDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudProvisioningDirectorAgent = Object.freeze(
  new CloudProvisioningDirectorAgent(),
);
