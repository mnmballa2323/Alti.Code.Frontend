...
class CloudProvisioningDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudProvisioningSystem(objective) {
    logger.info(
      `💻 [CloudProvisioningDeveloperAgent] Analyzing Cloud Provisioning Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Provisioning Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudProvisioningDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudProvisioningDeveloperAgent = Object.freeze(
  new CloudProvisioningDeveloperAgent(),
);
