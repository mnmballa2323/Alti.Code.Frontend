...
class CloudFaultToleranceOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudFaultToleranceSystem(objective) {
        logger.info(`💻 [CloudFaultToleranceOrchestratorAgent] Analyzing Cloud FaultTolerance Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud FaultTolerance Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudFaultToleranceOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudFaultToleranceOrchestratorAgent = Object.freeze(new CloudFaultToleranceOrchestratorAgent());
