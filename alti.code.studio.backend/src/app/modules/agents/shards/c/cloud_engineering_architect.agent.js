...
class CloudEngineeringArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudEngineeringSystem(objective) {
    logger.info(
      `💻 [CloudEngineeringArchitectAgent] Analyzing Cloud Engineering Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Engineering Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudEngineeringArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudEngineeringArchitectAgent = Object.freeze(
  new CloudEngineeringArchitectAgent(),
);
