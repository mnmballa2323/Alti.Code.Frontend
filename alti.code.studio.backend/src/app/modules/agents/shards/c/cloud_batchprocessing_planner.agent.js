...
class CloudBatchProcessingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudBatchProcessingSystem(objective) {
    logger.info(
      `💻 [CloudBatchProcessingPlannerAgent] Analyzing Cloud BatchProcessing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud BatchProcessing Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudBatchProcessingPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudBatchProcessingPlannerAgent = Object.freeze(
  new CloudBatchProcessingPlannerAgent(),
);
