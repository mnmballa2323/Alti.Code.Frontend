...
class CloudMigrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudMigrationSystem(objective) {
    logger.info(
      `💻 [CloudMigrationOrchestratorAgent] Analyzing Cloud Migration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Migration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudMigrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudMigrationOrchestratorAgent = Object.freeze(
  new CloudMigrationOrchestratorAgent(),
);
