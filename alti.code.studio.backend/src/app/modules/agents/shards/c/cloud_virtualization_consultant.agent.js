...
class CloudVirtualizationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudVirtualizationSystem(objective) {
        logger.info(`💻 [CloudVirtualizationConsultantAgent] Analyzing Cloud Virtualization Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Virtualization Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudVirtualizationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudVirtualizationConsultantAgent = Object.freeze(new CloudVirtualizationConsultantAgent());
