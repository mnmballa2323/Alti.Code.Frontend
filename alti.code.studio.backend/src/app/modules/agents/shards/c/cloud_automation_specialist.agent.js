...
class CloudAutomationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudAutomationSystem(objective) {
    logger.info(
      `💻 [CloudAutomationSpecialistAgent] Analyzing Cloud Automation Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Automation Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudAutomationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudAutomationSpecialistAgent = Object.freeze(
  new CloudAutomationSpecialistAgent(),
);
