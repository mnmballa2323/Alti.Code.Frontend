...
class CloudServerlessSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudServerlessSystem(objective) {
        logger.info(`💻 [CloudServerlessSpecialistAgent] Analyzing Cloud Serverless Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Serverless Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudServerlessSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudServerlessSpecialistAgent = Object.freeze(new CloudServerlessSpecialistAgent());
