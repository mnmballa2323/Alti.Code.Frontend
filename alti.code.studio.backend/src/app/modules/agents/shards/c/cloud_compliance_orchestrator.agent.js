...
class CloudComplianceOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudComplianceSystem(objective) {
    logger.info(
      `💻 [CloudComplianceOrchestratorAgent] Analyzing Cloud Compliance Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Compliance Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudComplianceOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudComplianceOrchestratorAgent = Object.freeze(
  new CloudComplianceOrchestratorAgent(),
);
