...
class CloudObservabilitySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudObservabilitySystem(objective) {
    logger.info(
      `💻 [CloudObservabilitySpecialistAgent] Analyzing Cloud Observability Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Observability Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudObservabilitySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudObservabilitySpecialistAgent = Object.freeze(
  new CloudObservabilitySpecialistAgent(),
);
