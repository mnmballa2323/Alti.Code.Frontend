...
class CloudLoadBalancingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudLoadBalancingSystem(objective) {
    logger.info(
      `💻 [CloudLoadBalancingOrchestratorAgent] Analyzing Cloud LoadBalancing Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud LoadBalancing Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudLoadBalancingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudLoadBalancingOrchestratorAgent = Object.freeze(
  new CloudLoadBalancingOrchestratorAgent(),
);
