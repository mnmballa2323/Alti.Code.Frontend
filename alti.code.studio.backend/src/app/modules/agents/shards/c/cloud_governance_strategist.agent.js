...
class CloudGovernanceStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudGovernanceSystem(objective) {
    logger.info(
      `💻 [CloudGovernanceStrategistAgent] Analyzing Cloud Governance Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Governance Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudGovernanceStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudGovernanceStrategistAgent = Object.freeze(
  new CloudGovernanceStrategistAgent(),
);
