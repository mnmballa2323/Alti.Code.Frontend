...
class CloudDeploymentOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateCloudDeploymentSystem(objective) {
    logger.info(
      `💻 [CloudDeploymentOrchestratorAgent] Analyzing Cloud Deployment Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Cloud Deployment Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [CloudDeploymentOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const cloudDeploymentOrchestratorAgent = Object.freeze(
  new CloudDeploymentOrchestratorAgent(),
);
