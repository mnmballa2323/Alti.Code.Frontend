...
class CloudFaultToleranceDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudFaultToleranceSystem(objective) {
        logger.info(`💻 [CloudFaultToleranceDesignerAgent] Analyzing Cloud FaultTolerance Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud FaultTolerance Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudFaultToleranceDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudFaultToleranceDesignerAgent = Object.freeze(new CloudFaultToleranceDesignerAgent());
