...
class CloudOrchestrationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateCloudOrchestrationSystem(objective) {
        logger.info(`💻 [CloudOrchestrationPlannerAgent] Analyzing Cloud Orchestration Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Cloud Orchestration Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [CloudOrchestrationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const cloudOrchestrationPlannerAgent = Object.freeze(new CloudOrchestrationPlannerAgent());
