...
class ReleaseArchitectureOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseArchitectureSystem(objective) {
        logger.info(`💻 [ReleaseArchitectureOrchestratorAgent] Analyzing Release Architecture Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Architecture Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseArchitectureOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseArchitectureOrchestratorAgent = Object.freeze(new ReleaseArchitectureOrchestratorAgent());
