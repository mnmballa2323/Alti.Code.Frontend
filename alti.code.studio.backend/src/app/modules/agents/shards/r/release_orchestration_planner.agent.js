...
class ReleaseOrchestrationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseOrchestrationSystem(objective) {
        logger.info(`💻 [ReleaseOrchestrationPlannerAgent] Analyzing Release Orchestration Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Orchestration Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseOrchestrationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseOrchestrationPlannerAgent = Object.freeze(new ReleaseOrchestrationPlannerAgent());
