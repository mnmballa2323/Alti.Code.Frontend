...
class ReleaseStreamingLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseStreamingSystem(objective) {
    logger.info(
      `💻 [ReleaseStreamingLeadAgent] Analyzing Release Streaming Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Streaming Lead Target',
      );
...
export const releaseStreamingLeadAgent = Object.freeze(
  new ReleaseStreamingLeadAgent(),
);
