...
class ReleaseIntegrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseIntegrationSystem(objective) {
    logger.info(
      `💻 [ReleaseIntegrationConsultantAgent] Analyzing Release Integration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Integration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseIntegrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseIntegrationConsultantAgent = Object.freeze(
  new ReleaseIntegrationConsultantAgent(),
);
