...
class ReleaseSecurityPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseSecuritySystem(objective) {
    logger.info(
      `💻 [ReleaseSecurityPlannerAgent] Analyzing Release Security Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Security Planner Target',
      );
...
export const releaseSecurityPlannerAgent = Object.freeze(
  new ReleaseSecurityPlannerAgent(),
);
