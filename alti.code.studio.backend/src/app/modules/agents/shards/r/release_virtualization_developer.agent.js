...
class ReleaseVirtualizationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseVirtualizationSystem(objective) {
        logger.info(`💻 [ReleaseVirtualizationDeveloperAgent] Analyzing Release Virtualization Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Virtualization Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseVirtualizationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseVirtualizationDeveloperAgent = Object.freeze(new ReleaseVirtualizationDeveloperAgent());
