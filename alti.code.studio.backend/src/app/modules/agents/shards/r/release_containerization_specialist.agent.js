...
class ReleaseContainerizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseContainerizationSystem(objective) {
        logger.info(`💻 [ReleaseContainerizationSpecialistAgent] Analyzing Release Containerization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Containerization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseContainerizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseContainerizationSpecialistAgent = Object.freeze(new ReleaseContainerizationSpecialistAgent());
