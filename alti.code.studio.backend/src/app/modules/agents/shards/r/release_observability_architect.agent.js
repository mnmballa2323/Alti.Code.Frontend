...
class ReleaseObservabilityArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseObservabilitySystem(objective) {
    logger.info(
      `💻 [ReleaseObservabilityArchitectAgent] Analyzing Release Observability Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Observability Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseObservabilityArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseObservabilityArchitectAgent = Object.freeze(
  new ReleaseObservabilityArchitectAgent(),
);
