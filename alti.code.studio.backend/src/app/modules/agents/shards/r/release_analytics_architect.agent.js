...
class ReleaseAnalyticsArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseAnalyticsSystem(objective) {
    logger.info(
      `💻 [ReleaseAnalyticsArchitectAgent] Analyzing Release Analytics Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Analytics Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseAnalyticsArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseAnalyticsArchitectAgent = Object.freeze(
  new ReleaseAnalyticsArchitectAgent(),
);
