...
class ReleaseResilienceArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseResilienceSystem(objective) {
    logger.info(
      `💻 [ReleaseResilienceArchitectAgent] Analyzing Release Resilience Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Resilience Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseResilienceArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseResilienceArchitectAgent = Object.freeze(
  new ReleaseResilienceArchitectAgent(),
);
