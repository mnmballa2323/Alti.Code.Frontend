...
class ReleaseSecurityStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseSecuritySystem(objective) {
    logger.info(
      `💻 [ReleaseSecurityStrategistAgent] Analyzing Release Security Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Security Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseSecurityStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseSecurityStrategistAgent = Object.freeze(
  new ReleaseSecurityStrategistAgent(),
);
