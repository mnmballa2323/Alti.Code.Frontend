...
class ReleaseTelemetryConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseTelemetrySystem(objective) {
    logger.info(
      `💻 [ReleaseTelemetryConsultantAgent] Analyzing Release Telemetry Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Telemetry Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseTelemetryConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseTelemetryConsultantAgent = Object.freeze(
  new ReleaseTelemetryConsultantAgent(),
);
