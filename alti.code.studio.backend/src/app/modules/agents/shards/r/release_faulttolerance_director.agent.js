...
class ReleaseFaultToleranceDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseFaultToleranceSystem(objective) {
        logger.info(`💻 [ReleaseFaultToleranceDirectorAgent] Analyzing Release FaultTolerance Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release FaultTolerance Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseFaultToleranceDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseFaultToleranceDirectorAgent = Object.freeze(new ReleaseFaultToleranceDirectorAgent());
