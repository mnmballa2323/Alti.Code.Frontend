...
class ReleaseStreamingAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseStreamingSystem(objective) {
    logger.info(
      `💻 [ReleaseStreamingAnalystAgent] Analyzing Release Streaming Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Streaming Analyst Target',
      );
...
export const releaseStreamingAnalystAgent = Object.freeze(
  new ReleaseStreamingAnalystAgent(),
);
