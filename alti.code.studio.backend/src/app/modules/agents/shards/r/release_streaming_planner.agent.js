...
class ReleaseStreamingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseStreamingSystem(objective) {
    logger.info(
      `💻 [ReleaseStreamingPlannerAgent] Analyzing Release Streaming Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Streaming Planner Target',
      );
...
export const releaseStreamingPlannerAgent = Object.freeze(
  new ReleaseStreamingPlannerAgent(),
);
