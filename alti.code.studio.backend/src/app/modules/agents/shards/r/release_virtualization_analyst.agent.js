...
class ReleaseVirtualizationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseVirtualizationSystem(objective) {
        logger.info(`💻 [ReleaseVirtualizationAnalystAgent] Analyzing Release Virtualization Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Virtualization Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseVirtualizationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseVirtualizationAnalystAgent = Object.freeze(new ReleaseVirtualizationAnalystAgent());
