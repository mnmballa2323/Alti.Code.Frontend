...
class ReleaseObservabilityAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseObservabilitySystem(objective) {
        logger.info(`💻 [ReleaseObservabilityAuditorAgent] Analyzing Release Observability Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Observability Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseObservabilityAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseObservabilityAuditorAgent = Object.freeze(new ReleaseObservabilityAuditorAgent());
