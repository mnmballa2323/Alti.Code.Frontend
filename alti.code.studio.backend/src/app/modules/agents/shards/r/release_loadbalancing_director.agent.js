...
class ReleaseLoadBalancingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseLoadBalancingSystem(objective) {
    logger.info(
      `💻 [ReleaseLoadBalancingDirectorAgent] Analyzing Release LoadBalancing Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release LoadBalancing Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseLoadBalancingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseLoadBalancingDirectorAgent = Object.freeze(
  new ReleaseLoadBalancingDirectorAgent(),
);
