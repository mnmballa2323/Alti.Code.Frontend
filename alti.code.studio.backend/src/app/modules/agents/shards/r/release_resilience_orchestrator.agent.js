...
class ReleaseResilienceOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseResilienceSystem(objective) {
        logger.info(`💻 [ReleaseResilienceOrchestratorAgent] Analyzing Release Resilience Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Resilience Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseResilienceOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseResilienceOrchestratorAgent = Object.freeze(new ReleaseResilienceOrchestratorAgent());
