...
class ReleaseOptimizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseOptimizationSystem(objective) {
    logger.info(
      `💻 [ReleaseOptimizationTesterAgent] Analyzing Release Optimization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Optimization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseOptimizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseOptimizationTesterAgent = Object.freeze(
  new ReleaseOptimizationTesterAgent(),
);
