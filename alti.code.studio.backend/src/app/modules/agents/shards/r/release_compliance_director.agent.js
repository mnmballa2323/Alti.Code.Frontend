...
class ReleaseComplianceDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseComplianceSystem(objective) {
        logger.info(`💻 [ReleaseComplianceDirectorAgent] Analyzing Release Compliance Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Compliance Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseComplianceDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseComplianceDirectorAgent = Object.freeze(new ReleaseComplianceDirectorAgent());
