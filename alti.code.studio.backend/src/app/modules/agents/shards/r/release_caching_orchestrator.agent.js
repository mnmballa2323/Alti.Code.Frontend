...
class ReleaseCachingOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseCachingSystem(objective) {
        logger.info(`💻 [ReleaseCachingOrchestratorAgent] Analyzing Release Caching Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Caching Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseCachingOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseCachingOrchestratorAgent = Object.freeze(new ReleaseCachingOrchestratorAgent());
