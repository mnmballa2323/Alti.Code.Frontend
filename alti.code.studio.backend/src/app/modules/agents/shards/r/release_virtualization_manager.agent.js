...
class ReleaseVirtualizationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseVirtualizationSystem(objective) {
    logger.info(
      `💻 [ReleaseVirtualizationManagerAgent] Analyzing Release Virtualization Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Virtualization Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseVirtualizationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseVirtualizationManagerAgent = Object.freeze(
  new ReleaseVirtualizationManagerAgent(),
);
