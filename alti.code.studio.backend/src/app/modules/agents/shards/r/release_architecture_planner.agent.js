...
class ReleaseArchitecturePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseArchitectureSystem(objective) {
        logger.info(`💻 [ReleaseArchitecturePlannerAgent] Analyzing Release Architecture Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Architecture Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseArchitecturePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseArchitecturePlannerAgent = Object.freeze(new ReleaseArchitecturePlannerAgent());
