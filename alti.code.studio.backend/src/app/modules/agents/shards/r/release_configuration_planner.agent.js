...
class ReleaseConfigurationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseConfigurationSystem(objective) {
    logger.info(
      `💻 [ReleaseConfigurationPlannerAgent] Analyzing Release Configuration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Configuration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseConfigurationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseConfigurationPlannerAgent = Object.freeze(
  new ReleaseConfigurationPlannerAgent(),
);
