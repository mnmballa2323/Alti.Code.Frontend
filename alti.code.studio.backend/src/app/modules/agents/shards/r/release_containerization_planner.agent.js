...
class ReleaseContainerizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseContainerizationSystem(objective) {
    logger.info(
      `💻 [ReleaseContainerizationPlannerAgent] Analyzing Release Containerization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Containerization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseContainerizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseContainerizationPlannerAgent = Object.freeze(
  new ReleaseContainerizationPlannerAgent(),
);
