...
class ReleaseLoadBalancingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseLoadBalancingSystem(objective) {
    logger.info(
      `💻 [ReleaseLoadBalancingDeveloperAgent] Analyzing Release LoadBalancing Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release LoadBalancing Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseLoadBalancingDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseLoadBalancingDeveloperAgent = Object.freeze(
  new ReleaseLoadBalancingDeveloperAgent(),
);
