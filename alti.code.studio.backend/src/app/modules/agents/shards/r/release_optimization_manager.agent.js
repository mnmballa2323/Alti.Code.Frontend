...
class ReleaseOptimizationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseOptimizationSystem(objective) {
    logger.info(
      `💻 [ReleaseOptimizationManagerAgent] Analyzing Release Optimization Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Optimization Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseOptimizationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseOptimizationManagerAgent = Object.freeze(
  new ReleaseOptimizationManagerAgent(),
);
