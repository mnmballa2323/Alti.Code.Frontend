...
class ReleaseTelemetryDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseTelemetrySystem(objective) {
    logger.info(
      `💻 [ReleaseTelemetryDeveloperAgent] Analyzing Release Telemetry Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Telemetry Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseTelemetryDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseTelemetryDeveloperAgent = Object.freeze(
  new ReleaseTelemetryDeveloperAgent(),
);
