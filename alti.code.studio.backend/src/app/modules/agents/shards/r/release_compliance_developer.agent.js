...
class ReleaseComplianceDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseComplianceSystem(objective) {
    logger.info(
      `💻 [ReleaseComplianceDeveloperAgent] Analyzing Release Compliance Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Compliance Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseComplianceDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseComplianceDeveloperAgent = Object.freeze(
  new ReleaseComplianceDeveloperAgent(),
);
