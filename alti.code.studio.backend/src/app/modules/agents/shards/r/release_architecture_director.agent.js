...
class ReleaseArchitectureDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseArchitectureSystem(objective) {
        logger.info(`💻 [ReleaseArchitectureDirectorAgent] Analyzing Release Architecture Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Architecture Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseArchitectureDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseArchitectureDirectorAgent = Object.freeze(new ReleaseArchitectureDirectorAgent());
