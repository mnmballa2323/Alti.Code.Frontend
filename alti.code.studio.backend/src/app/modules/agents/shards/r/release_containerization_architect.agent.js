...
class ReleaseContainerizationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseContainerizationSystem(objective) {
    logger.info(
      `💻 [ReleaseContainerizationArchitectAgent] Analyzing Release Containerization Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Containerization Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseContainerizationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseContainerizationArchitectAgent = Object.freeze(
  new ReleaseContainerizationArchitectAgent(),
);
