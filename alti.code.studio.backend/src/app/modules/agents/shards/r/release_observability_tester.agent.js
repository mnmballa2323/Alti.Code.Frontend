...
class ReleaseObservabilityTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseObservabilitySystem(objective) {
    logger.info(
      `💻 [ReleaseObservabilityTesterAgent] Analyzing Release Observability Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Observability Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseObservabilityTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseObservabilityTesterAgent = Object.freeze(
  new ReleaseObservabilityTesterAgent(),
);
