...
class ReleaseProvisioningDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseProvisioningSystem(objective) {
        logger.info(`💻 [ReleaseProvisioningDeveloperAgent] Analyzing Release Provisioning Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Provisioning Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseProvisioningDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseProvisioningDeveloperAgent = Object.freeze(new ReleaseProvisioningDeveloperAgent());
