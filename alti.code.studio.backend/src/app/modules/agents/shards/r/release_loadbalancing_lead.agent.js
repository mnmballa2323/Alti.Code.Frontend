...
class ReleaseLoadBalancingLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseLoadBalancingSystem(objective) {
    logger.info(
      `💻 [ReleaseLoadBalancingLeadAgent] Analyzing Release LoadBalancing Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release LoadBalancing Lead Target',
      );
...
export const releaseLoadBalancingLeadAgent = Object.freeze(
  new ReleaseLoadBalancingLeadAgent(),
);
