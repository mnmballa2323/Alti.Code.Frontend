...
class ReleaseResilienceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseResilienceSystem(objective) {
    logger.info(
      `💻 [ReleaseResilienceEngineerAgent] Analyzing Release Resilience Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Resilience Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseResilienceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseResilienceEngineerAgent = Object.freeze(
  new ReleaseResilienceEngineerAgent(),
);
