...
class ReleaseLoadBalancingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseLoadBalancingSystem(objective) {
    logger.info(
      `💻 [ReleaseLoadBalancingSpecialistAgent] Analyzing Release LoadBalancing Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release LoadBalancing Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseLoadBalancingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseLoadBalancingSpecialistAgent = Object.freeze(
  new ReleaseLoadBalancingSpecialistAgent(),
);
