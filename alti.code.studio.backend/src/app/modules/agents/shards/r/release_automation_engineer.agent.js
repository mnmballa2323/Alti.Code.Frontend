...
class ReleaseAutomationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseAutomationSystem(objective) {
    logger.info(
      `💻 [ReleaseAutomationEngineerAgent] Analyzing Release Automation Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Automation Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseAutomationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseAutomationEngineerAgent = Object.freeze(
  new ReleaseAutomationEngineerAgent(),
);
