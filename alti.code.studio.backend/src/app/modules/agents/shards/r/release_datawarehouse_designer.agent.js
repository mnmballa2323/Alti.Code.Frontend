...
class ReleaseDataWarehouseDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [ReleaseDataWarehouseDesignerAgent] Analyzing Release DataWarehouse Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release DataWarehouse Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseDataWarehouseDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseDataWarehouseDesignerAgent = Object.freeze(
  new ReleaseDataWarehouseDesignerAgent(),
);
