...
class ReleaseAutomationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseAutomationSystem(objective) {
        logger.info(`💻 [ReleaseAutomationSpecialistAgent] Analyzing Release Automation Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Automation Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseAutomationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseAutomationSpecialistAgent = Object.freeze(new ReleaseAutomationSpecialistAgent());
