...
class ReleaseMigrationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseMigrationSystem(objective) {
        logger.info(`💻 [ReleaseMigrationOrchestratorAgent] Analyzing Release Migration Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Migration Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseMigrationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseMigrationOrchestratorAgent = Object.freeze(new ReleaseMigrationOrchestratorAgent());
