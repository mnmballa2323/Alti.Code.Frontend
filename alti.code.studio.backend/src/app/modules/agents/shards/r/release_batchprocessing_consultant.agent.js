...
class ReleaseBatchProcessingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseBatchProcessingSystem(objective) {
        logger.info(`💻 [ReleaseBatchProcessingConsultantAgent] Analyzing Release BatchProcessing Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release BatchProcessing Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseBatchProcessingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseBatchProcessingConsultantAgent = Object.freeze(new ReleaseBatchProcessingConsultantAgent());
