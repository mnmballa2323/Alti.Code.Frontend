...
7. **Edge-to-Cloud Pattern**: Local pipeline: sensor → Python → local buffer (SQLite/Redis) → MQTT publish → GCP IoT Core. If network drops, buffer locally and flush on reconnect. Use \`google-cloud-iot\` or \`paho-mqtt\` for cloud connection. Watchdog: add cron to restart service if it crashes.
# BEST PRACTICES
