...
class ReleaseAutomationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseAutomationSystem(objective) {
    logger.info(
      `💻 [ReleaseAutomationConsultantAgent] Analyzing Release Automation Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Automation Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseAutomationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseAutomationConsultantAgent = Object.freeze(
  new ReleaseAutomationConsultantAgent(),
);
