...
class ReleaseBatchProcessingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseBatchProcessingSystem(objective) {
    logger.info(
      `💻 [ReleaseBatchProcessingPlannerAgent] Analyzing Release BatchProcessing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release BatchProcessing Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseBatchProcessingPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseBatchProcessingPlannerAgent = Object.freeze(
  new ReleaseBatchProcessingPlannerAgent(),
);
