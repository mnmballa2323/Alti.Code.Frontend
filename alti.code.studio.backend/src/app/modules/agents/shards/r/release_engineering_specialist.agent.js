...
class ReleaseEngineeringSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseEngineeringSystem(objective) {
        logger.info(`💻 [ReleaseEngineeringSpecialistAgent] Analyzing Release Engineering Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Engineering Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseEngineeringSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseEngineeringSpecialistAgent = Object.freeze(new ReleaseEngineeringSpecialistAgent());
