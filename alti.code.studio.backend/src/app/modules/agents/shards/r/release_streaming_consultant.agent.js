...
class ReleaseStreamingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseStreamingSystem(objective) {
    logger.info(
      `💻 [ReleaseStreamingConsultantAgent] Analyzing Release Streaming Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Streaming Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseStreamingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseStreamingConsultantAgent = Object.freeze(
  new ReleaseStreamingConsultantAgent(),
);
