...
class ReleaseConfigurationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseConfigurationSystem(objective) {
    logger.info(
      `💻 [ReleaseConfigurationDesignerAgent] Analyzing Release Configuration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Configuration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseConfigurationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseConfigurationDesignerAgent = Object.freeze(
  new ReleaseConfigurationDesignerAgent(),
);
