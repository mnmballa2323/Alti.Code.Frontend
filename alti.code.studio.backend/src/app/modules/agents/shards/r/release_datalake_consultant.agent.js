...
class ReleaseDataLakeConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseDataLakeSystem(objective) {
    logger.info(
      `💻 [ReleaseDataLakeConsultantAgent] Analyzing Release DataLake Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release DataLake Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseDataLakeConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseDataLakeConsultantAgent = Object.freeze(
  new ReleaseDataLakeConsultantAgent(),
);
