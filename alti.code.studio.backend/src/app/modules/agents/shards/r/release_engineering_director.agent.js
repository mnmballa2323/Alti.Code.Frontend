...
class ReleaseEngineeringDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseEngineeringSystem(objective) {
    logger.info(
      `💻 [ReleaseEngineeringDirectorAgent] Analyzing Release Engineering Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Engineering Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseEngineeringDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseEngineeringDirectorAgent = Object.freeze(
  new ReleaseEngineeringDirectorAgent(),
);
