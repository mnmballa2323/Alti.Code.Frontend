...
class ReleaseIntegrationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseIntegrationSystem(objective) {
        logger.info(`💻 [ReleaseIntegrationDirectorAgent] Analyzing Release Integration Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Integration Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseIntegrationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseIntegrationDirectorAgent = Object.freeze(new ReleaseIntegrationDirectorAgent());
