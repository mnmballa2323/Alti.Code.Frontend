...
class ReleaseConfigurationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseConfigurationSystem(objective) {
    logger.info(
      `💻 [ReleaseConfigurationOrchestratorAgent] Analyzing Release Configuration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Configuration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseConfigurationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseConfigurationOrchestratorAgent = Object.freeze(
  new ReleaseConfigurationOrchestratorAgent(),
);
