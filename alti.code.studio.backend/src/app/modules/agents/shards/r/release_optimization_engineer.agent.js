...
class ReleaseOptimizationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseOptimizationSystem(objective) {
    logger.info(
      `💻 [ReleaseOptimizationEngineerAgent] Analyzing Release Optimization Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Optimization Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseOptimizationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseOptimizationEngineerAgent = Object.freeze(
  new ReleaseOptimizationEngineerAgent(),
);
