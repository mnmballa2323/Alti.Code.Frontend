...
class ReleaseProvisioningSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseProvisioningSystem(objective) {
        logger.info(`💻 [ReleaseProvisioningSpecialistAgent] Analyzing Release Provisioning Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Provisioning Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseProvisioningSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseProvisioningSpecialistAgent = Object.freeze(new ReleaseProvisioningSpecialistAgent());
