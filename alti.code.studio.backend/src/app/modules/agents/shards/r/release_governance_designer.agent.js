...
class ReleaseGovernanceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseGovernanceSystem(objective) {
    logger.info(
      `💻 [ReleaseGovernanceDesignerAgent] Analyzing Release Governance Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Governance Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseGovernanceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseGovernanceDesignerAgent = Object.freeze(
  new ReleaseGovernanceDesignerAgent(),
);
