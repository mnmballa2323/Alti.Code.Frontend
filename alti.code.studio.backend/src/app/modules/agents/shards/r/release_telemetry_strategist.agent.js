...
class ReleaseTelemetryStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseTelemetrySystem(objective) {
        logger.info(`💻 [ReleaseTelemetryStrategistAgent] Analyzing Release Telemetry Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Telemetry Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseTelemetryStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseTelemetryStrategistAgent = Object.freeze(new ReleaseTelemetryStrategistAgent());
