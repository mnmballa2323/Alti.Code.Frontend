...
class ReleaseAnalyticsSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseAnalyticsSystem(objective) {
    logger.info(
      `💻 [ReleaseAnalyticsSpecialistAgent] Analyzing Release Analytics Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Analytics Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseAnalyticsSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseAnalyticsSpecialistAgent = Object.freeze(
  new ReleaseAnalyticsSpecialistAgent(),
);
