...
class ReleaseProvisioningEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseProvisioningSystem(objective) {
    logger.info(
      `💻 [ReleaseProvisioningEngineerAgent] Analyzing Release Provisioning Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Provisioning Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseProvisioningEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseProvisioningEngineerAgent = Object.freeze(
  new ReleaseProvisioningEngineerAgent(),
);
