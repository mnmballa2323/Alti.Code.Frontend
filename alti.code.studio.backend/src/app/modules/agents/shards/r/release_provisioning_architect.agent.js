...
class ReleaseProvisioningArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseProvisioningSystem(objective) {
        logger.info(`💻 [ReleaseProvisioningArchitectAgent] Analyzing Release Provisioning Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Provisioning Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseProvisioningArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseProvisioningArchitectAgent = Object.freeze(new ReleaseProvisioningArchitectAgent());
