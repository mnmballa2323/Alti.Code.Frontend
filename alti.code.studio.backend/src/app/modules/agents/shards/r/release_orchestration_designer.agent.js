...
class ReleaseOrchestrationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseOrchestrationSystem(objective) {
    logger.info(
      `💻 [ReleaseOrchestrationDesignerAgent] Analyzing Release Orchestration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Orchestration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseOrchestrationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseOrchestrationDesignerAgent = Object.freeze(
  new ReleaseOrchestrationDesignerAgent(),
);
