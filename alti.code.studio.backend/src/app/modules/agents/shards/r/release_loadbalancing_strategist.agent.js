...
class ReleaseLoadBalancingStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseLoadBalancingSystem(objective) {
        logger.info(`💻 [ReleaseLoadBalancingStrategistAgent] Analyzing Release LoadBalancing Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release LoadBalancing Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseLoadBalancingStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseLoadBalancingStrategistAgent = Object.freeze(new ReleaseLoadBalancingStrategistAgent());
