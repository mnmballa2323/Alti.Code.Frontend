...
class ReleaseFaultToleranceAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseFaultToleranceSystem(objective) {
        logger.info(`💻 [ReleaseFaultToleranceAuditorAgent] Analyzing Release FaultTolerance Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release FaultTolerance Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseFaultToleranceAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseFaultToleranceAuditorAgent = Object.freeze(new ReleaseFaultToleranceAuditorAgent());
