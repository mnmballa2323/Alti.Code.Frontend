...
class ReleaseOptimizationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseOptimizationSystem(objective) {
    logger.info(
      `💻 [ReleaseOptimizationSpecialistAgent] Analyzing Release Optimization Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Optimization Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseOptimizationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseOptimizationSpecialistAgent = Object.freeze(
  new ReleaseOptimizationSpecialistAgent(),
);
