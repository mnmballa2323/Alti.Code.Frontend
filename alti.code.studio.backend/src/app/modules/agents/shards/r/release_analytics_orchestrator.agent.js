...
class ReleaseAnalyticsOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseAnalyticsSystem(objective) {
    logger.info(
      `💻 [ReleaseAnalyticsOrchestratorAgent] Analyzing Release Analytics Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Analytics Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseAnalyticsOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseAnalyticsOrchestratorAgent = Object.freeze(
  new ReleaseAnalyticsOrchestratorAgent(),
);
