...
class ReleaseGovernanceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseGovernanceSystem(objective) {
    logger.info(
      `💻 [ReleaseGovernanceDirectorAgent] Analyzing Release Governance Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Governance Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseGovernanceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseGovernanceDirectorAgent = Object.freeze(
  new ReleaseGovernanceDirectorAgent(),
);
