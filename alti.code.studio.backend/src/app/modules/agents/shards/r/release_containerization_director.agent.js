...
class ReleaseContainerizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseContainerizationSystem(objective) {
        logger.info(`💻 [ReleaseContainerizationDirectorAgent] Analyzing Release Containerization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Containerization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseContainerizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseContainerizationDirectorAgent = Object.freeze(new ReleaseContainerizationDirectorAgent());
