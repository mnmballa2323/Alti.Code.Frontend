...
class ReleaseContainerizationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseContainerizationSystem(objective) {
        logger.info(`💻 [ReleaseContainerizationEngineerAgent] Analyzing Release Containerization Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Containerization Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseContainerizationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseContainerizationEngineerAgent = Object.freeze(new ReleaseContainerizationEngineerAgent());
