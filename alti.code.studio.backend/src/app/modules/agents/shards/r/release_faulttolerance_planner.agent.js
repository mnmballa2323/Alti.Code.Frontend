...
class ReleaseFaultTolerancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseFaultToleranceSystem(objective) {
    logger.info(
      `💻 [ReleaseFaultTolerancePlannerAgent] Analyzing Release FaultTolerance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release FaultTolerance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseFaultTolerancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseFaultTolerancePlannerAgent = Object.freeze(
  new ReleaseFaultTolerancePlannerAgent(),
);
