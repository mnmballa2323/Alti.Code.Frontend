...
class ReleaseResilienceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseResilienceSystem(objective) {
    logger.info(
      `💻 [ReleaseResilienceDirectorAgent] Analyzing Release Resilience Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Resilience Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseResilienceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseResilienceDirectorAgent = Object.freeze(
  new ReleaseResilienceDirectorAgent(),
);
