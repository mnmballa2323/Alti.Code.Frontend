...
class ReleaseObservabilityConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseObservabilitySystem(objective) {
        logger.info(`💻 [ReleaseObservabilityConsultantAgent] Analyzing Release Observability Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Observability Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseObservabilityConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseObservabilityConsultantAgent = Object.freeze(new ReleaseObservabilityConsultantAgent());
