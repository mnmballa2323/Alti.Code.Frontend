...
class ReleaseContainerizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseContainerizationSystem(objective) {
    logger.info(
      `💻 [ReleaseContainerizationLeadAgent] Analyzing Release Containerization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Containerization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseContainerizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseContainerizationLeadAgent = Object.freeze(
  new ReleaseContainerizationLeadAgent(),
);
