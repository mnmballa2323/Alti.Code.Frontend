...
class ReleaseFaultToleranceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseFaultToleranceSystem(objective) {
    logger.info(
      `💻 [ReleaseFaultToleranceEngineerAgent] Analyzing Release FaultTolerance Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release FaultTolerance Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseFaultToleranceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseFaultToleranceEngineerAgent = Object.freeze(
  new ReleaseFaultToleranceEngineerAgent(),
);
