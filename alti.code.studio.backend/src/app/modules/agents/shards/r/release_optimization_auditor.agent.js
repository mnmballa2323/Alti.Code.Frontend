...
class ReleaseOptimizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseOptimizationSystem(objective) {
    logger.info(
      `💻 [ReleaseOptimizationAuditorAgent] Analyzing Release Optimization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Optimization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseOptimizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseOptimizationAuditorAgent = Object.freeze(
  new ReleaseOptimizationAuditorAgent(),
);
