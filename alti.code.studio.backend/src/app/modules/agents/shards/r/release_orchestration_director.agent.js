...
class ReleaseOrchestrationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseOrchestrationSystem(objective) {
        logger.info(`💻 [ReleaseOrchestrationDirectorAgent] Analyzing Release Orchestration Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Orchestration Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseOrchestrationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseOrchestrationDirectorAgent = Object.freeze(new ReleaseOrchestrationDirectorAgent());
