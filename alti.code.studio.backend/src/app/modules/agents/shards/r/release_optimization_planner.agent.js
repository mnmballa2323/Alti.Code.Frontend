...
class ReleaseOptimizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseOptimizationSystem(objective) {
    logger.info(
      `💻 [ReleaseOptimizationPlannerAgent] Analyzing Release Optimization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Optimization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseOptimizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseOptimizationPlannerAgent = Object.freeze(
  new ReleaseOptimizationPlannerAgent(),
);
