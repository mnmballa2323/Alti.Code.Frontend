...
class ReleaseOptimizationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseOptimizationSystem(objective) {
        logger.info(`💻 [ReleaseOptimizationAnalystAgent] Analyzing Release Optimization Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Optimization Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseOptimizationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseOptimizationAnalystAgent = Object.freeze(new ReleaseOptimizationAnalystAgent());
