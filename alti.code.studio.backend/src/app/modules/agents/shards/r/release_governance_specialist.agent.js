...
class ReleaseGovernanceSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseGovernanceSystem(objective) {
        logger.info(`💻 [ReleaseGovernanceSpecialistAgent] Analyzing Release Governance Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Governance Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseGovernanceSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseGovernanceSpecialistAgent = Object.freeze(new ReleaseGovernanceSpecialistAgent());
