...
class ReleaseOrchestrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseOrchestrationSystem(objective) {
    logger.info(
      `💻 [ReleaseOrchestrationStrategistAgent] Analyzing Release Orchestration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Orchestration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseOrchestrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseOrchestrationStrategistAgent = Object.freeze(
  new ReleaseOrchestrationStrategistAgent(),
);
