...
class ReleaseContainerizationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseContainerizationSystem(objective) {
        logger.info(`💻 [ReleaseContainerizationConsultantAgent] Analyzing Release Containerization Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Containerization Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseContainerizationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseContainerizationConsultantAgent = Object.freeze(new ReleaseContainerizationConsultantAgent());
