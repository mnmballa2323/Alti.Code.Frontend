...
class ReleaseServerlessOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseServerlessSystem(objective) {
    logger.info(
      `💻 [ReleaseServerlessOrchestratorAgent] Analyzing Release Serverless Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Serverless Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseServerlessOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseServerlessOrchestratorAgent = Object.freeze(
  new ReleaseServerlessOrchestratorAgent(),
);
