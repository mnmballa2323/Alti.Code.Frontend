...
class ReleaseSecuritySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseSecuritySystem(objective) {
    logger.info(
      `💻 [ReleaseSecuritySpecialistAgent] Analyzing Release Security Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Security Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseSecuritySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseSecuritySpecialistAgent = Object.freeze(
  new ReleaseSecuritySpecialistAgent(),
);
