...
class ReleaseMigrationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseMigrationSystem(objective) {
    logger.info(
      `💻 [ReleaseMigrationDeveloperAgent] Analyzing Release Migration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Migration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseMigrationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseMigrationDeveloperAgent = Object.freeze(
  new ReleaseMigrationDeveloperAgent(),
);
