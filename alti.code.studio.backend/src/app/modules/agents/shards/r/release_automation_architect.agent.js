...
class ReleaseAutomationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseAutomationSystem(objective) {
        logger.info(`💻 [ReleaseAutomationArchitectAgent] Analyzing Release Automation Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Automation Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseAutomationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseAutomationArchitectAgent = Object.freeze(new ReleaseAutomationArchitectAgent());
