...
class ReleaseContainerizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseContainerizationSystem(objective) {
    logger.info(
      `💻 [ReleaseContainerizationAnalystAgent] Analyzing Release Containerization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Containerization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseContainerizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseContainerizationAnalystAgent = Object.freeze(
  new ReleaseContainerizationAnalystAgent(),
);
