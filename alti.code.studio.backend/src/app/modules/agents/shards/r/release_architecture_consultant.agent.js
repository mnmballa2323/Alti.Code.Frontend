...
class ReleaseArchitectureConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseArchitectureSystem(objective) {
    logger.info(
      `💻 [ReleaseArchitectureConsultantAgent] Analyzing Release Architecture Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Architecture Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseArchitectureConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseArchitectureConsultantAgent = Object.freeze(
  new ReleaseArchitectureConsultantAgent(),
);
