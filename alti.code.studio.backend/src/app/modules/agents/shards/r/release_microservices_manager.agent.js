...
class ReleaseMicroservicesManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseMicroservicesSystem(objective) {
    logger.info(
      `💻 [ReleaseMicroservicesManagerAgent] Analyzing Release Microservices Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Microservices Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseMicroservicesManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseMicroservicesManagerAgent = Object.freeze(
  new ReleaseMicroservicesManagerAgent(),
);
