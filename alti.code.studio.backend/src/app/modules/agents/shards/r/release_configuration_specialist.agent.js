...
class ReleaseConfigurationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseConfigurationSystem(objective) {
    logger.info(
      `💻 [ReleaseConfigurationSpecialistAgent] Analyzing Release Configuration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Configuration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseConfigurationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseConfigurationSpecialistAgent = Object.freeze(
  new ReleaseConfigurationSpecialistAgent(),
);
