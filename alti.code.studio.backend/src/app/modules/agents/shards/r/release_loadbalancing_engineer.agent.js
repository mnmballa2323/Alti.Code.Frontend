...
class ReleaseLoadBalancingEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseLoadBalancingSystem(objective) {
        logger.info(`💻 [ReleaseLoadBalancingEngineerAgent] Analyzing Release LoadBalancing Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release LoadBalancing Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseLoadBalancingEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseLoadBalancingEngineerAgent = Object.freeze(new ReleaseLoadBalancingEngineerAgent());
