...
class ReleaseFaultToleranceDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseFaultToleranceSystem(objective) {
    logger.info(
      `💻 [ReleaseFaultToleranceDeveloperAgent] Analyzing Release FaultTolerance Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release FaultTolerance Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseFaultToleranceDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseFaultToleranceDeveloperAgent = Object.freeze(
  new ReleaseFaultToleranceDeveloperAgent(),
);
