...
class ReleaseFaultToleranceLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseFaultToleranceSystem(objective) {
    logger.info(
      `💻 [ReleaseFaultToleranceLeadAgent] Analyzing Release FaultTolerance Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release FaultTolerance Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseFaultToleranceLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseFaultToleranceLeadAgent = Object.freeze(
  new ReleaseFaultToleranceLeadAgent(),
);
