...
class ReleaseMicroservicesTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseMicroservicesSystem(objective) {
        logger.info(`💻 [ReleaseMicroservicesTesterAgent] Analyzing Release Microservices Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Microservices Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseMicroservicesTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseMicroservicesTesterAgent = Object.freeze(new ReleaseMicroservicesTesterAgent());
