...
class ReleaseProvisioningDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseProvisioningSystem(objective) {
        logger.info(`💻 [ReleaseProvisioningDesignerAgent] Analyzing Release Provisioning Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Provisioning Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseProvisioningDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseProvisioningDesignerAgent = Object.freeze(new ReleaseProvisioningDesignerAgent());
