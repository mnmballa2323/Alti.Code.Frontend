...
class ReleaseLoadBalancingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseLoadBalancingSystem(objective) {
    logger.info(
      `💻 [ReleaseLoadBalancingConsultantAgent] Analyzing Release LoadBalancing Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release LoadBalancing Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseLoadBalancingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseLoadBalancingConsultantAgent = Object.freeze(
  new ReleaseLoadBalancingConsultantAgent(),
);
