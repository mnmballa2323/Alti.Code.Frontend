...
class ReleaseMigrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseMigrationSystem(objective) {
    logger.info(
      `💻 [ReleaseMigrationConsultantAgent] Analyzing Release Migration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Migration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseMigrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseMigrationConsultantAgent = Object.freeze(
  new ReleaseMigrationConsultantAgent(),
);
