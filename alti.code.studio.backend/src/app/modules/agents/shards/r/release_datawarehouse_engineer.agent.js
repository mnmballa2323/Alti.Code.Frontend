...
class ReleaseDataWarehouseEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [ReleaseDataWarehouseEngineerAgent] Analyzing Release DataWarehouse Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release DataWarehouse Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseDataWarehouseEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseDataWarehouseEngineerAgent = Object.freeze(
  new ReleaseDataWarehouseEngineerAgent(),
);
