...
class ReleaseContainerizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseContainerizationSystem(objective) {
    logger.info(
      `💻 [ReleaseContainerizationDesignerAgent] Analyzing Release Containerization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Containerization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseContainerizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseContainerizationDesignerAgent = Object.freeze(
  new ReleaseContainerizationDesignerAgent(),
);
