...
class ReleaseIntegrationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseIntegrationSystem(objective) {
        logger.info(`💻 [ReleaseIntegrationStrategistAgent] Analyzing Release Integration Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Integration Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseIntegrationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseIntegrationStrategistAgent = Object.freeze(new ReleaseIntegrationStrategistAgent());
