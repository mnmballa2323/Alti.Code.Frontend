...
class ReleaseConfigurationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseConfigurationSystem(objective) {
    logger.info(
      `💻 [ReleaseConfigurationArchitectAgent] Analyzing Release Configuration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Configuration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseConfigurationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseConfigurationArchitectAgent = Object.freeze(
  new ReleaseConfigurationArchitectAgent(),
);
