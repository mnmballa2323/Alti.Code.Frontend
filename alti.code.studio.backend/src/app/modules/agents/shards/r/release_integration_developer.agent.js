...
class ReleaseIntegrationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseIntegrationSystem(objective) {
        logger.info(`💻 [ReleaseIntegrationDeveloperAgent] Analyzing Release Integration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Integration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseIntegrationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseIntegrationDeveloperAgent = Object.freeze(new ReleaseIntegrationDeveloperAgent());
