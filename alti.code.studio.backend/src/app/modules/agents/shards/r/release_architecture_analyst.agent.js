...
class ReleaseArchitectureAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseArchitectureSystem(objective) {
    logger.info(
      `💻 [ReleaseArchitectureAnalystAgent] Analyzing Release Architecture Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Architecture Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseArchitectureAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseArchitectureAnalystAgent = Object.freeze(
  new ReleaseArchitectureAnalystAgent(),
);
