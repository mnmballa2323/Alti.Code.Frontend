...
class ReleaseIntegrationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseIntegrationSystem(objective) {
        logger.info(`💻 [ReleaseIntegrationDesignerAgent] Analyzing Release Integration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Integration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseIntegrationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseIntegrationDesignerAgent = Object.freeze(new ReleaseIntegrationDesignerAgent());
