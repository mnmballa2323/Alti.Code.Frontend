...
class ReleaseTestingOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseTestingSystem(objective) {
        logger.info(`💻 [ReleaseTestingOrchestratorAgent] Analyzing Release Testing Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Testing Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseTestingOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseTestingOrchestratorAgent = Object.freeze(new ReleaseTestingOrchestratorAgent());
