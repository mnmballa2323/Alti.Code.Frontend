...
class ReleaseVirtualizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseVirtualizationSystem(objective) {
    logger.info(
      `💻 [ReleaseVirtualizationTesterAgent] Analyzing Release Virtualization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Virtualization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseVirtualizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseVirtualizationTesterAgent = Object.freeze(
  new ReleaseVirtualizationTesterAgent(),
);
