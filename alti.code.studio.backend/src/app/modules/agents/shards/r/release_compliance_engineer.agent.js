...
class ReleaseComplianceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseComplianceSystem(objective) {
    logger.info(
      `💻 [ReleaseComplianceEngineerAgent] Analyzing Release Compliance Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Compliance Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseComplianceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseComplianceEngineerAgent = Object.freeze(
  new ReleaseComplianceEngineerAgent(),
);
