...
class ReleaseOrchestrationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseOrchestrationSystem(objective) {
        logger.info(`💻 [ReleaseOrchestrationDeveloperAgent] Analyzing Release Orchestration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Orchestration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseOrchestrationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseOrchestrationDeveloperAgent = Object.freeze(new ReleaseOrchestrationDeveloperAgent());
