...
class ReleaseServerlessArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseServerlessSystem(objective) {
    logger.info(
      `💻 [ReleaseServerlessArchitectAgent] Analyzing Release Serverless Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Serverless Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseServerlessArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseServerlessArchitectAgent = Object.freeze(
  new ReleaseServerlessArchitectAgent(),
);
