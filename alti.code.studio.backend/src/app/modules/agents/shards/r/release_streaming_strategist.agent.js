...
class ReleaseStreamingStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseStreamingSystem(objective) {
    logger.info(
      `💻 [ReleaseStreamingStrategistAgent] Analyzing Release Streaming Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Streaming Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseStreamingStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseStreamingStrategistAgent = Object.freeze(
  new ReleaseStreamingStrategistAgent(),
);
