...
class ReleaseObservabilitySpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseObservabilitySystem(objective) {
        logger.info(`💻 [ReleaseObservabilitySpecialistAgent] Analyzing Release Observability Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Observability Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseObservabilitySpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseObservabilitySpecialistAgent = Object.freeze(new ReleaseObservabilitySpecialistAgent());
