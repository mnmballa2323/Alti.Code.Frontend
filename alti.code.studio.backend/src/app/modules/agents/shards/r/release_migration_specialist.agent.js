...
class ReleaseMigrationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseMigrationSystem(objective) {
        logger.info(`💻 [ReleaseMigrationSpecialistAgent] Analyzing Release Migration Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Migration Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseMigrationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseMigrationSpecialistAgent = Object.freeze(new ReleaseMigrationSpecialistAgent());
