...
class ReleaseObservabilityAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseObservabilitySystem(objective) {
        logger.info(`💻 [ReleaseObservabilityAnalystAgent] Analyzing Release Observability Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Observability Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseObservabilityAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseObservabilityAnalystAgent = Object.freeze(new ReleaseObservabilityAnalystAgent());
