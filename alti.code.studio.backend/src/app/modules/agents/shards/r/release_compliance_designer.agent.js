...
class ReleaseComplianceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseComplianceSystem(objective) {
    logger.info(
      `💻 [ReleaseComplianceDesignerAgent] Analyzing Release Compliance Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Compliance Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseComplianceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseComplianceDesignerAgent = Object.freeze(
  new ReleaseComplianceDesignerAgent(),
);
