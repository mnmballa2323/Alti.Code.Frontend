...
class ReleaseObservabilityDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseObservabilitySystem(objective) {
        logger.info(`💻 [ReleaseObservabilityDirectorAgent] Analyzing Release Observability Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Observability Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseObservabilityDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseObservabilityDirectorAgent = Object.freeze(new ReleaseObservabilityDirectorAgent());
