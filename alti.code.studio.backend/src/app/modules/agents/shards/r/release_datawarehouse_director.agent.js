...
class ReleaseDataWarehouseDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [ReleaseDataWarehouseDirectorAgent] Analyzing Release DataWarehouse Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release DataWarehouse Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseDataWarehouseDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseDataWarehouseDirectorAgent = Object.freeze(
  new ReleaseDataWarehouseDirectorAgent(),
);
