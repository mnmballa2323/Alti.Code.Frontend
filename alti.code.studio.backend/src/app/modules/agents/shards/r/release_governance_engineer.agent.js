...
class ReleaseGovernanceEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseGovernanceSystem(objective) {
        logger.info(`💻 [ReleaseGovernanceEngineerAgent] Analyzing Release Governance Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Governance Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseGovernanceEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseGovernanceEngineerAgent = Object.freeze(new ReleaseGovernanceEngineerAgent());
