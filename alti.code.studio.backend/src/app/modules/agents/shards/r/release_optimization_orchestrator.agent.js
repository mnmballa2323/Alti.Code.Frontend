...
class ReleaseOptimizationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseOptimizationSystem(objective) {
    logger.info(
      `💻 [ReleaseOptimizationOrchestratorAgent] Analyzing Release Optimization Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Optimization Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseOptimizationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseOptimizationOrchestratorAgent = Object.freeze(
  new ReleaseOptimizationOrchestratorAgent(),
);
