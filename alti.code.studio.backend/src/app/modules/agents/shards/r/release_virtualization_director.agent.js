...
class ReleaseVirtualizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseVirtualizationSystem(objective) {
    logger.info(
      `💻 [ReleaseVirtualizationDirectorAgent] Analyzing Release Virtualization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Virtualization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseVirtualizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseVirtualizationDirectorAgent = Object.freeze(
  new ReleaseVirtualizationDirectorAgent(),
);
