...
class ReleaseDataWarehouseTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseDataWarehouseSystem(objective) {
        logger.info(`💻 [ReleaseDataWarehouseTesterAgent] Analyzing Release DataWarehouse Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release DataWarehouse Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseDataWarehouseTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseDataWarehouseTesterAgent = Object.freeze(new ReleaseDataWarehouseTesterAgent());
