...
class ReleaseLoadBalancingManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseLoadBalancingSystem(objective) {
        logger.info(`💻 [ReleaseLoadBalancingManagerAgent] Analyzing Release LoadBalancing Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release LoadBalancing Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseLoadBalancingManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseLoadBalancingManagerAgent = Object.freeze(new ReleaseLoadBalancingManagerAgent());
