...
class ReleaseSecurityConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseSecuritySystem(objective) {
    logger.info(
      `💻 [ReleaseSecurityConsultantAgent] Analyzing Release Security Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Security Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseSecurityConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseSecurityConsultantAgent = Object.freeze(
  new ReleaseSecurityConsultantAgent(),
);
