...
class ReleaseDataWarehouseSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseDataWarehouseSystem(objective) {
        logger.info(`💻 [ReleaseDataWarehouseSpecialistAgent] Analyzing Release DataWarehouse Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release DataWarehouse Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseDataWarehouseSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseDataWarehouseSpecialistAgent = Object.freeze(new ReleaseDataWarehouseSpecialistAgent());
