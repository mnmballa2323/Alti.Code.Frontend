...
class ReleaseVirtualizationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseVirtualizationSystem(objective) {
        logger.info(`💻 [ReleaseVirtualizationOrchestratorAgent] Analyzing Release Virtualization Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Virtualization Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseVirtualizationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseVirtualizationOrchestratorAgent = Object.freeze(new ReleaseVirtualizationOrchestratorAgent());
