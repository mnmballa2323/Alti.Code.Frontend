...
class ReleaseOrchestrationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseOrchestrationSystem(objective) {
        logger.info(`💻 [ReleaseOrchestrationConsultantAgent] Analyzing Release Orchestration Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Orchestration Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseOrchestrationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseOrchestrationConsultantAgent = Object.freeze(new ReleaseOrchestrationConsultantAgent());
