...
class ReleaseVirtualizationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseVirtualizationSystem(objective) {
        logger.info(`💻 [ReleaseVirtualizationConsultantAgent] Analyzing Release Virtualization Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Virtualization Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseVirtualizationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseVirtualizationConsultantAgent = Object.freeze(new ReleaseVirtualizationConsultantAgent());
