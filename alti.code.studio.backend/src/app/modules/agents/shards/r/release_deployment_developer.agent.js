...
class ReleaseDeploymentDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseDeploymentSystem(objective) {
        logger.info(`💻 [ReleaseDeploymentDeveloperAgent] Analyzing Release Deployment Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Deployment Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseDeploymentDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseDeploymentDeveloperAgent = Object.freeze(new ReleaseDeploymentDeveloperAgent());
