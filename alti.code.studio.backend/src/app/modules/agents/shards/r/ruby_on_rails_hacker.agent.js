...
class RubyOnRailsHackerAgent extends GeminiCliBaseAgent {
    ...
    async generateRubySystem(objective) {
        logger.info(`💎 [RubyOnRailsHacker] Analyzing ActiveRecord migrations and Rails controllers...`);
        ...
        try {
            ...
            return output.replace(/```javascript|```typescript|```json|```ruby|```/gi, '').trim();
        } catch (err) {
...
export const rubyOnRailsHackerAgent = Object.freeze(new RubyOnRailsHackerAgent());
