...
class ReleaseProvisioningStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseProvisioningSystem(objective) {
        logger.info(`💻 [ReleaseProvisioningStrategistAgent] Analyzing Release Provisioning Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Provisioning Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseProvisioningStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseProvisioningStrategistAgent = Object.freeze(new ReleaseProvisioningStrategistAgent());
