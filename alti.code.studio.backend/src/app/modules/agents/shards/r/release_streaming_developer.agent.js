...
class ReleaseStreamingDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseStreamingSystem(objective) {
        logger.info(`💻 [ReleaseStreamingDeveloperAgent] Analyzing Release Streaming Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Streaming Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseStreamingDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseStreamingDeveloperAgent = Object.freeze(new ReleaseStreamingDeveloperAgent());
