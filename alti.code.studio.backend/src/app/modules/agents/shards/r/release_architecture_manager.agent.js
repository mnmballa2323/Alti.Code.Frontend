...
class ReleaseArchitectureManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseArchitectureSystem(objective) {
    logger.info(
      `💻 [ReleaseArchitectureManagerAgent] Analyzing Release Architecture Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Architecture Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseArchitectureManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseArchitectureManagerAgent = Object.freeze(
  new ReleaseArchitectureManagerAgent(),
);
