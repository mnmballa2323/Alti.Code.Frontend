...
class ReleaseMicroservicesArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseMicroservicesSystem(objective) {
    logger.info(
      `💻 [ReleaseMicroservicesArchitectAgent] Analyzing Release Microservices Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Microservices Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseMicroservicesArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseMicroservicesArchitectAgent = Object.freeze(
  new ReleaseMicroservicesArchitectAgent(),
);
