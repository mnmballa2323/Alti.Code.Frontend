...
class ReleaseBatchProcessingManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseBatchProcessingSystem(objective) {
        logger.info(`💻 [ReleaseBatchProcessingManagerAgent] Analyzing Release BatchProcessing Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release BatchProcessing Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseBatchProcessingManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseBatchProcessingManagerAgent = Object.freeze(new ReleaseBatchProcessingManagerAgent());
