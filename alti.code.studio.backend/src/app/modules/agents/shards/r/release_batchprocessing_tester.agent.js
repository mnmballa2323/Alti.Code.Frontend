...
class ReleaseBatchProcessingTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseBatchProcessingSystem(objective) {
    logger.info(
      `💻 [ReleaseBatchProcessingTesterAgent] Analyzing Release BatchProcessing Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release BatchProcessing Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseBatchProcessingTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseBatchProcessingTesterAgent = Object.freeze(
  new ReleaseBatchProcessingTesterAgent(),
);
