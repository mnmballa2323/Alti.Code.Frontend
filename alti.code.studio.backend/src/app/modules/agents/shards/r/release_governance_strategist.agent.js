...
class ReleaseGovernanceStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseGovernanceSystem(objective) {
    logger.info(
      `💻 [ReleaseGovernanceStrategistAgent] Analyzing Release Governance Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Governance Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseGovernanceStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseGovernanceStrategistAgent = Object.freeze(
  new ReleaseGovernanceStrategistAgent(),
);
