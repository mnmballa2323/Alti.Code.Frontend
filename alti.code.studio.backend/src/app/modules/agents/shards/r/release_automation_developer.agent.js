...
class ReleaseAutomationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseAutomationSystem(objective) {
    logger.info(
      `💻 [ReleaseAutomationDeveloperAgent] Analyzing Release Automation Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Automation Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseAutomationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseAutomationDeveloperAgent = Object.freeze(
  new ReleaseAutomationDeveloperAgent(),
);
