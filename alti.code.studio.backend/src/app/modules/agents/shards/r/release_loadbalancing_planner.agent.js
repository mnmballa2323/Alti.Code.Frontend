...
class ReleaseLoadBalancingPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseLoadBalancingSystem(objective) {
        logger.info(`💻 [ReleaseLoadBalancingPlannerAgent] Analyzing Release LoadBalancing Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release LoadBalancing Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseLoadBalancingPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseLoadBalancingPlannerAgent = Object.freeze(new ReleaseLoadBalancingPlannerAgent());
