...
class ReleaseEngineeringOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseEngineeringSystem(objective) {
    logger.info(
      `💻 [ReleaseEngineeringOrchestratorAgent] Analyzing Release Engineering Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Engineering Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseEngineeringOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseEngineeringOrchestratorAgent = Object.freeze(
  new ReleaseEngineeringOrchestratorAgent(),
);
