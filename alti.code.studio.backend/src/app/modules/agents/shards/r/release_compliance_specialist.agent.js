...
class ReleaseComplianceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseComplianceSystem(objective) {
    logger.info(
      `💻 [ReleaseComplianceSpecialistAgent] Analyzing Release Compliance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Compliance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseComplianceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseComplianceSpecialistAgent = Object.freeze(
  new ReleaseComplianceSpecialistAgent(),
);
