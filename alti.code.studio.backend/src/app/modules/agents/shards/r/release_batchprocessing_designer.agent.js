...
class ReleaseBatchProcessingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseBatchProcessingSystem(objective) {
    logger.info(
      `💻 [ReleaseBatchProcessingDesignerAgent] Analyzing Release BatchProcessing Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release BatchProcessing Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseBatchProcessingDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseBatchProcessingDesignerAgent = Object.freeze(
  new ReleaseBatchProcessingDesignerAgent(),
);
