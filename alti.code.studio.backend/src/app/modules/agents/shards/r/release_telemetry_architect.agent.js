...
class ReleaseTelemetryArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseTelemetrySystem(objective) {
    logger.info(
      `💻 [ReleaseTelemetryArchitectAgent] Analyzing Release Telemetry Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Telemetry Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseTelemetryArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseTelemetryArchitectAgent = Object.freeze(
  new ReleaseTelemetryArchitectAgent(),
);
