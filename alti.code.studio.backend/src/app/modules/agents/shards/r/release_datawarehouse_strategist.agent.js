...
class ReleaseDataWarehouseStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [ReleaseDataWarehouseStrategistAgent] Analyzing Release DataWarehouse Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release DataWarehouse Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseDataWarehouseStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseDataWarehouseStrategistAgent = Object.freeze(
  new ReleaseDataWarehouseStrategistAgent(),
);
