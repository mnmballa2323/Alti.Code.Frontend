...
class ReleaseAutomationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseAutomationSystem(objective) {
        logger.info(`💻 [ReleaseAutomationDirectorAgent] Analyzing Release Automation Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Automation Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseAutomationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseAutomationDirectorAgent = Object.freeze(new ReleaseAutomationDirectorAgent());
