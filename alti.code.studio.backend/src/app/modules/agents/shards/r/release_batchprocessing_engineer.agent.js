...
class ReleaseBatchProcessingEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseBatchProcessingSystem(objective) {
    logger.info(
      `💻 [ReleaseBatchProcessingEngineerAgent] Analyzing Release BatchProcessing Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release BatchProcessing Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseBatchProcessingEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseBatchProcessingEngineerAgent = Object.freeze(
  new ReleaseBatchProcessingEngineerAgent(),
);
