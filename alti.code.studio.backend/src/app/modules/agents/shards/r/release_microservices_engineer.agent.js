...
class ReleaseMicroservicesEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseMicroservicesSystem(objective) {
        logger.info(`💻 [ReleaseMicroservicesEngineerAgent] Analyzing Release Microservices Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Microservices Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseMicroservicesEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseMicroservicesEngineerAgent = Object.freeze(new ReleaseMicroservicesEngineerAgent());
