...
class ReleaseObservabilityManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseObservabilitySystem(objective) {
    logger.info(
      `💻 [ReleaseObservabilityManagerAgent] Analyzing Release Observability Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Observability Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseObservabilityManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseObservabilityManagerAgent = Object.freeze(
  new ReleaseObservabilityManagerAgent(),
);
