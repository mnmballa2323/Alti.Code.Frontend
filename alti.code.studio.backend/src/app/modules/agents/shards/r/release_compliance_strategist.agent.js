...
class ReleaseComplianceStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseComplianceSystem(objective) {
    logger.info(
      `💻 [ReleaseComplianceStrategistAgent] Analyzing Release Compliance Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Compliance Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseComplianceStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseComplianceStrategistAgent = Object.freeze(
  new ReleaseComplianceStrategistAgent(),
);
