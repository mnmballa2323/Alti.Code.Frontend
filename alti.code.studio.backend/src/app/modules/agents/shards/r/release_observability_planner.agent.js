...
class ReleaseObservabilityPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseObservabilitySystem(objective) {
    logger.info(
      `💻 [ReleaseObservabilityPlannerAgent] Analyzing Release Observability Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Observability Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseObservabilityPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseObservabilityPlannerAgent = Object.freeze(
  new ReleaseObservabilityPlannerAgent(),
);
