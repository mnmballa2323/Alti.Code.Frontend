...
class ReleaseDataWarehouseArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseDataWarehouseSystem(objective) {
    logger.info(
      `💻 [ReleaseDataWarehouseArchitectAgent] Analyzing Release DataWarehouse Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release DataWarehouse Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseDataWarehouseArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseDataWarehouseArchitectAgent = Object.freeze(
  new ReleaseDataWarehouseArchitectAgent(),
);
