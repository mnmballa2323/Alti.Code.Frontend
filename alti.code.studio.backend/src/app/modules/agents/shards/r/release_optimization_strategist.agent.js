...
class ReleaseOptimizationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseOptimizationSystem(objective) {
    logger.info(
      `💻 [ReleaseOptimizationStrategistAgent] Analyzing Release Optimization Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Optimization Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseOptimizationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseOptimizationStrategistAgent = Object.freeze(
  new ReleaseOptimizationStrategistAgent(),
);
