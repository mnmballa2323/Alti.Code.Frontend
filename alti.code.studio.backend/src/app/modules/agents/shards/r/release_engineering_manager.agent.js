...
class ReleaseEngineeringManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseEngineeringSystem(objective) {
    logger.info(
      `💻 [ReleaseEngineeringManagerAgent] Analyzing Release Engineering Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Engineering Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseEngineeringManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseEngineeringManagerAgent = Object.freeze(
  new ReleaseEngineeringManagerAgent(),
);
