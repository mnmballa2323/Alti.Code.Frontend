...
class ReleaseOrchestrationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseOrchestrationSystem(objective) {
        logger.info(`💻 [ReleaseOrchestrationArchitectAgent] Analyzing Release Orchestration Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Orchestration Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseOrchestrationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseOrchestrationArchitectAgent = Object.freeze(new ReleaseOrchestrationArchitectAgent());
