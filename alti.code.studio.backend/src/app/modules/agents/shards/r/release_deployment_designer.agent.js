...
class ReleaseDeploymentDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseDeploymentSystem(objective) {
    logger.info(
      `💻 [ReleaseDeploymentDesignerAgent] Analyzing Release Deployment Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Deployment Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseDeploymentDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseDeploymentDesignerAgent = Object.freeze(
  new ReleaseDeploymentDesignerAgent(),
);
