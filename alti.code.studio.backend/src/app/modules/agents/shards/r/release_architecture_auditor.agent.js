...
class ReleaseArchitectureAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseArchitectureSystem(objective) {
    logger.info(
      `💻 [ReleaseArchitectureAuditorAgent] Analyzing Release Architecture Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Architecture Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseArchitectureAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseArchitectureAuditorAgent = Object.freeze(
  new ReleaseArchitectureAuditorAgent(),
);
