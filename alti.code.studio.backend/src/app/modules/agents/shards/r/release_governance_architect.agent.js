...
class ReleaseGovernanceArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseGovernanceSystem(objective) {
        logger.info(`💻 [ReleaseGovernanceArchitectAgent] Analyzing Release Governance Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Governance Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseGovernanceArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseGovernanceArchitectAgent = Object.freeze(new ReleaseGovernanceArchitectAgent());
