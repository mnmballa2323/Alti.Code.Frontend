...
class ReleaseResilienceStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseResilienceSystem(objective) {
        logger.info(`💻 [ReleaseResilienceStrategistAgent] Analyzing Release Resilience Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Resilience Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseResilienceStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseResilienceStrategistAgent = Object.freeze(new ReleaseResilienceStrategistAgent());
