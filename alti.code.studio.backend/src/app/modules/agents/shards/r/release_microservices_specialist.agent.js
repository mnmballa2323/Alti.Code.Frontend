...
class ReleaseMicroservicesSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseMicroservicesSystem(objective) {
        logger.info(`💻 [ReleaseMicroservicesSpecialistAgent] Analyzing Release Microservices Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Microservices Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseMicroservicesSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseMicroservicesSpecialistAgent = Object.freeze(new ReleaseMicroservicesSpecialistAgent());
