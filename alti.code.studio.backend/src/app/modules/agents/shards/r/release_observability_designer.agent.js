...
class ReleaseObservabilityDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseObservabilitySystem(objective) {
        logger.info(`💻 [ReleaseObservabilityDesignerAgent] Analyzing Release Observability Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Observability Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseObservabilityDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseObservabilityDesignerAgent = Object.freeze(new ReleaseObservabilityDesignerAgent());
