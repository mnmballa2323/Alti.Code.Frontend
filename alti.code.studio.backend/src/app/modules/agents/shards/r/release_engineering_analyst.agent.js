...
class ReleaseEngineeringAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseEngineeringSystem(objective) {
    logger.info(
      `💻 [ReleaseEngineeringAnalystAgent] Analyzing Release Engineering Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Engineering Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseEngineeringAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseEngineeringAnalystAgent = Object.freeze(
  new ReleaseEngineeringAnalystAgent(),
);
