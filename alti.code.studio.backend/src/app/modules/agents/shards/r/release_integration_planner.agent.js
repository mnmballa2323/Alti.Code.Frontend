...
class ReleaseIntegrationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseIntegrationSystem(objective) {
        logger.info(`💻 [ReleaseIntegrationPlannerAgent] Analyzing Release Integration Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Integration Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseIntegrationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseIntegrationPlannerAgent = Object.freeze(new ReleaseIntegrationPlannerAgent());
