...
class ReleaseProvisioningOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseProvisioningSystem(objective) {
    logger.info(
      `💻 [ReleaseProvisioningOrchestratorAgent] Analyzing Release Provisioning Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Provisioning Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseProvisioningOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseProvisioningOrchestratorAgent = Object.freeze(
  new ReleaseProvisioningOrchestratorAgent(),
);
