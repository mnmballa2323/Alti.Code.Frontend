...
class ReleaseGovernanceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseGovernanceSystem(objective) {
    logger.info(
      `💻 [ReleaseGovernanceConsultantAgent] Analyzing Release Governance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Governance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseGovernanceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseGovernanceConsultantAgent = Object.freeze(
  new ReleaseGovernanceConsultantAgent(),
);
