...
class ReleaseProvisioningAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseProvisioningSystem(objective) {
    logger.info(
      `💻 [ReleaseProvisioningAuditorAgent] Analyzing Release Provisioning Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Provisioning Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseProvisioningAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseProvisioningAuditorAgent = Object.freeze(
  new ReleaseProvisioningAuditorAgent(),
);
