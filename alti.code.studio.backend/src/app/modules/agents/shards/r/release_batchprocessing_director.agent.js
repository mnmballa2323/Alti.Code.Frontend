...
class ReleaseBatchProcessingDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseBatchProcessingSystem(objective) {
        logger.info(`💻 [ReleaseBatchProcessingDirectorAgent] Analyzing Release BatchProcessing Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release BatchProcessing Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseBatchProcessingDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseBatchProcessingDirectorAgent = Object.freeze(new ReleaseBatchProcessingDirectorAgent());
