...
class ReleaseConfigurationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseConfigurationSystem(objective) {
        logger.info(`💻 [ReleaseConfigurationManagerAgent] Analyzing Release Configuration Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Configuration Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseConfigurationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseConfigurationManagerAgent = Object.freeze(new ReleaseConfigurationManagerAgent());
