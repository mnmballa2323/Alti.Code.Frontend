...
class ReleaseAutomationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseAutomationSystem(objective) {
        logger.info(`💻 [ReleaseAutomationOrchestratorAgent] Analyzing Release Automation Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Automation Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseAutomationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseAutomationOrchestratorAgent = Object.freeze(new ReleaseAutomationOrchestratorAgent());
