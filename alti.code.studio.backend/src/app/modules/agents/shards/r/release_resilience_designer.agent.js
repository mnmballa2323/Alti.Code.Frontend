...
class ReleaseResilienceDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseResilienceSystem(objective) {
        logger.info(`💻 [ReleaseResilienceDesignerAgent] Analyzing Release Resilience Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Resilience Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseResilienceDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseResilienceDesignerAgent = Object.freeze(new ReleaseResilienceDesignerAgent());
