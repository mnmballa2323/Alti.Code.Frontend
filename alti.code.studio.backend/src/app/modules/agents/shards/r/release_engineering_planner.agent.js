...
class ReleaseEngineeringPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseEngineeringSystem(objective) {
        logger.info(`💻 [ReleaseEngineeringPlannerAgent] Analyzing Release Engineering Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release Engineering Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseEngineeringPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseEngineeringPlannerAgent = Object.freeze(new ReleaseEngineeringPlannerAgent());
