...
class ReleaseFaultToleranceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateReleaseFaultToleranceSystem(objective) {
        logger.info(`💻 [ReleaseFaultToleranceConsultantAgent] Analyzing Release FaultTolerance Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Release FaultTolerance Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [ReleaseFaultToleranceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const releaseFaultToleranceConsultantAgent = Object.freeze(new ReleaseFaultToleranceConsultantAgent());
