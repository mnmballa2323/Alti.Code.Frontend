...
class ReleaseIntegrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseIntegrationSystem(objective) {
    logger.info(
      `💻 [ReleaseIntegrationEngineerAgent] Analyzing Release Integration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Integration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [ReleaseIntegrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const releaseIntegrationEngineerAgent = Object.freeze(
  new ReleaseIntegrationEngineerAgent(),
);
