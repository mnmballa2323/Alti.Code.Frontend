...
class ReleaseCachingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateReleaseCachingSystem(objective) {
    logger.info(
      `💻 [ReleaseCachingPlannerAgent] Analyzing Release Caching Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Release Caching Planner Target',
      );
...
export const releaseCachingPlannerAgent = Object.freeze(
  new ReleaseCachingPlannerAgent(),
);
