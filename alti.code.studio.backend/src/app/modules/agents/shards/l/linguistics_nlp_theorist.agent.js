...
class LinguisticsNlpTheoristAgent extends GeminiCliBaseAgent {
  ...
  async generateLinguisticsSystem(linguisticsObjective) {
    logger.info(
      `🗣️ [LinguisticsNlpTheorist] Analyzing objective for syntax parsing and low-resource LLM architectures...`,
    );
...
${linguisticsObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [LinguisticsNlpTheorist] Linguistics architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [LinguisticsNlpTheorist] Failed to generate linguistics system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const linguisticsNlpTheoristAgent = Object.freeze(
  new LinguisticsNlpTheoristAgent(),
);
