...
class LiveblocksAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.description =
      'Real-time collaboration specialist for Liveblocks: Rooms, Presence (cursors/avatars), CRDT Storage, Threads/Comments, Notifications, Yjs/Lexical integration, and conflict-free collaborative editing.';
...
Output production TypeScript using \`@liveblocks/client\`, \`@liveblocks/react\`, \`@liveblocks/react-ui\`. Store \`LIVEBLOCKS_SECRET_KEY\` and \`NEXT_PUBLIC_LIVEBLOCKS_PUBLIC_KEY\` in environment variables.`;
  ...
  async consult(prompt, contextData = []) {
    logger.info(
      `🤝 Liveblocks Expert: Synthesizing real-time collaboration logic...`,
    );
...
export const liveblocksAgent = Object.freeze(new LiveblocksAgent());
