...
class LinterAgent {
    constructor() {
        this.name = 'Linter';
        this.description = 'Autonomous Code Quality fixes via ESLint/TSLint overrides.';
    }
    ...
    async fix(targetPath, language = 'typescript') {
        logger.info(`🧹 Linter Agent: Initiating deterministic sweep on: ${targetPath}`);
        ...
        try {
            if (language === 'typescript' || language === 'javascript') {
                ...
                try {
                    ...
                    logger.warn(`🧹 Linter Agent: Auto-fix completed with residual warnings. Passing to Overseer.`);
                    return {
                        success: false,
                ...
                await execAsync(cmd);
                return { success: true, report: 'Python Linter (Ruff/Black) sweep complete.', fixesApplied: -1 };
            }
...
export const linterAgent = Object.freeze(new LinterAgent());
