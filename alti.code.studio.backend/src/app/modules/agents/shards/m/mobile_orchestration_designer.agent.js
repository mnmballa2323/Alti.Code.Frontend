...
class MobileOrchestrationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileOrchestrationSystem(objective) {
    logger.info(
      `💻 [MobileOrchestrationDesignerAgent] Analyzing Mobile Orchestration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Orchestration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileOrchestrationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileOrchestrationDesignerAgent = Object.freeze(
  new MobileOrchestrationDesignerAgent(),
);
