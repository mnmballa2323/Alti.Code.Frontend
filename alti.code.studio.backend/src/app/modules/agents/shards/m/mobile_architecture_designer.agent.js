...
class MobileArchitectureDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileArchitectureSystem(objective) {
    logger.info(
      `💻 [MobileArchitectureDesignerAgent] Analyzing Mobile Architecture Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Architecture Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileArchitectureDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileArchitectureDesignerAgent = Object.freeze(
  new MobileArchitectureDesignerAgent(),
);
