...
class MobileArchitectureOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileArchitectureSystem(objective) {
        logger.info(`💻 [MobileArchitectureOrchestratorAgent] Analyzing Mobile Architecture Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Architecture Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileArchitectureOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileArchitectureOrchestratorAgent = Object.freeze(new MobileArchitectureOrchestratorAgent());
