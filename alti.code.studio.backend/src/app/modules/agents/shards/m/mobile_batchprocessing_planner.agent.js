...
class MobileBatchProcessingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileBatchProcessingSystem(objective) {
    logger.info(
      `💻 [MobileBatchProcessingPlannerAgent] Analyzing Mobile BatchProcessing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile BatchProcessing Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileBatchProcessingPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileBatchProcessingPlannerAgent = Object.freeze(
  new MobileBatchProcessingPlannerAgent(),
);
