...
class MobileContainerizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileContainerizationSystem(objective) {
    logger.info(
      `💻 [MobileContainerizationLeadAgent] Analyzing Mobile Containerization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Containerization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileContainerizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileContainerizationLeadAgent = Object.freeze(
  new MobileContainerizationLeadAgent(),
);
