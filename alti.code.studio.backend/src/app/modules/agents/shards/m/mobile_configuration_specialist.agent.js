...
class MobileConfigurationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileConfigurationSystem(objective) {
    logger.info(
      `💻 [MobileConfigurationSpecialistAgent] Analyzing Mobile Configuration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Configuration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileConfigurationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileConfigurationSpecialistAgent = Object.freeze(
  new MobileConfigurationSpecialistAgent(),
);
