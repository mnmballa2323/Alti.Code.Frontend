...
class MobileAnalyticsSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileAnalyticsSystem(objective) {
        logger.info(`💻 [MobileAnalyticsSpecialistAgent] Analyzing Mobile Analytics Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Analytics Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileAnalyticsSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileAnalyticsSpecialistAgent = Object.freeze(new MobileAnalyticsSpecialistAgent());
