...
class MobileObservabilityPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileObservabilitySystem(objective) {
        logger.info(`💻 [MobileObservabilityPlannerAgent] Analyzing Mobile Observability Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Observability Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileObservabilityPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileObservabilityPlannerAgent = Object.freeze(new MobileObservabilityPlannerAgent());
