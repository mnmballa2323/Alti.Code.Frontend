...
class MobileTelemetryOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileTelemetrySystem(objective) {
        logger.info(`💻 [MobileTelemetryOrchestratorAgent] Analyzing Mobile Telemetry Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Telemetry Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileTelemetryOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileTelemetryOrchestratorAgent = Object.freeze(new MobileTelemetryOrchestratorAgent());
