...
class MobileArchitectureStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileArchitectureSystem(objective) {
    logger.info(
      `💻 [MobileArchitectureStrategistAgent] Analyzing Mobile Architecture Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Architecture Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileArchitectureStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileArchitectureStrategistAgent = Object.freeze(
  new MobileArchitectureStrategistAgent(),
);
