...
class MobileProvisioningManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileProvisioningSystem(objective) {
    logger.info(
      `💻 [MobileProvisioningManagerAgent] Analyzing Mobile Provisioning Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Provisioning Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileProvisioningManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileProvisioningManagerAgent = Object.freeze(
  new MobileProvisioningManagerAgent(),
);
