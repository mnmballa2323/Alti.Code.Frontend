...
class MobileConfigurationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileConfigurationSystem(objective) {
    logger.info(
      `💻 [MobileConfigurationAuditorAgent] Analyzing Mobile Configuration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Configuration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileConfigurationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileConfigurationAuditorAgent = Object.freeze(
  new MobileConfigurationAuditorAgent(),
);
