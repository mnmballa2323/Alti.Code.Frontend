...
class MobileMigrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileMigrationSystem(objective) {
    logger.info(
      `💻 [MobileMigrationConsultantAgent] Analyzing Mobile Migration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Migration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileMigrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileMigrationConsultantAgent = Object.freeze(
  new MobileMigrationConsultantAgent(),
);
