...
class MobileMicroservicesEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileMicroservicesSystem(objective) {
    logger.info(
      `💻 [MobileMicroservicesEngineerAgent] Analyzing Mobile Microservices Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Microservices Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileMicroservicesEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileMicroservicesEngineerAgent = Object.freeze(
  new MobileMicroservicesEngineerAgent(),
);
