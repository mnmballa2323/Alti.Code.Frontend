...
class MobileVirtualizationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileVirtualizationSystem(objective) {
        logger.info(`💻 [MobileVirtualizationManagerAgent] Analyzing Mobile Virtualization Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Virtualization Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileVirtualizationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileVirtualizationManagerAgent = Object.freeze(new MobileVirtualizationManagerAgent());
