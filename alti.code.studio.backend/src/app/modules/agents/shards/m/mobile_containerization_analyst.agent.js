...
class MobileContainerizationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileContainerizationSystem(objective) {
        logger.info(`💻 [MobileContainerizationAnalystAgent] Analyzing Mobile Containerization Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Containerization Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileContainerizationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileContainerizationAnalystAgent = Object.freeze(new MobileContainerizationAnalystAgent());
