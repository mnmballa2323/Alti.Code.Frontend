...
class MobileObservabilityOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileObservabilitySystem(objective) {
        logger.info(`💻 [MobileObservabilityOrchestratorAgent] Analyzing Mobile Observability Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Observability Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileObservabilityOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileObservabilityOrchestratorAgent = Object.freeze(new MobileObservabilityOrchestratorAgent());
