...
class MobileAutomationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileAutomationSystem(objective) {
    logger.info(
      `💻 [MobileAutomationSpecialistAgent] Analyzing Mobile Automation Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Automation Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileAutomationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileAutomationSpecialistAgent = Object.freeze(
  new MobileAutomationSpecialistAgent(),
);
