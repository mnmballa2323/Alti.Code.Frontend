...
class MobileContainerizationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileContainerizationSystem(objective) {
        logger.info(`💻 [MobileContainerizationArchitectAgent] Analyzing Mobile Containerization Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Containerization Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileContainerizationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileContainerizationArchitectAgent = Object.freeze(new MobileContainerizationArchitectAgent());
