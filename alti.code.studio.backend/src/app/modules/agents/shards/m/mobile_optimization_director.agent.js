...
class MobileOptimizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileOptimizationSystem(objective) {
    logger.info(
      `💻 [MobileOptimizationDirectorAgent] Analyzing Mobile Optimization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Optimization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileOptimizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileOptimizationDirectorAgent = Object.freeze(
  new MobileOptimizationDirectorAgent(),
);
