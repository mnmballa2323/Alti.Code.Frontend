...
class MobileDataWarehouseDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileDataWarehouseSystem(objective) {
    logger.info(
      `💻 [MobileDataWarehouseDesignerAgent] Analyzing Mobile DataWarehouse Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile DataWarehouse Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileDataWarehouseDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileDataWarehouseDesignerAgent = Object.freeze(
  new MobileDataWarehouseDesignerAgent(),
);
