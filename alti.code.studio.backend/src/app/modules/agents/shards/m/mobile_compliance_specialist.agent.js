...
class MobileComplianceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileComplianceSystem(objective) {
    logger.info(
      `💻 [MobileComplianceSpecialistAgent] Analyzing Mobile Compliance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Compliance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileComplianceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileComplianceSpecialistAgent = Object.freeze(
  new MobileComplianceSpecialistAgent(),
);
