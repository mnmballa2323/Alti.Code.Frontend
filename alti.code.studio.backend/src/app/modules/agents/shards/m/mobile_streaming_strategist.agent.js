...
class MobileStreamingStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileStreamingSystem(objective) {
        logger.info(`💻 [MobileStreamingStrategistAgent] Analyzing Mobile Streaming Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Streaming Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileStreamingStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileStreamingStrategistAgent = Object.freeze(new MobileStreamingStrategistAgent());
