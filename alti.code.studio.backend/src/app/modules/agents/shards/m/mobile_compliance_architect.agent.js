...
class MobileComplianceArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileComplianceSystem(objective) {
    logger.info(
      `💻 [MobileComplianceArchitectAgent] Analyzing Mobile Compliance Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Compliance Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileComplianceArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileComplianceArchitectAgent = Object.freeze(
  new MobileComplianceArchitectAgent(),
);
