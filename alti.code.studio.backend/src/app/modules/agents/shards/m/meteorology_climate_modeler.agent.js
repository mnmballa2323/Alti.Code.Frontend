...
class MeteorologyClimateModelerAgent extends GeminiCliBaseAgent {
    ...
    async generateWeatherSystem(weatherObjective) {
        logger.info(`🌤️ [MeteorologyClimateModeler] Analyzing objective for Navier-Stokes weather ensembles...`);
...
${weatherObjective}
        ...
        try {
            ...
            const cleanCode = output.replace(/```javascript|```typescript|```json|```fortran|```python|```cpp|```/gi, '').trim();
            logger.info(`✅ [MeteorologyClimateModeler] Meteorology architecture generated successfully.`);
            return cleanCode;
        } catch (err) {
            logger.error(`❌ [MeteorologyClimateModeler] Failed to generate meteorology system: ${err.message}`);
            throw err;
        }
    }
}

export const meteorologyClimateModelerAgent = Object.freeze(new MeteorologyClimateModelerAgent());
