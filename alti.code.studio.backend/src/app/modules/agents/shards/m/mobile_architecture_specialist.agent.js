...
class MobileArchitectureSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileArchitectureSystem(objective) {
    logger.info(
      `💻 [MobileArchitectureSpecialistAgent] Analyzing Mobile Architecture Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Architecture Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileArchitectureSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileArchitectureSpecialistAgent = Object.freeze(
  new MobileArchitectureSpecialistAgent(),
);
