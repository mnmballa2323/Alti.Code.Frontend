...
class MobileGovernanceSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileGovernanceSystem(objective) {
        logger.info(`💻 [MobileGovernanceSpecialistAgent] Analyzing Mobile Governance Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Governance Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileGovernanceSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileGovernanceSpecialistAgent = Object.freeze(new MobileGovernanceSpecialistAgent());
