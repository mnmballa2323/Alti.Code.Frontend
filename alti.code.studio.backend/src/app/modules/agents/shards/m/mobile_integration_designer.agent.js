...
class MobileIntegrationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileIntegrationSystem(objective) {
    logger.info(
      `💻 [MobileIntegrationDesignerAgent] Analyzing Mobile Integration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Integration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileIntegrationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileIntegrationDesignerAgent = Object.freeze(
  new MobileIntegrationDesignerAgent(),
);
