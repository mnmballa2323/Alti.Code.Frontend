...
class MobileDeploymentArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileDeploymentSystem(objective) {
        logger.info(`💻 [MobileDeploymentArchitectAgent] Analyzing Mobile Deployment Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Deployment Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileDeploymentArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileDeploymentArchitectAgent = Object.freeze(new MobileDeploymentArchitectAgent());
