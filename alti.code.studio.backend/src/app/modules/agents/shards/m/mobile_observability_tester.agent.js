...
class MobileObservabilityTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileObservabilitySystem(objective) {
    logger.info(
      `💻 [MobileObservabilityTesterAgent] Analyzing Mobile Observability Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Observability Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileObservabilityTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileObservabilityTesterAgent = Object.freeze(
  new MobileObservabilityTesterAgent(),
);
