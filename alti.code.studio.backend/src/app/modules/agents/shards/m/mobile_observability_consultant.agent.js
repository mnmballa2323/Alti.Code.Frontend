...
class MobileObservabilityConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileObservabilitySystem(objective) {
    logger.info(
      `💻 [MobileObservabilityConsultantAgent] Analyzing Mobile Observability Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Observability Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileObservabilityConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileObservabilityConsultantAgent = Object.freeze(
  new MobileObservabilityConsultantAgent(),
);
