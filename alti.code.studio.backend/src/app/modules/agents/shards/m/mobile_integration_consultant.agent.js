...
class MobileIntegrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileIntegrationSystem(objective) {
    logger.info(
      `💻 [MobileIntegrationConsultantAgent] Analyzing Mobile Integration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Integration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileIntegrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileIntegrationConsultantAgent = Object.freeze(
  new MobileIntegrationConsultantAgent(),
);
