...
class MobileComplianceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileComplianceSystem(objective) {
    logger.info(
      `💻 [MobileComplianceConsultantAgent] Analyzing Mobile Compliance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Compliance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileComplianceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileComplianceConsultantAgent = Object.freeze(
  new MobileComplianceConsultantAgent(),
);
