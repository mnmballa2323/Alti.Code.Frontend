...
class MobileConfigurationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileConfigurationSystem(objective) {
    logger.info(
      `💻 [MobileConfigurationStrategistAgent] Analyzing Mobile Configuration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Configuration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileConfigurationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileConfigurationStrategistAgent = Object.freeze(
  new MobileConfigurationStrategistAgent(),
);
