...
class MobileContainerizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileContainerizationSystem(objective) {
    logger.info(
      `💻 [MobileContainerizationPlannerAgent] Analyzing Mobile Containerization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Containerization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileContainerizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileContainerizationPlannerAgent = Object.freeze(
  new MobileContainerizationPlannerAgent(),
);
