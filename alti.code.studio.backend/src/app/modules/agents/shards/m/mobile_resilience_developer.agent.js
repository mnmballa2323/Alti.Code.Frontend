...
class MobileResilienceDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileResilienceSystem(objective) {
    logger.info(
      `💻 [MobileResilienceDeveloperAgent] Analyzing Mobile Resilience Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Resilience Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileResilienceDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileResilienceDeveloperAgent = Object.freeze(
  new MobileResilienceDeveloperAgent(),
);
