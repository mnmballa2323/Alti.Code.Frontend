...
class MobileOrchestrationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileOrchestrationSystem(objective) {
        logger.info(`💻 [MobileOrchestrationDirectorAgent] Analyzing Mobile Orchestration Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Orchestration Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileOrchestrationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileOrchestrationDirectorAgent = Object.freeze(new MobileOrchestrationDirectorAgent());
