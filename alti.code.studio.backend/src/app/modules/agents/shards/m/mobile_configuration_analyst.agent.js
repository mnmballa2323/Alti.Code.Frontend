...
class MobileConfigurationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileConfigurationSystem(objective) {
        logger.info(`💻 [MobileConfigurationAnalystAgent] Analyzing Mobile Configuration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Configuration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileConfigurationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileConfigurationAnalystAgent = Object.freeze(new MobileConfigurationAnalystAgent());
