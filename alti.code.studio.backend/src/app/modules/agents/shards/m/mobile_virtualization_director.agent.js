...
class MobileVirtualizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileVirtualizationSystem(objective) {
    logger.info(
      `💻 [MobileVirtualizationDirectorAgent] Analyzing Mobile Virtualization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Virtualization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileVirtualizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileVirtualizationDirectorAgent = Object.freeze(
  new MobileVirtualizationDirectorAgent(),
);
