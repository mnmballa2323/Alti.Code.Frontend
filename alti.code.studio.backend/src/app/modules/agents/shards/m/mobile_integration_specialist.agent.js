...
class MobileIntegrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileIntegrationSystem(objective) {
    logger.info(
      `💻 [MobileIntegrationSpecialistAgent] Analyzing Mobile Integration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Integration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileIntegrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileIntegrationSpecialistAgent = Object.freeze(
  new MobileIntegrationSpecialistAgent(),
);
