...
class MobileObservabilityManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileObservabilitySystem(objective) {
        logger.info(`💻 [MobileObservabilityManagerAgent] Analyzing Mobile Observability Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Observability Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileObservabilityManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileObservabilityManagerAgent = Object.freeze(new MobileObservabilityManagerAgent());
