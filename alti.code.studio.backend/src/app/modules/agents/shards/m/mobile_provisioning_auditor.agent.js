...
class MobileProvisioningAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileProvisioningSystem(objective) {
        logger.info(`💻 [MobileProvisioningAuditorAgent] Analyzing Mobile Provisioning Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Provisioning Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileProvisioningAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileProvisioningAuditorAgent = Object.freeze(new MobileProvisioningAuditorAgent());
