...
class MobileMigrationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileMigrationSystem(objective) {
        logger.info(`💻 [MobileMigrationSpecialistAgent] Analyzing Mobile Migration Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Migration Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileMigrationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileMigrationSpecialistAgent = Object.freeze(new MobileMigrationSpecialistAgent());
