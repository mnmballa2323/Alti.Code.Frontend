...
class MobileDataWarehouseManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileDataWarehouseSystem(objective) {
        logger.info(`💻 [MobileDataWarehouseManagerAgent] Analyzing Mobile DataWarehouse Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile DataWarehouse Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileDataWarehouseManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileDataWarehouseManagerAgent = Object.freeze(new MobileDataWarehouseManagerAgent());
