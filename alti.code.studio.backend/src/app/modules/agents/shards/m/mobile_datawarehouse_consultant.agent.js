...
class MobileDataWarehouseConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileDataWarehouseSystem(objective) {
    logger.info(
      `💻 [MobileDataWarehouseConsultantAgent] Analyzing Mobile DataWarehouse Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile DataWarehouse Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileDataWarehouseConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileDataWarehouseConsultantAgent = Object.freeze(
  new MobileDataWarehouseConsultantAgent(),
);
