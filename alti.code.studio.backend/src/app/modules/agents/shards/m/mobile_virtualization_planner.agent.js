...
class MobileVirtualizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileVirtualizationSystem(objective) {
    logger.info(
      `💻 [MobileVirtualizationPlannerAgent] Analyzing Mobile Virtualization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Virtualization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileVirtualizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileVirtualizationPlannerAgent = Object.freeze(
  new MobileVirtualizationPlannerAgent(),
);
