...
class MobileAutomationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileAutomationSystem(objective) {
        logger.info(`💻 [MobileAutomationStrategistAgent] Analyzing Mobile Automation Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Automation Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileAutomationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileAutomationStrategistAgent = Object.freeze(new MobileAutomationStrategistAgent());
