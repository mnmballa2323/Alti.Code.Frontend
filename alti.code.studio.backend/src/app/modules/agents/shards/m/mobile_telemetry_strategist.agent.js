...
class MobileTelemetryStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileTelemetrySystem(objective) {
    logger.info(
      `💻 [MobileTelemetryStrategistAgent] Analyzing Mobile Telemetry Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Telemetry Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileTelemetryStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileTelemetryStrategistAgent = Object.freeze(
  new MobileTelemetryStrategistAgent(),
);
