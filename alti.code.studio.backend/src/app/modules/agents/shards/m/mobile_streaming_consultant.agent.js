...
class MobileStreamingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileStreamingSystem(objective) {
        logger.info(`💻 [MobileStreamingConsultantAgent] Analyzing Mobile Streaming Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Streaming Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileStreamingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileStreamingConsultantAgent = Object.freeze(new MobileStreamingConsultantAgent());
