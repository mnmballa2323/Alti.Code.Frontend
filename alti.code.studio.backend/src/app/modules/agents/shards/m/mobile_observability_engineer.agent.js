...
class MobileObservabilityEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileObservabilitySystem(objective) {
    logger.info(
      `💻 [MobileObservabilityEngineerAgent] Analyzing Mobile Observability Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Observability Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileObservabilityEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileObservabilityEngineerAgent = Object.freeze(
  new MobileObservabilityEngineerAgent(),
);
