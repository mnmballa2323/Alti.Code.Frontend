...
class MobileContainerizationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileContainerizationSystem(objective) {
        logger.info(`💻 [MobileContainerizationAuditorAgent] Analyzing Mobile Containerization Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Containerization Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileContainerizationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileContainerizationAuditorAgent = Object.freeze(new MobileContainerizationAuditorAgent());
