...
class MobileArchitecturePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileArchitectureSystem(objective) {
    logger.info(
      `💻 [MobileArchitecturePlannerAgent] Analyzing Mobile Architecture Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Architecture Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileArchitecturePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileArchitecturePlannerAgent = Object.freeze(
  new MobileArchitecturePlannerAgent(),
);
