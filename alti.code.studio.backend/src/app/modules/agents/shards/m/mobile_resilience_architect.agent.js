...
class MobileResilienceArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileResilienceSystem(objective) {
    logger.info(
      `💻 [MobileResilienceArchitectAgent] Analyzing Mobile Resilience Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Resilience Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileResilienceArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileResilienceArchitectAgent = Object.freeze(
  new MobileResilienceArchitectAgent(),
);
