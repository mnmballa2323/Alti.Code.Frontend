...
class MobileProvisioningStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileProvisioningSystem(objective) {
    logger.info(
      `💻 [MobileProvisioningStrategistAgent] Analyzing Mobile Provisioning Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Provisioning Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileProvisioningStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileProvisioningStrategistAgent = Object.freeze(
  new MobileProvisioningStrategistAgent(),
);
