...
class MobileServerlessArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileServerlessSystem(objective) {
    logger.info(
      `💻 [MobileServerlessArchitectAgent] Analyzing Mobile Serverless Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Serverless Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileServerlessArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileServerlessArchitectAgent = Object.freeze(
  new MobileServerlessArchitectAgent(),
);
