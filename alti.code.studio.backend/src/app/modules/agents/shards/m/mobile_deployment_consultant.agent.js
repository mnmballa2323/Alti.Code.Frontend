...
class MobileDeploymentConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileDeploymentSystem(objective) {
    logger.info(
      `💻 [MobileDeploymentConsultantAgent] Analyzing Mobile Deployment Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Deployment Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileDeploymentConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileDeploymentConsultantAgent = Object.freeze(
  new MobileDeploymentConsultantAgent(),
);
