...
class MobileContainerizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileContainerizationSystem(objective) {
    logger.info(
      `💻 [MobileContainerizationConsultantAgent] Analyzing Mobile Containerization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Containerization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileContainerizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileContainerizationConsultantAgent = Object.freeze(
  new MobileContainerizationConsultantAgent(),
);
