...
class MobileDataWarehouseArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileDataWarehouseSystem(objective) {
        logger.info(`💻 [MobileDataWarehouseArchitectAgent] Analyzing Mobile DataWarehouse Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile DataWarehouse Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileDataWarehouseArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileDataWarehouseArchitectAgent = Object.freeze(new MobileDataWarehouseArchitectAgent());
