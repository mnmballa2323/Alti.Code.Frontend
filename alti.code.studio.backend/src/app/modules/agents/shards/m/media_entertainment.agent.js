...
class MediaEntertainmentAgent extends GeminiCliBaseAgent {
  ...
  async generateMediaSystem(mediaObjective) {
    logger.info(
      `🎬 [MediaEntertainment] Analyzing objective for video streaming and edge caching...`,
    );
...
${mediaObjective}
    ...
    try {
      ...
      const cleanCode = output
        .replace(/```javascript|```bash|```sh|```json|```/gi, '')
        .trim();
      logger.info(
        `✅ [MediaEntertainment] Media architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [MediaEntertainment] Failed to generate media system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const mediaEntertainmentAgent = Object.freeze(
  new MediaEntertainmentAgent(),
);
