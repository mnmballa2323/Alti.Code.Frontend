...
class MobileAutomationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileAutomationSystem(objective) {
    logger.info(
      `💻 [MobileAutomationConsultantAgent] Analyzing Mobile Automation Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Automation Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileAutomationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileAutomationConsultantAgent = Object.freeze(
  new MobileAutomationConsultantAgent(),
);
