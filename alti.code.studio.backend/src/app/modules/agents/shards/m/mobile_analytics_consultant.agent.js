...
class MobileAnalyticsConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileAnalyticsSystem(objective) {
    logger.info(
      `💻 [MobileAnalyticsConsultantAgent] Analyzing Mobile Analytics Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Analytics Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileAnalyticsConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileAnalyticsConsultantAgent = Object.freeze(
  new MobileAnalyticsConsultantAgent(),
);
