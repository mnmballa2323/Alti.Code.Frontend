...
class MobileTelemetryConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileTelemetrySystem(objective) {
    logger.info(
      `💻 [MobileTelemetryConsultantAgent] Analyzing Mobile Telemetry Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Telemetry Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileTelemetryConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileTelemetryConsultantAgent = Object.freeze(
  new MobileTelemetryConsultantAgent(),
);
