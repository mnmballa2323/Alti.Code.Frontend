...
class MobileObservabilityAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileObservabilitySystem(objective) {
    logger.info(
      `💻 [MobileObservabilityAuditorAgent] Analyzing Mobile Observability Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Observability Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileObservabilityAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileObservabilityAuditorAgent = Object.freeze(
  new MobileObservabilityAuditorAgent(),
);
