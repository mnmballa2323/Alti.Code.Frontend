...
class MetallurgyMaterialsEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateMaterialsSystem(materialsObjective) {
    logger.info(
      `⚙️ [MetallurgyMaterialsEngineer] Analyzing objective for superalloys and CALPHAD thermodynamics...`,
    );
...
${materialsObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [MetallurgyMaterialsEngineer] Materials architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [MetallurgyMaterialsEngineer] Failed to generate materials system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const metallurgyMaterialsEngineerAgent = Object.freeze(
  new MetallurgyMaterialsEngineerAgent(),
);
