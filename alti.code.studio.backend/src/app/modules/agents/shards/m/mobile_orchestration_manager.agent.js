...
class MobileOrchestrationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileOrchestrationSystem(objective) {
    logger.info(
      `💻 [MobileOrchestrationManagerAgent] Analyzing Mobile Orchestration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Orchestration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileOrchestrationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileOrchestrationManagerAgent = Object.freeze(
  new MobileOrchestrationManagerAgent(),
);
