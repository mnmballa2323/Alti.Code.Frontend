...
class MobileProvisioningOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileProvisioningSystem(objective) {
        logger.info(`💻 [MobileProvisioningOrchestratorAgent] Analyzing Mobile Provisioning Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Provisioning Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileProvisioningOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileProvisioningOrchestratorAgent = Object.freeze(new MobileProvisioningOrchestratorAgent());
