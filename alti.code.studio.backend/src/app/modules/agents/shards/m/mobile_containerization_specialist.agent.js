...
class MobileContainerizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileContainerizationSystem(objective) {
        logger.info(`💻 [MobileContainerizationSpecialistAgent] Analyzing Mobile Containerization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Containerization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileContainerizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileContainerizationSpecialistAgent = Object.freeze(new MobileContainerizationSpecialistAgent());
