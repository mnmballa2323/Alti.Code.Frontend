...
class MobileVirtualizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileVirtualizationSystem(objective) {
    logger.info(
      `💻 [MobileVirtualizationAnalystAgent] Analyzing Mobile Virtualization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Virtualization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileVirtualizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileVirtualizationAnalystAgent = Object.freeze(
  new MobileVirtualizationAnalystAgent(),
);
