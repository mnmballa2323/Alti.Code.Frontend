...
class MobileComplianceDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileComplianceSystem(objective) {
    logger.info(
      `💻 [MobileComplianceDeveloperAgent] Analyzing Mobile Compliance Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Compliance Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileComplianceDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileComplianceDeveloperAgent = Object.freeze(
  new MobileComplianceDeveloperAgent(),
);
