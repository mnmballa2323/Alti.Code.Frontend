...
class MobileEngineeringEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileEngineeringSystem(objective) {
    logger.info(
      `💻 [MobileEngineeringEngineerAgent] Analyzing Mobile Engineering Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Engineering Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileEngineeringEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileEngineeringEngineerAgent = Object.freeze(
  new MobileEngineeringEngineerAgent(),
);
