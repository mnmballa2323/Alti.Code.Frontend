...
class MobileCachingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileCachingSystem(objective) {
    logger.info(
      `💻 [MobileCachingOrchestratorAgent] Analyzing Mobile Caching Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Caching Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileCachingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileCachingOrchestratorAgent = Object.freeze(
  new MobileCachingOrchestratorAgent(),
);
