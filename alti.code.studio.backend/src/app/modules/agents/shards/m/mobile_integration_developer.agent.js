...
class MobileIntegrationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileIntegrationSystem(objective) {
        logger.info(`💻 [MobileIntegrationDeveloperAgent] Analyzing Mobile Integration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Integration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileIntegrationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileIntegrationDeveloperAgent = Object.freeze(new MobileIntegrationDeveloperAgent());
