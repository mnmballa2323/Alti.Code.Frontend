...
class MacroeconomicsPolicyAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateEconomicModel(economicObjective) {
        logger.info(`📈 [MacroeconomicsPolicyAnalyst] Analyzing objective for econometric forecasting and CBDC tokenomics...`);
...
${economicObjective}
        ...
        try {
            ...
            logger.info(`✅ [MacroeconomicsPolicyAnalyst] Econometric architecture generated successfully.`);
            return cleanCode;
        } catch (err) {
            logger.error(`❌ [MacroeconomicsPolicyAnalyst] Failed to generate macroeconomic system: ${err.message}`);
            throw err;
        }
    }
}

export const macroeconomicsPolicyAnalystAgent = Object.freeze(new MacroeconomicsPolicyAnalystAgent());
