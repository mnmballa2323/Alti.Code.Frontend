...
class MobileProvisioningConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileProvisioningSystem(objective) {
    logger.info(
      `💻 [MobileProvisioningConsultantAgent] Analyzing Mobile Provisioning Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Provisioning Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileProvisioningConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileProvisioningConsultantAgent = Object.freeze(
  new MobileProvisioningConsultantAgent(),
);
