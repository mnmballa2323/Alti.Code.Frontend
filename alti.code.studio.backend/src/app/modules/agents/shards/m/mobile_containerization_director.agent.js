...
class MobileContainerizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileContainerizationSystem(objective) {
    logger.info(
      `💻 [MobileContainerizationDirectorAgent] Analyzing Mobile Containerization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Containerization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileContainerizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileContainerizationDirectorAgent = Object.freeze(
  new MobileContainerizationDirectorAgent(),
);
