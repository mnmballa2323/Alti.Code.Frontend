...
class MobileProvisioningDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileProvisioningSystem(objective) {
    logger.info(
      `💻 [MobileProvisioningDeveloperAgent] Analyzing Mobile Provisioning Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Provisioning Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileProvisioningDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileProvisioningDeveloperAgent = Object.freeze(
  new MobileProvisioningDeveloperAgent(),
);
