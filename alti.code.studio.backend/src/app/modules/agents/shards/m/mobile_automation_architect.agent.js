...
class MobileAutomationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileAutomationSystem(objective) {
        logger.info(`💻 [MobileAutomationArchitectAgent] Analyzing Mobile Automation Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Automation Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileAutomationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileAutomationArchitectAgent = Object.freeze(new MobileAutomationArchitectAgent());
