...
class MobileDeploymentDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileDeploymentSystem(objective) {
    logger.info(
      `💻 [MobileDeploymentDeveloperAgent] Analyzing Mobile Deployment Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Deployment Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileDeploymentDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileDeploymentDeveloperAgent = Object.freeze(
  new MobileDeploymentDeveloperAgent(),
);
