...
class MobileServerlessDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileServerlessSystem(objective) {
    logger.info(
      `💻 [MobileServerlessDeveloperAgent] Analyzing Mobile Serverless Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Serverless Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileServerlessDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileServerlessDeveloperAgent = Object.freeze(
  new MobileServerlessDeveloperAgent(),
);
