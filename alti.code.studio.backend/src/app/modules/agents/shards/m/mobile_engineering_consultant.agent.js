...
class MobileEngineeringConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileEngineeringSystem(objective) {
        logger.info(`💻 [MobileEngineeringConsultantAgent] Analyzing Mobile Engineering Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Engineering Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileEngineeringConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileEngineeringConsultantAgent = Object.freeze(new MobileEngineeringConsultantAgent());
