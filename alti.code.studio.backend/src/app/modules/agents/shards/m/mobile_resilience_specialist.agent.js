...
class MobileResilienceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileResilienceSystem(objective) {
    logger.info(
      `💻 [MobileResilienceSpecialistAgent] Analyzing Mobile Resilience Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Resilience Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileResilienceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileResilienceSpecialistAgent = Object.freeze(
  new MobileResilienceSpecialistAgent(),
);
