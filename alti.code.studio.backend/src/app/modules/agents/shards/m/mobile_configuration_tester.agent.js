...
class MobileConfigurationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileConfigurationSystem(objective) {
    logger.info(
      `💻 [MobileConfigurationTesterAgent] Analyzing Mobile Configuration Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Configuration Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileConfigurationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileConfigurationTesterAgent = Object.freeze(
  new MobileConfigurationTesterAgent(),
);
