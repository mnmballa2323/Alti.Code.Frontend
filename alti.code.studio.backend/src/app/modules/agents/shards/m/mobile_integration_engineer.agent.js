...
class MobileIntegrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileIntegrationSystem(objective) {
    logger.info(
      `💻 [MobileIntegrationEngineerAgent] Analyzing Mobile Integration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Integration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileIntegrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileIntegrationEngineerAgent = Object.freeze(
  new MobileIntegrationEngineerAgent(),
);
