...
class MobileArchitectureAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileArchitectureSystem(objective) {
        logger.info(`💻 [MobileArchitectureAuditorAgent] Analyzing Mobile Architecture Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Architecture Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileArchitectureAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileArchitectureAuditorAgent = Object.freeze(new MobileArchitectureAuditorAgent());
