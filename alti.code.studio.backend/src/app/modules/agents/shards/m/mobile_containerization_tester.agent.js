...
class MobileContainerizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileContainerizationSystem(objective) {
    logger.info(
      `💻 [MobileContainerizationTesterAgent] Analyzing Mobile Containerization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Containerization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileContainerizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileContainerizationTesterAgent = Object.freeze(
  new MobileContainerizationTesterAgent(),
);
