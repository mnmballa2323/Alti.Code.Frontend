...
class MobileDataWarehouseAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileDataWarehouseSystem(objective) {
        logger.info(`💻 [MobileDataWarehouseAnalystAgent] Analyzing Mobile DataWarehouse Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile DataWarehouse Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileDataWarehouseAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileDataWarehouseAnalystAgent = Object.freeze(new MobileDataWarehouseAnalystAgent());
