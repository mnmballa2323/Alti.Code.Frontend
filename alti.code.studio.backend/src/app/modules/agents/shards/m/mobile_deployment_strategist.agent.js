...
class MobileDeploymentStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileDeploymentSystem(objective) {
    logger.info(
      `💻 [MobileDeploymentStrategistAgent] Analyzing Mobile Deployment Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Deployment Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileDeploymentStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileDeploymentStrategistAgent = Object.freeze(
  new MobileDeploymentStrategistAgent(),
);
