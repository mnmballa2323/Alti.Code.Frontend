...
class MobileProvisioningAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileProvisioningSystem(objective) {
        logger.info(`💻 [MobileProvisioningAnalystAgent] Analyzing Mobile Provisioning Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Provisioning Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileProvisioningAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileProvisioningAnalystAgent = Object.freeze(new MobileProvisioningAnalystAgent());
