...
class MobileProvisioningArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileProvisioningSystem(objective) {
    logger.info(
      `💻 [MobileProvisioningArchitectAgent] Analyzing Mobile Provisioning Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Provisioning Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileProvisioningArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileProvisioningArchitectAgent = Object.freeze(
  new MobileProvisioningArchitectAgent(),
);
