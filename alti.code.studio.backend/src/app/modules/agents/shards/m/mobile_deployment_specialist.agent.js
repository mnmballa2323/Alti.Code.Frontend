...
class MobileDeploymentSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileDeploymentSystem(objective) {
    logger.info(
      `💻 [MobileDeploymentSpecialistAgent] Analyzing Mobile Deployment Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Deployment Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileDeploymentSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileDeploymentSpecialistAgent = Object.freeze(
  new MobileDeploymentSpecialistAgent(),
);
