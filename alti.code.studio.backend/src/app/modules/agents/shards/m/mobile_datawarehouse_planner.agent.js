...
class MobileDataWarehousePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileDataWarehouseSystem(objective) {
        logger.info(`💻 [MobileDataWarehousePlannerAgent] Analyzing Mobile DataWarehouse Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile DataWarehouse Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileDataWarehousePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileDataWarehousePlannerAgent = Object.freeze(new MobileDataWarehousePlannerAgent());
