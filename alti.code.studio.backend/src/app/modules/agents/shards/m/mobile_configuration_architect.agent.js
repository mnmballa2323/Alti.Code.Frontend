...
class MobileConfigurationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileConfigurationSystem(objective) {
    logger.info(
      `💻 [MobileConfigurationArchitectAgent] Analyzing Mobile Configuration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Configuration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileConfigurationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileConfigurationArchitectAgent = Object.freeze(
  new MobileConfigurationArchitectAgent(),
);
