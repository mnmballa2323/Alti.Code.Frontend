...
class MobileVirtualizationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileVirtualizationSystem(objective) {
        logger.info(`💻 [MobileVirtualizationAuditorAgent] Analyzing Mobile Virtualization Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Virtualization Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileVirtualizationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileVirtualizationAuditorAgent = Object.freeze(new MobileVirtualizationAuditorAgent());
