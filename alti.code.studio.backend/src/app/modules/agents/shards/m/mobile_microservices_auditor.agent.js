...
class MobileMicroservicesAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileMicroservicesSystem(objective) {
    logger.info(
      `💻 [MobileMicroservicesAuditorAgent] Analyzing Mobile Microservices Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Microservices Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileMicroservicesAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileMicroservicesAuditorAgent = Object.freeze(
  new MobileMicroservicesAuditorAgent(),
);
