...
class MobileArchitectureConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileArchitectureSystem(objective) {
    logger.info(
      `💻 [MobileArchitectureConsultantAgent] Analyzing Mobile Architecture Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Architecture Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileArchitectureConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileArchitectureConsultantAgent = Object.freeze(
  new MobileArchitectureConsultantAgent(),
);
