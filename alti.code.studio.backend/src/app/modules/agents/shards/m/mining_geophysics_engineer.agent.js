...
class MiningGeophysicsEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateMiningSystem(miningObjective) {
    logger.info(
      `⛏️ [MiningGeophysicsEngineer] Analyzing objective for geological modeling and autonomous extraction...`,
    );
...
${miningObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [MiningGeophysicsEngineer] Mining architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [MiningGeophysicsEngineer] Failed to generate mining system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const miningGeophysicsEngineerAgent = Object.freeze(
  new MiningGeophysicsEngineerAgent(),
);
