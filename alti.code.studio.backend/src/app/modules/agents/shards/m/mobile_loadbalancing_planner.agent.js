...
class MobileLoadBalancingPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileLoadBalancingSystem(objective) {
        logger.info(`💻 [MobileLoadBalancingPlannerAgent] Analyzing Mobile LoadBalancing Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile LoadBalancing Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileLoadBalancingPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileLoadBalancingPlannerAgent = Object.freeze(new MobileLoadBalancingPlannerAgent());
