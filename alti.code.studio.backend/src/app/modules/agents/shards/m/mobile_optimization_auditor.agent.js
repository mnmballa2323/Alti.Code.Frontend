...
class MobileOptimizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileOptimizationSystem(objective) {
    logger.info(
      `💻 [MobileOptimizationAuditorAgent] Analyzing Mobile Optimization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Optimization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileOptimizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileOptimizationAuditorAgent = Object.freeze(
  new MobileOptimizationAuditorAgent(),
);
