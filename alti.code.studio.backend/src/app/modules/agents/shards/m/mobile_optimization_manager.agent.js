...
class MobileOptimizationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileOptimizationSystem(objective) {
    logger.info(
      `💻 [MobileOptimizationManagerAgent] Analyzing Mobile Optimization Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Optimization Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileOptimizationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileOptimizationManagerAgent = Object.freeze(
  new MobileOptimizationManagerAgent(),
);
