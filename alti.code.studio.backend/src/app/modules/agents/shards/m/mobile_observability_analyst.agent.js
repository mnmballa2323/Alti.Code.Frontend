...
class MobileObservabilityAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileObservabilitySystem(objective) {
    logger.info(
      `💻 [MobileObservabilityAnalystAgent] Analyzing Mobile Observability Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Observability Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileObservabilityAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileObservabilityAnalystAgent = Object.freeze(
  new MobileObservabilityAnalystAgent(),
);
