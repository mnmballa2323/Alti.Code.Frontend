...
class MobileConfigurationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileConfigurationSystem(objective) {
        logger.info(`💻 [MobileConfigurationEngineerAgent] Analyzing Mobile Configuration Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Configuration Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileConfigurationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileConfigurationEngineerAgent = Object.freeze(new MobileConfigurationEngineerAgent());
