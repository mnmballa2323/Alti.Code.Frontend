...
class MobileEngineeringDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileEngineeringSystem(objective) {
    logger.info(
      `💻 [MobileEngineeringDirectorAgent] Analyzing Mobile Engineering Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Engineering Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileEngineeringDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileEngineeringDirectorAgent = Object.freeze(
  new MobileEngineeringDirectorAgent(),
);
