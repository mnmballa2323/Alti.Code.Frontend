...
class MobileMigrationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileMigrationSystem(objective) {
        logger.info(`💻 [MobileMigrationStrategistAgent] Analyzing Mobile Migration Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Migration Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileMigrationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileMigrationStrategistAgent = Object.freeze(new MobileMigrationStrategistAgent());
