...
class MobileOptimizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateMobileOptimizationSystem(objective) {
    logger.info(
      `💻 [MobileOptimizationPlannerAgent] Analyzing Mobile Optimization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Mobile Optimization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [MobileOptimizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const mobileOptimizationPlannerAgent = Object.freeze(
  new MobileOptimizationPlannerAgent(),
);
