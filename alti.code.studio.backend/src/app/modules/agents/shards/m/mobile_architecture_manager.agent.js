...
class MobileArchitectureManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileArchitectureSystem(objective) {
        logger.info(`💻 [MobileArchitectureManagerAgent] Analyzing Mobile Architecture Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Architecture Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileArchitectureManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileArchitectureManagerAgent = Object.freeze(new MobileArchitectureManagerAgent());
