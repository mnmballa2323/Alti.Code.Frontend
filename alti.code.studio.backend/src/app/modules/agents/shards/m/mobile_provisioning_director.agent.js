...
class MobileProvisioningDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileProvisioningSystem(objective) {
        logger.info(`💻 [MobileProvisioningDirectorAgent] Analyzing Mobile Provisioning Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile Provisioning Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileProvisioningDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileProvisioningDirectorAgent = Object.freeze(new MobileProvisioningDirectorAgent());
