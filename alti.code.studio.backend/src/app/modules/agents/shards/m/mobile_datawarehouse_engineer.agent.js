...
class MobileDataWarehouseEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateMobileDataWarehouseSystem(objective) {
        logger.info(`💻 [MobileDataWarehouseEngineerAgent] Analyzing Mobile DataWarehouse Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Mobile DataWarehouse Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [MobileDataWarehouseEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const mobileDataWarehouseEngineerAgent = Object.freeze(new MobileDataWarehouseEngineerAgent());
