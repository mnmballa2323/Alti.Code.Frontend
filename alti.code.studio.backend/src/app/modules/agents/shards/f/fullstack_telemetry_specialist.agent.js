...
class FullStackTelemetrySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackTelemetrySystem(objective) {
    logger.info(
      `💻 [FullStackTelemetrySpecialistAgent] Analyzing FullStack Telemetry Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Telemetry Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackTelemetrySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackTelemetrySpecialistAgent = Object.freeze(
  new FullStackTelemetrySpecialistAgent(),
);
