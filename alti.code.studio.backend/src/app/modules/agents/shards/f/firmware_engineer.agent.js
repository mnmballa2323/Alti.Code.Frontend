...
class FirmwareEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateFirmware(hardwareSpec, logicalObjective) {
        logger.info(`📟 [FirmwareEngineer] Synthesizing bare-metal C firmware for embedded target...`);
...
${logicalObjective}
        ...
        try {
            ...
            logger.info(`✅ [FirmwareEngineer] Embedded firmware synthesized successfully.`);
            return cleanC;
        } catch (err) {
            logger.error(`❌ [FirmwareEngineer] Failed to generate firmware: ${err.message}`);
            throw err;
        }
    }
}
