...
class FrontendProvisioningTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendProvisioningSystem(objective) {
    logger.info(
      `💻 [FrontendProvisioningTesterAgent] Analyzing Frontend Provisioning Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Provisioning Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendProvisioningTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendProvisioningTesterAgent = Object.freeze(
  new FrontendProvisioningTesterAgent(),
);
