...
class FullStackContainerizationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackContainerizationSystem(objective) {
        logger.info(`💻 [FullStackContainerizationDesignerAgent] Analyzing FullStack Containerization Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Containerization Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackContainerizationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackContainerizationDesignerAgent = Object.freeze(new FullStackContainerizationDesignerAgent());
