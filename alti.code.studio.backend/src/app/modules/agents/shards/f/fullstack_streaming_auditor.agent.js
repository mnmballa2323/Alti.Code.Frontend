...
class FullStackStreamingAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackStreamingSystem(objective) {
    logger.info(
      `💻 [FullStackStreamingAuditorAgent] Analyzing FullStack Streaming Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Streaming Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackStreamingAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackStreamingAuditorAgent = Object.freeze(
  new FullStackStreamingAuditorAgent(),
);
