...
class FrontendDataWarehouseDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendDataWarehouseSystem(objective) {
        logger.info(`💻 [FrontendDataWarehouseDesignerAgent] Analyzing Frontend DataWarehouse Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend DataWarehouse Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendDataWarehouseDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendDataWarehouseDesignerAgent = Object.freeze(new FrontendDataWarehouseDesignerAgent());
