...
class FullStackStreamingEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackStreamingSystem(objective) {
        logger.info(`💻 [FullStackStreamingEngineerAgent] Analyzing FullStack Streaming Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Streaming Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackStreamingEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackStreamingEngineerAgent = Object.freeze(new FullStackStreamingEngineerAgent());
