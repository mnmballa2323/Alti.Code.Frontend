...
class FrontendOrchestrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendOrchestrationSystem(objective) {
    logger.info(
      `💻 [FrontendOrchestrationSpecialistAgent] Analyzing Frontend Orchestration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Orchestration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendOrchestrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendOrchestrationSpecialistAgent = Object.freeze(
  new FrontendOrchestrationSpecialistAgent(),
);
