...
class FrontendResilienceEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendResilienceSystem(objective) {
        logger.info(`💻 [FrontendResilienceEngineerAgent] Analyzing Frontend Resilience Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Resilience Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendResilienceEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendResilienceEngineerAgent = Object.freeze(new FrontendResilienceEngineerAgent());
