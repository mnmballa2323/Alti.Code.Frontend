...
class FullStackCachingStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackCachingSystem(objective) {
        logger.info(`💻 [FullStackCachingStrategistAgent] Analyzing FullStack Caching Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Caching Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackCachingStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackCachingStrategistAgent = Object.freeze(new FullStackCachingStrategistAgent());
