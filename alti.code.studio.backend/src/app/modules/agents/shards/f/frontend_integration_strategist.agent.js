...
class FrontendIntegrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendIntegrationSystem(objective) {
    logger.info(
      `💻 [FrontendIntegrationStrategistAgent] Analyzing Frontend Integration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Integration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendIntegrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendIntegrationStrategistAgent = Object.freeze(
  new FrontendIntegrationStrategistAgent(),
);
