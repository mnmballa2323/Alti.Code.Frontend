...
class FullStackDeploymentStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDeploymentSystem(objective) {
    logger.info(
      `💻 [FullStackDeploymentStrategistAgent] Analyzing FullStack Deployment Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Deployment Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDeploymentStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDeploymentStrategistAgent = Object.freeze(
  new FullStackDeploymentStrategistAgent(),
);
