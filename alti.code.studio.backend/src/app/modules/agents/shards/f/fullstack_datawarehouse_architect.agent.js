...
class FullStackDataWarehouseArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDataWarehouseSystem(objective) {
    logger.info(
      `💻 [FullStackDataWarehouseArchitectAgent] Analyzing FullStack DataWarehouse Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack DataWarehouse Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDataWarehouseArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDataWarehouseArchitectAgent = Object.freeze(
  new FullStackDataWarehouseArchitectAgent(),
);
