...
class FrontendObservabilityDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendObservabilitySystem(objective) {
    logger.info(
      `💻 [FrontendObservabilityDeveloperAgent] Analyzing Frontend Observability Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Observability Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendObservabilityDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendObservabilityDeveloperAgent = Object.freeze(
  new FrontendObservabilityDeveloperAgent(),
);
