...
class FrontendArchitectureAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendArchitectureSystem(objective) {
        logger.info(`💻 [FrontendArchitectureAnalystAgent] Analyzing Frontend Architecture Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Architecture Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendArchitectureAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendArchitectureAnalystAgent = Object.freeze(new FrontendArchitectureAnalystAgent());
