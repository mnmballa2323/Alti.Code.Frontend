...
class FullStackTelemetryOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackTelemetrySystem(objective) {
    logger.info(
      `💻 [FullStackTelemetryOrchestratorAgent] Analyzing FullStack Telemetry Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Telemetry Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackTelemetryOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackTelemetryOrchestratorAgent = Object.freeze(
  new FullStackTelemetryOrchestratorAgent(),
);
