...
class FullStackComplianceEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackComplianceSystem(objective) {
        logger.info(`💻 [FullStackComplianceEngineerAgent] Analyzing FullStack Compliance Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Compliance Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackComplianceEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackComplianceEngineerAgent = Object.freeze(new FullStackComplianceEngineerAgent());
