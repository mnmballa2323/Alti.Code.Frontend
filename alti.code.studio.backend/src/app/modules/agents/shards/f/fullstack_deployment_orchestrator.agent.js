...
class FullStackDeploymentOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackDeploymentSystem(objective) {
        logger.info(`💻 [FullStackDeploymentOrchestratorAgent] Analyzing FullStack Deployment Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Deployment Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackDeploymentOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackDeploymentOrchestratorAgent = Object.freeze(new FullStackDeploymentOrchestratorAgent());
