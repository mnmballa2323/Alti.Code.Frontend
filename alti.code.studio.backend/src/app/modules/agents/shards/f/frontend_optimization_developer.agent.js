...
class FrontendOptimizationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendOptimizationSystem(objective) {
        logger.info(`💻 [FrontendOptimizationDeveloperAgent] Analyzing Frontend Optimization Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Optimization Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendOptimizationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendOptimizationDeveloperAgent = Object.freeze(new FrontendOptimizationDeveloperAgent());
