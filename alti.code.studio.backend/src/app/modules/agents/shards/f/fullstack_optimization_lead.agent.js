...
class FullStackOptimizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackOptimizationSystem(objective) {
    logger.info(
      `💻 [FullStackOptimizationLeadAgent] Analyzing FullStack Optimization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Optimization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackOptimizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackOptimizationLeadAgent = Object.freeze(
  new FullStackOptimizationLeadAgent(),
);
