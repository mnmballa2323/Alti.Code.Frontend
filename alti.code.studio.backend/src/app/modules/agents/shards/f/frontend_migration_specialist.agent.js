...
class FrontendMigrationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendMigrationSystem(objective) {
        logger.info(`💻 [FrontendMigrationSpecialistAgent] Analyzing Frontend Migration Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Migration Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendMigrationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendMigrationSpecialistAgent = Object.freeze(new FrontendMigrationSpecialistAgent());
