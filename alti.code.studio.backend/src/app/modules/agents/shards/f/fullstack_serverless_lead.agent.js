...
class FullStackServerlessLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackServerlessSystem(objective) {
    logger.info(
      `💻 [FullStackServerlessLeadAgent] Analyzing FullStack Serverless Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Serverless Lead Target',
      );
...
export const fullStackServerlessLeadAgent = Object.freeze(
  new FullStackServerlessLeadAgent(),
);
