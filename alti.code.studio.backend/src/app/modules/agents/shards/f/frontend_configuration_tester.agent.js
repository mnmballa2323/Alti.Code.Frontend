...
class FrontendConfigurationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendConfigurationSystem(objective) {
    logger.info(
      `💻 [FrontendConfigurationTesterAgent] Analyzing Frontend Configuration Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Configuration Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendConfigurationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendConfigurationTesterAgent = Object.freeze(
  new FrontendConfigurationTesterAgent(),
);
