...
class FrontendOptimizationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendOptimizationSystem(objective) {
        logger.info(`💻 [FrontendOptimizationTesterAgent] Analyzing Frontend Optimization Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Optimization Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendOptimizationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendOptimizationTesterAgent = Object.freeze(new FrontendOptimizationTesterAgent());
