...
class FullStackDataLakeArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDataLakeSystem(objective) {
    logger.info(
      `💻 [FullStackDataLakeArchitectAgent] Analyzing FullStack DataLake Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack DataLake Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDataLakeArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDataLakeArchitectAgent = Object.freeze(
  new FullStackDataLakeArchitectAgent(),
);
