...
class FrontendResilienceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendResilienceSystem(objective) {
    logger.info(
      `💻 [FrontendResilienceConsultantAgent] Analyzing Frontend Resilience Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Resilience Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendResilienceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendResilienceConsultantAgent = Object.freeze(
  new FrontendResilienceConsultantAgent(),
);
