...
class FrontendGovernanceStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendGovernanceSystem(objective) {
        logger.info(`💻 [FrontendGovernanceStrategistAgent] Analyzing Frontend Governance Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Governance Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendGovernanceStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendGovernanceStrategistAgent = Object.freeze(new FrontendGovernanceStrategistAgent());
