...
class FullStackOrchestrationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackOrchestrationSystem(objective) {
        logger.info(`💻 [FullStackOrchestrationPlannerAgent] Analyzing FullStack Orchestration Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Orchestration Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackOrchestrationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackOrchestrationPlannerAgent = Object.freeze(new FullStackOrchestrationPlannerAgent());
