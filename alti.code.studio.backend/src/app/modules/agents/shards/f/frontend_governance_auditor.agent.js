...
class FrontendGovernanceAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendGovernanceSystem(objective) {
    logger.info(
      `💻 [FrontendGovernanceAuditorAgent] Analyzing Frontend Governance Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Governance Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendGovernanceAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendGovernanceAuditorAgent = Object.freeze(
  new FrontendGovernanceAuditorAgent(),
);
