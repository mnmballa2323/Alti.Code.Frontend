...
class FullStackOrchestrationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackOrchestrationSystem(objective) {
        logger.info(`💻 [FullStackOrchestrationAuditorAgent] Analyzing FullStack Orchestration Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Orchestration Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackOrchestrationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackOrchestrationAuditorAgent = Object.freeze(new FullStackOrchestrationAuditorAgent());
