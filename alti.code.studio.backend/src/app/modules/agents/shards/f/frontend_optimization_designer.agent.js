...
class FrontendOptimizationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendOptimizationSystem(objective) {
        logger.info(`💻 [FrontendOptimizationDesignerAgent] Analyzing Frontend Optimization Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Optimization Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendOptimizationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendOptimizationDesignerAgent = Object.freeze(new FrontendOptimizationDesignerAgent());
