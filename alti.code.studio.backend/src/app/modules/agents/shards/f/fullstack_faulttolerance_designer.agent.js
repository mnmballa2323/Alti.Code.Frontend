...
class FullStackFaultToleranceDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackFaultToleranceSystem(objective) {
        logger.info(`💻 [FullStackFaultToleranceDesignerAgent] Analyzing FullStack FaultTolerance Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack FaultTolerance Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackFaultToleranceDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackFaultToleranceDesignerAgent = Object.freeze(new FullStackFaultToleranceDesignerAgent());
