...
class FullStackTelemetryAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackTelemetrySystem(objective) {
        logger.info(`💻 [FullStackTelemetryAnalystAgent] Analyzing FullStack Telemetry Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Telemetry Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackTelemetryAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackTelemetryAnalystAgent = Object.freeze(new FullStackTelemetryAnalystAgent());
