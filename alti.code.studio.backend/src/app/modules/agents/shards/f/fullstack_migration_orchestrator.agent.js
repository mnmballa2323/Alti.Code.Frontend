...
class FullStackMigrationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackMigrationSystem(objective) {
        logger.info(`💻 [FullStackMigrationOrchestratorAgent] Analyzing FullStack Migration Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Migration Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackMigrationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackMigrationOrchestratorAgent = Object.freeze(new FullStackMigrationOrchestratorAgent());
