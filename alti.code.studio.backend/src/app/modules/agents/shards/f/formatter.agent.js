...
class FormatterAgent {
  ...
  async formatCodeString(rawCode, parserType = 'typescript') {
    logger.info(
      `📐 Formatter Agent: Formatting string [Parser: ${parserType}]...`,
    );
    try {
      ...
      logger.error(
        `❌ Formatter Agent: Parsing failed. The generative LLM likely created a severe syntax error. Bypassing formatting.`,
        err.message,
      );
