...
class FullStackEngineeringManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackEngineeringSystem(objective) {
    logger.info(
      `💻 [FullStackEngineeringManagerAgent] Analyzing FullStack Engineering Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Engineering Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackEngineeringManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackEngineeringManagerAgent = Object.freeze(
  new FullStackEngineeringManagerAgent(),
);
