...
class FrontendMicroservicesAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendMicroservicesSystem(objective) {
        logger.info(`💻 [FrontendMicroservicesAnalystAgent] Analyzing Frontend Microservices Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Microservices Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendMicroservicesAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendMicroservicesAnalystAgent = Object.freeze(new FrontendMicroservicesAnalystAgent());
