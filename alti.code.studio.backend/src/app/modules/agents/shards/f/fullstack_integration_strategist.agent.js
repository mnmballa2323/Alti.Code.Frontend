...
class FullStackIntegrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackIntegrationSystem(objective) {
    logger.info(
      `💻 [FullStackIntegrationStrategistAgent] Analyzing FullStack Integration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Integration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackIntegrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackIntegrationStrategistAgent = Object.freeze(
  new FullStackIntegrationStrategistAgent(),
);
