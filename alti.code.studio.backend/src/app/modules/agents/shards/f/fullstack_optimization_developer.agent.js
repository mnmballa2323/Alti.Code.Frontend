...
class FullStackOptimizationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackOptimizationSystem(objective) {
    logger.info(
      `💻 [FullStackOptimizationDeveloperAgent] Analyzing FullStack Optimization Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Optimization Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackOptimizationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackOptimizationDeveloperAgent = Object.freeze(
  new FullStackOptimizationDeveloperAgent(),
);
