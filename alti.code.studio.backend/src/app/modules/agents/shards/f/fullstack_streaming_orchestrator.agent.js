...
class FullStackStreamingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackStreamingSystem(objective) {
    logger.info(
      `💻 [FullStackStreamingOrchestratorAgent] Analyzing FullStack Streaming Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Streaming Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackStreamingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackStreamingOrchestratorAgent = Object.freeze(
  new FullStackStreamingOrchestratorAgent(),
);
