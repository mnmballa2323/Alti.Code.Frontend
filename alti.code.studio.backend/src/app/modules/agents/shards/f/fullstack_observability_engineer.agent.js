...
class FullStackObservabilityEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackObservabilitySystem(objective) {
    logger.info(
      `💻 [FullStackObservabilityEngineerAgent] Analyzing FullStack Observability Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Observability Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackObservabilityEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackObservabilityEngineerAgent = Object.freeze(
  new FullStackObservabilityEngineerAgent(),
);
