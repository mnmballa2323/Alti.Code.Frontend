...
class FullStackOrchestrationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackOrchestrationSystem(objective) {
    logger.info(
      `💻 [FullStackOrchestrationManagerAgent] Analyzing FullStack Orchestration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Orchestration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackOrchestrationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackOrchestrationManagerAgent = Object.freeze(
  new FullStackOrchestrationManagerAgent(),
);
