...
class FusionTokamakOperatorAgent extends GeminiCliBaseAgent {
  ...
  async generateFusionSystem(fusionObjective) {
    logger.info(
      `⚛️ [FusionTokamakOperator] Analyzing objective for plasma confinement and MHD instability...`,
    );
...
${fusionObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [FusionTokamakOperator] Fusion architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [FusionTokamakOperator] Failed to generate fusion system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const fusionTokamakOperatorAgent = Object.freeze(
  new FusionTokamakOperatorAgent(),
);
