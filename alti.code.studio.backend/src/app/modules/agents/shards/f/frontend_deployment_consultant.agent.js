...
class FrontendDeploymentConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendDeploymentSystem(objective) {
        logger.info(`💻 [FrontendDeploymentConsultantAgent] Analyzing Frontend Deployment Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Deployment Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendDeploymentConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendDeploymentConsultantAgent = Object.freeze(new FrontendDeploymentConsultantAgent());
