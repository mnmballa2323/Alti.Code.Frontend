...
class FullStackArchitectureEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackArchitectureSystem(objective) {
    logger.info(
      `💻 [FullStackArchitectureEngineerAgent] Analyzing FullStack Architecture Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Architecture Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackArchitectureEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackArchitectureEngineerAgent = Object.freeze(
  new FullStackArchitectureEngineerAgent(),
);
