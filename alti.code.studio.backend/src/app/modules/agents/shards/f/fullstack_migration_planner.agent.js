...
class FullStackMigrationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackMigrationSystem(objective) {
    logger.info(
      `💻 [FullStackMigrationPlannerAgent] Analyzing FullStack Migration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Migration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackMigrationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackMigrationPlannerAgent = Object.freeze(
  new FullStackMigrationPlannerAgent(),
);
