...
class FullStackObservabilityAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackObservabilitySystem(objective) {
    logger.info(
      `💻 [FullStackObservabilityAuditorAgent] Analyzing FullStack Observability Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Observability Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackObservabilityAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackObservabilityAuditorAgent = Object.freeze(
  new FullStackObservabilityAuditorAgent(),
);
