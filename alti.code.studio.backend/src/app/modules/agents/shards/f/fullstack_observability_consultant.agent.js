...
class FullStackObservabilityConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackObservabilitySystem(objective) {
        logger.info(`💻 [FullStackObservabilityConsultantAgent] Analyzing FullStack Observability Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Observability Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackObservabilityConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackObservabilityConsultantAgent = Object.freeze(new FullStackObservabilityConsultantAgent());
