...
class FrontendAnalyticsDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendAnalyticsSystem(objective) {
    logger.info(
      `💻 [FrontendAnalyticsDeveloperAgent] Analyzing Frontend Analytics Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Analytics Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendAnalyticsDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendAnalyticsDeveloperAgent = Object.freeze(
  new FrontendAnalyticsDeveloperAgent(),
);
