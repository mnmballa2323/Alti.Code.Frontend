...
class FrontendTelemetrySpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendTelemetrySystem(objective) {
        logger.info(`💻 [FrontendTelemetrySpecialistAgent] Analyzing Frontend Telemetry Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Telemetry Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendTelemetrySpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendTelemetrySpecialistAgent = Object.freeze(new FrontendTelemetrySpecialistAgent());
