...
class FullStackServerlessAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackServerlessSystem(objective) {
        logger.info(`💻 [FullStackServerlessAuditorAgent] Analyzing FullStack Serverless Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Serverless Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackServerlessAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackServerlessAuditorAgent = Object.freeze(new FullStackServerlessAuditorAgent());
