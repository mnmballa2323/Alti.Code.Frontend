...
class FrontendEngineeringConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendEngineeringSystem(objective) {
        logger.info(`💻 [FrontendEngineeringConsultantAgent] Analyzing Frontend Engineering Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Engineering Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendEngineeringConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendEngineeringConsultantAgent = Object.freeze(new FrontendEngineeringConsultantAgent());
