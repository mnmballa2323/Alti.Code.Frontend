...
class FrontendSecurityConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendSecuritySystem(objective) {
    logger.info(
      `💻 [FrontendSecurityConsultantAgent] Analyzing Frontend Security Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Security Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendSecurityConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendSecurityConsultantAgent = Object.freeze(
  new FrontendSecurityConsultantAgent(),
);
