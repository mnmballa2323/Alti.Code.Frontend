...
class FrontendSecurityStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendSecuritySystem(objective) {
    logger.info(
      `💻 [FrontendSecurityStrategistAgent] Analyzing Frontend Security Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Security Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendSecurityStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendSecurityStrategistAgent = Object.freeze(
  new FrontendSecurityStrategistAgent(),
);
