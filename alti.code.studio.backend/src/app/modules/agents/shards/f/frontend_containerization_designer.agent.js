...
class FrontendContainerizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendContainerizationSystem(objective) {
    logger.info(
      `💻 [FrontendContainerizationDesignerAgent] Analyzing Frontend Containerization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Containerization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendContainerizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendContainerizationDesignerAgent = Object.freeze(
  new FrontendContainerizationDesignerAgent(),
);
