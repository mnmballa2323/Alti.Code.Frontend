...
class FullStackObservabilityDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackObservabilitySystem(objective) {
        logger.info(`💻 [FullStackObservabilityDeveloperAgent] Analyzing FullStack Observability Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Observability Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackObservabilityDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackObservabilityDeveloperAgent = Object.freeze(new FullStackObservabilityDeveloperAgent());
