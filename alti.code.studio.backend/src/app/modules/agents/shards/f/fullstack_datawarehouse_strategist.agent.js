...
class FullStackDataWarehouseStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackDataWarehouseSystem(objective) {
        logger.info(`💻 [FullStackDataWarehouseStrategistAgent] Analyzing FullStack DataWarehouse Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack DataWarehouse Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackDataWarehouseStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackDataWarehouseStrategistAgent = Object.freeze(new FullStackDataWarehouseStrategistAgent());
