...
class FullStackStreamingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackStreamingSystem(objective) {
    logger.info(
      `💻 [FullStackStreamingPlannerAgent] Analyzing FullStack Streaming Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Streaming Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackStreamingPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackStreamingPlannerAgent = Object.freeze(
  new FullStackStreamingPlannerAgent(),
);
