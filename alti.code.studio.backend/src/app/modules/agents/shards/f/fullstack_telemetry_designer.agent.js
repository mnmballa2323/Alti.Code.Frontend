...
class FullStackTelemetryDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackTelemetrySystem(objective) {
        logger.info(`💻 [FullStackTelemetryDesignerAgent] Analyzing FullStack Telemetry Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Telemetry Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackTelemetryDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackTelemetryDesignerAgent = Object.freeze(new FullStackTelemetryDesignerAgent());
