...
class FullStackDataLakeStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDataLakeSystem(objective) {
    logger.info(
      `💻 [FullStackDataLakeStrategistAgent] Analyzing FullStack DataLake Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack DataLake Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDataLakeStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDataLakeStrategistAgent = Object.freeze(
  new FullStackDataLakeStrategistAgent(),
);
