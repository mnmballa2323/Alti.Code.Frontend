...
class FrontendDeploymentAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendDeploymentSystem(objective) {
    logger.info(
      `💻 [FrontendDeploymentAnalystAgent] Analyzing Frontend Deployment Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Deployment Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendDeploymentAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendDeploymentAnalystAgent = Object.freeze(
  new FrontendDeploymentAnalystAgent(),
);
