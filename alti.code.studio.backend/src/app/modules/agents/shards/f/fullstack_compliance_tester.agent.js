...
class FullStackComplianceTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackComplianceSystem(objective) {
    logger.info(
      `💻 [FullStackComplianceTesterAgent] Analyzing FullStack Compliance Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Compliance Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackComplianceTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackComplianceTesterAgent = Object.freeze(
  new FullStackComplianceTesterAgent(),
);
