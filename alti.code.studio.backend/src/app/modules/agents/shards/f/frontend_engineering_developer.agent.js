...
class FrontendEngineeringDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendEngineeringSystem(objective) {
        logger.info(`💻 [FrontendEngineeringDeveloperAgent] Analyzing Frontend Engineering Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Engineering Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendEngineeringDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendEngineeringDeveloperAgent = Object.freeze(new FrontendEngineeringDeveloperAgent());
