...
class FrontendObservabilitySpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendObservabilitySystem(objective) {
        logger.info(`💻 [FrontendObservabilitySpecialistAgent] Analyzing Frontend Observability Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Observability Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendObservabilitySpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendObservabilitySpecialistAgent = Object.freeze(new FrontendObservabilitySpecialistAgent());
