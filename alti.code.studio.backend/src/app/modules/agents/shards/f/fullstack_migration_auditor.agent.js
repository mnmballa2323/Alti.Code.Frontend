...
class FullStackMigrationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackMigrationSystem(objective) {
    logger.info(
      `💻 [FullStackMigrationAuditorAgent] Analyzing FullStack Migration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Migration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackMigrationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackMigrationAuditorAgent = Object.freeze(
  new FullStackMigrationAuditorAgent(),
);
