...
class FrontendEngineeringAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendEngineeringSystem(objective) {
    logger.info(
      `💻 [FrontendEngineeringAuditorAgent] Analyzing Frontend Engineering Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Engineering Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendEngineeringAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendEngineeringAuditorAgent = Object.freeze(
  new FrontendEngineeringAuditorAgent(),
);
