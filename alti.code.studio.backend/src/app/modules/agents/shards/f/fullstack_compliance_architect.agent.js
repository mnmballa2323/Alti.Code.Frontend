...
class FullStackComplianceArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackComplianceSystem(objective) {
    logger.info(
      `💻 [FullStackComplianceArchitectAgent] Analyzing FullStack Compliance Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Compliance Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackComplianceArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackComplianceArchitectAgent = Object.freeze(
  new FullStackComplianceArchitectAgent(),
);
