...
class FullStackAnalyticsDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackAnalyticsSystem(objective) {
        logger.info(`💻 [FullStackAnalyticsDesignerAgent] Analyzing FullStack Analytics Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Analytics Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackAnalyticsDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackAnalyticsDesignerAgent = Object.freeze(new FullStackAnalyticsDesignerAgent());
