...
class FrontendComplianceDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendComplianceSystem(objective) {
    logger.info(
      `💻 [FrontendComplianceDeveloperAgent] Analyzing Frontend Compliance Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Compliance Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendComplianceDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendComplianceDeveloperAgent = Object.freeze(
  new FrontendComplianceDeveloperAgent(),
);
