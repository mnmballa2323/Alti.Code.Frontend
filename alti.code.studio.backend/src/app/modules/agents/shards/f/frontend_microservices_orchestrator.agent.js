...
class FrontendMicroservicesOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendMicroservicesSystem(objective) {
    logger.info(
      `💻 [FrontendMicroservicesOrchestratorAgent] Analyzing Frontend Microservices Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Microservices Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendMicroservicesOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendMicroservicesOrchestratorAgent = Object.freeze(
  new FrontendMicroservicesOrchestratorAgent(),
);
