...
class FrontendObservabilityAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendObservabilitySystem(objective) {
    logger.info(
      `💻 [FrontendObservabilityAnalystAgent] Analyzing Frontend Observability Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Observability Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendObservabilityAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendObservabilityAnalystAgent = Object.freeze(
  new FrontendObservabilityAnalystAgent(),
);
