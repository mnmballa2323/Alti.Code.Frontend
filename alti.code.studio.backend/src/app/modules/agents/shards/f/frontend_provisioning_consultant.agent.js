...
class FrontendProvisioningConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendProvisioningSystem(objective) {
        logger.info(`💻 [FrontendProvisioningConsultantAgent] Analyzing Frontend Provisioning Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Provisioning Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendProvisioningConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendProvisioningConsultantAgent = Object.freeze(new FrontendProvisioningConsultantAgent());
