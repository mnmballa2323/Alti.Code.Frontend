...
class FrontendScalingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendScalingSystem(objective) {
        logger.info(`💻 [FrontendScalingConsultantAgent] Analyzing Frontend Scaling Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Scaling Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendScalingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendScalingConsultantAgent = Object.freeze(new FrontendScalingConsultantAgent());
