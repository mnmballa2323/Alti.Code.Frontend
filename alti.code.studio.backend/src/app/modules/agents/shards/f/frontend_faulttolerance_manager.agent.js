...
class FrontendFaultToleranceManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendFaultToleranceSystem(objective) {
    logger.info(
      `💻 [FrontendFaultToleranceManagerAgent] Analyzing Frontend FaultTolerance Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend FaultTolerance Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendFaultToleranceManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendFaultToleranceManagerAgent = Object.freeze(
  new FrontendFaultToleranceManagerAgent(),
);
