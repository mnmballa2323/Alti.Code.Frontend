...
class FrontendTelemetryTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendTelemetrySystem(objective) {
    logger.info(
      `💻 [FrontendTelemetryTesterAgent] Analyzing Frontend Telemetry Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Telemetry Tester Target',
      );
...
export const frontendTelemetryTesterAgent = Object.freeze(
  new FrontendTelemetryTesterAgent(),
);
