...
class FullStackCachingDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackCachingSystem(objective) {
        logger.info(`💻 [FullStackCachingDeveloperAgent] Analyzing FullStack Caching Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Caching Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackCachingDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackCachingDeveloperAgent = Object.freeze(new FullStackCachingDeveloperAgent());
