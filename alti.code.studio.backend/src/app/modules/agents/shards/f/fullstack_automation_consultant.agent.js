...
class FullStackAutomationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackAutomationSystem(objective) {
        logger.info(`💻 [FullStackAutomationConsultantAgent] Analyzing FullStack Automation Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Automation Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackAutomationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackAutomationConsultantAgent = Object.freeze(new FullStackAutomationConsultantAgent());
