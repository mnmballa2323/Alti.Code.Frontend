...
class FullStackDataLakeDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackDataLakeSystem(objective) {
        logger.info(`💻 [FullStackDataLakeDesignerAgent] Analyzing FullStack DataLake Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack DataLake Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackDataLakeDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackDataLakeDesignerAgent = Object.freeze(new FullStackDataLakeDesignerAgent());
