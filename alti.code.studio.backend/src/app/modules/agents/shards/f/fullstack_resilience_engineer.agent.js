...
class FullStackResilienceEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackResilienceSystem(objective) {
        logger.info(`💻 [FullStackResilienceEngineerAgent] Analyzing FullStack Resilience Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Resilience Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackResilienceEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackResilienceEngineerAgent = Object.freeze(new FullStackResilienceEngineerAgent());
