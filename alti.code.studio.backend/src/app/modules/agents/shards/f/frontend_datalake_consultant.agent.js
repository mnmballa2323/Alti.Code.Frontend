...
class FrontendDataLakeConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendDataLakeSystem(objective) {
        logger.info(`💻 [FrontendDataLakeConsultantAgent] Analyzing Frontend DataLake Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend DataLake Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendDataLakeConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendDataLakeConsultantAgent = Object.freeze(new FrontendDataLakeConsultantAgent());
