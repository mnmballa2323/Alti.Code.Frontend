...
class FullStackAnalyticsStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackAnalyticsSystem(objective) {
        logger.info(`💻 [FullStackAnalyticsStrategistAgent] Analyzing FullStack Analytics Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Analytics Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackAnalyticsStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackAnalyticsStrategistAgent = Object.freeze(new FullStackAnalyticsStrategistAgent());
