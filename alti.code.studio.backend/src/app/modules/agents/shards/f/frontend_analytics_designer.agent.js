...
class FrontendAnalyticsDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendAnalyticsSystem(objective) {
    logger.info(
      `💻 [FrontendAnalyticsDesignerAgent] Analyzing Frontend Analytics Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Analytics Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendAnalyticsDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendAnalyticsDesignerAgent = Object.freeze(
  new FrontendAnalyticsDesignerAgent(),
);
