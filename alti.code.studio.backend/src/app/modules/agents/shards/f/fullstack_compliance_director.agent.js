...
class FullStackComplianceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackComplianceSystem(objective) {
    logger.info(
      `💻 [FullStackComplianceDirectorAgent] Analyzing FullStack Compliance Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Compliance Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackComplianceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackComplianceDirectorAgent = Object.freeze(
  new FullStackComplianceDirectorAgent(),
);
