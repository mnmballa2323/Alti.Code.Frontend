...
class FinOpsArchitectAgent extends GeminiCliBaseAgent {
  ...
  async optimizeCloudCosts(iacSource) {
    logger.info(
      `💸 [FinOpsArchitect] Scanning infrastructure code for Google Cloud billing waste...`,
    );
...
${iacSource}
    ...
    try {
      ...
      const cleanIac = output
        .replace(/```terraform|```yaml|```bash|```sh|```/gi, '')
        .trim();
      logger.info(
        `✅ [FinOpsArchitect] Infrastructure mathematically optimized for maximum cost savings.`,
      );
      return cleanIac;
    } catch (err) {
      logger.error(
        `❌ [FinOpsArchitect] Failed to optimize infrastructure costs: ${err.message}`,
      );
      throw err;
    }
  }
