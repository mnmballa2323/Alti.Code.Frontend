...
class FullStackComplianceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackComplianceSystem(objective) {
    logger.info(
      `💻 [FullStackComplianceConsultantAgent] Analyzing FullStack Compliance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Compliance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackComplianceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackComplianceConsultantAgent = Object.freeze(
  new FullStackComplianceConsultantAgent(),
);
