...
class FullStackDataLakeConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDataLakeSystem(objective) {
    logger.info(
      `💻 [FullStackDataLakeConsultantAgent] Analyzing FullStack DataLake Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack DataLake Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDataLakeConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDataLakeConsultantAgent = Object.freeze(
  new FullStackDataLakeConsultantAgent(),
);
