...
class FullStackComplianceManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackComplianceSystem(objective) {
    logger.info(
      `💻 [FullStackComplianceManagerAgent] Analyzing FullStack Compliance Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Compliance Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackComplianceManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackComplianceManagerAgent = Object.freeze(
  new FullStackComplianceManagerAgent(),
);
