...
class FullStackCachingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackCachingSystem(objective) {
    logger.info(
      `💻 [FullStackCachingOrchestratorAgent] Analyzing FullStack Caching Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Caching Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackCachingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackCachingOrchestratorAgent = Object.freeze(
  new FullStackCachingOrchestratorAgent(),
);
