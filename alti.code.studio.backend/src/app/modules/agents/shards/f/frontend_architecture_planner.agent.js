...
class FrontendArchitecturePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendArchitectureSystem(objective) {
    logger.info(
      `💻 [FrontendArchitecturePlannerAgent] Analyzing Frontend Architecture Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Architecture Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendArchitecturePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendArchitecturePlannerAgent = Object.freeze(
  new FrontendArchitecturePlannerAgent(),
);
