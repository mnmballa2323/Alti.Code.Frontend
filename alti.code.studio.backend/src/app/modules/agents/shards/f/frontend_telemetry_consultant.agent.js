...
class FrontendTelemetryConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendTelemetrySystem(objective) {
    logger.info(
      `💻 [FrontendTelemetryConsultantAgent] Analyzing Frontend Telemetry Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Telemetry Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendTelemetryConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendTelemetryConsultantAgent = Object.freeze(
  new FrontendTelemetryConsultantAgent(),
);
