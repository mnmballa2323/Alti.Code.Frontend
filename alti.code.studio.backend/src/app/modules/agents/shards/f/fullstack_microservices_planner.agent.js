...
class FullStackMicroservicesPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackMicroservicesSystem(objective) {
        logger.info(`💻 [FullStackMicroservicesPlannerAgent] Analyzing FullStack Microservices Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Microservices Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackMicroservicesPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackMicroservicesPlannerAgent = Object.freeze(new FullStackMicroservicesPlannerAgent());
