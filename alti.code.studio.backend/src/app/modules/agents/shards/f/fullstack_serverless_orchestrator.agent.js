...
class FullStackServerlessOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackServerlessSystem(objective) {
    logger.info(
      `💻 [FullStackServerlessOrchestratorAgent] Analyzing FullStack Serverless Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Serverless Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackServerlessOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackServerlessOrchestratorAgent = Object.freeze(
  new FullStackServerlessOrchestratorAgent(),
);
