...
class FullStackMigrationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackMigrationSystem(objective) {
        logger.info(`💻 [FullStackMigrationSpecialistAgent] Analyzing FullStack Migration Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Migration Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackMigrationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackMigrationSpecialistAgent = Object.freeze(new FullStackMigrationSpecialistAgent());
