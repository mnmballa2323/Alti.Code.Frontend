...
class FrontendFaultToleranceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendFaultToleranceSystem(objective) {
    logger.info(
      `💻 [FrontendFaultToleranceDesignerAgent] Analyzing Frontend FaultTolerance Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend FaultTolerance Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendFaultToleranceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendFaultToleranceDesignerAgent = Object.freeze(
  new FrontendFaultToleranceDesignerAgent(),
);
