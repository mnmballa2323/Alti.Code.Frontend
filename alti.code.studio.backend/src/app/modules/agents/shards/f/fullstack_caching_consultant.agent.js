...
class FullStackCachingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackCachingSystem(objective) {
    logger.info(
      `💻 [FullStackCachingConsultantAgent] Analyzing FullStack Caching Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Caching Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackCachingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackCachingConsultantAgent = Object.freeze(
  new FullStackCachingConsultantAgent(),
);
