...
class FrontendContainerizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendContainerizationSystem(objective) {
        logger.info(`💻 [FrontendContainerizationSpecialistAgent] Analyzing Frontend Containerization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Containerization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendContainerizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendContainerizationSpecialistAgent = Object.freeze(new FrontendContainerizationSpecialistAgent());
