...
class FrontendAutomationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendAutomationSystem(objective) {
    logger.info(
      `💻 [FrontendAutomationAnalystAgent] Analyzing Frontend Automation Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Automation Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendAutomationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendAutomationAnalystAgent = Object.freeze(
  new FrontendAutomationAnalystAgent(),
);
