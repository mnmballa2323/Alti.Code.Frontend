...
class FrontendAutomationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendAutomationSystem(objective) {
    logger.info(
      `💻 [FrontendAutomationDeveloperAgent] Analyzing Frontend Automation Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Automation Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendAutomationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendAutomationDeveloperAgent = Object.freeze(
  new FrontendAutomationDeveloperAgent(),
);
