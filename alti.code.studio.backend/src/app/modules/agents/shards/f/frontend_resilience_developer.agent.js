...
class FrontendResilienceDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendResilienceSystem(objective) {
    logger.info(
      `💻 [FrontendResilienceDeveloperAgent] Analyzing Frontend Resilience Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Resilience Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendResilienceDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendResilienceDeveloperAgent = Object.freeze(
  new FrontendResilienceDeveloperAgent(),
);
