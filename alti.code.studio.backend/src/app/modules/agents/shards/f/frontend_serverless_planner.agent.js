...
class FrontendServerlessPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendServerlessSystem(objective) {
        logger.info(`💻 [FrontendServerlessPlannerAgent] Analyzing Frontend Serverless Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Serverless Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendServerlessPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendServerlessPlannerAgent = Object.freeze(new FrontendServerlessPlannerAgent());
