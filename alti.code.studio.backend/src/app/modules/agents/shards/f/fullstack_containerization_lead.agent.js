...
class FullStackContainerizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackContainerizationSystem(objective) {
    logger.info(
      `💻 [FullStackContainerizationLeadAgent] Analyzing FullStack Containerization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Containerization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackContainerizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackContainerizationLeadAgent = Object.freeze(
  new FullStackContainerizationLeadAgent(),
);
