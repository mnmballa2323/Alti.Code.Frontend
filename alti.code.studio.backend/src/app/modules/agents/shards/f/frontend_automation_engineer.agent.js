...
class FrontendAutomationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendAutomationSystem(objective) {
        logger.info(`💻 [FrontendAutomationEngineerAgent] Analyzing Frontend Automation Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Automation Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendAutomationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendAutomationEngineerAgent = Object.freeze(new FrontendAutomationEngineerAgent());
