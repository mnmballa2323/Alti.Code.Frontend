...
class FullStackEngineeringPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackEngineeringSystem(objective) {
    logger.info(
      `💻 [FullStackEngineeringPlannerAgent] Analyzing FullStack Engineering Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Engineering Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackEngineeringPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackEngineeringPlannerAgent = Object.freeze(
  new FullStackEngineeringPlannerAgent(),
);
