...
class FrontendArchitectureStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendArchitectureSystem(objective) {
        logger.info(`💻 [FrontendArchitectureStrategistAgent] Analyzing Frontend Architecture Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Architecture Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendArchitectureStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendArchitectureStrategistAgent = Object.freeze(new FrontendArchitectureStrategistAgent());
