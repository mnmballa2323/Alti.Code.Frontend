...
class FullStackCachingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackCachingSystem(objective) {
    logger.info(
      `💻 [FullStackCachingSpecialistAgent] Analyzing FullStack Caching Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Caching Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackCachingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackCachingSpecialistAgent = Object.freeze(
  new FullStackCachingSpecialistAgent(),
);
