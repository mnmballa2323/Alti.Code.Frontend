...
class FullStackLoadBalancingLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackLoadBalancingSystem(objective) {
        logger.info(`💻 [FullStackLoadBalancingLeadAgent] Analyzing FullStack LoadBalancing Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack LoadBalancing Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackLoadBalancingLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackLoadBalancingLeadAgent = Object.freeze(new FullStackLoadBalancingLeadAgent());
