...
class FullStackDeploymentDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDeploymentSystem(objective) {
    logger.info(
      `💻 [FullStackDeploymentDirectorAgent] Analyzing FullStack Deployment Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Deployment Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDeploymentDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDeploymentDirectorAgent = Object.freeze(
  new FullStackDeploymentDirectorAgent(),
);
