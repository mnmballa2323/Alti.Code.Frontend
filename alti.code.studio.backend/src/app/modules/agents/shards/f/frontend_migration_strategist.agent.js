...
class FrontendMigrationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendMigrationSystem(objective) {
        logger.info(`💻 [FrontendMigrationStrategistAgent] Analyzing Frontend Migration Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Migration Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendMigrationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendMigrationStrategistAgent = Object.freeze(new FrontendMigrationStrategistAgent());
