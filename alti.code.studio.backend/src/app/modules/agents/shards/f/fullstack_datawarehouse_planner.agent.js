...
class FullStackDataWarehousePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackDataWarehouseSystem(objective) {
        logger.info(`💻 [FullStackDataWarehousePlannerAgent] Analyzing FullStack DataWarehouse Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack DataWarehouse Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackDataWarehousePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackDataWarehousePlannerAgent = Object.freeze(new FullStackDataWarehousePlannerAgent());
