...
class FrontendAnalyticsTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendAnalyticsSystem(objective) {
    logger.info(
      `💻 [FrontendAnalyticsTesterAgent] Analyzing Frontend Analytics Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Analytics Tester Target',
      );
...
export const frontendAnalyticsTesterAgent = Object.freeze(
  new FrontendAnalyticsTesterAgent(),
);
