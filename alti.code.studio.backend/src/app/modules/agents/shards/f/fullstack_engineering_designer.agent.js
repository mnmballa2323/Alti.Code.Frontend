...
class FullStackEngineeringDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackEngineeringSystem(objective) {
        logger.info(`💻 [FullStackEngineeringDesignerAgent] Analyzing FullStack Engineering Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Engineering Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackEngineeringDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackEngineeringDesignerAgent = Object.freeze(new FullStackEngineeringDesignerAgent());
