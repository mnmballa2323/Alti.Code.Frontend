...
class FrontendResilienceSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendResilienceSystem(objective) {
        logger.info(`💻 [FrontendResilienceSpecialistAgent] Analyzing Frontend Resilience Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Resilience Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendResilienceSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendResilienceSpecialistAgent = Object.freeze(new FrontendResilienceSpecialistAgent());
