...
class FullStackResilienceStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackResilienceSystem(objective) {
    logger.info(
      `💻 [FullStackResilienceStrategistAgent] Analyzing FullStack Resilience Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Resilience Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackResilienceStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackResilienceStrategistAgent = Object.freeze(
  new FullStackResilienceStrategistAgent(),
);
