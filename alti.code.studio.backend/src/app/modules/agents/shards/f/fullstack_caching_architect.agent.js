...
class FullStackCachingArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackCachingSystem(objective) {
    logger.info(
      `💻 [FullStackCachingArchitectAgent] Analyzing FullStack Caching Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Caching Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackCachingArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackCachingArchitectAgent = Object.freeze(
  new FullStackCachingArchitectAgent(),
);
