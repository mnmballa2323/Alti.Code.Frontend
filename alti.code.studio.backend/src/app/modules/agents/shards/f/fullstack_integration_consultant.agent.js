...
class FullStackIntegrationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackIntegrationSystem(objective) {
        logger.info(`💻 [FullStackIntegrationConsultantAgent] Analyzing FullStack Integration Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Integration Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackIntegrationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackIntegrationConsultantAgent = Object.freeze(new FullStackIntegrationConsultantAgent());
