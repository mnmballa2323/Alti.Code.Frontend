...
class FullStackFaultToleranceEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackFaultToleranceSystem(objective) {
        logger.info(`💻 [FullStackFaultToleranceEngineerAgent] Analyzing FullStack FaultTolerance Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack FaultTolerance Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackFaultToleranceEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackFaultToleranceEngineerAgent = Object.freeze(new FullStackFaultToleranceEngineerAgent());
