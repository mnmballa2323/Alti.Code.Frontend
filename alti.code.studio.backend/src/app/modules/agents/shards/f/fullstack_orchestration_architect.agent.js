...
class FullStackOrchestrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackOrchestrationSystem(objective) {
    logger.info(
      `💻 [FullStackOrchestrationArchitectAgent] Analyzing FullStack Orchestration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Orchestration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackOrchestrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackOrchestrationArchitectAgent = Object.freeze(
  new FullStackOrchestrationArchitectAgent(),
);
