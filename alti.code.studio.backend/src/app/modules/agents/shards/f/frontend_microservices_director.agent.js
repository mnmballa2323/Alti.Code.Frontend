...
class FrontendMicroservicesDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendMicroservicesSystem(objective) {
        logger.info(`💻 [FrontendMicroservicesDirectorAgent] Analyzing Frontend Microservices Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Microservices Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendMicroservicesDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendMicroservicesDirectorAgent = Object.freeze(new FrontendMicroservicesDirectorAgent());
