...
class FrontendGovernanceDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendGovernanceSystem(objective) {
        logger.info(`💻 [FrontendGovernanceDesignerAgent] Analyzing Frontend Governance Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Governance Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendGovernanceDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendGovernanceDesignerAgent = Object.freeze(new FrontendGovernanceDesignerAgent());
