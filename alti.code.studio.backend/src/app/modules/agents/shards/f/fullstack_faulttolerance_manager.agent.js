...
class FullStackFaultToleranceManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackFaultToleranceSystem(objective) {
        logger.info(`💻 [FullStackFaultToleranceManagerAgent] Analyzing FullStack FaultTolerance Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack FaultTolerance Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackFaultToleranceManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackFaultToleranceManagerAgent = Object.freeze(new FullStackFaultToleranceManagerAgent());
