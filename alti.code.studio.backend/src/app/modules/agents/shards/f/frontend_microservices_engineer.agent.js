...
class FrontendMicroservicesEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendMicroservicesSystem(objective) {
    logger.info(
      `💻 [FrontendMicroservicesEngineerAgent] Analyzing Frontend Microservices Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Microservices Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendMicroservicesEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendMicroservicesEngineerAgent = Object.freeze(
  new FrontendMicroservicesEngineerAgent(),
);
