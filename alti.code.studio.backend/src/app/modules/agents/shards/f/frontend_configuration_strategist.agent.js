...
class FrontendConfigurationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendConfigurationSystem(objective) {
    logger.info(
      `💻 [FrontendConfigurationStrategistAgent] Analyzing Frontend Configuration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Configuration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendConfigurationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendConfigurationStrategistAgent = Object.freeze(
  new FrontendConfigurationStrategistAgent(),
);
