...
class FrontendDeploymentDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendDeploymentSystem(objective) {
    logger.info(
      `💻 [FrontendDeploymentDirectorAgent] Analyzing Frontend Deployment Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Deployment Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendDeploymentDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendDeploymentDirectorAgent = Object.freeze(
  new FrontendDeploymentDirectorAgent(),
);
