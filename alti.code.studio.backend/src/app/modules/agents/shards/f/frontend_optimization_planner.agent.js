...
class FrontendOptimizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendOptimizationSystem(objective) {
    logger.info(
      `💻 [FrontendOptimizationPlannerAgent] Analyzing Frontend Optimization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Optimization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendOptimizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendOptimizationPlannerAgent = Object.freeze(
  new FrontendOptimizationPlannerAgent(),
);
