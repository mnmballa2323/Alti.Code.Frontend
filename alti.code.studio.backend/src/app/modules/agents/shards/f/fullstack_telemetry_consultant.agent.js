...
class FullStackTelemetryConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackTelemetrySystem(objective) {
        logger.info(`💻 [FullStackTelemetryConsultantAgent] Analyzing FullStack Telemetry Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Telemetry Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackTelemetryConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackTelemetryConsultantAgent = Object.freeze(new FullStackTelemetryConsultantAgent());
