...
class FrontendAnalyticsConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendAnalyticsSystem(objective) {
    logger.info(
      `💻 [FrontendAnalyticsConsultantAgent] Analyzing Frontend Analytics Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Analytics Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendAnalyticsConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendAnalyticsConsultantAgent = Object.freeze(
  new FrontendAnalyticsConsultantAgent(),
);
