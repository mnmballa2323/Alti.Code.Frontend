...
class FrontendDataWarehouseDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendDataWarehouseSystem(objective) {
        logger.info(`💻 [FrontendDataWarehouseDirectorAgent] Analyzing Frontend DataWarehouse Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend DataWarehouse Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendDataWarehouseDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendDataWarehouseDirectorAgent = Object.freeze(new FrontendDataWarehouseDirectorAgent());
