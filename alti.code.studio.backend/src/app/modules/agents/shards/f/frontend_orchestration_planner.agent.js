...
class FrontendOrchestrationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendOrchestrationSystem(objective) {
    logger.info(
      `💻 [FrontendOrchestrationPlannerAgent] Analyzing Frontend Orchestration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Orchestration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendOrchestrationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendOrchestrationPlannerAgent = Object.freeze(
  new FrontendOrchestrationPlannerAgent(),
);
