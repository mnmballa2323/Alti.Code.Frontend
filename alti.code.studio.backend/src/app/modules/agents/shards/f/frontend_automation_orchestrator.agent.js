...
class FrontendAutomationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendAutomationSystem(objective) {
        logger.info(`💻 [FrontendAutomationOrchestratorAgent] Analyzing Frontend Automation Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Automation Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendAutomationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendAutomationOrchestratorAgent = Object.freeze(new FrontendAutomationOrchestratorAgent());
