...
class FullStackDataWarehouseTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDataWarehouseSystem(objective) {
    logger.info(
      `💻 [FullStackDataWarehouseTesterAgent] Analyzing FullStack DataWarehouse Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack DataWarehouse Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDataWarehouseTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDataWarehouseTesterAgent = Object.freeze(
  new FullStackDataWarehouseTesterAgent(),
);
