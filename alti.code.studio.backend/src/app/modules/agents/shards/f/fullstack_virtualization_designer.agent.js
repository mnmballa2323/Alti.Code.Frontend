...
class FullStackVirtualizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackVirtualizationSystem(objective) {
    logger.info(
      `💻 [FullStackVirtualizationDesignerAgent] Analyzing FullStack Virtualization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Virtualization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackVirtualizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackVirtualizationDesignerAgent = Object.freeze(
  new FullStackVirtualizationDesignerAgent(),
);
