...
class FullStackStreamingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackStreamingSystem(objective) {
    logger.info(
      `💻 [FullStackStreamingConsultantAgent] Analyzing FullStack Streaming Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Streaming Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackStreamingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackStreamingConsultantAgent = Object.freeze(
  new FullStackStreamingConsultantAgent(),
);
