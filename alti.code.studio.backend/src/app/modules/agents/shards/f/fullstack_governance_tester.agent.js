...
class FullStackGovernanceTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackGovernanceSystem(objective) {
        logger.info(`💻 [FullStackGovernanceTesterAgent] Analyzing FullStack Governance Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Governance Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackGovernanceTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackGovernanceTesterAgent = Object.freeze(new FullStackGovernanceTesterAgent());
