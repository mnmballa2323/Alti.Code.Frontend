...
class FrontendMicroservicesDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendMicroservicesSystem(objective) {
        logger.info(`💻 [FrontendMicroservicesDesignerAgent] Analyzing Frontend Microservices Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Microservices Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendMicroservicesDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendMicroservicesDesignerAgent = Object.freeze(new FrontendMicroservicesDesignerAgent());
