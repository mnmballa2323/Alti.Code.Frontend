...
class FrontendLoadBalancingAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendLoadBalancingSystem(objective) {
        logger.info(`💻 [FrontendLoadBalancingAuditorAgent] Analyzing Frontend LoadBalancing Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend LoadBalancing Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendLoadBalancingAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendLoadBalancingAuditorAgent = Object.freeze(new FrontendLoadBalancingAuditorAgent());
