...
class FullStackGovernanceAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackGovernanceSystem(objective) {
    logger.info(
      `💻 [FullStackGovernanceAuditorAgent] Analyzing FullStack Governance Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Governance Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackGovernanceAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackGovernanceAuditorAgent = Object.freeze(
  new FullStackGovernanceAuditorAgent(),
);
