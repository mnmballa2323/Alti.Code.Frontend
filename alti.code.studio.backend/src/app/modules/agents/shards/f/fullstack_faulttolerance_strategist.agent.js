...
class FullStackFaultToleranceStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackFaultToleranceSystem(objective) {
        logger.info(`💻 [FullStackFaultToleranceStrategistAgent] Analyzing FullStack FaultTolerance Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack FaultTolerance Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackFaultToleranceStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackFaultToleranceStrategistAgent = Object.freeze(new FullStackFaultToleranceStrategistAgent());
