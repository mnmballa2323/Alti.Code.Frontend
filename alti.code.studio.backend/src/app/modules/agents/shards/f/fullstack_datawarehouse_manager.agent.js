...
class FullStackDataWarehouseManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDataWarehouseSystem(objective) {
    logger.info(
      `💻 [FullStackDataWarehouseManagerAgent] Analyzing FullStack DataWarehouse Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack DataWarehouse Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDataWarehouseManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDataWarehouseManagerAgent = Object.freeze(
  new FullStackDataWarehouseManagerAgent(),
);
