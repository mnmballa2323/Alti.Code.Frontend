...
class FullStackArchitectureTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackArchitectureSystem(objective) {
        logger.info(`💻 [FullStackArchitectureTesterAgent] Analyzing FullStack Architecture Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Architecture Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackArchitectureTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackArchitectureTesterAgent = Object.freeze(new FullStackArchitectureTesterAgent());
