...
class FrontendContainerizationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendContainerizationSystem(objective) {
    logger.info(
      `💻 [FrontendContainerizationManagerAgent] Analyzing Frontend Containerization Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Containerization Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendContainerizationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendContainerizationManagerAgent = Object.freeze(
  new FrontendContainerizationManagerAgent(),
);
