...
class FrontendDataWarehouseAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendDataWarehouseSystem(objective) {
    logger.info(
      `💻 [FrontendDataWarehouseAnalystAgent] Analyzing Frontend DataWarehouse Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend DataWarehouse Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendDataWarehouseAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendDataWarehouseAnalystAgent = Object.freeze(
  new FrontendDataWarehouseAnalystAgent(),
);
