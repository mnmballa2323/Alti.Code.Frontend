...
class FrontendProvisioningDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendProvisioningSystem(objective) {
    logger.info(
      `💻 [FrontendProvisioningDesignerAgent] Analyzing Frontend Provisioning Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Provisioning Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendProvisioningDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendProvisioningDesignerAgent = Object.freeze(
  new FrontendProvisioningDesignerAgent(),
);
