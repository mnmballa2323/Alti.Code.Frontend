...
class FrontendIntegrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendIntegrationSystem(objective) {
    logger.info(
      `💻 [FrontendIntegrationConsultantAgent] Analyzing Frontend Integration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Integration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendIntegrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendIntegrationConsultantAgent = Object.freeze(
  new FrontendIntegrationConsultantAgent(),
);
