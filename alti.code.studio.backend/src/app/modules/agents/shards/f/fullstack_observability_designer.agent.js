...
class FullStackObservabilityDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackObservabilitySystem(objective) {
        logger.info(`💻 [FullStackObservabilityDesignerAgent] Analyzing FullStack Observability Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Observability Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackObservabilityDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackObservabilityDesignerAgent = Object.freeze(new FullStackObservabilityDesignerAgent());
