...
class FullStackStreamingArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackStreamingSystem(objective) {
    logger.info(
      `💻 [FullStackStreamingArchitectAgent] Analyzing FullStack Streaming Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Streaming Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackStreamingArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackStreamingArchitectAgent = Object.freeze(
  new FullStackStreamingArchitectAgent(),
);
