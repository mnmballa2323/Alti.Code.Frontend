...
class FullStackContainerizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackContainerizationSystem(objective) {
        logger.info(`💻 [FullStackContainerizationSpecialistAgent] Analyzing FullStack Containerization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Containerization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackContainerizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackContainerizationSpecialistAgent = Object.freeze(new FullStackContainerizationSpecialistAgent());
