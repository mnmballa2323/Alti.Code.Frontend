...
class FullStackCompliancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackComplianceSystem(objective) {
    logger.info(
      `💻 [FullStackCompliancePlannerAgent] Analyzing FullStack Compliance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Compliance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackCompliancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackCompliancePlannerAgent = Object.freeze(
  new FullStackCompliancePlannerAgent(),
);
