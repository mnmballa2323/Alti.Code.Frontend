...
class FullStackAnalyticsAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackAnalyticsSystem(objective) {
    logger.info(
      `💻 [FullStackAnalyticsAnalystAgent] Analyzing FullStack Analytics Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Analytics Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackAnalyticsAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackAnalyticsAnalystAgent = Object.freeze(
  new FullStackAnalyticsAnalystAgent(),
);
