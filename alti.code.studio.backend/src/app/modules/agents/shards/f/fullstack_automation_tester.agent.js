...
class FullStackAutomationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackAutomationSystem(objective) {
    logger.info(
      `💻 [FullStackAutomationTesterAgent] Analyzing FullStack Automation Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Automation Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackAutomationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackAutomationTesterAgent = Object.freeze(
  new FullStackAutomationTesterAgent(),
);
