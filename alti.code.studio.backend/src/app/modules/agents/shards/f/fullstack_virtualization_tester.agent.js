...
class FullStackVirtualizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackVirtualizationSystem(objective) {
    logger.info(
      `💻 [FullStackVirtualizationTesterAgent] Analyzing FullStack Virtualization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Virtualization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackVirtualizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackVirtualizationTesterAgent = Object.freeze(
  new FullStackVirtualizationTesterAgent(),
);
