...
class FrontendOrchestrationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendOrchestrationSystem(objective) {
        logger.info(`💻 [FrontendOrchestrationOrchestratorAgent] Analyzing Frontend Orchestration Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Orchestration Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendOrchestrationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendOrchestrationOrchestratorAgent = Object.freeze(new FrontendOrchestrationOrchestratorAgent());
