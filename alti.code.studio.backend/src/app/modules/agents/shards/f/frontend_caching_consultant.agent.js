...
class FrontendCachingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendCachingSystem(objective) {
    logger.info(
      `💻 [FrontendCachingConsultantAgent] Analyzing Frontend Caching Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Caching Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendCachingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendCachingConsultantAgent = Object.freeze(
  new FrontendCachingConsultantAgent(),
);
