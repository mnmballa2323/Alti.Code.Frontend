...
class FrontendDeploymentDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendDeploymentSystem(objective) {
    logger.info(
      `💻 [FrontendDeploymentDesignerAgent] Analyzing Frontend Deployment Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Deployment Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendDeploymentDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendDeploymentDesignerAgent = Object.freeze(
  new FrontendDeploymentDesignerAgent(),
);
