...
class FullStackSecurityOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackSecuritySystem(objective) {
    logger.info(
      `💻 [FullStackSecurityOrchestratorAgent] Analyzing FullStack Security Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Security Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackSecurityOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackSecurityOrchestratorAgent = Object.freeze(
  new FullStackSecurityOrchestratorAgent(),
);
