...
class FullStackResilienceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackResilienceSystem(objective) {
        logger.info(`💻 [FullStackResilienceConsultantAgent] Analyzing FullStack Resilience Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Resilience Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackResilienceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackResilienceConsultantAgent = Object.freeze(new FullStackResilienceConsultantAgent());
