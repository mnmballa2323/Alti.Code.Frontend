...
class FrontendProvisioningPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendProvisioningSystem(objective) {
    logger.info(
      `💻 [FrontendProvisioningPlannerAgent] Analyzing Frontend Provisioning Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Provisioning Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendProvisioningPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendProvisioningPlannerAgent = Object.freeze(
  new FrontendProvisioningPlannerAgent(),
);
