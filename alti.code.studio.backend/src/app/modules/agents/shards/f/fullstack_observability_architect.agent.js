...
class FullStackObservabilityArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackObservabilitySystem(objective) {
        logger.info(`💻 [FullStackObservabilityArchitectAgent] Analyzing FullStack Observability Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Observability Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackObservabilityArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackObservabilityArchitectAgent = Object.freeze(new FullStackObservabilityArchitectAgent());
