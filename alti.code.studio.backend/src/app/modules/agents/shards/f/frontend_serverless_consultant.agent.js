...
class FrontendServerlessConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendServerlessSystem(objective) {
        logger.info(`💻 [FrontendServerlessConsultantAgent] Analyzing Frontend Serverless Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Serverless Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendServerlessConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendServerlessConsultantAgent = Object.freeze(new FrontendServerlessConsultantAgent());
