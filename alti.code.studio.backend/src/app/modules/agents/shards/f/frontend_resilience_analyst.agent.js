...
class FrontendResilienceAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendResilienceSystem(objective) {
    logger.info(
      `💻 [FrontendResilienceAnalystAgent] Analyzing Frontend Resilience Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Resilience Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendResilienceAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendResilienceAnalystAgent = Object.freeze(
  new FrontendResilienceAnalystAgent(),
);
