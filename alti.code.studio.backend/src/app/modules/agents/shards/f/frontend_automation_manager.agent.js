...
class FrontendAutomationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendAutomationSystem(objective) {
        logger.info(`💻 [FrontendAutomationManagerAgent] Analyzing Frontend Automation Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Automation Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendAutomationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendAutomationManagerAgent = Object.freeze(new FrontendAutomationManagerAgent());
