...
class FullStackEngineeringAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackEngineeringSystem(objective) {
    logger.info(
      `💻 [FullStackEngineeringAnalystAgent] Analyzing FullStack Engineering Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Engineering Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackEngineeringAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackEngineeringAnalystAgent = Object.freeze(
  new FullStackEngineeringAnalystAgent(),
);
