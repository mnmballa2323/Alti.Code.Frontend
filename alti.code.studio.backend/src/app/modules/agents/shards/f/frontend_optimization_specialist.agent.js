...
class FrontendOptimizationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendOptimizationSystem(objective) {
    logger.info(
      `💻 [FrontendOptimizationSpecialistAgent] Analyzing Frontend Optimization Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Optimization Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendOptimizationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendOptimizationSpecialistAgent = Object.freeze(
  new FrontendOptimizationSpecialistAgent(),
);
