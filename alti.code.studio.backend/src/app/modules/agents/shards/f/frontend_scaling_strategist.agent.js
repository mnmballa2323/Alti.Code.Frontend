...
class FrontendScalingStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendScalingSystem(objective) {
    logger.info(
      `💻 [FrontendScalingStrategistAgent] Analyzing Frontend Scaling Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Scaling Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendScalingStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendScalingStrategistAgent = Object.freeze(
  new FrontendScalingStrategistAgent(),
);
