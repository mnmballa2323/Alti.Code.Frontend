...
class FrontendResilienceArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendResilienceSystem(objective) {
    logger.info(
      `💻 [FrontendResilienceArchitectAgent] Analyzing Frontend Resilience Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Resilience Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendResilienceArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendResilienceArchitectAgent = Object.freeze(
  new FrontendResilienceArchitectAgent(),
);
