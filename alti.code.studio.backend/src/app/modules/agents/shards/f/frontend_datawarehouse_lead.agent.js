...
class FrontendDataWarehouseLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendDataWarehouseSystem(objective) {
    logger.info(
      `💻 [FrontendDataWarehouseLeadAgent] Analyzing Frontend DataWarehouse Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend DataWarehouse Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendDataWarehouseLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendDataWarehouseLeadAgent = Object.freeze(
  new FrontendDataWarehouseLeadAgent(),
);
