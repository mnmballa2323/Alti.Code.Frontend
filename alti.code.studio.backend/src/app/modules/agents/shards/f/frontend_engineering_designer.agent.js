...
class FrontendEngineeringDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendEngineeringSystem(objective) {
        logger.info(`💻 [FrontendEngineeringDesignerAgent] Analyzing Frontend Engineering Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Engineering Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendEngineeringDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendEngineeringDesignerAgent = Object.freeze(new FrontendEngineeringDesignerAgent());
