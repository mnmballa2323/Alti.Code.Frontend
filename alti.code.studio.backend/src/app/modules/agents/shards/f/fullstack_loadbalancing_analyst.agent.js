...
class FullStackLoadBalancingAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackLoadBalancingSystem(objective) {
    logger.info(
      `💻 [FullStackLoadBalancingAnalystAgent] Analyzing FullStack LoadBalancing Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack LoadBalancing Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackLoadBalancingAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackLoadBalancingAnalystAgent = Object.freeze(
  new FullStackLoadBalancingAnalystAgent(),
);
