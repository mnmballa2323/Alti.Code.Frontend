...
class FrontendTelemetryPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendTelemetrySystem(objective) {
        logger.info(`💻 [FrontendTelemetryPlannerAgent] Analyzing Frontend Telemetry Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Telemetry Planner Target");
...
export const frontendTelemetryPlannerAgent = Object.freeze(new FrontendTelemetryPlannerAgent());
