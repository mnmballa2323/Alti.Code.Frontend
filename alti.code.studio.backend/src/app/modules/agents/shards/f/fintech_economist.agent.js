...
class FintechEconomistAgent extends GeminiCliBaseAgent {
  ...
  async generateFinancialSystem(financialObjective) {
    logger.info(
      `🏦 [FintechEconomist] Analyzing objective for strict Double-Entry ledger compliance...`,
    );
...
${financialObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [FintechEconomist] Financial software architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [FintechEconomist] Failed to generate financial system: ${err.message}`,
      );
      throw err;
    }
  }
