...
class FullStackArchitectureConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackArchitectureSystem(objective) {
    logger.info(
      `💻 [FullStackArchitectureConsultantAgent] Analyzing FullStack Architecture Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Architecture Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackArchitectureConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackArchitectureConsultantAgent = Object.freeze(
  new FullStackArchitectureConsultantAgent(),
);
