...
class FullStackProvisioningOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackProvisioningSystem(objective) {
    logger.info(
      `💻 [FullStackProvisioningOrchestratorAgent] Analyzing FullStack Provisioning Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Provisioning Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackProvisioningOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackProvisioningOrchestratorAgent = Object.freeze(
  new FullStackProvisioningOrchestratorAgent(),
);
