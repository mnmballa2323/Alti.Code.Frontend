...
class FullStackIntegrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackIntegrationSystem(objective) {
    logger.info(
      `💻 [FullStackIntegrationSpecialistAgent] Analyzing FullStack Integration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Integration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackIntegrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackIntegrationSpecialistAgent = Object.freeze(
  new FullStackIntegrationSpecialistAgent(),
);
