...
class FullStackOptimizationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackOptimizationSystem(objective) {
    logger.info(
      `💻 [FullStackOptimizationOrchestratorAgent] Analyzing FullStack Optimization Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Optimization Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackOptimizationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackOptimizationOrchestratorAgent = Object.freeze(
  new FullStackOptimizationOrchestratorAgent(),
);
