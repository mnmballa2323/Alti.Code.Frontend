...
class FullStackContainerizationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackContainerizationSystem(objective) {
    logger.info(
      `💻 [FullStackContainerizationStrategistAgent] Analyzing FullStack Containerization Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Containerization Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackContainerizationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackContainerizationStrategistAgent = Object.freeze(
  new FullStackContainerizationStrategistAgent(),
);
