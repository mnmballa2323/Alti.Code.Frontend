...
class FrontendDeploymentAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendDeploymentSystem(objective) {
    logger.info(
      `💻 [FrontendDeploymentAuditorAgent] Analyzing Frontend Deployment Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Deployment Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendDeploymentAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendDeploymentAuditorAgent = Object.freeze(
  new FrontendDeploymentAuditorAgent(),
);
