...
class FrontendComplianceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendComplianceSystem(objective) {
    logger.info(
      `💻 [FrontendComplianceDesignerAgent] Analyzing Frontend Compliance Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Compliance Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendComplianceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendComplianceDesignerAgent = Object.freeze(
  new FrontendComplianceDesignerAgent(),
);
