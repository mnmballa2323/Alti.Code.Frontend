...
class FullStackOptimizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackOptimizationSystem(objective) {
    logger.info(
      `💻 [FullStackOptimizationDesignerAgent] Analyzing FullStack Optimization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Optimization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackOptimizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackOptimizationDesignerAgent = Object.freeze(
  new FullStackOptimizationDesignerAgent(),
);
