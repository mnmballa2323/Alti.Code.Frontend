...
class FrontendArchitectureArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendArchitectureSystem(objective) {
        logger.info(`💻 [FrontendArchitectureArchitectAgent] Analyzing Frontend Architecture Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Architecture Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendArchitectureArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendArchitectureArchitectAgent = Object.freeze(new FrontendArchitectureArchitectAgent());
