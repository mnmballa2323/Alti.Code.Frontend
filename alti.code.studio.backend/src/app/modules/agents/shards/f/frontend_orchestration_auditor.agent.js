...
class FrontendOrchestrationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendOrchestrationSystem(objective) {
        logger.info(`💻 [FrontendOrchestrationAuditorAgent] Analyzing Frontend Orchestration Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Orchestration Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendOrchestrationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendOrchestrationAuditorAgent = Object.freeze(new FrontendOrchestrationAuditorAgent());
