...
class FrontendResilienceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendResilienceSystem(objective) {
    logger.info(
      `💻 [FrontendResilienceDirectorAgent] Analyzing Frontend Resilience Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Resilience Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendResilienceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendResilienceDirectorAgent = Object.freeze(
  new FrontendResilienceDirectorAgent(),
);
