...
class FullStackDataWarehouseDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDataWarehouseSystem(objective) {
    logger.info(
      `💻 [FullStackDataWarehouseDesignerAgent] Analyzing FullStack DataWarehouse Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack DataWarehouse Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDataWarehouseDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDataWarehouseDesignerAgent = Object.freeze(
  new FullStackDataWarehouseDesignerAgent(),
);
