...
class FullStackFaultToleranceTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackFaultToleranceSystem(objective) {
    logger.info(
      `💻 [FullStackFaultToleranceTesterAgent] Analyzing FullStack FaultTolerance Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack FaultTolerance Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackFaultToleranceTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackFaultToleranceTesterAgent = Object.freeze(
  new FullStackFaultToleranceTesterAgent(),
);
