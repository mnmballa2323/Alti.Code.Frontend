...
class FullStackResilienceOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackResilienceSystem(objective) {
    logger.info(
      `💻 [FullStackResilienceOrchestratorAgent] Analyzing FullStack Resilience Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Resilience Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackResilienceOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackResilienceOrchestratorAgent = Object.freeze(
  new FullStackResilienceOrchestratorAgent(),
);
