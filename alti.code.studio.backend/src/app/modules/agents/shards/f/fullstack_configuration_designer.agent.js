...
class FullStackConfigurationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackConfigurationSystem(objective) {
        logger.info(`💻 [FullStackConfigurationDesignerAgent] Analyzing FullStack Configuration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Configuration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackConfigurationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackConfigurationDesignerAgent = Object.freeze(new FullStackConfigurationDesignerAgent());
