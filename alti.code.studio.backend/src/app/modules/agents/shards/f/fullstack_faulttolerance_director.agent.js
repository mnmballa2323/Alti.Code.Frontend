...
class FullStackFaultToleranceDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackFaultToleranceSystem(objective) {
        logger.info(`💻 [FullStackFaultToleranceDirectorAgent] Analyzing FullStack FaultTolerance Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack FaultTolerance Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackFaultToleranceDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackFaultToleranceDirectorAgent = Object.freeze(new FullStackFaultToleranceDirectorAgent());
