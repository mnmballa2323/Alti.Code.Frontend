...
class FullStackStreamingSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackStreamingSystem(objective) {
        logger.info(`💻 [FullStackStreamingSpecialistAgent] Analyzing FullStack Streaming Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Streaming Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackStreamingSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackStreamingSpecialistAgent = Object.freeze(new FullStackStreamingSpecialistAgent());
