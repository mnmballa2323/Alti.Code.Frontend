...
class FullStackLoadBalancingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackLoadBalancingSystem(objective) {
    logger.info(
      `💻 [FullStackLoadBalancingPlannerAgent] Analyzing FullStack LoadBalancing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack LoadBalancing Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackLoadBalancingPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackLoadBalancingPlannerAgent = Object.freeze(
  new FullStackLoadBalancingPlannerAgent(),
);
