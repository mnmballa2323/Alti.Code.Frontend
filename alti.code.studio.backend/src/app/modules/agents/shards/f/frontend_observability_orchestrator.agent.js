...
class FrontendObservabilityOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendObservabilitySystem(objective) {
    logger.info(
      `💻 [FrontendObservabilityOrchestratorAgent] Analyzing Frontend Observability Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Observability Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendObservabilityOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendObservabilityOrchestratorAgent = Object.freeze(
  new FrontendObservabilityOrchestratorAgent(),
);
