...
class FrontendIntegrationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendIntegrationSystem(objective) {
        logger.info(`💻 [FrontendIntegrationTesterAgent] Analyzing Frontend Integration Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Integration Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendIntegrationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendIntegrationTesterAgent = Object.freeze(new FrontendIntegrationTesterAgent());
