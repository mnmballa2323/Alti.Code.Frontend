...
class FullStackLoadBalancingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackLoadBalancingSystem(objective) {
    logger.info(
      `💻 [FullStackLoadBalancingOrchestratorAgent] Analyzing FullStack LoadBalancing Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack LoadBalancing Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackLoadBalancingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackLoadBalancingOrchestratorAgent = Object.freeze(
  new FullStackLoadBalancingOrchestratorAgent(),
);
