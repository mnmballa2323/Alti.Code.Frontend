...
class FullStackOptimizationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackOptimizationSystem(objective) {
        logger.info(`💻 [FullStackOptimizationArchitectAgent] Analyzing FullStack Optimization Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Optimization Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackOptimizationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackOptimizationArchitectAgent = Object.freeze(new FullStackOptimizationArchitectAgent());
