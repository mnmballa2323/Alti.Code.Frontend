...
class FrontendObservabilityEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendObservabilitySystem(objective) {
    logger.info(
      `💻 [FrontendObservabilityEngineerAgent] Analyzing Frontend Observability Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Observability Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendObservabilityEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendObservabilityEngineerAgent = Object.freeze(
  new FrontendObservabilityEngineerAgent(),
);
