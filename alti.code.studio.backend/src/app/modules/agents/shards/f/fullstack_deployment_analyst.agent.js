...
class FullStackDeploymentAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDeploymentSystem(objective) {
    logger.info(
      `💻 [FullStackDeploymentAnalystAgent] Analyzing FullStack Deployment Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Deployment Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDeploymentAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDeploymentAnalystAgent = Object.freeze(
  new FullStackDeploymentAnalystAgent(),
);
