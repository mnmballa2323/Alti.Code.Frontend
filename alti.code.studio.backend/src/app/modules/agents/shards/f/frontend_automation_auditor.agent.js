...
class FrontendAutomationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendAutomationSystem(objective) {
        logger.info(`💻 [FrontendAutomationAuditorAgent] Analyzing Frontend Automation Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Automation Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendAutomationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendAutomationAuditorAgent = Object.freeze(new FrontendAutomationAuditorAgent());
