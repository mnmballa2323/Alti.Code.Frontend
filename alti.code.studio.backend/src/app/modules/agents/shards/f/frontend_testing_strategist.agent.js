...
class FrontendTestingStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendTestingSystem(objective) {
        logger.info(`💻 [FrontendTestingStrategistAgent] Analyzing Frontend Testing Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Testing Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendTestingStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendTestingStrategistAgent = Object.freeze(new FrontendTestingStrategistAgent());
