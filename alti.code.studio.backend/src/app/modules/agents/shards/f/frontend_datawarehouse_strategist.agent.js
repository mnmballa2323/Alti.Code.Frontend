...
class FrontendDataWarehouseStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendDataWarehouseSystem(objective) {
    logger.info(
      `💻 [FrontendDataWarehouseStrategistAgent] Analyzing Frontend DataWarehouse Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend DataWarehouse Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendDataWarehouseStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendDataWarehouseStrategistAgent = Object.freeze(
  new FrontendDataWarehouseStrategistAgent(),
);
