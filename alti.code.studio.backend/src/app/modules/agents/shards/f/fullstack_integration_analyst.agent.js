...
class FullStackIntegrationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackIntegrationSystem(objective) {
        logger.info(`💻 [FullStackIntegrationAnalystAgent] Analyzing FullStack Integration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Integration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackIntegrationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackIntegrationAnalystAgent = Object.freeze(new FullStackIntegrationAnalystAgent());
