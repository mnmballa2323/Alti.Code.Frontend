...
class FullStackConfigurationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackConfigurationSystem(objective) {
        logger.info(`💻 [FullStackConfigurationDirectorAgent] Analyzing FullStack Configuration Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Configuration Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackConfigurationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackConfigurationDirectorAgent = Object.freeze(new FullStackConfigurationDirectorAgent());
