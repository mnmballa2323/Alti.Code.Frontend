...
class FullStackLoadBalancingEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackLoadBalancingSystem(objective) {
    logger.info(
      `💻 [FullStackLoadBalancingEngineerAgent] Analyzing FullStack LoadBalancing Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack LoadBalancing Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackLoadBalancingEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackLoadBalancingEngineerAgent = Object.freeze(
  new FullStackLoadBalancingEngineerAgent(),
);
