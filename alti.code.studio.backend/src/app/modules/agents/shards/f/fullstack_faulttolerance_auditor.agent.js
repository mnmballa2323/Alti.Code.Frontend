...
class FullStackFaultToleranceAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackFaultToleranceSystem(objective) {
    logger.info(
      `💻 [FullStackFaultToleranceAuditorAgent] Analyzing FullStack FaultTolerance Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack FaultTolerance Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackFaultToleranceAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackFaultToleranceAuditorAgent = Object.freeze(
  new FullStackFaultToleranceAuditorAgent(),
);
