...
class FullStackIntegrationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackIntegrationSystem(objective) {
    logger.info(
      `💻 [FullStackIntegrationDirectorAgent] Analyzing FullStack Integration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Integration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackIntegrationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackIntegrationDirectorAgent = Object.freeze(
  new FullStackIntegrationDirectorAgent(),
);
