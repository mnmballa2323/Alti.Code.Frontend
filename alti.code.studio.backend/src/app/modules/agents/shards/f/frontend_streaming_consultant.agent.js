...
class FrontendStreamingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendStreamingSystem(objective) {
        logger.info(`💻 [FrontendStreamingConsultantAgent] Analyzing Frontend Streaming Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Streaming Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendStreamingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendStreamingConsultantAgent = Object.freeze(new FrontendStreamingConsultantAgent());
