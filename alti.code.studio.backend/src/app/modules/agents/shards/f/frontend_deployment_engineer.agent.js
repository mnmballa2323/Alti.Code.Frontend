...
class FrontendDeploymentEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendDeploymentSystem(objective) {
    logger.info(
      `💻 [FrontendDeploymentEngineerAgent] Analyzing Frontend Deployment Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Deployment Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendDeploymentEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendDeploymentEngineerAgent = Object.freeze(
  new FrontendDeploymentEngineerAgent(),
);
