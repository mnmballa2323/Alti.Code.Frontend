...
class FrontendConfigurationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendConfigurationSystem(objective) {
    logger.info(
      `💻 [FrontendConfigurationOrchestratorAgent] Analyzing Frontend Configuration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Configuration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendConfigurationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendConfigurationOrchestratorAgent = Object.freeze(
  new FrontendConfigurationOrchestratorAgent(),
);
