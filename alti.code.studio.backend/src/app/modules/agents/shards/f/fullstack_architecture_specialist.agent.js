...
class FullStackArchitectureSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackArchitectureSystem(objective) {
    logger.info(
      `💻 [FullStackArchitectureSpecialistAgent] Analyzing FullStack Architecture Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Architecture Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackArchitectureSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackArchitectureSpecialistAgent = Object.freeze(
  new FullStackArchitectureSpecialistAgent(),
);
