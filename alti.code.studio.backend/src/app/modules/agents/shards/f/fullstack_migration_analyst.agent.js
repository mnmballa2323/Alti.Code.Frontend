...
class FullStackMigrationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackMigrationSystem(objective) {
        logger.info(`💻 [FullStackMigrationAnalystAgent] Analyzing FullStack Migration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Migration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackMigrationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackMigrationAnalystAgent = Object.freeze(new FullStackMigrationAnalystAgent());
