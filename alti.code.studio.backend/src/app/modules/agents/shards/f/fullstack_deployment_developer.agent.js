...
class FullStackDeploymentDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackDeploymentSystem(objective) {
        logger.info(`💻 [FullStackDeploymentDeveloperAgent] Analyzing FullStack Deployment Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Deployment Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackDeploymentDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackDeploymentDeveloperAgent = Object.freeze(new FullStackDeploymentDeveloperAgent());
