...
class FrontendMicroservicesStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendMicroservicesSystem(objective) {
        logger.info(`💻 [FrontendMicroservicesStrategistAgent] Analyzing Frontend Microservices Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Microservices Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendMicroservicesStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendMicroservicesStrategistAgent = Object.freeze(new FrontendMicroservicesStrategistAgent());
