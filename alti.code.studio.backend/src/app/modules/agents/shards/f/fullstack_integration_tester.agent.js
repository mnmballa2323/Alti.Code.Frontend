...
class FullStackIntegrationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackIntegrationSystem(objective) {
    logger.info(
      `💻 [FullStackIntegrationTesterAgent] Analyzing FullStack Integration Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Integration Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackIntegrationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackIntegrationTesterAgent = Object.freeze(
  new FullStackIntegrationTesterAgent(),
);
