...
class FullStackMicroservicesDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackMicroservicesSystem(objective) {
    logger.info(
      `💻 [FullStackMicroservicesDirectorAgent] Analyzing FullStack Microservices Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Microservices Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackMicroservicesDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackMicroservicesDirectorAgent = Object.freeze(
  new FullStackMicroservicesDirectorAgent(),
);
