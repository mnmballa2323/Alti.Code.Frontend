...
class FullStackAnalyticsPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackAnalyticsSystem(objective) {
        logger.info(`💻 [FullStackAnalyticsPlannerAgent] Analyzing FullStack Analytics Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Analytics Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackAnalyticsPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackAnalyticsPlannerAgent = Object.freeze(new FullStackAnalyticsPlannerAgent());
