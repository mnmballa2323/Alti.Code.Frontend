...
class FullStackTelemetryDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackTelemetrySystem(objective) {
        logger.info(`💻 [FullStackTelemetryDeveloperAgent] Analyzing FullStack Telemetry Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Telemetry Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackTelemetryDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackTelemetryDeveloperAgent = Object.freeze(new FullStackTelemetryDeveloperAgent());
