...
class FullStackContainerizationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackContainerizationSystem(objective) {
        logger.info(`💻 [FullStackContainerizationConsultantAgent] Analyzing FullStack Containerization Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Containerization Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackContainerizationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackContainerizationConsultantAgent = Object.freeze(new FullStackContainerizationConsultantAgent());
