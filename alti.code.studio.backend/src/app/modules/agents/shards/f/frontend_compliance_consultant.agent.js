...
class FrontendComplianceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendComplianceSystem(objective) {
        logger.info(`💻 [FrontendComplianceConsultantAgent] Analyzing Frontend Compliance Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Compliance Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendComplianceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendComplianceConsultantAgent = Object.freeze(new FrontendComplianceConsultantAgent());
