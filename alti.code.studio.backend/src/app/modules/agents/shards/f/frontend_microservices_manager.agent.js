...
class FrontendMicroservicesManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendMicroservicesSystem(objective) {
        logger.info(`💻 [FrontendMicroservicesManagerAgent] Analyzing Frontend Microservices Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Microservices Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendMicroservicesManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendMicroservicesManagerAgent = Object.freeze(new FrontendMicroservicesManagerAgent());
