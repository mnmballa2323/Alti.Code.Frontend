...
class FullStackLoadBalancingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackLoadBalancingSystem(objective) {
    logger.info(
      `💻 [FullStackLoadBalancingDesignerAgent] Analyzing FullStack LoadBalancing Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack LoadBalancing Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackLoadBalancingDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackLoadBalancingDesignerAgent = Object.freeze(
  new FullStackLoadBalancingDesignerAgent(),
);
