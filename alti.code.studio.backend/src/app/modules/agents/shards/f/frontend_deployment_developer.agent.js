...
class FrontendDeploymentDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendDeploymentSystem(objective) {
        logger.info(`💻 [FrontendDeploymentDeveloperAgent] Analyzing Frontend Deployment Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Deployment Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendDeploymentDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendDeploymentDeveloperAgent = Object.freeze(new FrontendDeploymentDeveloperAgent());
