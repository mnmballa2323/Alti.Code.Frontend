...
class FullStackVirtualizationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackVirtualizationSystem(objective) {
        logger.info(`💻 [FullStackVirtualizationEngineerAgent] Analyzing FullStack Virtualization Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Virtualization Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackVirtualizationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackVirtualizationEngineerAgent = Object.freeze(new FullStackVirtualizationEngineerAgent());
