...
class FrontendIntegrationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendIntegrationSystem(objective) {
    logger.info(
      `💻 [FrontendIntegrationDeveloperAgent] Analyzing Frontend Integration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Integration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendIntegrationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendIntegrationDeveloperAgent = Object.freeze(
  new FrontendIntegrationDeveloperAgent(),
);
