...
class FullStackOrchestrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackOrchestrationSystem(objective) {
    logger.info(
      `💻 [FullStackOrchestrationEngineerAgent] Analyzing FullStack Orchestration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Orchestration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackOrchestrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackOrchestrationEngineerAgent = Object.freeze(
  new FullStackOrchestrationEngineerAgent(),
);
