...
class FullStackResilienceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackResilienceSystem(objective) {
    logger.info(
      `💻 [FullStackResilienceDesignerAgent] Analyzing FullStack Resilience Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Resilience Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackResilienceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackResilienceDesignerAgent = Object.freeze(
  new FullStackResilienceDesignerAgent(),
);
