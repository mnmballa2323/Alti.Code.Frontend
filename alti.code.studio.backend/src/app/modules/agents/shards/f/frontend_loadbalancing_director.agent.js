...
class FrontendLoadBalancingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendLoadBalancingSystem(objective) {
    logger.info(
      `💻 [FrontendLoadBalancingDirectorAgent] Analyzing Frontend LoadBalancing Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend LoadBalancing Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendLoadBalancingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendLoadBalancingDirectorAgent = Object.freeze(
  new FrontendLoadBalancingDirectorAgent(),
);
