...
class FullStackResilienceLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackResilienceSystem(objective) {
    logger.info(
      `💻 [FullStackResilienceLeadAgent] Analyzing FullStack Resilience Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Resilience Lead Target',
      );
...
export const fullStackResilienceLeadAgent = Object.freeze(
  new FullStackResilienceLeadAgent(),
);
