...
class FullStackBatchProcessingAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackBatchProcessingSystem(objective) {
    logger.info(
      `💻 [FullStackBatchProcessingAnalystAgent] Analyzing FullStack BatchProcessing Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack BatchProcessing Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackBatchProcessingAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackBatchProcessingAnalystAgent = Object.freeze(
  new FullStackBatchProcessingAnalystAgent(),
);
