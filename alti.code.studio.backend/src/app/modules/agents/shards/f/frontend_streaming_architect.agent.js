...
class FrontendStreamingArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendStreamingSystem(objective) {
    logger.info(
      `💻 [FrontendStreamingArchitectAgent] Analyzing Frontend Streaming Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Streaming Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendStreamingArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendStreamingArchitectAgent = Object.freeze(
  new FrontendStreamingArchitectAgent(),
);
