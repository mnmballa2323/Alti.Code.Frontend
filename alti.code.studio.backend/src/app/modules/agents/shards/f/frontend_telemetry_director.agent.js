...
class FrontendTelemetryDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendTelemetrySystem(objective) {
    logger.info(
      `💻 [FrontendTelemetryDirectorAgent] Analyzing Frontend Telemetry Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Telemetry Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendTelemetryDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendTelemetryDirectorAgent = Object.freeze(
  new FrontendTelemetryDirectorAgent(),
);
