...
class FullStackVirtualizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackVirtualizationSystem(objective) {
        logger.info(`💻 [FullStackVirtualizationDirectorAgent] Analyzing FullStack Virtualization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Virtualization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackVirtualizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackVirtualizationDirectorAgent = Object.freeze(new FullStackVirtualizationDirectorAgent());
