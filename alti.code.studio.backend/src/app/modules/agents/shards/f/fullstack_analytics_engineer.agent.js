...
class FullStackAnalyticsEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackAnalyticsSystem(objective) {
        logger.info(`💻 [FullStackAnalyticsEngineerAgent] Analyzing FullStack Analytics Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Analytics Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackAnalyticsEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackAnalyticsEngineerAgent = Object.freeze(new FullStackAnalyticsEngineerAgent());
