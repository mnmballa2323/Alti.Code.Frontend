...
class FullStackProvisioningEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackProvisioningSystem(objective) {
    logger.info(
      `💻 [FullStackProvisioningEngineerAgent] Analyzing FullStack Provisioning Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Provisioning Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackProvisioningEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackProvisioningEngineerAgent = Object.freeze(
  new FullStackProvisioningEngineerAgent(),
);
