...
class FullStackDataLakeDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDataLakeSystem(objective) {
    logger.info(
      `💻 [FullStackDataLakeDeveloperAgent] Analyzing FullStack DataLake Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack DataLake Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDataLakeDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDataLakeDeveloperAgent = Object.freeze(
  new FullStackDataLakeDeveloperAgent(),
);
