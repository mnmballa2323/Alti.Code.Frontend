...
class FrontendAutomationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendAutomationSystem(objective) {
        logger.info(`💻 [FrontendAutomationSpecialistAgent] Analyzing Frontend Automation Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Automation Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendAutomationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendAutomationSpecialistAgent = Object.freeze(new FrontendAutomationSpecialistAgent());
