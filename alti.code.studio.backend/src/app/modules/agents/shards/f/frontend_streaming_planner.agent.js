...
class FrontendStreamingPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendStreamingSystem(objective) {
        logger.info(`💻 [FrontendStreamingPlannerAgent] Analyzing Frontend Streaming Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Streaming Planner Target");
...
export const frontendStreamingPlannerAgent = Object.freeze(new FrontendStreamingPlannerAgent());
