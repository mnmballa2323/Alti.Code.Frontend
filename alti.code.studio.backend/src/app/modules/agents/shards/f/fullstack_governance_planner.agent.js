...
class FullStackGovernancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackGovernanceSystem(objective) {
    logger.info(
      `💻 [FullStackGovernancePlannerAgent] Analyzing FullStack Governance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Governance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackGovernancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackGovernancePlannerAgent = Object.freeze(
  new FullStackGovernancePlannerAgent(),
);
