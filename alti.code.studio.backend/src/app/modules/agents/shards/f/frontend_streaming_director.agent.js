...
class FrontendStreamingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendStreamingSystem(objective) {
    logger.info(
      `💻 [FrontendStreamingDirectorAgent] Analyzing Frontend Streaming Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Streaming Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendStreamingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendStreamingDirectorAgent = Object.freeze(
  new FrontendStreamingDirectorAgent(),
);
