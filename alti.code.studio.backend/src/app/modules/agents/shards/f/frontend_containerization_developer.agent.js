...
class FrontendContainerizationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendContainerizationSystem(objective) {
        logger.info(`💻 [FrontendContainerizationDeveloperAgent] Analyzing Frontend Containerization Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Containerization Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendContainerizationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendContainerizationDeveloperAgent = Object.freeze(new FrontendContainerizationDeveloperAgent());
