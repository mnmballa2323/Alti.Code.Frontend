...
class FrontendComplianceAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendComplianceSystem(objective) {
        logger.info(`💻 [FrontendComplianceAnalystAgent] Analyzing Frontend Compliance Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Compliance Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendComplianceAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendComplianceAnalystAgent = Object.freeze(new FrontendComplianceAnalystAgent());
