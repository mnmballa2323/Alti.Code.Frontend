...
class FullStackStreamingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackStreamingSystem(objective) {
    logger.info(
      `💻 [FullStackStreamingDesignerAgent] Analyzing FullStack Streaming Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Streaming Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackStreamingDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackStreamingDesignerAgent = Object.freeze(
  new FullStackStreamingDesignerAgent(),
);
