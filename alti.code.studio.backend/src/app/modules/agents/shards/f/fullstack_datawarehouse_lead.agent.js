...
class FullStackDataWarehouseLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackDataWarehouseSystem(objective) {
        logger.info(`💻 [FullStackDataWarehouseLeadAgent] Analyzing FullStack DataWarehouse Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack DataWarehouse Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackDataWarehouseLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackDataWarehouseLeadAgent = Object.freeze(new FullStackDataWarehouseLeadAgent());
