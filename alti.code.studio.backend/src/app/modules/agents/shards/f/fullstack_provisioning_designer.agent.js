...
class FullStackProvisioningDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackProvisioningSystem(objective) {
        logger.info(`💻 [FullStackProvisioningDesignerAgent] Analyzing FullStack Provisioning Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Provisioning Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackProvisioningDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackProvisioningDesignerAgent = Object.freeze(new FullStackProvisioningDesignerAgent());
