...
class FullStackMigrationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackMigrationSystem(objective) {
        logger.info(`💻 [FullStackMigrationEngineerAgent] Analyzing FullStack Migration Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Migration Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackMigrationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackMigrationEngineerAgent = Object.freeze(new FullStackMigrationEngineerAgent());
