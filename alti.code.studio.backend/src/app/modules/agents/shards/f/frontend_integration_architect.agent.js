...
class FrontendIntegrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendIntegrationSystem(objective) {
    logger.info(
      `💻 [FrontendIntegrationArchitectAgent] Analyzing Frontend Integration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Integration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendIntegrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendIntegrationArchitectAgent = Object.freeze(
  new FrontendIntegrationArchitectAgent(),
);
