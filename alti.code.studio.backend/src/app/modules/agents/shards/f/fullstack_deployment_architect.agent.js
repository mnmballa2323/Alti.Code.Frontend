...
class FullStackDeploymentArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDeploymentSystem(objective) {
    logger.info(
      `💻 [FullStackDeploymentArchitectAgent] Analyzing FullStack Deployment Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Deployment Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDeploymentArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDeploymentArchitectAgent = Object.freeze(
  new FullStackDeploymentArchitectAgent(),
);
