...
class FullStackAutomationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackAutomationSystem(objective) {
        logger.info(`💻 [FullStackAutomationArchitectAgent] Analyzing FullStack Automation Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Automation Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackAutomationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackAutomationArchitectAgent = Object.freeze(new FullStackAutomationArchitectAgent());
