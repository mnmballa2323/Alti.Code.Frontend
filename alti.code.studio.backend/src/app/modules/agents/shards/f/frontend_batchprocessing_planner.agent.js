...
class FrontendBatchProcessingPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendBatchProcessingSystem(objective) {
        logger.info(`💻 [FrontendBatchProcessingPlannerAgent] Analyzing Frontend BatchProcessing Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend BatchProcessing Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendBatchProcessingPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendBatchProcessingPlannerAgent = Object.freeze(new FrontendBatchProcessingPlannerAgent());
