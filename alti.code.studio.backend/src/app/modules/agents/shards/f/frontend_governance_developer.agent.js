...
class FrontendGovernanceDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendGovernanceSystem(objective) {
    logger.info(
      `💻 [FrontendGovernanceDeveloperAgent] Analyzing Frontend Governance Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Governance Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendGovernanceDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendGovernanceDeveloperAgent = Object.freeze(
  new FrontendGovernanceDeveloperAgent(),
);
