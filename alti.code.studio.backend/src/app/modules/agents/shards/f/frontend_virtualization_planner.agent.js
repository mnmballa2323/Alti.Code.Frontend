...
class FrontendVirtualizationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendVirtualizationSystem(objective) {
        logger.info(`💻 [FrontendVirtualizationPlannerAgent] Analyzing Frontend Virtualization Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Virtualization Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendVirtualizationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendVirtualizationPlannerAgent = Object.freeze(new FrontendVirtualizationPlannerAgent());
