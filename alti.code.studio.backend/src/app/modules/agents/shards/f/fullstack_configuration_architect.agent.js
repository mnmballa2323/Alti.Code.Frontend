...
class FullStackConfigurationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackConfigurationSystem(objective) {
        logger.info(`💻 [FullStackConfigurationArchitectAgent] Analyzing FullStack Configuration Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Configuration Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackConfigurationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackConfigurationArchitectAgent = Object.freeze(new FullStackConfigurationArchitectAgent());
