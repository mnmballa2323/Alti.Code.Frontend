...
class FullStackTestingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackTestingSystem(objective) {
    logger.info(
      `💻 [FullStackTestingDeveloperAgent] Analyzing FullStack Testing Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Testing Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackTestingDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackTestingDeveloperAgent = Object.freeze(
  new FullStackTestingDeveloperAgent(),
);
