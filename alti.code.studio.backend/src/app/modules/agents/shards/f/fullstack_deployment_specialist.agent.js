...
class FullStackDeploymentSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackDeploymentSystem(objective) {
        logger.info(`💻 [FullStackDeploymentSpecialistAgent] Analyzing FullStack Deployment Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Deployment Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackDeploymentSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackDeploymentSpecialistAgent = Object.freeze(new FullStackDeploymentSpecialistAgent());
