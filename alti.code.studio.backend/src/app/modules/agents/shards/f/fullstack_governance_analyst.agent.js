...
class FullStackGovernanceAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackGovernanceSystem(objective) {
    logger.info(
      `💻 [FullStackGovernanceAnalystAgent] Analyzing FullStack Governance Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Governance Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackGovernanceAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackGovernanceAnalystAgent = Object.freeze(
  new FullStackGovernanceAnalystAgent(),
);
