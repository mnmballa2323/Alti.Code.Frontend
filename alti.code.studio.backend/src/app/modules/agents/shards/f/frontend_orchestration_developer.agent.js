...
class FrontendOrchestrationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendOrchestrationSystem(objective) {
        logger.info(`💻 [FrontendOrchestrationDeveloperAgent] Analyzing Frontend Orchestration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Orchestration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendOrchestrationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendOrchestrationDeveloperAgent = Object.freeze(new FrontendOrchestrationDeveloperAgent());
