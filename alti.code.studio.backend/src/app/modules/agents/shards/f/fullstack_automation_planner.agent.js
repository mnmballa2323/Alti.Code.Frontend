...
class FullStackAutomationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackAutomationSystem(objective) {
    logger.info(
      `💻 [FullStackAutomationPlannerAgent] Analyzing FullStack Automation Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Automation Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackAutomationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackAutomationPlannerAgent = Object.freeze(
  new FullStackAutomationPlannerAgent(),
);
