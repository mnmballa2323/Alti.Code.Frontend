...
class FullStackSecurityPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackSecuritySystem(objective) {
    logger.info(
      `💻 [FullStackSecurityPlannerAgent] Analyzing FullStack Security Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Security Planner Target',
      );
...
export const fullStackSecurityPlannerAgent = Object.freeze(
  new FullStackSecurityPlannerAgent(),
);
