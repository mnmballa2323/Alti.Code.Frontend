...
class FullStackIntegrationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackIntegrationSystem(objective) {
        logger.info(`💻 [FullStackIntegrationDesignerAgent] Analyzing FullStack Integration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Integration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackIntegrationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackIntegrationDesignerAgent = Object.freeze(new FullStackIntegrationDesignerAgent());
