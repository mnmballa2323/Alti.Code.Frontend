...
class FrontendFaultToleranceTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendFaultToleranceSystem(objective) {
    logger.info(
      `💻 [FrontendFaultToleranceTesterAgent] Analyzing Frontend FaultTolerance Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend FaultTolerance Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendFaultToleranceTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendFaultToleranceTesterAgent = Object.freeze(
  new FrontendFaultToleranceTesterAgent(),
);
