...
class FrontendConfigurationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendConfigurationSystem(objective) {
    logger.info(
      `💻 [FrontendConfigurationEngineerAgent] Analyzing Frontend Configuration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Configuration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendConfigurationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendConfigurationEngineerAgent = Object.freeze(
  new FrontendConfigurationEngineerAgent(),
);
