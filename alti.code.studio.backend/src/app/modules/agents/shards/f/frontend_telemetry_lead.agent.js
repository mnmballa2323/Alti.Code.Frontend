...
class FrontendTelemetryLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendTelemetrySystem(objective) {
    logger.info(
      `💻 [FrontendTelemetryLeadAgent] Analyzing Frontend Telemetry Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Telemetry Lead Target',
      );
...
export const frontendTelemetryLeadAgent = Object.freeze(
  new FrontendTelemetryLeadAgent(),
);
