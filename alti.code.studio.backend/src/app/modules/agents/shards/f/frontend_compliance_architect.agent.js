...
class FrontendComplianceArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendComplianceSystem(objective) {
        logger.info(`💻 [FrontendComplianceArchitectAgent] Analyzing Frontend Compliance Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Compliance Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendComplianceArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendComplianceArchitectAgent = Object.freeze(new FrontendComplianceArchitectAgent());
