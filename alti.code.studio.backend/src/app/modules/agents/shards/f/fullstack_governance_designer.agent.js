...
class FullStackGovernanceDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackGovernanceSystem(objective) {
        logger.info(`💻 [FullStackGovernanceDesignerAgent] Analyzing FullStack Governance Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Governance Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackGovernanceDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackGovernanceDesignerAgent = Object.freeze(new FullStackGovernanceDesignerAgent());
