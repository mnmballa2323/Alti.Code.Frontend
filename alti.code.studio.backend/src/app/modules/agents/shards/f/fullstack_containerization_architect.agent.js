...
class FullStackContainerizationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackContainerizationSystem(objective) {
    logger.info(
      `💻 [FullStackContainerizationArchitectAgent] Analyzing FullStack Containerization Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Containerization Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackContainerizationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackContainerizationArchitectAgent = Object.freeze(
  new FullStackContainerizationArchitectAgent(),
);
