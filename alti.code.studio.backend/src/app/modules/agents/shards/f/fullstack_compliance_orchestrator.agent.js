...
class FullStackComplianceOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackComplianceSystem(objective) {
    logger.info(
      `💻 [FullStackComplianceOrchestratorAgent] Analyzing FullStack Compliance Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Compliance Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackComplianceOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackComplianceOrchestratorAgent = Object.freeze(
  new FullStackComplianceOrchestratorAgent(),
);
