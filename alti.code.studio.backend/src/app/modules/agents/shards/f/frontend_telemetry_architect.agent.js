...
class FrontendTelemetryArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendTelemetrySystem(objective) {
        logger.info(`💻 [FrontendTelemetryArchitectAgent] Analyzing Frontend Telemetry Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Telemetry Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendTelemetryArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendTelemetryArchitectAgent = Object.freeze(new FrontendTelemetryArchitectAgent());
