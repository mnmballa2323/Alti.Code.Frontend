...
class FrontendFaultToleranceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendFaultToleranceSystem(objective) {
    logger.info(
      `💻 [FrontendFaultToleranceDirectorAgent] Analyzing Frontend FaultTolerance Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend FaultTolerance Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendFaultToleranceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendFaultToleranceDirectorAgent = Object.freeze(
  new FrontendFaultToleranceDirectorAgent(),
);
