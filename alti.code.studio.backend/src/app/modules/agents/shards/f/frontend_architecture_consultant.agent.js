...
class FrontendArchitectureConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendArchitectureSystem(objective) {
    logger.info(
      `💻 [FrontendArchitectureConsultantAgent] Analyzing Frontend Architecture Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Architecture Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendArchitectureConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendArchitectureConsultantAgent = Object.freeze(
  new FrontendArchitectureConsultantAgent(),
);
