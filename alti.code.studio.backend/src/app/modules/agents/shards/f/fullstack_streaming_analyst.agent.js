...
class FullStackStreamingAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackStreamingSystem(objective) {
    logger.info(
      `💻 [FullStackStreamingAnalystAgent] Analyzing FullStack Streaming Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Streaming Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackStreamingAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackStreamingAnalystAgent = Object.freeze(
  new FullStackStreamingAnalystAgent(),
);
