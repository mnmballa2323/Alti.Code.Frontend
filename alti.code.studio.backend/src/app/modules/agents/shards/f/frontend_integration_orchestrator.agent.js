...
class FrontendIntegrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendIntegrationSystem(objective) {
    logger.info(
      `💻 [FrontendIntegrationOrchestratorAgent] Analyzing Frontend Integration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Integration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendIntegrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendIntegrationOrchestratorAgent = Object.freeze(
  new FrontendIntegrationOrchestratorAgent(),
);
