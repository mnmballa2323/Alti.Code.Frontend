...
class FrontendResilienceStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendResilienceSystem(objective) {
    logger.info(
      `💻 [FrontendResilienceStrategistAgent] Analyzing Frontend Resilience Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Resilience Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendResilienceStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendResilienceStrategistAgent = Object.freeze(
  new FrontendResilienceStrategistAgent(),
);
