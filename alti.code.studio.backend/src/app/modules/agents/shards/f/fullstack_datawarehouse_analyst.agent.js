...
class FullStackDataWarehouseAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDataWarehouseSystem(objective) {
    logger.info(
      `💻 [FullStackDataWarehouseAnalystAgent] Analyzing FullStack DataWarehouse Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack DataWarehouse Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDataWarehouseAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDataWarehouseAnalystAgent = Object.freeze(
  new FullStackDataWarehouseAnalystAgent(),
);
