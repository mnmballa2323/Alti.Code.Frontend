...
class FrontendOrchestrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendOrchestrationSystem(objective) {
    logger.info(
      `💻 [FrontendOrchestrationStrategistAgent] Analyzing Frontend Orchestration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Orchestration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendOrchestrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendOrchestrationStrategistAgent = Object.freeze(
  new FrontendOrchestrationStrategistAgent(),
);
