...
class FullStackProvisioningSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackProvisioningSystem(objective) {
    logger.info(
      `💻 [FullStackProvisioningSpecialistAgent] Analyzing FullStack Provisioning Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Provisioning Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackProvisioningSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackProvisioningSpecialistAgent = Object.freeze(
  new FullStackProvisioningSpecialistAgent(),
);
