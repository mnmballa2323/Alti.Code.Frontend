...
class FullStackBatchProcessingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackBatchProcessingSystem(objective) {
    logger.info(
      `💻 [FullStackBatchProcessingConsultantAgent] Analyzing FullStack BatchProcessing Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack BatchProcessing Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackBatchProcessingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackBatchProcessingConsultantAgent = Object.freeze(
  new FullStackBatchProcessingConsultantAgent(),
);
