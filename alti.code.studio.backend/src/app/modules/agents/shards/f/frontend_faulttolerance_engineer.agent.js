...
class FrontendFaultToleranceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendFaultToleranceSystem(objective) {
    logger.info(
      `💻 [FrontendFaultToleranceEngineerAgent] Analyzing Frontend FaultTolerance Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend FaultTolerance Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendFaultToleranceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendFaultToleranceEngineerAgent = Object.freeze(
  new FrontendFaultToleranceEngineerAgent(),
);
