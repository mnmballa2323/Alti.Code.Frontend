...
class FrontendTelemetryManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendTelemetrySystem(objective) {
    logger.info(
      `💻 [FrontendTelemetryManagerAgent] Analyzing Frontend Telemetry Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Telemetry Manager Target',
      );
...
export const frontendTelemetryManagerAgent = Object.freeze(
  new FrontendTelemetryManagerAgent(),
);
