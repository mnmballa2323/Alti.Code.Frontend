...
class FrontendObservabilityLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendObservabilitySystem(objective) {
        logger.info(`💻 [FrontendObservabilityLeadAgent] Analyzing Frontend Observability Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Observability Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendObservabilityLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendObservabilityLeadAgent = Object.freeze(new FrontendObservabilityLeadAgent());
