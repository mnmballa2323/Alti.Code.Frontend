...
class FrontendOrchestrationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendOrchestrationSystem(objective) {
        logger.info(`💻 [FrontendOrchestrationAnalystAgent] Analyzing Frontend Orchestration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Orchestration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendOrchestrationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendOrchestrationAnalystAgent = Object.freeze(new FrontendOrchestrationAnalystAgent());
