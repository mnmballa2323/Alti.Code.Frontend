...
class FullStackTelemetryTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackTelemetrySystem(objective) {
    logger.info(
      `💻 [FullStackTelemetryTesterAgent] Analyzing FullStack Telemetry Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Telemetry Tester Target',
      );
...
export const fullStackTelemetryTesterAgent = Object.freeze(
  new FullStackTelemetryTesterAgent(),
);
