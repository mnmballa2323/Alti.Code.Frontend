...
class FrontendMicroservicesTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendMicroservicesSystem(objective) {
    logger.info(
      `💻 [FrontendMicroservicesTesterAgent] Analyzing Frontend Microservices Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Microservices Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendMicroservicesTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendMicroservicesTesterAgent = Object.freeze(
  new FrontendMicroservicesTesterAgent(),
);
