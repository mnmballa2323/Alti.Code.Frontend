...
class FullStackEngineeringDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackEngineeringSystem(objective) {
    logger.info(
      `💻 [FullStackEngineeringDeveloperAgent] Analyzing FullStack Engineering Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Engineering Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackEngineeringDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackEngineeringDeveloperAgent = Object.freeze(
  new FullStackEngineeringDeveloperAgent(),
);
