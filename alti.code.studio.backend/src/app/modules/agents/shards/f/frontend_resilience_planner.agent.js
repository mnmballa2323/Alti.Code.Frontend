...
class FrontendResiliencePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendResilienceSystem(objective) {
    logger.info(
      `💻 [FrontendResiliencePlannerAgent] Analyzing Frontend Resilience Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Resilience Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendResiliencePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendResiliencePlannerAgent = Object.freeze(
  new FrontendResiliencePlannerAgent(),
);
