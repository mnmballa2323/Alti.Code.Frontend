...
class FullStackResilienceManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackResilienceSystem(objective) {
        logger.info(`💻 [FullStackResilienceManagerAgent] Analyzing FullStack Resilience Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Resilience Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackResilienceManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackResilienceManagerAgent = Object.freeze(new FullStackResilienceManagerAgent());
