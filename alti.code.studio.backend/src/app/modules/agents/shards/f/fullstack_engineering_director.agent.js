...
class FullStackEngineeringDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackEngineeringSystem(objective) {
        logger.info(`💻 [FullStackEngineeringDirectorAgent] Analyzing FullStack Engineering Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Engineering Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackEngineeringDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackEngineeringDirectorAgent = Object.freeze(new FullStackEngineeringDirectorAgent());
