...
class FrontendGovernanceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendGovernanceSystem(objective) {
    logger.info(
      `💻 [FrontendGovernanceEngineerAgent] Analyzing Frontend Governance Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Governance Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendGovernanceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendGovernanceEngineerAgent = Object.freeze(
  new FrontendGovernanceEngineerAgent(),
);
