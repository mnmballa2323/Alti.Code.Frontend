...
class FullStackSecurityDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackSecuritySystem(objective) {
    logger.info(
      `💻 [FullStackSecurityDirectorAgent] Analyzing FullStack Security Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Security Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackSecurityDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackSecurityDirectorAgent = Object.freeze(
  new FullStackSecurityDirectorAgent(),
);
