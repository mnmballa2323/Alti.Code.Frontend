...
class FullStackContainerizationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackContainerizationSystem(objective) {
    logger.info(
      `💻 [FullStackContainerizationEngineerAgent] Analyzing FullStack Containerization Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Containerization Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackContainerizationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackContainerizationEngineerAgent = Object.freeze(
  new FullStackContainerizationEngineerAgent(),
);
