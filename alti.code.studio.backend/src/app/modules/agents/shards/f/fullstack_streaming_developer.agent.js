...
class FullStackStreamingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackStreamingSystem(objective) {
    logger.info(
      `💻 [FullStackStreamingDeveloperAgent] Analyzing FullStack Streaming Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Streaming Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackStreamingDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackStreamingDeveloperAgent = Object.freeze(
  new FullStackStreamingDeveloperAgent(),
);
