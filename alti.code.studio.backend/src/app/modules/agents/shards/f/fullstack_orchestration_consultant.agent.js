...
class FullStackOrchestrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackOrchestrationSystem(objective) {
    logger.info(
      `💻 [FullStackOrchestrationConsultantAgent] Analyzing FullStack Orchestration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Orchestration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackOrchestrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackOrchestrationConsultantAgent = Object.freeze(
  new FullStackOrchestrationConsultantAgent(),
);
