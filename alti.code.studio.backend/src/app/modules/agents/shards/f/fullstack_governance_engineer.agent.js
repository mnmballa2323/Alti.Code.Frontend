...
class FullStackGovernanceEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackGovernanceSystem(objective) {
        logger.info(`💻 [FullStackGovernanceEngineerAgent] Analyzing FullStack Governance Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Governance Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackGovernanceEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackGovernanceEngineerAgent = Object.freeze(new FullStackGovernanceEngineerAgent());
