...
class FullStackProvisioningArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackProvisioningSystem(objective) {
    logger.info(
      `💻 [FullStackProvisioningArchitectAgent] Analyzing FullStack Provisioning Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Provisioning Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackProvisioningArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackProvisioningArchitectAgent = Object.freeze(
  new FullStackProvisioningArchitectAgent(),
);
