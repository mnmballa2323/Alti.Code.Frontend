...
class FullStackComplianceAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackComplianceSystem(objective) {
        logger.info(`💻 [FullStackComplianceAuditorAgent] Analyzing FullStack Compliance Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Compliance Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackComplianceAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackComplianceAuditorAgent = Object.freeze(new FullStackComplianceAuditorAgent());
