...
class FullStackStreamingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackStreamingSystem(objective) {
    logger.info(
      `💻 [FullStackStreamingDirectorAgent] Analyzing FullStack Streaming Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Streaming Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackStreamingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackStreamingDirectorAgent = Object.freeze(
  new FullStackStreamingDirectorAgent(),
);
