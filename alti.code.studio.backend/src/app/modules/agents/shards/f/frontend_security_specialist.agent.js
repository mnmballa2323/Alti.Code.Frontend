...
class FrontendSecuritySpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendSecuritySystem(objective) {
        logger.info(`💻 [FrontendSecuritySpecialistAgent] Analyzing Frontend Security Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Security Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendSecuritySpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendSecuritySpecialistAgent = Object.freeze(new FrontendSecuritySpecialistAgent());
