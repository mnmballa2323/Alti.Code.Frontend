...
class FullStackStreamingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackStreamingSystem(objective) {
    logger.info(
      `💻 [FullStackStreamingManagerAgent] Analyzing FullStack Streaming Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Streaming Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackStreamingManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackStreamingManagerAgent = Object.freeze(
  new FullStackStreamingManagerAgent(),
);
