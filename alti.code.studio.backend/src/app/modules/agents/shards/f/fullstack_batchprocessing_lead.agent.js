...
class FullStackBatchProcessingLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackBatchProcessingSystem(objective) {
    logger.info(
      `💻 [FullStackBatchProcessingLeadAgent] Analyzing FullStack BatchProcessing Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack BatchProcessing Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackBatchProcessingLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackBatchProcessingLeadAgent = Object.freeze(
  new FullStackBatchProcessingLeadAgent(),
);
