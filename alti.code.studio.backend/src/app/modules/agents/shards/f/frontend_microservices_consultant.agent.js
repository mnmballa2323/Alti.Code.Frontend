...
class FrontendMicroservicesConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendMicroservicesSystem(objective) {
    logger.info(
      `💻 [FrontendMicroservicesConsultantAgent] Analyzing Frontend Microservices Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Microservices Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendMicroservicesConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendMicroservicesConsultantAgent = Object.freeze(
  new FrontendMicroservicesConsultantAgent(),
);
