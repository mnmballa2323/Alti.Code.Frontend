...
class FrontendOptimizationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendOptimizationSystem(objective) {
    logger.info(
      `💻 [FrontendOptimizationOrchestratorAgent] Analyzing Frontend Optimization Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Optimization Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendOptimizationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendOptimizationOrchestratorAgent = Object.freeze(
  new FrontendOptimizationOrchestratorAgent(),
);
