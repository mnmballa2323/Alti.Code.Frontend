...
class FrontendProvisioningArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendProvisioningSystem(objective) {
        logger.info(`💻 [FrontendProvisioningArchitectAgent] Analyzing Frontend Provisioning Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Provisioning Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendProvisioningArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendProvisioningArchitectAgent = Object.freeze(new FrontendProvisioningArchitectAgent());
