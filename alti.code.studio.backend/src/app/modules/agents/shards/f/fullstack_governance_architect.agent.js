...
class FullStackGovernanceArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackGovernanceSystem(objective) {
    logger.info(
      `💻 [FullStackGovernanceArchitectAgent] Analyzing FullStack Governance Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Governance Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackGovernanceArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackGovernanceArchitectAgent = Object.freeze(
  new FullStackGovernanceArchitectAgent(),
);
