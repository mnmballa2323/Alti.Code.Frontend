...
class FrontendGovernanceDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendGovernanceSystem(objective) {
        logger.info(`💻 [FrontendGovernanceDirectorAgent] Analyzing Frontend Governance Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Governance Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendGovernanceDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendGovernanceDirectorAgent = Object.freeze(new FrontendGovernanceDirectorAgent());
