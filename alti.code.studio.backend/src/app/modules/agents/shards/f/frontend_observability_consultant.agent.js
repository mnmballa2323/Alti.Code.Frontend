...
class FrontendObservabilityConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendObservabilitySystem(objective) {
    logger.info(
      `💻 [FrontendObservabilityConsultantAgent] Analyzing Frontend Observability Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Observability Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendObservabilityConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendObservabilityConsultantAgent = Object.freeze(
  new FrontendObservabilityConsultantAgent(),
);
