...
class FullStackAutomationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackAutomationSystem(objective) {
        logger.info(`💻 [FullStackAutomationDeveloperAgent] Analyzing FullStack Automation Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Automation Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackAutomationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackAutomationDeveloperAgent = Object.freeze(new FullStackAutomationDeveloperAgent());
