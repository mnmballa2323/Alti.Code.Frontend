...
class FullStackServerlessTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackServerlessSystem(objective) {
    logger.info(
      `💻 [FullStackServerlessTesterAgent] Analyzing FullStack Serverless Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Serverless Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackServerlessTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackServerlessTesterAgent = Object.freeze(
  new FullStackServerlessTesterAgent(),
);
