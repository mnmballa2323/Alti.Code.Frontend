...
class FrontendObservabilityAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendObservabilitySystem(objective) {
    logger.info(
      `💻 [FrontendObservabilityAuditorAgent] Analyzing Frontend Observability Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Observability Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendObservabilityAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendObservabilityAuditorAgent = Object.freeze(
  new FrontendObservabilityAuditorAgent(),
);
