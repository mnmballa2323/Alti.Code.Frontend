...
class FullStackFaultToleranceArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackFaultToleranceSystem(objective) {
        logger.info(`💻 [FullStackFaultToleranceArchitectAgent] Analyzing FullStack FaultTolerance Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack FaultTolerance Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackFaultToleranceArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackFaultToleranceArchitectAgent = Object.freeze(new FullStackFaultToleranceArchitectAgent());
