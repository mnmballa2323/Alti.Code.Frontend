...
class FrontendDataWarehouseTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendDataWarehouseSystem(objective) {
    logger.info(
      `💻 [FrontendDataWarehouseTesterAgent] Analyzing Frontend DataWarehouse Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend DataWarehouse Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendDataWarehouseTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendDataWarehouseTesterAgent = Object.freeze(
  new FrontendDataWarehouseTesterAgent(),
);
