...
class FullStackMigrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackMigrationSystem(objective) {
    logger.info(
      `💻 [FullStackMigrationConsultantAgent] Analyzing FullStack Migration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Migration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackMigrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackMigrationConsultantAgent = Object.freeze(
  new FullStackMigrationConsultantAgent(),
);
