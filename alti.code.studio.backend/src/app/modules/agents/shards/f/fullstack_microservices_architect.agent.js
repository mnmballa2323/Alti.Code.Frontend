...
class FullStackMicroservicesArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackMicroservicesSystem(objective) {
        logger.info(`💻 [FullStackMicroservicesArchitectAgent] Analyzing FullStack Microservices Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Microservices Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackMicroservicesArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackMicroservicesArchitectAgent = Object.freeze(new FullStackMicroservicesArchitectAgent());
