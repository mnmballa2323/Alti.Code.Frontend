...
class FrontendLoadBalancingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendLoadBalancingSystem(objective) {
        logger.info(`💻 [FrontendLoadBalancingConsultantAgent] Analyzing Frontend LoadBalancing Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend LoadBalancing Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendLoadBalancingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendLoadBalancingConsultantAgent = Object.freeze(new FrontendLoadBalancingConsultantAgent());
