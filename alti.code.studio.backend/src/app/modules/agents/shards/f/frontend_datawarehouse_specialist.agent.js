...
class FrontendDataWarehouseSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendDataWarehouseSystem(objective) {
        logger.info(`💻 [FrontendDataWarehouseSpecialistAgent] Analyzing Frontend DataWarehouse Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend DataWarehouse Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendDataWarehouseSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendDataWarehouseSpecialistAgent = Object.freeze(new FrontendDataWarehouseSpecialistAgent());
