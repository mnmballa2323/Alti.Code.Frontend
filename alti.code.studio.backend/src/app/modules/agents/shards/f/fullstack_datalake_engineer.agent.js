...
class FullStackDataLakeEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackDataLakeSystem(objective) {
        logger.info(`💻 [FullStackDataLakeEngineerAgent] Analyzing FullStack DataLake Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack DataLake Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackDataLakeEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackDataLakeEngineerAgent = Object.freeze(new FullStackDataLakeEngineerAgent());
