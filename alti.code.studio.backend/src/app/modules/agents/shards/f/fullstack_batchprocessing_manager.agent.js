...
class FullStackBatchProcessingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackBatchProcessingSystem(objective) {
    logger.info(
      `💻 [FullStackBatchProcessingManagerAgent] Analyzing FullStack BatchProcessing Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack BatchProcessing Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackBatchProcessingManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackBatchProcessingManagerAgent = Object.freeze(
  new FullStackBatchProcessingManagerAgent(),
);
