...
class FrontendAutomationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendAutomationSystem(objective) {
        logger.info(`💻 [FrontendAutomationDesignerAgent] Analyzing Frontend Automation Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Automation Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendAutomationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendAutomationDesignerAgent = Object.freeze(new FrontendAutomationDesignerAgent());
