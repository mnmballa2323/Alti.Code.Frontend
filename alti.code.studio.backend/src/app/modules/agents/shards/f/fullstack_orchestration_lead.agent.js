...
class FullStackOrchestrationLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackOrchestrationSystem(objective) {
        logger.info(`💻 [FullStackOrchestrationLeadAgent] Analyzing FullStack Orchestration Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Orchestration Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackOrchestrationLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackOrchestrationLeadAgent = Object.freeze(new FullStackOrchestrationLeadAgent());
