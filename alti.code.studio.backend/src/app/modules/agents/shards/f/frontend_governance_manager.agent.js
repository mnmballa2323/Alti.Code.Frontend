...
class FrontendGovernanceManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendGovernanceSystem(objective) {
    logger.info(
      `💻 [FrontendGovernanceManagerAgent] Analyzing Frontend Governance Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Governance Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendGovernanceManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendGovernanceManagerAgent = Object.freeze(
  new FrontendGovernanceManagerAgent(),
);
