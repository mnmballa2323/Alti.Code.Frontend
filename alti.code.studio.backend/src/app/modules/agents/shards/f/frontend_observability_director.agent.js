...
class FrontendObservabilityDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendObservabilitySystem(objective) {
        logger.info(`💻 [FrontendObservabilityDirectorAgent] Analyzing Frontend Observability Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Observability Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendObservabilityDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendObservabilityDirectorAgent = Object.freeze(new FrontendObservabilityDirectorAgent());
