...
class FullStackServerlessArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackServerlessSystem(objective) {
        logger.info(`💻 [FullStackServerlessArchitectAgent] Analyzing FullStack Serverless Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Serverless Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackServerlessArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackServerlessArchitectAgent = Object.freeze(new FullStackServerlessArchitectAgent());
