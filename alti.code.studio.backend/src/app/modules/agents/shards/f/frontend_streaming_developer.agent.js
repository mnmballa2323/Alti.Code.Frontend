...
class FrontendStreamingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendStreamingSystem(objective) {
    logger.info(
      `💻 [FrontendStreamingDeveloperAgent] Analyzing Frontend Streaming Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Streaming Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendStreamingDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendStreamingDeveloperAgent = Object.freeze(
  new FrontendStreamingDeveloperAgent(),
);
