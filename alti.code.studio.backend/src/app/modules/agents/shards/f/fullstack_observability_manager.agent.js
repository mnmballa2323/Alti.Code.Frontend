...
class FullStackObservabilityManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackObservabilitySystem(objective) {
    logger.info(
      `💻 [FullStackObservabilityManagerAgent] Analyzing FullStack Observability Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Observability Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackObservabilityManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackObservabilityManagerAgent = Object.freeze(
  new FullStackObservabilityManagerAgent(),
);
