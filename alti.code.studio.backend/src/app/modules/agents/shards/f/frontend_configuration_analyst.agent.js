...
class FrontendConfigurationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendConfigurationSystem(objective) {
        logger.info(`💻 [FrontendConfigurationAnalystAgent] Analyzing Frontend Configuration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Configuration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendConfigurationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendConfigurationAnalystAgent = Object.freeze(new FrontendConfigurationAnalystAgent());
