...
class FrontendEngineeringOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendEngineeringSystem(objective) {
    logger.info(
      `💻 [FrontendEngineeringOrchestratorAgent] Analyzing Frontend Engineering Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Engineering Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendEngineeringOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendEngineeringOrchestratorAgent = Object.freeze(
  new FrontendEngineeringOrchestratorAgent(),
);
