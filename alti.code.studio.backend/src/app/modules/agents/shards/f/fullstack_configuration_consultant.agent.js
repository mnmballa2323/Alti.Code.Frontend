...
class FullStackConfigurationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackConfigurationSystem(objective) {
    logger.info(
      `💻 [FullStackConfigurationConsultantAgent] Analyzing FullStack Configuration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Configuration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackConfigurationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackConfigurationConsultantAgent = Object.freeze(
  new FullStackConfigurationConsultantAgent(),
);
