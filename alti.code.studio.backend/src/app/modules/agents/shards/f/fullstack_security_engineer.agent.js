...
class FullStackSecurityEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackSecuritySystem(objective) {
    logger.info(
      `💻 [FullStackSecurityEngineerAgent] Analyzing FullStack Security Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Security Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackSecurityEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackSecurityEngineerAgent = Object.freeze(
  new FullStackSecurityEngineerAgent(),
);
