...
class FullStackAnalyticsDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackAnalyticsSystem(objective) {
        logger.info(`💻 [FullStackAnalyticsDirectorAgent] Analyzing FullStack Analytics Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Analytics Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackAnalyticsDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackAnalyticsDirectorAgent = Object.freeze(new FullStackAnalyticsDirectorAgent());
