...
class FrontendAutomationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendAutomationSystem(objective) {
    logger.info(
      `💻 [FrontendAutomationArchitectAgent] Analyzing Frontend Automation Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Automation Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendAutomationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendAutomationArchitectAgent = Object.freeze(
  new FrontendAutomationArchitectAgent(),
);
