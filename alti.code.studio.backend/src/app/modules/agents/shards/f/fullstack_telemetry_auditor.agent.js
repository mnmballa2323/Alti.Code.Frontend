...
class FullStackTelemetryAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackTelemetrySystem(objective) {
    logger.info(
      `💻 [FullStackTelemetryAuditorAgent] Analyzing FullStack Telemetry Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Telemetry Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackTelemetryAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackTelemetryAuditorAgent = Object.freeze(
  new FullStackTelemetryAuditorAgent(),
);
