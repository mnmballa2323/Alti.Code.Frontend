...
class FullStackTestingSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackTestingSystem(objective) {
        logger.info(`💻 [FullStackTestingSpecialistAgent] Analyzing FullStack Testing Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Testing Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackTestingSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackTestingSpecialistAgent = Object.freeze(new FullStackTestingSpecialistAgent());
