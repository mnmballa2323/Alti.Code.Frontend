...
class FullStackOrchestrationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackOrchestrationSystem(objective) {
    logger.info(
      `💻 [FullStackOrchestrationDesignerAgent] Analyzing FullStack Orchestration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Orchestration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackOrchestrationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackOrchestrationDesignerAgent = Object.freeze(
  new FullStackOrchestrationDesignerAgent(),
);
