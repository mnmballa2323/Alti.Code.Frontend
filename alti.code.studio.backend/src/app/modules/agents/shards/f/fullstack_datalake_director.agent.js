...
class FullStackDataLakeDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDataLakeSystem(objective) {
    logger.info(
      `💻 [FullStackDataLakeDirectorAgent] Analyzing FullStack DataLake Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack DataLake Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDataLakeDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDataLakeDirectorAgent = Object.freeze(
  new FullStackDataLakeDirectorAgent(),
);
