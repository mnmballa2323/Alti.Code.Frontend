...
class FrontendProvisioningEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendProvisioningSystem(objective) {
    logger.info(
      `💻 [FrontendProvisioningEngineerAgent] Analyzing Frontend Provisioning Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Provisioning Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendProvisioningEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendProvisioningEngineerAgent = Object.freeze(
  new FrontendProvisioningEngineerAgent(),
);
