...
class FrontendOrchestrationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendOrchestrationSystem(objective) {
    logger.info(
      `💻 [FrontendOrchestrationDirectorAgent] Analyzing Frontend Orchestration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Orchestration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendOrchestrationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendOrchestrationDirectorAgent = Object.freeze(
  new FrontendOrchestrationDirectorAgent(),
);
