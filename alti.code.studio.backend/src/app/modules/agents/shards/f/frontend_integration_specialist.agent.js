...
class FrontendIntegrationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendIntegrationSystem(objective) {
        logger.info(`💻 [FrontendIntegrationSpecialistAgent] Analyzing Frontend Integration Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Integration Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendIntegrationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendIntegrationSpecialistAgent = Object.freeze(new FrontendIntegrationSpecialistAgent());
