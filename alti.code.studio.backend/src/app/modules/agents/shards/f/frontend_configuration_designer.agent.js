...
class FrontendConfigurationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendConfigurationSystem(objective) {
    logger.info(
      `💻 [FrontendConfigurationDesignerAgent] Analyzing Frontend Configuration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Configuration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendConfigurationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendConfigurationDesignerAgent = Object.freeze(
  new FrontendConfigurationDesignerAgent(),
);
