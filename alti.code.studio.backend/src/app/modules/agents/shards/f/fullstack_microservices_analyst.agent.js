...
class FullStackMicroservicesAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackMicroservicesSystem(objective) {
        logger.info(`💻 [FullStackMicroservicesAnalystAgent] Analyzing FullStack Microservices Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Microservices Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackMicroservicesAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackMicroservicesAnalystAgent = Object.freeze(new FullStackMicroservicesAnalystAgent());
