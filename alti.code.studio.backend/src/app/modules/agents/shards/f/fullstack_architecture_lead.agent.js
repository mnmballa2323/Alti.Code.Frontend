...
class FullStackArchitectureLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackArchitectureSystem(objective) {
        logger.info(`💻 [FullStackArchitectureLeadAgent] Analyzing FullStack Architecture Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Architecture Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackArchitectureLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackArchitectureLeadAgent = Object.freeze(new FullStackArchitectureLeadAgent());
