...
class FrontendLoadBalancingTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendLoadBalancingSystem(objective) {
        logger.info(`💻 [FrontendLoadBalancingTesterAgent] Analyzing Frontend LoadBalancing Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend LoadBalancing Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendLoadBalancingTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendLoadBalancingTesterAgent = Object.freeze(new FrontendLoadBalancingTesterAgent());
