...
class FrontendGovernancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendGovernanceSystem(objective) {
    logger.info(
      `💻 [FrontendGovernancePlannerAgent] Analyzing Frontend Governance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Governance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendGovernancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendGovernancePlannerAgent = Object.freeze(
  new FrontendGovernancePlannerAgent(),
);
