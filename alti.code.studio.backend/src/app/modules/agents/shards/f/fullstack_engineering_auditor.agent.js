...
class FullStackEngineeringAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackEngineeringSystem(objective) {
    logger.info(
      `💻 [FullStackEngineeringAuditorAgent] Analyzing FullStack Engineering Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Engineering Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackEngineeringAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackEngineeringAuditorAgent = Object.freeze(
  new FullStackEngineeringAuditorAgent(),
);
