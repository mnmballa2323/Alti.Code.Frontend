...
class FrontendDataWarehouseAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendDataWarehouseSystem(objective) {
    logger.info(
      `💻 [FrontendDataWarehouseAuditorAgent] Analyzing Frontend DataWarehouse Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend DataWarehouse Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendDataWarehouseAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendDataWarehouseAuditorAgent = Object.freeze(
  new FrontendDataWarehouseAuditorAgent(),
);
