...
class FullStackOptimizationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackOptimizationSystem(objective) {
    logger.info(
      `💻 [FullStackOptimizationManagerAgent] Analyzing FullStack Optimization Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Optimization Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackOptimizationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackOptimizationManagerAgent = Object.freeze(
  new FullStackOptimizationManagerAgent(),
);
