...
class FullStackGovernanceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackGovernanceSystem(objective) {
    logger.info(
      `💻 [FullStackGovernanceConsultantAgent] Analyzing FullStack Governance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Governance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackGovernanceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackGovernanceConsultantAgent = Object.freeze(
  new FullStackGovernanceConsultantAgent(),
);
