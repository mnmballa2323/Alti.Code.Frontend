...
class FrontendGovernanceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendGovernanceSystem(objective) {
    logger.info(
      `💻 [FrontendGovernanceConsultantAgent] Analyzing Frontend Governance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Governance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendGovernanceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendGovernanceConsultantAgent = Object.freeze(
  new FrontendGovernanceConsultantAgent(),
);
