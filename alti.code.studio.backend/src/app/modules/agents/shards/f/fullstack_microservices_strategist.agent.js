...
class FullStackMicroservicesStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackMicroservicesSystem(objective) {
        logger.info(`💻 [FullStackMicroservicesStrategistAgent] Analyzing FullStack Microservices Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Microservices Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackMicroservicesStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackMicroservicesStrategistAgent = Object.freeze(new FullStackMicroservicesStrategistAgent());
