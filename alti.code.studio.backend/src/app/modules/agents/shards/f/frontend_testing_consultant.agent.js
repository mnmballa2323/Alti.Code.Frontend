...
class FrontendTestingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendTestingSystem(objective) {
        logger.info(`💻 [FrontendTestingConsultantAgent] Analyzing Frontend Testing Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Testing Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendTestingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendTestingConsultantAgent = Object.freeze(new FrontendTestingConsultantAgent());
