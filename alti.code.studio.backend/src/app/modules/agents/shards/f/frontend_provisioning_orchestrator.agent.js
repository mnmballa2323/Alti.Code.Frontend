...
class FrontendProvisioningOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendProvisioningSystem(objective) {
        logger.info(`💻 [FrontendProvisioningOrchestratorAgent] Analyzing Frontend Provisioning Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Provisioning Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendProvisioningOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendProvisioningOrchestratorAgent = Object.freeze(new FrontendProvisioningOrchestratorAgent());
