...
class FullStackTelemetryManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackTelemetrySystem(objective) {
    logger.info(
      `💻 [FullStackTelemetryManagerAgent] Analyzing FullStack Telemetry Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Telemetry Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackTelemetryManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackTelemetryManagerAgent = Object.freeze(
  new FullStackTelemetryManagerAgent(),
);
