...
class FullStackResilienceTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackResilienceSystem(objective) {
    logger.info(
      `💻 [FullStackResilienceTesterAgent] Analyzing FullStack Resilience Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Resilience Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackResilienceTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackResilienceTesterAgent = Object.freeze(
  new FullStackResilienceTesterAgent(),
);
