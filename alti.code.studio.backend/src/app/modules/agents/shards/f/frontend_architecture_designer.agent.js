...
class FrontendArchitectureDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendArchitectureSystem(objective) {
        logger.info(`💻 [FrontendArchitectureDesignerAgent] Analyzing Frontend Architecture Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Architecture Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendArchitectureDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendArchitectureDesignerAgent = Object.freeze(new FrontendArchitectureDesignerAgent());
