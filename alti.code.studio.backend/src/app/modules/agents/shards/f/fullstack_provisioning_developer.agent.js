...
class FullStackProvisioningDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackProvisioningSystem(objective) {
        logger.info(`💻 [FullStackProvisioningDeveloperAgent] Analyzing FullStack Provisioning Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Provisioning Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackProvisioningDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackProvisioningDeveloperAgent = Object.freeze(new FullStackProvisioningDeveloperAgent());
