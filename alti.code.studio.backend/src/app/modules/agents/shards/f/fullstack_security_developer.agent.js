...
class FullStackSecurityDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackSecuritySystem(objective) {
        logger.info(`💻 [FullStackSecurityDeveloperAgent] Analyzing FullStack Security Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Security Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackSecurityDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackSecurityDeveloperAgent = Object.freeze(new FullStackSecurityDeveloperAgent());
