...
class FrontendBatchProcessingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendBatchProcessingSystem(objective) {
    logger.info(
      `💻 [FrontendBatchProcessingDirectorAgent] Analyzing Frontend BatchProcessing Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend BatchProcessing Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendBatchProcessingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendBatchProcessingDirectorAgent = Object.freeze(
  new FrontendBatchProcessingDirectorAgent(),
);
