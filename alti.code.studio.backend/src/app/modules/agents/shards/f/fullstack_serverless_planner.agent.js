...
class FullStackServerlessPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackServerlessSystem(objective) {
    logger.info(
      `💻 [FullStackServerlessPlannerAgent] Analyzing FullStack Serverless Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Serverless Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackServerlessPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackServerlessPlannerAgent = Object.freeze(
  new FullStackServerlessPlannerAgent(),
);
