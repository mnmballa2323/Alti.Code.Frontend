...
class FrontendEngineeringDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendEngineeringSystem(objective) {
    logger.info(
      `💻 [FrontendEngineeringDirectorAgent] Analyzing Frontend Engineering Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Engineering Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendEngineeringDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendEngineeringDirectorAgent = Object.freeze(
  new FrontendEngineeringDirectorAgent(),
);
