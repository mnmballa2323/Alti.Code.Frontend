...
class FullStackFaultToleranceLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackFaultToleranceSystem(objective) {
        logger.info(`💻 [FullStackFaultToleranceLeadAgent] Analyzing FullStack FaultTolerance Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack FaultTolerance Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackFaultToleranceLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackFaultToleranceLeadAgent = Object.freeze(new FullStackFaultToleranceLeadAgent());
