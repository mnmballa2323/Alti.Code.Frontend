...
class FullStackVirtualizationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackVirtualizationSystem(objective) {
    logger.info(
      `💻 [FullStackVirtualizationManagerAgent] Analyzing FullStack Virtualization Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Virtualization Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackVirtualizationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackVirtualizationManagerAgent = Object.freeze(
  new FullStackVirtualizationManagerAgent(),
);
