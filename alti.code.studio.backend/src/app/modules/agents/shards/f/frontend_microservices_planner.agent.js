...
class FrontendMicroservicesPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendMicroservicesSystem(objective) {
        logger.info(`💻 [FrontendMicroservicesPlannerAgent] Analyzing Frontend Microservices Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Microservices Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendMicroservicesPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendMicroservicesPlannerAgent = Object.freeze(new FrontendMicroservicesPlannerAgent());
