...
class FrontendAnalyticsEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendAnalyticsSystem(objective) {
    logger.info(
      `💻 [FrontendAnalyticsEngineerAgent] Analyzing Frontend Analytics Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Analytics Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendAnalyticsEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendAnalyticsEngineerAgent = Object.freeze(
  new FrontendAnalyticsEngineerAgent(),
);
