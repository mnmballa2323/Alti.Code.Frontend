...
class FullStackIntegrationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackIntegrationSystem(objective) {
        logger.info(`💻 [FullStackIntegrationEngineerAgent] Analyzing FullStack Integration Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Integration Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackIntegrationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackIntegrationEngineerAgent = Object.freeze(new FullStackIntegrationEngineerAgent());
