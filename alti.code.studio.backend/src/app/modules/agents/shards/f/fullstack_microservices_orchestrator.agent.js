...
class FullStackMicroservicesOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackMicroservicesSystem(objective) {
    logger.info(
      `💻 [FullStackMicroservicesOrchestratorAgent] Analyzing FullStack Microservices Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Microservices Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackMicroservicesOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackMicroservicesOrchestratorAgent = Object.freeze(
  new FullStackMicroservicesOrchestratorAgent(),
);
