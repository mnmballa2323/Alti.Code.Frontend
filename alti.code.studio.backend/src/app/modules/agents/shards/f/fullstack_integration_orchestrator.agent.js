...
class FullStackIntegrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackIntegrationSystem(objective) {
    logger.info(
      `💻 [FullStackIntegrationOrchestratorAgent] Analyzing FullStack Integration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Integration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackIntegrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackIntegrationOrchestratorAgent = Object.freeze(
  new FullStackIntegrationOrchestratorAgent(),
);
