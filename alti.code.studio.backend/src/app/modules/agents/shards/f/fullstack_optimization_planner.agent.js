...
class FullStackOptimizationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackOptimizationSystem(objective) {
        logger.info(`💻 [FullStackOptimizationPlannerAgent] Analyzing FullStack Optimization Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Optimization Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackOptimizationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackOptimizationPlannerAgent = Object.freeze(new FullStackOptimizationPlannerAgent());
