...
class FullStackServerlessManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackServerlessSystem(objective) {
        logger.info(`💻 [FullStackServerlessManagerAgent] Analyzing FullStack Serverless Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Serverless Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackServerlessManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackServerlessManagerAgent = Object.freeze(new FullStackServerlessManagerAgent());
