...
class FrontendServerlessAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendServerlessSystem(objective) {
        logger.info(`💻 [FrontendServerlessAuditorAgent] Analyzing Frontend Serverless Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Serverless Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendServerlessAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendServerlessAuditorAgent = Object.freeze(new FrontendServerlessAuditorAgent());
