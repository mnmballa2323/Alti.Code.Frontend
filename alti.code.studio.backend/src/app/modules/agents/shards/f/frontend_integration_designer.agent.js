...
class FrontendIntegrationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendIntegrationSystem(objective) {
        logger.info(`💻 [FrontendIntegrationDesignerAgent] Analyzing Frontend Integration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Integration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendIntegrationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendIntegrationDesignerAgent = Object.freeze(new FrontendIntegrationDesignerAgent());
