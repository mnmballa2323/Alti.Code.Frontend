...
class FrontendMigrationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendMigrationSystem(objective) {
    logger.info(
      `💻 [FrontendMigrationDesignerAgent] Analyzing Frontend Migration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Migration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendMigrationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendMigrationDesignerAgent = Object.freeze(
  new FrontendMigrationDesignerAgent(),
);
