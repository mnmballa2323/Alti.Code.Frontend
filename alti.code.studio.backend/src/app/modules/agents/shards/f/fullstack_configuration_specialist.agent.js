...
class FullStackConfigurationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackConfigurationSystem(objective) {
    logger.info(
      `💻 [FullStackConfigurationSpecialistAgent] Analyzing FullStack Configuration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Configuration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackConfigurationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackConfigurationSpecialistAgent = Object.freeze(
  new FullStackConfigurationSpecialistAgent(),
);
