...
class FrontendProvisioningStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendProvisioningSystem(objective) {
    logger.info(
      `💻 [FrontendProvisioningStrategistAgent] Analyzing Frontend Provisioning Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Provisioning Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendProvisioningStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendProvisioningStrategistAgent = Object.freeze(
  new FrontendProvisioningStrategistAgent(),
);
