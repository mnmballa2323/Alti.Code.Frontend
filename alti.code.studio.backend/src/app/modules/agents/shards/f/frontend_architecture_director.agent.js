...
class FrontendArchitectureDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendArchitectureSystem(objective) {
        logger.info(`💻 [FrontendArchitectureDirectorAgent] Analyzing Frontend Architecture Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Architecture Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendArchitectureDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendArchitectureDirectorAgent = Object.freeze(new FrontendArchitectureDirectorAgent());
