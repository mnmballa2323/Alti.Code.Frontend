...
class FrontendCachingOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendCachingSystem(objective) {
        logger.info(`💻 [FrontendCachingOrchestratorAgent] Analyzing Frontend Caching Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Caching Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendCachingOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendCachingOrchestratorAgent = Object.freeze(new FrontendCachingOrchestratorAgent());
