...
class FrontendOptimizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendOptimizationSystem(objective) {
    logger.info(
      `💻 [FrontendOptimizationDirectorAgent] Analyzing Frontend Optimization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Optimization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendOptimizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendOptimizationDirectorAgent = Object.freeze(
  new FrontendOptimizationDirectorAgent(),
);
