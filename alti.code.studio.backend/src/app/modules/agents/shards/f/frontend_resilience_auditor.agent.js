...
class FrontendResilienceAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendResilienceSystem(objective) {
    logger.info(
      `💻 [FrontendResilienceAuditorAgent] Analyzing Frontend Resilience Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Resilience Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendResilienceAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendResilienceAuditorAgent = Object.freeze(
  new FrontendResilienceAuditorAgent(),
);
