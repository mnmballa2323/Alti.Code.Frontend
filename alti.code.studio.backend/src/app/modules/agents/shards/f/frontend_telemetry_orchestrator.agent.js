...
class FrontendTelemetryOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendTelemetrySystem(objective) {
    logger.info(
      `💻 [FrontendTelemetryOrchestratorAgent] Analyzing Frontend Telemetry Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Telemetry Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendTelemetryOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendTelemetryOrchestratorAgent = Object.freeze(
  new FrontendTelemetryOrchestratorAgent(),
);
