...
class FrontendStreamingEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendStreamingSystem(objective) {
        logger.info(`💻 [FrontendStreamingEngineerAgent] Analyzing Frontend Streaming Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Streaming Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendStreamingEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendStreamingEngineerAgent = Object.freeze(new FrontendStreamingEngineerAgent());
