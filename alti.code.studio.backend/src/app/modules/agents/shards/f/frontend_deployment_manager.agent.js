...
class FrontendDeploymentManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendDeploymentSystem(objective) {
    logger.info(
      `💻 [FrontendDeploymentManagerAgent] Analyzing Frontend Deployment Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Deployment Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendDeploymentManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendDeploymentManagerAgent = Object.freeze(
  new FrontendDeploymentManagerAgent(),
);
