...
class FrontendEngineeringEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendEngineeringSystem(objective) {
    logger.info(
      `💻 [FrontendEngineeringEngineerAgent] Analyzing Frontend Engineering Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Engineering Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendEngineeringEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendEngineeringEngineerAgent = Object.freeze(
  new FrontendEngineeringEngineerAgent(),
);
