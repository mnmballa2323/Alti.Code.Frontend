...
class FrontendContainerizationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendContainerizationSystem(objective) {
    logger.info(
      `💻 [FrontendContainerizationEngineerAgent] Analyzing Frontend Containerization Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Containerization Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendContainerizationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendContainerizationEngineerAgent = Object.freeze(
  new FrontendContainerizationEngineerAgent(),
);
