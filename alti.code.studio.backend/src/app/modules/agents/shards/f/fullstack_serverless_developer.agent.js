...
class FullStackServerlessDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackServerlessSystem(objective) {
    logger.info(
      `💻 [FullStackServerlessDeveloperAgent] Analyzing FullStack Serverless Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Serverless Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackServerlessDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackServerlessDeveloperAgent = Object.freeze(
  new FullStackServerlessDeveloperAgent(),
);
