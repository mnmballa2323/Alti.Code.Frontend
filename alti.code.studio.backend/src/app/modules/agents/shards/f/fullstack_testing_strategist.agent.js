...
class FullStackTestingStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackTestingSystem(objective) {
        logger.info(`💻 [FullStackTestingStrategistAgent] Analyzing FullStack Testing Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Testing Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackTestingStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackTestingStrategistAgent = Object.freeze(new FullStackTestingStrategistAgent());
