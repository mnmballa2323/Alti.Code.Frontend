...
class FrontendEngineeringTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendEngineeringSystem(objective) {
    logger.info(
      `💻 [FrontendEngineeringTesterAgent] Analyzing Frontend Engineering Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Engineering Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendEngineeringTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendEngineeringTesterAgent = Object.freeze(
  new FrontendEngineeringTesterAgent(),
);
