...
class FullStackObservabilityDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackObservabilitySystem(objective) {
    logger.info(
      `💻 [FullStackObservabilityDirectorAgent] Analyzing FullStack Observability Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Observability Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackObservabilityDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackObservabilityDirectorAgent = Object.freeze(
  new FullStackObservabilityDirectorAgent(),
);
