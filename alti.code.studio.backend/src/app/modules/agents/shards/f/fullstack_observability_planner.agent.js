...
class FullStackObservabilityPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackObservabilitySystem(objective) {
    logger.info(
      `💻 [FullStackObservabilityPlannerAgent] Analyzing FullStack Observability Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Observability Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackObservabilityPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackObservabilityPlannerAgent = Object.freeze(
  new FullStackObservabilityPlannerAgent(),
);
