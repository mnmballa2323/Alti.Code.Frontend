...
class FrontendFaultTolerancePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendFaultToleranceSystem(objective) {
        logger.info(`💻 [FrontendFaultTolerancePlannerAgent] Analyzing Frontend FaultTolerance Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend FaultTolerance Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendFaultTolerancePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendFaultTolerancePlannerAgent = Object.freeze(new FrontendFaultTolerancePlannerAgent());
