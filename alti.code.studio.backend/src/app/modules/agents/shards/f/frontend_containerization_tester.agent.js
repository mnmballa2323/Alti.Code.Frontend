...
class FrontendContainerizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendContainerizationSystem(objective) {
    logger.info(
      `💻 [FrontendContainerizationTesterAgent] Analyzing Frontend Containerization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Containerization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendContainerizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendContainerizationTesterAgent = Object.freeze(
  new FrontendContainerizationTesterAgent(),
);
