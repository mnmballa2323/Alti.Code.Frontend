...
class FrontendStreamingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendStreamingSystem(objective) {
    logger.info(
      `💻 [FrontendStreamingOrchestratorAgent] Analyzing Frontend Streaming Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Streaming Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendStreamingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendStreamingOrchestratorAgent = Object.freeze(
  new FrontendStreamingOrchestratorAgent(),
);
