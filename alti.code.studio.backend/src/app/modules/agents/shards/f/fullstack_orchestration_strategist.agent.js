...
class FullStackOrchestrationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackOrchestrationSystem(objective) {
        logger.info(`💻 [FullStackOrchestrationStrategistAgent] Analyzing FullStack Orchestration Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Orchestration Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackOrchestrationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackOrchestrationStrategistAgent = Object.freeze(new FullStackOrchestrationStrategistAgent());
