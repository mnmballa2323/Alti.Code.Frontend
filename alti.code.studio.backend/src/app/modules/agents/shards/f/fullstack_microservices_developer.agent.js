...
class FullStackMicroservicesDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackMicroservicesSystem(objective) {
    logger.info(
      `💻 [FullStackMicroservicesDeveloperAgent] Analyzing FullStack Microservices Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Microservices Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackMicroservicesDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackMicroservicesDeveloperAgent = Object.freeze(
  new FullStackMicroservicesDeveloperAgent(),
);
