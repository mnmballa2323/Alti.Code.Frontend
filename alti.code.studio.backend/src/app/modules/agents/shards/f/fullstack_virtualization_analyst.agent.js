...
class FullStackVirtualizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackVirtualizationSystem(objective) {
    logger.info(
      `💻 [FullStackVirtualizationAnalystAgent] Analyzing FullStack Virtualization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Virtualization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackVirtualizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackVirtualizationAnalystAgent = Object.freeze(
  new FullStackVirtualizationAnalystAgent(),
);
