...
class FullStackResilienceArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackResilienceSystem(objective) {
    logger.info(
      `💻 [FullStackResilienceArchitectAgent] Analyzing FullStack Resilience Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Resilience Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackResilienceArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackResilienceArchitectAgent = Object.freeze(
  new FullStackResilienceArchitectAgent(),
);
