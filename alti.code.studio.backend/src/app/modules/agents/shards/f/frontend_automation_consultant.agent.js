...
class FrontendAutomationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendAutomationSystem(objective) {
    logger.info(
      `💻 [FrontendAutomationConsultantAgent] Analyzing Frontend Automation Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Automation Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendAutomationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendAutomationConsultantAgent = Object.freeze(
  new FrontendAutomationConsultantAgent(),
);
