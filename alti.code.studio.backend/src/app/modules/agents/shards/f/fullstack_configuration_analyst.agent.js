...
class FullStackConfigurationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackConfigurationSystem(objective) {
    logger.info(
      `💻 [FullStackConfigurationAnalystAgent] Analyzing FullStack Configuration Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Configuration Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackConfigurationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackConfigurationAnalystAgent = Object.freeze(
  new FullStackConfigurationAnalystAgent(),
);
