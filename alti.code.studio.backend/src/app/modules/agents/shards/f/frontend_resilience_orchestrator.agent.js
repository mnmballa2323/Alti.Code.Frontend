...
class FrontendResilienceOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendResilienceSystem(objective) {
        logger.info(`💻 [FrontendResilienceOrchestratorAgent] Analyzing Frontend Resilience Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Resilience Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendResilienceOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendResilienceOrchestratorAgent = Object.freeze(new FrontendResilienceOrchestratorAgent());
