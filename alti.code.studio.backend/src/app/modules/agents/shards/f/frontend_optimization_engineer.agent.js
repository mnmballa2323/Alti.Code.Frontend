...
class FrontendOptimizationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendOptimizationSystem(objective) {
    logger.info(
      `💻 [FrontendOptimizationEngineerAgent] Analyzing Frontend Optimization Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Optimization Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendOptimizationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendOptimizationEngineerAgent = Object.freeze(
  new FrontendOptimizationEngineerAgent(),
);
