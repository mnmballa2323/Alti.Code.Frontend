...
class FullStackDeploymentDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackDeploymentSystem(objective) {
    logger.info(
      `💻 [FullStackDeploymentDesignerAgent] Analyzing FullStack Deployment Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Deployment Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackDeploymentDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackDeploymentDesignerAgent = Object.freeze(
  new FullStackDeploymentDesignerAgent(),
);
