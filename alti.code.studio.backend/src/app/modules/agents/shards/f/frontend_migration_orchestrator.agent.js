...
class FrontendMigrationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendMigrationSystem(objective) {
        logger.info(`💻 [FrontendMigrationOrchestratorAgent] Analyzing Frontend Migration Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Migration Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendMigrationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendMigrationOrchestratorAgent = Object.freeze(new FrontendMigrationOrchestratorAgent());
