...
class FullStackMicroservicesAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackMicroservicesSystem(objective) {
    logger.info(
      `💻 [FullStackMicroservicesAuditorAgent] Analyzing FullStack Microservices Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Microservices Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackMicroservicesAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackMicroservicesAuditorAgent = Object.freeze(
  new FullStackMicroservicesAuditorAgent(),
);
