...
class FullStackMicroservicesConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackMicroservicesSystem(objective) {
    logger.info(
      `💻 [FullStackMicroservicesConsultantAgent] Analyzing FullStack Microservices Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Microservices Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackMicroservicesConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackMicroservicesConsultantAgent = Object.freeze(
  new FullStackMicroservicesConsultantAgent(),
);
