...
class FullStackMicroservicesLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateFullStackMicroservicesSystem(objective) {
        logger.info(`💻 [FullStackMicroservicesLeadAgent] Analyzing FullStack Microservices Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - FullStack Microservices Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FullStackMicroservicesLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const fullStackMicroservicesLeadAgent = Object.freeze(new FullStackMicroservicesLeadAgent());
