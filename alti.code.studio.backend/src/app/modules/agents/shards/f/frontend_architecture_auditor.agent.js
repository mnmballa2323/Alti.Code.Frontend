...
class FrontendArchitectureAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendArchitectureSystem(objective) {
    logger.info(
      `💻 [FrontendArchitectureAuditorAgent] Analyzing Frontend Architecture Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Architecture Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendArchitectureAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendArchitectureAuditorAgent = Object.freeze(
  new FrontendArchitectureAuditorAgent(),
);
