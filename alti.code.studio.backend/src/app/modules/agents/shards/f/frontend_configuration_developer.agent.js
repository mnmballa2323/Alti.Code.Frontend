...
class FrontendConfigurationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateFrontendConfigurationSystem(objective) {
        logger.info(`💻 [FrontendConfigurationDeveloperAgent] Analyzing Frontend Configuration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Frontend Configuration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [FrontendConfigurationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const frontendConfigurationDeveloperAgent = Object.freeze(new FrontendConfigurationDeveloperAgent());
