...
class FrontendArchitectureEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateFrontendArchitectureSystem(objective) {
    logger.info(
      `💻 [FrontendArchitectureEngineerAgent] Analyzing Frontend Architecture Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Frontend Architecture Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FrontendArchitectureEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const frontendArchitectureEngineerAgent = Object.freeze(
  new FrontendArchitectureEngineerAgent(),
);
