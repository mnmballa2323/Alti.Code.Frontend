...
class FullStackContainerizationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateFullStackContainerizationSystem(objective) {
    logger.info(
      `💻 [FullStackContainerizationOrchestratorAgent] Analyzing FullStack Containerization Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - FullStack Containerization Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [FullStackContainerizationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const fullStackContainerizationOrchestratorAgent = Object.freeze(
  new FullStackContainerizationOrchestratorAgent(),
);
