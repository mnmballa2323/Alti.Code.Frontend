...
class TheGraphAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.description =
      'Blockchain indexing specialist for The Graph: subgraph development (schema.graphql, mappings.ts, subgraph.yaml), GraphQL API queries, subgraph deployment, Substreams, and decentralized querying via The Graph Network.';
...
When writing code, output robust AssemblyScript mapping layers organically paired with hyper-structured GraphQL definitions. Explicitly emphasize the decentralized execution network via The Graph Network rather than deprecated hosted services natively.`;
  ...
  async consult(prompt, contextData = []) {
    logger.info(
      `📊 The Graph Expert: Synthesizing blockchain indexing logic...`,
    );
