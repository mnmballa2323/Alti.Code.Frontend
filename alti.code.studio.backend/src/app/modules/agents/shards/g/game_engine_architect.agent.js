...
class GameEngineArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateEngineComponent(engineRequirement) {
    logger.info(
      `🎮 [GameEngineArchitect] Synthesizing extreme-performance C++ Game Engine component...`,
    );
...
${engineRequirement}
    ...
    try {
      ...
      logger.info(
        `✅ [GameEngineArchitect] Game Engine C++ component generated successfully.`,
      );
      return cleanCpp;
    } catch (err) {
      logger.error(
        `❌ [GameEngineArchitect] Failed to generate engine component: ${err.message}`,
      );
      throw err;
    }
  }
...
export const gameEngineArchitectAgent = Object.freeze(
  new GameEngineArchitectAgent(),
);
