...
class GpuKernelEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateCudaKernel(mathOperation) {
    logger.info(
      `🖥️ [GpuKernelEngineer] Analyzing mathematical bottleneck for CUDA parallelization...`,
    );
...
${mathOperation}
    ...
    try {
      ...
      logger.error(
        `❌ [GpuKernelEngineer] Failed to generate CUDA kernel: ${err.message}`,
      );
      throw err;
    }
  }
...
export const gpuKernelEngineerAgent = Object.freeze(
  new GpuKernelEngineerAgent(),
);
