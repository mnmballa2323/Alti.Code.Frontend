...
class GeopoliticalIntelligenceAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateIntelligenceSystem(osintObjective) {
    logger.info(
      `🌍 [GeopoliticalIntelligenceAnalyst] Analyzing objective for OSINT and geopolitical supply chain mapping...`,
    );
...
${osintObjective}
    ...
    try {
      ...
      const cleanCode = output
        .replace(
          /```javascript|```typescript|```json|```python|```cypher|```/gi,
          '',
        )
        .trim();
      logger.info(
        `✅ [GeopoliticalIntelligenceAnalyst] Intelligence architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [GeopoliticalIntelligenceAnalyst] Failed to generate intelligence system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const geopoliticalIntelligenceAnalystAgent = Object.freeze(
  new GeopoliticalIntelligenceAnalystAgent(),
);
