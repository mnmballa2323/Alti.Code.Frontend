...
class BackendProvisioningAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendProvisioningSystem(objective) {
    logger.info(
      `💻 [BackendProvisioningAuditorAgent] Analyzing Backend Provisioning Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Provisioning Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendProvisioningAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendProvisioningAuditorAgent = Object.freeze(
  new BackendProvisioningAuditorAgent(),
);
