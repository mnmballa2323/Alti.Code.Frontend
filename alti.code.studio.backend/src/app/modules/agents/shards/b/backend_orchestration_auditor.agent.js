...
class BackendOrchestrationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendOrchestrationSystem(objective) {
        logger.info(`💻 [BackendOrchestrationAuditorAgent] Analyzing Backend Orchestration Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Orchestration Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendOrchestrationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendOrchestrationAuditorAgent = Object.freeze(new BackendOrchestrationAuditorAgent());
