...
class BackendDataWarehouseArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDataWarehouseSystem(objective) {
    logger.info(
      `💻 [BackendDataWarehouseArchitectAgent] Analyzing Backend DataWarehouse Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend DataWarehouse Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendDataWarehouseArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendDataWarehouseArchitectAgent = Object.freeze(
  new BackendDataWarehouseArchitectAgent(),
);
