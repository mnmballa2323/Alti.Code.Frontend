...
class BackendDataWarehouseAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendDataWarehouseSystem(objective) {
        logger.info(`💻 [BackendDataWarehouseAuditorAgent] Analyzing Backend DataWarehouse Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend DataWarehouse Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendDataWarehouseAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendDataWarehouseAuditorAgent = Object.freeze(new BackendDataWarehouseAuditorAgent());
