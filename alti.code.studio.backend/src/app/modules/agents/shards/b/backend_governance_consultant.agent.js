...
class BackendGovernanceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendGovernanceSystem(objective) {
        logger.info(`💻 [BackendGovernanceConsultantAgent] Analyzing Backend Governance Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Governance Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendGovernanceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendGovernanceConsultantAgent = Object.freeze(new BackendGovernanceConsultantAgent());
