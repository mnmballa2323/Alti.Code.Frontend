...
class BackendProvisioningDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendProvisioningSystem(objective) {
        logger.info(`💻 [BackendProvisioningDesignerAgent] Analyzing Backend Provisioning Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Provisioning Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendProvisioningDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendProvisioningDesignerAgent = Object.freeze(new BackendProvisioningDesignerAgent());
