...
class BackendAnalyticsTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendAnalyticsSystem(objective) {
    logger.info(
      `💻 [BackendAnalyticsTesterAgent] Analyzing Backend Analytics Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Analytics Tester Target',
      );
...
export const backendAnalyticsTesterAgent = Object.freeze(
  new BackendAnalyticsTesterAgent(),
);
