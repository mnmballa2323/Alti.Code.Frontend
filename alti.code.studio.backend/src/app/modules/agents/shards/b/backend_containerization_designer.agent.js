...
class BackendContainerizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendContainerizationSystem(objective) {
    logger.info(
      `💻 [BackendContainerizationDesignerAgent] Analyzing Backend Containerization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Containerization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendContainerizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendContainerizationDesignerAgent = Object.freeze(
  new BackendContainerizationDesignerAgent(),
);
