...
class BackendObservabilityEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendObservabilitySystem(objective) {
    logger.info(
      `💻 [BackendObservabilityEngineerAgent] Analyzing Backend Observability Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Observability Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendObservabilityEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendObservabilityEngineerAgent = Object.freeze(
  new BackendObservabilityEngineerAgent(),
);
