...
class BackendLoadBalancingArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendLoadBalancingSystem(objective) {
    logger.info(
      `💻 [BackendLoadBalancingArchitectAgent] Analyzing Backend LoadBalancing Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend LoadBalancing Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendLoadBalancingArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendLoadBalancingArchitectAgent = Object.freeze(
  new BackendLoadBalancingArchitectAgent(),
);
