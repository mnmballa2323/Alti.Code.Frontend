...
class BackendConfigurationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendConfigurationSystem(objective) {
        logger.info(`💻 [BackendConfigurationDeveloperAgent] Analyzing Backend Configuration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Configuration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendConfigurationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendConfigurationDeveloperAgent = Object.freeze(new BackendConfigurationDeveloperAgent());
