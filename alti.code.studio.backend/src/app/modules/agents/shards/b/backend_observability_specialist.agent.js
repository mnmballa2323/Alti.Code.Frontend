...
class BackendObservabilitySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendObservabilitySystem(objective) {
    logger.info(
      `💻 [BackendObservabilitySpecialistAgent] Analyzing Backend Observability Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Observability Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendObservabilitySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendObservabilitySpecialistAgent = Object.freeze(
  new BackendObservabilitySpecialistAgent(),
);
