...
class BackendStreamingStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendStreamingSystem(objective) {
    logger.info(
      `💻 [BackendStreamingStrategistAgent] Analyzing Backend Streaming Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Streaming Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendStreamingStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendStreamingStrategistAgent = Object.freeze(
  new BackendStreamingStrategistAgent(),
);
