...
class BackendLoadBalancingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendLoadBalancingSystem(objective) {
    logger.info(
      `💻 [BackendLoadBalancingManagerAgent] Analyzing Backend LoadBalancing Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend LoadBalancing Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendLoadBalancingManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendLoadBalancingManagerAgent = Object.freeze(
  new BackendLoadBalancingManagerAgent(),
);
