...
class BackendProvisioningSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendProvisioningSystem(objective) {
        logger.info(`💻 [BackendProvisioningSpecialistAgent] Analyzing Backend Provisioning Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Provisioning Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendProvisioningSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendProvisioningSpecialistAgent = Object.freeze(new BackendProvisioningSpecialistAgent());
