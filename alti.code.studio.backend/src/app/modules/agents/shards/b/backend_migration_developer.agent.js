...
class BackendMigrationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendMigrationSystem(objective) {
        logger.info(`💻 [BackendMigrationDeveloperAgent] Analyzing Backend Migration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Migration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendMigrationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendMigrationDeveloperAgent = Object.freeze(new BackendMigrationDeveloperAgent());
