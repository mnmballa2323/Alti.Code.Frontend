...
class BackendOptimizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendOptimizationSystem(objective) {
    logger.info(
      `💻 [BackendOptimizationTesterAgent] Analyzing Backend Optimization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Optimization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendOptimizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendOptimizationTesterAgent = Object.freeze(
  new BackendOptimizationTesterAgent(),
);
