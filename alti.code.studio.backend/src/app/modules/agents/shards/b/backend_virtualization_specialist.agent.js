...
class BackendVirtualizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendVirtualizationSystem(objective) {
        logger.info(`💻 [BackendVirtualizationSpecialistAgent] Analyzing Backend Virtualization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Virtualization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendVirtualizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendVirtualizationSpecialistAgent = Object.freeze(new BackendVirtualizationSpecialistAgent());
