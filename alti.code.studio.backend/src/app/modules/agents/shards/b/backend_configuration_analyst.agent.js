...
class BackendConfigurationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendConfigurationSystem(objective) {
    logger.info(
      `💻 [BackendConfigurationAnalystAgent] Analyzing Backend Configuration Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Configuration Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendConfigurationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendConfigurationAnalystAgent = Object.freeze(
  new BackendConfigurationAnalystAgent(),
);
