...
class BackendDataWarehouseSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendDataWarehouseSystem(objective) {
        logger.info(`💻 [BackendDataWarehouseSpecialistAgent] Analyzing Backend DataWarehouse Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend DataWarehouse Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendDataWarehouseSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendDataWarehouseSpecialistAgent = Object.freeze(new BackendDataWarehouseSpecialistAgent());
