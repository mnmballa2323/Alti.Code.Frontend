...
class BackendBatchProcessingTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendBatchProcessingSystem(objective) {
    logger.info(
      `💻 [BackendBatchProcessingTesterAgent] Analyzing Backend BatchProcessing Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend BatchProcessing Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendBatchProcessingTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendBatchProcessingTesterAgent = Object.freeze(
  new BackendBatchProcessingTesterAgent(),
);
