...
class BackendConfigurationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendConfigurationSystem(objective) {
    logger.info(
      `💻 [BackendConfigurationConsultantAgent] Analyzing Backend Configuration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Configuration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendConfigurationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendConfigurationConsultantAgent = Object.freeze(
  new BackendConfigurationConsultantAgent(),
);
