...
class BackendProvisioningOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendProvisioningSystem(objective) {
    logger.info(
      `💻 [BackendProvisioningOrchestratorAgent] Analyzing Backend Provisioning Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Provisioning Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendProvisioningOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendProvisioningOrchestratorAgent = Object.freeze(
  new BackendProvisioningOrchestratorAgent(),
);
