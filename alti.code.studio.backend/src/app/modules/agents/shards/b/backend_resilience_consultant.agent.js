...
class BackendResilienceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendResilienceSystem(objective) {
        logger.info(`💻 [BackendResilienceConsultantAgent] Analyzing Backend Resilience Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Resilience Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendResilienceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendResilienceConsultantAgent = Object.freeze(new BackendResilienceConsultantAgent());
