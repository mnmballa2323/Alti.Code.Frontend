...
class BackendGovernanceArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendGovernanceSystem(objective) {
    logger.info(
      `💻 [BackendGovernanceArchitectAgent] Analyzing Backend Governance Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Governance Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendGovernanceArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendGovernanceArchitectAgent = Object.freeze(
  new BackendGovernanceArchitectAgent(),
);
