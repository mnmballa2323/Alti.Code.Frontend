...
class BackendArchitectureAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendArchitectureSystem(objective) {
    logger.info(
      `💻 [BackendArchitectureAnalystAgent] Analyzing Backend Architecture Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Architecture Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendArchitectureAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendArchitectureAnalystAgent = Object.freeze(
  new BackendArchitectureAnalystAgent(),
);
