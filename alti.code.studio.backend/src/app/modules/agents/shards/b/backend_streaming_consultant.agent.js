...
class BackendStreamingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendStreamingSystem(objective) {
    logger.info(
      `💻 [BackendStreamingConsultantAgent] Analyzing Backend Streaming Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Streaming Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendStreamingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendStreamingConsultantAgent = Object.freeze(
  new BackendStreamingConsultantAgent(),
);
