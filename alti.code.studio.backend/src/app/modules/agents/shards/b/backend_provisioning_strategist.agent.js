...
class BackendProvisioningStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendProvisioningSystem(objective) {
    logger.info(
      `💻 [BackendProvisioningStrategistAgent] Analyzing Backend Provisioning Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Provisioning Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendProvisioningStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendProvisioningStrategistAgent = Object.freeze(
  new BackendProvisioningStrategistAgent(),
);
