...
class BackendEngineeringDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendEngineeringSystem(objective) {
    logger.info(
      `💻 [BackendEngineeringDesignerAgent] Analyzing Backend Engineering Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Engineering Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendEngineeringDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendEngineeringDesignerAgent = Object.freeze(
  new BackendEngineeringDesignerAgent(),
);
