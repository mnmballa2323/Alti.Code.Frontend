...
class BackendServerlessOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendServerlessSystem(objective) {
        logger.info(`💻 [BackendServerlessOrchestratorAgent] Analyzing Backend Serverless Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Serverless Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendServerlessOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendServerlessOrchestratorAgent = Object.freeze(new BackendServerlessOrchestratorAgent());
