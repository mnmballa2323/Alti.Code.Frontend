...
class BackendObservabilityArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendObservabilitySystem(objective) {
    logger.info(
      `💻 [BackendObservabilityArchitectAgent] Analyzing Backend Observability Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Observability Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendObservabilityArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendObservabilityArchitectAgent = Object.freeze(
  new BackendObservabilityArchitectAgent(),
);
