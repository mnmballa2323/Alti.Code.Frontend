...
class BackendMicroservicesConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendMicroservicesSystem(objective) {
        logger.info(`💻 [BackendMicroservicesConsultantAgent] Analyzing Backend Microservices Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Microservices Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendMicroservicesConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendMicroservicesConsultantAgent = Object.freeze(new BackendMicroservicesConsultantAgent());
