...
class BackendAnalyticsDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendAnalyticsSystem(objective) {
        logger.info(`💻 [BackendAnalyticsDeveloperAgent] Analyzing Backend Analytics Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Analytics Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendAnalyticsDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendAnalyticsDeveloperAgent = Object.freeze(new BackendAnalyticsDeveloperAgent());
