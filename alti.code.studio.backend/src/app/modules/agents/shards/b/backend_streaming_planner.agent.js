...
class BackendStreamingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendStreamingSystem(objective) {
    logger.info(
      `💻 [BackendStreamingPlannerAgent] Analyzing Backend Streaming Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Streaming Planner Target',
      );
...
export const backendStreamingPlannerAgent = Object.freeze(
  new BackendStreamingPlannerAgent(),
);
