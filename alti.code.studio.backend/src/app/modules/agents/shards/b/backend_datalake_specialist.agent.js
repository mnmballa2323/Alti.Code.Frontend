...
class BackendDataLakeSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDataLakeSystem(objective) {
    logger.info(
      `💻 [BackendDataLakeSpecialistAgent] Analyzing Backend DataLake Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend DataLake Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendDataLakeSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendDataLakeSpecialistAgent = Object.freeze(
  new BackendDataLakeSpecialistAgent(),
);
