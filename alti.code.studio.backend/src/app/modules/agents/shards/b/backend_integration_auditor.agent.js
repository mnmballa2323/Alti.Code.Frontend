...
class BackendIntegrationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendIntegrationSystem(objective) {
    logger.info(
      `💻 [BackendIntegrationAuditorAgent] Analyzing Backend Integration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Integration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendIntegrationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendIntegrationAuditorAgent = Object.freeze(
  new BackendIntegrationAuditorAgent(),
);
