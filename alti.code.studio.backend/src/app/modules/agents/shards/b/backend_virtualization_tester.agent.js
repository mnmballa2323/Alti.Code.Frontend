...
class BackendVirtualizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendVirtualizationSystem(objective) {
    logger.info(
      `💻 [BackendVirtualizationTesterAgent] Analyzing Backend Virtualization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Virtualization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendVirtualizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendVirtualizationTesterAgent = Object.freeze(
  new BackendVirtualizationTesterAgent(),
);
