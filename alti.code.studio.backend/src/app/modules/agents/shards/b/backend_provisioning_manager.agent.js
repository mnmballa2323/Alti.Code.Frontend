...
class BackendProvisioningManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendProvisioningSystem(objective) {
    logger.info(
      `💻 [BackendProvisioningManagerAgent] Analyzing Backend Provisioning Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Provisioning Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendProvisioningManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendProvisioningManagerAgent = Object.freeze(
  new BackendProvisioningManagerAgent(),
);
