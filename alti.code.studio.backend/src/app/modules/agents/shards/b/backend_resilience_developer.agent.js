...
class BackendResilienceDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendResilienceSystem(objective) {
        logger.info(`💻 [BackendResilienceDeveloperAgent] Analyzing Backend Resilience Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Resilience Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendResilienceDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendResilienceDeveloperAgent = Object.freeze(new BackendResilienceDeveloperAgent());
