...
class BackendLoadBalancingPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendLoadBalancingSystem(objective) {
        logger.info(`💻 [BackendLoadBalancingPlannerAgent] Analyzing Backend LoadBalancing Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend LoadBalancing Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendLoadBalancingPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendLoadBalancingPlannerAgent = Object.freeze(new BackendLoadBalancingPlannerAgent());
