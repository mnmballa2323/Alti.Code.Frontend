...
class BackendOptimizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendOptimizationSystem(objective) {
        logger.info(`💻 [BackendOptimizationSpecialistAgent] Analyzing Backend Optimization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Optimization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendOptimizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendOptimizationSpecialistAgent = Object.freeze(new BackendOptimizationSpecialistAgent());
