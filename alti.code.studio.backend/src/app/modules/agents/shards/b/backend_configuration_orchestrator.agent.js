...
class BackendConfigurationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendConfigurationSystem(objective) {
        logger.info(`💻 [BackendConfigurationOrchestratorAgent] Analyzing Backend Configuration Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Configuration Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendConfigurationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendConfigurationOrchestratorAgent = Object.freeze(new BackendConfigurationOrchestratorAgent());
