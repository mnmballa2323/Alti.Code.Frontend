...
class BackendComplianceDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendComplianceSystem(objective) {
        logger.info(`💻 [BackendComplianceDesignerAgent] Analyzing Backend Compliance Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Compliance Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendComplianceDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendComplianceDesignerAgent = Object.freeze(new BackendComplianceDesignerAgent());
