...
class BackendVirtualizationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendVirtualizationSystem(objective) {
        logger.info(`💻 [BackendVirtualizationDesignerAgent] Analyzing Backend Virtualization Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Virtualization Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendVirtualizationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendVirtualizationDesignerAgent = Object.freeze(new BackendVirtualizationDesignerAgent());
