...
class BackendObservabilityDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendObservabilitySystem(objective) {
        logger.info(`💻 [BackendObservabilityDesignerAgent] Analyzing Backend Observability Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Observability Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendObservabilityDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendObservabilityDesignerAgent = Object.freeze(new BackendObservabilityDesignerAgent());
