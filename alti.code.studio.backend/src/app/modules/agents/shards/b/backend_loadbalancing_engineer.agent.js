...
class BackendLoadBalancingEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendLoadBalancingSystem(objective) {
        logger.info(`💻 [BackendLoadBalancingEngineerAgent] Analyzing Backend LoadBalancing Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend LoadBalancing Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendLoadBalancingEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendLoadBalancingEngineerAgent = Object.freeze(new BackendLoadBalancingEngineerAgent());
