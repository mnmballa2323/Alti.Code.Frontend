...
class BackendOptimizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendOptimizationSystem(objective) {
    logger.info(
      `💻 [BackendOptimizationAnalystAgent] Analyzing Backend Optimization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Optimization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendOptimizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendOptimizationAnalystAgent = Object.freeze(
  new BackendOptimizationAnalystAgent(),
);
