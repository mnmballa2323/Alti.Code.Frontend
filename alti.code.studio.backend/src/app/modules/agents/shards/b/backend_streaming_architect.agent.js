...
class BackendStreamingArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendStreamingSystem(objective) {
    logger.info(
      `💻 [BackendStreamingArchitectAgent] Analyzing Backend Streaming Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Streaming Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendStreamingArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendStreamingArchitectAgent = Object.freeze(
  new BackendStreamingArchitectAgent(),
);
