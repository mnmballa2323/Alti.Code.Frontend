...
class BackendAutomationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendAutomationSystem(objective) {
    logger.info(
      `💻 [BackendAutomationDesignerAgent] Analyzing Backend Automation Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Automation Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendAutomationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendAutomationDesignerAgent = Object.freeze(
  new BackendAutomationDesignerAgent(),
);
