...
class BackendLoadBalancingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendLoadBalancingSystem(objective) {
    logger.info(
      `💻 [BackendLoadBalancingSpecialistAgent] Analyzing Backend LoadBalancing Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend LoadBalancing Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendLoadBalancingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendLoadBalancingSpecialistAgent = Object.freeze(
  new BackendLoadBalancingSpecialistAgent(),
);
