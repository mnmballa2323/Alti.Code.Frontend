...
class BackendContainerizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendContainerizationSystem(objective) {
        logger.info(`💻 [BackendContainerizationDirectorAgent] Analyzing Backend Containerization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Containerization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendContainerizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendContainerizationDirectorAgent = Object.freeze(new BackendContainerizationDirectorAgent());
