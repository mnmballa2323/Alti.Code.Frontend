...
class BackendCachingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendCachingSystem(objective) {
    logger.info(
      `💻 [BackendCachingOrchestratorAgent] Analyzing Backend Caching Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Caching Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendCachingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendCachingOrchestratorAgent = Object.freeze(
  new BackendCachingOrchestratorAgent(),
);
