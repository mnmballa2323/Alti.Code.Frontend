...
class BackendOrchestrationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendOrchestrationSystem(objective) {
    logger.info(
      `💻 [BackendOrchestrationLeadAgent] Analyzing Backend Orchestration Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Orchestration Lead Target',
      );
...
export const backendOrchestrationLeadAgent = Object.freeze(
  new BackendOrchestrationLeadAgent(),
);
