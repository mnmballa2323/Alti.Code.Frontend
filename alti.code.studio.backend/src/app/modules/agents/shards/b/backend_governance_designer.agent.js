...
class BackendGovernanceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendGovernanceSystem(objective) {
    logger.info(
      `💻 [BackendGovernanceDesignerAgent] Analyzing Backend Governance Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Governance Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendGovernanceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendGovernanceDesignerAgent = Object.freeze(
  new BackendGovernanceDesignerAgent(),
);
