...
class BackendGovernanceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendGovernanceSystem(objective) {
    logger.info(
      `💻 [BackendGovernanceSpecialistAgent] Analyzing Backend Governance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Governance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendGovernanceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendGovernanceSpecialistAgent = Object.freeze(
  new BackendGovernanceSpecialistAgent(),
);
