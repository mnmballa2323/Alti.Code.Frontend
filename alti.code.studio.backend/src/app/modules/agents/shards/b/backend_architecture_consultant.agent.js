...
class BackendArchitectureConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendArchitectureSystem(objective) {
    logger.info(
      `💻 [BackendArchitectureConsultantAgent] Analyzing Backend Architecture Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Architecture Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendArchitectureConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendArchitectureConsultantAgent = Object.freeze(
  new BackendArchitectureConsultantAgent(),
);
