...
class BackendServerlessArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendServerlessSystem(objective) {
    logger.info(
      `💻 [BackendServerlessArchitectAgent] Analyzing Backend Serverless Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Serverless Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendServerlessArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendServerlessArchitectAgent = Object.freeze(
  new BackendServerlessArchitectAgent(),
);
