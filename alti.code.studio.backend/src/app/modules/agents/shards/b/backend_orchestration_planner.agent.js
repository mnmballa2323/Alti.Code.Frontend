...
class BackendOrchestrationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendOrchestrationSystem(objective) {
    logger.info(
      `💻 [BackendOrchestrationPlannerAgent] Analyzing Backend Orchestration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Orchestration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendOrchestrationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendOrchestrationPlannerAgent = Object.freeze(
  new BackendOrchestrationPlannerAgent(),
);
