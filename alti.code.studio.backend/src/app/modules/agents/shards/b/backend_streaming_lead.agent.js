...
class BackendStreamingLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendStreamingSystem(objective) {
    logger.info(
      `💻 [BackendStreamingLeadAgent] Analyzing Backend Streaming Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Streaming Lead Target',
      );
...
export const backendStreamingLeadAgent = Object.freeze(
  new BackendStreamingLeadAgent(),
);
