...
class BackendDataWarehouseTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendDataWarehouseSystem(objective) {
        logger.info(`💻 [BackendDataWarehouseTesterAgent] Analyzing Backend DataWarehouse Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend DataWarehouse Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendDataWarehouseTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendDataWarehouseTesterAgent = Object.freeze(new BackendDataWarehouseTesterAgent());
