...
class BackendArchitectureOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendArchitectureSystem(objective) {
        logger.info(`💻 [BackendArchitectureOrchestratorAgent] Analyzing Backend Architecture Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Architecture Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendArchitectureOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendArchitectureOrchestratorAgent = Object.freeze(new BackendArchitectureOrchestratorAgent());
