...
class BackendIntegrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendIntegrationSystem(objective) {
    logger.info(
      `💻 [BackendIntegrationOrchestratorAgent] Analyzing Backend Integration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Integration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendIntegrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendIntegrationOrchestratorAgent = Object.freeze(
  new BackendIntegrationOrchestratorAgent(),
);
