...
class BackendTelemetryDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendTelemetrySystem(objective) {
    logger.info(
      `💻 [BackendTelemetryDirectorAgent] Analyzing Backend Telemetry Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Telemetry Director Target',
      );
...
export const backendTelemetryDirectorAgent = Object.freeze(
  new BackendTelemetryDirectorAgent(),
);
