...
class BackendLoadBalancingDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendLoadBalancingSystem(objective) {
        logger.info(`💻 [BackendLoadBalancingDesignerAgent] Analyzing Backend LoadBalancing Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend LoadBalancing Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendLoadBalancingDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendLoadBalancingDesignerAgent = Object.freeze(new BackendLoadBalancingDesignerAgent());
