...
class BackendEngineeringAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendEngineeringSystem(objective) {
    logger.info(
      `💻 [BackendEngineeringAnalystAgent] Analyzing Backend Engineering Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Engineering Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendEngineeringAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendEngineeringAnalystAgent = Object.freeze(
  new BackendEngineeringAnalystAgent(),
);
