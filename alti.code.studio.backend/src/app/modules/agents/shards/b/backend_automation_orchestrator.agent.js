...
class BackendAutomationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendAutomationSystem(objective) {
    logger.info(
      `💻 [BackendAutomationOrchestratorAgent] Analyzing Backend Automation Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Automation Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendAutomationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendAutomationOrchestratorAgent = Object.freeze(
  new BackendAutomationOrchestratorAgent(),
);
