...
class BackendMigrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendMigrationSystem(objective) {
    logger.info(
      `💻 [BackendMigrationArchitectAgent] Analyzing Backend Migration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Migration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendMigrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendMigrationArchitectAgent = Object.freeze(
  new BackendMigrationArchitectAgent(),
);
