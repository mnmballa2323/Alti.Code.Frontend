...
class BackendMigrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendMigrationSystem(objective) {
    logger.info(
      `💻 [BackendMigrationSpecialistAgent] Analyzing Backend Migration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Migration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendMigrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendMigrationSpecialistAgent = Object.freeze(
  new BackendMigrationSpecialistAgent(),
);
