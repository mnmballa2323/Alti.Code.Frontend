...
class BackendLoadBalancingAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendLoadBalancingSystem(objective) {
    logger.info(
      `💻 [BackendLoadBalancingAnalystAgent] Analyzing Backend LoadBalancing Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend LoadBalancing Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendLoadBalancingAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendLoadBalancingAnalystAgent = Object.freeze(
  new BackendLoadBalancingAnalystAgent(),
);
