...
class BackendComplianceLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendComplianceSystem(objective) {
    logger.info(
      `💻 [BackendComplianceLeadAgent] Analyzing Backend Compliance Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Compliance Lead Target',
      );
...
export const backendComplianceLeadAgent = Object.freeze(
  new BackendComplianceLeadAgent(),
);
