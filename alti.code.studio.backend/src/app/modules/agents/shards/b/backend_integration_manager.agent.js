...
class BackendIntegrationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendIntegrationSystem(objective) {
    logger.info(
      `💻 [BackendIntegrationManagerAgent] Analyzing Backend Integration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Integration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendIntegrationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendIntegrationManagerAgent = Object.freeze(
  new BackendIntegrationManagerAgent(),
);
