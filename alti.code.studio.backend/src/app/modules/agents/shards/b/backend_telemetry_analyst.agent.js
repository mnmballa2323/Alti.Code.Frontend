...
class BackendTelemetryAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendTelemetrySystem(objective) {
    logger.info(
      `💻 [BackendTelemetryAnalystAgent] Analyzing Backend Telemetry Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Telemetry Analyst Target',
      );
...
export const backendTelemetryAnalystAgent = Object.freeze(
  new BackendTelemetryAnalystAgent(),
);
