...
class BackendIntegrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendIntegrationSystem(objective) {
    logger.info(
      `💻 [BackendIntegrationSpecialistAgent] Analyzing Backend Integration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Integration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendIntegrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendIntegrationSpecialistAgent = Object.freeze(
  new BackendIntegrationSpecialistAgent(),
);
