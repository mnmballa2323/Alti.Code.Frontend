...
class BackendVirtualizationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendVirtualizationSystem(objective) {
        logger.info(`💻 [BackendVirtualizationStrategistAgent] Analyzing Backend Virtualization Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Virtualization Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendVirtualizationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendVirtualizationStrategistAgent = Object.freeze(new BackendVirtualizationStrategistAgent());
