...
class BackendVirtualizationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendVirtualizationSystem(objective) {
    logger.info(
      `💻 [BackendVirtualizationOrchestratorAgent] Analyzing Backend Virtualization Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Virtualization Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendVirtualizationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendVirtualizationOrchestratorAgent = Object.freeze(
  new BackendVirtualizationOrchestratorAgent(),
);
