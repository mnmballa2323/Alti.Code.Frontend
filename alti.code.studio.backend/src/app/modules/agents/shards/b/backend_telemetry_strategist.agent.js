...
class BackendTelemetryStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendTelemetrySystem(objective) {
    logger.info(
      `💻 [BackendTelemetryStrategistAgent] Analyzing Backend Telemetry Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Telemetry Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendTelemetryStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendTelemetryStrategistAgent = Object.freeze(
  new BackendTelemetryStrategistAgent(),
);
