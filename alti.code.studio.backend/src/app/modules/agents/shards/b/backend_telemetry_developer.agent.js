...
class BackendTelemetryDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendTelemetrySystem(objective) {
        logger.info(`💻 [BackendTelemetryDeveloperAgent] Analyzing Backend Telemetry Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Telemetry Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendTelemetryDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendTelemetryDeveloperAgent = Object.freeze(new BackendTelemetryDeveloperAgent());
