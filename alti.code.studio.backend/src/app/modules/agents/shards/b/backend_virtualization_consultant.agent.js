...
class BackendVirtualizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendVirtualizationSystem(objective) {
    logger.info(
      `💻 [BackendVirtualizationConsultantAgent] Analyzing Backend Virtualization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Virtualization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendVirtualizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendVirtualizationConsultantAgent = Object.freeze(
  new BackendVirtualizationConsultantAgent(),
);
