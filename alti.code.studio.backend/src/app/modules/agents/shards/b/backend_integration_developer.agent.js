...
class BackendIntegrationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendIntegrationSystem(objective) {
    logger.info(
      `💻 [BackendIntegrationDeveloperAgent] Analyzing Backend Integration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Integration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendIntegrationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendIntegrationDeveloperAgent = Object.freeze(
  new BackendIntegrationDeveloperAgent(),
);
