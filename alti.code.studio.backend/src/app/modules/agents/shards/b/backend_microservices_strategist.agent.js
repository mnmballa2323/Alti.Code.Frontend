...
class BackendMicroservicesStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendMicroservicesSystem(objective) {
    logger.info(
      `💻 [BackendMicroservicesStrategistAgent] Analyzing Backend Microservices Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Microservices Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendMicroservicesStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendMicroservicesStrategistAgent = Object.freeze(
  new BackendMicroservicesStrategistAgent(),
);
