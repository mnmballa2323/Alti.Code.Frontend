...
class BackendComplianceStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendComplianceSystem(objective) {
    logger.info(
      `💻 [BackendComplianceStrategistAgent] Analyzing Backend Compliance Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Compliance Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendComplianceStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendComplianceStrategistAgent = Object.freeze(
  new BackendComplianceStrategistAgent(),
);
