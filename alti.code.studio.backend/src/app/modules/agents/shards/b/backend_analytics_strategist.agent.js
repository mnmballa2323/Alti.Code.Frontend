...
class BackendAnalyticsStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendAnalyticsSystem(objective) {
    logger.info(
      `💻 [BackendAnalyticsStrategistAgent] Analyzing Backend Analytics Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Analytics Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendAnalyticsStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendAnalyticsStrategistAgent = Object.freeze(
  new BackendAnalyticsStrategistAgent(),
);
