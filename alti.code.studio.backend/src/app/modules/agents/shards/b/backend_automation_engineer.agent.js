...
class BackendAutomationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendAutomationSystem(objective) {
    logger.info(
      `💻 [BackendAutomationEngineerAgent] Analyzing Backend Automation Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Automation Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendAutomationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendAutomationEngineerAgent = Object.freeze(
  new BackendAutomationEngineerAgent(),
);
