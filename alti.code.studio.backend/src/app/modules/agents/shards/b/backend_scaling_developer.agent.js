...
class BackendScalingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendScalingSystem(objective) {
    logger.info(
      `💻 [BackendScalingDeveloperAgent] Analyzing Backend Scaling Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Scaling Developer Target',
      );
...
export const backendScalingDeveloperAgent = Object.freeze(
  new BackendScalingDeveloperAgent(),
);
