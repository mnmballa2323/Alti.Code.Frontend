...
class BackendIntegrationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendIntegrationSystem(objective) {
    logger.info(
      `💻 [BackendIntegrationPlannerAgent] Analyzing Backend Integration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Integration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendIntegrationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendIntegrationPlannerAgent = Object.freeze(
  new BackendIntegrationPlannerAgent(),
);
