...
class BackendProvisioningAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendProvisioningSystem(objective) {
        logger.info(`💻 [BackendProvisioningAnalystAgent] Analyzing Backend Provisioning Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Provisioning Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendProvisioningAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendProvisioningAnalystAgent = Object.freeze(new BackendProvisioningAnalystAgent());
