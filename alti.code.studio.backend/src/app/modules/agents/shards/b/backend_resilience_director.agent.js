...
class BackendResilienceDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendResilienceSystem(objective) {
        logger.info(`💻 [BackendResilienceDirectorAgent] Analyzing Backend Resilience Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Resilience Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendResilienceDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendResilienceDirectorAgent = Object.freeze(new BackendResilienceDirectorAgent());
