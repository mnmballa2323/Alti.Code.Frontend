...
class BackendProvisioningTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendProvisioningSystem(objective) {
    logger.info(
      `💻 [BackendProvisioningTesterAgent] Analyzing Backend Provisioning Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Provisioning Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendProvisioningTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendProvisioningTesterAgent = Object.freeze(
  new BackendProvisioningTesterAgent(),
);
