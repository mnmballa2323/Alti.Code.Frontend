...
class BackendTelemetryPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendTelemetrySystem(objective) {
        logger.info(`💻 [BackendTelemetryPlannerAgent] Analyzing Backend Telemetry Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Telemetry Planner Target");
...
export const backendTelemetryPlannerAgent = Object.freeze(new BackendTelemetryPlannerAgent());
