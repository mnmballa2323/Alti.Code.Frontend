...
class BackendEngineeringStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendEngineeringSystem(objective) {
    logger.info(
      `💻 [BackendEngineeringStrategistAgent] Analyzing Backend Engineering Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Engineering Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendEngineeringStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendEngineeringStrategistAgent = Object.freeze(
  new BackendEngineeringStrategistAgent(),
);
