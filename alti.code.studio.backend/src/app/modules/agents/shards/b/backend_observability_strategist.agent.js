...
class BackendObservabilityStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendObservabilitySystem(objective) {
        logger.info(`💻 [BackendObservabilityStrategistAgent] Analyzing Backend Observability Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Observability Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendObservabilityStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendObservabilityStrategistAgent = Object.freeze(new BackendObservabilityStrategistAgent());
