...
class BackendDeploymentPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDeploymentSystem(objective) {
    logger.info(
      `💻 [BackendDeploymentPlannerAgent] Analyzing Backend Deployment Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Deployment Planner Target',
      );
...
export const backendDeploymentPlannerAgent = Object.freeze(
  new BackendDeploymentPlannerAgent(),
);
