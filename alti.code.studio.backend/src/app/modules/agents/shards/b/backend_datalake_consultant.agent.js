...
class BackendDataLakeConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDataLakeSystem(objective) {
    logger.info(
      `💻 [BackendDataLakeConsultantAgent] Analyzing Backend DataLake Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend DataLake Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendDataLakeConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendDataLakeConsultantAgent = Object.freeze(
  new BackendDataLakeConsultantAgent(),
);
