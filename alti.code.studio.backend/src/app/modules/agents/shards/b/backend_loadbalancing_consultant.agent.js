...
class BackendLoadBalancingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendLoadBalancingSystem(objective) {
        logger.info(`💻 [BackendLoadBalancingConsultantAgent] Analyzing Backend LoadBalancing Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend LoadBalancing Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendLoadBalancingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendLoadBalancingConsultantAgent = Object.freeze(new BackendLoadBalancingConsultantAgent());
