...
class BackendResilienceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendResilienceSystem(objective) {
    logger.info(
      `💻 [BackendResilienceDesignerAgent] Analyzing Backend Resilience Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Resilience Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendResilienceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendResilienceDesignerAgent = Object.freeze(
  new BackendResilienceDesignerAgent(),
);
