...
class BackendVirtualizationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendVirtualizationSystem(objective) {
    logger.info(
      `💻 [BackendVirtualizationEngineerAgent] Analyzing Backend Virtualization Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Virtualization Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendVirtualizationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendVirtualizationEngineerAgent = Object.freeze(
  new BackendVirtualizationEngineerAgent(),
);
