...
class BackendOptimizationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendOptimizationSystem(objective) {
        logger.info(`💻 [BackendOptimizationManagerAgent] Analyzing Backend Optimization Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Optimization Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendOptimizationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendOptimizationManagerAgent = Object.freeze(new BackendOptimizationManagerAgent());
