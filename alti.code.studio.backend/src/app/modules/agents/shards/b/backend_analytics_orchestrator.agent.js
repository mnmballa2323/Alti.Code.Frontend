...
class BackendAnalyticsOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendAnalyticsSystem(objective) {
        logger.info(`💻 [BackendAnalyticsOrchestratorAgent] Analyzing Backend Analytics Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Analytics Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendAnalyticsOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendAnalyticsOrchestratorAgent = Object.freeze(new BackendAnalyticsOrchestratorAgent());
