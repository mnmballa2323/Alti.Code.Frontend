...
class BackendMigrationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendMigrationSystem(objective) {
        logger.info(`💻 [BackendMigrationStrategistAgent] Analyzing Backend Migration Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Migration Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendMigrationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendMigrationStrategistAgent = Object.freeze(new BackendMigrationStrategistAgent());
