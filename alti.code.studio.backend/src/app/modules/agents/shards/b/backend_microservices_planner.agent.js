...
class BackendMicroservicesPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendMicroservicesSystem(objective) {
    logger.info(
      `💻 [BackendMicroservicesPlannerAgent] Analyzing Backend Microservices Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Microservices Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendMicroservicesPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendMicroservicesPlannerAgent = Object.freeze(
  new BackendMicroservicesPlannerAgent(),
);
