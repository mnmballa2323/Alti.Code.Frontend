...
class BackendArchitectureEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendArchitectureSystem(objective) {
    logger.info(
      `💻 [BackendArchitectureEngineerAgent] Analyzing Backend Architecture Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Architecture Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendArchitectureEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendArchitectureEngineerAgent = Object.freeze(
  new BackendArchitectureEngineerAgent(),
);
