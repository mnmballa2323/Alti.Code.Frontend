...
class BackendServerlessEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendServerlessSystem(objective) {
        logger.info(`💻 [BackendServerlessEngineerAgent] Analyzing Backend Serverless Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Serverless Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendServerlessEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendServerlessEngineerAgent = Object.freeze(new BackendServerlessEngineerAgent());
