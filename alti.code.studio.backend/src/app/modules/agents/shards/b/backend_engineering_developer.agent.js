...
class BackendEngineeringDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendEngineeringSystem(objective) {
        logger.info(`💻 [BackendEngineeringDeveloperAgent] Analyzing Backend Engineering Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Engineering Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendEngineeringDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendEngineeringDeveloperAgent = Object.freeze(new BackendEngineeringDeveloperAgent());
