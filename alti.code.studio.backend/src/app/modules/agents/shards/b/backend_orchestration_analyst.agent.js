...
class BackendOrchestrationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendOrchestrationSystem(objective) {
    logger.info(
      `💻 [BackendOrchestrationAnalystAgent] Analyzing Backend Orchestration Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Orchestration Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendOrchestrationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendOrchestrationAnalystAgent = Object.freeze(
  new BackendOrchestrationAnalystAgent(),
);
