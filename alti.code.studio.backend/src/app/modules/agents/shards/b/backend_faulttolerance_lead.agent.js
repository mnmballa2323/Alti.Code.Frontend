...
class BackendFaultToleranceLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendFaultToleranceSystem(objective) {
    logger.info(
      `💻 [BackendFaultToleranceLeadAgent] Analyzing Backend FaultTolerance Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend FaultTolerance Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendFaultToleranceLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendFaultToleranceLeadAgent = Object.freeze(
  new BackendFaultToleranceLeadAgent(),
);
