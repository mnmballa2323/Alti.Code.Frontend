...
class BackendDataLakeStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDataLakeSystem(objective) {
    logger.info(
      `💻 [BackendDataLakeStrategistAgent] Analyzing Backend DataLake Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend DataLake Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendDataLakeStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendDataLakeStrategistAgent = Object.freeze(
  new BackendDataLakeStrategistAgent(),
);
