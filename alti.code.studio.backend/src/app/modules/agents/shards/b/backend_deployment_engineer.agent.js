...
class BackendDeploymentEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDeploymentSystem(objective) {
    logger.info(
      `💻 [BackendDeploymentEngineerAgent] Analyzing Backend Deployment Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Deployment Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendDeploymentEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendDeploymentEngineerAgent = Object.freeze(
  new BackendDeploymentEngineerAgent(),
);
