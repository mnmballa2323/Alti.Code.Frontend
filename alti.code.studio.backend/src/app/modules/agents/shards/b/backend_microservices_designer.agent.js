...
class BackendMicroservicesDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendMicroservicesSystem(objective) {
    logger.info(
      `💻 [BackendMicroservicesDesignerAgent] Analyzing Backend Microservices Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Microservices Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendMicroservicesDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendMicroservicesDesignerAgent = Object.freeze(
  new BackendMicroservicesDesignerAgent(),
);
