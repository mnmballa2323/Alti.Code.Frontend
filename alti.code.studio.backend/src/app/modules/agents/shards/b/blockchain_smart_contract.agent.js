...
class BlockchainSmartContractAgent extends GeminiCliBaseAgent {
  ...
  async generateSmartContract(businessRules) {
    logger.info(
      `⛓️ [BlockchainSmartContract] Synthesizing mathematically verified Web3 Smart Contract...`,
    );
...
${businessRules}
    ...
    try {
      ...
      const cleanContract = output
        .replace(/```solidity|```rust|```/g, '')
        .trim();
      logger.info(
        `✅ [BlockchainSmartContract] Secure Smart Contract generated successfully.`,
      );
      return cleanContract;
    } catch (err) {
      logger.error(
        `❌ [BlockchainSmartContract] Failed to generate Smart Contract: ${err.message}`,
      );
      throw err;
    }
  }
...
export const blockchainSmartContractAgent = Object.freeze(
  new BlockchainSmartContractAgent(),
);
