...
class BackendFaultToleranceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendFaultToleranceSystem(objective) {
    logger.info(
      `💻 [BackendFaultToleranceEngineerAgent] Analyzing Backend FaultTolerance Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend FaultTolerance Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendFaultToleranceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendFaultToleranceEngineerAgent = Object.freeze(
  new BackendFaultToleranceEngineerAgent(),
);
