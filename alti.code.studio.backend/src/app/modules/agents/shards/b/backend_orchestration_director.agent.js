...
class BackendOrchestrationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendOrchestrationSystem(objective) {
    logger.info(
      `💻 [BackendOrchestrationDirectorAgent] Analyzing Backend Orchestration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Orchestration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendOrchestrationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendOrchestrationDirectorAgent = Object.freeze(
  new BackendOrchestrationDirectorAgent(),
);
