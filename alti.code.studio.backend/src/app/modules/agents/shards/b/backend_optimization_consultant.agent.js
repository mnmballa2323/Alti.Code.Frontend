...
class BackendOptimizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendOptimizationSystem(objective) {
    logger.info(
      `💻 [BackendOptimizationConsultantAgent] Analyzing Backend Optimization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Optimization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendOptimizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendOptimizationConsultantAgent = Object.freeze(
  new BackendOptimizationConsultantAgent(),
);
