...
class BackendBatchProcessingManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendBatchProcessingSystem(objective) {
        logger.info(`💻 [BackendBatchProcessingManagerAgent] Analyzing Backend BatchProcessing Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend BatchProcessing Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendBatchProcessingManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendBatchProcessingManagerAgent = Object.freeze(new BackendBatchProcessingManagerAgent());
