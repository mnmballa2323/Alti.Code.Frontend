...
class BackendDataWarehouseDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDataWarehouseSystem(objective) {
    logger.info(
      `💻 [BackendDataWarehouseDesignerAgent] Analyzing Backend DataWarehouse Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend DataWarehouse Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendDataWarehouseDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendDataWarehouseDesignerAgent = Object.freeze(
  new BackendDataWarehouseDesignerAgent(),
);
