...
class BackendArchitectureSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendArchitectureSystem(objective) {
    logger.info(
      `💻 [BackendArchitectureSpecialistAgent] Analyzing Backend Architecture Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Architecture Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendArchitectureSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendArchitectureSpecialistAgent = Object.freeze(
  new BackendArchitectureSpecialistAgent(),
);
