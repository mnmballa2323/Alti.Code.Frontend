...
class BackendLoadBalancingDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendLoadBalancingSystem(objective) {
    logger.info(
      `💻 [BackendLoadBalancingDeveloperAgent] Analyzing Backend LoadBalancing Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend LoadBalancing Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendLoadBalancingDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendLoadBalancingDeveloperAgent = Object.freeze(
  new BackendLoadBalancingDeveloperAgent(),
);
