...
class BackendLoadBalancingDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendLoadBalancingSystem(objective) {
        logger.info(`💻 [BackendLoadBalancingDirectorAgent] Analyzing Backend LoadBalancing Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend LoadBalancing Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendLoadBalancingDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendLoadBalancingDirectorAgent = Object.freeze(new BackendLoadBalancingDirectorAgent());
