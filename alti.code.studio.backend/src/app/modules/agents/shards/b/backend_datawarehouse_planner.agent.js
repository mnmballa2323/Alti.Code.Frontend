...
class BackendDataWarehousePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDataWarehouseSystem(objective) {
    logger.info(
      `💻 [BackendDataWarehousePlannerAgent] Analyzing Backend DataWarehouse Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend DataWarehouse Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendDataWarehousePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendDataWarehousePlannerAgent = Object.freeze(
  new BackendDataWarehousePlannerAgent(),
);
