...
class BackendResilienceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendResilienceSystem(objective) {
    logger.info(
      `💻 [BackendResilienceEngineerAgent] Analyzing Backend Resilience Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Resilience Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendResilienceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendResilienceEngineerAgent = Object.freeze(
  new BackendResilienceEngineerAgent(),
);
