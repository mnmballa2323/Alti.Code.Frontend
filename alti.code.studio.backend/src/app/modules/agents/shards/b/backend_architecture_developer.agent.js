...
class BackendArchitectureDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendArchitectureSystem(objective) {
        logger.info(`💻 [BackendArchitectureDeveloperAgent] Analyzing Backend Architecture Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Architecture Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendArchitectureDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendArchitectureDeveloperAgent = Object.freeze(new BackendArchitectureDeveloperAgent());
