...
class BackendServerlessSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendServerlessSystem(objective) {
    logger.info(
      `💻 [BackendServerlessSpecialistAgent] Analyzing Backend Serverless Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Serverless Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendServerlessSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendServerlessSpecialistAgent = Object.freeze(
  new BackendServerlessSpecialistAgent(),
);
