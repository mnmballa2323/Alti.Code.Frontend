...
class BackendComplianceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendComplianceSystem(objective) {
    logger.info(
      `💻 [BackendComplianceEngineerAgent] Analyzing Backend Compliance Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Compliance Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendComplianceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendComplianceEngineerAgent = Object.freeze(
  new BackendComplianceEngineerAgent(),
);
