...
class BackendConfigurationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendConfigurationSystem(objective) {
    logger.info(
      `💻 [BackendConfigurationDirectorAgent] Analyzing Backend Configuration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Configuration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendConfigurationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendConfigurationDirectorAgent = Object.freeze(
  new BackendConfigurationDirectorAgent(),
);
