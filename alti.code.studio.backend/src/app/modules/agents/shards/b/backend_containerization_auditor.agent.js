...
class BackendContainerizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendContainerizationSystem(objective) {
    logger.info(
      `💻 [BackendContainerizationAuditorAgent] Analyzing Backend Containerization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Containerization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendContainerizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendContainerizationAuditorAgent = Object.freeze(
  new BackendContainerizationAuditorAgent(),
);
