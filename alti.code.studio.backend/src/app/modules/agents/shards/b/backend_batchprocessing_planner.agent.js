...
class BackendBatchProcessingPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendBatchProcessingSystem(objective) {
        logger.info(`💻 [BackendBatchProcessingPlannerAgent] Analyzing Backend BatchProcessing Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend BatchProcessing Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendBatchProcessingPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendBatchProcessingPlannerAgent = Object.freeze(new BackendBatchProcessingPlannerAgent());
