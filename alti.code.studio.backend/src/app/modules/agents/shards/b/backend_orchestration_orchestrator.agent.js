...
class BackendOrchestrationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendOrchestrationSystem(objective) {
        logger.info(`💻 [BackendOrchestrationOrchestratorAgent] Analyzing Backend Orchestration Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Orchestration Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendOrchestrationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendOrchestrationOrchestratorAgent = Object.freeze(new BackendOrchestrationOrchestratorAgent());
