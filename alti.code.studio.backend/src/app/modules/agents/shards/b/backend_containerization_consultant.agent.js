...
class BackendContainerizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendContainerizationSystem(objective) {
    logger.info(
      `💻 [BackendContainerizationConsultantAgent] Analyzing Backend Containerization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Containerization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendContainerizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendContainerizationConsultantAgent = Object.freeze(
  new BackendContainerizationConsultantAgent(),
);
