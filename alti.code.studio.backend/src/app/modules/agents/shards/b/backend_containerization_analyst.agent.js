...
class BackendContainerizationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendContainerizationSystem(objective) {
        logger.info(`💻 [BackendContainerizationAnalystAgent] Analyzing Backend Containerization Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Containerization Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendContainerizationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendContainerizationAnalystAgent = Object.freeze(new BackendContainerizationAnalystAgent());
