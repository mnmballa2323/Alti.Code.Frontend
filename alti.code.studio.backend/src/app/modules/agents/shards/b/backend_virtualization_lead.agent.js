...
class BackendVirtualizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendVirtualizationSystem(objective) {
    logger.info(
      `💻 [BackendVirtualizationLeadAgent] Analyzing Backend Virtualization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Virtualization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendVirtualizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendVirtualizationLeadAgent = Object.freeze(
  new BackendVirtualizationLeadAgent(),
);
