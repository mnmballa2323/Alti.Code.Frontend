...
class BackendOptimizationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendOptimizationSystem(objective) {
        logger.info(`💻 [BackendOptimizationDesignerAgent] Analyzing Backend Optimization Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Optimization Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendOptimizationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendOptimizationDesignerAgent = Object.freeze(new BackendOptimizationDesignerAgent());
