...
class BackendAutomationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendAutomationSystem(objective) {
        logger.info(`💻 [BackendAutomationArchitectAgent] Analyzing Backend Automation Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Automation Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendAutomationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendAutomationArchitectAgent = Object.freeze(new BackendAutomationArchitectAgent());
