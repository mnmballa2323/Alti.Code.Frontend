...
class BackendDeploymentSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDeploymentSystem(objective) {
    logger.info(
      `💻 [BackendDeploymentSpecialistAgent] Analyzing Backend Deployment Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Deployment Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendDeploymentSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendDeploymentSpecialistAgent = Object.freeze(
  new BackendDeploymentSpecialistAgent(),
);
