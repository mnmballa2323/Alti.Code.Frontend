...
class BackendServerlessDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendServerlessSystem(objective) {
    logger.info(
      `💻 [BackendServerlessDesignerAgent] Analyzing Backend Serverless Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Serverless Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendServerlessDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendServerlessDesignerAgent = Object.freeze(
  new BackendServerlessDesignerAgent(),
);
