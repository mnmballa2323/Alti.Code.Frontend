...
class BackendMicroservicesDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendMicroservicesSystem(objective) {
        logger.info(`💻 [BackendMicroservicesDeveloperAgent] Analyzing Backend Microservices Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Microservices Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendMicroservicesDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendMicroservicesDeveloperAgent = Object.freeze(new BackendMicroservicesDeveloperAgent());
