...
class BackendAnalyticsArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendAnalyticsSystem(objective) {
    logger.info(
      `💻 [BackendAnalyticsArchitectAgent] Analyzing Backend Analytics Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Analytics Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendAnalyticsArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendAnalyticsArchitectAgent = Object.freeze(
  new BackendAnalyticsArchitectAgent(),
);
