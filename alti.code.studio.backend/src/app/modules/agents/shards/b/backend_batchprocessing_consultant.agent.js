...
class BackendBatchProcessingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendBatchProcessingSystem(objective) {
        logger.info(`💻 [BackendBatchProcessingConsultantAgent] Analyzing Backend BatchProcessing Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend BatchProcessing Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendBatchProcessingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendBatchProcessingConsultantAgent = Object.freeze(new BackendBatchProcessingConsultantAgent());
