...
class BackendDataLakeLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDataLakeSystem(objective) {
    logger.info(
      `💻 [BackendDataLakeLeadAgent] Analyzing Backend DataLake Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend DataLake Lead Target',
      );
...
export const backendDataLakeLeadAgent = Object.freeze(
  new BackendDataLakeLeadAgent(),
);
