...
class BackendIntegrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendIntegrationSystem(objective) {
    logger.info(
      `💻 [BackendIntegrationEngineerAgent] Analyzing Backend Integration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Integration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendIntegrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendIntegrationEngineerAgent = Object.freeze(
  new BackendIntegrationEngineerAgent(),
);
