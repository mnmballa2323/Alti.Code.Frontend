...
class BackendMicroservicesAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendMicroservicesSystem(objective) {
    logger.info(
      `💻 [BackendMicroservicesAuditorAgent] Analyzing Backend Microservices Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Microservices Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendMicroservicesAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendMicroservicesAuditorAgent = Object.freeze(
  new BackendMicroservicesAuditorAgent(),
);
