...
class BackendConfigurationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendConfigurationSystem(objective) {
    logger.info(
      `💻 [BackendConfigurationDesignerAgent] Analyzing Backend Configuration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Configuration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendConfigurationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendConfigurationDesignerAgent = Object.freeze(
  new BackendConfigurationDesignerAgent(),
);
