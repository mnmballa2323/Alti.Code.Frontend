...
class BackendServerlessLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendServerlessSystem(objective) {
    logger.info(
      `💻 [BackendServerlessLeadAgent] Analyzing Backend Serverless Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Serverless Lead Target',
      );
...
export const backendServerlessLeadAgent = Object.freeze(
  new BackendServerlessLeadAgent(),
);
