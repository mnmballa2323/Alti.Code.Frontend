...
class BackendOptimizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendOptimizationSystem(objective) {
    logger.info(
      `💻 [BackendOptimizationDirectorAgent] Analyzing Backend Optimization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Optimization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendOptimizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendOptimizationDirectorAgent = Object.freeze(
  new BackendOptimizationDirectorAgent(),
);
