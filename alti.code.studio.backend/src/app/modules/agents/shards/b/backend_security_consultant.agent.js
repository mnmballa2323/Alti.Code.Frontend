...
class BackendSecurityConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendSecuritySystem(objective) {
    logger.info(
      `💻 [BackendSecurityConsultantAgent] Analyzing Backend Security Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Security Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendSecurityConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendSecurityConsultantAgent = Object.freeze(
  new BackendSecurityConsultantAgent(),
);
