...
class BackendServerlessStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendServerlessSystem(objective) {
        logger.info(`💻 [BackendServerlessStrategistAgent] Analyzing Backend Serverless Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Serverless Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendServerlessStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendServerlessStrategistAgent = Object.freeze(new BackendServerlessStrategistAgent());
