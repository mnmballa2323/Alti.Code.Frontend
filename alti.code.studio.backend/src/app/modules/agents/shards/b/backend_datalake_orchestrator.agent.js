...
class BackendDataLakeOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDataLakeSystem(objective) {
    logger.info(
      `💻 [BackendDataLakeOrchestratorAgent] Analyzing Backend DataLake Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend DataLake Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendDataLakeOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendDataLakeOrchestratorAgent = Object.freeze(
  new BackendDataLakeOrchestratorAgent(),
);
