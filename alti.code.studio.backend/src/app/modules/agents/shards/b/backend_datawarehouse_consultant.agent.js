...
class BackendDataWarehouseConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendDataWarehouseSystem(objective) {
        logger.info(`💻 [BackendDataWarehouseConsultantAgent] Analyzing Backend DataWarehouse Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend DataWarehouse Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendDataWarehouseConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendDataWarehouseConsultantAgent = Object.freeze(new BackendDataWarehouseConsultantAgent());
