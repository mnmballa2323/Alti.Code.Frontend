...
class BackendEngineeringEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendEngineeringSystem(objective) {
    logger.info(
      `💻 [BackendEngineeringEngineerAgent] Analyzing Backend Engineering Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Engineering Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendEngineeringEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendEngineeringEngineerAgent = Object.freeze(
  new BackendEngineeringEngineerAgent(),
);
