...
class BackendAutomationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendAutomationSystem(objective) {
        logger.info(`💻 [BackendAutomationDeveloperAgent] Analyzing Backend Automation Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Automation Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendAutomationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendAutomationDeveloperAgent = Object.freeze(new BackendAutomationDeveloperAgent());
