...
class BackendAutomationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendAutomationSystem(objective) {
    logger.info(
      `💻 [BackendAutomationSpecialistAgent] Analyzing Backend Automation Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Automation Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendAutomationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendAutomationSpecialistAgent = Object.freeze(
  new BackendAutomationSpecialistAgent(),
);
