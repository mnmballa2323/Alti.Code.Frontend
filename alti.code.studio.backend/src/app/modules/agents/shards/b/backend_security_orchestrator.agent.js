...
class BackendSecurityOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendSecuritySystem(objective) {
        logger.info(`💻 [BackendSecurityOrchestratorAgent] Analyzing Backend Security Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Security Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendSecurityOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendSecurityOrchestratorAgent = Object.freeze(new BackendSecurityOrchestratorAgent());
