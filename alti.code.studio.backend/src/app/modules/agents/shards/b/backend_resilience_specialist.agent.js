...
class BackendResilienceSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendResilienceSystem(objective) {
        logger.info(`💻 [BackendResilienceSpecialistAgent] Analyzing Backend Resilience Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Resilience Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendResilienceSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendResilienceSpecialistAgent = Object.freeze(new BackendResilienceSpecialistAgent());
