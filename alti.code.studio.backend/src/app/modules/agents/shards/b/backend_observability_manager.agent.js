...
class BackendObservabilityManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendObservabilitySystem(objective) {
    logger.info(
      `💻 [BackendObservabilityManagerAgent] Analyzing Backend Observability Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Observability Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendObservabilityManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendObservabilityManagerAgent = Object.freeze(
  new BackendObservabilityManagerAgent(),
);
