...
class BackendDataWarehouseAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDataWarehouseSystem(objective) {
    logger.info(
      `💻 [BackendDataWarehouseAnalystAgent] Analyzing Backend DataWarehouse Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend DataWarehouse Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendDataWarehouseAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendDataWarehouseAnalystAgent = Object.freeze(
  new BackendDataWarehouseAnalystAgent(),
);
