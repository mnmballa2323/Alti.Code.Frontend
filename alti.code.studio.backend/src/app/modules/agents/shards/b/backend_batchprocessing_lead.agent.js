...
class BackendBatchProcessingLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendBatchProcessingSystem(objective) {
    logger.info(
      `💻 [BackendBatchProcessingLeadAgent] Analyzing Backend BatchProcessing Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend BatchProcessing Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendBatchProcessingLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendBatchProcessingLeadAgent = Object.freeze(
  new BackendBatchProcessingLeadAgent(),
);
