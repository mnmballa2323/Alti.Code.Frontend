...
class BackendScalingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendScalingSystem(objective) {
    logger.info(
      `💻 [BackendScalingPlannerAgent] Analyzing Backend Scaling Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Scaling Planner Target',
      );
...
export const backendScalingPlannerAgent = Object.freeze(
  new BackendScalingPlannerAgent(),
);
