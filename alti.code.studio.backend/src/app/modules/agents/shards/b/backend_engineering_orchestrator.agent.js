...
class BackendEngineeringOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendEngineeringSystem(objective) {
        logger.info(`💻 [BackendEngineeringOrchestratorAgent] Analyzing Backend Engineering Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Engineering Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendEngineeringOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendEngineeringOrchestratorAgent = Object.freeze(new BackendEngineeringOrchestratorAgent());
