...
class BackendDeploymentConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDeploymentSystem(objective) {
    logger.info(
      `💻 [BackendDeploymentConsultantAgent] Analyzing Backend Deployment Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Deployment Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendDeploymentConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendDeploymentConsultantAgent = Object.freeze(
  new BackendDeploymentConsultantAgent(),
);
