...
class BackendResilienceOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendResilienceSystem(objective) {
        logger.info(`💻 [BackendResilienceOrchestratorAgent] Analyzing Backend Resilience Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Resilience Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendResilienceOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendResilienceOrchestratorAgent = Object.freeze(new BackendResilienceOrchestratorAgent());
