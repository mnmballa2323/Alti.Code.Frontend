...
class BackendDeploymentDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendDeploymentSystem(objective) {
        logger.info(`💻 [BackendDeploymentDeveloperAgent] Analyzing Backend Deployment Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Deployment Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendDeploymentDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendDeploymentDeveloperAgent = Object.freeze(new BackendDeploymentDeveloperAgent());
