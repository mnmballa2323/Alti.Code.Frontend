...
class BackendSecurityPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendSecuritySystem(objective) {
    logger.info(
      `💻 [BackendSecurityPlannerAgent] Analyzing Backend Security Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Security Planner Target',
      );
...
export const backendSecurityPlannerAgent = Object.freeze(
  new BackendSecurityPlannerAgent(),
);
