...
class BackendIntegrationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendIntegrationSystem(objective) {
        logger.info(`💻 [BackendIntegrationAnalystAgent] Analyzing Backend Integration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Integration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendIntegrationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendIntegrationAnalystAgent = Object.freeze(new BackendIntegrationAnalystAgent());
