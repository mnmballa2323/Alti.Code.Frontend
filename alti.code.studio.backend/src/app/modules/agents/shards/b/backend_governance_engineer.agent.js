...
class BackendGovernanceEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendGovernanceSystem(objective) {
        logger.info(`💻 [BackendGovernanceEngineerAgent] Analyzing Backend Governance Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Governance Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendGovernanceEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendGovernanceEngineerAgent = Object.freeze(new BackendGovernanceEngineerAgent());
