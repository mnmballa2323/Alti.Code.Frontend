...
class BackendLoadBalancingTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendLoadBalancingSystem(objective) {
        logger.info(`💻 [BackendLoadBalancingTesterAgent] Analyzing Backend LoadBalancing Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend LoadBalancing Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendLoadBalancingTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendLoadBalancingTesterAgent = Object.freeze(new BackendLoadBalancingTesterAgent());
