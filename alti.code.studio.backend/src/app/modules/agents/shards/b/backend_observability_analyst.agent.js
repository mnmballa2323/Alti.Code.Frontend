...
class BackendObservabilityAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendObservabilitySystem(objective) {
    logger.info(
      `💻 [BackendObservabilityAnalystAgent] Analyzing Backend Observability Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Observability Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendObservabilityAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendObservabilityAnalystAgent = Object.freeze(
  new BackendObservabilityAnalystAgent(),
);
