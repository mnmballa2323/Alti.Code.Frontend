...
class BackendStreamingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendStreamingSystem(objective) {
    logger.info(
      `💻 [BackendStreamingManagerAgent] Analyzing Backend Streaming Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Streaming Manager Target',
      );
...
export const backendStreamingManagerAgent = Object.freeze(
  new BackendStreamingManagerAgent(),
);
