...
class BackendDeploymentStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDeploymentSystem(objective) {
    logger.info(
      `💻 [BackendDeploymentStrategistAgent] Analyzing Backend Deployment Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Deployment Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendDeploymentStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendDeploymentStrategistAgent = Object.freeze(
  new BackendDeploymentStrategistAgent(),
);
