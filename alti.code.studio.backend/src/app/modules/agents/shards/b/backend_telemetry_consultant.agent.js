...
class BackendTelemetryConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendTelemetrySystem(objective) {
    logger.info(
      `💻 [BackendTelemetryConsultantAgent] Analyzing Backend Telemetry Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Telemetry Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendTelemetryConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendTelemetryConsultantAgent = Object.freeze(
  new BackendTelemetryConsultantAgent(),
);
