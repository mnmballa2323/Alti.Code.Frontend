...
class BackendArchitectureDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendArchitectureSystem(objective) {
    logger.info(
      `💻 [BackendArchitectureDesignerAgent] Analyzing Backend Architecture Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Architecture Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendArchitectureDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendArchitectureDesignerAgent = Object.freeze(
  new BackendArchitectureDesignerAgent(),
);
