...
class BackendMicroservicesManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendMicroservicesSystem(objective) {
    logger.info(
      `💻 [BackendMicroservicesManagerAgent] Analyzing Backend Microservices Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Microservices Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendMicroservicesManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendMicroservicesManagerAgent = Object.freeze(
  new BackendMicroservicesManagerAgent(),
);
