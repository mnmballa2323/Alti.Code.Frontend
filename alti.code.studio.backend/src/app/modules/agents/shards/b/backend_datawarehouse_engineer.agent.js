...
class BackendDataWarehouseEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDataWarehouseSystem(objective) {
    logger.info(
      `💻 [BackendDataWarehouseEngineerAgent] Analyzing Backend DataWarehouse Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend DataWarehouse Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendDataWarehouseEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendDataWarehouseEngineerAgent = Object.freeze(
  new BackendDataWarehouseEngineerAgent(),
);
