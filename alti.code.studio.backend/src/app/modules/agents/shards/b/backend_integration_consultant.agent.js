...
class BackendIntegrationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendIntegrationSystem(objective) {
        logger.info(`💻 [BackendIntegrationConsultantAgent] Analyzing Backend Integration Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Integration Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendIntegrationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendIntegrationConsultantAgent = Object.freeze(new BackendIntegrationConsultantAgent());
