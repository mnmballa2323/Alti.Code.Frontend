...
class BackendComplianceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendComplianceSystem(objective) {
    logger.info(
      `💻 [BackendComplianceConsultantAgent] Analyzing Backend Compliance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Compliance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendComplianceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendComplianceConsultantAgent = Object.freeze(
  new BackendComplianceConsultantAgent(),
);
