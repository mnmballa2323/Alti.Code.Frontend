...
class BackendServerlessDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendServerlessSystem(objective) {
    logger.info(
      `💻 [BackendServerlessDeveloperAgent] Analyzing Backend Serverless Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Serverless Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendServerlessDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendServerlessDeveloperAgent = Object.freeze(
  new BackendServerlessDeveloperAgent(),
);
