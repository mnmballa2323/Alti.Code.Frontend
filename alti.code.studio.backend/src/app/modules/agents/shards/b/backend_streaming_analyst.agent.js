...
class BackendStreamingAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendStreamingSystem(objective) {
    logger.info(
      `💻 [BackendStreamingAnalystAgent] Analyzing Backend Streaming Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Streaming Analyst Target',
      );
...
export const backendStreamingAnalystAgent = Object.freeze(
  new BackendStreamingAnalystAgent(),
);
