...
class BackendDeploymentDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendDeploymentSystem(objective) {
    logger.info(
      `💻 [BackendDeploymentDesignerAgent] Analyzing Backend Deployment Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Deployment Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendDeploymentDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendDeploymentDesignerAgent = Object.freeze(
  new BackendDeploymentDesignerAgent(),
);
