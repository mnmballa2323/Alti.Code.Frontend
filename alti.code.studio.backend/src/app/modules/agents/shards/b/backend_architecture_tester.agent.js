...
class BackendArchitectureTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendArchitectureSystem(objective) {
    logger.info(
      `💻 [BackendArchitectureTesterAgent] Analyzing Backend Architecture Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Architecture Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendArchitectureTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendArchitectureTesterAgent = Object.freeze(
  new BackendArchitectureTesterAgent(),
);
