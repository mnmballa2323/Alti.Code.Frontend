...
class BackendContainerizationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendContainerizationSystem(objective) {
    logger.info(
      `💻 [BackendContainerizationSpecialistAgent] Analyzing Backend Containerization Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Containerization Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendContainerizationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendContainerizationSpecialistAgent = Object.freeze(
  new BackendContainerizationSpecialistAgent(),
);
