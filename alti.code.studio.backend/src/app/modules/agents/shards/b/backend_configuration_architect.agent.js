...
class BackendConfigurationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendConfigurationSystem(objective) {
    logger.info(
      `💻 [BackendConfigurationArchitectAgent] Analyzing Backend Configuration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Configuration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendConfigurationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendConfigurationArchitectAgent = Object.freeze(
  new BackendConfigurationArchitectAgent(),
);
