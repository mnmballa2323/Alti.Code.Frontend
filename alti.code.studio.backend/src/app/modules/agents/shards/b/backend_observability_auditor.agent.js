...
class BackendObservabilityAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendObservabilitySystem(objective) {
        logger.info(`💻 [BackendObservabilityAuditorAgent] Analyzing Backend Observability Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Observability Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendObservabilityAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendObservabilityAuditorAgent = Object.freeze(new BackendObservabilityAuditorAgent());
