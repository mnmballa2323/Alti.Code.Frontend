...
class BackendOrchestrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendOrchestrationSystem(objective) {
    logger.info(
      `💻 [BackendOrchestrationEngineerAgent] Analyzing Backend Orchestration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Orchestration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendOrchestrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendOrchestrationEngineerAgent = Object.freeze(
  new BackendOrchestrationEngineerAgent(),
);
