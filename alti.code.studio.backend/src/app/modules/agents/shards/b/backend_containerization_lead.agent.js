...
class BackendContainerizationLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendContainerizationSystem(objective) {
        logger.info(`💻 [BackendContainerizationLeadAgent] Analyzing Backend Containerization Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Containerization Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendContainerizationLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendContainerizationLeadAgent = Object.freeze(new BackendContainerizationLeadAgent());
