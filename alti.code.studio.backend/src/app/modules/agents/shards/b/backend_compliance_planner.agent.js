...
class BackendCompliancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendComplianceSystem(objective) {
    logger.info(
      `💻 [BackendCompliancePlannerAgent] Analyzing Backend Compliance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Compliance Planner Target',
      );
...
export const backendCompliancePlannerAgent = Object.freeze(
  new BackendCompliancePlannerAgent(),
);
