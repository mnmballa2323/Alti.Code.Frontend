...
class BackendFaultTolerancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendFaultToleranceSystem(objective) {
    logger.info(
      `💻 [BackendFaultTolerancePlannerAgent] Analyzing Backend FaultTolerance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend FaultTolerance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendFaultTolerancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendFaultTolerancePlannerAgent = Object.freeze(
  new BackendFaultTolerancePlannerAgent(),
);
