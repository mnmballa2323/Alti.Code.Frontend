...
class BackendProvisioningArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendProvisioningSystem(objective) {
        logger.info(`💻 [BackendProvisioningArchitectAgent] Analyzing Backend Provisioning Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Provisioning Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendProvisioningArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendProvisioningArchitectAgent = Object.freeze(new BackendProvisioningArchitectAgent());
