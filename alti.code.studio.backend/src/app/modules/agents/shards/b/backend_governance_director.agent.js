...
class BackendGovernanceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendGovernanceSystem(objective) {
    logger.info(
      `💻 [BackendGovernanceDirectorAgent] Analyzing Backend Governance Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Governance Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendGovernanceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendGovernanceDirectorAgent = Object.freeze(
  new BackendGovernanceDirectorAgent(),
);
