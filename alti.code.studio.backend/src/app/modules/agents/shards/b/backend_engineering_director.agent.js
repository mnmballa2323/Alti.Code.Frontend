...
class BackendEngineeringDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendEngineeringSystem(objective) {
    logger.info(
      `💻 [BackendEngineeringDirectorAgent] Analyzing Backend Engineering Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Engineering Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [BackendEngineeringDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const backendEngineeringDirectorAgent = Object.freeze(
  new BackendEngineeringDirectorAgent(),
);
