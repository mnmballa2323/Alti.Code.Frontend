...
class BackendOrchestrationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateBackendOrchestrationSystem(objective) {
        logger.info(`💻 [BackendOrchestrationTesterAgent] Analyzing Backend Orchestration Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Backend Orchestration Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [BackendOrchestrationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const backendOrchestrationTesterAgent = Object.freeze(new BackendOrchestrationTesterAgent());
