...
class BackendAnalyticsDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateBackendAnalyticsSystem(objective) {
    logger.info(
      `💻 [BackendAnalyticsDirectorAgent] Analyzing Backend Analytics Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Backend Analytics Director Target',
      );
...
export const backendAnalyticsDirectorAgent = Object.freeze(
  new BackendAnalyticsDirectorAgent(),
);
