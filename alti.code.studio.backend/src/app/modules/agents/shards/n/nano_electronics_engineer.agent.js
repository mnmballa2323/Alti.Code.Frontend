...
class NanoElectronicsEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateNanoSystem(objective) {
    logger.info(
      `🔬 [NanoElectronicsEngineer] Analyzing 2nm gate routing and EUV masks...`,
    );
    ...
    try {
      ...
      return output
        .replace(
          /```javascript|```typescript|```json|```vhdl|```verilog|```/gi,
          '',
        )
        .trim();
    } catch (err) {
...
export const nanoElectronicsEngineerAgent = Object.freeze(
  new NanoElectronicsEngineerAgent(),
);
