...
class NeurotechnologyBciEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateNeuroSystem(neuroObjective) {
    logger.info(
      `🧠 [NeurotechnologyBciEngineer] Analyzing objective for neural decoding and BCI hardware...`,
    );
...
${neuroObjective}
    ...
    try {
      ...
      const cleanCode = output
        .replace(
          /```javascript|```typescript|```json|```cpp|```rust|```python|```/gi,
          '',
        )
        .trim();
      logger.info(
        `✅ [NeurotechnologyBciEngineer] Neurotechnology architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [NeurotechnologyBciEngineer] Failed to generate neuro system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const neurotechnologyBciEngineerAgent = Object.freeze(
  new NeurotechnologyBciEngineerAgent(),
);
