...
class NuclearFusionPlasmaPhysicistAgent extends GeminiCliBaseAgent {
  ...
  async generateFusionSystem(fusionObjective) {
    logger.info(
      `☢️ [NuclearFusionPlasmaPhysicist] Analyzing objective for MHD simulations and Tokamak plasma confinement...`,
    );
...
${fusionObjective}
    ...
    try {
      ...
      const cleanCode = output
        .replace(
          /```javascript|```typescript|```json|```cpp|```fortran|```python|```cuda|```/gi,
          '',
        )
        .trim();
      logger.info(
        `✅ [NuclearFusionPlasmaPhysicist] Fusion architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [NuclearFusionPlasmaPhysicist] Failed to generate fusion system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const nuclearFusionPlasmaPhysicistAgent = Object.freeze(
  new NuclearFusionPlasmaPhysicistAgent(),
);
