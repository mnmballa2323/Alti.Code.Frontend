...
class KnowledgeAgent extends BaseSpecialistAgent {
  ...
  async extractTicketContext(ticketId) {
    logger.info(
      `🧠 KnowledgeAgent: Digesting Enterprise Intelligence for constraint ${ticketId}...`,
    );
  ...
  async markJiraTicketDone(ticketId) {
    ...
    logger.info(
      `🧠 KnowledgeAgent: Autonomously transitioning Epic ${ticketId} to DONE.`,
    );

    try {
      // Find "Done" transition
      const transRes = await fetch(
        `https://${process.env.JIRA_DOMAIN}.atlassian.net/rest/api/3/issue/${ticketId}/transitions`,
        {
          headers: {
            Authorization: `Basic ${Buffer.from(`${process.env.JIRA_EMAIL}:${process.env.JIRA_API_TOKEN}`).toString('base64')}`,
            Accept: 'application/json',
      ...
      const doneTransition = transitions?.transitions?.find(
        t =>
          t.name.toLowerCase() === 'done' || t.name.toLowerCase() === 'closed',
      );
  ...
  async broadcastToSlack(channelId, message) {
    ...
    logger.info(
      `🧠 KnowledgeAgent: Broadcasting Sprint Status to Slack channel ${channelId}...`,
    );
