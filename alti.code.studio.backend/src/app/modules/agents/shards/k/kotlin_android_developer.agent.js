...
class KotlinAndroidDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateKotlinSystem(objective) {
        logger.info(`🤖 [KotlinAndroidDeveloper] Analyzing Jetpack Compose state and Kotlin Coroutines...`);
        ...
        try {
            ...
            return output.replace(/```javascript|```typescript|```json|```kotlin|```/gi, '').trim();
        } catch (err) {
...
export const kotlinAndroidDeveloperAgent = Object.freeze(new KotlinAndroidDeveloperAgent());
