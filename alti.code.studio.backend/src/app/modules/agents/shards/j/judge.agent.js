...
class JudgeAgent {
    ...
    async review(targetPath) {
        ...
        try {
            ...
            const scanDir = stat.isDirectory() ? targetPath : path.dirname(targetPath);
            ...
            const relevantVulns = vulnerabilities.filter(v => normalizedTargets.includes(path.resolve(v.file)));

            if (relevantVulns.length > 0) {
                verdict.status = 'FAIL';
                verdict.score -= (relevantVulns.length * 20); // Penalty
                verdict.issues.push(...relevantVulns.map(v => `[SECURITY] ${v.type} in ${path.basename(v.file)}`));
            }
            ...
            for (const file of filesToScan) {
                ...
                if (insight && insight.prediction === 'High Risk') {
                    verdict.score -= 10;
                    verdict.warnings.push(`[COMPLEXITY] ${path.basename(file)} is High Risk: ${insight.reason}`);
                }
            }
            ...
            logger.info(`⚖️ Judge: Ruling delivered. Verdict: ${verdict.status} (Score: ${verdict.score})`);
            return verdict;
...
export const judgeAgent = Object.freeze(new JudgeAgent());
