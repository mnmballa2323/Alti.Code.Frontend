...
class JavaEnterpriseArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateJavaSystem(objective) {
    logger.info(
      `☕ [JavaEnterpriseArchitect] Analyzing JVM heap limits and Spring Context...`,
    );
    ...
    try {
      ...
      return output
        .replace(/```javascript|```typescript|```json|```java|```xml|```/gi, '')
        .trim();
    } catch (err) {
...
export const javaEnterpriseArchitectAgent = Object.freeze(
  new JavaEnterpriseArchitectAgent(),
);
