...
class UXVirtualizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateUXVirtualizationSystem(objective) {
        logger.info(`💻 [UXVirtualizationSpecialistAgent] Analyzing UX Virtualization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UX Virtualization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UXVirtualizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uXVirtualizationSpecialistAgent = Object.freeze(new UXVirtualizationSpecialistAgent());
