...
class UXLoadBalancingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateUXLoadBalancingSystem(objective) {
    logger.info(
      `💻 [UXLoadBalancingPlannerAgent] Analyzing UX LoadBalancing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UX LoadBalancing Planner Target',
      );
...
export const uXLoadBalancingPlannerAgent = Object.freeze(
  new UXLoadBalancingPlannerAgent(),
);
