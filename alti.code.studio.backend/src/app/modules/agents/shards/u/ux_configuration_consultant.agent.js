...
class UXConfigurationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateUXConfigurationSystem(objective) {
        logger.info(`💻 [UXConfigurationConsultantAgent] Analyzing UX Configuration Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UX Configuration Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UXConfigurationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uXConfigurationConsultantAgent = Object.freeze(new UXConfigurationConsultantAgent());
