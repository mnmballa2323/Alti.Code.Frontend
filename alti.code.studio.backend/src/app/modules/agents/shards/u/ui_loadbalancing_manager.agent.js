...
class UILoadBalancingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateUILoadBalancingSystem(objective) {
    logger.info(
      `💻 [UILoadBalancingManagerAgent] Analyzing UI LoadBalancing Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UI LoadBalancing Manager Target',
      );
...
export const uILoadBalancingManagerAgent = Object.freeze(
  new UILoadBalancingManagerAgent(),
);
