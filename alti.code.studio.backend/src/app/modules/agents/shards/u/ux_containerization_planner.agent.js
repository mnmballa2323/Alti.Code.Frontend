...
class UXContainerizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateUXContainerizationSystem(objective) {
    logger.info(
      `💻 [UXContainerizationPlannerAgent] Analyzing UX Containerization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UX Containerization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UXContainerizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uXContainerizationPlannerAgent = Object.freeze(
  new UXContainerizationPlannerAgent(),
);
