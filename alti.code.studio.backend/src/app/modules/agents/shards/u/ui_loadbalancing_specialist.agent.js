...
class UILoadBalancingSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateUILoadBalancingSystem(objective) {
        logger.info(`💻 [UILoadBalancingSpecialistAgent] Analyzing UI LoadBalancing Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UI LoadBalancing Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UILoadBalancingSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uILoadBalancingSpecialistAgent = Object.freeze(new UILoadBalancingSpecialistAgent());
