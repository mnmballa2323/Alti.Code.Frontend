...
class UIOrchestrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateUIOrchestrationSystem(objective) {
    logger.info(
      `💻 [UIOrchestrationConsultantAgent] Analyzing UI Orchestration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UI Orchestration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UIOrchestrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uIOrchestrationConsultantAgent = Object.freeze(
  new UIOrchestrationConsultantAgent(),
);
