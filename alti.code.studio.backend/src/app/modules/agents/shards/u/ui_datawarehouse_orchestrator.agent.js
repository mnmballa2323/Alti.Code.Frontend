...
class UIDataWarehouseOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateUIDataWarehouseSystem(objective) {
    logger.info(
      `💻 [UIDataWarehouseOrchestratorAgent] Analyzing UI DataWarehouse Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UI DataWarehouse Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UIDataWarehouseOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uIDataWarehouseOrchestratorAgent = Object.freeze(
  new UIDataWarehouseOrchestratorAgent(),
);
