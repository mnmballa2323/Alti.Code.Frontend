...
class UXContainerizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateUXContainerizationSystem(objective) {
    logger.info(
      `💻 [UXContainerizationAuditorAgent] Analyzing UX Containerization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UX Containerization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UXContainerizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uXContainerizationAuditorAgent = Object.freeze(
  new UXContainerizationAuditorAgent(),
);
