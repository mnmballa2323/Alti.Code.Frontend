...
class UIContainerizationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateUIContainerizationSystem(objective) {
        logger.info(`💻 [UIContainerizationManagerAgent] Analyzing UI Containerization Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UI Containerization Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UIContainerizationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uIContainerizationManagerAgent = Object.freeze(new UIContainerizationManagerAgent());
