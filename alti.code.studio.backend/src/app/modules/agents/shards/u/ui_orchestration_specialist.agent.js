...
class UIOrchestrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateUIOrchestrationSystem(objective) {
    logger.info(
      `💻 [UIOrchestrationSpecialistAgent] Analyzing UI Orchestration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UI Orchestration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UIOrchestrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uIOrchestrationSpecialistAgent = Object.freeze(
  new UIOrchestrationSpecialistAgent(),
);
