...
class UXDataWarehouseConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateUXDataWarehouseSystem(objective) {
        logger.info(`💻 [UXDataWarehouseConsultantAgent] Analyzing UX DataWarehouse Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UX DataWarehouse Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UXDataWarehouseConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uXDataWarehouseConsultantAgent = Object.freeze(new UXDataWarehouseConsultantAgent());
