...
class UXSecurityPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateUXSecuritySystem(objective) {
    logger.info(
      `💻 [UXSecurityPlannerAgent] Analyzing UX Security Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UX Security Planner Target',
      );
...
export const uXSecurityPlannerAgent = Object.freeze(
  new UXSecurityPlannerAgent(),
);
