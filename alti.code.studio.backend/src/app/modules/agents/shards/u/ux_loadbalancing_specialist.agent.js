...
class UXLoadBalancingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateUXLoadBalancingSystem(objective) {
    logger.info(
      `💻 [UXLoadBalancingSpecialistAgent] Analyzing UX LoadBalancing Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UX LoadBalancing Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UXLoadBalancingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uXLoadBalancingSpecialistAgent = Object.freeze(
  new UXLoadBalancingSpecialistAgent(),
);
