...
class UXObservabilitySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateUXObservabilitySystem(objective) {
    logger.info(
      `💻 [UXObservabilitySpecialistAgent] Analyzing UX Observability Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UX Observability Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UXObservabilitySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uXObservabilitySpecialistAgent = Object.freeze(
  new UXObservabilitySpecialistAgent(),
);
