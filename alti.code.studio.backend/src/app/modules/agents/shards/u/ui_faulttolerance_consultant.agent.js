...
class UIFaultToleranceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateUIFaultToleranceSystem(objective) {
    logger.info(
      `💻 [UIFaultToleranceConsultantAgent] Analyzing UI FaultTolerance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UI FaultTolerance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UIFaultToleranceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uIFaultToleranceConsultantAgent = Object.freeze(
  new UIFaultToleranceConsultantAgent(),
);
