...
class UXContainerizationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateUXContainerizationSystem(objective) {
    logger.info(
      `💻 [UXContainerizationArchitectAgent] Analyzing UX Containerization Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UX Containerization Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UXContainerizationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uXContainerizationArchitectAgent = Object.freeze(
  new UXContainerizationArchitectAgent(),
);
