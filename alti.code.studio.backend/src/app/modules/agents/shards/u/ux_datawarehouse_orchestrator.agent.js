...
class UXDataWarehouseOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateUXDataWarehouseSystem(objective) {
        logger.info(`💻 [UXDataWarehouseOrchestratorAgent] Analyzing UX DataWarehouse Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UX DataWarehouse Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UXDataWarehouseOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uXDataWarehouseOrchestratorAgent = Object.freeze(new UXDataWarehouseOrchestratorAgent());
