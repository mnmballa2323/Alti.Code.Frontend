...
class UXContainerizationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateUXContainerizationSystem(objective) {
        logger.info(`💻 [UXContainerizationManagerAgent] Analyzing UX Containerization Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UX Containerization Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UXContainerizationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uXContainerizationManagerAgent = Object.freeze(new UXContainerizationManagerAgent());
