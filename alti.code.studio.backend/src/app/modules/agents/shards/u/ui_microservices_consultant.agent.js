...
class UIMicroservicesConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateUIMicroservicesSystem(objective) {
        logger.info(`💻 [UIMicroservicesConsultantAgent] Analyzing UI Microservices Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UI Microservices Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UIMicroservicesConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uIMicroservicesConsultantAgent = Object.freeze(new UIMicroservicesConsultantAgent());
