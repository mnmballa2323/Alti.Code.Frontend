...
class UXConfigurationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateUXConfigurationSystem(objective) {
    logger.info(
      `💻 [UXConfigurationSpecialistAgent] Analyzing UX Configuration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UX Configuration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UXConfigurationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uXConfigurationSpecialistAgent = Object.freeze(
  new UXConfigurationSpecialistAgent(),
);
