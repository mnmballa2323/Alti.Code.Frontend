...
class UIContainerizationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateUIContainerizationSystem(objective) {
        logger.info(`💻 [UIContainerizationDesignerAgent] Analyzing UI Containerization Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UI Containerization Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UIContainerizationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uIContainerizationDesignerAgent = Object.freeze(new UIContainerizationDesignerAgent());
