...
class UIEngineeringOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateUIEngineeringSystem(objective) {
    logger.info(
      `💻 [UIEngineeringOrchestratorAgent] Analyzing UI Engineering Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UI Engineering Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UIEngineeringOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uIEngineeringOrchestratorAgent = Object.freeze(
  new UIEngineeringOrchestratorAgent(),
);
