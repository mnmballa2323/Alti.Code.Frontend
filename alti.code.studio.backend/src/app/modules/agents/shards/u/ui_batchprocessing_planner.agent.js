...
class UIBatchProcessingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateUIBatchProcessingSystem(objective) {
    logger.info(
      `💻 [UIBatchProcessingPlannerAgent] Analyzing UI BatchProcessing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UI BatchProcessing Planner Target',
      );
...
export const uIBatchProcessingPlannerAgent = Object.freeze(
  new UIBatchProcessingPlannerAgent(),
);
