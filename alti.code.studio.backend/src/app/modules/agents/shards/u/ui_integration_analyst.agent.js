...
class UIIntegrationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateUIIntegrationSystem(objective) {
        logger.info(`💻 [UIIntegrationAnalystAgent] Analyzing UI Integration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UI Integration Analyst Target");
...
export const uIIntegrationAnalystAgent = Object.freeze(new UIIntegrationAnalystAgent());
