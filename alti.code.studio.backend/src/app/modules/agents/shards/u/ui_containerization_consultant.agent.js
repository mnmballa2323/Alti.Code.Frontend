...
class UIContainerizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateUIContainerizationSystem(objective) {
    logger.info(
      `💻 [UIContainerizationConsultantAgent] Analyzing UI Containerization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UI Containerization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UIContainerizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uIContainerizationConsultantAgent = Object.freeze(
  new UIContainerizationConsultantAgent(),
);
