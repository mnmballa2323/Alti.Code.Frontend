...
class UXContainerizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateUXContainerizationSystem(objective) {
    logger.info(
      `💻 [UXContainerizationConsultantAgent] Analyzing UX Containerization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UX Containerization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UXContainerizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uXContainerizationConsultantAgent = Object.freeze(
  new UXContainerizationConsultantAgent(),
);
