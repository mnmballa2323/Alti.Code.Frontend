...
class UXContainerizationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateUXContainerizationSystem(objective) {
        logger.info(`💻 [UXContainerizationAnalystAgent] Analyzing UX Containerization Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UX Containerization Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UXContainerizationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uXContainerizationAnalystAgent = Object.freeze(new UXContainerizationAnalystAgent());
