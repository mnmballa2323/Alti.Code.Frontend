...
class UXOrchestrationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateUXOrchestrationSystem(objective) {
        logger.info(`💻 [UXOrchestrationSpecialistAgent] Analyzing UX Orchestration Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UX Orchestration Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UXOrchestrationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uXOrchestrationSpecialistAgent = Object.freeze(new UXOrchestrationSpecialistAgent());
