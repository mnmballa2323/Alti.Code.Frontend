...
class UIContainerizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateUIContainerizationSystem(objective) {
    logger.info(
      `💻 [UIContainerizationAuditorAgent] Analyzing UI Containerization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UI Containerization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UIContainerizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uIContainerizationAuditorAgent = Object.freeze(
  new UIContainerizationAuditorAgent(),
);
