...
class UXProvisioningOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateUXProvisioningSystem(objective) {
        logger.info(`💻 [UXProvisioningOrchestratorAgent] Analyzing UX Provisioning Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UX Provisioning Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UXProvisioningOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uXProvisioningOrchestratorAgent = Object.freeze(new UXProvisioningOrchestratorAgent());
