...
class UXOrchestrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateUXOrchestrationSystem(objective) {
    logger.info(
      `💻 [UXOrchestrationOrchestratorAgent] Analyzing UX Orchestration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UX Orchestration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UXOrchestrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uXOrchestrationOrchestratorAgent = Object.freeze(
  new UXOrchestrationOrchestratorAgent(),
);
