...
class UXBatchProcessingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateUXBatchProcessingSystem(objective) {
    logger.info(
      `💻 [UXBatchProcessingDirectorAgent] Analyzing UX BatchProcessing Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UX BatchProcessing Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UXBatchProcessingDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uXBatchProcessingDirectorAgent = Object.freeze(
  new UXBatchProcessingDirectorAgent(),
);
