...
class UXContainerizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateUXContainerizationSystem(objective) {
    logger.info(
      `💻 [UXContainerizationDirectorAgent] Analyzing UX Containerization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UX Containerization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UXContainerizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uXContainerizationDirectorAgent = Object.freeze(
  new UXContainerizationDirectorAgent(),
);
