...
class UIContainerizationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateUIContainerizationSystem(objective) {
    logger.info(
      `💻 [UIContainerizationArchitectAgent] Analyzing UI Containerization Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UI Containerization Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UIContainerizationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uIContainerizationArchitectAgent = Object.freeze(
  new UIContainerizationArchitectAgent(),
);
