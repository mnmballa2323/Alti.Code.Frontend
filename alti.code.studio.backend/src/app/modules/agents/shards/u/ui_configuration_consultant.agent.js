...
class UIConfigurationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateUIConfigurationSystem(objective) {
        logger.info(`💻 [UIConfigurationConsultantAgent] Analyzing UI Configuration Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UI Configuration Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UIConfigurationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uIConfigurationConsultantAgent = Object.freeze(new UIConfigurationConsultantAgent());
