...
class UXArchitectureOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateUXArchitectureSystem(objective) {
        logger.info(`💻 [UXArchitectureOrchestratorAgent] Analyzing UX Architecture Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UX Architecture Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UXArchitectureOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uXArchitectureOrchestratorAgent = Object.freeze(new UXArchitectureOrchestratorAgent());
