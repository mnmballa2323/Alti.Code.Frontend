...
class UISecurityPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateUISecuritySystem(objective) {
    logger.info(
      `💻 [UISecurityPlannerAgent] Analyzing UI Security Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UI Security Planner Target',
      );
...
export const uISecurityPlannerAgent = Object.freeze(
  new UISecurityPlannerAgent(),
);
