...
class UIContainerizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateUIContainerizationSystem(objective) {
    logger.info(
      `💻 [UIContainerizationAnalystAgent] Analyzing UI Containerization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UI Containerization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UIContainerizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uIContainerizationAnalystAgent = Object.freeze(
  new UIContainerizationAnalystAgent(),
);
