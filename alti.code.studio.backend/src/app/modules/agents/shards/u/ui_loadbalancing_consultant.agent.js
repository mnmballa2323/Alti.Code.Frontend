...
class UILoadBalancingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateUILoadBalancingSystem(objective) {
        logger.info(`💻 [UILoadBalancingConsultantAgent] Analyzing UI LoadBalancing Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UI LoadBalancing Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UILoadBalancingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uILoadBalancingConsultantAgent = Object.freeze(new UILoadBalancingConsultantAgent());
