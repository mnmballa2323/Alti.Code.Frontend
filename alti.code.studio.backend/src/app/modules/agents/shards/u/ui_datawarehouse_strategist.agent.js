...
class UIDataWarehouseStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateUIDataWarehouseSystem(objective) {
    logger.info(
      `💻 [UIDataWarehouseStrategistAgent] Analyzing UI DataWarehouse Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UI DataWarehouse Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UIDataWarehouseStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uIDataWarehouseStrategistAgent = Object.freeze(
  new UIDataWarehouseStrategistAgent(),
);
