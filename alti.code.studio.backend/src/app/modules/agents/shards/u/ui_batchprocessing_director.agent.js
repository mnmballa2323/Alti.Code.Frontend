...
class UIBatchProcessingDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateUIBatchProcessingSystem(objective) {
        logger.info(`💻 [UIBatchProcessingDirectorAgent] Analyzing UI BatchProcessing Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UI BatchProcessing Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [UIBatchProcessingDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const uIBatchProcessingDirectorAgent = Object.freeze(new UIBatchProcessingDirectorAgent());
