...
class UIContainerizationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateUIContainerizationSystem(objective) {
    logger.info(
      `💻 [UIContainerizationDeveloperAgent] Analyzing UI Containerization Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UI Containerization Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UIContainerizationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uIContainerizationDeveloperAgent = Object.freeze(
  new UIContainerizationDeveloperAgent(),
);
