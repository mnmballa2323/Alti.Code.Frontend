...
class UXServerlessDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateUXServerlessSystem(objective) {
        logger.info(`💻 [UXServerlessDirectorAgent] Analyzing UX Serverless Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - UX Serverless Director Target");
...
export const uXServerlessDirectorAgent = Object.freeze(new UXServerlessDirectorAgent());
