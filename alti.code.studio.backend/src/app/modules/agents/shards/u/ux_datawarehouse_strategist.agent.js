...
class UXDataWarehouseStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateUXDataWarehouseSystem(objective) {
    logger.info(
      `💻 [UXDataWarehouseStrategistAgent] Analyzing UX DataWarehouse Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - UX DataWarehouse Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [UXDataWarehouseStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const uXDataWarehouseStrategistAgent = Object.freeze(
  new UXDataWarehouseStrategistAgent(),
);
