...
class QuantumComputingResearcherAgent extends GeminiCliBaseAgent {
    ...
    async generateQuantumSystem(quantumObjective) {
        logger.info(`⚛️ [QuantumComputingResearcher] Analyzing objective for quantum circuits and error correction...`);
...
${quantumObjective}
        ...
        try {
            ...
            const cleanCode = output.replace(/```javascript|```typescript|```json|```python|```qsharp|```/gi, '').trim();
            logger.info(`✅ [QuantumComputingResearcher] Quantum architecture generated successfully.`);
            return cleanCode;
        } catch (err) {
            logger.error(`❌ [QuantumComputingResearcher] Failed to generate quantum system: ${err.message}`);
            throw err;
        }
    }
}

export const quantumComputingResearcherAgent = Object.freeze(new QuantumComputingResearcherAgent());
