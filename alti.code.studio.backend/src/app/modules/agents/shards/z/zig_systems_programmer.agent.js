...
class ZigSystemsProgrammerAgent extends GeminiCliBaseAgent {
  ...
  async generateZigSystem(objective) {
    logger.info(
      `⚡ [ZigSystemsProgrammer] Analyzing comptime evaluation and allocators...`,
    );
    ...
    try {
      ...
      return output
        .replace(/```javascript|```typescript|```json|```zig|```/gi, '')
        .trim();
    } catch (err) {
...
export const zigSystemsProgrammerAgent = Object.freeze(
  new ZigSystemsProgrammerAgent(),
);
