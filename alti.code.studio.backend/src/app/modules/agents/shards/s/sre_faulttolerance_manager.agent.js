...
class SREFaultToleranceManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSREFaultToleranceSystem(objective) {
    logger.info(
      `💻 [SREFaultToleranceManagerAgent] Analyzing SRE FaultTolerance Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE FaultTolerance Manager Target',
      );
...
export const sREFaultToleranceManagerAgent = Object.freeze(
  new SREFaultToleranceManagerAgent(),
);
