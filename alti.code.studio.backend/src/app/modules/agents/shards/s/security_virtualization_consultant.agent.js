...
class SecurityVirtualizationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityVirtualizationSystem(objective) {
        logger.info(`💻 [SecurityVirtualizationConsultantAgent] Analyzing Security Virtualization Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Virtualization Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityVirtualizationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityVirtualizationConsultantAgent = Object.freeze(new SecurityVirtualizationConsultantAgent());
