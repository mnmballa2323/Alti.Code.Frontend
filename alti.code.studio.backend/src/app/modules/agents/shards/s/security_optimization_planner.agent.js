...
class SecurityOptimizationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityOptimizationSystem(objective) {
        logger.info(`💻 [SecurityOptimizationPlannerAgent] Analyzing Security Optimization Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Optimization Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityOptimizationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityOptimizationPlannerAgent = Object.freeze(new SecurityOptimizationPlannerAgent());
