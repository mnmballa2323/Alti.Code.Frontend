...
class SiteBatchProcessingPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteBatchProcessingSystem(objective) {
        logger.info(`💻 [SiteBatchProcessingPlannerAgent] Analyzing Site BatchProcessing Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site BatchProcessing Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteBatchProcessingPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteBatchProcessingPlannerAgent = Object.freeze(new SiteBatchProcessingPlannerAgent());
