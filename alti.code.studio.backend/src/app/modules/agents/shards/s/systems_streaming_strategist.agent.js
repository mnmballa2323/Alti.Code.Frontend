...
class SystemsStreamingStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsStreamingSystem(objective) {
        logger.info(`💻 [SystemsStreamingStrategistAgent] Analyzing Systems Streaming Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Streaming Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsStreamingStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsStreamingStrategistAgent = Object.freeze(new SystemsStreamingStrategistAgent());
