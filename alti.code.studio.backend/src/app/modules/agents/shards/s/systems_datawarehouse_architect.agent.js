...
class SystemsDataWarehouseArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsDataWarehouseSystem(objective) {
        logger.info(`💻 [SystemsDataWarehouseArchitectAgent] Analyzing Systems DataWarehouse Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems DataWarehouse Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsDataWarehouseArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsDataWarehouseArchitectAgent = Object.freeze(new SystemsDataWarehouseArchitectAgent());
