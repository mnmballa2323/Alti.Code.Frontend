...
class SREContainerizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSREContainerizationSystem(objective) {
    logger.info(
      `💻 [SREContainerizationDesignerAgent] Analyzing SRE Containerization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Containerization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREContainerizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREContainerizationDesignerAgent = Object.freeze(
  new SREContainerizationDesignerAgent(),
);
