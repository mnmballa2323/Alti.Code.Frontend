...
class SRELoadBalancingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSRELoadBalancingSystem(objective) {
    logger.info(
      `💻 [SRELoadBalancingDesignerAgent] Analyzing SRE LoadBalancing Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE LoadBalancing Designer Target',
      );
...
export const sRELoadBalancingDesignerAgent = Object.freeze(
  new SRELoadBalancingDesignerAgent(),
);
