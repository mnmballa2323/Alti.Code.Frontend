...
class SecurityObservabilityAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityObservabilitySystem(objective) {
        logger.info(`💻 [SecurityObservabilityAnalystAgent] Analyzing Security Observability Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Observability Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityObservabilityAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityObservabilityAnalystAgent = Object.freeze(new SecurityObservabilityAnalystAgent());
