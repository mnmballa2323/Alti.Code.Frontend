...
class SystemsOrchestrationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsOrchestrationSystem(objective) {
    logger.info(
      `💻 [SystemsOrchestrationDeveloperAgent] Analyzing Systems Orchestration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Orchestration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsOrchestrationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsOrchestrationDeveloperAgent = Object.freeze(
  new SystemsOrchestrationDeveloperAgent(),
);
