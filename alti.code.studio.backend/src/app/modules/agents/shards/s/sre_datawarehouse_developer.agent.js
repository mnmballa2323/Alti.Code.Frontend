...
class SREDataWarehouseDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSREDataWarehouseSystem(objective) {
    logger.info(
      `💻 [SREDataWarehouseDeveloperAgent] Analyzing SRE DataWarehouse Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE DataWarehouse Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREDataWarehouseDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREDataWarehouseDeveloperAgent = Object.freeze(
  new SREDataWarehouseDeveloperAgent(),
);
