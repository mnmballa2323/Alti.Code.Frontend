...
class SystemsFaultToleranceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsFaultToleranceSystem(objective) {
        logger.info(`💻 [SystemsFaultToleranceConsultantAgent] Analyzing Systems FaultTolerance Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems FaultTolerance Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsFaultToleranceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsFaultToleranceConsultantAgent = Object.freeze(new SystemsFaultToleranceConsultantAgent());
