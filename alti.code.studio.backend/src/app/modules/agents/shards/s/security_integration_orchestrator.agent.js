...
class SecurityIntegrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityIntegrationSystem(objective) {
    logger.info(
      `💻 [SecurityIntegrationOrchestratorAgent] Analyzing Security Integration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Integration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityIntegrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityIntegrationOrchestratorAgent = Object.freeze(
  new SecurityIntegrationOrchestratorAgent(),
);
