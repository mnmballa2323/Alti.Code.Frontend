...
class SiteMicroservicesDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteMicroservicesSystem(objective) {
        logger.info(`💻 [SiteMicroservicesDesignerAgent] Analyzing Site Microservices Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Microservices Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteMicroservicesDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteMicroservicesDesignerAgent = Object.freeze(new SiteMicroservicesDesignerAgent());
