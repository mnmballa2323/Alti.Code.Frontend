...
class SREProvisioningOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSREProvisioningSystem(objective) {
    logger.info(
      `💻 [SREProvisioningOrchestratorAgent] Analyzing SRE Provisioning Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Provisioning Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREProvisioningOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREProvisioningOrchestratorAgent = Object.freeze(
  new SREProvisioningOrchestratorAgent(),
);
