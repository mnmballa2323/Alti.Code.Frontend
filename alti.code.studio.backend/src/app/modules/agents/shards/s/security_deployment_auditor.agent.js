...
class SecurityDeploymentAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityDeploymentSystem(objective) {
        logger.info(`💻 [SecurityDeploymentAuditorAgent] Analyzing Security Deployment Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Deployment Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityDeploymentAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityDeploymentAuditorAgent = Object.freeze(new SecurityDeploymentAuditorAgent());
