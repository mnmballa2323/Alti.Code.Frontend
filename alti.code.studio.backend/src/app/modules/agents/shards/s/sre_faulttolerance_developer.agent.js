...
class SREFaultToleranceDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSREFaultToleranceSystem(objective) {
        logger.info(`💻 [SREFaultToleranceDeveloperAgent] Analyzing SRE FaultTolerance Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE FaultTolerance Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SREFaultToleranceDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const sREFaultToleranceDeveloperAgent = Object.freeze(new SREFaultToleranceDeveloperAgent());
