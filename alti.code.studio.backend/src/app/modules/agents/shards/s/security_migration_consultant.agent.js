...
class SecurityMigrationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityMigrationSystem(objective) {
        logger.info(`💻 [SecurityMigrationConsultantAgent] Analyzing Security Migration Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Migration Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityMigrationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityMigrationConsultantAgent = Object.freeze(new SecurityMigrationConsultantAgent());
