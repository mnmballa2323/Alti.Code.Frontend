...
class SecurityOptimizationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityOptimizationSystem(objective) {
    logger.info(
      `💻 [SecurityOptimizationStrategistAgent] Analyzing Security Optimization Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Optimization Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityOptimizationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityOptimizationStrategistAgent = Object.freeze(
  new SecurityOptimizationStrategistAgent(),
);
