...
class SystemsServerlessDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsServerlessSystem(objective) {
    logger.info(
      `💻 [SystemsServerlessDesignerAgent] Analyzing Systems Serverless Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Serverless Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsServerlessDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsServerlessDesignerAgent = Object.freeze(
  new SystemsServerlessDesignerAgent(),
);
