...
class SystemsConfigurationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsConfigurationSystem(objective) {
    logger.info(
      `💻 [SystemsConfigurationArchitectAgent] Analyzing Systems Configuration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Configuration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsConfigurationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsConfigurationArchitectAgent = Object.freeze(
  new SystemsConfigurationArchitectAgent(),
);
