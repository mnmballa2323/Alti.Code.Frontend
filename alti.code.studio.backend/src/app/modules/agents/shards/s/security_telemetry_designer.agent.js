...
class SecurityTelemetryDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityTelemetrySystem(objective) {
        logger.info(`💻 [SecurityTelemetryDesignerAgent] Analyzing Security Telemetry Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Telemetry Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityTelemetryDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityTelemetryDesignerAgent = Object.freeze(new SecurityTelemetryDesignerAgent());
