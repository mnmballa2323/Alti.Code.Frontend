...
class SystemsConfigurationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsConfigurationSystem(objective) {
    logger.info(
      `💻 [SystemsConfigurationDeveloperAgent] Analyzing Systems Configuration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Configuration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsConfigurationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsConfigurationDeveloperAgent = Object.freeze(
  new SystemsConfigurationDeveloperAgent(),
);
