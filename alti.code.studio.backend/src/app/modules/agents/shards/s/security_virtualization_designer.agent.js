...
class SecurityVirtualizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityVirtualizationSystem(objective) {
    logger.info(
      `💻 [SecurityVirtualizationDesignerAgent] Analyzing Security Virtualization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Virtualization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityVirtualizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityVirtualizationDesignerAgent = Object.freeze(
  new SecurityVirtualizationDesignerAgent(),
);
