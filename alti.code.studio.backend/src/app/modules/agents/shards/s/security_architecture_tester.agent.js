...
class SecurityArchitectureTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityArchitectureSystem(objective) {
        logger.info(`💻 [SecurityArchitectureTesterAgent] Analyzing Security Architecture Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Architecture Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityArchitectureTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityArchitectureTesterAgent = Object.freeze(new SecurityArchitectureTesterAgent());
