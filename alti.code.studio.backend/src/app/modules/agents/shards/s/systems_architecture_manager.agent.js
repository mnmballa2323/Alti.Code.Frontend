...
class SystemsArchitectureManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsArchitectureSystem(objective) {
    logger.info(
      `💻 [SystemsArchitectureManagerAgent] Analyzing Systems Architecture Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Architecture Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsArchitectureManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsArchitectureManagerAgent = Object.freeze(
  new SystemsArchitectureManagerAgent(),
);
