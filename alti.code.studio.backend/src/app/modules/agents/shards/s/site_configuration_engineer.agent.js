...
class SiteConfigurationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteConfigurationSystem(objective) {
    logger.info(
      `💻 [SiteConfigurationEngineerAgent] Analyzing Site Configuration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Configuration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteConfigurationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteConfigurationEngineerAgent = Object.freeze(
  new SiteConfigurationEngineerAgent(),
);
