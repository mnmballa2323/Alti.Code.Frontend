...
class SystemsFaultToleranceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsFaultToleranceSystem(objective) {
    logger.info(
      `💻 [SystemsFaultToleranceDesignerAgent] Analyzing Systems FaultTolerance Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems FaultTolerance Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsFaultToleranceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsFaultToleranceDesignerAgent = Object.freeze(
  new SystemsFaultToleranceDesignerAgent(),
);
