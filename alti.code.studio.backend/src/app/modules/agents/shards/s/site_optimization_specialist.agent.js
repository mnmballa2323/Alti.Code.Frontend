...
class SiteOptimizationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteOptimizationSystem(objective) {
        logger.info(`💻 [SiteOptimizationSpecialistAgent] Analyzing Site Optimization Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Optimization Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteOptimizationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteOptimizationSpecialistAgent = Object.freeze(new SiteOptimizationSpecialistAgent());
