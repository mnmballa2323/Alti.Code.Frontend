...
class SiteOrchestrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteOrchestrationSystem(objective) {
    logger.info(
      `💻 [SiteOrchestrationSpecialistAgent] Analyzing Site Orchestration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Orchestration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteOrchestrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteOrchestrationSpecialistAgent = Object.freeze(
  new SiteOrchestrationSpecialistAgent(),
);
