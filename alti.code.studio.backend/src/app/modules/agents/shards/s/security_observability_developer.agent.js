...
class SecurityObservabilityDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityObservabilitySystem(objective) {
    logger.info(
      `💻 [SecurityObservabilityDeveloperAgent] Analyzing Security Observability Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Observability Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityObservabilityDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityObservabilityDeveloperAgent = Object.freeze(
  new SecurityObservabilityDeveloperAgent(),
);
