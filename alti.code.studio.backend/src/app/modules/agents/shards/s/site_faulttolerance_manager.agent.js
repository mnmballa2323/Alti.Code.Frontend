...
class SiteFaultToleranceManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteFaultToleranceSystem(objective) {
        logger.info(`💻 [SiteFaultToleranceManagerAgent] Analyzing Site FaultTolerance Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site FaultTolerance Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteFaultToleranceManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteFaultToleranceManagerAgent = Object.freeze(new SiteFaultToleranceManagerAgent());
