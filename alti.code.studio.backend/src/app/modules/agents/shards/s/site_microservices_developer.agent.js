...
class SiteMicroservicesDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteMicroservicesSystem(objective) {
    logger.info(
      `💻 [SiteMicroservicesDeveloperAgent] Analyzing Site Microservices Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Microservices Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteMicroservicesDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteMicroservicesDeveloperAgent = Object.freeze(
  new SiteMicroservicesDeveloperAgent(),
);
