...
class SiteConfigurationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteConfigurationSystem(objective) {
    logger.info(
      `💻 [SiteConfigurationConsultantAgent] Analyzing Site Configuration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Configuration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteConfigurationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteConfigurationConsultantAgent = Object.freeze(
  new SiteConfigurationConsultantAgent(),
);
