...
class SystemsLoadBalancingAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsLoadBalancingSystem(objective) {
    logger.info(
      `💻 [SystemsLoadBalancingAnalystAgent] Analyzing Systems LoadBalancing Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems LoadBalancing Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsLoadBalancingAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsLoadBalancingAnalystAgent = Object.freeze(
  new SystemsLoadBalancingAnalystAgent(),
);
