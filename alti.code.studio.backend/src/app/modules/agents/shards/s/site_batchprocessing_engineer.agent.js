...
class SiteBatchProcessingEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteBatchProcessingSystem(objective) {
    logger.info(
      `💻 [SiteBatchProcessingEngineerAgent] Analyzing Site BatchProcessing Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site BatchProcessing Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteBatchProcessingEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteBatchProcessingEngineerAgent = Object.freeze(
  new SiteBatchProcessingEngineerAgent(),
);
