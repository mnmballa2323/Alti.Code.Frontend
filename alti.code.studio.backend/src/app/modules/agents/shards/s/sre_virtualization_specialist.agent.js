...
class SREVirtualizationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSREVirtualizationSystem(objective) {
    logger.info(
      `💻 [SREVirtualizationSpecialistAgent] Analyzing SRE Virtualization Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Virtualization Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREVirtualizationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREVirtualizationSpecialistAgent = Object.freeze(
  new SREVirtualizationSpecialistAgent(),
);
