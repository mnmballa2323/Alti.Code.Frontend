...
class SecurityTelemetryArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityTelemetrySystem(objective) {
        logger.info(`💻 [SecurityTelemetryArchitectAgent] Analyzing Security Telemetry Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Telemetry Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityTelemetryArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityTelemetryArchitectAgent = Object.freeze(new SecurityTelemetryArchitectAgent());
