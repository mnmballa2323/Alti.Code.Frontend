...
class SiteObservabilityArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteObservabilitySystem(objective) {
    logger.info(
      `💻 [SiteObservabilityArchitectAgent] Analyzing Site Observability Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Observability Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteObservabilityArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteObservabilityArchitectAgent = Object.freeze(
  new SiteObservabilityArchitectAgent(),
);
