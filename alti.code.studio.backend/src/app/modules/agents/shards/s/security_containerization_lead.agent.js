...
class SecurityContainerizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityContainerizationSystem(objective) {
    logger.info(
      `💻 [SecurityContainerizationLeadAgent] Analyzing Security Containerization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Containerization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityContainerizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityContainerizationLeadAgent = Object.freeze(
  new SecurityContainerizationLeadAgent(),
);
