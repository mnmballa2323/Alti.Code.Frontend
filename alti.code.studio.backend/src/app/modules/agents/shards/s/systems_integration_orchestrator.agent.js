...
class SystemsIntegrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsIntegrationSystem(objective) {
    logger.info(
      `💻 [SystemsIntegrationOrchestratorAgent] Analyzing Systems Integration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Integration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsIntegrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsIntegrationOrchestratorAgent = Object.freeze(
  new SystemsIntegrationOrchestratorAgent(),
);
