...
class SystemsDataLakeStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsDataLakeSystem(objective) {
    logger.info(
      `💻 [SystemsDataLakeStrategistAgent] Analyzing Systems DataLake Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems DataLake Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsDataLakeStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsDataLakeStrategistAgent = Object.freeze(
  new SystemsDataLakeStrategistAgent(),
);
