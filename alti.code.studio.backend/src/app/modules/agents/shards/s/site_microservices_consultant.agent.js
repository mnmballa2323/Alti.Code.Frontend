...
class SiteMicroservicesConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteMicroservicesSystem(objective) {
    logger.info(
      `💻 [SiteMicroservicesConsultantAgent] Analyzing Site Microservices Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Microservices Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteMicroservicesConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteMicroservicesConsultantAgent = Object.freeze(
  new SiteMicroservicesConsultantAgent(),
);
