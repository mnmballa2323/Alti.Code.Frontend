...
class SecurityIntegrationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityIntegrationSystem(objective) {
    logger.info(
      `💻 [SecurityIntegrationPlannerAgent] Analyzing Security Integration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Integration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityIntegrationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityIntegrationPlannerAgent = Object.freeze(
  new SecurityIntegrationPlannerAgent(),
);
