...
class SiteContainerizationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteContainerizationSystem(objective) {
        logger.info(`💻 [SiteContainerizationDesignerAgent] Analyzing Site Containerization Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Containerization Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteContainerizationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteContainerizationDesignerAgent = Object.freeze(new SiteContainerizationDesignerAgent());
