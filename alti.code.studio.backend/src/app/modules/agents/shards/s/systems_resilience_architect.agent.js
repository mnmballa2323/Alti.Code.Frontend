...
class SystemsResilienceArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsResilienceSystem(objective) {
    logger.info(
      `💻 [SystemsResilienceArchitectAgent] Analyzing Systems Resilience Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Resilience Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsResilienceArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsResilienceArchitectAgent = Object.freeze(
  new SystemsResilienceArchitectAgent(),
);
