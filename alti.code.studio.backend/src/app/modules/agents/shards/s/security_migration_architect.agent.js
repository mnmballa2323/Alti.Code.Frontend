...
class SecurityMigrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityMigrationSystem(objective) {
    logger.info(
      `💻 [SecurityMigrationArchitectAgent] Analyzing Security Migration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Migration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityMigrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityMigrationArchitectAgent = Object.freeze(
  new SecurityMigrationArchitectAgent(),
);
