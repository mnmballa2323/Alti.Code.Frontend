...
class SystemsObservabilityConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsObservabilitySystem(objective) {
        logger.info(`💻 [SystemsObservabilityConsultantAgent] Analyzing Systems Observability Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Observability Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsObservabilityConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsObservabilityConsultantAgent = Object.freeze(new SystemsObservabilityConsultantAgent());
