...
class SREOptimizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSREOptimizationSystem(objective) {
    logger.info(
      `💻 [SREOptimizationConsultantAgent] Analyzing SRE Optimization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Optimization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREOptimizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREOptimizationConsultantAgent = Object.freeze(
  new SREOptimizationConsultantAgent(),
);
