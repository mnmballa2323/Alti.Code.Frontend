...
class SecurityIntegrationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityIntegrationSystem(objective) {
    logger.info(
      `💻 [SecurityIntegrationManagerAgent] Analyzing Security Integration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Integration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityIntegrationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityIntegrationManagerAgent = Object.freeze(
  new SecurityIntegrationManagerAgent(),
);
