...
class SREStreamingAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSREStreamingSystem(objective) {
    logger.info(
      `💻 [SREStreamingAnalystAgent] Analyzing SRE Streaming Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Streaming Analyst Target',
      );
...
export const sREStreamingAnalystAgent = Object.freeze(
  new SREStreamingAnalystAgent(),
);
