...
class SecurityGovernanceDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityGovernanceSystem(objective) {
        logger.info(`💻 [SecurityGovernanceDeveloperAgent] Analyzing Security Governance Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Governance Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityGovernanceDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityGovernanceDeveloperAgent = Object.freeze(new SecurityGovernanceDeveloperAgent());
