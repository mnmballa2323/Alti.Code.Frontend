...
class SREAutomationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSREAutomationSystem(objective) {
    logger.info(
      `💻 [SREAutomationOrchestratorAgent] Analyzing SRE Automation Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Automation Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREAutomationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREAutomationOrchestratorAgent = Object.freeze(
  new SREAutomationOrchestratorAgent(),
);
