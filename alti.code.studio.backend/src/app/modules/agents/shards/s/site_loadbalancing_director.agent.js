...
class SiteLoadBalancingDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteLoadBalancingSystem(objective) {
        logger.info(`💻 [SiteLoadBalancingDirectorAgent] Analyzing Site LoadBalancing Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site LoadBalancing Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteLoadBalancingDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteLoadBalancingDirectorAgent = Object.freeze(new SiteLoadBalancingDirectorAgent());
