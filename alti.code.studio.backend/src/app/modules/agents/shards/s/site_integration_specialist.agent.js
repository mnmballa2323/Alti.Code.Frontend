...
class SiteIntegrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteIntegrationSystem(objective) {
    logger.info(
      `💻 [SiteIntegrationSpecialistAgent] Analyzing Site Integration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Integration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteIntegrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteIntegrationSpecialistAgent = Object.freeze(
  new SiteIntegrationSpecialistAgent(),
);
