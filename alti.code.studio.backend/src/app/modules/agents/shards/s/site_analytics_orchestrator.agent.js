...
class SiteAnalyticsOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteAnalyticsSystem(objective) {
    logger.info(
      `💻 [SiteAnalyticsOrchestratorAgent] Analyzing Site Analytics Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Analytics Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteAnalyticsOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteAnalyticsOrchestratorAgent = Object.freeze(
  new SiteAnalyticsOrchestratorAgent(),
);
