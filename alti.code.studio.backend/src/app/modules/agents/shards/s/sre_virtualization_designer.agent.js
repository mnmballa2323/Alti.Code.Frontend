...
class SREVirtualizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSREVirtualizationSystem(objective) {
    logger.info(
      `💻 [SREVirtualizationDesignerAgent] Analyzing SRE Virtualization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Virtualization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREVirtualizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREVirtualizationDesignerAgent = Object.freeze(
  new SREVirtualizationDesignerAgent(),
);
