...
class SREConfigurationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSREConfigurationSystem(objective) {
    logger.info(
      `💻 [SREConfigurationConsultantAgent] Analyzing SRE Configuration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Configuration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREConfigurationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREConfigurationConsultantAgent = Object.freeze(
  new SREConfigurationConsultantAgent(),
);
