...
class SecurityComplianceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityComplianceSystem(objective) {
    logger.info(
      `💻 [SecurityComplianceDirectorAgent] Analyzing Security Compliance Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Compliance Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityComplianceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityComplianceDirectorAgent = Object.freeze(
  new SecurityComplianceDirectorAgent(),
);
