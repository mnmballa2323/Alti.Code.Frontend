...
class SiteDataWarehouseSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteDataWarehouseSystem(objective) {
    logger.info(
      `💻 [SiteDataWarehouseSpecialistAgent] Analyzing Site DataWarehouse Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site DataWarehouse Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteDataWarehouseSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteDataWarehouseSpecialistAgent = Object.freeze(
  new SiteDataWarehouseSpecialistAgent(),
);
