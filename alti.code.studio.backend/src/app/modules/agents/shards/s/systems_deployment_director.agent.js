...
class SystemsDeploymentDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsDeploymentSystem(objective) {
    logger.info(
      `💻 [SystemsDeploymentDirectorAgent] Analyzing Systems Deployment Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Deployment Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsDeploymentDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsDeploymentDirectorAgent = Object.freeze(
  new SystemsDeploymentDirectorAgent(),
);
