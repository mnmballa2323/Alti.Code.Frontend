...
class SecurityAutomationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityAutomationSystem(objective) {
    logger.info(
      `💻 [SecurityAutomationEngineerAgent] Analyzing Security Automation Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Automation Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityAutomationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityAutomationEngineerAgent = Object.freeze(
  new SecurityAutomationEngineerAgent(),
);
