...
class SystemsArchitectureOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsArchitectureSystem(objective) {
        logger.info(`💻 [SystemsArchitectureOrchestratorAgent] Analyzing Systems Architecture Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Architecture Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsArchitectureOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsArchitectureOrchestratorAgent = Object.freeze(new SystemsArchitectureOrchestratorAgent());
