...
class SREContainerizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSREContainerizationSystem(objective) {
    logger.info(
      `💻 [SREContainerizationAnalystAgent] Analyzing SRE Containerization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Containerization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREContainerizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREContainerizationAnalystAgent = Object.freeze(
  new SREContainerizationAnalystAgent(),
);
