...
class SecurityServerlessConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityServerlessSystem(objective) {
        logger.info(`💻 [SecurityServerlessConsultantAgent] Analyzing Security Serverless Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Serverless Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityServerlessConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityServerlessConsultantAgent = Object.freeze(new SecurityServerlessConsultantAgent());
