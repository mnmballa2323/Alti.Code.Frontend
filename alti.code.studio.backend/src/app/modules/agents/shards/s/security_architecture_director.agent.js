...
class SecurityArchitectureDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityArchitectureSystem(objective) {
    logger.info(
      `💻 [SecurityArchitectureDirectorAgent] Analyzing Security Architecture Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Architecture Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityArchitectureDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityArchitectureDirectorAgent = Object.freeze(
  new SecurityArchitectureDirectorAgent(),
);
