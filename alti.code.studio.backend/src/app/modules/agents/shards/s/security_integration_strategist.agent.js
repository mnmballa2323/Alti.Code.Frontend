...
class SecurityIntegrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityIntegrationSystem(objective) {
    logger.info(
      `💻 [SecurityIntegrationStrategistAgent] Analyzing Security Integration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Integration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityIntegrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityIntegrationStrategistAgent = Object.freeze(
  new SecurityIntegrationStrategistAgent(),
);
