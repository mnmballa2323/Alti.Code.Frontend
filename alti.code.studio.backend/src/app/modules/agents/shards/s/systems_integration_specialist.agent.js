...
class SystemsIntegrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsIntegrationSystem(objective) {
    logger.info(
      `💻 [SystemsIntegrationSpecialistAgent] Analyzing Systems Integration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Integration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsIntegrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsIntegrationSpecialistAgent = Object.freeze(
  new SystemsIntegrationSpecialistAgent(),
);
