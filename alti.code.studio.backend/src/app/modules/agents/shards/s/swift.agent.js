...
class SwiftAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.description =
      'Language specialist for Swift 6: structured concurrency, actors, SwiftUI, Vapor, and Swift Package Manager.';
...
Output Swift code with explicit access levels (\`public\`, \`internal\`, \`private\`). Use \`guard let\` for early unwrapping and never force-unwrap (\`!\`) in production code.`;
  ...
  async consult(prompt, contextData = []) {
    ...
    try {
      ...
      throw new Error(`Swift Synthesis Failed: ${e.message}`);
    }
  }
}
export const swiftAgent = Object.freeze(new SwiftAgent());
