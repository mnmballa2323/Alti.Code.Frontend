...
class SecurityTelemetryManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityTelemetrySystem(objective) {
    logger.info(
      `💻 [SecurityTelemetryManagerAgent] Analyzing Security Telemetry Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Telemetry Manager Target',
      );
...
export const securityTelemetryManagerAgent = Object.freeze(
  new SecurityTelemetryManagerAgent(),
);
