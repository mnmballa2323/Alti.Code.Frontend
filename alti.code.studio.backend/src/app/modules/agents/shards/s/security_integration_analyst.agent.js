...
class SecurityIntegrationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityIntegrationSystem(objective) {
        logger.info(`💻 [SecurityIntegrationAnalystAgent] Analyzing Security Integration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Integration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityIntegrationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityIntegrationAnalystAgent = Object.freeze(new SecurityIntegrationAnalystAgent());
