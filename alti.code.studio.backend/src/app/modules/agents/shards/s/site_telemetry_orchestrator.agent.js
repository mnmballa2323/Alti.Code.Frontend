...
class SiteTelemetryOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteTelemetrySystem(objective) {
    logger.info(
      `💻 [SiteTelemetryOrchestratorAgent] Analyzing Site Telemetry Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Telemetry Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteTelemetryOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteTelemetryOrchestratorAgent = Object.freeze(
  new SiteTelemetryOrchestratorAgent(),
);
