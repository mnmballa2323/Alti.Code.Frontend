...
class SecurityOrchestrationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityOrchestrationSystem(objective) {
    logger.info(
      `💻 [SecurityOrchestrationManagerAgent] Analyzing Security Orchestration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Orchestration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityOrchestrationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityOrchestrationManagerAgent = Object.freeze(
  new SecurityOrchestrationManagerAgent(),
);
