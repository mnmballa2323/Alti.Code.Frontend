...
class SystemsContainerizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsContainerizationSystem(objective) {
    logger.info(
      `💻 [SystemsContainerizationConsultantAgent] Analyzing Systems Containerization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Containerization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsContainerizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsContainerizationConsultantAgent = Object.freeze(
  new SystemsContainerizationConsultantAgent(),
);
