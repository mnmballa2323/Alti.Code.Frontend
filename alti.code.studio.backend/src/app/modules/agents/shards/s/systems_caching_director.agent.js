...
class SystemsCachingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsCachingSystem(objective) {
    logger.info(
      `💻 [SystemsCachingDirectorAgent] Analyzing Systems Caching Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Caching Director Target',
      );
...
export const systemsCachingDirectorAgent = Object.freeze(
  new SystemsCachingDirectorAgent(),
);
