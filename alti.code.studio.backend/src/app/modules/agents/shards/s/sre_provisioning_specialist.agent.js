...
class SREProvisioningSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSREProvisioningSystem(objective) {
    logger.info(
      `💻 [SREProvisioningSpecialistAgent] Analyzing SRE Provisioning Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Provisioning Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREProvisioningSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREProvisioningSpecialistAgent = Object.freeze(
  new SREProvisioningSpecialistAgent(),
);
