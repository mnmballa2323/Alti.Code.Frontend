...
class SecurityAutomationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityAutomationSystem(objective) {
        logger.info(`💻 [SecurityAutomationPlannerAgent] Analyzing Security Automation Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Automation Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityAutomationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityAutomationPlannerAgent = Object.freeze(new SecurityAutomationPlannerAgent());
