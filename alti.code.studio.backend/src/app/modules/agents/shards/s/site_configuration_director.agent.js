...
class SiteConfigurationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteConfigurationSystem(objective) {
        logger.info(`💻 [SiteConfigurationDirectorAgent] Analyzing Site Configuration Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Configuration Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteConfigurationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteConfigurationDirectorAgent = Object.freeze(new SiteConfigurationDirectorAgent());
