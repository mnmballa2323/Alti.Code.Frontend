...
class SiteConfigurationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteConfigurationSystem(objective) {
        logger.info(`💻 [SiteConfigurationSpecialistAgent] Analyzing Site Configuration Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Configuration Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteConfigurationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteConfigurationSpecialistAgent = Object.freeze(new SiteConfigurationSpecialistAgent());
