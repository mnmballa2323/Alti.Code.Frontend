...
class SecurityProvisioningPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityProvisioningSystem(objective) {
    logger.info(
      `💻 [SecurityProvisioningPlannerAgent] Analyzing Security Provisioning Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Provisioning Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityProvisioningPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityProvisioningPlannerAgent = Object.freeze(
  new SecurityProvisioningPlannerAgent(),
);
