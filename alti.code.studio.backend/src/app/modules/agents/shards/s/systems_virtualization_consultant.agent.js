...
class SystemsVirtualizationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsVirtualizationSystem(objective) {
    logger.info(
      `💻 [SystemsVirtualizationConsultantAgent] Analyzing Systems Virtualization Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Virtualization Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsVirtualizationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsVirtualizationConsultantAgent = Object.freeze(
  new SystemsVirtualizationConsultantAgent(),
);
