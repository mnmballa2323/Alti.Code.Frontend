...
class SecurityMigrationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityMigrationSystem(objective) {
    logger.info(
      `💻 [SecurityMigrationDesignerAgent] Analyzing Security Migration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Migration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityMigrationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityMigrationDesignerAgent = Object.freeze(
  new SecurityMigrationDesignerAgent(),
);
