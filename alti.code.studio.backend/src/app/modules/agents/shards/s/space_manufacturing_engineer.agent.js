...
class SpaceManufacturingEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSpaceMfgSystem(spaceMfgObjective) {
    logger.info(
      `🌌 [SpaceManufacturingEngineer] Analyzing objective for microgravity fluid dynamics and off-world manufacturing...`,
    );
...
${spaceMfgObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [SpaceManufacturingEngineer] Space manufacturing architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [SpaceManufacturingEngineer] Failed to generate space mfg system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const spaceManufacturingEngineerAgent = Object.freeze(
  new SpaceManufacturingEngineerAgent(),
);
