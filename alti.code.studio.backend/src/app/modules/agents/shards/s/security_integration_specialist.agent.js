...
class SecurityIntegrationSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityIntegrationSystem(objective) {
        logger.info(`💻 [SecurityIntegrationSpecialistAgent] Analyzing Security Integration Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Integration Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityIntegrationSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityIntegrationSpecialistAgent = Object.freeze(new SecurityIntegrationSpecialistAgent());
