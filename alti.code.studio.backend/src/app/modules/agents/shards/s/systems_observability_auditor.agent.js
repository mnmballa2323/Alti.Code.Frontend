...
class SystemsObservabilityAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsObservabilitySystem(objective) {
        logger.info(`💻 [SystemsObservabilityAuditorAgent] Analyzing Systems Observability Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Observability Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsObservabilityAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsObservabilityAuditorAgent = Object.freeze(new SystemsObservabilityAuditorAgent());
