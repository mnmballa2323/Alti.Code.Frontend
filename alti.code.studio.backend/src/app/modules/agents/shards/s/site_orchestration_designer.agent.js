...
class SiteOrchestrationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteOrchestrationSystem(objective) {
        logger.info(`💻 [SiteOrchestrationDesignerAgent] Analyzing Site Orchestration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Orchestration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteOrchestrationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteOrchestrationDesignerAgent = Object.freeze(new SiteOrchestrationDesignerAgent());
