...
class SecurityTestingStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityTestingSystem(objective) {
    logger.info(
      `💻 [SecurityTestingStrategistAgent] Analyzing Security Testing Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Testing Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityTestingStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityTestingStrategistAgent = Object.freeze(
  new SecurityTestingStrategistAgent(),
);
