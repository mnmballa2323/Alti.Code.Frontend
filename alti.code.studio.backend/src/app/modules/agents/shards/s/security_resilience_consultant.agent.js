...
class SecurityResilienceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityResilienceSystem(objective) {
    logger.info(
      `💻 [SecurityResilienceConsultantAgent] Analyzing Security Resilience Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Resilience Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityResilienceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityResilienceConsultantAgent = Object.freeze(
  new SecurityResilienceConsultantAgent(),
);
