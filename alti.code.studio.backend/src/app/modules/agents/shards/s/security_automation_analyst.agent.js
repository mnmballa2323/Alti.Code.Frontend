...
class SecurityAutomationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityAutomationSystem(objective) {
    logger.info(
      `💻 [SecurityAutomationAnalystAgent] Analyzing Security Automation Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Automation Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityAutomationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityAutomationAnalystAgent = Object.freeze(
  new SecurityAutomationAnalystAgent(),
);
