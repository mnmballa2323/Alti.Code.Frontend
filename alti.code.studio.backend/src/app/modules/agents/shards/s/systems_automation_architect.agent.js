...
class SystemsAutomationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsAutomationSystem(objective) {
    logger.info(
      `💻 [SystemsAutomationArchitectAgent] Analyzing Systems Automation Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Automation Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsAutomationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsAutomationArchitectAgent = Object.freeze(
  new SystemsAutomationArchitectAgent(),
);
