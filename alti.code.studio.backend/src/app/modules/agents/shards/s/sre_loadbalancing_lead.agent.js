...
class SRELoadBalancingLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateSRELoadBalancingSystem(objective) {
    logger.info(
      `💻 [SRELoadBalancingLeadAgent] Analyzing SRE LoadBalancing Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE LoadBalancing Lead Target',
      );
...
export const sRELoadBalancingLeadAgent = Object.freeze(
  new SRELoadBalancingLeadAgent(),
);
