...
class SystemsOrchestrationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsOrchestrationSystem(objective) {
    logger.info(
      `💻 [SystemsOrchestrationAuditorAgent] Analyzing Systems Orchestration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Orchestration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsOrchestrationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsOrchestrationAuditorAgent = Object.freeze(
  new SystemsOrchestrationAuditorAgent(),
);
