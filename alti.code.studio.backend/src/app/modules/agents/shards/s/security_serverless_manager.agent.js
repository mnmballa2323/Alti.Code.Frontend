...
class SecurityServerlessManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityServerlessSystem(objective) {
    logger.info(
      `💻 [SecurityServerlessManagerAgent] Analyzing Security Serverless Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Serverless Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityServerlessManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityServerlessManagerAgent = Object.freeze(
  new SecurityServerlessManagerAgent(),
);
