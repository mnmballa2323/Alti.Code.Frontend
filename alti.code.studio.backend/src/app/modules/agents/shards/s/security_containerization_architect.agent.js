...
class SecurityContainerizationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityContainerizationSystem(objective) {
    logger.info(
      `💻 [SecurityContainerizationArchitectAgent] Analyzing Security Containerization Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Containerization Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityContainerizationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityContainerizationArchitectAgent = Object.freeze(
  new SecurityContainerizationArchitectAgent(),
);
