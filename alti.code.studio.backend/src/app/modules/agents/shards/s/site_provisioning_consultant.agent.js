...
class SiteProvisioningConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteProvisioningSystem(objective) {
    logger.info(
      `💻 [SiteProvisioningConsultantAgent] Analyzing Site Provisioning Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Provisioning Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteProvisioningConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteProvisioningConsultantAgent = Object.freeze(
  new SiteProvisioningConsultantAgent(),
);
