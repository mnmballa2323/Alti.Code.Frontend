...
class SystemsVirtualizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsVirtualizationSystem(objective) {
    logger.info(
      `💻 [SystemsVirtualizationAuditorAgent] Analyzing Systems Virtualization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Virtualization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsVirtualizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsVirtualizationAuditorAgent = Object.freeze(
  new SystemsVirtualizationAuditorAgent(),
);
