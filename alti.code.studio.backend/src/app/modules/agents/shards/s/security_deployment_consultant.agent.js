...
class SecurityDeploymentConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityDeploymentSystem(objective) {
        logger.info(`💻 [SecurityDeploymentConsultantAgent] Analyzing Security Deployment Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Deployment Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityDeploymentConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityDeploymentConsultantAgent = Object.freeze(new SecurityDeploymentConsultantAgent());
