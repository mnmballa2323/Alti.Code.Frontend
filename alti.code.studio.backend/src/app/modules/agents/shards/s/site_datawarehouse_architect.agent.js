...
class SiteDataWarehouseArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteDataWarehouseSystem(objective) {
    logger.info(
      `💻 [SiteDataWarehouseArchitectAgent] Analyzing Site DataWarehouse Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site DataWarehouse Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteDataWarehouseArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteDataWarehouseArchitectAgent = Object.freeze(
  new SiteDataWarehouseArchitectAgent(),
);
