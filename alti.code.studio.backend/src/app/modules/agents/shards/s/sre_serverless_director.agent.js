...
class SREServerlessDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSREServerlessSystem(objective) {
    logger.info(
      `💻 [SREServerlessDirectorAgent] Analyzing SRE Serverless Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Serverless Director Target',
      );
...
export const sREServerlessDirectorAgent = Object.freeze(
  new SREServerlessDirectorAgent(),
);
