...
class SecurityIntegrationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityIntegrationSystem(objective) {
        logger.info(`💻 [SecurityIntegrationTesterAgent] Analyzing Security Integration Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Integration Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityIntegrationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityIntegrationTesterAgent = Object.freeze(new SecurityIntegrationTesterAgent());
