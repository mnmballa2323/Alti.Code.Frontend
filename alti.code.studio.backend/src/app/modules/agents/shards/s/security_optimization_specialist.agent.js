...
class SecurityOptimizationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityOptimizationSystem(objective) {
    logger.info(
      `💻 [SecurityOptimizationSpecialistAgent] Analyzing Security Optimization Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Optimization Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityOptimizationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityOptimizationSpecialistAgent = Object.freeze(
  new SecurityOptimizationSpecialistAgent(),
);
