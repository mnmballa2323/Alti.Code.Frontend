...
class SystemsMicroservicesDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsMicroservicesSystem(objective) {
    logger.info(
      `💻 [SystemsMicroservicesDirectorAgent] Analyzing Systems Microservices Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Microservices Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsMicroservicesDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsMicroservicesDirectorAgent = Object.freeze(
  new SystemsMicroservicesDirectorAgent(),
);
