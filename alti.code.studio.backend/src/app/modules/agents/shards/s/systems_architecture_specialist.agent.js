...
class SystemsArchitectureSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsArchitectureSystem(objective) {
    logger.info(
      `💻 [SystemsArchitectureSpecialistAgent] Analyzing Systems Architecture Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Architecture Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsArchitectureSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsArchitectureSpecialistAgent = Object.freeze(
  new SystemsArchitectureSpecialistAgent(),
);
