...
class SREFaultToleranceStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateSREFaultToleranceSystem(objective) {
        logger.info(`💻 [SREFaultToleranceStrategistAgent] Analyzing SRE FaultTolerance Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE FaultTolerance Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SREFaultToleranceStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const sREFaultToleranceStrategistAgent = Object.freeze(new SREFaultToleranceStrategistAgent());
