...
class SecurityMicroservicesArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityMicroservicesSystem(objective) {
    logger.info(
      `💻 [SecurityMicroservicesArchitectAgent] Analyzing Security Microservices Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Microservices Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityMicroservicesArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityMicroservicesArchitectAgent = Object.freeze(
  new SecurityMicroservicesArchitectAgent(),
);
