...
class SystemsObservabilityStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsObservabilitySystem(objective) {
    logger.info(
      `💻 [SystemsObservabilityStrategistAgent] Analyzing Systems Observability Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Observability Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsObservabilityStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsObservabilityStrategistAgent = Object.freeze(
  new SystemsObservabilityStrategistAgent(),
);
