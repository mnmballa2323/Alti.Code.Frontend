...
class SecurityOrchestrationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityOrchestrationSystem(objective) {
    logger.info(
      `💻 [SecurityOrchestrationDirectorAgent] Analyzing Security Orchestration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Orchestration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityOrchestrationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityOrchestrationDirectorAgent = Object.freeze(
  new SecurityOrchestrationDirectorAgent(),
);
