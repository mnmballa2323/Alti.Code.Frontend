...
class SystemsBatchProcessingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsBatchProcessingSystem(objective) {
    logger.info(
      `💻 [SystemsBatchProcessingPlannerAgent] Analyzing Systems BatchProcessing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems BatchProcessing Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsBatchProcessingPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsBatchProcessingPlannerAgent = Object.freeze(
  new SystemsBatchProcessingPlannerAgent(),
);
