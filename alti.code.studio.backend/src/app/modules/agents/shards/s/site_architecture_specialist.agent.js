...
class SiteArchitectureSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteArchitectureSystem(objective) {
    logger.info(
      `💻 [SiteArchitectureSpecialistAgent] Analyzing Site Architecture Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Architecture Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteArchitectureSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteArchitectureSpecialistAgent = Object.freeze(
  new SiteArchitectureSpecialistAgent(),
);
