...
class SREStreamingManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSREStreamingSystem(objective) {
    logger.info(
      `💻 [SREStreamingManagerAgent] Analyzing SRE Streaming Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Streaming Manager Target',
      );
...
export const sREStreamingManagerAgent = Object.freeze(
  new SREStreamingManagerAgent(),
);
