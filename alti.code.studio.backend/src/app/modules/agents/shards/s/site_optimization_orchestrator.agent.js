...
class SiteOptimizationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteOptimizationSystem(objective) {
    logger.info(
      `💻 [SiteOptimizationOrchestratorAgent] Analyzing Site Optimization Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Optimization Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteOptimizationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteOptimizationOrchestratorAgent = Object.freeze(
  new SiteOptimizationOrchestratorAgent(),
);
