...
class SecurityGovernanceEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityGovernanceSystem(objective) {
        logger.info(`💻 [SecurityGovernanceEngineerAgent] Analyzing Security Governance Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Governance Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityGovernanceEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityGovernanceEngineerAgent = Object.freeze(new SecurityGovernanceEngineerAgent());
