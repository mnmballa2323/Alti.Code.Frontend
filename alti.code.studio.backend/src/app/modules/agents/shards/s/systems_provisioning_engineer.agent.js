...
class SystemsProvisioningEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsProvisioningSystem(objective) {
    logger.info(
      `💻 [SystemsProvisioningEngineerAgent] Analyzing Systems Provisioning Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Provisioning Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsProvisioningEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsProvisioningEngineerAgent = Object.freeze(
  new SystemsProvisioningEngineerAgent(),
);
