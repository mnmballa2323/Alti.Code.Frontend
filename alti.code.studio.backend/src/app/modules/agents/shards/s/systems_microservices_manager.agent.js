...
class SystemsMicroservicesManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsMicroservicesSystem(objective) {
    logger.info(
      `💻 [SystemsMicroservicesManagerAgent] Analyzing Systems Microservices Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Microservices Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsMicroservicesManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsMicroservicesManagerAgent = Object.freeze(
  new SystemsMicroservicesManagerAgent(),
);
