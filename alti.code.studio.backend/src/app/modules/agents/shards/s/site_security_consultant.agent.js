...
class SiteSecurityConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteSecuritySystem(objective) {
    logger.info(
      `💻 [SiteSecurityConsultantAgent] Analyzing Site Security Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Security Consultant Target',
      );
...
export const siteSecurityConsultantAgent = Object.freeze(
  new SiteSecurityConsultantAgent(),
);
