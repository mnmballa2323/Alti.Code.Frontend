...
class SystemsTelemetryAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsTelemetrySystem(objective) {
    logger.info(
      `💻 [SystemsTelemetryAnalystAgent] Analyzing Systems Telemetry Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Telemetry Analyst Target',
      );
...
export const systemsTelemetryAnalystAgent = Object.freeze(
  new SystemsTelemetryAnalystAgent(),
);
