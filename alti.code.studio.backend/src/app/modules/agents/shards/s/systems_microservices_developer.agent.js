...
class SystemsMicroservicesDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsMicroservicesSystem(objective) {
        logger.info(`💻 [SystemsMicroservicesDeveloperAgent] Analyzing Systems Microservices Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Microservices Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsMicroservicesDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsMicroservicesDeveloperAgent = Object.freeze(new SystemsMicroservicesDeveloperAgent());
