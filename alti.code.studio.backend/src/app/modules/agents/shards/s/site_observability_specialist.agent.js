...
class SiteObservabilitySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteObservabilitySystem(objective) {
    logger.info(
      `💻 [SiteObservabilitySpecialistAgent] Analyzing Site Observability Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Observability Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteObservabilitySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteObservabilitySpecialistAgent = Object.freeze(
  new SiteObservabilitySpecialistAgent(),
);
