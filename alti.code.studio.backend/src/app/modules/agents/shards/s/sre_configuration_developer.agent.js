...
class SREConfigurationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSREConfigurationSystem(objective) {
    logger.info(
      `💻 [SREConfigurationDeveloperAgent] Analyzing SRE Configuration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Configuration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREConfigurationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREConfigurationDeveloperAgent = Object.freeze(
  new SREConfigurationDeveloperAgent(),
);
