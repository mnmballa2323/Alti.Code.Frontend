...
class SystemsFaultToleranceAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsFaultToleranceSystem(objective) {
    logger.info(
      `💻 [SystemsFaultToleranceAuditorAgent] Analyzing Systems FaultTolerance Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems FaultTolerance Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsFaultToleranceAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsFaultToleranceAuditorAgent = Object.freeze(
  new SystemsFaultToleranceAuditorAgent(),
);
