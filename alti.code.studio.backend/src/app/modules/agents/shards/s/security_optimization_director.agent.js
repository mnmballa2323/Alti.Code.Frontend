...
class SecurityOptimizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityOptimizationSystem(objective) {
        logger.info(`💻 [SecurityOptimizationDirectorAgent] Analyzing Security Optimization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Optimization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityOptimizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityOptimizationDirectorAgent = Object.freeze(new SecurityOptimizationDirectorAgent());
