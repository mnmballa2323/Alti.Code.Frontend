...
class SecurityAutomationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityAutomationSystem(objective) {
        logger.info(`💻 [SecurityAutomationArchitectAgent] Analyzing Security Automation Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Automation Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityAutomationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityAutomationArchitectAgent = Object.freeze(new SecurityAutomationArchitectAgent());
