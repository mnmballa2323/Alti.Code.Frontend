...
class SiteConfigurationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteConfigurationSystem(objective) {
    logger.info(
      `💻 [SiteConfigurationDesignerAgent] Analyzing Site Configuration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Configuration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteConfigurationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteConfigurationDesignerAgent = Object.freeze(
  new SiteConfigurationDesignerAgent(),
);
