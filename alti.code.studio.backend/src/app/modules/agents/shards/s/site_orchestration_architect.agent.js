...
class SiteOrchestrationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteOrchestrationSystem(objective) {
        logger.info(`💻 [SiteOrchestrationArchitectAgent] Analyzing Site Orchestration Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Orchestration Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteOrchestrationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteOrchestrationArchitectAgent = Object.freeze(new SiteOrchestrationArchitectAgent());
