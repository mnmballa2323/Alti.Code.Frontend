...
class SREContainerizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateSREContainerizationSystem(objective) {
        logger.info(`💻 [SREContainerizationDirectorAgent] Analyzing SRE Containerization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE Containerization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SREContainerizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const sREContainerizationDirectorAgent = Object.freeze(new SREContainerizationDirectorAgent());
