...
class SecurityOptimizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityOptimizationSystem(objective) {
    logger.info(
      `💻 [SecurityOptimizationAnalystAgent] Analyzing Security Optimization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Optimization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityOptimizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityOptimizationAnalystAgent = Object.freeze(
  new SecurityOptimizationAnalystAgent(),
);
