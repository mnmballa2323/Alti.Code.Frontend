...
class SystemsLoadBalancingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsLoadBalancingSystem(objective) {
    logger.info(
      `💻 [SystemsLoadBalancingSpecialistAgent] Analyzing Systems LoadBalancing Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems LoadBalancing Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsLoadBalancingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsLoadBalancingSpecialistAgent = Object.freeze(
  new SystemsLoadBalancingSpecialistAgent(),
);
