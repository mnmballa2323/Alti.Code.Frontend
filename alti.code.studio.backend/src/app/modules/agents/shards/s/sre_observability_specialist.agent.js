...
class SREObservabilitySpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSREObservabilitySystem(objective) {
    logger.info(
      `💻 [SREObservabilitySpecialistAgent] Analyzing SRE Observability Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Observability Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREObservabilitySpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREObservabilitySpecialistAgent = Object.freeze(
  new SREObservabilitySpecialistAgent(),
);
