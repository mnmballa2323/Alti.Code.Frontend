...
class SREEngineeringOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSREEngineeringSystem(objective) {
    logger.info(
      `💻 [SREEngineeringOrchestratorAgent] Analyzing SRE Engineering Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Engineering Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREEngineeringOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREEngineeringOrchestratorAgent = Object.freeze(
  new SREEngineeringOrchestratorAgent(),
);
