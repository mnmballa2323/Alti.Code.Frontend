...
class SystemsMicroservicesSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsMicroservicesSystem(objective) {
    logger.info(
      `💻 [SystemsMicroservicesSpecialistAgent] Analyzing Systems Microservices Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Microservices Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsMicroservicesSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsMicroservicesSpecialistAgent = Object.freeze(
  new SystemsMicroservicesSpecialistAgent(),
);
