...
class SecurityConfigurationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityConfigurationSystem(objective) {
    logger.info(
      `💻 [SecurityConfigurationPlannerAgent] Analyzing Security Configuration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Configuration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityConfigurationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityConfigurationPlannerAgent = Object.freeze(
  new SecurityConfigurationPlannerAgent(),
);
