...
class SRETelemetryAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSRETelemetrySystem(objective) {
    logger.info(
      `💻 [SRETelemetryAnalystAgent] Analyzing SRE Telemetry Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Telemetry Analyst Target',
      );
...
export const sRETelemetryAnalystAgent = Object.freeze(
  new SRETelemetryAnalystAgent(),
);
