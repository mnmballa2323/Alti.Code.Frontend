...
class SystemsIntegrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsIntegrationSystem(objective) {
    logger.info(
      `💻 [SystemsIntegrationStrategistAgent] Analyzing Systems Integration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Integration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsIntegrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsIntegrationStrategistAgent = Object.freeze(
  new SystemsIntegrationStrategistAgent(),
);
