...
class SREDataWarehouseStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateSREDataWarehouseSystem(objective) {
        logger.info(`💻 [SREDataWarehouseStrategistAgent] Analyzing SRE DataWarehouse Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE DataWarehouse Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SREDataWarehouseStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const sREDataWarehouseStrategistAgent = Object.freeze(new SREDataWarehouseStrategistAgent());
