...
class SecurityProvisioningAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityProvisioningSystem(objective) {
    logger.info(
      `💻 [SecurityProvisioningAuditorAgent] Analyzing Security Provisioning Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Provisioning Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityProvisioningAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityProvisioningAuditorAgent = Object.freeze(
  new SecurityProvisioningAuditorAgent(),
);
