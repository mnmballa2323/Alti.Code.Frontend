...
class SystemsArchitectureAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsArchitectureSystem(objective) {
    logger.info(
      `💻 [SystemsArchitectureAnalystAgent] Analyzing Systems Architecture Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Architecture Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsArchitectureAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsArchitectureAnalystAgent = Object.freeze(
  new SystemsArchitectureAnalystAgent(),
);
