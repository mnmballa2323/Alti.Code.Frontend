...
class SiteLoadBalancingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteLoadBalancingSystem(objective) {
    logger.info(
      `💻 [SiteLoadBalancingConsultantAgent] Analyzing Site LoadBalancing Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site LoadBalancing Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteLoadBalancingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteLoadBalancingConsultantAgent = Object.freeze(
  new SiteLoadBalancingConsultantAgent(),
);
