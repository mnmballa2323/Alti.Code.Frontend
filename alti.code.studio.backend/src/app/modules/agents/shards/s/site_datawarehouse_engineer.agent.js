...
class SiteDataWarehouseEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteDataWarehouseSystem(objective) {
    logger.info(
      `💻 [SiteDataWarehouseEngineerAgent] Analyzing Site DataWarehouse Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site DataWarehouse Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteDataWarehouseEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteDataWarehouseEngineerAgent = Object.freeze(
  new SiteDataWarehouseEngineerAgent(),
);
