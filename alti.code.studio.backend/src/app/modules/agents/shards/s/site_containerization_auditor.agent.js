...
class SiteContainerizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteContainerizationSystem(objective) {
    logger.info(
      `💻 [SiteContainerizationAuditorAgent] Analyzing Site Containerization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Containerization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteContainerizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteContainerizationAuditorAgent = Object.freeze(
  new SiteContainerizationAuditorAgent(),
);
