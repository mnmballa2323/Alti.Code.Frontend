...
class SystemsLoadBalancingEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsLoadBalancingSystem(objective) {
    logger.info(
      `💻 [SystemsLoadBalancingEngineerAgent] Analyzing Systems LoadBalancing Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems LoadBalancing Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsLoadBalancingEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsLoadBalancingEngineerAgent = Object.freeze(
  new SystemsLoadBalancingEngineerAgent(),
);
