...
class SRELoadBalancingAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSRELoadBalancingSystem(objective) {
    logger.info(
      `💻 [SRELoadBalancingAuditorAgent] Analyzing SRE LoadBalancing Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE LoadBalancing Auditor Target',
      );
...
export const sRELoadBalancingAuditorAgent = Object.freeze(
  new SRELoadBalancingAuditorAgent(),
);
