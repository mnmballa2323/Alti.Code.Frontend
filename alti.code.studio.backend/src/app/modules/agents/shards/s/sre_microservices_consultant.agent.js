...
class SREMicroservicesConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSREMicroservicesSystem(objective) {
        logger.info(`💻 [SREMicroservicesConsultantAgent] Analyzing SRE Microservices Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE Microservices Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SREMicroservicesConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const sREMicroservicesConsultantAgent = Object.freeze(new SREMicroservicesConsultantAgent());
