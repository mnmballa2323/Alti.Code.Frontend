...
class SystemsEngineeringDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsEngineeringSystem(objective) {
    logger.info(
      `💻 [SystemsEngineeringDeveloperAgent] Analyzing Systems Engineering Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Engineering Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsEngineeringDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsEngineeringDeveloperAgent = Object.freeze(
  new SystemsEngineeringDeveloperAgent(),
);
