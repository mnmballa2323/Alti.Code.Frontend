...
class SREArchitectureStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSREArchitectureSystem(objective) {
    logger.info(
      `💻 [SREArchitectureStrategistAgent] Analyzing SRE Architecture Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Architecture Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREArchitectureStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREArchitectureStrategistAgent = Object.freeze(
  new SREArchitectureStrategistAgent(),
);
