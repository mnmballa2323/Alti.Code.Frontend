...
class SecurityMicroservicesManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityMicroservicesSystem(objective) {
        logger.info(`💻 [SecurityMicroservicesManagerAgent] Analyzing Security Microservices Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Microservices Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityMicroservicesManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityMicroservicesManagerAgent = Object.freeze(new SecurityMicroservicesManagerAgent());
