...
class SecurityScalingConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityScalingSystem(objective) {
        logger.info(`💻 [SecurityScalingConsultantAgent] Analyzing Security Scaling Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Scaling Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityScalingConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityScalingConsultantAgent = Object.freeze(new SecurityScalingConsultantAgent());
