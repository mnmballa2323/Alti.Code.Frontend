...
class SecurityDeploymentManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityDeploymentSystem(objective) {
    logger.info(
      `💻 [SecurityDeploymentManagerAgent] Analyzing Security Deployment Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Deployment Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityDeploymentManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityDeploymentManagerAgent = Object.freeze(
  new SecurityDeploymentManagerAgent(),
);
