...
class SiteObservabilityDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteObservabilitySystem(objective) {
    logger.info(
      `💻 [SiteObservabilityDirectorAgent] Analyzing Site Observability Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Observability Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteObservabilityDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteObservabilityDirectorAgent = Object.freeze(
  new SiteObservabilityDirectorAgent(),
);
