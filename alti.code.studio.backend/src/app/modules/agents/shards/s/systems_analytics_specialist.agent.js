...
class SystemsAnalyticsSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsAnalyticsSystem(objective) {
    logger.info(
      `💻 [SystemsAnalyticsSpecialistAgent] Analyzing Systems Analytics Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Analytics Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsAnalyticsSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsAnalyticsSpecialistAgent = Object.freeze(
  new SystemsAnalyticsSpecialistAgent(),
);
