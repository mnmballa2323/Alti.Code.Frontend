...
class SystemsFaultTolerancePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsFaultToleranceSystem(objective) {
        logger.info(`💻 [SystemsFaultTolerancePlannerAgent] Analyzing Systems FaultTolerance Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems FaultTolerance Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsFaultTolerancePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsFaultTolerancePlannerAgent = Object.freeze(new SystemsFaultTolerancePlannerAgent());
