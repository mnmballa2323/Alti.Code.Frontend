...
class SystemsAnalyticsOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsAnalyticsSystem(objective) {
    logger.info(
      `💻 [SystemsAnalyticsOrchestratorAgent] Analyzing Systems Analytics Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Analytics Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsAnalyticsOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsAnalyticsOrchestratorAgent = Object.freeze(
  new SystemsAnalyticsOrchestratorAgent(),
);
