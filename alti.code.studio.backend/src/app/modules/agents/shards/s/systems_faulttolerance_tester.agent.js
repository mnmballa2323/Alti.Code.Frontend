...
class SystemsFaultToleranceTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsFaultToleranceSystem(objective) {
    logger.info(
      `💻 [SystemsFaultToleranceTesterAgent] Analyzing Systems FaultTolerance Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems FaultTolerance Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsFaultToleranceTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsFaultToleranceTesterAgent = Object.freeze(
  new SystemsFaultToleranceTesterAgent(),
);
