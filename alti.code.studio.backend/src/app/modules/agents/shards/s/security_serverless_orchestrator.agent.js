...
class SecurityServerlessOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityServerlessSystem(objective) {
        logger.info(`💻 [SecurityServerlessOrchestratorAgent] Analyzing Security Serverless Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Serverless Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityServerlessOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityServerlessOrchestratorAgent = Object.freeze(new SecurityServerlessOrchestratorAgent());
