...
class SREContainerizationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateSREContainerizationSystem(objective) {
        logger.info(`💻 [SREContainerizationAuditorAgent] Analyzing SRE Containerization Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE Containerization Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SREContainerizationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const sREContainerizationAuditorAgent = Object.freeze(new SREContainerizationAuditorAgent());
