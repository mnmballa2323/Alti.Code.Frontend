...
class SecurityProvisioningAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityProvisioningSystem(objective) {
    logger.info(
      `💻 [SecurityProvisioningAnalystAgent] Analyzing Security Provisioning Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Provisioning Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityProvisioningAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityProvisioningAnalystAgent = Object.freeze(
  new SecurityProvisioningAnalystAgent(),
);
