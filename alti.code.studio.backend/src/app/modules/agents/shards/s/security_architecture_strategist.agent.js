...
class SecurityArchitectureStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityArchitectureSystem(objective) {
    logger.info(
      `💻 [SecurityArchitectureStrategistAgent] Analyzing Security Architecture Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Architecture Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityArchitectureStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityArchitectureStrategistAgent = Object.freeze(
  new SecurityArchitectureStrategistAgent(),
);
