...
class SystemsComplianceOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsComplianceSystem(objective) {
        logger.info(`💻 [SystemsComplianceOrchestratorAgent] Analyzing Systems Compliance Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Compliance Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsComplianceOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsComplianceOrchestratorAgent = Object.freeze(new SystemsComplianceOrchestratorAgent());
