...
class SystemsEngineeringConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsEngineeringSystem(objective) {
    logger.info(
      `💻 [SystemsEngineeringConsultantAgent] Analyzing Systems Engineering Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Engineering Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsEngineeringConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsEngineeringConsultantAgent = Object.freeze(
  new SystemsEngineeringConsultantAgent(),
);
