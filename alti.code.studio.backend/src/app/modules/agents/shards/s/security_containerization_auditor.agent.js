...
class SecurityContainerizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityContainerizationSystem(objective) {
    logger.info(
      `💻 [SecurityContainerizationAuditorAgent] Analyzing Security Containerization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Containerization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityContainerizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityContainerizationAuditorAgent = Object.freeze(
  new SecurityContainerizationAuditorAgent(),
);
