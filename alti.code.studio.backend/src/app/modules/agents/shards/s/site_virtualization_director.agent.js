...
class SiteVirtualizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteVirtualizationSystem(objective) {
    logger.info(
      `💻 [SiteVirtualizationDirectorAgent] Analyzing Site Virtualization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Virtualization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteVirtualizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteVirtualizationDirectorAgent = Object.freeze(
  new SiteVirtualizationDirectorAgent(),
);
