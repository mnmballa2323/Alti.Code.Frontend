...
class SiteFaultToleranceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteFaultToleranceSystem(objective) {
    logger.info(
      `💻 [SiteFaultToleranceConsultantAgent] Analyzing Site FaultTolerance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site FaultTolerance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteFaultToleranceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteFaultToleranceConsultantAgent = Object.freeze(
  new SiteFaultToleranceConsultantAgent(),
);
