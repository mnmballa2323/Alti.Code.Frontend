...
class SiteLoadBalancingEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteLoadBalancingSystem(objective) {
        logger.info(`💻 [SiteLoadBalancingEngineerAgent] Analyzing Site LoadBalancing Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site LoadBalancing Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteLoadBalancingEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteLoadBalancingEngineerAgent = Object.freeze(new SiteLoadBalancingEngineerAgent());
