...
class SiteIntegrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteIntegrationSystem(objective) {
    logger.info(
      `💻 [SiteIntegrationConsultantAgent] Analyzing Site Integration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Integration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteIntegrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteIntegrationConsultantAgent = Object.freeze(
  new SiteIntegrationConsultantAgent(),
);
