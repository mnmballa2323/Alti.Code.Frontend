...
class SystemsOptimizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsOptimizationSystem(objective) {
    logger.info(
      `💻 [SystemsOptimizationAnalystAgent] Analyzing Systems Optimization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Optimization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsOptimizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsOptimizationAnalystAgent = Object.freeze(
  new SystemsOptimizationAnalystAgent(),
);
