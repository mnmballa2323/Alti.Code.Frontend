...
class SRELoadBalancingEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSRELoadBalancingSystem(objective) {
    logger.info(
      `💻 [SRELoadBalancingEngineerAgent] Analyzing SRE LoadBalancing Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE LoadBalancing Engineer Target',
      );
...
export const sRELoadBalancingEngineerAgent = Object.freeze(
  new SRELoadBalancingEngineerAgent(),
);
