...
class SecurityResilienceEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityResilienceSystem(objective) {
        logger.info(`💻 [SecurityResilienceEngineerAgent] Analyzing Security Resilience Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Resilience Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityResilienceEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityResilienceEngineerAgent = Object.freeze(new SecurityResilienceEngineerAgent());
