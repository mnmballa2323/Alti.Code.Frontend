...
class SystemsTelemetryPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsTelemetrySystem(objective) {
    logger.info(
      `💻 [SystemsTelemetryPlannerAgent] Analyzing Systems Telemetry Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Telemetry Planner Target',
      );
...
export const systemsTelemetryPlannerAgent = Object.freeze(
  new SystemsTelemetryPlannerAgent(),
);
