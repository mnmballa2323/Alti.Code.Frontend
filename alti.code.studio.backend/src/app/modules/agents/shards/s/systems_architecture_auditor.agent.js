...
class SystemsArchitectureAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsArchitectureSystem(objective) {
    logger.info(
      `💻 [SystemsArchitectureAuditorAgent] Analyzing Systems Architecture Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Architecture Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsArchitectureAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsArchitectureAuditorAgent = Object.freeze(
  new SystemsArchitectureAuditorAgent(),
);
