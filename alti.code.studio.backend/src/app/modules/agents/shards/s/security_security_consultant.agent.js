...
class SecuritySecurityConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSecuritySecuritySystem(objective) {
    logger.info(
      `💻 [SecuritySecurityConsultantAgent] Analyzing Security Security Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Security Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecuritySecurityConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securitySecurityConsultantAgent = Object.freeze(
  new SecuritySecurityConsultantAgent(),
);
