...
class SecurityTelemetryDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityTelemetrySystem(objective) {
        logger.info(`💻 [SecurityTelemetryDeveloperAgent] Analyzing Security Telemetry Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Telemetry Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityTelemetryDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityTelemetryDeveloperAgent = Object.freeze(new SecurityTelemetryDeveloperAgent());
