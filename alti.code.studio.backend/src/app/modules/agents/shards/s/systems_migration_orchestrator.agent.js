...
class SystemsMigrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsMigrationSystem(objective) {
    logger.info(
      `💻 [SystemsMigrationOrchestratorAgent] Analyzing Systems Migration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Migration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsMigrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsMigrationOrchestratorAgent = Object.freeze(
  new SystemsMigrationOrchestratorAgent(),
);
