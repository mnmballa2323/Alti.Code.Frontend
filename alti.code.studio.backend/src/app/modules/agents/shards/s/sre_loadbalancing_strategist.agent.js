...
class SRELoadBalancingStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSRELoadBalancingSystem(objective) {
    logger.info(
      `💻 [SRELoadBalancingStrategistAgent] Analyzing SRE LoadBalancing Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE LoadBalancing Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SRELoadBalancingStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sRELoadBalancingStrategistAgent = Object.freeze(
  new SRELoadBalancingStrategistAgent(),
);
