...
class SecurityEngineeringDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityEngineeringSystem(objective) {
    logger.info(
      `💻 [SecurityEngineeringDeveloperAgent] Analyzing Security Engineering Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Engineering Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityEngineeringDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityEngineeringDeveloperAgent = Object.freeze(
  new SecurityEngineeringDeveloperAgent(),
);
