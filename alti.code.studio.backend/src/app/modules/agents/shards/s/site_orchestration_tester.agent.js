...
class SiteOrchestrationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteOrchestrationSystem(objective) {
    logger.info(
      `💻 [SiteOrchestrationTesterAgent] Analyzing Site Orchestration Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Orchestration Tester Target',
      );
...
export const siteOrchestrationTesterAgent = Object.freeze(
  new SiteOrchestrationTesterAgent(),
);
