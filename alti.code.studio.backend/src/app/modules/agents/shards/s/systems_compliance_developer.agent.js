...
class SystemsComplianceDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsComplianceSystem(objective) {
    logger.info(
      `💻 [SystemsComplianceDeveloperAgent] Analyzing Systems Compliance Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Compliance Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsComplianceDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsComplianceDeveloperAgent = Object.freeze(
  new SystemsComplianceDeveloperAgent(),
);
