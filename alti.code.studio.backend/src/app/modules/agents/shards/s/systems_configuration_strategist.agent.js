...
class SystemsConfigurationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsConfigurationSystem(objective) {
        logger.info(`💻 [SystemsConfigurationStrategistAgent] Analyzing Systems Configuration Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Configuration Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsConfigurationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsConfigurationStrategistAgent = Object.freeze(new SystemsConfigurationStrategistAgent());
