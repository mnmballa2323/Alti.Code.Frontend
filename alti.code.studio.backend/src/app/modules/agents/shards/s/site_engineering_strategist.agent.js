...
class SiteEngineeringStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteEngineeringSystem(objective) {
    logger.info(
      `💻 [SiteEngineeringStrategistAgent] Analyzing Site Engineering Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Engineering Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteEngineeringStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteEngineeringStrategistAgent = Object.freeze(
  new SiteEngineeringStrategistAgent(),
);
