...
class SiteMicroservicesSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteMicroservicesSystem(objective) {
    logger.info(
      `💻 [SiteMicroservicesSpecialistAgent] Analyzing Site Microservices Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Microservices Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteMicroservicesSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteMicroservicesSpecialistAgent = Object.freeze(
  new SiteMicroservicesSpecialistAgent(),
);
