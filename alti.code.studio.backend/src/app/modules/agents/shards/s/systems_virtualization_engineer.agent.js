...
class SystemsVirtualizationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsVirtualizationSystem(objective) {
    logger.info(
      `💻 [SystemsVirtualizationEngineerAgent] Analyzing Systems Virtualization Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Virtualization Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsVirtualizationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsVirtualizationEngineerAgent = Object.freeze(
  new SystemsVirtualizationEngineerAgent(),
);
