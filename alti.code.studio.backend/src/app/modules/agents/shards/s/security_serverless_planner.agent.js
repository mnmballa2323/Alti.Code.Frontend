...
class SecurityServerlessPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityServerlessSystem(objective) {
    logger.info(
      `💻 [SecurityServerlessPlannerAgent] Analyzing Security Serverless Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Serverless Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityServerlessPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityServerlessPlannerAgent = Object.freeze(
  new SecurityServerlessPlannerAgent(),
);
