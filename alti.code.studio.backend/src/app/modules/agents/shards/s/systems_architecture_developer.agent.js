...
class SystemsArchitectureDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsArchitectureSystem(objective) {
    logger.info(
      `💻 [SystemsArchitectureDeveloperAgent] Analyzing Systems Architecture Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Architecture Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsArchitectureDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsArchitectureDeveloperAgent = Object.freeze(
  new SystemsArchitectureDeveloperAgent(),
);
