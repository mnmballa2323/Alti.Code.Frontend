...
class SiteBatchProcessingManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteBatchProcessingSystem(objective) {
        logger.info(`💻 [SiteBatchProcessingManagerAgent] Analyzing Site BatchProcessing Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site BatchProcessing Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteBatchProcessingManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteBatchProcessingManagerAgent = Object.freeze(new SiteBatchProcessingManagerAgent());
