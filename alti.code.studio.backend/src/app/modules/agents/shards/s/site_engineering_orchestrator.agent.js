...
class SiteEngineeringOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteEngineeringSystem(objective) {
    logger.info(
      `💻 [SiteEngineeringOrchestratorAgent] Analyzing Site Engineering Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Engineering Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteEngineeringOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteEngineeringOrchestratorAgent = Object.freeze(
  new SiteEngineeringOrchestratorAgent(),
);
