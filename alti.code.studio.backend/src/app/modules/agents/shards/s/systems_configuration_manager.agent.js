...
class SystemsConfigurationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsConfigurationSystem(objective) {
        logger.info(`💻 [SystemsConfigurationManagerAgent] Analyzing Systems Configuration Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Configuration Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsConfigurationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsConfigurationManagerAgent = Object.freeze(new SystemsConfigurationManagerAgent());
