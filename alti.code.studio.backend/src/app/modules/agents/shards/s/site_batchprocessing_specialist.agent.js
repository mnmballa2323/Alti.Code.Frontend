...
class SiteBatchProcessingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteBatchProcessingSystem(objective) {
    logger.info(
      `💻 [SiteBatchProcessingSpecialistAgent] Analyzing Site BatchProcessing Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site BatchProcessing Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteBatchProcessingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteBatchProcessingSpecialistAgent = Object.freeze(
  new SiteBatchProcessingSpecialistAgent(),
);
