...
class SREDataWarehouseSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateSREDataWarehouseSystem(objective) {
        logger.info(`💻 [SREDataWarehouseSpecialistAgent] Analyzing SRE DataWarehouse Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE DataWarehouse Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SREDataWarehouseSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const sREDataWarehouseSpecialistAgent = Object.freeze(new SREDataWarehouseSpecialistAgent());
