...
class SiteObservabilityOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteObservabilitySystem(objective) {
    logger.info(
      `💻 [SiteObservabilityOrchestratorAgent] Analyzing Site Observability Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Observability Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteObservabilityOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteObservabilityOrchestratorAgent = Object.freeze(
  new SiteObservabilityOrchestratorAgent(),
);
