...
class SREProvisioningStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSREProvisioningSystem(objective) {
    logger.info(
      `💻 [SREProvisioningStrategistAgent] Analyzing SRE Provisioning Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Provisioning Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREProvisioningStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREProvisioningStrategistAgent = Object.freeze(
  new SREProvisioningStrategistAgent(),
);
