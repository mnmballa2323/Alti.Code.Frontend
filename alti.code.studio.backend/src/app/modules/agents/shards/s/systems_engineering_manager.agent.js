...
class SystemsEngineeringManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsEngineeringSystem(objective) {
    logger.info(
      `💻 [SystemsEngineeringManagerAgent] Analyzing Systems Engineering Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Engineering Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsEngineeringManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsEngineeringManagerAgent = Object.freeze(
  new SystemsEngineeringManagerAgent(),
);
