...
class SREVirtualizationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSREVirtualizationSystem(objective) {
    logger.info(
      `💻 [SREVirtualizationEngineerAgent] Analyzing SRE Virtualization Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Virtualization Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREVirtualizationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREVirtualizationEngineerAgent = Object.freeze(
  new SREVirtualizationEngineerAgent(),
);
