...
class SREDataWarehouseConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSREDataWarehouseSystem(objective) {
        logger.info(`💻 [SREDataWarehouseConsultantAgent] Analyzing SRE DataWarehouse Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE DataWarehouse Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SREDataWarehouseConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const sREDataWarehouseConsultantAgent = Object.freeze(new SREDataWarehouseConsultantAgent());
