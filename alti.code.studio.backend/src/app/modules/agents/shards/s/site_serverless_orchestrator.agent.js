...
class SiteServerlessOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteServerlessSystem(objective) {
        logger.info(`💻 [SiteServerlessOrchestratorAgent] Analyzing Site Serverless Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Serverless Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteServerlessOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteServerlessOrchestratorAgent = Object.freeze(new SiteServerlessOrchestratorAgent());
