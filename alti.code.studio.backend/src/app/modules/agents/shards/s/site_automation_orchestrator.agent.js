...
class SiteAutomationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteAutomationSystem(objective) {
        logger.info(`💻 [SiteAutomationOrchestratorAgent] Analyzing Site Automation Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Automation Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteAutomationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteAutomationOrchestratorAgent = Object.freeze(new SiteAutomationOrchestratorAgent());
