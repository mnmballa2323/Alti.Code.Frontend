...
class SiteFaultToleranceAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteFaultToleranceSystem(objective) {
    logger.info(
      `💻 [SiteFaultToleranceAuditorAgent] Analyzing Site FaultTolerance Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site FaultTolerance Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteFaultToleranceAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteFaultToleranceAuditorAgent = Object.freeze(
  new SiteFaultToleranceAuditorAgent(),
);
