...
class SecurityGovernanceArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityGovernanceSystem(objective) {
        logger.info(`💻 [SecurityGovernanceArchitectAgent] Analyzing Security Governance Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Governance Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityGovernanceArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityGovernanceArchitectAgent = Object.freeze(new SecurityGovernanceArchitectAgent());
