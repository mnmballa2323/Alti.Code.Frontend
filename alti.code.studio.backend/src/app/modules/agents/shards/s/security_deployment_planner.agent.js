...
class SecurityDeploymentPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityDeploymentSystem(objective) {
    logger.info(
      `💻 [SecurityDeploymentPlannerAgent] Analyzing Security Deployment Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Deployment Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityDeploymentPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityDeploymentPlannerAgent = Object.freeze(
  new SecurityDeploymentPlannerAgent(),
);
