...
class SiteObservabilityDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteObservabilitySystem(objective) {
        logger.info(`💻 [SiteObservabilityDeveloperAgent] Analyzing Site Observability Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Observability Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteObservabilityDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteObservabilityDeveloperAgent = Object.freeze(new SiteObservabilityDeveloperAgent());
