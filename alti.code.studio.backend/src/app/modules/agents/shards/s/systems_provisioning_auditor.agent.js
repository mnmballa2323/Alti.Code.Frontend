...
class SystemsProvisioningAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsProvisioningSystem(objective) {
    logger.info(
      `💻 [SystemsProvisioningAuditorAgent] Analyzing Systems Provisioning Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Provisioning Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsProvisioningAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsProvisioningAuditorAgent = Object.freeze(
  new SystemsProvisioningAuditorAgent(),
);
