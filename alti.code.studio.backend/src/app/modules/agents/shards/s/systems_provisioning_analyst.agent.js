...
class SystemsProvisioningAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsProvisioningSystem(objective) {
        logger.info(`💻 [SystemsProvisioningAnalystAgent] Analyzing Systems Provisioning Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Provisioning Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsProvisioningAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsProvisioningAnalystAgent = Object.freeze(new SystemsProvisioningAnalystAgent());
