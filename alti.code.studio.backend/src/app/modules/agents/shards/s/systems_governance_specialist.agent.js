...
class SystemsGovernanceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsGovernanceSystem(objective) {
    logger.info(
      `💻 [SystemsGovernanceSpecialistAgent] Analyzing Systems Governance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Governance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsGovernanceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsGovernanceSpecialistAgent = Object.freeze(
  new SystemsGovernanceSpecialistAgent(),
);
