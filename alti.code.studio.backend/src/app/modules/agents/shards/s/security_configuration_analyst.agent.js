...
class SecurityConfigurationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityConfigurationSystem(objective) {
        logger.info(`💻 [SecurityConfigurationAnalystAgent] Analyzing Security Configuration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Configuration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityConfigurationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityConfigurationAnalystAgent = Object.freeze(new SecurityConfigurationAnalystAgent());
