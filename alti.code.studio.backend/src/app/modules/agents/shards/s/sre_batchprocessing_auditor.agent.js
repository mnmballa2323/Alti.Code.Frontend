...
class SREBatchProcessingAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSREBatchProcessingSystem(objective) {
    logger.info(
      `💻 [SREBatchProcessingAuditorAgent] Analyzing SRE BatchProcessing Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE BatchProcessing Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREBatchProcessingAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREBatchProcessingAuditorAgent = Object.freeze(
  new SREBatchProcessingAuditorAgent(),
);
