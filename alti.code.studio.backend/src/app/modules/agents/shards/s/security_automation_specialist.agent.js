...
class SecurityAutomationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityAutomationSystem(objective) {
    logger.info(
      `💻 [SecurityAutomationSpecialistAgent] Analyzing Security Automation Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Automation Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityAutomationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityAutomationSpecialistAgent = Object.freeze(
  new SecurityAutomationSpecialistAgent(),
);
