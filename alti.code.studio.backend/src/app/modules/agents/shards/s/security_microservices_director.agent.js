...
class SecurityMicroservicesDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityMicroservicesSystem(objective) {
    logger.info(
      `💻 [SecurityMicroservicesDirectorAgent] Analyzing Security Microservices Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Microservices Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityMicroservicesDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityMicroservicesDirectorAgent = Object.freeze(
  new SecurityMicroservicesDirectorAgent(),
);
