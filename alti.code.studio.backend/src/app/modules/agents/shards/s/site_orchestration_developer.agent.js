...
class SiteOrchestrationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteOrchestrationSystem(objective) {
        logger.info(`💻 [SiteOrchestrationDeveloperAgent] Analyzing Site Orchestration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Orchestration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteOrchestrationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteOrchestrationDeveloperAgent = Object.freeze(new SiteOrchestrationDeveloperAgent());
