...
class SecurityTelemetryLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityTelemetrySystem(objective) {
    logger.info(
      `💻 [SecurityTelemetryLeadAgent] Analyzing Security Telemetry Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Telemetry Lead Target',
      );
...
export const securityTelemetryLeadAgent = Object.freeze(
  new SecurityTelemetryLeadAgent(),
);
