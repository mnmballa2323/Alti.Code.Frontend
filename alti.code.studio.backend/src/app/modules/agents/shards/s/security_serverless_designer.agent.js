...
class SecurityServerlessDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityServerlessSystem(objective) {
    logger.info(
      `💻 [SecurityServerlessDesignerAgent] Analyzing Security Serverless Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Serverless Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityServerlessDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityServerlessDesignerAgent = Object.freeze(
  new SecurityServerlessDesignerAgent(),
);
