...
class SecurityComplianceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityComplianceSystem(objective) {
    logger.info(
      `💻 [SecurityComplianceEngineerAgent] Analyzing Security Compliance Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Compliance Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityComplianceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityComplianceEngineerAgent = Object.freeze(
  new SecurityComplianceEngineerAgent(),
);
