...
class SystemsVirtualizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsVirtualizationSystem(objective) {
    logger.info(
      `💻 [SystemsVirtualizationLeadAgent] Analyzing Systems Virtualization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Virtualization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsVirtualizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsVirtualizationLeadAgent = Object.freeze(
  new SystemsVirtualizationLeadAgent(),
);
