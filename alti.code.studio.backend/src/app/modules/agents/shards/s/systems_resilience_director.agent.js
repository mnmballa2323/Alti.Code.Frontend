...
class SystemsResilienceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsResilienceSystem(objective) {
    logger.info(
      `💻 [SystemsResilienceDirectorAgent] Analyzing Systems Resilience Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Resilience Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsResilienceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsResilienceDirectorAgent = Object.freeze(
  new SystemsResilienceDirectorAgent(),
);
