...
class SecurityGovernanceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityGovernanceSystem(objective) {
        logger.info(`💻 [SecurityGovernanceConsultantAgent] Analyzing Security Governance Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Governance Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityGovernanceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityGovernanceConsultantAgent = Object.freeze(new SecurityGovernanceConsultantAgent());
