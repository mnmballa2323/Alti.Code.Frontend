...
class SystemsArchitecturePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsArchitectureSystem(objective) {
        logger.info(`💻 [SystemsArchitecturePlannerAgent] Analyzing Systems Architecture Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Architecture Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsArchitecturePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsArchitecturePlannerAgent = Object.freeze(new SystemsArchitecturePlannerAgent());
