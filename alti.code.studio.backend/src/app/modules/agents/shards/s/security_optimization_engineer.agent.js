...
class SecurityOptimizationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityOptimizationSystem(objective) {
        logger.info(`💻 [SecurityOptimizationEngineerAgent] Analyzing Security Optimization Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Optimization Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityOptimizationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityOptimizationEngineerAgent = Object.freeze(new SecurityOptimizationEngineerAgent());
