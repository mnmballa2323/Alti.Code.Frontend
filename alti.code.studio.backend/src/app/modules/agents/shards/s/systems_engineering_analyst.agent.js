...
class SystemsEngineeringAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsEngineeringSystem(objective) {
        logger.info(`💻 [SystemsEngineeringAnalystAgent] Analyzing Systems Engineering Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Engineering Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsEngineeringAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsEngineeringAnalystAgent = Object.freeze(new SystemsEngineeringAnalystAgent());
