...
class SystemsGovernanceArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsGovernanceSystem(objective) {
    logger.info(
      `💻 [SystemsGovernanceArchitectAgent] Analyzing Systems Governance Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Governance Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsGovernanceArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsGovernanceArchitectAgent = Object.freeze(
  new SystemsGovernanceArchitectAgent(),
);
