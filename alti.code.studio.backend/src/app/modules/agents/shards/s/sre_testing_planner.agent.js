...
class SRETestingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSRETestingSystem(objective) {
    logger.info(
      `💻 [SRETestingPlannerAgent] Analyzing SRE Testing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Testing Planner Target',
      );
...
export const sRETestingPlannerAgent = Object.freeze(
  new SRETestingPlannerAgent(),
);
