...
class SiteFaultToleranceDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteFaultToleranceSystem(objective) {
        logger.info(`💻 [SiteFaultToleranceDeveloperAgent] Analyzing Site FaultTolerance Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site FaultTolerance Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteFaultToleranceDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteFaultToleranceDeveloperAgent = Object.freeze(new SiteFaultToleranceDeveloperAgent());
