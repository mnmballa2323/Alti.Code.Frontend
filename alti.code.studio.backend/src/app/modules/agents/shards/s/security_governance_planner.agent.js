...
class SecurityGovernancePlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityGovernanceSystem(objective) {
        logger.info(`💻 [SecurityGovernancePlannerAgent] Analyzing Security Governance Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Governance Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityGovernancePlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityGovernancePlannerAgent = Object.freeze(new SecurityGovernancePlannerAgent());
