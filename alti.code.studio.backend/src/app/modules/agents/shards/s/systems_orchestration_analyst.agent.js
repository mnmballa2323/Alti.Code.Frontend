...
class SystemsOrchestrationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsOrchestrationSystem(objective) {
        logger.info(`💻 [SystemsOrchestrationAnalystAgent] Analyzing Systems Orchestration Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Orchestration Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsOrchestrationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsOrchestrationAnalystAgent = Object.freeze(new SystemsOrchestrationAnalystAgent());
