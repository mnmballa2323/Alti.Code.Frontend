...
class SecurityEngineeringAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityEngineeringSystem(objective) {
        logger.info(`💻 [SecurityEngineeringAnalystAgent] Analyzing Security Engineering Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Engineering Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityEngineeringAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityEngineeringAnalystAgent = Object.freeze(new SecurityEngineeringAnalystAgent());
