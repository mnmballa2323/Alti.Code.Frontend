...
class SiteLoadBalancingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteLoadBalancingSystem(objective) {
    logger.info(
      `💻 [SiteLoadBalancingSpecialistAgent] Analyzing Site LoadBalancing Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site LoadBalancing Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteLoadBalancingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteLoadBalancingSpecialistAgent = Object.freeze(
  new SiteLoadBalancingSpecialistAgent(),
);
