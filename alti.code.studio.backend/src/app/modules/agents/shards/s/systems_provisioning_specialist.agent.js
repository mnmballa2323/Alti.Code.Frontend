...
class SystemsProvisioningSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsProvisioningSystem(objective) {
    logger.info(
      `💻 [SystemsProvisioningSpecialistAgent] Analyzing Systems Provisioning Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Provisioning Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsProvisioningSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsProvisioningSpecialistAgent = Object.freeze(
  new SystemsProvisioningSpecialistAgent(),
);
