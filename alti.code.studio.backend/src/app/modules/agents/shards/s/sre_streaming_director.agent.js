...
class SREStreamingDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSREStreamingSystem(objective) {
    logger.info(
      `💻 [SREStreamingDirectorAgent] Analyzing SRE Streaming Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Streaming Director Target',
      );
...
export const sREStreamingDirectorAgent = Object.freeze(
  new SREStreamingDirectorAgent(),
);
