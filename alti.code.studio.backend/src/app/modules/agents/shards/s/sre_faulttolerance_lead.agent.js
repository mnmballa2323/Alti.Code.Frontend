...
class SREFaultToleranceLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateSREFaultToleranceSystem(objective) {
        logger.info(`💻 [SREFaultToleranceLeadAgent] Analyzing SRE FaultTolerance Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE FaultTolerance Lead Target");
...
export const sREFaultToleranceLeadAgent = Object.freeze(new SREFaultToleranceLeadAgent());
