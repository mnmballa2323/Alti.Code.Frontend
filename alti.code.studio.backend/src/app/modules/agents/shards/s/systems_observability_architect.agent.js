...
class SystemsObservabilityArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsObservabilitySystem(objective) {
        logger.info(`💻 [SystemsObservabilityArchitectAgent] Analyzing Systems Observability Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Observability Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsObservabilityArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsObservabilityArchitectAgent = Object.freeze(new SystemsObservabilityArchitectAgent());
