...
class SystemsDataWarehousePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [SystemsDataWarehousePlannerAgent] Analyzing Systems DataWarehouse Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems DataWarehouse Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsDataWarehousePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsDataWarehousePlannerAgent = Object.freeze(
  new SystemsDataWarehousePlannerAgent(),
);
