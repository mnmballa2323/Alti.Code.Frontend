...
class SRETelemetryDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSRETelemetrySystem(objective) {
    logger.info(
      `💻 [SRETelemetryDirectorAgent] Analyzing SRE Telemetry Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Telemetry Director Target',
      );
...
export const sRETelemetryDirectorAgent = Object.freeze(
  new SRETelemetryDirectorAgent(),
);
