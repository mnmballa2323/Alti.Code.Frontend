...
class SecurityGovernanceStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityGovernanceSystem(objective) {
    logger.info(
      `💻 [SecurityGovernanceStrategistAgent] Analyzing Security Governance Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Governance Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityGovernanceStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityGovernanceStrategistAgent = Object.freeze(
  new SecurityGovernanceStrategistAgent(),
);
