...
class SecurityResiliencePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityResilienceSystem(objective) {
    logger.info(
      `💻 [SecurityResiliencePlannerAgent] Analyzing Security Resilience Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Resilience Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityResiliencePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityResiliencePlannerAgent = Object.freeze(
  new SecurityResiliencePlannerAgent(),
);
