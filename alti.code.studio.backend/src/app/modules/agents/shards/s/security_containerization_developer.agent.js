...
class SecurityContainerizationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityContainerizationSystem(objective) {
    logger.info(
      `💻 [SecurityContainerizationDeveloperAgent] Analyzing Security Containerization Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Containerization Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityContainerizationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityContainerizationDeveloperAgent = Object.freeze(
  new SecurityContainerizationDeveloperAgent(),
);
