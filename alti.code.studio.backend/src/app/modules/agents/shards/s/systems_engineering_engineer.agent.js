...
class SystemsEngineeringEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsEngineeringSystem(objective) {
    logger.info(
      `💻 [SystemsEngineeringEngineerAgent] Analyzing Systems Engineering Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Engineering Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsEngineeringEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsEngineeringEngineerAgent = Object.freeze(
  new SystemsEngineeringEngineerAgent(),
);
