...
class SREArchitectureOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSREArchitectureSystem(objective) {
    logger.info(
      `💻 [SREArchitectureOrchestratorAgent] Analyzing SRE Architecture Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Architecture Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREArchitectureOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREArchitectureOrchestratorAgent = Object.freeze(
  new SREArchitectureOrchestratorAgent(),
);
