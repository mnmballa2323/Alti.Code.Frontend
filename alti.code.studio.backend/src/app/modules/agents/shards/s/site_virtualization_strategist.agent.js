...
class SiteVirtualizationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteVirtualizationSystem(objective) {
    logger.info(
      `💻 [SiteVirtualizationStrategistAgent] Analyzing Site Virtualization Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Virtualization Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteVirtualizationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteVirtualizationStrategistAgent = Object.freeze(
  new SiteVirtualizationStrategistAgent(),
);
