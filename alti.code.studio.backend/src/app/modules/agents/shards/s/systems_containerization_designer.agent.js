...
class SystemsContainerizationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsContainerizationSystem(objective) {
        logger.info(`💻 [SystemsContainerizationDesignerAgent] Analyzing Systems Containerization Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Containerization Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsContainerizationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsContainerizationDesignerAgent = Object.freeze(new SystemsContainerizationDesignerAgent());
