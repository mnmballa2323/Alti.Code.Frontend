...
class SecurityCompliancePlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityComplianceSystem(objective) {
    logger.info(
      `💻 [SecurityCompliancePlannerAgent] Analyzing Security Compliance Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Compliance Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityCompliancePlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityCompliancePlannerAgent = Object.freeze(
  new SecurityCompliancePlannerAgent(),
);
