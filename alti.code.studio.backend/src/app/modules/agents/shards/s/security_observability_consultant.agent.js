...
class SecurityObservabilityConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityObservabilitySystem(objective) {
        logger.info(`💻 [SecurityObservabilityConsultantAgent] Analyzing Security Observability Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Observability Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityObservabilityConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityObservabilityConsultantAgent = Object.freeze(new SecurityObservabilityConsultantAgent());
