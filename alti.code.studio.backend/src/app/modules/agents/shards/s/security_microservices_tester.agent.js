...
class SecurityMicroservicesTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityMicroservicesSystem(objective) {
    logger.info(
      `💻 [SecurityMicroservicesTesterAgent] Analyzing Security Microservices Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Microservices Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityMicroservicesTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityMicroservicesTesterAgent = Object.freeze(
  new SecurityMicroservicesTesterAgent(),
);
