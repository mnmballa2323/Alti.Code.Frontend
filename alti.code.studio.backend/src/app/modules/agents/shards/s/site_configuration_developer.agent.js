...
class SiteConfigurationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteConfigurationSystem(objective) {
    logger.info(
      `💻 [SiteConfigurationDeveloperAgent] Analyzing Site Configuration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Configuration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteConfigurationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteConfigurationDeveloperAgent = Object.freeze(
  new SiteConfigurationDeveloperAgent(),
);
