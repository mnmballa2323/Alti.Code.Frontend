...
class SystemsFaultToleranceStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsFaultToleranceSystem(objective) {
        logger.info(`💻 [SystemsFaultToleranceStrategistAgent] Analyzing Systems FaultTolerance Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems FaultTolerance Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsFaultToleranceStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsFaultToleranceStrategistAgent = Object.freeze(new SystemsFaultToleranceStrategistAgent());
