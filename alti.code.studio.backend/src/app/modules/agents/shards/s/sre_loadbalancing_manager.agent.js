...
class SRELoadBalancingManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateSRELoadBalancingSystem(objective) {
        logger.info(`💻 [SRELoadBalancingManagerAgent] Analyzing SRE LoadBalancing Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE LoadBalancing Manager Target");
...
export const sRELoadBalancingManagerAgent = Object.freeze(new SRELoadBalancingManagerAgent());
