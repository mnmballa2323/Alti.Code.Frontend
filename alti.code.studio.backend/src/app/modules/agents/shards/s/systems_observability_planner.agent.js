...
class SystemsObservabilityPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsObservabilitySystem(objective) {
    logger.info(
      `💻 [SystemsObservabilityPlannerAgent] Analyzing Systems Observability Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Observability Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsObservabilityPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsObservabilityPlannerAgent = Object.freeze(
  new SystemsObservabilityPlannerAgent(),
);
