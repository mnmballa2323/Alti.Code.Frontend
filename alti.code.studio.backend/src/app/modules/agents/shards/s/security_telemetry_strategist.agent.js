...
class SecurityTelemetryStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityTelemetrySystem(objective) {
    logger.info(
      `💻 [SecurityTelemetryStrategistAgent] Analyzing Security Telemetry Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Telemetry Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityTelemetryStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityTelemetryStrategistAgent = Object.freeze(
  new SecurityTelemetryStrategistAgent(),
);
