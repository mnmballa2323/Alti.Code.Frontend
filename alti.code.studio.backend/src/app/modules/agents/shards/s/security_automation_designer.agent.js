...
class SecurityAutomationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityAutomationSystem(objective) {
    logger.info(
      `💻 [SecurityAutomationDesignerAgent] Analyzing Security Automation Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Automation Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityAutomationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityAutomationDesignerAgent = Object.freeze(
  new SecurityAutomationDesignerAgent(),
);
