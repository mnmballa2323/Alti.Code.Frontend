...
class SecurityEngineeringManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityEngineeringSystem(objective) {
    logger.info(
      `💻 [SecurityEngineeringManagerAgent] Analyzing Security Engineering Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Engineering Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityEngineeringManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityEngineeringManagerAgent = Object.freeze(
  new SecurityEngineeringManagerAgent(),
);
