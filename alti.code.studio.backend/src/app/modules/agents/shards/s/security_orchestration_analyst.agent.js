...
class SecurityOrchestrationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityOrchestrationSystem(objective) {
    logger.info(
      `💻 [SecurityOrchestrationAnalystAgent] Analyzing Security Orchestration Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Orchestration Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityOrchestrationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityOrchestrationAnalystAgent = Object.freeze(
  new SecurityOrchestrationAnalystAgent(),
);
