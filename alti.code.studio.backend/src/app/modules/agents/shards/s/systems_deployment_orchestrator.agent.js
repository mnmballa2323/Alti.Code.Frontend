...
class SystemsDeploymentOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsDeploymentSystem(objective) {
        logger.info(`💻 [SystemsDeploymentOrchestratorAgent] Analyzing Systems Deployment Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Deployment Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsDeploymentOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsDeploymentOrchestratorAgent = Object.freeze(new SystemsDeploymentOrchestratorAgent());
