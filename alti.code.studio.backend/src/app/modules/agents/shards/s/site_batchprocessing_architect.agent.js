...
class SiteBatchProcessingArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteBatchProcessingSystem(objective) {
    logger.info(
      `💻 [SiteBatchProcessingArchitectAgent] Analyzing Site BatchProcessing Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site BatchProcessing Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteBatchProcessingArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteBatchProcessingArchitectAgent = Object.freeze(
  new SiteBatchProcessingArchitectAgent(),
);
