...
class SREFaultToleranceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSREFaultToleranceSystem(objective) {
    logger.info(
      `💻 [SREFaultToleranceDesignerAgent] Analyzing SRE FaultTolerance Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE FaultTolerance Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREFaultToleranceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREFaultToleranceDesignerAgent = Object.freeze(
  new SREFaultToleranceDesignerAgent(),
);
