...
class SystemsFaultToleranceDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsFaultToleranceSystem(objective) {
        logger.info(`💻 [SystemsFaultToleranceDirectorAgent] Analyzing Systems FaultTolerance Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems FaultTolerance Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsFaultToleranceDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsFaultToleranceDirectorAgent = Object.freeze(new SystemsFaultToleranceDirectorAgent());
