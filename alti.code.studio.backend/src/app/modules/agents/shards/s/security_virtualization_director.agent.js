...
class SecurityVirtualizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityVirtualizationSystem(objective) {
        logger.info(`💻 [SecurityVirtualizationDirectorAgent] Analyzing Security Virtualization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Virtualization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityVirtualizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityVirtualizationDirectorAgent = Object.freeze(new SecurityVirtualizationDirectorAgent());
