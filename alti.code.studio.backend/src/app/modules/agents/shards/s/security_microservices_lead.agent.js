...
class SecurityMicroservicesLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityMicroservicesSystem(objective) {
        logger.info(`💻 [SecurityMicroservicesLeadAgent] Analyzing Security Microservices Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Microservices Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityMicroservicesLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityMicroservicesLeadAgent = Object.freeze(new SecurityMicroservicesLeadAgent());
