...
class SRELoadBalancingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSRELoadBalancingSystem(objective) {
    logger.info(
      `💻 [SRELoadBalancingOrchestratorAgent] Analyzing SRE LoadBalancing Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE LoadBalancing Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SRELoadBalancingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sRELoadBalancingOrchestratorAgent = Object.freeze(
  new SRELoadBalancingOrchestratorAgent(),
);
