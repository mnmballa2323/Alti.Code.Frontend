...
class SystemsAutomationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsAutomationSystem(objective) {
    logger.info(
      `💻 [SystemsAutomationConsultantAgent] Analyzing Systems Automation Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Automation Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsAutomationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsAutomationConsultantAgent = Object.freeze(
  new SystemsAutomationConsultantAgent(),
);
