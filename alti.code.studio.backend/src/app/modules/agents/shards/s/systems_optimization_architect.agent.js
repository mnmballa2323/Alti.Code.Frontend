...
class SystemsOptimizationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsOptimizationSystem(objective) {
    logger.info(
      `💻 [SystemsOptimizationArchitectAgent] Analyzing Systems Optimization Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Optimization Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsOptimizationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsOptimizationArchitectAgent = Object.freeze(
  new SystemsOptimizationArchitectAgent(),
);
