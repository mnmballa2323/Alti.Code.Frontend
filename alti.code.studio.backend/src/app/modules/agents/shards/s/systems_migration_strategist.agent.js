...
class SystemsMigrationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsMigrationSystem(objective) {
        logger.info(`💻 [SystemsMigrationStrategistAgent] Analyzing Systems Migration Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Migration Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsMigrationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsMigrationStrategistAgent = Object.freeze(new SystemsMigrationStrategistAgent());
