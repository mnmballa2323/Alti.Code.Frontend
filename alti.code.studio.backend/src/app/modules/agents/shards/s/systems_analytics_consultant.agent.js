...
class SystemsAnalyticsConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsAnalyticsSystem(objective) {
    logger.info(
      `💻 [SystemsAnalyticsConsultantAgent] Analyzing Systems Analytics Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Analytics Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsAnalyticsConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsAnalyticsConsultantAgent = Object.freeze(
  new SystemsAnalyticsConsultantAgent(),
);
