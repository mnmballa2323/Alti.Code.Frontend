...
class SystemsProvisioningManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsProvisioningSystem(objective) {
    logger.info(
      `💻 [SystemsProvisioningManagerAgent] Analyzing Systems Provisioning Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Provisioning Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsProvisioningManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsProvisioningManagerAgent = Object.freeze(
  new SystemsProvisioningManagerAgent(),
);
