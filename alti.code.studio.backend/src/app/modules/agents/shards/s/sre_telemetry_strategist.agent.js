...
class SRETelemetryStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSRETelemetrySystem(objective) {
    logger.info(
      `💻 [SRETelemetryStrategistAgent] Analyzing SRE Telemetry Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Telemetry Strategist Target',
      );
...
export const sRETelemetryStrategistAgent = Object.freeze(
  new SRETelemetryStrategistAgent(),
);
