...
class SystemsFaultToleranceManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsFaultToleranceSystem(objective) {
        logger.info(`💻 [SystemsFaultToleranceManagerAgent] Analyzing Systems FaultTolerance Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems FaultTolerance Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsFaultToleranceManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsFaultToleranceManagerAgent = Object.freeze(new SystemsFaultToleranceManagerAgent());
