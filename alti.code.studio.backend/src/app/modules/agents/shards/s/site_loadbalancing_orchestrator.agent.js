...
class SiteLoadBalancingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteLoadBalancingSystem(objective) {
    logger.info(
      `💻 [SiteLoadBalancingOrchestratorAgent] Analyzing Site LoadBalancing Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site LoadBalancing Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteLoadBalancingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteLoadBalancingOrchestratorAgent = Object.freeze(
  new SiteLoadBalancingOrchestratorAgent(),
);
