...
class SecurityOrchestrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityOrchestrationSystem(objective) {
    logger.info(
      `💻 [SecurityOrchestrationConsultantAgent] Analyzing Security Orchestration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Orchestration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityOrchestrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityOrchestrationConsultantAgent = Object.freeze(
  new SecurityOrchestrationConsultantAgent(),
);
