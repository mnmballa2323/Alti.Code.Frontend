...
class SecurityOrchestrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityOrchestrationSystem(objective) {
    logger.info(
      `💻 [SecurityOrchestrationArchitectAgent] Analyzing Security Orchestration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Orchestration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityOrchestrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityOrchestrationArchitectAgent = Object.freeze(
  new SecurityOrchestrationArchitectAgent(),
);
