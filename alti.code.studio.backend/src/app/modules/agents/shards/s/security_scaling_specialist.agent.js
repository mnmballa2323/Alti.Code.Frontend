...
class SecurityScalingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityScalingSystem(objective) {
    logger.info(
      `💻 [SecurityScalingSpecialistAgent] Analyzing Security Scaling Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Scaling Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityScalingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityScalingSpecialistAgent = Object.freeze(
  new SecurityScalingSpecialistAgent(),
);
