...
class SiteBatchProcessingDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteBatchProcessingSystem(objective) {
        logger.info(`💻 [SiteBatchProcessingDeveloperAgent] Analyzing Site BatchProcessing Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site BatchProcessing Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteBatchProcessingDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteBatchProcessingDeveloperAgent = Object.freeze(new SiteBatchProcessingDeveloperAgent());
