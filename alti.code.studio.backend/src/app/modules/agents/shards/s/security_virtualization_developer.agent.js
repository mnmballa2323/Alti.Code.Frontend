...
class SecurityVirtualizationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityVirtualizationSystem(objective) {
        logger.info(`💻 [SecurityVirtualizationDeveloperAgent] Analyzing Security Virtualization Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Virtualization Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityVirtualizationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityVirtualizationDeveloperAgent = Object.freeze(new SecurityVirtualizationDeveloperAgent());
