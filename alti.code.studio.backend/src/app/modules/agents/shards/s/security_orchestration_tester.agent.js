...
class SecurityOrchestrationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityOrchestrationSystem(objective) {
        logger.info(`💻 [SecurityOrchestrationTesterAgent] Analyzing Security Orchestration Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Orchestration Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityOrchestrationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityOrchestrationTesterAgent = Object.freeze(new SecurityOrchestrationTesterAgent());
