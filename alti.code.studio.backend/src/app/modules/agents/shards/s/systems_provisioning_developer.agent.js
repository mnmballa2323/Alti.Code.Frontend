...
class SystemsProvisioningDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsProvisioningSystem(objective) {
        logger.info(`💻 [SystemsProvisioningDeveloperAgent] Analyzing Systems Provisioning Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Provisioning Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsProvisioningDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsProvisioningDeveloperAgent = Object.freeze(new SystemsProvisioningDeveloperAgent());
