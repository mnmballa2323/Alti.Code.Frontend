...
class SystemsDataWarehouseAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsDataWarehouseSystem(objective) {
        logger.info(`💻 [SystemsDataWarehouseAnalystAgent] Analyzing Systems DataWarehouse Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems DataWarehouse Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsDataWarehouseAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsDataWarehouseAnalystAgent = Object.freeze(new SystemsDataWarehouseAnalystAgent());
