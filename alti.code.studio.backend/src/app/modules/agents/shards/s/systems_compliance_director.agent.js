...
class SystemsComplianceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsComplianceSystem(objective) {
    logger.info(
      `💻 [SystemsComplianceDirectorAgent] Analyzing Systems Compliance Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Compliance Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsComplianceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsComplianceDirectorAgent = Object.freeze(
  new SystemsComplianceDirectorAgent(),
);
