...
class SecurityServerlessDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityServerlessSystem(objective) {
    logger.info(
      `💻 [SecurityServerlessDeveloperAgent] Analyzing Security Serverless Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Serverless Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityServerlessDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityServerlessDeveloperAgent = Object.freeze(
  new SecurityServerlessDeveloperAgent(),
);
