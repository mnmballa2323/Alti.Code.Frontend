...
class SREServerlessOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSREServerlessSystem(objective) {
    logger.info(
      `💻 [SREServerlessOrchestratorAgent] Analyzing SRE Serverless Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Serverless Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREServerlessOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREServerlessOrchestratorAgent = Object.freeze(
  new SREServerlessOrchestratorAgent(),
);
