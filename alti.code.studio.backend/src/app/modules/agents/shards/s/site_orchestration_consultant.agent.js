...
class SiteOrchestrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteOrchestrationSystem(objective) {
    logger.info(
      `💻 [SiteOrchestrationConsultantAgent] Analyzing Site Orchestration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Orchestration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteOrchestrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteOrchestrationConsultantAgent = Object.freeze(
  new SiteOrchestrationConsultantAgent(),
);
