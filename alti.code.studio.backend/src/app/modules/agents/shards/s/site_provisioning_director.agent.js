...
class SiteProvisioningDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteProvisioningSystem(objective) {
    logger.info(
      `💻 [SiteProvisioningDirectorAgent] Analyzing Site Provisioning Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Provisioning Director Target',
      );
...
export const siteProvisioningDirectorAgent = Object.freeze(
  new SiteProvisioningDirectorAgent(),
);
