...
class SecurityContainerizationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityContainerizationSystem(objective) {
        logger.info(`💻 [SecurityContainerizationStrategistAgent] Analyzing Security Containerization Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Containerization Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityContainerizationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityContainerizationStrategistAgent = Object.freeze(new SecurityContainerizationStrategistAgent());
