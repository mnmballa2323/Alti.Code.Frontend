...
class SystemsContainerizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsContainerizationSystem(objective) {
        logger.info(`💻 [SystemsContainerizationDirectorAgent] Analyzing Systems Containerization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Containerization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsContainerizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsContainerizationDirectorAgent = Object.freeze(new SystemsContainerizationDirectorAgent());
