...
class SiteOrchestrationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteOrchestrationSystem(objective) {
        logger.info(`💻 [SiteOrchestrationOrchestratorAgent] Analyzing Site Orchestration Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Orchestration Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteOrchestrationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteOrchestrationOrchestratorAgent = Object.freeze(new SiteOrchestrationOrchestratorAgent());
