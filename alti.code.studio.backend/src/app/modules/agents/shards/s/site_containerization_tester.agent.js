...
class SiteContainerizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteContainerizationSystem(objective) {
    logger.info(
      `💻 [SiteContainerizationTesterAgent] Analyzing Site Containerization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Containerization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteContainerizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteContainerizationTesterAgent = Object.freeze(
  new SiteContainerizationTesterAgent(),
);
