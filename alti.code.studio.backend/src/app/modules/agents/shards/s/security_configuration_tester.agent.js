...
class SecurityConfigurationTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityConfigurationSystem(objective) {
        logger.info(`💻 [SecurityConfigurationTesterAgent] Analyzing Security Configuration Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Configuration Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityConfigurationTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityConfigurationTesterAgent = Object.freeze(new SecurityConfigurationTesterAgent());
