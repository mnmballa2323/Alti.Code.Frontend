...
class SystemsOrchestrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsOrchestrationSystem(objective) {
    logger.info(
      `💻 [SystemsOrchestrationStrategistAgent] Analyzing Systems Orchestration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Orchestration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsOrchestrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsOrchestrationStrategistAgent = Object.freeze(
  new SystemsOrchestrationStrategistAgent(),
);
