...
class SecurityEngineeringDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityEngineeringSystem(objective) {
    logger.info(
      `💻 [SecurityEngineeringDesignerAgent] Analyzing Security Engineering Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Engineering Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityEngineeringDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityEngineeringDesignerAgent = Object.freeze(
  new SecurityEngineeringDesignerAgent(),
);
