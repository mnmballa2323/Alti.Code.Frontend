...
class SystemsResilienceDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsResilienceSystem(objective) {
        logger.info(`💻 [SystemsResilienceDeveloperAgent] Analyzing Systems Resilience Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Resilience Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsResilienceDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsResilienceDeveloperAgent = Object.freeze(new SystemsResilienceDeveloperAgent());
