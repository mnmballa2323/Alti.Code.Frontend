...
class SecuritySecurityLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateSecuritySecuritySystem(objective) {
    logger.info(
      `💻 [SecuritySecurityLeadAgent] Analyzing Security Security Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Security Lead Target',
      );
...
export const securitySecurityLeadAgent = Object.freeze(
  new SecuritySecurityLeadAgent(),
);
