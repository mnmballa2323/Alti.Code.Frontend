...
class SiteOrchestrationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteOrchestrationSystem(objective) {
        logger.info(`💻 [SiteOrchestrationStrategistAgent] Analyzing Site Orchestration Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Orchestration Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteOrchestrationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteOrchestrationStrategistAgent = Object.freeze(new SiteOrchestrationStrategistAgent());
