...
export class SchemaAgent {
  constructor() {
    this.name = 'schema';
    this.description =
      'Dynamic Ontology Architect and Knowledge Graph Maintainer';
    this.capabilities = [
  ...
  async mapOntology(rawSchema, boundedContext = 'Core') {
    if (
      ...
      rawSchema.trim().length === 0
    ) {
      throw new Error('SchemaAgent: rawSchema must be a non-empty string.');
    }

    logger.info(
      `🕸️ Schema Agent: Traversing '${boundedContext}' domain — Initiating Ontology extraction.`,
    );
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          `🕸️ Schema Agent: JSON parse failed — returning partial result.`,
        );
        ontologyReport = {
          extractedEntities: [],
          relationships: [],
      ...
      logger.info(
        `🕸️ Schema Agent: Ontology extraction complete. Bound [${entityCount}] entities.`,
      );
  ...
  async process(state) {
    ...
    if (!schemaString) {
      return {
        ...
        results: [
          ...(state.results ?? []),
          '❌ FAILED SchemaAgent: No schema content provided',
        ],
      };
    }
    ...
    try {
      ...
      return {
        ...
        results: [
          ...(state.results ?? []),
          `❌ FAILED SchemaAgent: ${err.message}`,
        ],
      };
    }
  }
