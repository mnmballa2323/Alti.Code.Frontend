...
class SiteArchitectureDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteArchitectureSystem(objective) {
    logger.info(
      `💻 [SiteArchitectureDeveloperAgent] Analyzing Site Architecture Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Architecture Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteArchitectureDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteArchitectureDeveloperAgent = Object.freeze(
  new SiteArchitectureDeveloperAgent(),
);
