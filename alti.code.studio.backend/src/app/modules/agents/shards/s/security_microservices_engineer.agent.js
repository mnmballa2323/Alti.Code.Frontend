...
class SecurityMicroservicesEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityMicroservicesSystem(objective) {
        logger.info(`💻 [SecurityMicroservicesEngineerAgent] Analyzing Security Microservices Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Microservices Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityMicroservicesEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityMicroservicesEngineerAgent = Object.freeze(new SecurityMicroservicesEngineerAgent());
