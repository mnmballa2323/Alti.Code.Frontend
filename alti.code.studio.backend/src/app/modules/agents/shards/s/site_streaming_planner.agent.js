...
class SiteStreamingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteStreamingSystem(objective) {
    logger.info(
      `💻 [SiteStreamingPlannerAgent] Analyzing Site Streaming Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Streaming Planner Target',
      );
...
export const siteStreamingPlannerAgent = Object.freeze(
  new SiteStreamingPlannerAgent(),
);
