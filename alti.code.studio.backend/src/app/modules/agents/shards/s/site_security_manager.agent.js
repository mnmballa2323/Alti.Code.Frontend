...
class SiteSecurityManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteSecuritySystem(objective) {
    logger.info(
      `💻 [SiteSecurityManagerAgent] Analyzing Site Security Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Security Manager Target',
      );
...
export const siteSecurityManagerAgent = Object.freeze(
  new SiteSecurityManagerAgent(),
);
