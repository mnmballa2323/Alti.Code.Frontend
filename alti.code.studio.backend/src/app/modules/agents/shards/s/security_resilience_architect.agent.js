...
class SecurityResilienceArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityResilienceSystem(objective) {
        logger.info(`💻 [SecurityResilienceArchitectAgent] Analyzing Security Resilience Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Resilience Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityResilienceArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityResilienceArchitectAgent = Object.freeze(new SecurityResilienceArchitectAgent());
