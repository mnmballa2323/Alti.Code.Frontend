...
class SecurityVirtualizationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityVirtualizationSystem(objective) {
    logger.info(
      `💻 [SecurityVirtualizationEngineerAgent] Analyzing Security Virtualization Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Virtualization Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityVirtualizationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityVirtualizationEngineerAgent = Object.freeze(
  new SecurityVirtualizationEngineerAgent(),
);
