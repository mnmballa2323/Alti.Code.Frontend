...
class SystemsStreamingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsStreamingSystem(objective) {
    logger.info(
      `💻 [SystemsStreamingSpecialistAgent] Analyzing Systems Streaming Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Streaming Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsStreamingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsStreamingSpecialistAgent = Object.freeze(
  new SystemsStreamingSpecialistAgent(),
);
