...
class SREServerlessAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSREServerlessSystem(objective) {
    logger.info(
      `💻 [SREServerlessAnalystAgent] Analyzing SRE Serverless Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Serverless Analyst Target',
      );
...
export const sREServerlessAnalystAgent = Object.freeze(
  new SREServerlessAnalystAgent(),
);
