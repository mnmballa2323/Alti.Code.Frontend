...
class SystemsBatchProcessingManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsBatchProcessingSystem(objective) {
        logger.info(`💻 [SystemsBatchProcessingManagerAgent] Analyzing Systems BatchProcessing Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems BatchProcessing Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsBatchProcessingManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsBatchProcessingManagerAgent = Object.freeze(new SystemsBatchProcessingManagerAgent());
