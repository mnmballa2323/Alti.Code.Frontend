...
class SiteSecurityPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteSecuritySystem(objective) {
    logger.info(
      `💻 [SiteSecurityPlannerAgent] Analyzing Site Security Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Security Planner Target',
      );
...
export const siteSecurityPlannerAgent = Object.freeze(
  new SiteSecurityPlannerAgent(),
);
