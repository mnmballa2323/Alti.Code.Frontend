...
class SecurityOrchestrationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityOrchestrationSystem(objective) {
        logger.info(`💻 [SecurityOrchestrationStrategistAgent] Analyzing Security Orchestration Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Orchestration Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityOrchestrationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityOrchestrationStrategistAgent = Object.freeze(new SecurityOrchestrationStrategistAgent());
