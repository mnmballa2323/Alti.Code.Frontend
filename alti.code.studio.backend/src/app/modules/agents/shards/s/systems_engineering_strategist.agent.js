...
class SystemsEngineeringStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsEngineeringSystem(objective) {
    logger.info(
      `💻 [SystemsEngineeringStrategistAgent] Analyzing Systems Engineering Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Engineering Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsEngineeringStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsEngineeringStrategistAgent = Object.freeze(
  new SystemsEngineeringStrategistAgent(),
);
