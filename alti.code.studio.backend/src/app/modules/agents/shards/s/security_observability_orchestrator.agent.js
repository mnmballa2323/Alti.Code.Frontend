...
class SecurityObservabilityOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityObservabilitySystem(objective) {
        logger.info(`💻 [SecurityObservabilityOrchestratorAgent] Analyzing Security Observability Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Observability Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityObservabilityOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityObservabilityOrchestratorAgent = Object.freeze(new SecurityObservabilityOrchestratorAgent());
