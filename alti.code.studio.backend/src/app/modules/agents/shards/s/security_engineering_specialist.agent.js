...
class SecurityEngineeringSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityEngineeringSystem(objective) {
    logger.info(
      `💻 [SecurityEngineeringSpecialistAgent] Analyzing Security Engineering Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Engineering Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityEngineeringSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityEngineeringSpecialistAgent = Object.freeze(
  new SecurityEngineeringSpecialistAgent(),
);
