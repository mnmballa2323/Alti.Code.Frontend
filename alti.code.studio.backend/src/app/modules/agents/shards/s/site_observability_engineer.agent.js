...
class SiteObservabilityEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteObservabilitySystem(objective) {
        logger.info(`💻 [SiteObservabilityEngineerAgent] Analyzing Site Observability Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Observability Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteObservabilityEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteObservabilityEngineerAgent = Object.freeze(new SiteObservabilityEngineerAgent());
