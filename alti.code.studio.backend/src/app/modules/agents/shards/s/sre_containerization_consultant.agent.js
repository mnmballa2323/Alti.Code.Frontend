...
class SREContainerizationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSREContainerizationSystem(objective) {
        logger.info(`💻 [SREContainerizationConsultantAgent] Analyzing SRE Containerization Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE Containerization Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SREContainerizationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const sREContainerizationConsultantAgent = Object.freeze(new SREContainerizationConsultantAgent());
