...
class SREMicroservicesDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSREMicroservicesSystem(objective) {
    logger.info(
      `💻 [SREMicroservicesDeveloperAgent] Analyzing SRE Microservices Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Microservices Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREMicroservicesDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREMicroservicesDeveloperAgent = Object.freeze(
  new SREMicroservicesDeveloperAgent(),
);
