...
class SecurityOrchestrationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityOrchestrationSystem(objective) {
    logger.info(
      `💻 [SecurityOrchestrationAuditorAgent] Analyzing Security Orchestration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Orchestration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityOrchestrationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityOrchestrationAuditorAgent = Object.freeze(
  new SecurityOrchestrationAuditorAgent(),
);
