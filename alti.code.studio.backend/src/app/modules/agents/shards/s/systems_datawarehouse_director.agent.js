...
class SystemsDataWarehouseDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsDataWarehouseSystem(objective) {
        logger.info(`💻 [SystemsDataWarehouseDirectorAgent] Analyzing Systems DataWarehouse Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems DataWarehouse Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsDataWarehouseDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsDataWarehouseDirectorAgent = Object.freeze(new SystemsDataWarehouseDirectorAgent());
