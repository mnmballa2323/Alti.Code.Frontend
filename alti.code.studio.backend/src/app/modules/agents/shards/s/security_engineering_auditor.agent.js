...
class SecurityEngineeringAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityEngineeringSystem(objective) {
        logger.info(`💻 [SecurityEngineeringAuditorAgent] Analyzing Security Engineering Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Engineering Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityEngineeringAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityEngineeringAuditorAgent = Object.freeze(new SecurityEngineeringAuditorAgent());
