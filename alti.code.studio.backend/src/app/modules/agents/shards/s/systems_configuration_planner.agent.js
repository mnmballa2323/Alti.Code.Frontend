...
class SystemsConfigurationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsConfigurationSystem(objective) {
    logger.info(
      `💻 [SystemsConfigurationPlannerAgent] Analyzing Systems Configuration Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Configuration Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsConfigurationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsConfigurationPlannerAgent = Object.freeze(
  new SystemsConfigurationPlannerAgent(),
);
