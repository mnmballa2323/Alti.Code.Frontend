...
class SecurityMicroservicesPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityMicroservicesSystem(objective) {
    logger.info(
      `💻 [SecurityMicroservicesPlannerAgent] Analyzing Security Microservices Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Microservices Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityMicroservicesPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityMicroservicesPlannerAgent = Object.freeze(
  new SecurityMicroservicesPlannerAgent(),
);
