...
class SystemsVirtualizationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsVirtualizationSystem(objective) {
        logger.info(`💻 [SystemsVirtualizationPlannerAgent] Analyzing Systems Virtualization Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Virtualization Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsVirtualizationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsVirtualizationPlannerAgent = Object.freeze(new SystemsVirtualizationPlannerAgent());
