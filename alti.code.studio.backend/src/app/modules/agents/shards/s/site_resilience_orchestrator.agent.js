...
class SiteResilienceOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteResilienceSystem(objective) {
        logger.info(`💻 [SiteResilienceOrchestratorAgent] Analyzing Site Resilience Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Resilience Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteResilienceOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteResilienceOrchestratorAgent = Object.freeze(new SiteResilienceOrchestratorAgent());
