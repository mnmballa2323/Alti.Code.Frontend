...
class SystemsConfigurationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsConfigurationSystem(objective) {
    logger.info(
      `💻 [SystemsConfigurationEngineerAgent] Analyzing Systems Configuration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Configuration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsConfigurationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsConfigurationEngineerAgent = Object.freeze(
  new SystemsConfigurationEngineerAgent(),
);
