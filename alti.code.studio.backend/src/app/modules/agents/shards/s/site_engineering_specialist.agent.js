...
class SiteEngineeringSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteEngineeringSystem(objective) {
    logger.info(
      `💻 [SiteEngineeringSpecialistAgent] Analyzing Site Engineering Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Engineering Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteEngineeringSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteEngineeringSpecialistAgent = Object.freeze(
  new SiteEngineeringSpecialistAgent(),
);
