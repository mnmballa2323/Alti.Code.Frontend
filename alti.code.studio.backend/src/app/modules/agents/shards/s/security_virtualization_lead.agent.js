...
class SecurityVirtualizationLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityVirtualizationSystem(objective) {
    logger.info(
      `💻 [SecurityVirtualizationLeadAgent] Analyzing Security Virtualization Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Virtualization Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityVirtualizationLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityVirtualizationLeadAgent = Object.freeze(
  new SecurityVirtualizationLeadAgent(),
);
