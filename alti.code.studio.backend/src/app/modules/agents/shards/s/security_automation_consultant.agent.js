...
class SecurityAutomationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityAutomationSystem(objective) {
    logger.info(
      `💻 [SecurityAutomationConsultantAgent] Analyzing Security Automation Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Automation Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityAutomationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityAutomationConsultantAgent = Object.freeze(
  new SecurityAutomationConsultantAgent(),
);
