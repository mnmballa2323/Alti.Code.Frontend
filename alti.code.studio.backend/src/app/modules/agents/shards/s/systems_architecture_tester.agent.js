...
class SystemsArchitectureTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsArchitectureSystem(objective) {
        logger.info(`💻 [SystemsArchitectureTesterAgent] Analyzing Systems Architecture Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Architecture Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsArchitectureTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsArchitectureTesterAgent = Object.freeze(new SystemsArchitectureTesterAgent());
