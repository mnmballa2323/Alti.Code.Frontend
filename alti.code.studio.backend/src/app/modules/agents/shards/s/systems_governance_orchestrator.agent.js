...
class SystemsGovernanceOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsGovernanceSystem(objective) {
        logger.info(`💻 [SystemsGovernanceOrchestratorAgent] Analyzing Systems Governance Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Governance Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsGovernanceOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsGovernanceOrchestratorAgent = Object.freeze(new SystemsGovernanceOrchestratorAgent());
