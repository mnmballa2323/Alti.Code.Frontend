...
class SecurityArchitectureArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityArchitectureSystem(objective) {
        logger.info(`💻 [SecurityArchitectureArchitectAgent] Analyzing Security Architecture Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Architecture Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityArchitectureArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityArchitectureArchitectAgent = Object.freeze(new SecurityArchitectureArchitectAgent());
