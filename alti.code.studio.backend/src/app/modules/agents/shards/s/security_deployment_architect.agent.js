...
class SecurityDeploymentArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityDeploymentSystem(objective) {
    logger.info(
      `💻 [SecurityDeploymentArchitectAgent] Analyzing Security Deployment Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Deployment Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityDeploymentArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityDeploymentArchitectAgent = Object.freeze(
  new SecurityDeploymentArchitectAgent(),
);
