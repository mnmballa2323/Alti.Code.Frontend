...
class SiteContainerizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteContainerizationSystem(objective) {
    logger.info(
      `💻 [SiteContainerizationDirectorAgent] Analyzing Site Containerization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Containerization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteContainerizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteContainerizationDirectorAgent = Object.freeze(
  new SiteContainerizationDirectorAgent(),
);
