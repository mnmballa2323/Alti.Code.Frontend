...
class SystemsAnalyticsArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsAnalyticsSystem(objective) {
    logger.info(
      `💻 [SystemsAnalyticsArchitectAgent] Analyzing Systems Analytics Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Analytics Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsAnalyticsArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsAnalyticsArchitectAgent = Object.freeze(
  new SystemsAnalyticsArchitectAgent(),
);
