...
class SystemsLoadBalancingTesterAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsLoadBalancingSystem(objective) {
        logger.info(`💻 [SystemsLoadBalancingTesterAgent] Analyzing Systems LoadBalancing Tester specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems LoadBalancing Tester Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsLoadBalancingTesterAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsLoadBalancingTesterAgent = Object.freeze(new SystemsLoadBalancingTesterAgent());
