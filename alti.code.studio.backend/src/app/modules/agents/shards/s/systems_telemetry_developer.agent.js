...
class SystemsTelemetryDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsTelemetrySystem(objective) {
    logger.info(
      `💻 [SystemsTelemetryDeveloperAgent] Analyzing Systems Telemetry Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Telemetry Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsTelemetryDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsTelemetryDeveloperAgent = Object.freeze(
  new SystemsTelemetryDeveloperAgent(),
);
