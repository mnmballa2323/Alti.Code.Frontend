...
class SystemsResilienceOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsResilienceSystem(objective) {
        logger.info(`💻 [SystemsResilienceOrchestratorAgent] Analyzing Systems Resilience Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Resilience Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsResilienceOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsResilienceOrchestratorAgent = Object.freeze(new SystemsResilienceOrchestratorAgent());
