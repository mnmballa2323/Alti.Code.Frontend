...
class SiteFaultToleranceAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteFaultToleranceSystem(objective) {
        logger.info(`💻 [SiteFaultToleranceAnalystAgent] Analyzing Site FaultTolerance Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site FaultTolerance Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteFaultToleranceAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteFaultToleranceAnalystAgent = Object.freeze(new SiteFaultToleranceAnalystAgent());
