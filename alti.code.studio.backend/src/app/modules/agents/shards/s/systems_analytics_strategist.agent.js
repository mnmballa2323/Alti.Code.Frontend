...
class SystemsAnalyticsStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsAnalyticsSystem(objective) {
    logger.info(
      `💻 [SystemsAnalyticsStrategistAgent] Analyzing Systems Analytics Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Analytics Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsAnalyticsStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsAnalyticsStrategistAgent = Object.freeze(
  new SystemsAnalyticsStrategistAgent(),
);
