...
class SystemsGovernanceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsGovernanceSystem(objective) {
    logger.info(
      `💻 [SystemsGovernanceDesignerAgent] Analyzing Systems Governance Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Governance Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsGovernanceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsGovernanceDesignerAgent = Object.freeze(
  new SystemsGovernanceDesignerAgent(),
);
