...
class SecurityDeploymentDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityDeploymentSystem(objective) {
    logger.info(
      `💻 [SecurityDeploymentDesignerAgent] Analyzing Security Deployment Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Deployment Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityDeploymentDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityDeploymentDesignerAgent = Object.freeze(
  new SecurityDeploymentDesignerAgent(),
);
