...
class SystemsComplianceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsComplianceSystem(objective) {
    logger.info(
      `💻 [SystemsComplianceSpecialistAgent] Analyzing Systems Compliance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Compliance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsComplianceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsComplianceSpecialistAgent = Object.freeze(
  new SystemsComplianceSpecialistAgent(),
);
