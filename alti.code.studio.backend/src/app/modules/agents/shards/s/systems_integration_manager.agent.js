...
class SystemsIntegrationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsIntegrationSystem(objective) {
    logger.info(
      `💻 [SystemsIntegrationManagerAgent] Analyzing Systems Integration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Integration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsIntegrationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsIntegrationManagerAgent = Object.freeze(
  new SystemsIntegrationManagerAgent(),
);
