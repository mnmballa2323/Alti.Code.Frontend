...
class SecurityProvisioningSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityProvisioningSystem(objective) {
    logger.info(
      `💻 [SecurityProvisioningSpecialistAgent] Analyzing Security Provisioning Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Provisioning Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityProvisioningSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityProvisioningSpecialistAgent = Object.freeze(
  new SecurityProvisioningSpecialistAgent(),
);
