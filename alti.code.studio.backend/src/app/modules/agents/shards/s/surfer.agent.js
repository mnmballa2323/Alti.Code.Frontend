...
export class SurferAgent {
    ...
    async surfWeb(searchContext) {
        if (!searchContext || typeof searchContext !== 'string' || searchContext.trim().length === 0) {
            throw new Error('SurferAgent: searchContext must be a non-empty string.');
        }

        logger.info(`🌊 Surfer Agent: Dispatching autonomous browser for search vector: [${searchContext.substring(0, 80)}]...`);
        ...
        try {
            ...
            const browserOutput = await browserUseService.executeBrowserTask(browserTask, true);
            ...
            try {
                ...
                logger.warn(`🌊 Surfer Agent: JSON parse failed — returning raw text as synthesizedSolution.`);
                surferReport = {
            ...
            logger.info(`🌊 Surfer Agent: Scraping complete. Sources checked: ${surferReport.sourcesChecked?.length ?? 0}`);
     ...
     */
    async process(state) {
        const query = state.data?.content || state.goal || 'React 19 Hooks documentation';
        ...
        try {
            ...
            return {
                ...
                status: 'failed',
                error: err.message,
                results: [...(state.results ?? []), `❌ FAILED SurferAgent: ${err.message}`],
            };
        }
    }
}

export const surferAgent = Object.freeze(new SurferAgent());
