...
class SiteVirtualizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteVirtualizationSystem(objective) {
    logger.info(
      `💻 [SiteVirtualizationDesignerAgent] Analyzing Site Virtualization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Virtualization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteVirtualizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteVirtualizationDesignerAgent = Object.freeze(
  new SiteVirtualizationDesignerAgent(),
);
