...
class SystemsServerlessDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsServerlessSystem(objective) {
        logger.info(`💻 [SystemsServerlessDirectorAgent] Analyzing Systems Serverless Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Serverless Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsServerlessDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsServerlessDirectorAgent = Object.freeze(new SystemsServerlessDirectorAgent());
