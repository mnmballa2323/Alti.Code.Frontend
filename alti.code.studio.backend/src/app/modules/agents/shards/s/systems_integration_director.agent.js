...
class SystemsIntegrationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsIntegrationSystem(objective) {
    logger.info(
      `💻 [SystemsIntegrationDirectorAgent] Analyzing Systems Integration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Integration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsIntegrationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsIntegrationDirectorAgent = Object.freeze(
  new SystemsIntegrationDirectorAgent(),
);
