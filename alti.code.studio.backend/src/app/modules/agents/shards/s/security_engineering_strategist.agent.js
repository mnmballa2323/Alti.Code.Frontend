...
class SecurityEngineeringStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityEngineeringSystem(objective) {
    logger.info(
      `💻 [SecurityEngineeringStrategistAgent] Analyzing Security Engineering Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Engineering Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityEngineeringStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityEngineeringStrategistAgent = Object.freeze(
  new SecurityEngineeringStrategistAgent(),
);
