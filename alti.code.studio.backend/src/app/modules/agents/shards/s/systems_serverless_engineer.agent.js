...
class SystemsServerlessEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsServerlessSystem(objective) {
    logger.info(
      `💻 [SystemsServerlessEngineerAgent] Analyzing Systems Serverless Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Serverless Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsServerlessEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsServerlessEngineerAgent = Object.freeze(
  new SystemsServerlessEngineerAgent(),
);
