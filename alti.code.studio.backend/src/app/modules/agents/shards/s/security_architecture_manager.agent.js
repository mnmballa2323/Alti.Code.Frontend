...
class SecurityArchitectureManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityArchitectureSystem(objective) {
    logger.info(
      `💻 [SecurityArchitectureManagerAgent] Analyzing Security Architecture Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Architecture Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityArchitectureManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityArchitectureManagerAgent = Object.freeze(
  new SecurityArchitectureManagerAgent(),
);
