...
class SecurityResilienceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityResilienceSystem(objective) {
    logger.info(
      `💻 [SecurityResilienceDesignerAgent] Analyzing Security Resilience Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Resilience Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityResilienceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityResilienceDesignerAgent = Object.freeze(
  new SecurityResilienceDesignerAgent(),
);
