...
class SiteOrchestrationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteOrchestrationSystem(objective) {
    logger.info(
      `💻 [SiteOrchestrationDirectorAgent] Analyzing Site Orchestration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Orchestration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteOrchestrationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteOrchestrationDirectorAgent = Object.freeze(
  new SiteOrchestrationDirectorAgent(),
);
