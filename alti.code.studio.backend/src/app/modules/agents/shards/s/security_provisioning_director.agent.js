...
class SecurityProvisioningDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityProvisioningSystem(objective) {
    logger.info(
      `💻 [SecurityProvisioningDirectorAgent] Analyzing Security Provisioning Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Provisioning Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityProvisioningDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityProvisioningDirectorAgent = Object.freeze(
  new SecurityProvisioningDirectorAgent(),
);
