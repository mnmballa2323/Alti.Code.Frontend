...
class SecurityMigrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityMigrationSystem(objective) {
    logger.info(
      `💻 [SecurityMigrationEngineerAgent] Analyzing Security Migration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Migration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityMigrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityMigrationEngineerAgent = Object.freeze(
  new SecurityMigrationEngineerAgent(),
);
