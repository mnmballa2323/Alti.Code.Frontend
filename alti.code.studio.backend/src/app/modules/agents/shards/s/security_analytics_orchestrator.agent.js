...
class SecurityAnalyticsOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityAnalyticsSystem(objective) {
    logger.info(
      `💻 [SecurityAnalyticsOrchestratorAgent] Analyzing Security Analytics Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Analytics Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityAnalyticsOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityAnalyticsOrchestratorAgent = Object.freeze(
  new SecurityAnalyticsOrchestratorAgent(),
);
