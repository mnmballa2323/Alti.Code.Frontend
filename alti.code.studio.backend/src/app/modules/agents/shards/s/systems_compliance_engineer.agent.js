...
class SystemsComplianceEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsComplianceSystem(objective) {
    logger.info(
      `💻 [SystemsComplianceEngineerAgent] Analyzing Systems Compliance Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Compliance Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsComplianceEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsComplianceEngineerAgent = Object.freeze(
  new SystemsComplianceEngineerAgent(),
);
