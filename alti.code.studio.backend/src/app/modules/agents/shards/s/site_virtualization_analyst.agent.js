...
class SiteVirtualizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteVirtualizationSystem(objective) {
    logger.info(
      `💻 [SiteVirtualizationAnalystAgent] Analyzing Site Virtualization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Virtualization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteVirtualizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteVirtualizationAnalystAgent = Object.freeze(
  new SiteVirtualizationAnalystAgent(),
);
