...
class SecurityResilienceDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityResilienceSystem(objective) {
        logger.info(`💻 [SecurityResilienceDeveloperAgent] Analyzing Security Resilience Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Resilience Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityResilienceDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityResilienceDeveloperAgent = Object.freeze(new SecurityResilienceDeveloperAgent());
