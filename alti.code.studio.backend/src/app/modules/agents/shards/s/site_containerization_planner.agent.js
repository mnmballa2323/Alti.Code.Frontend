...
class SiteContainerizationPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteContainerizationSystem(objective) {
        logger.info(`💻 [SiteContainerizationPlannerAgent] Analyzing Site Containerization Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Containerization Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteContainerizationPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteContainerizationPlannerAgent = Object.freeze(new SiteContainerizationPlannerAgent());
