...
class SecurityComplianceArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityComplianceSystem(objective) {
    logger.info(
      `💻 [SecurityComplianceArchitectAgent] Analyzing Security Compliance Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Compliance Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityComplianceArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityComplianceArchitectAgent = Object.freeze(
  new SecurityComplianceArchitectAgent(),
);
