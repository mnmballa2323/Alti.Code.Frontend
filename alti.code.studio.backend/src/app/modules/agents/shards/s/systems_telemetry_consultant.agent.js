...
class SystemsTelemetryConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsTelemetrySystem(objective) {
    logger.info(
      `💻 [SystemsTelemetryConsultantAgent] Analyzing Systems Telemetry Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Telemetry Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsTelemetryConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsTelemetryConsultantAgent = Object.freeze(
  new SystemsTelemetryConsultantAgent(),
);
