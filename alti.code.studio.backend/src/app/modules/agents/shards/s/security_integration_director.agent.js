...
class SecurityIntegrationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityIntegrationSystem(objective) {
    logger.info(
      `💻 [SecurityIntegrationDirectorAgent] Analyzing Security Integration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Integration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityIntegrationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityIntegrationDirectorAgent = Object.freeze(
  new SecurityIntegrationDirectorAgent(),
);
