...
class SyntheticBiologyArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateBioSystem(bioObjective) {
    logger.info(
      `🧫 [SyntheticBiologyArchitect] Analyzing objective for artificial genomes and DNA synthesis...`,
    );
...
${bioObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [SyntheticBiologyArchitect] Synthetic biology architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [SyntheticBiologyArchitect] Failed to generate biology system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const syntheticBiologyArchitectAgent = Object.freeze(
  new SyntheticBiologyArchitectAgent(),
);
