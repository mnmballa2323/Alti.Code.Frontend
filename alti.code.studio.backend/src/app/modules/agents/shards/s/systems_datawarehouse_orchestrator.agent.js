...
class SystemsDataWarehouseOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsDataWarehouseSystem(objective) {
        logger.info(`💻 [SystemsDataWarehouseOrchestratorAgent] Analyzing Systems DataWarehouse Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems DataWarehouse Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsDataWarehouseOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsDataWarehouseOrchestratorAgent = Object.freeze(new SystemsDataWarehouseOrchestratorAgent());
