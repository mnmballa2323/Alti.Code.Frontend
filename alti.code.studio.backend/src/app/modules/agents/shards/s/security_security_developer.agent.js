...
class SecuritySecurityDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSecuritySecuritySystem(objective) {
    logger.info(
      `💻 [SecuritySecurityDeveloperAgent] Analyzing Security Security Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Security Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecuritySecurityDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securitySecurityDeveloperAgent = Object.freeze(
  new SecuritySecurityDeveloperAgent(),
);
