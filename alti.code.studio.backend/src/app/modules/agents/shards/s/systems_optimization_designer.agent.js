...
class SystemsOptimizationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsOptimizationSystem(objective) {
        logger.info(`💻 [SystemsOptimizationDesignerAgent] Analyzing Systems Optimization Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Optimization Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsOptimizationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsOptimizationDesignerAgent = Object.freeze(new SystemsOptimizationDesignerAgent());
