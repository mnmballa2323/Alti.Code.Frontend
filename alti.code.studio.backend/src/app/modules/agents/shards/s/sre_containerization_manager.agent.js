...
class SREContainerizationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateSREContainerizationSystem(objective) {
        logger.info(`💻 [SREContainerizationManagerAgent] Analyzing SRE Containerization Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE Containerization Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SREContainerizationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const sREContainerizationManagerAgent = Object.freeze(new SREContainerizationManagerAgent());
