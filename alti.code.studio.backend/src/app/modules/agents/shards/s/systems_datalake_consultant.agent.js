...
class SystemsDataLakeConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsDataLakeSystem(objective) {
        logger.info(`💻 [SystemsDataLakeConsultantAgent] Analyzing Systems DataLake Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems DataLake Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsDataLakeConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsDataLakeConsultantAgent = Object.freeze(new SystemsDataLakeConsultantAgent());
