...
class SiteOrchestrationEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteOrchestrationSystem(objective) {
    logger.info(
      `💻 [SiteOrchestrationEngineerAgent] Analyzing Site Orchestration Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Orchestration Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteOrchestrationEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteOrchestrationEngineerAgent = Object.freeze(
  new SiteOrchestrationEngineerAgent(),
);
