...
class SystemsDataWarehouseDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [SystemsDataWarehouseDeveloperAgent] Analyzing Systems DataWarehouse Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems DataWarehouse Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsDataWarehouseDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsDataWarehouseDeveloperAgent = Object.freeze(
  new SystemsDataWarehouseDeveloperAgent(),
);
