...
class SecurityComplianceSpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityComplianceSystem(objective) {
        logger.info(`💻 [SecurityComplianceSpecialistAgent] Analyzing Security Compliance Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Compliance Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityComplianceSpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityComplianceSpecialistAgent = Object.freeze(new SecurityComplianceSpecialistAgent());
