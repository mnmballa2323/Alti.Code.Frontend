...
class SystemsProvisioningConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsProvisioningSystem(objective) {
        logger.info(`💻 [SystemsProvisioningConsultantAgent] Analyzing Systems Provisioning Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Provisioning Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsProvisioningConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsProvisioningConsultantAgent = Object.freeze(new SystemsProvisioningConsultantAgent());
