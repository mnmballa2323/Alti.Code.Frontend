...
class SiteVirtualizationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteVirtualizationSystem(objective) {
    logger.info(
      `💻 [SiteVirtualizationSpecialistAgent] Analyzing Site Virtualization Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Virtualization Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteVirtualizationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteVirtualizationSpecialistAgent = Object.freeze(
  new SiteVirtualizationSpecialistAgent(),
);
