...
class SREMicroservicesStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSREMicroservicesSystem(objective) {
    logger.info(
      `💻 [SREMicroservicesStrategistAgent] Analyzing SRE Microservices Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Microservices Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREMicroservicesStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREMicroservicesStrategistAgent = Object.freeze(
  new SREMicroservicesStrategistAgent(),
);
