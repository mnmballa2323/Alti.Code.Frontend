...
class SiteFaultToleranceOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteFaultToleranceSystem(objective) {
    logger.info(
      `💻 [SiteFaultToleranceOrchestratorAgent] Analyzing Site FaultTolerance Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site FaultTolerance Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteFaultToleranceOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteFaultToleranceOrchestratorAgent = Object.freeze(
  new SiteFaultToleranceOrchestratorAgent(),
);
