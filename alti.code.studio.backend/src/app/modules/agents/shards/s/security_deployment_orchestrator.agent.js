...
class SecurityDeploymentOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityDeploymentSystem(objective) {
    logger.info(
      `💻 [SecurityDeploymentOrchestratorAgent] Analyzing Security Deployment Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Deployment Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityDeploymentOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityDeploymentOrchestratorAgent = Object.freeze(
  new SecurityDeploymentOrchestratorAgent(),
);
