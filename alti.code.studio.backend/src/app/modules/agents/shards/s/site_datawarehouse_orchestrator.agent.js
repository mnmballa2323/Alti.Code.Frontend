...
class SiteDataWarehouseOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteDataWarehouseSystem(objective) {
    logger.info(
      `💻 [SiteDataWarehouseOrchestratorAgent] Analyzing Site DataWarehouse Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site DataWarehouse Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteDataWarehouseOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteDataWarehouseOrchestratorAgent = Object.freeze(
  new SiteDataWarehouseOrchestratorAgent(),
);
