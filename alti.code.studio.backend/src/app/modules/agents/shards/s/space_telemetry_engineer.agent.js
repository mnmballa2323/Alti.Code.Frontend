...
class SpaceTelemetryEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateSpaceSystem(aerospaceObjective) {
        logger.info(`🛰️ [SpaceTelemetryEngineer] Analyzing objective for orbital mechanics and satellite telemetry...`);
...
${aerospaceObjective}
        ...
        try {
            ...
            logger.info(`✅ [SpaceTelemetryEngineer] Aerospace architecture generated successfully.`);
            return cleanCode;
        } catch (err) {
            logger.error(`❌ [SpaceTelemetryEngineer] Failed to generate space system: ${err.message}`);
            throw err;
        }
    }
}

export const spaceTelemetryEngineerAgent = Object.freeze(new SpaceTelemetryEngineerAgent());
