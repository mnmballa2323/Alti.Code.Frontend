...
class SecurityConfigurationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityConfigurationSystem(objective) {
    logger.info(
      `💻 [SecurityConfigurationDesignerAgent] Analyzing Security Configuration Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Configuration Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityConfigurationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityConfigurationDesignerAgent = Object.freeze(
  new SecurityConfigurationDesignerAgent(),
);
