...
class SiteFaultToleranceStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteFaultToleranceSystem(objective) {
    logger.info(
      `💻 [SiteFaultToleranceStrategistAgent] Analyzing Site FaultTolerance Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site FaultTolerance Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteFaultToleranceStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteFaultToleranceStrategistAgent = Object.freeze(
  new SiteFaultToleranceStrategistAgent(),
);
