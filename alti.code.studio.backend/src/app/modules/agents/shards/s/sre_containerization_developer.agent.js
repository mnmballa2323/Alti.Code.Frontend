...
class SREContainerizationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSREContainerizationSystem(objective) {
    logger.info(
      `💻 [SREContainerizationDeveloperAgent] Analyzing SRE Containerization Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Containerization Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREContainerizationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREContainerizationDeveloperAgent = Object.freeze(
  new SREContainerizationDeveloperAgent(),
);
