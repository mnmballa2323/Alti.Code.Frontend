...
class SystemsProvisioningArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsProvisioningSystem(objective) {
    logger.info(
      `💻 [SystemsProvisioningArchitectAgent] Analyzing Systems Provisioning Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Provisioning Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsProvisioningArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsProvisioningArchitectAgent = Object.freeze(
  new SystemsProvisioningArchitectAgent(),
);
