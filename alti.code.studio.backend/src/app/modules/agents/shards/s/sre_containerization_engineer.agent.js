...
class SREContainerizationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateSREContainerizationSystem(objective) {
        logger.info(`💻 [SREContainerizationEngineerAgent] Analyzing SRE Containerization Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE Containerization Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SREContainerizationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const sREContainerizationEngineerAgent = Object.freeze(new SREContainerizationEngineerAgent());
