...
class SecurityAutomationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityAutomationSystem(objective) {
    logger.info(
      `💻 [SecurityAutomationAuditorAgent] Analyzing Security Automation Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Automation Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityAutomationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityAutomationAuditorAgent = Object.freeze(
  new SecurityAutomationAuditorAgent(),
);
