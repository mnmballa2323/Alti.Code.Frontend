...
class SystemsVirtualizationStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsVirtualizationSystem(objective) {
        logger.info(`💻 [SystemsVirtualizationStrategistAgent] Analyzing Systems Virtualization Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Virtualization Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsVirtualizationStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsVirtualizationStrategistAgent = Object.freeze(new SystemsVirtualizationStrategistAgent());
