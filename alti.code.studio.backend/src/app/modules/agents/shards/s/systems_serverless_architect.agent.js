...
class SystemsServerlessArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsServerlessSystem(objective) {
    logger.info(
      `💻 [SystemsServerlessArchitectAgent] Analyzing Systems Serverless Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Serverless Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsServerlessArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsServerlessArchitectAgent = Object.freeze(
  new SystemsServerlessArchitectAgent(),
);
