...
class SystemsVirtualizationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsVirtualizationSystem(objective) {
    logger.info(
      `💻 [SystemsVirtualizationSpecialistAgent] Analyzing Systems Virtualization Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Virtualization Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsVirtualizationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsVirtualizationSpecialistAgent = Object.freeze(
  new SystemsVirtualizationSpecialistAgent(),
);
