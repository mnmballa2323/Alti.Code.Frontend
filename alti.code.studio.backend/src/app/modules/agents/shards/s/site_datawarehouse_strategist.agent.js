...
class SiteDataWarehouseStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteDataWarehouseSystem(objective) {
    logger.info(
      `💻 [SiteDataWarehouseStrategistAgent] Analyzing Site DataWarehouse Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site DataWarehouse Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteDataWarehouseStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteDataWarehouseStrategistAgent = Object.freeze(
  new SiteDataWarehouseStrategistAgent(),
);
