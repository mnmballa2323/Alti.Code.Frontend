...
class SystemsOrchestrationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsOrchestrationSystem(objective) {
    logger.info(
      `💻 [SystemsOrchestrationTesterAgent] Analyzing Systems Orchestration Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Orchestration Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsOrchestrationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsOrchestrationTesterAgent = Object.freeze(
  new SystemsOrchestrationTesterAgent(),
);
