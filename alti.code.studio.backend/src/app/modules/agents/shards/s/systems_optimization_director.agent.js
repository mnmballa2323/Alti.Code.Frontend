...
class SystemsOptimizationDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsOptimizationSystem(objective) {
        logger.info(`💻 [SystemsOptimizationDirectorAgent] Analyzing Systems Optimization Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Optimization Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsOptimizationDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsOptimizationDirectorAgent = Object.freeze(new SystemsOptimizationDirectorAgent());
