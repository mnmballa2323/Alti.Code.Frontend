...
class SiteConfigurationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteConfigurationSystem(objective) {
        logger.info(`💻 [SiteConfigurationOrchestratorAgent] Analyzing Site Configuration Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Configuration Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteConfigurationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteConfigurationOrchestratorAgent = Object.freeze(new SiteConfigurationOrchestratorAgent());
