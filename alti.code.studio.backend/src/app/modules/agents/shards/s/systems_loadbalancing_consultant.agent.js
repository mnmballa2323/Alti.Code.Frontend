...
class SystemsLoadBalancingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsLoadBalancingSystem(objective) {
    logger.info(
      `💻 [SystemsLoadBalancingConsultantAgent] Analyzing Systems LoadBalancing Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems LoadBalancing Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsLoadBalancingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsLoadBalancingConsultantAgent = Object.freeze(
  new SystemsLoadBalancingConsultantAgent(),
);
