...
class SiteArchitectureConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteArchitectureSystem(objective) {
    logger.info(
      `💻 [SiteArchitectureConsultantAgent] Analyzing Site Architecture Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Architecture Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteArchitectureConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteArchitectureConsultantAgent = Object.freeze(
  new SiteArchitectureConsultantAgent(),
);
