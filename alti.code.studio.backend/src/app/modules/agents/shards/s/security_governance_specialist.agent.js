...
class SecurityGovernanceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityGovernanceSystem(objective) {
    logger.info(
      `💻 [SecurityGovernanceSpecialistAgent] Analyzing Security Governance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Governance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityGovernanceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityGovernanceSpecialistAgent = Object.freeze(
  new SecurityGovernanceSpecialistAgent(),
);
