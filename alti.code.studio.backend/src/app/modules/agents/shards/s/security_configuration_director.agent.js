...
class SecurityConfigurationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityConfigurationSystem(objective) {
    logger.info(
      `💻 [SecurityConfigurationDirectorAgent] Analyzing Security Configuration Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Configuration Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityConfigurationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityConfigurationDirectorAgent = Object.freeze(
  new SecurityConfigurationDirectorAgent(),
);
