...
class SportsAnalyticsQuantAgent extends GeminiCliBaseAgent {
    ...
    async generateSportsSystem(sportsObjective) {
        logger.info(`⚾ [SportsAnalyticsQuant] Analyzing objective for player tracking and draft analytics...`);
...
${sportsObjective}
        ...
        try {
            ...
            logger.info(`✅ [SportsAnalyticsQuant] Sports architecture generated successfully.`);
            return cleanCode;
        } catch (err) {
            logger.error(`❌ [SportsAnalyticsQuant] Failed to generate sports system: ${err.message}`);
            throw err;
        }
    }
}

export const sportsAnalyticsQuantAgent = Object.freeze(new SportsAnalyticsQuantAgent());
