...
class SecuritySecurityStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecuritySecuritySystem(objective) {
    logger.info(
      `💻 [SecuritySecurityStrategistAgent] Analyzing Security Security Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Security Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecuritySecurityStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securitySecurityStrategistAgent = Object.freeze(
  new SecuritySecurityStrategistAgent(),
);
