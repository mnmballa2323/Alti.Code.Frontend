...
class SREResilienceOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSREResilienceSystem(objective) {
    logger.info(
      `💻 [SREResilienceOrchestratorAgent] Analyzing SRE Resilience Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Resilience Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREResilienceOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREResilienceOrchestratorAgent = Object.freeze(
  new SREResilienceOrchestratorAgent(),
);
