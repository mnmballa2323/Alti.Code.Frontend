...
class SecurityGovernanceAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityGovernanceSystem(objective) {
    logger.info(
      `💻 [SecurityGovernanceAuditorAgent] Analyzing Security Governance Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Governance Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityGovernanceAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityGovernanceAuditorAgent = Object.freeze(
  new SecurityGovernanceAuditorAgent(),
);
