...
class SecuritySecurityManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecuritySecuritySystem(objective) {
    logger.info(
      `💻 [SecuritySecurityManagerAgent] Analyzing Security Security Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Security Manager Target',
      );
...
export const securitySecurityManagerAgent = Object.freeze(
  new SecuritySecurityManagerAgent(),
);
