...
class SecurityOptimizationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityOptimizationSystem(objective) {
    logger.info(
      `💻 [SecurityOptimizationManagerAgent] Analyzing Security Optimization Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Optimization Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityOptimizationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityOptimizationManagerAgent = Object.freeze(
  new SecurityOptimizationManagerAgent(),
);
