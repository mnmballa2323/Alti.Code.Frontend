...
class SREOrchestrationConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSREOrchestrationSystem(objective) {
        logger.info(`💻 [SREOrchestrationConsultantAgent] Analyzing SRE Orchestration Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE Orchestration Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SREOrchestrationConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const sREOrchestrationConsultantAgent = Object.freeze(new SREOrchestrationConsultantAgent());
