...
class SystemsDeploymentDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsDeploymentSystem(objective) {
    logger.info(
      `💻 [SystemsDeploymentDeveloperAgent] Analyzing Systems Deployment Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Deployment Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsDeploymentDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsDeploymentDeveloperAgent = Object.freeze(
  new SystemsDeploymentDeveloperAgent(),
);
