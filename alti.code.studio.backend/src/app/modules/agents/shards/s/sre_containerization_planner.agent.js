...
class SREContainerizationPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSREContainerizationSystem(objective) {
    logger.info(
      `💻 [SREContainerizationPlannerAgent] Analyzing SRE Containerization Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Containerization Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREContainerizationPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREContainerizationPlannerAgent = Object.freeze(
  new SREContainerizationPlannerAgent(),
);
