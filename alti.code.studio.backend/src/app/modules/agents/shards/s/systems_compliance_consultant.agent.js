...
class SystemsComplianceConsultantAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsComplianceSystem(objective) {
        logger.info(`💻 [SystemsComplianceConsultantAgent] Analyzing Systems Compliance Consultant specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Compliance Consultant Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsComplianceConsultantAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsComplianceConsultantAgent = Object.freeze(new SystemsComplianceConsultantAgent());
