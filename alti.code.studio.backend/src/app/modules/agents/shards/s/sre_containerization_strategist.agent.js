...
class SREContainerizationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSREContainerizationSystem(objective) {
    logger.info(
      `💻 [SREContainerizationStrategistAgent] Analyzing SRE Containerization Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Containerization Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREContainerizationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREContainerizationStrategistAgent = Object.freeze(
  new SREContainerizationStrategistAgent(),
);
