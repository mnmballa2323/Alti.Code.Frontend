...
class SREFaultToleranceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSREFaultToleranceSystem(objective) {
    logger.info(
      `💻 [SREFaultToleranceConsultantAgent] Analyzing SRE FaultTolerance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE FaultTolerance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREFaultToleranceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREFaultToleranceConsultantAgent = Object.freeze(
  new SREFaultToleranceConsultantAgent(),
);
