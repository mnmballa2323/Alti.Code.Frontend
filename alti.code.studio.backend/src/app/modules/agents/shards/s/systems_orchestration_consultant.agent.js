...
class SystemsOrchestrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsOrchestrationSystem(objective) {
    logger.info(
      `💻 [SystemsOrchestrationConsultantAgent] Analyzing Systems Orchestration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Orchestration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsOrchestrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsOrchestrationConsultantAgent = Object.freeze(
  new SystemsOrchestrationConsultantAgent(),
);
