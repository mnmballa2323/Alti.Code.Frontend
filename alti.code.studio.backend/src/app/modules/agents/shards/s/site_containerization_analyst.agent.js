...
class SiteContainerizationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteContainerizationSystem(objective) {
        logger.info(`💻 [SiteContainerizationAnalystAgent] Analyzing Site Containerization Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Containerization Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteContainerizationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteContainerizationAnalystAgent = Object.freeze(new SiteContainerizationAnalystAgent());
