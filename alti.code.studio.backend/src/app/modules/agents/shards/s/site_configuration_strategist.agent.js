...
class SiteConfigurationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteConfigurationSystem(objective) {
    logger.info(
      `💻 [SiteConfigurationStrategistAgent] Analyzing Site Configuration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Configuration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteConfigurationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteConfigurationStrategistAgent = Object.freeze(
  new SiteConfigurationStrategistAgent(),
);
