...
class SREObservabilityStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSREObservabilitySystem(objective) {
    logger.info(
      `💻 [SREObservabilityStrategistAgent] Analyzing SRE Observability Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Observability Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREObservabilityStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREObservabilityStrategistAgent = Object.freeze(
  new SREObservabilityStrategistAgent(),
);
