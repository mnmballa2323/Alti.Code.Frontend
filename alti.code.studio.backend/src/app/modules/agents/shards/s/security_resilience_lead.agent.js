...
class SecurityResilienceLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityResilienceSystem(objective) {
    logger.info(
      `💻 [SecurityResilienceLeadAgent] Analyzing Security Resilience Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Resilience Lead Target',
      );
...
export const securityResilienceLeadAgent = Object.freeze(
  new SecurityResilienceLeadAgent(),
);
