...
class SecuritySecurityArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSecuritySecuritySystem(objective) {
    logger.info(
      `💻 [SecuritySecurityArchitectAgent] Analyzing Security Security Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Security Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecuritySecurityArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securitySecurityArchitectAgent = Object.freeze(
  new SecuritySecurityArchitectAgent(),
);
