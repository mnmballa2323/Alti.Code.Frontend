...
class SREBatchProcessingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSREBatchProcessingSystem(objective) {
    logger.info(
      `💻 [SREBatchProcessingPlannerAgent] Analyzing SRE BatchProcessing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE BatchProcessing Planner Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREBatchProcessingPlannerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREBatchProcessingPlannerAgent = Object.freeze(
  new SREBatchProcessingPlannerAgent(),
);
