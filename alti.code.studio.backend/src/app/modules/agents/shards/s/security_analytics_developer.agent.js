...
class SecurityAnalyticsDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityAnalyticsSystem(objective) {
    logger.info(
      `💻 [SecurityAnalyticsDeveloperAgent] Analyzing Security Analytics Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Analytics Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityAnalyticsDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityAnalyticsDeveloperAgent = Object.freeze(
  new SecurityAnalyticsDeveloperAgent(),
);
