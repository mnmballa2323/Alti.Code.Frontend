...
class SecurityIntegrationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityIntegrationSystem(objective) {
    logger.info(
      `💻 [SecurityIntegrationAuditorAgent] Analyzing Security Integration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Integration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityIntegrationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityIntegrationAuditorAgent = Object.freeze(
  new SecurityIntegrationAuditorAgent(),
);
