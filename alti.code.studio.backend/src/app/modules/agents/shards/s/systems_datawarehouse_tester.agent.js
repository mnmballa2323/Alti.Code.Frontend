...
class SystemsDataWarehouseTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [SystemsDataWarehouseTesterAgent] Analyzing Systems DataWarehouse Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems DataWarehouse Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsDataWarehouseTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsDataWarehouseTesterAgent = Object.freeze(
  new SystemsDataWarehouseTesterAgent(),
);
