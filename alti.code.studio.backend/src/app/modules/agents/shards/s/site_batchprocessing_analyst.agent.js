...
class SiteBatchProcessingAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteBatchProcessingSystem(objective) {
        logger.info(`💻 [SiteBatchProcessingAnalystAgent] Analyzing Site BatchProcessing Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site BatchProcessing Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteBatchProcessingAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteBatchProcessingAnalystAgent = Object.freeze(new SiteBatchProcessingAnalystAgent());
