...
class SiteOptimizationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteOptimizationSystem(objective) {
    logger.info(
      `💻 [SiteOptimizationArchitectAgent] Analyzing Site Optimization Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Optimization Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteOptimizationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteOptimizationArchitectAgent = Object.freeze(
  new SiteOptimizationArchitectAgent(),
);
