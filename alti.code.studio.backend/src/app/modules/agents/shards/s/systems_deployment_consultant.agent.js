...
class SystemsDeploymentConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsDeploymentSystem(objective) {
    logger.info(
      `💻 [SystemsDeploymentConsultantAgent] Analyzing Systems Deployment Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Deployment Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsDeploymentConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsDeploymentConsultantAgent = Object.freeze(
  new SystemsDeploymentConsultantAgent(),
);
