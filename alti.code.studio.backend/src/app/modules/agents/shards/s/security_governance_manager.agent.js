...
class SecurityGovernanceManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityGovernanceSystem(objective) {
        logger.info(`💻 [SecurityGovernanceManagerAgent] Analyzing Security Governance Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Governance Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityGovernanceManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityGovernanceManagerAgent = Object.freeze(new SecurityGovernanceManagerAgent());
