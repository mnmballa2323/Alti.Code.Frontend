...
class SystemsTelemetryStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsTelemetrySystem(objective) {
    logger.info(
      `💻 [SystemsTelemetryStrategistAgent] Analyzing Systems Telemetry Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Telemetry Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsTelemetryStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsTelemetryStrategistAgent = Object.freeze(
  new SystemsTelemetryStrategistAgent(),
);
