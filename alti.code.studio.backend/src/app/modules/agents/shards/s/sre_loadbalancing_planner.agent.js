...
class SRELoadBalancingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSRELoadBalancingSystem(objective) {
    logger.info(
      `💻 [SRELoadBalancingPlannerAgent] Analyzing SRE LoadBalancing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE LoadBalancing Planner Target',
      );
...
export const sRELoadBalancingPlannerAgent = Object.freeze(
  new SRELoadBalancingPlannerAgent(),
);
