...
class SecurityResilienceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityResilienceSystem(objective) {
    logger.info(
      `💻 [SecurityResilienceSpecialistAgent] Analyzing Security Resilience Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Resilience Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityResilienceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityResilienceSpecialistAgent = Object.freeze(
  new SecurityResilienceSpecialistAgent(),
);
