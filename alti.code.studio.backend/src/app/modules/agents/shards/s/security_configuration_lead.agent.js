...
class SecurityConfigurationLeadAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityConfigurationSystem(objective) {
        logger.info(`💻 [SecurityConfigurationLeadAgent] Analyzing Security Configuration Lead specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Configuration Lead Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityConfigurationLeadAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityConfigurationLeadAgent = Object.freeze(new SecurityConfigurationLeadAgent());
