...
class SiteProvisioningStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteProvisioningSystem(objective) {
    logger.info(
      `💻 [SiteProvisioningStrategistAgent] Analyzing Site Provisioning Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Provisioning Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteProvisioningStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteProvisioningStrategistAgent = Object.freeze(
  new SiteProvisioningStrategistAgent(),
);
