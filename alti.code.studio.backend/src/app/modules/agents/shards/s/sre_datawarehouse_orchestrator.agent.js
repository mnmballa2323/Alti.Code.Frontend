...
class SREDataWarehouseOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSREDataWarehouseSystem(objective) {
    logger.info(
      `💻 [SREDataWarehouseOrchestratorAgent] Analyzing SRE DataWarehouse Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE DataWarehouse Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREDataWarehouseOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREDataWarehouseOrchestratorAgent = Object.freeze(
  new SREDataWarehouseOrchestratorAgent(),
);
