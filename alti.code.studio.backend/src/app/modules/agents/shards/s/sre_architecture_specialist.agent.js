...
class SREArchitectureSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSREArchitectureSystem(objective) {
    logger.info(
      `💻 [SREArchitectureSpecialistAgent] Analyzing SRE Architecture Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Architecture Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREArchitectureSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREArchitectureSpecialistAgent = Object.freeze(
  new SREArchitectureSpecialistAgent(),
);
