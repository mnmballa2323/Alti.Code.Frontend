...
class SecurityConfigurationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityConfigurationSystem(objective) {
    logger.info(
      `💻 [SecurityConfigurationOrchestratorAgent] Analyzing Security Configuration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Configuration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityConfigurationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityConfigurationOrchestratorAgent = Object.freeze(
  new SecurityConfigurationOrchestratorAgent(),
);
