...
class SystemsMigrationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsMigrationSystem(objective) {
    logger.info(
      `💻 [SystemsMigrationDeveloperAgent] Analyzing Systems Migration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Migration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsMigrationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsMigrationDeveloperAgent = Object.freeze(
  new SystemsMigrationDeveloperAgent(),
);
