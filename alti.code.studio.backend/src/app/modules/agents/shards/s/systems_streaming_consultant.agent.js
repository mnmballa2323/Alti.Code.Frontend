...
class SystemsStreamingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsStreamingSystem(objective) {
    logger.info(
      `💻 [SystemsStreamingConsultantAgent] Analyzing Systems Streaming Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Streaming Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsStreamingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsStreamingConsultantAgent = Object.freeze(
  new SystemsStreamingConsultantAgent(),
);
