...
class SiteMicroservicesArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteMicroservicesSystem(objective) {
    logger.info(
      `💻 [SiteMicroservicesArchitectAgent] Analyzing Site Microservices Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Microservices Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteMicroservicesArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteMicroservicesArchitectAgent = Object.freeze(
  new SiteMicroservicesArchitectAgent(),
);
