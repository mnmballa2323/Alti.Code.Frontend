...
class SystemsMicroservicesOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsMicroservicesSystem(objective) {
    logger.info(
      `💻 [SystemsMicroservicesOrchestratorAgent] Analyzing Systems Microservices Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Microservices Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsMicroservicesOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsMicroservicesOrchestratorAgent = Object.freeze(
  new SystemsMicroservicesOrchestratorAgent(),
);
