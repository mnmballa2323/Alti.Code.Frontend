...
class SystemsTelemetryArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsTelemetrySystem(objective) {
    logger.info(
      `💻 [SystemsTelemetryArchitectAgent] Analyzing Systems Telemetry Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Telemetry Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsTelemetryArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsTelemetryArchitectAgent = Object.freeze(
  new SystemsTelemetryArchitectAgent(),
);
