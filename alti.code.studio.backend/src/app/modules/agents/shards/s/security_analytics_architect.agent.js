...
class SecurityAnalyticsArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityAnalyticsSystem(objective) {
    logger.info(
      `💻 [SecurityAnalyticsArchitectAgent] Analyzing Security Analytics Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Analytics Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityAnalyticsArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityAnalyticsArchitectAgent = Object.freeze(
  new SecurityAnalyticsArchitectAgent(),
);
