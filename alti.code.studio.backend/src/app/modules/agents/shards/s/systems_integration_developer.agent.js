...
class SystemsIntegrationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsIntegrationSystem(objective) {
        logger.info(`💻 [SystemsIntegrationDeveloperAgent] Analyzing Systems Integration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Integration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsIntegrationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsIntegrationDeveloperAgent = Object.freeze(new SystemsIntegrationDeveloperAgent());
