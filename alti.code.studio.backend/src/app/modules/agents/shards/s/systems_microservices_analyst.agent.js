...
class SystemsMicroservicesAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsMicroservicesSystem(objective) {
        logger.info(`💻 [SystemsMicroservicesAnalystAgent] Analyzing Systems Microservices Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Microservices Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsMicroservicesAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsMicroservicesAnalystAgent = Object.freeze(new SystemsMicroservicesAnalystAgent());
