...
class SystemsDataLakeSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsDataLakeSystem(objective) {
    logger.info(
      `💻 [SystemsDataLakeSpecialistAgent] Analyzing Systems DataLake Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems DataLake Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsDataLakeSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsDataLakeSpecialistAgent = Object.freeze(
  new SystemsDataLakeSpecialistAgent(),
);
