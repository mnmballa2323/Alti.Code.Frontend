...
class SecurityVirtualizationAuditorAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityVirtualizationSystem(objective) {
        logger.info(`💻 [SecurityVirtualizationAuditorAgent] Analyzing Security Virtualization Auditor specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Virtualization Auditor Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityVirtualizationAuditorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityVirtualizationAuditorAgent = Object.freeze(new SecurityVirtualizationAuditorAgent());
