...
class SystemsArchitectureDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsArchitectureSystem(objective) {
    logger.info(
      `💻 [SystemsArchitectureDirectorAgent] Analyzing Systems Architecture Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Architecture Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsArchitectureDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsArchitectureDirectorAgent = Object.freeze(
  new SystemsArchitectureDirectorAgent(),
);
