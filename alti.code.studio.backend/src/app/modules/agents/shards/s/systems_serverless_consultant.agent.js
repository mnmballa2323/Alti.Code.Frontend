...
class SystemsServerlessConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsServerlessSystem(objective) {
    logger.info(
      `💻 [SystemsServerlessConsultantAgent] Analyzing Systems Serverless Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Serverless Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsServerlessConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsServerlessConsultantAgent = Object.freeze(
  new SystemsServerlessConsultantAgent(),
);
