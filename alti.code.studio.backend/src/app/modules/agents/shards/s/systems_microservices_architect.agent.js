...
class SystemsMicroservicesArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsMicroservicesSystem(objective) {
    logger.info(
      `💻 [SystemsMicroservicesArchitectAgent] Analyzing Systems Microservices Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Microservices Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsMicroservicesArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsMicroservicesArchitectAgent = Object.freeze(
  new SystemsMicroservicesArchitectAgent(),
);
