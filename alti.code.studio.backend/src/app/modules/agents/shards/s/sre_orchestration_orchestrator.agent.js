...
class SREOrchestrationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSREOrchestrationSystem(objective) {
        logger.info(`💻 [SREOrchestrationOrchestratorAgent] Analyzing SRE Orchestration Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE Orchestration Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SREOrchestrationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const sREOrchestrationOrchestratorAgent = Object.freeze(new SREOrchestrationOrchestratorAgent());
