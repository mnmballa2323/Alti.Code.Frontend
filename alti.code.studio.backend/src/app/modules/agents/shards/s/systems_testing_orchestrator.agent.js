...
class SystemsTestingOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsTestingSystem(objective) {
        logger.info(`💻 [SystemsTestingOrchestratorAgent] Analyzing Systems Testing Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Testing Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsTestingOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsTestingOrchestratorAgent = Object.freeze(new SystemsTestingOrchestratorAgent());
