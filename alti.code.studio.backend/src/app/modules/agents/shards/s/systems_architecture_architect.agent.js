...
class SystemsArchitectureArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsArchitectureSystem(objective) {
        logger.info(`💻 [SystemsArchitectureArchitectAgent] Analyzing Systems Architecture Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Architecture Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsArchitectureArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsArchitectureArchitectAgent = Object.freeze(new SystemsArchitectureArchitectAgent());
