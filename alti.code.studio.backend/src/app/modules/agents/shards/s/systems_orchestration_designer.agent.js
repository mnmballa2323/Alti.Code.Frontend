...
class SystemsOrchestrationDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsOrchestrationSystem(objective) {
        logger.info(`💻 [SystemsOrchestrationDesignerAgent] Analyzing Systems Orchestration Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Orchestration Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsOrchestrationDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsOrchestrationDesignerAgent = Object.freeze(new SystemsOrchestrationDesignerAgent());
