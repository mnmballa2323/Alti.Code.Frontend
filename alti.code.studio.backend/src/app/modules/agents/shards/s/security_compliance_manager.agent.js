...
class SecurityComplianceManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityComplianceSystem(objective) {
    logger.info(
      `💻 [SecurityComplianceManagerAgent] Analyzing Security Compliance Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Compliance Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityComplianceManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityComplianceManagerAgent = Object.freeze(
  new SecurityComplianceManagerAgent(),
);
