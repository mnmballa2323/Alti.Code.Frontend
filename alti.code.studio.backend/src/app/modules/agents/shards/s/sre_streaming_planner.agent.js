...
class SREStreamingPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateSREStreamingSystem(objective) {
        logger.info(`💻 [SREStreamingPlannerAgent] Analyzing SRE Streaming Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - SRE Streaming Planner Target");
...
export const sREStreamingPlannerAgent = Object.freeze(new SREStreamingPlannerAgent());
