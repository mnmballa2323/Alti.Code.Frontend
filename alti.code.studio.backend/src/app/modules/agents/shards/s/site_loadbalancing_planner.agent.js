...
class SiteLoadBalancingPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteLoadBalancingSystem(objective) {
    logger.info(
      `💻 [SiteLoadBalancingPlannerAgent] Analyzing Site LoadBalancing Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site LoadBalancing Planner Target',
      );
...
export const siteLoadBalancingPlannerAgent = Object.freeze(
  new SiteLoadBalancingPlannerAgent(),
);
