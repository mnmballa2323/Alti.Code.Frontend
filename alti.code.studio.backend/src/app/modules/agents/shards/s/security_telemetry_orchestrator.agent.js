...
class SecurityTelemetryOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityTelemetrySystem(objective) {
    logger.info(
      `💻 [SecurityTelemetryOrchestratorAgent] Analyzing Security Telemetry Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Telemetry Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityTelemetryOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityTelemetryOrchestratorAgent = Object.freeze(
  new SecurityTelemetryOrchestratorAgent(),
);
