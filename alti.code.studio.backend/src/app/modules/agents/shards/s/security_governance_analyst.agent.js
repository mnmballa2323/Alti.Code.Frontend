...
class SecurityGovernanceAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityGovernanceSystem(objective) {
        logger.info(`💻 [SecurityGovernanceAnalystAgent] Analyzing Security Governance Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Governance Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityGovernanceAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityGovernanceAnalystAgent = Object.freeze(new SecurityGovernanceAnalystAgent());
