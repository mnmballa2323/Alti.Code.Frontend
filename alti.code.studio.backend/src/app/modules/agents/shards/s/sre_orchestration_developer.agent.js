...
class SREOrchestrationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSREOrchestrationSystem(objective) {
    logger.info(
      `💻 [SREOrchestrationDeveloperAgent] Analyzing SRE Orchestration Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Orchestration Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREOrchestrationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREOrchestrationDeveloperAgent = Object.freeze(
  new SREOrchestrationDeveloperAgent(),
);
