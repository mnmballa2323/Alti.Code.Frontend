...
class SecurityIntegrationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityIntegrationSystem(objective) {
        logger.info(`💻 [SecurityIntegrationDeveloperAgent] Analyzing Security Integration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Integration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityIntegrationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityIntegrationDeveloperAgent = Object.freeze(new SecurityIntegrationDeveloperAgent());
