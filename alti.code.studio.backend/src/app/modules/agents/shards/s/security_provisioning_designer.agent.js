...
class SecurityProvisioningDesignerAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityProvisioningSystem(objective) {
        logger.info(`💻 [SecurityProvisioningDesignerAgent] Analyzing Security Provisioning Designer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Provisioning Designer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityProvisioningDesignerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityProvisioningDesignerAgent = Object.freeze(new SecurityProvisioningDesignerAgent());
