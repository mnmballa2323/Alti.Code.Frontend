...
class SiteFaultToleranceEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteFaultToleranceSystem(objective) {
        logger.info(`💻 [SiteFaultToleranceEngineerAgent] Analyzing Site FaultTolerance Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site FaultTolerance Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteFaultToleranceEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteFaultToleranceEngineerAgent = Object.freeze(new SiteFaultToleranceEngineerAgent());
