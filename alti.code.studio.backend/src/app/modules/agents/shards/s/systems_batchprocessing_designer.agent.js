...
class SystemsBatchProcessingDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsBatchProcessingSystem(objective) {
    logger.info(
      `💻 [SystemsBatchProcessingDesignerAgent] Analyzing Systems BatchProcessing Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems BatchProcessing Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsBatchProcessingDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsBatchProcessingDesignerAgent = Object.freeze(
  new SystemsBatchProcessingDesignerAgent(),
);
