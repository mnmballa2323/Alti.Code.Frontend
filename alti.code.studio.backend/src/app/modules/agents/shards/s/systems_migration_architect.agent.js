...
class SystemsMigrationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsMigrationSystem(objective) {
    logger.info(
      `💻 [SystemsMigrationArchitectAgent] Analyzing Systems Migration Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Migration Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsMigrationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsMigrationArchitectAgent = Object.freeze(
  new SystemsMigrationArchitectAgent(),
);
