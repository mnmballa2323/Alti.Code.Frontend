...
class SecurityTestingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityTestingSystem(objective) {
    logger.info(
      `💻 [SecurityTestingOrchestratorAgent] Analyzing Security Testing Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Testing Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityTestingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityTestingOrchestratorAgent = Object.freeze(
  new SecurityTestingOrchestratorAgent(),
);
