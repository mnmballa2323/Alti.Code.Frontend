...
class SystemsDataWarehouseEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [SystemsDataWarehouseEngineerAgent] Analyzing Systems DataWarehouse Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems DataWarehouse Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsDataWarehouseEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsDataWarehouseEngineerAgent = Object.freeze(
  new SystemsDataWarehouseEngineerAgent(),
);
