...
class SecurityMigrationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityMigrationSystem(objective) {
    logger.info(
      `💻 [SecurityMigrationOrchestratorAgent] Analyzing Security Migration Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Migration Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityMigrationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityMigrationOrchestratorAgent = Object.freeze(
  new SecurityMigrationOrchestratorAgent(),
);
