...
class SREProvisioningConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateSREProvisioningSystem(objective) {
    logger.info(
      `💻 [SREProvisioningConsultantAgent] Analyzing SRE Provisioning Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Provisioning Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREProvisioningConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREProvisioningConsultantAgent = Object.freeze(
  new SREProvisioningConsultantAgent(),
);
