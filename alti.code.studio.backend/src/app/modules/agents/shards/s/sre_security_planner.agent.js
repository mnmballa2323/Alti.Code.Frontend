...
class SRESecurityPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSRESecuritySystem(objective) {
    logger.info(
      `💻 [SRESecurityPlannerAgent] Analyzing SRE Security Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Security Planner Target',
      );
...
export const sRESecurityPlannerAgent = Object.freeze(
  new SRESecurityPlannerAgent(),
);
