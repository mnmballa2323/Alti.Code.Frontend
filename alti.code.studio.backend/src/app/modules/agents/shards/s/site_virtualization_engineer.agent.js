...
class SiteVirtualizationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteVirtualizationSystem(objective) {
        logger.info(`💻 [SiteVirtualizationEngineerAgent] Analyzing Site Virtualization Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Virtualization Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteVirtualizationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteVirtualizationEngineerAgent = Object.freeze(new SiteVirtualizationEngineerAgent());
