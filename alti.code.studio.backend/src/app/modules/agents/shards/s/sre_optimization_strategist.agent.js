...
class SREOptimizationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateSREOptimizationSystem(objective) {
    logger.info(
      `💻 [SREOptimizationStrategistAgent] Analyzing SRE Optimization Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Optimization Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREOptimizationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREOptimizationStrategistAgent = Object.freeze(
  new SREOptimizationStrategistAgent(),
);
