...
class SecurityGovernanceDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityGovernanceSystem(objective) {
    logger.info(
      `💻 [SecurityGovernanceDirectorAgent] Analyzing Security Governance Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Governance Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityGovernanceDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityGovernanceDirectorAgent = Object.freeze(
  new SecurityGovernanceDirectorAgent(),
);
