...
class SecurityConfigurationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityConfigurationSystem(objective) {
    logger.info(
      `💻 [SecurityConfigurationManagerAgent] Analyzing Security Configuration Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Configuration Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityConfigurationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityConfigurationManagerAgent = Object.freeze(
  new SecurityConfigurationManagerAgent(),
);
