...
class SecurityProvisioningArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityProvisioningSystem(objective) {
    logger.info(
      `💻 [SecurityProvisioningArchitectAgent] Analyzing Security Provisioning Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Provisioning Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityProvisioningArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityProvisioningArchitectAgent = Object.freeze(
  new SecurityProvisioningArchitectAgent(),
);
