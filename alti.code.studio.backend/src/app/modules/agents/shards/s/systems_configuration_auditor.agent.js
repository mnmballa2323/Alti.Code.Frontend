...
class SystemsConfigurationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsConfigurationSystem(objective) {
    logger.info(
      `💻 [SystemsConfigurationAuditorAgent] Analyzing Systems Configuration Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Configuration Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsConfigurationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsConfigurationAuditorAgent = Object.freeze(
  new SystemsConfigurationAuditorAgent(),
);
