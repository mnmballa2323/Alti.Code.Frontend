...
class SecurityServerlessEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityServerlessSystem(objective) {
    logger.info(
      `💻 [SecurityServerlessEngineerAgent] Analyzing Security Serverless Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Serverless Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityServerlessEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityServerlessEngineerAgent = Object.freeze(
  new SecurityServerlessEngineerAgent(),
);
