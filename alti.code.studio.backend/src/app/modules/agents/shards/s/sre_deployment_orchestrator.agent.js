...
class SREDeploymentOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSREDeploymentSystem(objective) {
    logger.info(
      `💻 [SREDeploymentOrchestratorAgent] Analyzing SRE Deployment Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Deployment Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREDeploymentOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREDeploymentOrchestratorAgent = Object.freeze(
  new SREDeploymentOrchestratorAgent(),
);
