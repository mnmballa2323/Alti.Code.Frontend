...
class SystemsObservabilityDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsObservabilitySystem(objective) {
    logger.info(
      `💻 [SystemsObservabilityDesignerAgent] Analyzing Systems Observability Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Observability Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsObservabilityDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsObservabilityDesignerAgent = Object.freeze(
  new SystemsObservabilityDesignerAgent(),
);
