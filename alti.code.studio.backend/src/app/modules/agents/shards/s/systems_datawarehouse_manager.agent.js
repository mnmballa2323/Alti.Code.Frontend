...
class SystemsDataWarehouseManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsDataWarehouseSystem(objective) {
    logger.info(
      `💻 [SystemsDataWarehouseManagerAgent] Analyzing Systems DataWarehouse Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems DataWarehouse Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsDataWarehouseManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsDataWarehouseManagerAgent = Object.freeze(
  new SystemsDataWarehouseManagerAgent(),
);
