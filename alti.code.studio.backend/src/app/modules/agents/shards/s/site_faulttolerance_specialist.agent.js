...
class SiteFaultToleranceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteFaultToleranceSystem(objective) {
    logger.info(
      `💻 [SiteFaultToleranceSpecialistAgent] Analyzing Site FaultTolerance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site FaultTolerance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteFaultToleranceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteFaultToleranceSpecialistAgent = Object.freeze(
  new SiteFaultToleranceSpecialistAgent(),
);
