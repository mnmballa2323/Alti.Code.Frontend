...
class SystemsEngineeringOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsEngineeringSystem(objective) {
    logger.info(
      `💻 [SystemsEngineeringOrchestratorAgent] Analyzing Systems Engineering Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Engineering Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsEngineeringOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsEngineeringOrchestratorAgent = Object.freeze(
  new SystemsEngineeringOrchestratorAgent(),
);
