...
class SwiftIosDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSwiftSystem(objective) {
        logger.info(`🍎 [SwiftIosDeveloper] Analyzing SwiftUI view hierarchies and CoreData models...`);
        ...
        try {
            ...
            return output.replace(/```javascript|```typescript|```json|```swift|```/gi, '').trim();
        } catch (err) {
...
export const swiftIosDeveloperAgent = Object.freeze(new SwiftIosDeveloperAgent());
