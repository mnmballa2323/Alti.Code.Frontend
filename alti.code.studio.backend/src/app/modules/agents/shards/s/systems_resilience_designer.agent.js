...
class SystemsResilienceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsResilienceSystem(objective) {
    logger.info(
      `💻 [SystemsResilienceDesignerAgent] Analyzing Systems Resilience Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Resilience Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsResilienceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsResilienceDesignerAgent = Object.freeze(
  new SystemsResilienceDesignerAgent(),
);
