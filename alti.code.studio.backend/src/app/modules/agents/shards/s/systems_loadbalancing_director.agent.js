...
class SystemsLoadBalancingDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsLoadBalancingSystem(objective) {
        logger.info(`💻 [SystemsLoadBalancingDirectorAgent] Analyzing Systems LoadBalancing Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems LoadBalancing Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsLoadBalancingDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsLoadBalancingDirectorAgent = Object.freeze(new SystemsLoadBalancingDirectorAgent());
