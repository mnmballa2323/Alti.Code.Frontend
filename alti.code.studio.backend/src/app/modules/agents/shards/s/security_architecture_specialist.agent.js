...
class SecurityArchitectureSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityArchitectureSystem(objective) {
    logger.info(
      `💻 [SecurityArchitectureSpecialistAgent] Analyzing Security Architecture Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Architecture Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityArchitectureSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityArchitectureSpecialistAgent = Object.freeze(
  new SecurityArchitectureSpecialistAgent(),
);
