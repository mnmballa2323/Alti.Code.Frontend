...
class SecurityAutomationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityAutomationSystem(objective) {
    logger.info(
      `💻 [SecurityAutomationManagerAgent] Analyzing Security Automation Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Automation Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityAutomationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityAutomationManagerAgent = Object.freeze(
  new SecurityAutomationManagerAgent(),
);
