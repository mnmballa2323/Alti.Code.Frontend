...
class SecurityMicroservicesOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityMicroservicesSystem(objective) {
        logger.info(`💻 [SecurityMicroservicesOrchestratorAgent] Analyzing Security Microservices Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Microservices Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityMicroservicesOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityMicroservicesOrchestratorAgent = Object.freeze(new SecurityMicroservicesOrchestratorAgent());
