...
class SystemsFaultToleranceAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsFaultToleranceSystem(objective) {
        logger.info(`💻 [SystemsFaultToleranceAnalystAgent] Analyzing Systems FaultTolerance Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems FaultTolerance Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsFaultToleranceAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsFaultToleranceAnalystAgent = Object.freeze(new SystemsFaultToleranceAnalystAgent());
