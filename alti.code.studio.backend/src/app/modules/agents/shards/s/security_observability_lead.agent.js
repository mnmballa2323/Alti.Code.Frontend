...
class SecurityObservabilityLeadAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityObservabilitySystem(objective) {
    logger.info(
      `💻 [SecurityObservabilityLeadAgent] Analyzing Security Observability Lead specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Observability Lead Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityObservabilityLeadAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityObservabilityLeadAgent = Object.freeze(
  new SecurityObservabilityLeadAgent(),
);
