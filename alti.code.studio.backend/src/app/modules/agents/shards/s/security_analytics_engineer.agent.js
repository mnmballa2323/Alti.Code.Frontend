...
class SecurityAnalyticsEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityAnalyticsSystem(objective) {
    logger.info(
      `💻 [SecurityAnalyticsEngineerAgent] Analyzing Security Analytics Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Analytics Engineer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityAnalyticsEngineerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityAnalyticsEngineerAgent = Object.freeze(
  new SecurityAnalyticsEngineerAgent(),
);
