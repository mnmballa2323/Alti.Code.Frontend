...
class SystemsCachingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsCachingSystem(objective) {
    logger.info(
      `💻 [SystemsCachingOrchestratorAgent] Analyzing Systems Caching Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Caching Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsCachingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsCachingOrchestratorAgent = Object.freeze(
  new SystemsCachingOrchestratorAgent(),
);
