...
class SecurityContainerizationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityContainerizationSystem(objective) {
    logger.info(
      `💻 [SecurityContainerizationAnalystAgent] Analyzing Security Containerization Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Containerization Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityContainerizationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityContainerizationAnalystAgent = Object.freeze(
  new SecurityContainerizationAnalystAgent(),
);
