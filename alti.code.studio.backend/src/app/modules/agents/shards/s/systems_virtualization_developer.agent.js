...
class SystemsVirtualizationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsVirtualizationSystem(objective) {
        logger.info(`💻 [SystemsVirtualizationDeveloperAgent] Analyzing Systems Virtualization Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Virtualization Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsVirtualizationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsVirtualizationDeveloperAgent = Object.freeze(new SystemsVirtualizationDeveloperAgent());
