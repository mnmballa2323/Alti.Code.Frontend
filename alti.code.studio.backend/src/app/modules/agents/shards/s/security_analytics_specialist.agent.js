...
class SecurityAnalyticsSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityAnalyticsSystem(objective) {
    logger.info(
      `💻 [SecurityAnalyticsSpecialistAgent] Analyzing Security Analytics Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Analytics Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityAnalyticsSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityAnalyticsSpecialistAgent = Object.freeze(
  new SecurityAnalyticsSpecialistAgent(),
);
