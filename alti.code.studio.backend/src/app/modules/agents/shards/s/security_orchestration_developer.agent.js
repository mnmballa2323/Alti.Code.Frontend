...
class SecurityOrchestrationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityOrchestrationSystem(objective) {
        logger.info(`💻 [SecurityOrchestrationDeveloperAgent] Analyzing Security Orchestration Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Orchestration Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityOrchestrationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityOrchestrationDeveloperAgent = Object.freeze(new SecurityOrchestrationDeveloperAgent());
