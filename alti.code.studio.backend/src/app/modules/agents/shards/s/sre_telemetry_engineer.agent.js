...
class SRETelemetryEngineerAgent extends GeminiCliBaseAgent {
  ...
  async generateSRETelemetrySystem(objective) {
    logger.info(
      `💻 [SRETelemetryEngineerAgent] Analyzing SRE Telemetry Engineer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE Telemetry Engineer Target',
      );
...
export const sRETelemetryEngineerAgent = Object.freeze(
  new SRETelemetryEngineerAgent(),
);
