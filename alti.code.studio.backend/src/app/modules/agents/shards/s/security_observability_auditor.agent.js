...
class SecurityObservabilityAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityObservabilitySystem(objective) {
    logger.info(
      `💻 [SecurityObservabilityAuditorAgent] Analyzing Security Observability Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Observability Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityObservabilityAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityObservabilityAuditorAgent = Object.freeze(
  new SecurityObservabilityAuditorAgent(),
);
