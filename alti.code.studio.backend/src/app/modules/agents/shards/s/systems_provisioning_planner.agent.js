...
class SystemsProvisioningPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsProvisioningSystem(objective) {
        logger.info(`💻 [SystemsProvisioningPlannerAgent] Analyzing Systems Provisioning Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Provisioning Planner Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsProvisioningPlannerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsProvisioningPlannerAgent = Object.freeze(new SystemsProvisioningPlannerAgent());
