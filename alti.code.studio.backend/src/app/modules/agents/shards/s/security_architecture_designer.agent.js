...
class SecurityArchitectureDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityArchitectureSystem(objective) {
    logger.info(
      `💻 [SecurityArchitectureDesignerAgent] Analyzing Security Architecture Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Architecture Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityArchitectureDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityArchitectureDesignerAgent = Object.freeze(
  new SecurityArchitectureDesignerAgent(),
);
