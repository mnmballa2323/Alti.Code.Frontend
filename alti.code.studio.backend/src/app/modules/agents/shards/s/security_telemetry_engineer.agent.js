...
class SecurityTelemetryEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityTelemetrySystem(objective) {
        logger.info(`💻 [SecurityTelemetryEngineerAgent] Analyzing Security Telemetry Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Telemetry Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityTelemetryEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityTelemetryEngineerAgent = Object.freeze(new SecurityTelemetryEngineerAgent());
