...
class StackdriverAnalystAgent extends GeminiCliBaseAgent {
    ...
    async analyzeLogs(cloudLogs) {
        logger.info(`📊 [StackdriverAnalyst] Analyzing ${cloudLogs.length} Cloud Logging entries...`);
...
${JSON.stringify(cloudLogs, null, 2)}
        ...
        try {
            ...
            if (diagnosis.status !== 'HEALTHY') {
                logger.warn(`⚠️ [StackdriverAnalyst] Anomaly detected: ${diagnosis.reason}`);
            } else {
                logger.info(`✅ [StackdriverAnalyst] Deployment verified stable via Stackdriver logs.`);
            }

            return diagnosis;
        } catch (err) {
            logger.error(`❌ [StackdriverAnalyst] Failed to analyze logs: ${err.message}`);
            throw err;
        }
    }
}

export const stackdriverAnalystAgent = Object.freeze(new StackdriverAnalystAgent());
