...
class SystemsTelemetrySpecialistAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsTelemetrySystem(objective) {
        logger.info(`💻 [SystemsTelemetrySpecialistAgent] Analyzing Systems Telemetry Specialist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Telemetry Specialist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsTelemetrySpecialistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsTelemetrySpecialistAgent = Object.freeze(new SystemsTelemetrySpecialistAgent());
