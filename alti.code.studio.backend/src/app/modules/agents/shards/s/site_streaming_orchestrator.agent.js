...
class SiteStreamingOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteStreamingSystem(objective) {
    logger.info(
      `💻 [SiteStreamingOrchestratorAgent] Analyzing Site Streaming Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Streaming Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteStreamingOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteStreamingOrchestratorAgent = Object.freeze(
  new SiteStreamingOrchestratorAgent(),
);
