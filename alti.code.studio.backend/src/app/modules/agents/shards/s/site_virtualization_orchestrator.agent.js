...
class SiteVirtualizationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteVirtualizationSystem(objective) {
        logger.info(`💻 [SiteVirtualizationOrchestratorAgent] Analyzing Site Virtualization Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Virtualization Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteVirtualizationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteVirtualizationOrchestratorAgent = Object.freeze(new SiteVirtualizationOrchestratorAgent());
