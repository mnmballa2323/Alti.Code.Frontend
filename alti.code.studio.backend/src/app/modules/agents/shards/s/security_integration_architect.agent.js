...
class SecurityIntegrationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityIntegrationSystem(objective) {
        logger.info(`💻 [SecurityIntegrationArchitectAgent] Analyzing Security Integration Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Integration Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityIntegrationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityIntegrationArchitectAgent = Object.freeze(new SecurityIntegrationArchitectAgent());
