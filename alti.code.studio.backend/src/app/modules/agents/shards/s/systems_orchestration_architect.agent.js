...
class SystemsOrchestrationArchitectAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsOrchestrationSystem(objective) {
        logger.info(`💻 [SystemsOrchestrationArchitectAgent] Analyzing Systems Orchestration Architect specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Orchestration Architect Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsOrchestrationArchitectAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsOrchestrationArchitectAgent = Object.freeze(new SystemsOrchestrationArchitectAgent());
