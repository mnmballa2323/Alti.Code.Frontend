...
class SystemsOptimizationManagerAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsOptimizationSystem(objective) {
        logger.info(`💻 [SystemsOptimizationManagerAgent] Analyzing Systems Optimization Manager specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Optimization Manager Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsOptimizationManagerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsOptimizationManagerAgent = Object.freeze(new SystemsOptimizationManagerAgent());
