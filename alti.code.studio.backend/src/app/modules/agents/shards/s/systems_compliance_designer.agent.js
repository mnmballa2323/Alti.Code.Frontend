...
class SystemsComplianceDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsComplianceSystem(objective) {
    logger.info(
      `💻 [SystemsComplianceDesignerAgent] Analyzing Systems Compliance Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Compliance Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsComplianceDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsComplianceDesignerAgent = Object.freeze(
  new SystemsComplianceDesignerAgent(),
);
