...
class SiteContainerizationDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteContainerizationSystem(objective) {
    logger.info(
      `💻 [SiteContainerizationDeveloperAgent] Analyzing Site Containerization Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Containerization Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteContainerizationDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteContainerizationDeveloperAgent = Object.freeze(
  new SiteContainerizationDeveloperAgent(),
);
