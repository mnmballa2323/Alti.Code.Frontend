...
class SiteDeploymentOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteDeploymentSystem(objective) {
        logger.info(`💻 [SiteDeploymentOrchestratorAgent] Analyzing Site Deployment Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Deployment Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteDeploymentOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteDeploymentOrchestratorAgent = Object.freeze(new SiteDeploymentOrchestratorAgent());
