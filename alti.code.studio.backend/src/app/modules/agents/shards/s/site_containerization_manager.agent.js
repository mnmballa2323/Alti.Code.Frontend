...
class SiteContainerizationManagerAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteContainerizationSystem(objective) {
    logger.info(
      `💻 [SiteContainerizationManagerAgent] Analyzing Site Containerization Manager specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Containerization Manager Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteContainerizationManagerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteContainerizationManagerAgent = Object.freeze(
  new SiteContainerizationManagerAgent(),
);
