...
class SecuritySecurityPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecuritySecuritySystem(objective) {
    logger.info(
      `💻 [SecuritySecurityPlannerAgent] Analyzing Security Security Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Security Planner Target',
      );
...
export const securitySecurityPlannerAgent = Object.freeze(
  new SecuritySecurityPlannerAgent(),
);
