...
class SecurityTelemetryPlannerAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityTelemetrySystem(objective) {
    logger.info(
      `💻 [SecurityTelemetryPlannerAgent] Analyzing Security Telemetry Planner specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Telemetry Planner Target',
      );
...
export const securityTelemetryPlannerAgent = Object.freeze(
  new SecurityTelemetryPlannerAgent(),
);
