...
class SystemsContainerizationAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsContainerizationSystem(objective) {
    logger.info(
      `💻 [SystemsContainerizationAuditorAgent] Analyzing Systems Containerization Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Containerization Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsContainerizationAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsContainerizationAuditorAgent = Object.freeze(
  new SystemsContainerizationAuditorAgent(),
);
