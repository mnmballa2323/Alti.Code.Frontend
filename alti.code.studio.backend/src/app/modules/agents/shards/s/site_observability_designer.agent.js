...
class SiteObservabilityDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSiteObservabilitySystem(objective) {
    logger.info(
      `💻 [SiteObservabilityDesignerAgent] Analyzing Site Observability Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Site Observability Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SiteObservabilityDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const siteObservabilityDesignerAgent = Object.freeze(
  new SiteObservabilityDesignerAgent(),
);
