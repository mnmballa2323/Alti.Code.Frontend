...
class SystemsConfigurationAnalystAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsConfigurationSystem(objective) {
    logger.info(
      `💻 [SystemsConfigurationAnalystAgent] Analyzing Systems Configuration Analyst specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Configuration Analyst Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsConfigurationAnalystAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsConfigurationAnalystAgent = Object.freeze(
  new SystemsConfigurationAnalystAgent(),
);
