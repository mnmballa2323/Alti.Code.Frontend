...
class SystemsLoadBalancingArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsLoadBalancingSystem(objective) {
    logger.info(
      `💻 [SystemsLoadBalancingArchitectAgent] Analyzing Systems LoadBalancing Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems LoadBalancing Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsLoadBalancingArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsLoadBalancingArchitectAgent = Object.freeze(
  new SystemsLoadBalancingArchitectAgent(),
);
