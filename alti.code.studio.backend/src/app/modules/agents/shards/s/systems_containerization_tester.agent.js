...
class SystemsContainerizationTesterAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsContainerizationSystem(objective) {
    logger.info(
      `💻 [SystemsContainerizationTesterAgent] Analyzing Systems Containerization Tester specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Containerization Tester Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsContainerizationTesterAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsContainerizationTesterAgent = Object.freeze(
  new SystemsContainerizationTesterAgent(),
);
