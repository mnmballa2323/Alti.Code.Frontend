...
class SiteArchitectureOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteArchitectureSystem(objective) {
        logger.info(`💻 [SiteArchitectureOrchestratorAgent] Analyzing Site Architecture Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Architecture Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SiteArchitectureOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const siteArchitectureOrchestratorAgent = Object.freeze(new SiteArchitectureOrchestratorAgent());
