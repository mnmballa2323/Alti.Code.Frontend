...
class SystemsArchitectureDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsArchitectureSystem(objective) {
    logger.info(
      `💻 [SystemsArchitectureDesignerAgent] Analyzing Systems Architecture Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Architecture Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsArchitectureDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsArchitectureDesignerAgent = Object.freeze(
  new SystemsArchitectureDesignerAgent(),
);
