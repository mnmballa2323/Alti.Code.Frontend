...
class SREFaultToleranceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateSREFaultToleranceSystem(objective) {
    logger.info(
      `💻 [SREFaultToleranceSpecialistAgent] Analyzing SRE FaultTolerance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - SRE FaultTolerance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SREFaultToleranceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const sREFaultToleranceSpecialistAgent = Object.freeze(
  new SREFaultToleranceSpecialistAgent(),
);
