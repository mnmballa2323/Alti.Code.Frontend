...
class SystemsDataWarehouseStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsDataWarehouseSystem(objective) {
        logger.info(`💻 [SystemsDataWarehouseStrategistAgent] Analyzing Systems DataWarehouse Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems DataWarehouse Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsDataWarehouseStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsDataWarehouseStrategistAgent = Object.freeze(new SystemsDataWarehouseStrategistAgent());
