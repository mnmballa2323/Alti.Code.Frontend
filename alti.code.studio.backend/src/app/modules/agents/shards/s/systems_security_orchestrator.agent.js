...
class SystemsSecurityOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateSystemsSecuritySystem(objective) {
        logger.info(`💻 [SystemsSecurityOrchestratorAgent] Analyzing Systems Security Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Systems Security Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SystemsSecurityOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const systemsSecurityOrchestratorAgent = Object.freeze(new SystemsSecurityOrchestratorAgent());
