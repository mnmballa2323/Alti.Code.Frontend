...
class SystemsDeploymentArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateSystemsDeploymentSystem(objective) {
    logger.info(
      `💻 [SystemsDeploymentArchitectAgent] Analyzing Systems Deployment Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Systems Deployment Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SystemsDeploymentArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const systemsDeploymentArchitectAgent = Object.freeze(
  new SystemsDeploymentArchitectAgent(),
);
