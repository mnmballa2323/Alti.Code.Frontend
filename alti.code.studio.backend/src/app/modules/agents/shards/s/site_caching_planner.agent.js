...
class SiteCachingPlannerAgent extends GeminiCliBaseAgent {
    ...
    async generateSiteCachingSystem(objective) {
        logger.info(`💻 [SiteCachingPlannerAgent] Analyzing Site Caching Planner specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Site Caching Planner Target");
...
export const siteCachingPlannerAgent = Object.freeze(new SiteCachingPlannerAgent());
