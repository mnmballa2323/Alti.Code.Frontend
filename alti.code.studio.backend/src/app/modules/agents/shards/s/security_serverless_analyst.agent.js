...
class SecurityServerlessAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateSecurityServerlessSystem(objective) {
        logger.info(`💻 [SecurityServerlessAnalystAgent] Analyzing Security Serverless Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - Security Serverless Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [SecurityServerlessAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const securityServerlessAnalystAgent = Object.freeze(new SecurityServerlessAnalystAgent());
