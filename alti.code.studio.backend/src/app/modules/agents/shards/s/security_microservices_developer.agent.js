...
class SecurityMicroservicesDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityMicroservicesSystem(objective) {
    logger.info(
      `💻 [SecurityMicroservicesDeveloperAgent] Analyzing Security Microservices Developer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Microservices Developer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityMicroservicesDeveloperAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityMicroservicesDeveloperAgent = Object.freeze(
  new SecurityMicroservicesDeveloperAgent(),
);
