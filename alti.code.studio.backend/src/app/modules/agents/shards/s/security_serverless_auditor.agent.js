...
class SecurityServerlessAuditorAgent extends GeminiCliBaseAgent {
  ...
  async generateSecurityServerlessSystem(objective) {
    logger.info(
      `💻 [SecurityServerlessAuditorAgent] Analyzing Security Serverless Auditor specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - Security Serverless Auditor Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [SecurityServerlessAuditorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const securityServerlessAuditorAgent = Object.freeze(
  new SecurityServerlessAuditorAgent(),
);
