...
class OceanographyMarineBiologistAgent extends GeminiCliBaseAgent {
  ...
  async generateOceanSystem(oceanObjective) {
    logger.info(
      `🌊 [OceanographyMarineBiologist] Analyzing objective for deep-sea sonar and ROV telemetry...`,
    );
...
${oceanObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [OceanographyMarineBiologist] Oceanography architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [OceanographyMarineBiologist] Failed to generate ocean system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const oceanographyMarineBiologistAgent = Object.freeze(
  new OceanographyMarineBiologistAgent(),
);
