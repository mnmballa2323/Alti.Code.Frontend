...
class OsLevelMacroOrchestratorAgent extends BaseSpecialistAgent {
  constructor() {
    super(
      'OsLevelMacroOrchestratorAgent',
      'Os Level Macro Orchestrator Agent',
      'Tier 10+',
    );
...
`;
  ...
  async _invoke(query, args = {}) {
    ...
    if (qLower.includes('wifi') || qLower.includes('network')) {
      ...
      hardwareIntent = {
        domain: 'hardware',
        action: 'SET_DISPLAY_STATE',
        value: args.displayState || 'SLEEP',
      };
    } else {
      ...
      hardwareIntent = {
        ...
        sequence: args.sequence || query,
      };
    }

    try {
      logger.info(
        `[Phase 40] OS-Level Macro Orchestrator delegating intent to OpenClaw: ${hardwareIntent.action}`,
      );
...
export const osLevelMacroOrchestratorAgent = Object.freeze(
  new OsLevelMacroOrchestratorAgent(),
);
