...
class OracleCloudInfrastructureAgent extends GeminiCliBaseAgent {
    ...
    async generateOracleSystem(oracleObjective) {
        logger.info(`☁️ [OracleCloudInfrastructure] Analyzing objective for Oracle Autonomous Database and Exadata...`);
...
${oracleObjective}
        ...
        try {
            ...
            const cleanCode = output.replace(/```javascript|```typescript|```json|```sql|```hcl|```/gi, '').trim();
            logger.info(`✅ [OracleCloudInfrastructure] Oracle architecture generated successfully.`);
            return cleanCode;
        } catch (err) {
            logger.error(`❌ [OracleCloudInfrastructure] Failed to generate Oracle system: ${err.message}`);
            throw err;
        }
    }
}

export const oracleCloudInfrastructureAgent = Object.freeze(new OracleCloudInfrastructureAgent());
