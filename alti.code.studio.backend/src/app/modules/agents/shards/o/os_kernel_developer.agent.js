...
class OsKernelDeveloperAgent extends GeminiCliBaseAgent {
  ...
  async generateKernelComponent(kernelObjective) {
    logger.info(
      `🐧 [OsKernelDeveloper] Synthesizing low-level OS Kernel / Driver code...`,
    );
...
${kernelObjective}
    ...
    try {
      ...
      logger.info(
        `✅ [OsKernelDeveloper] Kernel component synthesized successfully.`,
      );
      return cleanC;
    } catch (err) {
      logger.error(
        `❌ [OsKernelDeveloper] Failed to generate kernel component: ${err.message}`,
      );
      throw err;
    }
  }
...
export const osKernelDeveloperAgent = Object.freeze(
  new OsKernelDeveloperAgent(),
);
