...
class OmniSwarmOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async orchestrateSwarm(macroObjective) {
    logger.info(
      `👑 [OmniSwarmOrchestrator] The Apex Agent is analyzing a macro-objective and assembling the Swarm...`,
    );
...
${macroObjective}
    ...
    try {
      ...
      const cleanCode = output
        .replace(/```javascript|```typescript|```json|```/gi, '')
        .trim();
      logger.info(
        `✅ [OmniSwarmOrchestrator] Swarm Orchestration Plan generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [OmniSwarmOrchestrator] Failed to generate orchestration plan: ${err.message}`,
      );
      throw err;
    }
  }
...
export const omniSwarmOrchestratorAgent = Object.freeze(
  new OmniSwarmOrchestratorAgent(),
);
