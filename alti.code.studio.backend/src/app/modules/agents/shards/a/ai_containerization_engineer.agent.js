...
class AIContainerizationEngineerAgent extends GeminiCliBaseAgent {
    ...
    async generateAIContainerizationSystem(objective) {
        logger.info(`💻 [AIContainerizationEngineerAgent] Analyzing AI Containerization Engineer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - AI Containerization Engineer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [AIContainerizationEngineerAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const aIContainerizationEngineerAgent = Object.freeze(new AIContainerizationEngineerAgent());
