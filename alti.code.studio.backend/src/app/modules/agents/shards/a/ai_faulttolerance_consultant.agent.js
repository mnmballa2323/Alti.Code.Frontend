...
class AIFaultToleranceConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateAIFaultToleranceSystem(objective) {
    logger.info(
      `💻 [AIFaultToleranceConsultantAgent] Analyzing AI FaultTolerance Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI FaultTolerance Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIFaultToleranceConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIFaultToleranceConsultantAgent = Object.freeze(
  new AIFaultToleranceConsultantAgent(),
);
