...
class AIContainerizationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateAIContainerizationSystem(objective) {
    logger.info(
      `💻 [AIContainerizationSpecialistAgent] Analyzing AI Containerization Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Containerization Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIContainerizationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIContainerizationSpecialistAgent = Object.freeze(
  new AIContainerizationSpecialistAgent(),
);
