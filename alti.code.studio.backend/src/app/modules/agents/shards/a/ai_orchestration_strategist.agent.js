...
class AIOrchestrationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateAIOrchestrationSystem(objective) {
    logger.info(
      `💻 [AIOrchestrationStrategistAgent] Analyzing AI Orchestration Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Orchestration Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIOrchestrationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIOrchestrationStrategistAgent = Object.freeze(
  new AIOrchestrationStrategistAgent(),
);
