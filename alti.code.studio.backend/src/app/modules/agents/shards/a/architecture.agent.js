...
export class ArchitectureAgent {
    ...
    async mapArchitecture(structureData, componentName = 'System') {
        logger.info(`🏗️ Architecture Agent: Initiating macro-analysis of [${componentName}] topological data...`);
        ...
        try {
            ...
            logger.info(`🏗️ Architecture Agent: Mapping complete. Complexity: ${architectureReport.complexityScore}/10.`);
            if (architectureReport.antiPatterns.length > 0 && architectureReport.antiPatterns[0] !== 'None') {
                logger.warn(`   Anti-Patterns Detected: ${architectureReport.antiPatterns.join(', ')}`);
            }
    ...
    async process(state) {
        ...
        return {
            ...state,
            status: 'success',
            results: [...(state.results || []), `Architecture Complexity: ${report.complexityScore}/10`]
        };
    }
}

export const architectureAgent = Object.freeze(new ArchitectureAgent());
