...
class AIVirtualizationArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateAIVirtualizationSystem(objective) {
    logger.info(
      `💻 [AIVirtualizationArchitectAgent] Analyzing AI Virtualization Architect specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Virtualization Architect Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIVirtualizationArchitectAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIVirtualizationArchitectAgent = Object.freeze(
  new AIVirtualizationArchitectAgent(),
);
