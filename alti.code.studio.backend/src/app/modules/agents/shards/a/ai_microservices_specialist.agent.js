...
class AIMicroservicesSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateAIMicroservicesSystem(objective) {
    logger.info(
      `💻 [AIMicroservicesSpecialistAgent] Analyzing AI Microservices Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Microservices Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIMicroservicesSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIMicroservicesSpecialistAgent = Object.freeze(
  new AIMicroservicesSpecialistAgent(),
);
