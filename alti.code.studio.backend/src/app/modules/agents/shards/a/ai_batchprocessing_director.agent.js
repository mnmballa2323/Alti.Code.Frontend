...
class AIBatchProcessingDirectorAgent extends GeminiCliBaseAgent {
    ...
    async generateAIBatchProcessingSystem(objective) {
        logger.info(`💻 [AIBatchProcessingDirectorAgent] Analyzing AI BatchProcessing Director specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - AI BatchProcessing Director Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [AIBatchProcessingDirectorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const aIBatchProcessingDirectorAgent = Object.freeze(new AIBatchProcessingDirectorAgent());
