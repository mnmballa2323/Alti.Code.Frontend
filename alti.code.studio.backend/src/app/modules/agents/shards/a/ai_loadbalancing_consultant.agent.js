...
class AILoadBalancingConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateAILoadBalancingSystem(objective) {
    logger.info(
      `💻 [AILoadBalancingConsultantAgent] Analyzing AI LoadBalancing Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI LoadBalancing Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AILoadBalancingConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aILoadBalancingConsultantAgent = Object.freeze(
  new AILoadBalancingConsultantAgent(),
);
