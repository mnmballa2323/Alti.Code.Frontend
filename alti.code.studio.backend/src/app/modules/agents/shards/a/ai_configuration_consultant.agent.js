...
class AIConfigurationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateAIConfigurationSystem(objective) {
    logger.info(
      `💻 [AIConfigurationConsultantAgent] Analyzing AI Configuration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Configuration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIConfigurationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIConfigurationConsultantAgent = Object.freeze(
  new AIConfigurationConsultantAgent(),
);
