...
class AIOrchestrationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateAIOrchestrationSystem(objective) {
    logger.info(
      `💻 [AIOrchestrationSpecialistAgent] Analyzing AI Orchestration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Orchestration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIOrchestrationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIOrchestrationSpecialistAgent = Object.freeze(
  new AIOrchestrationSpecialistAgent(),
);
