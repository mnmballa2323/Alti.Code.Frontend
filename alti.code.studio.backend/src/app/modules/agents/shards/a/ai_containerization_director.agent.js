...
class AIContainerizationDirectorAgent extends GeminiCliBaseAgent {
  ...
  async generateAIContainerizationSystem(objective) {
    logger.info(
      `💻 [AIContainerizationDirectorAgent] Analyzing AI Containerization Director specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Containerization Director Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIContainerizationDirectorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIContainerizationDirectorAgent = Object.freeze(
  new AIContainerizationDirectorAgent(),
);
