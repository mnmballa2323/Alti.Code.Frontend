...
class AIFaultToleranceSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateAIFaultToleranceSystem(objective) {
    logger.info(
      `💻 [AIFaultToleranceSpecialistAgent] Analyzing AI FaultTolerance Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI FaultTolerance Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIFaultToleranceSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIFaultToleranceSpecialistAgent = Object.freeze(
  new AIFaultToleranceSpecialistAgent(),
);
