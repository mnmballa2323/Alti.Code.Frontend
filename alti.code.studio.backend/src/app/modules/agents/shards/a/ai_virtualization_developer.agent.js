...
class AIVirtualizationDeveloperAgent extends GeminiCliBaseAgent {
    ...
    async generateAIVirtualizationSystem(objective) {
        logger.info(`💻 [AIVirtualizationDeveloperAgent] Analyzing AI Virtualization Developer specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - AI Virtualization Developer Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [AIVirtualizationDeveloperAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const aIVirtualizationDeveloperAgent = Object.freeze(new AIVirtualizationDeveloperAgent());
