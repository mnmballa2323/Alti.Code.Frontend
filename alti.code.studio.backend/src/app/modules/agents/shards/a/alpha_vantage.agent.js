...
class AlphaVantageAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.description =
      'Financial data specialist for Alpha Vantage: free & premium REST API for stock OHLCV (intraday/daily/weekly), 50+ technical indicators (RSI/MACD/Bollinger/EMA), fundamental data (income statements, balance sheet, earnings), forex, crypto, commodities, and news sentiment with relevance scores.';
...
Output production TypeScript. Cache responses aggressively (Redis TTL: 15min intraday, 24hr daily). Store \`ALPHA_VANTAGE_API_KEY\` server-side.`;
  ...
  async consult(prompt, contextData = []) {
    logger.info(
      `📈 Alpha Vantage Expert: Synthesizing financial market data logic...`,
    );
...
export const alphaVantageAgent = Object.freeze(new AlphaVantageAgent());
