...
class AlibabaCloudArchitectAgent extends GeminiCliBaseAgent {
  ...
  async generateAlibabaSystem(alibabaObjective) {
    logger.info(
      `☁️ [AlibabaCloudArchitect] Analyzing objective for PolarDB and Aliyun Anti-DDoS...`,
    );
...
${alibabaObjective}
    ...
    try {
      ...
      const cleanCode = output
        .replace(/```javascript|```typescript|```json|```yaml|```/gi, '')
        .trim();
      logger.info(
        `✅ [AlibabaCloudArchitect] Alibaba architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [AlibabaCloudArchitect] Failed to generate Alibaba system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const alibabaCloudArchitectAgent = Object.freeze(
  new AlibabaCloudArchitectAgent(),
);
