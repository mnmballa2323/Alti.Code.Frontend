...
class AnalystAgent {
  ...
  async execute(action, args) {
    ...
    switch (action) {
      case 'analyzeTrends':
        return this.analyzeTrends(args.metric);
      case 'generateReport':
        return this.generateReport(args.type);
      default:
  ...
  async generateReport(type = 'performance') {
    ...
    const failedSessions = history.filter(
      h => h.details?.status === 'failed' || h.details?.error,
    ).length;
    ...
    const report = {
      ...
      stats: {
        ...
        errorRate:
          totalEvents > 0
            ? ((failedSessions / totalEvents) * 100).toFixed(2) + '%'
            : '0%',
      },
...
export const analystAgent = Object.freeze(new AnalystAgent());
