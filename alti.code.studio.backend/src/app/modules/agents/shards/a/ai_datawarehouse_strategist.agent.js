...
class AIDataWarehouseStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateAIDataWarehouseSystem(objective) {
        logger.info(`💻 [AIDataWarehouseStrategistAgent] Analyzing AI DataWarehouse Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - AI DataWarehouse Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [AIDataWarehouseStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const aIDataWarehouseStrategistAgent = Object.freeze(new AIDataWarehouseStrategistAgent());
