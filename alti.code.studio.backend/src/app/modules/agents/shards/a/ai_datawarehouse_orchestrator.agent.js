...
class AIDataWarehouseOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateAIDataWarehouseSystem(objective) {
        logger.info(`💻 [AIDataWarehouseOrchestratorAgent] Analyzing AI DataWarehouse Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - AI DataWarehouse Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [AIDataWarehouseOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const aIDataWarehouseOrchestratorAgent = Object.freeze(new AIDataWarehouseOrchestratorAgent());
