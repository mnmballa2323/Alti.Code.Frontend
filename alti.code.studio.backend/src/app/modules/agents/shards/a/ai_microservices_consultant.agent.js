...
class AIMicroservicesConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateAIMicroservicesSystem(objective) {
    logger.info(
      `💻 [AIMicroservicesConsultantAgent] Analyzing AI Microservices Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Microservices Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIMicroservicesConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIMicroservicesConsultantAgent = Object.freeze(
  new AIMicroservicesConsultantAgent(),
);
