...
class AIMicroservicesOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateAIMicroservicesSystem(objective) {
    logger.info(
      `💻 [AIMicroservicesOrchestratorAgent] Analyzing AI Microservices Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Microservices Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIMicroservicesOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIMicroservicesOrchestratorAgent = Object.freeze(
  new AIMicroservicesOrchestratorAgent(),
);
