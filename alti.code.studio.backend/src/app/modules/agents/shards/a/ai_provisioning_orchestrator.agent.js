...
class AIProvisioningOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateAIProvisioningSystem(objective) {
        logger.info(`💻 [AIProvisioningOrchestratorAgent] Analyzing AI Provisioning Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - AI Provisioning Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [AIProvisioningOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const aIProvisioningOrchestratorAgent = Object.freeze(new AIProvisioningOrchestratorAgent());
