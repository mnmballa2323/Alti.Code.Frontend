...
class AIContainerizationStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateAIContainerizationSystem(objective) {
    logger.info(
      `💻 [AIContainerizationStrategistAgent] Analyzing AI Containerization Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Containerization Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIContainerizationStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIContainerizationStrategistAgent = Object.freeze(
  new AIContainerizationStrategistAgent(),
);
