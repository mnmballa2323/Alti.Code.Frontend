...
class AstrophysicsCosmologistAgent extends GeminiCliBaseAgent {
  ...
  async generateAstrophysicsSystem(astroObjective) {
    logger.info(
      `🌌 [AstrophysicsCosmologist] Analyzing objective for N-body simulations and telescope interferometry...`,
    );
...
${astroObjective}
    ...
    try {
      ...
      const cleanCode = output
        .replace(
          /```javascript|```typescript|```json|```cpp|```cuda|```python|```/gi,
          '',
        )
        .trim();
      logger.info(
        `✅ [AstrophysicsCosmologist] Astrophysics architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [AstrophysicsCosmologist] Failed to generate astrophysics system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const astrophysicsCosmologistAgent = Object.freeze(
  new AstrophysicsCosmologistAgent(),
);
