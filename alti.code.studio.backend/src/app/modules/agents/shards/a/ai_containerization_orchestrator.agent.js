...
class AIContainerizationOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateAIContainerizationSystem(objective) {
    logger.info(
      `💻 [AIContainerizationOrchestratorAgent] Analyzing AI Containerization Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Containerization Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIContainerizationOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIContainerizationOrchestratorAgent = Object.freeze(
  new AIContainerizationOrchestratorAgent(),
);
