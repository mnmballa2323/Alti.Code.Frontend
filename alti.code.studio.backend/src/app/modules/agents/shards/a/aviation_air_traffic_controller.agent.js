...
class AviationAirTrafficControllerAgent extends GeminiCliBaseAgent {
  ...
  async generateAtcSystem(aviationObjective) {
    logger.info(
      `✈️ [AviationAirTrafficController] Analyzing objective for dynamic routing and collision avoidance...`,
    );
...
${aviationObjective}
    ...
    try {
      ...
      const cleanCode = output
        ...
        .trim();
      logger.info(
        `✅ [AviationAirTrafficController] ATC architecture generated successfully.`,
      );
      return cleanCode;
    } catch (err) {
      logger.error(
        `❌ [AviationAirTrafficController] Failed to generate ATC system: ${err.message}`,
      );
      throw err;
    }
  }
...
export const aviationAirTrafficControllerAgent = Object.freeze(
  new AviationAirTrafficControllerAgent(),
);
