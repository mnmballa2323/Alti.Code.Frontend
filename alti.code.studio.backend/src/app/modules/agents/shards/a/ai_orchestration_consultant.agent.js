...
class AIOrchestrationConsultantAgent extends GeminiCliBaseAgent {
  ...
  async generateAIOrchestrationSystem(objective) {
    logger.info(
      `💻 [AIOrchestrationConsultantAgent] Analyzing AI Orchestration Consultant specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Orchestration Consultant Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIOrchestrationConsultantAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIOrchestrationConsultantAgent = Object.freeze(
  new AIOrchestrationConsultantAgent(),
);
