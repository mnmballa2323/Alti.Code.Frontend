...
class AIFaultToleranceStrategistAgent extends GeminiCliBaseAgent {
    ...
    async generateAIFaultToleranceSystem(objective) {
        logger.info(`💻 [AIFaultToleranceStrategistAgent] Analyzing AI FaultTolerance Strategist specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - AI FaultTolerance Strategist Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [AIFaultToleranceStrategistAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const aIFaultToleranceStrategistAgent = Object.freeze(new AIFaultToleranceStrategistAgent());
