...
class AIVirtualizationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateAIVirtualizationSystem(objective) {
    logger.info(
      `💻 [AIVirtualizationSpecialistAgent] Analyzing AI Virtualization Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Virtualization Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIVirtualizationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIVirtualizationSpecialistAgent = Object.freeze(
  new AIVirtualizationSpecialistAgent(),
);
