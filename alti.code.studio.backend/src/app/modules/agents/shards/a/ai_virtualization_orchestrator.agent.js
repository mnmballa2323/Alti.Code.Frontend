...
class AIVirtualizationOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateAIVirtualizationSystem(objective) {
        logger.info(`💻 [AIVirtualizationOrchestratorAgent] Analyzing AI Virtualization Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - AI Virtualization Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [AIVirtualizationOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const aIVirtualizationOrchestratorAgent = Object.freeze(new AIVirtualizationOrchestratorAgent());
