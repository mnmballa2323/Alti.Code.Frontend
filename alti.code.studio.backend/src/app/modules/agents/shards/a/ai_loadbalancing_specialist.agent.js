...
class AILoadBalancingSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateAILoadBalancingSystem(objective) {
    logger.info(
      `💻 [AILoadBalancingSpecialistAgent] Analyzing AI LoadBalancing Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI LoadBalancing Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AILoadBalancingSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aILoadBalancingSpecialistAgent = Object.freeze(
  new AILoadBalancingSpecialistAgent(),
);
