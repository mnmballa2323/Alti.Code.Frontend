...
class AutonomousWeaponsSystemsAgent extends GeminiCliBaseAgent {
    ...
    async generateDefenseSystem(defenseObjective) {
        logger.info(`🚁 [AutonomousWeaponsSystems] Analyzing objective for drone swarm coordination and ROE constraints...`);
...
${defenseObjective}
        ...
        try {
            ...
            logger.info(`✅ [AutonomousWeaponsSystems] Defense architecture generated successfully.`);
            return cleanCode;
        } catch (err) {
            logger.error(`❌ [AutonomousWeaponsSystems] Failed to generate defense system: ${err.message}`);
            throw err;
        }
    }
}

export const autonomousWeaponsSystemsAgent = Object.freeze(new AutonomousWeaponsSystemsAgent());
