...
class AIContainerizationDesignerAgent extends GeminiCliBaseAgent {
  ...
  async generateAIContainerizationSystem(objective) {
    logger.info(
      `💻 [AIContainerizationDesignerAgent] Analyzing AI Containerization Designer specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Containerization Designer Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIContainerizationDesignerAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIContainerizationDesignerAgent = Object.freeze(
  new AIContainerizationDesignerAgent(),
);
