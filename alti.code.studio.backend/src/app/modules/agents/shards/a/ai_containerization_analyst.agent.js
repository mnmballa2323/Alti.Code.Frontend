...
class AIContainerizationAnalystAgent extends GeminiCliBaseAgent {
    ...
    async generateAIContainerizationSystem(objective) {
        logger.info(`💻 [AIContainerizationAnalystAgent] Analyzing AI Containerization Analyst specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - AI Containerization Analyst Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [AIContainerizationAnalystAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const aIContainerizationAnalystAgent = Object.freeze(new AIContainerizationAnalystAgent());
