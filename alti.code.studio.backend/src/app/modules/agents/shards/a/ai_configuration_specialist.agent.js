...
class AIConfigurationSpecialistAgent extends GeminiCliBaseAgent {
  ...
  async generateAIConfigurationSystem(objective) {
    logger.info(
      `💻 [AIConfigurationSpecialistAgent] Analyzing AI Configuration Specialist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Configuration Specialist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIConfigurationSpecialistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIConfigurationSpecialistAgent = Object.freeze(
  new AIConfigurationSpecialistAgent(),
);
