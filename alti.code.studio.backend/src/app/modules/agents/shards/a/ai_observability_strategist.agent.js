...
class AIObservabilityStrategistAgent extends GeminiCliBaseAgent {
  ...
  async generateAIObservabilitySystem(objective) {
    logger.info(
      `💻 [AIObservabilityStrategistAgent] Analyzing AI Observability Strategist specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Observability Strategist Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIObservabilityStrategistAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIObservabilityStrategistAgent = Object.freeze(
  new AIObservabilityStrategistAgent(),
);
