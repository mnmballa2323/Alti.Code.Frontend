...
class AIEngineeringOrchestratorAgent extends GeminiCliBaseAgent {
    ...
    async generateAIEngineeringSystem(objective) {
        logger.info(`💻 [AIEngineeringOrchestratorAgent] Analyzing AI Engineering Orchestrator specifications...`);
        ...
        try {
            const output = await this._invoke(prompt, "N/A - AI Engineering Orchestrator Target");
            return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
        } catch (err) {
            logger.error(`❌ [AIEngineeringOrchestratorAgent] Failed: ${err.message}`);
            throw err;
        }
    }
}
export const aIEngineeringOrchestratorAgent = Object.freeze(new AIEngineeringOrchestratorAgent());
