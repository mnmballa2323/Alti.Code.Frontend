...
class AIArchitectureOrchestratorAgent extends GeminiCliBaseAgent {
  ...
  async generateAIArchitectureSystem(objective) {
    logger.info(
      `💻 [AIArchitectureOrchestratorAgent] Analyzing AI Architecture Orchestrator specifications...`,
    );
    ...
    try {
      const output = await this._invoke(
        prompt,
        'N/A - AI Architecture Orchestrator Target',
      );
      return output.replace(/```[a-zA-Z0-9_-]*|```/gi, '').trim();
    } catch (err) {
      logger.error(
        `❌ [AIArchitectureOrchestratorAgent] Failed: ${err.message}`,
      );
      throw err;
    }
  }
}
export const aIArchitectureOrchestratorAgent = Object.freeze(
  new AIArchitectureOrchestratorAgent(),
);
