...
class FazmAgent extends BaseSpecialistAgent {
    ...
    async _invoke(context, contextBlock) {
        ...
        logger.info(`🤖 Fazm Operator: Receiving swarm intent for desktop automation: "${goal}"`);
    ...
    async runTask(userId, taskDescription) {
        if (typeof userId === 'object' && userId !== null && !taskDescription) {
            const args = userId;
            return FazmAgentService.runTask(args.userId || 'default-user', args.task || args.goal || args.query);
        }
    ...
    async getStatus(userId, taskId) {
        if (typeof userId === 'object' && userId !== null && !taskId) {
            const args = userId;
            return FazmAgentService.getStatus(args.userId || 'default-user', args.taskId);
        }
