...
function findDuplicates(arr1, arr2) {
    ...
    return matches;
            ...
            `)
            // Round 1 Optimizer
            .mockResolvedValueOnce(`This is an O(N^2) double-nested loop trap. Convert arr2 to a Set to achieve O(N) lookups.`)
            // Round 1 Skeptic
            .mockResolvedValueOnce(`Add input validation. What if arr1 or arr2 is null?`)
...
function findDuplicates(arr1, arr2) {
    ...
    return arr1.filter(item => lookup.has(item));
        ...
        const result = await debateService.converge('Write a function to find items in arr1 that exist in arr2 (intersection).');
...
function isEven(num) {
    ...
    return num % 2 === 0;
        ...
        const result = await debateService.converge('Write a secure, flawless function to check if a number is even.');
