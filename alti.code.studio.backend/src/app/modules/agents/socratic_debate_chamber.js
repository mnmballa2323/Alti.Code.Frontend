...
`;
            ...
            history.push(`[Round ${round} - Proposer (Claude)]:\n${refactoredProposal}`);
...
`;
            ...
            logger.info(`🗣️ [Socratic Debate] Auditor verdict: ${auditVerdict.split('\n')[0]}`);

            if (auditVerdict.includes('DECISION: APPROVED')) {
                logger.info(`🗣️ [Socratic Debate] Consensus reached and approved in Round ${round}!`);
                return {
                    consensus: true,
        ...
        logger.warn(`⚠️ [Socratic Debate] Failed to reach consensus within ${maxRounds} rounds. Returning last refactored state.`);
        return {
            consensus: false,
            approvedProposal: currentProposal,
            history
