...
class GooglePuppeteerAgent {
  ...
  async scrapeRenderedContext(url) {
    ...
    if (
      ...
      sentinelService.reportEvent(
        'ILLEGAL_NETWORK_TRAVERSAL_ATTEMPT',
        'CRITICAL',
        { url },
      );
    ...
    try {
      ...
      logger.info(
        `✅ [Puppeteer] Successfully extracted ${contextText.length} bytes of raw rendered context.`,
      );

      return contextText.substring(0, 50000); // Hard cap at 50k chars to prevent memory flooding
    } catch (error) {
      logger.error(
        `❌ [Puppeteer] Exploration failed for ${url}:`,
        error.message,
      );
