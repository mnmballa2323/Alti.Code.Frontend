...
export class DebateService {
    ...
    async converge(problemStatement, maxRounds = 3) {
        logger.info(`⚖️ DebateService: Initiating consensus loop for maximum ${maxRounds} rounds.`);
        ...
        while (attempt <= maxRounds) {
            ...
            currentDraft = rawDraft.replace(/^```[a-z]*\n/i, '').replace(/\n```$/i, '').trim();
            ...
            try {
                structuralComplexity = astService.calculateComplexity(currentDraft) || 1;
            } catch (e) {
            ...
            const optimizerCritique = await GeminiAiService.generateContent(optimizerPrompt);
            ...
            const skepticCritique = await GeminiAiService.generateContent(skepticPrompt);
            ...
            if (optimizerApproved && skepticApproved) {
                logger.info(`✅ DebateService: Reached full consensus on Round ${attempt}. Mathematical and structural approval granted.`);
                return {
                    code: currentDraft,
                    rounds: attempt,
                    consensus: true
            ...
            history = [];
            if (!optimizerApproved) history.push(`OPTIMIZER CRITIQUE: ${optimizerCritique}`);
            if (!skepticApproved) history.push(`SKEPTIC CRITIQUE: ${skepticCritique}`);

            logger.warn(`⚠️ DebateService: Consensus failed. Rejecting draft and sending critiques back to Proposer.`);
            attempt++;
        }

        logger.error(`❌ DebateService: Failed to reach consensus within ${maxRounds} rounds.`);
        return {
            code: currentDraft,
            rounds: maxRounds,
            consensus: false,
