 ...
 * neighborhood demographics, school data, hazard risk, and market trends.
 */
...
class AttomAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Real estate data specialist for ATTOM: property detail (bedrooms/baths/sqft/year built), ownership + deed history, sale comparables (comps), AVM (Automated Valuation Model), neighborhood demographics, school profile, flood/fire/wind/earthquake hazard risk, and rental estimates.';
...
Output production TypeScript. Store \`ATTOM_API_KEY\` server-side. Cache responses — property data rarely changes daily, reduce costs with TTL-based caching (Redis).`;
    ...
    async consult(prompt, contextData = []) {
        logger.info(`📊 ATTOM Expert: Synthesizing property data and analytics logic...`);
...
export const attomAgent = new AttomAgent();
