...
class SyntheticUserAgent extends BaseSpecialistAgent {
  constructor() {
    super({
      ...
      description:
        'Deploys vision-capable headless browsers to simulate human personas and identify UX dead-ends.',
      tier: 26,
      version: '6.8.0',
      capabilities: [
    ...
    this.personas = {
      impatient:
        'You are an impatient mobile user who clicks rapidly, scrolls haphazardly, and abandons flows if pages take >2 seconds to load.',
      elderly:
        'You are an elderly user viewing the site on a desktop. You prefer large click targets, high contrast, and slow, deliberate navigation.',
      screen_reader:
        'You are a legally blind user dependent on semantic HTML, aria-labels, and tab-key keyboard navigation.',
      power_user:
        'You are a developer power user who aggressively uses keyboard shortcuts, opens multiple tabs, and attempts to find edge cases.',
    };
  }
  ...
  async simulateUserSession(targetUrl, personaKey = 'power_user', taskGoal) {
    logger.info(
      `🤖 SyntheticUserAgent: Spawning demographic [${personaKey}] against ${targetUrl} to evaluate: "${taskGoal}"`,
    );
    ...
    const result = {
      ...
      telemetry: {
        ...
        a11yViolations:
          personaKey === 'screen_reader' ? Math.floor(Math.random() * 3) : 0,
      },
    };
  ...
  async execute(action, context) {
    if (!context)
      throw new Error('Context is required for Synthetic User operations');
    ...
    if (action === 'simulate') {
      ...
      const personaMatch = context.query?.match(
        /impatient|elderly|screen.reader|power.user/i,
      );
      const persona = personaMatch
        ? personaMatch[0].toLowerCase().replace(/[\s-]/g, '_')
        : 'power_user';

      const userSession = await this.simulateUserSession(
        targetUrl,
        persona,
        context.query,
      );
