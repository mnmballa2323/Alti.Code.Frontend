...
class AutonomousRefactorService {
    ...
    init() {
        ...
        this.isRunning = true;
        logger.info(`🔄 [OmniRefactor] Background Code Modernization Engine activated.`);
    ...
    async _refactorLoop() {
        while (this.isRunning) {
            try {
                logger.info(`🔄 [OmniRefactor] Initiating full workspace sweep for algorithm optimization...`);
                ...
                for (const filePath of shuffledFiles) {
                    ...
                    if (hasNestedLoops || hasNestedPromises) {
                        logger.warn(`⚠️ [OmniRefactor] Sub-optimal complexity detected in: ${path.basename(filePath)}`);
                        
                        // Dispatch the Performance Agent to rewrite the file
                        logger.info(`🧠 [OmniRefactor] Dispatching PerformanceAgent for O(1) mitigation...`);
                        ...
                        const generatedPatch = await performanceAgent._invoke(refactorGoal, content);
                        const cleanCode = generatedPatch.replace(/```javascript|```js|```/g, '').trim();

                        // Route through the Holodeck to prove the refactored code compiles securely
                        logger.info(`🧪 [OmniRefactor] Verifying O(1) refactored code in the Holodeck...`);
                        const simulation = await holodeckService.simulateExecution(cleanCode);

                        if (simulation.success) {
                            logger.info(`✅ [OmniRefactor] Holodeck validated performance patch. Proceeding with AST injection.`);
                            ...
                            logger.info(`✨ [OmniRefactor] Successfully optimized ${path.basename(filePath)} (Simulated Write)`);
                        } else {
                            logger.warn(`❌ [OmniRefactor] Performance patch failed Holodeck compilation. Abandoning patch.`);
                        }
                    }
                ...
                logger.info(`💤 [OmniRefactor] Workspace sweep complete. Engine cooling down...`);
                await new Promise(resolve => setTimeout(resolve, 1800000));
                
            } catch (err) {
                logger.error(`❌ [OmniRefactor] Engine encountered an error: ${err.message}`);
