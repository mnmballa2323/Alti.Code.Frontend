...
export class GeminiCliBaseAgent extends BaseSpecialistAgent {
  ...
  async _invoke(prompt, contextBlock, opts = {}) {
    ...
    if (extensionName) {
      logger.info(
        `⚡ [${this.name}] Invoking Gemini CLI Extension: ${extensionName}`,
      );
      try {
        return await GeminiExtensionService.invokeExtension(
          extensionName,
          extensionArgs,
        );
      } catch (extErr) {
        logger.error(
          `❌ [${this.name}] Extension ${extensionName} failed: ${extErr.message}`,
        );
        throw new AgentError(
          `Extension failed: ${extErr.message}`,
          'CLI_EXT_ERROR',
          false,
        );
      }
...
${safePrompt}
    ...
    const cachedOutput =
      await semanticCacheService.getCachedResponse(fullPayload);
    if (cachedOutput) {
      logger.info(
        `⚡ [${this.name}] [cid:${correlationId}] Elite Specialist Cache Hit! Bypassing Vertex AI.`,
      );
      return cachedOutput;
    }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        logger.info(
          `⚡ [${this.name}] [cid:${correlationId}] Executing Vertex AI generation via GcpGenAiService (Attempt ${attempt}/${maxRetries})...`,
        );

        // Use GcpGenAiService (Vertex AI integrated) instead of the raw native CLI
        const result = await GcpGenAiService.generateContent(fullPayload);
        const output = result.content;

        // 🧠 Store the successful outcome into the pgvector semantic cache
        semanticCacheService
          .setCachedResponse(fullPayload, output)
          .catch(() => {});

        return output;
      } catch (err) {
        logger.warn(
          `⚠️ [${this.name}] [cid:${correlationId}] CLI invocation attempt ${attempt} failed: ${err.message}`,
        );

        if (attempt === maxRetries) {
          logger.error(`❌ [${this.name}] All CLI retry attempts exhausted.`);
          throw new AgentError(
            `CLI execution failed after ${maxRetries} attempts: ${err.message}`,
            'CLI_ERROR',
            true,
          );
        }
