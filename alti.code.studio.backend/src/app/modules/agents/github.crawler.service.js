...
class GithubCrawlerService {
    ...
    init() {
        if (!process.env.GITHUB_PAT) {
            logger.warn('⚠️ [Sentinel Crawler] Missing GITHUB_PAT env var. Proactive Bug Bounty disabled.');
            return;
        }

        logger.info('🕷️ [Sentinel Crawler] Initialized. Will search GitHub for open vulnerabilities every 6 hours.');
    ...
    async huntForBugs() {
        ...
        logger.info('🕷️ [Sentinel Crawler] Starting proactive scan for Unpatched CVEs & Good First Issues...');

        try {
            // Search for open issues labeled "vulnerability", "security", or "bug" in popular repos
            const query = encodeURIComponent(`is:issue is:open label:security,vulnerability,"good first issue" created:>$(new Date(Date.now() - 7 * 24*60*60*1000).toISOString().split('T')[0]) sort:updated-desc`);
            ...
            for (const issue of issues) {
                ...
                logger.info('🕷️ [Sentinel Crawler] Handing off to Capability Router for autonomous resolution...');
                capabilityRouter.dispatch(prompt, [contextData]).catch(e => {
                    logger.error(`[Sentinel Crawler] Swarm failed to resolve issue ${issue.id}: ${e.message}`);
                });
            }
