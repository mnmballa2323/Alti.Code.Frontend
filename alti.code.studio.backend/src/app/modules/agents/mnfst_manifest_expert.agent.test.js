...
describe('MnfstManifestExpertAgent Unit Tests', () => {
    ...
    it('should initialize with correct name, description, and preamble', () => {
        ...
        expect(mnfstManifestExpertAgent.description).toContain('mnfst/manifest, the definition-first TypeScript framework');
        expect(mnfstManifestExpertAgent.preamble).toContain('Definition-First Backend Generation');
    });

    it('should successfully consult the LLM with context and prompt', async () => {
        const mockResponse = 'Manifest: Compiling postgres schema and custom form dashboards.';
