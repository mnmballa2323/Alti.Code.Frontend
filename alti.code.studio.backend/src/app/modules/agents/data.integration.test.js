...
describe('Modular Data Engineering & Stream Processing Declarative Agents & Swarm Router Integration', () => {
    describe('Recursive Declarative YAML Parsing & Ingestion', () => {
        ...
        it('should successfully parse and load Apache Spark & Large-Scale Batch Tuning Specialist', async () => {
            ...
            expect(def.name).toBe('Apache Spark & Large-Scale Batch Tuning Specialist');
            expect(def.capabilities).toContain('spark_shuffle_partition_optimization');
        ...
        it('should successfully parse and load dbt (Data Build Tool) Analytics Engineer', async () => {
            ...
            expect(def.capabilities).toContain('incremental_materialization_strategies');
        ...
        it('should successfully parse and load Apache Pulsar & Multi-Tenant Messaging Specialist', async () => {
            ...
            expect(def.name).toBe('Apache Pulsar & Multi-Tenant Messaging Specialist');
    ...
    describe('Precision Swarm Routing for All 10 Data Engineering & Stream Processing Agents', () => {
        it('should route Kafka keywords to the Kafka tuner agent', async () => {
            const output = 'Resolve consumer_lag_mitigation policies and coordinate schema_registry_evolution setups';
            ...
            expect(agentIds).toContain('Apache Kafka & Event Stream Tuning Specialist');
        });

        it('should route Spark keywords to the Spark optimizer agent', async () => {
            const output = 'Audit spark_shuffle_partition_optimization configurations and minimize disk_spill_mitigation errors';
            const { strategy, sequence } = await agenticRouter.routeDownstreamSwarm(output);

            expect(strategy).toBe('Data Engineering Swarm: Apache Spark Optimization');
            const agentIds = sequence.map(s => s.agentId);
            expect(agentIds).toContain('Apache Spark & Large-Scale Batch Tuning Specialist');
        });

        it('should route Flink keywords to the Flink stream agent', async () => {
            const output = 'Create stateful_stream_computations logic and run exactly_once_checkpointing tasks';
            const { strategy, sequence } = await agenticRouter.routeDownstreamSwarm(output);
        ...
        it('should route Snowflake keywords to the Snowflake warehouse agent', async () => {
            const output = 'Perform search_optimization_services updates and check zero_copy_cloning_layouts layouts';
            const { strategy, sequence } = await agenticRouter.routeDownstreamSwarm(output);

            expect(strategy).toBe('Data Engineering Swarm: Snowflake Data Warehousing');
            const agentIds = sequence.map(s => s.agentId);
            expect(agentIds).toContain('Snowflake & Enterprise Data Warehouse Architect');
        });

        it('should route dbt keywords to the dbt modeler agent', async () => {
            const output = 'Implement incremental_materialization_strategies views and compile source_freshness_validations checks';
            const { strategy, sequence } = await agenticRouter.routeDownstreamSwarm(output);

            expect(strategy).toBe('Data Engineering Swarm: dbt Analytics Engineering');
            const agentIds = sequence.map(s => s.agentId);
        ...
        it('should route Airflow keywords to the Airflow governor agent', async () => {
            const output = 'Build dynamic_dag_generation scripts and configure custom_sla_callbacks SLA triggers';
            const { strategy, sequence } = await agenticRouter.routeDownstreamSwarm(output);

            expect(strategy).toBe('Data Engineering Swarm: Apache Airflow Orchestration');
            const agentIds = sequence.map(s => s.agentId);
        ...
        it('should route ClickHouse keywords to the ClickHouse OLAP agent', async () => {
            const output = 'Tune mergetree_engine_tuning properties and compile materialized_views_opt dashboards';
            const { strategy, sequence } = await agenticRouter.routeDownstreamSwarm(output);
        ...
        it('should route Iceberg keywords to the Iceberg open table agent', async () => {
            const output = 'Launch acid_transactions_lakehouse commits and monitor metadata_file_compaction jobs';
            const { strategy, sequence } = await agenticRouter.routeDownstreamSwarm(output);

            expect(strategy).toBe('Data Engineering Swarm: Apache Iceberg Lakehouse');
            const agentIds = sequence.map(s => s.agentId);
            expect(agentIds).toContain('Apache Iceberg & Open Table Format Specialist');
        });

        it('should route Data Lineage keywords to the governance officer agent', async () => {
            const output = 'Scan metadata_scanning_maps sources and track column_level_lineage_tracking changes';
            const { strategy, sequence } = await agenticRouter.routeDownstreamSwarm(output);
        ...
        it('should route Pulsar keywords to the Pulsar multi-tenant broker agent', async () => {
            const output = 'Define pulsar_multi_tenancy_namespaces parameters and setup pulsar_geo_replication synchronizations';
            const { strategy, sequence } = await agenticRouter.routeDownstreamSwarm(output);

            expect(strategy).toBe('Data Engineering Swarm: Apache Pulsar Messaging');
            const agentIds = sequence.map(s => s.agentId);
            expect(agentIds).toContain('Apache Pulsar & Multi-Tenant Messaging Specialist');
        });
    });
});
