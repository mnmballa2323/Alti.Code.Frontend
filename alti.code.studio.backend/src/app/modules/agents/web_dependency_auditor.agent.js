...
class WebDependencyAuditorAgent extends BaseSpecialistAgent {
  ...
  async _invoke(context, contextBlock) {
    ...
    if (!packageName) {
      throw new Error(
        'Web dependency auditor requires a package name or search query.',
      );
    }

    logger.info(
      `🤖 Web Dependency Auditor: Auditing package: "${packageName}"`,
    );
    ...
    try {
      ...
      logger.error(
        `❌ Web Dependency Auditor: Browser automation failed: ${error.message}`,
      );
      throw error;
    }
  }
