...
describe('AgentshieldSecurityAgent Unit Tests', () => {
    ...
    it('should initialize with correct name, description, and preamble', () => {
        ...
        expect(agentshieldSecurityAgent.description).toContain('Zero-Trust IAM, Container, & Cloud Security Auditor Specialist');
        expect(agentshieldSecurityAgent.preamble).toContain('ZERO-TRUST CONTAINER, IAM, & CLOUD SECURITY AUDITOR');
    });

    it('should successfully consult the LLM with context and prompt', async () => {
        const mockResponse = 'Audit Report: Dockerfile passes non-root checks. IAM Role has optimal PoLP mappings.';
        ...
        expect(generatedPrompt).toContain('ZERO-TRUST CONTAINER, IAM, & CLOUD SECURITY AUDITOR');
        expect(generatedPrompt).toContain('Audit docker container execution boundary');
    });
    ...
    it('should handle API errors and trigger retries in BaseSpecialistAgent', async () => {
        ...
        const result = await agentshieldSecurityAgent.consult('Scan IAM configurations', []);
