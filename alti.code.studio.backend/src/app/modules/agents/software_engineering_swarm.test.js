...
describe('5-Agent Collaborative Developer Swarm with Crabbox Remote Execution', () => {
    ...
    it('should successfully run the entire 5-agent SDLC pipeline executing via Crabbox', async () => {
        ...
        const runSpy = vi.spyOn(crabboxService, 'run').mockImplementation(async (command, opts = {}) => {
            const scriptPath = path.join(opts.cwd, command.replace('node ', ''));
            
            return new Promise((resolve) => {
                cp.exec(`node ${scriptPath}`, { cwd: opts.cwd }, (error, stdout, stderr) => {
                    resolve({
                        success: !error,
                        stdout: stdout,
                        stderr: stderr,
        ...
        const destroySpy = vi.spyOn(WorkspaceIsolator.prototype, 'destroy').mockImplementation(() => {});
        ...
        const userRequest = [{ role: 'user', content: 'Build token rotation service' }];
        const result = await engine.run(SwarmArchitectAgent, userRequest, contextVariables, 6);
