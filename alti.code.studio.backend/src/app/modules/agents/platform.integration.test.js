...
describe('Modular Platform Engineering, IaC & Cloud-Native Systems Swarm Router Integration', () => {
  describe('Recursive Declarative YAML Parsing & Ingestion', () => {
    ...
    it('should successfully parse and load Prometheus Telemetry & Alerting Specialist', async () => {
      const agentPath = path.join(
        PLATFORM_DIR,
        'platform.prometheus.agent.yaml',
      );
      const def = parseYaml(await fs.readFile(agentPath, 'utf8'));
    ...
    it('should successfully parse and load Crabbox Remote Sandbox & Secure Execution Specialist', async () => {
      ...
      expect(def.name).toBe(
        'Crabbox Remote Sandbox & Secure Execution Specialist',
      );
  ...
  describe('Precision Swarm Routing for All 11 Platform Engineering & IaC Agents', () => {
    it('should route Terraform keywords to Terraform State & IaC Optimizer agent', async () => {
      const output =
        'Perform terraform_state_lock_auditing validations and check hcl_syntax_linting formats';
    ...
    it('should route Ansible keywords to Ansible Configuration & Playbook Specialist agent', async () => {
      const output =
        'Examine dynamic_inventory_mapping mappings and ensure task_idempotent_validations are verified';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);

      expect(strategy).toBe(
        'Platform Engineering Swarm: Ansible Configurations',
      );
      const agentIds = sequence.map(s => s.agentId);
    ...
    it('should route Helm keywords to Helm Chart & Package Manager Architect agent', async () => {
      const output =
        'Compile values_overlays_structure files and resolve dynamic_template_dependencies dynamic configurations';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);
    ...
    it('should route Docker keywords to Docker Engine & Host Virtualization Specialist agent', async () => {
      const output =
        'Set docker_socket_security_access credentials and check docker_daemon_configurations JSON settings';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);

      expect(strategy).toBe(
        'Platform Engineering Swarm: Docker & Virtualization',
      );
      const agentIds = sequence.map(s => s.agentId);
      expect(agentIds).toContain(
        'Docker Engine & Host Virtualization Specialist',
      );
    });

    it('should route Consul keywords to Consul Service Discovery & Key-Value Specialist agent', async () => {
      const output =
        'Setup catalog_synchronization_periods metrics and check raft_consensus_cluster_configs settings';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);

      expect(strategy).toBe(
        'Platform Engineering Swarm: Consul Discovery & KV',
      );
      const agentIds = sequence.map(s => s.agentId);
      expect(agentIds).toContain(
        'Consul Service Discovery & Key-Value Specialist',
      );
    });

    it('should route Prometheus keywords to Prometheus Telemetry & Alerting Specialist agent', async () => {
      const output =
        'Tweak metrics_scraping_intervals rates and adjust alertmanager_configurations endpoints';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);
    ...
    it('should route NGINX keywords to NGINX Reverse Proxy & HTTP Engine Specialist agent', async () => {
      const output =
        'Optimize server_proxy_blocks_configurations values and enable ssl_session_caching params';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);

      expect(strategy).toBe(
        'Platform Engineering Swarm: NGINX Proxy & Reverse Web',
      );
      const agentIds = sequence.map(s => s.agentId);
      expect(agentIds).toContain(
        'NGINX Reverse Proxy & HTTP Engine Specialist',
      );
    });

    it('should route ArgoCD keywords to ArgoCD GitOps Git Sync Specialist agent', async () => {
      const output =
        'Inspect application_synchronization_queues sizes and build applicationset_matrix_generators dynamic matrices';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);
    ...
    it('should route Envoy keywords to Envoy Proxy & Service Mesh Specialist agent', async () => {
      const output =
        'Monitor dynamic_cluster_discovery_xds stats and activate custom_filters_wasm_injection filters';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);

      expect(strategy).toBe(
        'Platform Engineering Swarm: Envoy Mesh & Sidecars',
      );
      const agentIds = sequence.map(s => s.agentId);
    ...
    it('should route Packer keywords to Packer Machine Image & Template Specialist agent', async () => {
      const output =
        'Compile vm_golden_image_builds pipelines and enforce parallel_build_optimizations options';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);

      expect(strategy).toBe(
        'Platform Engineering Swarm: Packer Machine Images',
      );
      const agentIds = sequence.map(s => s.agentId);
    ...
    it('should route Crabbox keywords to Crabbox Remote Sandbox & Secure Execution Specialist agent', async () => {
      const output =
        'Perform remote_lease_lifecycle_management checks and execute workspace_diff_synchronization operations';
      const { strategy, sequence } =
        await agenticRouter.routeDownstreamSwarm(output);
