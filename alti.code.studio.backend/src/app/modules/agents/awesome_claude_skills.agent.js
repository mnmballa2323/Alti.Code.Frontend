...
class AwesomeClaudeSkillsAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'The Awesome Claude Skills Catalog Specialist. Indexes and executes reusable instruction packages and local MCP automation workflows.';
        this.preamble = `You are the Awesome Claude Skills Catalog Specialist.
Your focus is to parse, select, and compile reusable markdown-based instruction playbooks and local MCP integration configurations.

OPERATIONAL PARAMETERS:
1. **Catalog Parsing**: You recursively scan submodules/awesome-claude-skills to discover verified community skills.
2. **App Automation**: You configure handshakes and tool invocation schemas for local MCP services.
3. **Execution Delivery**: You load and inject selected skills dynamically into prompt context templates.
`;
    }

    async _invoke(prompt, contextBlock) {
        return `🔌 **Awesome Claude Skills Catalog Report:**\n- Ingested: submodules/awesome-claude-skills/\n- Match: Located optimal local MCP integration configuration.\n- Status: Active.`;
    }
}
...
agentRegistry.register({
    ...
    capabilities: ['skills-catalog', 'saas-automation', 'mcp-recipes'],
    version: '1.0.0',
