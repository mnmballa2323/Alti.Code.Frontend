...
class EphemeralAgentSynthesizer {
    constructor() {
        ...
        logger.info('🧬 EphemeralAgentSynthesizer initialized. Ready for dynamic context generation.');
    }
    ...
    _extractHeuristics(codeBuffer) {
        ...
        if (content.includes('react') || content.includes('jsx')) stackNodes.push('React DOM/JSX');
        if (content.includes('next') || content.includes('getserversideprops')) stackNodes.push('Next.js App/Pages Router');
        if (content.includes('prisma') || content.includes('schema.prisma')) stackNodes.push('Prisma ORM');
        if (content.includes('drizzle')) stackNodes.push('Drizzle ORM');
        if (content.includes('express') || content.includes('app.use(')) stackNodes.push('Express.js Backend');
        if (content.includes('fastapi')) stackNodes.push('FastAPI/Pydantic');
        if (content.includes('eks') || content.includes('apiVersion: v1')) stackNodes.push('Kubernetes / GKE');
        if (content.includes('terraform') || content.includes('resource "aws_')) stackNodes.push('Terraform AWS Provider');
        if (content.includes('class') && content.includes('public static void')) stackNodes.push('Java Enterprise JVM');

        return stackNodes.length > 0 ? stackNodes : ['Vanilla ECMAScript / Generic Backend'];
    }
    ...
    async synthesize(context) {
        ...
        logger.info(`🧬 Synthesizing ephemeral agent for context: ${activeFilePath}`);
        ...
        const hash = crypto.createHash('sha256').update(activeFileCode + activeFilePath).digest('hex').substring(0, 8);
...
`;
        ...
        const transientKeywords = [hash, activeFilePath.split('/').pop(), 'synthesize route'];
        capabilityRouter.registerAgent(syntheticAgent, transientKeywords);

        logger.info(`✨ Successfully mounted ${agentName} (Stack: ${stackString}) to Capability Router.`);
