...
class SkillRegistry {
    ...
    async registerSkill(namespace, skillDefinition, sourcePath) {
        ...
        if (!scanResult.isSafe) {
            throw new Error(`[SkillRegistry] Validation failed for ${namespace}. Malicious intent detected.`);
        }

        // 2. Normalize and Register
        console.log(`[SkillRegistry] Registering validated skill namespace: ${namespace}`);
        ...
        console.log(`[SkillRegistry] Active namespaces: ${Array.from(this.namespaces).join(', ')}`);
        return true;
    }
