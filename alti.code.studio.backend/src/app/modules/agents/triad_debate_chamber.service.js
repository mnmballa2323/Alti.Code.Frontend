...
class TriadDebateChamberService {
    ...
    async initiateDebate(objective) {
        logger.info(`🏛️ [The Triad] Convening Adversarial Debate Chamber for objective: "${objective.substring(0, 50)}..."`);

        try {
            // ── Phase 1: Independent Generation ──
            logger.info(`🐺 [The Triad] Alpha (The Innovator) is formulating a cutting-edge solution...`);
            ...
            logger.info(`🛡️ [The Triad] Beta (The Pragmatist) is formulating a highly resilient, enterprise solution...`);
            ...
            logger.info(`⚔️ [The Triad] Forcing Cross-Examination. Alpha and Beta are tearing apart each other's architectures...`);
            ...
            const alphaCritique = await GeminiAiService.generateContent(alphaCritiquePrompt);
            ...
            const betaCritique = await GeminiAiService.generateContent(betaCritiquePrompt);

            // ── Phase 3: The Arbiter Synthesis ──
            logger.info(`⚖️ [The Triad] Gamma (The Arbiter) is synthesizing the final, mathematically optimal consensus...`);
            ...
            logger.info(`✅ [The Triad] Mathematical Consensus Reached. Dissolving Chamber.`);
