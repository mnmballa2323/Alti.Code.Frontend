...
class AgentCapabilityIndex {
    ...
    _buildIndex() {
        ...
        logger.info(`📋 AgentIndex: Indexed ${this.entries.length} specialist agents.`);
    }
     ...
     */
    stats() {
        const totalKeywords = this.entries.reduce((acc, e) => acc + e.keywords.length, 0);
        return {
            totalAgents: this.entries.length,
            totalKeywords,
            avgKeywordsPerAgent: (totalKeywords / (this.entries.length || 1)).toFixed(1),
    ...
    rebuild() {
        this._buildIndex();
        logger.info(`📋 AgentIndex: Rebuilt — ${this.entries.length} agents indexed.`);
    }
}
