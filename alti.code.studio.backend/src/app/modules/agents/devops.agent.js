...
export class DevopsAgent {
  constructor() {
    ...
    logger.info(
      '🌩️ DevopsAgent: Autonomous Infrastructure-as-Code Architect online.',
    );
  }
  ...
  async provisionInfrastructure(rawRequest, terraformDir, envPath) {
    logger.info(
      `🌩️ DevopsAgent: Analyzing architecture prompt: "${rawRequest}"`,
    );
    ...
    logger.info(
      `🌩️ DevopsAgent: Generated \`main.tf\` configuration in ${terraformDir}`,
    );
  ...
  async _mockTerraformApply(tfFilePath) {
    logger.info(
      `🌩️ [MOCK SHELL] Executing: \`cd ${path.dirname(tfFilePath)} && terraform init\``,
    );
  ...
  async _injectEnvOutputs(envPath, newVarsObj) {
    ...
    logger.info(
      `🌩️ DevopsAgent: Successfully injected new environment variables into ${path.basename(envPath)}`,
    );
  }
}
