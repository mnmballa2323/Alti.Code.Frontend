...
class BackgroundAgentService {
  constructor() {
    this.controlPlaneUrl =
      process.env.OPEN_INSPECT_CONTROL_PLANE_URL ||
      'https://api.open-inspect.mock';
  ...
  async dispatchSession(prompt, contextBlock, options = {}) {
    logger.info(
      `🌐 [BackgroundAgentService] Dispatching session to Open-Inspect...`,
    );
    logger.debug(
      `Prompt length: ${prompt.length}, Context length: ${contextBlock ? contextBlock.length : 0}`,
    );
    ...
    try {
      ...
      logger.info(
        `🌐 [BackgroundAgentService] Session ${sessionId} successfully created on Control Plane.`,
      );
      ...
      logger.error(
        `❌ [BackgroundAgentService] Failed to create session: ${error.message}`,
      );
      throw error;
    }
  }
