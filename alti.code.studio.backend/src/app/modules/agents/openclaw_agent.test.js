...
describe('OpenClawAgent Operations with Socratic Self-Healing & DB Sync', () => {
    ...
    it('should execute successfully on standard local host tunnel delegation', async () => {
        ...
        openclawProxyService.delegateToLocalHost.mockResolvedValueOnce('stdout test results');
        ...
        expect(openclawProxyService.delegateToLocalHost).toHaveBeenCalledWith('Fetch Jira ticket', expect.any(String));
    ...
    it('should trigger local socratic debate & self-healing when execution fails, evolve intent, register success in DB, and succeed on retry', async () => {
        ...
        expect(openclawProxyService.delegateToLocalHost).toHaveBeenNthCalledWith(1, 'Fetch Jira ticket', expect.any(String));
        expect(openclawProxyService.delegateToLocalHost).toHaveBeenNthCalledWith(2, 'Corrected fetch Jira ticket', expect.any(String));
