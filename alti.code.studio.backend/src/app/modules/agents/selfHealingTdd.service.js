...
const runTddLoop = async (
  ...
  logger.info(
    `🧪 [TDD Loop] Starting Self-Healing TDD Execution for: ${featurePrompt}`,
  );
  ...
  let currentCode = codeResult.reply
  ...
  let currentTest = testResult.reply
    .replace(/```javascript/g, '')
    .replace(/```/g, '');
  ...
  for (let i = 1; i <= maxIterations; i++) {
    logger.info(
      `🔄 [TDD Loop] Iteration ${i}/${maxIterations}: Running tests in Docker sandbox...`,
    );
...
Analyze the stack trace and fix the logic. Return ONLY the fully updated raw javascript implementation code, no markdown or explanations.`;
  ...
  logger.error(
    `❌ [TDD Loop] Failed to self-heal within ${maxIterations} iterations.`,
  );
  return {
    success: false,
    iterations: maxIterations,
    code: currentCode,
    error: 'Exceeded max iterations without passing tests.',
  };
};
