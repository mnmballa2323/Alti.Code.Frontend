...
class VoiceCalendarSchedulerAgent extends BaseSpecialistAgent {
  ...
  async _invoke(context, contextBlock) {
    ...
    if (typeof context === 'object' && context !== null) {
      scheduleRequest =
        context.scheduleRequest || context.goal || context.query;
    ...
    if (!scheduleRequest) {
      throw new Error(
        'Voice calendar scheduler requires calendar event scheduling instructions.',
      );
    }

    logger.info(
      `🤖 Voice Calendar Scheduler: Mapping schedule request: "${scheduleRequest}"`,
    );
    ...
    try {
      ...
      logger.error(
        `❌ Voice Calendar Scheduler: Voice scheduling failed: ${error.message}`,
      );
      throw error;
    }
  }
