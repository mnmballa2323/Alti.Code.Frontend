...
const evolveInstincts = async (userId, sessionId, userFeedbackLog) => {
    logger.info(`🧠 [Evolution Engine] Analyzing user interaction logs to forge new Instincts...`);
...
Return ONLY the raw instruction string. If no strong pattern is found, return "NO_EVOLUTION".`;
        ...
        const result = await LlmGatewayService.routeCompletion(
            userId, sessionId, evolutionPrompt, 'gpt-5.5', 'Instinct-Evolution'
        );
        ...
        logger.warn(`🧬 [Evolution Engine] New Instinct Discovered! Forging memory...`);
        ...
        await fs.writeFile(path.join(instinctDir, `${instinctId}.md`), formattedRule);

        logger.info(`✅ [Evolution Engine] Instinct successfully burned into core memory (${instinctId}.md).`);
