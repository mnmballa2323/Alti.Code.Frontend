...
class SuperpowerSkillAgent extends BaseSpecialistAgent {
    constructor(config) {
        super(config.name, config.title || config.name, 'Tier 18: Superpowers Framework');
        this.description = config.description || 'Superpowers Agentic Skill Framework';
...
class SuperpowersLoaderService {
    constructor() {
        ...
        this.skillsRootDir = path.resolve(process.cwd(), '../submodules/superpowers/skills');
        this.loadedSkills = new Set();
    }

    init() {
        logger.info('🦸 SuperpowersLoaderService: Initializing Superpowers Agentic Skills Framework...');
        this.loadAll();
    }
    ...
    loadAll() {
        try {
            if (!fs.existsSync(this.skillsRootDir)) {
                logger.warn(`Superpowers Skills directory not found at ${this.skillsRootDir}. Skipping.`);
                return;
            }
    ...
    loadSkill(filePath, skillName) {
        try {
            ...
            const keywords = ['superpower', 'obra', skillName.replace(/-/g, ' '), 'framework', 'workflow'];
            ...
            logger.info(`✨ Ingested Superpower Protocol [${config.title}] into Swarm Capability Router.`);

        } catch (e) {
            logger.error(`[SuperpowersLoader Loader] Failed to parse skill ${skillName}: ${e.message}`);
        }
    }
}
