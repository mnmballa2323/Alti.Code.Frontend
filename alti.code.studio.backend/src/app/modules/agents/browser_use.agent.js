...
class BrowserUseAgent extends BaseSpecialistAgent {
    ...
    async _invoke(context, contextBlock) {
        ...
        logger.info(`🤖 Browser Operator: Receiving swarm intent for browser automation control: "${goal}"`);
        ...
        try {
            ...
            logger.error(`❌ Browser Operator: Browser-Use execution failed: ${error.message}`);
            throw error;
        }
    }
    ...
    async runBrowserTask(taskDescription, options = {}) {
        if (typeof taskDescription === 'object' && taskDescription !== null) {
            ...
            return BrowserUseAgentService.runBrowserTask(args.task || args.goal || args.query, taskOptions);
        }
