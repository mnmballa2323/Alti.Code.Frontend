...
class GithubFnGithubActionsRunnerGroupsManagerAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub Actions Runner Groups Manager expert in structuring secure runner groups and organizing compute resources.';
        this.manifest = {
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubActionsRunnerGroupsManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Actions CI/CD runner groups, enterprise runner groups, access policies ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubActionsRunnerGroupsManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
...
export const pluginInstance = new GithubFnGithubActionsRunnerGroupsManagerAgent();
