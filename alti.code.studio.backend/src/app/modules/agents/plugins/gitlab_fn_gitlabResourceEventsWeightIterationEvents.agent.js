...
class GitlabFnGitlabResourceEventsWeightIterationEventsAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitLab Resource Events Weight/Iteration Auditor expert in weight and sprint transitions.';
        this.manifest = {
            id: 'gitlabResourceEventsWeightIterationEvents',
            capabilities: ["gitlab-get-issue-weight-event","gitlab-get-issue-iteration-event"],
            version: '39.6.0'
        };
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
    ...
    async _invoke(prompt, contextBlock, tenantId, spanId) {
        ...
        logger.info(`🦊 [gitlabResourceEventsWeightIterationEvents] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await gitlabDocsService.searchDocs(`GitLab Milestones & Iterations weight events history, iteration events history, event auditing ${prompt}`, 5, tenantId);
        } catch (err) {
            logger.warn(`🦊 [gitlabResourceEventsWeightIterationEvents] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
        ...
        try {
            ...
            logger.debug(`🦊 [gitlabResourceEventsWeightIterationEvents] Memory retrieval bypassed: ${memErr.message}`);
        }
...
${prompt}`;
        ...
        if (response.includes('```') && !prompt.includes('no-reflect')) {
            logger.info(`🦊 [gitlabResourceEventsWeightIterationEvents] Code blocks detected. Initiating automated reflection loop...`);
            try {
                const reflectionPrompt = `You are the critic evaluator for [gitlabResourceEventsWeightIterationEvents].
...
${response}`;
                ...
                if (!reviewResult.includes('APPROVED')) {
                    logger.info(`🦊 [gitlabResourceEventsWeightIterationEvents] Reflection loop identified feedback. Correcting response...`);
...
Please correct the previous attempt based on the feedback above.`;
                ...
                logger.warn(`🦊 [gitlabResourceEventsWeightIterationEvents] Reflection loop failed (non-blocking): ${reflectErr.message}`);
            }
        }
        ...
        try {
            ...
            if (agentMemoryService.isReady) {
                agentMemoryService.observe({
                    content: `Agent [gitlabResourceEventsWeightIterationEvents] processed query: "${prompt.substring(0, 150)}..." and generated output.`,
                    type: 'agent_consultation',
                    metadata: { agentId: 'gitlabResourceEventsWeightIterationEvents', query: prompt }
                }).catch(() => {});
            }
        } catch (observeErr) {
            logger.debug(`🦊 [gitlabResourceEventsWeightIterationEvents] Storing consultation observation failed (non-blocking): ${observeErr.message}`);
        }
...
export const pluginInstance = new GitlabFnGitlabResourceEventsWeightIterationEventsAgent();
