...
class GithubFnGithubIssuesCommentsManagerAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub Issues Comments Manager expert in issue feedback comments and threaded discussions.';
        this.manifest = {
            id: 'githubIssuesCommentsManager',
            capabilities: ["github-create-issue-comment"],
            version: '39.6.0'
        };
        this.preamble = `You are the Alti.Code.Studio Specialist GitHub Issues Comments Manager expert in issue feedback comments and threaded discussions.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubIssuesCommentsManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Issues issue comments, issue conversation threads ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubIssuesCommentsManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
