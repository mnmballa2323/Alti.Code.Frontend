...
class GithubFnGithubAppWebhookVerifierAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.preamble = `You are the Inso Code Specialist GitHub App Webhook Verifier expert in cryptographically verifying incoming event headers using HMAC-SHA256.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubAppWebhookVerifier] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Apps & Integrations x-hub-signature-256 header validation, HMAC-SHA256 secret verification ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubAppWebhookVerifier] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
