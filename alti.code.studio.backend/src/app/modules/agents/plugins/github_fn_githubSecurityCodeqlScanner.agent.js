...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubSecurityCodeqlScanner] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Security & Compliance codeql security analyses, static application security testing (SAST) ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubSecurityCodeqlScanner] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
