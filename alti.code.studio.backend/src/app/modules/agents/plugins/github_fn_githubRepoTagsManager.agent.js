...
class GithubFnGithubRepoTagsManagerAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub Repository Tags Manager expert in managing annotated tags, protected tag rules, and repository release labels.';
        this.manifest = {
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubRepoTagsManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Repositories tag creation, annotated tags, release tags, tag protections ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubRepoTagsManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
