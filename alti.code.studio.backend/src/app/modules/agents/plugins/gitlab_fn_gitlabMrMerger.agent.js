...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
  ...
  async _invoke(prompt, contextBlock, tenantId, spanId) {
    ...
    try {
      ...
      logger.debug(
        `🦊 [gitlabMrMerger] Memory retrieval bypassed: ${memErr.message}`,
      );
    }
...
${prompt}`;
    ...
    if (response.includes('```') && !prompt.includes('no-reflect')) {
      logger.info(
        `🦊 [gitlabMrMerger] Code blocks detected. Initiating automated reflection loop...`,
      );
      try {
...
${response}`;
        ...
        if (!reviewResult.includes('APPROVED')) {
          logger.info(
            `🦊 [gitlabMrMerger] Reflection loop identified feedback. Correcting response...`,
          );
...
Please correct the previous attempt based on the feedback above.`;
        ...
        logger.warn(
          `🦊 [gitlabMrMerger] Reflection loop failed (non-blocking): ${reflectErr.message}`,
        );
      }
    }
    ...
    try {
      ...
      logger.debug(
        `🦊 [gitlabMrMerger] Storing consultation observation failed (non-blocking): ${observeErr.message}`,
      );
    }
