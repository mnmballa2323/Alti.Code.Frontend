...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubProjectsBoardSyncer] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Projects v2 syncing projects boards with issue/pr status ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubProjectsBoardSyncer] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
