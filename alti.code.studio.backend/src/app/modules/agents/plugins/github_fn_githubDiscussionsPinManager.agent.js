...
class GithubFnGithubDiscussionsPinManagerAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.preamble = `You are the Inso Code Specialist GitHub Discussions Pin Manager expert in spotlighting priority community announcements.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubDiscussionsPinManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Discussions pinning discussions, spotlighting announcements ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubDiscussionsPinManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
