...
class GitlabFnGitlabIssueCommenterAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitLab Issue Commenter expert in handling conversational notes and comments on issues.';
        this.manifest = {
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
    ...
    async _invoke(prompt, contextBlock, tenantId, spanId) {
        ...
        logger.info(`🦊 [gitlabIssueCommenter] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await gitlabDocsService.searchDocs(`GitLab Issues issue comments, note discussions, comment threading ${prompt}`, 5, tenantId);
        } catch (err) {
            logger.warn(`🦊 [gitlabIssueCommenter] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
        ...
        try {
            ...
            logger.debug(`🦊 [gitlabIssueCommenter] Memory retrieval bypassed: ${memErr.message}`);
        }
...
${prompt}`;
        ...
        if (response.includes('```') && !prompt.includes('no-reflect')) {
            logger.info(`🦊 [gitlabIssueCommenter] Code blocks detected. Initiating automated reflection loop...`);
            try {
                const reflectionPrompt = `You are the critic evaluator for [gitlabIssueCommenter].
...
${response}`;
                ...
                if (!reviewResult.includes('APPROVED')) {
                    logger.info(`🦊 [gitlabIssueCommenter] Reflection loop identified feedback. Correcting response...`);
...
Please correct the previous attempt based on the feedback above.`;
                ...
                logger.warn(`🦊 [gitlabIssueCommenter] Reflection loop failed (non-blocking): ${reflectErr.message}`);
            }
        }
        ...
        try {
            ...
            if (agentMemoryService.isReady) {
                agentMemoryService.observe({
                    content: `Agent [gitlabIssueCommenter] processed query: "${prompt.substring(0, 150)}..." and generated output.`,
                    type: 'agent_consultation',
                    metadata: { agentId: 'gitlabIssueCommenter', query: prompt }
                }).catch(() => {});
            }
        } catch (observeErr) {
            logger.debug(`🦊 [gitlabIssueCommenter] Storing consultation observation failed (non-blocking): ${observeErr.message}`);
        }
