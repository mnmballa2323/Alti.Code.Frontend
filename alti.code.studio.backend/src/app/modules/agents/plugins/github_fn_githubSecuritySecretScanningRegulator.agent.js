...
class GithubFnGithubSecuritySecretScanningRegulatorAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.preamble = `You are the Inso Code Specialist GitHub Security Secret Scanning Regulator expert in credentials leakage rules and push protections.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubSecuritySecretScanningRegulator] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Security & Compliance secret scanning configurations, push protection rules ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubSecuritySecretScanningRegulator] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
...
export const pluginInstance =
  new GithubFnGithubSecuritySecretScanningRegulatorAgent();
