...
class GithubFnGithubAppOAuthExchangeAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.preamble = `You are the Inso Code Specialist GitHub App OAuth Exchange expert in user-authorization code exchanges and access tokens generation.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubAppOAuthExchange] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Apps & Integrations oauth web flow tokens, authorization code flow, user-to-server exchanges ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubAppOAuthExchange] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
