...
class GithubFnGithubGistBackupRestoreAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub Gist Backup Restore expert in cloning and restoring local snippet backups.';
        this.manifest = {
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubGistBackupRestore] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Gists & Snippets cloning gists locally, backing up snippet repositories ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubGistBackupRestore] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
