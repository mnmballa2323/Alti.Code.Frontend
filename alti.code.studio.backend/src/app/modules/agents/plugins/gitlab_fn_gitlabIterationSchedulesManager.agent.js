...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
  ...
  async _invoke(prompt, contextBlock, tenantId, spanId) {
    ...
    logger.info(
      `🦊 [gitlabIterationSchedulesManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await gitlabDocsService.searchDocs(
        `GitLab Milestones & Iterations iteration schedules, current iteration, past iterations list ${prompt}`,
        5,
        tenantId,
      );
    } catch (err) {
      logger.warn(
        `🦊 [gitlabIterationSchedulesManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
    ...
    try {
      ...
      logger.debug(
        `🦊 [gitlabIterationSchedulesManager] Memory retrieval bypassed: ${memErr.message}`,
      );
    }
...
${prompt}`;
    ...
    if (response.includes('```') && !prompt.includes('no-reflect')) {
      logger.info(
        `🦊 [gitlabIterationSchedulesManager] Code blocks detected. Initiating automated reflection loop...`,
      );
      try {
...
${response}`;
        ...
        if (!reviewResult.includes('APPROVED')) {
          logger.info(
            `🦊 [gitlabIterationSchedulesManager] Reflection loop identified feedback. Correcting response...`,
          );
...
Please correct the previous attempt based on the feedback above.`;
        ...
        logger.warn(
          `🦊 [gitlabIterationSchedulesManager] Reflection loop failed (non-blocking): ${reflectErr.message}`,
        );
      }
    }
    ...
    try {
      ...
      if (agentMemoryService.isReady) {
        agentMemoryService
          .observe({
            ...
            metadata: {
              agentId: 'gitlabIterationSchedulesManager',
              query: prompt,
            },
          })
          .catch(() => {});
      ...
      logger.debug(
        `🦊 [gitlabIterationSchedulesManager] Storing consultation observation failed (non-blocking): ${observeErr.message}`,
      );
    }
...
export const pluginInstance =
  new GitlabFnGitlabIterationSchedulesManagerAgent();
