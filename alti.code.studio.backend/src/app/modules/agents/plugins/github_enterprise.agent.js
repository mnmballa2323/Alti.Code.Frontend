...
class GithubEnterpriseAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.description =
      'Specialized GitHub Enterprise Governance Specialist expert in SAML/SCIM SSO, organization policies, enterprise SSH CAs, and audit log analysis.';
    this.manifest = {
...
- Never invent undocumented enterprise API endpoints or policy properties.`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [GitHub Enterprise] Grounding enterprise query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Enterprise organization SAML SCIM audit log policies ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [GitHub Enterprise] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
