...
class GithubFnGithubSecurityAdvisoriesParserAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub Security Advisories Parser expert in CVE cataloging and advisory linkages.';
        this.manifest = {
            id: 'githubSecurityAdvisoriesParser',
            capabilities: ["github-get-security-advisories"],
            version: '39.6.0'
        };
        this.preamble = `You are the Alti.Code.Studio Specialist GitHub Security Advisories Parser expert in CVE cataloging and advisory linkages.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubSecurityAdvisoriesParser] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Security & Compliance github security advisories, vulnerability matching ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubSecurityAdvisoriesParser] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
