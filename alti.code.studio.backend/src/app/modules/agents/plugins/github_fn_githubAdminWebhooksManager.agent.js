...
class GithubFnGithubAdminWebhooksManagerAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.preamble = `You are the Inso Code Specialist GitHub Admin Webhooks Manager expert in administering organizational webhook endpoints.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubAdminWebhooksManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Billing & Admin organization webhooks configuration, event subscriptions ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubAdminWebhooksManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
