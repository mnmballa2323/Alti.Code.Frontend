...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
    ...
    async _invoke(prompt, contextBlock, tenantId, spanId) {
        ...
        try {
            ...
            docsContext = await gitlabDocsService.searchDocs(`GitLab Projects project creation, repository template setup, naming, visibility ${prompt}`, 5, tenantId);
        } catch (err) {
        ...
        try {
            ...
            logger.debug(`🦊 [gitlabProjectCreator] Memory retrieval bypassed: ${memErr.message}`);
        }
...
${prompt}`;
        ...
        if (response.includes('```') && !prompt.includes('no-reflect')) {
            logger.info(`🦊 [gitlabProjectCreator] Code blocks detected. Initiating automated reflection loop...`);
            try {
...
${response}`;
                ...
                if (!reviewResult.includes('APPROVED')) {
                    logger.info(`🦊 [gitlabProjectCreator] Reflection loop identified feedback. Correcting response...`);
...
Please correct the previous attempt based on the feedback above.`;
                ...
                logger.warn(`🦊 [gitlabProjectCreator] Reflection loop failed (non-blocking): ${reflectErr.message}`);
            }
        }
        ...
        try {
            ...
            logger.debug(`🦊 [gitlabProjectCreator] Storing consultation observation failed (non-blocking): ${observeErr.message}`);
        }
