...
class GithubFnGithubAppPrivateKeysManagerAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub App Private Keys Manager expert in key rotation and secure JWT credentials generation.';
        this.manifest = {
            id: 'githubAppPrivateKeysManager',
            capabilities: ["github-generate-app-private-key"],
            version: '39.6.0'
        };
        this.preamble = `You are the Alti.Code.Studio Specialist GitHub App Private Keys Manager expert in key rotation and secure JWT credentials generation.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubAppPrivateKeysManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Apps & Integrations github app private keys generation, jwt token signing ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubAppPrivateKeysManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
