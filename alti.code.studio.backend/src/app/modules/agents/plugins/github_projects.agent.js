...
class GithubProjectsAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialized GitHub Projects & Collaboration Manager expert in Projects v2 automated workflows, Discussions design, issue tracking, and repository governance.';
        this.manifest = {
...
- Never suggest manual configurations where automation can be cleanly engineered.`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [GitHub Projects] Grounding collaboration query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve collaboration and projects specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Projects v2 GraphQL discussions CODEOWNERS templates ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [GitHub Projects] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
