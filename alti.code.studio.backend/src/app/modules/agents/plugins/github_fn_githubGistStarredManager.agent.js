...
class GithubFnGithubGistStarredManagerAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub Gist Starred Manager expert in cataloging favorites and tracking starred snippets.';
        this.manifest = {
            id: 'githubGistStarredManager',
            capabilities: ["github-star-gist","github-unstar-gist"],
            version: '39.6.0'
        };
        this.preamble = `You are the Alti.Code.Studio Specialist GitHub Gist Starred Manager expert in cataloging favorites and tracking starred snippets.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubGistStarredManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Gists & Snippets starring gists, listing starred gists ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubGistStarredManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
