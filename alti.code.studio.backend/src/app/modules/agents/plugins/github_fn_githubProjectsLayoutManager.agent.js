...
class GithubFnGithubProjectsLayoutManagerAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub Projects Layout Manager expert in configuring roadmap timelines, grid views, and kanban templates.';
        this.manifest = {
            id: 'githubProjectsLayoutManager',
            capabilities: ["github-configure-project-v2-layout"],
            version: '39.6.0'
        };
        this.preamble = `You are the Alti.Code.Studio Specialist GitHub Projects Layout Manager expert in configuring roadmap timelines, grid views, and kanban templates.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubProjectsLayoutManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Projects v2 board layouts, table layouts, roadmap layouts ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubProjectsLayoutManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
