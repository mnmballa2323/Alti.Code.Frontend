...
class GithubFnGithubAdminSelfHostedRunnerLimitsAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.preamble = `You are the Inso Code Specialist GitHub Admin Self Hosted Runner Limits expert in actions spending caps and usage metrics.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubAdminSelfHostedRunnerLimits] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Billing & Admin actions minutes quotas, spending limits for runners ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubAdminSelfHostedRunnerLimits] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
...
export const pluginInstance =
  new GithubFnGithubAdminSelfHostedRunnerLimitsAgent();
