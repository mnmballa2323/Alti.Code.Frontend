...
class GithubFnGithubIssuesReactionsManagerAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.preamble = `You are the Inso Code Specialist GitHub Issues Reactions Manager expert in applying emoji reactions and aggregating user feedback.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubIssuesReactionsManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Issues issue emoji reactions, user feedback collection ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubIssuesReactionsManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
