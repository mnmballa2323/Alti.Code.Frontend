...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubCopilotSkillsConfigurator] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Copilot custom copilot skills yml configurations ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubCopilotSkillsConfigurator] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
...
export const pluginInstance =
  new GithubFnGithubCopilotSkillsConfiguratorAgent();
