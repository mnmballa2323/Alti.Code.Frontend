...
class GithubFnGithubAdminBillingExportCronAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.preamble = `You are the Inso Code Specialist GitHub Admin Billing Export Cron expert in scheduling billing usage exports.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubAdminBillingExportCron] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Billing & Admin billing usage exports, automated schedule ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubAdminBillingExportCron] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
