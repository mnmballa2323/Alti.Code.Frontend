...
class GithubFnGithubPrTimelineAuditorAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.preamble = `You are the Inso Code Specialist GitHub PR Timeline Auditor expert in scraping pull request activity trails, review history logs, and commit sequences.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubPrTimelineAuditor] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Pull Requests pull request commits history, timeline events, review history audits ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubPrTimelineAuditor] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
