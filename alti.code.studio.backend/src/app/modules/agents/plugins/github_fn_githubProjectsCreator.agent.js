...
class GithubFnGithubProjectsCreatorAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.preamble = `You are the Inso Code Specialist GitHub Projects Creator expert in establishing customizable organizational Projects v2 kanban boards.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubProjectsCreator] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Projects v2 projects v2 boards creation, organization projects ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubProjectsCreator] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
