...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
  ...
  async _invoke(prompt, contextBlock, tenantId, spanId) {
    ...
    logger.info(
      `🦊 [gitlabMarkdownGFMRenderer] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await gitlabDocsService.searchDocs(
        `GitLab Snippets GFM markdown rendering, GitLab Flavored Markdown parsing ${prompt}`,
        5,
        tenantId,
      );
    } catch (err) {
      logger.warn(
        `🦊 [gitlabMarkdownGFMRenderer] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
    ...
    try {
      ...
      logger.debug(
        `🦊 [gitlabMarkdownGFMRenderer] Memory retrieval bypassed: ${memErr.message}`,
      );
    }
...
${prompt}`;
    ...
    if (response.includes('```') && !prompt.includes('no-reflect')) {
      logger.info(
        `🦊 [gitlabMarkdownGFMRenderer] Code blocks detected. Initiating automated reflection loop...`,
      );
      try {
        const reflectionPrompt = `You are the critic evaluator for [gitlabMarkdownGFMRenderer].
...
${response}`;
        ...
        if (!reviewResult.includes('APPROVED')) {
          logger.info(
            `🦊 [gitlabMarkdownGFMRenderer] Reflection loop identified feedback. Correcting response...`,
          );
...
Please correct the previous attempt based on the feedback above.`;
        ...
        logger.warn(
          `🦊 [gitlabMarkdownGFMRenderer] Reflection loop failed (non-blocking): ${reflectErr.message}`,
        );
      }
    }
    ...
    try {
      ...
      if (agentMemoryService.isReady) {
        agentMemoryService
          .observe({
            content: `Agent [gitlabMarkdownGFMRenderer] processed query: "${prompt.substring(0, 150)}..." and generated output.`,
            type: 'agent_consultation',
            metadata: { agentId: 'gitlabMarkdownGFMRenderer', query: prompt },
          })
          .catch(() => {});
      }
    } catch (observeErr) {
      logger.debug(
        `🦊 [gitlabMarkdownGFMRenderer] Storing consultation observation failed (non-blocking): ${observeErr.message}`,
      );
    }
