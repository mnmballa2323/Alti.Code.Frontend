...
class GitlabFnGitlabAdminOAuthOrchestratorAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitLab Admin OAuth Orchestrator expert in credential sync checks.';
        this.manifest = {
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
    ...
    async _invoke(prompt, contextBlock, tenantId, spanId) {
        ...
        logger.info(`🦊 [gitlabAdminOAuthOrchestrator] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await gitlabDocsService.searchDocs(`GitLab Administration & OAuth listing instance OAuth applications, credential audits ${prompt}`, 5, tenantId);
        } catch (err) {
            logger.warn(`🦊 [gitlabAdminOAuthOrchestrator] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
        ...
        try {
            ...
            logger.debug(`🦊 [gitlabAdminOAuthOrchestrator] Memory retrieval bypassed: ${memErr.message}`);
        }
...
${prompt}`;
        ...
        if (response.includes('```') && !prompt.includes('no-reflect')) {
            logger.info(`🦊 [gitlabAdminOAuthOrchestrator] Code blocks detected. Initiating automated reflection loop...`);
            try {
                const reflectionPrompt = `You are the critic evaluator for [gitlabAdminOAuthOrchestrator].
...
${response}`;
                ...
                if (!reviewResult.includes('APPROVED')) {
                    logger.info(`🦊 [gitlabAdminOAuthOrchestrator] Reflection loop identified feedback. Correcting response...`);
...
Please correct the previous attempt based on the feedback above.`;
                ...
                logger.warn(`🦊 [gitlabAdminOAuthOrchestrator] Reflection loop failed (non-blocking): ${reflectErr.message}`);
            }
        }
        ...
        try {
            ...
            if (agentMemoryService.isReady) {
                agentMemoryService.observe({
                    content: `Agent [gitlabAdminOAuthOrchestrator] processed query: "${prompt.substring(0, 150)}..." and generated output.`,
                    type: 'agent_consultation',
                    metadata: { agentId: 'gitlabAdminOAuthOrchestrator', query: prompt }
                }).catch(() => {});
            }
        } catch (observeErr) {
            logger.debug(`🦊 [gitlabAdminOAuthOrchestrator] Storing consultation observation failed (non-blocking): ${observeErr.message}`);
        }
