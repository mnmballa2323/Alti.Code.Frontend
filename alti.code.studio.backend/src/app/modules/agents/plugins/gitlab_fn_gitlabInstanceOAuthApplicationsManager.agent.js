...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
    ...
    async _invoke(prompt, contextBlock, tenantId, spanId) {
        ...
        try {
            ...
            docsContext = await gitlabDocsService.searchDocs(`GitLab Administration & OAuth instance OAuth applications management, redirect URIs, OAuth scope limits ${prompt}`, 5, tenantId);
        } catch (err) {
        ...
        try {
            ...
            logger.debug(`🦊 [gitlabInstanceOAuthApplicationsManager] Memory retrieval bypassed: ${memErr.message}`);
        }
...
${prompt}`;
        ...
        if (response.includes('```') && !prompt.includes('no-reflect')) {
            logger.info(`🦊 [gitlabInstanceOAuthApplicationsManager] Code blocks detected. Initiating automated reflection loop...`);
            try {
...
${response}`;
                ...
                if (!reviewResult.includes('APPROVED')) {
                    logger.info(`🦊 [gitlabInstanceOAuthApplicationsManager] Reflection loop identified feedback. Correcting response...`);
...
Please correct the previous attempt based on the feedback above.`;
                ...
                logger.warn(`🦊 [gitlabInstanceOAuthApplicationsManager] Reflection loop failed (non-blocking): ${reflectErr.message}`);
            }
        }
        ...
        try {
            ...
            if (agentMemoryService.isReady) {
                agentMemoryService.observe({
                    ...
                    metadata: { agentId: 'gitlabInstanceOAuthApplicationsManager', query: prompt }
                }).catch(() => {});
            }
        } catch (observeErr) {
            logger.debug(`🦊 [gitlabInstanceOAuthApplicationsManager] Storing consultation observation failed (non-blocking): ${observeErr.message}`);
        }
