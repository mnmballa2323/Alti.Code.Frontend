...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
  ...
  async _invoke(prompt, contextBlock, tenantId, spanId) {
    ...
    try {
      ...
      logger.debug(
        `🦊 [gitlabIssueLinksManager] Memory retrieval bypassed: ${memErr.message}`,
      );
    }
...
${prompt}`;
    ...
    if (response.includes('```') && !prompt.includes('no-reflect')) {
      logger.info(
        `🦊 [gitlabIssueLinksManager] Code blocks detected. Initiating automated reflection loop...`,
      );
      try {
...
${response}`;
        ...
        if (!reviewResult.includes('APPROVED')) {
          logger.info(
            `🦊 [gitlabIssueLinksManager] Reflection loop identified feedback. Correcting response...`,
          );
...
Please correct the previous attempt based on the feedback above.`;
        ...
        logger.warn(
          `🦊 [gitlabIssueLinksManager] Reflection loop failed (non-blocking): ${reflectErr.message}`,
        );
      }
    }
    ...
    try {
      ...
      logger.debug(
        `🦊 [gitlabIssueLinksManager] Storing consultation observation failed (non-blocking): ${observeErr.message}`,
      );
    }
