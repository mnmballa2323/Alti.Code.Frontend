...
class GitlabFnGitlabRunnerVerifierAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitLab Runner Verifier expert in auditing runner connectivity and registration tokens.';
        this.manifest = {
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
    ...
    async _invoke(prompt, contextBlock, tenantId, spanId) {
        ...
        logger.info(`🦊 [gitlabRunnerVerifier] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await gitlabDocsService.searchDocs(`GitLab Runners runner validation, token verification, runner online status check ${prompt}`, 5, tenantId);
        } catch (err) {
            logger.warn(`🦊 [gitlabRunnerVerifier] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
        ...
        try {
            ...
            logger.debug(`🦊 [gitlabRunnerVerifier] Memory retrieval bypassed: ${memErr.message}`);
        }
...
${prompt}`;
        ...
        if (response.includes('```') && !prompt.includes('no-reflect')) {
            logger.info(`🦊 [gitlabRunnerVerifier] Code blocks detected. Initiating automated reflection loop...`);
            try {
                const reflectionPrompt = `You are the critic evaluator for [gitlabRunnerVerifier].
...
${response}`;
                ...
                if (!reviewResult.includes('APPROVED')) {
                    logger.info(`🦊 [gitlabRunnerVerifier] Reflection loop identified feedback. Correcting response...`);
...
Please correct the previous attempt based on the feedback above.`;
                ...
                logger.warn(`🦊 [gitlabRunnerVerifier] Reflection loop failed (non-blocking): ${reflectErr.message}`);
            }
        }
        ...
        try {
            ...
            if (agentMemoryService.isReady) {
                agentMemoryService.observe({
                    content: `Agent [gitlabRunnerVerifier] processed query: "${prompt.substring(0, 150)}..." and generated output.`,
                    type: 'agent_consultation',
                    metadata: { agentId: 'gitlabRunnerVerifier', query: prompt }
                }).catch(() => {});
            }
        } catch (observeErr) {
            logger.debug(`🦊 [gitlabRunnerVerifier] Storing consultation observation failed (non-blocking): ${observeErr.message}`);
        }
