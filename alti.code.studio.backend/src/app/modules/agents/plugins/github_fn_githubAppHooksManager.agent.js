...
class GithubFnGithubAppHooksManagerAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub App Hooks Manager expert in establishing organization-level webhook listeners for ecosystem scale integrations.';
        this.manifest = {
            id: 'githubAppHooksManager',
            capabilities: ["github-create-app-hook"],
            version: '39.6.0'
        };
        this.preamble = `You are the Alti.Code.Studio Specialist GitHub App Hooks Manager expert in establishing organization-level webhook listeners for ecosystem scale integrations.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubAppHooksManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Apps & Integrations organization-wide hooks, app webhooks updates ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubAppHooksManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
