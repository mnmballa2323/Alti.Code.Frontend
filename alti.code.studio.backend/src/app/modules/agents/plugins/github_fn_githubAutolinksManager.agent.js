...
class GithubFnGithubAutolinksManagerAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.preamble = `You are the Inso Code Specialist GitHub Autolinks Manager expert in configuring external autolink references and third-party ticket trackers sync.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubAutolinksManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Repositories autolink references, ticket links, external integration references ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubAutolinksManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
