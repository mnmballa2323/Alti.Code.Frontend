...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
    ...
    async _invoke(prompt, contextBlock, tenantId, spanId) {
        ...
        try {
            ...
            docsContext = await gitlabDocsService.searchDocs(`GitLab Administration & OAuth instance audit events, project audit logs, group compliance histories ${prompt}`, 5, tenantId);
        } catch (err) {
        ...
        try {
            ...
            logger.debug(`🦊 [gitlabAuditLogsManager] Memory retrieval bypassed: ${memErr.message}`);
        }
...
${prompt}`;
        ...
        if (response.includes('```') && !prompt.includes('no-reflect')) {
            logger.info(`🦊 [gitlabAuditLogsManager] Code blocks detected. Initiating automated reflection loop...`);
            try {
...
${response}`;
                ...
                if (!reviewResult.includes('APPROVED')) {
                    logger.info(`🦊 [gitlabAuditLogsManager] Reflection loop identified feedback. Correcting response...`);
...
Please correct the previous attempt based on the feedback above.`;
                ...
                logger.warn(`🦊 [gitlabAuditLogsManager] Reflection loop failed (non-blocking): ${reflectErr.message}`);
            }
        }
        ...
        try {
            ...
            logger.debug(`🦊 [gitlabAuditLogsManager] Storing consultation observation failed (non-blocking): ${observeErr.message}`);
        }
