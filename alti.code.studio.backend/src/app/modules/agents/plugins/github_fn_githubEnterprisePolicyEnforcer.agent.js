...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubEnterprisePolicyEnforcer] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Governance & Enterprise enterprise-wide repository policies, actions permission rules ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubEnterprisePolicyEnforcer] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
