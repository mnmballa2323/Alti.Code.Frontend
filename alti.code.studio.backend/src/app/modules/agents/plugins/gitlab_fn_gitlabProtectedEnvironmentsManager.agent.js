...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
    ...
    async _invoke(prompt, contextBlock, tenantId, spanId) {
        ...
        try {
            ...
            docsContext = await gitlabDocsService.searchDocs(`GitLab Security & Compliance protected environments, environment deployment gates, deploy approvals ${prompt}`, 5, tenantId);
        } catch (err) {
        ...
        try {
            ...
            logger.debug(`🦊 [gitlabProtectedEnvironmentsManager] Memory retrieval bypassed: ${memErr.message}`);
        }
...
${prompt}`;
        ...
        if (response.includes('```') && !prompt.includes('no-reflect')) {
            logger.info(`🦊 [gitlabProtectedEnvironmentsManager] Code blocks detected. Initiating automated reflection loop...`);
            try {
...
${response}`;
                ...
                if (!reviewResult.includes('APPROVED')) {
                    logger.info(`🦊 [gitlabProtectedEnvironmentsManager] Reflection loop identified feedback. Correcting response...`);
...
Please correct the previous attempt based on the feedback above.`;
                ...
                logger.warn(`🦊 [gitlabProtectedEnvironmentsManager] Reflection loop failed (non-blocking): ${reflectErr.message}`);
            }
        }
        ...
        try {
            ...
            if (agentMemoryService.isReady) {
                agentMemoryService.observe({
                    ...
                    metadata: { agentId: 'gitlabProtectedEnvironmentsManager', query: prompt }
                }).catch(() => {});
            }
        } catch (observeErr) {
            logger.debug(`🦊 [gitlabProtectedEnvironmentsManager] Storing consultation observation failed (non-blocking): ${observeErr.message}`);
        }
