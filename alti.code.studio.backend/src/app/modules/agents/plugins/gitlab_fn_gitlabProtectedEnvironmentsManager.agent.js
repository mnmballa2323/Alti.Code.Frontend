...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
  ...
  async _invoke(prompt, contextBlock, tenantId, spanId) {
    ...
    logger.info(
      `🦊 [gitlabProtectedEnvironmentsManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await gitlabDocsService.searchDocs(
        `GitLab Security & Compliance protected environments, environment deployment gates, deploy approvals ${prompt}`,
        5,
        tenantId,
      );
    } catch (err) {
      logger.warn(
        `🦊 [gitlabProtectedEnvironmentsManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
    ...
    try {
      ...
      logger.debug(
        `🦊 [gitlabProtectedEnvironmentsManager] Memory retrieval bypassed: ${memErr.message}`,
      );
    }
...
${prompt}`;
    ...
    if (response.includes('```') && !prompt.includes('no-reflect')) {
      logger.info(
        `🦊 [gitlabProtectedEnvironmentsManager] Code blocks detected. Initiating automated reflection loop...`,
      );
      try {
        const reflectionPrompt = `You are the critic evaluator for [gitlabProtectedEnvironmentsManager].
...
${response}`;
        ...
        if (!reviewResult.includes('APPROVED')) {
          logger.info(
            `🦊 [gitlabProtectedEnvironmentsManager] Reflection loop identified feedback. Correcting response...`,
          );
...
Please correct the previous attempt based on the feedback above.`;
        ...
        logger.warn(
          `🦊 [gitlabProtectedEnvironmentsManager] Reflection loop failed (non-blocking): ${reflectErr.message}`,
        );
      }
    }
    ...
    try {
      ...
      if (agentMemoryService.isReady) {
        agentMemoryService
          .observe({
            content: `Agent [gitlabProtectedEnvironmentsManager] processed query: "${prompt.substring(0, 150)}..." and generated output.`,
            type: 'agent_consultation',
            metadata: {
              agentId: 'gitlabProtectedEnvironmentsManager',
              query: prompt,
            },
          })
          .catch(() => {});
      ...
      logger.debug(
        `🦊 [gitlabProtectedEnvironmentsManager] Storing consultation observation failed (non-blocking): ${observeErr.message}`,
      );
    }
...
export const pluginInstance =
  new GitlabFnGitlabProtectedEnvironmentsManagerAgent();
