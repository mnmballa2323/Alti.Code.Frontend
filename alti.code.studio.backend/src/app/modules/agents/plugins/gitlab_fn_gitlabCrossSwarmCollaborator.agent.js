...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
    ...
    async _invoke(prompt, contextBlock, tenantId, spanId) {
        ...
        try {
            ...
            docsContext = await gitlabDocsService.searchDocs(`GitLab Administration & OAuth cross-platform replication, syncing pipelines, issues, and merge requests between GitHub and GitLab ${prompt}`, 5, tenantId);
        } catch (err) {
