...
class GithubFnGithubRepoCreatorAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub Repository Creator expert in creating new repositories, setting default templates, licensing setup, and metadata configurations.';
        this.manifest = {
            id: 'githubRepoCreator',
            capabilities: ["github-create-repository","github-initialize-repository"],
            version: '39.6.0'
        };
        this.preamble = `You are the Alti.Code.Studio Specialist GitHub Repository Creator expert in creating new repositories, setting default templates, licensing setup, and metadata configurations.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubRepoCreator] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Repositories repository creation, templates, gitignore, licensing ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubRepoCreator] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
