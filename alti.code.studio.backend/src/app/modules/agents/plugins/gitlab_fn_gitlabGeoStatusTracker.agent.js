...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
  ...
  async _invoke(prompt, contextBlock, tenantId, spanId) {
    ...
    logger.info(
      `🦊 [gitlabGeoStatusTracker] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await gitlabDocsService.searchDocs(
        `GitLab Geo & Replication Geo node status tracking, verification/replication percentages, sync lag checking ${prompt}`,
        5,
        tenantId,
      );
    } catch (err) {
      logger.warn(
        `🦊 [gitlabGeoStatusTracker] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
    ...
    try {
      ...
      logger.debug(
        `🦊 [gitlabGeoStatusTracker] Memory retrieval bypassed: ${memErr.message}`,
      );
    }
...
${prompt}`;
    ...
    if (response.includes('```') && !prompt.includes('no-reflect')) {
      logger.info(
        `🦊 [gitlabGeoStatusTracker] Code blocks detected. Initiating automated reflection loop...`,
      );
      try {
...
${response}`;
        ...
        if (!reviewResult.includes('APPROVED')) {
          logger.info(
            `🦊 [gitlabGeoStatusTracker] Reflection loop identified feedback. Correcting response...`,
          );
...
Please correct the previous attempt based on the feedback above.`;
        ...
        logger.warn(
          `🦊 [gitlabGeoStatusTracker] Reflection loop failed (non-blocking): ${reflectErr.message}`,
        );
      }
    }
    ...
    try {
      ...
      logger.debug(
        `🦊 [gitlabGeoStatusTracker] Storing consultation observation failed (non-blocking): ${observeErr.message}`,
      );
    }
