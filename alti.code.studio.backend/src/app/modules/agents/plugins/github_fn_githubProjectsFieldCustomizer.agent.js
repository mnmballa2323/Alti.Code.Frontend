...
class GithubFnGithubProjectsFieldCustomizerAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub Projects Field Customizer expert in defining select-option, number, and status metadata fields.';
        this.manifest = {
            id: 'githubProjectsFieldCustomizer',
            capabilities: ["github-create-project-v2-field"],
            version: '39.6.0'
        };
        this.preamble = `You are the Alti.Code.Studio Specialist GitHub Projects Field Customizer expert in defining select-option, number, and status metadata fields.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubProjectsFieldCustomizer] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Projects v2 projects v2 custom fields, single select fields, number/date fields ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubProjectsFieldCustomizer] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
