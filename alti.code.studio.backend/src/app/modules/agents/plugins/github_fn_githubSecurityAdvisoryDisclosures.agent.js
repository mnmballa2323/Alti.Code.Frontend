...
class GithubFnGithubSecurityAdvisoryDisclosuresAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub Security Advisory Disclosures expert in private vulnerability tracking and fixes coordination.';
        this.manifest = {
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubSecurityAdvisoryDisclosures] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Security & Compliance private security advisory disclosures, collaborative vulnerability fixes ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubSecurityAdvisoryDisclosures] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
...
export const pluginInstance = new GithubFnGithubSecurityAdvisoryDisclosuresAgent();
