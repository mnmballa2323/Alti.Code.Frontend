...
class GitlabFnGitlabRunnerConfiguratorAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitLab Runner Configurator expert in tweaking tags, locks, and active states on runners.';
        this.manifest = {
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
    ...
    async _invoke(prompt, contextBlock, tenantId, spanId) {
        ...
        logger.info(`🦊 [gitlabRunnerConfigurator] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await gitlabDocsService.searchDocs(`GitLab Runners runner description, runner tags, locked runners, active/inactive runner status ${prompt}`, 5, tenantId);
        } catch (err) {
            logger.warn(`🦊 [gitlabRunnerConfigurator] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
        ...
        try {
            ...
            logger.debug(`🦊 [gitlabRunnerConfigurator] Memory retrieval bypassed: ${memErr.message}`);
        }
...
${prompt}`;
        ...
        if (response.includes('```') && !prompt.includes('no-reflect')) {
            logger.info(`🦊 [gitlabRunnerConfigurator] Code blocks detected. Initiating automated reflection loop...`);
            try {
                const reflectionPrompt = `You are the critic evaluator for [gitlabRunnerConfigurator].
...
${response}`;
                ...
                if (!reviewResult.includes('APPROVED')) {
                    logger.info(`🦊 [gitlabRunnerConfigurator] Reflection loop identified feedback. Correcting response...`);
...
Please correct the previous attempt based on the feedback above.`;
                ...
                logger.warn(`🦊 [gitlabRunnerConfigurator] Reflection loop failed (non-blocking): ${reflectErr.message}`);
            }
        }
        ...
        try {
            ...
            if (agentMemoryService.isReady) {
                agentMemoryService.observe({
                    content: `Agent [gitlabRunnerConfigurator] processed query: "${prompt.substring(0, 150)}..." and generated output.`,
                    type: 'agent_consultation',
                    metadata: { agentId: 'gitlabRunnerConfigurator', query: prompt }
                }).catch(() => {});
            }
        } catch (observeErr) {
            logger.debug(`🦊 [gitlabRunnerConfigurator] Storing consultation observation failed (non-blocking): ${observeErr.message}`);
        }
