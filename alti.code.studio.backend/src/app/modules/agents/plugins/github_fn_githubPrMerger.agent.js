...
class GithubFnGithubPrMergerAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub PR Merger expert in executing safe merges (squash, rebase, standard) and performing pre-merge integrity audits.';
        this.manifest = {
            id: 'githubPrMerger',
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubPrMerger] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Pull Requests merging pull requests, squash merges, rebase merges, merge conflict checks ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubPrMerger] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
