...
class GithubGistAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.description =
      'Specialized GitHub Gists & Snippets Developer expert in multi-file Gist APIs, anonymous scratchpads, embeds, and snippet sharing.';
    this.manifest = {
...
- Never invent undocumented query variables or API endpoints.`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [GitHub Gists] Grounding Gists query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Gists API secret public snippets embed collaborate ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [GitHub Gists] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
