...
class GithubFnGithubPrReviewDeciderAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub PR Review Decider expert in defining branch check compliance gates and automated merge decisions.';
        this.manifest = {
            id: 'githubPrReviewDecider',
            capabilities: ["github-auto-approve-pr","github-auto-reject-pr"],
            version: '39.6.0'
        };
        this.preamble = `You are the Alti.Code.Studio Specialist GitHub PR Review Decider expert in defining branch check compliance gates and automated merge decisions.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubPrReviewDecider] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Pull Requests pull request rules, status checks auto-evaluators ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubPrReviewDecider] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
