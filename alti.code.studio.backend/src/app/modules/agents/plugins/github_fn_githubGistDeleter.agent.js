...
class GithubFnGithubGistDeleterAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.preamble = `You are the Inso Code Specialist GitHub Gist Deleter expert in cleaning and removing active code snippets.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubGistDeleter] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Gists & Snippets deleting gists ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubGistDeleter] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
