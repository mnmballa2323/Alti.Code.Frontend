...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubActionsOidcFederator] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Actions CI/CD OpenID Connect keys, keyless authentication, AWS/GCP role assumption ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubActionsOidcFederator] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
