...
class GithubPackagesAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.description =
      'Specialized GitHub Packages & Container Registry Architect expert in OCI container publishing, package auth, and security scanning.';
    this.manifest = {
...
- Never invent undocumented registry features or registry-specific variables.`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [GitHub Packages] Grounding packages query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Packages Container Registry ghcr.io docker npm maven publish ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [GitHub Packages] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
