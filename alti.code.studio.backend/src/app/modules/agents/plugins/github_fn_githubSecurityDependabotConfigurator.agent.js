...
class GithubFnGithubSecurityDependabotConfiguratorAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub Security Dependabot Configurator expert in configuring dependabot.yml manifest definitions.';
        this.manifest = {
            id: 'githubSecurityDependabotConfigurator',
            capabilities: ["github-configure-dependabot"],
            version: '39.6.0'
        };
        this.preamble = `You are the Alti.Code.Studio Specialist GitHub Security Dependabot Configurator expert in configuring dependabot.yml manifest definitions.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubSecurityDependabotConfigurator] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Security & Compliance dependabot.yml file configurations, package managers schedule ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubSecurityDependabotConfigurator] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
...
export const pluginInstance = new GithubFnGithubSecurityDependabotConfiguratorAgent();
