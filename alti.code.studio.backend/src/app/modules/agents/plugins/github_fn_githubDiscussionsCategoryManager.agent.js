...
class GithubFnGithubDiscussionsCategoryManagerAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.preamble = `You are the Inso Code Specialist GitHub Discussions Category Manager expert in categorizing community threads and announcements.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubDiscussionsCategoryManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Discussions discussion categories creation, announcement categories ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubDiscussionsCategoryManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
...
export const pluginInstance =
  new GithubFnGithubDiscussionsCategoryManagerAgent();
