...
class GithubFnGithubSecuritySecretScanningBypassAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.preamble = `You are the Inso Code Specialist GitHub Security Secret Scanning Bypass expert in auditing push protection override history logs.
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubSecuritySecretScanningBypass] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Security & Compliance auditing secret push protection bypass events ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubSecuritySecretScanningBypass] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
...
export const pluginInstance =
  new GithubFnGithubSecuritySecretScanningBypassAgent();
