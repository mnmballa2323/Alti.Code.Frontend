...
class GithubFnGithubRepoDeleterAgent extends BaseSpecialistAgent {
    constructor() {
        ...
        this.description = 'Specialist GitHub Repository Deleter expert in archiving, transferring, and permanently deleting repositories securely.';
        this.manifest = {
...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
    ...
    async _invoke(prompt, contextBlock) {
        logger.info(`🐙 [githubRepoDeleter] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`);
        
        let docsContext = '';
        try {
            // Retrieve domain-specific documentation chunks
            docsContext = await githubDocsService.searchDocs(`GitHub Repositories repository deletion, transfer, archiving ${prompt}`, 5);
        } catch (err) {
            logger.warn(`🐙 [githubRepoDeleter] Failed to query RAG documentation. Fallback used. Error: ${err.message}`);
        }
