...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs).`;
  ...
  async _invoke(prompt, contextBlock, tenantId, spanId) {
    ...
    logger.info(
      `🦊 [gitlabComplianceFrameworksManager] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await gitlabDocsService.searchDocs(
        `GitLab Security & Compliance compliance frameworks, pipeline compliance controls, security templates ${prompt}`,
        5,
        tenantId,
      );
    } catch (err) {
      logger.warn(
        `🦊 [gitlabComplianceFrameworksManager] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
    ...
    try {
      ...
      logger.debug(
        `🦊 [gitlabComplianceFrameworksManager] Memory retrieval bypassed: ${memErr.message}`,
      );
    }
...
${prompt}`;
    ...
    if (response.includes('```') && !prompt.includes('no-reflect')) {
      logger.info(
        `🦊 [gitlabComplianceFrameworksManager] Code blocks detected. Initiating automated reflection loop...`,
      );
      try {
        const reflectionPrompt = `You are the critic evaluator for [gitlabComplianceFrameworksManager].
...
${response}`;
        ...
        if (!reviewResult.includes('APPROVED')) {
          logger.info(
            `🦊 [gitlabComplianceFrameworksManager] Reflection loop identified feedback. Correcting response...`,
          );
...
Please correct the previous attempt based on the feedback above.`;
        ...
        logger.warn(
          `🦊 [gitlabComplianceFrameworksManager] Reflection loop failed (non-blocking): ${reflectErr.message}`,
        );
      }
    }
    ...
    try {
      ...
      if (agentMemoryService.isReady) {
        agentMemoryService
          .observe({
            content: `Agent [gitlabComplianceFrameworksManager] processed query: "${prompt.substring(0, 150)}..." and generated output.`,
            type: 'agent_consultation',
            metadata: {
              agentId: 'gitlabComplianceFrameworksManager',
              query: prompt,
            },
          })
          .catch(() => {});
      ...
      logger.debug(
        `🦊 [gitlabComplianceFrameworksManager] Storing consultation observation failed (non-blocking): ${observeErr.message}`,
      );
    }
...
export const pluginInstance =
  new GitlabFnGitlabComplianceFrameworksManagerAgent();
