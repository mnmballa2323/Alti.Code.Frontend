...
- Respond with clear, structured markdown. When generating code blocks, provide clean, production-grade snippets (JavaScript/TypeScript for APIs, YAML for Actions).`;
  ...
  async _invoke(prompt, contextBlock) {
    logger.info(
      `🐙 [githubPrReviewCommenter] Grounding specialized query in ingested developer docs: "${prompt.substring(0, 60)}..."`,
    );
    ...
    try {
      ...
      docsContext = await githubDocsService.searchDocs(
        `GitHub Pull Requests diff comments, review thread resolutions, single line comments ${prompt}`,
        5,
      );
    } catch (err) {
      logger.warn(
        `🐙 [githubPrReviewCommenter] Failed to query RAG documentation. Fallback used. Error: ${err.message}`,
      );
    }
