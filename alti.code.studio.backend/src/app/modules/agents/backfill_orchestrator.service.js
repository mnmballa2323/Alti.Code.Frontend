...
class BackfillOrchestrator {
  constructor() {
    ...
    logger.info(
      '🗄️ BackfillOrchestrator initialized. Ready for batched data mutations.',
    );
  }
  ...
  async startBackfill(jobId, fetchBatchFn, processBatchFn, options = {}) {
    ...
    logger.info(
      `🗄️ [${jobId}] Starting backfill. Batch size: ${config.batchSize}, Throttle: ${config.throttleMs}ms`,
    );

    // Fire-and-forget background execution
    this._executeLoop(jobId, fetchBatchFn, processBatchFn).catch(err =>
      logger.error(`🗄️ [${jobId}] Critical backfill failure: ${err.message}`),
    );
    ...
    try {
      while (config.status === 'running') {
        ...
        if (!rows || rows.length === 0) {
          ...
          logger.info(
            `🗄️ [${jobId}] Backfill completed. Processed ${config.rowsProcessed} rows total.`,
          );
          break;
        }
        ...
        try {
          ...
          config.errors++;
          logger.warn(
            `🗄️ [${jobId}] Batch processing error. Count: ${config.errors}. Error: ${batchErr.message}`,
          );
          if (config.errors > 10) {
            throw new Error(
              'Exceeded maximum error threshold. Aborting backfill loop.',
            );
          }
        }
      ...
      logger.error(
        `🗄️ [${jobId}] FAILED. Rows Processed: ${config.rowsProcessed}. Error: ${fatalError.message}`,
      );
    } finally {
