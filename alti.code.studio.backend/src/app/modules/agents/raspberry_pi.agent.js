 ...
 * systemd services, Linux networking, and IoT edge patterns.
 */
...
7. **Edge-to-Cloud Pattern**: Local pipeline: sensor → Python → local buffer (SQLite/Redis) → MQTT publish → AWS IoT/Azure IoT Hub. If network drops, buffer locally and flush on reconnect. Use \`AWSIoTPythonSDK.MQTTLib\` or \`paho-mqtt\` for cloud connection. Watchdog: add cron to restart service if it crashes.
# BEST PRACTICES
