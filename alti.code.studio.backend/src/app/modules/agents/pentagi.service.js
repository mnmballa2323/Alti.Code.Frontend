...
class PentagiService {
  ...
  async startFlow(prompt, contextBlock) {
    logger.info(
      `🛡️ [PentagiService] Dispatching auto-pentest to PentAGI container...`,
    );
    ...
    try {
      ...
      if (
        response.data &&
        response.data.data &&
        response.data.data.createFlow
      ) {
        const flow = response.data.data.createFlow;
        logger.info(
          `🛡️ [PentagiService] Flow ${flow.id} successfully generated on PentAGI Orchestrator.`,
        );
        return {
          id: flow.id,
          status: flow.status,
      ...
      logger.warn(
        `⚠️ [PentagiService] Real API request failed (${error.message}). Falling back to simulated proxy wrapper...`,
      );
      ...
      logger.info(
        `🛡️ [PentagiService] Flow ${flowId} successfully generated via proxy fallback.`,
      );
