...
class HermesAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.description =
      'The Hermes Vanguard CLI python executor bridge. Executes the autonomous self-improving Hermes Agent locally.';
    this.hermesPath = path.resolve(
      process.cwd(),
      'submodules/hermes-agent/cli.py',
    );
    this.pythonPath = path.resolve(
      process.cwd(),
      'submodules/hermes-agent/.venv/bin/python',
    );
  }
  ...
  async _runHermesCliProcess(prompt) {
    return new Promise((resolve, reject) => {
      logger.info(
        `🤖 Initiating Hermes Agent (alti_coder) Task: ${prompt.substring(0, 100)}...`,
      );

      // Execute the Hermes agent via CLI directly inside the workspace
      const hermesProcess = spawn(
        this.pythonPath,
        [this.hermesPath, 'run', prompt],
        {
          ...
          env: {
            ...
            GOOGLE_CLOUD_PROJECT:
              config.gcp_project_id || process.env.GOOGLE_CLOUD_PROJECT,
      ...
      hermesProcess.on('close', code => {
        if (code === 0) {
          ...
          reject(
            new Error(`Hermes execution failed:\n${errorOutput || output}`),
          );
        }
      });
    });
  ...
  async executeTask(prompt) {
    ...
    while (attempt <= maxAttempts) {
      try {
        ...
        logger.warn(
          `⚠️ Hermes Agent: Task execution failed on Attempt ${attempt}/${maxAttempts}: ${error.message}`,
        );
        ...
        try {
          logger.info(
            `🧠 [Self-Improving] Triggering Neuromorphic Hermes Socratic Debate to analyze failed execution trajectory...`,
          );
...
${error.message}`;
          ...
          logger.info(
            `🧠 [Self-Improving] Socratic Debate Verdict: ${debateResolution.substring(0, 300)}...`,
          );
          ...
          logger.info(
            `🧠 [Self-Improving] Evolved prompt trajectory completed. Retrying task...`,
          );
        } catch (debateErr) {
          logger.warn(
            `Socratic debate failed: ${debateErr.message}. Retrying with original prompt.`,
          );
        }
