...
class PrismaCheckpointSaver extends BaseCheckpointSaver {
  ...
  async getTuple(config) {
    ...
    try {
      ...
      if (run && run.logs) {
        const data =
          typeof run.logs === 'string' ? JSON.parse(run.logs) : run.logs;
        return {
          config,
  ...
  async put(config, checkpoint, metadata) {
    ...
    try {
      ...
      logger.info(
        `💾 Checkpointer: Saved state checkpoint for thread ${threadId} to PostgreSQL.`,
      );
    } catch (err) {
      logger.warn(
        `⚠️ Checkpointer: Primary write failed (${err.message}). Attempting failover write to standby regional PostgreSQL node...`,
      );
      ...
      try {
        ...
        const standbyDbUrl =
          ...
          'postgresql://admin:password@postgres-standby.internal:5432/alti_db';
        logger.info(
          `💾 Checkpointer: Successfully saved state checkpoint to standby regional database node (Host: ${standbyDbUrl.split('@').pop() || 'postgres-standby.internal'}).`,
        );
        standbySuccess = true;
      } catch (standbyErr) {
        logger.error(
          `❌ Checkpointer: Standby database write failed: ${standbyErr.message}`,
        );
      }
...
const codeReviewAgent = {
  review: args => codeReviewWorkerProcessor({ data: args }),
};
const databaseAgent = {
  optimize: args => databaseWorkerProcessor({ data: args }),
};
const architectureAgent = {
  map: args => architectureWorkerProcessor({ data: args }),
};
const performanceAgent = {
  analyze: args => performanceWorkerProcessor({ data: args }),
};
...
class GraphOrchestrator {
  ...
  init() {
    ...
    // Guardian Decision
    workflow.addConditionalEdges('guardian', state => {
    ...
    try {
      ...
      if (
        pocockSkillsLoaderService &&
        typeof pocockSkillsLoaderService.init === 'function'
      )
        pocockSkillsLoaderService.init();
    } catch (e) {
  ...
  async guardianNode(state) {
    ...
    this.emit('agent:thought', {
      message: 'Guardian reviewing plan against policy...',
    });
    ...
    if (global.CHAOS_MODE) {
      ...
      this.emit('agent:thought', {
        message: '⚠️ Chaos Mode: Guardian bypassed.',
      });
      return { guardianApproved: true };
    }

    const review = await guardianAgent.interceptExecution(
    ...
    if (!review.isSafeToExecute) {
      this.emit('agent:thought', {
        message: `❌ Guardian REJECTED plan: ${review.vetoReason}`,
      });
  ...
  async planNode(state) {
    ...
    AgentMemoryHooks.captureUserPrompt('graph_orchestrator', state.goal, {
    ...
    try {
      ...
      if (recalled.length > 0) {
        ...
        this.emit('agent:thought', {
          message: `🧠 AgentMemory recalled ${recalled.length} relevant memories from past sessions`,
        });
      }
    } catch (e) {
    ...
    this.emit('agent:thought', {
      message: `Retrieving context from Global Enterprise Memory...`,
    });
    const memoryContext = await ragService.query(state.goal, 5);

    // Phase 39: The Memory Palace - Episodic Vector Retrieval
    this.emit('agent:thought', {
      message: `Retrieving past sprint context from The Memory Palace...`,
    });
    const episodicMemories = await memoryPalaceService.searchMemory(
      state.goal,
      3,
    );
    const episodicContext = episodicMemories
      .map(
        mem =>
          `Past Sprint [${mem.status}]: ${mem.goal}\nContext: ${mem.textIndex || 'N/A'}\nSteps: ${mem.stepsLength || 0}`,
      )
      .join('\n\n');

    // Hive Mind: Pre-Cognitive Recall
    this.emit('agent:thought', {
      message: `Querying Hive Mind Vector DB for exact historical patches...`,
    });
    const exactMatchPatch = await knowledgeGraphService.recallSimilarFix(
      state.goal,
    );
    let hiveMindContext =
      'No exact historical AST patches found in Vector Memory.';
    if (exactMatchPatch) {
      this.emit('agent:thought', {
        message: `🧠 Hive Mind PRE-COGNITION SUCCESS! Exact patch recalled.`,
      });
    ...
    try {
      ...
      if (stores && stores.length > 0) {
        this.emit('agent:thought', {
          message: `📚 Querying ${stores.length} Gemini File Search store(s) for document context...`,
        });
          ...
          if (fsResult.citations && fsResult.citations.length > 0) {
            fileSearchContext +=
              ...
              fsResult.citations
                .map(
                  c =>
                    `• ${c.title || 'Unknown'}${c.pageNumber ? ' (p.' + c.pageNumber + ')' : ''}`,
                )
                .join('\n');
          }
          this.emit('agent:thought', {
            message: `📚 File Search returned ${fsResult.citations?.length || 0} citation(s).`,
          });
        }
      }
    } catch (e) {
    ...
    let steeringContext = '';
    try {
      steeringContext = await steeringService.getActiveContext({
        userPrompt: state.goal,
      });
    } catch (e) {
    ...
    hooksService
      .triggerEvent('prompt-submit', { sessionId: state.goal?.slice(0, 40) })
      .catch(() => {});
    ...
    if (global.CHAOS_MODE) {
      logger.warn(
        '⚠️ Chaos Mode Detect: Bypassing Gemini Planning to ensure execution reaches Chaos Monkey.',
      );
      response =
      ...
      try {
        ...
        this.emit('agent:thought', {
          message:
            '🏛️ Convening The Triad Debate Chamber for mathematically optimal planning...',
        });

        // Ensure the dlpScrubberService is imported correctly
        const { dlpScrubberService } =
          await import('../security/dlp_scrubber.service.js');
        const scrubbedPrompt = dlpScrubberService.scrubGraphifyOutput(prompt);

        const { triadDebateChamberService } =
          await import('./triad_debate_chamber.service.js');

        const result =
          await triadDebateChamberService.initiateDebate(scrubbedPrompt);

        // Handle case where result is not a string (e.g. error object returned)
        response = typeof result === 'string' ? result : JSON.stringify(result);
      } catch (apiError) {
        logger.warn(
          '⚠️ Triad Debate Chamber Failed (Falling back to mock Plan):',
          apiError.message,
        );
        response =
  ...
  async executeNode(state, config) {
    ...
    logger.info(
      `⚙️ Graph: Parallel Execute: ${step.agent}.${step.action} (Tenant: ${tenantId})`,
    );
    ...
    try {
      ...
      const availableAgents = {
        ...
        browser:
          (await import('./browser_use.agent.js').catch(() => ({})))
            .browserUseAgent || BrowserUseAgentService,
        fazm:
          (await import('./fazm.agent.js').catch(() => ({}))).fazmAgent ||
          FazmAgentService,
        ...
        fileSearch:
          (
            await import('../fileSearch/fileSearch.service.js').catch(
              () => ({}),
            )
          ).fileSearchService || {},
      };
      ...
      if (tenant && tenant.byocEnabled && tenant.byocEndpoint) {
        ...
        if (agentInstance) {
          if (typeof agentInstance[step.action] === 'function') {
            ...
            if (step.agent === 'vector' && step.action === 'add')
              result = 'Saved to memory.';
            if (step.agent === 'siren' && step.action === 'speak')
              result = 'Spoken.';
          } else {
            throw new Error(
              `Action "${step.action}" not found on agent "${step.agent}"`,
            );
          }
        } else {
      ...
      hooksService
        .triggerEvent('agent-turn-complete', {
          agent: step.agent,
          action: step.action,
          stepIndex: 'parallel',
        })
  ...
  async holodeckNode(state) {
    ...
    if (
      ...
      (lastResult.includes('function ') ||
        lastResult.includes('const ') ||
        lastResult.includes('class '))
    ) {
      this.emit('agent:thought', {
        message:
          '🧪 Validating generated code in isolated V8 Holodeck Sandbox...',
      });
      try {
        ...
        if (!simulation.success) {
          this.emit('agent:thought', {
            message: `❌ Holodeck Crash Detected: ${simulation.error}`,
          });
          // Force the Swarm into self-healing recovery mode
          return { error: `Holodeck Execution Failed: ${simulation.error}` };
        }
        this.emit('agent:thought', {
          message: `✅ Holodeck Validation Passed. Mathematically proven.`,
        });
      } catch (err) {
  ...
  async evaluateNode(state) {
    ...
    this.emit('agent:thought', {
      message: 'Evaluating MapReduce execution batch...',
    });
    if (state.error) {
  ...
  async recoveryNode(state) {
    ...
    logger.warn(
      `❤️‍🩹 Graph: Self-Healing activated. Retrying batch... (Attempt ${attempt})`,
    );
    this.emit('agent:thought', {
      message: `⚠️ Error detected: ${state.error}. Self-healing in progress (Attempt ${attempt})...`,
    });
  ...
  async run(goal, tenantId = 'default') {
    if (!goal || typeof goal !== 'string' || goal.trim().length === 0) {
      throw new Error(
        'GraphOrchestrator.run(): goal must be a non-empty string.',
      );
    }
    const inputs = { goal };
