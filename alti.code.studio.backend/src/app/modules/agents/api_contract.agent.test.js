...
describe('ApiContractAgent Unit Tests', () => {
    ...
    it('should initialize with correct name, description, and preamble', () => {
        ...
        expect(apiContractAgent.description).toContain('API Contract & Integration SDK Specialist');
        expect(apiContractAgent.preamble).toContain('API CONTRACT & INTEGRATION SDK SPECIALIST');
    });

    it('should successfully consult the LLM with context and prompt', async () => {
        const mockResponse = 'Audit Report: Route /api/v1/auth/register has correct Joi validation schema. Dynamic SDK types generated successfully.';
        ...
        expect(generatedPrompt).toContain('API CONTRACT & INTEGRATION SDK SPECIALIST');
    ...
    it('should handle API errors and trigger retries in BaseSpecialistAgent', async () => {
        ...
        const result = await apiContractAgent.consult('Generate client SDK fetcher code', []);
