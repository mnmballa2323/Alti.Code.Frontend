...
assert.strictEqual(add(5, 5), 10);
    ...
    sandboxService.execute
      .mockRejectedValueOnce(
        new Error(
          'AssertionError [ERR_ASSERTION]: Expected values to be strictly equal: -0 !== 10',
        ),
      )
      .mockResolvedValueOnce('Success'); // Second attempt passes

    const result = await tddAgent.developSecurely(
      'Write a function add(a, b) that adds two numbers.',
    );
