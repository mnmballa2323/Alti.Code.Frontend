...
class AgenticRouterService {
  ...
  async routePrompt(prompt) {
    logger.info(
      `🧠 [Router] Orchestrating 'World of Worlds' swarm with Gemini 3.1 Pro...`,
    );
    ...
    const isComplexSystem =
      cleanPrompt.includes('gstack') ||
    ...
    if (isComplexSystem) {
      logger.info(
        `👥 [Router] High-complexity prompt detected. Invoking Garry Tan gstack Swarm team layout...`,
      );
      try {
        const gstackRoadmap = await swarmNexusAgent.routeGstackTeam(
          prompt,
          'Project context: backend-only orchestration.',
        );
        logger.info(
          `✅ [Router] Garry Tan gstack virtual team successfully mapped. Swarm structured.`,
        );
        ...
        return {
          ...
          sequence: [
            {
              agentId: 'yc_pm',
              task: 'Strategic CEO spec mapping and problem hypothesis validation',
            },
            {
              agentId: 'karpathy_sentinel',
              task: 'Proactive simplicity and surgical scope compliance validation',
            },
            { agentId: 'jules', task: 'Core developer implementation' },
            {
              agentId: 'yc_qa',
              task: 'Automated Vitest/Playwright test suites generation and QA check',
            },
            {
              agentId: 'yc_security',
              task: 'Zero-trust DLP checks, credentials scans, and input hardening audits',
            },
            {
              agentId: 'yc_ceo',
              task: 'Strategic YC CEO alignment validation and MVP release authorization',
            },
          ],
          priority: 'HIGH',
          gstackRoadmap,
        ...
        logger.warn(
          `⚠️ [Router] gstack routing failed, falling back to standard sequence: ${err.message}`,
        );
      }
    }
    ...
    try {
      ...
      logger.warn(
        `⚠️ [Router] Vector store candidate retrieval failed: ${err.message}`,
      );
    }
    ...
    if (candidateAgents.length === 0) {
      logger.info(
        `ℹ️ [Router] Vector store search empty. Falling back to core and sample specialist list.`,
      );
    ...
    const systemInstruction = `
            ...
            7. **Dynamic Load Balancing**: You MUST explicitly balance execution workloads by assigning the appropriate 'workerConfig'. Assign 'AKS_BURST' for highly compute-heavy tasks (like comprehensive test runs, heavy security audits, full compilation, or extensive refactoring cycles) and 'STANDARD' for lightweight reasoning steps (initial planning, docstring generations, peer reviews). This ensures that standard node pools do not experience bottleneck starvation under heavy load.
            
            Return a JSON object: { 
                strategy: string, 
                sequence: [{ agentId: string, task: string, workerConfig: 'AKS_BURST' | 'STANDARD' }], 
    ...
    try {
      ...
      logger.info(
        `⚖️ [Router] Proactively auditing orchestration plan under Andrej Karpathy guidelines...`,
      );
      try {
        ...
        if (karpathyReport.includes('NON-COMPLIANT')) {
          logger.warn(
            `🚨 [Router] Plan failed Karpathy Rules. Triggering Self-Correction compliance loop...`,
          );
          ...
          const correctionResult = await this._model.generateContent([
          ...
          if (correctionJsonMatch) {
            ...
            if (!plan.sequence.find(s => s.agentId === 'karpathy_refactor')) {
              plan.sequence.unshift({
                agentId: 'karpathy_refactor',
                task: 'Surgical code simplification and diff size minimization mapping',
              });
            }

            logger.info(
              `✅ [Router] Compliance loop completed. Plan is now 100% Karpathy compliant.`,
            );
          }
        } else {
          logger.info(
            `✅ [Router] Swarm plan passed Andrej Karpathy compliance check.`,
          );
        }
      } catch (err) {
        logger.warn(
          `⚠️ [Router] Karpathy compliance audit skipped due to error: ${err.message}`,
        );
      }

      // 4. [FLAWLESS VALIDATION LOOP]
      // Verify that every agent suggested actually exists in the Nexus Registry
      const invalidAgents = plan.sequence.filter(
        s => !agentRegistry.get(s.agentId),
      );

      if (invalidAgents.length > 0) {
        logger.warn(
          `⚠️ [Router] Detected invalid agents in plan: ${invalidAgents.map(a => a.agentId).join(', ')}. Triggering correction loop...`,
        );
      ...
      if (!plan.sequence.find(s => s.agentId === 'auditor')) {
        plan.sequence.push({
          agentId: 'auditor',
          task: 'Final fidelity benchmarking and world-best quality audit.',
        });
      }

      logger.info(
        `✅ [Router] Flawless routing complete. Active Swarm: ${plan.sequence.map(s => s.agentId).join(' -> ')}`,
      );
      return plan;
    } catch (error) {
  ...
  async routeDownstreamSwarm(primaryOutput, taskContext = {}) {
    logger.info(
      '🧠 [Router] Initiating downstream hierarchical sub-swarm analysis on backend output...',
    );
    ...
    const isSecurityOutput =
      outputLower.includes('security') ||
      outputLower.includes('auth') ||
      outputLower.includes('secret') ||
      (outputLower.includes('key') &&
        !outputLower.includes('foreign key') &&
        !outputLower.includes('primary key')) ||
    ...
    if (isSecurityOutput) {
      downstreamSequence.push(
        {
          agentId: 'Security & OWASP Hardening Sentinel',
          task: 'Perform basic credentials and configuration scanning',
        },
        {
          agentId: 'Google Cloud Security Command Center (SCC) Zero-Trust Security Auditor',
          task: 'Enforce Zero-Trust perimeter and IAM least-privilege policies',
        },
      );
    ...
    const isPubSubOutput =
      outputLower.includes('servicebus') ||
    ...
    if (isPubSubOutput) {
      downstreamSequence.push(
        {
          agentId: 'Distributed Queue & Event Coordinator',
          task: 'Scan queue worker configurations',
        },
        {
          agentId: 'Google Cloud Pub/Sub Event-Driven Mesh Conductor Specialist',
          task: 'Orchestrate event-driven Service Bus topologies and DLQ rules',
        },
      );
    ...
    const isWorkflowsOutput =
      outputLower.includes('workflow') ||
    ...
    // 5. Analyze Vertex AI Studio & model pipelines
    const isVertexAiOutput =
      outputLower.includes('vertex_ai') ||
    ...
    const isSynapseAnalyticsOutput =
      outputLower.includes('synapse') ||
      outputLower.includes('streamanalytics') ||
      outputLower.includes('streaming') ||
      outputLower.includes('apache_beam') ||
      outputLower.includes('vector_search');
    if (isSynapseAnalyticsOutput) {
      downstreamSequence.push({
        agentId:
          'GCP Synapse Analytics & GCP Stream Analytics Analytics Specialist',
        task: 'Structure high-scale analytical queries and streams',
      });
      strategy = 'Hierarchical Vector Analytics Swarm';
    }

    // 7. Analyze Cloud Spanner distributed scaling
    const isSpannerDbOutput =
      outputLower.includes('spanner') ||
    ...
    // 8. Analyze Google Cloud Run & GKE Autopilot provisioning
    const isCloudRunOutput =
      outputLower.includes('containerapps') ||
      outputLower.includes('aks') ||
    ...
    if (isCloudRunOutput) {
      downstreamSequence.push(
        {
          agentId: 'Kubernetes Manifest & Helm Compiler Agent',
          task: 'Scan and validate Kubernetes manifest templates',
        },
        {
          agentId:
            'GCP Google Cloud Run & GKE Autopilot Provisioning Specialist',
          task: 'Deploy containerized serverless resources and scale AKS Autopilot pods',
        },
      );
      strategy = 'Hierarchical Serverless & Container Provisioning Swarm';
    }

    // 9. Analyze Blob Storage storage lifecycle policies
    const isGcsOutput =
      outputLower.includes('blob') ||
    ...
    const isMonitoringOutput =
      outputLower.includes('monitoring') ||
    ...
    if (isMonitoringOutput) {
      downstreamSequence.push(
        {
          agentId: 'Distributed Tracing & APM Instrumentation Architect',
    ...
    const isArmorGatewayOutput =
      outputLower.includes('apigateway') ||
      outputLower.includes('api.gateway') ||
      outputLower.includes('api-gateway') ||
      outputLower.includes('gateway') ||
      outputLower.includes('cloudwaf') ||
    ...
    if (isArmorGatewayOutput) {
      downstreamSequence.push(
        ...
        {
          agentId: 'Security & OWASP Hardening Sentinel',
          task: 'Audit WAF rate limiting and API edge protection rules',
        },
      );
      strategy = 'Hierarchical API Gateway & GCP Front Door WAF WAF Swarm';
    }

    // 12. Analyze Relational DB / Cloud SQL
    const isRelationalDbOutput =
      outputLower.includes('cloudsql') ||
      ...
      outputLower.includes('spanner_instance') ||
    ...
    if (isRelationalDbOutput) {
      ...
      strategy = 'Hierarchical Relational DB & Cloud Spanner Tuning Swarm';
    }

    // 13. Analyze Identity & Federated login
    const isIdentityOutput =
      outputLower.includes('firebase_auth') ||
    ...
    if (isIdentityOutput) {
      downstreamSequence.push(
        {
          agentId: 'GCP Google Cloud IAM & Google Cloud IAM External Identities Specialist',
          task: 'Configure enterprise identity federations and custom JWT tokens',
        },
        {
          agentId: 'Google Cloud Security Command Center (SCC) Zero-Trust Security Auditor',
          task: 'Audit user access controls and authentication boundaries',
        },
      );
      strategy = 'Hierarchical IAM & Federated Identity Swarm';
    }

    // 14. Analyze Event Grid and serverless functions triggers
    const isFunctionsMeshOutput =
      outputLower.includes('cloudfunction') ||
      outputLower.includes('cloudfunctions') ||
      outputLower.includes('eventgrid') ||
      ...
      outputLower.includes('servicebus_trigger') ||
      outputLower.includes('serverless_function');
    if (isFunctionsMeshOutput) {
      downstreamSequence.push(
        {
          agentId: 'GCP Google Cloud Functions & Event Grid Micro-Mesh Specialist',
    ...
    // 15. Analyze Key Vault Secrets & Key Vault encryption keys
    const isKmsOfficerOutput =
      outputLower.includes('secretmanager') ||
      outputLower.includes('keyvault') ||
    ...
    if (isKmsOfficerOutput) {
      downstreamSequence.push(
        ...
        {
          agentId: 'Security & OWASP Hardening Sentinel',
          task: 'Scan configurations for encryption and secrets leaks compliance',
        },
      );
      strategy = 'Hierarchical Cryptographic & KMS Officer Swarm';
    }

    // 16. Analyze GCP Pipelines & Artifact Registry CI/CD
    const isBuildCicdOutput =
      outputLower.includes('cloudbuild') ||
    ...
    if (isBuildCicdOutput) {
      downstreamSequence.push(
        ...
        {
          agentId: 'Kubernetes Manifest & Helm Compiler Agent',
          task: 'Audit pipeline deployment manifests',
        },
      );
    ...
    const isEdgeNetworkOutput =
      outputLower.includes('cloudcdn') ||
    ...
    const isDlpGovernanceOutput =
      outputLower.includes('data_catalog') ||
    ...
    if (isDlpGovernanceOutput) {
      downstreamSequence.push(
        ...
        {
          agentId: 'Google Cloud Security Command Center (SCC) Zero-Trust Security Auditor',
          task: 'Verify data privacy access perimeters',
        },
      );
    ...
    const isFeatureStoreOutput =
      outputLower.includes('feature_store') ||
      ...
      outputLower.includes('vertex_ai_endpoint') ||
    ...
    if (isFeatureStoreOutput) {
      downstreamSequence.push(
        {
          agentId:
            'GCP GCP AI Studio Feature Store & Model Registry Specialist',
    ...
    const isRunJobsOutput =
      outputLower.includes('containerapps_job') ||
    ...
    const isDnsFailoverOutput =
      outputLower.includes('dnssec') ||
    ...
    // 22. Analyze Cloud Spanner Columnar Engine and Cache configurations
    const isAlloydbColumnarOutput =
      outputLower.includes('columnar_engine') ||
    ...
    // 23. Analyze Cloud Spanner Graph property databases
    const isSpannerDbGraphOutput =
      outputLower.includes('spanner_graph') ||
    ...
    const isVertexAiVectorOutput =
      outputLower.includes('matching_engine') ||
    ...
    if (isVertexAiVectorOutput) {
      downstreamSequence.push({
        agentId:
          'GCP GCP AI Studio Vector Search & Matching Engine Specialist',
    ...
    const isSynapseAnalyticsOmniOutput =
      outputLower.includes('synapse_omni') ||
    ...
    const isBigtableSpecialistOutput =
      outputLower.includes('bigtable_schema') ||
    ...
    const isMemorystoreTunerOutput =
      outputLower.includes('memorystore_redis') ||
    ...
    const isNatNetworkOutput =
      outputLower.includes('cloud_nat') ||
    ...
    const isAssetAuditorOutput =
      outputLower.includes('asset_inventory') ||
    ...
    // 30. Analyze Google Cloud Composer & Airflow orchestration
    const isComposerOutput =
      outputLower.includes('composer_env') ||
    ...
    if (isComposerOutput) {
      ...
      strategy =
        'Hierarchical GCP Dataflow & Airflow Orchestration Swarm';
    }

    // 31. Analyze Cloud Dataflow & Beam pipelines
    const isDataflowOutput =
      outputLower.includes('cloud_dataflow') ||
      outputLower.includes('apache_beam') ||
    ...
    if (isDataflowOutput) {
      downstreamSequence.push(
        {
          agentId:
            'GCP Cloud GCP Stream Analytics & Apache Beam Specialist',
          task: 'Design real-time Apache Beam pipelines and optimize Cloud Dataflow execution',
        },
        {
          agentId:
            'GCP Synapse Analytics & GCP Stream Analytics Analytics Specialist',
          task: 'Optimize big data analytical downstream query structures',
        },
      );
      strategy =
        'Hierarchical Cloud GCP Stream Analytics & Beam Pipeline Swarm';
    }

    // 32. Analyze Vertex AI Feature Store Feast
    const isVertexAiFeastOutput =
      outputLower.includes('vertex_ai_feast') ||
    ...
    const isFilestoreOutput =
      outputLower.includes('cloud_filestore') ||
    ...
    if (isFilestoreOutput) {
      downstreamSequence.push(
        {
          agentId:
            'GCP Cloud GCP Files & Distributed NFS Storage Specialist',
          task: 'Configure enterprise NFS file systems and Cloud Filestore capacity tiers',
        },
        {
          agentId:
            'GCP Google Cloud Storage & Data Lifecycle Governor Specialist',
          task: 'Integrate shared file shares with object storage boundaries',
        },
      );
      strategy = 'Hierarchical Cloud GCP Files & NFS Storage Swarm';
    }

    // 34. Analyze Web Security Scanner & vulnerability auditing
    const isSecurityScannerOutput =
      outputLower.includes('web_security_scanner') ||
    ...
    if (isSecurityScannerOutput) {
      downstreamSequence.push(
        ...
        {
          agentId: 'Security & OWASP Hardening Sentinel',
          task: 'Remediate identified application vulnerabilities and secure boundaries',
        },
      );
    ...
    const isEventarcOutput =
      outputLower.includes('eventarc_mesh') ||
    ...
    if (isEventarcOutput) {
      downstreamSequence.push(
        ...
        {
          agentId: 'Google Cloud Pub/Sub Event-Driven Mesh Conductor Specialist',
          task: 'Integrate Eventarc triggers with high-throughput Pub/Sub message topics',
        },
      );
      strategy = 'Hierarchical Event Grid & CloudEvents Mesh Swarm';
    }

    // 36. Analyze Cloud SQL Auth Proxy & IAM database setup
    const isSqlProxyOutput =
      outputLower.includes('sql_auth_proxy') ||
    ...
    const isBinaryAuthorizationOutput =
      outputLower.includes('binauth_guardian') ||
    ...
    if (isBinaryAuthorizationOutput) {
      downstreamSequence.push(
        {
          agentId:
            'GCP Binary Authorization & Software Supply Chain Specialist',
      ...
      strategy =
        'Hierarchical Binary Authorization & Supply Chain Swarm';
    }

    // 38. Analyze Cloud Interconnect & Secure VPN connections
    const isInterconnectOutput =
      outputLower.includes('cloud_interconnect') ||
    ...
    const isWorkloadFederationOutput =
      outputLower.includes('workload_identity_federation') ||
      outputLower.includes('oidc_provider') ||
      outputLower.includes('gcp_identity_pool') ||
    ...
    const isGkeFleetOutput =
      outputLower.includes('gke_fleet') ||
    ...
    const isVertexAiMonitoringOutput =
      outputLower.includes('vertex_model_monitoring') ||
    ...
    const isHealthcareFhirOutput =
      outputLower.includes('healthcare_api') ||
    ...
    const isApigeeEnterpriseOutput =
      outputLower.includes('apigee_proxy') ||
    ...
    if (isApigeeEnterpriseOutput) {
      downstreamSequence.push({
        agentId:
          'GCP Google Cloud Apigee Enterprise API Management Specialist',
    ...
    const isDataplexMeshOutput =
      outputLower.includes('dataplex_lake') ||
    ...
    const isDataprocSparkOutput =
      outputLower.includes('dataproc_serverless') ||
    ...
    const isVertexAiSearchOutput =
      outputLower.includes('vertex_search_engine') ||
    ...
    if (isVertexAiSearchOutput) {
      downstreamSequence.push({
        agentId:
          'GCP GCP AI Studio Search & Conversation Agentic Specialist',
    ...
    const isSecureProxyOutput =
      outputLower.includes('secure_web_proxy') ||
    ...
    const isRunMulticontainerOutput =
      outputLower.includes('run_multicontainer') ||
    ...
    const isTpuHpcOutput =
      outputLower.includes('cloud_tpu_v5') ||
    ...
    if (isTpuHpcOutput) {
      downstreamSequence.push({
        agentId:
          'GCP Cloud TPU & High-Performance Compute Cluster Specialist',
    ...
    const isSectorFinanceOutput =
      outputLower.includes('sector_finance') ||
      outputLower.includes('iso_20022') ||
      outputLower.includes('actuarial_model');
    if (isSectorFinanceOutput) {
      downstreamSequence.push(
        {
          agentId: 'Global Investment & Commercial Banking Swarm Specialist',
          task: 'Review Basel compliance ratios, syndicated loans, and ISO 20022 messaging structures',
        },
        {
          agentId: 'Premium Actuarial Underwriting & Insurance Risk Modeler',
          task: 'Formulate actuarial reserves estimations and catastrophe PML calculations',
        },
      );
    ...
    const isSectorHealthcareOutput =
      outputLower.includes('sector_healthcare') ||
      outputLower.includes('clinical_diagnostics') ||
      outputLower.includes('bioinformatics_pharma');
    if (isSectorHealthcareOutput) {
      downstreamSequence.push(
        {
          agentId: 'Clinical Diagnostics & Medical Imaging AI Coordinator',
          task: 'Formulate DICOM annotations PACS storage guidelines and HL7/FHIR mappings',
        },
        {
          agentId: 'Bioinformatics Genomic Assembly & Biopharma Specialist',
          task: 'Analyze next-gen genomic sequencing pipelines and molecular binding affinities',
        },
      );
    ...
    const isSectorEnergyOutput =
      outputLower.includes('sector_energy') ||
      outputLower.includes('microgrid_powerflow') ||
      outputLower.includes('thermal_hydraulics');
    if (isSectorEnergyOutput) {
      downstreamSequence.push(
        {
          agentId: 'Smart Renewable Grid Optimization & Load Balancing Auditor',
          task: 'Optimize microgrid powerflows and solar battery state-of-charge degradation metrics',
        },
        {
          agentId: 'Nuclear Reactor Instrumentation & SCADA Safety Inspector',
          task: 'Monitor pressurized thermal hydraulics safety loops and encrypt SCADA Modbus networks',
        },
      );
    ...
    const isSectorIndustrialOutput =
      outputLower.includes('sector_industrial') ||
      outputLower.includes('robotic_kinematics') ||
      outputLower.includes('shop_floor_mes');
    if (isSectorIndustrialOutput) {
      downstreamSequence.push(
        {
          agentId: 'Automotive Assembly Line Robotics & PLC Coordinator',
          task: 'Formulate 6-DOF robotic kinematics trajectories and program PLC ladder logic feedback loops',
        },
        {
          agentId:
            'Manufacturing Execution System (MES) & Supply Chain Planner',
          task: 'Map WIP floor scheduling bottlenecks and audit supplier lead time buffers',
        },
      );
    ...
    const isSectorRetailOutput =
      outputLower.includes('sector_retail') ||
      ...
      downstreamSequence.push(
        {
          ...
          task: 'Formulate logarithmic demand regressions and optimize margin defense triggers',
        },
        {
          agentId: 'Hyper-Personalized Retail CRM & Retention Conductor',
          task: 'Score customer lifetime values and model churn-risk loyalty cohorts',
        },
      );
    ...
    const isSectorTransportOutput =
      outputLower.includes('sector_transport') ||
      outputLower.includes('ocean_freight_routing') ||
      outputLower.includes('vrptw_fleet_opt');
    if (isSectorTransportOutput) {
      downstreamSequence.push(
        {
          agentId: 'Intermodal Global Freight & Maritime Routing Conductor',
          task: 'Optimize container ocean vessel routes and minimize port demurrage delays',
        },
        {
          agentId: 'Autonomous Drone & Delivery Fleet Coordinator',
          task: 'Direct low-altitude flight paths airspace NFZ perimeters and solve dynamic VRPTW tasks',
        },
      );
    ...
    const isSectorAgricultureOutput =
      outputLower.includes('sector_agriculture') ||
      ...
      downstreamSequence.push(
        {
          ...
          task: 'Evaluate NDVI satellite vegetation health and calculate Penman-Monteith smart waterings',
        },
        {
          agentId: 'Macro Crop Yield Forecasting & Market Hedging Specialist',
          task: 'Predict meteorological soybean yield vectors and formulate CBOT grain future hedges',
        },
      );
    ...
    const isSectorTechMediaOutput =
      outputLower.includes('sector_tech_media') ||
      outputLower.includes('5g_6g_network_slicing') ||
      outputLower.includes('webgl_threejs_rendering');
    if (isSectorTechMediaOutput) {
      downstreamSequence.push(
        {
          agentId: 'Next-Gen 5G/6G Network Slice & Edge Compute Architect',
          task: 'Design software-defined SDN slicings and map low-latency NFV topologies',
        },
        {
    ...
    const isSectorEducationOutput =
      outputLower.includes('sector_education') ||
      outputLower.includes('adaptive_learning_graph') ||
      outputLower.includes('gaze_anomaly_detection');
    if (isSectorEducationOutput) {
      downstreamSequence.push(
        {
          agentId: 'Adaptive Curriculum Pathing & EdTech Analytics Architect',
          task: 'Establish personalized learning graphs and spacing repetitions matrices',
        },
        {
          agentId: 'Remote Proctoring & Academic Integrity Security Guard',
          task: 'Monitor remote exam gaze-detection logs and browser lockdown API violations',
        },
      );
    ...
    const isSectorGovernmentOutput =
      outputLower.includes('sector_government') ||
      outputLower.includes('traffic_signal_opt') ||
      outputLower.includes('disaster_hazard_map');
    if (isSectorGovernmentOutput) {
      downstreamSequence.push(
        {
          agentId: 'Smart Urban Municipal Infrastructure & Traffic Engineer',
          task: 'Optimize municipal signal phases public transit routes and urban lighting rules',
        },
        {
          agentId: 'Disaster Preparedness & FEMA Emergency Relief Specialist',
          task: 'Audit wildfire GIS hazard maps coordinate emergency food logistics and track recovery grants',
        },
      );
    ...
    const isRustSafetyOutput =
      outputLower.includes('rust_unsafe_audit') ||
      outputLower.includes('lifetime_sanitization');
    if (isRustSafetyOutput) {
      downstreamSequence.push({
        agentId: 'Rust Borrow Checker & Lifetime Sanitizer Specialist',
        task: 'Audit raw pointers, lifetime variables, and unsafe code boundaries',
      });
    ...
    const isGoConcurrencyOutput =
      outputLower.includes('goroutine_leak_audit') ||
      outputLower.includes('go_race_detector');
    if (isGoConcurrencyOutput) {
      downstreamSequence.push({
        agentId: 'Go Goroutine Concurrency & Race Detector Specialist',
        task: 'Audit channels select locks deadlocks and evaluate go race outputs',
      });
      strategy = 'Advanced Compiler Swarm: Go Concurrency & Race Detection';
    }

    // 62. Analyze Runtime/Compiler: Wasm Linear Memory Sandbox
    const isWasmSandboxOutput =
      outputLower.includes('wasm_linear_memory') ||
      outputLower.includes('wasi_capability_access');
    if (isWasmSandboxOutput) {
      downstreamSequence.push({
        agentId: 'Wasm WebAssembly Linear Memory Sandbox Auditor',
        task: 'Enforce Wasm linear memory bounds and map WASI secure file capabilities',
      });
      strategy = 'Advanced Compiler Swarm: Wasm Sandbox Isolation';
    }

    // 63. Analyze Runtime/Compiler: JVM GC & Memory leak tuning
    const isJvmMemoryOutput =
      outputLower.includes('jvm_gc_tuning') ||
      outputLower.includes('oom_leak_diagnosis');
    if (isJvmMemoryOutput) {
      downstreamSequence.push({
        agentId: 'JVM Garbage Collection & Memory Management Tuner',
        task: 'Calibrate JVM garbage collector generational spaces and diagnose OOM heap traces',
      });
      strategy = 'Advanced Compiler Swarm: JVM GC & Memory Optimization';
    }

    // 64. Analyze Runtime/Compiler: C/C++ Pointer Safety & Valgrind
    const isCppValgrindOutput =
      outputLower.includes('cpp_pointer_safety') ||
      outputLower.includes('valgrind_log_parse');
    if (isCppValgrindOutput) {
      downstreamSequence.push({
        agentId: 'C/C++ Pointer Safety & Valgrind Diagnostic Auditor',
        task: 'Scan pointer allocations buffer overflows and parse Valgrind leak logs',
      });
      strategy = 'Advanced Compiler Swarm: C/C++ Pointer Safety & Valgrind';
    }

    // 65. Analyze Runtime/Compiler: Python CPython GIL & AsyncIO
    const isPythonInterpreterOutput =
      outputLower.includes('gil_thread_contention') ||
      outputLower.includes('asyncio_eventloop');
    if (isPythonInterpreterOutput) {
      downstreamSequence.push({
        agentId: 'Python CPython GIL & AsyncIO Performance Optimizer',
        task: 'Analyze GIL lock contention metrics and tune AsyncIO socket select loops',
      });
      strategy = 'Advanced Compiler Swarm: Python Interpreter & GIL';
    }

    // 66. Analyze Runtime/Compiler: PostgreSQL AST & Query Plan
    const isPostgresQueryplanOutput =
      outputLower.includes('explain_plan_parsing') ||
      outputLower.includes('ast_query_rewriting');
    if (isPostgresQueryplanOutput) {
      downstreamSequence.push({
        agentId: 'PostgreSQL AST Query Execution Plan Optimizer',
        task: 'Parse EXPLAIN output graphs and rewrite SQL query AST structures',
      });
      strategy = 'Advanced Compiler Swarm: PostgreSQL AST Query Planner';
    }

    // 67. Analyze Runtime/Compiler: TS strictMode compiler
    const isTypescriptCompilerOutput =
      outputLower.includes('strict_type_checks') ||
      outputLower.includes('tsc_compiler_diagnostics');
    if (isTypescriptCompilerOutput) {
      downstreamSequence.push({
        agentId: 'TypeScript strictNullChecks & Compiler Error Resolver',
        task: 'Audit strict tsc settings and compile TS generic types declarations',
      });
    ...
    const isDockerLayersOutput =
      outputLower.includes('docker_multistage_build') ||
      outputLower.includes('layer_cache_opt');
    if (isDockerLayersOutput) {
      downstreamSequence.push({
        agentId: 'Docker Container Multi-Stage Layer Minimization Specialist',
        task: 'Audit Dockerfile build layer caches and reduce minimal base footprints',
      });
      strategy = 'Advanced Compiler Swarm: Docker Layer Minimization';
    }

    // 69. Analyze Runtime/Compiler: Kubernetes Policy & OPA Gatekeeper
    const isK8sPolicyOutput =
      outputLower.includes('opa_rego_policy') ||
      outputLower.includes('kustomize_overlays');
    if (isK8sPolicyOutput) {
      downstreamSequence.push({
        agentId: 'Kubernetes Kustomize & OPA Gatekeeper Policy Auditor',
        task: 'Audit Kubernetes Kustomize resource policies and enforce OPA rego rules',
      });
    ...
    const isConsensusOutput =
      outputLower.includes('consensus_raft_paxos') ||
    ...
    if (isConsensusOutput) {
      downstreamSequence.push({
        agentId: 'Distributed Consensus & Raft/Paxos Protocol Specialist',
        task: 'Design fault-tolerant replicated state machines and consensus protocols',
      });
    ...
    const isGraphOutput =
      outputLower.includes('graph_theory') ||
      outputLower.includes('directed_acyclic') ||
      outputLower.includes('topological_sort') ||
      outputLower.includes('cyclic_dependency');
    if (isGraphOutput) {
      downstreamSequence.push({
        agentId: 'Graph Theory & Directed Acyclic Graph (DAG) Specialist',
        task: 'Optimize directed acyclic graph structures and cycle detection routines',
      });
    ...
    const isCompressionOutput =
      outputLower.includes('lossless_compression') ||
    ...
    if (isCompressionOutput) {
      downstreamSequence.push({
        agentId: 'Huffman & LZW Lossless Data Compression Specialist',
        task: 'Design optimized lossless compression and dictionary encoding pipelines',
      });
    ...
    const isAstParserOutput =
      outputLower.includes('abstract_syntax_tree') ||
    ...
    if (isAstParserOutput) {
      downstreamSequence.push({
        agentId: 'Abstract Syntax Tree (AST) & Lexer/Parser Specialist',
        task: 'Build custom parsing lexers, tokenizers, and AST validation structures',
      });
    ...
    const isCryptographyOutput =
      outputLower.includes('zero_knowledge_proof') ||
    ...
    if (isCryptographyOutput) {
      downstreamSequence.push({
        agentId: 'Zero-Knowledge Proof & Cryptographic Protocol Specialist',
        task: 'Formulate mathematical zero-knowledge proof protocols and secure cryptography parameters',
      });
    ...
    const isOptimizationOutput =
      outputLower.includes('np_hard_optimization') ||
    ...
    if (isOptimizationOutput) {
      downstreamSequence.push({
        agentId: 'NP-Hard & Integer Linear Programming (ILP) Specialist',
        task: 'Design linear programming solvers and heuristic optimizations for NP-hard problems',
      });
    ...
    const isGeometryOutput =
      outputLower.includes('hnsw_indexing') ||
    ...
    if (isGeometryOutput) {
      downstreamSequence.push({
        agentId: 'Vector Index HNSW & High-Dimensional Geometry Specialist',
        task: 'Optimize high-dimensional geometric partitioning and vector search indices',
      });
    ...
    const isHashingOutput =
      outputLower.includes('consistent_hashing') ||
      outputLower.includes('dht_protocol') ||
      outputLower.includes('cache_eviction_opt') ||
      outputLower.includes('distributed_cache_ring');
    if (isHashingOutput) {
      downstreamSequence.push({
        agentId: 'Consistent Hashing & DHT Distributed Cache Specialist',
        task: 'Design virtual nodes and consistent hashing distribution cache models',
      });
    ...
    const isTimeseriesOutput =
      outputLower.includes('kalman_filtering') ||
    ...
    if (isTimeseriesOutput) {
      downstreamSequence.push({
        agentId: 'Time-Series Forecasting & Kalman Filter Specialist',
        task: 'Implement recursive Kalman filtering and signal processing models',
      });
    ...
    const isStorageEngineOutput =
      outputLower.includes('lsm_tree_storage') ||
    ...
    if (isStorageEngineOutput) {
      downstreamSequence.push({
        agentId: 'B-Tree & LSM-Tree Storage Engine Specialist',
        task: 'Architect high-throughput LSM-Tree storage compaction and crash recovery systems',
      });
    ...
    const isCqrsOutput =
      outputLower.includes('cqrs_read_write_segregation') ||
    ...
    if (isCqrsOutput) {
      downstreamSequence.push({
        agentId: 'CQRS & Event Sourcing Architect',
        task: 'Design segregated read/write paths and transaction-safe event replay projections',
      });
    ...
    const isMicroservicesOutput =
      outputLower.includes('saga_transaction_coordination') ||
    ...
    if (isMicroservicesOutput) {
      downstreamSequence.push({
        agentId: 'Choreographed & Orchestrated Microservices Coordinator',
        task: 'Orchestrate Saga transactions, outbox publication pipelines, and API Gateway mesh routes',
      });
    ...
    const isEventDrivenOutput =
      outputLower.includes('message_broker_topology') ||
    ...
    if (isEventDrivenOutput) {
      downstreamSequence.push({
        agentId: 'Event-Driven Architecture & Message Broker Specialist',
        task: 'Structure message broker queue topologies and optimize DLQ retry-backoff paths',
      });
    ...
    const isServerlessOutput =
      outputLower.includes('faas_concurrency_sizing') ||
    ...
    if (isServerlessOutput) {
      downstreamSequence.push({
        agentId: 'Serverless Micro-Mesh & Cloud Workflows Architect',
        task: 'Configure FaaS memory profiles, cold start mitigations, and serverless workflow steps',
      });
    ...
    const isCachingOutput =
      outputLower.includes('cache_access_topologies') ||
    ...
    if (isCachingOutput) {
      downstreamSequence.push({
        agentId: 'Multi-Tier Distributed Caching Architect',
        task: 'Formulate cache access structures and Redis Cluster connection optimizations',
      });
    ...
    const isDatabasesOutput =
      outputLower.includes('database_sharding_keys') ||
    ...
    if (isDatabasesOutput) {
      downstreamSequence.push({
        agentId: 'Distributed Database Sharding & Replication Specialist',
        task: 'Optimize database horizontal sharding maps and failover replication steps',
      });
    ...
    const isHybridCloudOutput =
      outputLower.includes('hybrid_interconnect_vpn') ||
    ...
    if (isHybridCloudOutput) {
      downstreamSequence.push({
        agentId: 'Hybrid & Multi-Cloud Infrastructure Architect',
        task: 'Design secure multi-cloud federations, VPC transit routes, and Workload Identity pools',
      });
    ...
    const isResilienceOutput =
      outputLower.includes('circuit_breaker_states') ||
    ...
    if (isResilienceOutput) {
      downstreamSequence.push({
        agentId: 'Site Reliability & High-Availability Chaos Engineer',
        task: 'Enforce circuit breaker limits, bulkhead isolation pool sizing, and retry backoffs',
      });
    ...
    const isObservabilityOutput =
      outputLower.includes('telemetry_tracing_propagation') ||
    ...
    if (isObservabilityOutput) {
      downstreamSequence.push({
        agentId: 'Observability & OpenTelemetry APM Architect',
        task: 'Examine trace context headers propagation and continuous V8 cpu profiling metrics',
      });
    ...
    const isRealtimeOutput =
      outputLower.includes('websocket_sfu_connections') ||
    ...
    if (isRealtimeOutput) {
      downstreamSequence.push({
        agentId: 'Real-Time WebSockets & CRDT Collaboration Specialist',
        task: 'Structure low-latency WebSockets collaborative server synchronization loops',
      });
    ...
    const isPromptEngineerOutput =
      outputLower.includes('prompt_template_design') ||
    ...
    if (isPromptEngineerOutput) {
      downstreamSequence.push({
        agentId: 'LLM Prompt Optimization & Few-Shot Specialist',
        task: 'Optimize dynamic LLM prompt envelopes and system prompt shielding boundaries',
      });
    ...
    const isRagOptimizerOutput =
      outputLower.includes('semantic_chunking_strategies') ||
    ...
    if (isRagOptimizerOutput) {
      downstreamSequence.push({
        agentId: 'Retrieval-Augmented Generation (RAG) & Chunking Specialist',
        task: 'Design recursive sliding window chunking maps and hybrid vector search retrievals',
      });
      strategy = 'Advanced AI Swarm: RAG & Chunking Optimization';
    }

    // 92. AI Engineering: Vector Database & Indexing Specialist
    const isVectorTunerOutput =
      outputLower.includes('hnsw_index_tuning') ||
    ...
    if (isVectorTunerOutput) {
      downstreamSequence.push({
        agentId: 'Vector Database & Indexing Specialist',
        task: 'Tune high-dimensional HNSW index parameter profiles and metadata filter caches',
      });
      strategy = 'Advanced AI Swarm: Vector Database & Indexing';
    }

    // 93. AI Engineering: LLM Fine-Tuning & PEFT Specialist
    const isFineTuningOutput =
      outputLower.includes('lora_peft_tuning') ||
    ...
    if (isFineTuningOutput) {
      downstreamSequence.push({
        agentId: 'LLM Fine-Tuning & PEFT Specialist',
        task: 'Configure LoRA rank adapters, Instruction JSONL formatting pipelines, and DPO alignments',
      });
      strategy = 'Advanced AI Swarm: LLM Fine-Tuning & PEFT';
    }

    // 94. AI Engineering: Multi-Agent Frameworks & Orchestrator Specialist
    const isAgentOrchestratorOutput =
      outputLower.includes('multi_agent_graphs') ||
    ...
    if (isAgentOrchestratorOutput) {
      downstreamSequence.push({
        agentId: 'Multi-Agent Frameworks & Orchestrator Specialist',
        task: 'Formulate crewAI multi-agent supervisors patterns and loop monitors state updates',
      });
      strategy = 'Advanced AI Swarm: Multi-Agent Frameworks';
    }

    // 95. AI Engineering: AI Safety & Guardrails Officer
    const isGuardrailsOutput =
      outputLower.includes('jailbreak_detection_gates') ||
    ...
    if (isGuardrailsOutput) {
      downstreamSequence.push({
        agentId: 'AI Safety & Guardrails Officer',
        task: 'Enforce LlamaGuard toxicity moderators and hallucination mitigation filters',
      });
      strategy = 'Advanced AI Swarm: Safety & Guardrails';
    }

    // 96. AI Engineering: LLM Evaluation & Benchmarking Auditor
    const isLlmEvaluatorOutput =
      outputLower.includes('automated_benchmarking_harness') ||
    ...
    if (isLlmEvaluatorOutput) {
      downstreamSequence.push({
        agentId: 'LLM Evaluation & Benchmarking Auditor',
        task: 'Generate synthetic evaluation datasets and evaluate faithfulness scoring metrics',
      });
      strategy = 'Advanced AI Swarm: Evaluation & Benchmarking';
    }

    // 97. AI Engineering: Model Inference Serving & Quantization Optimizer
    const isModelServingOutput =
      outputLower.includes('inference_engine_tuning') ||
    ...
    if (isModelServingOutput) {
      downstreamSequence.push({
        agentId: 'Model Inference Serving & Quantization Optimizer',
        task: 'Calibrate vLLM serving parameters and INT4/FP8 quantization configurations',
      });
      strategy = 'Advanced AI Swarm: Serving & Quantization';
    }

    // 98. AI Engineering: Semantic Prompt Caching Specialist
    const isSemanticCacheOutput =
      outputLower.includes('semantic_caching_thresholds') ||
    ...
    if (isSemanticCacheOutput) {
      downstreamSequence.push({
        agentId: 'Semantic Prompt Caching Specialist',
        task: 'Tune semantic cosine similarity thresholds and cache invalidation policies',
      });
      strategy = 'Advanced AI Swarm: Semantic Prompt Caching';
    }

    // 99. AI Engineering: Synthetic Dataset Generator & Data Augmenter
    const isSyntheticDataOutput =
      outputLower.includes('synthetic_data_pipelines') ||
    ...
    if (isSyntheticDataOutput) {
      downstreamSequence.push({
        agentId: 'Synthetic Dataset Generator & Data Augmenter',
        task: 'Orchestrate high-diversity synthetic questions data pipelines and outlier checks',
      });
      strategy = 'Advanced AI Swarm: Synthetic Data & Augmentation';
    }

    // 100. Data Engineering: Apache Kafka & Event Stream Tuning Specialist
    const isKafkaTunerOutput =
      outputLower.includes('kafka_partition_calibration') ||
    ...
    if (isKafkaTunerOutput) {
      downstreamSequence.push({
        agentId: 'Apache Kafka & Event Stream Tuning Specialist',
        task: 'Troubleshoot consumer offset lag patterns and calibrate idempotent producers',
      });
    ...
    const isSparkOptimizerOutput =
      outputLower.includes('spark_shuffle_partition_optimization') ||
    ...
    if (isSparkOptimizerOutput) {
      downstreamSequence.push({
        agentId: 'Apache Spark & Large-Scale Batch Tuning Specialist',
        task: 'Tune AQE shuffle partition limits and optimize executor heap storage allocations',
      });
      strategy = 'Data Engineering Swarm: Apache Spark Optimization';
    }

    // 102. Data Engineering: Apache Flink & Real-Time Stream Processor
    const isFlinkStreamOutput =
      outputLower.includes('stateful_stream_computations') ||
    ...
    if (isFlinkStreamOutput) {
      downstreamSequence.push({
        agentId: 'Apache Flink & Real-Time Stream Processor',
        task: 'Configure RocksDB state backends and define watermarks for out-of-order streams',
      });
      strategy = 'Data Engineering Swarm: Apache Flink Streaming';
    }

    // 103. Data Engineering: Snowflake & Enterprise Data Warehouse Architect
    const isSnowflakeDwhOutput =
      outputLower.includes('micro_partitioning_alignment') ||
    ...
    if (isSnowflakeDwhOutput) {
      downstreamSequence.push({
        agentId: 'Snowflake & Enterprise Data Warehouse Architect',
        task: 'Design micro-partition pruning strategies and define auto-suspending warehouses',
      });
      strategy = 'Data Engineering Swarm: Snowflake Data Warehousing';
    }

    // 104. Data Engineering: dbt (Data Build Tool) Analytics Engineer
    const isDbtModelerOutput =
      outputLower.includes('incremental_materialization_strategies') ||
    ...
    if (isDbtModelerOutput) {
      downstreamSequence.push({
        agentId: 'dbt (Data Build Tool) Analytics Engineer',
        task: 'Formulate incremental ref models unique keys and write data validation tests',
      });
      strategy = 'Data Engineering Swarm: dbt Analytics Engineering';
    }

    // 105. Data Engineering: Apache Airflow & Workflow Conductor
    const isAirflowGovernorOutput =
      outputLower.includes('dynamic_dag_generation') ||
    ...
    if (isAirflowGovernorOutput) {
      downstreamSequence.push({
        agentId: 'Apache Airflow & Workflow Conductor',
        task: 'Generate dynamic DAG structures and coordinate TaskFlow SLA callback triggers',
      });
      strategy = 'Data Engineering Swarm: Apache Airflow Orchestration';
    }

    // 106. Data Engineering: ClickHouse & Columnar OLAP Specialist
    const isClickhouseOlapOutput =
      outputLower.includes('mergetree_engine_tuning') ||
    ...
    if (isClickhouseOlapOutput) {
      downstreamSequence.push({
        agentId: 'ClickHouse & Columnar OLAP Specialist',
        task: 'Configure CollapsingMergeTree indices and structure analytical materialized views',
      });
      strategy = 'Data Engineering Swarm: ClickHouse Columnar OLAP';
    }

    // 107. Data Engineering: Apache Iceberg & Open Table Format Specialist
    const isIcebergLakeOutput =
      outputLower.includes('acid_transactions_lakehouse') ||
    ...
    if (isIcebergLakeOutput) {
      downstreamSequence.push({
        agentId: 'Apache Iceberg & Open Table Format Specialist',
        task: 'Structure schema evolution maps and design dynamic compaction routines',
      });
      strategy = 'Data Engineering Swarm: Apache Iceberg Lakehouse';
    }

    // 108. Data Engineering: Data Lineage & Governance Officer
    const isDataLineageOutput =
      outputLower.includes('metadata_scanning_maps') ||
    ...
    if (isDataLineageOutput) {
      downstreamSequence.push({
        agentId: 'Data Lineage & Governance Officer',
        task: 'Integrate OpenLineage collection hooks and detect sensitive PII fields',
      });
      strategy = 'Data Engineering Swarm: Governance & Lineage';
    }

    // 109. Data Engineering: Apache Pulsar & Multi-Tenant Messaging Specialist
    const isPulsarBrokerOutput =
      outputLower.includes('pulsar_multi_tenancy_namespaces') ||
    ...
    if (isPulsarBrokerOutput) {
      downstreamSequence.push({
        agentId: 'Apache Pulsar & Multi-Tenant Messaging Specialist',
        task: 'Configure multi-cluster active-active geo-replication namespaces',
      });
      strategy = 'Data Engineering Swarm: Apache Pulsar Messaging';
    }

    // 110. Security Engineering: Static Analysis & CVE Auditing Specialist
    const isCveAuditorOutput =
      outputLower.includes('dependency_vulnerability_scanning') ||
    ...
    if (isCveAuditorOutput) {
      downstreamSequence.push({
        agentId: 'Static Analysis & CVE Auditing Specialist',
        task: 'Scan dependency trees for out-of-date locks and calibrate SAST Semgrep rules',
      });
      strategy = 'Cybersecurity Swarm: CVE & Static Code Analysis';
    }

    // 111. Security Engineering: Automated Penetration Tester & Red Teamer
    const isPenetrationOutput =
      outputLower.includes('sql_injection_penetration') ||
    ...
    if (isPenetrationOutput) {
      downstreamSequence.push({
        agentId: 'Automated Penetration Tester & Red Teamer',
        task: 'Simulate SQL injection vectors and input bypass payloads to verify filters robustness',
      });
      strategy = 'Cybersecurity Swarm: Penetration & Red Teaming';
    }

    // 112. Security Engineering: Cryptographic Key & Key Vault Secrets Sentinel
    const isKmsSentinelOutput =
      outputLower.includes('keyvault_cryptographic_key_rotation') ||
    ...
    if (isKmsSentinelOutput) {
      downstreamSequence.push({
        agentId: 'Cryptographic Key & Key Vault Secrets Sentinel',
        task: 'Enforce AES-GCM envelope encryption configurations and audit token scanning alerts',
      });
      strategy = 'Cybersecurity Swarm: Cryptographic Keys & Secrets';
    }

    // 113. Security Engineering: Zero-Trust Network Perimeter Auditor
    const isZeroTrustOutput =
      outputLower.includes('vpc_service_connect_perimeters') ||
    ...
    if (isZeroTrustOutput) {
      downstreamSequence.push({
        agentId: 'Zero-Trust Network Perimeter Auditor',
        task: 'Audit VPC service perimeters boundaries and mutual TLS Istio access mesh policies',
      });
      strategy = 'Cybersecurity Swarm: Zero-Trust Network Mesh';
    }

    // 114. Security Engineering: IAM & Identity Privilege Sentinel
    const isIamGovernorOutput =
      outputLower.includes('cross_account_role_assumption') ||
    ...
    if (isIamGovernorOutput) {
      downstreamSequence.push({
        agentId: 'IAM & Identity Privilege Sentinel',
        task: 'Trim unused permissions policies and map federated OIDC identity pool boundaries',
      });
      strategy = 'Cybersecurity Swarm: IAM & Identity Privileges';
    }

    // 115. Security Engineering: Threat Modeling & Attack Graph Specialist
    const isThreatModelingOutput =
      outputLower.includes('stride_threat_model_classification') ||
      outputLower.includes('streamanalytics_trust_boundary_mapping') ||
    ...
    if (isThreatModelingOutput) {
      downstreamSequence.push({
        agentId: 'Threat Modeling & Attack Graph Specialist',
        task: 'Map trust boundaries architecture diagrams and score vulnerabilities exploitability levels',
      });
      strategy = 'Cybersecurity Swarm: Threat Modeling & Attack Graphs';
    }

    // 116. Security Engineering: SOC2, PCI-DSS & HIPAA Compliance Auditor
    const isComplianceOutput =
      outputLower.includes('soc2_trust_services_audit') ||
    ...
    if (isComplianceOutput) {
      downstreamSequence.push({
        agentId: 'SOC2, PCI-DSS & HIPAA Compliance Auditor',
        task: 'Run compliance-as-code Open Policy Agent assertions against cloud configurations',
      });
      strategy = 'Cybersecurity Swarm: Compliance & Auditing';
    }

    // 117. Security Engineering: WAF & DDoS Mitigation Coordinator
    const isWafSentinelOutput =
      outputLower.includes('waf_custom_rule_exclusions') ||
    ...
    if (isWafSentinelOutput) {
      downstreamSequence.push({
        agentId: 'WAF & DDoS Mitigation Coordinator',
        task: 'Examine rate limiting profiles and SSL/TLS cipher suite security enforcement limits',
      });
      strategy = 'Cybersecurity Swarm: WAF & DDoS Protection';
    }

    // 118. Security Engineering: Binary Authorization & Supply Chain Guardian
    const isBinaryAuthOutput =
      outputLower.includes('container_cryptographic_signing') ||
    ...
    if (isBinaryAuthOutput) {
      downstreamSequence.push({
        agentId: 'Binary Authorization & Supply Chain Guardian',
        task: 'Enforce container SLSA provenance verifications and check SBOM vulnerability alerts',
      });
      strategy = 'Cybersecurity Swarm: Software Supply Chain Trust';
    }

    // 119. Security Engineering: SIEM & Secure Audit Trail Specialist
    const isAuditLoggerOutput =
      outputLower.includes('immutable_log_sinks') ||
    ...
    if (isAuditLoggerOutput) {
      downstreamSequence.push({
        agentId: 'SIEM & Secure Audit Trail Specialist',
        task: 'Structure secure write-once-read-many log sinks and SIEM Chronicle parser maps',
      });
      strategy = 'Cybersecurity Swarm: SIEM & Immutable Audit Logs';
    }

    // 120. Platform Engineering: Terraform State & IaC Optimizer
    const isTerraformOptimizerOutput =
      outputLower.includes('terraform_state_lock_auditing') ||
    ...
    if (isTerraformOptimizerOutput) {
      downstreamSequence.push({
        agentId: 'Terraform State & IaC Optimizer',
        task: 'Audit state lock files and validate HCL module dependencies',
      });
      strategy = 'Platform Engineering Swarm: Terraform & IaC';
    }

    // 121. Platform Engineering: Ansible Configuration & Playbook Specialist
    const isAnsibleSpecialistOutput =
      outputLower.includes('play_execution_concurrency') ||
    ...
    if (isAnsibleSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'Ansible Configuration & Playbook Specialist',
        task: 'Optimize Ansible task execution concurrency and dynamic inventory groups',
      });
      strategy = 'Platform Engineering Swarm: Ansible Configurations';
    }

    // 122. Platform Engineering: Helm Chart & Package Manager Architect
    const isHelmArchitectOutput =
      outputLower.includes('values_overlays_structure') ||
    ...
    if (isHelmArchitectOutput) {
      downstreamSequence.push({
        agentId: 'Helm Chart & Package Manager Architect',
        task: 'Evaluate Helm values schema overlays and dynamic template dependency trees',
      });
    ...
    const isDockerSpecialistOutput =
      outputLower.includes('docker_socket_security_access') ||
    ...
    if (isDockerSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'Docker Engine & Host Virtualization Specialist',
        task: 'Enforce container multi-architecture builds and Docker socket host security policies',
      });
    ...
    const isConsulSpecialistOutput =
      outputLower.includes('catalog_synchronization_periods') ||
    ...
    if (isConsulSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'Consul Service Discovery & Key-Value Specialist',
        task: 'Tune Consul service discovery synchronization periods and Raft consensus configuration states',
      });
      strategy = 'Platform Engineering Swarm: Consul Discovery & KV';
    }

    // 125. Platform Engineering: Prometheus Telemetry & Alerting Specialist
    const isPrometheusSpecialistOutput =
      outputLower.includes('metrics_scraping_intervals') ||
    ...
    if (isPrometheusSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'Prometheus Telemetry & Alerting Specialist',
        task: 'Calibrate metrics scraping timeouts and TSDB storage block compaction parameters',
      });
    ...
    const isNginxSpecialistOutput =
      outputLower.includes('server_proxy_blocks_configurations') ||
    ...
    if (isNginxSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'NGINX Reverse Proxy & HTTP Engine Specialist',
        task: 'Tune HTTP keepalive session buffers and optimize reverse proxy upstream load balancing limits',
      });
      strategy = 'Platform Engineering Swarm: NGINX Proxy & Reverse Web';
    }

    // 127. Platform Engineering: ArgoCD GitOps Git Sync Specialist
    const isArgoCdSpecialistOutput =
      outputLower.includes('application_synchronization_queues') ||
    ...
    if (isArgoCdSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'ArgoCD GitOps Git Sync Specialist',
        task: 'Monitor ArgoCD dynamic target sync windows and map matrix applicationset definitions',
      });
      strategy = 'Platform Engineering Swarm: ArgoCD GitOps';
    }

    // 128. Platform Engineering: Envoy Proxy & Service Mesh Specialist
    const isEnvoySpecialistOutput =
      outputLower.includes('dynamic_cluster_discovery_xds') ||
    ...
    if (isEnvoySpecialistOutput) {
      downstreamSequence.push({
        agentId: 'Envoy Proxy & Service Mesh Specialist',
        task: 'Integrate dynamic cluster discovery EDS metrics and monitor Envoy circuit breaker filter status',
      });
      strategy = 'Platform Engineering Swarm: Envoy Mesh & Sidecars';
    }

    // 129. Platform Engineering: Packer Machine Image & Template Specialist
    const isPackerSpecialistOutput =
      outputLower.includes('vm_golden_image_builds') ||
    ...
    if (isPackerSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'Packer Machine Image & Template Specialist',
        task: 'Calibrate golden machine VM provisioning flows and orchestrate image compression post-processors',
      });
    ...
    const isCrabboxSpecialistOutput =
      outputLower.includes('remote_lease_lifecycle_management') ||
    ...
    if (isCrabboxSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'Crabbox Remote Sandbox & Secure Execution Specialist',
        task: 'Manage remote sandbox environment leases, synchronize code diffs, and execute sandboxed code securely',
      });
      strategy = 'Platform Engineering Swarm: Crabbox Sandboxing';
    }

    // 130. API Systems: GraphQL Federation & Supergraph Architect
    const isGraphQlFederationOutput =
      outputLower.includes('graphql_federation_supergraph_routing') ||
    ...
    if (isGraphQlFederationOutput) {
      downstreamSequence.push({
        agentId: 'GraphQL Federation & Supergraph Architect',
        task: 'Design federated supergraph schemas and analyze subgraphs entity resolution query costs',
      });
      strategy = 'API Swarm: GraphQL Federation';
    }

    // 131. API Systems: gRPC & ProtoBuf Interface Optimizer
    const isGrpcOptimizerOutput =
      outputLower.includes('grpc_protobuf_binary_serialization') ||
    ...
    if (isGrpcOptimizerOutput) {
      downstreamSequence.push({
        agentId: 'gRPC & ProtoBuf Interface Optimizer',
        task: 'Optimize ProtoBuf message structures and evaluate HTTP/2 flow control multiplexed streams',
      });
      strategy = 'API Swarm: gRPC & ProtoBuf';
    }

    // 132. API Systems: WebSockets & Real-Time Sync Coordinator
    const isWebSocketSyncOutput =
      outputLower.includes('websocket_connection_heartbeat_tuning') ||
      outputLower.includes('servicebus_channel_state_distribution') ||
    ...
    if (isWebSocketSyncOutput) {
      downstreamSequence.push({
        agentId: 'WebSockets & Real-Time Sync Coordinator',
        task: 'Manage websocket reconnect limits and scale state distributions using Redis socket adapters',
      });
      strategy = 'API Swarm: Real-Time WebSockets';
    }

    // 133. API Systems: OpenAPI Contract & Schema Enforcer
    const isOpenApiEnforcerOutput =
      outputLower.includes('openapi_contract_validation_assertions') ||
    ...
    if (isOpenApiEnforcerOutput) {
      downstreamSequence.push({
        agentId: 'OpenAPI Contract & Schema Enforcer',
        task: 'Enforce OpenAPI schema structures and evaluate Mock integration test assertion rules',
      });
      strategy = 'API Swarm: OpenAPI Specs & Contract Tests';
    }

    // 134. API Systems: Apache Kafka Event-Driven Architect
    const isKafkaEventArchitectOutput =
      outputLower.includes('kafka_partition_key_rebalancing') ||
    ...
    if (isKafkaEventArchitectOutput) {
      downstreamSequence.push({
        agentId: 'Apache Kafka Event-Driven Architect',
        task: 'Audit Kafka partition key distribution schemas and configure exactly-once event transactional flows',
      });
      strategy = 'API Swarm: Kafka Event Streaming';
    }

    // 135. API Systems: RabbitMQ AMQP Broker Specialist
    const isRabbitMqBrokerOutput =
      outputLower.includes('amqp_exchange_routing_bindings') ||
    ...
    if (isRabbitMqBrokerOutput) {
      downstreamSequence.push({
        agentId: 'RabbitMQ AMQP Broker Specialist',
        task: 'Map RabbitMQ dead-letter exchange bindings and balance consumer prefetch limits',
      });
      strategy = 'API Swarm: RabbitMQ Messaging';
    }

    // 136. API Systems: APIs Gateway Policy Controller
    const isApiGatewayControllerOutput =
      outputLower.includes('api_gateway_request_transformations') ||
    ...
    if (isApiGatewayControllerOutput) {
      downstreamSequence.push({
        agentId: 'APIs Gateway Policy Controller',
        task: 'Configure gateway request header rewrites and monitor OAuth2/JWT verification boundaries',
      });
      strategy = 'API Swarm: APIs Gateways & Policies';
    }

    // 137. API Systems: JSON-RPC & XML-RPC Protocol Specialist
    const isRpcProtocolSpecialistOutput =
      outputLower.includes('rpc_request_batching_payloads') ||
    ...
    if (isRpcProtocolSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'JSON-RPC & XML-RPC Protocol Specialist',
        task: 'Audit JSON-RPC batching payloads and standard error code dispatch structures',
      });
      strategy = 'API Swarm: JSON-RPC & XML-RPC Protocols';
    }

    // 138. API Systems: tRPC Type-Safe Contract Coordinator
    const isTrpcContractOutput =
      outputLower.includes('trpc_client_server_type_propagation') ||
    ...
    if (isTrpcContractOutput) {
      downstreamSequence.push({
        agentId: 'tRPC Type-Safe Contract Coordinator',
        task: 'Validate tRPC procedural type exports and scale middleware query batching limits',
      });
      strategy = 'API Swarm: tRPC Type-Safe Contracts';
    }

    // 139. API Systems: WebRTC Peer-to-Peer & SFU Engine Specialist
    const isWebRtcSpecialistOutput =
      outputLower.includes('webrtc_signaling_ice_negotiations') ||
    ...
    if (isWebRtcSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'WebRTC Peer-to-Peer & SFU Engine Specialist',
        task: 'Optimize WebRTC ICE negotiation handshakes and manage SFU media layer allocations',
      });
      strategy = 'API Swarm: WebRTC Peer-to-Peer & SFU Channels';
    }

    // 140. Edge Systems: iOS Swift & CocoaTouch Specialist
    const isIosSpecialistOutput =
      outputLower.includes('swift_concurrency_structured_async') ||
    ...
    if (isIosSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'iOS Swift & CocoaTouch Specialist',
        task: 'Audit structured async Swift tasks and profile weak references inside reference graphs',
      });
      strategy = 'Edge Swarm: iOS Development';
    }

    // 141. Edge Systems: Android Kotlin & Jetpack Compose Architect
    const isAndroidArchitectOutput =
      outputLower.includes('kotlin_coroutine_flow_orchestration') ||
    ...
    if (isAndroidArchitectOutput) {
      downstreamSequence.push({
        agentId: 'Android Kotlin & Jetpack Compose Architect',
        task: 'Tune Jetpack Compose state hoisting recomposition scopes and audit Room database transactions',
      });
      strategy = 'Edge Swarm: Android Development';
    }

    // 142. Edge Systems: Kotlin Multiplatform (KMP) Shared Core Specialist
    const isKmpSpecialistOutput =
      outputLower.includes('kmp_expect_actual_platform_mappings') ||
    ...
    if (isKmpSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'Kotlin Multiplatform (KMP) Shared Core Specialist',
        task: 'Orchestrate KMP shared common modules and configure cross-platform Ktor engines',
      });
      strategy = 'Edge Swarm: Kotlin Multiplatform';
    }

    // 143. Edge Systems: React Native Bridge & Native Modules Specialist
    const isReactNativeSpecialistOutput =
      outputLower.includes('jsi_native_bridge_communication') ||
    ...
    if (isReactNativeSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'React Native Bridge & Native Modules Specialist',
        task: 'Bridge native Fabric components and debug JavaScript thread event loops',
      });
      strategy = 'Edge Swarm: React Native Bridges';
    }

    // 144. Edge Systems: Electron Desktop & Native Interop Architect
    const isElectronArchitectOutput =
      outputLower.includes('electron_ipc_renderer_security_sandbox') ||
    ...
    if (isElectronArchitectOutput) {
      downstreamSequence.push({
        agentId: 'Electron Desktop & Native Interop Architect',
        task: 'Enforce preload contextBridge security perimeters and compile node-gyp bindings',
      });
      strategy = 'Edge Swarm: Electron Desktop';
    }

    // 145. Edge Systems: WebAssembly WebAPI & WASI Compiler Specialist
    const isWasmSpecialistOutput =
      outputLower.includes('wasi_filesystem_abstraction_layers') ||
    ...
    if (isWasmSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'WebAssembly WebAPI & WASI Compiler Specialist',
        task: 'Compile high-fidelity C++/Rust modules and configure WASI filesystem abstractions',
      });
      strategy = 'Edge Swarm: WebAssembly Systems';
    }

    // 146. Edge Systems: Embedded RTOS & Microcontroller Systems Specialist
    const isEmbeddedSpecialistOutput =
      outputLower.includes('rtos_kernel_scheduler_boundaries') ||
    ...
    if (isEmbeddedSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'Embedded RTOS & Microcontroller Systems Specialist',
        task: 'Audit FreeRTOS kernel mutex locks and debug peripheral microcontroller registers',
      });
      strategy = 'Edge Swarm: Embedded RTOS Hardware';
    }

    // 147. Edge Systems: WebUSB & WebBluetooth System Interop Specialist
    const isWebUsbSpecialistOutput =
      outputLower.includes('webusb_endpoint_packet_transfers') ||
    ...
    if (isWebUsbSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'WebUSB & WebBluetooth System Interop Specialist',
        task: 'Parse hardware vendor buffers and manage GATT service characteristic callbacks',
      });
      strategy = 'Edge Swarm: WebUSB & WebBluetooth';
    }

    // 148. Edge Systems: Edge Compute Serverless & CDN Architect
    const isEdgeServerlessOutput =
      outputLower.includes('cloudflare_workers_v8_isolates_compute') ||
    ...
    if (isEdgeServerlessOutput) {
      downstreamSequence.push({
        agentId: 'Edge Compute Serverless & CDN Architect',
        task: 'Map distributed V8 serverless worker isolates and optimize edge invalidation cache tags',
      });
      strategy = 'Edge Swarm: Edge Serverless & CDN';
    }

    // 149. Edge Systems: Progressive Web Application (PWA) Offline Sync Specialist
    const isPwaSpecialistOutput =
      outputLower.includes('service_worker_lifecycle_caching') ||
    ...
    if (isPwaSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'Progressive Web Application (PWA) Offline Sync Specialist',
        task: 'Calibrate Workbox background sync queues and structure payload push notification keys',
      });
      strategy = 'Edge Swarm: PWAs & Offline Sync';
    }

    // 150. Operating Systems: Linux Kernel & Device Driver Engineer
    const isKernelEngineerOutput =
      outputLower.includes('linux_kernel_module_optimizations') ||
    ...
    if (isKernelEngineerOutput) {
      downstreamSequence.push({
        agentId: 'Linux Kernel & Device Driver Engineer',
        task: 'Audit kernel module sysfs parameters and configure device driver tasklets/workqueues',
      });
      strategy = 'Systems Swarm: Linux Kernel & Device Drivers';
    }

    // 151. Operating Systems: Compiler Optimization & LLVM Specialist
    const isCompilerSpecialistOutput =
      outputLower.includes('llvm_ir_optimization_passes') ||
    ...
    if (isCompilerSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'Compiler Optimization & LLVM Specialist',
        task: 'Implement LLVM IR optimization passes and tune compile flags for code generation vectorizations',
      });
      strategy = 'Systems Swarm: Compiler Optimizations & LLVM';
    }

    // 152. Operating Systems: Linker, Loader & Static Binary Analyzer
    const isLinkerAnalyzerOutput =
      outputLower.includes('elf_macho_binary_header_audits') ||
    ...
    if (isLinkerAnalyzerOutput) {
      downstreamSequence.push({
        agentId: 'Linker, Loader & Static Binary Analyzer',
        task: 'Audit dynamic GOT/PLT symbol resolution offsets and map ELF/Mach-O segment alignments',
      });
      strategy = 'Systems Swarm: Linkers, Loaders & Symbols';
    }

    // 153. Operating Systems: Garbage Collection & Memory Runtime Specialist
    const isGcSpecialistOutput =
      outputLower.includes('generational_mark_sweep_garbage_collection') ||
    ...
    if (isGcSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'Garbage Collection & Memory Runtime Specialist',
        task: 'Calibrate generational mark-sweep GC compact bounds and design memory allocation arenas',
      });
    ...
    const isAssemblerExpertOutput =
      outputLower.includes('x86_arm_assembly_parsing') ||
    ...
    if (isAssemblerExpertOutput) {
      downstreamSequence.push({
        agentId: 'Assembler, Disassembler & Reverse Engineering Expert',
        task: 'Decode disassembly stack layout variables and trace CPU registers via Ghidra pipelines',
      });
      strategy = 'Systems Swarm: Assembly & Reverse Engineering';
    }

    // 155. Operating Systems: Virtualization Hypervisor & Xen/KVM Architect
    const isHypervisorArchitectOutput =
      outputLower.includes('kvm_xen_hypervisor_optimizations') ||
    ...
    if (isHypervisorArchitectOutput) {
      downstreamSequence.push({
        agentId: 'Virtualization Hypervisor & Xen/KVM Architect',
        task: 'Configure hardware-assisted VT-x/EPT guest environments and manage guest VFIO PCI passthroughs',
      });
      strategy = 'Systems Swarm: Virtualization & Hypervisors';
    }

    // 156. Operating Systems: POSIX System Programming & C Library Specialist
    const isPosixSpecialistOutput =
      outputLower.includes('posix_syscall_bindings_threads') ||
    ...
    if (isPosixSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'POSIX System Programming & C Library Specialist',
        task: 'Structure multi-process signal safe isolation scopes and optimize memory-mapped mmap bounds',
      });
      strategy = 'Systems Swarm: POSIX System Programming';
    }

    // 157. Operating Systems: Debugger Engine & GDB/LLDB Interop Specialist
    const isDebuggerSpecialistOutput =
      outputLower.includes('ptrace_process_attach_boundaries') ||
    ...
    if (isDebuggerSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'Debugger Engine & GDB/LLDB Interop Specialist',
        task: 'Inject hardware breakpoints in register space and parse DWARF/PDB symbol line tables',
      });
      strategy = 'Systems Swarm: Process Debugging & Symbols';
    }

    // 158. Operating Systems: Binary Compactor & Packer Specialist
    const isBinPackerOutput =
      outputLower.includes('runtime_executable_packer_upx') ||
    ...
    if (isBinPackerOutput) {
      downstreamSequence.push({
        agentId: 'Binary Compactor & Packer Specialist',
        task: 'Calibrate runtime UPX compact stubs and apply control flow flattening obfuscations',
      });
      strategy = 'Systems Swarm: Obfuscation & Executable Packers';
    }

    // 159. Operating Systems: Microkernel & Operating Systems Architect
    const isMicrokernelArchitectOutput =
      outputLower.includes('capability_based_security_sel4') ||
    ...
    if (isMicrokernelArchitectOutput) {
      downstreamSequence.push({
        agentId: 'Microkernel & Operating Systems Architect',
        task: 'Structure capability-based seL4 CSpace mappings and configure microkernel IPC message channels',
      });
    ...
    if (isWebGpuPipelineOutput) {
      downstreamSequence.push({
        agentId: 'WebGPU & WebGL Rendering Pipeline Engineer',
        task: 'Compile WGSL rendering and compute pipelines and optimize bind group layouts',
      });
      strategy = 'Graphics Swarm: WebGPU & GPU Pipelines';
    }

    // 161. Graphics: Ray Tracing & Global Illumination Specialist
    const isRayTracingOutput =
      outputLower.includes('bounding_volume_hierarchies_bvh') ||
    ...
    if (isRayTracingOutput) {
      downstreamSequence.push({
        agentId: 'Ray Tracing & Global Illumination Specialist',
        task: 'Traverse bounding volume hierarchies and optimize path tracing BRDF integrations',
      });
    ...
    if (isShaderArtistOutput) {
      downstreamSequence.push({
        agentId: 'Creative Shader & Visual Effects Artist',
        task: 'Design screenspace post-processing filters and render screenspace SDF raymarching bounds',
      });
      strategy = 'Graphics Swarm: Shaders & Visual Effects';
    }

    // 163. Graphics: Game Physics Engine & Rigid Body Dynamics Specialist
    const isPhysicsEngineOutput =
      outputLower.includes('collision_detection_gjk_aabb') ||
    ...
    if (isPhysicsEngineOutput) {
      downstreamSequence.push({
        agentId: 'Game Physics Engine & Rigid Body Dynamics Specialist',
        task: 'Solve GJK collision impulses and calibrate mass-spring soft body structures',
      });
      strategy = 'Graphics Swarm: Game Physics & Dynamics';
    }

    // 164. Graphics: Scene Graph & Entity Component System (ECS) Architect
    const isSceneGraphOutput =
      outputLower.includes('quadtree_octree_spatial_partitioning') ||
    ...
    if (isSceneGraphOutput) {
      downstreamSequence.push({
        agentId: 'Scene Graph & Entity Component System (ECS) Architect',
        task: 'Update ECS contiguous storage blocks and propagate local-to-world transform hierarchies',
      });
      strategy = 'Graphics Swarm: ECS & Scene Graph Architecture';
    }

    // 165. Graphics: Video & Image Compression Codec Specialist
    const isVideoCodecOutput =
      outputLower.includes('h264_hevc_av1_quantization') ||
    ...
    if (isVideoCodecOutput) {
      downstreamSequence.push({
        agentId: 'Video & Image Compression Codec Specialist',
        task: 'Calibrate CABAC entropy bounds and optimize motion vector search matrices',
      });
      strategy = 'Graphics Swarm: Codecs & Image Compression';
    }

    // 166. Graphics: Spatial Audio & Interactive Acoustics Engineer
    const isSpatialAudioOutput =
      outputLower.includes('web_audio_api_node_connections') ||
    ...
    if (isSpatialAudioOutput) {
      downstreamSequence.push({
        agentId: 'Spatial Audio & Interactive Acoustics Engineer',
        task: 'Structure AudioWorklet processor kernels and pan binaural listener vectors',
      });
      strategy = 'Graphics Swarm: Spatial Audio & Acoustics';
    }

    // 167. Graphics: AR/VR XR Runtime & Spatial Computing Specialist
    const isArVrSpecialistOutput =
      outputLower.includes('webxr_device_pose_estimation') ||
    ...
    if (isArVrSpecialistOutput) {
      downstreamSequence.push({
        agentId: 'AR/VR XR Runtime & Spatial Computing Specialist',
        task: 'Map projection poses in immersive loops and compensate lens distortion warp offsets',
      });
      strategy = 'Graphics Swarm: AR/VR & XR Runtimes';
    }

    // 168. Graphics: CAD Engine & Vector Graphics Specialist
    const isCadVectorOutput =
      outputLower.includes('bezier_bspline_subdivision_curves') ||
    ...
    if (isCadVectorOutput) {
      downstreamSequence.push({
        agentId: 'CAD Engine & Vector Graphics Specialist',
        task: 'Perform boolean clipping polygon splits and triangulate Delaunay mesh control splines',
      });
      strategy = 'Graphics Swarm: CAD Engines & Vector Graphics';
    }

    // 169. Graphics: Texture Mapping & Materials Shader Specialist
    const isTextureMaterialOutput =
      outputLower.includes('pbr_material_reflectance_rendering') ||
    ...
    if (isTextureMaterialOutput) {
      downstreamSequence.push({
        agentId: 'Texture Mapping & Materials Shader Specialist',
        task: 'Construct PBR reflectance BRDF stages and allocate mipmap anisotropic filtering buffers',
      });
    ...
    if (downstreamSequence.length === 0) {
      downstreamSequence.push({
        agentId: 'auditor',
        task: 'Fidelity quality gate audit on backend output',
      });
    }

    logger.info(
      `✅ [Router] Downstream Swarm resolved successfully: ${downstreamSequence.map(s => s.agentId).join(' -> ')}`,
    );
    return {
      strategy,
