...
class AcousticWorkspaceService {
  ...
  async processAudioCommand(audioBuffer) {
    logger.info(
      `🎙️ AcousticWorkspace: Received audio stream (${audioBuffer.byteLength} bytes). Processing via Google Cloud STT...`,
    );

    try {
      // Unmocked: Using Azure Cognitive Services Speech API
      const { speechClient } =
        await import('../gcpCloud/gcpSpeech.service.js');
      ...
      logger.info(
        `🎙️ AcousticWorkspace: Triggering Autonomous Sprint for voice command.`,
      );
      ...
      logger.error(
        `🎙️ AcousticWorkspace: Failed to process audio via Google STT: ${error.message}`,
      );
      throw error;
    }
  }
