...
export class GuardianAgent {
  ...
  async reviewDesign(goal, design) {
    logger.info(
      `🛡️ Guardian Agent: Reviewing design for security threats: "${goal}"`,
    );
    ...
    if (
      ...
      return {
        ...
        riskLevel: 'CRITICAL',
        reason:
          'Malicious design goal detected: unauthorized data exfiltration or system compromise.',
      };
    }
    return {
      approved: true,
  ...
  async interceptExecution(executionPayload, vectorType = 'BASH') {
    logger.warn(
      `🛡️ Guardian Agent: Intercepting Execution Vector [${vectorType}] -> "${executionPayload}"`,
    );
    ...
    try {
      ...
      if (!securityReport.isSafeToExecute) {
        logger.error(
          `🛑 GUARDIAN OVERRIDE TRIGGERED [Level: ${securityReport.threatLevel}]`,
        );
  ...
  async process(state) {
    ...
    return {
      ...
      results: [
        ...(state.results || []),
        `Guardian Check: ${report.isSafeToExecute ? 'CLEARED' : 'VETOED'}`,
      ],
    };
  }
}
