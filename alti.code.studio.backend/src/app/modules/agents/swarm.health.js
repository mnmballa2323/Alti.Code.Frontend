...
class SwarmHealthMonitor {
  ...
  async pingAgent(agentEntry) {
    ...
    const isStaticDefinition =
      typeof agent.consult !== 'function' && !agent.preamble;
    ...
    if (hasRequiredFields) {
      ...
      this.statuses.set(name, {
        ...
        latencyMs,
        error:
          'Missing required fields: name, description, preamble, or consult()',
      });
    }
  }
  ...
  async sweep() {
    ...
    const healthy = [...this.statuses.values()].filter(
      s => s.status === 'healthy',
    ).length;
    const degraded = [...this.statuses.values()].filter(
      s => s.status === 'degraded',
    ).length;

    logger.info(
      `🏥 SwarmHealth: ${healthy} healthy / ${degraded} degraded / ${agents.length} total`,
    );

    // ── DRIFT DETECTION ──────────────────────────────────────────────────
    if (degraded > 0) {
      const degradedEntries = [...this.statuses.entries()].filter(
        ([, s]) => s.status === 'degraded',
      );
      ...
      if (targetFiles.length > 0) {
        logger.info(
          `🚨 SwarmHealth: Triggering autonomic sweep on ${targetFiles.length} degraded agent(s)...`,
        );
        autonomicService
          .commenceProactiveSweep(targetFiles)
          .then(result =>
            logger.info(
              `🚨 SwarmHealth: Autonomic sweep complete — eliminated ${result.eliminated} debt item(s).`,
            ),
          )
          .catch(err =>
            logger.error('🚨 SwarmHealth: Autonomic sweep failed:', err),
          );
      } else {
        logger.info(
          '🚨 SwarmHealth: No resolvable source files for degraded agents. Manual review needed.',
        );
      }
    }
  }
  ...
  start(intervalMs = 5 * 60 * 1000) {
    ...
    this.sweep().catch(e =>
      logger.error('SwarmHealth initial sweep failed:', e),
    );
    ...
    logger.info(
      `🏥 SwarmHealth: Monitor started (interval: ${intervalMs / 1000}s)`,
    );
  }
   ...
   */
  getReport() {
    const agents = [...this.statuses.entries()].map(([name, status]) => ({
      name,
      ...status,
    }));
