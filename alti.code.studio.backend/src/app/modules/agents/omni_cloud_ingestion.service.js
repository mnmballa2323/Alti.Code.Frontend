...
class OmniCloudIngestionService {
  ...
  async init() {
    ...
    if (!this.githubToken) {
      logger.warn(
        '⚠️ [Omni-Cloud Ingestion] Missing GITHUB_PAT in Secret Manager or env. Massive Overhaul disabled.',
      );
      return;
    }
  ...
  async syncOmniCloud() {
    ...
    if (!lockAcquired) {
      logger.info(
        '🌌 [Omni-Cloud Ingestion] Sync already in progress by another node. Skipping.',
      );
      return;
    }

    logger.info(
      '🌌 [Omni-Cloud Ingestion] Starting massive multi-cloud repository scan...',
    );
    let totalIngested = 0;
        ...
        while (hasMore) {
          try {
            ...
            for (const repo of repos) {
              ...
              logger.info(
                `✅ [Omni-Cloud Ingestion] Compliant Repo Found: ${repo.full_name} (${licenseKey.toUpperCase()})`,
              );
              ...
              capabilityRouter.dispatch(prompt, [contextData]).catch(e => {
                logger.error(
                  `[Omni-Cloud] Failed to dispatch ingestion for ${repo.full_name}: ${e.message}`,
                );
              });
            ...
            logger.error(
              `❌ [Omni-Cloud Ingestion] Failed scanning ${org} on page ${page}: ${error.message}`,
            );
    ...
    logger.info(
      `🌌 [Omni-Cloud Ingestion] Massive Overhaul Sync Complete. Dispatched ${totalIngested} compliant repositories to the Swarm.`,
    );
  }
}
