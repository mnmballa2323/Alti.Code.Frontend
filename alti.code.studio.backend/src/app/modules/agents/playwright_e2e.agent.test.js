...
describe('PlaywrightE2eAgent Unit Tests', () => {
    ...
    it('should initialize with correct name, description, and preamble', () => {
        ...
        expect(playwrightE2eAgent.description).toContain('End-to-End Test Automation & Browser Automation Specialist');
        expect(playwrightE2eAgent.preamble).toContain('END-TO-END TEST AUTOMATION & BROWSER SPECIALIST');
    });

    it('should successfully consult the LLM with context and prompt', async () => {
        const mockResponse = 'const test = async () => { await page.goto("/login"); await page.fill("[data-testid=user]", "test"); };';
        ...
        expect(generatedPrompt).toContain('END-TO-END TEST AUTOMATION & BROWSER SPECIALIST');
    ...
    it('should handle API errors and trigger retries in BaseSpecialistAgent', async () => {
        ...
        const result = await playwrightE2eAgent.consult('Audit checkout page visual selectors', []);
