...
import {
  ...
  swarmBus,
} from './self_expanding_swarm.service.js';
...
router.post(
  '/route',
  validateRequest(SwarmValidation.routeQueryZodSchema),
  (req, res) => {
    try {
      ...
      if (!match)
        return res.json({ success: true, matched: false, data: null });
...
router.post(
  '/dispatch',
  validateRequest(SwarmValidation.dispatchQueryZodSchema),
  async (req, res) => {
    try {
...
router.post(
  '/fanout',
  validateRequest(SwarmValidation.fanoutQueryZodSchema),
  async (req, res) => {
    try {
      ...
      const results = await capabilityRouter.fanOut(
        query,
        contextData,
        Math.min(n, 5),
      );
  ...
  try {
    ...
    if (!domain || !triggerQuery) {
      return res.status(400).json({
        success: false,
        error: 'domain and triggerQuery are required',
      });
    }
    res.json({
      success: true,
      message: 'Synthesis triggered. Connect to /stream for live logs.',
    });
    // Run background async
    selfExpandingSwarmService
      .synthesizeAgent(domain, triggerQuery)
      .catch(e => logger.error('Synthesis Background Error:', e));
  } catch (err) {
...
router.get('/synthesis-report', (req, res) => {
  try {
    res.json({
      success: true,
      data: selfExpandingSwarmService.getSynthesisReport(),
    });
  } catch (err) {
