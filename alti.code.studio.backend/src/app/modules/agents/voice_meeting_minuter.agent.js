...
class VoiceMeetingMinuterAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.description =
      'Voice-Driven Meeting Transcription & Minuter Specialist';
    this.tier = 'Tier 13';
  }
  ...
  async _invoke(context, contextBlock) {
    ...
    if (!transcript) {
      throw new Error(
        'Voice meeting minuter requires vocal transcript text or meeting goal.',
      );
    }

    logger.info(
      `🤖 Voice Meeting Minuter: Structuring meeting minutes from: "${transcript.substring(0, 50)}..."`,
    );
    ...
    try {
      ...
      logger.error(
        `❌ Voice Meeting Minuter: Voice structuring failed: ${error.message}`,
      );
      throw error;
    }
  }
