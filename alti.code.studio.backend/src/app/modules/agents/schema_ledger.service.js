...
class SchemaLedger {
  constructor() {
    ...
    logger.info(
      '📜 SchemaLedger initialized. Tracking application-to-database structural constraints.',
    );
  }
  ...
  recordDeployment(gitSha, schemaVersion, isBackwardCompatible) {
    ...
    logger.info(
      `📜 Recorded Deployment Matrix: Git [${gitSha.substring(0, 7)}] -> Schema [${schemaVersion}]. Backward Compatible: ${isBackwardCompatible}`,
    );
  ...
  calculateRollbackPath(targetGitSha) {
    ...
    if (!targetState) {
      throw new Error(
        `CRITICAL: Cannot calculate rollback path. Git SHA ${targetGitSha} not found in Schema Ledger.`,
      );
    }
    ...
    logger.warn(
      `📜 Emergency Rollback requested to SHA [${targetGitSha.substring(0, 7)}]. Current Schema: [${currentState.schemaVersion}].`,
    );
    ...
    if (currentState.isBackwardCompatible) {
      return {
        ...
        targetSchema: currentState.schemaVersion,
        reason:
          'Current deployment is an Expand phase and explicitly handles backward compatibility.',
      };
    }
    ...
    logger.error(
      `💣 DANGER: Required rollback to SHA [${targetGitSha}] requires schema DOWNGRADE to [${targetState.schemaVersion}].`,
    );
    return {
      action: 'REQUIRE_SCHEMA_DOWNGRADE',
      targetSchema: targetState.schemaVersion,
      reason:
        'Application state incompatible with current database schema. Execute DDL downgrade IMMEDIATELY.',
    };
  }
}
