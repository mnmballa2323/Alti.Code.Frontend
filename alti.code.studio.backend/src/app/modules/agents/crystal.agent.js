/**
 * "The Crystal Master" - Tier 20 Language Specialist
 * Expert in Crystal 1.x, compile-time macros, Fiber concurrency, Shards, and Lucky framework.
 */
...
class CrystalAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.description =
      'Language specialist for Crystal 1.x: Ruby-like syntax with static types, Fiber-based concurrency, macros, and Lucky web framework.';
...
When writing code, output beautiful Crystal 1.7+. It should look identical to highly-disciplined Ruby, but execute with C-like performance bounds. Use \`crystal tool format\` natively.`;
  ...
  async consult(prompt, contextData = []) {
    ...
    try {
      ...
      throw new Error(`Crystal Synthesis Failed: ${e.message}`);
    }
  }
}
export const crystalAgent = new CrystalAgent();
