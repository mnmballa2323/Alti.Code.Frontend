...
class SocialMediaMonitorAgent extends BaseSpecialistAgent {
    ...
    async _invoke(context, contextBlock) {
        ...
        if (!keywords) {
            throw new Error('Social media monitor requires keywords or search query.');
        }

        logger.info(`🤖 Social Media Monitor: Monitoring mentions for: "${keywords}"`);
        ...
        try {
            ...
            logger.error(`❌ Social Media Monitor: Browser automation failed: ${error.message}`);
            throw error;
        }
    }
}
