...
export class HiveMindAgent {
  ...
  async architectSwarm(macroGoal) {
    logger.info(
      `🧠 HiveMind Agent: Initiating cognitive breakdown of macro-goal: "${macroGoal.substring(0, 40)}..."`,
    );
    ...
    try {
      ...
      logger.info(
        `🧠 HiveMind Agent: Swarm Topology Constructed [${hiveReport.swarmArchitectureName}]. Complexity: ${hiveReport.estimatedComplexity}/10`,
      );
      logger.info(
        `   Spawning ${hiveReport.subAgentsToSpawn.length} Ephemeral Sub-Agents...`,
      );

      for (const agent of hiveReport.subAgentsToSpawn) {
        logger.info(
          `   -> 🧬 Spawning [${agent.name}]: Mission => ${agent.mission}`,
        );
      }
  ...
  async process(state) {
    ...
    return {
      ...
      results: [
        ...(state.results || []),
        `Spawned Swarm: ${report.swarmArchitectureName} (${report.subAgentsToSpawn.length} agents)`,
      ],
    };
  }
}

export const hiveMindAgent = new HiveMindAgent();
