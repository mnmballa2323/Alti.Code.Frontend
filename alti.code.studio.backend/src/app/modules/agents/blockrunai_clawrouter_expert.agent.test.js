...
describe('BlockRunAIClawRouterExpertAgent Unit Tests', () => {
  ...
  it('should initialize with correct name, description, and preamble', () => {
    expect(blockrunaiClawrouterExpertAgent.name).toBe(
      'BlockRunAI_ClawRouter_Expert',
    );
    expect(blockrunaiClawrouterExpertAgent.description).toContain(
      'BlockRunAI/ClawRouter, the agent-native cost-optimizing LLM proxy',
    );
    expect(blockrunaiClawrouterExpertAgent.preamble).toContain(
      'BlockRunAI/ClawRouter AI Proxy & Micropayments Specialist',
    );
  });

  it('should successfully consult the LLM with context and prompt', async () => {
    const mockResponse =
      'ClawRouter: Proxying request on port 8402 with pay-per-request signature.';
    GeminiAiService.generateContent.mockResolvedValue(mockResponse);

    const prompt = 'Proxy DeepSeek-R1 query';
    const context = [];

    const result = await blockrunaiClawrouterExpertAgent.consult(
      prompt,
      context,
    );
