 ...
 *
 * DEER-FLOW AGENT — ByteDance SuperAgent Deep Research Harness
 * Delegates complex, long-running research and coding tasks to deer-flow.
 * MIT License — https://github.com/bytedance/deer-flow
 *
 * Tier 13 Specialist Agent — Tasks that take minutes to hours
 * Handles: deep research, multi-document synthesis, complex code generation,
 * podcast scripts, technical reports, and multi-step coding projects.
 */
...
4. Return structured results with citations`;
    ...
    async _invoke(prompt, contextBlock, opts = {}) {
        ...
        if (!caps.available) {
            ...
            logger.warn('[deer-flow-agent] deer-flow not available, falling back to Gemini deep research');
...
Score:`
        ...
        if (complexity >= 7) {
            ...
            logger.info('[deer-flow-agent] High-complexity task — delegating to deer-flow harness');
            const result = await deerFlowService.runTask(prompt, {
                timeout: 300000, // 5 min max
                maxSteps: 20,
                concurrency: opts.maxConcurrentAgents || process.env.MAX_CONCURRENT_AGENTS || 10,
                telemetryLevel: opts.telemetryLevel || process.env.TELEMETRY_LEVEL || 'info'
            });
...
export const deerFlowAgent = new DeerFlowAgent();
