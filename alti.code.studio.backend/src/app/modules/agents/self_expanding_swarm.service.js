...
class SelfExpandingSwarmService {
  ...
  async synthesizeAgent(domain, triggerQuery) {
    ...
    const report = {
      ...
      status: 'started',
      logs: [
        `[${new Date().toISOString()}] Ignition sequence started for domain: "${domain}"`,
      ],
    ...
    swarmBus.emit('swarm:synthesis_started', {
      domain,
      triggerQuery,
      id: reportId,
    });
    ...
    try {
      ...
      addLog(
        `Invoking Agent Forge to code dynamic specialist for "${domain}"...`,
      );

      const generatedCode =
        await agentForgeGeneratorAgent.generateAgent(domain);

      // 2. Parse target agent name
      const agentNameMatch = generatedCode.match(/name:\s*['"]([^'"]+)['"]/);
      let agentName = agentNameMatch
        ? agentNameMatch[1]
        : `dynamic_agent_${Date.now()}`;
      agentName = agentName.replace(/[^a-zA-Z0-9_]/g, '').toLowerCase();
      report.agentName = agentName;
      addLog(
        `Successfully forged agent class structure in-memory: "${agentName}".`,
      );

      // 3. Write files to disk
      report.status = 'writing';
      const customDirPath = path.join(
        process.cwd(),
        'src',
        'app',
        'modules',
      ...
      const normalizedCode = generatedCode.replace(
        /['"]\.\/gemini_cli_base\.agent\.js['"]/g,
        "'../gemini_cli_base.agent.js'",
      );
      ...
      try {
        ...
        const exportedKey = Object.keys(module).find(
          k => module[k] && typeof module[k] === 'object',
        );
        ...
        if (agentInstance) {
          ...
          addLog(
            `Successfully mounted dynamic agent [${agentName}] into Swarm Capability Router.`,
          );
          swarmBus.emit('swarm:agent_registered', { agentName, id: reportId });
          return agentName;
        } else {
          addLog(
            `Agent generated successfully but exported instance was not found. Loaded companion JSON configuration fallback.`,
          );
        }
      } catch (importErr) {
        addLog(
          `Dynamic import failed: ${importErr.message}. Loader fallback will register agent on next startup.`,
        );
      }

      report.status = 'failed';
      swarmBus.emit('swarm:synthesis_failed', {
        domain,
      ...
      report.status = 'failed';
      addLog(
        `Failed to synthesize agent for capability "${domain}": ${error.message}`,
      );
      swarmBus.emit('swarm:synthesis_failed', {
        domain,
        error: error.message,
  ...
  async expandSwarm(missingCapabilityRequirement) {
    logger.info(
      `🌌 [SelfExpandingSwarm] Triggered expansion for capability: ${missingCapabilityRequirement}`,
    );

    // Directly invoke our robust synthesis pipeline!
    return this.synthesizeAgent(
      missingCapabilityRequirement,
      `Automated capability gap: ${missingCapabilityRequirement}`,
    );
  }
}
