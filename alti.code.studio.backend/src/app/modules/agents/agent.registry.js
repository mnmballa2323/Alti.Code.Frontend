...
class AgentRegistry {
  ...
  register(definition) {
    ...
    if (!definition.silent) {
      logger.info(
        `🤖 AgentRegistry: Registered [${definition.name}] — Policy: ${definition.policy?.accessLevel || 'DEVELOPER'}`,
      );
    }
      ...
      import('../memory/vector.store.js')
        .then(({ vectorStoreService }) => {
          vectorStoreService
            ...
            .then(() => {
              logger.info(
                `🗺️ [Nexus-RAG] Dynamically indexed newly registered [${definition.name}] in pgvector store.`,
              );
            })
            .catch(e => {
              logger.debug(
                `[Nexus-RAG] Dynamic indexing skipped or failed for [${definition.name}]: ${e.message}`,
              );
            });
        })
        .catch(() => {});
  ...
  canAccess(agentName, userRole = 'DEVELOPER') {
    ...
    if (!isAllowed) {
      logger.warn(
        `🛑 [Nexus-RBAC] Access Denied: User role ${userRole} attempted to activate ${agentName} (Required: ${agent.policy.accessLevel})`,
      );
    }
  ...
  get(name, tenantId = null) {
    if (name && name.toLowerCase().startsWith('gitlab')) {
      if (
        ...
        logger.warn(
          `AgentRegistry: get failed for GitLab agent [${name}] because tenantId is omitted or invalid.`,
        );
        return null;
      }
    }
  ...
  async loadPlugins() {
    ...
    try {
          ...
          try {
            ...
            if (pluginModule.pluginInstance) {
              ...
              this.register({
                ...
                description:
                  instance.description || 'Swarm Marketplace Community Agent',
            ...
            logger.error(
              `❌ Failed to load Marketplace Plugin: ${file}`,
              pluginErr.message,
            );
          }
        }
      ...
      logger.info(
        `🧩 AgentRegistry: External Marketplace Plugins loaded successfully.`,
      );
    } catch (e) {
      logger.warn(
        `AgentRegistry: Could not scan plugins directory: ${e.message}`,
      );
    }
  }
}
...
agentRegistry.register({
  name: 'jules',
  description:
    'Autonomous Software Engineering Agent — Repository-scale reasoning and flawless asynchronous coding.',
  queue: 'jules-queue',
  capabilities: [
    'autonomous-coding',
    'refactoring',
...
agentRegistry.register({
  ...
  description:
    'Enforces the MIT & Apache 2.0 Hard Law — Zero-trust legal compliance for all code and dependencies.',
  queue: 'legal-queue',
...
agentRegistry.register({
  name: 'diplomat',
  description:
    'Universal A2A Diplomat — Autonomous negotiation and task hand-off with 3rd-party enterprise agents.',
...
agentRegistry.register({
  name: 'auditor',
  description:
    'Google Cloud Vertex AI Sentinel — Autonomous self-evaluation, fidelity benchmarking, and model hyper-tuning.',
  queue: 'auditor-queue',
  capabilities: [
    'agentic-eval',
    'benchmarking',
...
agentRegistry.register({
  name: 'architect',
  description:
    'Gemini Code-Assist Architect — Large-scale structural refactoring and codebase-aware strategy.',
...
agentRegistry.register({
  name: 'conductor',
  description:
    'Swarm Conductor — Real-time multi-agent CRDT synchronization and focus management.',
  queue: 'sync-queue',
  capabilities: [
    'live-collaboration',
    'swarm-sync',
...
agentRegistry.register({
  name: 'zx',
  description:
    'GCP zx DevOps Agent — executes native shell scripts autonomously for CI/CD and system orchestration.',
  queue: 'zx-queue',
...
agentRegistry.register({
  name: 'octopus',
  description:
    'GitHub Master Orchestrator — Autonomous Pull Requests, Actions, and Release Management.',
  queue: 'github-queue',
  capabilities: [
...
agentRegistry.register({
  name: 'specter',
  description:
    'Preview Environment Engine — Autonomous transient clones for every Pull Request.',
  queue: 'preview-queue',
  capabilities: [
    'preview-environment',
...
agentRegistry.register({
  name: 'titan',
  description:
    'Multi-Region HA Architect — Global high-availability and regional failover orchestration.',
  queue: 'ha-queue',
  capabilities: [
    'multi-region',
    'high-availability',
...
agentRegistry.register({
  name: 'audit',
  description:
    'Cryptographically seals every system action into an immutable ledger.',
  queue: 'audit-queue',
...
agentRegistry.register({
  name: 'git',
  description:
    'Autonomous version control. Auto-commits, pushes, and documents milestones.',
  queue: 'git-queue',
...
agentRegistry.register({
  name: 'refactor',
  description:
    'Cognitive code refactoring with RAG recall, reasoning, and self-correction.',
...
agentRegistry.register({
  name: 'security',
  description:
    'Scans codebases for vulnerabilities and memorizes threat signatures.',
...
agentRegistry.register({
  name: 'sandyaa',
  description:
    'Autonomous autonomous security auditing tool that navigates the file system and AST to find structural vulnerabilities.',
  queue: 'sandyaa-queue',
  capabilities: [
    'security-audit',
    'ast-vulnerability-scan',
    'sandyaa',
    'autonomous-auditing',
  ],
  version: '1.0.0',
...
agentRegistry.register({
  name: 'qa',
  description:
    'Generates tests, executes them, and self-heals code on failure.',
  queue: 'qa-queue',
...
agentRegistry.register({
  name: 'codeReview',
  description:
    'AI-powered code reviews — finds bugs, security flaws, and style issues.',
...
agentRegistry.register({
  name: 'documentation',
  description:
    'Auto-generates JSDoc, README, and API reference from source code.',
...
agentRegistry.register({
  name: 'debug',
  description:
    'Automated root cause analysis and debugging from errors/stack traces.',
  queue: 'debug-queue',
...
agentRegistry.register({
  name: 'architecture',
  description:
    'System design review, pattern detection, architecture recommendations.',
...
agentRegistry.register({
  name: 'migration',
  description:
    'Zero-downtime database/API migration planning and SQL generation.',
  queue: 'migration-queue',
...
agentRegistry.register({
  name: 'performance',
  description:
    'Profiling, bottleneck detection, caching and parallelization strategies.',
...
agentRegistry.register({
  name: 'cost',
  description:
    'Cloud FinOps — cost optimization, right-sizing, reserved instance planning.',
  queue: 'cost-queue',
...
agentRegistry.register({
  name: 'accessibility',
  description:
    'WCAG 2.1 / ADA / Section 508 accessibility compliance checking.',
...
agentRegistry.register({
  name: 'dependency',
  description:
    'Dependency management — vulnerability patching, update planning, unused detection.',
...
agentRegistry.register({
  name: 'overseer',
  description:
    'CI/CD pipeline orchestrator — intercepts failures, executes tests in shadows, and triggers self-healing.',
...
agentRegistry.register({
  name: 'incident',
  description:
    'Incident response — triage, root cause, mitigation, postmortem generation.',
...
agentRegistry.register({
  name: 'analytics',
  description:
    'Data analytics — trend detection, anomaly identification, predictions.',
...
agentRegistry.register({
  name: 'knowledge',
  description:
    'Knowledge base management — RAG retrieval, indexing, explanation.',
...
agentRegistry.register({
  name: 'seo',
  description:
    'SEO optimization — metadata, structured data, technical SEO audits.',
  queue: 'seo-queue',
...
agentRegistry.register({
  name: 'onboarding',
  description:
    'Developer onboarding — quick start guides, architecture overview, recipes.',
...
agentRegistry.register({
  name: 'apiDesign',
  description:
    'API architecture — OpenAPI/GraphQL schema design, contract-first development.',
...
agentRegistry.register({
  name: 'python',
  description:
    'Python expert — Django, FastAPI, async patterns, type hints, packaging.',
  queue: 'python-queue',
...
agentRegistry.register({
  name: 'typescript',
  description:
    'TypeScript expert — strict types, generics, Next.js, React patterns.',
...
agentRegistry.register({
  name: 'rust',
  description:
    'Rust expert — ownership, lifetimes, async, unsafe, FFI, performance.',
  queue: 'rust-queue',
...
agentRegistry.register({
  name: 'golang',
  description:
    'Go expert — goroutines, channels, interfaces, error handling, microservices.',
  queue: 'golang-queue',
...
agentRegistry.register({
  name: 'linter',
  description:
    'Micro-agent: Lints code and returns errors/warnings with auto-fixes.',
  queue: 'linter-queue',
...
agentRegistry.register({
  name: 'formatter',
  description:
    'Micro-agent: Formats code to consistent style (Prettier/Black/gofmt).',
...
agentRegistry.register({
  ...
  description:
    'Dispatches long-running, real-time multiplayer coding tasks via Open-Inspect.',
...
agentRegistry.register({
  name: 'pentagi',
  description:
    'Autonomous Deep Penetration Testing and Security Auditing via isolated containers.',
  queue: 'pentagi-queue',
  capabilities: [
    'pentest',
    'security-audit',
...
agentRegistry.register({
  name: 'fossflow',
  description:
    'Autonomous Isometric Architecture Designer powered by FossFLOW.',
...
agentRegistry.register({
  name: 'envValidator',
  description:
    'Micro-agent: Validates .env files for missing vars, exposed secrets, insecure defaults.',
...
agentRegistry.register({
  name: 'secretScanner',
  description:
    'Micro-agent: Scans code for exposed API keys, tokens, passwords, credentials.',
...
agentRegistry.register({
  name: 'deadCode',
  description:
    'Micro-agent: Detects dead code — unused functions, variables, imports.',
...
agentRegistry.register({
  name: 'importSorter',
  description:
    'Micro-agent: Sorts and organizes imports by group (builtin, external, internal).',
...
agentRegistry.register({
  name: 'gcpSearch',
  description:
    'Grounds prompts and outputs in real-time web data using Vertex AI Agent Builder Search.',
  queue: 'gcp-search-queue',
...
agentRegistry.register({
  name: 'surfer',
  description:
    'Web browsing agent. Visits URLs, extracts content, and performs research.',
...
agentRegistry.register({
  name: 'siren',
  description:
    'Text-to-Speech agent. Announces system events and results vocally.',
  queue: 'siren-queue',
...
agentRegistry.register({
  name: 'vector',
  description:
    'Long-term memory access (Vector DB). Stores and retrieves semantic data.',
  queue: 'memory-queue',
...
agentRegistry.register({
  name: 'engine',
  description:
    'Python AI Engine Proxy — Access to NeMo, RAG, and Graph Analysis.',
  queue: 'engine-queue',
...
agentRegistry.register({
  name: 'strategist',
  description:
    'The Planner. Breaks down complex goals into atomic steps for the swarm.',
...
agentRegistry.register({
  name: 'critic',
  description:
    'The Reviewer. Validates plans for security, performance, and correctness.',
  queue: 'critic-queue',
...
agentRegistry.register({
  name: 'regex',
  description:
    'Micro-agent dedicated entirely to regular expressions and catastrophic backtracking avoidance.',
  queue: 'regex-queue',
...
agentRegistry.register({
  name: 'docker',
  description:
    'Specialist generating multi-stage Dockerfiles and container optimization strategies.',
  queue: 'docker-queue',
...
agentRegistry.register({
  name: 'kubernetes',
  description:
    'Generates and validates K8s manifests, Helm charts, and Custom Resource Definitions.',
...
agentRegistry.register({
  name: 'nginx',
  description:
    'Handles strict Nginx configurations, reverse proxying, rate limiting, and SSL bindings.',
  queue: 'nginx-queue',
...
agentRegistry.register({
  name: 'graphql',
  description:
    'Focuses solely on writing optimal GraphQL schemas and resolvers preventing N+1 queries.',
  queue: 'graphql-queue',
...
agentRegistry.register({
  name: 'jwt',
  description:
    'Auth Token Master — parses and generates secure JWT issuance logic and claims.',
  queue: 'jwt-queue',
...
agentRegistry.register({
  name: 'stripe',
  description:
    'SDK specialist for Stripe payments, subscriptions, Connect, and Radar fraud rules.',
  queue: 'stripe-queue',
...
agentRegistry.register({
  name: 'redis',
  description:
    'SDK specialist for Redis caching, Service Bus, atomic pipelines, and cluster config.',
  queue: 'redis-queue',
...
agentRegistry.register({
  name: 'supabase',
  description:
    'SDK specialist for Supabase Auth, RLS policies, Realtime, and Edge Functions.',
...
agentRegistry.register({
  name: 'chairman',
  description:
    'Chairman of the Board — Governance, Risk, and Long-term Strategy.',
...
agentRegistry.register({
  name: 'coo',
  description:
    'Chief Operating Officer — Operations, Logistics, and Efficiency.',
  queue: 'coo-queue',
...
agentRegistry.register({
  name: 'cmo',
  description:
    'Chief Marketing Officer — Growth, Branding, and User Acquisition.',
  queue: 'cmo-queue',
...
agentRegistry.register({
  name: 'cro',
  description:
    'Chief Revenue Officer — Enterprise Sales, Monetization, and Pricing.',
  queue: 'cro-queue',
...
agentRegistry.register({
  name: 'accountant',
  description:
    'Chief Accountant — Audits, Immutable Ledger, and Financial Records.',
...
agentRegistry.register({
  name: 'pm',
  description:
    'Product Manager — PRDs, User Stories, and Backlog Prioritization.',
  queue: 'pm-queue',
...
agentRegistry.register({
  name: 'designer',
  description:
    'UX/UI Designer — Pixel-perfect interfaces, Tailwind, and Design Systems.',
...
agentRegistry.register({
  name: 'qa',
  description:
    'QA Engineer — Destructive testing, Playwright, and release stability.',
...
agentRegistry.register({
  name: 'frontend',
  description:
    'Frontend Developer — React, Next.js, and complex client-side state.',
...
agentRegistry.register({
  name: 'dba',
  description:
    'Database Admin — Schema design, SQL optimization, and migrations.',
  queue: 'dba-queue',
...
agentRegistry.register({
  name: 'writer',
  description:
    'Technical Writer — Swagger, READMEs, and technical documentation.',
  queue: 'writer-queue',
...
agentRegistry.register({
  name: 'support',
  description:
    'Support Engineer — Triage, Incident Response, and User Mitigations.',
  queue: 'support-queue',
...
agentRegistry.register({
  name: 'cto',
  description:
    'Chief Technology Officer — Architecture, Tech Stack, and Technical Debt.',
  queue: 'cto-queue',
...
agentRegistry.register({
  name: 'ciso',
  description:
    'Chief Information Security Officer — Zero-Trust, Pen-testing, and Security.',
  queue: 'ciso-queue',
...
agentRegistry.register({
  name: 'prisma',
  description:
    'ORM specialist for Prisma schema design, migrations, and query optimization.',
  queue: 'prisma-queue',
...
agentRegistry.register({
  name: 'solidity',
  description:
    'Web3 specialist for EVM smart contracts, ERC standards, gas optimization, and Hardhat.',
...
agentRegistry.register({
  name: 'svelte',
  description:
    'Framework specialist for Svelte 5 runes, SvelteKit SSR/SSG, and reactive state.',
  queue: 'svelte-queue',
...
agentRegistry.register({
  name: 'vue',
  description:
    'Framework specialist for Vue 3 Composition API, Pinia, Nuxt 3, and Vue Router.',
  queue: 'vue-queue',
...
agentRegistry.register({
  name: 'storybook',
  description:
    'DevEx specialist for Storybook CSF3 stories, interaction tests, and visual regression.',
...
agentRegistry.register({
  name: 'ansible',
  description:
    'IaC specialist for Ansible playbooks, roles, Vault encryption, and dynamic inventories.',
  queue: 'ansible-queue',
  capabilities: [
    'ansible',
    'automation',
...
agentRegistry.register({
  name: 'terraform',
  description:
    'IaC specialist for Terraform HCL modules, remote state, workspaces, and Terragrunt.',
...
agentRegistry.register({
  name: 'langchain',
  description:
    'AI/ML specialist for LangChain chains, RAG pipelines, tool-calling agents, and LCEL.',
...
agentRegistry.register({
  name: 'mlops',
  description:
    'MLOps specialist for MLflow tracking, model registries, drift detection, and serving.',
  queue: 'mlops-queue',
...
agentRegistry.register({
  name: 'prompteng',
  description:
    'AI specialist for prompt engineering, chain-of-thought, few-shot, and structured outputs.',
...
agentRegistry.register({
  name: 'reactnative',
  description:
    'Mobile specialist for React Native + Expo, native modules, and EAS builds.',
...
agentRegistry.register({
  name: 'flutter',
  description:
    'Mobile specialist for Flutter, Riverpod state, platform channels, and animations.',
  queue: 'flutter-queue',
...
agentRegistry.register({
  name: 'pwa',
  description:
    'Mobile/Web specialist for Service Workers, Workbox, and Progressive Web App architecture.',
  queue: 'pwa-queue',
...
agentRegistry.register({
  name: 'playwright',
  description:
    'E2E testing specialist for Playwright POM, parallelization, and network mocking.',
...
agentRegistry.register({
  name: 'elasticsearch',
  description:
    'Search specialist for Elasticsearch mappings, relevance tuning, aggregations, and ILM.',
...
agentRegistry.register({
  name: 'kafka',
  description:
    'Streaming specialist for Kafka topics, consumer groups, EOS, and Kafka Streams topologies.',
  queue: 'kafka-queue',
...
agentRegistry.register({
  name: 'pentest',
  description:
    'Security specialist for OWASP Top 10, penetration testing methodologies, and red team tactics.',
  queue: 'pentest-queue',
...
agentRegistry.register({
  name: 'githubactions',
  description:
    'Platform specialist for GitHub Actions workflows, OIDC federation, and matrix builds.',
...
agentRegistry.register({
  name: 'figma',
  description:
    'Creative specialist for Figma design-to-code, design tokens, and component variant systems.',
  queue: 'figma-queue',
  capabilities: [
    'figma',
    'design-tokens',
...
agentRegistry.register({
  name: 'wasm',
  description:
    'Platform specialist for WebAssembly compilation, WASI, and browser/Node.js integration.',
  queue: 'wasm-queue',
...
agentRegistry.register({
  name: 'deno',
  description:
    'Platform specialist for Deno 2 runtime, Fresh framework, and Deno Deploy edge network.',
  queue: 'deno-queue',
...
agentRegistry.register({
  name: 'mongodb',
  description:
    'Data specialist for MongoDB Mongoose schemas, aggregation pipelines, Atlas Search, and change streams.',
  queue: 'mongodb-queue',
...
agentRegistry.register({
  name: 'grpc',
  description:
    'Platform specialist for Protocol Buffers design, gRPC services, streaming, and interceptors.',
  queue: 'grpc-queue',
...
agentRegistry.register({
  name: 'tailwind',
  description:
    'Creative specialist for Tailwind v4, CVA component variants, and utility-first design systems.',
...
agentRegistry.register({
  name: 'i18n',
  description:
    'Localization specialist for next-intl, ICU messages, pluralization, and RTL layout support.',
  queue: 'i18n-queue',
...
agentRegistry.register({
  name: 'websocket',
  description:
    'Real-time specialist for Socket.IO rooms, namespaces, Redis adapter, and heartbeat patterns.',
...
agentRegistry.register({
  name: 'cloudflare',
  description:
    'Edge cloud specialist for Cloudflare Workers, Durable Objects, D1, R2, KV, and Pages.',
...
agentRegistry.register({
  name: 'vercel',
  description:
    'Deployment specialist for Vercel Edge Functions, ISR, vercel.json, and Fluid compute.',
  queue: 'vercel-queue',
...
agentRegistry.register({
  name: 'netlify',
  description:
    'Deployment specialist for Netlify Functions, Edge Functions, netlify.toml, and Blobs.',
  queue: 'netlify-queue',
...
agentRegistry.register({
  name: 'digitalocean',
  description:
    'Cloud specialist for DigitalOcean Droplets, App Platform, Spaces, DOKS, and Managed DBs.',
...
agentRegistry.register({
  name: 'oracle',
  description:
    'Enterprise cloud specialist for OCI Compute, OKE, Autonomous Database, and Object Storage.',
  queue: 'oracle-queue',
...
agentRegistry.register({
  name: 'ibmcloud',
  description:
    'Enterprise cloud specialist for IBM Code Engine, Watson AI, IKS, and Db2.',
...
agentRegistry.register({
  name: 'alibaba',
  description:
    'Cloud specialist for Alibaba Cloud ECS, OSS, ACK, Function Compute, and PAI ML.',
  queue: 'alibaba-queue',
...
agentRegistry.register({
  name: 'hetzner',
  description:
    'Cost-optimized specialist for Hetzner VPS, Load Balancers, Volumes, and K3s clusters.',
  queue: 'hetzner-queue',
...
agentRegistry.register({
  name: 'flyio',
  description:
    'PaaS specialist for Fly.io fly.toml, Machines API, Volumes, and WireGuard networking.',
  queue: 'flyio-queue',
...
agentRegistry.register({
  name: 'railway',
  description:
    'PaaS specialist for Railway deployments, nixpacks, service linking, and Cron Jobs.',
  queue: 'railway-queue',
...
agentRegistry.register({
  name: 'render',
  description:
    'PaaS specialist for Render web services, background workers, and PostgreSQL.',
  queue: 'render-queue',
...
agentRegistry.register({
  name: 'linode',
  description:
    'Cloud computing specialist for Linode instances, NodeBalancers, and LKE.',
  queue: 'linode-queue',
...
agentRegistry.register({
  name: 'scaleway',
  description:
    'European cloud specialist for Scaleway Elements, Kapsule, and Serverless.',
...
agentRegistry.register({
  name: 'ovhcloud',
  description:
    'Cloud hosting specialist for OVHcloud Public Cloud, dedicated servers, and VRack.',
...
agentRegistry.register({
  name: 'neon',
  description:
    'Serverless Postgres specialist for Neon DB branching, autoscaling, and bottomless storage.',
  queue: 'neon-queue',
...
agentRegistry.register({
  name: 'fastly',
  description:
    'Edge cloud platform specialist for Fastly Compute@Edge, VCL, and CDN orchestration.',
  queue: 'fastly-queue',
...
agentRegistry.register({
  name: 'backblaze',
  description:
    'Cloud storage specialist for Backblaze B2, S3-compatible APIs, and cloud backup.',
...
agentRegistry.register({
  name: 'heroku',
  description:
    'PaaS specialist for Heroku Dynos, buildpacks, and Heroku Postgres.',
  queue: 'heroku-queue',
...
agentRegistry.register({
  name: 'planetscale',
  description:
    'Serverless MySQL specialist for PlanetScale Vitess clusters, branching, and migrations.',
...
agentRegistry.register({
  name: 'upcloud',
  description:
    'European cloud hosting specialist for UpCloud MaxIOPS servers and managed databases.',
  queue: 'upcloud-queue',
...
agentRegistry.register({
  name: 'exoscale',
  description:
    'Swiss cloud specialist for Exoscale instances, SKS, and secure cloud storage.',
...
agentRegistry.register({
  name: 'civo',
  description:
    'Cloud native service provider focusing on lightning-fast K3s Kubernetes.',
  queue: 'civo-queue',
...
agentRegistry.register({
  name: 'coreweave',
  description:
    'Specialized cloud for GPU-accelerated workloads, machine learning, and VFX rendering.',
...
agentRegistry.register({
  name: 'lambdalabs',
  description:
    'GPU cloud specialist for Lambda Labs AI/ML compute infrastructure.',
...
agentRegistry.register({
  name: 'runpod',
  description:
    'Serverless GPU cloud specialist for RunPod pods and AI endpoints.',
  queue: 'runpod-queue',
...
agentRegistry.register({
  name: 'paperspace',
  description:
    'Cloud computing specialist for Paperspace Core and Gradient ML workflows.',
...
agentRegistry.register({
  name: 'tencent',
  description:
    'Asian cloud provider specialist for Tencent Cloud CVM, TKE, and CDN.',
  queue: 'tencent-queue',
...
agentRegistry.register({
  name: 'huawei',
  description:
    'Global cloud specialist for Huawei Cloud ECS, CCE, and GaussDB.',
  queue: 'huawei-queue',
...
agentRegistry.register({
  name: 'baidu',
  description:
    'Cloud specialist for Baidu AI Cloud BCC and smart edge computing.',
  queue: 'baidu-queue',
...
agentRegistry.register({
  name: 'yandex',
  description:
    'Russian cloud platform specialist for Yandex Cloud compute and Managed ClickHouse.',
  queue: 'yandex-queue',
...
agentRegistry.register({
  name: 'equinix',
  description:
    'Bare metal cloud specialist for Equinix Metal provisioning and interconnections.',
  queue: 'equinix-queue',
...
agentRegistry.register({
  name: 'cloudinary',
  description:
    'Cloud media specialist for Cloudinary image and video transformations and CDN.',
...
agentRegistry.register({
  name: 'snowflake',
  description:
    'Data cloud specialist for Snowflake warehouses, Snowpipe, and secure data sharing.',
...
agentRegistry.register({
  name: 'databricks',
  description:
    'Lakehouse specialist for Databricks Apache Spark, Delta Lake, and MLflow.',
...
agentRegistry.register({
  name: 'denodeploy',
  description:
    'Serverless V8 isolate cloud specialist for Deno Deploy and edge functions.',
...
agentRegistry.register({
  name: 'turso',
  description:
    'Edge database specialist for Turso (libSQL) distributed SQLite.',
  queue: 'turso-queue',
...
agentRegistry.register({
  name: 'akamai',
  description:
    'Edge cloud and CDN specialist for Akamai EdgeWorkers and security.',
  queue: 'akamai-queue',
...
agentRegistry.register({
  name: 'supabasecloud',
  description:
    'BaaS cloud specialist for Supabase hosted instances, Edge Functions, and Realtime.',
...
agentRegistry.register({
  name: 'apprunner',
  description:
    'Managed container application service specialist for Google Cloud Run.',
  queue: 'apprunner-queue',
  capabilities: ['cloud-run', 'managed-containers', 'gcp'],
  version: '1.0.0',
});
...
agentRegistry.register({
  ...
  description:
    'Context Engineering Meta-Agent — applies SCRATCHPAD, SELECTOR, COMPRESSOR, TOKEN-BUDGET, and CONTEXT-DISTILLERY patterns from the Agent-Skills-for-Context-Engineering collection for multi-agent context optimization.',
...
agentRegistry.register({
  name: 'deepResearch',
  description:
    'Deep Research SuperAgent powered by ByteDance deer-flow. Handles tasks that take minutes to hours: comprehensive research reports, multi-source synthesis, long-form content, complex coding projects via LangGraph subagent coordination.',
...
agentRegistry.register({
  name: 'pageIndexRAG',
  description:
    'Vectorless Reasoning RAG Agent powered by VectifyAI/PageIndex. Indexes documents at page level and retrieves answers with page-level citations — no vector database required.',
...
agentRegistry.register({
  name: 'nanoCodeAgent',
  description:
    'Minimal Autonomous Code Agent implementing the read/write/bash tool loop from learn-claude-code. Iteratively reads files, writes code, runs bash commands, and self-corrects until tasks complete.',
...
agentRegistry.register({
  ...
  description:
    'Autonomous Enterprise Product Manager. Generates Epics and tasks from user requirements.',
  queue: 'pm-queue',
...
agentRegistry.register({
  ...
  description:
    'Principal Systems Architect. Generates Mermaid topography and Terraform structures.',
  queue: 'architect-queue',
...
agentRegistry.register({
  name: 'gemini_coder',
  description:
    'Senior Software Engineer. Bootstraps raw applications via Gemini CLI.',
...
agentRegistry.register({
  ...
  description:
    'Google Cloud DevOps Engineer. Deploys Dockerized services seamlessly to Google Cloud Run.',
...
agentRegistry.register({
  ...
  description:
    'Google Cloud SRE Observability Analyst. Ingests Cloud Logging data to detect anomalies.',
...
try {
  ...
  logger.info(
    `⚡ Swarm Registry: Dynamically loaded ${dynamicAgents.length} auto-generated specialists from polyglots_registry.json.`,
  );
} catch (err) {
