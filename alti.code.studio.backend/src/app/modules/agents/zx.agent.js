...
class ZxDevOpsAgent {
  ...
  async executeScript(script) {
    logger.info(
      `🛠️ [zx DevOps] Swarm is executing an autonomous shell script...`,
    );

    if (process.env.USE_E2B_SANDBOX === 'true' && sandboxService.isActive()) {
      logger.info(
        `🔒 [zx DevOps] Routing execution to secure E2B sandbox microVM.`,
      );
    ...
    try {
      ...
      logger.info(
        `✅ [zx DevOps] Execution successful: ${chalk.green('0 Exit Code')}`,
      );
      return {
        success: true,
      ...
      logger.error(
        `❌ [zx DevOps] Execution failed with exit code ${p.exitCode}`,
      );
      return {
        success: false,
        stdout: p.stdout,
