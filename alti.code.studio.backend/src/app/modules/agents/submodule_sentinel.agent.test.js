...
describe('SubmoduleSentinelAgent Unit Tests', () => {
  ...
  it('should initialize with correct name, description, and preamble', () => {
    ...
    expect(submoduleSentinelAgent.description).toContain(
      'Submodule Sentinel and Cross-Repository Consistency Specialist',
    );
    expect(submoduleSentinelAgent.preamble).toContain(
      'Submodule Sentinel & Cross-Repo Consistency Specialist',
    );
  });
  ...
  it('should successfully consult the LLM with context and prompt', async () => {
    ...
    const context = [
      {
        path: '.gitmodules',
        content: '[submodule "submodules/ruflo"]\npath = submodules/ruflo',
      },
    ];
  ...
  it('should report code_block type if output contains markdown code backticks', async () => {
    ...
    const result = await submoduleSentinelAgent.consult(
