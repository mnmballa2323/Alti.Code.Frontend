...
class ResearchPaperScraperAgent extends BaseSpecialistAgent {
    ...
    async _invoke(context, contextBlock) {
        ...
        if (!topic) {
            throw new Error('Research paper scraper requires a search topic or keyword.');
        }

        logger.info(`🤖 Research Paper Scraper: Browsing academic sites for topic: "${topic}"`);
        ...
        try {
            ...
            logger.error(`❌ Research Paper Scraper: Browser automation failed: ${error.message}`);
            throw error;
        }
    }
}
