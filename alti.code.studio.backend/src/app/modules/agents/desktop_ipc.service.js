...
class DesktopIpcService {
  ...
  start(port = 8081) {
    ...
    logger.info(
      `🖥️ [Desktop IPC] Native Binary Bridge listening on ws://localhost:${port}`,
    );

    this.wss.on('connection', ws => {
      logger.info(
        '⚡ [Desktop IPC] Tauri Rust Client Connected via Native Binary Stream.',
      );
      ...
      ws.on('message', async (data, isBinary) => {
        if (!isBinary) {
          logger.warn(
            '⚠️ [Desktop IPC] Received non-binary payload. Rejecting. (Strict Binary Mode Enforced)',
          );
          return;
        }
        ...
        try {
          ...
          logger.debug(
            `⚡ [Desktop IPC] Decompressed ${data.byteLength} bytes from Rust into ${decompressed.length} byte AST.`,
          );
          ...
          if (payload.type === 'FS_READ_ACK') {
            logger.info(
              `📂 [Desktop IPC] Native FS Read Acknowledged for ${payload.path}`,
            );
            socketService.broadcast('desktop_ipc', 'fs_read_success', {
              path: payload.path,
              content: payload.content,
            });
          } else if (payload.type === 'FS_WRITE_ACK') {
            logger.info(
              `💾 [Desktop IPC] Native FS Write Acknowledged for ${payload.path}`,
            );
            socketService.broadcast('desktop_ipc', 'fs_write_success', {
              path: payload.path,
            });
          }
        } catch (err) {
  ...
  async broadcastBinaryState(stateObject) {
    ...
    try {
      ...
      logger.debug(
        `⚡ [Desktop IPC] Broadcasted ${compressedBuffer.length} bytes to Rust native UI.`,
      );
    } catch (err) {
