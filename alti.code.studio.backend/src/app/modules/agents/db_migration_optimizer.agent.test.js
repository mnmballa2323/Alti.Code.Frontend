...
describe('DbMigrationOptimizerAgent Unit Tests', () => {
    ...
    it('should initialize with correct name, description, and preamble', () => {
        ...
        expect(dbMigrationOptimizerAgent.description).toContain('Database Migration & Schema Optimizer Specialist');
        expect(dbMigrationOptimizerAgent.preamble).toContain('DATABASE MIGRATION & SCHEMA OPTIMIZER SPECIALIST');
    });

    it('should successfully consult the LLM with context and prompt', async () => {
        const mockResponse = 'Audit Report: Adding nullable column is 100% safe. Index on foreign key is missing; created suggestion.';
        ...
        expect(generatedPrompt).toContain('DATABASE MIGRATION & SCHEMA OPTIMIZER SPECIALIST');
    ...
    it('should handle API errors and trigger retries in BaseSpecialistAgent', async () => {
        ...
        const result = await dbMigrationOptimizerAgent.consult('Optimize DDL queries lock hazard', []);
