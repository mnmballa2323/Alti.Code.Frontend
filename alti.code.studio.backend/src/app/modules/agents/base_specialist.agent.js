...
function withTimeout(promise, ms, label) {
    return Promise.race([
        ...
        new Promise((_, reject) =>
            setTimeout(() => reject(new AgentError(`${label} timed out after ${ms}ms`, 'TIMEOUT', true)), ms)
        )
    ]);
}
...
export class BaseSpecialistAgent {
    ...
    constructor() {
        ...
        try { this.name = 'BaseSpecialist'; } catch (e) { }
        try { this.description = 'Abstract base for all specialist agents.'; } catch (e) { }
        try { this.preamble = ''; } catch (e) { }
    ...
    async _invoke(prompt, contextBlock) {
        throw new AgentError(`${this.name}._invoke() not implemented.`, 'NOT_IMPLEMENTED', false);
    }
    ...
    _validatePrompt(prompt) {
        if (!prompt || typeof prompt !== 'string') {
            throw new AgentError('Prompt must be a non-empty string.', 'VALIDATION_ERROR', false);
        }
        const trimmed = prompt.trim();
        if (trimmed.length === 0) {
            throw new AgentError('Prompt cannot be blank.', 'VALIDATION_ERROR', false);
        }
        if (trimmed.length > MAX_PROMPT_CHARS) {
            // Truncate with a warning suffix instead of hard-rejecting
            return trimmed.substring(0, MAX_PROMPT_CHARS) + '\n[...PROMPT TRUNCATED BY AGENT HARDENING...]';
        }
        return trimmed;
    }
    ...
    _validateResponse(result) {
        if (!result || typeof result !== 'string') {
            throw new AgentError('LLM returned a non-string response.', 'EMPTY_RESPONSE', true);
        }
        if (result.trim().length < 10) {
            throw new AgentError(`LLM response too short (${result.trim().length} chars) — likely a failure.`, 'EMPTY_RESPONSE', true);
        }
    }
     ...
     */
    getMetrics() {
        const avgLatencyMs = this._metrics.calls > 0
        ...
        return {
            ...
            avgLatencyMs,
            errorRate: this._metrics.calls > 0
...
Return only the JSON block.
    ...
    async generateSDKSetup(opts = {}, contextData = []) {
        const { language = 'Node.js', framework = '', envVars = [], authType = '' } = opts;
        const prompt = `
...
Output only runnable code with brief inline comments.
    ...
    async generateWebhookHandler(opts = {}, contextData = []) {
        const { eventTypes = [], language = 'Node.js', framework = 'Express', signingSecret = true } = opts;
        const prompt = `
...
Output production-grade, copy-paste-ready code.
    ...
    async generateApiCall(opts = {}, contextData = []) {
        const { endpoint = '', method = 'GET', language = 'TypeScript', withRetry = true, withTypes = true } = opts;
        const prompt = `
