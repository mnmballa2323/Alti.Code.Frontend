...
class SystemPreferencesTunerAgent extends BaseSpecialistAgent {
  ...
  async _invoke(context, contextBlock) {
    ...
    if (!goal) {
      throw new Error(
        'System preferences tuner requires a settings change prompt.',
      );
    }

    logger.info(
      `🤖 System Preferences Tuner: Translating tuner intent: "${goal}"`,
    );
    ...
    try {
      ...
      logger.error(
        `❌ System Preferences Tuner: Automation failed: ${error.message}`,
      );
      throw error;
    }
  }
