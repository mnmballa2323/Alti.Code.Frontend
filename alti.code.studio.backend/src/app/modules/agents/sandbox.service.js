...
export class SandboxService {
    ...
    async executeCommand(command) {
        if (!this.isActive()) {
            throw new Error('E2B_API_KEY is not configured. Sandbox execution unavailable.');
        }
        ...
        try {
            ...
            logger.info(`🔒 [Sandbox] Sandbox created (ID: ${sandbox.id}). Running command...`);
            const run = await sandbox.commands.run(command);
            
            logger.info(`🔒 [Sandbox] Command finished with exit code ${run.exitCode}`);
            return {
