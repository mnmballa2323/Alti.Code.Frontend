/**
 * "The F# Master" - Tier 19 Functional Language Specialist
 * Expert in F# 8, computation expressions, discriminated unions, Fable, and Giraffe.
 */
...
class FsharpAgent extends BaseSpecialistAgent {
  constructor() {
    ...
    this.description =
      'Functional specialist for F# 8/.NET: discriminated unions, computation expressions, Fable compiler, and Giraffe.';
...
Output F# with 4-space indentation, \`let\` bindings for all values, and \`module\` organization. Use \`Result<'T, 'E>\` for error handling — never throw exceptions in pure functions.`;
  ...
  async consult(prompt, contextData = []) {
    ...
    try {
      ...
      throw new Error(`FSharp Synthesis Failed: ${e.message}`);
    }
  }
}
export const fsharpAgent = new FsharpAgent();
