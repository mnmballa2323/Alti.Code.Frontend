...
export class NeuralRouterService {
    constructor() {
        ...
        this.WEIGHTS_FILE = path.join(process.cwd(), '.alti', 'neural_weights.json');
    ...
    async initialize() {
        try {
            ...
            logger.info(`🧠 NeuralRouter: Loaded ${this.connectionWeights.size} synaptic weights from disk.`);
        } catch {
    ...
    async route(query, context = {}) {
        logger.info(`🧠 NeuralRouter: Calculating pathway vectors for query: "${query.substring(0, 50)}..."`);
        ...
        try {
            ...
            logger.warn(`NeuralRouter Semantic Mapping failed: ${e.message}. Falling back to default router.`);
        ...
        for (const entry of CAPABILITY_TABLE) {
            if (entry.agent && candidateIds.includes(entry.agent.id)) {
                const weight = this.connectionWeights.get(entry.agent.id) || this.BASE_WEIGHT;
                ...
                logger.debug(`🧠 Pathway [${entry.agent.id}]: Weight=${weight.toFixed(2)}, Activation=${activation.toFixed(2)}`);
        ...
        logger.info(`🎯 NeuralRouter selected [${bestAgent.id}] with Synaptic Activation: ${highestActivation.toFixed(2)}`);
    ...
    async backpropagate(agentId, sprintScore) {
        ...
        const currentWeight = this.connectionWeights.get(agentId) || this.BASE_WEIGHT;
        ...
        logger.info(`📈 NeuralRouter Backprop: [${agentId}] SprintScore=${sprintScore} -> Weight adjusted ${currentWeight.toFixed(2)} to ${newWeight.toFixed(2)}`);
