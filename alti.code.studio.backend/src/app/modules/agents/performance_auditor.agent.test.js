...
describe('PerformanceAuditorAgent Unit Tests', () => {
    ...
    it('should initialize with correct name, description, and preamble', () => {
        ...
        expect(performanceAuditorAgent.description).toContain('Performance Bottleneck Auditor & Log Profiler Specialist');
        expect(performanceAuditorAgent.preamble).toContain('Performance Bottleneck Auditor & Log Profiler Specialist');
    });

    it('should successfully consult the LLM with context and prompt', async () => {
        const mockResponse = 'Big-O Analysis: The nested loop is quadratic O(N^2) and can be reduced to O(N) using a Map lookup.';
        ...
        expect(generatedPrompt).toContain('Performance Bottleneck Auditor & Log Profiler Specialist');
    ...
    it('should handle API errors and trigger retries in BaseSpecialistAgent', async () => {
        ...
        const result = await performanceAuditorAgent.consult('Analyze memory heap leaks', []);
