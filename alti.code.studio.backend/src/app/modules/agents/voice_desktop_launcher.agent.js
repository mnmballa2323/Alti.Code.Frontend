...
class VoiceDesktopLauncherAgent extends BaseSpecialistAgent {
  ...
  async _invoke(context, contextBlock) {
    ...
    if (!command) {
      throw new Error(
        'Voice desktop launcher requires a verbal command description.',
      );
    }

    logger.info(
      `🤖 Voice Desktop Launcher: Mapping voice command: "${command}"`,
    );
    ...
    try {
      ...
      logger.error(
        `❌ Voice Desktop Launcher: Voice automation failed: ${error.message}`,
      );
      throw error;
    }
  }
