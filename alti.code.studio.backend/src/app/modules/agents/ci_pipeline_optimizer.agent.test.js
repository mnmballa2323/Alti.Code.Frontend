...
describe('CiPipelineOptimizerAgent Unit Tests', () => {
    ...
    it('should initialize with correct name, description, and preamble', () => {
        ...
        expect(ciPipelineOptimizerAgent.description).toContain('CI/CD Workflow & Build Pipeline Optimizer Specialist');
        expect(ciPipelineOptimizerAgent.preamble).toContain('CI/CD WORKFLOW & BUILD PIPELINE OPTIMIZER');
    });

    it('should successfully consult the LLM with context and prompt', async () => {
        const mockResponse = 'Audit Report: GitHub Actions workflow lacks caching for npm packages. Generated optimized yaml using actions/cache.';
        ...
        expect(generatedPrompt).toContain('CI/CD WORKFLOW & BUILD PIPELINE OPTIMIZER');
    ...
    it('should handle API errors and trigger retries in BaseSpecialistAgent', async () => {
        ...
        const result = await ciPipelineOptimizerAgent.consult('Scan Dockerfile layers for cache caching', []);
