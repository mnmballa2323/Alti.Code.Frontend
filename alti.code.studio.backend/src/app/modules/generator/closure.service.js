...
class GoogleClosureService {
  ...
  async optimizeCode(sourceCode) {
    logger.info(
      `⚙️ [Closure] Initiating Google Closure Compiler optimization sequence...`,
    );
    ...
    return new Promise((resolve, reject) => {
      ...
      const compilerProcess = compiler.run((exitCode, stdOut, stdErr) => {
        if (exitCode === 0) {
          ...
          logger.info(
            `✅ [Closure] Optimization successful. Saved ${savings} bytes.`,
          );
          resolve(stdOut);
        } else {
          logger.warn(
            `⚠️ [Closure] Compilation failed. Attempting graceful degradation.`,
          );
