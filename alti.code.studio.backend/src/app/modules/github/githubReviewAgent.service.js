...
const autoReviewPR = async (userId, sessionId, owner, repo, pull_number) => {
    logger.info(`🤖 [ReviewAgent] Starting Tri-Brain review for PR ${owner}/${repo}#${pull_number}`);
...
Provide a comprehensive, senior-level code review comment suitable for posting directly to GitHub.`;
        ...
        logger.info(`✅ [ReviewAgent] Successfully posted review to PR ${owner}/${repo}#${pull_number}`);
        return { success: true, message: 'Review posted successfully.', review: finalReview };
