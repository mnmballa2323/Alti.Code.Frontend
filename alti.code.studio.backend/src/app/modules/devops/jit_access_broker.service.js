...
class JitAccessBroker {
  constructor() {
    ...
    logger.info(
      '🔐 JitAccessBroker initialized. Banishing permanent IAM roles from the enterprise.',
    );
  }
   ...
   */
  async evaluateAndProvisionLease(
    gcpProjectId,
    userEmail,
    requestedRole,
    durationMinutes,
    ...
    logger.info(
      `🔐 [JIT-${leaseId}] Evaluating ephemeral access request for ${userEmail} -> ${requestedRole}`,
    );

    // 1. Justification Validation
    if (!justification || justification.length < 10) {
      logger.error(
        `❌ [JIT-${leaseId}] Rejected. Insufficient forensic justification provided.`,
      );
      return {
        granted: false,
        reason: 'Missing valid incident correlation context.',
      };
    }
    ...
    if (durationMinutes > 120) {
      logger.warn(
        `⚠️ [JIT-${leaseId}] Requested lease exceeds maximum enterprise bound (120m). Truncating to 60m.`,
      );
      durationMinutes = 60;
    }
    ...
    try {
      logger.info(
        `🔐 [JIT-${leaseId}] Instructing GCP IAM to generate conditional time-bound token...`,
      );
      const expiryTime = new Date(Date.now() + durationMinutes * 60000);

      const leaseRecord = await this._simulateGcpIamBinding(
        userEmail,
        requestedRole,
        expiryTime,
      );
      ...
      logger.info(
        `✅ [JIT-${leaseId}] Lease granted. Role self-destructs precisely at ${expiryTime.toISOString()}.`,
      );
      return { granted: true, leaseId, expires: expiryTime };
    } catch (error) {
      logger.error(
        `❌ [JIT-${leaseId}] GCP IAM propagation failure: ${error.message}`,
      );
  ...
  async _simulateGcpIamBinding(email, role, expiry) {
    return new Promise(resolve => {
      setTimeout(() => {
        resolve({
          binding: true,
          condition: `request.time < timestamp('${expiry.toISOString()}')`,
        });
      }, 450);
    });
  }
