...
const prepareClonerTemplate = async (targetUrl, projectName, workspacePath) => {
  ...
  if (!urlVal)
    throw new ApiError(httpStatus.BAD_REQUEST, 'targetUrl is required.');
  if (!nameVal)
    throw new ApiError(httpStatus.BAD_REQUEST, 'projectName is required.');
  ...
  try {
    ...
    try {
      ...
      throw new ApiError(
        httpStatus.CONFLICT,
        `Directory website-clones/${nameVal} already exists.`,
      );
    } catch (err) {
    ...
    logger.info(
      `🌐 Cloner: Cloning JCodesMore/ai-website-cloner-template to website-clones/${nameVal}...`,
    );

    // 2. Clone the template repository
    await new Promise((resolve, reject) => {
      exec(
        `git clone https://github.com/JCodesMore/ai-website-cloner-template.git "${targetDir}"`,
        (err, stdout, stderr) => {
          if (err) {
            ...
            return reject(
              new ApiError(
                ...
                `Failed to clone cloner template repository: ${stderr || err.message}`,
              ),
            );
          }
          resolve();
    ...
    logger.info(
      `📦 Cloner: Running npm install in website-clones/${nameVal}...`,
    );
      ...
      exec(`npm install`, { cwd: targetDir }, (err, stdout, stderr) => {
        if (err) {
          ...
          return reject(
            new ApiError(
              httpStatus.INTERNAL_SERVER_ERROR,
              `Dependency installation failed: ${stderr || err.message}`,
            ),
          );
        }
        resolve();
    ...
    logger.info(
      `🚀 Cloner: Template setup complete. Website cloner project ready at website-clones/${nameVal}`,
    );
    ...
    return {
      success: true,
      message:
        'AI Website Cloner project generated and initialized successfully.',
      details: {
        projectName: nameVal,
        targetUrl: urlVal,
