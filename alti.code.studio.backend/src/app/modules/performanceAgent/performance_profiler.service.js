...
class PerformanceProfilerService {
    ...
    async profileTask(taskName, taskFunction, limits = {}) {
        ...
        if (limits.maxDurationMs && durationMs > limits.maxDurationMs) {
            limitsExceeded = true;
            breaches.push(`Duration limit exceeded: ${durationMs.toFixed(2)}ms > ${limits.maxDurationMs}ms`);
        }

        if (limits.maxMemoryBytes && memoryDeltaBytes > limits.maxMemoryBytes) {
            limitsExceeded = true;
            breaches.push(`Memory limit exceeded: ${memoryDeltaBytes} bytes > ${limits.maxMemoryBytes} bytes`);
        }

        if (limitsExceeded) {
            logger.warn(`⏱️ Profiler Alert: Task [${taskName}] breached resource limits! ${breaches.join(', ')}`);
        } else {
            logger.info(`⏱️ Profiler: Task [${taskName}] completed. Latency: ${durationMs.toFixed(2)}ms, Memory Delta: ${memoryDeltaBytes} bytes, CPU User: ${cpuDeltaUserMs.toFixed(2)}ms`);
        }
