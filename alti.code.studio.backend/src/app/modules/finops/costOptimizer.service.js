...
const optimizePerformance = async (
  ...
  logger.info(
    `💸 [FinOps Optimizer] Analyzing telemetry for inefficiencies...`,
  );
...
Return ONLY the raw optimized javascript code, no markdown or explanations.`;
    ...
    const optimizedCode = result.reply
      .replace(/```javascript/g, '')
      .replace(/```/g, '');
    logger.info(
      `✅ [FinOps Optimizer] Code successfully refactored for maximum efficiency.`,
    );
    ...
    logger.error(
      `❌ [FinOps Optimizer] Optimization critically failed:`,
      error,
    );
    throw error;
  }
