...
export const finopsMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        if (!billingState) {
            ...
            if (!billing) {
                return res.status(400).json({ error: 'Tenant billing configuration not found.' });
            }
        ...
        if (billingState.currentSpendUsd >= billingState.monthlyBudgetUsd) {
            if (billingState.hardLimitAction === 'pause') {
                console.warn(`[FinOps] 🔴 Tenant ${tenantId} exceeded hard budget limit of $${billingState.monthlyBudgetUsd}. Request blocked.`);
                ...
                console.warn(`[FinOps] 🟡 Tenant ${tenantId} exceeded budget limit of $${billingState.monthlyBudgetUsd}, but action is 'warn'. Proceeding.`);
            }
        }
 ...
 */
export const reportTokenUsage = async (tenantId, model, promptTokens, completionTokens) => {
    if (!tenantId) return;
    ...
    const cost = ((promptTokens / 1000) * pricePer1kPrompt) + ((completionTokens / 1000) * pricePer1kCompletion);
    ...
    try {
        ...
        console.log(`[FinOps] 💸 Tenant ${tenantId} charged $${cost.toFixed(5)}. Total spend: $${billing.currentSpendUsd.toFixed(2)}`);
    } catch (e) {
