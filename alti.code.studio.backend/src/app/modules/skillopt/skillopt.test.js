    ...
    Skill: {
      ...
      create: vi
        ...
        .mockImplementation(data =>
          Promise.resolve({ ...data, failures: [], save: vi.fn() }),
        ),
    },
    SkillOptRun: {
...
// Mock GcpGenAiService to prevent real API calls in testing
vi.mock('../googleGenAi/gcpGenAi.service.js', () => {
  return {
    GcpGenAiService: {
      getGenerativeModel: vi.fn().mockReturnValue({
        generateContent: vi.fn().mockResolvedValue({
          response: {
            text: () =>
              JSON.stringify({ success: true, feedback: 'Scored perfect' }),
          },
        }),
      }),
    },
...
describe('SkillOptService Core Operations', () => {
  ...
  describe('applyEdits()', () => {
    it('should correctly apply ADD edits by appending them at the end', () => {
      ...
      const edits = [{ type: 'ADD', replacementText: 'Be concise.' }];
    ...
    it('should correctly apply REPLACE edits on matching substrings', () => {
      ...
      const edits = [
        {
          type: 'REPLACE',
          targetText: 'Never use Javascript.',
          replacementText: 'Use Javascript for dynamic pages.',
        },
      ];

      const result = SkillOptService.applyEdits(baseText, edits);
      expect(result).toBe(
        'Use Python to write code. Use Javascript for dynamic pages.',
      );
    });

    it('should correctly apply DELETE edits by removing target substrings', () => {
      const baseText = 'Always log results. Ignore errors.';
      const edits = [{ type: 'DELETE', targetText: ' Ignore errors.' }];
  ...
  describe('runEvaluation()', () => {
    it('should evaluate dataset using a custom harness and report correct accuracy score', async () => {
      ...
      const mockHarness = vi
        .fn()
        .mockResolvedValueOnce({ success: true, output: '4' })
        .mockResolvedValueOnce({
          success: false,
          output: '5',
          feedback: 'Math mistake',
        });

      const result = await SkillOptService.runEvaluation(
        systemInstruction,
        dataset,
        mockHarness,
      );
  ...
  describe('optimizeSkill() Training Loop', () => {
    it('should run multi-epoch optimization loop, apply validation gate and update model', async () => {
      ...
      const runEvaluationSpy = vi
        .spyOn(SkillOptService, 'runEvaluation')
        // Baseline: fails
        .mockResolvedValueOnce({
          score: 0.0,
          failedTraces: [{ input: '2+2', expected: '4', feedback: 'Failed' }],
        })
        // Epoch 1 evaluation: fails
        .mockResolvedValueOnce({
          score: 0.0,
          failedTraces: [
            { input: '2+2', expected: '4', feedback: 'Failed again' },
          ],
        })
        // Epoch 1 candidate validation gate: succeeds!
        .mockResolvedValueOnce({ score: 1.0, failedTraces: [] });

      const computeTextualGradientSpy = vi
        .spyOn(SkillOptService, 'computeTextualGradient')
        .mockResolvedValueOnce([
          {
            type: 'ADD',
            replacementText: 'Optimized mathematical helper guideline.',
            rationale: 'Fix addition logic',
          },
        ]);

      const applyEditsSpy = vi
        .spyOn(SkillOptService, 'applyEdits')
        .mockReturnValue('Evolved instructions prompt');

      const { run, optimizedInstructions } =
        await SkillOptService.optimizeSkill(
          'math-expert',
  ...
  describe('registerFailure() Dynamic Calibration', () => {
    it('should correctly register a failure and trigger optimizeSkill when threshold reached', async () => {
      const optimizeSkillSpy = vi
        .spyOn(SkillOptService, 'optimizeSkill')
        .mockResolvedValue({
          optimizedInstructions: 'Perfect system prompt instructions',
        });

      await SkillOptService.registerFailure(
        'math-expert',
        '2+2',
        '4',
        'Criteria',
