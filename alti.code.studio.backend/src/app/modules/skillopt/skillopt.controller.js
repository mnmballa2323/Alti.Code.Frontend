...
const optimizeSkill = catchAsync(async (req, res) => {
  ...
  SkillOptService.optimizeSkill(skillName, initialInstructions, dataset, {
    maxEpochs,
  })
    .then(async ({ run: completedRun }) => {
      console.log(
        `[SkillOpt] Optimization run ${completedRun._id} completed successfully.`,
      );
    })
    .catch(async err => {
...
const getSkill = catchAsync(async (req, res) => {
  ...
  if (!skill) {
    throw new ApiError(
      ...
      `Skill [${name}] not found in catalogue`,
    );
  }
