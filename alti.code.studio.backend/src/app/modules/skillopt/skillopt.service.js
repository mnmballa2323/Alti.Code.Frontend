...
export class SkillOptService {
  ...
  static async runEvaluation(systemInstruction, dataset, harness = null) {
    ...
    logger.info(
      `🔬 Evaluating instruction set against ${dataset.length} validation instances...`,
    );
    ...
    for (const instance of dataset) {
      ...
      try {
        if (harness) {
          ...
          const scoringResult =
            await scorerModel.generateContent(validationPrompt);
          const cleanJson = scoringResult.response
            .text()
            .replace(/```json/g, '')
            .replace(/```/g, '')
            .trim();
    ...
    logger.info(
      `🔬 Accuracy: ${(score * 100).toFixed(1)}% (${correctCount}/${dataset.length})`,
    );
  ...
  static async computeTextualGradient(currentInstruction, failedTraces) {
    logger.info(
      `🧠 Computing Textual Gradient (TGD) across ${failedTraces.length} failures...`,
    );
    ...
    try {
      ...
      logger.warn(
        `Failed to parse optimizer JSON: ${cleanJson}. Defaulting to empty edits.`,
      );
      return [];
    }
  }
    ...
    for (const edit of edits) {
      ...
      if (type === 'REPLACE') {
        if (text.includes(targetText)) {
          ...
          logger.warn(
            `Replace failed: Substring not found in instruction text: "${targetText}"`,
          );
        }
      } else if (type === 'DELETE') {
        if (text.includes(targetText)) {
          ...
          logger.warn(
            `Delete failed: Substring not found in instruction text: "${targetText}"`,
          );
        }
      } else if (type === 'ADD') {
  ...
  static async optimizeSkill(
    ...
    try {
      // Initial evaluation
      const initEval = await this.runEvaluation(
        currentInstruction,
        dataset,
        harness,
      );
      ...
      for (let epoch = 1; epoch <= maxEpochs; epoch++) {
        ...
        const currentEval = await this.runEvaluation(
          currentInstruction,
          dataset,
          harness,
        );
        ...
        if (currentEval.failedTraces.length === 0) {
          logger.info(
            `✨ Zero failure traces remaining! Terminating optimization loop.`,
          );
          break;
        }

        // 2. Compute Textual Gradient using Gemini optimizer
        const proposedEdits = await this.computeTextualGradient(
          currentInstruction,
          currentEval.failedTraces,
        );
        if (proposedEdits.length === 0) {
          logger.warn(
            `⚠️ No edits proposed by optimizer LLM. Terminating epoch.`,
          );
          break;
        }

        // 3. Apply proposed edits to construct Candidate Skill instructions
        const candidateInstruction = this.applyEdits(
          currentInstruction,
          proposedEdits,
        );
        ...
        logger.info(
          `🔬 Validation Gate: Evaluating proposed candidate instructions...`,
        );
        const candidateEval = await this.runEvaluation(
        ...
        logger.info(
          `⚖️ Validation Result: Candidate score: ${(candidateEval.score * 100).toFixed(1)}% | Best score: ${(bestScore * 100).toFixed(1)}%`,
        );
        ...
        if (accepted) {
          ...
          logger.info(
            `❌ VALIDATION GATE REJECTED: Reverting to previous best instructions.`,
          );
        }
      }
  ...
  static async registerFailure(skillName, input, expected, criteria, feedback) {
    logger.warn(
      `🚨 [SkillOpt] Registering execution failure trace for agent [${skillName}]`,
    );
    try {
      ...
      if (!skill) {
        ...
        skill = await Skill.create({
          ...
          description: def
            ? def.description
            : 'Dynamic Specialist Agent Prompt',
          systemInstruction: def
            ? def.description
            : 'Solve the tasks efficiently.',
        });
      }
      ...
      if (skill.failures.length >= threshold) {
        logger.warn(
          `⚙️ [SkillOpt] Accumulated ${skill.failures.length} failures for [${skillName}]. Triggering Autonomous Prompt Calibration...`,
        );
        ...
        this.optimizeSkill(skillName, skill.systemInstruction, dataset, {
          maxEpochs: 2,
        })
          .then(async () => {
            logger.info(
              `✨ [SkillOpt] Autonomous Prompt Calibration for [${skillName}] complete. Clearing failures array.`,
            );
            await Skill.findOneAndUpdate(
              { name: skillName },
              { $set: { failures: [] } },
          ...
          .catch(err => {
            logger.error(
              `❌ [SkillOpt] Autonomous Calibration failed for [${skillName}]: ${err.message}`,
            );
          });
      }
    } catch (err) {
      logger.error(
        `❌ [SkillOpt] Failed to register failure trace for ${skillName}`,
        err,
      );
    }
  }
  ...
  static async registerSuccess(skillName, input, output) {
    logger.info(
      `✨ [SkillOpt] Registering successful execution trace for agent [${skillName}]`,
    );
    try {
      let skill = await Skill.findOne({ name: skillName });
      if (!skill) {
      ...
      await skill.save();
      logger.info(
        `✅ [SkillOpt] Successfully recorded positive reinforcement example for [${skillName}].`,
      );
    } catch (err) {
      logger.error(
        `❌ [SkillOpt] Failed to record success trace for ${skillName}`,
        err,
      );
    }
  }
