...
vi.mock('../audit/audit.worker.js', () => ({
  auditWorkerProcessor: vi
    .fn()
    .mockResolvedValue({ hash: 'abc123', previousHash: '000000' }),
}));
vi.mock('../gitAgent/git.worker.js', () => ({
  gitWorkerProcessor: vi
    .fn()
    .mockResolvedValue({ status: 'pushed', commit: 'abc' }),
}));
...
describe('The Citadel — 11-Agent Federation', () => {
  ...
  it('should initialize ALL workers in the Federation', async () => {
    ...
    console.log(
      ...
      workerService.failedWorkers,
    );
    console.log('NODE ENV:', process.env.NODE_ENV);
    for (const agent of expectedAgents) {
      expect(
        workerService.workers[agent],
