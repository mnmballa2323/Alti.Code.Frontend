...
vi.mock('../ai/ai.provider.js', () => ({
  aiProvider: {
    reason: vi
      .fn()
      .mockResolvedValue(
        '[{"agent": "debug", "data": {"task": "analyze"}, "description": "Debug the issue", "parallel": false}]',
      ),
...
describe('Legion Processor Stress Test — All 72 Agents', () => {
  it('should execute every single Legion processor and return structured output', async () => {
    ...
    aiProvider.reason.mockResolvedValue(
      '{"result": "test output", "summary": "test summary", "confidence": 95}',
    );
