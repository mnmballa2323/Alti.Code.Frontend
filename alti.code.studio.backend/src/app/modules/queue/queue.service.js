...
class QueueService {
  ...
  init() {
    ...
    if (
      ...
      logger.info(
        '⚠️ QueueService: Redis disabled via env. Bypassing queue initialization.',
      );
      return;
    }
  ...
  async addJob(queueName, data, opts = {}) {
    ...
    try {
      ...
      logger.error(
        `❌ QueueService: Failed to add job to "${queueName}"`,
        error,
      );
      throw error;
    }
  ...
  async getJob(queueName, jobId) {
    ...
    try {
      ...
      logger.error(
        `❌ QueueService: Failed to get job "${jobId}" from "${queueName}"`,
        error,
      );
      return null;
    }
  ...
  async getQueueStats(queueName) {
    ...
    try {
      ...
      logger.error(
        `❌ QueueService: Failed to get stats for "${queueName}"`,
        error,
      );
