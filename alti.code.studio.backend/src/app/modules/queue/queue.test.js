...
vi.mock('bullmq', () => {
  return {
    Queue: class {
      constructor() {}
      add(...args) {
        return mocks.queueInstance.add(...args);
      }
      on(...args) {
        return mocks.queueInstance.on(...args);
      }
    },
    Worker: class {
      constructor() {}
      on(...args) {
        return mocks.workerInstance.on(...args);
      }
    },
  };
});
...
describe('QueueService & WorkerService', () => {
  ...
  describe('QueueService', () => {
    ...
    it('should add job to queue', async () => {
      ...
      expect(mocks.queueInstance.add).toHaveBeenCalledWith(
        'audit',
        { foo: 'bar' },
        expect.any(Object),
      );
