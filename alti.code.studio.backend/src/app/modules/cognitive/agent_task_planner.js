...
export class AgentTaskPlanner {
  ...
  async executeAll() {
    console.log(
      `🤖 Starting topological task execution across ${this.tasks.size} registered nodes...`,
    );
      ...
      if (executableTasks.length > 0) {
        ...
        await Promise.all(
          executableTasks.map(async task => {
            ...
            try {
              const executionResult = await task.executor(
              ...
              if (executionResult && executionResult.success) {
                ...
                await this._reflectAndRetry(
                  task,
                  executionResult ? executionResult.error : 'Execution failed',
                );
              }
            } catch (err) {
  ...
  async _reflectAndRetry(task, errorMessage) {
    ...
    console.warn(
      `  ⚠️ Cognitive Reflection triggered for failed task [${task.id}]. Reason: "${errorMessage}"`,
    );
    ...
    if (task.retries <= 2) {
      ...
      console.log(
        `  🔧 HEURISTIC REPAIR: Retrying task [${task.id}] with self-healed parameters...`,
      );
      ...
      try {
        ...
        if (retryResult && retryResult.success) {
          ...
          this.completed.add(task.id);
          console.log(
            `  ✅ [${task.id}] successfully recovered after self-repair.`,
          );
          return;
        }
      } catch (retryErr) {
