...
class GoogleBazelService {
  ...
  async initializeBazelWorkspace(projectRoot) {
    logger.info(
      `🏗️ [Bazel] Swarm is transitioning codebase at ${projectRoot} to Google Bazel...`,
    );
...
nodejs_binary(
    ...
    data = glob(["src/**", "config/**", "package.json"]),
      ...
      await fs.writeFile(
        path.join(projectRoot, 'BUILD.bazel'),
        buildBazelContent,
      );

      logger.info(
        `✅ [Bazel] Bazel WORKSPACE and BUILD.bazel successfully generated.`,
      );
      return true;
    } catch (error) {
      logger.error(
        `❌ [Bazel] Failed to initialize Bazel workspace:`,
        error.message,
      );
      return false;
    }
