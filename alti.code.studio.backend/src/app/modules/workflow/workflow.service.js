...
class WorkflowService {
    ...
    async deployWorkflow(payload) {
        const { name, triggerType, cronExpression, webhookEvent, prompt, selectedTools } = payload;
        ...
        let compiledSteps = [];
        try {
            const result = await GoogleGenAiService.generateContent(compilerPrompt, 'gemini-2.5-flash', 0.1);
            const cleaned = result.content.replace(/^```json?\n?/m, '').replace(/\n?```$/m, '').trim();
            compiledSteps = JSON.parse(cleaned);
        } catch (e) {
            logger.warn(`⚠️ [Workflow Service] Failed to compile prompt into DAG: ${e.message}`);
        ...
        logger.info(`✅ [Workflow Service] Deployed ${workflowId} with ${compiledSteps.length} compiled steps.`);
