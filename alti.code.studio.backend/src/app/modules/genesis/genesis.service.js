...
class GenesisService {
  ...
  async igniteCreation(prompt, repoName, isPrivate = true) {
    ...
    logger.info(
      `✨ Genesis Engine [${genesisId}]: Ignition. Target: ${repoName}`,
    );

    try {
      // STEP 1: Repository Initialization
      this._emit(genesisId, 'PHASE_START', 'Repository Initialization');
      const initResult = await GitAgentService.initializeRepository(
        repoName,
        prompt,
        isPrivate,
      );
      logger.info(
        `✨ Genesis Engine [${genesisId}]: Github Repo created & cloned at ${initResult.shadowSpace}`,
      );
      this._emit(
        genesisId,
        'PHASE_COMPLETE',
      ...
      if (figmaUrlMatch) {
        ...
        this._emit(
          genesisId,
          'PHASE_START',
          'Visual Cortex Extraction (Figma)',
        );
        figmaData = await designerAgent.analyzeFigmaNode(fileId);
        logger.info(
          `✨ Genesis Engine [${genesisId}]: Figma Design Tokens & Component Extracted.`,
        );
        this._emit(genesisId, 'PHASE_COMPLETE', 'Visual Cortex Extraction', {
          feature: 'Design System & UI Scaffold',
        });
      }
      ...
      if (figmaData && figmaData.component) {
        ...
        designDoc.files.push({
          ...
          instruction:
            'Write the extracted Figma layout component exactly as provided.',
          precompiledCode: figmaData.component,
        });
      }

      logger.info(
        `✨ Genesis Engine [${genesisId}]: Architecture Synthesized (${designDoc.feature})`,
      );
      this._emit(genesisId, 'PHASE_COMPLETE', 'Architectural Synthesis', {
        feature: designDoc.feature,
      });
      ...
      logger.info(
        `✨ Genesis Engine [${genesisId}]: Scheduled ${epicPlan.phases?.length || 0} autonomous sprints.`,
      );
      this._emit(genesisId, 'PHASE_COMPLETE', 'Sprint Deconstruction', {
        totalPhases: epicPlan.phases?.length,
      });
      ...
      const deployResult = await deploymentAgent.deployToVercelZeroTouch(
        repoName,
        'nextjs',
      );
      logger.info(
        `✨ Genesis Engine [${genesisId}]: Production URL -> ${deployResult.url}`,
      );
      this._emit(genesisId, 'PHASE_COMPLETE', 'Cloud Genesis', deployResult);

      logger.info(
        `🌟 Genesis Engine [${genesisId}]: Total Autonomy Achieved. The Swarm is now executing the backlog.`,
      );
      ...
      logger.error(
        `💥 Genesis Engine [${genesisId}] Catastrophic Failure:`,
        error,
      );
  ...
  _emit(id, status, message, data = {}) {
    genesisBus.emit('genesis:progress', {
      genesisId: id,
      status,
      message,
      ...data,
