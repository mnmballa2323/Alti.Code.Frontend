...
class ScimService {
  ...
  async createUser(scimPayload, tenantId) {
    try {
      const email =
        scimPayload.emails?.find(e => e.primary)?.value || scimPayload.userName;
      ...
      const clearanceLevel = parseInt(
        scimPayload.urn_ietf_params_scim_schemas_extension_enterprise_2_0_User
          ?.clearanceLevel || '1',
        10,
      );
