...
export const projectTool = {
  name: 'manage_task',
  description:
    "Manage project tasks (list, add, complete). Acts as the 'Manager' memory.",
  inputSchema: {
    type: 'object',
    properties: {
      action: {
  ...
  handler: async args => {
    ...
    if (action === 'list') {
      ...
      const formatted = tasks
        .map(
          t =>
            `[${t.status === 'completed' ? 'x' : ' '}] ${t.description} (ID: ${t.id})`,
        )
        .join('\n');
      return {
    ...
    if (action === 'add') {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Task added: ${task.description} (ID: ${task.id})`,
          },
        ],
      };
    }
    ...
    if (action === 'complete') {
      ...
      return {
        content: [
          {
            type: 'text',
            text: success ? 'Task marked as complete.' : 'Task not found.',
          },
        ],
      };
    }
