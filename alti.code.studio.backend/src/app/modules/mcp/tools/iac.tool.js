...
export const iacTool = {
  name: 'generate_iac',
  description:
    'Generate Infrastructure-as-Code (IaC) scaffolds like Terraform or Kubernetes manifests for a designated tech stack.',
  inputSchema: {
    type: 'object',
    properties: {
      tool: {
      ...
      stackName: {
        type: 'string',
        description:
          "Name of the stack, environment, or project (e.g. 'production-cluster', 'user-service').",
      },
    },
  ...
  handler: async args => {
    try {
      const result = await IacAgentService.applyInfrastructure(
        args.tool,
        args.stackName,
      );
      ...
      return {
        isError: true,
        content: [
          { type: 'text', text: `❌ Failed to generate IaC: ${error.message}` },
        ],
      };
    }
  },
