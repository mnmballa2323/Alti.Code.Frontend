...
class MCPGateway {
  constructor() {
    ...
    logger.info(
      '🔌 MCPGateway initialized. Scanning for local and remote MCP hosts.',
    );
  }
  ...
  async init() {
    ...
    for (const preset of PRESETS) {
      if (process.env[preset.envKey] === 'true') {
        try {
          logger.info(
            `🔌 [MCPGateway] Auto-mounting preset server: ${preset.name}`,
          );
          ...
          logger.error(
            `❌ [MCPGateway] Failed to auto-mount preset ${preset.name}: ${err.message}`,
          );
        }
      }
    }
    ...
    if (fs.existsSync(customServersFilePath)) {
      try {
        ...
        for (const server of customServers) {
          logger.info(
            `🔌 [MCPGateway] Auto-mounting custom server: ${server.name}`,
          );
        ...
        logger.error(
          `❌ [MCPGateway] Failed to load custom MCP servers from JSON: ${err.message}`,
        );
      }
    }
  }
  ...
  async mountServer(serverName, connectionConfig) {
    ...
    if (!connectionConfig) {
      ...
      if (foundPreset) {
        ...
        throw new Error(
          `Connection configuration is required for custom MCP server '${serverName}'`,
        );
      }
    }
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          `⚠️ Live MCP transport for '${serverName}' failed: ${transportError.message}. Synthesizing failover specialist with default capabilities.`,
        );
      ...
      logger.info(
        `✨ Successfully mounted MCP Server '${serverName}'. Injected ${tools.length} abstract tools into the Swarm.`,
      );
      ...
      logger.error(
        `❌ Failed to mount MCP Server '${serverName}': ${error.message}`,
      );
      return {
        success: false,
        error: error.message,
  ...
  async _establishTransport(serverName, config) {
    ...
    if (!config || !config.command) {
      throw new Error(
        `Invalid transport configuration for server '${serverName}'`,
      );
    }
