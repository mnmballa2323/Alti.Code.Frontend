...
class AltiMCPServer {
  ...
  setupHandlers() {
    ...
    this.localTools = [
      ...
      },
      {
        name: 'audit_code',
        description:
          'Audit code for security vulnerabilities and quality issues',
        inputSchema: {
          type: 'object',
          properties: {
            code: { type: 'string' },
            context: {
              type: 'string',
              description: "Context of the code (e.g., 'auth module')",
            },
          },
          required: ['code'],
        },
      ...
      },
      {
        ...
        description:
          "Search for symbols (classes, functions, methods, routes) in the codebase using Colby McHenry's CodeGraph index",
        inputSchema: {
          type: 'object',
          properties: {
            search: {
              type: 'string',
              description: 'The symbol name or query to search for',
            },
            limit: {
              type: 'number',
            ...
            kind: {
              type: 'string',
              description:
                'Filter by kind (class, function, method, file, route, etc.)',
            },
          },
          required: ['search'],
        },
      },
      {
        ...
        description:
          'Explore file structure or subdirectory file list using CodeGraph',
        inputSchema: {
          type: 'object',
          properties: {
            filterDir: {
              type: 'string',
              description:
                'Filter to files under this directory (relative to workspace root)',
            },
            pattern: {
              type: 'string',
    ...
    try {
      ...
      switch (name) {
        ...
        case 'audit_code': {
          ...
          const result = await GuardianService.auditCode(
            code,
            context || 'general',
          );
          return {
        ...
        case 'analyze_repo':
          return {
            content: [
              { type: 'text', text: 'Repo analysis not implemented yet.' },
            ],
          };
        case 'filesystem':
  ...
  async handleMessage(req, res) {
    logger.warn(
      'MCP: HTTP Message handling requires persistent transport mapping. Implemented basic setup.',
    );
