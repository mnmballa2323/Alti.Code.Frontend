...
class Neo4jService {
  constructor() {
    ...
    try {
      ...
      logger.info(
        '🔷 [Neo4jService] Apache 2.0 Driver initialized for Cognitive Mapping.',
      );
    } catch (error) {
      logger.warn(
        '⚠️ [Neo4jService] Failed to initialize Neo4j driver. Is the database running?',
      );
    }
  }
  ...
  async executeCypher(query, params = {}) {
    ...
    try {
      ...
      logger.error(
        `❌ [Neo4jService] Error executing Cypher: ${error.message}`,
      );
      throw error;
    } finally {
      await session.close();
  ...
  async logAgentReasoningNode(id, agentName, executionTimeMs) {
    logger.info(
      `🔷 [Neo4jService] Committing Reasoning Node [${id}] for Agent [${agentName}] to the Knowledge Graph...`,
    );
    const query = `
  ...
  async ingestAstContext(graphifyOutput) {
    if (!graphifyOutput || !graphifyOutput.nodes || !graphifyOutput.links)
      return;

    logger.info(
      `🔷 [Neo4jService] Ingesting ${graphifyOutput.nodes.length} AST Nodes into Neo4j...`,
    );
    const session = this.driver.session();
    ...
    try {
      ...
      await tx.commit();
      logger.info(
        `✅ [Neo4jService] Successfully persisted Cognitive Context to Neo4j.`,
      );
    } catch (error) {
