...
export const startDataRetentionCron = () => {
  ...
  cron.schedule('0 0 * * *', async () => {
    logger.info(
      '[RETENTION] Starting daily automated data retention expungement...',
    );
    ...
    try {
      ...
      for (const tenant of expiringTenants) {
        ...
        if (deletedMetrics.count > 0 || deletedUsers.count > 0) {
          logger.info(
            `[RETENTION] Tenant ${tenant.name}: Expunged ${deletedMetrics.count} metric records and ${deletedUsers.count} stale users older than ${tenant.retentionPeriodDays} days.`,
          );
        }
      }
      ...
      logger.error(
        `[RETENTION] Error during auto-expungement: ${error.message}`,
      );
    }
  });
