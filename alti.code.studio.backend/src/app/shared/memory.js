...
class AntigravityMemory {
    ...
    async getEmbedding(text) {
        ...
        if (cached) {
            logger.debug(`🧠 Memory: Embedding cache HIT (cache size: ${embeddingCache.size})`);
            return cached;
        }

        const embedding = await GeminiAiService.getEmbeddings(text);
        embeddingCache.set(text, embedding);
        logger.debug(`🧠 Memory: Embedding cache MISS — stored (cache size: ${embeddingCache.size})`);
        return embedding;
    }
    ...
    embeddingCacheStats() {
        return {
            ...
            utilizationPct: +((embeddingCache.size / CACHE_MAX_SIZE) * 100).toFixed(1),
        };
    }
}
