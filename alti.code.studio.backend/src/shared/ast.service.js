...
class AstService {
  constructor() {
    logger.info(
      '🌳 AST: AST service initialized with Babel, ast-grep, and Recast capability.',
    );
  }
  ...
  extractFunction(sourceCode, functionName) {
    try {
      ...
      logger.warn(
        `⚠️ [AST] Recast extraction failed: ${err.message}. Falling back to standard Babel.`,
        err,
      );
      ...
      traverse(ast, {
        ...
        VariableDeclarator(path) {
          if (
            ...
            extractedFunc = sourceCode.slice(
