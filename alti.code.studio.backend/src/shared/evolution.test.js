...
const fetchData = async (url) => {
    ...
    return res.json();
        ...
        const expectedWeightsPath = path.join(mockWorkspaceDir, '.alti-style-weights.json');
...
[
    ...
    "Strictly prefer async/await over Promise.then chaining."
        ...
        const learnedRules = await evolutionService.processEvolutionaryDelta(aiLegacyCode, humanModernCode, mockWorkspaceDir);
    ...
    it('should securely inject the learned style rules into the system prompt parameters', async () => {
        ...
        fs.readFile.mockResolvedValueOnce(JSON.stringify({
            version: '1.0.0',
            enforced_human_styles: ['Strictly prefer async/await over Promise.then chaining.']
        }));

        const promptInjection = await evolutionService.getStylisticParameters(mockWorkspaceDir);

        expect(promptInjection).toContain('CRITICAL SYSTEM STYLISTIC ENFORCEMENT:');
        expect(promptInjection).toContain('Strictly prefer async/await over Promise.then chaining.');
    });
