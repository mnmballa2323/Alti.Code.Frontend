...
export class NexusAgent {
  constructor() {
    this.name = 'TheNexus';
    logger.info(
      '🌐 NexusAgent: Cross-Repository Swarm Orchestrator initialized.',
    );
  }
  ...
  async executeCrossRepoFeature(globalIntent, targetRepositories) {
    logger.info(
      `🌐 NexusAgent: Received global intent: "${globalIntent}". Orchestrating swarm across ${targetRepositories.length} repositories.`,
    );
    ...
    if (!allSucceeded) {
      logger.error(
        `🌐 NexusAgent: ATOMIC LOCK FAILURE. One or more repos failed testing. Initiating global rollback.`,
      );
      await this._rollbackRepositories(targetRepositories);
      return {
        success: false,
        reason: 'Atomic cross-repo verification failed.',
      };
    }

    logger.info(
      `🌐 NexusAgent: Atomic Swarm Verification PASSED. Committing trans-repo changes.`,
    );
  ...
  async _dispatchSubAgent(repoPath, intent) {
    logger.info(
      `🌐 [NEXUS SWARM] Dispatching isolated agent to repository: ${path.basename(repoPath)}`,
    );
    ...
    rawSubAgentOutput = rawSubAgentOutput
      .replace(/^```[a-z]*\n/i, '')
      .replace(/\n```$/i, '')
      .trim();
  ...
  async _rollbackRepositories(repoPaths) {
    logger.warn(
      `🌐 NexusAgent: Executing \`git reset --hard\` globally across all targets...`,
    );
