...
describe('Sentient Self-Rewriting (Phase 32 - The Metamorphic Core)', () => {
    ...
    it('should run a diagnostic on its own orchestration code and generate an optimization PR', async () => {
        ...
        const selfPrPayload = {
            ...
            description: "Identified a sequential loop bottleneck in task delegation. Upgrading to Promise.all.",
            diff: "- await agent.execute(task);\n+ await Promise.all(swarm.map(a => a.execute(task)));"
        };
        GeminiAiService.generateContent.mockResolvedValueOnce(JSON.stringify(selfPrPayload));

        const pr = await metamorphicAgent.runSelfDiagnostic('/mock/src/app/modules/agents');

        // ASSERTIONS

        // 1. It read its own source code
        expect(fs.readFile).toHaveBeenCalledWith('/mock/src/app/modules/agents/orchestrator.agent.js', 'utf8');
