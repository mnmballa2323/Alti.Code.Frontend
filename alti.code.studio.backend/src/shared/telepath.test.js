    ...
    GeminiAiService.generateContent.mockResolvedValueOnce(`
{
    ...
    expect(telepathService.telemetryBuffer[3].content).toEqual(
      'export const Checkout = () => {',
    );
    ...
    const expectedGhostDir = path.dirname(
      path.join(telepathService.ghostCachePath, 'src/routes/stripe.webhook.js'),
    );
    const expectedGhostFile = path.join(
      telepathService.ghostCachePath,
      'src/routes/stripe.webhook.js',
    );

    expect(fs.mkdir).toHaveBeenCalledWith(expectedGhostDir, {
      recursive: true,
    });
