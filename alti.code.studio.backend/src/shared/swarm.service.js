...
export class SwarmService {
    constructor() {
        ...
        logger.info(`🐝 SwarmService: Hive Mind initialized with capacity for [${this.maxPoolSize}] parallel sub-agents.`);
    }
    ...
    async unleash(tasks) {
        ...
        if (tasks.length > this.maxPoolSize) {
            logger.warn(`🐝 SwarmService: Requested ${tasks.length} threads. Truncating to SWARM_MAX_THREADS (${this.maxPoolSize}).`);
            tasks = tasks.slice(0, this.maxPoolSize);
        }

        logger.info(`🐝 SwarmService: Dispatching swarm... Deploying [${tasks.length}] autonomous worker threads.`);
        ...
        return results.map((res, index) => {
            if (res.status === 'fulfilled') {
                ...
                logger.error(`🐝 SwarmService: Thread ${tasks[index].id} crashed.`, res.reason);
