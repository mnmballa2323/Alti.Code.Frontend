...
class JobQueueService {
  constructor() {
    ...
    if (process.env.DISABLE_REDIS === 'true') {
      ...
      logger.warn(
        '⚠️ JobQueue: Redis disabled via env. Switching to MOCK MODE.',
      );
      return;
    }
    ...
    try {
      ...
      this.readinessPromise = new Promise(resolve => {
        ...
        testRedis.on('error', err => {
          ...
          logger.warn(
            '⚠️ JobQueue: Redis not available. Switching to MOCK MODE.',
          );
          this.isMockMode = true;
          try {
            testRedis.disconnect();
    ...
    const worker = new Worker(
      ...
      async job => {
        ...
        try {
          ...
          logger.error(
            `JobQueue: Failed job ${job.name} in ${queueName}`,
            error,
          );
          throw error;
        }
