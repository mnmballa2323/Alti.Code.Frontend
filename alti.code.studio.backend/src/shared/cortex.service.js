...
export class CortexService {
  constructor() {
    ...
    logger.info(
      '🧠 CortexService: Neural Context Synthesis engine initialized.',
    );
  }
  ...
  addDeterministicEdge(dependentPath, dependencyPath) {
    if (!this.graph.has(dependentPath))
      this.graph.set(dependentPath, new CortexNode(dependentPath, 'file'));
    if (!this.graph.has(dependencyPath))
      this.graph.set(dependencyPath, new CortexNode(dependencyPath, 'file'));
  ...
  calculateImpactRadius(targetFilePath) {
    if (!this.graph.has(targetFilePath)) {
      logger.warn(
        `🧠 CortexService: Target [${targetFilePath}] not found in Neural Graph.`,
      );
      return [];
    }
    ...
    logger.info(
      `🧠 CortexService: Calculated impact radius for [${targetFilePath}]. Affects ${impactArray.length} downstream nodes.`,
    );
    return impactArray;
  }
}
