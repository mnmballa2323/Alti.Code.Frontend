...
export class PhoenixService {
  ...
  mount() {
    ...
    logger.info(
      '🦅 Phoenix Protocol: Daemon mounted. Listening for fatal runtime crashes.',
    );
  ...
  async handleFatalCrash(error, type) {
    ...
    try {
      ...
      const targetedStackLine = stackLines.find(
        line =>
          line.includes('alti.code.studio.backend') &&
          !line.includes('node_modules'),
      );

      if (!targetedStackLine) {
        logger.warn(
          '🦅 Phoenix Protocol: Could not map crash to local project file. Aborting autonomous repair.',
        );
        return { success: false, reason: 'No local stack trace found' };
      }

      // Standardize path extraction (this handles Windows C:/ and file:/// URIs)
      const fileMatch = targetedStackLine.match(
        /\((?:file:\/\/\/)?([a-zA-Z0-9_:\/\.\-\\]+\.js)/,
      );
      ...
      if (!fileMatch) {
        logger.warn(
          '🦅 Phoenix Protocol: Regex failed to extract filepath from stack trace.',
        );
      ...
      logger.info(
        `🦅 Phoenix Protocol: Traced crash to ${functionName || 'anonymous'} in ${absolutePath}`,
      );
      ...
      logger.info(
        '🦅 Phoenix Protocol: Asking Gemini to hallucinate a structural patch...',
      );
      ...
      logger.info(
        '✅ Phoenix Protocol: Codebase patch successfully synthesized.',
      );
      logger.info(`\n${engineeredPatch}\n`);
      ...
      logger.error(
        '❌ Phoenix Protocol: Daemon catastrophically failed during recovery attempt.',
        recoveryError,
      );
