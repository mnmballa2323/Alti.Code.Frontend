...
export class ReverserAgent {
  constructor() {
    ...
    logger.info(
      '⚙️ ReverserAgent: Native Machine-Code Decompiler initialized.',
    );
  }
  ...
  async decompileNativeCrash(processName, rawAssemblyDump) {
    logger.error(
      `⚙️ ReverserAgent: Intercepted fatal SIGSEGV (Segmentation Fault) in native module [${processName}].`,
    );
    logger.info(
      `⚙️ ReverserAgent: Extracting core dump assembly and initializing Gemini Decompiler...`,
    );

    try {
      const explanation = await this._translateMachineCode(
        processName,
        rawAssemblyDump,
      );
      logger.info(
        `✅ ReverserAgent: Binary successfully translated to logical intent.`,
      );
      ...
      logger.error(
        `⚙️ ReverserAgent: Failed to decompile binary: ${error.message}`,
      );
      return {
        status: 'error',
