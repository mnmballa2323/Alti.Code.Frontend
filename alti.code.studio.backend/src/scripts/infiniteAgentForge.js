...
const BATCH_SIZE = 20;
const GENERATED_DIR = path.join(process.cwd(), 'src', 'app', 'modules', 'generated');
const MCP_REGISTRY_PATH = path.join(process.cwd(), 'src', 'app', 'modules', 'mcp', 'tools', 'githubSearch.tool.js');
...
const fallbackBrainstorm = () => {
    const concepts = [
        { name: "LocalizationExpert", desc: "Autonomously translates all strings into 40 languages" },
        ...
        { name: "LegalContractGenerator", desc: "Autonomously drafts NDAs and MSA agreements based on context" },
        { name: "AccessibilityAuditor", desc: "Autonomously scans frontends to ensure WCAG compliance" }
    ];

    const randomConcept = concepts[Math.floor(Math.random() * concepts.length)];
    const id = Math.floor(Math.random() * 10000);
    const agentName = `${randomConcept.name}Service_${id}`;
    const mcpCommand = `${randomConcept.name.replace(/([A-Z])/g, "_$1").toLowerCase().substring(1)}_${id}`;
...
const forgeNewAgent = async () => {
    logger.info(`🔥 [Infinite Forge] Brainstorming a completely new corporate macro-agent...`);
    let agentData;

    try {
        // Attempt Enterprise Gateway (will likely crash if DB offline)
        const { LlmGatewayService } = await import('../app/modules/llmGateway/llmGateway.service.js');
        const brainstormPrompt = "Invent a completely new, highly-specialized autonomous corporate agent...";
        const result = await LlmGatewayService.routeCompletion('system', 'infinite-forge', brainstormPrompt, 'gpt-5.5', 'Agent-Forge');
        
        const jsonMatch = result.reply.match(/\{[\s\S]*\}/);
        agentData = JSON.parse(jsonMatch ? jsonMatch[0] : result.reply);
    } catch (error) {
        logger.warn(`🩹 [Self-Heal Triggered] Enterprise Gateway failed (likely offline DB/Redis). Bypassing to Raw LLM Engine.`);
    ...
    logger.info(`✅ [Infinite Forge] New Agent Created: ${agentData.agentName} (${fileName})`);
...
const injectIntoRegistry = async (agentData) => {
    logger.info(`💉 [Infinite Forge] Injecting ${agentData.mcpCommand} into MCP Registry...`);
...
`;
    ...
    registryCode = registryCode.replace(/default:/, `${switchCaseCode}\n                default:`);

    await fs.writeFile(MCP_REGISTRY_PATH, registryCode);
    logger.info(`✅ [Infinite Forge] ${agentData.mcpCommand} is now live on the Swarm MCP!`);
};
...
const performGithubCheckpoint = async () => {
    logger.warn(`🐙 [Infinite Forge] BATCH LIMIT REACHED (${BATCH_SIZE}). Committing and Pushing to GitHub...`);
    try {
        await execAsync('git add .');
        await execAsync(`git commit -m "Auto-Forge: Generated ${BATCH_SIZE} new specialized Swarm Agents"`);
        await execAsync('git push origin main');
        logger.info(`✅ [Infinite Forge] Push successful. The codebase has expanded permanently.`);
    } catch (error) {
...
const runInfiniteForge = async () => {
    ...
    while (true) {
        try {
            ...
            agentsGenerated++;
            logger.info(`[Infinite Forge] Agents generated this session: ${agentsGenerated}`);
            ...
            logger.error(`❌ [Infinite Forge] Unexpected failure. The self-healer will attempt to patch and retry in 5s...`, error);
