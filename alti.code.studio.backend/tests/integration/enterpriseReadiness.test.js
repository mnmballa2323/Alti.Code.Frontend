...
describe('Enterprise Readiness: Isolation, Tracing & Sandboxing Integration Tests', () => {
    ...
    describe('Multi-Tenant Data Isolation (pgvector)', () => {
        it('should perform pgvector searches strictly scoped to the tenantId parameter', async () => {
            const querySpy = vi.spyOn(vectorStoreService.pool, 'query').mockResolvedValue({
                rows: [
                    { id: 'doc_1', metadata: {}, document: 'Tenant A doc', distance: 0.1 }
                ]
            });

            // Mock vertex embeddings call
            const { vertexService } = await import('../../src/app/modules/ai/vertex.service.js');
            vi.spyOn(vertexService, 'getEmbeddings').mockResolvedValue([0.1, 0.2, 0.3]);

            const result = await vectorStoreService.search('some query', 5, 'tenant_A');
        ...
        it('should fail safely if tenantId is omitted during search', async () => {
            ...
            const result = await vectorStoreService.search('some query', 5, undefined);
    ...
    describe('Swarm Telemetry Tracing (OpenTelemetry)', () => {
        it('should trace specialist consultations and propagate parent-child span IDs', async () => {
            const startSpanSpy = vi.spyOn(swarmTraceService, 'startSpan').mockResolvedValue('mock_span_id');
            const endSpanSpy = vi.spyOn(swarmTraceService, 'endSpan').mockResolvedValue();

            // Mock Gemini content generation
            vi.spyOn(GeminiAiService, 'generateContent').mockResolvedValue('Mocked response content');

            const agent = agentRegistry.get('gitlabProjectCreator', 'tenant_123');
            expect(agent).toBeDefined();

            // Consult agent with explicit parent span ID
            await agent.instance.consult('Hello agent', [], 'tenant_123', 'parent_span_999');
        ...
        it('should automatically propagate parent span ID down nested agent call stack', async () => {
            ...
            const dbQuerySpy = vi.spyOn(swarmTraceService.pool, 'query').mockResolvedValue({ rows: [] });
            vi.spyOn(GeminiAiService, 'generateContent').mockResolvedValue('Mocked response');

            const agentA = agentRegistry.get('gitlabProjectCreator', 'tenant_123').instance;
            ...
            agentA._invoke = async (prompt, context, tenantId, spanId) => {
                return (await agentB.consult('child query', [], tenantId, spanId)).content;
            };
            ...
            try {
                ...
                const insertCalls = dbQuerySpy.mock.calls.filter(call => call[0].includes('INSERT INTO swarm_traces'));
    ...
    describe('Secure VM Sandboxed Execution', () => {
        it('should execute safe JS scripts inside the sandboxed context', async () => {
            const agent = agentRegistry.get('gitlabProjectCreator', 'tenant_123').instance;
        ...
        it('should block require / process access and throw a sandboxing security violation', async () => {
            ...
            expect(() => {
                agent.runSandboxed("const fs = require('fs'); fs.writeFileSync('/tmp/hack.txt', 'compromised');");
            ...
            expect(() => {
                agent.runSandboxed("const cp = require('child_process'); cp.exec('ls');");
            }).toThrow('Security Violation');
        });
    });
});
