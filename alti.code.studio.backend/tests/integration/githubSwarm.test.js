...
describe('GitHub Swarm Nexus & Dynamic Specialists Integration', () => {
    beforeEach(() => {
        vi.restoreAllMocks();
    });

    it('should declare valid BaseSpecialist schemas for all dynamic Swarm agents', () => {
        const swarmList = [
    ...
    it('should dynamically register all four Swarm specialists on dynamic scan bootstrap', async () => {
        agentRegistry.pluginsLoaded = false;
        await agentRegistry.loadPlugins();

        const ids = ['githubExpert', 'githubActionsSpecialist', 'githubAppAuditor', 'githubProjectsManager'];
        for (const id of ids) {
    ...
    it('should execute grounded RAG lookups across all swarm specialists', async () => {
        const swarmList = [
            { instance: githubExpert, query: 'gh issue list --label bug', mockDoc: 'GitHub CLI docs', searchKeyword: 'gh issue list --label bug' },
            { instance: githubActions, query: 'setup concurrency limits in yaml', mockDoc: 'Actions concurrency docs', searchKeyword: 'concurrency limits' },
            { instance: githubApp, query: 'verify GITHUB_WEBHOOK_SECRET', mockDoc: 'App cryptographic verify', searchKeyword: 'GITHUB_WEBHOOK_SECRET' },
            { instance: githubProjects, query: 'configure codeowners reviewers', mockDoc: 'Governance CODEOWNERS', searchKeyword: 'codeowners reviewers' }
        ];
