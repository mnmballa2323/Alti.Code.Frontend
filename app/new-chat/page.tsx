...
export default function Home() {
  ...
  const [selectedNavItem, setSelectedNavItem] = useState("Code");
  ...
  useEffect(() => {
    const handleNavChange = (e: Event) => {
      ...
      if (detail) setSelectedNavItem(detail);
    };

    window.addEventListener("nav-item-selected", handleNavChange);

    return () =>
      window.removeEventListener("nav-item-selected", handleNavChange);
  }, []);
