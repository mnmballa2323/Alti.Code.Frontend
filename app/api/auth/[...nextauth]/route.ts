    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        if (credentials?.accessToken) {
          // Try to validate the token against the backend
          try {
            ...
            // Backend unreachable (e.g. PostgreSQL offline in dev)
            console.warn(
              "[NextAuth] Backend /single-user unreachable:",
              fetchError,
            );
          }

          // Development fallback: decode JWT payload to extract user info
          // This allows login when the backend DB is offline
          try {
            ...
            console.error(
              "[NextAuth] JWT decode fallback failed:",
              decodeError,
            );
    ...
    async jwt({ token, user, account }) {
      ...
      if (user && account) {
        if (["github", "google"].includes(account.provider)) {
          ...
          token.refreshToken = (
            user as ExtendedUser & { refreshToken?: string }
          ).refreshToken;
