...
- **State machine logic**: `ApplyChanges` implements three-phase stack management (Observability → NoopTrace → PentAGI) with integrity validation; the wizard performs a pre-phase interactive integrity check with Y/N decision for force mode
...
- **Three-phase execution**: Observability → NoopTrace → PentAGI with state validation after each phase
