...
export default function NotificationBell() {
  ...
  const handleMarkAllRead = async () => {
    ...
    try {
      const headers = { Authorization: `Bearer ${token}` };
      await axios.patch(`${API_BASE_URL}/notification/read-all`, {}, { headers });
  ...
  const handleMarkRead = async (id: string, actionUrl?: string) => {
    ...
    try {
      const headers = { Authorization: `Bearer ${token}` };
      await axios.patch(`${API_BASE_URL}/notification/${id}/read`, {}, { headers });
              ...
              notifications.map((notification) => (
                <div
                  ...
                  className={`p-3.5 flex gap-3 transition-colors cursor-pointer text-left ${
                    !notification.read ? "bg-primary/5 dark:bg-primary/10" : "hover:bg-default-50/50 dark:hover:bg-default-50/10"
                  }`}
                >
                  {getIcon(notification.type)}
                  <div className="flex-1 overflow-hidden flex flex-col gap-1">
                    <span className={`text-[11px] font-semibold text-foreground truncate ${!notification.read ? 'pr-2' : ''}`}>
                      {notification.title}
                    </span>
