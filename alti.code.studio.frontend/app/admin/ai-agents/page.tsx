...
const MOCK_AGENTS: Agent[] = [
  { id: "1", name: "Code Architect", type: "code", status: "active", lastRun: "2 min ago", runCount: 142, icon: "code" },
  { id: "2", name: "Security Auditor", type: "security", status: "active", lastRun: "15 min ago", runCount: 87, icon: "security" },
  { id: "3", name: "Test Generator", type: "testing", status: "active", lastRun: "5 min ago", runCount: 234, icon: "testing" },
  { id: "4", name: "Doc Writer", type: "docs", status: "inactive", lastRun: "2 days ago", runCount: 45, icon: "docs" },
  { id: "5", name: "API Designer", type: "code", status: "active", lastRun: "30 min ago", runCount: 63, icon: "code" },
  { id: "6", name: "Performance Analyzer", type: "perf", status: "active", lastRun: "1 hour ago", runCount: 112, icon: "perf" },
];
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            ...
            { label: "Agents Running", value: "3 / 10", color: "text-emerald-500" },
          ].map((stat) => (
            <Card key={stat.label} className="bg-white dark:bg-black/40 border border-default-200 dark:border-white/10 shadow-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredAgents.map((agent) => {
            ...
            return (
              <Card key={agent.id} className="bg-white dark:bg-black/40 border border-default-200 dark:border-white/10 shadow-sm hover:shadow-md transition-shadow">
                <CardBody className="p-5">
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${TYPE_COLORS[agent.type] || "bg-default-100"}`}>
                        <TypeIcon className="w-5 h-5" />
                      </div>
                      <div>
                        <h3 className="text-sm font-bold text-foreground">{agent.name}</h3>
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${TYPE_COLORS[agent.type] || "bg-default-100 text-default-500"}`}>
                          {agent.type}
                        </span>
                      </div>
                    </div>
                    <span className="flex items-center gap-1">
                      <span className={`w-2 h-2 rounded-full ${agent.status === "active" ? "bg-emerald-500 animate-pulse" : agent.status === "error" ? "bg-red-500" : "bg-default-300"}`} />
                      <span className="text-xs text-default-400 capitalize">{agent.status}</span>
                    </span>
                  </div>
