  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const res = await fetch(`${API_URL}/analytics/ai-intelligence`, {
          headers,
        });
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            {
              label: "Cost Today",
              value: `$${data.totalCostToday.toFixed(2)}`,
              icon: DollarSign,
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-default-200 dark:border-white/10">
                    {[
                      ...
                      <th
                        key={h}
                        className="text-left px-5 py-3 text-xs font-bold text-default-500 uppercase"
                      >
                        {h}
                      </th>
                    ))}
                ...
                <tbody>
                  {data.models.map((m) => (
                    <tr
                      key={m.id}
                      className="border-b border-default-100 dark:border-white/5 last:border-0 hover:bg-default-50 dark:hover:bg-white/5"
                    >
                      ...
                      <td className="px-5 py-3.5 font-mono text-default-500">
                      ...
                      <td className="px-5 py-3.5 font-mono text-amber-500">
                        ${m.cost.toFixed(2)}
                      </td>
                      <td className="px-5 py-3.5">
                        <span className="flex items-center gap-1.5">
                          <span
                            className={`w-2 h-2 rounded-full ${m.status === "active" ? "bg-emerald-500" : m.status === "degraded" ? "bg-amber-500" : "bg-red-500"}`}
                          />
          ...
          <Card className="bg-white dark:bg-black/40 border border-default-200 dark:border-white/10 shadow-sm">
            ...
            <CardBody className="p-0">
              {data.recentEvals.map((ev, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between px-5 py-3.5 border-b border-default-100 dark:border-white/5 last:border-0"
                >
                  <div>
