...
export default function RepositoryPage() {
  ...
  // Save to backend
  const saveRules = async (updatedRepos: { id: string; name: string }[]) => {
    if (!token) return;
    try {
      // Fetch current rules first to preserve other sections
      ...
      window.dispatchEvent(
        new CustomEvent("refresh-rules-sidebar", {
          detail: { ...data, repositories: updatedRepos },
        }),
      );
    } catch (err) {
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    const updated = [
      ...repositories,
      { id: "repo-" + Date.now(), name: inputValue.trim() },
    ];
