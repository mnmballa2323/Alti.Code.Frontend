...
import { Database, Network, Server, HardDrive, ShieldCheck, Activity, Terminal, Code2, Link, CloudLightning } from "lucide-react";
...
export default function DataPage() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setLogs((prev) => {
        const newLog = simulatedLogs[Math.floor(Math.random() * simulatedLogs.length)];
  ...
  const metrics = [
    { label: "Total Vectors", value: "84.2M", icon: Database, color: "text-blue-400" },
    { label: "Active Connections", value: "1,048", icon: Network, color: "text-emerald-400" },
    { label: "Ingestion Latency", value: "12ms", icon: Activity, color: "text-purple-400" },
    { label: "Shard Health", value: "100%", icon: ShieldCheck, color: "text-green-400" },
  ];

  const connectors = [
    { name: "GitHub Enterprise", status: "Connected", icon: Code2, syncTime: "2 mins ago" },
    { name: "Jira Software", status: "Connected", icon: Activity, syncTime: "5 mins ago" },
    { name: "AWS S3 Buckets", status: "Syncing", icon: CloudLightning, syncTime: "In progress" },
    { name: "Confluence", status: "Connected", icon: Link, syncTime: "1 hour ago" },
  ];
          ...
          {metrics.map((m, i) => (
            ...
            >
              <div className="flex items-center justify-between mb-4">
                <div className={`p-3 rounded-xl bg-black/50 border border-white/5 ${m.color} group-hover:scale-110 transition-transform`}>
                  <m.icon size={24} />
                </div>
              </div>
              <h3 className="text-gray-400 text-sm font-medium uppercase tracking-wider">{m.label}</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {connectors.map((c, i) => (
                <div key={i} className="flex items-center justify-between p-4 bg-black/40 border border-white/5 rounded-xl hover:border-white/20 transition-colors">
                  <div className="flex items-center gap-4">
                    <div className="p-2 bg-white/5 rounded-lg border border-white/10">
                      <c.icon size={20} className="text-gray-300" />
                    </div>
                    <div>
                      <h4 className="font-semibold">{c.name}</h4>
                      <p className="text-xs text-gray-500">Last synced: {c.syncTime}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="relative flex h-3 w-3">
                      ...
                      <span className={`relative inline-flex rounded-full h-3 w-3 ${c.status === "Connected" ? 'bg-emerald-500' : 'bg-purple-500'}`}></span>
                    </span>
                    <span className="text-xs font-medium text-gray-400">{c.status}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          >
            ...
            <div className="flex items-center gap-2 mb-4 border-b border-gray-800 pb-4">
              ...
              <span className="text-gray-400 font-semibold tracking-widest uppercase text-xs">Live Swarm Ingestion</span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto space-y-2">
              {logs.map((log, index) => (
                ...
                >
                  ...
                  <span className={log.includes("VectorDB") ? "text-purple-400" : log.includes("Gemini") ? "text-blue-400" : "text-emerald-400"}>
                    {log}
                  </span>
                </motion.div>
              ))}
