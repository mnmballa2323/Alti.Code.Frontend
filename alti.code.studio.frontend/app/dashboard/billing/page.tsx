...
import { Button, Card, CardBody, Progress, Chip, Input } from "@heroui/react";
        ...
        if (report) {
          ...
          const byModelMapped = (report.byModel || []).map((m: any) => {
            ...
            const inputCost = pricing
              ? (inputTokens / 1000000) * pricing.inputPrice
              : 0;
          ...
          setUsageReport({
            ...
            byModel: byModelMapped.length
              ? byModelMapped
              : fallbackUsageReport.byModel,
          });
        }
      } catch (err) {
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1 max-w-xl">
            Sovereign platform seat subscriptions are flat-rate, and model
            execution usage costs are passed through transparently at base
            wholesale pricing.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Chip
            className="bg-green-500/10 text-green-400 border border-green-500/20 px-3 py-1 font-semibold"
            variant="flat"
          >
            Active Subscription
          </Chip>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <Card className="bg-[#161b22]/70 backdrop-blur-md border border-gray-800 shadow-xl rounded-2xl">
          <CardBody className="p-6">
            ...
            <p className="text-xs text-gray-400 mt-1">
              Available balance for auto-refill pass-through
            </p>
            <div className="mt-4 flex gap-2">
              <Button
                className="bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-xl w-full"
                size="sm"
              >
                Add Balance
              </Button>
        ...
        <Card className="bg-[#161b22]/70 backdrop-blur-md border border-gray-800 shadow-xl rounded-2xl">
          <CardBody className="p-6">
            ...
            <p className="text-xs text-gray-400 mt-1">
              For {usageReport?.totalTokens?.toLocaleString() || 0} tokens
              consumed this cycle
            </p>
            <div className="mt-4 pt-4 border-t border-gray-800/60 flex items-center justify-between text-xs text-gray-400">
            ...
            <div className="space-y-2 mb-8">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="text-white font-semibold">
                  {usageReport?.totalTokens?.toLocaleString()} /{" "}
                  {billingAccount?.monthlyAllowance?.toLocaleString()}
                </span>
              </div>
              <Progress
            ...
            <div className="space-y-4">
              {usageReport?.byModel?.map((m: any, index: number) => {
                ...
                return (
                  <div
                    key={index}
                    className="bg-[#0d1117]/80 rounded-xl p-4 border border-gray-800/80 flex items-center justify-between gap-4"
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-white">
                          {catalogItem?.name || m.name}
                        </p>
                        <p className="text-xs text-gray-400 mt-0.5">
                          {m.input?.toLocaleString()} in /{" "}
                          {m.output?.toLocaleString()} out
                        </p>
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  classNames={{
                    inputWrapper:
                      "bg-[#0d1117] border border-gray-800 group-data-[focus=true]:border-blue-500",
                    input: "text-white",
                  }}
                  size="sm"
                  type="number"
          ...
          <Card className="bg-[#161b22]/50 border border-gray-800 shadow-xl rounded-2xl p-6 h-full flex flex-col">
            ...
            <div className="space-y-4 flex-1 overflow-y-auto max-h-[500px]">
              {filteredCatalog.map((item, index) => (
                <div
                  key={index}
                  className="bg-[#0d1117] rounded-xl p-4 border border-gray-800/80 flex items-center justify-between gap-4"
                >
                  <div className="flex items-center gap-3">
                    <div className="size-8 rounded-lg bg-gray-800/50 flex items-center justify-center">
                      <Icon
        ...
        <Card className="bg-[#161b22]/50 border border-gray-800 shadow-xl rounded-2xl p-6">
          ...
          <div className="space-y-3 max-h-[220px] overflow-y-auto pr-2">
            {[
              {
                date: "Jun 15, 2026",
                type: "Professional Plan + Model Usage",
                amount: 167.45,
              },
              {
                date: "May 15, 2026",
              ...
              <div
                key={idx}
                className="flex items-center justify-between bg-[#0d1117] rounded-xl p-3 border border-gray-800/80"
              >
                <div>
