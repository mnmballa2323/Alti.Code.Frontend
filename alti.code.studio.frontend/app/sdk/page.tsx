...
export default function SDKPage() {
  ...
  const [sdks, setSdks] = useState<{ id: string; name: string }[]>([]);
  ...
  // Save to backend
  const saveRules = async (updatedSdks: { id: string; name: string }[]) => {
    if (!token) return;
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("refresh-rules-sidebar", {
          detail: { ...data, sdks: updatedSdks },
        }),
      );
    } catch (err) {
      console.error("Failed to save SDKs:", err);
    }
  };
  ...
  const handleSend = async (e?: React.FormEvent) => {
    ...
    const updated = [
      ...sdks,
      { id: "sdk-" + Date.now(), name: inputValue.trim() },
    ];
          ...
          <div className="w-full max-w-2xl mb-14">
            <form className="relative flex items-center w-full shadow-sm rounded-2xl" onSubmit={handleSend}>
              <Input
                ...
                placeholder="Enter an SDK package..."
                value={inputValue}
