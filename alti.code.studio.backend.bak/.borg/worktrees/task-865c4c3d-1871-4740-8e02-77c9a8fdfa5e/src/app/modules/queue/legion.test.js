...
vi.mock('../ai/ai.provider.js', () => ({
    aiProvider: {
        reason: vi.fn().mockResolvedValue('{"result": "test", "summary": "test", "confidence": 100}'),
        generate: vi.fn().mockResolvedValue('{}'),
        getInfo: vi.fn().mockReturnValue({ provider: 'gemini', model: 'gemini-2.0-flash' }),
        switchTo: vi.fn()
    }
}));
...
describe('Legion Processor Generation', () => {
    ...
    it('should execute a processor and return structured output', async () => {
        ...
        const mockJob = {
            id: 'test-1',
            data: { task: 'Write unit tests for UserService', code: 'class UserService {}' }
        };
