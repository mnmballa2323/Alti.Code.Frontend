...
router.post('/compliance/erasure', rbac('compliance:read'), (req, res) => {
    const request = complianceEngine.requestErasure(
        req.tenantId,
        req.identity.userId,
...
router.get('/admin/overview', rbac('tenants:configure'), (req, res) => {
    res.json(adminDashboard.getSystemOverview());
});

router.get('/admin/agents', rbac('agents:list'), (req, res) => {
...
router.get('/admin/executive-summary', rbac('tenants:configure'), (req, res) => {
    res.json(adminDashboard.generateExecutiveSummary());
});

router.get('/admin/capacity', rbac('audit:read'), (req, res) => {
...
router.post('/secrets', rbac('tenants:configure'), (req, res) => {
    try {
        const result = secretManager.createSecret(req.body);
        res.status(201).json(result);
    } catch (err) {
        res.status(400).json({ error: err.message });
    }
...
router.post('/secrets/:secretId/rotate', rbac('tenants:configure'), (req, res) => {
    try {
        const result = secretManager.rotateSecret(req.params.secretId, req.body);
        res.json(result);
    } catch (err) {
        res.status(400).json({ error: err.message });
    }
...
router.post('/admin/tenants', rbac('tenants:configure'), (req, res) => {
...
router.get('/admin/tenants', rbac('audit:read'), (req, res) => {
...
router.post('/admin/maintenance', rbac('tenants:configure'), (req, res) => {
    res.json(adminConsole.setMaintenanceMode(req.body.enabled, req.body.message));
});

router.post('/admin/feature-flags', rbac('tenants:configure'), (req, res) => {
    try { res.json(adminConsole.setFeatureFlag(req.body.flag, req.body.enabled)); }
    catch (err) { res.status(400).json({ error: err.message }); }
});

router.post('/admin/announcements', rbac('tenants:configure'), (req, res) => {
    res.status(201).json(adminConsole.addAnnouncement(req.body.message, req.body.severity));
});

router.get('/admin/system', rbac('audit:read'), (req, res) => {
    res.json(adminConsole.getSystemState());
});

router.get('/admin/audit-log', rbac('audit:read'), (req, res) => {
    res.json(adminConsole.getAuditLog(parseInt(req.query.limit) || 50));
});

router.get('/admin/stats', rbac('audit:read'), (req, res) => {
