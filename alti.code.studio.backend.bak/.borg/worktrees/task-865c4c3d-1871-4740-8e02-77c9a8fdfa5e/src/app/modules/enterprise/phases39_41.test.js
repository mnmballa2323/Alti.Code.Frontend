...
describe('Phase 39: API Key Management', () => {
    ...
    it('should rotate keys with grace period', async () => {
        ...
        const original = apiKeyManager.generateKey('goldman-sachs', { name: 'rotating-key' });
...
describe('Phase 40: Enterprise Caching Layer', () => {
    ...
    it('should cascade L1 miss → L2 hit and promote', async () => {
        ...
        await cacheController.set('cascade:test', { msg: 'promoted' }, {
    ...
    it('should invalidate by tag', async () => {
        ...
        await cacheController.set('tagged:3', { id: 3 }, { tags: ['tenant:other'] });
    ...
    it('should deduplicate agent results', async () => {
        ...
        // First call — miss
        const first = cacheController.dedup('codeReview', 'Review this function');
        expect(first.hit).toBe(false);

        // Store result
        cacheController.dedupStore(first.key, { optimized: true });

        // Second call with same input — hit
        const second = cacheController.dedup('codeReview', 'Review this function');
...
describe('Phases 39-41 Cross Integration', () => {
    ...
    it('should integrate API key + rate limiter + validator', async () => {
        const { apiKeyManager, rateLimiter, schemaValidator } = await import('./index.js');

        // 1. Generate a key
        const key = apiKeyManager.generateKey('integration-test', { name: 'flow-key' });
