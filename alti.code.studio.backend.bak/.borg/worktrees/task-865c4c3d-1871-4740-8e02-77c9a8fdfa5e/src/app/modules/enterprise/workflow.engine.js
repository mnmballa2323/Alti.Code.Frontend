...
const STEP_TYPES = {
    ...
    condition: { label: 'Conditional', description: 'Branch based on expression' },
    transform: { label: 'Data Transform', description: 'Transform data between steps' },
    parallel: { label: 'Parallel Fan-Out', description: 'Execute steps in parallel' },
...
const WORKFLOW_STATUSES = ['DRAFT', 'ACTIVE', 'RUNNING', 'COMPLETED', 'FAILED', 'CANCELLED', 'PAUSED'];
...
class WorkflowEngine {
    constructor() {
        ...
        this.stats = { totalDefined: 0, totalExecutions: 0, totalCompleted: 0, totalFailed: 0 };
    ...
    defineWorkflow(options) {
        ...
        logger.info(`⚙️ Workflow defined: ${name} (${validatedSteps.length} steps) [${workflowId}]`);
    ...
    async executeWorkflow(workflowId, input = {}) {
        ...
        if (definition.status !== 'ACTIVE') throw new Error(`Workflow is ${definition.status}`);
        ...
        try {
            ...
            for (const step of order) {
                ...
                if (depsFailed && step.onFailure === 'skip') {
                    execution.stepResults[step.id] = { status: 'SKIPPED', reason: 'dependency_failed' };
                    continue;
                }
        ...
        logger.info(`${execution.status === 'COMPLETED' ? '✅' : '❌'} Workflow ${execution.status}: ${definition.name} (${execution.duration}ms)`);
        return execution;
    }
    ...
    async _executeStep(step, execution) {
        ...
        for (let attempt = 0; attempt <= maxRetries; attempt++) {
            try {
                ...
                switch (step.type) {
                    case 'agent':
                        result = { output: { agentResult: `${step.config.agentName || 'agent'} completed`, input: step.config.input } };
                        break;
                    case 'condition':
                        const condResult = this._evaluateCondition(step.config, execution.stepResults);
                        result = { output: { branch: condResult } };
                        break;
                    case 'transform':
                        result = { output: this._applyTransform(step.config, execution.stepResults) };
                        break;
                    case 'wait':
                    ...
                    case 'webhook':
                        ...
                        break;
                    case 'notify':
                        result = { output: { notified: true, channels: step.config.channels || ['in_app'] } };
                        break;
                    case 'approval':
                ...
                if (attempt < maxRetries) {
                    await new Promise(r => setTimeout(r, step.retryPolicy.backoffMs * (attempt + 1)));
                }
            }
        }
    ...
    _evaluateCondition(config, stepResults) {
        if (config.checkStep && config.checkField) {
            ...
            if (result && result.output) {
                return result.output[config.checkField] === config.expectedValue ? 'true' : 'false';
            }
        }
    ...
    _registerDefaultTemplates() {
        this.templates.set('code-review-pipeline', {
            ...
            steps: [
                { id: 'scan', name: 'Security Scan', type: 'agent', config: { agentName: 'securityScanner' } },
                { id: 'review', name: 'Code Review', type: 'agent', config: { agentName: 'codeReview' }, dependsOn: ['scan'] },
                { id: 'notify', name: 'Notify Results', type: 'notify', config: { channels: ['email', 'in_app'] }, dependsOn: ['review'] },
            ],
        });
        ...
        this.templates.set('data-processing', {
            ...
            steps: [
                { id: 'ingest', name: 'Ingest Data', type: 'agent', config: { agentName: 'dataIngest' } },
                { id: 'transform', name: 'Transform', type: 'transform', config: { sourceStep: 'ingest' }, dependsOn: ['ingest'] },
                { id: 'validate', name: 'Validate', type: 'agent', config: { agentName: 'validator' }, dependsOn: ['transform'] },
                { id: 'export', name: 'Export Results', type: 'webhook', config: { url: '/api/export' }, dependsOn: ['validate'] },
            ],
        });
        ...
        this.templates.set('approval-flow', {
            ...
            steps: [
                { id: 'analyze', name: 'AI Analysis', type: 'agent', config: { agentName: 'analyst' } },
                { id: 'approve', name: 'Human Approval', type: 'approval', dependsOn: ['analyze'] },
                { id: 'execute', name: 'Execute Action', type: 'agent', config: { agentName: 'executor' }, dependsOn: ['approve'] },
            ],
        });
    }
    ...
    createFromTemplate(templateId, overrides = {}) {
        ...
        return this.defineWorkflow({ ...template, ...overrides, steps: overrides.steps || template.steps });
    }
    ...
    listWorkflows(tenantId) {
        ...
        return workflows.map(w => ({ id: w.id, name: w.name, steps: w.steps.length, status: w.status, version: w.version }));
    }

    listExecutions(workflowId) {
        let execs = [...this.executions.values()];
        if (workflowId) execs = execs.filter(e => e.workflowId === workflowId);
        return execs.map(e => ({ id: e.id, workflowName: e.workflowName, status: e.status, duration: e.duration, completedAt: e.completedAt }));
    }
