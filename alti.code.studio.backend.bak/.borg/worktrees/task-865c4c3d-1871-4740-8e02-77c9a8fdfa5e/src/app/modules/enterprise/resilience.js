...
class CircuitBreaker {
    ...
    recordFailure(error) {
        ...
        if (this.state === STATES.HALF_OPEN) {
            ...
            this.successCount = 0;
            logger.warn(`🔴 CircuitBreaker [${this.name}]: HALF_OPEN → OPEN (still failing)`);
        } else if (this.failureCount >= this.failureThreshold) {
            this.state = STATES.OPEN;
            logger.error(`🔴 CircuitBreaker [${this.name}]: CLOSED → OPEN (${this.failureCount} failures)`);
        }
    }
    ...
    async execute(aiCallFactory, options = {}) {
        ...
        for (const provider of this.providers) {
            ...
            if (!circuit.isAvailable()) {
                logger.debug(`⏭️ Failover: Skipping ${provider.name} (circuit OPEN)`);
                continue;
            }
