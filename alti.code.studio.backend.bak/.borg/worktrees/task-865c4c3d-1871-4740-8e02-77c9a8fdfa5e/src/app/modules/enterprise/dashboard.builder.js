...
const WIDGET_TYPES = {
    ...
    scatter_plot: { category: 'chart', description: 'Scatter plot / bubble chart' },
...
const DASHBOARD_TEMPLATES = {
    executive: {
        ...
        widgets: ['kpi_card', 'line_chart', 'pie_chart', 'stat_card', 'activity_feed'],
...
class DashboardBuilder {
    ...
    createDashboard(config) {
        ...
        if (template && DASHBOARD_TEMPLATES[template]) {
            ...
            tmpl.widgets.forEach((type, i) => {
                const widget = this._createWidget(type, {
                    position: { col: (i * 4) % 12, row: Math.floor(i / 3) * 4, w: 4, h: 4 },
                });
        ...
        return { id: dashboard.id, name, widgets: dashboard.widgets.length, template };
    }
    ...
    updateDashboard(dashboardId, updates) {
        ...
        if (updates.refreshInterval) dashboard.refreshInterval = updates.refreshInterval;
    ...
    shareDashboard(dashboardId) {
        ...
        return { id: dashboardId, shared: true, shareUrl: `/dashboards/shared/${dashboardId}` };
    }
    ...
    getDashboard(dashboardId) { return this.dashboards.get(dashboardId); }
    ...
    getTemplates() { return DASHBOARD_TEMPLATES; }
    ...
    getStats() {
        return {
            ...
            sharedDashboards: [...this.dashboards.values()].filter(d => d.shared).length,
        };
    }
}
