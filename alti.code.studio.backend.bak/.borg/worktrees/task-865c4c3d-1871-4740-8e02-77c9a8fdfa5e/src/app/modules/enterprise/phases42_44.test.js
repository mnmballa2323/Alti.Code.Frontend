...
describe('Phase 44: Secret Rotation Automation', () => {
    it('should create secrets with AES-256-GCM encryption', async () => {
        ...
        const result = secretManager.createSecret({
            name: 'gemini-api-key',
            type: 'api_key',
            tenantId: 'jpmorgan',
    ...
    it('should rotate secrets with versioning and overlap', async () => {
        ...
        const secret = secretManager.createSecret({
            name: 'db-password',
            type: 'db_password',
        });

        const rotation = secretManager.rotateSecret(secret.secretId, {
    ...
    it('should maintain version history', async () => {
        ...
        const secret = secretManager.createSecret({ name: 'test-history', type: 'webhook' });
        secretManager.rotateSecret(secret.secretId);
        secretManager.rotateSecret(secret.secretId);
    ...
    it('should rollback to previous versions', async () => {
        ...
        const secret = secretManager.createSecret({ name: 'rollback-test', type: 'signing_key' });
        secretManager.rotateSecret(secret.secretId);
        secretManager.rotateSecret(secret.secretId);
    ...
    it('should set rotation schedules', async () => {
        ...
        const secret = secretManager.createSecret({ name: 'scheduled', type: 'oauth_token' });
    ...
    it('should encrypt/decrypt with envelope encryption', async () => {
        ...
        const encrypted = secretManager._encrypt(plaintext);

        expect(encrypted.algorithm).toBe('aes-256-gcm');
        expect(encrypted.iv).toBeDefined();
        expect(encrypted.tag).toBeDefined();

        const decrypted = secretManager._decrypt(encrypted);
...
describe('Phases 42-44 Cross Integration', () => {
    ...
    it('should audit secret rotation and export', async () => {
        ...
        // Create and rotate
        const secret = secretManager.createSecret({ name: 'cross-test', type: 'api_key', tenantId: 'test' });
        secretManager.rotateSecret(secret.secretId);
