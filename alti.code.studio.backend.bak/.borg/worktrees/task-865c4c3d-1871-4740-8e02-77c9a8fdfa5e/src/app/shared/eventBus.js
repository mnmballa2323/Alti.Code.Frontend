...
class AntigravityEventBus {
    ...
    async connect() {
        ...
        try {
            ...
            this.publisher.on('error', (err) => {
                // logger.error('Redis Publisher Error', err); // Quiet down
            });
            ...
            logger.info('🧠 Nervous System (Event Bus) Connected [REDIS]');
            this.isConnected = true;
        } catch (error) {
            logger.warn('⚠️ Redis unreachable. Switching to local Nervous System (Mock Mode).');
            this.isMock = true;
            this.isConnected = true;
        }
    }
    ...
    async publish(event, data) {
        if (!this.isConnected) {
            // logger.warn(`Event Bus Disconnected. Dropping event: ${event}`);
            return;
        }
        ...
        try {
            ...
            if (this.isMock) {
                ...
                if (!event.includes('sys.log')) {
                    logger.debug(`⚡ [MOCK BUS] Published: ${event}`);
                }
            } else {
                await this.publisher.publish(event, JSON.stringify(payload));
                if (!event.includes('sys.log')) {
                    logger.debug(`⚡ [BUS] Published: ${event}`);
                }
            }
        } catch (error) {
    ...
    async subscribe(event, callback) {
        ...
        try {
            if (this.isMock) {
                ...
                logger.info(`👂 [MOCK BUS] Subscribed to: ${event}`);
            } else {
                // Redis v4 subscribe
                await this.subscriber.subscribe(event, (message) => {
                    try {
                        ...
                        logger.error(`Failed to handle message for ${event}`, e);
                    }
                });
                logger.info(`👂 [BUS] Subscribed to: ${event}`);
            }
        } catch (error) {
